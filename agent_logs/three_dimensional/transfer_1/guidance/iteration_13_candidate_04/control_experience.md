...
- Prefer sparse actuator-envelope relief over direct hydrodynamic-moment phase
  allocation once an achieved-course/intercept controller has the useful
  route but wastes carrier effort at the joint limits. The sampled reserve
  policy softened only previously saturated outward carrier acceleration and
  captured at `0.7480L` after `18.205T` while retaining a coherent wake and
  `53.7/53.7 deg` sub-`4L` excursion; terminal action clipping fell modestly
  to `71.6%/74.6%` from the LOS-only capture's `72.6%/75.5%`. In contrast,
  the assigned parent's adverse-yaw-moment half-cycle reallocation retained
  `52.9/52.1 deg` excursion but missed at `1.7842L`, clipped on
  `73.3%/75.2%` of terminal rows, and repeated the lower exit. Treat the
  reserve capture as provisional because speed-limit residence stayed near
  `10.6%/10.7%` and terminal force/moment RMS rose slightly: require an exact
  replay before tuning thresholds or composing mechanisms. Falsify this
  implication if replay loses capture, wake/excursion, or load behavior; then
  avoid both reserve-gain tuning and instantaneous-moment allocation and test
  a separately normalized yaw/slip residual or different joint allocation.
