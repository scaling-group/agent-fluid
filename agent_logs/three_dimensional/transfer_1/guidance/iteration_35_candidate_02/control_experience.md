...
- Do not call the exact speed-reserve baseline robust from its four current
  ...
  captured twice with exact bytes at `0.74975--0.74986L` and
  `18.3205--18.6560T`; the sampled repeat preserves the traveling top-down and
  oblique wakes, `0.8369L/T` terminal speed, `68.75%/70.70%` action clipping,
  `10.32%/11.29%` speed-limit residence, and the baseline force/moment
  envelope. This is positive repeat evidence for retaining raw achieved
  course plus the narrow qualifier, not permission to widen or scalar-tune its
  gates. Require another exact repeat; falsify robustness if it changes motion
  outside its gate, retains the lower branch, weakens either wake, loses
  capture, or leaves the repeat-backed actuator/load envelope.
