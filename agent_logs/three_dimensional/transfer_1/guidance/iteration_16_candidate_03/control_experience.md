...
- Do not replace a repeat-supported course request with signed projected miss
  on the strength of one threshold crossing. A terminal blend inside `2L`
  first captured at `0.7477L` and `18.5405T` with the coherent wake and load
  envelope preserved, but two exact-byte repeats then missed at `1.6366L` and
  `1.7680L`, turned below the target, and left the lower domain while retaining
  active alternating top-down and oblique wake structures. This falsifies the
  signed projected-miss blend as a robust steering replacement and reinforces
  the three-repeat speed-reserve controller as the baseline. Avoid tuning or
  stacking that signed replacement; if testing projected geometry again, use
  it first as a magnitude-only capture-corridor condition that cannot reverse
  the route request or suppress the carrier. This boundary applies after broad
  route acquisition in still water; revise it only if exact repeats of a
  projected-geometry controller preserve capture and match or improve arrival,
  load, saturation, and wake coherence rather than merely producing another
  isolated threshold crossing.
