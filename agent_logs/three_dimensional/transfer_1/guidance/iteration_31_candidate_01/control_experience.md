...
- Reject the tested outer-terminal bearing rescue rather than widening its
  annulus or tuning its residual. Added only in body-frame bearing/course
  disagreement, it preserved the active alternating top-down street and
  bilateral oblique structures but still joined the lower branch, missed at
  `1.5629L`, and exited with final distance `10.3858L`; this contrasts with all
  four sampled captures at `0.7480--0.7494L` and leaves the exact speed-reserve
  record at `3/5`, not robust. An inherited burden-conditioned spatial
  allocator is a more discriminating repeat target because its first rollout
  captured at `0.7474L` and `18.3040T` while leaving raw achieved course,
  total steering, and the traveling carrier unchanged. Treat that as one
  compatibility result, not established improvement: preserve the posterior
  propulsive role and require an exact repeat before promoting actuator-state
  allocation. Falsify it on a repeat miss, the same lower branch, degraded
  wake/arrival, or actuator and load behavior outside the speed-reserve
  envelope; if falsified, avoid stacking another bearing, projected-miss,
  observer, or scalar residual onto this topology.
