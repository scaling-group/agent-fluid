...
- Treat terminal locomotor reserve as necessary but not sufficient once the
  ...
  additive curvature safe or effective. By contrast, releasing only shared
  steering when joint-compensated yaw response was established improved a
  direct course-servo near miss from `1.0435L` to `0.9312L` without
  attenuating the carrier. Guarding that release with the signed inertial
  line-of-sight rate computed from normalized body-frame target and achieved
  velocity then produced the first sampled capture at `0.749345L` and
  `18.606T`, while both visual rows retained the alternating wake through
  arrival. Sub-`4L` acceleration-envelope occupancy remained high at about
  `72.6%/75.5%`, so the semantic improvement supports geometric steering
  timing rather than reduced effort or another scalar gain. Together with the
  `1.5454L` static-curvature result, this falsifies locomotor recovery,
  additive/wholesale mean curvature, and additional scalar course authority as
  sufficient capture fixes while establishing line-of-sight rate as a useful
  guard on response-triggered release. This implication applies only after
  broad route acquisition; falsify it if exact reevaluation loses capture,
  changes the far route, weakens the wake, increases saturation materially, or
  fails under held-out initial pose or target geometry.
