...
- Reject post-pass progress-loss mean curvature in this topology, and verify
  that every offline discriminator actually exists in the released policy
  observation before trusting counterfactual selectivity. The inherited burst
  redirect requested `window_closing_speed_L`, which the evaluator does not
  expose, so it silently used one-step `closing_speed_L` rather than the
  claimed eight-row signal. It stayed stable and lowered head/tail action
  clipping to about `53.1%/51.0%`, but improved its assigned-parent miss only
  from `1.1846L` to `1.1608L`, retained the coherent-wake lower exit, and
  finished at `9.8423L`; lower clipping was not recovery. Do not retune that
  gate or stack another recovery bend. In the current traces, useful separation
  appears before the pass instead: at the first `2.75L` crossing, relevant
  speed-reserve captures have projected misses near `0.60--0.88L`, while three
  inherited lower exits are already at `1.37--1.95L`. Test whether projected
  capture compatibility can certify steering release before divergence while
  preserving the carrier and allocation. This applies only to a coherent,
  broadly acquired first approach; falsify it if certification loses sampled
  capture, retains the lower branch, or worsens wake, speed, clipping, force,
  or moment beyond the repeat-backed envelope.
- Do not promote or scalar-tune a fixed outer-terminal LOS-rate residual from
  threshold captures. The exact `dogfish3d_outer_los_rate_intercept_v1` bytes
  now have two captures (`0.74605L` and `0.74957L`) but also a direct-uniform
  sibling replay that retained the active alternating and bilateral 3D wakes,
  passed at `1.59620L` near `0.829L/T`, and exited below at `10.33579L`.
  Its failure stayed inside the sampled peak-load envelope and had
  `70.25%/71.79%` head/tail action clipping, so the `2/3` record is terminal
  path fragility rather than weak propulsion or inadequate residual magnitude.
  Preserve the traveling bend and avoid strengthening or stacking that LOS
  cue. The distinct partial outer steering-retention mechanism remains a
  one-run hypothesis: it captured at `0.74852L` and `18.29849T` with organized
  wakes, `68.80%/70.81%` clipping, `10.64%/11.33%` speed-limit residence, and
  peak body-force/moment coefficients `0.01495/0.02783/0.01620`. Test an exact
  repeat before tuning it. This applies only after broad route acquisition;
  falsify partial retention if its repeat loses capture, retains the lower
  branch, weakens either wake, or leaves the repeat-backed actuator/load
  envelope.
