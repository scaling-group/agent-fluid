...
- Preserve the projected-intercept veto plus selective outward-carrier reserve
  as the repeat-supported baseline before changing terminal geometry. Three
  byte-identical samples captured at `0.7466--0.7494L` across visibly different
  terminal headings while retaining coherent alternating top-down and oblique
  wakes, mean speed near `0.698--0.701L/T`, and comparable force/moment peaks.
  A continuous body-frame signed projected-miss turn inside `2L` also captured
  once with comparable wake, speed, loads, and `68.5%/71.0%` action clamping,
  so it is a semantics-preserving terminal-error candidate that requires an
  exact repeat, not yet evidence of better efficiency. In contrast, two
  total-command speed-feasibility projections lost capture with `1.3877L` and
  `1.5963L` lower-exit passes even when near-limit residence fell; do not trade
  the repeatable crossing for actuator metrics by directly reversing or
  attenuating the combined carrier-plus-steering command. This implication
  applies after broad route acquisition with a sustained traveling wake;
  falsify it if exact signed-miss repeats lose capture or increase loads, or if
  a later total-command allocation preserves capture while materially reducing
  saturation without coasting.
