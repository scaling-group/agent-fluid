...
- Do not let an all-capture sample batch rescue terminal yaw damping or phase-
  synchronous posterior curvature after complete exact-repeat failures. In the
  current direct-uniform sample, two phase-independent speed-reserve runs and
  two intercept-gated posterior-pulse runs all capture at `0.7480--0.7494L`;
  both visual rows retain the same coherent traveling wake, and their arrival
  (`18.20--18.60T`), score, distance integral, clipping (`68.2--68.6%` and
  `70.6--71.0%`), exact speed-limit residence (`10.4--10.6%` and
  `11.3--11.5%`), force, and yaw-moment ranges overlap. The pulse therefore
  supplies no sampled secondary benefit, while its inherited exact replay
  kept an active wake but missed at `1.2589L`; the distinct yaw brake likewise
  missed at `1.5097L`. The assigned parent's only new inherited score is
  another `left_domain` result after a `1.4503L` pass, but without policy bytes,
  trace, or wake sheet it cannot identify a mechanism or overturn the complete
  evidence. For a coherent, broadly acquired route, restore the repeat-backed
  phase-independent achieved-course/intercept controller; do not scalar-tune
  or stack joint-phase, short-window body-bearing, or yaw-rate triggers, which
  can mistake beat motion for task convergence. Revisit only a distinct
  normalized task-error-conditioned mechanism that survives multiple exact
  repeats and improves capture, arrival, loads, or actuator use without
  changing far-field closure or either wake view.
