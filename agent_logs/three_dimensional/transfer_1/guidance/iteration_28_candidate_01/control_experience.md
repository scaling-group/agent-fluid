...
- Treat correlation-based removal of beat-synchronous sway as descriptive,
  not a validated course observer. After the assigned parent's mean-curvature
  servo missed below at `1.8818L`, the bounded joint-speed phase projection
  did reduce offline route-error variation but its closed-loop rollout still
  missed below at `1.5445L`; a subsequent slow target-bearing recovery also
  retained the active two-view wake yet worsened closest approach to
  `1.8004L`. Do not retune either observation residual or use offline signal
  smoothing as a capture surrogate. The first unsafe-terminal spatial
  allocator instead preserved total steering authority, terminal speed,
  clipping, speed residence, loads, and the coherent traveling wake while
  capturing at `0.7492L`: this supports an exact repeat of anterior steering /
  posterior propulsion role separation, not a robustness claim or a larger
  transfer. This implication applies to the coherent lower-pass topology after
  both phase and slow-bearing observation repairs have failed; falsify it if
  the exact allocator repeat misses, changes the miss side, weakens either wake
  view, or leaves the sampled speed-reserve actuator/load envelope.
