...
- Reject both the tested outer-terminal bearing rescue and burden-conditioned
  spatial steering allocation rather than widening or scalar-tuning them. The
  bearing residual retained both organized wake views but missed at `1.5629L`;
  the allocator's exact repeat also retained its active carrier yet missed at
  `1.8544L`, exited on the same lower branch, and raised action clipping to
  about `70.2%/72.2%`, making its record `1/2` after the earlier `0.7474L`
  capture. Across the four current sampled captures, absolute body-frame
  achieved-course error at the first `4L` crossing is `0.050--0.113 rad` and at
  `3L` is `0.128--0.275 rad`; the allocator failure is already at `0.369` and
  `0.489 rad`, respectively, while propulsion, planar loads, moment, and local
  crossflow remain inside the sampled capture envelope. For a coherent,
  broadly acquired trajectory, treat persistent outer-terminal course error
  as unfinished redirect rather than evidence for another target-bearing or
  actuator-allocation residual: test whether it should veto correct-yaw-based
  steering release while preserving the raw achieved-course servo, original
  steering shares, and posterior carrier. This is an unevaluated implication,
  not a claimed improvement; falsify it if repeated capture, either wake,
  arrival, terminal speed, loads, clipping, or the lower-pass topology worsens.
