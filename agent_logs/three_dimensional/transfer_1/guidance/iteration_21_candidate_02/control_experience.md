...
- Treat carrier-synchronous achieved-course oscillation as an observation
  problem before adding another terminal actuator residual. Two sampled exact
  posterior-wave runs captured at `0.7480--0.7492L`, but the assigned parent's
  exact repeat retained an active alternating wake, reached only `1.7867L`,
  and exited below; it was already outside the useful projected corridor at
  `2.75L` (`2.00L` miss versus `0.64--1.02L` for the captures), so a pulse
  beginning there was too late and the mechanism is only `2/3` without a load
  or actuator benefit. Across those runs and the sampled speed-reserve
  captures, raw body-frame lateral velocity correlates about `-0.95` with
  anterior joint speed, with a consistent fitted carrier component of
  `-(0.094--0.102) qdot_1`; removing that component reduces lateral-velocity
  scatter roughly threefold. Preserve raw velocity for LOS and closest-pass
  geometry, but test joint-state phase compensation in the achieved-course
  observation before more posterior shaping, half-cycle allocation, or yaw
  damping. This applies while the wake and forward speed remain coherent;
  falsify it if compensation biases the mean course, worsens broad closure or
  capture, weakens the wake, or increases saturation and loads.
