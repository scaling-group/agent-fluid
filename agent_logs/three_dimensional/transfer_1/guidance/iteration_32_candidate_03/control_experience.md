...
- Treat the first-`3L` target-bearing signature as diagnostic, not a controller
  input to keep tuning. The assigned parent found that the mixed-record exact
  speed-reserve baseline (`3/5` captures) separated one lower exit from current
  captures by bearing/course sign, but the inherited bounded target-bearing
  recovery retained active `0.84--0.90L/T` propulsion and still exited below
  after a `1.8004L` pass; a wider closest-pass response veto likewise missed at
  `1.3725L`. Preserve raw achieved course, the response/intercept scaffold, and
  the traveling bend rather than adding another terminal observer. Reject
  tail-to-head steering reallocation in this topology, including the formerly
  unevaluated conditional form. A fixed unsafe-terminal transfer captured at
  `0.7492L` with both coherent wake views but arrived at `18.7495T`, later than
  the two current exact-baseline captures, while head/tail clipping rose to
  `68.76%/70.75%` and speed-limit residence to `10.74%/11.62%` instead of
  improving the baseline envelope. The assigned parent then conserved total
  steering and activated transfer only when normalized posterior speed/action
  burden exceeded anterior burden, the residual worsened posterior motion,
  and anterior margin was present; it nevertheless missed at `1.5231L`, exited
  the lower domain, and finished `10.4210L` away with score `-11.2989`.
  Restore the exact achieved-course speed-reserve policy after this semantic
  regression; do not tune the pressure gates, transfer fraction, or fixed
  share. This applies to terminal spatial allocation on the established
  coherent, broadly acquired route. Revisit only with a genuinely different
  actuator mechanism and repeated capture plus a clear wake, load, or envelope
  benefit; the parent artifact has no wake sheet, so its scalar/termination
  result alone does not establish a wake-failure cause.
