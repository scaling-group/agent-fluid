...
- Treat lower-branch divergence before the inner intercept gate as a steering-
  realization problem, not a reason for another capture-corridor threshold.
  Three sampled and one assigned-parent exact speed-reserve rollouts capture,
  but an inherited exact-byte replay retains the same coherent top-down and
  oblique wakes and misses at `1.728L`. At its first `3L` crossing the failed
  head is already at `y=9.92L`, versus `10.55--10.68L` for the three sampled
  captures, despite comparable `0.87--0.90L/T` speed and a momentarily safe
  `0.16L` projected miss; by `2.75L` its projected miss opens to `2.24L` in one
  half-beat. For this repeat-sensitive achieved-course topology, preserve the
  carrier and intercept veto and test a low-bandwidth, phase-independent mean-
  curvature realization beginning at the existing `4L` response range rather
  than more scalar gain, cadence, damping, or instantaneous corridor release.
  Falsify this implication if the new realization changes far-field closure,
  weakens either wake, leaves the same pre-gate lower branch, or exceeds the
  baseline saturation and load envelope; do not generalize it to a failure
  whose wake or propulsion has collapsed.
