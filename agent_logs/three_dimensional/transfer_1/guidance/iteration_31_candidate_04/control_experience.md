...
- Direction, not absolute activity, is essential when actuator burden gates a
  spatial steering allocator. The sampled fixed unsafe-intercept tail-to-head
  transfer captured at `0.7492L` but arrived later (`18.7495T`) and did not
  reduce clipping or speed-limit residence relative to exact speed-reserve
  captures at `18.2875--18.6010T`. The inherited step-29 allocator then treated
  simultaneous speed/action magnitude as burden; despite a coherent active
  wake it passed below at `1.1961L` and exited. The step-30 refinement required
  outward previous action, posterior-over-anterior burden, anterior margin, and
  a steering residual that would push the tail farther outward; it retained
  both wake views and captured at `0.7474L` in `18.3040T`. This isolates a
  reusable sign-sensitive allocation condition, but not an actuator benefit:
  its head/tail clipping (`68.69%/70.61%`) and speed-limit residence
  (`10.61%/11.60%`) still overlap the sampled baseline envelope. Preserve raw
  achieved course, the intercept scaffold, traveling bend, and total steering
  share; never infer unavailable authority from unsigned activity alone. Treat
  the narrow allocator as repeat-worthy rather than superior, and reject it if
  an exact repeat loses capture or if repeated captures do not lower posterior
  burden without worse arrival, far-field closure, wake coherence, speed,
  force, or moment.
