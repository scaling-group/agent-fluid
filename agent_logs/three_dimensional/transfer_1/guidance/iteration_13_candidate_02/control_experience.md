...
- Do not optimize terminal control allocation for clamp fraction alone. From
  ...
  captured at `0.7480L` and `18.2050T` with `0.9083L/T` crossing speed and no
  material load increase over the earlier threshold capture (peak force
  `0.03023` versus `0.03019`; peak yaw moment `0.01570` versus `0.01592`).
  Because both joints still touched `260 deg/T` and acceleration still clamped
  on about `68.6%/71.0%` of rows, treat selective outward relief as a promising
  authority-allocation mechanism rather than proof that saturation is solved;
  require an exact-policy repeat before tuning its reserve thresholds or adding
  another terminal primitive. Falsify the implication if the replay misses the
  capture disk, returns the lower-exit topology, loses the coherent carrier, or
  worsens speed saturation or loads.
