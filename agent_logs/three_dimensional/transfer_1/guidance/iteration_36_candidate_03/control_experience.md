...
- Do not call the exact speed-reserve baseline robust because three currently
  sampled exact-byte sheets capture. The assigned parent's rollback still
  passed below at `1.4107L` and exited, leaving the accumulated baseline record
  at only `4/7`; coherent thrust and unchanged bytes therefore do not remove
  terminal branch sensitivity. In contrast, the narrow unsupported-bearing
  qualifier has now captured twice with exact bytes at `0.74881--0.74986L` and
  about `18.3205--18.6560T`. Its sampled sheet retains the self-propelled
  alternating top-down street and bilateral oblique wake at `0.8369L/T`, while
  head/tail clipping (`68.75%/70.70%`), speed-limit residence
  (`10.32%/11.29%`), and peak planar-force/yaw-moment coefficients
  (`0.0314/0.0165`) remain in the repeat-backed baseline envelope. Preserve
  raw achieved course and the traveling bend, and require another exact repeat
  of this qualifier rather than widening or scalar-tuning its gates. This is
  evidence for the specific outer-terminal disagreement qualifier, not for
  generic bearing injection. Falsify it if the repeat changes far-field
  closure, loses capture, weakens either wake, or leaves the actuator/load
  envelope.
...
- Reject the tested burden-conditioned spatial allocator rather than widening
  or gain-tuning it. Its required exact repeat preserved the active alternating
  ...
  capture. This does not contradict the narrow unsupported-bearing qualifier:
  that mechanism changes only the route request in an evidenced course/bearing
  disagreement and leaves spatial allocation fixed. Revisit spatial allocation
  only if a different normalized state signature independently repeats capture
  while improving, rather than merely redistributing, clipping, speed
  residence, force, or moment.
- Do not tune the tested post-pass recovery family further. The inherited
  progress-loss curvature replacement missed at `1.2402L`, never recovered
  positive closure, entrained the realized motion after about `20T`, and
  visibly coasted without a substantial new alternating wake. Adding measured
  correct-sign response release improved the next closest pass to `1.1790L`
  and reduced clipping, but still produced no second approach and exited at
  `10.2248L`; replacing curvature with an opening-only, response-released
  posterior wave-shape burst then regressed to a `1.6057L` miss and
  `10.0363L` lower exit. The last result has only inherited policy/score
  evidence here, so do not infer its wake or actuator cause; it nevertheless
  supplies no semantic recovery counterexample. Avoid tuning the closing gate,
  release threshold, curvature, or posterior shift in these variants. This
  applies only after measured closing loss and does not challenge the
  successful first approach; revisit recovery only with a distinct normalized
  geometry primitive that demonstrates an actual propelled second approach
  without sacrificing capture, either wake view, or the actuator/load
  envelope.
