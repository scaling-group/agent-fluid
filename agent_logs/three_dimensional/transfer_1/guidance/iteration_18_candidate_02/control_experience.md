...
- Do not optimize terminal control allocation for clamp fraction alone.
  ...
  `0.8268--0.9083L/T`, and remain in a similar load envelope. Yet their
  terminal headings span `0.116--0.790 rad` and their head crossings span
  `y=9.115--9.891L`, straddling the target ordinate. Wake/load consistency
  across that endpoint spread establishes the interception guard plus
  selective outward relief as a repeat-supported geometry/allocation
  mechanism rather than one favorable beat phase. It does not solve
  saturation: both joints still touch
  ...
  when testing terminal observations; avoid endpoint heading, beat-phase, or
  crossing-side locks, total-command projections, and reserve-threshold tuning
  unless repeated capture, wake coherence, and loads remain primary criteria.
  Falsify this lesson if further exact repeats lose capture or a different
  phase-conditioned allocation improves both repeat success and envelope
