...
- Do not treat instantaneous membership in a projected capture corridor as
  proof that additive steering is expendable. Starting from the repeat-backed
  speed-reserve controller, a phase-independent floor that released only ten
  percent of steering inside the existing projected-miss, approach-alignment,
  and intercept-distance gates retained the active alternating wake and about
  `0.823L/T` speed, yet missed below at `1.4597L` and exited with final distance
  `10.4685L`. This contrasts with the baseline's repeated captures, whose two
  current sampled crossings already span reconstructed projected miss of about
  `0.24--0.73L` and alignment of about `0.95--0.23`; transient gate membership
  is therefore not a safe-release certificate in a beat-scale terminal pass.
  Preserve the evaluated steering/intercept logic after broad acquisition;
  avoid widening or scalar-tuning the release floor, and require repeated
  capture before adopting another terminal authority reduction. This applies
  to coherent, actively propelled approaches; revisit only if exact repeats
  preserve capture and demonstrate a consistent arrival, load, or actuator
  benefit beyond baseline variability without weakening either wake view.
