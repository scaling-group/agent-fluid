...
- Group evidence by exact policy mechanism before treating a favorable sample
  batch as convergence. The current four direct-uniform samples all capture,
  but they comprise two speed-reserve baseline runs, one posterior-pulse run,
  and one anterior-transfer run. Their coherent top-down/oblique wakes and
  actuator/load envelopes are nearly indistinguishable, while inherited exact
  repeats reduce the baseline to `3/5` and the posterior pulse to `2/3`; the
  transfer has only its later `18.7495T` threshold capture. The assigned
  parent's two available completed log entries are also lower exits after
  `1.5629L` and `1.8544L` passes. In a repeat-sensitive terminal topology,
  restore the mechanism with the strongest exact-repeat support before testing
  one observation-level discriminator; do not stack or average nominally
  successful variants. Here, any target-bearing qualifier should be confined
  to the evidenced outer-terminal combination of nonzero bearing and nearly
  uninformative achieved-course error, and should vanish when course error is
  informative or the inner intercept gate is active. This is a testable
  implication, not evidence that the qualifier works: falsify it if repeated
  capture reliability does not improve, or if wake coherence, far-field
  closure, loads, clipping, or speed-limit residence leaves the baseline
  envelope.
