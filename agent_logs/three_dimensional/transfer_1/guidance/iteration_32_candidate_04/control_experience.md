...
- Stop perturbing the inbound course observation or spatial steering allocation
  in this lower-branch topology. The assigned fixed anterior transfer has one
  `0.7492L` capture but an exact-byte inherited miss at `1.3915L`; two bounded
  bearing rescues miss at `1.8004L` and `1.5629L`, while burden/pressure-
  conditioned anterior transfers miss at `1.1961L`, `1.5231L`, and `1.8544L`.
  The inspected failures keep an alternating top-down street, bilateral
  oblique structures, and active self-propulsion after closest pass, just as
  all four assigned `0.7466--0.7494L` captures do. Thus neither another bearing
  coefficient nor a fixed/conditional tail-to-head transfer is a supported
  inbound repair. Preserve the repeat-backed achieved-course carrier and test
  a semantically separate missed-pass recovery only when normalized body-frame
  target-aft projection and negative closing speed agree; recorded-trace replay
  makes that conjunction zero through all assigned captures but active on all
  seven available lower exits. This selectivity is not closed-loop evidence.
  Falsify recovery if it changes first-pass closure, fails to produce a second
  approach before exit, weakens either wake, or moves saturation and loads
  outside the speed-reserve envelope; on failure avoid stacking another
  bearing, projected-miss, phase, allocation, or scalar residual on the inbound
  controller.
