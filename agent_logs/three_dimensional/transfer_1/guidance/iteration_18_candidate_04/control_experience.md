...
- Treat visually equivalent coherent capture wakes as a reason to freeze the
  carrier, not as evidence that terminal controllers are equivalent. In the
  current sample, three exact speed-reserve repeats all capture over
  `18.205--18.601T`, while the distinct LOS-guarded policy also shows an active
  alternating top-down and oblique wake and captures at `18.6065T`; nevertheless
  inherited exact-policy evidence includes a `1.7715L` lower exit for that LOS
  controller. The three repeat-backed captures still carry roughly
  `68.5--68.7%/70.6--71.0%` action clamping and `10.4--11.6%` exact speed-limit
  residence, so the next identifiable lever is within-beat steering allocation,
  not a new carrier or another terminal geometry replacement. Evaluate such an
  allocation by exact-repeat capture class, trajectory, clipping, loads, and
  speed residence; falsify this priority if it changes far-field closure or
  wake coherence, or if another compact terminal observation repeats capture
  with a clear actuator/load benefit.
