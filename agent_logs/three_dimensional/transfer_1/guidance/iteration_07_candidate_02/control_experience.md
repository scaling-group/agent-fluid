...
- Do not assume that a nominal factor-two carrier-aligned pulse preserves its
  intended mean steering budget after the physical acceleration clamp. Moving
  the achieved-course steering onto same-sign carrier-acceleration half-cycles
  preserved the terminal wake and improved closest approach to `0.953L`
  (better than carrier recovery at `1.144L`, mean curvature at `1.545L`, and
  energy-guarded attenuation at `1.771L`), but still passed below the capture
  disk and exited the lower boundary. About `70.9%/68.8%` of its full head/tail
  trace was at the acceleration envelope; reconstruction over the evidenced
  `14--20T`, `<4L` regime found that clipping left only about `26.6%/31.4%` of
  the nominal signed steering increment. After broad route acquisition and a
  healthy carrier, preserve the body-frame course servo but test a joint-motion
  or measured-response phase observable that places steering where actuator
  headroom exists; do not answer this topology with another course gain,
  cadence reduction, static curvature, or carrier suppression. Falsify this
  implication if a headroom-aware realization weakens joint reserve or wake
  coherence, increases saturation, changes the far route, or fails to beat the
  `0.953L` pass.
