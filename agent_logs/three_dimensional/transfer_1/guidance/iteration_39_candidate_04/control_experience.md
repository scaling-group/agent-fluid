...
- Treat outer-annulus LOS-rate interception as compatible with the established
  carrier, but not yet as a reliable replacement baseline. After the binary
  capture-corridor guard kept both organized wake views active yet missed at
  `1.6783L`, a separately bounded `2 rad/T^2` residual opposing inertial LOS
  rotation only on an approaching `4.0--1.5L` pass captured at `0.7461L` and
  `18.2985T` with the same active alternating wake. It arrived earlier than
  three of the four sampled captures (`18.4525--18.6560T`) but just after the
  fastest (`18.2875T`); its score (`-0.15938`) and mean distance (`2.04677L`)
  did not improve their scalar envelope. Head/tail clipping
  (`68.50%/70.51%`), speed-limit residence (`10.58%/11.66%`), and peak body-
  force/moment coefficients (`0.01475/0.02916/0.01593`) also remained
  effectively inside the sampled speed-reserve range, so the useful evidence
  is semantic compatibility, not actuator relief. Test exact-policy
  repeatability before changing its bound, annulus, or carrier, and do not
  stack the unreliable unsupported-bearing qualifier. Falsify the mechanism
  if a repeat loses capture, retains the coherent-wake lower exit, fails to
  reduce pre-pass LOS rotation, or moves wake, clipping, speed residence,
  force, or moment outside that envelope.
