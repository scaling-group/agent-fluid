...
- Reject the outer-terminal unsupported-bearing qualifier as a robust repair,
  ...
  `0.74986L` but the inherited direct-uniform repeat kept the same organized
  top-down and oblique wakes, passed at `1.18462L`, and exited below; its peak
  force/moment coefficients (`0.01462/0.02753/0.01535`) remained inside the
  repeat-backed envelope. The qualifier is therefore `1/2` and did not remove
  the coherent post-pass failure topology despite its first run's best sampled
  scalar score. Preserve raw achieved course and the traveling bend; do not
  stack or scalar-tune this bearing cue. Revisit only if a distinct response
  observation first establishes repeated capture without changing far-field
  closure, either wake, or the actuator/load envelope.
...
- Prefer a bounded pre-pass response residual over late recovery or a binary
  projected-corridor rule in this coherent-wake topology, but require exact
  repeats before tuning or stacking it. The post-pass burst requested the
  unavailable `window_closing_speed_L`, engaged only after its `1.1608L`
  minimum, and retained the lower exit despite lower clipping; the hard
  capture-corridor certificate then worsened closest approach to `1.6783L`,
  and unchanged rollback missed at `1.0260L`. In contrast, the next two soft
  outer-terminal response mechanisms each captured at `18.29849T`: partial
  steering retention at `0.74852L` and a bounded inertial-LOS-rate residual at
  `0.74605L`. Both retained the organized top-down and oblique wakes. The LOS
  residual also stayed near the repeat-backed envelope, with action clipping
  `68.50%/70.51%`, speed-limit residence `10.58%/11.66%`, and peak body-force/
  moment coefficients `0.01475/0.02916/0.01593`; its worse scalar score than
  the sampled parent came from mean-distance cost, not failed capture. Each is
  still only a one-run mechanism demonstration, and the earlier qualifier's
  `1/2` record makes an unchanged independent repeat the next reliability test.
  Apply this lesson only after broad route acquisition with an active traveling
  wake; falsify either soft response mechanism if its repeat misses, retains
  the lower branch, changes far-field closure, weakens either wake, or moves
  actuator/load metrics outside the established envelope.
