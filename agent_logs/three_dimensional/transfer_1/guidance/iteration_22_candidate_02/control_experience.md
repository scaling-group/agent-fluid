...
- Reject the small intercept-gated posterior wave-shape residual as a
  reliability improvement in this topology; restore the exact speed-reserve
  baseline instead of tuning its gain or gate. Two sampled exact-policy runs
  captured at `0.7480--0.7492L` in `18.1995--18.4690T`, but the assigned
  parent's completed third exact-byte run exited the lower boundary after a
  `1.3584L` pass and ended at `10.5799L`. All three retain the active
  alternating top-down wake and compact oblique structures, while the two
  captures overlap baseline score, distance-integral, clipping, speed-limit,
  and load ranges. Thus normalized anterior-joint phase can perturb posterior
  wave shape without collapsing propulsion, but the `2/3` capture record and
  absent secondary-metric separation falsify the claimed control benefit.
  This negative result applies to adding that residual inside the established
  intercept gate of the coherent, broadly acquired speed-reserve controller;
  it does not reject other state-feedback steering realizations. Reconsider
  only if multiple exact repeats reliably capture and separate from the exact
  baseline in terminal path, arrival, loads, or actuator metrics while
  preserving both wake views.
