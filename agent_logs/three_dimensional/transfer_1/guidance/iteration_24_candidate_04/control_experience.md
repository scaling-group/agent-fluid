...
- Reject the intercept-gated posterior wave-shape pulse as a robust terminal
  improvement, rather than tuning its gain after mixed threshold crossings.
  The exact policy has three sampled or inherited captures at
  `0.7480--0.7492L`, but the two latest exact replays passed below at
  `1.2589L` and `1.3584L` and exited the lower boundary. Both failures retain
  self-propulsion and coherent alternating top-down and oblique wakes, while
  the successful runs' scores, clipping, speed residence, and force/moment
  ranges overlap the four-capture speed-reserve baseline. The failure is thus
  a phase-sensitive terminal-path change without a repeatable secondary
  benefit, not deficient propulsion. In this coherent, broadly acquired
  topology, preserve the baseline carrier and phase-independent steering;
  avoid scalar pulse tuning or stacking posterior shaping with the failed yaw
  brake. Revisit posterior wave shaping only if a different observation-level
  mechanism survives multiple exact repeats and separates terminal geometry,
  arrival, loads, or actuator use from baseline variation without changing
  far-field closure.
...
- Do not treat instantaneous projected-corridor membership as permission to
  relax terminal steering. The assigned capture-hold policy was apparently
  safe near `2L` (projected miss about `0.51L`, alignment about `0.97`) but
  retained a coherent active wake while the miss grew to `1.43L`, alignment
  became negative, and it exited below; the assigned parent's next exact
  speed-reserve rollback captured at `0.7485L`. Across three sampled baseline
  captures and that failure below `4L`, body-frame lateral velocity is also
  strongly carrier-correlated (`corr(v_y,qd1)=-0.938-- -0.965`, fitted slope
  `-0.093-- -0.104L/rad`). For this coherent, broadly acquired topology,
  preserve raw velocity for physical LOS/interception gates and test removal
  of the evidenced joint-phase component only from the slow achieved-course
  observation; avoid widening or scalar-tuning geometry-only release floors.
  Falsify this implication if exact capture-hold repeats establish reliable
  capture plus a secondary benefit, or if course phase compensation changes
  far-field closure, weakens the traveling wake, or loses the repeat-backed
  capture class.
