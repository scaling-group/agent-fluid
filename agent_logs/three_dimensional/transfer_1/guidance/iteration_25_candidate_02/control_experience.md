...
- Treat low-bandwidth mean-curvature tracking as another falsified actuation-
  side response to this terminal error, not as a gain-tuning opportunity. The
  assigned parent's target-signed average-joint servo activated only below
  `4L`, retained the alternating top-down and oblique wakes, yet reached only
  `1.8818L` and exited below at `31.119T` with final distance `9.7529L`.
  Across four sampled captures and that failure, instantaneous terminal body-
  frame lateral velocity correlates `-0.937` to `-0.961` with anterior joint
  speed; an offline one-state phase projection of those traces reduces its
  standard deviation from `0.225--0.336 U` to `0.062--0.117 U`. When this
  signature is present, first test a bounded, distance-gated phase compensation
  of the achieved-course observation while retaining raw velocity for closest-
  pass geometry and the evaluated traveling-bend actuation; do not add or
  retune average curvature.
  This implication applies to coherent, broadly acquired approaches with
  beat-synchronous sway; falsify it if compensation changes the path outside
  its gate, weakens the wake, loses capture, or moves saturation and loads
  outside the speed-reserve envelope.
