...
- Reject the terminal posterior wave-shape pulse on its current repeat record,
  rather than rescuing it with isolated gain or gate tuning. The pulse used
  normalized anterior-joint phase and body-frame turn request only inside
  `2.75L`; three exact runs captured with coherent wakes and baseline-like
  clipping and loads, but the fourth reached only `1.3584L` and exited below.
  In that failure the fish entered the terminal region with projected miss
  about `1.94L`, was already receding at its closest pass, still swam at about
  `0.826L/T`, and retained an active alternating top-down and oblique wake.
  Thus the mechanism is neither a propulsion nor actuator-envelope cure, and
  its `3/4` threshold record is weaker than the speed-reserve baseline's
  inherited `4/4`. For this fixed still-water topology, restore the exact
  baseline and test a distinct terminal observation or steering mechanism only
  after repeat evidence warrants it. Revisit the pulse only if independent
  exact repeats establish reliable capture plus a terminal-path, arrival,
  load, or saturation benefit without weakening the traveling wake.
