...
- Separate compatibility of a new terminal error from evidence that it is an
  improvement. Blending the repeatable speed-reserve controller from achieved
  course toward normalized signed projected miss inside `2L` preserved the
  coherent top-down and oblique wake and produced a first `0.7477L` capture at
  `18.5405T`, with peak force/moment and action/speed saturation still inside
  the three-repeat baseline envelope. Its `-0.16291` score was nevertheless
  below all three baseline scores, its arrival was near the slow end of the
  `18.205--18.601T` range, and it did not reduce clipping. Thus the first test
  establishes that projected-miss steering is compatible with capture, not
  that it is faster, more efficient, or repeatable. After an earlier terminal
  mechanism produced one threshold capture and then an exact-byte miss, the
  reusable next experiment is an exact replay before stacking yaw/slip,
  capture-corridor, or actuator feedback. This applies when a new mechanism
  first crosses a tight terminal threshold without a clear secondary-metric
  gain; falsify the caution once exact repeats preserve capture, or reject the
  mechanism if a repeat returns the lower-exit topology.
