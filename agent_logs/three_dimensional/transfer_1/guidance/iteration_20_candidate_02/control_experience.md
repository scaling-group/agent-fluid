...
- Distinguish a terminal-gated posterior wave-shape pulse from the rejected
  all-joint half-cycle and yaw-residual variants, but do not promote it from one
  run. The sampled posterior policy is exactly baseline-equivalent outside
  `2.75L` and captured at `0.7492L` in `18.4690T` with an active alternating
  two-view wake; its action clamping (`68.40%/70.88%`), speed-limit residence
  (`10.51%/11.32%`), peak lateral-force coefficient (`0.0286`), and peak
  yaw-moment coefficient (`0.0161`) stay within the three-capture speed-reserve
  envelope. In contrast, inherited carrier-signed half-cycle allocation,
  terminal yaw damping, and ungated velocity-half-cycle asymmetry retained
  coherent wakes but exited after closest approaches of `1.6860L`, `1.4601L`,
  and `3.7595L`; the ungated case had already changed the route before the
  claimed terminal regime. This establishes one mechanism-compatible capture,
  not superiority over the `3/3` simpler parent. Repeat the exact posterior
  policy before changing its gain or stacking another residual; retain the
  simpler baseline if repeats lose capture or fail to add a reliable arrival,
  load, or actuator benefit. This implication applies only when the pulse is
  algebraically absent in the far field; falsify it if exact repeats establish
  either a recurrent lower exit or a consistent benefit outside the parent
  saturation/load envelope.
