...
- Preserving oscillation alone does not validate a replacement terminal
  actuator. Relative to the `1.267L` opposing-half attenuation pass, releasing
  attenuation from low joint energy still ended `left_domain` at `1.771L`,
  while replacing shared steering with posterior mean curvature kept an
  alternating wake but widened the miss to `1.545L` and put both actions on the
  acceleration envelope at closest approach. After broad course acquisition,
  preserve both the traveling carrier and the already useful direct course
  steering; test a phase-selective steering allocation that does not subtract
  carrier authority before adding more cadence, route gain, carrier
  attenuation, or static curvature. Falsify this implication if such an
  allocation weakens early closure or wake coherence, worsens saturation, or
  cannot beat the `1.267L` failure class.
