...
- Reject normalized signed projected miss as a replacement terminal steering
  ...
  exact `dogfish3d_speed_reserve_projected_miss_turn_v1` bytes produced one
  threshold capture at `0.7477L` but then two direct-uniform, stable lower exits
  at `1.7680L` and `1.6366L`; both failures retained active alternating wakes
  and continued self-propelling after closest pass. Its lone capture was also
  slower-scoring than all three exact speed-reserve captures and did not reduce
  clipping. This `1/3` success rate contrasts with `3/3` captures for the
  achieved-course/intercept speed-reserve baseline, so the failure is terminal
  path geometry, not insufficient propulsion. Revert to that repeat-supported
  error and test a bounded within-beat steering realization while leaving the
  carrier active; do not stack more terminal observations onto projected miss.
  This conclusion applies to replacing the route command inside the last `2L`;
  revisit it only if multiple exact repeats become reliable and add a clear
  arrival, load, or saturation benefit without changing far-field closure.
- Verify a claimed terminal scope in the executable feedback expression; a
  prose hypothesis is not evidence that far-field behavior is preserved. The
  inherited velocity-half-cycle controller omitted its distance gate, changed
  steering on the whole route, and was already near `(13.09,8.47)L` at the
  closing `4L` crossing versus `(13.14,11.21)L` for the highest-scoring sampled
  baseline; it never entered the terminal corridor, reached only `3.7595L`,
  and exited below despite an active coherent wake. An explicitly gated
  carrier-acceleration half-cycle variant and a `1.5L` yaw-damping residual
  also lost the four-repeat baseline capture class at `1.6860L` and `1.4601L`.
  Avoid further all-joint additive-steering phase allocation or terminal yaw
  residuals in this topology. If testing a distinct posterior wave-shape
  mechanism, require an algebraically exact zero outside its body-frame range
  gate and verify command identity on far-field and mirrored states before
  evaluation. Revisit the rejected mechanisms only if a gated implementation
  retains capture and wake coherence without worsening clipping, speed-limit
  residence, force, or moment loads.
- Treat a small, explicitly gated posterior wave-shape pulse as compatible
  with the repeat-supported carrier, but not yet as an improvement. Its first
  direct-uniform sample captured at `0.7492L` and retained the alternating
  top-down and compact oblique wake; head clamp fraction (`68.40%`), head
  speed-limit residence (`10.51%`), and peak lateral-force/yaw-moment
  coefficients (`0.0286/0.0161`) stayed within the three exact baseline
  capture ranges. However, tail effort did not improve and mean distance
  (`2.04564L`) was worse than the best baseline sample (`2.03873L`). Unlike
  the failed all-joint half-cycle and yaw-residual variants, this mechanism is
  algebraically zero outside the `2.75L` intercept gate and changes only the
  posterior target through normalized anterior joint speed. Before tuning its
  gain or adding another terminal residual, require an exact-policy repeat;
  one capture establishes compatibility, not robustness or score benefit.
  Apply this only after broad route acquisition with the coherent carrier;
  reject it if a repeat loses capture, changes far-field closure, weakens the
  wake, or exceeds the baseline saturation and load envelope.
