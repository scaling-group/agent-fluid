...
- Do not call the exact speed-reserve baseline robust from its four current
  ...
  `18.3205--18.7440T`. The two detailed repeats preserve the traveling
  top-down and oblique wakes, `0.8369--0.8663L/T` terminal speed,
  `68.69--68.75%/70.70--70.80%` action clipping,
  `10.38--10.45%/11.35--11.41%` speed-limit residence, and the baseline
  force/moment envelope. This is positive semantic reliability evidence for
  retaining raw achieved course plus the narrow qualifier, not permission to
  widen or scalar-tune its gates. Apply it only to the evidenced outer-terminal
  disagreement signature; falsify robustness if an exact repeat changes motion
...
- Reject the burden-conditioned spatial allocator rather than widening or
  ...
  semantic or envelope benefit. Preserve the exact speed-reserve allocation
  and its posterior traveling bend; do not stack projected-miss, observer,
  phase, or actuator-state residuals onto the outer-terminal qualifier. The
  unqualified carrier remains a useful rollback, not a robust `4/7` capture
  policy. Revisit spatial allocation only if a different normalized state
