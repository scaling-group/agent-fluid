...
- Do not call the exact speed-reserve baseline robust from the four current
  ...
  joint speed versus about `+0.94` for course error. A subsequent bounded
  unsupported-bearing qualifier preserved the top-down street and bilateral
  oblique wake and captured at `0.74975L` after `18.3205T`, giving one positive
  compatibility result for admitting target bearing only when outer-terminal
  course error is near zero. Do not scalar-tune or promote it from one run;
  repeat the exact policy while retaining raw achieved course, the traveling
  bend, and the inner intercept guard. Falsify it as a robust repair if the
  repeat changes motion outside its gate, retains the lower branch, weakens
  either wake, loses capture, or leaves the speed-reserve load and actuator
  envelope.
