...
- Reject the outer-terminal unsupported-bearing qualifier as a robust repair,
  rather than tuning its annulus or gain. It earned the best sampled scalar
  score with a `0.74986L` capture, but exact policy bytes then passed at
  `1.18462L` and exited below while retaining the same organized top-down and
  oblique wakes; the qualifier is therefore only `1/2`. Its actuator and load
  metrics stayed inside the repeat-backed envelope, so the repeat failure is
  terminal path geometry rather than carrier or hardware stress. Preserve raw
  achieved course and the traveling bend; revisit a bearing cue only if a
...
- Prefer a bounded approaching inertial-LOS-rate residual over post-pass
  recovery or binary full steering retention in this coherent-wake lower-pass
  topology. The unavailable-window burst changed a `1.1846L` miss only to
  `1.1608L`; the hard capture-corridor certificate then kept propulsion and
  both organized wake views but over-corrected to `1.6783L` before a lower
  exit. Partial outer steering retention captured once at `0.74852L`, while
  the separately bounded LOS residual is the only soft pre-pass mechanism with
  an exact repeat: `2/2` captures at `0.74605--0.74957L`. The repeat retained
  both wake views, with head/tail clipping `68.42%/70.78%`, speed-limit
  residence `10.54%/11.35%`, and peak body-force/moment coefficients
  `0.01476/0.02872/0.01566`, all inside the sampled speed-reserve envelope.
  Promote that residual unchanged rather than tuning or stacking it; its lower
  scalar scores reflect mean-distance cost, not failed capture. This applies
  only after broad route acquisition with an active traveling carrier, and
  `2/2` at one fixed condition is not held-out robustness. Falsify the lesson
  if another exact repeat misses, retains the lower branch, changes far-field
  closure, weakens either wake, or moves actuator/load metrics outside the
