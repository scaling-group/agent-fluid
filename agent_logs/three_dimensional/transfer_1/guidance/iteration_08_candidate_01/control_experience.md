...
- Separate terminal actuator placement from terminal response feedback. A
  factor-two carrier-aligned pulse preserved the wake and reached `0.953L`
  despite clipping retaining only about `26.6%/31.4%` of its nominal signed
  head/tail steering increment. The assigned parent's joint-motion allocation
  was predicted on that trace to expose more headroom, but its completed CFD
  pass regressed to `1.222L` and retained the below-target lower exit; more
  surviving open-loop steering is therefore not sufficient. That result is
  consistent with sampled static mean curvature at `1.545L` and energy-guarded
  attenuation at `1.771L`; adding normalized target-normal slip as posterior
  mean curvature also reached only `1.056L`. In contrast, using
  joint-compensated yaw response to release the direct shared-steering channel
  reached `0.931L`, the best completed pass, while the alternating top-down
  street and oblique structures remained visible. Once broad course acquisition
  and a healthy carrier are established, prefer measured response as a bounded
  release condition for a phase-selective actuator over more course gain,
  cadence relief, static curvature, carrier suppression, or another additive
  slip bend. This applies only to the coherent sub-`4L` flyby regime; falsify
  it if response-released phase allocation fails to beat `0.931L` or improve
  termination, changes the far route, weakens joint reserve or wake coherence,
  or increases saturation.
