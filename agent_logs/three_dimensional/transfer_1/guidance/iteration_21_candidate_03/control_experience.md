...
- Treat repeated compatibility without a measurable benefit as a negative
  decision, not an invitation to tune the added scalar. The assigned parent's
  requested exact replay makes two current captures for the small
  intercept-gated posterior wave-shape pulse (`0.7480L` at `18.1995T` and
  `0.7492L` at `18.4690T`), and both combined sheets retain the baseline's
  coherent top-down and oblique wakes. Yet their scores (`-0.15678`,
  `-0.15855`) and mean distances (`2.0436L`, `2.0456L`) do not improve on the
  current exact speed-reserve samples, including the population-best
  `-0.15140` score and `2.0387L` mean distance; inherited trace summaries also
  place clipping, speed-limit residence, and force/moment peaks inside rather
  than below the baseline ranges. In this coherent, broadly acquired topology,
  restore the exact achieved-course/intercept speed-reserve controller and do
  not scalar-tune or stack this phase-gated posterior equilibrium pulse. This
  negative result applies to the small pulse inside the existing intercept
  gate, not to every posterior wave-shape controller; revisit only when a
  distinct normalized observation conditions the mechanism and exact repeats
  show a path, arrival, load, or actuator benefit without weakening the
  traveling wake or far-field closure.
