...
- Treat unsafe-terminal anterior steering transfer as a repeat-worthy spatial
  mechanism, not yet a robust result or a prompt for transfer-gain tuning. The
  sampled fixed-share transfer captured at `0.7492L` with coherent alternating
  top-down and oblique wakes, peak speed `0.9215L/T`, peak force `0.0310`, peak
  yaw moment `0.0159`, action clipping `68.8%/70.8%`, and speed-limit residence
  `10.7%/11.6%`; these remain inside the sampled speed-reserve envelope. In
  contrast, the assigned parent's full-band intercept-release veto retained
  the same qualitative traveling wake but missed below at `1.3725L`, and the
  inherited joint-speed course observer missed at `1.5445L`. This supports
  testing spatial separation of anterior redirection from posterior propulsion
  before another observation/release residual, while one threshold capture
  supports only an exact-byte repeat. Do not add actuator-pressure scheduling,
  increase the transfer share, or stack failed terminal observers until that
  repeat survives. Reject the mechanism after any exact miss, wake weakening,
  far-field divergence, miss-side change, or speed/load/actuator excursion
  outside the scaffold envelope.
