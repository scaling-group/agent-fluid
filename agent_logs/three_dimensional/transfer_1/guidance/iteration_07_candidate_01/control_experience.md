...
- Audit the signed route-to-yaw map before escalating steering authority. With
  body-frame `-x` forward, the preserved-carrier rollout had target and course
  angles near `+0.23/-0.67 rad` at `16T`, so their positive difference required
  negative yaw; instead, positive requests produced positive cycle-smoothed
  yaw, stayed saturated through the pass, and led to the common lower exit.
  The inherited logs' still-closer `1.1444L` and `0.9532L` misses retained
  `left_domain` and worsened final distance to `10.39L` and `11.27L`, so a
  smaller minimum distance alone does not validate more same-sign authority.
  Preserve the evidenced carrier and first correct or falsify the observation-
  to-actuator sign using normalized target/course geometry. This lesson is
  falsified if the inverted map weakens early closure, reverses the miss to an
  upper exit without reducing course error, disrupts the alternating wake, or
  fails to reduce persistent clipping and the lower-exit topology.
