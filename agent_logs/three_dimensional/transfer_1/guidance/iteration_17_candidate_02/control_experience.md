...
- Reject carrier-acceleration-signed half-cycle redistribution as the next
  terminal steering realization for this baseline, rather than tuning its
  contrast. The assigned parent's direct-uniform, stable evaluation preserved
  an active alternating wake but entered the `2.75L` intercept region with a
  projected miss near `2.24L`, reached only `1.6860L`, and then bent below the
  target to a `left_domain` finish at `10.3091L`; the three exact-byte
  speed-reserve captures entered at roughly `0.59--0.99L` projected miss.
  Shifting additive steering away from carrier-aligned acceleration with
  `0.45` phase contrast slightly increased action clamping to about
  `70.0%/71.5%` while reducing speed-limit residence to `8.5%/9.5%`, so lower
  speed residence did not represent useful authority. Preserve the proven
  phase-independent steering and sparse carrier reserve; if beat-scale yaw is
  still targeted, test a separately bounded response-damping or wave-shape
  mechanism without carrier-sign redistribution. This negative result applies
  to terminal carrier-acceleration phase weighting inside the intercept gate;
  revisit only with evidence for a different phase/sign observable and with
  capture, projected miss, wake coherence, loads, and clipping all checked.
