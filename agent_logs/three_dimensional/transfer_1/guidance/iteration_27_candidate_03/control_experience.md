...
- Do not treat offline suppression of beat-synchronous course variation as a
  ...
  three exact speed-reserve captures at `0.7466--0.7494L` with active
  alternating top-down and oblique wakes. Restore that raw achieved-course
  baseline after this observer failure, and do not tune the compensation
  coefficient or stack it with prior terminal residuals. This result
...
- Reject extending the projected-corridor veto across the whole terminal-
  response band as a robustness repair, rather than tuning its corridor
  thresholds. The assigned parent's full-band release policy retained an
  active alternating wake and still traveled `0.8415L/T` at closest pass, but
  missed below at `1.3725L`, exited with final distance `10.6694L`, and scored
  `-11.5021`; its action channels still clamped on about `70.3%/70.8%` of
  trace rows. Its head was already lower at the first `4L` crossing
  (`10.714L`) than all four current captures (`10.946--11.069L`), including
  two exact posterior-wave captures at `0.7480--0.7492L`. Thus removing the
  intercept distance blend neither repaired the early terminal branch nor
  reduced clipping enough to explain control improvement. Preserve the
  evaluated distance-blended release architecture and use an exact posterior-
  wave repeat to test its existing compatibility evidence; do not widen the
  full-band veto or stack it with prior terminal residuals. Falsify this
  negative lesson only if repeated full-band evaluations capture while
  preserving far-field closure, both wake views, and the sampled actuator/load
  envelope.
