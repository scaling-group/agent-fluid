...
- Reject additive target-bearing recovery and an inner-only release veto as
  repairs for the exact speed-reserve baseline's `3/5` capture record. The
  assigned parent's bounded bearing residual reinforced an already same-sign
  bearing/course request at the first `4L` and `3L` crossings, retained active
  coherent top-down and oblique wakes, but missed below at `1.8004L` and exited
  at `10.1819L`; a sibling full-band geometry veto also kept both wakes active
  but missed at `1.3725L` and exited at `10.6694L`. These failures falsify both
  the earlier bearing-qualifier proposal and further steering-release edits
  confined inside `4L`. The useful branch difference is already present at
  `5L`: current captures have head `y=11.386--11.501L`, projected miss
  `1.712--2.194L`, and approach alignment `0.898--0.940`, while two exact-
  baseline failures and the bearing-residual failure are lower at
  `y=11.023--11.127L`, with miss `2.769--2.988L` and alignment
  `0.802--0.832`; one or both commands are clamped and a joint is commonly
  near its speed limit there. Preserve raw achieved course and the traveling
  bend; test a middle-distance geometry-conditioned use of the existing sparse
  actuator reserve rather than another bearing term, scalar steering increase,
  or inner-terminal stack. This is an evidence-led test implication, not proof
  that earlier reserve will capture; falsify it if motion outside its gate or
  wake coherence changes, the lower branch remains, carrier collapse appears,
  or actuator/load metrics leave the evaluated speed-reserve envelope.
