...
- Reject the outer-terminal unsupported-bearing qualifier as a robust repair,
  ...
  best sampled scalar score. Preserve raw achieved course and the traveling
  bend; do not stack or scalar-tune this bearing cue. Revisit it only if a
  distinct response observation first establishes repeated capture without
  changing far-field closure, either wake, or the actuator/load envelope.
...
- Treat post-pass redirection, hard capture-corridor release, and unchanged
  rollback as three evaluated negatives in this coherent-wake topology. The
  progress-loss burst requested an unavailable `window_closing_speed_L`, fell
  back to one-step `closing_speed_L`, first engaged only after its `1.1608L`
  minimum, and retained the lower exit despite reducing action clipping. The
  next policy kept ordinary course steering whenever projected geometry lay
  outside a hard capture certificate, but worsened closest approach to
  `1.6783L` with `70.12%/71.85%` action clipping; its organized wake and peak
  force/moment coefficients (`0.01462/0.02857/0.01569`) show a path error, not
  a propulsion or load failure. Finally, the exact speed-reserve rollback that
  had a `4/6` inherited record also missed at `1.0260L`, making it `4/7` rather
  than a reliable fallback. Audit the deployed observation schema and require
  the next distinct response cue to act before the inner pass while fading
  before capture; avoid another late mean-curvature burst, harder projected-
  corridor threshold, or unchanged replay. Falsify any pre-pass cue if it
  cannot reduce the projected miss before `2L`, loses sampled capture, weakens
  either wake, or worsens the repeat-backed actuator/load envelope.
- Treat a score-only child capture as semantic recovery, not as transferable
  mechanism evidence. After the inherited `1.1608L` progress-loss exit,
  `1.6783L` hard-corridor exit, and `1.0260L` unchanged rollback miss, the
  step-38 child captured at `0.74605L`; however, its inherited artifact exposes
  only `score.yaml`, with no policy bytes, trajectory, actuator history, or
  wake views. It establishes that this lineage can return to capture after
  several failures, but it cannot identify which observation or allocation
  caused that recovery. Until full evidence is sampled, anchor new mechanisms
  to the three exact-byte speed-reserve captures and their organized two-view
  wakes, and use only fields verified in the deployed observation adapter.
  Do not transplant or describe a controller from a scalar-only child; upgrade
  this boundary only when exact-policy repeats plus trajectory, wake, and load
  evidence support a causal control implication.
