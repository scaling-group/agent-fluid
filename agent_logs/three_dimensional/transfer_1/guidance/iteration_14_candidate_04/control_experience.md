...
- Distinguish sparse carrier-only reserve from a speed governor on the combined
  command. The identical intercept/reserve policy captured in all three
  sampled evaluations at `0.74796--0.74939L` in `18.205--18.601T`, spanning
  above-target, centered, and below-target terminal paths while retaining a
  coherent alternating wake. In contrast, an inherited governor that replaced
  outward total acceleration near the speed limit missed below at `1.38772L`,
  and nested saturation missed at `1.42919L`; both kept swimming and exited the
  lower boundary. For this coherent intercept topology, preserve the evidenced
  carrier-only reserve and, before adding another envelope layer, test whether
  its relief can be conditioned on normalized target-feedback demand so aligned
  approaches retain propulsion while difficult passes retain steering room.
  Falsify this implication if demand conditioning loses capture, weakens the
  terminal wake, slows arrival, or raises joint-limit residence, clamping, or
  loads; if so, revert to unconditioned carrier-only reserve rather than another
  total-command governor.
