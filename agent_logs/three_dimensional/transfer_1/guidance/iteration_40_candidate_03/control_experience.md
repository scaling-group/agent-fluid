...
- Treat the outer-terminal unsupported-bearing qualifier as compatible but not
  yet reliable. The exact speed-reserve baseline remains only `4/6`, with two
  coherent-wake lower exits at `1.464--1.646L`; one such failure enters `3L`
  with `+0.171 rad` target bearing but only `-0.008 rad` course error. Admitting
  only that otherwise unsupported bearing in the `2--4L` annulus subsequently
  captured at `0.74986L` and `18.6560T`, earned the best sampled score
  (`-0.14991`), and preserved both organized wake views; the assigned parent's
  fixed steering transfer captured later at `18.7495T` and scored `-0.15335`
  without an actuator benefit. The qualifier's head/tail action clipping
  (`68.75%/70.70%`), speed-limit residence (`10.29%/11.29%`), and peak
  body-force/moment coefficients (`0.0149/0.0286/0.0165`) remain effectively
  inside the sampled baseline envelope (`68.46%/71.00%`, `10.49%/11.33%`,
  `0.0147/0.0284/0.0162`). This supports an exact-policy repeat before any gain
  change or stacked terminal mechanism; one threshold capture does not establish
  robustness. Apply the qualifier only to the evidenced near-zero course-error,
  nonzero-bearing signature outside the inner intercept gate. Falsify it if a
  repeat exits, changes far-field closure, weakens either wake, loses capture,
  or materially worsens the repeat-backed actuator or load envelope.
...
- Reject both post-pass progress-loss curvature and binary full steering
  retention as repairs for this coherent-wake lower-pass topology. The burst
  redirect requested unavailable `window_closing_speed_L`, silently fell back
  to one-step `closing_speed_L`, reduced head/tail clipping to about
  `53.1%/51.0%`, but only changed its assigned-parent miss from `1.1846L` to
  `1.1608L` before the same lower exit. The subsequent continuous capture-
  corridor certificate did use available target/velocity projection and acted
  before the pass, but by nearly eliminating response-conditioned release
  throughout the terminal region it over-corrected to a `1.6783L` miss and
  `10.2835L` lower exit while both organized wake views and active propulsion
  persisted. In contrast, the current assigned-parent inner-guard policy and
  an exact-byte sampled sibling capture at `0.74953L` and `0.74939L`. Thus the
  projected-miss separation near `2.75L` is a useful qualifier, not permission
  to replace the parent's graded release with an all-or-nothing certificate.
  Preserve its inner intercept veto and carrier; if testing earlier
  compatibility, retain only bounded partial steering in the `4--2.75L`
  annulus and hand off continuously to the inner guard. Falsify that bracketed
  implication if it loses exact-policy capture, retains the lower branch,
  collapses release toward the failed binary controller, or worsens either
  wake, closure, speed residence, clipping, force, or moment beyond the
  repeat-backed envelope.
- Reject a fixed additive outer-annulus LOS-rate residual as a reliable
  replacement for response-conditioned steering release in this coherent-wake
  topology. Exact controller bytes produced inherited threshold captures but
  the assigned repeat then passed below at `1.5962L` and exited at `10.3358L`;
  both visual rows remain organized, closest-pass speed is about `0.828L/T`,
  and head/tail action still clips on `70.25%/71.79%` of rows. The failure
  therefore falsifies tuning the residual's `2 rad/T^2` bound or `4--1.5L`
  annulus as the next repair: its lower speed-limit residence
  (`8.42%/9.37%`) did not buy interception. In contrast, partial retention of
  otherwise released steering only when projected approach is incompatible in
  the `4--2.75L` annulus captured once at `0.74852L` and `18.2985T`, preserved
  both wakes, and stayed inside the exact-baseline clipping, speed, and load
  envelope. Prefer an exact repeat of that compatibility gate before changing
  its bound or stacking another terminal cue. This implication applies only
  after broad route acquisition with an active carrier; falsify it if the
  partial gate loses repeat capture, collapses toward binary full retention,
  weakens either wake, worsens actuator/load metrics, or if independent LOS-
  residual repeats establish reliable capture.
