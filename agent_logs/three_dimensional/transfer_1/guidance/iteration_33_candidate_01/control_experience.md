...
- Distinguish a pre-pass unsupported-bearing discriminator from broad bearing
  feedback or post-pass recovery. Broad bearing recovery/rescue retained
  coherent propulsion but exited below after `1.8004L` and `1.5629L` passes;
  the assigned parent's aft-target/opening-range reacquisition likewise
  engaged only after the route was lost, reached `1.6538L`, and self-propelled
  to a `10.2816L` lower exit with both wake views still organized. By contrast,
  the sampled sibling qualifier admits target bearing only when achieved-
  course error is nearly uninformative, only before the inner intercept gate,
  and produced its first capture at `0.74975L` and `18.3205T`. That capture
  preserved the traveling wake and stayed inside the speed-reserve envelope:
  peak speed/force/moment were `0.9293L/T`, `0.03048`, and `0.01630`, clamp
  fractions were `68.60%/70.61%`, and speed-limit residence was
  `10.57%/11.56%`. This promotes the narrow discriminator to an exact-repeat
  test, not robust success; do not widen it, substitute bearing for achieved
  course, or strengthen post-pass reacquisition. Falsify it on any repeat miss,
  lower exit, wake weakening, far-field change, or actuator/load excursion
  outside the repeat-backed baseline envelope.
