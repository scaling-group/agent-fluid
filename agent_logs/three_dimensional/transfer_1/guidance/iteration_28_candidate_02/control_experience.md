...
- Do not use the speed-reserve baseline's first-`3L` bearing/course discrepancy
  to justify further target-bearing qualification. The baseline remains only
  `3/5`: two exact-byte restorations retained self-propulsion and coherent
  top-down/oblique wakes but exited below after `1.6463L` and `1.4642L` passes.
  The proposed bounded bearing recovery then retained those wakes yet worsened
  closest approach to `1.8004L`; at its first `4L` and `3L` crossings bearing
  and raw course error already had the same positive sign
  (`+0.562/+0.358` and `+0.672/+0.296 rad`), so the residual reinforced an
  existing request rather than recovering a vanished one. Treat that mechanism
  as falsified, not as a threshold-tuning opportunity. Revisit bearing only if
  a different normalized signature predicts repeated branch selection before
  the terminal gate without changing far-field closure, either wake, or the
  speed-reserve load and actuator envelope.
- Treat geometry-gated spatial steering allocation as a mechanism
  demonstration that merits an exact repeat, not as proven robustness or an
  actuator-saturation cure. The first policy to preserve total steering while
  transferring only unsafe-intercept residual from the posterior propulsive
  joint to the anterior joint captured at `0.7492L` and `18.7495T`; its
  top-down alternating street and bilateral oblique Lambda2 structures remain
  coherent, mean speed is about `0.699L/T`, and clipping (`68.8%/70.8%`) and
  speed-limit residence (`10.7%/11.6%`) stay inside the sampled speed-reserve
  range. This is a more useful result than the assigned parent's target-
  bearing recovery and midcourse carrier-reserve gates, which retained active
  wakes but missed below at `1.8004L` and `1.6664L`. For the coherent,
  broadly acquired topology, repeat the spatial allocator unchanged before
  tuning its fraction or stacking another observer, carrier-phase allocator,
  or reserve gate. Falsify the mechanism on any exact-repeat miss, wake
  weakening, miss-side change, or load/actuator excursion; a repeat failure
  should restore the repeat-backed speed-reserve scaffold rather than trigger
  scalar tuning.
