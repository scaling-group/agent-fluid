...
- Treat projected-intercept gating as a useful terminal discriminator, not a
  ...
  transfer also retained propulsion yet missed at `1.6818L`. Sparse
  outward-carrier reserve is now a replicated semantic improvement over that
  near-miss lineage: identical policy bytes captured in three direct-uniform
  rollouts at `0.7466--0.7494L` and `18.205--18.601T`, each with a coherent
  alternating 3D wake. Do not mislabel it an efficiency result, however;
  action still clamped on about `68.5--68.7%/70.7--71.0%` of rows and both
  joints still touched `260 deg/T`. Its scalar-score and arrival spread also
  define a repeat-noise band that small gain changes must exceed. Preserve this
  fixed-allocation carrier/intercept controller as the current capture baseline
  and avoid more route gain, tighter corridors, cadence tuning, global carrier
  suppression, static curvature, half-cycle weights, or phase-lag tuning.
  Instantaneous actuator feasibility is not by itself a safe steering signal:
  reallocating the unchanged residual by carrier headroom lowered full-trace
  clamping but lost capture at `0.9632L` before a lower-boundary exit, and a
  hard terminal speed governor likewise regressed to a `1.3877L` miss and exit.
  Avoid dynamic steering-share redistribution and hard outward-command
  projection on this base. A later terminal yaw/slip residual should keep the
  posterior carrier and fixed shares intact and remain subordinate to the
  intercept-release veto. Falsify that boundary if such a residual loses the
  replicated capture/wake class, raises saturation or loads, or if a later
  allocator or governor captures repeatedly without carrier collapse.
