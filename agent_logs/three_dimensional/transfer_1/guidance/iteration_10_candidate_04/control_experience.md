...
- Require semantic replication before promoting a threshold-gated capture as
  a durable mechanism. A line-of-sight-guarded yaw-response release captured
  once at `0.7493L`, only `0.00065L` inside the radius, but two semantically
  identical inherited replays missed at `1.5891L` and `1.7715L` and retained
  the coherent-wake lower exit. Carrier recovery (`1.1444L`), additive slip
  curvature (`1.0561L`), and closing-speed cadence relief (`1.0959L`) also
  preserved that exit class. Thus a single narrow capture does not justify
  tuning its yaw-response thresholds or the already saturated course gain;
  test a geometrically wider terminal decision such as a normalized
  velocity-to-target collision corridor while preserving the carrier, and
  falsify it on repeatability, terminal cross-track speed, saturation, and
  exit topology rather than minimum distance alone.
