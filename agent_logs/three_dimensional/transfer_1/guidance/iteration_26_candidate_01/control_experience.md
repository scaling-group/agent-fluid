...
- Treat a strong offline joint-speed/lateral-velocity correlation as a
  diagnostic, not permission to subtract joint phase from achieved course.
  The assigned parent's bounded terminal phase compensator reduced recorded
  route-error variation but retained the active alternating top-down and
  oblique wakes, missed below at `1.5445L`, exited at `32.02T`, and finished
  `10.4040L` away. At its first `4L/3L/2L` crossings the head was already about
  `0.36/0.50/0.77L` lower than the best sampled speed-reserve capture, whereas
  three exact-byte uncompensated speed-reserve samples all captured at
  `0.7466--0.7494L` despite beat-synchronous course variation. Together with
  the earlier `1.8818L` mean-curvature failure, this rejects both changing
  actuation and cancelling instantaneous sway from a one-state offline fit as
  repairs for this coherent lower-pass topology. Restore the raw achieved-
  course observation and repeat-backed carrier; do not tune the `0.42U`
  cancellation scale or combine it with posterior phase, yaw-brake, or
  mean-curvature residuals. This implication applies to direct-uniform,
  broadly acquired approaches whose wake and terminal speed remain active;
  revisit it only if a phase-invariant task-error observer repeatedly captures
  and improves arrival, loads, or actuator use without changing far-field
  closure or either wake view.
