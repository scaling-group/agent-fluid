...
- Treat normalized signed projected miss as a terminal steering observation,
  not yet a scalar improvement. Blending the repeat-supported achieved-course
  request toward body-frame `target x velocity / speed` only inside `2L`
  retained the alternating 3D wake and captured at `0.7477L` and `18.5405T`
  with `0.8592L/T` crossing speed and baseline-like loads. However, its score
  (`-0.16291`), mean distance (`2.05046L`), clamp fractions
  (`68.6%/71.0%`), and speed-limit residence did not improve over the three
  parent captures. Replay this exact policy before changing its scale or blend
  distances; reject the primitive if replay loses capture, grows projected
  miss after activation, weakens propulsion, or raises loads.
