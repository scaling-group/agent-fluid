...
- When achieved-yaw feedback releases steering during a fast close pass, gate
  that release with inertial line-of-sight rate rather than body yaw alone.
  The inherited response-release policy kept a coherent carrier but missed at
  `0.9312L` while target-normal motion still rotated the line of sight away
  from capture; deriving LOS rate from body-frame target and velocity and
  restoring the existing course-steering acceleration only during a growing
  signed miss converted that topology into the sampled `0.7493L` capture at
  `18.6065T`. Both visual rows retain an alternating wake through crossing,
  so the improvement supports response-conditioned authority rather than
  carrier suppression, additive slip curvature, or more route gain. Apply
  this only after broad course acquisition with a coherent traveling bend,
  and do not infer robustness from the single tight success: head/tail action
  is clamped on about `68.8%/71.0%` of trace rows and both joints touch the
  speed limit. Falsify the lesson if LOS guarding loses capture, changes early
  closure, weakens terminal wake production, or worsens saturation or loads
  under repeated or held-out conditions.
