...
- Use terminal progress sign as a response discriminator, not as evidence that
  recovery has already been solved. Across the four current direct-uniform
  captures—including two exact speed-reserve runs, anterior transfer, and the
  posterior pulse—the inherited eight-row closing-speed analysis stays
  positive below `4L`, with per-run minima of `0.202--0.644L/T`; all four
  combined sheets retain a coherent alternating top-down street and bilateral
  oblique structures through capture. By contrast, the assigned parent's
  exact-byte lower exits necessarily reverse closing after closest passes of
  `1.464--1.646L`, and the sampled optimizer logs add lower exits at
  `1.240--1.411L`. Preserve the first-pass carrier, course error, steering
  ceiling, and allocation; test a bounded progress-loss veto on steering
  release that is exactly inactive while closure remains positive, rather
  than another bearing, cadence, phase, or allocation residual. This is an
  unvalidated recovery hypothesis, not a same-worker improvement. Falsify the
  discriminator if a capture approach crosses its loss gate, or falsify the
  mechanism if it changes first-pass closure, loses capture, retains the lower
  exit, creates static curvature/coasting, weakens either wake, or worsens the
  established actuator/load envelope.
