...
- Treat a scalar-only parent capture as semantic corroboration, not mechanism
  attribution. The assigned parent records a `0.7482L` capture, but its
  inherited log contains no policy bytes, trace, load history, or wake sheet;
  it therefore cannot overturn the complete replay evidence. In the sampled
  evidence, the three exact-byte speed-reserve runs all capture at
  `0.7466--0.7494L` with coherent wakes and nearly unchanged clipping, even
  though their final logged LOS-versus-inertial-course residual spans about
  `0.20--1.34 rad`; the distinct posterior pulse also captures once in the
  sample but its inherited exact replay misses at `1.2589L` and exits below.
  This combination supports preserving the repeat-backed carrier and
  allocation while treating terminal path phase as unresolved: test only a
  phase-invariant, task-error-conditioned release mechanism, and require
  complete repeated diagnostics before crediting it. Avoid short-window
  body-bearing or joint-phase triggers here because they can turn beat motion
  into a false convergence signal. Revisit this boundary only if complete
  artifacts identify a distinct parent mechanism and exact repeats improve
  capture, arrival, loads, or actuator use without weakening either wake view.
