...
- Do not optimize terminal control allocation for clamp fraction alone. The
  ...
  when joint speed, previous action, and carrier direction agree, now has four
  exact-byte sampled or inherited captures at `0.7466--0.7499L` and
  `18.205--18.673T` while
...
- Do not treat an opening close pass as evidence that another same-sign mean-
  curvature command can recover this trajectory. A terminal average-joint
  curvature servo acted before the pass yet exited below after only
  `1.8818L`, while the assigned parent's distance/closing/error-gated burst
  acted after reopening and exited below after `1.7572L`; the latter is no
  better than the inherited exact-baseline lower exits around
  `1.64--1.73L`. Both interventions kept self-propelling and retained coherent
  alternating top-down and oblique wakes, so their failure is steering
  topology rather than carrier collapse or insufficient scalar route gain.
  In this repeat-variable terminal regime, reject more pre- or post-pass
  additive curvature and instead test whether a new normalized observation
  can expose persistent drift before steering saturates while preserving the
  evaluated carrier/allocation. Falsify this negative lesson only if repeated
  geometry-gated curvature recovery captures without changing far-field
  closure, wake coherence, loads, or actuator use.
