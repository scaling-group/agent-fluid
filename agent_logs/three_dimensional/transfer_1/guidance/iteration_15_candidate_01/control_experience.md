...
- Treat projected-intercept gating as a useful terminal discriminator, not a
  ...
  `70.0%/72.2%` clamping. For this coherent, broadly acquired topology, retain
  the intercept veto provisionally but change how bounded steering is realized
  within the beat; avoid tighter corridor thresholds, more route gain,
  carrier suppression, or another static-curvature edit. Falsify this lesson
  if exact repeats establish robust LOS-only capture, intercept gating fails
  to reproduce its pass improvement, or a within-beat realization weakens the
  wake, worsens clearance, or raises saturation and loads.
- Treat sparse outward-carrier reserve as a supported trajectory mechanism,
  but not yet as an actuator-saturation cure. Added to the projected-intercept
  controller, it converted the inherited `1.0509L` coherent lower pass into a
  `0.7480L` capture at `18.205T` while retaining the alternating top-down and
  oblique wake and comparable load peaks. However, returned acceleration still
  clamped on about `68.6%/71.0%` of rows and exact joint-speed-limit residence
  rose to about `10.6%/11.3%` from `8.6%/7.9%`; joint 2 arrived at the
  `260 deg/T` limit with outward command remaining. Preserve the sparse
  reserve provisionally, but do not deepen distance-only or carrier-only
  attenuation as if the envelope problem were solved; next test a smooth
  terminal feasibility projection on the combined carrier-plus-steering
  command that passes restoring motion unchanged. This applies only to the
  coherent, broadly acquired intercept topology. Falsify it if capture does
  not repeat, the pass improvement disappears, total-command projection
  weakens the wake or raises loads, or lower saturation is obtained by
  coasting rather than maintained propulsion.
- Treat normalized signed projected miss as a provisional terminal steering
  observation, not an actuator-envelope remedy. Blending the repeat-supported
  speed-reserve course request toward signed `target_body x velocity_body`
  only inside `2L` preserved the coherent alternating wake and produced a
  `0.7477L` capture at `18.5405T`, so it can select interception side without
  a world route or carrier suppression. However, relative to three unblended
  captures at `18.2050--18.6010T`, its first run had a worse score
  (`-0.16291`), comparable action clamping (`68.5%/71.0%`), and comparable
  exact speed-limit residence (`10.6%/11.4%`); it does not justify more
  steering gain or another envelope layer. In the coherent, broadly acquired
  topology, require an exact repeat before combining this observation with a
  new terminal mechanism. Falsify it if the repeat loses capture, grows
  projected miss after the blend activates, weakens wake production, raises
  loads or envelope residence, or restores the lower-domain exit; then prefer
  the three-repeat speed-reserve controller while seeking an observation that
  distinguishes the divergent terminal approaches.
