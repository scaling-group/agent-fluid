...
- Reject phase-compensated terminal yaw damping in this topology, rather than
  tuning its scalar or geometry gates. Distance-only damping retained an
  active wake but replaced the repeat-backed speed-reserve captures with a
  lower exit after a `1.4601L` pass. Projected-miss and approach-alignment
  gating then produced one `0.7486L` crossing, but the assigned parent's exact
  repeat missed at `1.5097L` and exited below at `10.4372L`; both gated runs
  retained coherent propulsion and comparable force/moment and actuator
  envelopes. Thus gating prevented indiscriminate damping but did not make the
  mechanism reliable, and the failure is terminal path response rather than
  carrier collapse or instability. Preserve the speed-reserve baseline and
  test a different steering realization. Revisit yaw damping only if multiple
  independent exact repeats capture and provide a path, arrival, load, or
  saturation benefit over the baseline without weakening either wake view.
- Treat posterior wave-shape steering as repeat-worthy but unestablished. A
  small clock-free posterior bias, activated only by joint-state phase and the
  existing intercept geometry, preserved one `0.7492L` capture, coherent
  top-down and oblique wakes, baseline-like action clipping
  (`68.46%/70.94%`), speed-limit residence (`10.81%/11.58%`), and force/moment
  peaks. This supports an exact-policy repeat before any gain or gate change,
  not a claim of superiority over the three sampled baseline captures. Reject
  the addition if a repeat misses, changes far-field closure, weakens the
  traveling bend, or exceeds the baseline actuator/load envelope; continue it
  only if repeated captures develop a terminal-path, arrival, load, or
  saturation benefit.
