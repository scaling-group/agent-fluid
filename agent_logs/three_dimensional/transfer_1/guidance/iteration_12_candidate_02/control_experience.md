...
- Treat projected-intercept gating as useful state discrimination but not as a
  complete steering realization. The same LOS-guarded bytes first crossed at
  `0.7493L` and then repeated as a `1.7715L` lower exit; adding normalized
  target/velocity projected-pass and approach gates sharpened the completed
  repeat-failure pass to `1.0509L` at `0.8656L/T` while preserving the
  alternating wake, but still exited low and clamped returned acceleration on
  about `70.4%/69.7%` of rows. Joint-state half-cycle weighting on that guard
  then regressed to `1.7456L` at `0.7858L/T`, and a separate posterior
  phase-lag transfer reached only `1.6818L` at `0.7863L/T`; both retained the
  lower exit, touched the joint-speed limit, and left roughly `70--72%`
  acceleration clamping. Preserve the intercept observation, but avoid more
  phase estimators, lag modulation, or scalar steering multipliers in this
  saturated regime. Test whether saturation-aware control allocation can
  expose the existing turn residual without changing the far-field carrier.
  Falsify that direction if it weakens the terminal wake, worsens the
  `1.0509L` pass, retains the same topology without a meaningfully different
  trajectory, or increases joint-speed saturation and hydrodynamic loads; a
  new threshold crossing still requires an exact-policy repeat.
