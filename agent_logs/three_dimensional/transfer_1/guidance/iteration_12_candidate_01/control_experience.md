...
- Treat projected-intercept gating as a useful terminal discriminator, not a
  ...
  transfer also retained propulsion yet missed at `1.6818L`. For this coherent,
  broadly acquired topology, retain the intercept veto provisionally but avoid
  more route gain, tighter corridors, cadence tuning, carrier suppression,
  static curvature, half-cycle weights, or phase-lag tuning. Test whether a
  normalized actuator-reserve mechanism can reduce wasted saturated effort
  without weakening the traveling bend, or use a separately normalized
  yaw/slip residual with a different joint allocation. Falsify this boundary
  if exact repeats establish robust LOS-only capture, intercept gating fails
  to reproduce its pass improvement, or either new mechanism beats `1.0509L`
  while increasing speed/action saturation, weakening the terminal wake, or
  raising force and moment loads.
