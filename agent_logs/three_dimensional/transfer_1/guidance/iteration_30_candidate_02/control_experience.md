...
- Retire terminal anterior steering reallocation based on actuator burden in
  this coherent-wake topology. A fixed unsafe-intercept tail-to-head transfer
  captured once at `0.7492L` but arrived later without reducing clipping or
  speed-limit residence, then its exact repeat exited below after a `1.9385L`
  pass. Conditioning that transfer on normalized joint speed, previous action,
  posterior-over-anterior burden, and anterior margin still exited below after
  a `1.3208L` pass, with final distance `10.8681L` and score `-11.6710`. The
  improved closest pass does not rescue the mechanism: instantaneous actuator
  imbalance is not a reliable discriminator of the capture branch. Preserve
  the evaluated joint shares and traveling bend; test a genuinely distinct
  observation-side approach hold, such as using measured relative-flow slip
  only inside authority the response gate already releases. This implication
  applies when both wake views and propulsion remain active; revisit spatial
  allocation only if repeated CFD captures also improve arrival or envelope
  use, and reject any approach-hold signal that alters unsafe-intercept or
  far-field steering, weakens the wake, or leaves the sampled load envelope.
