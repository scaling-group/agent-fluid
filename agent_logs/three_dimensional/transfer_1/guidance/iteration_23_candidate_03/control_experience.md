...
- Do not infer robust capture from coherent wakes, overlapping actuator/load
  summaries, or even three exact-byte successes. The conditional speed-reserve
  policy captured three times at `0.7466--0.7494L` and `18.205--18.601T`, but
  the assigned parent's exact replay reached only `1.7276L` and exited below at
  `10.2037L` while still self-propelling with organized top-down and oblique
  wakes. Through closest pass, its clamp (`68.42%/70.91%`), speed-residence
  (`10.05%/11.13%`), peak force (`0.03046`), and peak moment (`0.01571`) all
  overlap the captures; geometry separates the result earlier. At about
  `4.27L` the failed path already has projected miss `1.370L`, versus
  `0.544--0.665L` in two sampled captures, and by `2.740L` its
  target/velocity alignment is `0.597`, versus `0.822--0.859` near comparable
  capture ranges. Sparse carrier reserve remains a useful propulsion scaffold,
  but unchanged replay and clamp-only optimization are both falsified as next
  steps. Preserve the traveling bend and test a bounded steering realization
  conditioned on this normalized alignment/course deterioration; avoid the
  already failed total-command governor, which lowered speed residence but
  missed at `1.3877L`. This applies after broad acquisition with an active
  wake; falsify it if further exact baseline repeats restore reliable capture,
  or if geometry-conditioned steering changes far-field closure, weakens the
  wake, or raises saturation and loads without improving the miss topology.
...
- Reject normalized signed projected miss as a replacement terminal steering
  ...
  contrasts with the then-current `3/3` captures at `0.7466--0.7494L` for the
  achieved-course, intercept-guarded speed-reserve baseline; a later baseline
  miss lowers that record to `3/4` but does not rehabilitate projected miss's
  `1/3` result. The failure is terminal path geometry rather than insufficient
  propulsion. Preserve the achieved-course error and traveling bend; do not
  stack yaw/slip, capture-
...
- Reject the phase-synchronized posterior wave-shape pulse as a robust
  ...
  Do not re-add the posterior residual or stack it with the separately
  falsified yaw brake. The subsequent exact speed-reserve rollback itself
  missed at `1.7276L`, so preserve that controller only as the stronger
  propulsion-and-routing scaffold, not as an unchanged candidate or robust
  capture claim. This conclusion applies to the evaluated small tail bias
  inside the existing intercept gate; revisit wave-shape steering only as a
  materially different observation-to-curvature mechanism, and test any new
  realization against both the captured and failed baseline geometries.
