...
- Do not optimize terminal control allocation for clamp fraction alone. The
  ...
  exact-byte sampled captures at `0.7466--0.7499L` and `18.205--18.672T` while
...
- Treat a small intercept-gated posterior wave-shape residual as
  repeat-compatible steering, not as established performance improvement.
  Three exact-policy direct-uniform evaluations captured at
  `0.7480--0.7492L` in `18.1995--18.5680T`; their top-down and oblique sheets
  retain the speed-reserve baseline's active alternating wake, and action
  clipping (`68.2--68.5%/70.8--70.9%`), speed-limit residence
  (`10.4--10.6%/11.3--11.5%`), and force/moment peaks remain inside the
  sampled baseline envelope. Normalized anterior-joint phase can therefore
  realize a subordinate posterior steering perturbation without destroying
  this traveling bend. The three terminal crossings have projected miss
  `0.33--0.64L` and target/velocity alignment `0.51--0.90`, avoiding the most
  tangential of two sampled baseline crossings (`0.73L`, `0.23`), but their
  scores and mean distances overlap or trail baseline variation. Preserve the
  evaluated gain and intercept gate for interpretable repeats; do not tune
  either scalar from capture alone. This applies only to the coherent,
  broadly acquired speed-reserve topology. Falsify the mechanism if a repeat
  misses, weakens either wake view, exceeds the actuator/load envelope, or a
  larger repeat set shows no terminal-path or secondary-metric separation;
  then restore the exact baseline instead of escalating the bias.
- Treat geometry gating as a necessary safety boundary, but not yet an
  established benefit, for terminal yaw damping. Distance-only damping of
  phase-compensated yaw retained an active wake yet replaced the four-repeat
  speed-reserve capture topology with a lower exit after a `1.4601L` pass. A
  residual using the same yaw observation only when projected miss and
  target/velocity alignment both indicated an approaching capture corridor
  did preserve one capture at `0.7486L` and `18.381T` with a coherent wake,
  but its action clipping (`68.8%/70.8%`) and speed-limit residence
  (`10.4%/11.6%`) did not improve on the baseline envelope. In this topology,
  do not widen the yaw-damping region or tune its scalar from the single
  crossing; first repeat the exact geometry-gated policy and require reliable
  capture plus a terminal-path, arrival, load, or saturation benefit. Falsify
  the safety boundary if an independently repeated distance-only damper
  captures without degrading the wake or actuator/load metrics, and reject
  the gated mechanism if exact repeats miss or remain indistinguishable from
  the baseline.
