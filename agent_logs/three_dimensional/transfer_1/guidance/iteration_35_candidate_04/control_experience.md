...
- Do not call the exact speed-reserve baseline robust from the four current
  `0.7466--0.7495L` captures alone. Two inherited exact-byte restorations also
  ...
  `4/6`. At the first `3L` crossing, current captures have body-frame target
  ...
  joint speed versus about `+0.94` for course error. A bounded
  unsupported-bearing qualifier has now captured on both exact-byte trials at
  `0.74975L/18.3205T` and `0.74986L/18.6560T`, preserving the active top-down
  street and bilateral oblique wake. On the sampled repeat, head/tail clipping
  is `68.75%/70.70%`, speed-limit residence is `10.32%/11.29%`, and peak
  planar force/yaw moment is `0.03143/0.01648`, all within the exact-baseline
  envelope. This upgrades admitting target bearing only when outer-terminal
  course error is near zero from isolated compatibility to repeat-backed
  evidence, while the fixed steering-transfer sample is slower and offers no
  actuator benefit. Preserve the qualifier bytes, raw achieved course,
  traveling bend, and inner intercept guard for a third independent trial;
  do not scalar-tune or stack another residual yet. Treat `2/2` as promising,
  not robust, because the underlying exact baseline has branched across
  repeats. Falsify the qualifier as a reliable repair if another exact run
  retains the lower branch, weakens either wake, or moves clipping, speed
  residence, force, or moment outside the repeat-backed envelope.
...
- Do not confuse detection of a missed pass with authority to recover it. The
  four sampled direct-uniform captures keep eight-row closing speed above
  `0.202--0.644L/T` below `4L`, so progress loss does separate those recorded
  approaches from a pass that has begun opening. But the assigned parent's
  closing-speed-triggered redirect, which replaced additive steering with a
  same-signed two-joint mean-curvature servo while retaining the carrier,
  reached only `1.2402L` and then exited below at `10.1095L`. The run remained
  stable and active at about `0.775L/T` near termination, so this is incorrect
  recovery geometry rather than stall or loss of propulsion. Avoid tuning the
  closing threshold, curvature gain, or damping of this redirect; restore the
  repeat-backed first-pass controller and test a distinct pre-pass target-
  geometry mechanism. This negative lesson applies after broad acquisition in
  a coherent wake; revisit recovery only if a different bounded primitive
  demonstrates an actual second approach without losing repeated capture,
  wake coherence, or the actuator/load envelope.
