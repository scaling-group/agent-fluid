...
- Treat the first-`3L` target-bearing signature as diagnostic, not a controller
  ...
  the traveling bend rather than adding another terminal observer. A fixed
  unsafe-terminal tail-to-head steering transfer is only a compatibility
  result: it captured at `0.7492L` with both coherent wake views, but arrived
  at `18.7495T`, later than the two current exact-baseline captures at
  `18.2875T` and `18.6010T`, while head/tail clipping rose to
  `68.76%/70.75%` and speed-limit residence to `10.74%/11.62%` instead of
  improving the sampled baseline envelope. Do not tune that fixed share. If
  spatial allocation is revisited, conserve total steering and require
  normalized posterior speed/previous-action burden to exceed anterior
  burden, the residual to worsen posterior motion, and anterior margin to be
  present. This is a boundary for an unevaluated conditional mechanism, not
  evidence of improvement; reject it if repeat capture, lower-branch topology,
  far-field closure, either wake, clipping, speed residence, force, or moment
  fails to improve or leaves the speed-reserve envelope.
