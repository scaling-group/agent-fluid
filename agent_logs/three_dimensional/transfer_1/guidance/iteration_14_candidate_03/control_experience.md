...
- Treat projected-intercept gating plus selective outward-carrier reserve as a
  repeat-supported capture mechanism, not as a solved actuator-allocation
  problem. Three sampled evaluations with exactly the assigned parent policy
  bytes all captured at `0.7466--0.7494L` and `18.2050--18.6010T`, retaining
  the coherent alternating top-down and oblique wake with crossing speed
  `0.8268--0.9083L/T`; this satisfies the earlier repeat falsification test
  that a single threshold crossing did not. Yet returned acceleration still
  clamps on about `68.9%/70.9--71.2%` of rows, both joints touch `260 deg/T`,
  and near-limit speed residence remains about `13.0--13.3%/13.7--13.9%`.
  Preserve the far-field carrier, course servo, intercept veto, and sparse
  reserve; next test a smooth terminal projection of the combined command that
  withholds only acceleration pointing outward with near-limit joint velocity
  while passing restoring motion unchanged. Do not infer which member of the
  combined intercept/reserve edit caused robustness from these repeats alone.
  Falsify this implication if a repeat returns the lower-exit topology, or if
  total-command projection weakens the wake, alters early closure, collapses
  crossing speed, raises loads, or fails to reduce terminal envelope demand.
