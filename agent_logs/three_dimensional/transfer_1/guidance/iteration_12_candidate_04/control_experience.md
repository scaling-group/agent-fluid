...
- Separate route sensing from steering realization when a free swimmer's body
  ...
  how that request is realized. Two later phase-selective realizations now
  close that alternative: joint-state steering weighting reached `1.7456L`
  and posterior lag transfer reached `1.6818L`; both retained substantial
  alternating wakes but repeated the lower exit. Preserve the normalized
  achieved-course outer signal, but avoid another yaw-rate cascade,
  collision-corridor threshold, scalar course-gain edit, or joint-phase
  steering/lag allocation unless it changes the first pass or termination
  class. The next distinct test should use a separately normalized measured
  yaw/slip or hydrodynamic-load disagreement only as a bounded response veto,
  not as a route command. Falsify it if it cannot beat the `0.9532L`
  carrier-aligned pass while retaining the alternating wake and early route,
  or if it increases action saturation or loads.
