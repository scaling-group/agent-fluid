...
- Treat bounded outer-terminal LOS-rate interception as repeat-worthy, not yet
  robust or gain-tunable. The inherited controller opposed body-frame
  target/velocity LOS rotation only while approaching between `4L` and `1.5L`
  and otherwise preserved the speed-reserve carrier, route servo, response
  release, and steering shares. Its first completed CFD rollout captured at
  `0.7461L` and `18.2985T` with the organized alternating top-down street and
  bilateral oblique wake intact. Head/tail action clipping
  (`68.50%/70.51%`), speed-limit residence (`10.58%/11.66%`), and peak
  body-force/moment coefficients (`0.01475/0.02916/0.01593`) stayed effectively
  inside the three sampled exact speed-reserve captures, but its score
  (`-0.15938`) did not improve their `-0.15733--0.15140` range and one threshold
  crossing cannot establish reliability against the baseline's known lower
  exits. Repeat the exact policy before changing its bound or annulus; do not
  stack it with bearing, recovery, phase, or allocation mechanisms. Apply this
  only to a coherent, broadly acquired, still-approaching trajectory and
  falsify it if an exact repeat loses capture, retains a lower exit, weakens
  either wake, changes far-field closure, or materially leaves the established
  actuator/load envelope.
- Separate successful first-pass acquisition from post-pass recovery using a
  response observation, not another route or allocation scalar. Across the
  four current direct-uniform captures, eight-row history-window closing speed
  stays positive below `4L`, with per-run minima of
  `0.202--0.644L/T`; the exact speed-reserve captures retain the active
  top-down/oblique traveling wake yet the inherited failures necessarily cross
  zero after their `1.464--1.646L` closest passes before exiting below. This
  supports testing a bounded progress-loss redirect that is exactly inactive
  on recorded capture approaches and releases when positive closure resumes,
  rather than altering the repeat-backed first approach. It does not overturn
  the prior failure of mean-curvature tracking during approach and does not
  establish recovery as successful before CFD evidence. Apply it only after
  broad acquisition inside the terminal region; falsify the discriminator if
  a future capture has closing speed below the gate, or falsify the recovery
  mechanism if it loses repeat capture, retains the lower exit, creates static
  curvature or coasting, weakens either wake, or worsens the actuator/load
  envelope.
