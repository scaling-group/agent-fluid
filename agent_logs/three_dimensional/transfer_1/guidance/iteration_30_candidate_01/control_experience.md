...
- Treat unconditional spatial steering transfer as a compatibility result, not
  an actuator-allocation improvement. The assigned parent's fixed unsafe-
  intercept tail-to-head transfer preserved the alternating top-down street
  and bilateral oblique wake and captured at `0.7492L`, but arrived at
  `18.7495T`, later than all three sampled exact speed-reserve captures at
  `18.2875--18.6010T`. Its head/tail clipping (`68.76%/70.75%`) and speed-limit
  residence (`10.74%/11.62%`) were marginally higher, not lower, than the three
  baseline ranges (`68.45--68.66%/70.62--70.67%` and
  `10.41--10.63%/11.30--11.56%`), while speed, force, moment, score, and mean
  distance remained inside the same envelope. Inherited bounded-bearing and
  response-veto rollouts also retained active `0.84--0.90L/T` propulsion but
  exited below after `1.8004L` and `1.3725L` passes, so preserve raw achieved
  course, the response/intercept scaffold, and the traveling bend rather than
  stacking another terminal observer or tuning the fixed share. If spatial
  allocation is revisited, conserve total steering and condition transfer on
  normalized outward previous-action/current-speed burden: posterior burden
  must exceed anterior burden, the residual must worsen posterior motion, and
  the anterior must have margin. This is a boundary on future tests, not
  evidence for the current unevaluated conditional allocator; reject it if
  repeated capture or useful envelope use does not improve, the lower branch
  remains, or far-field closure, either wake, speed, clipping, force, or moment
  leaves the sampled speed-reserve envelope.
