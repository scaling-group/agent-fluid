...
- Do not call the exact speed-reserve baseline robust because the current
  sampled sheets all capture. The assigned parent's new exact-byte rollback
  retains the same self-propelled alternating top-down street and bilateral
  oblique wake yet passes below at `1.4107L` and exits, moving the accumulated
  exact-policy record from `4/6` to `4/7`. This confirms that coherent thrust
  and unchanged bytes do not resolve the terminal branch sensitivity. Exact
  replay is now a rollback control, not a next mechanism: a useful repair
  should be inactive on the sampled closing first approaches and demonstrate
  either repeatable capture or a propelled recovery trajectory after a miss.
  Falsify that implication if exact repeats establish robust capture, or if a
  gated repair changes far-field closure, loses the sampled capture class,
  weakens either wake, or leaves the speed-reserve load/actuator envelope.
