...
- Treat line-of-sight-guarded steering release as a useful mechanism signal,
  not a robust incumbent, until exact-policy replay agrees. It converted an
  inherited `0.9312L` response-release miss into one sampled `0.7493L`
  capture at `18.6065T`, with an alternating wake through crossing, but the
  assigned parent replayed the same policy bytes with matching case, geometry,
  and IBM hashes and instead missed at `1.7715L` before the same lower exit.
  Both runs retained the carrier and about `0.82--0.85L/T` terminal speed;
  acceleration was clamped on roughly `69--73%` of rows and both joints hit
  the speed limit, so neither more route gain nor carrier suppression addresses
  the reproducibility failure. After broad course acquisition, require release
  compatibility with normalized body-frame interception geometry: positive
  target/velocity alignment and a projected closest-pass corridor can retain
  existing steering on a predicted miss without adding authority. Parent trace
  projection is discriminative but not closed-loop proof: below `2L`, repeat-
  failure release rows projected passes of at least `1.27L`, while most release
  retained by the capture was already capture-compatible. Falsify this lesson
  if an intercept guard changes far-field closure, weakens the terminal wake,
  fails to improve the `1.7715L` replay baseline or lower-exit class, oversteers
  an already compatible pass, or increases saturation or loads; require another
  exact replay before calling any new tight capture robust.
