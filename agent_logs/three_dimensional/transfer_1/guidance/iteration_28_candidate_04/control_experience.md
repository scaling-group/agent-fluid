...
- Do not use offline demodulation quality, a persistent bearing supplement, or
  a wider closest-pass release veto as another terminal-observer tuning loop.
  After the `1.8818L` mean-curvature failure, projecting the joint-speed-
  correlated component out of achieved course reduced recorded signal
  variation but still missed below at `1.5445L`; the next bounded target-
  bearing recovery missed at `1.8004L`. A full-band geometry veto then kept
  response-based steering active until raw target/velocity projection was
  capture-compatible and improved closest approach to `1.3725L`, yet it also
  exited below. All three retained coherent top-down and oblique wakes and
  active roughly `0.84--0.90L/T` terminal propulsion, as did sampled baseline
  captures, so the surviving problem is not carrier loss and none of these
  observation/release mechanisms warrants coefficient or threshold tuning.
  In this coherent, broadly acquired topology, preserve the route and release
  scaffold and test a distinct spatial steering realization that protects the
  posterior traveling-wave actuator; posterior action clips slightly more
  often than anterior action while the evaluated residual is tail-dominant.
  Falsify that allocation implication if it changes motion outside its unsafe-
  intercept gate, weakens either wake view, flips the miss side, or moves
  capture, speed, clipping, force, or moment outside the speed-reserve
  envelope.
- Do not promote a fixed unsafe-terminal anterior steering transfer from one
  threshold capture. The assigned-parent transfer preserved the alternating
  top-down street and bilateral oblique wake and captured at `0.7492L`, but it
  arrived later (`18.7495T`) than both current exact speed-reserve samples
  (`18.2050T` and `18.6010T`) and did not improve its proposed actuator
  mechanism: head/tail clipping remained `68.82%/70.81%`, speed-limit
  residence remained `11.12%/11.85%`, and force and moment stayed inside
  rather than bettered the baseline envelope. Together with inherited
  half-cycle and total-command allocation failures, this rejects increasing
  or scalar-tuning an unconditional share transfer. A recorded-trace replay
  also found no sampled row where same-direction tail steering itself was the
  clipped component, so total clamp fraction cannot justify a clipping-
  recovery allocator. In the coherent, broadly-acquired terminal topology, a
  narrower allocation test should condition the parent's already-safe spatial
  transfer on normalized previous-action and current-speed burden, act only
  while the tail is more burdened and the head has envelope margin, and keep
  the carrier and total steering share invariant. Falsify that implication if
  exact repeats do not improve arrival reliability or useful actuation, if the
  lower miss remains, or if wake, speed, clipping, force, or moment leaves the
  evaluated speed-reserve envelope.
