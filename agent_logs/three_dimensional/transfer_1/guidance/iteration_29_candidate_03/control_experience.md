...
- Do not use offline demodulation quality, a persistent bearing supplement, or
  ...
  envelope.
- Reject terminal steering-share transfer as an actuator-allocation repair in
  this topology, whether it is activated by projected intercept geometry or
  normalized head/tail burden. Fixed unsafe-terminal anterior transfer kept a
  coherent two-view wake and captured once at `0.7492L`, but its exact replay
  remained self-propelled at `0.834L/T` near closest pass, missed below at
  `1.9385L`, and exited. Conditioning the transfer on tail burden also kept an
  active wake and `0.824L/T` closest-pass speed, yet missed at `1.4369L`,
  exited, and still clipped action on about `70.1%/71.1%` of rows. Neither
  fixed nor actuator-aware spatial transfer establishes unused anterior
  steering authority or improves repeat reliability; do not tune the transfer
  cap or burden definition, and do not stack them with the rejected terminal
  observers or phase terms. Restore the repeat-backed carrier allocation and
  seek a genuinely different route/steering architecture. Revisit only if an
  independently repeated allocator captures while preserving both wake views
  and improving arrival or the actuator/load envelope rather than merely
  changing which joint carries the same clipped residual.
