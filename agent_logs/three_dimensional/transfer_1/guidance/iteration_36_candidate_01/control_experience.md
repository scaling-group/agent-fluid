...
- Do not call the exact speed-reserve baseline robust from its four current
  `0.7466--0.7495L` captures alone. Three exact-byte restorations also retain
  self-propulsion and coherent top-down/oblique wakes but exit below after
  closest passes of `1.6463L`, `1.4642L`, and `1.4107L`, making the available
  record `4/7`. At the first `3L` crossing, captures have body-frame target
  bearings `-0.076-- -0.195 rad` with same-sign course errors, while one
  restored failure has `+0.171 rad` bearing but only `-0.008 rad` course error;
  its projected miss opens to `1.95L` by `2.75L`. In the sampled `2.5--4L`
  annulus, target bearing correlates only about `-0.24-- -0.27` with anterior
  joint speed versus about `+0.94` for course error. A bounded qualifier that
  admits target bearing only in this outer-terminal disagreement state has now
  captured three times with exact bytes at `0.74881--0.74986L` and
  `18.3205--18.7440T`. The assigned parent's newly completed third rollout is
  direct-uniform and retains the traveling top-down and bilateral oblique
  wakes through capture, `0.8663L/T` terminal speed, `68.69%/70.80%` action
  clipping, `10.36%/11.27%` speed-limit residence, and peak force/moment of
  `0.03147/0.01643`, within the sampled carrier envelope. This makes raw
  achieved course plus the narrow unsupported-bearing qualifier repeat-backed
  for the fixed initial condition; it is not permission to widen or scalar-
  tune its gates, nor evidence of held-out robustness. Preserve the carrier
  and exact qualifier while testing reliability. Falsify the mechanism if an
  exact repeat changes motion outside its gate, retains the lower branch,
  weakens either wake, loses capture, or leaves the repeat-backed actuator/load
  envelope.
...
- Reject the burden-conditioned spatial allocator rather than widening or
  gain-tuning it. Its required exact repeat preserved the active alternating
  ...
  and preserve its posterior traveling bend; do not stack projected-miss,
  observer, phase, or actuator-state residuals onto this topology. The narrow
  unsupported-bearing result above does not rehabilitate spatial transfer.
  Revisit allocation only if a different normalized state
...
- Do not confuse response-based release or lower actuator burden with a useful
  second-approach mechanism. The assigned parent's closing-speed-triggered,
  response-released mean-curvature bend remained stable and self-propelled,
  improved closest approach over the immediately preceding baseline failure
  from `1.4107L` to `1.1790L`, and reduced head/tail action clipping from about
  `70.2%/71.1%` to `45.3%/46.2%`; nevertheless it continued below the target,
  never made a second approach, and exited at `10.2248L` final distance. Its
  post-`20T` mean absolute joint-speed sum also fell from about `5.16` to
  `2.26 rad/T`, consistent with the visibly diminished late alternating wake.
  Avoid tuning the progress threshold, recovery curvature, damping, or release
  of this same-signed two-joint redirect: reduced clipping after a missed pass
  is not a routing benefit. This applies to coherent broad acquisition followed
  by the evidenced lower pass; revisit recovery only if a distinct normalized
  geometry primitive demonstrates an actual propelled second approach without
  sacrificing repeat capture, either wake view, or the actuator/load envelope.
