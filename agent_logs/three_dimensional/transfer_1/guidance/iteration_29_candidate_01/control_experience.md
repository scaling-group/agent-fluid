...
- Do not continue terminal-observer tuning or unconditional spatial steering
  transfer after the repeat-variable speed-reserve result. The exact baseline
  has both threshold captures and coherent-wake lower exits; the subsequently
  tested bounded target-bearing recovery and wider closest-pass response veto
  also retained active roughly `0.84--0.90L/T` propulsion but exited below
  after `1.8004L` and `1.3725L` passes. A fixed unsafe-intercept transfer of
  steering from tail to head then preserved the alternating top-down street
  and bilateral oblique wake and captured at `0.7492L`, establishing
  compatibility, but arrived later (`18.7495T`) than both sampled exact
  baseline captures (`18.2875T` and `18.6010T`). It did not improve its stated
  actuator objective: head/tail clipping remained about `68.8%/70.8%` and
  speed-limit residence about `10.7%/11.6%`, marginally above rather than below
  the baseline samples, while force and moment stayed inside the same envelope.
  Preserve raw achieved course, the response/intercept scaffold, and the
  traveling bend; do not increase or scalar-tune the fixed transfer. A distinct
  allocation test may condition that same total-share-preserving transfer on
  normalized previous-action and current-speed burden, acting only while the
  posterior actuator is more burdened and the anterior has margin. This is an
  applicability hypothesis, not evidence for the unevaluated conditional
  allocator; falsify it if repeated capture or useful envelope use does not
  improve, the lower branch remains, or far-field closure, either wake, speed,
  clipping, force, or moment leaves the sampled speed-reserve envelope.
