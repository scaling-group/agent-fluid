...
- Once achieved-course steering and a coherent traveling carrier establish a
  ...
  the lower-exit class. The assigned parent's terminal mean-curvature transfer
  also preserved a coherent alternating wake, but reconstructed body-frame
  geometry exposed a growing signed inertial line-of-sight miss: about
  `0.13/T` at `3.31L`, `0.31/T` at `1.66L`, and `0.48/T` when closure stopped
  at `1.545L`, before the lower exit. In contrast, the sampled controller that
  allowed joint-compensated yaw to release shared steering only while this
  independent target/velocity signal remained small achieved the first
  capture at `0.7493L`; its top-down and oblique wake stayed coherent with
  about `54--55 deg` sub-`4L` joint excursion. The reusable implication is to
  treat signed inertial line-of-sight rotation as a collision-course guard:
  re-engage existing authority when it grows rather than adding route gain,
  static mean curvature, another yaw-rate cascade, carrier recovery, or cadence
  relief. This applies only after broad route acquisition and an active
  propulsive rhythm, and the signal must be formed from normalized body-frame
  target and velocity rather than raw bearing rate, which contains body-beat
  yaw. The capture margin was only about `0.00065L` and terminal
  acceleration-envelope occupancy was about `72.8%/75.9%`, so success is
  demonstrated but not robust; falsify the lesson if reproduction or held-out
  pose/target tests lose capture, weaken the wake, increase saturation, alter
  early closure, or restore the lower-exit topology.
