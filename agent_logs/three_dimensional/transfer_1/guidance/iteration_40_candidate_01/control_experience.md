...
- Reject the bounded outer-terminal inertial-LOS-rate residual as a robust
  improvement, rather than tuning or stacking it after isolated captures. Its
  original inherited run captured at `0.74605L/18.29849T`, and the assigned
  parent captured at `0.74957L/18.8980T`, but a sibling evaluation with the
  exact same policy bytes passed at `1.59620L` and exited below at `10.33579L`.
  The failed combined sheet retains the alternating top-down street and
  bilateral oblique structures after closest pass, while clipping, speed-limit
  residence, and peak force/moment stay inside the repeat-backed envelope; the
  failure is again fragile terminal path geometry, not propulsion collapse or
  instability. By contrast, the raw achieved-course speed-reserve baseline has
  three exact sampled captures at `0.74846--0.74953L`, all earlier
  (`18.2875--18.6010T`) and higher-scoring (`-0.15733---0.15140`) than the
  parent's LOS repeat, with the same coherent wake and actuator/load class.
  Restore that baseline instead of scalar-tuning the LOS annulus, combining it
  with the one-run partial-retention cue, or adding another terminal residual.
  This negative lesson applies after broad route acquisition with an active
  traveling bend; revisit a separate response observation only if independent
  exact repeats beat the baseline's capture reliability and add an arrival,
  load, or actuator benefit without changing far-field closure or either wake.
