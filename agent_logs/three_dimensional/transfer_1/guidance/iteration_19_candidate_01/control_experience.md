...
- Do not optimize terminal control allocation for clamp fraction alone. The
  ...
  assigned exact-byte captures at `0.7466--0.7499L` and `18.205--18.672T` while
...
- Do not infer that carrier-aligned clipping makes half-cycle steering
  reallocation safe. Starting from the repeatable speed-reserve baseline, a
  ...
  baseline's four assigned `0.7466--0.7499L` captures. In this coherent, broadly
...
- Reject normalized signed projected miss as a replacement terminal steering
  ...
  then two direct-uniform, stable lower exits at `1.7680L` and `1.6366L`; all
  three retained active alternating wakes and continued self-propelling. Its
  `1/3` capture record and lack of clipping or score benefit contrast with the
  achieved-course/intercept speed-reserve baseline's four assigned captures.
  Preserve that repeat-backed route error and test a distinct terminal response
  residual rather than stacking more geometry onto projected miss. Revisit the
  replacement only if multiple exact repeats become reliable and improve a
  secondary metric without altering far-field closure.
- Treat a first capture from a bounded terminal response residual as a
  replication target, not an improvement. Geometry-gated yaw braking preserved
  both coherent wake views and captured once at `0.7486L` and `18.381T`, but its
  `-0.15985` score was below all four assigned baseline repeats, it did not
  reduce action clipping or speed-limit residence, and its peak yaw moment
  (`0.01679`) was about three percent above the baseline maximum (`0.01627`).
  Exact replay is warranted before tuning or stacking slip, carrier-phase, or
  new interception feedback. Accept the mechanism only if repeats retain
  capture and establish a trajectory, load, or actuator benefit; reject it if
  a repeat returns the lower-exit topology or merely damps yaw while worsening
  capture geometry.
