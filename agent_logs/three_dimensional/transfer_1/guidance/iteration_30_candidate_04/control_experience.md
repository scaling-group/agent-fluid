...
- Reject unsafe-terminal anterior steering transfer, including normalized
  actuator-burden conditioning, as a robustness repair in this topology. The
  fixed transfer's lone `0.7492L` capture was later than the sampled
  speed-reserve captures and did not improve clipping or speed-limit residence;
  its exact replay then missed at `1.9385L`. Two distinct gates that permitted
  transfer only when posterior speed/action burden exceeded anterior burden
  and the head retained margin also exited below after `1.1961L` and `1.3208L`
  closest passes. The assigned-parent `1.3208L` sheet retains an active
  alternating top-down and bilateral oblique wake, reaches about `0.841L/T` at
  closest pass, and clips returned action on about `70.2%/70.0%` of rows, so
  neither broader transfer nor burden-threshold tuning addresses the failure.
  Across the four sampled captures and seven distinct inherited lower exits,
  the branch is already separated by the first `10L--6L` crossings, before
  these terminal allocators activate: capture head heights at `6L` are
  `11.753--11.860L`, versus `11.471--11.679L` for the failures. Preserve the
  traveling bend and baseline steering shares; test an outer, normalized
  body-motion observation that prevents target-aligned sideslip from silencing
  the route request, rather than another terminal allocation or threshold.
  Falsify that implication if repeated CFD does not reduce the early branch
  split while preserving capture-class wake, speed, loads, and actuator use.
