...
- Reject a response-opposing yaw brake inside the predicted capture corridor
  for this controller topology, rather than retuning its small bound or range.
  Four sampled exact-byte evaluations of the achieved-course/intercept
  speed-reserve baseline captured at `0.7466--0.7499L` and
  `18.2050--18.6725T`, whereas adding a corridor-gated brake bounded at only
  `2.4 rad/T^2` produced a stable lower exit after a `1.1336L` closest pass.
  The failed run remained self-propelled at about `0.839L/T` at closest
  approach and retained coherent top-down and oblique wakes, so its lower
  joint-speed residence (`8.4%/8.0%` versus roughly `10.4--11.6%`) is not a
  benefit: the residual perturbed terminal steering geometry without fixing
  propulsion, action clipping, or capture. Preserve the exact baseline rather
  than stacking yaw-response damping; test a new terminal primitive only when
  an observed state distinguishes the divergent approaches before its action
  would begin. Falsify this negative lesson only if repeated, independently
  evaluated response-brake rollouts recover capture and add a consistent load
  or saturation benefit without weakening the traveling wake.
- Reject normalized signed projected miss as a replacement terminal steering
  ...
  then two stable, direct-uniform lower exits at `1.7680L` and `1.6366L`; the
  ...
  sampled exact speed-reserve captures and did not reduce clipping. This `1/3`
  reliability contrasts with the achieved-course/intercept speed-reserve
  baseline's three sampled captures at `0.7466--0.7494L`, so the failed
  mechanism is terminal path geometry rather than insufficient propulsion.
  Preserve the repeat-supported route error, intercept veto, sparse carrier
  reserve, and phase-independent steering; if terminal yaw remains the target,
  test a separately bounded response brake that acts only on an approaching
  pass already inside the capture corridor. Revisit projected-miss replacement
  only if multiple exact repeats become reliable and add a clear arrival,
  load, or saturation benefit without changing far-field closure.
