...
- Do not optimize terminal control allocation for clamp fraction alone. The
  ...
  exact-byte captures at `0.7466--0.7494L` and `18.205--18.601T`, but inherited
  exact repeats now add lower exits at `1.7276L` and `1.6463L`. The available
  `3/5` record makes it the best-defined control baseline, not a robust policy.
  Its successful top-down and oblique wakes remain coherent, but action still
  clamps on about `68.5--68.7%/70.6--71.0%` of rows and exact speed-limit
  residence is about `10.4--11.6%`. A total-command speed governor reduced
  speed residence to `6.9%/7.2%` yet missed below at `1.3877L`, exited, and
  retained `69.8%/70.9%` action clamping with a coherent wake. Preserve sparse
  carrier reserve as the comparison controller; avoid governors that replace
  outward combined commands with restoring acceleration merely to lower limit
  residence. This applies to coherent, broadly acquired trajectories; revise
  it only if a distinct controller repeatedly captures while lowering both
  speed residence and clipping without increasing loads or coasting.
...
- Reject normalized signed projected miss as a replacement terminal steering
  ...
  after closest pass. Its lone capture was also slower-scoring than the
  captured speed-reserve runs and did not reduce clipping. Later exact misses
  reduce the achieved-course baseline itself to `3/5`, but they do not rescue
  projected miss as a replacement route error: its `1/3` record has the same
  active-wake lower-exit topology and no load or actuator benefit. Preserve
  the achieved-course error as the comparison observation; do not stack yaw,
  capture-corridor, or actuator feedback onto projected miss. This conclusion
  applies to replacing the route command inside the last `2L`; revisit it only
  if multiple exact repeats add reliable capture and a clear arrival, load, or
...
- Do not promote terminal yaw damping from a single threshold capture, even
  ...
  or tuning its damping scalar. Do not stack it with another terminal residual.
- Reject both the posterior wave-shape pulse and an unqualified full-terminal
  projected-corridor veto in this topology. The pulse's two sampled captures
  at `0.7480--0.7492L` are followed by an exact `1.2589L` lower-exit miss, so
  its `2/3` record does not justify retention or gain tuning. The full-terminal
  veto then reached `1.0757L` but still exited below; all combined sheets show
  an active alternating top-down and oblique wake, and closest-pass speeds
  remain about `0.83--0.90L/T`. Reconstructed projected miss crosses in and
  out of the corridor with carrier phase, consistent with the measured
  `-0.937` to `-0.961` correlation between terminal lateral velocity and
  anterior joint speed. Therefore treat instantaneous projection as a
  confidence-qualified observation, not a continuous terminal command: if it
  is tested before the existing `2.75--2.0L` intercept guard, trust it only
  where normalized joint state indicates small gait-synchronous sway, and do
  not change the carrier or half-cycle allocation. This applies to the active-
  wake, broadly acquired lower-branch topology; falsify it if phase-qualified
  projection changes far-field closure, loses capture, weakens either wake
  view, leaves the same exit, or worsens the actuator/load envelope.
