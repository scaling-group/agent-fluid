...
- Spatial steering transfer must be sparse and direction-sensitive; absolute
  actuator activity is not enough. A broad load-conditioned allocator retained
  the traveling wake but passed below at `1.1961L` and exited. Requiring
  normalized posterior speed and previous action to be outward, posterior
  burden to exceed anterior burden, the requested steering to push the tail
  farther outward, and the head to have margin instead produced a `0.7474L`
  capture at `18.3040T` with both coherent wake views. This is a semantic
  improvement over the broad allocator and the fixed transfer's slower
  `18.7495T` capture, but it is not yet a robust or actuator-envelope benefit:
  the refined policy has only one run, and its head/tail clipping
  (`68.69%/70.61%`) and speed-limit residence (`10.61%/11.60%`) remain within,
  rather than below, the repeat-backed baseline envelope. Preserve raw
  achieved course, the intercept scaffold, total steering share, and the
  traveling bend; repeat the exact sparse allocator before changing its
  thresholds or stacking another terminal residual. Promote it only after an
  exact repeat captures without degrading far-field closure, either wake,
  speed, force, moment, clipping, or speed-limit residence; reject it after an
  exact lower-branch miss or if repeated runs show no useful allocation
  benefit beyond the baseline's existing capture reliability.
