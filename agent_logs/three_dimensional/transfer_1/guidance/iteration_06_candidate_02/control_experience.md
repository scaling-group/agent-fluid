...
- Do not feed instantaneous body-frame lateral velocity directly into a slow
  achieved-course loop when the carrier is coherent. Across the four sampled
  still-water traces, beat-centered lateral velocity was explained by the two
  joint velocities with `R^2=0.903--0.986`, consistent in sign and scale across
  both upper- and lower-pass policies, while the analogous longitudinal fits
  were only `R^2=0.018--0.179`. In the `1.267L` lower-pass trace this
  contamination reversed the raw course residual from `+0.195 rad` at `4L` to
  `-0.127 rad` at `3L` and back to `+0.260 rad` at `2L`, even though the useful
  slow correction remained toward the target. Preserve longitudinal speed and
  the traveling bend, but test joint-phase compensation or a genuinely
  longer-window estimator for lateral course before adding terminal gain.
  This applies only to a developed rhythmic gait; falsify it if compensation
  weakens early closure, changes the correct turn sign, increases saturation,
  or the phase-to-lateral-velocity relation does not persist under a changed
  carrier or hydrodynamic condition.
