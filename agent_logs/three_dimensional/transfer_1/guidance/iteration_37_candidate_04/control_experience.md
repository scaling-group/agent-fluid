...
- Do not call the exact speed-reserve baseline robust from the three current
  `0.7466--0.7494L` captures alone. Two inherited exact-byte restorations also
  retain self-propulsion and coherent top-down/oblique wakes but exit below
  after closest passes of `1.6463L` and `1.4642L`, making the available record
  `3/5`. At the first `3L` crossing, current captures have body-frame target
  ...
  joint speed versus about `+0.94` for course error. This supports testing a
  bounded target-bearing qualifier before the inner intercept gate while
  retaining, rather than replacing, raw achieved course and the traveling
  bend. It does not establish the unevaluated qualifier as an improvement.
  Falsify the implication if it changes motion outside its gate, retains the
  lower branch, weakens either wake, loses capture, or leaves the speed-reserve
  load and actuator envelope.
...
- Treat material-point inconsistency in terminal geometry feedback as a sign
  defect, not a gain-tuning opportunity. All four current sampled policies
  self-propel to threshold capture with organized alternating top-down and
  bilateral oblique wakes, yet the inherited response veto combines a
  target-to-head vector with center velocity. Offline reconstruction over the
  sampled `4L`-to-capture intervals shows that this surrogate and the actual
  head LOS rate disagree in sign on `38.2--50.2%` of rows, with
  `0.203--0.237 rad/T` RMS discrepancy. Assigned-parent steps 33, 34, and 36
  meanwhile retained the lower-exit class after closest approaches of
  `1.4107L`, `1.1790L`, and `1.4731L`, so another bearing gain or fixed joint
  reallocation is not the supported repair. When capture and distance are
  head-based and normalized `target_body_rate_L` is available, test a
  head-consistent LOS response while preserving the repeat-backed traveling
  bend and spatial allocation. Falsify this implication if exact repeats do
  not improve capture reliability, or if far-field closure, either wake,
  arrival, clipping, speed residence, force, or moment leaves the sampled
  baseline envelope; do not generalize it to tasks whose target geometry and
  success rule use the center.
