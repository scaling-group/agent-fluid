...
- Treat a new terminal residual's first capture as compatibility evidence and
  spend the next test on exact replay before combining or tuning it. The
  repeat-backed speed-reserve baseline captured in all three current samples
  at `0.7466--0.7494L`; a small intercept-gated posterior wave-shape pulse also
  preserved both coherent wake views and captured once at `0.7492L`, but did
  not improve the baseline score range. By contrast, the assigned parent's
  geometry-gated terminal yaw brake retained self-propulsion and an organized
  wake yet reached only `1.5097L` before a stable lower exit. For coherent,
  broadly acquired trajectories, prefer exact replication of a single
  posterior wave-shape result over stacking yaw damping or tuning a terminal
  scalar; promote the mechanism only after repeated capture plus a path, load,
  arrival, or actuator benefit. Falsify this ordering if an exact posterior
  replay misses or changes far-field commands, or if independently repeated
  geometry-gated yaw damping captures without worsening wake, loads, or the
  actuator envelope.
