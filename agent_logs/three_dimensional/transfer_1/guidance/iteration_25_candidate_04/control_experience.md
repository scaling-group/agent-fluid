...
- Do not add or tune a low-bandwidth mean-curvature servo merely because an
  achieved-course trajectory branches before the inner intercept gate. The
  assigned parent's phase-independent average-bend residual remained stable,
  self-propelled, and wake-coherent, with peak planar force and yaw moment
  inside the sampled envelope, yet missed at `1.882L` and exited below while
  all four current samples captured at `0.7480--0.7494L`. Its head was already
  lower at the first `4L` crossing (`y=10.436L` versus `10.946--11.164L` in the
  captures), and the added bend did not recover that branch. Across both
  sampled policy families, terminal body-frame sway over `4L` to `2.75L` is
  instead consistently explained by the joint-state carrier projection
  `v_y ≈ -0.096*qdot_1 + 0.038*qdot_2` (residual RMSE
  `0.019--0.030L/T`); the captures have near-zero pooled residual sway while
  the failed parent retains about `-0.085L/T`. For this active-wake topology,
  preserve the carrier, actuator allocation, and raw geometric guards and test
  whether terminal-only demodulation prevents beat sway from masquerading as
  achieved-course error; avoid replaying mean curvature, scalar route gain,
  or another capture threshold. Falsify the demodulation implication if the
  fitted relation does not repeat, if the approach merely flips to an upper
  miss, or if exact repeats lose capture, weaken either wake, or exceed the
  repeat-backed actuator and load envelope. Do not generalize it to stalled or
