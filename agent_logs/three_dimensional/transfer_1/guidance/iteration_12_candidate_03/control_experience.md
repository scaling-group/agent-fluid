...
- Do not promote a threshold-grazing capture to a robust controller without a
  completed replay. Identical line-of-sight-guarded policy bytes captured once
  at `0.74934L` but an inherited replay missed at `1.5891L`; replacing its
  release with a velocity-ray corridor reached `1.6045L`, and transferring
  terminal authority into posterior lag reached `1.6818L`. All three failures
  retained a strong alternating terminal wake and the same lower exit, so the
  evidence rejects further tuning of release corridors, cadence, course gain,
  or lag magnitude in this already propelled, broadly acquired topology.
  Restore the intact carrier and treat the remaining error as a plant-response
  allocation problem: test one bounded, normalized yaw/slip or hydrodynamic-
  moment residual without enlarging the steering envelope. This lesson does
  not apply when propulsion or broad route acquisition is absent, and is
  falsified by repeated capture or a better termination class from a held-out
  release/lag replay with no increase in saturation or loads.
