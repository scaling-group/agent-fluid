...
- Use exact rollback to the speed-reserve baseline as a mechanism-isolation
  control, but do not call it robust. The assigned parent removed the failed
  posterior pulse and burden-conditioned allocation, then captured at
  `0.7495L` and `18.4525T` with an active alternating top-down street and
  bilateral oblique wake. Its head/tail action clipping (`68.46%/71.00%`),
  speed-limit residence (`10.49%/11.39%`), peak planar force (`0.0312`), and
  peak yaw moment (`0.0162`) match the sampled exact-baseline capture envelope;
  thus the semantic recovery is not a scalar-score artifact. This new result
  raises the exact-byte record from `3/5` to `4/6`, but the two coherent-wake
  lower exits at `1.6463L` and `1.4642L` still prohibit a reliability claim.
  When an added terminal mechanism fails an exact repeat while propulsion stays
  coherent, first restore and repeat the simpler controller instead of stacking
  another residual. Apply this only as an isolation baseline; falsify it as the
  useful rollback if further exact runs lose capture, weaken either wake, or
  leave the sampled load and actuator envelope.
...
- Reject both the tested outer-terminal bearing rescue and the burden-
  ...
  topology. The assigned parent's subsequent exact rollback captured and
  brought the baseline record to `4/6`, but it remains an isolation control,
  not evidence of robust capture. Revisit spatial allocation only if a
  different normalized state
...
- Do not treat closure reversal after a failed closest pass as a sufficient
  trigger for terminal recovery in this lower-exit topology. The inherited
  progress-loss redirect was designed to be inactive on all four sampled
  capture approaches, whose history-window closing speed remained positive
  below `4L`, but its completed rollout still passed outside capture at
  `1.2402L` and exited with final distance `10.1095L`. Together with the
  assigned parent's posterior-pulse repeat miss at `1.2589L` and the inherited
  outer-bearing miss at `1.4107L`, this shows that a clean post-pass gate does
  not repair first-pass geometry before the lower boundary. Restore and repeat
  the speed-reserve carrier rather than widening or gain-tuning the recovery.
  Apply this negative lesson only to redirects that begin after closing speed
  changes sign; falsify it if an independently repeated recovery captures
  without changing the evidenced first approach, traveling wake, actuator
  envelope, or loads. The available inherited log supplies termination and
  distance but no failure keyframe sheet, so it does not support a stronger
  wake-mechanism claim.
