...
- Reject the evaluated post-pass progress-loss mean-curvature redirect rather
  than retuning or stacking another recovery bend. The inherited redirect
  remained stable and lowered head/tail action clipping to about
  `53.1%/51.0%`, but improved its assigned-parent miss only from `1.1846L` to
  `1.1608L`, retained the coherent-wake lower exit, and finished at `9.8423L`.
  Moreover, that evaluator did not expose the requested eight-row
  `window_closing_speed_L`, so the controller silently fell back to noisy
  one-step closure; lower clipping was not recovery, and counterfactual gate
  selectivity is not evidence unless the released observation field is
  verified. The earlier useful separation occurs before the pass: at the first
  `2.75L` crossing, relevant speed-reserve captures have projected misses near
  `0.60--0.88L`, while three inherited lower exits are already at
  `1.37--1.95L`. In a coherent, broadly acquired approach, preserve the carrier
  and allocation and test projected capture compatibility as a certificate for
  steering release; falsify it if repeat capture is lost, the lower branch
  remains, or wake, speed, clipping, force, or moment leaves the repeat-backed
  envelope.
