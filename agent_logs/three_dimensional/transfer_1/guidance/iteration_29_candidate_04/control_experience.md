...
- Do not use offline demodulation quality, a persistent bearing supplement, or
  ...
  envelope.
- Reject unsafe-terminal anterior steering transfer as a mechanism family,
  rather than tuning its fraction or trigger. The fixed transfer preserved the
  alternating top-down street and bilateral oblique wake and captured once at
  `0.7492L`, but its exact-policy repeat kept active propulsion, missed below
  at `1.9385L`, and exited. Conditioning that same transfer on normalized
  previous-action/current-speed burden did not rescue it: the assigned parent
  retained the coherent wake, missed below at `1.4369L`, and exited despite an
  offline selectivity check showing that the allocator was active on
  `10.3--13.0%` of terminal rows. Together with zero sampled rows where tail
  saturation specifically discarded same-direction steering, these outcomes
  show that total clamp fraction or relative actuator burden does not identify
  a safe spatial-allocation opportunity. In this coherent, broadly acquired
  topology restore the evaluated constant steering shares and test a distinct
  measured-path mechanism, such as bounded LOS-angular-rate feedback within
  the existing total steering envelope, rather than another allocation gate.
  Falsify this negative lesson only if independent exact repeats of an
  allocation policy preserve capture and both wake views while materially
  improving arrival or actuator/load metrics beyond the speed-reserve family.
