...
- Do not let an all-capture survivor sample turn the conditional speed-reserve
  policy into a claim of robust terminal interception. Its three current
  exact-byte samples capture at `0.7466--0.7494L` in `18.287--18.601T` with
  coherent top-down and oblique wakes, but inherited evaluations of those
  exact bytes also contain two stable lower exits after `1.6441L` and
  `1.7276L` passes. The failures retain the alternating wake and about
  `0.82L/T` speed while the saturated route error and acceleration commands
  continue driving an opening pass, so this is terminal reacquisition
  variability rather than missing propulsion. Preserve the carrier and its
  sparse outward reserve, but test a signed-closure-gated redirect that is
  identically inactive on a still-closing approach; avoid unchanged replay,
  more route gain, or suppressing the beat. This also retains the prior
  negative result that a total-command governor lowered exact speed-limit
  residence to `6.9%/7.2%` yet missed at `1.3877L` and did not reduce action
  clipping. Apply the reacquisition implication only after a near pass with a
  coherent traveling wake; falsify it if exact repeats make the exits
  disappear, or if recovery feedback changes far-field closure, interrupts
  the wake, raises loads/saturation materially, or cannot restore closure
  before leaving the near region.
...
- Reject the intercept-gated posterior wave-shape pulse despite an all-capture
  current sample batch; do not let survivor sampling erase inherited exact-byte
  failures. The assigned parent and a second current pulse sample captured at
  `0.7480--0.7492L`, and both current speed-reserve samples captured at
  `0.7466--0.7494L`, with coherent alternating top-down and oblique wakes in
  all four direct-uniform runs. Yet the accumulated pulse record is only three
  captures versus two lower exits after `1.2589L` and `1.3584L` passes, while
  the phase-independent speed-reserve baseline has four captures. Current
  pulse and baseline score ranges overlap, as do action clipping (about
  `68.2--68.7%/70.6--70.9%`), exact speed-limit residence
  (`10.4--11.6%`), terminal speed (`0.827--0.932L/T`), force, and yaw-moment
  ranges; the pulse therefore adds phase-sensitive terminal-path variance
  without a repeatable secondary benefit, not missing propulsion. In this
  coherent, broadly acquired topology, restore the evaluated baseline carrier
  and phase-independent mean-curvature steering; avoid pulse gain tuning or
  stacking posterior shaping with the failed yaw brake. Revisit wave-shape
  steering only as a materially different normalized observation-to-curvature
  mechanism that survives multiple exact repeats and separates capture,
  arrival, loads, or actuator use from baseline variability without changing
  far-field closure or wake coherence.
