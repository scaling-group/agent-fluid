...
- Do not optimize terminal control allocation for speed-limit or clamp
  fraction alone. Conditional speed reserve, which softens only evidenced
  outward carrier effort, has now captured in four exact-byte evaluations at
  `18.2050--18.6010T`; all retain coherent top-down and oblique traveling
  wakes, cross at `0.8268--0.9083L/T`, and stay in the same load envelope.
  This establishes selective outward relief as a repeat-supported allocation
  mechanism, not a saturation cure: acceleration still clamps on about
  `68.5--68.8%/70.6--71.0%` of rows and exact joint-speed residence is about
  `10.4--10.6%/11.3--11.6%`. Carrier-first nested saturation previously
  missed at `1.4292L`, and a total-command speed governor missed at `1.3877L`.
  State-synchronous half-cycle steering now adds a sharper negative result: it
  kept the alternating wake and baseline-like peak loads and lowered speed
  residence to `8.47%/9.48%`, yet raised action clamping to `70.0%/71.5%`,
  missed below at `1.6860L`, and exited. Preserve the sparse carrier-only
  reserve and phase-independent achieved-course steering; avoid total-command
  projections, reserve-threshold tuning, or shifting steering toward the
  carrier-opposing half-cycle merely to improve envelope statistics. This
  implication applies after broad route acquisition with a coherent finite
  wake; falsify it only if a different allocation repeats capture while
  improving both envelope residence and loads without changing far-field
  closure.
- Reject normalized signed projected miss as a replacement terminal steering
  ...
  exact projected-miss bytes retained an alternating 3D wake in every run, but
  produced one threshold capture at `0.7477L` followed by two stable lower
  exits at `1.7680L` and `1.6366L`. The lone capture scored worse than the
  repeat-supported speed-reserve captures and did not reduce clipping; the two
  failures continued self-propelling after closest pass, so the `1/3` result
  is a terminal path-geometry failure rather than insufficient thrust. Keep
  the achieved-course/intercept error when testing another mechanism and do
  not stack terminal observations onto projected miss. Revisit it only if
  multiple exact replays recover capture and add a clear arrival, load, or
  saturation benefit without changing far-field closure.
