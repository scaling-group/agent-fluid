...
- In a coherent sub-`2L` approach, neither nominal actuator headroom nor
  target-normal motion is by itself a steering command. Carrier-aligned duty
  steering reached `0.953L`; moving the pulse to a joint-velocity phase raised
  the reconstructed unclipped steering increment but worsened the pass to
  `1.222L`, and adding target-normal velocity as posterior mean curvature
  reached `1.056L`. By contrast, releasing the direct course steering when
  joint-compensated yaw was established improved the inherited `1.044L` miss
  to `0.931L` while retaining the alternating wake. Preserve that
  response-release mechanism, and use target-normal or inertial line-of-sight
  motion as a bounded guard on whether release is still safe rather than as
  more curvature; avoid selecting a duty phase solely by predicted unclipped
  impulse. This implication applies only after broad course acquisition with
  an active carrier. Falsify it if guarded release changes far-field closure,
  increases saturation, weakens the terminal wake, or fails to improve the
  `0.931L` pass or lower-exit class.
