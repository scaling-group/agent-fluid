...
- Treat terminal locomotor reserve as necessary but not sufficient once the
  ...
  again crossed below the target. Together with the `1.5454L` static-curvature
  result, this falsifies locomotor recovery, wholesale mean curvature, and
  additional scalar course authority as sufficient capture fixes. For a
  coherent sub-`3L` pass, preserve the carrier and phase-selective actuator but
  test a separately bounded response to normalized target-normal velocity or
  phase-compensated line-of-sight rate. This implication does not apply before
  broad route acquisition; falsify it if response feedback worsens the
  `0.9532L` pass, increases saturation, weakens the wake, or leaves the same
  lower-exit topology.
