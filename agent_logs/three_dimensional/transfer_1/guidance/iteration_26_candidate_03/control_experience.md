...
- Reject terminal course demodulation as a robust cure for the lower branch,
  even when a joint-state fit explains sampled beat sway. The assigned
  parent's two-joint projection retained coherent top-down and oblique wakes
  and reduced exact speed-limit residence to about `8.1%/8.6%`, but missed
  below at `1.3787L` and exited; an independent anterior-speed compensation
  reproduced the baseline failure at `1.5445L`. Do not tune those projection
  coefficients or stack another course observer. More importantly, the exact
  speed-reserve bytes now have three sampled captures at `0.7466--0.7494L` but
  two inherited lower exits at `1.7276L` and `1.5478L`, so preserve their
  coherent carrier and allocation without calling them repeat-robust. In the
  `2.75L` to `1.5L` band, all four sampled captures keep body-frame target
  angle below `0.65 rad`, whereas the baseline, anterior-only, and two-joint
  demodulation failures spend `74--82%` of rows above that angle and have mean
  projected miss `1.24--1.44L`; their action is already mostly clipped, so
  scalar route gain is not the missing mechanism. If testing branch recovery,
  require a bounded steering realization that is identically absent inside
  the evidenced capture-bearing envelope, fades before the established inner
  corridor, and recruits posterior wave-shape authority only after persistent
  large error, rather than changing the successful route observation. An
  unconditional posterior pulse is not yet
  sufficient evidence: one sampled capture at `0.7480L` coexists with an
  inherited exact miss at `1.2589L`. Falsify the branch separation if a repeat
  capture crosses the angle boundary, or reject the recovery mechanism if it
  flips the miss upward, weakens either wake, or exceeds the sampled capture
  actuator and load envelope. Do not generalize this lesson to stalled or
  externally advected swimmers.
