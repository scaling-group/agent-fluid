...
- Treat terminal locomotor reserve as necessary but not sufficient once the
  ...
  direct course-servo near miss from `1.0435L` to the best completed `0.9312L`
  pass without attenuating the carrier, although it too retained the lower
  exit. Together with the `1.5454L` static-curvature result, this falsifies
  locomotor recovery, additive/wholesale mean curvature, and additional scalar
  course authority as sufficient capture fixes while supporting
  response-triggered authority release. For a coherent sub-`3L` pass, preserve
  that release structure and test an orthogonal bounded approach-hold or
  independently signed phase-lag mechanism driven by normalized line-of-sight
  motion. This implication does not apply before broad route acquisition;
  falsify an approach hold if it changes the far route, fails to beat
  `0.9312L`, increases saturation, collapses the wake or excursion, or leaves
  the same fast lower-exit topology.
