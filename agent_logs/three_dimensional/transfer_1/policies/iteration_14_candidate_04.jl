...
# carrier, then blend terminal steering toward normalized signed projected
# miss. No clock, world coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_speed_reserve_projected_miss_turn_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        projected_miss_turn_scale_L=0.60,
        projected_turn_start_distance_L=2.00,
        projected_turn_full_distance_L=0.90,
...
function guidance_module(state, params)
    ...
    signed_projected_miss_L = target_velocity_cross / projection_speed
    projected_miss_distance_L = abs(signed_projected_miss_L)
    ...
    projected_turn_span = max(
        params.projected_turn_start_distance_L -
            params.projected_turn_full_distance_L,
        1.0e-6,
    )
    projected_turn_gate = _smoothstep01(
        (params.projected_turn_start_distance_L - distance_L) /
            projected_turn_span,
    )
    projected_miss_turn_command = tanh(
        signed_projected_miss_L /
            max(params.projected_miss_turn_scale_L, 1.0e-6),
    )
    turn_command =
        (1.0 - projected_turn_gate) * route_turn_command +
        projected_turn_gate * projected_miss_turn_command
    ...
    return (
        ...
        route_turn_command=route_turn_command,
        signed_projected_miss_L=signed_projected_miss_L,
        projected_miss_turn_command=projected_miss_turn_command,
        projected_turn_gate=projected_turn_gate,
...
function target_policy(state, params)
    ...
    # Keep the evaluated intercept veto, carrier reserve, and additive steering
    # allocation. Only the terminal geometric steering error is blended from
    # course pursuit toward signed projected closest-pass miss.
