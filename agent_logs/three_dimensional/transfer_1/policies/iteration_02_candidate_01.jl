# B-test candidate derived from the iteration-20 2D champion control law, with
# normalized 3D observations and a response-gated course/yaw brake.
# Source policy SHA256 (LF artifact bytes): ff36d930636985ae6259d28d502b0532ff78bef48f090cf447ae11c33dfd441a
# The target geometry ratios are resolution independent. Oscillation phase
# remains entirely in observed joint state; no clock or world route is used.

function target_policy_params()
    return (
        version="dogfish_target_control_v20_wrongway_yaw_brake",
        drive_period=0.55,
        control_period=0.55,
        ...
        course_error_gain=1.40,
        course_speed_scale=0.25,
        course_forward_floor=0.08,
        course_angle_limit=1.25,
        yaw_brake_course_scale=0.45,
        yaw_brake_product_scale=0.20,
        yaw_brake_tail_limit=4.5 * pi / 180,
        bearing_gain=3.2,
...
function drive_module(
    ...
    yaw_brake_command = clamp(_safe(yaw_brake_request, 0.0), -1.0, 1.0)
    mean_tail_tangent =
        ...
        params.yaw_brake_tail_limit * yaw_brake_command
    ...
    return (
        ...
        yaw_brake_command=yaw_brake_command,
...
function guidance_module(state, params)
    ...
    # Track achieved course only after speed makes its direction meaningful.
    # Body-frame -x is the fish's forward axis.
    ...
    course_gate = tanh(course_speed / max(params.course_speed_scale, 1.0e-6))^2
    course_error = clamp(
        vector_angle - course_angle,
        -params.course_angle_limit,
        params.course_angle_limit,
    )
    course_residual = params.course_error_gain * course_gate * course_error
    ...
    # The course residual improved early alignment but still reversed into a
    # long wrong-way turn. Add posterior mean curvature only when recent yaw
    # has the sign that increases the current target-versus-course error.
    wrong_way_yaw = max(course_error * turn_rate, 0.0)
    yaw_brake_request =
        -course_gate *
        tanh(course_error / max(params.yaw_brake_course_scale, 1.0e-6)) *
        tanh(wrong_way_yaw / max(params.yaw_brake_product_scale, 1.0e-6))
    target_turn_rate =
    ...
    return (
        ...
        course_residual=course_residual,
        wrong_way_yaw=wrong_way_yaw,
        yaw_brake_request=yaw_brake_request,
...
function target_policy(state, params)
    ...
    drive = drive_module(
        ...
        guidance.yaw_brake_request,
    )
    turn = turn_actuator_module(drive, guidance, params)
    return (
        phi_ddot=(
            drive.head_accel + turn.head_delta,
