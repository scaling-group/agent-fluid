...
# propulsive traveling bend, then redistribute terminal steering toward the
# requested joint-state half-cycle without attenuating the carrier. No clock,
# world coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_intercept_guarded_half_cycle_steering_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        half_cycle_phase_lead=0.35,
        half_cycle_phase_scale=0.70,
...
function guidance_module(state, params)
    ...
    # These body-frame cross and dot products describe instantaneous inertial
    # closest-pass geometry without a memorized route or external phase.
...
function turn_response_module(state, guidance, drive, params)
    # Remove the observed beat component from body yaw before allowing a
    # correct-sign response to release steering. LOS and projected-intercept
    # geometry can independently veto that release.
...
function half_cycle_steering_module(guidance, drive, params)
    # Joint state is the phase sensor: angle plus a bounded velocity lead
    # identifies the current bend side without a clock or mutable oscillator.
    phase_coordinate =
        drive.q1 +
        params.half_cycle_phase_lead *
        drive.qd1 / max(drive.omega, eps(Float64))
    normalized_phase =
        phase_coordinate /
        max(
            drive.amplitude * params.half_cycle_phase_scale,
            eps(Float64),
        )
    beat_side = tanh(normalized_phase)
    asymmetry = abs(params.half_cycle_asymmetry)
    requested_half_cycle = guidance.turn_command * beat_side
    phase_weight = clamp(
        1.0 +
            guidance.terminal_response_gate *
            asymmetry *
            requested_half_cycle,
        max(1.0 - asymmetry, 0.0),
        1.0 + asymmetry,
    )
    return (
        beat_side=beat_side,
        requested_half_cycle=requested_half_cycle,
...
function target_policy(state, params)
    ...
    # Keep the evaluated intercept veto, then redistribute the same bounded
    # steering across observed half-cycles. The cycle-centered phase signal
    # changes wave shape without attenuating or replacing the carrier.
    response = turn_response_module(state, guidance, drive, params)
    half_cycle = half_cycle_steering_module(guidance, drive, params)
    steering_accel =
        params.steering_accel_limit *
        guidance.turn_command *
        (1.0 - response.release) *
        half_cycle.phase_weight
    head_accel =
        drive.head_accel +
