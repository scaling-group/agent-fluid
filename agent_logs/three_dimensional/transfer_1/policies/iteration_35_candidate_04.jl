# L64 3D candidate: retain the intercept-guarded speed-reserve first approach,
# then use an opening-only, response-released posterior wave-shape burst after
# a measured missed pass. No clock, world coordinate, route, or mutable phase
# is used.

function target_policy_params()
    return (
        version="dogfish3d_response_released_recovery_burst_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        recovery_opening_start=0.04,
        recovery_opening_full=0.16,
        recovery_tail_target_shift=6.0 * pi / 180,
...
function recovery_burst_module(guidance, response, drive, params)
    # Negative history-window closure distinguishes an opening pass from all
    # sampled successful approaches.  Shift only the posterior wave target:
    # the anterior oscillator, base tail lag, and additive route steering stay
    # active.  Correct-sign phase-compensated yaw response releases the shift
    # before it can become the held static curvature seen in the parent.
    distance_span = max(
    ...
    opening_speed = max(-guidance.closing_speed_L, 0.0)
    opening_span = max(
        params.recovery_opening_full - params.recovery_opening_start,
        1.0e-6,
    )
    opening_gate = _smoothstep01(
        (opening_speed - params.recovery_opening_start) / opening_span,
    )
    response_hold = 1.0 - response.response_gate
    burst_gate = distance_gate * opening_gate * response_hold
    tail_target_shift =
        params.recovery_tail_target_shift *
        guidance.turn_command *
        burst_gate
    return (
        distance_gate=distance_gate,
        opening_gate=opening_gate,
        response_hold=response_hold,
        burst_gate=burst_gate,
        tail_target_shift=tail_target_shift,
        tail_accel=drive.omega^2 * tail_target_shift,
    )
end
...
function target_policy(state, params)
    ...
    # outward-carrier reserve.  The recovery burst is exactly inactive while
    # the history-window distance is still closing.
    ...
    recovery = recovery_burst_module(guidance, response, drive, params)
    steering_accel =
