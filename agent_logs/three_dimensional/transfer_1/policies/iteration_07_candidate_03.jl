...
# bend, then place terminal steering on the observed joint-motion half-cycle
# instead of an already saturated carrier-acceleration extremum. No clock,
# world coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_course_servo_motion_aligned_steering_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        terminal_motion_pulse_gain=2.00,
        steering_velocity_phase_scale=0.35,
...
@inline function motion_aligned_steering_scale(
    turn_command,
    joint_velocity,
    omega,
    amplitude,
    ...
    # Joint velocity identifies motion toward the requested mean bend. Unlike
    # carrier acceleration, it stays informative through the zero crossing and
    # the restoring portion of that half-cycle, where additive steering has
    # physical headroom instead of being erased by the final hard clamp.
    normalized_motion =
        turn_command * joint_velocity /
        max(
            params.steering_velocity_phase_scale * omega * amplitude,
            1.0e-6,
        )
    useful_motion_gate = 0.5 * (1 + tanh(normalized_motion))
    motion_pulse =
        clamp(params.terminal_motion_pulse_gain, 0.0, 2.0) *
        useful_motion_gate

    # Outside the approach region this is exactly shared steering. At full
    # allocation the same bounded impulse is redistributed within the observed
    # joint cycle. The propulsive carrier itself is never scaled.
    return (
        (1 - terminal_gate) +
        terminal_gate * motion_pulse
    )
end
...
function target_policy(state, params)
    ...
    # Motion-aligned duty steering changes only the within-beat placement of
    # the inherited steering impulse. Each joint uses its own observed motion,
    # naturally retaining the posterior phase lag and reflection equivariance.
    steering_accel =
        params.steering_accel_limit * guidance.turn_command
    head_steering_scale = motion_aligned_steering_scale(
    ...
    tail_steering_scale = motion_aligned_steering_scale(
        guidance.turn_command,
        drive.qd2,
        drive.omega,
        drive.amplitude,
