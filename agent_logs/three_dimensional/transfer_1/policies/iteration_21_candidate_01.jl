...
# remove the evidenced carrier-synchronous lateral component only from the
# achieved-course observation. Physical intercept geometry still uses raw
...
function guidance_module(state, params)
    ...
    # Sampled traces show that raw lateral velocity is dominated by a stable
    # carrier component near -0.10*qd1. Remove only that bounded component from
    # achieved course so route feedback does not chase its own tailbeat. Raw
    # velocity below still owns physical LOS and projected-intercept geometry.
    bounded_qd1 = clamp(
        qd1,
    ...
    route_velocity_y =
        velocity_y + params.course_phase_qd1_gain * bounded_qd1
