# L64 3D candidate: retain the repeat-backed speed-reserve controller, then
# remove evidenced carrier-phase sway from only the terminal achieved-course
# observation. Raw velocity still owns physical interception geometry. No
# clock, world coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_terminal_phase_compensated_course_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        course_phase_sway_qd1_gain=0.10,
...
function guidance_module(state, params)
    ...
    # Terminal traces show that most instantaneous body-frame sway is the
    # anterior carrier phase, not the mean inertial course. Remove only that
    # locally evidenced component from the course estimator. Raw velocity is
    # retained below for speed and interception geometry.
    qd1 = _safe(state.phi_dot[1], 0.0)
    phase_sway_correction = terminal_response_gate * clamp(
        params.course_phase_sway_qd1_gain * qd1,
        -course_speed,
        course_speed,
    )
    course_lateral_velocity = velocity_y + phase_sway_correction
    course_angle = atan(
...
function target_policy(state, params)
    ...
    # Keep the evaluated raw-velocity intercept veto, additive steering, and
    # sparse carrier reserve. The only new mechanism is the terminal course
    # observation compensation inside guidance_module.
