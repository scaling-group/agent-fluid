...
# controller, then qualify achieved-course steering with only the part of the
# outer-terminal target bearing that its course error does not already support.
# No clock, world coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_outer_unsupported_bearing_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        bearing_qualifier_gain=0.75,
        bearing_course_error_full=0.03,
        bearing_course_error_off=0.10,
...
function guidance_module(state, params)
    ...
    # An informative achieved-course request already owns the turn. Target
    # bearing is admitted only while that error is near zero and only before
    # the inner intercept guard. The smooth cue is reflection equivariant and
    # exactly zero in the far field and capture corridor.
    course_support_span = max(
        params.bearing_course_error_off -
            params.bearing_course_error_full,
        1.0e-6,
    )
    unsupported_course_gate = _smoothstep01(
        (params.bearing_course_error_off - abs(course_error)) /
            course_support_span,
    )
    unsupported_bearing = unsupported_course_gate * target_angle
    intercept_distance_span = max(
        params.intercept_guard_start_distance_L -
            params.intercept_guard_full_distance_L,
        1.0e-6,
    )
    intercept_distance_gate = _clamp01(
        (params.intercept_guard_start_distance_L - distance_L) /
            intercept_distance_span,
    )
    outer_terminal_gate =
        terminal_response_gate * (1.0 - intercept_distance_gate)
    bearing_qualifier =
        params.bearing_qualifier_gain *
        outer_terminal_gate *
        unsupported_bearing
    route_error = clamp(
        base_route_error + bearing_qualifier,
        -params.course_angle_limit,
        params.course_angle_limit,
    )
    turn_command =
    ...
    return (
        ...
        unsupported_course_gate=unsupported_course_gate,
        unsupported_bearing=unsupported_bearing,
        outer_terminal_gate=outer_terminal_gate,
        bearing_qualifier=bearing_qualifier,
...
function target_policy(state, params)
    ...
    # carrier reserve. The target-bearing qualifier changes only the route
    # request before this allocation and cannot alter the carrier directly.
