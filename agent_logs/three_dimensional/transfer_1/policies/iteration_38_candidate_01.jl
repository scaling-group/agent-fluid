# L64 3D candidate: retain the repeat-backed inner intercept guard and
# propulsive traveling bend, then partially retain outer-terminal steering
# when projected approach geometry is not capture-compatible. No clock, world
# coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_partial_outer_intercept_guard_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        outer_guard_start_distance_L=4.00,
        outer_guard_full_distance_L=2.75,
        outer_incompatible_steering_retention=0.35,
...
function turn_response_module(state, guidance, drive, params)
    ...
    # Outside the repeat-backed inner veto, incompatibility retains only a
    # bounded fraction of steering; the failed full certificate retained all
    # steering and over-turned the approach.
    ...
    outer_guard_span = max(
        params.outer_guard_start_distance_L -
            params.outer_guard_full_distance_L,
        1.0e-6,
    )
    outer_distance_gate =
        _smoothstep01(
            (params.outer_guard_start_distance_L - guidance.distance_L) /
                outer_guard_span,
        ) *
        (1.0 - intercept_distance_gate)
    outer_incompatibility =
        outer_distance_gate * (1.0 - intercept_compatible)
    outer_release_guard =
        1.0 -
        clamp(
            params.outer_incompatible_steering_retention,
            0.0,
            0.80,
        ) * outer_incompatibility
    release_guard = intercept_release_guard * outer_release_guard
    release =
    ...
    return (
        ...
        outer_distance_gate=outer_distance_gate,
        outer_incompatibility=outer_incompatibility,
        outer_release_guard=outer_release_guard,
...
function target_policy(state, params)
    ...
    # Keep the evaluated inner veto, full additive allocation, and sparse
    # carrier reserve. The outer governor changes only how much steering the
    # observed yaw response may release before the inner guard takes over.
