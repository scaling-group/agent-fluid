...
# condition unsafe-terminal anterior steering transfer on normalized head/tail
# actuator burden and available anterior envelope. Total steering authority
# stays constant. No clock, world coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_actuator_burden_allocation_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        max_burden_steering_transfer=0.55,
...
function target_policy(state, params)
    ...
    # carrier reserve. When raw projection is not capture-compatible, move a
    # bounded steering share anteriorly only while normalized speed/action
    # state says the tail is more burdened and the head has envelope margin.
    # The share sum and carrier are invariant.
    ...
    acceleration_limit = max(abs(params.acceleration_limit), 1.0e-6)
    joint_speed_limit = max(abs(params.joint_speed_limit), 1.0e-6)
    head_burden = _clamp01(max(
        abs(previous_head_action) / acceleration_limit,
        abs(drive.qd1) / joint_speed_limit,
    ))
    tail_burden = _clamp01(max(
        abs(previous_tail_action) / acceleration_limit,
        abs(drive.qd2) / joint_speed_limit,
    ))
    unused_head_envelope = 1.0 - head_burden
    burden_advantage_gate = unused_head_envelope > 1.0e-6 ?
        _clamp01(
            (tail_burden - head_burden) / unused_head_envelope,
        ) :
        0.0
    head_available_steering_share =
        unused_head_envelope *
        acceleration_limit /
        max(abs(params.steering_accel_limit), 1.0e-6)
    steering_transfer = min(
        max(params.max_burden_steering_transfer, 0.0) *
            unsafe_terminal_gate *
            burden_advantage_gate,
        head_available_steering_share,
