# L64 3D candidate: retain the repeat-supported intercept-guarded course servo,
# traveling bend, and sparse carrier reserve, then project only the terminal
# combined acceleration that would push a joint farther into its speed limit.
# No clock, world coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_terminal_velocity_feasibility_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        feasibility_speed_start=0.90,
        feasibility_speed_full=0.995,
        feasibility_outward_floor=0.05,
...
@inline function velocity_feasible_accel(
    joint_velocity,
    proposed_accel,
    terminal_gate,
    params,
)
    # This one-sided projection is inactive in the far field and below the
    # observed speed reserve. Near the envelope it withholds only acceleration
    # aligned with velocity; acceleration that restores speed passes unchanged.
    speed_ratio =
        abs(joint_velocity) / max(params.joint_speed_limit, 1.0e-6)
    speed_span = max(
        params.feasibility_speed_full - params.feasibility_speed_start,
        1.0e-6,
    )
    speed_gate = _smoothstep01(
        (speed_ratio - params.feasibility_speed_start) / speed_span,
    )
    if joint_velocity * proposed_accel <= 0.0
        return proposed_accel
    end

    velocity_sign = copysign(1.0, joint_velocity)
    outward_accel = velocity_sign * proposed_accel
    retained_fraction = clamp(
        params.feasibility_outward_floor,
        0.0,
        1.0,
    )
    projection_gate = _clamp01(terminal_gate) * speed_gate
    removed_outward =
        (1.0 - retained_fraction) * projection_gate * outward_accel
    return proposed_accel - velocity_sign * removed_outward
end
...
function target_policy(state, params)
    ...
    # Keep the evaluated intercept veto and full additive steering. The
    # inherited reserve first trims demonstrably unusable outward carrier;
    # the new feasibility stage below then sees the combined command.
    ...
    head_accel = velocity_feasible_accel(
        drive.qd1,
        combined_head_accel,
        guidance.terminal_response_gate,
        params,
    )
    tail_accel = velocity_feasible_accel(
        drive.qd2,
        combined_tail_accel,
        guidance.terminal_response_gate,
        params,
