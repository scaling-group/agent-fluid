# L64 3D candidate: retain the closest inherited response-released course
# servo, then reduce carrier cadence only when near-target transverse motion
# dominates closure. Carrier amplitude and posterior lag remain intact; no
# clock, world route, mean-curvature response, or mutable state is used.

function target_policy_params()
    return (
        version="dogfish3d_response_release_transverse_hold_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        approach_hold_start_distance_L=3.00,
        approach_hold_full_distance_L=1.00,
        transverse_dominance_release=0.70,
        transverse_dominance_full=0.92,
        terminal_cadence_relief=0.20,
...
function drive_module(state, params, drive_frequency_scale=1.0)
    ...
    # Joint state carries the carrier phase. Cadence may slow on approach, but
    # the amplitude homeostat and posterior traveling-bend lag are preserved.
    vdp_drive =
        params.drive_mu *
        (1 - (q1 / amplitude)^2) *
        qd1
...
function guidance_module(state, params)
    ...
    # Body-frame -x is forward. Target angle initializes the request; achieved
    # course takes over as translation becomes informative.
    target_angle = atan(
        target_y,
    ...
    terminal_response_gate = _clamp01(
        (params.terminal_response_start_distance_L - distance_L) /
            terminal_response_span,
    )
    approach_hold_span = max(
        params.approach_hold_start_distance_L -
            params.approach_hold_full_distance_L,
        1.0e-6,
    )
    approach_hold_distance_gate = _clamp01(
        (params.approach_hold_start_distance_L - distance_L) /
            approach_hold_span,
    )

    # cross(target, velocity)/distance is a signed, rotation-invariant
    # target-normal speed. Compare its magnitude with nonnegative closure so a
    # pure retreat does not masquerade as useful longitudinal progress.
    target_normal_velocity =
        (target_x * velocity_y - target_y * velocity_x) / distance_L
    nonnegative_closing_speed = max(closing_speed, 0.0)
    observed_motion = hypot(
        abs(target_normal_velocity),
        nonnegative_closing_speed,
    )
    transverse_dominance = observed_motion > 1.0e-6 ?
        abs(target_normal_velocity) / observed_motion :
        0.0
    return (
        ...
        approach_hold_distance_gate=approach_hold_distance_gate,
        target_normal_velocity=target_normal_velocity,
        transverse_dominance=transverse_dominance,
    )
end
...
function turn_response_module(state, guidance, drive, params)
    # Instantaneous body yaw contains a strong beat-frequency component. Joint
    # velocity compensation isolates the slow signed response; only established
    # response in the requested direction is allowed to release steering.
...
@inline function approach_hold_gate(guidance, params)
    dominance_span = max(
        params.transverse_dominance_full -
            params.transverse_dominance_release,
        1.0e-6,
    )
    dominance_gate = _smoothstep01(
        (guidance.transverse_dominance -
            params.transverse_dominance_release) /
            dominance_span,
    )
    return guidance.approach_hold_distance_gate * dominance_gate
end
...
function target_policy(state, params)
    ...
    # Reduce cadence—not amplitude or a carrier half—only when near-target
    # target-normal motion dominates closure. Outside the distance gate this is
    # exactly the inherited response-released course servo.
    hold_gate = approach_hold_gate(guidance, params)
    drive_frequency_scale =
        base_drive_frequency_scale *
        (1.0 - params.terminal_cadence_relief * hold_gate)
