...
# propulsive traveling bend, but permit terminal steering release only while
# current target/velocity geometry projects an approaching capture corridor.
# The carrier is never attenuated. No clock, world coordinate, route, or
...
function turn_response_module(state, guidance, drive, params)
    ...
    # yaw. Correct-sign slow yaw may release steering, but LOS rate and the new
    ...
    # steering. The achieved velocity must also point toward the target and
    # project inside the capture corridor. The distance blend preserves the
    # sampled far/middle route and introduces no additional authority.
...
function target_policy(state, params)
    ...
    # Preserve the inherited response-triggered release only while both LOS
    # rate and projected intercept geometry agree. Otherwise restore the
    # existing steering; never add gain or alter the propulsive carrier.
