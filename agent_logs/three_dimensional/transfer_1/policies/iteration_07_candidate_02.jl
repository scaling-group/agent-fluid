# L64 3D candidate: retain the achieved-course observation and propulsive
# traveling bend, but map target-minus-course angle to the empirically
# corrective bend sign. Terminal steering is reallocated within the beat only
# while observed joint energy can sustain the carrier. No clock, world
# coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_sign_corrected_course_energy_halfcycle_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        carrier_energy_release=0.50,
        carrier_energy_full=0.72,
...
@inline function carrier_reserve_gate(drive, params)
    velocity_scale = max(
        drive.omega * drive.amplitude,
        1.0e-6,
    )
    head_energy = hypot(
        drive.q1 / drive.amplitude,
        drive.qd1 / velocity_scale,
    )
    tail_energy = hypot(
        drive.q2 / drive.amplitude,
        drive.qd2 / velocity_scale,
    )
    carrier_energy = min(head_energy, tail_energy)
    energy_span = max(
        params.carrier_energy_full - params.carrier_energy_release,
        1.0e-6,
    )
    return _clamp01(
        (carrier_energy - params.carrier_energy_release) /
            energy_span,
    )
end
...
function target_policy(state, params)
    ...
    # With body-frame -x forward, a positive target-minus-course angle asks
    # for negative yaw. Sampled cycle-smoothed traces show that positive shared
    # bend acceleration produces positive yaw, so the actuator request must
    # carry the opposite sign to close rather than reinforce the route error.
    steering_request = -guidance.turn_command
    cadence_gain =
    ...
    # The course servo retains the sampled shared-acceleration realization
    # with the corrected request sign. Near the target, opposing-half
    # attenuation is available only while both joints retain enough normalized
    # phase-plane energy. This observed-state reserve releases the attenuation
    # before the traveling bend collapses; it never amplifies the carrier or
    # changes cadence.
    half_relief = clamp(
    ...
    asymmetry_strength =
        ...
        half_relief *
        carrier_reserve_gate(drive, params)
    ...
    steering_accel =
        params.steering_accel_limit * steering_request
    head_accel =
