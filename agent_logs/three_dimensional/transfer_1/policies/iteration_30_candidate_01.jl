# L64 3D candidate: retain the intercept-guarded speed-reserve carrier and add
# a bounded body-frame bearing rescue only when outer-terminal target bearing
# and achieved-course error disagree. No clock, world coordinate, route, or
# mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_outer_bearing_rescue_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        bearing_rescue_outer_full_distance_L=3.25,
        bearing_rescue_inner_full_distance_L=3.00,
        bearing_disagreement_scale=0.020,
        bearing_rescue_angle_scale=0.18,
        bearing_rescue_limit=0.16,
...
function guidance_module(state, params)
    ...
    # The inherited failure signature has unresolved body-frame bearing while
    # achieved-course error is nearly zero or requests the other turn. Supply
    # a bounded target-vector-to-curvature residual only before the existing
    # inner intercept guard; same-sign course control remains authoritative.
    bearing_outer_span = max(
        params.terminal_response_start_distance_L -
            params.bearing_rescue_outer_full_distance_L,
        1.0e-6,
    )
    bearing_outer_gate = _smoothstep01(
        (params.terminal_response_start_distance_L - distance_L) /
            bearing_outer_span,
    )
    bearing_inner_span = max(
        params.bearing_rescue_inner_full_distance_L -
            params.intercept_guard_start_distance_L,
        1.0e-6,
    )
    bearing_inner_gate = _smoothstep01(
        (distance_L - params.intercept_guard_start_distance_L) /
            bearing_inner_span,
    )
    bearing_annulus_gate = bearing_outer_gate * bearing_inner_gate
    bearing_disagreement_gate =
        0.5 *
        (1 - tanh(
            target_angle * course_error /
            max(params.bearing_disagreement_scale, 1.0e-6),
        ))
    bearing_rescue =
        course_gate *
        bearing_annulus_gate *
        bearing_disagreement_gate *
        params.bearing_rescue_limit *
        tanh(
            target_angle /
            max(params.bearing_rescue_angle_scale, 1.0e-6),
        )
    route_error = clamp(
        base_route_error + bearing_rescue,
        -params.course_angle_limit,
        params.course_angle_limit,
    )
    turn_command =
    ...
    return (
        ...
        bearing_annulus_gate=bearing_annulus_gate,
        bearing_disagreement_gate=bearing_disagreement_gate,
        bearing_rescue=bearing_rescue,
