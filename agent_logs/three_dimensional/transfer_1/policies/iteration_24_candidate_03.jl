...
# achieved-course servo, then guard terminal steering release with the
# phase-invariant LOS-versus-velocity course residual. No clock, world
# coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_speed_reserve_course_convergence_guard_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        course_release_error_inner=0.12,
        course_release_error_outer=0.35,
...
function turn_response_module(state, guidance, drive, params)
    ...
    course_release_span = max(
        params.course_release_error_outer -
            params.course_release_error_inner,
        1.0e-6,
    )
    course_convergence_gate =
        1.0 -
        _smoothstep01(
            (abs(guidance.course_error) -
                params.course_release_error_inner) /
                course_release_span,
        )

    # A correct-sign yaw response is not yet a useful maneuver response when
    # inertial velocity remains oblique to the LOS. Apply this extra semantic
    # condition only inside the existing intercept gate; far-field response,
    # carrier kinematics, and actuator allocation remain unchanged.
    intercept_compatible =
        projected_corridor_gate *
        approach_gate *
        course_convergence_gate
    ...
    return (
        ...
        course_convergence_gate=course_convergence_gate,
...
function target_policy(state, params)
    ...
    # mechanism changes only whether an otherwise eligible terminal response
    # release is accepted after achieved-course convergence.
