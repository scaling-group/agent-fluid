...
# posterior actuator is more burdened and the anterior actuator has margin.
# Total steering authority stays constant. No clock, world coordinate, route,
...
function target_policy_params()
    return (
        ...
        burden_steering_transfer_limit=0.55,
        allocation_speed_weight=0.50,
        burden_difference_start=0.05,
        burden_difference_full=0.25,
        head_margin_start=0.15,
        head_margin_full=0.45,
...
@inline function actuator_burden(
    joint_velocity,
    previous_action,
    params,
)
    speed_ratio = _clamp01(
        abs(joint_velocity) / max(params.joint_speed_limit, 1.0e-6),
    )
    action_ratio = _clamp01(
        abs(previous_action) / max(params.acceleration_limit, 1.0e-6),
    )
    speed_weight = clamp(params.allocation_speed_weight, 0.0, 1.0)
    return speed_weight * speed_ratio +
        (1.0 - speed_weight) * action_ratio
end

@inline function burden_conditioned_transfer(
    head_joint_velocity,
    tail_joint_velocity,
    ...
    head_burden = actuator_burden(
        head_joint_velocity,
        previous_head_action,
        params,
    )
    tail_burden = actuator_burden(
        tail_joint_velocity,
        previous_tail_action,
        params,
    )
    difference_span = max(
        params.burden_difference_full - params.burden_difference_start,
        1.0e-6,
    )
    tail_excess_gate = _smoothstep01(
        (
            tail_burden - head_burden -
            params.burden_difference_start
        ) / difference_span,
    )
    head_margin = 1.0 - head_burden
    margin_span = max(
        params.head_margin_full - params.head_margin_start,
        1.0e-6,
    )
    head_margin_gate = _smoothstep01(
        (head_margin - params.head_margin_start) / margin_span,
    )
    transfer_limit = min(
        max(params.burden_steering_transfer_limit, 0.0),
        max(params.tail_steering_share, 0.0),
    )
    return transfer_limit *
        _clamp01(unsafe_terminal_gate) *
        tail_excess_gate *
        head_margin_gate
end
...
function target_policy(state, params)
    ...
    # carrier reserve. In an unsafe terminal intercept, move a bounded share of
    # steering anteriorly only when state-normalized actuator burden says the
    # tail is more constrained and the head has margin. The share sum is
    # invariant, and the carrier is never reallocated.
    ...
    steering_transfer = burden_conditioned_transfer(
        drive.qd1,
        drive.qd2,
        previous_head_action,
        previous_tail_action,
