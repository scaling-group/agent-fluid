# L64 3D candidate: retain the intercept-guarded speed-reserve controller, then
# add a posterior wave-shape redirect only after inner-approach target bearing
# leaves the sampled capture envelope. No clock, world coordinate, route, or
# mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_speed_reserve_large_bearing_redirect_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        redirect_bearing_start=0.62,
        redirect_bearing_full=0.90,
        redirect_inner_fade_start_distance_L=1.60,
        redirect_inner_fade_full_distance_L=1.50,
        redirect_posterior_bias_gain=0.08,
...
@inline function large_bearing_redirect_accel(
    guidance,
    response,
    drive,
    params,
)
    # Recent captures remain below the start angle in the branch-separation
    # band, whereas lower-exit branches persist above the full angle. A
    # neutral-crossing posterior target pulse supplies wave-shape steering to
    # that branch and fades before close-range bearing can rise in a capture.
    bearing_span = max(
        params.redirect_bearing_full - params.redirect_bearing_start,
        1.0e-6,
    )
    large_bearing_gate = _smoothstep01(
        (abs(guidance.target_angle) - params.redirect_bearing_start) /
            bearing_span,
    )
    inner_fade_span = max(
        params.redirect_inner_fade_start_distance_L -
            params.redirect_inner_fade_full_distance_L,
        1.0e-6,
    )
    inner_preserve_gate = _smoothstep01(
        (guidance.distance_L -
            params.redirect_inner_fade_full_distance_L) /
            inner_fade_span,
    )
    normalized_phase_speed = clamp(
        abs(drive.qd1) /
            max(drive.omega * drive.amplitude, 1.0e-6),
        0.0,
        1.0,
    )
    posterior_target_bias =
        params.redirect_posterior_bias_gain *
        drive.amplitude *
        guidance.turn_command *
        (1.0 - response.release) *
        response.intercept_distance_gate *
        inner_preserve_gate *
        large_bearing_gate *
        normalized_phase_speed
    return drive.omega^2 * posterior_target_bias
end
...
function target_policy(state, params)
    ...
    # Keep the evaluated intercept veto, additive steering, and sparse carrier
    # reserve. The separate redirect can act only on the evidenced large-error
    # inner branch and vanishes continuously as target bearing recovers.
    ...
    steering_accel =
        ...
        guidance.turn_command *
        (1.0 - response.release)
    redirect_accel = large_bearing_redirect_accel(
        guidance,
        response,
        drive,
        params,
