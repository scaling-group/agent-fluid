...
# controller, then damp residual yaw only during an intercept-compatible
# steering-release state. No clock, world coordinate, route, or mutable phase
# is used.
...
function target_policy_params()
    return (
        ...
        terminal_yaw_damping_limit=2.0,
        terminal_yaw_rate_scale=0.35,
...
@inline function terminal_yaw_damping(response, params)
    # The established steering polarity makes an acceleration residual with
    # the yaw-rate sign oppose that response. Gate it by the existing release
    # decision and by terminal range: LOS or projected-miss disagreement turns
    # damping off and restores the full achieved-course request.
    damping_gate =
        response.intercept_distance_gate * response.release
    normalized_yaw_rate =
        response.phase_compensated_turn_rate /
        max(params.terminal_yaw_rate_scale, 1.0e-6)
    return (
        clamp(params.terminal_yaw_damping_limit, 0.0, 4.0) *
        damping_gate *
        tanh(normalized_yaw_rate)
    )
end
...
function target_policy(state, params)
    ...
    # Keep the evaluated intercept veto, additive steering allocation, and
    # carrier reserve. The new approach-hold residual acts only after those
    # geometric and response gates already permit steering release.
    ...
    damping_accel = terminal_yaw_damping(response, params)
    steering_accel = route_steering_accel + damping_accel
    head_accel =
