# L64 3D candidate: retain the captured intercept-guarded traveling bend and
# outward-only speed relief, then allocate its unchanged steering request by
# instantaneous directional acceleration reserve. No clock, world coordinate,
# route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_directional_reserve_allocation_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        steering_allocation_floor=0.30,
...
function steering_reserve_allocation(
    head_carrier_accel,
    tail_carrier_accel,
    steering_accel,
    terminal_gate,
    ...
    # A carrier already pushing toward the requested acceleration direction
    # has less room for the additive steering residual before the hard clamp.
    # Preserve the nominal total steering share, but move it smoothly toward
    # the joint with directional room. The floor prevents either joint from
    # dropping out and keeps the evidenced two-joint traveling bend active.
    # Gate the change to the diagnosed terminal regime so the sampled
    # successful far-field route is exactly preserved.
    steering_direction = sign(_safe(steering_accel, 0.0))
    acceleration_scale = max(params.acceleration_limit, 1.0e-6)
    allocation_gate = _clamp01(_safe(terminal_gate, 0.0))
    head_occupancy =
        allocation_gate *
        _clamp01(
            steering_direction * _safe(head_carrier_accel, 0.0) /
                acceleration_scale,
        )
    tail_occupancy =
        allocation_gate *
        _clamp01(
            steering_direction * _safe(tail_carrier_accel, 0.0) /
                acceleration_scale,
        )
    allocation_floor = clamp(params.steering_allocation_floor, 0.0, 1.0)
    head_score =
        params.head_steering_share *
        (allocation_floor + (1.0 - allocation_floor) * (1.0 - head_occupancy))
    tail_score =
        params.tail_steering_share *
        (allocation_floor + (1.0 - allocation_floor) * (1.0 - tail_occupancy))
    nominal_total =
        max(params.head_steering_share + params.tail_steering_share, 1.0e-6)
    score_total = max(head_score + tail_score, 1.0e-6)
    normalization = nominal_total / score_total
    return (
        head_share=head_score * normalization,
        tail_share=tail_score * normalization,
...
function target_policy(state, params)
    ...
    # Keep the evaluated intercept veto and outward-only carrier relief. The
    # added allocator does not increase total steering; it changes which joint
    # realizes that residual when one carrier is already using its directional
    ...
    head_carrier_accel = head_carrier_scale * drive.head_accel
    tail_carrier_accel = tail_carrier_scale * drive.tail_accel
    allocation = steering_reserve_allocation(
        head_carrier_accel,
        tail_carrier_accel,
        steering_accel,
        guidance.terminal_response_gate,
    ...
    head_accel =
        head_carrier_accel + allocation.head_share * steering_accel
    tail_accel =
        tail_carrier_accel + allocation.tail_share * steering_accel
