...
# propulsive traveling bend, and sparse speed reserve, then add a small
# terminal mean-curvature state servo below the carrier bandwidth. No clock,
# world coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_speed_reserve_mean_curvature_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        mean_curvature_bias_limit=5.0 * pi / 180,
        mean_curvature_frequency_ratio=0.30,
        mean_curvature_damping=0.12,
        mean_curvature_head_share=1.00,
        mean_curvature_tail_share=1.00,
...
@inline function terminal_mean_curvature_accel(
    guidance,
    response,
    drive,
    params,
)
    # The joint average is a physical low-pass state for mean bend. Tracking a
    # small target-signed offset below the carrier bandwidth supplies steering
    # that does not select or suppress a carrier half-cycle. Both the target
    # and the restoring feedback vanish outside the evidenced terminal gate or
    # when the existing yaw/geometry response has already released steering.
    authority =
        guidance.terminal_response_gate *
    ...
    target_curvature =
        params.mean_curvature_bias_limit * guidance.turn_command
    curvature_omega =
        params.mean_curvature_frequency_ratio * drive.omega
    curvature_accel =
        curvature_omega^2 * (target_curvature - mean_curvature) -
        2 * params.mean_curvature_damping * curvature_omega *
        mean_curvature_rate
    return authority * curvature_accel
end
...
function target_policy(state, params)
    ...
    # Keep the evaluated intercept veto, additive steering, and sparse relief
    # of outward carrier effort that is already unusable near both actuator
    # envelopes. Mean-curvature feedback is added separately below.
    ...
    steering_accel =
        ...
        (1.0 - response.release)
    mean_curvature_accel = terminal_mean_curvature_accel(
        guidance,
        response,
        drive,
        params,
    ...
    head_accel =
        ...
        params.mean_curvature_head_share * mean_curvature_accel
    tail_accel =
        tail_carrier_scale * drive.tail_accel +
        params.tail_steering_share * steering_accel +
        params.mean_curvature_tail_share * mean_curvature_accel
