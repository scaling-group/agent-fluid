...
# bend, and carrier-aligned terminal steering, then release route steering only
# when the measured velocity ray lies inside a conservative target collision
# corridor. No clock, world coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_course_servo_collision_corridor_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        collision_gate_start_distance_L=3.00,
        collision_gate_full_distance_L=1.25,
        collision_corridor_radius_L=0.60,
...
function guidance_module(state, params)
    ...
    raw_course_error = clamp(
        target_angle - course_angle,
        -params.course_angle_limit,
        params.course_angle_limit,
    )
    collision_span = max(
        params.collision_gate_start_distance_L -
            params.collision_gate_full_distance_L,
        1.0e-6,
    )
    collision_gate = _clamp01(
        (params.collision_gate_start_distance_L - distance_L) /
            collision_span,
    )
    collision_ratio = clamp(
        params.collision_corridor_radius_L / distance_L,
        0.0,
        0.95,
    )
    collision_closing_gate = _clamp01(
        closing_speed /
            max(params.progress_closing_speed_scale, 1.0e-6),
    )
    collision_half_angle =
        collision_gate *
        collision_closing_gate *
        asin(collision_ratio)
    course_error = copysign(
        max(abs(raw_course_error) - collision_half_angle, 0.0),
        raw_course_error,
    )
    route_error =
    ...
    return (
        ...
        collision_closing_gate=collision_closing_gate,
        collision_half_angle=collision_half_angle,
...
@inline function carrier_aligned_steering_scale(
    ...
    # Outside the approach region this is the inherited shared steering. At
    # full allocation the same impulse is moved onto the requested carrier
    # half-cycle; the propulsive carrier itself is never scaled.
    return (
        (1 - terminal_gate) +
        terminal_gate * aligned_pulse
    )
end
...
function target_policy(state, params)
    ...
    # Terminal collision-corridor guidance changes only whether route
    # steering is needed. When it is, the evidenced carrier-aligned allocator
    # places the same bounded steering impulse without suppressing propulsion.
    steering_accel =
