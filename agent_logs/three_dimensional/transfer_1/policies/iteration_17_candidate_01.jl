...
# controller, then brake joint-compensated yaw only when projected geometry
# already predicts an approaching terminal capture. No clock, world coordinate,
# route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_speed_reserve_terminal_yaw_brake_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        terminal_yaw_brake_limit=3.0,
        terminal_yaw_rate_scale=0.45,
...
@inline function terminal_yaw_brake(guidance, response, params)
    # The existing steering convention produces yaw opposite to the sign of
    # its acceleration residual. Apply a counter-yaw residual only after both
    # projected miss and approach alignment identify an approaching capture
    # corridor. The traveling-bend carrier is never attenuated here.
    capture_corridor_gate =
        response.intercept_distance_gate *
        response.projected_corridor_gate *
        response.approach_gate
    normalized_yaw =
        response.phase_compensated_turn_rate /
        max(params.terminal_yaw_rate_scale, 1.0e-6)
    return (
        capture_corridor_gate *
        params.terminal_yaw_brake_limit *
        tanh(normalized_yaw)
    )
end
...
function target_policy(state, params)
    ...
    # steering envelope. The new mechanism adds only a geometry-gated yaw
    # brake inside that same envelope.
    ...
    route_steering_accel =
        params.steering_accel_limit *
        guidance.turn_command *
        (1.0 - response.release)
    yaw_brake_accel = terminal_yaw_brake(guidance, response, params)
    steering_accel = clamp(
        route_steering_accel + yaw_brake_accel,
        -params.steering_accel_limit,
        params.steering_accel_limit,
    )
    head_accel =
