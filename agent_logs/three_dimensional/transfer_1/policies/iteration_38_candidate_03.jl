...
# add a bounded pre-pass anterior redirect only when adverse LOS response and
# poor approach alignment coincide. No clock, world coordinate, route, or
# mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_prepass_response_redirect_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        redirect_start_distance_L=1.60,
        redirect_full_distance_L=1.15,
        redirect_los_miss_start=0.35,
        redirect_los_miss_full=0.65,
        redirect_alignment_safe=0.60,
        redirect_alignment_full=0.25,
        redirect_accel_limit=6.0,
...
function target_policy(state, params)
    ...
    # Keep the evaluated intercept veto, baseline additive steering, and sparse
    # carrier reserve. The new redirect is a separate response residual: it
    # acts before a predicted miss and releases when either LOS response or
    # target-directed approach recovers. It does not alter posterior drive.
    ...
    redirect_distance_gate = _smoothstep01(
        (params.redirect_start_distance_L - guidance.distance_L) /
            redirect_distance_span,
    )
    redirect_los_span = max(
        params.redirect_los_miss_full - params.redirect_los_miss_start,
        1.0e-6,
    )
    redirect_los_gate = _smoothstep01(
        (response.signed_los_miss - params.redirect_los_miss_start) /
            redirect_los_span,
    )
    redirect_alignment_span = max(
        params.redirect_alignment_safe - params.redirect_alignment_full,
        1.0e-6,
    )
    redirect_alignment_gate = _smoothstep01(
        (params.redirect_alignment_safe - guidance.approach_alignment) /
            redirect_alignment_span,
    )
    redirect_gate =
        redirect_distance_gate *
        redirect_los_gate *
        redirect_alignment_gate
    anterior_redirect =
        params.redirect_accel_limit *
        guidance.turn_command *
        redirect_gate
    head_accel =
