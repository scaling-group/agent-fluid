...
# controller, then conserve total steering while conditionally moving burden
# from the propulsive posterior joint to the anterior joint. No clock, world
# coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_conditional_pressure_transfer_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        conditional_steering_transfer=0.55,
...
@inline function actuator_steering_pressure(
    joint_velocity,
    previous_action,
    steering_accel,
    params,
)
    # Pressure combines normalized speed and the previous returned action.
    # The directional gate is positive only when this steering residual would
    # drive the joint farther along its current motion.
    speed_ratio =
        abs(joint_velocity) / max(params.joint_speed_limit, 1.0e-6)
    action_ratio =
        abs(previous_action) / max(params.acceleration_limit, 1.0e-6)
    ...
    pressure = 0.5 * (speed_gate + action_gate)
    normalized_residual_outward =
        joint_velocity * steering_accel /
        max(
            params.joint_speed_limit * params.acceleration_limit,
            1.0e-6,
        )
    ...
    return (pressure=pressure, outward_gate=outward_gate)
end
...
function target_policy(state, params)
    ...
    # carrier reserve. Transfer is zero unless posterior pressure exceeds
    # anterior pressure, tail steering is outward, and the head has margin.
    ...
    head_pressure = actuator_steering_pressure(
        drive.qd1,
        previous_head_action,
        steering_accel,
        params,
    )
    tail_pressure = actuator_steering_pressure(
        drive.qd2,
        previous_tail_action,
        steering_accel,
        params,
    ...
    posterior_pressure_excess = max(
        tail_pressure.pressure - head_pressure.pressure,
        0.0,
    )
    anterior_margin = 1.0 - head_pressure.pressure
    steering_transfer =
        min(
            max(params.conditional_steering_transfer, 0.0),
            max(params.tail_steering_share, 0.0),
        ) *
        unsafe_terminal_gate *
        tail_pressure.outward_gate *
        posterior_pressure_excess *
        anterior_margin
    head_steering_share =
