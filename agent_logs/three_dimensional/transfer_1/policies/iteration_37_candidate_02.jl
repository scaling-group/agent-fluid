...
# traveling bend, and speed-reserve allocation, but make the terminal LOS
# response consistent with the head point used by target distance and capture.
# No clock, world coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_head_consistent_los_response_v1",
        drive_period=0.55,
        control_period=0.55,
...
function guidance_module(state, params)
    ...
    center_velocity_los_rate =
        -target_velocity_cross / max(los_distance^2, 1.0e-6)

    # distance_L and target_body_L are measured from the head, so their LOS
    # response must use the same point. The body-frame target derivative plus
    # body rotation reconstructs inertial head-relative target motion. Keep
    # the center-velocity expression only as a compatibility fallback.
    has_head_target_rate = hasproperty(state, :target_body_rate_L)
    target_rate = has_head_target_rate ?
        state.target_body_rate_L : (0.0, 0.0)
    target_rate_x = _safe(target_rate[1], 0.0)
    target_rate_y = _safe(target_rate[2], 0.0)
    heading_rate = hasproperty(state, :heading_rate) ?
        _safe(state.heading_rate, 0.0) :
        _safe(state.turn_rate_recent, 0.0)
    target_radius_squared = target_x^2 + target_y^2
    head_target_los_rate =
        (
            target_x * target_rate_y - target_y * target_rate_x +
            heading_rate * target_radius_squared
        ) / max(los_distance^2, 1.0e-6)
    inertial_los_rate = has_head_target_rate ?
        head_target_los_rate : center_velocity_los_rate
    ...
    return (
        ...
        center_velocity_los_rate=center_velocity_los_rate,
        head_target_los_rate=head_target_los_rate,
...
function target_policy(state, params)
    ...
    # carrier reserve. The only changed mechanism is the head-consistent LOS
    # observation consumed by the response gate above.
