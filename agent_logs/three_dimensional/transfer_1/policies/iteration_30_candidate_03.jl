# L64 3D candidate: retain the intercept-guarded achieved-course servo,
# propulsive traveling bend, and sparse speed reserve. When safe response-based
# release opens, use only a bounded part of that released authority to damp
# measured body-relative slip. No clock, world coordinate, route, or mutable
# phase is used.

function target_policy_params()
    return (
        version="dogfish3d_released_relative_slip_hold_v1",
        drive_period=0.55,
        control_period=0.55,
...
function guidance_module(state, params)
    ...
    # Relative flow supplies a body-frame hydrodynamic slip observation. In
    # quiescent fluid it is dominated by achieved translation, while the
    # measured local-flow term retains wake-induced deviations. The speed gate
    # makes this observation vanish when translation is not informative.
    relative_flow = hasproperty(state, :relative_flow_velocity_body_U) ?
        state.relative_flow_velocity_body_U : (
            _safe(state.local_flow_velocity_body_U[1], 0.0) - velocity_x,
            _safe(state.local_flow_velocity_body_U[2], 0.0) - velocity_y,
        )
    relative_flow_x = _safe(relative_flow[1], 0.0)
    relative_flow_y = _safe(relative_flow[2], 0.0)
    relative_flow_speed = hypot(relative_flow_x, relative_flow_y)
    slip_angle = clamp(
        -atan(
            relative_flow_y,
            max(relative_flow_x, params.course_forward_floor),
        ),
        -params.course_angle_limit,
        params.course_angle_limit,
    )
    ...
    slip_command =
        slip_gate *
        tanh(slip_angle / max(params.route_error_scale, 1.0e-6))
    ...
    return (
        ...
        relative_flow_speed=relative_flow_speed,
        slip_angle=slip_angle,
        slip_gate=slip_gate,
        slip_command=slip_command,
...
function target_policy(state, params)
    ...
    # Keep the evaluated intercept veto and full additive steering. The new
    # mechanism changes only outward carrier effort that the observed actuator
    # state says is already unusable near both envelopes.
    ...
    # Unsafe interception retains the evaluated target command exactly. Only
    # the authority already released after compatible yaw response is blended
    # into slip centering, so this mechanism cannot increase the normalized
    # steering envelope or suppress the traveling carrier.
    released_slip_command =
        clamp(params.released_slip_share, 0.0, 1.0) *
        guidance.slip_command
    steering_command =
        (1.0 - response.release) * guidance.turn_command +
        response.release * released_slip_command
    steering_accel =
        params.steering_accel_limit *
        clamp(steering_command, -1.0, 1.0)
    head_accel =
