...
# propulsive traveling bend, then open the evaluated sparse carrier reserve
# earlier only when target-relative alignment is unsafe. No clock, world
# coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_geometry_gated_midcourse_reserve_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        midcourse_reserve_start_distance_L=6.00,
        midcourse_reserve_full_distance_L=5.00,
        midcourse_reserve_handoff_start_distance_L=4.00,
        midcourse_reserve_handoff_full_distance_L=3.00,
        reserve_alignment_start=0.90,
        reserve_alignment_full=0.82,
...
@inline function reserve_context_gate(guidance, params)
    # The inherited terminal gate remains unchanged. In the evidenced middle
    # band, poor target/velocity alignment may enable the same sparse reserve;
    # all joint-speed, previous-action, and carrier-direction guards still
    # apply inside carrier_reserve_scale.
    outer_span = max(
        params.midcourse_reserve_start_distance_L -
            params.midcourse_reserve_full_distance_L,
        1.0e-6,
    )
    outer_gate = _smoothstep01(
        (params.midcourse_reserve_start_distance_L - guidance.distance_L) /
            outer_span,
    )
    handoff_span = max(
        params.midcourse_reserve_handoff_start_distance_L -
            params.midcourse_reserve_handoff_full_distance_L,
        1.0e-6,
    )
    handoff_gate = _smoothstep01(
        (guidance.distance_L -
            params.midcourse_reserve_handoff_full_distance_L) /
            handoff_span,
    )
    alignment_span = max(
        params.reserve_alignment_start - params.reserve_alignment_full,
        1.0e-6,
    )
    unsafe_alignment_gate = _smoothstep01(
        (params.reserve_alignment_start - guidance.approach_alignment) /
            alignment_span,
    )
    midcourse_gate = outer_gate * handoff_gate * unsafe_alignment_gate
    return max(guidance.terminal_response_gate, midcourse_gate)
end
...
function target_policy(state, params)
    ...
    # Keep the evaluated intercept veto and full additive steering. Geometry
    # can open the reserve before 4L, but only the existing actuator-state
    # guards can actually soften outward carrier effort.
    response = turn_response_module(state, guidance, drive, params)
    reserve_gate = reserve_context_gate(guidance, params)
