...
# a bounded derivative steering residual only when windowed body-frame target
# bearing is diverging inside the intercept region. No clock, world coordinate,
# route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_bearing_divergence_residual_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        bearing_divergence_start=0.030,
        bearing_divergence_full=0.080,
        bearing_direction_scale=0.20,
        bearing_divergence_accel_limit=4.0,
...
function turn_response_module(state, guidance, drive, params)
    ...
    # Normalize target-image divergence by the observed carrier rate so the
    # qualifier is dimensionless and tied to state rather than a clock. The
    # product is reflection invariant; target bearing supplies the equivariant
    # correction direction without changing the route command.
    bearing_angle = hasproperty(state, :bearing) ?
        _safe(state.bearing, guidance.target_angle) :
        guidance.target_angle
    bearing_window_rate = hasproperty(state, :bearing_window_rate) ?
        _safe(state.bearing_window_rate, 0.0) :
        _safe(state.bearing_rate, 0.0)
    normalized_bearing_divergence = max(
        bearing_angle * bearing_window_rate /
            max(drive.omega, eps(Float64)),
        0.0,
    )
    divergence_span = max(
        params.bearing_divergence_full -
            params.bearing_divergence_start,
        1.0e-6,
    )
    bearing_divergence_gate = _smoothstep01(
        (normalized_bearing_divergence -
            params.bearing_divergence_start) /
            divergence_span,
    )
    bearing_direction = tanh(
        bearing_angle /
            max(params.bearing_direction_scale, 1.0e-6),
    )
    bearing_divergence_accel =
        params.bearing_divergence_accel_limit *
        intercept_distance_gate *
        bearing_divergence_gate *
        bearing_direction
    ...
    return (
        ...
        normalized_bearing_divergence=normalized_bearing_divergence,
        bearing_divergence_gate=bearing_divergence_gate,
        bearing_direction=bearing_direction,
        bearing_divergence_accel=bearing_divergence_accel,
...
function target_policy(state, params)
    ...
    # Keep the evaluated route, release logic, spatial allocation, and sparse
    # carrier reserve. The new bounded residual uses only pre-separation target
    # motion and cannot alter the carrier directly.
    ...
    steering_accel =
        ...
        response.bearing_divergence_accel
    head_accel =
