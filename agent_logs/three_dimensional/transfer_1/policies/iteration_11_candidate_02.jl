# L64 3D candidate: preserve the captured LOS-guarded achieved-course servo,
# but allow response-based steering release only when current body-frame
# target/velocity geometry projects to an approaching intercept corridor. The
# carrier is never attenuated. No clock, world route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_course_servo_intercept_guarded_release_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        drive_frequency_scale_floor=0.50,
        ...
        intercept_speed_floor=0.08,
        intercept_approach_alignment_full=0.25,
        intercept_pass_inner_L=0.60,
        intercept_pass_outer_L=1.00,
...
function drive_module(state, params, drive_frequency_scale=1.0)
    omega =
        ...
        max(
            ...
            params.drive_frequency_scale_floor,
        )
...
function guidance_module(state, params)
    ...
    # The cross product is invariant to body-frame rotation. Dividing by
    # range squared yields inertial line-of-sight rate without beat yaw.
    ...
    # Project current translation onto the target ray. A positive dot product
    # means approach; |r x v|/|v| is the closest-pass distance of that ray.
    # These are normalized, body-frame geometric tests, not a memorized route.
    intercept_speed = max(course_speed, params.intercept_speed_floor)
    approach_alignment = clamp(
        (target_x * velocity_x + target_y * velocity_y) /
            max(distance_L * intercept_speed, 1.0e-6),
        -1.0,
        1.0,
    )
    projected_pass_distance_L =
        abs(target_velocity_cross) / intercept_speed
    ...
    intercept_distance_gate = _smoothstep01(
        (params.intercept_guard_start_distance_L - distance_L) /
            intercept_distance_span,
    )
    approach_compatibility = _smoothstep01(
        approach_alignment /
            max(params.intercept_approach_alignment_full, 1.0e-6),
    )
    pass_span = max(
        params.intercept_pass_outer_L - params.intercept_pass_inner_L,
        1.0e-6,
    )
    pass_compatibility = 1.0 - _smoothstep01(
        (projected_pass_distance_L - params.intercept_pass_inner_L) /
            pass_span,
    )
    intercept_compatibility =
        (1.0 - intercept_distance_gate) +
        intercept_distance_gate * approach_compatibility * pass_compatibility
    ...
    return (
        ...
        projected_pass_distance_L=projected_pass_distance_L,
        intercept_distance_gate=intercept_distance_gate,
        approach_compatibility=approach_compatibility,
        pass_compatibility=pass_compatibility,
        intercept_compatibility=intercept_compatibility,
    )
end
...
function turn_response_module(state, guidance, drive, params)
    ...
    # Correct-sign slow yaw may release steering only while LOS rate and
    # projected inertial motion both remain compatible with interception.
    ...
    release =
        ...
        response_gate *
        (1.0 - los_miss_gate) *
        guidance.intercept_compatibility
    return (
...
function target_policy(state, params)
    ...
    # The new guard can only retain existing shared steering; it cannot add
    # authority or modify either half of the propulsive carrier.
