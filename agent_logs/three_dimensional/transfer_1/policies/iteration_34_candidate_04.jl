# L64 3D candidate: retain the intercept-guarded speed-reserve carrier, then
# add a progress-triggered recovery bend that releases on measured correct-sign
# yaw response. No clock, world coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_response_released_recovery_bend_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        recovery_closing_speed_start=0.10,
        recovery_closing_speed_full=-0.10,
        recovery_curvature_limit=12.0 * pi / 180,
        recovery_frequency_scale=0.45,
        recovery_damping=0.85,
        recovery_accel_limit=12.0,
...
function response_released_recovery(guidance, response, drive, params)
    # Sampled captures stay above +0.20L/T closing speed below 4L. A stalled
    # or opening terminal pass admits a bounded C-bend, but correct-sign yaw
    # response releases it immediately so the traveling carrier cannot be
    # held in the quasi-static state observed for the prior recovery servo.
    closing_span = max(
        params.recovery_closing_speed_start -
            params.recovery_closing_speed_full,
        1.0e-6,
    )
    progress_loss_gate = _smoothstep01(
        (params.recovery_closing_speed_start -
            guidance.closing_speed_L) /
            closing_span,
    )
    response_release = response.response_gate
    recovery_gate =
        guidance.terminal_response_gate *
        progress_loss_gate *
        (1.0 - response_release)

    mean_curvature = 0.5 * (drive.q1 + drive.q2)
    mean_curvature_rate = 0.5 * (drive.qd1 + drive.qd2)
    target_curvature =
        params.recovery_curvature_limit * guidance.turn_command
    recovery_omega =
        max(params.recovery_frequency_scale, 0.0) * drive.omega
    curvature_accel = clamp(
        recovery_omega^2 * (target_curvature - mean_curvature) -
            2 * params.recovery_damping * recovery_omega *
            mean_curvature_rate,
        -params.recovery_accel_limit,
        params.recovery_accel_limit,
    )
    return (
        progress_loss_gate=progress_loss_gate,
...
function target_policy(state, params)
    ...
    # Keep the evaluated intercept veto, sparse carrier reserve, and successful
    # first approach. A lost-progress bend may replace only the steering
    # residual, and measured correct-sign yaw restores baseline steering and
    # the unchanged differential carrier before the bend can persist.
    response = turn_response_module(state, guidance, drive, params)
    recovery = response_released_recovery(
        guidance,
        response,
        drive,
        params,
    ...
    baseline_head_steering =
        params.head_steering_share * steering_accel
    baseline_tail_steering =
        params.tail_steering_share * steering_accel
    head_steering =
        (1.0 - recovery.recovery_gate) * baseline_head_steering +
        recovery.recovery_gate * recovery.curvature_accel
    tail_steering =
        (1.0 - recovery.recovery_gate) * baseline_tail_steering +
        recovery.recovery_gate * recovery.curvature_accel
    head_accel =
        head_carrier_scale * drive.head_accel + head_steering
    tail_accel =
        tail_carrier_scale * drive.tail_accel + tail_steering
