# L64 3D candidate: retain the repeatable intercept-guarded speed-reserve
# controller, then allocate terminal steering across the observed carrier
# half-cycle so it is not spent into same-sign clipping. No clock, world
# coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_speed_reserve_half_cycle_steering_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        steering_phase_contrast=0.45,
        steering_phase_scale=0.35,
...
@inline function steering_half_cycle_weight(
    turn_command,
    carrier_accel,
    intercept_gate,
    params,
)
    # Reflection preserves the product of requested turn and carrier sign.
    # Shift steering away from carrier-aligned acceleration that is prone to
    # same-sign clipping and toward the opposing half-cycle, without changing
    # the far-field controller or suppressing the carrier itself.
    carrier_ratio = clamp(
        carrier_accel / max(params.acceleration_limit, 1.0e-6),
        -1.0,
        1.0,
    )
    signed_alignment = turn_command * carrier_ratio
    phase_side = tanh(
        signed_alignment / max(params.steering_phase_scale, 1.0e-6),
    )
    contrast = clamp(params.steering_phase_contrast, 0.0, 0.85)
    return 1.0 - _clamp01(intercept_gate) * contrast * phase_side
end
...
function target_policy(state, params)
    ...
    # Keep the evaluated route error, intercept veto, carrier reserve, and
    # steering magnitude. The new mechanism changes only when each joint
    # receives that steering residual inside the existing intercept gate.
    ...
    head_steering_weight = steering_half_cycle_weight(
        guidance.turn_command,
        drive.head_accel,
        response.intercept_distance_gate,
        params,
    )
    tail_steering_weight = steering_half_cycle_weight(
        guidance.turn_command,
        drive.tail_accel,
        response.intercept_distance_gate,
        params,
    ...
    head_accel =
        ...
        params.head_steering_share * head_steering_weight * steering_accel
    tail_accel =
        tail_carrier_scale * drive.tail_accel +
        params.tail_steering_share * tail_steering_weight * steering_accel
