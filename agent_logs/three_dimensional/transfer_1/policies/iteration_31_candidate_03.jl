...
# transfer unsafe-terminal steering toward the anterior joint only when the
# posterior actuator has greater outward burden and the anterior has margin.
# Total steering authority stays constant. No clock, route, or mutable phase.

function target_policy_params()
    return (
        version="dogfish3d_burden_conditioned_steering_transfer_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        allocation_burden_advantage_start=0.05,
        allocation_burden_advantage_full=0.25,
...
@inline function outward_actuator_burden(
    joint_velocity,
    previous_action,
    params,
)
    # Burden is not absolute activity: it requires speed and previous action
    # near their envelopes and the action pushing farther in the speed sign.
    speed_ratio =
        abs(joint_velocity) / max(params.joint_speed_limit, 1.0e-6)
    action_ratio =
        abs(previous_action) / max(params.acceleration_limit, 1.0e-6)
    ...
    previous_outward_gate = tanh(
        max(normalized_previous_outward, 0.0) /
        max(params.reserve_outward_scale, 1.0e-6),
    )
    return speed_gate * action_gate * previous_outward_gate
end
...
function target_policy(state, params)
    ...
    # carrier reserve. Spatial allocation changes only when observations show
    # unusable posterior authority and usable anterior margin.
    ...
    head_burden = outward_actuator_burden(
        drive.qd1,
        previous_head_action,
        params,
    )
    tail_burden = outward_actuator_burden(
        drive.qd2,
        previous_tail_action,
        params,
    )
    posterior_burden_advantage = max(tail_burden - head_burden, 0.0)
    burden_advantage_span = max(
        params.allocation_burden_advantage_full -
            params.allocation_burden_advantage_start,
        1.0e-6,
    )
    posterior_burden_gate = _smoothstep01(
        (posterior_burden_advantage -
            params.allocation_burden_advantage_start) /
            burden_advantage_span,
    )
    anterior_margin = 1.0 - head_burden
    normalized_tail_steering_outward =
        drive.qd2 * steering_accel /
        max(
            params.joint_speed_limit * params.steering_accel_limit,
            1.0e-6,
        )
    tail_steering_outward_gate = tanh(
        max(normalized_tail_steering_outward, 0.0) /
        max(params.reserve_outward_scale, 1.0e-6),
    )
    steering_transfer =
        min(
        ...
        unsafe_terminal_gate *
        posterior_burden_gate *
        anterior_margin *
        tail_steering_outward_gate
    head_steering_share =
