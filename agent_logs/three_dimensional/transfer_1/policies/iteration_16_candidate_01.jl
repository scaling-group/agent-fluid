...
# carrier, then add posterior mean-curvature recovery only after an observed
# near-target receding pass. No clock, world coordinate, route, or mutable
# phase is used.

function target_policy_params()
    return (
        version="dogfish3d_speed_reserve_postpass_curvature_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        recovery_start_distance_L=2.50,
        recovery_full_distance_L=1.50,
        recovery_receding_start=0.04,
        recovery_receding_full=0.18,
        recovery_alignment_start=0.05,
        recovery_alignment_full=0.20,
        recovery_route_error_start=0.45,
        recovery_route_error_full=0.90,
        recovery_tail_curvature=6.0 * pi / 180,
...
function guidance_module(state, params)
    ...
    # A recovery request exists only after a nearby miss: range must be
    # increasing both over the observation window and instantaneously, while
    # target/course misalignment remains large. Approaching captures therefore
    # retain the evaluated controller exactly.
    recovery_distance_span = max(
        params.recovery_start_distance_L -
            params.recovery_full_distance_L,
        1.0e-6,
    )
    recovery_distance_gate = _smoothstep01(
        (params.recovery_start_distance_L - distance_L) /
            recovery_distance_span,
    )
    recovery_receding_span = max(
        params.recovery_receding_full - params.recovery_receding_start,
        1.0e-6,
    )
    recovery_receding_gate = _smoothstep01(
        (-closing_speed - params.recovery_receding_start) /
            recovery_receding_span,
    )
    recovery_alignment_span = max(
        params.recovery_alignment_full - params.recovery_alignment_start,
        1.0e-6,
    )
    recovery_alignment_gate = _smoothstep01(
        (-approach_alignment - params.recovery_alignment_start) /
            recovery_alignment_span,
    )
    recovery_route_span = max(
        params.recovery_route_error_full -
            params.recovery_route_error_start,
        1.0e-6,
    )
    recovery_route_gate = _smoothstep01(
        (abs(route_error) - params.recovery_route_error_start) /
            recovery_route_span,
    )
    recovery_gate =
        recovery_distance_gate *
        recovery_receding_gate *
        recovery_alignment_gate *
        recovery_route_gate
    ...
    return (
        ...
        recovery_distance_gate=recovery_distance_gate,
        recovery_receding_gate=recovery_receding_gate,
        recovery_alignment_gate=recovery_alignment_gate,
        recovery_route_gate=recovery_route_gate,
...
function target_policy(state, params)
    ...
    # Keep the evaluated intercept veto, full additive steering, and sparse
    # outward-carrier reserve. Recovery is computed separately and remains
    # dormant while the fish is approaching the capture disk.
    ...
    # Translate the post-pass redirect into posterior mean curvature. Keeping
    # this target-signed term outside carrier relief preserves both restoring
    # carrier effort and the recovery authority around its shifted mean.
    recovery_tail_accel =
        drive.omega^2 *
        params.recovery_tail_curvature *
        guidance.turn_command *
        guidance.recovery_gate
    head_accel =
