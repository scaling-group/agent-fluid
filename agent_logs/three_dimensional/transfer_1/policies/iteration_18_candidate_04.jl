...
# speed-reserve carrier, then brake only an already-correct terminal yaw
# response inside the projected capture corridor. No clock, world coordinate,
# route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_intercept_speed_reserve_response_brake_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        terminal_response_brake_limit=2.40,
...
function turn_response_module(state, guidance, drive, params)
    ...
    return (
        ...
        intercept_compatible=intercept_compatible,
...
function target_policy(state, params)
    ...
    # Preserve the evaluated intercept veto, sparse outward-carrier reserve,
    # and phase-independent additive steering. The new brake is separate from
    # the carrier and acts only on an already-correct yaw response while the
    # projected pass is both approaching and inside the capture corridor.
    ...
    response_brake_accel =
        -params.terminal_response_brake_limit *
        guidance.turn_command *
        guidance.terminal_response_gate *
        response.response_gate *
        response.intercept_distance_gate *
        response.intercept_compatible
    net_steering_accel = steering_accel + response_brake_accel
    head_accel =
        head_carrier_scale * drive.head_accel +
        params.head_steering_share * net_steering_accel
    tail_accel =
        tail_carrier_scale * drive.tail_accel +
        params.tail_steering_share * net_steering_accel
