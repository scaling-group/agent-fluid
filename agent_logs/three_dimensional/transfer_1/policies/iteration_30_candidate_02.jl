...
# traveling bend, but prevent target-aligned sideslip from making the route
# request vanish while the body axis remains misaligned. No clock, world
# coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_course_aligned_slip_stabilizer_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        slip_alignment_start=0.94,
        slip_alignment_full=0.995,
        slip_angle_scale=0.30,
        slip_error_limit=0.18,
...
function guidance_module(state, params)
    ...
    # Achieved course remains the route signal. If that course already points
    # at the target, retain a bounded course-angle residual so body sideslip
    # cannot masquerade as completed steering. It fades with low speed or any
    # loss of target/velocity alignment and never modifies the carrier.
    slip_alignment_span = max(
        params.slip_alignment_full - params.slip_alignment_start,
        1.0e-6,
    )
    slip_alignment_gate = _smoothstep01(
        (approach_alignment - params.slip_alignment_start) /
            slip_alignment_span,
    )
    slip_error =
        params.slip_error_limit *
        course_gate *
        slip_alignment_gate *
        tanh(course_angle / max(params.slip_angle_scale, 1.0e-6))
    route_error =
    ...
    return (
        ...
        slip_alignment_gate=slip_alignment_gate,
