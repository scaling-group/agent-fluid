# L64 3D candidate: retain the achieved-course servo, traveling bend, and
# sparse carrier reserve, but release steering only when projected approach
# geometry certifies a pass through the capture corridor. No clock, world
# coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_capture_corridor_certified_release_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        capture_corridor_L=0.75,
        capture_release_margin_L=0.15,
...
function turn_response_module(state, guidance, drive, params)
    ...
    # projected closest-pass geometry remain compatible with capture.
    ...
    # A distance-delayed veto reacted only after inherited failures had opened
    # an unrecoverable miss. Instead, certify release against the normalized
    # capture corridor everywhere that the existing terminal gate is active.
    # The inward margin makes the transition continuous without treating a
    # threshold-touching projection as robust interception.
    projected_corridor_gate = _smoothstep01(
        (params.capture_corridor_L -
            guidance.projected_miss_distance_L) /
            max(params.capture_release_margin_L, 1.0e-6),
    )
    approach_span = max(
    ...
    intercept_release_guard = intercept_compatible
    release =
...
function target_policy(state, params)
    ...
    # Keep full additive steering unless the response module certifies a safe
    # projected pass. Carrier actuation and its sparse speed reserve are
    # otherwise unchanged.
