# B-test seed derived from the iteration-20 2D champion control law, with the
# 3D evaluator's normalized observation fields used directly below.
# Source policy SHA256 (LF artifact bytes): ff36d930636985ae6259d28d502b0532ff78bef48f090cf447ae11c33dfd441a
# The target geometry ratios are algebraically identical to the champion's
# cell-unit formulas without retaining grid resolution in the policy schema.

function target_policy_params()
    return (
        version="dogfish_target_control_v19_course_residual",
        drive_period=0.55,
        control_period=0.55,
...
function guidance_module(state, params)
    ...
    # A free swimmer can translate far across its body axis while yaw inertia
    # keeps the target visually centered.  Compare the achieved velocity course
    # with the target direction, but fade this residual smoothly from rest where
    # velocity direction is undefined.  Body-frame -x is the forward axis.
