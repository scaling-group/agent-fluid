# L64 3D candidate: retain the intercept-guarded achieved-course servo and
# propulsive traveling bend, then add a geometry-gated common-mode redirect
# when terminal course and alignment jointly predict a missed corridor. No
# clock, world coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_alignment_gated_common_mode_redirect_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        redirect_start_distance_L=4.75,
        redirect_full_distance_L=3.25,
        redirect_inner_release_start_distance_L=1.80,
        redirect_inner_release_full_distance_L=1.20,
        redirect_course_start=0.35,
        redirect_course_full=0.80,
        redirect_alignment_start=0.90,
        redirect_alignment_full=0.55,
        redirect_accel_limit=18.0,
...
function common_mode_redirect_module(guidance, params)
    # A large course error can be a harmless carrier-scale oscillation. Require
    # it to agree with deteriorating inertial target/velocity alignment before
    # changing the evaluated steering realization. The outer range gate keeps
    # far-field acquisition untouched; an inner fade hands a recovered close
    # pass back to the evaluated capture controller.
    distance_span = max(
    ...
    distance_gate = _smoothstep01(
        (params.redirect_start_distance_L - guidance.distance_L) /
            distance_span,
    )
    inner_release_span = max(
        params.redirect_inner_release_start_distance_L -
            params.redirect_inner_release_full_distance_L,
        1.0e-6,
    )
    inner_release_gate = _smoothstep01(
        (guidance.distance_L -
            params.redirect_inner_release_full_distance_L) /
            inner_release_span,
    )
    course_span = max(
        params.redirect_course_full - params.redirect_course_start,
        1.0e-6,
    )
    course_gate = _smoothstep01(
        (abs(guidance.course_error) - params.redirect_course_start) /
            course_span,
    )
    alignment_span = max(
        params.redirect_alignment_start - params.redirect_alignment_full,
        1.0e-6,
    )
    alignment_gate = _smoothstep01(
        (params.redirect_alignment_start - guidance.approach_alignment) /
            alignment_span,
    )
    gate =
        distance_gate *
        ...
        alignment_gate
    redirect_accel =
        params.redirect_accel_limit * guidance.turn_command
    return (
        distance_gate=distance_gate,
        inner_release_gate=inner_release_gate,
        course_gate=course_gate,
        alignment_gate=alignment_gate,
        gate=gate,
        redirect_accel=redirect_accel,
    )
end
...
function target_policy(state, params)
    ...
    # Keep the evaluated intercept veto and speed reserve. If approach geometry
    # jointly indicates a missed corridor, smoothly replace the usual unequal
    # steering shares with equal common-mode acceleration. The differential
    # traveling-bend carrier remains active throughout the redirect.
    response = turn_response_module(state, guidance, drive, params)
    redirect = common_mode_redirect_module(guidance, params)
    ...
    head_steering_accel =
        ...
        base_steering_accel +
        redirect.gate * redirect.redirect_accel
    tail_steering_accel =
        (1.0 - redirect.gate) *
