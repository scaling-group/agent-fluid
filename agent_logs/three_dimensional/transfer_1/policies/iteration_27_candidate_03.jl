# L64 3D candidate: retain the evaluated intercept-guarded speed-reserve
# controller, then let joint state qualify gait-sensitive projected-motion
# feedback before the existing intercept guard. No clock, world coordinate,
# route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_phase_qualified_projection_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        projection_phase_speed_full=0.20,
        projection_phase_speed_ignore=0.55,
...
function turn_response_module(state, guidance, drive, params)
    ...
    # Instantaneous projected motion contains the evidenced carrier-synchronous
    # sway. Before the original intercept guard, trust that observation only
    # near anterior-joint reversal; joint state qualifies the observation but
    # never reallocates carrier or steering effort between half-cycles.
    ...
    phase_speed_span = max(
        params.projection_phase_speed_ignore -
            params.projection_phase_speed_full,
        1.0e-6,
    )
    projection_reliability = _smoothstep01(
        (params.projection_phase_speed_ignore - normalized_phase_speed) /
            phase_speed_span,
    )
    phase_qualified_pre_guard =
        1.0 - projection_reliability * (1.0 - intercept_compatible)
    intercept_release_guard =
        (1.0 - intercept_distance_gate) * phase_qualified_pre_guard +
    ...
    return (
        ...
        normalized_phase_speed=normalized_phase_speed,
        projection_reliability=projection_reliability,
        phase_qualified_pre_guard=phase_qualified_pre_guard,
...
function target_policy(state, params)
    ...
    # Keep the evaluated additive steering and sparse carrier reserve. The new
    # observation-confidence guard is zero-effect outside the existing
    # terminal response region and never modifies the traveling-wave carrier.
