...
# propulsive traveling bend, and carrier reserve, then add a bounded
# opening-pass burst redirect from normalized body-frame feedback. No clock,
# world coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_opening_pass_burst_recovery_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        recovery_start_distance_L=3.00,
        recovery_full_distance_L=2.25,
        recovery_opening_start=0.03,
        recovery_opening_full=0.16,
        recovery_error_start=0.55,
        recovery_error_full=0.95,
        recovery_accel_limit=10.0,
        recovery_head_share=1.00,
        recovery_tail_share=1.00,
...
function opening_recovery_module(guidance, params)
    # A close pass that has begun opening is a different control state from a
    # still-closing approach. Combine range, signed progress, and body-frame
    # route error so successful closing trajectories retain the exact baseline
    # command while an opening miss receives transient mean curvature.
    distance_span = max(
    ...
    distance_gate = _smoothstep01(
        (params.recovery_start_distance_L - guidance.distance_L) /
            distance_span,
    )
    opening_span = max(
        params.recovery_opening_full - params.recovery_opening_start,
        1.0e-6,
    )
    opening_gate = _smoothstep01(
        (-guidance.closing_speed_L - params.recovery_opening_start) /
            opening_span,
    )
    error_span = max(
        params.recovery_error_full - params.recovery_error_start,
        1.0e-6,
    )
    error_gate = _smoothstep01(
        (abs(guidance.route_error) - params.recovery_error_start) /
            error_span,
    )
    recovery_gate = distance_gate * opening_gate * error_gate
    recovery_accel =
...
function target_policy(state, params)
    ...
    # burst mechanism remains identically zero on a closing approach and adds
    # bounded mean curvature only after an evidenced opening near pass.
    response = turn_response_module(state, guidance, drive, params)
    recovery = opening_recovery_module(guidance, params)
    ...
    head_accel =
        ...
        params.recovery_head_share * recovery.recovery_accel
    tail_accel =
        tail_carrier_scale * drive.tail_accel +
        params.tail_steering_share * steering_accel +
        params.recovery_tail_share * recovery.recovery_accel
