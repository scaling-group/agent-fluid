...
# propulsive traveling bend, then blend terminal route feedback toward direct
# body-frame target bearing before the evidenced lower-branch divergence. No
# clock, world coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_speed_reserve_terminal_bearing_blend_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        terminal_bearing_start_distance_L=6.00,
        terminal_bearing_full_distance_L=3.00,
...
function guidance_module(state, params)
    ...
    achieved_course_route_error =
        (1 - course_gate) * target_angle +
        course_gate * course_error
    terminal_bearing_span = max(
        params.terminal_bearing_start_distance_L -
            params.terminal_bearing_full_distance_L,
        1.0e-6,
    )
    terminal_bearing_fraction = _clamp01(
        (params.terminal_bearing_start_distance_L - distance_L) /
            terminal_bearing_span,
    )
    terminal_bearing_gate =
        terminal_bearing_fraction^2 *
        (3 - 2 * terminal_bearing_fraction)
    route_error =
        (1 - terminal_bearing_gate) * achieved_course_route_error +
        terminal_bearing_gate * target_angle
    turn_command =
    ...
    return (
        ...
        achieved_course_route_error=achieved_course_route_error,
        terminal_bearing_gate=terminal_bearing_gate,
...
function target_policy(state, params)
    ...
    # outward-carrier reserve. Only the distance-scheduled route observation
    # above differs from the repeat-backed policy.
