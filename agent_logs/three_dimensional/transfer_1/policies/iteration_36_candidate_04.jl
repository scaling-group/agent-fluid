...
# add a bounded target-signed burst redirect only after terminal progress is
# observably lost. No clock, world coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_progress_loss_burst_redirect_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        recovery_full_distance_L=2.40,
        recovery_closing_loss_start=0.03,
        recovery_closing_loss_full=0.16,
        recovery_bend_amplitude=20.0 * pi / 180,
        recovery_natural_frequency=4.0,
        recovery_damping=0.80,
...
function recovery_redirect_module(guidance, drive, params)
    # A capture approach keeps the history-window closing speed positive, so
    # this gate is exactly zero there. After an evidenced close pass, sustained
    # negative closure requests a temporary C-shaped bend in the target-turn
    # direction and releases continuously when useful progress returns.
    distance_span = max(
    ...
    closing_loss_span = max(
        params.recovery_closing_loss_full -
            params.recovery_closing_loss_start,
        1.0e-6,
    )
    ...
    recovery_gate = distance_gate * progress_loss_gate

    mean_bend = 0.5 * (drive.q1 + drive.q2)
    mean_bend_rate = 0.5 * (drive.qd1 + drive.qd2)
    bend_target =
        params.recovery_bend_amplitude * guidance.turn_command
    recovery_frequency = max(
        params.recovery_natural_frequency,
        1.0e-6,
    )
    raw_curvature_accel =
        recovery_frequency^2 * (bend_target - mean_bend) -
        2 *
        params.recovery_damping *
        recovery_frequency *
        mean_bend_rate
    curvature_accel =
        recovery_gate *
...
function target_policy(state, params)
    ...
    # Keep the evaluated intercept veto, carrier reserve, and first-pass
    # steering. The new residual is inactive until observed progress is lost.
    response = turn_response_module(state, guidance, drive, params)
    recovery = recovery_redirect_module(guidance, drive, params)
