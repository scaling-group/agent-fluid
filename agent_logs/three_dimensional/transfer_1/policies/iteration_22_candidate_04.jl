...
# controller, then add a small geometry-only steering-release floor on an
# already safe approaching capture corridor. No clock, world coordinate,
# route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_speed_reserve_capture_hold_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        capture_hold_release_fraction=0.10,
...
function turn_response_module(state, guidance, drive, params)
    ...
    response_release =
        ...
        (1.0 - los_miss_gate) *
        intercept_release_guard
    # Geometry may relax a small amount of additive steering only when the
    # projected pass is already inside the approaching capture corridor. This
    # phase-independent floor leaves the carrier and unsafe-pass correction
    # unchanged; the existing response release remains authoritative above it.
    capture_hold_release =
        clamp(params.capture_hold_release_fraction, 0.0, 0.25) *
        intercept_distance_gate *
        projected_corridor_gate *
        approach_gate
    release = max(response_release, capture_hold_release)
    return (
        ...
        capture_hold_release=capture_hold_release,
        release=release,
    )
end
...
function target_policy(state, params)
    ...
    # Keep the evaluated intercept veto, traveling bend, and sparse carrier
    # reserve. The new mechanism changes only a small portion of additive
    # steering after interception geometry already predicts a safe pass.
