# L64 3D candidate: retain the captured LOS-guarded course servo and
# propulsive traveling bend, then blend terminal angular guidance toward a
# rotation-invariant projected cross-track miss. The carrier is never
# attenuated. No clock, world coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_projected_miss_guarded_release_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        projected_miss_speed_floor=0.10,
        projected_miss_scale_L=1.00,
...
function guidance_module(state, params)
    ...
    # The body-frame cross product is invariant to frame rotation. Dividing
    # by speed estimates the signed closest-pass distance under the observed
    # translation, so terminal steering responds before a small angular error
    # becomes a finite miss. Low-speed startup retains target-bearing guidance.
    target_velocity_cross =
        target_x * velocity_y - target_y * velocity_x
    projected_miss_L =
        target_velocity_cross /
        max(course_speed, params.projected_miss_speed_floor)
    projected_miss_error =
        params.target_angle_limit *
        tanh(
            projected_miss_L /
            max(params.projected_miss_scale_L, 1.0e-6),
        )
    projected_miss_gate = terminal_response_gate * course_gate
    route_error =
        (1 - projected_miss_gate) * angular_route_error +
        projected_miss_gate * projected_miss_error
    turn_command =
        tanh(route_error / max(params.route_error_scale, 1.0e-6))

    # Dividing the same cross product by range squared gives inertial
    # line-of-sight angular rate without beat-frequency body yaw.
    los_distance = max(
        distance_L,
    ...
    return (
        ...
        angular_route_error=angular_route_error,
        projected_miss_L=projected_miss_L,
        projected_miss_error=projected_miss_error,
        projected_miss_gate=projected_miss_gate,
...
function drive_module(state, params, drive_frequency_scale=1.0)
    ...
    # Joint state supplies carrier phase. The posterior target preserves the
    # lagged traveling bend associated with every coherent sampled 3D wake.
    vdp_drive =
        params.drive_mu *
        (1 - (q1 / amplitude)^2) *
        qd1
...
function turn_response_module(state, guidance, drive, params)
    # Remove the evidenced joint-phase component from measured body yaw.
    # Useful slow turn response may release steering only while independent
    # translational LOS geometry does not show a growing miss.
