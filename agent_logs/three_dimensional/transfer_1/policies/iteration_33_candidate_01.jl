# L64 3D candidate: retain the intercept-guarded speed-reserve controller and
# its evaluated steering allocation, but permit response-based steering release
# only after achieved-course error has also reduced. No clock, world coordinate,
# route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_course_error_release_veto_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        response_error_veto_start=0.42,
        response_error_veto_full=0.70,
...
function turn_response_module(state, guidance, drive, params)
    ...
    # Correct-sign instantaneous yaw is not completion of a redirect while
    # the achieved course remains materially misaligned. This continuous veto
    # only preserves the already bounded steering residual; it cannot increase
    # its magnitude or alter the propulsive carrier.
    response_error_span = max(
        params.response_error_veto_full - params.response_error_veto_start,
        1.0e-6,
    )
    response_error_veto = _smoothstep01(
        (abs(guidance.course_error) - params.response_error_veto_start) /
            response_error_span,
    )
    release =
    ...
    return (
        ...
        response_error_veto=response_error_veto,
        release=release,
    )
end
...
function target_policy(state, params)
    ...
    # Keep the evaluated intercept veto, steering magnitude and allocation, and
    # sparse carrier reserve. Only the response release condition changes.
