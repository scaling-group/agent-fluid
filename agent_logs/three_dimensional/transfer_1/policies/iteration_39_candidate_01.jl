# L64 3D candidate: retain the repeat-backed intercept-guarded speed-reserve
# controller, then add a bounded outer-approach line-of-sight-rate lead that
# fades before capture. No clock, world coordinate, route, or mutable phase is
# used.

function target_policy_params()
    return (
        version="dogfish3d_outer_los_rate_lead_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        los_lead_start_distance_L=4.00,
        los_lead_full_distance_L=2.75,
        los_lead_fade_start_distance_L=2.00,
        los_lead_off_distance_L=1.25,
        los_lead_rate_scale=0.18,
        los_lead_error_limit=0.10,
...
function guidance_module(state, params)
    ...
    # Add response lead only before the inner pass. The inertial LOS rate is
    # derived from normalized body-frame geometry, so it is independent of
    # heading coordinates and storage-window motion. The lead changes neither
    # the carrier nor its posterior lag and is exactly zero near capture.
    los_lead_entry_span = max(
        params.los_lead_start_distance_L -
            params.los_lead_full_distance_L,
        1.0e-6,
    )
    los_lead_entry_gate = _smoothstep01(
        (params.los_lead_start_distance_L - distance_L) /
            los_lead_entry_span,
    )
    los_lead_exit_span = max(
        params.los_lead_fade_start_distance_L -
            params.los_lead_off_distance_L,
        1.0e-6,
    )
    los_lead_exit_gate = _smoothstep01(
        (distance_L - params.los_lead_off_distance_L) /
            los_lead_exit_span,
    )
    los_lead_gate = los_lead_entry_gate * los_lead_exit_gate
    los_rate_lead =
        -params.los_lead_error_limit *
        los_lead_gate *
        tanh(
            inertial_los_rate /
            max(params.los_lead_rate_scale, 1.0e-6),
        )
    route_error = clamp(
        base_route_error + los_rate_lead,
        -params.course_angle_limit,
        params.course_angle_limit,
    )
    turn_command =
    ...
    return (
        ...
        los_lead_entry_gate=los_lead_entry_gate,
        los_lead_exit_gate=los_lead_exit_gate,
        los_lead_gate=los_lead_gate,
        los_rate_lead=los_rate_lead,
...
function target_policy(state, params)
    ...
    # carrier reserve. The LOS-rate lead changes only the outer route request
    # and cannot alter the traveling-bend carrier directly.
