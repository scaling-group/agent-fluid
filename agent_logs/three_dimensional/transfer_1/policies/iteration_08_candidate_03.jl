# L64 3D candidate: retain the achieved-course servo, traveling carrier, and
# carrier-aligned steering, then relieve cadence only during fast terminal
# closure. Full cadence returns as closure vanishes. There is no clock, world
# route, mutable state, carrier attenuation, or additive mean curvature.

function target_policy_params()
    return (
        version="dogfish3d_course_servo_closure_gated_cadence_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        terminal_cadence_start_distance_L=3.00,
        terminal_cadence_full_distance_L=1.00,
        terminal_closing_speed_center=0.20,
        terminal_closing_speed_scale=0.12,
        terminal_frequency_relief=0.20,
        terminal_frequency_floor=0.75,
...
function guidance_module(state, params)
    ...
    cadence_span = max(
        params.terminal_cadence_start_distance_L -
            params.terminal_cadence_full_distance_L,
        1.0e-6,
    )
    terminal_cadence_gate = _smoothstep01(
        (params.terminal_cadence_start_distance_L - distance_L) /
            cadence_span,
    )
    terminal_closing_gate =
        0.5 *
        (1.0 + tanh(
            (closing_speed - params.terminal_closing_speed_center) /
    ...
    return (
        ...
        terminal_cadence_gate=terminal_cadence_gate,
...
function drive_module(state, params, drive_frequency_scale=1.0)
    omega =
        (2.0 * pi / params.drive_period) *
        max(_safe(drive_frequency_scale, 1.0), params.terminal_frequency_floor)
    ...
    tail_accel =
        ...
        2.0 * params.drive_tail_damping * omega * qd2
...
@inline function carrier_aligned_steering_scale(
    ...
    aligned_half_gate = 0.5 * (1.0 + tanh(normalized_alignment))
    aligned_pulse =
        clamp(params.terminal_aligned_pulse_gain, 0.0, 2.0) *
        aligned_half_gate
    return (1.0 - terminal_gate) + terminal_gate * aligned_pulse
end
...
function target_policy(state, params)
    ...
    base_frequency_scale =
        1.0 +
        cadence_gain *
        guidance.far_drive_gate *
        (1.0 - params.far_drive_turn_relief * turn_load)

    # Only fast positive closure activates the approach hold. As closure
    # vanishes, the relief goes to zero and restores the complete carrier for
    # the terminal cross-track correction. The frequency floor prevents a
    # repeat of the evidenced locomotor-reserve collapse.
    frequency_relief =
        clamp(params.terminal_frequency_relief, 0.0, 0.40) *
        guidance.terminal_cadence_gate *
        guidance.terminal_closing_gate
    drive_frequency_scale = max(
        base_frequency_scale - frequency_relief,
        params.terminal_frequency_floor,
    )
