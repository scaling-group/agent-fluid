# L64 3D candidate: retain the replicated-capture intercept/reserve controller,
# then brake only phase-compensated terminal yaw for which its existing
# intercept logic already releases steering. Posterior propulsion is unchanged.
# No clock, world coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_response_gated_yaw_brake_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        terminal_yaw_damping_accel_limit=2.40,
...
@inline function carrier_reserve_scale(
    ...
    # Soften only carrier effort that is observably unusable: terminal range,
    # speed near its envelope, a previously saturated outward action, and a
    # current carrier acceleration that remains outward. Restoring carrier
    # effort and the separately computed steering residual remain intact.
    speed_ratio =
...
function target_policy(state, params)
    ...
    # Preserve the replicated intercept veto and sparse outward-only carrier
    # relief. The new brake is subordinate to the same release signal: it can
    # act only after correct-sign yaw and intercept-compatible geometry have
    # already caused the route servo to withdraw steering.
    ...
    yaw_brake_accel =
        params.terminal_yaw_damping_accel_limit *
        response.release *
        tanh(
