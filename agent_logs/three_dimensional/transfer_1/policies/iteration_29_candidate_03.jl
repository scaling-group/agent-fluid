# L64 3D candidate: retain the intercept-guarded speed-reserve controller, but
# condition unsafe-terminal additive-steering transfer on normalized actuator
# burden. Only a more-burdened posterior joint may hand steering to an anterior
# joint with margin; total steering authority stays constant. No clock, world
# coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_burden_conditioned_anterior_transfer_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        unsafe_steering_transfer=0.55,
        allocator_tail_burden_start=0.78,
        allocator_tail_burden_full=0.95,
        allocator_head_burden_start=0.85,
        allocator_head_burden_full=0.65,
        allocator_burden_advantage_start=0.04,
        allocator_burden_advantage_full=0.18,
...
function steering_allocation_module(
    drive,
    previous_head_action,
    previous_tail_action,
    params,
)
    # A joint is burdened only when both its present speed and its previous
    # returned action use the envelope. The geometric mean prevents a large
    # value in only one channel from triggering a spatial steering transfer.
    head_speed_ratio = _clamp01(
        abs(drive.qd1) / max(params.joint_speed_limit, 1.0e-6),
    )
    tail_speed_ratio = _clamp01(
        abs(drive.qd2) / max(params.joint_speed_limit, 1.0e-6),
    )
    head_action_ratio = _clamp01(
        abs(previous_head_action) /
            max(params.acceleration_limit, 1.0e-6),
    )
    tail_action_ratio = _clamp01(
        abs(previous_tail_action) /
            max(params.acceleration_limit, 1.0e-6),
    )
    head_burden = sqrt(head_speed_ratio * head_action_ratio)
    tail_burden = sqrt(tail_speed_ratio * tail_action_ratio)

    tail_span = max(
        params.allocator_tail_burden_full -
            params.allocator_tail_burden_start,
        1.0e-6,
    )
    tail_burden_gate = _smoothstep01(
        (tail_burden - params.allocator_tail_burden_start) / tail_span,
    )
    head_span = max(
        params.allocator_head_burden_start -
            params.allocator_head_burden_full,
        1.0e-6,
    )
    head_margin_gate = _smoothstep01(
        (params.allocator_head_burden_start - head_burden) / head_span,
    )
    burden_advantage = tail_burden - head_burden
    advantage_span = max(
        params.allocator_burden_advantage_full -
            params.allocator_burden_advantage_start,
        1.0e-6,
    )
    burden_advantage_gate = _smoothstep01(
        (burden_advantage - params.allocator_burden_advantage_start) /
            advantage_span,
    )
    transfer_gate =
        tail_burden_gate * head_margin_gate * burden_advantage_gate

    return (
        head_burden=head_burden,
        tail_burden=tail_burden,
        burden_advantage=burden_advantage,
...
function target_policy(state, params)
    ...
    # Keep the evaluated intercept veto, steering magnitude, and sparse
    # carrier reserve. When raw projection is not capture-compatible, transfer
    # steering anteriorly only if measured tail burden exceeds head burden and
    # the head retains margin. The share sum is invariant.
    ...
    allocation = steering_allocation_module(
        drive,
        previous_head_action,
        previous_tail_action,
        params,
    ...
    intercept_compatible =
        response.projected_corridor_gate * response.approach_gate
    unsafe_terminal_gate =
        guidance.terminal_response_gate * (1.0 - intercept_compatible)
    steering_transfer =
        min(
            max(params.unsafe_steering_transfer, 0.0),
            max(params.tail_steering_share, 0.0),
        ) * unsafe_terminal_gate * allocation.transfer_gate
    head_steering_share =
        params.head_steering_share + steering_transfer
    tail_steering_share =
        params.tail_steering_share - steering_transfer
    head_accel =
        head_carrier_scale * drive.head_accel +
        head_steering_share * steering_accel
    tail_accel =
        tail_carrier_scale * drive.tail_accel +
        tail_steering_share * steering_accel
