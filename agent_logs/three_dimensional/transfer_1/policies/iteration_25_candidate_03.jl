# L64 3D candidate: retain the intercept-guarded speed-reserve controller,
# but remove carrier-synchronous sway from its terminal achieved-course
# observation using joint state. No clock, world coordinate, route, or mutable
# phase is used.

function target_policy_params()
    return (
        version="dogfish3d_phase_compensated_course_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        course_phase_start_distance_L=4.50,
        course_phase_full_distance_L=3.50,
        course_lateral_qd1_gain=-0.070,
        course_lateral_qd2_gain=0.020,
...
function guidance_module(state, params)
    ...
    # Free-swimming traces show that joint velocity explains nearly all of
    # the beat-scale lateral-velocity component. Remove only that component
    # from terminal course estimation; speed and interception geometry retain
    # the actual body velocity. The smooth range gate preserves the evaluated
    # far-field controller exactly.
    course_phase_span = max(
        params.course_phase_start_distance_L -
            params.course_phase_full_distance_L,
        1.0e-6,
    )
    course_phase_gate = _smoothstep01(
        (params.course_phase_start_distance_L - distance_L) /
            course_phase_span,
    )
    carrier_lateral_velocity =
        params.course_lateral_qd1_gain * _safe(state.phi_dot[1], 0.0) +
        params.course_lateral_qd2_gain * _safe(state.phi_dot[2], 0.0)
    course_lateral_velocity =
        velocity_y - course_phase_gate * carrier_lateral_velocity
    course_angle = atan(
        course_lateral_velocity,
    ...
    return (
        ...
        course_phase_gate=course_phase_gate,
        carrier_lateral_velocity=carrier_lateral_velocity,
