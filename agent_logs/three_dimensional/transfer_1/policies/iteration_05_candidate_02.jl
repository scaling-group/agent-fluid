...
# bend, then reallocate terminal steering within the beat while observed joint
# energy remains sufficient to sustain the carrier. Low carrier energy releases
# the attenuation before it can quench propulsion. No clock, world coordinate,
# route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_course_servo_energy_guarded_halfcycle_v1",
        drive_period=0.55,
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # steering. Near the target, opposing-half attenuation is available only
    # while both joints retain enough normalized phase-plane energy. This
    # observed-state reserve releases the attenuation before the traveling
    # bend collapses; it never amplifies the carrier or changes cadence.
    half_relief = clamp(
