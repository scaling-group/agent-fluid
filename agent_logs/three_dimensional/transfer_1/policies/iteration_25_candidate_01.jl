...
# controller, then remove carrier-synchronous sway from the terminal achieved-
# course observation. No clock, world coordinate, route, or mutable phase is
# used.

function target_policy_params()
    return (
        version="dogfish3d_speed_reserve_phase_compensated_course_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        course_phase_sway_scale_U=0.42,
...
function guidance_module(state, params)
    ...
    # The terminal traces show that most instantaneous lateral body velocity
    # is carrier-synchronous sway. Use bounded joint-state phase to compensate
    # only the achieved-course observation; physical interception geometry
    # below continues to use raw velocity.
    normalized_phase_speed = clamp(
        _safe(state.phi_dot[1], 0.0) /
            max(params.joint_speed_limit, 1.0e-6),
        -1.0,
        1.0,
    )
    phase_sway_correction =
        terminal_response_gate *
        params.course_phase_sway_scale_U *
        normalized_phase_speed
    compensated_velocity_y = velocity_y + phase_sway_correction
    course_angle = atan(
    ...
    return (
        ...
        phase_sway_correction=phase_sway_correction,
...
function target_policy(state, params)
    ...
    # carrier reserve. The new mechanism changes the terminal course observer,
    # not the repeat-supported actuation structure.
