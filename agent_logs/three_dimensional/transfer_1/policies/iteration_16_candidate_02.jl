# L64 3D candidate: retain the repeat-supported intercept-guarded
# speed-reserve carrier, then relieve only excess additive steering on an
# approaching, safely centered terminal intercept. No clock, world coordinate,
# route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_speed_reserve_capture_corridor_hold_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        capture_hold_start_distance_L=1.50,
        capture_hold_full_distance_L=0.90,
        capture_hold_miss_inner_L=0.40,
        capture_hold_miss_outer_L=0.65,
        capture_hold_alignment_start=0.20,
        capture_hold_alignment_full=0.50,
        capture_hold_steering_relief=0.30,
...
function capture_hold_module(guidance, params)
    # This gate is even under lateral reflection. It cannot choose a turn
    # direction; it only recognizes a currently approaching straight-line
    # intercept that lies conservatively inside the capture disk.
    distance_span = max(
        params.capture_hold_start_distance_L -
            params.capture_hold_full_distance_L,
        1.0e-6,
    )
    distance_gate = _smoothstep01(
        (params.capture_hold_start_distance_L - guidance.distance_L) /
            distance_span,
    )
    miss_span = max(
        params.capture_hold_miss_outer_L - params.capture_hold_miss_inner_L,
        1.0e-6,
    )
    inside_corridor_gate = _smoothstep01(
        (params.capture_hold_miss_outer_L -
            guidance.projected_miss_distance_L) /
            miss_span,
    )
    alignment_span = max(
        params.capture_hold_alignment_full -
            params.capture_hold_alignment_start,
        1.0e-6,
    )
    approaching_gate = _smoothstep01(
        (guidance.approach_alignment -
            params.capture_hold_alignment_start) /
            alignment_span,
    )
    hold_gate = distance_gate * inside_corridor_gate * approaching_gate
    return (
        distance_gate=distance_gate,
        inside_corridor_gate=inside_corridor_gate,
...
function target_policy(state, params)
    ...
    # Keep the evaluated intercept veto and conditional carrier reserve. The
    # new hold changes only additive steering after translational geometry says
    # the current pass is approaching and safely inside the capture corridor.
    response = turn_response_module(state, guidance, drive, params)
    capture_hold = capture_hold_module(guidance, params)
    ...
    steering_scale = 1.0 -
        clamp(params.capture_hold_steering_relief, 0.0, 0.80) *
        capture_hold.hold_gate
    steering_accel =
