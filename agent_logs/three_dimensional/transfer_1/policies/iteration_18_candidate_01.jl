...
# propulsive traveling bend, and sparse carrier reserve, then redistribute the
# signed steering residual within each joint's observed beat. No clock, world
...
function target_policy_params()
    return (
        ...
        steering_half_cycle_asymmetry=0.45,
        steering_phase_velocity_width=0.35,
...
@inline function within_beat_steering_scale(
    turn_command,
    joint_velocity,
    params,
)
    # Joint velocity is a clock-free local phase cue. Strengthen the existing
    # signed steering residual while this joint moves toward requested
    # curvature and soften it on the return half-cycle. The residual never
    # reverses and its nominal scale is recovered at a turning point.
    phase_velocity_scale = max(
        params.steering_phase_velocity_width * params.joint_speed_limit,
        1.0e-6,
    )
    toward_requested_curvature = tanh(
        turn_command * joint_velocity / phase_velocity_scale,
    )
    asymmetry = clamp(params.steering_half_cycle_asymmetry, 0.0, 0.75)
    return 1.0 + asymmetry * toward_requested_curvature
end
...
function target_policy(state, params)
    ...
    # Keep the evaluated intercept veto and sparse carrier reserve. The new
    # mechanism changes only within-beat allocation of the signed steering
    # residual, using each joint's normalized motion as its local phase cue.
    ...
    head_steering_scale = within_beat_steering_scale(
        guidance.turn_command,
        drive.qd1,
        params,
    )
    tail_steering_scale = within_beat_steering_scale(
        guidance.turn_command,
        drive.qd2,
        params,
    )
