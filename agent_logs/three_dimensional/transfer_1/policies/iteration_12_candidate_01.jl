# L64 3D candidate: retain the achieved-course servo, propulsive traveling
# bend, and bounded carrier-aligned steering, then transfer a minority of the
# terminal steering pulse toward the half-cycle where measured yaw moment
# opposes the route request. No carrier attenuation, clock, world route, or
# mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_course_servo_moment_residual_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        terminal_moment_transfer=0.35,
        terminal_moment_scale=0.008,
...
@inline function moment_residual_steering_scale(
    turn_command,
    carrier_accel,
    moment_z_L2,
    terminal_gate,
    ...
    # Each pulse lies in [0, pulse_gain] and is reflection-even: route request,
    # carrier acceleration, and yaw moment all reverse under lateral
    # reflection. The carrier pulse preserves the evidenced allocator. The
    # moment pulse favors only the half-cycle in which hydrodynamic torque
    # opposes the requested yaw, without changing the carrier itself.
    pulse_gain = clamp(params.terminal_aligned_pulse_gain, 0.0, 2.0)
    carrier_alignment =
    ...
    carrier_pulse =
        pulse_gain * 0.5 * (1 + tanh(carrier_alignment))
    moment_opposition =
        turn_command * moment_z_L2 /
        max(params.terminal_moment_scale, 1.0e-6)
    moment_pulse =
        pulse_gain * 0.5 * (1 + tanh(moment_opposition))
    transfer = clamp(params.terminal_moment_transfer, 0.0, 1.0)
    terminal_pulse =
        (1 - transfer) * carrier_pulse +
        transfer * moment_pulse
    return (
        (1 - terminal_gate) +
        terminal_gate * terminal_pulse
    )
end
...
function target_policy(state, params)
    ...
    # Keep the full traveling carrier. Terminally, replace only a minority of
    # the existing kinematic steering schedule with measured moment opposition;
    # the convex blend cannot exceed the inherited [0,2] pulse envelope.
    moment_z_L2 = hasproperty(state, :moment_z_L2) ?
        _safe(state.moment_z_L2, 0.0) : 0.0
    head_steering_scale = moment_residual_steering_scale(
    ...
    tail_steering_scale = moment_residual_steering_scale(
        guidance.turn_command,
        drive.tail_accel,
        moment_z_L2,
        guidance.terminal_allocation_gate,
