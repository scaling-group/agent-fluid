...
# add a response-triggered mean-curvature redirect only if terminal closing
# progress is lost. No clock, world coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_progress_loss_redirect_v1",
        drive_period=0.55,
        control_period=0.55,
...
function progress_loss_redirect(guidance, drive, params)
    # The sampled captures keep their history-window closing speed above
    # +0.20L/T below 4L. Only a stalled or opening terminal pass admits this
    # response-triggered C-bend; positive closure releases it continuously.
    closing_span = max(
    ...
    recovery_gate =
        guidance.terminal_response_gate * progress_loss_gate
...
function target_policy(state, params)
    ...
    # first approach. If measured closure is lost, blend only the steering
    # residual into a bounded same-signed two-joint curvature redirect; the
    # differential propulsive carrier remains active throughout.
    response = turn_response_module(state, guidance, drive, params)
    recovery = progress_loss_redirect(guidance, drive, params)
    ...
    baseline_head_steering = params.head_steering_share * steering_accel
    baseline_tail_steering = params.tail_steering_share * steering_accel
    head_steering =
