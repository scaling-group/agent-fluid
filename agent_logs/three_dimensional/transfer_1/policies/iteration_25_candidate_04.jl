...
# demodulate joint-state-predicted beat sway from terminal achieved-course
# feedback. Raw velocity still owns all geometric and actuator guards. No
# clock, world coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_speed_reserve_course_demod_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        course_demod_start_distance_L=4.00,
        course_demod_full_distance_L=3.00,
        course_phase_qd1_gain=0.096,
        course_phase_qd2_gain=-0.038,
...
function guidance_module(state, params)
    ...
    # Sampled capture traces agree that most terminal lateral velocity is the
    # carrier projection -0.096*qd1 + 0.038*qd2. Remove that predictable sway
    # only from course feedback; physical LOS and intercept guards below keep
    # the measured velocity. The distance blend leaves the proven far field
    # unchanged and avoids selecting a carrier half-cycle.
    qd1 = _safe(state.phi_dot[1], 0.0)
    qd2 = _safe(state.phi_dot[2], 0.0)
    course_demod_span = max(
        params.course_demod_start_distance_L -
            params.course_demod_full_distance_L,
        1.0e-6,
    )
    course_demod_gate = _smoothstep01(
        (params.course_demod_start_distance_L - distance_L) /
            course_demod_span,
    )
    course_velocity_y =
        velocity_y +
        course_demod_gate *
        (
            params.course_phase_qd1_gain * qd1 +
            params.course_phase_qd2_gain * qd2
        )
    course_angle = atan(
        course_velocity_y,
    ...
    return (
        ...
        course_demod_gate=course_demod_gate,
        course_velocity_y=course_velocity_y,
...
function target_policy(state, params)
    ...
    # mechanism changes only the terminal course observation above; outward
    # carrier reserve and actuator allocation remain the sampled baseline.
