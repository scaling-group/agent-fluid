# L64 3D candidate: retain the intercept-guarded speed-reserve first pass, then
# reacquire only when body-frame geometry says the target is aft while distance
# is opening. The traveling carrier remains active. No clock, world coordinate,
# route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_missed_pass_reacquisition_v1",
        drive_period=0.55,
        control_period=0.55,
...
function guidance_module(state, params)
    ...
    # A successful first pass terminates before this mode can engage. If the
    # target is instead aft and range is opening, use bounded body bearing to
    # turn back. Both factors release continuously on a useful response.
    target_aft_fraction = clamp(target_x / distance_L, 0.0, 1.0)
    target_aft_gate = _smoothstep01(target_aft_fraction)
    opening_gate = _smoothstep01(
        -closing_speed /
            max(params.progress_closing_speed_scale, 1.0e-6),
    )
    miss_recovery_gate = target_aft_gate * opening_gate
    recovery_turn_command = tanh(
        target_angle / max(params.route_error_scale, 1.0e-6),
    )
    turn_command =
        (1.0 - miss_recovery_gate) * base_turn_command +
        miss_recovery_gate * recovery_turn_command
    closing_deficit =
        0.5 *
        (1 - tanh(
            closing_speed /
    ...
    return (
        ...
        base_turn_command=base_turn_command,
        turn_command=turn_command,
        closing_speed_L=closing_speed,
        target_aft_gate=target_aft_gate,
        opening_gate=opening_gate,
        miss_recovery_gate=miss_recovery_gate,
...
function turn_response_module(state, guidance, drive, params)
    ...
    release =
        ...
        (1.0 - los_miss_gate) *
        intercept_release_guard *
        (1.0 - guidance.miss_recovery_gate)
    return (
...
function target_policy(state, params)
    ...
    # Keep the evaluated intercept veto, steering magnitude, fixed spatial
    # allocation, and sparse carrier reserve. Reacquisition changes only the
    # observed route request and release gate after an actual missed pass.
