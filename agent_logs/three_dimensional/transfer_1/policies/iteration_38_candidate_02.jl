# L64 3D candidate: retain the repeat-backed speed-reserve carrier and
# achieved-course servo, then require projected capture compatibility before
# terminal response can progressively release steering. No clock, world
# coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_projected_capture_certificate_v1",
        drive_period=0.55,
        control_period=0.55,
...
function turn_response_module(state, guidance, drive, params)
    ...
    # Correct-sign response may release steering only while LOS rate and
    ...
    capture_certificate = projected_corridor_gate * approach_gate

    # Preserve the evaluated full inner guard, but introduce the same
    # response certificate progressively over the terminal region. Thus the
    # far-field controller is byte-for-byte equivalent in behavior, whereas
    # an already divergent projected pass cannot release bounded steering as
    # readily at the evidenced 2.75L branch point.
    capture_certification_weight = max(
        intercept_distance_gate,
        guidance.terminal_response_gate,
    )
    capture_release_guard =
        (1.0 - capture_certification_weight) +
        capture_certification_weight * capture_certificate
    release =
        guidance.terminal_response_gate *
        response_gate *
        (1.0 - los_miss_gate) *
    ...
    return (
        ...
        capture_certification_weight=capture_certification_weight,
        capture_release_guard=capture_release_guard,
        release=release,
    )
end
...
function target_policy(state, params)
    ...
    # Keep the evaluated carrier-reserve allocation and full additive steering.
    # The new mechanism changes only whether observed terminal response is
    # sufficiently capture-compatible to release that steering.
