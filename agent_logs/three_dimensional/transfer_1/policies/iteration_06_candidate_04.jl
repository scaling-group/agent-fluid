...
# bend, then reallocate only terminal steering onto carrier-aligned beat
# half-cycles without attenuating either half of the carrier. No clock, world
# coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_course_servo_carrier_aligned_steering_v1",
        drive_period=0.55,
        control_period=0.55,
...
@inline function carrier_aligned_steering_scale(
    ...
    # Outside the approach region this is exactly the inherited shared
    # steering. At full allocation it shifts that impulse onto the carrier
    # half-cycle with the requested sign. The carrier itself is never scaled.
    return (
        (1 - terminal_gate) +
...
function target_policy(state, params)
    ...
    # Carrier-aligned duty-ratio steering changes the within-beat placement of
    # the existing bounded steering impulse. Unlike the sampled attenuation
    # failure, it cannot quench either carrier half-cycle.
    steering_accel =
