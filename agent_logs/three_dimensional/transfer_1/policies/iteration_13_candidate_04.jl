# L64 3D candidate: retain the captured intercept/reserve controller and add a
# terminal speed governor on the combined carrier-plus-steering command. Only
# infeasible outward acceleration near a measured joint-speed boundary is
# blended into restoring acceleration. No clock, world route, or mutable phase.

function target_policy_params()
    return (
        version="dogfish3d_intercept_reserve_speed_governor_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        speed_governor_start=0.90,
        speed_governor_full=0.985,
        speed_governor_outward_scale=0.10,
        speed_governor_restoring_accel=4.0,
...
@inline function terminal_speed_governor(
    joint_velocity,
    proposed_accel,
    terminal_gate,
    params,
)
    # At the measured velocity envelope, further outward acceleration is
    # infeasible regardless of whether it came from carrier or steering. Blend
    # only that outward total into a small restoring command; inward motion is
    # passed through exactly.
    speed_ratio =
        abs(joint_velocity) / max(params.joint_speed_limit, 1.0e-6)
    speed_span = max(
        params.speed_governor_full - params.speed_governor_start,
        1.0e-6,
    )
    speed_gate = _smoothstep01(
        (speed_ratio - params.speed_governor_start) / speed_span,
    )
    normalized_outward = max(
        joint_velocity * proposed_accel /
            max(
                params.joint_speed_limit * params.acceleration_limit,
                1.0e-6,
            ),
    ...
    outward_gate = tanh(
        normalized_outward /
            max(params.speed_governor_outward_scale, 1.0e-6),
    )^2
    governor_gate =
        _clamp01(terminal_gate) * speed_gate * outward_gate
    restoring_accel =
        -copysign(params.speed_governor_restoring_accel, joint_velocity)
    return (1.0 - governor_gate) * proposed_accel +
        governor_gate * restoring_accel
end
...
function target_policy(state, params)
    ...
    # Keep the evaluated intercept veto, additive steering, and sparse
    # carrier-reserve mechanism. The governor below acts only after those
    ...
    head_accel = terminal_speed_governor(
        drive.qd1,
        proposed_head_accel,
        guidance.terminal_response_gate,
        params,
    )
    tail_accel = terminal_speed_governor(
        drive.qd2,
        proposed_tail_accel,
        guidance.terminal_response_gate,
        params,
