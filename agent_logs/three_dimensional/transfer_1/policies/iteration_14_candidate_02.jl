# L64 3D candidate: retain the repeatable intercept/reserve controller, but
# couple outward-carrier relief to the normalized steering residual actually
# requested after response release. No clock, world coordinate, route, or
# mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_feedback_coherent_speed_reserve_v2",
        drive_period=0.55,
        control_period=0.55,
...
@inline function carrier_reserve_scale(
    ...
    # Relief requires terminal range, active target-feedback demand, speed near
    # its envelope, a previously saturated outward action, and current carrier
    # acceleration that is still outward. Restoring carrier effort and the
    # separate steering residual are never attenuated.
    speed_ratio =
...
function target_policy(state, params)
    ...
    # Keep the evaluated intercept veto and full additive steering. Reserve
    # only the fraction of outward carrier effort justified by the normalized
    # steering residual that remains after response release.
    response = turn_response_module(state, guidance, drive, params)
    steering_demand =
        abs(guidance.turn_command) * (1.0 - response.release)
