...
# while one body-frame achieved-course servo supplies bounded steering and a
# continuous terminal drive-relief schedule gives that servo response time.
# No clock, world coordinate, task route, or mutable controller state is used.

function target_policy_params()
    return (
        version="dogfish3d_course_servo_terminal_relief_v2",
        drive_period=0.55,
        control_period=0.55,
...
function target_policy(state, params)
    ...
    drive_frequency_scale =
        cruise_frequency_scale * guidance.terminal_drive_scale
