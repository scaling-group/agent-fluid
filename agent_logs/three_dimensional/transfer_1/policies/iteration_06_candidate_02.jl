...
# bend, then make terminal half-cycle steering conditional on observed carrier
# reserve. Falling reserve releases the attenuation and activates bounded,
# phase-aligned energy recovery. No clock, world coordinate, route, or mutable
# phase is used.

function target_policy_params()
    return (
        version="dogfish3d_course_servo_energy_recovery_halfcycle_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        carrier_recovery_gain=3.0,
        carrier_recovery_accel_limit=6.0,
...
@inline function _smooth01(value)
...
@inline function carrier_reserve_feedback(drive, params)
    velocity_scale = max(
    ...
    return (
        ...
        recovery_deficit=1.0 - reserve_gate,
    )
end
...
function target_policy(state, params)
    ...
    reserve = carrier_reserve_feedback(drive, params)

    # The far-field course servo retains its sampled shared-acceleration
    # steering. Near the target, opposing-half attenuation is released as the
    # observed carrier reserve falls. A bounded acceleration along each
    # joint's velocity then restores oscillatory energy without adding a mean
    # bend. Both paths are exactly inactive outside the terminal gate.
    half_relief = clamp(
    ...
    recovery_strength =
        guidance.terminal_asymmetry_gate *
        reserve.recovery_deficit
    recovery_limit = abs(params.carrier_recovery_accel_limit)
    head_recovery = clamp(
        params.carrier_recovery_gain *
            recovery_strength *
            drive.qd1,
        -recovery_limit,
        recovery_limit,
