...
# bend, then release only terminal shared steering when joint-compensated yaw
# shows that the requested turn is already established. The carrier is never
# attenuated. No clock, world coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_course_servo_response_released_steering_v1",
        drive_period=0.55,
        control_period=0.55,
...
function turn_response_module(state, guidance, drive, params)
    # Instantaneous body yaw contains a strong beat-frequency component. The
    # observed joint velocities provide a memoryless phase compensation; only
    # response with the requested yaw sign is allowed to release steering.
...
function target_policy(state, params)
    ...
    # Preserve the sampled direct course servo away from the target. Near the
    # target, a correct signed yaw response smoothly releases only the shared
    # steering term. Wrong-sign response retains full authority, release never
    # reverses the route request, and the traveling-bend carrier is untouched.
