# L64 3D candidate: an achieved-course outer loop cascades through a
# joint-phase-compensated yaw-rate loop and bounded posterior mean curvature.
# The evidenced state-feedback carrier is preserved. All route geometry is
# normalized and body-frame; there is no clock, world route, or mutable state.

function target_policy_params()
    return (
        version="dogfish3d_achieved_course_rate_cascade_v1",
        drive_period=0.55,
        control_period=0.55,
...
function course_rate_guidance_module(state, params)
    ...
    # The sampled 3D traces show that instantaneous heading rate is dominated
    # by beat-phase motion. Compensate that joint-velocity component before
    # feedback so the inner loop releases mean curvature on route response.
    ...
    target_turn_rate =
        -params.target_turn_rate_limit * route_request
...
function course_rate_drive_module(
    state,
    params,
    turn_command=0.0,
    ...
    mean_tail_tangent =
        params.mean_tail_curvature_limit * bounded_turn_command
    tail_target =
...
function target_policy(state, params)
    guidance = course_rate_guidance_module(state, params)
    ...
    drive = course_rate_drive_module(
        state,
        params,
