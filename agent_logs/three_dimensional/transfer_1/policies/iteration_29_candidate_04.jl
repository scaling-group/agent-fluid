...
# add a bounded proportional-navigation residual from body-frame target and
# velocity. No clock, world coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_speed_reserve_los_rate_navigation_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        los_rate_steering_weight=0.25,
        los_rate_steering_scale=0.15,
...
function target_policy(state, params)
    ...
    # Keep the evaluated response veto, constant spatial steering shares, and
    # sparse carrier reserve. The new residual opposes inertial LOS rotation
    # only until target/velocity projection is capture-compatible. It is soft
    # far away because angular rate naturally decays with target distance, and
    # the combined steering remains inside the evaluated unit authority.
    ...
    los_rate_command =
        params.los_rate_steering_weight *
        tanh(
            -guidance.inertial_los_rate /
                max(params.los_rate_steering_scale, 1.0e-6),
        ) *
        guidance.course_gate *
        (1.0 - intercept_compatible)
    ...
    steering_accel = params.steering_accel_limit * steering_command
    head_accel =
