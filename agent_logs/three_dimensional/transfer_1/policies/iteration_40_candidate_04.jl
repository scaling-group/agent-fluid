...
# propulsive traveling bend, and sparse carrier reserve, then oppose observed
# inertial LOS rotation with a bounded outer-terminal steering residual. No
# clock, world coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_outer_los_rate_intercept_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        navigation_start_distance_L=4.00,
        navigation_outer_full_distance_L=3.25,
        navigation_inner_full_distance_L=2.25,
        navigation_off_distance_L=1.50,
        navigation_los_rate_scale=0.12,
        navigation_accel_limit=2.0,
...
function outer_los_navigation_module(guidance, params)
    # Oppose inertial LOS rotation only after broad route acquisition. The
    # annular gate fades before the inner capture regime, and approach
    # alignment prevents this pre-pass residual from becoming recovery logic.
    outer_span = max(
        params.navigation_start_distance_L -
            params.navigation_outer_full_distance_L,
        1.0e-6,
    )
    outer_gate = _smoothstep01(
        (params.navigation_start_distance_L - guidance.distance_L) /
            outer_span,
    )
    inner_span = max(
        params.navigation_inner_full_distance_L -
            params.navigation_off_distance_L,
        1.0e-6,
    )
    inner_gate = _smoothstep01(
        (guidance.distance_L - params.navigation_off_distance_L) /
            inner_span,
    )
    approach_span = max(
        params.approach_alignment_full - params.approach_alignment_start,
    ...
    navigation_gate = outer_gate * inner_gate * approach_gate
    navigation_command = tanh(
        -guidance.inertial_los_rate /
            max(params.navigation_los_rate_scale, 1.0e-6),
    )
    steering_accel =
        params.navigation_accel_limit *
        navigation_gate *
        navigation_command
    return (
...
function target_policy(state, params)
    ...
    # Keep the evaluated intercept veto, allocation, and carrier reserve. The
    # new cue is a separately bounded interception residual through the same
    # joint shares, not a replacement route command or carrier-phase change.
    response = turn_response_module(state, guidance, drive, params)
    navigation = outer_los_navigation_module(guidance, params)
