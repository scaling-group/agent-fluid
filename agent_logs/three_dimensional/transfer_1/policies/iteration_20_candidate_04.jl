...
# controller, then add a small posterior wave-shape steering pulse that is
# exactly zero outside the existing intercept gate. No clock, world coordinate,
# route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_speed_reserve_posterior_wave_shape_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        posterior_wave_bias_gain=0.025,
...
@inline function posterior_wave_shape_accel(
    guidance,
    response,
    drive,
    params,
)
    # The anterior joint state supplies a clock-free carrier phase cue. A
    # signed posterior target bias is strongest while the anterior bend moves
    # through neutral and is identically absent before the intercept gate.
    ...
    posterior_target_bias =
        params.posterior_wave_bias_gain *
        drive.amplitude *
...
function target_policy(state, params)
    ...
    # carrier reserve. The new posterior wave-shape term is separately bounded
    # and cannot alter the repeat-supported far-field controller.
    ...
    steering_accel =
        ...
        guidance.turn_command *
        (1.0 - response.release)
    posterior_wave_accel = posterior_wave_shape_accel(
        guidance,
        response,
        drive,
        params,
