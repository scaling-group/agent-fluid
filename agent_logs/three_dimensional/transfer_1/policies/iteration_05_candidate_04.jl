...
# bend, then blend terminal steering from shared acceleration into bounded
# posterior mean curvature without attenuating the carrier. No clock, world
# coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_course_servo_terminal_mean_curvature_v1",
        drive_period=0.55,
        control_period=0.55,
        ...
        terminal_curvature_start_distance_L=4.00,
        terminal_curvature_full_distance_L=1.25,
        terminal_mean_tail_tangent_limit=12.0 * pi / 180,
...
function drive_module(
    ...
    bounded_mean_tail_tangent = clamp(
        ...
        -abs(params.terminal_mean_tail_tangent_limit),
        abs(params.terminal_mean_tail_tangent_limit),
    )
    tail_target =
...
function guidance_module(state, params)
    ...
    terminal_span = max(
        params.terminal_curvature_start_distance_L -
            params.terminal_curvature_full_distance_L,
        1.0e-6,
    )
    terminal_curvature_gate = _clamp01(
        (params.terminal_curvature_start_distance_L - distance_L) /
            terminal_span,
    )
    ...
    return (
        ...
        terminal_curvature_gate=terminal_curvature_gate,
    )
end
...
function target_policy(state, params)
    ...
    # Far from the target, preserve the sampled shared-acceleration course
    # steering. In the terminal regime, transfer that slow request into an odd
    # posterior mean tangent while leaving both carrier accelerations intact.
    terminal_blend = guidance.terminal_curvature_gate
    mean_tail_tangent =
        params.terminal_mean_tail_tangent_limit *
        terminal_blend *
