...
# propulsive traveling bend, then give the existing terminal steering residual
# priority over carrier acceleration that lies beyond the actuator envelope.
# No clock, world coordinate, route, or mutable phase is used.

function target_policy_params()
    return (
        version="dogfish3d_intercept_guarded_saturation_priority_v1",
        drive_period=0.55,
        control_period=0.55,
...
function guidance_module(state, params)
    ...
    # The cross and dot products are invariant to rotation of the body frame.
    # They describe the instantaneous inertial closest-pass geometry without
    # introducing a memorized route or an external phase signal.
    ...
    projection_speed = max(
        course_speed,
        params.projection_speed_floor,
    )
...
function turn_response_module(state, guidance, drive, params)
    ...
    # yaw. Correct-sign slow yaw may release steering, but LOS rate and the
    # closest-pass guard can independently veto that release.
    ...
    # Near capture, response alone is insufficient evidence for releasing
    # steering. Achieved velocity must approach the target and project inside
    # the capture corridor. This preserves the completed far/middle route.
    ...
    approach_span = max(
        params.approach_alignment_full -
            params.approach_alignment_start,
        1.0e-6,
    )
    approach_gate = _smoothstep01(
        (guidance.approach_alignment -
            params.approach_alignment_start) /
            approach_span,
    )
...
@inline function _terminal_allocate(
    drive_accel,
    steering_accel,
    terminal_gate,
    acceleration_limit,
)
    # The inherited final-only clamp can erase steering when the opposing raw
    # carrier already exceeds the envelope. Bounding the carrier first lets the
    # same residual shift that saturated half-beat. A convex terminal blend
    # preserves the exact inherited command outside the approach region.
    inherited_accel = clamp(
        drive_accel + steering_accel,
        -acceleration_limit,
        acceleration_limit,
    )
    priority_accel = clamp(
        clamp(
            drive_accel,
            -acceleration_limit,
            acceleration_limit,
    ...
    allocation_gate = _smoothstep01(terminal_gate)
    return (
        (1.0 - allocation_gate) * inherited_accel +
        allocation_gate * priority_accel
    )
end
...
function target_policy(state, params)
    ...
    # Keep the completed intercept veto, then apply the unchanged steering
    # residual through saturation-aware allocation. This changes no command
    # outside the existing terminal gate and adds no steering authority.
