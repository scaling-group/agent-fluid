...
- Preserve the now-evaluated posterior turn-phase allocation as a promising
  trajectory mechanism, but do not treat its single timing gain as portable
  yet. Adding a symmetric `0.10` lag modulation from normalized anterior-joint
  phase and body-frame turn request to the plain LOS-led scaffold captured at
  `19.635T/2.10246L/-0.21272`, versus
  `19.706T/2.10594L/-0.21598` for the plain sample and
  `19.701--19.817T/2.10438--2.10672L/-0.21449-- -0.21655` for the unconfirmed
  corridor pair. More importantly than the sub-repeat-span timing difference,
  it shortened head path from `12.444L` to `12.222L`, reduced mean absolute
  course error below `4L` from `0.290` to `0.233 rad`, and reduced lateral
  speed below `2L` from `0.141` to `0.109U`, while remaining in the coherent
  approximately `0.025/0.013` force/moment class. Retain the invariant—bounded
  posterior wave-shape allocation from observed turn and joint phase—when
  testing one response mechanism, not the sampled coefficient. Its boundary
  is the final reversal: below `1L`, course error/lateral speed rose from
  `0.184 rad/0.037U` to `0.272 rad/0.098U` while requested turn opposed still-
  positive yaw, and mean command rose modestly. Reject additions that lose the
  short approach, increase limit residence or loads, or merely trade earlier
  progress for a larger late hook; require replication or a held-out release
  before calling the phase allocation generally robust.
