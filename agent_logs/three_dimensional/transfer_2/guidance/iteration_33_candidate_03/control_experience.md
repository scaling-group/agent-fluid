...
- Treat response-conditioned carrier-reversal release as range-specific, but
  do not regard the fixed `6--4L` distance handoff as an established
  improvement. Two byte-identical fully released runs captured tightly at
  `16.071--16.088T`, distance integral `1.82203--1.82366L`, and head path
  `13.129--13.166L`. The exact fixed-handoff repeats instead spread across
  `15.939--16.143T`, score `0.06189--0.05620`, integral
  `1.82008--1.82620L`, path `13.107--13.171L`, and peak planar-force/yaw-
  moment `0.03477/0.01715--0.03546/0.01791`; their favorable first result and
  every `6--1L` milestone regressed on repeat while both wakes stayed coherent.
  The handoff pair therefore straddles rather than robustly beats the fully
  released class, despite preserving capture. Keep full target-conditioned
  redirect steering—releasing it produced a `13.363L` late hook and
  `16.247T` capture—but replace or challenge distance-only carrier recoupling
  with bounded measured demand such as normalized body-frame velocity-course
  error, without adding another steering residual. Require repeat evidence
  beyond the `0.203T/0.00612L` handoff spread before claiming a timing or
  integral gain, and reject demand-conditioned recoupling if it loses early
  progress, exceeds the repeat path/rate/load envelope, adds noisy switching,
  or degrades joint margin, terminal yaw/slip, capture, or either wake view.
