...
- Do not use a near-target redirect gate to solve a far-field joint-rate
  allocation problem, and do not treat a single response-aware timing win as
  resolved allocation. Three byte-identical response-aware handoff repeats
  span `19.162--19.338T`, `2.06924--2.07983L` distance integral, and
  `12.304--12.370L` head path; the distance-only handoff at
  `19.3545T/2.07892L/12.416L` overlaps that integral spread even though its
  path is longer. Across all four direct-quiescent captures, both joints reach
  the `260 deg/T` rate ceiling, and acceleration remains aligned with joint
  velocity at greater-than-99%-rate samples on `82.6--83.4%` of anterior and
  `77.6--79.5%` of posterior events, while peak planar-force/yaw-moment
  coefficients stay within `0.02523--0.02542/0.01333--0.01356`. This supports
  one distinct test: preserve the response-aware route/capture scaffold and
  use normalized joint-rate proximity plus action-velocity alignment to
  smoothly withdraw outward rhythmic authority, without a distance gate or
  added steering. The implication is falsified unless repeat evaluation lowers
  rate residence and outward-command impulse near the envelope while
  preserving the `10/8/6L` milestones, capture, short-path envelope, joint
  margin, load class, and coherent wake; otherwise restore the unguarded
  response-aware handoff.
