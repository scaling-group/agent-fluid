...
- Prefer target-relative intercept geometry over measured yaw response when
  releasing transient anterior redirect, and keep the posterior redirect
  continuous. In the current matched batch, projected miss alone
  (`19.723T/2.10565L/-0.21601`) and anterior release on yaw response
  (`19.701T/2.10634L/-0.21675`) did not improve the plain LOS-led controller
  (`19.706T/2.10594L/-0.21598`), whereas their geometry-led composition reached
  `19.701T`, reduced mean distance to `2.10438L`, and produced the batch-best
  `-0.21449` score with a coherent wake and slightly lower peak planar
  force/yaw-moment coefficients (`0.02297/0.01285`). Transfer the mechanism,
  not its corridor values: blend bounded body-frame course error with projected
  miss, relax only the extra anterior curvature after the velocity course
  enters an observed target corridor, and retain the posterior traveling-wave
  contribution. Treat this as provisional until replicated because inherited
  identical-policy captures span a wider scalar band; reject it if the gain
  disappears on repeat or held-out conditions, or if its higher mean command
  (`18.53/17.32` versus `18.20/17.02 rad/T^2`) grows into near-bound residence,
  reduced joint margin, higher loads, wake loss, or a missed capture.
