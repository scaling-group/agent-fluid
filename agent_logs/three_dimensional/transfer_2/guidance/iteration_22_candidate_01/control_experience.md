...
- Treat posterior half-cycle allocation as regime-specific and release it with
  ...
  `19.613T/2.09432L`). Releasing that handoff when the bounded body-frame turn
  request and measured yaw agreed has now replicated at
  `19.162/19.338T`, `2.06924/2.07622L`, and scores
  `-0.18047/-0.18691`, with coherent wakes and nearly identical
  `12.309/12.304L` head paths. In contrast, replacing the task-turn signal with
  velocity-course-correction/yaw agreement regressed to
  `19.398T/2.08187L/-0.19253`, lengthened path to `12.341L`, and raised peak
  planar force to `0.02587` from `0.02535--0.02537`, despite slightly lower
  mean command and the same capture/wake class. This is a semantic negative
  result: an observation validated for terminal redirect is not automatically
  a valid gait-release signal. Restore the replicated task-turn/yaw handoff;
  do not tune the course-agreement gain or add terminal curvature. Reconsider
  the release semantic only if a held-out pose or inflow produces a repeatable
  response mismatch, and require capture, path, timing/integral, command,
  joint-margin, load, and wake improvements outside exact-repeat variation.
