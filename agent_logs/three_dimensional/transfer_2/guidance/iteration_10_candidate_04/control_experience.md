...
- Response-release on the added redirect is the first mechanism-level fix that
  directly targets the terminal hook without deleting the successful capture
  scaffold. Preserve the base LOS-led map, distance relief, and half-cycle
  asymmetry, then taper only the extra redirect curvature when recent yaw is
  already aligned with the commanded turn. This is reusable as a bounded body-
  frame feedback pattern, but only for the additive redirect term. Do not turn
  it into a global gain cut, because the earlier evidence already shows that
  capture depends on keeping the base course lead active.
