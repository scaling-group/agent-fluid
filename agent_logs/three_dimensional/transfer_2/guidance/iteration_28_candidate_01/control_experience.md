...
- Carrier/steering separation with one common traveling-wave scale is supported
  as a progress mechanism, and the assigned parent's response-conditioned
  redirect priority is the first sampled useful-response trigger to improve its
  semantic class. From the same direct-quiescent release, the uncancelled-work,
  predictive-barrier, and dominant-joint governors captured coherently at
  `17.688/17.418/17.990T` with distance integrals
  `1.96419/1.93044/1.98918L`; redirect priority advanced capture to `16.044T`,
  integral to `1.82409L`, and score to `0.05824`. Preserve its invariant—while
  a normalized body-frame course redirect remains unresolved, withdraw one
  common propulsive carrier scale but leave target-conditioned steering
  residuals intact—rather than treating it as another rate threshold. Its
  boundary is actuator and route quality: the fastest path widened to
  `13.178L` from `12.170--12.528L`, peak planar force/yaw moment rose to
  `0.03579/0.01770`, and anterior residence above 99% joint rate was `12.24%`
  versus `4.23%` for the predictive barrier. Inherited logs state that rhythmic
  authority should return when the observed response appears; therefore the
  release test must use the same signed redirect request being prioritized,
  not a distinct LOS-turn request that can oppose it. Reject this mechanism on
  repeat or held-out conditions if it loses coherent capture, gives back the
  timing/integral gain without material path/load relief, or worsens rate,
  command, load, joint-margin, or wake diagnostics; do not call it an actuator
  protector from score alone.
