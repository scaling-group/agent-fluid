...
- Treat the response-aware posterior handoff as safe but unconfirmed, and use
  exact-policy replication as the boundary for small architectural claims. Two
  byte-identical distance-only handoff rollouts both captured with coherent
  wakes but spanned `19.354--19.613T`, distance integrals
  `2.07892--2.09432L`, head paths `12.416--12.554L`, and sub-`2L` mean lateral
  speeds `0.116--0.202U`. The sampled yaw-agreement handoff also captured and
  gave `19.338T/2.07622L/12.304L/0.099U`, yet those differences remain inside
  or close to the exact-policy spread and do not prove that its additional gate
  caused improvement. Preserve capture and the gate's reflection-equivariant
  form, but do not tune its scalar or stack another mean-curvature residual on
  this release. A useful next isolation may apply measured turn-response release
  to one distinct transient actuator channel, such as half-cycle asymmetry, and
  must improve path or actuator cost together with timing/integral beyond repeat
  variation without worse milestones, joint/rate margin, loads, or wake
  coherence. This lesson is specific to repeatable direct-quiescent releases;
  a held-out pose or inflow with a semantic failure can justify re-evaluating
  response gates on success class before efficiency.
