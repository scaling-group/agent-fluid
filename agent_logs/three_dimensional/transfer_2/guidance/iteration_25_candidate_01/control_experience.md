...
- Separate far-field actuator-envelope feedback from terminal steering. A
  near-target redirect reserve regressed from the response-aware repeats'
  `19.162--19.338T` and `2.06924--2.07622L` envelope to
  `19.3325T/2.07854L` while barely changing command or rate residence. In
  contrast, a shared, reversal-safe guard driven directly by normalized joint-
  rate proximity and outward command alignment preserved the coherent
  top-down/oblique traveling wake, advanced every `10/8/6/4/2/1L` milestone,
  and improved capture/integral from `19.162T/2.06924L` to
  `18.755T/2.04031L`; mean anterior/posterior command fell from
  `18.46/17.56` to `17.87/17.07 rad/T^2`, and greater-than-99%-rate residence
  fell from `8.35/4.36%` to `8.06/3.28%`. Preserve this observation-to-
  allocation mechanism rather than tuning a redirect scalar. Its boundary is
  incomplete envelope avoidance: both joints still touched the hard rate
  limit, command remained outward at about `81.5/75.0%` of greater-than-99%
  events, head path grew from `12.309L` to `12.421L`, and peak planar force/yaw
  moment rose from `0.02537/0.01356` to `0.02686/0.01397`. A later refinement
  may anticipate normalized rate headroom while retaining shared-wave and
  reversal authority; reject it unless it further reduces hard-rate residence
  without losing the earlier milestones, capture, short-path/load class,
  joint margin, or coherent wake.
