...
- Keep carrier-energy allocation response-aware, but match response to the
  command being released: a target-route response is not automatically a
  velocity-course-redirect response. Earlier inherited common and joint-wise
  carrier guards improved unguarded `19.162--19.338T` captures to `18.111T`,
  while full-carrier redirect priority reached `16.044T/1.82409L`; predictive
  rate, unconditional negative-work release, and norm-triggered body-load
  protection all reduced some path/load measures but regressed to
  `17.413--17.418T/1.93044--1.94829L` or failed to provide a material load
  benefit. The assigned parent's response-gated negative-work restoration is
  now evaluated twice byte-identically: `16.071/16.088T`, integrals
  `1.82366/1.82203L`, paths `13.166/13.129L`, peak force
  `0.03427/0.03575`, and anterior above-99%-rate residence `12.01/11.93%`.
  Those changes remain within the prior full-carrier repeat envelope and do
  not establish a repeatable improvement; applying the same route-response
  residual directly to the redirect instead lengthened path to `13.363L`,
  raised sub-`2L` mean yaw to `0.269 rad/T`, and ended at `0.589 rad/T` yaw.
  Logged-signal replay explains the boundary: route and course-redirect signs
  disagree in `18.8--19.1%` of redirect-active approach rows in the two
  repeats, while yaw follows the redirect in at least `99.5%` of those rows.
  Future response handoffs should compare measured yaw with the specific
  bounded route or redirect request they govern, not stack another load,
  distance, clock, or terminal-slip gate. Falsify command matching if it does
  not improve middle-field progress or trajectory quality beyond repeat
  variation without worse capture, wake coherence, command, joint margin,
  rate residence, force/moment, or terminal yaw/slip; reconsider load feedback
