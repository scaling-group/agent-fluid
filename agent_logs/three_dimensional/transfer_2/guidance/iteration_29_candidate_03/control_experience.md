...
- Carrier-energy withdrawal is supported only with a formulation boundary:
  preserve response-aware targeting, separate the rhythmic carrier from its
  steering residual, and use one scale for the two-joint traveling wave rather
  than clipping complete joint commands independently. The evidence now also
  separates response allocation from rate protection. A common nonnegative-
  work guard captured at `17.688T/1.96419L`; a predictive rate-margin barrier
  reduced greater-than-90%-rate residence to `15.03/0.73%` and captured at
  `17.418T/1.93044L`. Adding an unfulfilled body-frame velocity-course redirect
  to the common carrier request then produced a genuinely different useful
  trajectory: every `10/8/6/4/2/1L` milestone advanced to
  `5.863/7.838/9.779/11.726/14.201/15.604T`, capture advanced to `16.044T`, and
  the integral fell to `1.82409L`, with a coherent wake and zero angle-limit
  residence. The cost is a wider `13.178L` path, force/moment peaks of
  `0.03579/0.01770`, and `17.76/8.12%` residence above 90% joint rate. A direct
  normalized-load composition failed its material-benefit boundary: suppressing
  positive common-carrier work above force/moment thresholds reduced the peaks
  only to `0.03500/0.01755` and rate residence only to `17.49/7.85%`, while
  capture regressed from `16.044T` to `16.258T`, distance integral from
  `1.82409L` to `1.83377L`, and path from `13.178L` to `13.191L`. Do not tune
  those load thresholds in isolation or infer that instantaneous load magnitude
  identifies expendable thrust. Transfer the supported invariant—give bounded
  target-conditioned redirect priority until measured signed yaw responds,
  while preserving carrier phase and steering residual—not the sampled scales.
  If actuator protection is composed next, the separately supported predictive
  rate-margin allowance is the relevant test, but its composition with redirect
  remains unevaluated; reject it if it loses the early milestones, capture,
  coherent wake, joint margin, or worsens path/load/rate costs. Held-out poses
  or inflows that turn response-priority withdrawal into thrust loss also
  falsify its portability.
