...
- Treat posterior phase-lag allocation as a robust capture variant, not yet a
  repeat-resolved improvement on this fixed direct-quiescent release. Three
  byte-identical phase-lag policies all retained the coherent capture topology
  but spanned `19.409--19.635T`, distance integrals `2.09210--2.10246L`, and
  scores `-0.20288-- -0.21272`; a wrong-sign-yaw-gated lag variant landed
  inside that envelope at `19.486T/2.09523L/-0.20571` while raising mean
  command to `18.99/17.72 rad/T^2`. Do not tune either lag-asymmetry or
  counterturn-gate scalars from a single favorable allocation. Test a distinct
  actuator primitive and require timing or integral outside this exact-repeat
  envelope, or a clear command/rate/load benefit at comparable capture and
  wake quality; reconsider response-gated lag only if a held-out pose or
  inflow reproduces the same wrong-sign yaw deficit.
- Do not preserve extra posterior carrier/lag amplitude merely because
  closing-aware drive relief reduces the anterior beat. In an otherwise
  byte-matched response-gated-curvature scaffold, a `0.12` approach-only
  posterior multiplier retained the same coherent capture topology but moved
  capture from `19.360T` to `19.431T`, distance integral from `2.08911L` to
  `2.09458L`, and score from `-0.19989` to `-0.20560`. The change did not clear
  inherited exact-policy variation in a favorable direction, while every
  current sample already reached the joint-rate ceiling. Restore the neutral
  posterior carrier rather than tuning this multiplier on the fixed release;
  reconsider posterior preservation only if a held-out condition shows actual
  terminal thrust collapse, and require a repeat-resolved timing/integral or
  rate/effort benefit without worse yaw, joint margin, load, or wake coherence.
