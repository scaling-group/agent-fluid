...
- Do not equate same-sign body yaw with satisfaction of a body-frame velocity-
  course redirect. Restoring negative-work carrier reversal from that proxy
  repeated at `16.071--16.088T/1.82203--1.82366L`: early `10/8/6L`
  milestones improved slightly, but `4/2/1L` and capture slowed relative to
  the `16.044T/1.82409L` plain redirect sample, while path, peak load, and
  greater-than-90%-rate residence stayed inside the established redirect
  repeat envelope and sub-`2L` mean yaw rose from `0.0943` to
  `0.1149--0.1275 rad/T`. Releasing the redirect residual from the same proxy
  was worse (`16.247T/1.82240L`, `13.363L` path, `0.2690 rad/T` sub-`2L`
  yaw). Restore the common full-carrier redirect governor instead of tuning a
  yaw-response release gain. Reconsider an authority handoff only when a
  normalized observation certifies course satisfaction rather than turn sign,
  and require timing/integral or a distinct useful trajectory plus path, rate,
  load, joint-margin, and two-view wake evidence beyond repeat variation.
