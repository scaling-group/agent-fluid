...
- Treat the route-request/yaw response advance within that posterior handoff as
  a repeatable path-shaping detail, not a proven timing or integral gain. Three
  byte-identical response-aware executions captured at `19.162/19.338/19.338T`
  with integrals `2.06924/2.07622/2.07983L` and head paths
  `12.309/12.304/12.370L`; the simpler distance-only handoff captured at
  `19.3545T/2.07892L` on a `12.416L` path. The response form consistently
  retained a shorter route, but its arrival and integral overlap execution
  spread, so do not tune another response scalar or stack a terminal gate.
  A distinct supported test axis is normalized proprioceptive carrier
  allocation: these captures spend `8.22--8.35%/4.35--4.44%` of anterior/
  posterior samples above 99% of the joint-rate envelope, and inherited trace
  analysis found outward carrier acceleration in about `82--83%/78%` of those
  samples, while angle-limit residence stays zero and the wake/load class is
  stable. Test smooth common-carrier relief only near that rate boundary while
  preserving steering; reject it unless rate residence falls without losing
  capture, early progress, the short path, coherent wakes, joint margin, or the
  approximately `0.0254/0.0136` force/moment class.
