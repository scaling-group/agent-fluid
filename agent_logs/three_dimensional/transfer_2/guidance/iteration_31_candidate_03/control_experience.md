...
- Treat response-conditioned carrier-reversal release as range-specific, not
  as one global refinement of the redirect-priority carrier. Two byte-identical
  releases crossed `10/8/6L` at
  `5.808--5.841/7.761--7.799/9.724--9.762T`, ahead of the plain carrier at
  `5.863/7.838/9.779T`, with slightly shorter `13.129--13.166L` paths and
  no worse coherent-wake or joint-margin class. But the plain carrier had
  caught them by `4L` (`11.726T` versus `11.759--11.776T`) and remained ahead
  through `3/2/1L` and capture (`16.044T` versus `16.071--16.088T`); peak
  force/moment and rate residence overlapped repeat-scale variation. Releasing
  added redirect curvature from the same yaw-response signal was not the cure:
  it lengthened path to `13.363L`, raised late yaw, and delayed capture to
  `16.247T`. Preserve the early reversal-release observation only through a
  smooth normalized-distance handoff back to the full redirect-priority
  carrier before the terminal regime; do not globally retune either release
  gain or use same-sign yaw alone to remove target steering. Falsify the
  handoff if it cannot retain the earlier far milestones and recover the plain
  carrier's 4L-to-capture timing/integral without worse path, command, rate,
  load, angle margin, terminal yaw/slip, or two-view wake coherence.
