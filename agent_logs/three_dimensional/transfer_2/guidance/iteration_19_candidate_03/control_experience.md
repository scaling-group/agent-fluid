...
- Treat direct posterior response as a provisional trajectory-shaping mechanism,
  not a scalar champion. In the four sampled direct-quiescent captures, the
  response-gated posterior bend reached `19.360T/2.08911L/-0.19989` and reduced
  mean absolute yaw below `2L` to `0.285 rad/T`, versus `0.349--0.406 rad/T` for
  two exact posterior-lag evaluations and a posterior-amplitude variant. Both
  wake views retained the same coherent alternating street and terminal hook,
  however, and its mean commands rose to `19.25/17.71 rad/T^2` compared with
  `18.09/17.33` for the amplitude variant. The exact lag pair itself varied
  from `19.409T/-0.20288` to `19.508T/-0.20789`, while assigned-parent logs add
  nearby capture scores `-0.21272`, `-0.20571`, and `-0.20065`. Preserve a
  bounded yaw-opposition posterior bend when terminal-yaw authority matters,
  but test a structurally distinct posterior allocation rather than tuning its
  phase or gate scalars. Require repeat- or held-out-resolved timing/integral or
  yaw benefit, and reject it if command headroom, joint margin, load envelope,
  capture, or wake coherence regresses.
- Treat actuator headroom as a constraint on a useful trajectory, not an
  independent objective, and do not rescale the posterior carrier merely
  because joint rate reaches its ceiling. Inherited selective posterior-rate
  unloading lowered mean commands to `18.16/16.93 rad/T^2`, but both joints
  still reached `260 deg/T`; relative to the sampled response-gated bend it
  delayed capture from `19.360T` to `20.207T`, raised mean distance from
  `2.08911L` to `2.14149L`, lengthened head path from `12.052L` to `12.539L`,
  and increased sub-`2L` mean yaw from `0.285` to `0.405 rad/T`. The assigned
  parent's opposite intervention—amplifying posterior carrier and lag during
  closing relief—also failed to improve its exact base, reaching
  `19.431T/2.09458L/-0.20560` versus `19.360T/2.08911L/-0.19989` and raising
  posterior near-command residence from `33.32%` to `34.16%`. Avoid further
  posterior carrier scaling on this release; reconsider actuator-state
  redistribution only if it preserves the traveling bend and braking and
  yields repeat- or held-out-resolved trajectory benefit without worse joint
  margin, loads, capture, or wake coherence.
