...
- Treat projected-miss release as an actuator-allocation result, not a generic
  terminal gain. After several response/phase gates and geometric blends stayed
  in the same capture band, the sampled anterior-only corridor became the
  finite leader at `19.701T`, score `-0.21449`, and mean distance `2.10438L`,
  versus `19.706T/-0.21598/2.10594L` for the plain LOS-led redirect. It retained
  the coherent three-dimensional wake, `0.0241/0.0129` peak planar-force/yaw-
  moment class, sub-`34deg` joints, and roughly `33--37%` near-command-bound
  residence. The improvement is modest and did not cure the terminal hook:
  at first crossing `1L` its projected miss was only `0.028L` while yaw was
  `0.511rad/T`, and it captured with `0.150L` miss and `0.482rad/T` yaw. In
  contrast, the assigned parent's release of both redirect shares through the
  broader `2L--1.5L` approach reduced terminal yaw to about `0.012rad/T` but
  delayed capture to `19.784T`, worsened mean distance to `2.10886L`, and
  scored `-0.21870`. Preserve the anterior corridor and posterior propulsive
  lag; avoid releasing posterior redirect throughout the `2L` approach. A
  later worker may test posterior straightening only after a sub-`1.2L`,
  positive-closing, inside-corridor observation. Reject it if capture or the
  sampled arrival/integral band is lost, or if terminal yaw/hook, wake
  coherence, loads, joint margin, and command headroom do not improve together.
