...
- Do not tune or stack either phase-conditioned posterior carrier amplitude or
  ...
  moment to `0.02535/0.01343`, again without a new visual class. Stacking the
  prefill's drive-relief posterior carrier multiplier on the direct-curvature
  gate also regressed its exact base from `19.360T/2.08911L/-0.19989` to
  `19.431T/2.09458L/-0.20560` and raised posterior 90%-smooth-bound residence
  from `33.32%` to `34.16%`. Treat all three as concrete negative mechanism
  results despite finite capture and small scalar leads: restore constant
  carrier amplitude, base lag, and mean curvature before testing a distinct
  posterior actuator path; reconsider them only if a held-out pose exposes
  repeatable response or amplitude insufficiency and the mechanism then clears
  exact-policy timing/integral variation or materially improves actuator
  headroom without worse path, load, joint margin, capture, or wake coherence.
...
- Do not treat the repeatable late hook of this fixed direct-quiescent release
  as evidence for another terminal release, projected-miss corridor, or
  unfiltered slip residual. Full response release was slowest at
  `19.850T/2.11128L/-0.22093`, and anterior-only release varied between
  `19.701--19.877T` without clearing executable repeat variation. An exact
  projected-miss/intercept controller then replicated at
  `19.701--19.817T/2.10438--2.10672L`, straddling the simpler LOS-led sample's
  `19.706T/2.10594L` while using more mean command. Finally, an approach-gated
  relative-crossflow tail residual retained the same wake and capture class but
  regressed to `19.894T/2.11284L/-0.22287`, raised sub-`2L` mean yaw from
  `0.437` to `0.486 rad/T`, and increased head-path inefficiency. Restore the
  continuous LOS/course scaffold on this release; revisit one of these
  observations only when a held-out pose or inflow exposes a repeatable
  intercept or slip failure, and require a different useful trajectory plus a
  timing/integral benefit beyond repeats without worse command, load, joint
