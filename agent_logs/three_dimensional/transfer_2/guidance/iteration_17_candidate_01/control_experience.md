...
- Separate a semantic response improvement from an unresolved scalar gain when
  evaluating terminal steering. The sampled wrong-sign-yaw-gated posterior
  bend retained the coherent capture/load class and reduced yaw rate at
  `1L`/capture to `0.142/0.073 rad/T`, versus `0.287--0.334/0.142--0.169`
  for two clean phase-lag captures; final course error likewise fell to
  `0.278 rad` from `0.350--0.404`. Its `19.360T/2.08911L/-0.19989` result,
  however, remains inside the inherited `0.226T` identical-policy variation
  boundary and raised anterior mean command/near-bound residence to
  `19.25 rad/T^2/37.07%`, so preserve the normalized response-disagreement
  observation but do not tune the curvature limit as a proven score gain. A
  standalone phase-conditioned posterior-amplitude allocator supplied a
  complementary headroom signature (`18.09 rad/T^2/35.13%`) and smaller final
  course error (`0.137 rad`), but slowed capture to `19.552T`, ended with
  `0.513 rad/T` yaw, and raised peak planar force/yaw moment to about
  `0.02535/0.01343`. Do not promote that allocator alone; a combination test
  is useful only if it retains the response gate's sub-`1L` yaw/course benefit
  and the amplitude sample's headroom without worse arrival, integral, loads,
  joint/rate residence, or wake topology.
