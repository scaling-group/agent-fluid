...
- Treat posterior half-cycle allocation as regime-specific and release it with
  the task-level turn response, not with every directional error used elsewhere
  in the controller. A global far-field carrier-amplitude asymmetry reached
  `10/8/6L` about `0.12--0.14T` early and lowered mean anterior command from
  `19.03` to `18.09 rad/T^2`, but lengthened head path from `12.095L` to
  `12.549L` and doubled sub-`2L` lateral speed; a distance-only amplitude-to-lag
  handoff then varied across exact executions (`19.354T/2.07892L` versus
  `19.613T/2.09432L`). Three exact task-turn/yaw releases all captured with
  coherent wakes but now span `19.162--19.420T`, `2.06924--2.08114L`, scores
  `-0.18047-- -0.19159`, and `12.304--12.380L` head paths. This wider repeat
  envelope supports the release semantic as a stable scaffold but no longer
  supports claiming that it alone guarantees the shorter path. Replacing the
  task-turn signal with velocity-course-correction/yaw agreement produced
  `19.398T/2.08187L/-0.19253`, a `12.341L` path, and peak planar force
  `0.02587`; an observation validated for terminal redirect is therefore not
  automatically a valid gait-release signal. Keep the task-turn/yaw semantic,
  do not tune the course-agreement gain, and require performance outside the
  enlarged exact-repeat envelope before attributing benefit to another release.
- Do not infer actuator headroom from redirect-gated carrier attenuation. That
  mechanism captured at `19.333T/2.07854L` with the same coherent wake and a
  `12.370L` path, all inside the exact task-turn/yaw execution envelope. It
  changed mean command to `18.23/17.37 rad/T^2` but left greater-than-90%
  command residence at `35.51/33.74%` and greater-than-99% rate residence at
  `6.26/3.04%`, versus `35.57--35.99/33.73--33.96%` and
  `6.40--6.43/3.02--3.04%` for exact response releases. This is a concrete
  negative result: indirect carrier budgeting neither created a semantic
  improvement nor materially relieved the repeatedly touched rate envelope.
  Restore the undivided carrier instead of tuning its reserve fraction; if
  actuator residence is the objective, test normalized proprioceptive joint-
  rate or wave-shape feedback directly and require reduced residence without
  losing capture, path, joint margin, loads, or wake coherence.
