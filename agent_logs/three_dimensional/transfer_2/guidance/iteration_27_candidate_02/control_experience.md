...
- Carrier/steering separation with one common traveling-wave scale is supported
  as a progress mechanism, but no sampled power/rate trigger yet resolves its
  speed-versus-headroom tradeoff. Relative to the inherited whole-command
  governor's `18.755T/2.04031L` capture/integral, all four separated-carrier
  samples captured coherently at `17.688--18.111T` with integrals
  `1.96419--2.00209L`. The summed-nonnegative-work trigger was fastest, reaching
  `10/8/6/2L` at `6.496/8.938/11.220/16.027T`, but it also produced the widest
  `12.330L` head path, peak planar force/yaw moment `0.03198/0.01651`, and
  `12.13%` anterior residence above 99% of the rate envelope. A dominant-joint
  common trigger shortened the path to `12.170L` and reduced anterior/posterior
  99%-rate residence to `8.44/0.89%`, but captured `0.302T` later; independent
  carrier attenuation reached `8.17/0.43%` and lower `0.02716/0.01414` loads,
  yet raised sub-`2L` mean absolute yaw to `0.373 rad/T`. Preserve the common
  scale and untouched target-conditioned residual: inherited worker notes show
  that this was the intended formulation boundary and that arbitrary rate
  suppression had already regressed. Do not rank or tune its trigger by score
  or rate threshold alone. The reusable next class of test is to condition
  near-envelope carrier withdrawal on a separate normalized useful-response
  signal such as unresolved body-frame course error and signed yaw response.
  Retain such allocation only if repeat CFD preserves capture and coherent wake
  while combining the `17.688--18.111T` timing class with a path near
  `12.17--12.20L`, lower near-rate residence, and inherited load/joint margins;
  otherwise restore the best sampled common governor.
