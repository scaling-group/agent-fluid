...
- Stop subdividing response/phase release around the LOS-led redirect; neither
  actuator path has produced evidence beyond the executable repeat band. The
  strongest unmodulated sample captured at `19.706T`, score `-0.21598`, and
  mean distance `2.10594L`, while its semantic repeat captured at `19.888T`,
  `-0.22270`, and `2.11324L`. Releasing both redirect contributions gave
  `19.850T/-0.22093/2.11128L`, and posterior half-cycle scaling regressed to
  `20.168T/-0.24199/2.13279L` without a useful wake or load change. The
  assigned parent's narrower anterior-only release then captured at
  `19.877T`, `-0.22325`, and `2.11339L`: effectively the semantic repeat, not
  the predicted improvement, despite retaining continuous posterior
  curvature. Preserve the unmodulated posterior traveling wave and treat this
  as a concrete negative result against another release gate. Test a different
  normalized observation or geometric error, such as bounded projected miss,
  and reject it unless capture, path topology, arrival or mean distance changes
  beyond repeat variation without degrading wake coherence, loads, joint
  margin, or command headroom.
