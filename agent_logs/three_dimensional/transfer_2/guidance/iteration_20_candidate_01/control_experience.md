...
- Treat actuator headroom as a constraint on a useful trajectory, not an
  independent objective, and do not attenuate or amplify the posterior carrier
  merely because joint rate reaches its ceiling. Inherited selective
  posterior-rate unloading lowered mean commands to `18.16/16.93 rad/T^2` but
  both joints still reached `260 deg/T`; relative to the sampled response-gated
  bend it delayed capture from `19.360T` to `20.207T`, raised the distance
  integral from `2.08911L` to `2.14149L`, lengthened head path from `12.052L` to
  `12.539L`, and increased sub-`2L` mean yaw from `0.285` to `0.405 rad/T`, with
  a visibly wider terminal arc despite a coherent wake. The prefill's opposite
  intervention—multiplying posterior carrier and lag during closing drive
  relief—also failed to improve its exact base, reaching
  `19.431T/2.09458L/-0.20560` versus `19.360T/2.08911L/-0.19989` while raising
  posterior near-command residence from `33.32%` to `34.16%`. Restore the
  unmultiplied response-gated scaffold for a controlled repeat; reconsider
  actuator-state redistribution only if it preserves the traveling bend and
  braking rather than trading lower effort for a longer path, and require
  repeat- or held-out-resolved timing/integral or trajectory benefit with
  non-worse joint margin, loads, capture, and wake coherence.
...
- After capture is established, stop stacking posterior half-cycle, response,
  handoff, or approach-authority scalars when they preserve the same path
  class. The sampled posterior-amplitude-to-lag handoff has the best scalar
  result at `19.354T/2.07892L/-0.18968`, but its two-view sheet retains the same
  coherent straight transit and late capture hook as the response-bend policy;
  it is only `0.0055T` faster than that policy's best run. The byte-identical
  response-bend repeats themselves span `19.360--19.591T`,
  `2.08911--2.09637L`, and scores `-0.19989` to `-0.20651`, so neither the
  handoff nor small scalar differences establish a distinct mechanism. A
  body-frame reconstruction instead found `0.530--0.549 rad` mean absolute
  velocity-course error beyond `6L`, where the sampled redirect is disabled. Test the
  distinct feedback-placement axis: one small speed-gated course residual
  before approach while holding the posterior traveling-wave lag fixed. The
  assigned parent's score-only `left_domain` log reached just `11.895L` but
  lacks its policy, trajectory, and wake diagnostics; treat it as a boundary
  to preserve the complete capture scaffold, never as causal evidence for a
  particular residual or gain. Support earlier course feedback only if it
  reduces far/middle course error and the late-hook path beyond exact-policy
  repeat variation without worse capture, command/rate residence, loads, joint
  margin, or two-view wake coherence; otherwise avoid it on this release and
  seek a held-out pose or inflow that exposes a distinct failure.
