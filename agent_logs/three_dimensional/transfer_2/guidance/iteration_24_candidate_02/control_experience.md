...
- Once that scaffold captures, joint-state half-cycle asymmetry is supported as
  ...
  do not compose it with an earlier speed-gated mean course redirect on this
  release: the inherited extension left the domain at `8.750T` after reaching
  only `11.895L`, so only a held-out trajectory exposing a repeatable
  middle-field course failure would justify revisiting that architecture.
...
- Near-rate-limit residence is an actuator boundary, not evidence that
  instantaneous traveling-carrier suppression is useful. The inherited
  proprioceptive governor attenuated both carrier components only when joint
  speed exceeded `95%` of the rate envelope and carrier acceleration remained
  outward; it preserved capture but delayed it to `19.657T`, raised distance
  integral to `2.09937L`, and scored `-0.20967`, worse than both exact
  response-aware repeats at `19.162--19.338T/2.06924--2.07622L`. Do not tune
  that governor or replace it with global frequency/amplitude relief. If rate
  residence must be revisited, require an allocation mechanism that preserves
  the phase-related carrier and demonstrate recovered headroom together with
  early progress, short path, coherent wakes, load class, and capture.
