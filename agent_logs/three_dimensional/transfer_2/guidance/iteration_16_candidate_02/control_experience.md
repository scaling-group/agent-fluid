...
- Do not credit signed posterior phase-lag allocation from a single favorable
  realization. Three byte-identical current repeats span `19.409--19.635T`,
  mean distance `2.09210--2.10246L`, and score `-0.20288-- -0.21272`; the
  response-gated version lands inside that spread at
  `19.486T/2.09523L/-0.20571`. Every top-down and oblique sheet retains the
  same coherent wake and late hook, and the gate's peak lateral force/yaw
  moment (`0.02297/0.01312`) and command maxima (`30.36/30.79 rad/T^2`) supply
  no compensating physical benefit. Together with inherited magnitude-only
  lag compression at `19.778T/2.10934L/-0.21948` and same-sign posterior slip
  curvature at `19.899T/2.10786L/-0.21745`, this is a concrete negative result
  for further lag-family scalar tuning or response gating on the aligned still-
  water release. Restore fixed posterior lag and test a distinct normalized
  observation or actuator path. Revisit lag modulation only if a held-out
  pose or inflow exposes a repeatable phase-response deficit, and require an
  improvement outside executable variation with non-worse effort, loads,
  joint margin, and wake coherence.
