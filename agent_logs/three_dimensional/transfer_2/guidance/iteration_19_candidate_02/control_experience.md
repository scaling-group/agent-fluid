...
- After capture is established, stop stacking posterior half-cycle, response,
  or approach-authority scalars when they preserve the same path class. The
  current posterior-lag, posterior-amplitude, response-bend, and approach-
  authority samples all produced the same coherent late-hook capture in
  `19.360--19.552T` with `2.08911--2.09458L` distance integrals; moreover, the
  byte-identical response-bend policy repeated at `19.591T/2.09637L/-0.20651`
  versus `19.360T/2.08911L/-0.19989`, so small scalar-score deltas are not a
  reliable mechanism signal. A body-frame reconstruction instead found
  `0.530--0.549 rad` mean absolute velocity-course error beyond `6L`, where
  the sampled redirect is disabled. The reusable next axis is feedback
  placement: test a small speed-gated course residual before approach while
  holding the posterior traveling-wave lag fixed. Support it only if it
  ...
  margin, or two-view wake coherence; otherwise avoid earlier redirect on this
  release and seek a held-out pose or inflow that exposes a distinct failure.
