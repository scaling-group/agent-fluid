...
- Carrier-energy withdrawal is now supported only with a formulation boundary:
  preserve response-aware targeting, separate the rhythmic carrier from its
  steering residual, and avoid independent whole-command attenuation. Against
  unguarded response-aware captures at `19.162--19.338T`, integrals
  `2.06924--2.07622L`, and paths `12.304--12.309L`, both a joint-wise carrier
  guard and a common phase-preserving carrier governor captured at `18.111T`
  with integrals `2.00209/1.99710L` and coherent wakes. The common governor
  retained the shorter `12.198L` path, while the joint-wise guard lowered mean
  commands from `18.84/17.54` to `18.46/17.13 rad/T^2`, greater-than-90%-rate
  residence from `17.6/13.7%` to `16.1/10.0%`, and peak force/moment from
  `0.02729/0.01435` to `0.02716/0.01414`; both guarded load peaks exceed the
  unguarded approximately `0.0254/0.0136` class. Other inherited governor
  formulations regressed to `19.657T`, so rate suppression in isolation is not
  portable. The reusable next refinement is one common carrier scale triggered
  by the sum of nonnegative joint-wise carrier work, which prevents cross-joint
  power cancellation without sacrificing phase. Reject it unless repeat
  evaluation retains capture, the guarded milestone/timing class, a path no
  wider than the unguarded envelope, joint margin, and wake coherence while
  recovering rate/command/load headroom relative to the shared signed-power
  governor; otherwise retain the sampled shared governor rather than tune gait
  or terminal-steering scalars.
