...
- The first compact bearing-to-curvature child improved the layered 2D
  ...
  residence.
- The inherited follow-ups now falsify the proposed direct fixes to that
  compact controller.  Coordinating the anterior acceleration and posterior
  mean bend (`solver_8019a82512ce`) scored `-14.7619`, reached only
  `12.1609 L`, and exited at `12.3200 L`; replacing mean bend with posterior
  half-cycle asymmetry (`solver_369f0b43550e`) scored `-14.7660`, reached only
  `12.0424 L`, and exited at `12.3285 L`.  The independent sampled
  course/half-cycle policy corroborates the latter failure with a short wake,
  an upper exit at `8.59 T`, and a `11.9932 L` minimum.  Thus the reusable
  positive result is narrower than previously stated: preserve the centered
  joint-state oscillator and posterior lag, but do not scalar-retune either a
  same-sign DC bend across the two actuation paths or posterior half-cycle
  amplitude as the next steering test.  Change how persistent route error
  moves the propulsive wave (for example, its bounded equilibrium offset) and
  require both target-course contraction and retained wake/translation before
  interpreting a different exit as progress.  This negative result does not
  rule out those primitives for another morphology or a genuinely cycle-scale
  route estimator; it applies to the present two-joint L64 gait and its
  sub-cycle bearing/rate observations.
