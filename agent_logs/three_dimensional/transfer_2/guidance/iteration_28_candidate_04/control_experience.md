...
- Treat response-gated redirect priority as useful but actuator-costly, and
  preserve reversal authority when governing the traveling carrier. On the
  latest direct-quiescent samples it advanced the `10/8/6/2L` milestones to
  `5.863/7.838/9.779/14.201T` and capture to `16.044T/1.82409L` (score
  `0.05824`), versus `17.418--17.990T/1.93044--1.98918L` for predictive,
  uncancelled-work, and dominant-joint guards. But it also produced the longest
  head path (`13.178L` versus `12.170--12.528L`), the highest planar-force/yaw-
  moment peaks (`0.03579/0.01770` versus at most `0.03198/0.01651`), and
  `12.24%` anterior residence above 99% of the rate envelope; the predictive
  barrier reduced that residence to `4.23%` and peaks to `0.03001/0.01495` at
  the cost of slower capture. Together with the inherited matched comparison,
  this shows that a common full-carrier scale can trade speed against protection
  by weakening negative-work reversal when either joint injects energy. Keep
  the body-frame redirect residual, but test an energy-direction decomposition
  that attenuates only positive-work carrier components while passing reversal
  and steering unchanged; do not tune the 98% threshold or clip full joint
  commands independently. Falsify this implication if rate residence, outward
  impulse, load, and path do not improve while early milestones, capture,
  distance integral, joint margin, terminal yaw, and the coherent two-view wake
