...
- Treat posterior half-cycle allocation as regime-specific and response-
  released, not as one global amplitude/lag choice or a scalar-score gain. A
  far-field carrier-amplitude asymmetry reached `10/8/6L` about `0.12--0.14T`
  before the posterior-lag sample and lowered mean anterior command from
  `19.03` to `18.09 rad/T^2`, but its global use lengthened head path from
  `12.095L` to `12.549L` and doubled sub-`2L` lateral speed. The resulting
  distance handoff then captured twice but varied materially across identical
  executions (`19.354/2.07892L` versus `19.613T/2.09432L`). Advancing that
  handoff when body-frame turn request and yaw response agreed preserved the
  coherent capture class and improved head path to `12.304L`, `2--4L` course
  error/lateral speed to `0.289 rad/0.196U`, and integral to `2.07622L`, versus
  `12.416--12.554L`, `0.347--0.441 rad/0.230--0.250U`, and
  `2.07892--2.09432L` for the distance-only executions. This supports measured-
  response release as a path-shaping observation, but not its sampled gain:
  the `0.00270L` best-case integral margin is smaller than repeat spread and
  mean anterior command rose to `18.45 rad/T^2`. Require replication or a
  meaningfully different lower-slip trajectory beyond that spread without
  worse command, joints, loads, or wakes; if response semantics cannot clear
  that boundary, restore the evaluated handoff rather than tuning another
  agreement scalar or adding terminal curvature.
- Treat normalized joint-rate proximity and outward action alignment as a
  control observation rather than relying on the hard rate clamp. Added to the
  response-aware traveling-wave handoff, a smooth shared rate trigger with
  per-joint outward-work attenuation advanced the `10/8/6/4/2L` crossings by
  `0.137/0.242/0.231/0.325/0.395T` and capture from `19.162T` to `18.755T`.
  It lowered mean anterior/posterior command from `18.46/17.56` to
  `17.87/17.07 rad/T^2`, posterior greater-than-90%-command residence from
  `33.96%` to `30.65%`, and posterior greater-than-99%-rate residence from
  `4.36%` to `3.28%`, while preserving ample angle margin and the coherent
  two-view wake. This is also the only current sampled capture to separate
  materially from the inherited roughly `-0.18--0.19` score cluster, at
  `-0.15214`, but it has only one evaluation: head path widened from
  `12.309L` to `12.421L` and peak planar force/yaw moment rose from
  `0.02537/0.01357` to `0.02686/0.01397`. Transfer the mechanism, not its
  thresholds: use normalized state and a smooth envelope-proximity gate, keep
  reversal authority, and next distinguish rhythmic carrier from route
  steering rather than adding another terminal turn. Require replication of
  the early-milestone/capture gain plus reduced rate residence; falsify it if
  path, load, joint margin, capture class, or wake coherence regresses, or if
  the benefit collapses under a held-out pose or inflow.
