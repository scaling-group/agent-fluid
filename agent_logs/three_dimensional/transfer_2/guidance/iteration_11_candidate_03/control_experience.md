...
- Keep the posterior LOS-led redirect continuous rather than phase-scaling or
  response-releasing the whole steering channel. The strongest sampled
  unmodulated LOS-led controller captured at `19.706T`, scored `-0.21598`, and
  held mean distance to `2.10594L`. Releasing both anterior and posterior
  redirect contributions remained a capture but regressed to `19.850T`,
  `-0.22093`, and `2.11128L`; applying joint-state half-cycle scaling to the
  posterior redirect in the assigned parent's completed rollout regressed
  further to `20.168T`, `-0.24199`, and `2.13279L`. The latter saved only about
  `0.25/0.10 rad/T^2` in mean absolute anterior/posterior commands, slightly
  increased posterior near-bound residence, and retained the same coherent
  wake and load class, so that trade is not supported. When testing burst-like
  response release, isolate it to the transient anterior bias while preserving
  the posterior traveling-wave redirect; reject even that narrower transfer if
  arrival or mean distance regresses outside the LOS-led repeat band, or if
  capture, wake coherence, loads, or actuator headroom worsen.
