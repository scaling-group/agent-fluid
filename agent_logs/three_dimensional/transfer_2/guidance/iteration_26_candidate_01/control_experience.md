...
- Distinguish carrier-separated rate feedback from the earlier whole-command
  proprioceptive governor: the latter delayed capture to
  `19.657T/2.09937L/12.427L`, but both current carrier/steering decompositions
  advanced every distance milestone and captured at `18.111T`, beyond the
  unguarded response-aware repeat envelope of `19.162--19.338T`. Independent
  carrier alignment reached `2.00209L/12.281L`, cut posterior residence above
  99% of the rate bound from `4.35--4.36%` to `0.43%`, held anterior residence at
  `8.17%`, and retained coherent top-down and oblique wakes. A shared
  phase-preserving carrier scale improved integral/path further to
  `1.99710L/12.198L`, but shifted residence into the anterior joint (`10.45%`),
  left posterior residence at `3.55%`, and raised peak planar force/yaw moment
  to `0.02729/0.01435` from the unguarded
  `0.02535--0.02537/0.01335--0.01356` class. Transfer the supported invariant—
  separate rhythmic carrier from target steering and withdraw only measured
  outward work near an owned actuator boundary—not a global gait reduction.
  Treat posterior-selective withdrawal as the next causal ablation because
  posterior headroom is the consistent actuator benefit; reject it if the
  earlier milestones/capture, sub-`2.01L` integral, at-most-`12.31L` path,
  posterior headroom, anterior residence, joint margin, load class, or either
  coherent wake view regresses. The shared scale remains unsuitable when it
  merely transfers the constraint into the anterior wave generator.
