...
- Do not trade target progress for posterior actuator headroom by
  redistributing tracking damping or clipping same-direction acceleration near
  the rate ceiling. Against the sampled clean phase-lag capture at
  `19.409T/2.09210L/-0.20288`, the assigned parent's posterior
  damping-asymmetry replacement materially reduced posterior mean command from
  `17.72` to `15.96 rad/T^2`, 90%-bound residence from `33.8%` to `27.4%`, and
  80%-rate residence from `22.0%` to `15.6%`, but lengthened the head path from
  `12.095L` to `12.533L` and slowed capture to `20.647T/2.16229L/-0.27030`.
  The inherited same-direction posterior rate governor likewise retained
  capture but reached only `20.207T/2.14149L/-0.25057`, with almost unchanged
  posterior 90%-command and 80%-rate residence (`33.8%/21.8%`). These are
  concrete negative mechanism results: preserve braking and the established
  posterior wave allocation; revisit actuator-state relief only if it retains
  the clean arrival/integral and coherent trajectory class while delivering a
  material headroom gain, not merely lower effort statistics.
