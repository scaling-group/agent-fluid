...
- Treat posterior wave-shape allocation as a robust capture family, not a
  repeat-resolved ranking on this fixed direct-quiescent release. Exact
  phase-lag repeats already spanned `19.409--19.635T`, distance integrals
  `2.09210--2.10246L`, and scores `-0.20288-- -0.21272`; the current posterior
  amplitude and wrong-sign-yaw curvature variants also retained the same
  coherent alternating wake and late-hook capture at
  `19.552T/2.09001L/-0.20065` and `19.360T/2.08911L/-0.19989`. The favorable
  curvature realization is therefore not enough to tune or stack another
  counterturn scalar, especially because it used `19.25/17.71 rad/T^2` mean
  absolute command and all four current samples reached exactly the
  `260 deg/T` joint-rate ceiling. Adding approach-only posterior carrier/lag
  authority to that curvature controller is a concrete negative result: it
  retained the same visible wake and hook but moved capture from `19.360T` to
  `19.431T`, score from `-0.19989` to `-0.20560`, posterior mean command from
  `17.71` to `17.82 rad/T^2`, and posterior residence above 80% rate from
  `21.8%` to `22.1%`. Do not stack terminal posterior amplitude to recover
  authority on this trajectory family. Across all four samples, posterior rate
  exceeded 80% of the ceiling for `21.8--22.1%` of samples and its acceleration
  pushed farther into that near-limit band for `9.7--10.3%`, making this a
  localized actuator-path observation rather than a reason to suppress the
  whole rhythm. Inherited command-history gating is also a negative boundary:
  it cut anterior mean command only `18.20 -> 18.09 rad/T^2` while slowing
  capture to `19.949T` and worsening score to `-0.23265`. Prefer normalized,
  sign-selective posterior rate anti-windup: unload only raw acceleration whose
  product with measured posterior rate is positive near the envelope, and
  leave braking/reversal intact. Accept it only if it materially reduces
  command/rate residence at preserved capture, timing/integral, loads, joint
  margin, and wake coherence. Otherwise restore the clean scaffold, and
  reconsider response gating only when a held-out pose or inflow reproduces a
  repeatable wrong-sign yaw deficit.
