...
- Uncancelled joint-wise carrier work is supported as a propulsion-allocation
  mechanism, not as a rate/load protector. Replacing shared signed power with
  summed nonnegative work advances every `10/8/6/4/2/1L` milestone to
  `6.496/8.938/11.220/13.459/16.027/17.347T`, improves capture from the shared
  governor's `18.111T/1.99710L` integral to `17.688T/1.96419L`, and retains a
  coherent traveling wake in both visual views. But its head path grows from
  `12.198L` to `12.330L`, greater-than-90%-rate residence changes from
  `17.6/13.7%` to `18.6/13.4%`, and peak planar force/yaw moment rises from
  `0.02729/0.01435` to `0.03198/0.01651`; the peak occurs at `9.592T/7.445L`
  while posterior rate is only `3.799 rad/T` but outward acceleration is
  `27.63 rad/T^2`, before the instantaneous 98%-rate gate can respond. Preserve
  carrier/steering separation and a common phase-preserving scale, but do not
  strengthen or tune the work scalar as if it were a safety margin. The
  reusable next test is a short gait-phase-derived prediction of normalized
  rate-margin consumption by outward carrier acceleration, with steering and
  inward reversal untouched. Reject it unless capture stays within the
  `17.688--17.990T` class while rate residence and the `0.03198/0.01651` load
  peaks fall without a path wider than `12.330L`, terminal hook, joint-margin
  loss, or wake degradation; otherwise retain uncancelled work for speed and
  address measured load through a separately evidenced feedback mechanism.
