...
- Calibrate the complete two-joint steering map in 3D, not only the sign of one
  ...
  closest approach. Preserve the evidenced traveling-wave drive, make anterior
  and posterior steering contributions agree with the empirically calibrated
  bearing-to-cycle-mean-heading response, and require recovery after a bearing
  crossing. Because the supplied bearing uses the absolute longitudinal target
  component, a bearing-only controller cannot distinguish target-ahead from
  target-astern; if a rollout passes abeam, test a bounded normalized
  `target_body_L[1]` gate rather than merely increasing bearing gain. Reject
  this lesson if the aligned map worsens closest approach or termination class,
  destroys wake coherence, or increases action/joint-limit residence.
