...
- Treat the actual-rate-only positive-work guard as a late intervention, not
  established actuator protection. Across the four current direct-quiescent
  captures, anterior residence above 90/99% of the hard rate remains
  `17.50--17.71/11.93--12.18%`; the assigned-parent range-restored repeat is
  still `17.39/11.85%`, while posterior rate also reaches the envelope and
  soft-limited acceleration peaks approach `30.5/31.0 rad/T^2`. Moreover, the
  same range-restored policy bytes span `15.939--16.198T`,
  `1.82008--1.82807L` distance integral, and `13.107--13.185L` head path, so a
  smaller timing shift cannot establish a mechanism. Test a normalized
  joint-state lookahead that anticipates only energy-adding carrier action,
  withdraws it before rate contact, and leaves negative-work reversal plus
  body-frame steering intact; do not merely lower the rate-onset scalar or
  clip full joint commands independently. Falsify the mechanism unless rate
  residence falls materially while early milestones, capture, integral/path
  beyond repeat spread, command, loads, joint margin, terminal yaw/slip, and
  both coherent wake views remain in the useful sampled class.
