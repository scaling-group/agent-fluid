...
- Treat carrier/steering separation as a supported trajectory mechanism, but
  do not infer actuator protection from its score. Relative to the inherited
  shared signed-power governor (`18.111T`, `1.99710L` integral), a dominant-
  joint rate/work trigger captures at `17.990T`, shortens the head path from
  `12.198L` to `12.170L`, and lowers anterior/posterior residence above 99% of
  the rate envelope from `10.45/3.55%` to `8.44/0.89%`. Summing uncancelled
  positive work from both joints is faster still (`17.688T`, `1.96419L`) and
  both visual rows retain a coherent traveling wake, but the path lengthens to
  `12.330L`, rate residence rebounds to `12.13/2.02%`, and peak planar
  force/yaw moment rises from the dominant trigger's `0.02909/0.01499` to
  `0.03198/0.01651`. Thus a common whole-carrier scale can improve progress
  while weakening useful inward work at the unthreatened joint; do not tune
  its summed-power scalar or call it a rate guard. A reusable next mechanism is
  one rate-triggered dissipative coefficient opposing both carrier velocities,
  with restoring/reversal acceleration and body-frame steering residuals left
  additive. Retain that translation only if repeat CFD preserves the sub-
  `18T` capture, integral, coherent wake, short path, and terminal yaw while
  lowering both-joint near-bound residence and load peaks; otherwise restore
  the dominant-joint governor rather than independent phase clipping.
