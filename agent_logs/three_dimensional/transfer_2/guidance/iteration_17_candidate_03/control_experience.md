...
- Treat posterior wave-shape allocation as a robust capture family, not a
  ...
  `260 deg/T` joint-rate ceiling. Posterior rate exceeded 80% of that ceiling
  for `21.8--22.1%` of samples and its acceleration pushed farther into the
  near-limit band for `9.7--10.3%`, making this a localized actuator-path
  observation rather than a reason to suppress the whole rhythm. This repeated
  rate clipping is evidence for testing normalized actuator-state feedback,
  but inherited
  command-history gating is a negative boundary: it cut anterior mean command
  only `18.20 -> 18.09 rad/T^2` while slowing capture to `19.949T` and worsening
  score to `-0.23265`. Prefer a posterior-path rate anti-windup test that
  unloads only acceleration pushing farther into the measured limit and leaves
  braking intact; accept that mechanism only if it materially reduces
  command/rate residence at preserved capture, timing/integral, loads, joint
  margin, and wake coherence. Otherwise restore the clean lag scaffold, and
