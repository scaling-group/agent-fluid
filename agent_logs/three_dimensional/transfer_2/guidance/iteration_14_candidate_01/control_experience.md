...
- Keep posterior LOS-led redirect continuous when testing a release trigger.
  ...
  joint margin, and approximate `0.023/0.013` force/moment envelope. Its
  completed intercept-corridor anterior release also retained capture at
  `19.701T/2.10438L/-0.21449`, but produced no visibly different terminal
  topology and its `0.005T` advantage over the strongest plain sample is far
  inside the inherited `0.182T` identical-policy repeat span. Moreover,
  anterior mean command rose from `18.20` to `18.53 rad/T^2` and residence
  above 90% of the smooth bound rose from `35.25%` to `36.63%`. Treat this as a
  concrete boundary: projected miss is a viable geometric observation, not a
  proven gain or a reason to tune another corridor scalar. Preserve continuous
  posterior redirect and test a different actuator allocation; revisit the
  corridor only if held-out evidence shows a distinct terminal miss topology,
  and reject it if command headroom, capture, integral, loads, joint margin, or
