...
- Treat carrier/steering separation as a supported trajectory mechanism, but
  not yet as a validated joint-rate protector. The inherited logged shared-
  carrier sample has the best score (`-0.11054`) and, against the assigned
  unguarded repeat envelope, advances the `10/8/6/2L` milestones to
  `6.677/9.191/11.539/16.467T`, captures at `18.111T`, improves the integral to
  `1.99710L`, shortens head path to `12.198L`, and lowers sub-`2L` mean absolute
  yaw rate to `0.0317 rad/T`; both visual rows retain a coherent traveling wake.
  Yet its algebraic joint-power trigger raises anterior/posterior residence
  above 99% of the rate envelope to `10.45/3.55%` from the unguarded
  `8.35/4.36%`, and peak planar force/yaw moment rises from
  `0.02537/0.01356` to `0.02729/0.01435`. Independent carrier guards instead
  reach `8.17/0.43%` rate residence but produce a longer `12.281L` path and
  `0.373 rad/T` terminal yaw. Thus do not tune a shared summed-power scalar or
  return to phase-distorting independent clipping. The reusable refinement is
  one common carrier scale triggered by the dominant joint's own normalized
  rate proximity and positive carrier work, with target-conditioned residuals
  untouched. Retain it only if repeat CFD lowers near-bound residence and
  outward impulse in both joints while preserving the fast `12.198L` route,
  capture, coherent wake, joint margin, and load class; otherwise restore the
  sampled shared trajectory mechanism or the unguarded response-aware handoff.
