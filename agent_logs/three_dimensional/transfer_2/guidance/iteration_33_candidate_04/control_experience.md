...
- Treat response-conditioned carrier-reversal release as range-specific, not
  as a global refinement or a reason to remove target steering. Against the
  redirect-priority carrier's `5.863/7.838/9.779T` crossings at `10/8/6L`, two
  byte-identical global releases crossed at
  `5.808--5.841/7.761--7.799/9.724--9.762T`, retained coherent top-down and
  oblique wakes, and shortened path to `13.129--13.166L`; however, the plain
  carrier caught them by `4L` and captured at `16.044T`, versus
  `16.071--16.088T`. Withdrawing only that release across the existing
  normalized `6--4L` approach handoff retained the far progress, advanced
  capture to `15.939T`, reduced distance integral/path to
  `1.82008L/13.107L`, and kept peak force/moment at `0.03477/0.01715`, within
  or below the global-repeat range. In contrast, releasing the added course
  curvature from same-sign yaw delayed capture to `16.247T`, lengthened path
  to `13.363L`, and raised sub-`2L` yaw to `0.269 rad/T`; instantaneous load
  relief and positive-work-only carrier guards likewise bought no material
  timing/path/load benefit in inherited tests. Transfer the mechanism as a
  smooth normalized-range handoff that preserves the target-conditioned
  steering and phase-coupled carrier, not as another gain or threshold tune.
  Its unresolved boundary is actuator rate: the handoff's anterior/posterior
  residence above 90% rate was `17.70/8.25%`, slightly above the global
  repeats' `17.50--17.52/7.93--8.08%`. Falsify it if repeat or held-out
  evidence loses the far or late milestones, exceeds known timing/integral
  variation, or worsens path, rate residence, load, joint margin, terminal
  yaw/slip, finite action, or two-view wake coherence.
