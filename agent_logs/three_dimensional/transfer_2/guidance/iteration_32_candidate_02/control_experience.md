...
- Refine that carrier-response lesson by separating far and approach regimes:
  global route-yaw-conditioned reversal release captured at
  `16.071--16.088T/1.82203--1.82366L`, slightly improving far milestones but
  losing to the coupled carrier from `4L` onward, while releasing redirect
  curvature from the same proxy regressed to a `16.247T/13.363L` late-hook
  trajectory with `0.2690 rad/T` sub-`2L` mean yaw. A smooth normalized
  `6--4L` handoff that retained reversal release while far and restored full
  carrier coupling for approach instead captured at `15.939T/1.82008L`,
  shortened path to `13.107L`, kept coherent wakes and zero greater-than-90%-
  angle residence, and held force/moment peaks to `0.03477/0.01715`; its
  `17.70/8.25%` high-rate residence remained in the plain carrier's repeat
  class. Preserve this regime handoff rather than globally tuning work or
  curvature. Treat it as provisional until repeated: reject portability if a
  repeat loses the timing/integral advantage or if held-out pose/inflow
  requires a different boundary. Because broad route-aligned yaw did not
  certify satisfaction of the distinct velocity-course redirect, any further
  response release should be gated by the specific normalized directional
  request it claims to fulfill and must improve trajectory metrics beyond the
  established repeat envelope without worse joint, command, load, or wake
  diagnostics.
