...
- Distinguish an observed rate-envelope guard from failed raw-command-overlap
  attenuation. On the direct-quiescent release, the latter retained capture
  but widened the response-aware route to `12.370L`, regressed the distance
  integral to `2.07854L`, and recovered negligible command/rate headroom. In
  contrast, a smooth guard driven by normalized measured joint-rate proximity
  and per-joint acceleration-velocity alignment preserved the same coherent
  two-view traveling wake and improved the exact assigned parent from
  `19.162T/2.06924L/-0.18047` to `18.755T/2.04031L/-0.15214`; mean commands fell
  from `18.46/17.56` to `17.87/17.07 rad/T^2`, posterior greater-than-90%-bound
  command residence fell from `33.96%` to `30.65%`, and posterior greater-than-
  99%-envelope rate residence fell from `4.36%` to `3.28%`. Transfer only the
  invariant—withdraw command that would accelerate an already near-limit joint
  outward while preserving low-rate and reversal authority—not the sampled
  thresholds or gains. This is supported for timing/integral and actuator
  allocation, not route or load reduction: head path grew from `12.309L` to
  `12.421L`, and peak planar-force/yaw-moment coefficients rose from
  `0.02537/0.01357` to `0.02686/0.01397`. Require repeat or held-out capture,
  early-milestone retention, lower near-envelope residence, coherent wakes,
  adequate joint margin, and a timing/integral benefit commensurate with any
  path or load increase; otherwise restore the unguarded response-aware wave.
