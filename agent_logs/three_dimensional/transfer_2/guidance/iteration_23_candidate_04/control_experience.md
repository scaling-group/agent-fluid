...
- Do not use unresolved-redirect raw-command reservation as a proxy for joint-
  rate regulation on this capture scaffold. Relative to two byte-identical
  response-aware executions, the sampled carrier-reserve policy reduced mean
  anterior/posterior command to `18.23/17.37` from
  `18.45--18.46/17.53--17.56 rad/T^2`, but greater-than-90%-command residence
  changed by at most `0.49/0.22` percentage point and greater-than-99%-rate
  residence remained `8.22%/4.35%` versus `8.33--8.35%/4.35--4.36%`; both
  joints still touched `260 deg/T`. It also lengthened head path to `12.370L`
  from `12.304/12.309L` and regressed to `19.333T/2.07854L` from the parent
  repeat range of `19.162--19.338T/2.06924--2.07622L`, despite the same
  coherent wake and `~0.0254/~0.0136` force/moment class. Avoid tuning the
  reserve fraction for this direct-quiescent late-hook trajectory; test
  normalized joint-rate feedback as a distinct mechanism, and reconsider
  command reservation only if a held-out redirect shows command clipping
  without rate-envelope contact and improves path or capture semantics.
