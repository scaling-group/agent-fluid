...
- Treat posterior half-cycle allocation as regime-specific rather than choosing
  ...
  increased from `0.085U/0.349 rad/T` to `0.116U/0.431 rad/T`. Therefore keep
  the regime handoff rather than tuning either asymmetry globally, but treat
  distance alone as insufficient to release the turn-biased wave; next test a
  bounded release from normalized turn-demand/yaw agreement, not added terminal
  curvature. Reject that extension if it loses the early milestones or fails
  to reduce middle/near path and slip without worse timing, command residence,
  joint margin, loads, capture, or wake coherence. The wrong-sign-yaw posterior
  bend's byte-identical `19.360--19.591T/2.08911--2.09637L` spread remains the
  repeat boundary for claims of small terminal gains.
