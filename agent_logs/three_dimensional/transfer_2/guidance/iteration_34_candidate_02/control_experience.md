...
- Treat the normalized `6--4L` carrier-reversal handoff as unresolved rather
  than a supported range mechanism. Four byte-identical handoff runs now span
  `15.939--16.170T`, distance integral `1.82008--1.82951L`, path
  `13.107--13.142L`, and score `0.05319--0.06189`; two byte-identical global
  response releases span `16.071--16.088T`, `1.82203--1.82366L`,
  `13.129--13.166L`, and `0.05893--0.06072`. The assigned-parent repeat thus
  erased the apparent single-run timing/integral advantage while both classes
  retained coherent top-down and oblique wakes. Do not stack another distance
  handoff or tune its threshold until repeats or a held-out condition separate
  the trajectory classes; preserve target-conditioned steering in either base.
  Response-gating half-cycle asymmetry is also a concrete actuator negative:
  its completed `16.022T/1.82375L` capture retained the useful wake and
  `13.144L` path but used `17.71/8.07%` anterior/posterior residence above 90%
  rate and `16.65/15.38 rad/T^2` mean command, no improvement over the handoff
  repeats' `17.49--17.70/7.82--8.25%` envelope. The rate cost is instead
  localized outside `6L`, where the global release spends `26.09/11.66%`
  above 90% rate and `18.45/2.15%` above 99%, versus zero for both joints
  inside `4L`; its nominal anterior `omega*A` is about `123%` of the owned rate
  envelope. Test a distinct normalized joint-state oscillator or energy
  allocation mechanism, not another steering gate, approach schedule, or
  scalar guard threshold. Falsify it unless far-field rate/command residence
  falls while repeat-supported milestones, capture, path/integral, loads,
  joint margin, terminal course, finite action, and both wake views remain in
  the useful class.
