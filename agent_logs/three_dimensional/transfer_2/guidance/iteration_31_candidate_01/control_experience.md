...
- Prefer the measured-response redirect mechanism over another rate or load
  ...
  redirect curvature fully active is the shared boundary. Yet releasing that
  curvature from same-sign yaw alone is now also a concrete negative: the
  assigned-parent response residual captured at `16.247T/1.82240L` but
  lengthened head path to `13.363L`, raised sub-`2L` mean yaw to
  `0.269 rad/T`, ended at `-0.589 rad/T`, and raised anterior mean command to
  `17.34 rad/T^2`, versus the plain redirect's
  `16.044T/1.82409L/13.178L/0.094/-0.205/16.65` class, without material load
  or rate-residence relief. The two byte-identical response-released carrier
  samples also retained shorter `13.129--13.166L` paths and only
  `0.115--0.128 rad/T` near-target yaw. Same-sign yaw aligned to an aggregate
  rate-braked route request therefore marks response onset, not redirect
  completion: the parent's body-frame course error was still about `0.78 rad`
  when its logic treated the response as roughly 86% fulfilled. Restore the
  plain redirect instead of tuning carrier/load thresholds; if response-based
  release is revisited, qualify it by a shrinking redirect-specific course
  demand while retaining baseline route/rate feedback and the coherent
  carrier. Reject it if timing or distance integral regress beyond repeat
  spread without material path, yaw, load, command, or rate benefit, or if
  angle margin, terminal course, or either wake view worsens.
