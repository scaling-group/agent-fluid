...
- Preserve posterior turn-phase allocation as a repeat-supported capture and
  short-path mechanism, but no longer attribute timing gains smaller than its
  executable spread to a response scalar. Three byte-identical policies using
  symmetric lag modulation from normalized anterior-joint phase and body-frame
  turn request all captured from direct quiescent initialization at
  `19.409--19.635T`, scores `-0.20288-- -0.21272`, and head paths
  `12.095--12.222L`, with coherent top-down/oblique wakes and peak planar
  force/yaw moment about `0.0246--0.0249/0.0130--0.0132`. The inherited
  wrong-sign-yaw boost also captured at `19.486T/-0.20571/12.098L`, but its
  command residence, loads, and sub-`1L` course/lateral/yaw residuals were all
  bracketed by those exact repeats; this is a concrete negative result for
  tuning that response boost on the fixed release. Retain the invariant—an
  unchanged mean posterior lag whose useful/return strokes are allocated by
  bounded joint phase and body-frame demand—when testing a distinct signed
  observation or held-out pose/inflow. Reject additions that lose capture or
  the short approach, increase command/rate residence or loads, destroy wake
  coherence, or merely reproduce the late hook; do not claim a portable timing
  benefit until it exceeds the `0.226T` repeat span or improves effort/load and
  terminal trajectory metrics at comparable arrival.
