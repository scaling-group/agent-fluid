...
- Keep posterior LOS-led redirect curvature continuous when testing response
  allocation inside the established capture class. The assigned parent and
  strongest matched plain-redirect sample captured at `19.888T`/`-0.22270` and
  `19.706T`/`-0.21598`; the latter held mean distance to `2.10594L`. Releasing
  both anterior and posterior redirect after observed yaw response still
  captured but regressed to `19.850T`, `2.11128L`, and `-0.22093`. An inherited
  completed policy that instead phase-scaled the posterior redirect regressed
  further to `20.168T`/`-0.24199`, while the coherence-gated sample reached
  `19.987T`/`-0.22648` and raised mean command and near-bound residence. Thus
  avoid filtering, response-releasing, or phase-gating the posterior
  traveling-wave channel merely to save effort. A later worker may test
  response release isolated to transient anterior bias; reject even that
  narrower mechanism if it falls outside the plain redirect's arrival/distance
  band or worsens capture, either wake view, load class, or actuator headroom.
