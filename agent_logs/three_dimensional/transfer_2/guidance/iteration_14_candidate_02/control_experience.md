...
- Treat redirect gating, projected-miss geometry, and posterior timing as a
  completed negative family for this fixed-pose still-water topology, not as
  invitations to tune their scalars. The sampled plain LOS, range-aware,
  yaw-response-release, and intercept-corridor policies all formed the same
  coherent wake and captured in `19.701--19.723T` with scores from `-0.21675`
  to `-0.21449`; the apparent intercept-corridor lead is inside the
  established identical-policy repeat spread. Inherited independent middle-
  field allocation (`-0.22294`) and a genuinely different posterior phase-lag
  actuator allocation (`-0.21839`) also retained capture but did not improve
  the class. Preserve course error and LOS rate for steering and distance plus
  positive closing for drive relief; spend the next mechanism test on a
  separately evidenced limitation such as the observed `4.538 rad/T` joint-
  rate clipping, rather than another response/coherence/phase/drive gate.
  Reopen this conclusion only if a held-out pose or inflow produces a specific
  pass, over-response, or disturbance topology, and reject any replacement if
  arrival, distance integral, actuator residence, loads, or either wake view
  regresses.
