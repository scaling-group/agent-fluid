...
- Carrier-energy withdrawal is supported only when it remains response-aware:
  separate the rhythmic carrier from target steering and preserve one common
  scale, but do not equate earlier suppression with better protection. The
  inherited common/joint-wise guards improved unguarded
  `19.162--19.338T/2.06924--2.07622L` captures to the `18.111T/1.99710--2.00209L`
  class with coherent wakes. The current response-priority common governor
  then captured twice at `16.044--16.093T/1.82409--1.82848L`; in contrast, a
  predictive rate barrier lowered peak planar force/yaw moment to
  `0.03001/0.01495` and path to `12.528L` but delayed every milestone and
  capture to `17.418T/1.93044L`. Instantaneous norm-triggered load relief also
  captured later at `16.258T/1.83377L`, widened path from `13.178L` to
  `13.191L`, and reduced peaks only from `0.03579/0.01770` to
  `0.03500/0.01755`; productive propulsion and turning load are not generic
  overload signals. Retain the measured-response redirect instead of tuning
  the rate or load thresholds. The next reusable test is response-gated
  restoration of net negative carrier work after same-sign yaw begins
  satisfying the redirect, so reversal authority can return without weakening
  positive-work protection or steering priority. Reject it if capture timing
  or distance integral leaves the baseline-repeat class without material
  path/load/rate benefit, or if joint margin, terminal control, or coherent
  top-down and oblique wakes regress.
