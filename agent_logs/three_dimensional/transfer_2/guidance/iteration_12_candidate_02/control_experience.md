...
- Do not stack response, coherence, phase, or drive-allocation gates around the
  captured LOS-led redirect without a new failure topology. The same LOS-led
  half-cycle policy, differing only in its inert version label, spans
  `19.706T/-0.21598` to `19.888T/-0.22270`, which establishes the current
  repeat band. Releasing redirect curvature on aligned yaw
  (`19.850T/-0.22093`), admitting LOS lead only when a kinematic sign check
  agrees (`19.987T/-0.22648`), and phase-scaling posterior redirect in the
  inherited logs (`20.168T/-0.24199`) all preserve capture and coherent wakes
  but do not beat that band. Keep course error and LOS rate as steering signals
  and distance plus positive closing as drive-relief signals; revisit gating
  only if a held-out rollout shows a specific over-response or disturbance
  topology, and reject it if arrival, distance integral, actuator residence,
