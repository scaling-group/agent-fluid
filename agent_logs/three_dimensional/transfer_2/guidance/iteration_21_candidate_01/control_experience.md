...
- Regime-conditioned posterior wave allocation is supported, while advancing
  its handoff from measured yaw response remains provisional and should be
  exact-repeated before composition. The distance-only handoff used joint-
  phase carrier allocation while far and restored posterior lag near the
  target; against the all-distance lag parent it reached `10/8/6L` about
  `0.143/0.154/0.171T` earlier, reduced mean anterior command and greater-than-
  90%-bound residence from `19.03/36.92%` to `18.26/35.52%`, and first captured
  at `19.354T/2.07892L`. But a byte-identical repeat ranged to
  `19.613T/2.09432L`, with head path and sub-`2L` lateral speed ranging from
  `12.416--12.554L` and `0.116--0.202U`; small scalar differences therefore do
  not justify stacking another gate. A response-aware variant used normalized
  agreement between body-frame turn demand and recent yaw to release carrier
  allocation earlier during the same distance transition. It retained the
  early milestones, captured at `19.338T/2.07622L/-0.18691`, and directionally
  reduced head path, `2--4L` lateral speed/course error, and sub-`2L` lateral
  speed/yaw relative to the better distance-only run (`12.304` versus
  `12.416L`, `0.196U/0.289 rad` versus `0.230U/0.347 rad`, and
  `0.099U/0.404 rad/T` versus `0.116U/0.431 rad/T`) at the same peak-load class
  (`0.02535/0.01335`). Transfer only the invariant—blend bounded joint-state
  wave allocations by normalized approach geometry, and release extra turn-
  biased allocation when yaw already follows the turn request—not the sampled
  gains. Retain the response condition only if an exact repeat preserves
  capture, early progress, wake coherence, and confirms the path/slip benefit
  outside repeat variation without worse command residence, joint/rate margin,
  or loads; otherwise restore the simpler distance handoff.
