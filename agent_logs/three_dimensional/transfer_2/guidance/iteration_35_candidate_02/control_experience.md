...
- Preserve the full measured-response velocity-course redirect, but stop using
  same-sign yaw, closure, or course fulfillment to release either its added
  curvature or carrier reversal on this direct-quiescent pose. The original
  unfulfilled body-frame redirect advanced the predictive barrier's
  `17.418T/1.93044L` distance-integral class to `16.044T/1.82409L`, with every
  `10--1L` milestone earlier and the same coherent two-view wake. Inherited
  force/moment relief and response-released reversal variants retained capture
  but delayed it to `16.220--16.258T`, lengthened path to `13.191--13.206L`,
  and did not materially reduce load or rate residence. The completed
  response-residual redirect test now closes the remaining branch: releasing
  only the extra curvature after aligned yaw captured at
  `16.247T/1.82240L` but lengthened head path to `13.363L`, raised sub-`2L`
  mean absolute yaw rate to `0.269 rad/T`, anterior mean command to
  `17.34 rad/T^2`, and peak force/moment to `0.03520/0.01783`. The sampled
  range-specific full-redirect carrier instead reached
  `15.939T/1.82008L/13.107L`, with `0.106 rad/T` late yaw,
  `16.66 rad/T^2` anterior command, and `0.03477/0.01715` peaks. Both visual
  rows remain coherent, so the regression is control allocation rather than
  wake failure. Restore that full redirect and approach carrier coupling;
  avoid further scalar response-release gates unless a held-out pose exposes
  a new topology. The common `17.50--17.70%` anterior greater-than-90%-rate
  residence may justify one actuator-specific test tied directly to signed
  auxiliary joint work, but reject it unless command or rate cost improves
  materially without leaving the repeat-supported timing, integral, short-
  path, load, joint-margin, terminal-course, or coherent-wake class.
