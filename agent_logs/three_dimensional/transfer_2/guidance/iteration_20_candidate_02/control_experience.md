...
- Prefer regime-specific posterior wave allocation over another response-gate
  scalar on this direct-quiescent release, while retaining terminal geometry as
  a falsification boundary. The sampled far-amplitude/near-lag handoff reached
  `10/8/6L` at `6.897/9.674/12.293T`, captured at `19.354T`, and improved the
  distance integral/score to `2.07892L/-0.18968`; byte-identical response-gated
  posterior-bend runs captured at `19.360--19.591T` with
  integrals `2.08911--2.09637L` and scores `-0.19989` and `-0.20651`. Both
  visual rows retained coherent self-propelled wakes and the same terminal
  hook, and assigned-parent logs add
  captures at `-0.20560` and `-0.21951`, so the handoff is evidence for earlier
  progress rather than a new topology or a universally cleaner turn. Indeed,
  its recorded head path and sub-`2L` mean absolute yaw rose to
  `12.829L/0.431 rad/T`, versus `12.591--12.693L/0.285--0.382 rad/T` for the
  response-gated repeats. Transfer only the invariant: use normalized
  body-frame regime feedback to move posterior allocation smoothly from
  propulsive carrier shaping toward approach lag while preserving the captured
  scaffold. Reconsider a response-gated bend only when terminal-yaw quality is
  the priority; reject the handoff if repeat or held-out evidence loses its
  timing/integral gain, capture, wake coherence, command headroom, joint margin,
  or load envelope, or turns the longer terminal arc into a near miss.
