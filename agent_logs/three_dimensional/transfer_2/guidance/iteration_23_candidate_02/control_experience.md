...
- Do not use a near-target redirect gate to solve a far-field joint-rate
  allocation problem. The redirect-reserve candidate preserved capture but
  regressed from the two response-aware repeats' `19.162--19.338T`,
  `2.06924--2.07622L`, and `12.304--12.309L` path envelope to
  `19.3325T/2.07854L/12.370L`; it changed greater-than-90%-command residence
  by only `0.19/0.05` percentage points and greater-than-99%-rate residence by
  only `0.11/0.03` points. All sampled greater-than-99%-rate events occur
  outside `4L`, mostly outside `6L`, and at those events the command remains
  outward on roughly `82--83%/78%` of anterior/posterior samples. Restore the
  response-aware handoff rather than tuning the reserve fraction. A later
  actuator-allocation test should observe normalized joint-rate proximity and
  outward carrier alignment directly, attenuate the common traveling carrier
  without adding steering, and be rejected unless it reduces rate residence
  while preserving the early milestones, capture, short path, load class,
  joint margin, and coherent wake.
