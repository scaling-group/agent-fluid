...
- Regime-conditioned posterior wave allocation and its yaw-response handoff
  are now supported on this direct-quiescent release. The distance-only
  controller used joint-phase carrier allocation while far and restored
  posterior lag near the target; it preserved coherent capture and early
  progress but its byte-identical pair ranged across
  `19.354--19.613T/2.07892--2.09432L`, with `12.416--12.554L` head paths. The
  response-aware pair instead used normalized agreement between body-frame
  turn demand and recent yaw to release carrier allocation during the `6--4L`
  handoff and captured at `19.162--19.338T/2.06924--2.07622L`, with
  `12.304--12.309L` paths and the same coherent top-down/oblique wake and peak
  planar force/yaw-moment class (`0.02535--0.02537/0.01335--0.01356`). This
  exact repeat separates its arrival, integral, and path ranges from the
  distance-only pair, so retain the response condition rather than tuning the
  distance schedule. Transfer only the invariant—blend bounded joint-state
  wave allocations by normalized approach geometry and release extra turn-
  biased allocation after measured yaw follows the turn request—not the
  sampled gains. Its boundary remains actuator cost and within-pair slip
  variation: anterior mean command stayed near `18.45--18.46 rad/T^2`, about
  `35.7--36.0%` of anterior actions remained above 90% of the smooth bound,
  both runs touched the rate limit, and `2--4L` lateral speed/course error
  varied across `0.196--0.204U/0.289--0.318 rad`. Test any further refinement
  by removing one existing allocation only; reject it if early progress,
  capture, path/integral, joint margin, load class, or wake coherence regress.
