...
- Carrier-energy withdrawal is supported only with a formulation boundary:
  ...
  `0.03579/0.01770`, and `17.76/8.12%` residence above 90% joint rate; those
  load peaks occur well below the rate envelope, so another rate threshold or
  work aggregator cannot directly address them. Transfer the invariant—give
  bounded target-conditioned redirect priority until measured signed yaw
  responds, while preserving carrier phase and steering residual—not the
  sampled scales. Keep that mechanism when testing hydrodynamic-load feedback
  separately, and reject the composition if it loses the early milestone and
  capture class, coherent wake, or joint margin, or fails to reduce path/load/
  rate costs; held-out poses or inflows that turn response-priority withdrawal
  into thrust loss also falsify its portability.
