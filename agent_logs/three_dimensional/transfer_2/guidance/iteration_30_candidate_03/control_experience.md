...
- Treat carrier reversal authority as response-contingent, not something to
  suppress or pass unconditionally near the rate envelope. Earlier matched
  evidence showed that one common uncancelled-positive-work scale captured at
  `17.688T/1.96419L`, ahead of three more protective guards at
  `17.990--18.111T/1.98918--2.00209L`, but had worse anterior greater-than-99%-
  rate residence (`12.13%`), force/moment peaks (`0.03198/0.01651`), and path
  (`12.330L`). Unconditionally passing each negative-work carrier component
  then improved path, load, and terminal yaw but regressed to
  `17.413T/1.94829L` with higher mean command. In the current cohort, two
  byte-identical full-carrier redirect runs bound useful repeat variation at
  `16.044--16.093T/1.82409--1.82848L`; an instantaneous force/moment gate only
  changed this to `16.258T/1.83377L` while reducing peaks by at most about
  `2.2%/0.9%`, and a predictive rate barrier fell to
  `17.418T/1.93044L`. Retain aggregate positive-work sensing and the full
  carrier while a body-frame redirect lacks measured yaw response; test
  releasing only negative-work reversal as signed yaw aligns, rather than
  tuning rate/load thresholds or independently clipping joint commands.
  Reject that handoff if early progress or capture/integral leaves the repeat
  class without a material reduction in rate residence, path, load, or
  terminal yaw, or if joint margin and the coherent two-view wake regress.
