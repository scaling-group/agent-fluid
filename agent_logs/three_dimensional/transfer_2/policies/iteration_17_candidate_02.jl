...
# response and retain bounded posterior wave authority during drive relief.

function target_policy_params()
    return (
        version="dogfish3d_approach_posterior_authority_v1",
        control_period=0.55,
        ...
        posterior_approach_authority=0.12,
...
function target_policy(state, params)
    ...
    # Drive relief reduces anterior lateral motion. Retain a bounded fraction
    # of posterior carrier/lag authority instead of attenuating both joints in
    # lockstep; the multiplier is exactly neutral outside a closing approach.
    posterior_approach_scale =
        1.0 + params.posterior_approach_authority * drive_relief
    ...
    tail_target =
        mean_tail_tangent -
        posterior_approach_scale * (
            q1 + active_tail_lag * qd1 / max(omega, eps(Float64))
        )
    tail_drive =
