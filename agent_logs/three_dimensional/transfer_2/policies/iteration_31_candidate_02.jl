# Compact L64 target controller: match each response-driven handoff to the
# route or velocity-course command whose fulfillment it measures.

function target_policy_params()
    return (
        version="dogfish3d_command_matched_redirect_response_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    route_turn_response = max(
        turn_command * normalized_turn_response,
        0.0,
    )
    redirect_turn_response = max(
        redirect_command * normalized_turn_response,
        0.0,
    )
    # C-start-like allocation without a hidden stage: measured yaw must be
    # compared with the particular command whose authority is handed off.
    # Route response advances the posterior wave allocation, while course-
    # redirect response releases redirect-priority rhythmic carrier work.
    unfulfilled_redirect =
        course_authority *
        redirect_magnitude *
        (1.0 - clamp(redirect_turn_response, 0.0, 1.0))
    ...
    reversal_release = clamp(
        (1.0 - redirect_magnitude) * route_turn_response +
        redirect_magnitude * redirect_turn_response,
        0.0,
        1.0,
    )
