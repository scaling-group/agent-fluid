# L64 target controller: compose the captured middle-field velocity-course
# redirect with joint-state half-cycle steering of the traveling bend.

function target_policy_params()
    return (
        version="dogfish3d_middle_field_half_cycle_redirect_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Preserve the captured distance/closing allocation for propulsion relief.
    approach_span = max(
    ...
    # Course steering starts in the middle field without removing the carrier.
    # The independent distance weight reaches full authority at the same inner
    # radius as approach relief, while remaining inactive in the far field.
    course_span = max(
    ...
    redirect_weight =
        course_weight * course_authority * redirect_magnitude
    ...
    # Infer beat alignment from joint state, not time. Strengthen steering on
    # the half-cycle moving with the requested route turn and relax the return
    # stroke, while leaving the propulsive oscillator and course redirect
    # continuously active.
