# Compact L64 target controller: preserve response-aware targeting while an
# energy-direction governor protects the two-joint traveling wave.

function target_policy_params()
    return (
        version="dogfish3d_positive_work_carrier_governor_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Near the measured rate envelope, aggregate each joint's nonnegative
    # carrier work so braking at one joint cannot hide outward work at the
    # other. The resulting single scale governs both carrier components when
    # both inject energy, preserving their traveling-wave relationship.
    ...
    # Do not suppress a carrier component that is already removing kinetic
    # energy. This sign decomposition preserves measured-rate reversal and all
    # target-conditioned steering; only positive rhythmic work is withdrawn.
    head_carrier_scale =
        carrier_head_accel * qd1 > 0.0 ? carrier_scale : 1.0
    tail_carrier_scale =
        carrier_tail_accel * qd2 > 0.0 ? carrier_scale : 1.0
    governed_head_accel =
        head_carrier_scale * carrier_head_accel + steering_head_accel
    governed_tail_accel =
        tail_carrier_scale * carrier_tail_accel + steering_tail_accel
