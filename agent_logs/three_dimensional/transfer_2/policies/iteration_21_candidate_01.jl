...
# and release it into posterior lag when yaw corrects measured velocity course.

function target_policy_params()
    return (
        version="dogfish3d_course_response_wave_handoff_v1",
        control_period=0.55,
        ...
        course_response_handoff_gain=4.0,
...
function target_policy(state, params)
    ...
    # correcting the actual body-frame velocity-course error. This prevents a
    # yaw-braked route command from hiding useful course correction, adds no
    ...
    normalized_course_correction = tanh(
        -course_error / max(params.course_error_scale, eps(Float64)),
    )
    aligned_course_response = max(
        normalized_course_correction * normalized_turn_response,
        0.0,
    )
    response_handoff_weight = clamp(
        params.course_response_handoff_gain *
        approach_weight *
        (1.0 - distance_wave_weight) *
        aligned_course_response,
        0.0,
        1.0,
    )
