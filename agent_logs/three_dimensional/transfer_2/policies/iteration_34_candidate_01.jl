# Compact L64 target controller: restore carrier redirect priority through the
# approach only while measured velocity-course error remains unresolved.

function target_policy_params()
    return (
        version="dogfish3d_course_confirmed_carrier_handoff_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Preserve response-released reversal while far. Through the approach,
    # restore redirect-priority carrier coupling only in proportion to the
    # still-unresolved velocity-course error; range alone is not evidence that
    # the extra carrier priority remains useful.
    ...
    approach_course_priority =
        distance_wave_weight * course_authority * redirect_magnitude
    reversal_release =
        (1.0 - approach_course_priority) *
