# Compact L64 target controller: course-resolved carrier protection plus
# rate-margin allocation of target-conditioned mean curvature.

function target_policy_params()
    return (
        version="dogfish3d_rate_margin_steering_allocation_v1",
        control_period=0.55,
        ...
        steering_reallocation_strength=1.0,
...
function target_policy(state, params)
    ...
    # Closed-loop CPG steering allocation: keep the desired posterior tangent
    # and traveling carrier intact, but reduce the anterior share of the mean
    # bend when that joint has less normalized rate margin. Because the tail
    # target below is expressed relative to q1, it continuously takes up the
    # released curvature without a route stage, clock, or direct acceleration
    # transfer. Allocation vanishes when the velocity course is resolved or
    # the posterior joint has no relative margin.
    rate_scale = max(params.joint_rate_guard_scale, eps(Float64))
    anterior_rate_fraction = clamp(abs(qd1) / rate_scale, 0.0, 1.0)
    posterior_rate_fraction = clamp(abs(qd2) / rate_scale, 0.0, 1.0)
    posterior_rate_advantage = max(
        anterior_rate_fraction - posterior_rate_fraction,
        0.0,
    )
    steering_reallocation = clamp(
        params.steering_reallocation_strength,
        0.0,
        1.0,
    ) * unresolved_course_demand * posterior_rate_advantage
    allocated_head_bias = (1.0 - steering_reallocation) * head_bias
    centered_q1 = q1 - allocated_head_bias
