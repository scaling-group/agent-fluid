...
# predictive guard withdraws outward carrier work but retains reversal.

function target_policy_params()
    return (
        version="dogfish3d_predictive_reversal_guard_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Preserve the full velocity-course redirect. Measured same-sign yaw is
    # used below to allocate the traveling carrier, not to release steering.
    redirect_weight =
    ...
    # Only bounded positive-work carrier acceleration advances this short
    # normalized phase-space preview; steering stays outside the guard.
    ...
    # Negative carrier work moves joint rate back toward the admissible
    # interior. Pass that reversal at every distance while applying one common
    # scale to outward work, preserving steering and posterior phase intent.
    governed_head_accel =
        reversing_head_carrier +
        carrier_scale * outward_head_carrier +
        steering_head_accel
    governed_tail_accel =
        reversing_tail_carrier +
