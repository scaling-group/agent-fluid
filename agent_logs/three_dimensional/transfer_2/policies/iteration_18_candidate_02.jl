...
# and release only transient anterior redirect using projected-miss geometry.

function target_policy_params()
    return (
        version="dogfish3d_projected_miss_anterior_release_v1",
        control_period=0.55,
        ...
        projected_miss_release_scale=0.50,
...
function target_policy(state, params)
    ...
    # Straight-course closest-approach miss in body lengths. A target that
    # projects behind the measured course uses current range, preventing a
    # zero line miss from spuriously releasing a necessary reversal.
    course_projection =
        (velocity_x * target_x + velocity_y * target_y) /
        max(course_speed, eps(Float64))
    signed_line_miss =
        (velocity_x * target_y - velocity_y * target_x) /
        max(course_speed, eps(Float64))
    projected_miss_magnitude = course_projection > 0.0 ?
        abs(signed_line_miss) : hypot(target_x, target_y)
    projected_miss_release = tanh(
        projected_miss_magnitude /
        max(params.projected_miss_release_scale, eps(Float64)),
    )
    ...
    # Unlike the transient anterior boost above, the posterior LOS-led redirect
    # remains continuous so projected-miss release cannot erase propulsion.
