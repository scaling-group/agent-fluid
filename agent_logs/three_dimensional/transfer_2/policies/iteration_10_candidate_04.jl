# Compact L64 target controller: align joint-state half-cycle steering with
# the net route-plus-LOS course redirect through the traveling bend.

function target_policy_params()
    return (
        version="dogfish3d_net_half_cycle_los_redirect_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Body-frame bearing change plus body turn reconstructs inertial LOS
    # rotation, providing a bounded closing-gated lead without a clock.
    ...
    # Allocate the useful half-cycle to the actual net tail-turn request. This
    # keeps the phase-shaped carrier aligned when LOS/course feedback, rather
    # than the slower route channel, dominates the terminal redirect.
    curvature_authority =
        params.tail_curvature_limit +
        params.approach_tail_curvature_boost * approach_weight
    route_tail_tangent = curvature_authority * turn_command
    redirect_tail_tangent =
        params.redirect_curvature_boost * redirect_weight * redirect_command
    phase_turn_request = tanh(
        (route_tail_tangent +
         params.redirect_phase_share * redirect_tail_tangent) /
        max(curvature_authority, eps(Float64)),
    )
    phase_velocity = qd1 / max(omega * active_amplitude, eps(Float64))
    turn_phase_alignment =
        phase_turn_request * phase_velocity /
    ...
    mean_tail_tangent =
        (route_tail_tangent + redirect_tail_tangent) * half_cycle_scale
    tail_target =
        mean_tail_tangent -
        q1 -
    ...
    raw_head_accel =
        head_drive +
        params.head_steer_accel * phase_turn_request * half_cycle_scale
