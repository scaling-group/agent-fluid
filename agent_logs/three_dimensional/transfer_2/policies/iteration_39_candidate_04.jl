# Compact L64 target controller: local full-demand previews protect single
# joints while a co-injection guard preserves the traveling carrier's phase.

function target_policy_params()
    return (
        version="dogfish3d_phase_coherent_coinjection_guard_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # A single-joint bottleneck stays local so the other joint can sustain the
    # traveling bend. The product below is a smooth phase-coincidence signal:
    # it withdraws a coupled carrier burst only when both joints inject
    # normalized rhythmic power, without moving the sampled rate threshold.
    co_injection_alignment =
        head_outward_alignment * tail_outward_alignment
    co_injection_guard =
        guard_strength * co_injection_alignment
    ...
    # Same-sign yaw restores phase-coupled reversal while far, then the sampled
    # distance handoff returns redirect priority on the terminal approach.
    reversal_release =
        (1.0 - distance_wave_weight) *
