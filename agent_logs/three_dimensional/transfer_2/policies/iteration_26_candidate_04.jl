# Compact L64 target controller: preserve response-aware targeting while the
# joint most at risk triggers one phase-preserving traveling-carrier governor.

function target_policy_params()
    return (
        version="dogfish3d_dominant_joint_carrier_governor_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # remaining target-conditioned steering. A common carrier scale retains
    # anterior/posterior phase; the residuals restore the existing mean bend,
    # response-aware redirect, and approach feedback without attenuation.
    carrier_vdp_drive =
        params.oscillator_mu *
    ...
    # Couple rate proximity and outward carrier work at each joint before
    # taking the dominant request. This prevents inward work at one joint from
    # cancelling hazardous outward work at the other, while the resulting one
    # scale still preserves the traveling wave rather than clipping its joints
    # independently.
    ...
    rate_span = max(1.0 - onset_fraction, eps(Float64))
    head_rate_progress = clamp(
        (abs(qd1) / rate_scale - onset_fraction) / rate_span,
        0.0,
        1.0,
    )
    tail_rate_progress = clamp(
        (abs(qd2) / rate_scale - onset_fraction) / rate_span,
        0.0,
        1.0,
    )
    head_rate_gate =
    ...
    head_outward_power =
        max(carrier_head_accel * qd1, 0.0) / carrier_power_scale
    tail_outward_power =
        max(carrier_tail_accel * qd2, 0.0) / carrier_power_scale
    head_guard_request = head_rate_gate * tanh(
        head_outward_power /
        max(params.rate_guard_power_scale, eps(Float64)),
    )
    tail_guard_request = tail_rate_gate * tanh(
        tail_outward_power /
        max(params.rate_guard_power_scale, eps(Float64)),
    )
    carrier_guard =
        clamp(params.rate_guard_strength, 0.0, 1.0) *
        max(head_guard_request, tail_guard_request)
