# Compact L64 target controller: preserve the captured traveling-bend scaffold
# and expose a small speed-gated course residual before terminal approach.

function target_policy_params()
    return (
        version="dogfish3d_speed_gated_course_redirect_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Preserve full terminal redirect authority while allowing a small course
    # residual once the fish has established forward speed. This addresses
    # measured far-field course error without adding thrust or a fixed route.
    redirect_schedule =
        approach_weight +
        (1.0 - approach_weight) * params.cruise_redirect_fraction
    redirect_weight =
        redirect_schedule * course_authority * redirect_magnitude
