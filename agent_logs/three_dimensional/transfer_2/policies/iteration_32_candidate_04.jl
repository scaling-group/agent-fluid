# Compact L64 target controller: release carrier reversal only from measured
# velocity-course response, then restore carrier coupling for approach.

function target_policy_params()
    return (
        version="dogfish3d_course_response_carrier_handoff_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Keep route response for the posterior wave allocation, but require yaw
    # to follow the specific velocity-course redirect before carrier release.
    # Broad route-aligned yaw need not satisfy a disagreeing slip correction.
    ...
    redirect_aligned_response = max(
        redirect_command * normalized_turn_response,
        0.0,
    )
    redirect_weight =
        approach_weight *
    ...
    # redirect has priority until measured yaw follows that same redirect.
    # Route-aligned response still advances the posterior allocation, while
    # this specific bounded response governs rhythmic carrier work.
    unfulfilled_redirect =
        course_authority *
        redirect_magnitude *
        (1.0 - clamp(redirect_aligned_response, 0.0, 1.0))
    ...
    # While far, course-aligned yaw can restore negative-work reversal without
    # changing target-conditioned steering. The evidenced 6--4L handoff
    # withdraws that release and restores the complete coupled carrier before
    # terminal capture.
    ...
    reversal_release =
        (1.0 - distance_wave_weight) *
        clamp(redirect_aligned_response, 0.0, 1.0)
