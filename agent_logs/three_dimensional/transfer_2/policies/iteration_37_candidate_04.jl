...
# steering-aware state preview withdraws positive carrier work before contact.

function target_policy_params()
    return (
        version="dogfish3d_steering_aware_predictive_carrier_guard_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Anticipate rate-envelope contact from joint state and the full pre-limit
    # acceleration demand. Unlike a lower scalar onset, this short phase-space
    # preview advances contact only when carrier or steering is pushing the
    # corresponding joint outward. The resulting guard still withdraws only
    # positive carrier work, leaving reversal and target steering available.
    ...
    # Preserve phase-coherent reversal while far, then restore the sampled
    # redirect-priority carrier coupling over the existing approach handoff.
    reversal_release =
        (1.0 - distance_wave_weight) *
