# Compact L64 target controller: preserve response-priority redirect while a
# predictive common-carrier barrier protects the traveling-wave joint rates.

function target_policy_params()
    return (
        version="dogfish3d_predictive_response_priority_carrier_v1",
        control_period=0.55,
        ...
        rate_guard_prediction_phase=0.30,
...
function target_policy(state, params)
    ...
    # Forecast each joint's outward rate excursion over a short fraction of
    # observed gait phase. The most restrictive allowance scales both carrier
    # commands, preserving the traveling-wave relationship. The evaluated
    # response-priority redirect remains a separate near-envelope request;
    # target-conditioned steering and inward carrier reversal stay available.
    ...
    prediction_horizon =
        max(params.rate_guard_prediction_phase, 0.0) /
        max(omega, eps(Float64))
    guarded_rate = onset_fraction * rate_scale

    head_motion_sign = qd1 == 0.0 ?
        sign(carrier_head_accel + steering_head_accel) : sign(qd1)
    tail_motion_sign = qd2 == 0.0 ?
        sign(carrier_tail_accel + steering_tail_accel) : sign(qd2)
    head_carrier_outward = max(
        carrier_head_accel * head_motion_sign,
        0.0,
    )
    tail_carrier_outward = max(
        carrier_tail_accel * tail_motion_sign,
        0.0,
    )
    head_steering_outward = max(
        steering_head_accel * head_motion_sign,
        0.0,
    )
    tail_steering_outward = max(
        steering_tail_accel * tail_motion_sign,
        0.0,
    )
    head_carrier_allowance =
        prediction_horizon > eps(Float64) &&
        head_carrier_outward > eps(Float64) ?
        clamp(
            (guarded_rate - abs(qd1) -
                prediction_horizon * head_steering_outward) /
            (prediction_horizon * head_carrier_outward),
            0.0,
            1.0,
        ) : 1.0
    tail_carrier_allowance =
        prediction_horizon > eps(Float64) &&
        tail_carrier_outward > eps(Float64) ?
        clamp(
            (guarded_rate - abs(qd2) -
                prediction_horizon * tail_steering_outward) /
            (prediction_horizon * tail_carrier_outward),
            0.0,
            1.0,
        ) : 1.0
    requested_carrier_scale = min(
        head_carrier_allowance,
        tail_carrier_allowance,
    )
    guard_strength = clamp(params.rate_guard_strength, 0.0, 1.0)
    predictive_carrier_scale =
        1.0 - guard_strength * (1.0 - requested_carrier_scale)
    redirect_carrier_scale =
        1.0 - guard_strength * smooth_rate_gate * unfulfilled_redirect
    carrier_scale = min(
        predictive_carrier_scale,
        redirect_carrier_scale,
    )
    governed_head_accel =
        carrier_scale * carrier_head_accel + steering_head_accel
    governed_tail_accel =
        carrier_scale * carrier_tail_accel + steering_tail_accel
