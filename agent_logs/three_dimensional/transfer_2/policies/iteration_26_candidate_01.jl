# Compact L64 target controller: preserve response-aware targeting while
# selectively withdrawing posterior carrier work at its measured rate bound.

function target_policy_params()
    return (
        version="dogfish3d_posterior_carrier_governor_v1",
        control_period=0.55,
        ...
        posterior_rate_scale=260.0 * pi / 180,
        posterior_guard_onset_fraction=0.98,
        posterior_guard_strength=0.75,
        posterior_guard_alignment_scale=0.25,
...
function target_policy(state, params)
    ...
    steering_tail_drive = omega^2 * mean_tail_tangent

    # Both sampled carrier-separated governors advanced every milestone, but
    # only posterior outward-work withdrawal consistently recovered rate
    # headroom. A common carrier scale instead shifted rate residence into the
    # anterior wave generator and raised loads. Preserve that anterior carrier
    # and both steering residuals exactly; near the posterior rate envelope,
    # withdraw only posterior carrier acceleration that reinforces its motion.
    posterior_rate_scale = max(
        params.posterior_rate_scale,
        eps(Float64),
    )
    onset_fraction = clamp(
        params.posterior_guard_onset_fraction,
        0.0,
        1.0 - eps(Float64),
    )
    posterior_rate_fraction = abs(qd2) / posterior_rate_scale
    rate_progress = clamp(
        (posterior_rate_fraction - onset_fraction) /
    ...
    alignment_scale = max(
        params.posterior_guard_alignment_scale *
        params.command_accel_limit * posterior_rate_scale,
        eps(Float64),
    )
    outward_alignment = tanh(
        max(carrier_tail_drive * qd2, 0.0) / alignment_scale,
    )
    posterior_guard =
        clamp(params.posterior_guard_strength, 0.0, 1.0) *
        smooth_rate_gate * outward_alignment
    governed_head_accel = carrier_head_drive + steering_head_drive
    governed_tail_accel =
        carrier_tail_drive * (1.0 - posterior_guard) +
        steering_tail_drive
