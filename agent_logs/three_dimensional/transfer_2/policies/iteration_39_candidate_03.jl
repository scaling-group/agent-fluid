# Compact L64 target controller: full-demand joint previews retain reversal
# according to unresolved body-frame directional demand.

function target_policy_params()
    return (
        version="dogfish3d_directional_demand_reversal_handoff_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    aligned_turn_fulfillment = clamp(aligned_turn_response, 0.0, 1.0)
    course_redirect_demand = course_authority * redirect_magnitude
    unfulfilled_redirect =
        course_redirect_demand * (1.0 - aligned_turn_fulfillment)
    # Same-sign yaw alone does not establish that a course redirect is
    # complete. Combine it with the bounded magnitude of both target-turn and
    # velocity-course requests so carrier allocation reflects how much
    # directional work remains, not merely the sign of the response.
    directional_demand = clamp(
        max(abs(turn_command), course_redirect_demand),
        0.0,
        1.0,
    )
    unfulfilled_directional_demand =
        directional_demand * (1.0 - aligned_turn_fulfillment)
    ...
    # Preview rate-envelope contact from each joint's complete outward demand.
    # Steering therefore advances the warning when it shares the velocity
    # direction, but the resulting guards withdraw only rhythmic carrier work.
    ...
    # Decompose direction before governing. Each local rate/power trigger
    # withdraws only same-joint positive carrier work, preserving posterior
    # thrust when the anterior steering joint is the sole bottleneck.
    ...
    # Response-gated burst-to-cruise handoff: preserve redirect priority while
    # a large body-frame request remains unresolved, but pass phase-coherent
    # reversal during aligned cruise even when yaw is naturally near zero.
    reversal_release =
        (1.0 - distance_wave_weight) *
        (1.0 - unfulfilled_directional_demand)
