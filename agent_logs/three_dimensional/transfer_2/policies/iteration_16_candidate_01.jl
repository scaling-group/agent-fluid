...
# allocate posterior phase with the signed velocity-course redirect.

function target_policy_params()
    return (
        version="dogfish3d_signed_course_posterior_phase_v1",
        control_period=0.55,
        ...
        posterior_course_phase_gain=0.40,
...
function target_policy(state, params)
    ...
    # Keep anterior half-cycle steering on the route/yaw request. Posterior
    # wave shape also receives the signed velocity-course redirect, so the
    # already-supported terminal correction can reallocate bend timing without
    # changing mean lag, static curvature, or raw drive.
    posterior_phase_command =
        turn_command + params.posterior_course_phase_gain *
        (1.0 - abs(turn_command)) * redirect_weight * redirect_command
    posterior_phase_alignment =
        posterior_phase_command * phase_velocity /
        max(params.half_cycle_velocity_scale, eps(Float64))
