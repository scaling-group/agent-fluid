# Compact L64 target controller: preserve the response-aware traveling wave
# and predict rate-envelope entry before withdrawing outward acceleration.

function target_policy_params()
    return (
        version="dogfish3d_predictive_rate_guarded_wave_handoff_v1",
        control_period=0.55,
        ...
        rate_guard_alignment_scale=0.25,
        rate_guard_prediction_fraction=0.02,
...
function target_policy(state, params)
    ...
    # Preserve the sampled shared rate guard, but predict imminent envelope
    # entry from measured rate plus a short fraction of one controller period.
    # Only acceleration aligned with the current joint velocity contributes to
    # that prediction or is attenuated; reversal authority is unchanged.
    rate_scale = max(params.joint_rate_guard_scale, eps(Float64))
    prediction_horizon =
        max(params.rate_guard_prediction_fraction, 0.0) *
        max(params.control_period, eps(Float64))
    head_outward_accel = max(raw_head_accel * sign(qd1), 0.0)
    tail_outward_accel = max(tail_drive * sign(qd2), 0.0)
    predicted_head_rate = abs(qd1) + prediction_horizon * head_outward_accel
    predicted_tail_rate = abs(qd2) + prediction_horizon * tail_outward_accel
    predicted_peak_rate_fraction =
        max(predicted_head_rate, predicted_tail_rate) / rate_scale
    ...
    rate_progress = clamp(
        (predicted_peak_rate_fraction - onset_fraction) /
    ...
    rate_guard =
        clamp(params.rate_guard_strength, 0.0, 1.0) * smooth_rate_gate
    alignment_scale = max(
        params.rate_guard_alignment_scale *
        params.command_accel_limit * rate_scale,
        eps(Float64),
    )
    head_outward_alignment = tanh(
        max(raw_head_accel * qd1, 0.0) / alignment_scale,
    )
    tail_outward_alignment = tanh(
        max(tail_drive * qd2, 0.0) / alignment_scale,
    )
    governed_head_accel =
        raw_head_accel * (1.0 - rate_guard * head_outward_alignment)
    governed_tail_accel =
        tail_drive * (1.0 - rate_guard * tail_outward_alignment)
