# Compact L64 target controller: preserve the response-aware posterior handoff
# and reserve smooth-limit headroom for steering during unresolved redirects.

function target_policy_params()
    return (
        version="dogfish3d_redirect_reserve_wave_handoff_v1",
        control_period=0.55,
...
@inline function _policy_carrier_scale(carrier, steering, raw_budget)
    safe_carrier = _policy_safe(carrier, 0.0)
    safe_steering = _policy_safe(steering, 0.0)
    budget = max(_policy_safe(raw_budget, 1.0), eps(Float64))
    combined = safe_carrier + safe_steering
    if abs(combined) <= budget
        return 1.0
    elseif combined > budget && safe_carrier > 0.0
        return clamp(
            (budget - safe_steering) / safe_carrier,
            0.0,
            1.0,
        )
    elseif combined < -budget && safe_carrier < 0.0
...
function target_policy(state, params)
    ...
    head_carrier_drive = vdp_drive - omega^2 * q1
    head_steering_drive =
        omega^2 * head_bias -
        params.approach_damping * drive_relief * qd1 +
        params.head_steer_accel * turn_command * half_cycle_scale
    tail_carrier_drive = omega^2 * (tail_carrier_target - q2)
    tail_steering_drive =
        omega^2 * mean_tail_tangent -
        2 * params.tail_damping * omega * qd2

    # A burst redirect should not ask the traveling carrier and steering bend
    # to consume the same smooth-limit authority. Convert the desired output
    # reserve into the limiter's raw-command coordinates, then attenuate both
    # carrier components together only while redirect demand remains
    # unanswered by measured yaw. A common scale preserves the two-joint wave,
    # and zero gate weight recovers the exact parent accelerations.
    reserve_fraction = clamp(
        params.command_reserve_fraction,
        eps(Float64),
        1.0 - eps(Float64),
    )
    raw_command_budget =
        atanh(reserve_fraction) * params.command_accel_limit
    head_carrier_scale = _policy_carrier_scale(
        head_carrier_drive,
        head_steering_drive,
        raw_command_budget,
    )
    tail_carrier_scale = _policy_carrier_scale(
        tail_carrier_drive,
        tail_steering_drive,
        raw_command_budget,
    )
    unresolved_redirect =
        redirect_weight * (1.0 - aligned_turn_response)
    requested_carrier_scale = min(
        head_carrier_scale,
        tail_carrier_scale,
    )
    carrier_scale = clamp(
        1.0 - unresolved_redirect * (1.0 - requested_carrier_scale),
        0.0,
        1.0,
    )
    raw_head_accel =
        carrier_scale * head_carrier_drive + head_steering_drive
    tail_drive =
        carrier_scale * tail_carrier_drive + tail_steering_drive
