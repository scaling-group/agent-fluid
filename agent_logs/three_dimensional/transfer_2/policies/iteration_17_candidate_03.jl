# Compact L64 target controller: preserve the captured posterior-lag scaffold
# and unload only posterior acceleration that pushes into the joint-rate limit.

function target_policy_params()
    return (
        version="dogfish3d_posterior_rate_governor_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Rate-limit anti-windup for the posterior path. The smooth margin acts
    # only when acceleration and measured rate share a sign, so return-stroke
    # braking remains available and no external phase or command history is
    # introduced. At low rate this is exactly the evaluated tail controller.
