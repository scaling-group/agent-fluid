...
# and concentrate it while observed yaw still opposes the requested turn.

function target_policy_params()
    return (
        version="dogfish3d_response_gated_posterior_counterturn_v1",
        control_period=0.55,
        ...
        posterior_counterturn_boost=0.65,
...
function target_policy(state, params)
    ...
    # Preserve the supported posterior wave-shape allocation, then concentrate
    # it only while measured yaw still opposes the requested approach turn.
    # The signed-product gate is reflection invariant and releases smoothly as
    # the yaw response aligns; it changes neither base lag nor mean curvature.
    ...
    posterior_phase_authority =
        1.0 + params.posterior_counterturn_boost * counterturn_gate
    active_tail_lag = params.tail_lag_gain * (
        1.0 + params.posterior_lag_asymmetry *
        posterior_phase_authority * tanh(turn_phase_alignment)
    )
