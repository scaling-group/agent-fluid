# L64 target controller: retain the captured velocity-course redirect and
# restore part of the propulsive carrier as measured course response improves.

function target_policy_params()
    return (
        version="dogfish3d_course_response_drive_release_v1",
        control_period=0.55,
        ...
        course_aligned_relief_floor=0.30,
...
function target_policy(state, params)
    ...
    # Reuse the captured distance/closing allocation and compute the actual
    # velocity-course response before deciding how much carrier to remove.
    approach_span = max(
    ...
    course_misalignment = course_authority * redirect_magnitude

    # Keep a nonzero approach-relief floor for capture robustness. The rest is
    # released continuously when course aligns or speed is too low to provide
    # a meaningful course, restoring propulsion without a hidden mode clock.
    relief_response =
        params.course_aligned_relief_floor +
        (1.0 - params.course_aligned_relief_floor) * course_misalignment
    drive_relief = approach_weight * closing_gate * relief_response

    redirect_weight = approach_weight * course_misalignment
