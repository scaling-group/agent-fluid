# Compact L64 target controller: preserve captured posterior phase allocation
# while unloading only tail acceleration that pushes into the rate envelope.

function target_policy_params()
    return (
        version="dogfish3d_posterior_rate_antiwindup_v1",
        control_period=0.55,
        ...
        posterior_rate_guard_onset=0.80,
...
function target_policy(state, params)
    ...
    # Preserve the supported posterior wave-shape allocation about its mean.
    active_tail_lag = params.tail_lag_gain * (
        1.0 + params.posterior_lag_asymmetry * tanh(turn_phase_alignment)
    )

    # A signed-product response gate is reflection invariant: it activates only
    # when measured yaw opposes the current body-frame turn request and releases
    # continuously as the response aligns. Unlike the inconclusive lag booster,
    # this gate will act directly through a small posterior mean bend below.
    ...
    # Posterior rate anti-windup is inactive through the supported carrier and
    # rises smoothly only near the owned rate envelope. It unloads acceleration
    # that increases |qd2| while leaving braking/reversal completely intact.
    rate_guard_onset = clamp(
        params.posterior_rate_guard_onset,
        0.0,
        1.0 - eps(Float64),
    )
    posterior_rate_fraction =
        abs(qd2) / max(params.posterior_rate_limit, eps(Float64))
    rate_guard_coordinate = clamp(
        (posterior_rate_fraction - rate_guard_onset) /
        (1.0 - rate_guard_onset),
        0.0,
        1.0,
    )
    posterior_rate_guard =
        rate_guard_coordinate^2 * (3.0 - 2.0 * rate_guard_coordinate)
    guarded_tail_drive = (tail_drive * qd2 > 0.0) ?
        (1.0 - posterior_rate_guard) * tail_drive :
        tail_drive
    raw_head_accel =
        head_drive +
    ...
    return (
        phi_ddot=(
            ...
            _policy_soft_limit(guarded_tail_drive, params.command_accel_limit),
        ),
    )
end
