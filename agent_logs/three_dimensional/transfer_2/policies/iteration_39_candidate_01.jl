# Compact L64 target controller: preserve posterior positive work when the
# measured velocity course is resolved and the tail has projected rate margin.

function target_policy_params()
    return (
        version="dogfish3d_course_resolved_tail_work_v1",
        control_period=0.55,
        ...
        posterior_work_release_strength=0.70,
...
function target_policy(state, params)
    ...
    # A head-rate bottleneck should not automatically withdraw propulsive tail
    # work after the measured velocity course has converged. Restore only the
    # shared redirect component of positive posterior carrier work, and only
    # while the tail's own full-demand preview retains margin. Its local
    # positive-work guard and all negative-work reversal remain unchanged.
    course_resolution =
        course_authority * (
            1.0 - tanh(
                abs(course_error) /
    ...
    posterior_work_release =
        clamp(params.posterior_work_release_strength, 0.0, 1.0) *
        course_resolution *
        (1.0 - tail_rate_gate)
    tail_redirect_guard =
        redirect_guard * (1.0 - posterior_work_release)
    ...
    # Keep phase-coupled reversal shared under unresolved redirection, while
    # retaining the sampled far-field release when measured yaw follows route.
    reversal_release =
        (1.0 - distance_wave_weight) *
