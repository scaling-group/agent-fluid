...
# scaffold and release extra redirect inside a measured capture corridor.

function target_policy_params()
    return (
        version="dogfish3d_capture_corridor_redirect_v1",
        control_period=0.55,
        ...
        capture_corridor_outer_distance=2.5,
        capture_corridor_inner_distance=1.5,
        capture_corridor_radius=0.50,
        capture_corridor_transition=0.12,
...
function target_policy(state, params)
    ...
    # Its signed course error and LOS lead retain the strongest sampled
    # controller outside the terminal interception corridor.
    ...
    # line-of-sight rotation and leads the bounded intercept without a clock.
    ...
    # Release only the extra two-joint redirect when the measured velocity
    # course already predicts a pass inside a conservative corridor. Distance,
    # closing, and absolute miss make this continuous and reflection invariant;
    # baseline route steering and the traveling-wave carrier stay active.
    corridor_span = max(
        params.capture_corridor_outer_distance -
        params.capture_corridor_inner_distance,
        eps(Float64),
    )
    corridor_range_weight = clamp(
        (params.capture_corridor_outer_distance - distance) / corridor_span,
        0.0,
        1.0,
    )
    outside_corridor = 0.5 * (1.0 + tanh(
        (abs(projected_miss) - params.capture_corridor_radius) /
        max(params.capture_corridor_transition, eps(Float64)),
    ))
    capture_release =
        corridor_range_weight * closing_gate * (1.0 - outside_corridor)
    redirect_weight =
