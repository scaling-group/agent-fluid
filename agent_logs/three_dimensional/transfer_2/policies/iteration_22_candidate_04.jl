# Compact L64 target controller: release turn-biased posterior allocation and
# half-cycle steering when approach yaw follows the turn request.
...
function target_policy_params()
    return (
        ...
        response_steering_release=1.00,
...
function target_policy(state, params)
    ...
    # The same measured response also releases a bounded fraction of the
    # useful/return-stroke steering asymmetry. Far-field propulsion and opposed
    # yaw recover the sampled controller exactly; established corrective yaw
    # cannot remove more than the owned release fraction during the handoff.
    steering_release_weight = clamp(
        params.response_steering_release * response_handoff_weight,
        0.0,
        1.0,
    )
    half_cycle_scale = 1.0 + params.half_cycle_asymmetry *
        (1.0 - steering_release_weight) * phase_allocation
