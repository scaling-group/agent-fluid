...
# allocate velocity-course correction independently in the middle field.

function target_policy_params()
    return (
        version="dogfish3d_middle_course_los_lead_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Allocate course correction before the terminal drive-relief regime. The
    # independent weight reaches full authority at the same inner distance,
    # while distance and closing remain the only carrier-relief observations.
    course_span = max(
        params.course_outer_distance - params.approach_inner_distance,
        eps(Float64),
    )
    course_weight = clamp(
        (params.course_outer_distance - distance) / course_span,
        0.0,
        1.0,
    )

    # Recent body-frame bearing rate plus body yaw rate reconstructs inertial
    # line-of-sight rotation and leads the bounded course redirect while the
    # fish is closing, without changing the far-field carrier.
    ...
    redirect_weight = course_weight * course_authority * redirect_magnitude
