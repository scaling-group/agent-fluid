...
# shared carrier-energy governor protects the two-joint traveling wave.

function target_policy_params()
    return (
        version="dogfish3d_phase_preserving_carrier_governor_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Near the measured rate envelope, withdraw rhythmic energy only when the
    # carrier as a coupled two-joint command is accelerating the current joint
    # motion. A single bounded scale avoids the independent phase clipping and
    # steering attenuation seen in the evaluated whole-command governor.
    ...
    outward_power_alignment = tanh(
        max(normalized_carrier_power, 0.0) /
