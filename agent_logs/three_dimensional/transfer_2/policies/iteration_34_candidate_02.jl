# Compact L64 target controller: release reversal only for measured useful
# far-field progress, then restore carrier coupling through the approach.

function target_policy_params()
    return (
        version="dogfish3d_progress_confirmed_carrier_release_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Confirm that measured motion, not merely same-sign yaw, is closing on the
    # target. Normalizing by planar speed makes this a bounded course-quality
    # observation and suppresses the release continuously during startup.
    progress_confirmation =
        course_authority * clamp(
            max(closing_speed, 0.0) /
            max(course_speed, params.course_speed_scale, eps(Float64)),
            0.0,
            1.0,
        )
    ...
    # While far from capture, aligned yaw releases negative-work reversal only
    # when actual motion also closes on the target. Withdraw that release over
    # the existing 6--4L handoff so the sampled redirect-priority carrier is
    # restored unchanged for the approach.
