...
# allocate posterior tracking effort through bounded damping asymmetry.

function target_policy_params()
    return (
        version="dogfish3d_turn_phase_posterior_damping_v1",
        control_period=0.55,
        ...
        posterior_damping_asymmetry=0.16,
        posterior_damping_velocity_scale=0.45,
...
function target_policy(state, params)
    ...
    # Allocate posterior tracking dissipation by its own observed stroke side.
    # Damping is reduced while posterior motion aligns with the requested turn
    # and increased symmetrically on the return; carrier amplitude, mean bend,
    # and base lag remain unchanged.
    posterior_phase_velocity =
        qd2 / max(omega * active_amplitude, eps(Float64))
    posterior_phase_alignment =
        turn_command * posterior_phase_velocity /
        max(params.posterior_damping_velocity_scale, eps(Float64))
    active_tail_damping = params.tail_damping * (
        1.0 - params.posterior_damping_asymmetry *
        tanh(posterior_phase_alignment)
    )
    ...
    tail_drive =
        ...
        2 * active_tail_damping * omega * qd2
    raw_head_accel =
        head_drive +
