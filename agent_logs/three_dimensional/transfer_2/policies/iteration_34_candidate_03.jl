# Compact L64 target controller: preserve response-released targeting while a
# joint-state phase-space oscillator keeps rhythmic pumping rate-aware.

function target_policy_params()
    return (
        version="dogfish3d_rate_aware_phase_space_oscillator_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # The prior angle-only pump requests omega*A = 1.23 times the owned rate
    # envelope while far. Treat normalized angle and measured rate as the two
    # coordinates of one phase-space orbit: pump inside it, coast on it, and
    # damp outside it without altering harmonic reversal or steering.
    steered_orbit_energy =
        (centered_q1 / active_amplitude)^2 +
        (qd1 / rate_scale)^2
    vdp_drive =
        params.oscillator_mu *
        (1 - steered_orbit_energy) *
        qd1
    head_drive =
        vdp_drive -
        omega^2 * centered_q1 -
    ...
    carrier_orbit_energy =
        (q1 / active_amplitude)^2 +
        (qd1 / rate_scale)^2
    carrier_vdp_drive =
        params.oscillator_mu *
        (1 - carrier_orbit_energy) *
        qd1
    carrier_head_accel =
        carrier_vdp_drive -
        omega^2 * q1 -
