# Compact L64 target controller: retain the response-aware posterior wave
# handoff and govern only outward traveling-wave drive near the joint-rate edge.

function target_policy_params()
    return (
        version="dogfish3d_proprioceptive_carrier_governor_v1",
        control_period=0.55,
        ...
        rate_governor_start_fraction=0.90,
        rate_governor_max_relief=0.35,
        rate_governor_alignment_scale=0.20,
...
function target_policy(state, params)
    ...
    # then hand off to the shorter-path posterior lag allocation. During the
    # existing distance transition, advance that release when measured yaw is
    # already aligned with the body-frame turn request. This prevents continued
    # asymmetric wave allocation from feeding an established redirect, adds no
    # mean bend, and is reflection equivariant through the signed product.
    ...
    vdp_drive =
        ...
        qd1
    # Separate the traveling carrier from task steering and approach damping.
    # This decomposition is algebraically identical to the parent while the
    # governor is inactive.
    head_carrier_drive = vdp_drive - omega^2 * centered_q1
    head_task_drive =
    ...
    tail_carrier_drive =
        omega^2 * (tail_carrier_target - q2) -
        2 * params.tail_damping * omega * qd2
    tail_task_drive = omega^2 * mean_tail_tangent

    # At the active rate envelope, outward carrier acceleration cannot add
    # useful joint speed. Normalize both proprioceptive signals by their known
    # envelopes, then attenuate the common carrier continuously. A common
    # factor preserves the two-joint traveling-wave relation; route steering,
    # redirect curvature, and approach damping retain full authority.
    safe_rate_limit = max(params.joint_rate_limit, eps(Float64))
    normalized_joint_rate = max(abs(qd1), abs(qd2)) / safe_rate_limit
    governor_span = max(
        1.0 - params.rate_governor_start_fraction,
        eps(Float64),
    )
    rate_gate_coordinate = clamp(
        (normalized_joint_rate - params.rate_governor_start_fraction) /
        governor_span,
        0.0,
        1.0,
    )
    rate_gate =
        rate_gate_coordinate^2 * (3.0 - 2.0 * rate_gate_coordinate)
    safe_command_limit = max(params.command_accel_limit, eps(Float64))
    outward_head = max(
        (qd1 / safe_rate_limit) *
        (head_carrier_drive / safe_command_limit),
        0.0,
    )
    outward_tail = max(
        (qd2 / safe_rate_limit) *
        (tail_carrier_drive / safe_command_limit),
        0.0,
    )
    outward_gate = tanh(
        max(outward_head, outward_tail) /
        max(params.rate_governor_alignment_scale, eps(Float64)),
    )
    carrier_scale = 1.0 - params.rate_governor_max_relief *
        rate_gate * outward_gate
    raw_head_accel = carrier_scale * head_carrier_drive + head_task_drive
    tail_drive = carrier_scale * tail_carrier_drive + tail_task_drive
