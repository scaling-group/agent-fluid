...
# and govern outward carrier drive at the measured joint-rate envelope.

function target_policy_params()
    return (
        version="dogfish3d_proprioceptive_rate_governor_handoff_v1",
        control_period=0.55,
        ...
        rate_governor_start_fraction=0.95,
        rate_governor_strength=0.75,
...
function target_policy(state, params)
    ...
    # The evaluated scaffold repeatedly reaches the hard joint-rate envelope
    # while the carrier still accelerates outward. Use normalized proprioception
    # to remove only that nonproductive carrier demand. A common factor keeps
    # the traveling-wave relationship intact, while steering, approach damping,
    # and every inward carrier command retain their parent authority.
    joint_rate_limit = max(
        _policy_safe(params.joint_rate_limit, 1.0),
        eps(Float64),
    )
    governor_start = clamp(
        params.rate_governor_start_fraction,
        0.0,
        1.0 - eps(Float64),
    )
    peak_rate_fraction = max(abs(qd1), abs(qd2)) / joint_rate_limit
    rate_proximity = clamp(
        (peak_rate_fraction - governor_start) /
        max(1.0 - governor_start, eps(Float64)),
        0.0,
        1.0,
    )
    command_scale = max(params.command_accel_limit, eps(Float64))
    head_outward_alignment = max(
        tanh(qd1 / joint_rate_limit) *
        tanh(head_carrier_drive / command_scale),
        0.0,
    )
    tail_outward_alignment = max(
        tanh(qd2 / joint_rate_limit) *
        tanh(tail_carrier_drive / command_scale),
        0.0,
    )
    outward_alignment = max(
        head_outward_alignment,
        tail_outward_alignment,
    )
    governor_weight =
        clamp(params.rate_governor_strength, 0.0, 1.0) *
        rate_proximity^2 * outward_alignment
    carrier_scale = clamp(
        1.0 - governor_weight,
        0.0,
        1.0,
