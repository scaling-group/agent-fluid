# Compact L64 target controller: retain the approach-coupled traveling wave
# while protecting the steering half-cycle by measured joint-work direction.

function target_policy_params()
    return (
        version="dogfish3d_signed_work_half_cycle_guard_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    half_cycle_delta =
        params.half_cycle_asymmetry * tanh(turn_phase_alignment)
    half_cycle_scale = 1.0 + half_cycle_delta
    ...
    # Exact acceleration increments contributed by the joint-phase asymmetry.
    # Keeping these separate lets the rate guard distinguish outward steering
    # work from useful reversal without changing the carrier or mean C-bend.
    half_cycle_head_accel =
        params.head_steer_accel * turn_command * half_cycle_delta
    half_cycle_tail_accel =
        omega^2 * curvature_authority * turn_command * half_cycle_delta

    # Separate the exact commands into a zero-mean traveling carrier and the
    # remaining target-conditioned steering. Scaling the two carrier commands
    # together retains their phase relationship; the residuals restore the
    # existing mean bend, response-aware redirect, and approach feedback.
    carrier_vdp_drive =
        params.oscillator_mu *
    ...
    # Near the measured rate envelope, withdraw rhythmic energy when either
    # positive carrier work or an unfulfilled body-frame redirect requests it.
    # While far from capture, aligned yaw response releases negative-work
    # reversal without changing the steering residual. Withdraw that release
    # over the existing 6--4L handoff so the sampled redirect-priority carrier
    # is restored for the approach, where it reached every late milestone
    # earlier than the response-released repeats.
    ...
    peak_rate_fraction = max(abs(qd1), abs(qd2)) / rate_scale
    rate_progress = clamp(
        (peak_rate_fraction - onset_fraction) /
        max(1.0 - onset_fraction, eps(Float64)),
        0.0,
        1.0,
    )
    smooth_rate_gate = rate_progress^2 * (3.0 - 2.0 * rate_progress)
    normalized_carrier_power =
        (max(carrier_head_accel * qd1, 0.0) +
            max(carrier_tail_accel * qd2, 0.0)) /
        max(2 * params.command_accel_limit * rate_scale, eps(Float64))
    outward_power_alignment = tanh(
        normalized_carrier_power /
        max(params.rate_guard_power_scale, eps(Float64)),
    )
    carrier_guard_request = max(
        outward_power_alignment,
        unfulfilled_redirect,
    )
    carrier_guard =
        clamp(params.rate_guard_strength, 0.0, 1.0) *
        smooth_rate_gate *
        carrier_guard_request
    carrier_scale = 1.0 - carrier_guard

    # Govern only positive-work half-cycle steering with the same normalized
    # rate/power envelope. Negative-work reversal remains fully available, as
    # do baseline target curvature, course redirect, and the traveling carrier.
    outward_head_half_cycle = half_cycle_head_accel * qd1 > 0.0 ?
        half_cycle_head_accel : 0.0
    outward_tail_half_cycle = half_cycle_tail_accel * qd2 > 0.0 ?
        half_cycle_tail_accel : 0.0
    reversing_head_half_cycle =
        half_cycle_head_accel - outward_head_half_cycle
    reversing_tail_half_cycle =
        half_cycle_tail_accel - outward_tail_half_cycle
    normalized_half_cycle_power =
        (outward_head_half_cycle * qd1 +
            outward_tail_half_cycle * qd2) /
        max(2 * params.command_accel_limit * rate_scale, eps(Float64))
    half_cycle_work_request = tanh(
        normalized_half_cycle_power /
        max(params.rate_guard_power_scale, eps(Float64)),
    )
    half_cycle_work_guard =
        clamp(params.rate_guard_strength, 0.0, 1.0) *
        smooth_rate_gate *
        half_cycle_work_request
    half_cycle_work_scale = 1.0 - half_cycle_work_guard
    governed_half_cycle_head =
        reversing_head_half_cycle +
        half_cycle_work_scale * outward_head_half_cycle
    governed_half_cycle_tail =
        reversing_tail_half_cycle +
        half_cycle_work_scale * outward_tail_half_cycle
    protected_steering_head_accel =
        steering_head_accel -
        half_cycle_head_accel +
        governed_half_cycle_head
    protected_steering_tail_accel =
        steering_tail_accel -
        half_cycle_tail_accel +
        governed_half_cycle_tail
    ...
    reversing_carrier_scale =
        carrier_scale + (1.0 - carrier_scale) * reversal_release
    governed_head_accel =
        reversing_carrier_scale * reversing_head_carrier +
        carrier_scale * outward_head_carrier +
        protected_steering_head_accel
    governed_tail_accel =
        reversing_carrier_scale * reversing_tail_carrier +
        carrier_scale * outward_tail_carrier +
        protected_steering_tail_accel
