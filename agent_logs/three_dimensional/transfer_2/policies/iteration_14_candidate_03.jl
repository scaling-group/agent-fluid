# Compact L64 target controller: preserve the captured LOS-led half-cycle
# scaffold while softly guarding its two joint-rate envelopes.

function target_policy_params()
    return (
        version="dogfish3d_los_joint_rate_guard_v1",
        control_period=0.55,
...
@inline function _policy_rate_guard(
    command,
    velocity,
    rate_limit,
    onset_fraction,
)
    safe_command = _policy_safe(command, 0.0)
    safe_velocity = _policy_safe(velocity, 0.0)
    safe_rate_limit = max(_policy_safe(rate_limit, 1.0), eps(Float64))
    safe_onset = clamp(_policy_safe(onset_fraction, 0.9), 0.0, 1.0)
    guard_span = max(1.0 - safe_onset, eps(Float64))
    guard_position = clamp(
        (abs(safe_velocity) / safe_rate_limit - safe_onset) / guard_span,
        0.0,
        1.0,
    )
    smooth_guard = guard_position^2 * (3.0 - 2.0 * guard_position)
    drives_outward = safe_command * safe_velocity > 0.0
    return drives_outward ? safe_command * (1.0 - smooth_guard) : safe_command
end
...
function target_policy(state, params)
    ...
    # The episode's hard rate clamp can flatten the observed beat phase. Apply
    # the policy-owned envelope after the smooth acceleration limit and only
    # attenuate acceleration that drives farther outward; braking is untouched.
    head_command = _policy_soft_limit(
        raw_head_accel,
        params.command_accel_limit,
    )
    tail_command = _policy_soft_limit(
        tail_drive,
        params.command_accel_limit,
    )
