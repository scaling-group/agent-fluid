# Compact L64 target controller: preserve the strongest sampled terminal yaw
# response and unload only posterior acceleration that pushes a rate limit.

function target_policy_params()
    return (
        version="dogfish3d_response_curvature_rate_governor_v1",
        control_period=0.55,
        ...
        posterior_counterturn_curvature_limit=4.0 * pi / 180,
        posterior_rate_limit=260.0 * pi / 180,
        posterior_rate_relief_start=0.80,
        posterior_rate_relief_strength=0.45,
...
function target_policy(state, params)
    ...
    # Preserve the sampled useful/return-stroke posterior wave allocation.
    active_tail_lag = params.tail_lag_gain * (
        1.0 + params.posterior_lag_asymmetry * tanh(turn_phase_alignment)
    )

    # The best sampled terminal response applied a small posterior bend only
    # while measured yaw opposed the requested turn. Its signed-product gate is
    # reflection invariant and releases continuously as the yaw response aligns.
    normalized_turn_rate = tanh(
        turn_rate / max(params.turn_rate_scale, eps(Float64)),
    )
    counterturn_gate = approach_weight * max(
        -turn_command * normalized_turn_rate,
        0.0,
    )
    posterior_counterturn_curvature =
        params.posterior_counterturn_curvature_limit *
        counterturn_gate * turn_command
    ...
    # Smooth posterior rate anti-windup. Only same-direction acceleration is
    # attenuated in the near-limit band, leaving return-stroke braking intact.
    # At low joint rate this is exactly the sampled response-curvature policy.
    posterior_rate_fraction =
        abs(qd2) / max(params.posterior_rate_limit, eps(Float64))
    posterior_rate_band = max(
        1.0 - params.posterior_rate_relief_start,
        eps(Float64),
    )
    posterior_rate_excess = clamp(
        (posterior_rate_fraction - params.posterior_rate_relief_start) /
        posterior_rate_band,
        0.0,
        1.0,
    )
    posterior_rate_weight =
        posterior_rate_excess^2 * (3.0 - 2.0 * posterior_rate_excess)
    posterior_pushes_limit = tail_drive * qd2 > 0.0
    posterior_relief = posterior_pushes_limit ?
        params.posterior_rate_relief_strength * posterior_rate_weight :
        0.0
    governed_tail_drive = tail_drive * (1.0 - posterior_relief)
    raw_head_accel =
        head_drive +
    ...
    return (
        phi_ddot=(
            ...
            _policy_soft_limit(governed_tail_drive, params.command_accel_limit),
        ),
    )
end
