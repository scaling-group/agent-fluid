# Compact L64 target controller: combine bounded posterior amplitude allocation
# with a direct posterior bend while yaw opposes the requested turn.

function target_policy_params()
    return (
        version="dogfish3d_response_gated_posterior_amplitude_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Reallocate the posterior carrier amplitude on useful/return half-cycles.
    # Base lag remains fixed, so this changes one wave-shape actuator rather
    # than stacking another lag or range gain onto the response gate.
