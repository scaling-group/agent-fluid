# Compact L64 target controller: preserve the captured traveling bend and add
# a bounded velocity-course residual before the terminal redirect takes over.

function target_policy_params()
    return (
        version="dogfish3d_preapproach_course_residual_v1",
        control_period=0.55,
        ...
        preapproach_course_curvature_limit=4.0 * pi / 180,
...
function target_policy(state, params)
    ...
    # Correct actual translation before approach instead of waiting for the
    # terminal redirect. The residual starts from zero at release, is bounded
    # by observed speed and course mismatch, and fades continuously as the
    # established approach controller gains authority.
    course_residual_signal =
        -course_error / max(params.course_error_scale, eps(Float64))
    course_residual_command = tanh(course_residual_signal)
    course_residual_magnitude = tanh(abs(course_residual_signal))
    preapproach_course_weight =
        (1.0 - approach_weight) *
        course_authority *
        course_residual_magnitude
    preapproach_course_curvature =
        params.preapproach_course_curvature_limit *
        preapproach_course_weight *
        course_residual_command
