...
# uncancelled carrier-work governor protects the two-joint traveling wave.

function target_policy_params()
    return (
        version="dogfish3d_uncancelled_carrier_work_governor_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Near the measured rate envelope, withdraw rhythmic energy only when a
    # carrier command is accelerating its joint. Sum nonnegative joint-wise
    # work so inward work at one joint cannot hide outward work at the other;
    # one bounded scale still preserves their traveling-wave relationship.
