...
# while releasing only transient anterior redirect after observed yaw response.

function target_policy_params()
    return (
        version="dogfish3d_anterior_response_release_los_v1",
        control_period=0.55,
        ...
        redirect_response_scale=0.45,
        anterior_redirect_release_fraction=0.60,
...
function target_policy(state, params)
    ...
    # The calibrated steering map turns yaw opposite to the signed redirect.
    # Release only the transient anterior C-bend after that response appears;
    # the posterior redirect remains continuous to preserve the traveling wave.
    redirect_response = tanh(
        max(-redirect_command * turn_rate, 0.0) /
        max(params.redirect_response_scale, eps(Float64)),
    )
    anterior_redirect_weight = redirect_weight * (
        1.0 -
        params.anterior_redirect_release_fraction * redirect_response
    )
