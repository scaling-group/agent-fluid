# Compact L64 target controller: establish measured forward propulsion before
# admitting the full target-steering asymmetry, then preserve the captured
# response-aware posterior wave handoff.

function target_policy_params()
    return (
        version="dogfish3d_forward_response_steering_gate_v1",
        control_period=0.55,
        ...
        steering_forward_speed_scale=0.20,
        steering_authority_floor=0.20,
...
function target_policy(state, params)
    ...
    raw_turn_command = tanh(
        (route_request + rate_brake) /
        max(params.turn_request_scale, eps(Float64)),
    )

    # The sampled parent initially spends far more motion laterally than in
    # the forward (-body-x) direction. Preserve a small turning floor, but let
    # measured forward response continuously admit the full mean-bend and
    # half-cycle steering structure. This protects carrier formation without
    # using a clock, a world-frame route, or extra oscillator authority.
    forward_speed = max(-velocity_x, 0.0)
    steering_floor = clamp(params.steering_authority_floor, 0.0, 1.0)
    steering_authority = steering_floor + (1.0 - steering_floor) * tanh(
        forward_speed /
        max(params.steering_forward_speed_scale, eps(Float64)),
    )
    turn_command = steering_authority * raw_turn_command
