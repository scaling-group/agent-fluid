...
# joint-selective preview protects rate without suppressing posterior thrust.

function target_policy_params()
    return (
        version="dogfish3d_posterior_priority_predictive_guard_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Decompose direction before governing. The rate/power trigger withdraws
    # only each joint's own outward carrier work, so an anterior bottleneck no
    # longer suppresses posterior propulsion. Negative-work reversal remains
    # shared and can be attenuated only by the inherited redirect-priority
    # request; measured same-sign yaw releases it while far from capture.
