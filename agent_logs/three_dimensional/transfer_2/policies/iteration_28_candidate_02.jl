...
# request-consistent redirect governor protects the traveling-wave carrier.

function target_policy_params()
    return (
        version="dogfish3d_redirect_consistent_carrier_release_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # request. Keep this response separate from the broader LOS turn response
    # above because the two commands can legitimately oppose one another.
    ...
    # The response gate above releases only when yaw follows the actual course
    # redirect. A single bounded scale preserves the traveling-wave phase.
