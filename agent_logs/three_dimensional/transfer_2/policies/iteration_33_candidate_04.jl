# Compact L64 target controller: let measured redirect demand recouple the
# response-released carrier while preserving the two-joint traveling wave.

function target_policy_params()
    return (
        version="dogfish3d_redirect_demand_carrier_recoupling_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Signed yaw response governs carrier reversal below; retain the full
    # target-conditioned redirect because releasing that steering residual
    # produced the sampled long terminal hook.
    ...
    # Split positive-work and reversing carrier components so measured yaw may
    # release braking/reversal without changing target-conditioned steering.
    ...
    # Replace the sampled fixed 6--4L reversal handoff with measured-demand
    # recoupling. Far away, aligned yaw releases carrier reversal exactly as
    # in the useful early-progress controller. During approach, a large
    # velocity-course redirect smoothly restores phase-coupled carrier
    # authority; a settled course retains reversal instead of recoupling only
    # because a distance boundary was crossed.
    redirect_recoupling_request = clamp(
        approach_weight * redirect_magnitude,
        0.0,
        1.0,
    )
    smooth_redirect_recoupling =
        redirect_recoupling_request^2 *
        (3.0 - 2.0 * redirect_recoupling_request)
    reversal_release =
        (1.0 - smooth_redirect_recoupling) *
