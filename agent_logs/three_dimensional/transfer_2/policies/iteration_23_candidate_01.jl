# Compact L64 target controller: preserve the response-aware wave handoff and
# govern observed joint phase speed before the actuator rate envelope clips it.

function target_policy_params()
    return (
        version="dogfish3d_rate_governed_response_handoff_v1",
        control_period=0.55,
        ...
        joint_rate_reference=260.0 * pi / 180,
        rate_guard_onset_fraction=0.82,
        rate_guard_damping=5.0,
...
@inline function _policy_rate_govern(
    acceleration,
    velocity,
    rate_reference,
    onset_fraction,
    ...
    safe_acceleration = _policy_safe(acceleration, 0.0)
    safe_velocity = _policy_safe(velocity, 0.0)
    safe_rate = max(_policy_safe(rate_reference, 1.0), eps(Float64))
    safe_onset = clamp(
        _policy_safe(onset_fraction, 0.8),
        0.0,
        1.0 - eps(Float64),
    )
    normalized_rate = abs(safe_velocity) / safe_rate
    transition = clamp(
        (normalized_rate - safe_onset) / (1.0 - safe_onset),
        0.0,
        1.0,
    )
    guard_weight = transition^2 * (3.0 - 2.0 * transition)
    velocity_sign = sign(safe_velocity)
    outward_acceleration = max(
        velocity_sign * safe_acceleration,
        0.0,
    )
    excess_rate = max(
        abs(safe_velocity) - safe_onset * safe_rate,
        0.0,
    )
    brake_acceleration =
        max(_policy_safe(damping, 0.0), 0.0) * excess_rate
    return safe_acceleration - velocity_sign * guard_weight * (
        outward_acceleration + brake_acceleration
    )
end
...
function target_policy(state, params)
    ...
    # The sampled capture policies repeatedly ride the joint-rate envelope.
    # Use measured phase speed to remove only outward acceleration and add a
    # smooth opposing brake near that envelope. The map is odd under reflected
    # joint state and is exactly inactive below the normalized onset.
    guarded_head_accel = _policy_rate_govern(
        raw_head_accel,
        qd1,
        params.joint_rate_reference,
        params.rate_guard_onset_fraction,
        params.rate_guard_damping,
    )
    guarded_tail_accel = _policy_rate_govern(
        tail_drive,
        qd2,
        params.joint_rate_reference,
        params.rate_guard_onset_fraction,
    ...
    return (
        phi_ddot=(
            _policy_soft_limit(guarded_head_accel, params.command_accel_limit),
            _policy_soft_limit(guarded_tail_accel, params.command_accel_limit),
        ),
    )
end
