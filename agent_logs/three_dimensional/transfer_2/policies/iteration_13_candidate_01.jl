# Compact L64 target controller: preserve captured LOS-led steering and
# recover a bounded traveling bend once the closing course becomes steady.

function target_policy_params()
    return (
        version="dogfish3d_los_aligned_drive_recovery_v1",
        control_period=0.55,
        ...
        drive_recovery_fraction=0.35,
        drive_recovery_rate_scale=0.20,
...
function target_policy(state, params)
    ...
    # The approach allocator creates steering headroom by damping the carrier.
    # Restore only a bounded part when the fish is closing on an approximately
    # constant inertial line of sight; renewed LOS rotation immediately returns
    # the headroom. This state-only transition refreshes the posterior wave
    # without releasing either evaluated steering path.
    line_of_sight_alignment = 1.0 - tanh(
        abs(line_of_sight_rate) /
        max(params.drive_recovery_rate_scale, eps(Float64)),
    )
    drive_recovery =
        clamp(params.drive_recovery_fraction, 0.0, 1.0) *
        course_authority *
        line_of_sight_alignment
    effective_drive_relief = drive_relief * (1.0 - drive_recovery)

    amplitude_fraction =
        1.0 - effective_drive_relief *
        (1.0 - params.approach_amplitude_floor)
    ...
    head_drive =
        vdp_drive -
        omega^2 * centered_q1 -
        params.approach_damping * effective_drive_relief * qd1
