# Compact L64 target controller: preserve the strongest sampled response-gated
# scaffold and let middle-field course error select the useful steering stroke.

function target_policy_params()
    return (
        version="dogfish3d_middle_course_half_cycle_v1",
        control_period=0.55,
        ...
        middle_course_half_cycle_gain=0.55,
...
function target_policy(state, params)
    ...
    # Use actual velocity-course error only in the middle approach. This
    # window is exactly neutral outside 6L and from 2L through capture, leaving
    # both the far-field carrier and the sampled terminal response unchanged.
    middle_course_weight =
        4.0 * approach_weight * (1.0 - approach_weight)
    course_phase_request = tanh(
        -course_error / max(params.course_error_scale, eps(Float64)),
    )
    phase_turn_command = clamp(
        turn_command +
        params.middle_course_half_cycle_gain * middle_course_weight *
        course_authority * course_phase_request,
        -1.0,
        1.0,
    )
    ...
    # stroke aligned with the requested route/course turn and weaker on its
    # return. Joint velocity supplies phase without a clock or route state.
    phase_velocity = qd1 / max(omega * active_amplitude, eps(Float64))
    turn_phase_alignment =
        phase_turn_command * phase_velocity /
        max(params.half_cycle_velocity_scale, eps(Float64))
    half_cycle_scale =
        1.0 + params.half_cycle_asymmetry * tanh(turn_phase_alignment)

    # Preserve the sampled posterior wave-shape allocation about its mean.
    active_tail_lag = params.tail_lag_gain * (
        1.0 + params.posterior_lag_asymmetry * tanh(turn_phase_alignment)
    )

    # Preserve the sampled direct response gate: add a small posterior mean
    # bend only while yaw opposes the current body-frame route request.
