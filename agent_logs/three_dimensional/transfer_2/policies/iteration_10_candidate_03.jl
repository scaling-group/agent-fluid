...
# scaffold and release extra redirect curvature as measured yaw responds.

function target_policy_params()
    return (
        version="dogfish3d_response_released_los_redirect_v1",
        control_period=0.55,
        ...
        redirect_release_fraction=0.60,
...
function target_policy(state, params)
    ...
    # The calibrated steering map turns yaw opposite to its signed curvature
    # request. Keep full burst authority for absent or wrong-signed response,
    # but smoothly release only the extra redirect once measured yaw follows
    # the request. The product is reflection invariant and has no clock/state.
    redirect_response = tanh(
        max(-redirect_command * turn_rate, 0.0) /
        max(params.redirect_response_scale, eps(Float64)),
    )
    redirect_release =
        params.redirect_release_fraction * redirect_response
    redirect_weight =
        approach_weight * course_authority * redirect_magnitude *
        (1.0 - redirect_release)
