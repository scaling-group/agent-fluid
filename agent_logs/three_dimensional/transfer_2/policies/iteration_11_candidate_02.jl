# Compact L64 target controller: preserve captured LOS-led steering while
# rejecting history/yaw lead that disagrees with body-frame collision geometry.

function target_policy_params()
    return (
        version="dogfish3d_los_coherence_redirect_v1",
        control_period=0.55,
        ...
        line_of_sight_range_floor=0.50,
        line_of_sight_agreement_scale=0.10,
...
function target_policy(state, params)
    ...
    # Admit history/yaw LOS lead only when its sign agrees with an independent
    # instantaneous target/velocity cross-product cue. This reflection-
    # equivariant gate rejects beat-scale yaw without changing the carrier.
    ...
    line_of_sight_range_sq = max(
        target_x^2 + target_y^2,
        params.line_of_sight_range_floor^2,
    )
    kinematic_line_of_sight_rate =
        (target_y * velocity_x - target_x * velocity_y) /
        line_of_sight_range_sq
    line_of_sight_agreement = tanh(
        max(
            line_of_sight_rate * kinematic_line_of_sight_rate,
            0.0,
        ) /
        max(params.line_of_sight_agreement_scale^2, eps(Float64)),
    )
    coherent_line_of_sight_rate =
        line_of_sight_agreement * line_of_sight_rate
    redirect_signal =
