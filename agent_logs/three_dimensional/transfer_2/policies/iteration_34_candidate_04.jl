# Compact L64 target controller: retain range-specific carrier coupling and
# anticipate outward carrier work that would cross the joint-rate barrier.

function target_policy_params()
    return (
        version="dogfish3d_predictive_outward_carrier_guard_v1",
        control_period=0.55,
        ...
        rate_prediction_phase_fraction=0.05,
...
function target_policy(state, params)
    ...
    # Retain the sampled common guard and response-released reversal. Add a
    # phase-aware predictive supplement only to same-sign carrier work: joint
    # rate plus outward carrier acceleration estimates whether that component
    # would cross the normalized barrier over a small fraction of one beat.
    # Steering and carrier reversal are not part of this anticipatory scale.
    ...
    prediction_horizon =
        clamp(params.rate_prediction_phase_fraction, 0.0, 0.25) *
        params.control_period
    predicted_head_rate_fraction =
        (abs(qd1) + prediction_horizon * abs(outward_head_carrier)) /
        rate_scale
    predicted_tail_rate_fraction =
        (abs(qd2) + prediction_horizon * abs(outward_tail_carrier)) /
        rate_scale
    predicted_head_progress = clamp(
        (predicted_head_rate_fraction - onset_fraction) /
        max(1.0 - onset_fraction, eps(Float64)),
        0.0,
        1.0,
    )
    predicted_tail_progress = clamp(
        (predicted_tail_rate_fraction - onset_fraction) /
        max(1.0 - onset_fraction, eps(Float64)),
        0.0,
        1.0,
    )
    predicted_head_gate =
        predicted_head_progress^2 * (3.0 - 2.0 * predicted_head_progress)
    predicted_tail_gate =
        predicted_tail_progress^2 * (3.0 - 2.0 * predicted_tail_progress)
    predictive_head_excess = max(predicted_head_gate - smooth_rate_gate, 0.0)
    predictive_tail_excess = max(predicted_tail_gate - smooth_rate_gate, 0.0)
    head_outward_alignment = tanh(
        max(outward_head_carrier * qd1, 0.0) /
        max(
            params.command_accel_limit * rate_scale *
            params.rate_guard_power_scale,
            eps(Float64),
        ),
    )
    tail_outward_alignment = tanh(
        max(outward_tail_carrier * qd2, 0.0) /
        max(
            params.command_accel_limit * rate_scale *
            params.rate_guard_power_scale,
            eps(Float64),
    ...
    predictive_head_guard =
        clamp(params.rate_guard_strength, 0.0, 1.0) *
        predictive_head_excess * head_outward_alignment
    predictive_tail_guard =
        clamp(params.rate_guard_strength, 0.0, 1.0) *
        predictive_tail_excess * tail_outward_alignment
    ...
    outward_head_scale = carrier_scale * (1.0 - predictive_head_guard)
    outward_tail_scale = carrier_scale * (1.0 - predictive_tail_guard)
    governed_head_accel =
        reversing_carrier_scale * reversing_head_carrier +
        outward_head_scale * outward_head_carrier +
        steering_head_accel
    governed_tail_accel =
        reversing_carrier_scale * reversing_tail_carrier +
        outward_tail_scale * outward_tail_carrier +
        steering_tail_accel
