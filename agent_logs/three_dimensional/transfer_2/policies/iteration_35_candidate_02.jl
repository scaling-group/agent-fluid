# Compact L64 target controller: restore carrier priority only while the
# normalized body-frame velocity-course redirect remains unresolved.

function target_policy_params()
    return (
        version="dogfish3d_directional_demand_carrier_handoff_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Through the approach, restore phase-coupled carrier priority only while
    # the normalized velocity-course redirect remains unanswered. Once the
    # course is aligned or measured yaw answers the request, retain reversal
    # authority without changing the steering residual.
    ...
    directional_carrier_priority =
        distance_wave_weight * clamp(unfulfilled_redirect, 0.0, 1.0)
    reversal_release =
        (1.0 - directional_carrier_priority) *
