# Compact L64 target controller: release anterior redirect inside an observed
# intercept, then release posterior redirect only during terminal capture.

function target_policy_params()
    return (
        version="dogfish3d_sequential_terminal_redirect_release_v1",
        control_period=0.55,
        ...
        projected_miss_scale=1.00,
        projected_miss_blend=0.35,
        line_of_sight_lead_gain=0.30,
        intercept_corridor_inner=0.30,
        intercept_corridor_outer=0.80,
        anterior_redirect_floor=0.25,
        posterior_release_outer_distance=1.20,
        posterior_release_inner_distance=0.80,
...
function target_policy(state, params)
    ...
    # Blend angular course error with its bounded projected cross-track miss.
    # The range-aware term preserves earlier intercept authority while making
    # the same angular discrepancy less important as the target gets close.
    ...
    course_error_component =
        course_error / max(params.course_error_scale, eps(Float64))
    projected_miss = distance * sin(course_error)
    projected_miss_component = tanh(
        projected_miss / max(params.projected_miss_scale, eps(Float64)),
    )
    projected_miss_blend = clamp(params.projected_miss_blend, 0.0, 1.0)
    intercept_error =
        (1.0 - projected_miss_blend) * course_error_component +
        projected_miss_blend * projected_miss_component
    ...
    # line-of-sight rotation and leads the bounded intercept while closing.
    ...
    # Release only the transient anterior C-bend when measured velocity course
    # already projects through a normalized target corridor. A bounded floor
    # avoids coasting, while posterior redirect remains fully continuous to
    # preserve the propulsive traveling wave supported by sampled rollouts.
    intercept_corridor_span = max(
        params.intercept_corridor_outer - params.intercept_corridor_inner,
        eps(Float64),
    )
    outside_corridor_weight = clamp(
        (abs(projected_miss) - params.intercept_corridor_inner) /
        intercept_corridor_span,
        0.0,
        1.0,
    )
    anterior_redirect_floor = clamp(params.anterior_redirect_floor, 0.0, 1.0)
    anterior_redirect_weight = redirect_weight * (
        anterior_redirect_floor +
        (1.0 - anterior_redirect_floor) * outside_corridor_weight
    )

    # Preserve posterior intercept authority through the evidenced 2L approach,
    # then straighten only after a close, closing velocity course remains
    # inside the same capture corridor. This releases extra steering, not the
    # posteriorly lagged propulsive wave or baseline route curvature.
    posterior_release_span = max(
        params.posterior_release_outer_distance -
        params.posterior_release_inner_distance,
        eps(Float64),
    )
    posterior_range_weight = clamp(
        (params.posterior_release_outer_distance - distance) /
        posterior_release_span,
        0.0,
        1.0,
    )
    posterior_release_weight =
        posterior_range_weight * closing_gate *
        (1.0 - outside_corridor_weight)
    posterior_redirect_weight =
        redirect_weight * (1.0 - posterior_release_weight)
    ...
    head_bias =
        ...
        params.redirect_head_bias_boost * anterior_redirect_weight *
        redirect_command
    centered_q1 = q1 - head_bias

    # State-derived half-cycle asymmetry: strengthen route steering while the
    # anterior joint moves with the requested turn and relax it on the return
    # stroke. The normalized phase signal contains no clock or route memory.
    ...
    mean_tail_tangent =
        ...
        params.redirect_curvature_boost * posterior_redirect_weight *
        redirect_command
    tail_target =
        mean_tail_tangent -
        q1 -
