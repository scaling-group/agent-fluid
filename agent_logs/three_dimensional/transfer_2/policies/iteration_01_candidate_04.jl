...
# Parent adaptations used the 3D evaluator's field-name aliases. This candidate
# consumes the evaluator's normalized observations directly, preserves the
# parent's effective lateral-velocity weight, and corrects the mean-curvature
# actuator sign from the observed 3D response.

function target_policy_params()
    return (
        version="dogfish_target_control_v19_3d_curvature_sign",
        drive_period=0.55,
        control_period=0.55,
        ...
        target_lateral_velocity_gain=0.003125,
...
function drive_module(state, params, turn_request=0.0, drive_frequency_scale=1.0, recovery_request=0.0)
    ...
    # Bounded target-curvature feedback uses the measured 3D convention:
    # negative requests create negative mean tail tangent. This opposes the
    # parent's positive-bearing/positive-heading drift without a hidden clock.
