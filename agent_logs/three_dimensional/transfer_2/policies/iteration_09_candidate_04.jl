# L64 target controller: preserve the captured course redirect and allocate
# state-derived half-cycle steering to posterior curvature.

function target_policy_params()
    return (
        version="dogfish3d_posterior_half_cycle_redirect_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # It stays inactive at low speed and far from the target, then requests a
    # bounded C-bend only while inertial course remains misaligned.
    ...
    # Infer beat phase from normalized joint state, not clock time. Strengthen
    # posterior route curvature on the useful turning stroke and relax it on
    # the return stroke, while keeping anterior steering continuous.
