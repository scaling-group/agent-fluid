...
# scaffold and allocate terminal redirect through bounded posterior wave lag.

function target_policy_params()
    return (
        version="dogfish3d_redirect_phase_lag_v1",
        control_period=0.55,
        oscillator_amplitude=28.0 * pi / 180,
        oscillator_mu=0.35,
        tail_lag_gain=0.80,
        redirect_lag_modulation=0.24,
        tail_damping=0.65,
...
function target_policy(state, params)
    ...
    # Allocate the terminal redirect through posterior wave timing as well as
    # mean curvature. Signed redirect and joint phase flip together under
    # reflection, so the bounded lag modulation remains equivariant. It
    # vanishes with redirect demand and leaves the far-field carrier intact.
    redirect_phase_alignment =
        redirect_command * phase_velocity /
        max(params.half_cycle_velocity_scale, eps(Float64))
    effective_tail_lag = params.tail_lag_gain * (
        1.0 +
        params.redirect_lag_modulation * redirect_weight *
        tanh(redirect_phase_alignment)
    )
