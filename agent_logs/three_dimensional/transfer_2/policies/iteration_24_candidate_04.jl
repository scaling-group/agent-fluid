# Compact L64 target controller: use posterior amplitude allocation while far
# and release it into posterior lag when yaw follows geometric route demand.

function target_policy_params()
    return (
        version="dogfish3d_geometric_response_wave_handoff_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # already aligned with persistent body-frame geometric route demand. Keep
    # the fast yaw-rate brake in mean steering, but do not let it reverse the
    # response signal that releases the gait allocation. This prevents
    # continued asymmetric wave allocation from feeding an established
    # redirect, adds no mean bend, and is reflection equivariant through the
    # signed product.
    ...
    aligned_turn_response = max(
        geometric_route_request * normalized_turn_response,
        0.0,
    )
