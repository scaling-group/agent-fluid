# Compact L64 target controller: release the added course redirect with
# measured yaw response while preserving the two-joint traveling wave.

function target_policy_params()
    return (
        version="dogfish3d_response_residual_redirect_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Treat the extra C-bend as an unmet-response residual. Same-sign yaw
    # continuously releases only this course-redirect boost; baseline route
    # steering and yaw-rate braking remain available throughout the approach.
