...
# coupled work-selective governor protects the traveling-wave carrier.

function target_policy_params()
    return (
        version="dogfish3d_work_selective_carrier_governor_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Separate the commands into a zero-mean traveling carrier and the exact
    # target-conditioned residual. Steering, response-aware redirect, and the
    # approach mean bend remain outside the carrier-energy governor.
    carrier_vdp_drive =
        params.oscillator_mu *
    ...
    # A single normalized energy gate keeps the two-joint intervention
    # coupled. Apply it only to carrier work that accelerates current joint
    # motion; carrier reversal and body-frame steering remain available when
    # the other joint is still injecting rhythmic energy.
    ...
    head_outward_power = max(carrier_head_accel * qd1, 0.0)
    tail_outward_power = max(carrier_tail_accel * qd2, 0.0)
    carrier_power_scale = max(
        params.command_accel_limit * rate_scale,
        eps(Float64),
    )
    normalized_outward_power =
        (head_outward_power + tail_outward_power) /
        (2.0 * carrier_power_scale)
    shared_energy_gate =
    ...
    alignment_scale = max(
        params.rate_guard_power_scale * carrier_power_scale,
        eps(Float64),
    )
    head_outward_fraction = tanh(head_outward_power / alignment_scale)
    tail_outward_fraction = tanh(tail_outward_power / alignment_scale)
    governed_head_accel =
        carrier_head_accel *
        (1.0 - shared_energy_gate * head_outward_fraction) +
        steering_head_accel
    governed_tail_accel =
        carrier_tail_accel *
        (1.0 - shared_energy_gate * tail_outward_fraction) +
        steering_tail_accel
