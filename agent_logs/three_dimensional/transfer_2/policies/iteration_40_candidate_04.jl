...
# traveling carrier and release outward steering work after course resolution.

function target_policy_params()
    return (
        version="dogfish3d_course_resolved_total_work_guard_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Once the measured velocity course has resolved the body-frame redirect,
    # an outward target-conditioned stroke no longer deserves unconditional
    # priority at a rate contact. Use the same joint-local full-demand preview
    # to withdraw only that outward steering work. Low-speed, unresolved-course,
    # inward-steering, and cross-joint authority remain unchanged.
    resolved_course_weight =
        course_authority * (
            1.0 - tanh(
                abs(course_error) /
    ...
    head_steering_outward_scale =
        1.0 - guard_strength * head_rate_gate * resolved_course_weight
    tail_steering_outward_scale =
        1.0 - guard_strength * tail_rate_gate * resolved_course_weight
    ...
    outward_tail_steering = steering_tail_accel * qd2 > 0.0 ?
        steering_tail_accel : 0.0
    reversing_head_steering = steering_head_accel - outward_head_steering
    reversing_tail_steering = steering_tail_accel - outward_tail_steering
    ...
    governed_head_accel =
        ...
        reversing_head_steering +
        head_steering_outward_scale * outward_head_steering
    governed_tail_accel =
        reversing_carrier_scale * reversing_tail_carrier +
        tail_outward_scale * outward_tail_carrier +
        reversing_tail_steering +
        tail_steering_outward_scale * outward_tail_steering
