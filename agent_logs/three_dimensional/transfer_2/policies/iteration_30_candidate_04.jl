# Compact L64 target controller: preserve response-aware targeting while a
# response-released carrier governor protects the two-joint traveling wave.

function target_policy_params()
    return (
        version="dogfish3d_response_released_reversal_governor_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # The latter prioritizes steering only while course error lacks the signed
    # yaw response that will release the carrier again. A single bounded scale
    # preserves the traveling-wave phase relationship.
    ...
    # Keep the phase-preserving common scale while restoring reversal only
    # after the signed yaw response begins satisfying the body-frame redirect.
    # Net negative carrier work is a joint-state phase signal: it releases the
    # traveling bend without weakening the positive-work guard or using time.
    normalized_net_reversal = max(
        -(carrier_head_accel * qd1 + carrier_tail_accel * qd2),
        0.0,
    ) / max(2 * params.command_accel_limit * rate_scale, eps(Float64))
    reversal_alignment = tanh(
        normalized_net_reversal /
        max(params.rate_guard_power_scale, eps(Float64)),
    )
    response_reversal_release =
        clamp(aligned_turn_response, 0.0, 1.0) * reversal_alignment
    carrier_scale =
        1.0 - carrier_guard * (1.0 - response_reversal_release)
    governed_head_accel =
