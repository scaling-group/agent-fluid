...
# but guard only its carrier when measured joint rate nears the envelope.

function target_policy_params()
    return (
        version="dogfish3d_carrier_rate_guarded_handoff_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # The sampled rate guard advanced every distance milestone while reducing
    # command and rate residence, but it also guarded the route terms and took
    # a slightly longer path than its unguarded parent. Preserve its shared,
    # smooth joint-rate trigger while applying attenuation only to outward
    # traveling-wave work. Bounded body-frame steering remains available, and
    # reversal authority is unchanged.
    ...
    head_outward_alignment = tanh(
        max(carrier_head_drive * qd1, 0.0) / alignment_scale,
    )
    ...
    governed_head_accel =
        carrier_head_drive *
        (1.0 - rate_guard * head_outward_alignment) +
        steering_head_drive
    governed_tail_accel =
        carrier_tail_drive *
        (1.0 - rate_guard * tail_outward_alignment) +
        steering_tail_drive
