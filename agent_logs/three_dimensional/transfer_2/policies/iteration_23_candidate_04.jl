...
# and protect the traveling wave with a proprioceptive joint-rate envelope.

function target_policy_params()
    return (
        version="dogfish3d_proprioceptive_rate_envelope_v1",
        control_period=0.55,
        ...
        rate_guard_start_fraction=0.90,
        rate_guard_full_fraction=0.98,
...
@inline function _policy_rate_envelope(
    raw_accel,
    joint_velocity,
    rate_limit,
    start_fraction,
    ...
    safe_accel = _policy_safe(raw_accel, 0.0)
    safe_velocity = _policy_safe(joint_velocity, 0.0)
    safe_rate_limit = max(_policy_safe(rate_limit, 1.0), eps(Float64))
    start = clamp(_policy_safe(start_fraction, 0.90), 0.0, 1.0)
    full = clamp(
        _policy_safe(full_fraction, 0.98),
        start + eps(Float64),
        1.0,
    )
    speed_fraction = abs(safe_velocity) / safe_rate_limit
    guard_weight = clamp(
        (speed_fraction - start) / max(full - start, eps(Float64)),
        0.0,
        1.0,
    )
    velocity_sign = sign(safe_velocity)
    outward_accel = velocity_sign * max(velocity_sign * safe_accel, 0.0)
    return safe_accel - guard_weight * outward_accel
end
...
function target_policy(state, params)
    ...
    # Hard rate clipping is present in every sampled gait. Preserve the exact
    # parent command away from that envelope; near it, remove only acceleration
    # that reinforces the current joint motion. Stroke-reversing authority is
    # untouched, so this bounded proprioceptive guard cannot add mean bend or
    # erase the state-derived traveling-wave direction.
    guarded_head_accel = _policy_rate_envelope(
        raw_head_accel,
        qd1,
        params.joint_rate_limit,
        params.rate_guard_start_fraction,
        params.rate_guard_full_fraction,
    )
    guarded_tail_accel = _policy_rate_envelope(
        tail_drive,
        qd2,
        params.joint_rate_limit,
        params.rate_guard_start_fraction,
