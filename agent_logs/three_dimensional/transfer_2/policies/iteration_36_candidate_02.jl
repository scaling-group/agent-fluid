# Compact L64 target controller: preserve the response-released traveling
# carrier while sharing outward target steering with phase-local tail headroom.

function target_policy_params()
    return (
        version="dogfish3d_phase_headroom_steering_transfer_v1",
        control_period=0.55,
        ...
        steering_transfer_onset_fraction=0.90,
        steering_transfer_strength=0.70,
...
function target_policy(state, params)
    ...
    # The sampled anterior joint reaches the rate envelope while the posterior
    # joint is in its low-rate phase. Transfer only the outward part of the
    # explicit target-aligned head steering into same-sign tail acceleration.
    # Carrier work and reversing head steering retain their original channels.
    rate_scale = max(params.joint_rate_guard_scale, eps(Float64))
    steering_onset_fraction = clamp(
        params.steering_transfer_onset_fraction,
        0.0,
        1.0 - eps(Float64),
    )
    head_rate_fraction = abs(qd1) / rate_scale
    tail_rate_fraction = abs(qd2) / rate_scale
    steering_rate_progress = clamp(
        (head_rate_fraction - steering_onset_fraction) /
        max(1.0 - steering_onset_fraction, eps(Float64)),
        0.0,
        1.0,
    )
    steering_rate_gate =
        steering_rate_progress^2 * (3.0 - 2.0 * steering_rate_progress)
    posterior_rate_headroom = clamp(1.0 - tail_rate_fraction, 0.0, 1.0)
    direct_head_steering =
        params.head_steer_accel * turn_command * half_cycle_scale
    outward_direct_steering = direct_head_steering * qd1 > 0.0 ? 1.0 : 0.0
    steering_transfer_fraction =
        clamp(params.steering_transfer_strength, 0.0, 1.0) *
        steering_rate_gate *
        posterior_rate_headroom *
        outward_direct_steering
    transferred_steering =
        steering_transfer_fraction * direct_head_steering
    tail_drive =
    ...
    # While the redirect lacks same-sign yaw response, scale the full carrier
    # to prioritize steering. As that response appears, continuously restore
    # negative-work reversal without changing the steering residual.
    ...
    reversal_release = clamp(aligned_turn_response, 0.0, 1.0)
