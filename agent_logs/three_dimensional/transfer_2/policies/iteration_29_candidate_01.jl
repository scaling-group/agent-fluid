# Compact L64 target controller: preserve redirect-priority propulsion while
# signed hydrodynamic response releases surplus half-cycle steering.

function target_policy_params()
    return (
        version="dogfish3d_moment_anticipated_halfcycle_release_v1",
        control_period=0.55,
        ...
        yaw_moment_response_scale=0.018,
        half_cycle_response_release=0.50,
...
function target_policy(state, params)
    ...
    normalized_moment_response = tanh(
        yaw_moment /
        max(params.yaw_moment_response_scale, eps(Float64)),
    )
    aligned_moment_response = max(
        turn_command * normalized_moment_response,
        0.0,
    )
    fulfilled_turn_response = clamp(
        max(aligned_turn_response, aligned_moment_response),
        0.0,
        1.0,
    )
    half_cycle_release =
        1.0 -
        clamp(params.half_cycle_response_release, 0.0, 1.0) *
        fulfilled_turn_response
    half_cycle_scale =
        1.0 +
