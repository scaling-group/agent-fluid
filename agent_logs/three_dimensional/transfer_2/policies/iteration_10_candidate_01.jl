# Compact L64 target controller: preserve captured phase-shaped steering and
# allocate near-target carrier relief from observed course-redirect demand.

function target_policy_params()
    return (
        version="dogfish3d_course_gated_relief_v1",
        control_period=0.55,
        ...
        aligned_course_relief_floor=0.10,
...
function target_policy(state, params)
    ...
    available_drive_relief = approach_weight * closing_gate
    ...
    # Yield carrier authority while the observed course still needs a turn,
    # but retain the traveling bend on an aligned collision course. A small
    # relief floor keeps the transition continuous and preserves some terminal
    # damping; the existing distance/closing envelope remains the outer gate.
    aligned_relief_floor = clamp(
        params.aligned_course_relief_floor,
        0.0,
        1.0,
    )
    course_relief_demand =
        aligned_relief_floor +
        (1.0 - aligned_relief_floor) *
        course_authority * redirect_magnitude
    drive_relief = available_drive_relief * course_relief_demand
