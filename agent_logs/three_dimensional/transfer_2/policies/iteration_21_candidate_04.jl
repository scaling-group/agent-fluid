# Compact L64 target controller: release transient half-cycle steering as the
# measured approach yaw follows the turn request, preserving the posterior wave.

function target_policy_params()
    return (
        version="dogfish3d_response_released_half_cycle_v1",
        control_period=0.55,
        ...
        half_cycle_response_release=0.65,
...
function target_policy(state, params)
    ...
    # A response-triggered redirect should release rather than keep reinforcing
    # an established yaw. Attenuate only the transient half-cycle asymmetry
    # while the fish is both closing and turning in the requested direction;
    # loss of either response immediately restores steering authority. The gate
    # is unsigned, so the reflected target produces the reflected action.
    steering_release_weight =
        approach_weight * closing_gate * aligned_turn_response
    active_half_cycle_asymmetry = params.half_cycle_asymmetry * (
        1.0 - params.half_cycle_response_release * steering_release_weight
    )
    half_cycle_scale =
        1.0 + active_half_cycle_asymmetry * tanh(turn_phase_alignment)
