...
# and hand off smoothly to captured posterior lag allocation on approach.

function target_policy_params()
    return (
        version="dogfish3d_far_amplitude_near_lag_handoff_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # then hand off through body-frame distance to the shorter-path posterior
    # lag allocation before the terminal region. Both share the same observed
    # joint phase and target-derived turn request; neither adds a clock or mean
    # curvature. The far and near endpoints exactly recover sampled mechanisms.
