...
# and withdraw outward acceleration as measured joint rate nears its envelope.

function target_policy_params()
    return (
        version="dogfish3d_rate_guarded_wave_handoff_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # The sampled captures repeatedly reached the joint-rate envelope while
    # the acceleration still reinforced the measured velocity. Use one shared,
    # smooth rate-proximity gate so the two-joint wave loses authority together,
    # but attenuate each command only when it would increase that joint's speed.
    # Reversal authority and all low-rate steering/propulsion remain unchanged.
