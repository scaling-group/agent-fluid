...
# redirect and release only its anterior C-bend as measured yaw responds.

function target_policy_params()
    return (
        version="dogfish3d_anterior_released_los_redirect_v1",
        control_period=0.55,
        ...
        redirect_head_release_fraction=0.60,
...
function target_policy(state, params)
    ...
    # line-of-sight rotation. During closing it leads the bounded course-error
    # redirect without changing the far-field carrier.
    ...
    # The calibrated curvature map turns yaw opposite to its signed request.
    # Once yaw follows the redirect, release only the transient anterior bias;
    # the evidenced posterior redirect remains continuous and unmodulated.
    redirect_response = tanh(
        max(-redirect_command * turn_rate, 0.0) /
        max(params.redirect_response_scale, eps(Float64)),
    )
    redirect_head_weight = redirect_weight * (
        1.0 - params.redirect_head_release_fraction * redirect_response
    )
    ...
    head_bias =
        ...
        params.redirect_head_bias_boost * redirect_head_weight * redirect_command
