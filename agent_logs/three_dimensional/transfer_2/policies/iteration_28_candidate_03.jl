# Compact L64 target controller: align redirect-priority release with the
# measured response to that redirect while preserving the traveling wave.

function target_policy_params()
    return (
        version="dogfish3d_redirect_response_aligned_carrier_governor_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # redirect has priority only until measured yaw follows that same redirect,
    # which can differ in sign from the line-of-sight route request. This
    # bounded response signal governs rhythmic carrier work while leaving the
    # target-conditioned steering residual available.
    aligned_redirect_response = max(
    ...
    unfulfilled_redirect =
        course_authority *
        redirect_magnitude *
        (1.0 - clamp(aligned_redirect_response, 0.0, 1.0))
