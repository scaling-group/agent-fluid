# Compact L64 target controller: phase-allocate the captured course/LOS
# redirect while retaining its posterior-lagged traveling-bend carrier.

function target_policy_params()
    return (
        version="dogfish3d_redirect_half_cycle_los_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Allocate both route and active redirect curvature to the useful observed
    # half-stroke. Conflicting requests cancel before phase selection; joint
    # velocity supplies beat side without a clock or route state.
    phase_velocity = qd1 / max(omega * active_amplitude, eps(Float64))
    phase_turn_request = tanh(
        turn_command +
        params.redirect_phase_share * redirect_weight * redirect_command,
    )
    ...
    route_curvature = curvature_authority * turn_command
    redirect_curvature =
        params.redirect_curvature_boost * redirect_weight * redirect_command
    mean_tail_tangent =
        (route_curvature + redirect_curvature) * half_cycle_scale
    tail_target =
        mean_tail_tangent -
        q1 -
