# Compact L64 target controller: preserve LOS-led capture while braking only
# oscillation-envelope excess, rather than extinguishing the approach beat.

function target_policy_params()
    return (
        version="dogfish3d_orbit_braked_los_redirect_v1",
        control_period=0.55,
        ...
        approach_orbit_radius=1.0,
        approach_energy_scale=0.30,
...
function target_policy(state, params)
    ...
    # Distance scheduling defines a smaller desired orbit. Brake only radial
    # excess above that normalized orbit so the approach retains a bounded
    # traveling bend instead of settling into a static C-shape. This gate is
    # state-only, reflection invariant, and inactive in the far field.
    phase_position = centered_q1 / active_amplitude
    phase_radius = hypot(phase_position, phase_velocity)
    orbit_excess = max(phase_radius - params.approach_orbit_radius, 0.0)
    excess_energy_gate = tanh(
        orbit_excess / max(params.approach_energy_scale, eps(Float64)),
    )
    approach_brake =
        params.approach_damping * drive_relief * excess_energy_gate * qd1
