# Compact L64 target controller: retain range-specific carrier coupling while
# measured yaw response releases excess steering half-cycle asymmetry.

function target_policy_params()
    return (
        version="dogfish3d_response_gated_half_cycle_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # State-derived half-cycle asymmetry: start with full useful-stroke
    # authority when the turn is unfulfilled, then release only the excess
    # duty asymmetry as same-sign yaw response appears. Current normalized
    # target bearing supplies a floor, and an opposing rate-brake request
    # restores full authority automatically. Joint velocity supplies phase
    # without a clock or hidden maneuver state.
    ...
    route_error_fraction = clamp(
        abs(target_angle) /
        max(params.target_angle_limit, eps(Float64)),
        0.0,
        1.0,
    )
    half_cycle_authority = max(
        route_error_fraction,
        1.0 - clamp(aligned_turn_response, 0.0, 1.0),
    )
    half_cycle_scale =
        1.0 +
        params.half_cycle_asymmetry *
        half_cycle_authority *
        tanh(turn_phase_alignment)
