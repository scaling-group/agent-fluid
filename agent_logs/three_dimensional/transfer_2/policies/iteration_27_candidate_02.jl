...
# common dissipative brake protects the two-joint traveling carrier.

function target_policy_params()
    return (
        version="dogfish3d_common_carrier_damping_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # remaining target-conditioned steering. Common carrier damping retains
    # the anterior/posterior relationship; the residuals restore the existing
    # mean bend, response-aware redirect, and approach feedback unattenuated.
    carrier_vdp_drive =
        params.oscillator_mu *
    ...
    # Convert the dominant normalized rate proximity into one nonnegative
    # damping coefficient for both carrier joints. Unlike whole-carrier
    # scaling, this withdraws kinetic energy without weakening the restoring
    # or reversal part of either carrier command when its partner is at risk.
    # The target-conditioned residuals remain additive and unscaled.
    ...
    common_carrier_damping =
        clamp(params.rate_guard_strength, 0.0, 1.0) *
        params.command_accel_limit / rate_scale *
        smooth_rate_gate
    governed_head_accel =
        carrier_head_accel - common_carrier_damping * qd1 +
        steering_head_accel
    governed_tail_accel =
        carrier_tail_accel - common_carrier_damping * qd2 +
        steering_tail_accel
