# Compact L64 target controller: preserve the captured traveling-wave scaffold
# and add a bounded anterior useful-stroke burst during observed counterturn.

function target_policy_params()
    return (
        version="dogfish3d_anterior_useful_stroke_counterturn_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # When yaw still opposes the current route request, concentrate a bounded
    # anterior correction on the useful stroke and release it as yaw aligns.
    # This changes neither the fixed posterior lag nor the mean tail curvature.
    ...
    useful_stroke = max(tanh(turn_phase_alignment), 0.0)
    counterturn_burst =
        params.counterturn_head_accel * counterturn_gate *
        useful_stroke * turn_command
