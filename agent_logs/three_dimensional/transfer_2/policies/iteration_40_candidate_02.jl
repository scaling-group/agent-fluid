# Compact L64 target controller: local full-demand previews withdraw only the
# contacting joint's positive work while shared reversal protects wave phase.

function target_policy_params()
    return (
        version="dogfish3d_local_work_shared_reversal_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Decompose energy direction before governing. Positive rhythmic work is
    # withdrawn only by the same joint's contact prediction, so an anterior
    # bottleneck cannot erase posterior thrust. The maximum local redirect
    # event still governs both reversing components to preserve wave phase.
    ...
    reversing_carrier_scale =
        1.0 - shared_reversal_guard * (1.0 - reversal_release)
    governed_head_accel =
