...
# body-load governor protects the phase-preserving traveling carrier.

function target_policy_params()
    return (
        version="dogfish3d_load_aware_redirect_carrier_governor_v1",
        control_period=0.55,
        ...
        carrier_load_force_scale=0.036,
        carrier_load_moment_scale=0.018,
        carrier_load_onset_fraction=0.83,
        carrier_load_guard_strength=0.45,
...
function target_policy(state, params)
    ...
    # Rate proximity and unfulfilled body-frame redirects retain the sampled
    # common-carrier allocation. In addition, normalized hydrodynamic load can
    # request carrier relief below the rate envelope, where the strongest
    # sampled force/moment events occurred. Both requests act only with
    # positive carrier work; target-conditioned steering remains untouched.
    ...
    force_x = _policy_safe(state.force_body_L[1], 0.0)
    force_y = _policy_safe(state.force_body_L[2], 0.0)
    yaw_moment = _policy_safe(state.moment_z_L2, 0.0)
    force_load_fraction = hypot(force_x, force_y) /
        max(params.carrier_load_force_scale, eps(Float64))
    moment_load_fraction = abs(yaw_moment) /
        max(params.carrier_load_moment_scale, eps(Float64))
    peak_load_fraction = max(force_load_fraction, moment_load_fraction)
    load_onset_fraction = clamp(
        params.carrier_load_onset_fraction,
        0.0,
        1.0 - eps(Float64),
    )
    load_progress = clamp(
        (peak_load_fraction - load_onset_fraction) /
        max(1.0 - load_onset_fraction, eps(Float64)),
        0.0,
        1.0,
    )
    smooth_load_gate = load_progress^2 * (3.0 - 2.0 * load_progress)
    load_carrier_guard =
        clamp(params.carrier_load_guard_strength, 0.0, 1.0) *
        smooth_load_gate *
        outward_power_alignment
    carrier_scale = 1.0 - clamp(
        max(rate_carrier_guard, load_carrier_guard),
        0.0,
        1.0,
    )
    governed_head_accel =
