...
# is deliberately excluded because sampled short-window response loops reversed
# curvature while the persistent course error remained.
...
function course_vector_guidance(state, params)
    ...
    # The fish swims along negative body x. These normalized directions make
    # the cross-product bounded and invariant to rigid world-frame heading.
    ...
    # This is sin(target direction - course direction). Before self-propulsion
    # makes course meaningful, target lateral geometry supplies the same sign.
    course_error =
...
function target_policy(state, params)
    ...
    # posterior target retain the coherent self-propelled wake.
    vdp_drive =
    ...
    # The sampled 3D response maps positive target-side error to positive mean
    # tail tangent and a small opposite anterior acceleration. This distributed
    # bend steers without shifting the anterior oscillator equilibrium.
    mean_tail_tangent =
