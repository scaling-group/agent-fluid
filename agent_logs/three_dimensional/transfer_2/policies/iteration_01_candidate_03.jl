...
# and add one bounded body-frame bearing-to-curvature steering mechanism.

function target_policy_params()
    return (
        version="dogfish3d_bearing_curvature_rate_brake_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # The sampled rollout establishes this sign during its initially successful
    # 0--4 T correction.  Recent yaw rate supplies continuous overshoot braking.
    bearing = clamp(
    ...
    # A bounded mean bend steers without destroying the traveling wave.  The
    # small anterior term gives prompt authority while the tail target carries
    # most of the curvature request.
