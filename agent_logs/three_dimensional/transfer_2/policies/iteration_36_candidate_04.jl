# Compact L64 target controller: preserve redirect-priority targeting while
# course-resolved yaw response releases phase-coherent carrier reversal.

function target_policy_params()
    return (
        version="dogfish3d_course_resolved_reversal_release_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Same-sign yaw is only response onset. Release negative-work reversal
    # when that response coincides with resolution of the speed-authorized
    # body-frame course demand; otherwise keep the common carrier subordinate
    # to steering without changing the target-conditioned residual.
