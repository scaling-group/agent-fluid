# Compact L64 target controller: a joint-state preview protects the traveling
# carrier while measured velocity-course resolution releases its reversal.

function target_policy_params()
    return (
        version="dogfish3d_course_resolved_predictive_guard_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # This short phase-space preview advances only with bounded positive-work
    # carrier acceleration; reversal and target steering remain outside it.
    ...
    # Preserve the sampled far response release. During the approach, pass
    # negative-work reversal only as the measured velocity course resolves the
    # body-frame redirect; same-sign yaw alone is insufficient evidence.
    course_resolution = 1.0 - clamp(redirect_magnitude, 0.0, 1.0)
    approach_release_authority =
        (1.0 - distance_wave_weight) +
        distance_wave_weight * course_resolution
    reversal_release =
        approach_release_authority *
