# Compact L64 target controller: joint-local full-demand previews protect the
# carrier and allocate outward steering through available posterior rate margin.

function target_policy_params()
    return (
        version="dogfish3d_rate_reserve_curvature_allocator_v1",
        control_period=0.55,
        ...
        steering_reallocation_strength=0.75,
...
function target_policy(state, params)
    ...
    # C-start-like allocation without a hidden stage: carrier work yields to
    # the speed-authorized body-frame direction residual until the actual
    # velocity course resolves. Same-sign yaw alone is response onset, not
    # evidence that the requested redirect is complete.
    unresolved_course_demand = clamp(
        course_authority * redirect_magnitude,
        0.0,
        1.0,
    )
    ...
    # remaining target-conditioned steering. This lets each joint withdraw its
    # own outward rhythmic work while the residual preserves the existing mean
    # bend, response-aware redirect, and approach feedback.
    carrier_vdp_drive =
        params.oscillator_mu *
    ...
    # Anticipate rate-envelope contact from each joint's complete pre-limit
    # demand. The short phase-space preview sees outward carrier and steering
    # demand, but the governor below withdraws only same-joint positive carrier
    # work so target feedback and the other joint's thrust remain available.
    ...
    bounded_head_outward_accel = raw_head_accel * qd1 > 0.0 ?
        min(abs(raw_head_accel), params.command_accel_limit) : 0.0
    bounded_tail_outward_accel = tail_drive * qd2 > 0.0 ?
        min(abs(tail_drive), params.command_accel_limit) : 0.0
    projected_head_rate =
        abs(qd1) + preview_time * bounded_head_outward_accel
    projected_tail_rate =
        abs(qd2) + preview_time * bounded_tail_outward_accel
    head_rate_progress = clamp(
        (max(abs(qd1), projected_head_rate) / rate_scale - onset_fraction) /
        max(1.0 - onset_fraction, eps(Float64)),
        0.0,
        1.0,
    )
    tail_rate_progress = clamp(
        (max(abs(qd2), projected_tail_rate) / rate_scale - onset_fraction) /
        max(1.0 - onset_fraction, eps(Float64)),
        0.0,
        1.0,
    )
    ...
    tail_outward_scale = 1.0 - max(
        redirect_guard,
        tail_positive_work_guard,
    )

    # Decompose energy direction before governing. A joint-local demand preview
    # can remove only that joint's outward carrier work; shared negative-work
    # reversal is subordinated solely to unresolved body-frame redirection.
    ...
    # Joint 1 is the sampled rate bottleneck while joint 2 usually retains
    # margin. When target-conditioned head steering would add outward speed,
    # transfer a bounded share to joint 2. Removing and adding the same
    # acceleration preserves the pre-limit mean-curvature channel; a second
    # full-demand preview refuses transfer that would consume tail rate margin.
    outward_head_steering = steering_head_accel * qd1 > 0.0 ?
        steering_head_accel : 0.0
    preliminary_steering_transfer =
        clamp(params.steering_reallocation_strength, 0.0, 1.0) *
        head_rate_gate *
        (1.0 - tail_rate_gate) *
        outward_head_steering
    bounded_tail_transfer_outward =
        preliminary_steering_transfer * qd2 > 0.0 ?
        min(abs(preliminary_steering_transfer), params.command_accel_limit) :
        0.0
    projected_tail_rate_with_transfer = max(
        projected_tail_rate,
        abs(qd2) + preview_time * bounded_tail_transfer_outward,
    )
    tail_transfer_progress = clamp(
        (projected_tail_rate_with_transfer / rate_scale - onset_fraction) /
        max(1.0 - onset_fraction, eps(Float64)),
        0.0,
        1.0,
    )
    tail_transfer_guard =
        tail_transfer_progress^2 * (3.0 - 2.0 * tail_transfer_progress)
    steering_transfer =
        (1.0 - tail_transfer_guard) * preliminary_steering_transfer
    allocated_head_steering = steering_head_accel - steering_transfer
    allocated_tail_steering = steering_tail_accel + steering_transfer

    # Release phase-coherent reversal only when measured yaw follows the route
    # request and the velocity-course redirect has actually resolved. This
    # state-conditioned transition replaces a hard near-distance stage.
    reversal_release = clamp(
        aligned_turn_response * (1.0 - unresolved_course_demand),
        0.0,
        1.0,
    )
