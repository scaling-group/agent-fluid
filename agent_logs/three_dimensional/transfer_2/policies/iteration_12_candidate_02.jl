# Compact L64 target controller: preserve captured LOS-led phase steering and
# withdraw only acceleration that would drive a joint farther into its rate cap.

function target_policy_params()
    return (
        version="dogfish3d_joint_rate_headroom_los_v1",
        control_period=0.55,
...
@inline function _policy_rate_headroom(accel, velocity, params)
    safe_accel = _policy_safe(accel, 0.0)
    safe_velocity = _policy_safe(velocity, 0.0)
    rate_reference = max(
        _policy_safe(params.joint_rate_reference, 1.0),
        eps(Float64),
    )
    guard_start = clamp(
        _policy_safe(params.rate_guard_start, 0.82),
        0.0,
        1.0 - eps(Float64),
    )
    rate_fraction = abs(safe_velocity) / rate_reference
    guard_weight = clamp(
        (rate_fraction - guard_start) / (1.0 - guard_start),
        0.0,
        1.0,
    )

    # Remove only the acceleration component that is aligned with velocity.
    # Opposing acceleration remains intact for the return stroke and redirect.
    velocity_sign = sign(safe_velocity)
    outward_accel =
        velocity_sign * max(velocity_sign * safe_accel, 0.0)
    guard_strength = clamp(
        _policy_safe(params.rate_guard_strength, 0.75),
        0.0,
        1.0,
    )
    return safe_accel -
        guard_strength * guard_weight * outward_accel
end
...
function target_policy(state, params)
    ...
    guarded_head_accel = _policy_rate_headroom(
        raw_head_accel,
        qd1,
        params,
    )
    guarded_tail_accel = _policy_rate_headroom(
        tail_drive,
        qd2,
        params,
    )
