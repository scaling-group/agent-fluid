...
# while separating persistent route-directed gait shaping from yaw damping.

function target_policy_params()
    return (
        version="dogfish3d_persistent_route_gait_direction_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Keep persistent route direction in the gait channel while leaving the
    # faster yaw-rate brake in mean steering. This prevents a stabilizing yaw
    # reversal from also swapping the useful and return strokes of an otherwise
    # coherent traveling wave. Joint velocity supplies phase without a clock.
    phase_velocity = qd1 / max(omega * active_amplitude, eps(Float64))
    turn_phase_alignment =
        route_gait_command * phase_velocity /
