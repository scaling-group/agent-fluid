# Compact L64 target controller: preserve full course redirect while resolved
# body-frame directional demand releases phase-coherent carrier reversal.

function target_policy_params()
    return (
        version="dogfish3d_directional_demand_reversal_release_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Keep a common scale so the guarded carrier retains its traveling-wave
    # phase relationship.
    ...
    # Decompose carrier work only at the final allocation. Negative work is
    # useful joint reversal, but it may oppose a still-needed redirect. Release
    # that reversal from the guard only when yaw follows the route request and
    # the normalized velocity-course redirect has become small. Unlike the
    # sampled hard distance handoff, this transition follows measured response.
    ...
    reversal_release = clamp(
        aligned_turn_response * (1.0 - directional_demand),
        0.0,
        1.0,
    )
