# Compact L64 target controller: full-demand joint previews allocate unresolved
# redirect priority without suppressing the other joint's traveling carrier.

function target_policy_params()
    return (
        version="dogfish3d_joint_local_redirect_guard_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Anticipate rate-envelope contact from each full pre-limit joint demand.
    # Steering can therefore advance its own joint's phase-space preview, but
    # the downstream guard still yields only rhythmic carrier work.
    ...
    # Keep redirect priority local to the joint approaching its envelope. This
    # prevents an anterior steering event from withdrawing posterior carrier
    # authority solely because the head joint has less state-space margin.
    head_redirect_guard =
        guard_strength * head_rate_gate * unfulfilled_redirect
    tail_redirect_guard =
        guard_strength * tail_rate_gate * unfulfilled_redirect
    ...
    # Decompose direction before governing. Each joint's rate/demand trigger
    # withdraws only its own carrier contribution; target steering remains
    # outside the guard, and response-conditioned reversal is also joint-local.
    ...
    head_reversing_scale =
        1.0 - head_redirect_guard * (1.0 - reversal_release)
    tail_reversing_scale =
        1.0 - tail_redirect_guard * (1.0 - reversal_release)
    governed_head_accel =
        head_reversing_scale * reversing_head_carrier +
        head_outward_scale * outward_head_carrier +
        steering_head_accel
    governed_tail_accel =
        tail_reversing_scale * reversing_tail_carrier +
