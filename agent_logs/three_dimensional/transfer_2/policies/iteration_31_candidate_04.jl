# Compact L64 target controller: release the added course redirect only when
# measured yaw response is accompanied by reduced body-frame course demand.

function target_policy_params()
    return (
        version="dogfish3d_error_confirmed_redirect_release_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Treat the extra C-bend as an unmet-response residual, but do not confuse
    # response onset with completion. Release it only when same-sign yaw is
    # accompanied by a reduction in its own bounded course-redirect demand;
    # baseline route steering and yaw-rate braking remain available.
    ...
    redirect_completion =
        ...
        (1.0 - redirect_magnitude)
    redirect_response_residual =
        1.0 - clamp(redirect_completion, 0.0, 1.0)
    redirect_weight =
        approach_weight *
        course_authority *
        redirect_magnitude *
