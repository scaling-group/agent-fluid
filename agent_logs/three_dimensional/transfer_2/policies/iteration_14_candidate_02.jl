# Compact L64 target controller: preserve the captured LOS-led posterior
# half-cycle while making anterior useful-stroke reinforcement effort-aware.

function target_policy_params()
    return (
        version="dogfish3d_effort_aware_half_cycle_los_v1",
        control_period=0.55,
        ...
        anterior_effort_center=0.85,
        anterior_effort_width=0.08,
        anterior_feasibility_floor=0.35,
...
function target_policy(state, params)
    ...
    # line-of-sight rotation. While closing, that observed rate leads the
    # bounded C-bend without changing the far-field carrier.
    ...
    # Preserve the evaluated signed half-cycle allocation in the posterior
    # joint. For the anterior joint, retain return-stroke relaxation but reduce
    # only positive useful-stroke reinforcement when its previous command is
    # already near the smooth acceleration envelope. Command magnitude is
    # reflection invariant; turn times joint phase preserves left-right sign.
    ...
    signed_phase_bias = tanh(turn_phase_alignment)
    previous_head_accel = hasproperty(state, :previous_action) ?
        _policy_safe(state.previous_action[1], 0.0) : 0.0
    anterior_effort =
        abs(previous_head_accel) /
        max(params.command_accel_limit, eps(Float64))
    effort_transition = 0.5 * (
        1.0 - tanh(
            (anterior_effort - params.anterior_effort_center) /
            max(params.anterior_effort_width, eps(Float64)),
        )
    )
    feasibility_floor = clamp(
        params.anterior_feasibility_floor,
        0.0,
        1.0,
    )
    anterior_feasibility =
        feasibility_floor + (1.0 - feasibility_floor) * effort_transition
    useful_phase_bias = max(signed_phase_bias, 0.0)
    return_phase_bias = min(signed_phase_bias, 0.0)
    posterior_half_cycle_scale =
        1.0 + params.half_cycle_asymmetry * signed_phase_bias
    anterior_half_cycle_scale = 1.0 + params.half_cycle_asymmetry * (
        return_phase_bias + anterior_feasibility * useful_phase_bias
    )
    ...
    mean_tail_tangent =
        curvature_authority * turn_command * posterior_half_cycle_scale +
    ...
    raw_head_accel =
        head_drive +
        params.head_steer_accel * turn_command * anterior_half_cycle_scale
