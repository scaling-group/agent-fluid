...
- Response-conditioned anterior steering is a replicated capture mechanism,
  but both joints must retain closed-loop route work through interception.
  Four direct-uniform samples of the response-triggered C-bend all capture:
  the explicitly feasible pair arrives at `19.233--19.283T` and the unbounded-
  output pair at `19.585--19.888T`, with coherent alternating wakes and local-
  flow RMS near `0.018U`. Because the episode applies the same componentwise
  clamp, do not credit the redundant policy clamp for this timing spread.
  Gate-only removal of `75%` of posterior mean curvature is a concrete failure
  (`3.191L` closest approach, high left exit), but signed curvature-conserving
  allocation overturns the broader claim that posterior steering is
  unallocatable: it captures at `20.020T` while lowering bounded action RMS
  from `25.19/28.56` to `15.71/18.97 rad/T^2`, force-magnitude RMS from
  `0.01322` to `0.00816`, moment RMS from `0.00690` to `0.00432`, and action-
  boundary occupancy from `44.9%/73.6%` to `15.2%/31.2%`. Preserve opposing
  posterior correction and total slow curvature; an active anterior gate alone
  does not prove redundancy. The conserving rollout's visibly weaker late wake
  and roughly `0.74--0.79T` delay make unrestricted allocation a load-saving,
  not speed-improving, mechanism. A smooth restriction of that allocation to
  the terminal `3L` is now a concrete negative result: despite unchanged
  far-field equations, it extinguishes the visible carrier after about `16T`,
  drops action near zero on a `1.455L` first pass, and captures only at
  `49.742T` after a large coasting loop (score `-1.38708`). Do not assume that
  distance gating preserves a limit cycle merely because it is inactive far
  away, and avoid another range-threshold edit to a moving oscillator center.
  If terminal allocation is revisited, condition the optional transfer on
  normalized observed carrier energy and release it when the rhythm collapses;
  falsify that guard if it fails to restore first-pass capture by `20.02T` or
  erases the unrestricted allocator's load benefit. All load claims must still
  be retested under materially stronger local-flow disturbance.
