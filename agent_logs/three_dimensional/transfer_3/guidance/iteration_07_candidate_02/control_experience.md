...
- After polarity is calibrated, do not treat improved closest distance as
  ...
  An earlier half-cycle result was confounded by weakening the carrier to
  `20 degree`, `0.70T`, but the assigned parent's isolated test resolves that
  ambiguity: multiplicative posterior half-cycle scaling on the strong carrier
  accumulated a `0.645 rad` mean posterior offset, visibly curled the body and
  wake, exited upward at `8.657T`, and improved minimum distance only from
  `12.328L` to `11.994L`. In this still-water/no-cylinder regime, do not tune
  posterior half-cycle scaling as though it preserved propulsion; change the
  steering actuator, such as distributing one bounded slow curvature across
  centered joint equilibria, and require both a lower cross-track pass and an
  intact alternating wake. Falsify this boundary only if a later half-cycle
  law demonstrably keeps near-zero joint-cycle means and the strong carrier
  while crossing materially below the sampled `y=13.7L` target-abscissa pass.
