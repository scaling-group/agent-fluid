...
- One-sided carrier-demodulated phase recruitment does not have a replicated
  effort benefit. Its first run captures at `18.8980T` with `74.24%` posterior
  acceleration-limit occupancy and `0.01306/0.00680` force/moment RMS, but the
  exact-policy sampled repeat captures at `18.8045T` while rebounding to
  `75.93%`, `25.03/28.79 rad/T^2` action RMS, and
  `0.01352/0.00704` force/moment RMS. Those values exceed the inherited
  `74.44%` and `0.013105/0.006822` falsification bounds despite the same
  coherent body-led wake and only `0.01801U` local-flow RMS; faster capture in
  the repeat does not rescue a non-repeating load claim. The current raw
  helpful-moment half-cycle relief sample is the only alternative that moves
  route and effort together: versus the actuator-consistent prefill it changes
  arrival `18.6725 -> 18.6560T`, score `-0.13362 -> -0.13321`, posterior
  occupancy `76.11 -> 75.74%`, and force/moment RMS
  `0.01350/0.00703 -> 0.01328/0.00691`, but that small edge has only one run
  and remains inside known variability. A separate inherited LOS-response
  anterior-release candidate captures with score `-0.15658`, so neither that
  release nor another moment-residual scale is a supported next step. Preserve
  the LOS C-bend, traveling carrier, persistent phase path, and feasible action
  projection; resolve the raw helpful-response result only by exact replication,
  not scalar tuning. Falsify it if a repeat leaves the inherited
  `18.6725--19.0080T` capture band, exceeds `76.11%` posterior occupancy or
  `0.01350/0.00703` force/moment RMS, or weakens either wake view; if so retain
  the plain actuator-consistent route until a genuinely beat-scale response is
  observable. This boundary applies only to direct-uniform still water near
  `0.018U` local-flow RMS and says nothing yet about external-wake rejection.
