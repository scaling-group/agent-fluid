...
- A four-way capture ablation isolates posterior phase-recruitment timing from
  the already successful carrier and route loop. Gating coefficient-norm-
  preserving phase rotation with current predicted acceleration demand
  captures at `18.799T` (score `-0.14330`, mean score distance `2.03111L`),
  ahead of the half-cycle-only path by `0.1320T`, a gate driven by the previous
  applied action by `0.0935T`, and always-on phase rotation by `0.1980T`. All
  four have coherent alternating wakes and only `0.01816--0.01843U` local-flow
  RMS, while the current-demand gate stays in the lower phase-variant load
  band (`0.01315/0.00684` force/moment RMS) with `41.54%/73.96%` anterior/
  posterior acceleration-limit occupancy. Preserve current-demand recruitment
  as the phase baseline; delayed saturation confirmation and continuous phase
  authority are concrete negative controls, not reasons for gain tuning. If
  timing is extended, test a bounded state-based lead only on a rising demand
  flank, and falsify it unless capture remains earlier than `18.799T` without
  exceeding the half-cycle-only `0.01357/0.00707` load or `44.77%/76.09%`
  occupancy bounds. This implication applies to the coherent still-water route
  at about `0.018U` local-flow RMS and does not establish wake robustness.
- Phase recruitment has an evidenced timing-versus-load tradeoff, so exposure
  should not be increased by another scalar gate edit. The assigned-parent
  one-step demand lead only advances the current-demand gate from `18.7990T`
  to `18.7880T`, while raising posterior limit occupancy from `74.20%` to
  `75.67%` and force/moment RMS from `0.01315/0.00684` to
  `0.01333/0.00694`. Requiring current demand and previous applied action to
  remain on the same actuator side instead advances capture to `18.6725T`,
  improves mean distance to `2.02129L`, and preserves the coherent 3D wake,
  but returns occupancy and load to `76.41%` and `0.01350/0.00703`, nearly the
  half-cycle-only band. Thus selective release of phase authority improves the
  route while broad phase exposure relieves clipping; neither benefit implies
  the other. Preserve the same-side temporal cue, avoid another lead or
  activation-gain sweep, and test complementary transfer between amplitude
  asymmetry and phase rather than stacking both. Falsify that implication if
  authority transfer cannot retain capture by `18.931T` while reducing at
  least one of occupancy or load below the actuator-consistent sample; this
  boundary currently applies only to direct still water near `0.018U` local
  flow.
- Complementary amplitude-to-phase transfer is a useful intermediate, but a
  phase gate is not itself evidence that phase has replaced amplitude. The
  completed inherited handoff captures at `18.7495T`, faster than the
  demand-lead prefill's `18.7880T`, while lowering the fastest stacked route's
  posterior limit occupancy from `76.41%` to `74.48%` and force/moment RMS
  from `0.01350/0.00703` to `0.01336/0.00696`. Yet it gives up `0.0770T` and
  worsens mean distance from `2.02129L` to `2.02989L` relative to that stacked
  route. Preserve the same-side actuator-stress gate, but avoid either full
  stacking or gate-proportional amplitude removal as an endpoint; test
  command-space substitution that releases amplitude only when phase produces
  an aligned posterior target displacement. Falsify this implication if such
  aligned transfer cannot capture by `18.7495T` while keeping at least one
  load or occupancy measure below the stacked sample. This boundary is based
  on coherent direct-still-water wakes at about `0.018U` local-flow RMS and
  does not claim disturbance robustness.
