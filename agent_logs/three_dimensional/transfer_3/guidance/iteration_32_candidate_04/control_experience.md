...
- Instantaneous yaw-moment allocation is below the current identifiability
  floor and should not replace the plain actuator-consistent route. Two current
  same-hash plain runs both capture at `18.6725T` with scores
  `-0.13219/-0.13362`, yet already span `75.46--76.11%` posterior
  acceleration-limit occupancy and `0.01331--0.01350` / `0.00693--0.00703`
  force/moment RMS. Raw helpful-moment relief changes arrival by only one
  three-step interval to `18.6560T` (score `-0.13321`, `75.74%` posterior
  occupancy, `0.01328/0.00691` loads), while a carrier-demodulated residual is
  slower at `18.6890T` and `-0.13525` despite similarly low sampled loads.
  Their top-down and oblique sheets all retain the same coherent body-led wake
  near `0.018U` local-flow RMS, so no moment variant establishes a distinct
  route or wake mechanism. More importantly, inherited exact-policy evidence
  makes the one-sided residual range from `74.24%` and `0.01306/0.00680` to
  `75.93%` and `0.01352/0.00704`, and sampled optimizer guidance records later
  raw-helpful and plain recoveries near `18.99T`; these spreads erase the
  favorable one-run allocator claims. Preserve the normalized LOS C-bend,
  traveling carrier, persistent same-side phase path, and feasible-action
  projection, but recover the plain actuator-consistent controller rather than
  scalar-tuning or composing another instantaneous moment/LOS allocator. Test
  physical response again only when a genuinely beat-scale signal is
  observable and require replicated capture plus load separation from the
  plain route. Falsify this recovery if repeated plain runs lose capture,
  weaken either wake view, leave the inherited `18.6725--19.0080T` band, exceed
  `76.11%` posterior occupancy, or exceed `0.01350/0.00703` force/moment RMS;
  the boundary applies only to direct-uniform still water and establishes no
