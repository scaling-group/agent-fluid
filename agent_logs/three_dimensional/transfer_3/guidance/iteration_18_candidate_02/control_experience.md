...
- Response-conditioned anterior steering is a useful capture mechanism but is
  branch-sensitive, so identical-policy replication must be counted across
  inherited optimizer logs rather than only the current solver sample. The
  explicitly bounded policy hash captures three times at `19.233--19.739T`,
  yet the same hash also retains a coherent wake, misses at `1.845L`, and exits
  left at `28.875T` with score `-9.77873`; its route separates from the capture
  family around `6--10T` and sustains a saturated yaw request while passing
  high. Local-flow magnitude remains near `0.018U`, so this is marginal route
  closure rather than advection or wake collapse. Allocation evidence sharpens
  the remedy and its boundary: gate-only removal of `75%` of posterior mean
  curvature loses capture at `3.191L`, whereas moving only same-sign posterior
  correction forward while conserving total slow curvature captures at
  `20.020T` and reduces action RMS from `25.19/28.56` to `15.71/18.97 rad/T^2`,
  force-magnitude RMS from `0.01322` to `0.00816`, and moment RMS from `0.00690`
  to `0.00432`. Restricting that transfer by normalized range is a concrete
  negative result: a `1.62L` first pass becomes a broad orbit and only captures
  at `49.742T`. Preserve continuous two-joint route closure and the early
  carrier. The completed large-demand, same-sign allocator sharpens this
  boundary: it captures at `19.751T` while lowering action RMS to
  `20.88/25.33 rad/T^2`, body-lateral speed RMS to `0.214U`, force-magnitude
  RMS to `0.01067`, and moment RMS to `0.00562`; it is faster than unrestricted
  allocation but remains slower and higher at `18T` than the independently
  sampled response-reversing tail half-cycle, which captures at `18.931T`
  with action RMS `25.22/28.86`, force RMS `0.01357`, and moment RMS `0.00707`.
  Treat demand-gated curvature transfer and observed-response half-cycle
  asymmetry as a load-versus-arrival frontier, not interchangeable gain
  settings: if combined, preserve total slow curvature, opposing posterior
  correction, and the full traveling wave. Falsify compatibility if capture
  is lost or later than `19.751T`, either high-pass/orbit topology returns, or
  loads fail to improve on the half-cycle result. These load conclusions apply
  to this coherent still-water carrier and require retesting under stronger
