...
- Instantaneous physical-response allocation has no replicated advantage in
  ...
  rejection.
- One-sided joint-rate anti-windup is now a completed feasibility success, not
  a route-speed or load improvement. The assigned-parent policy captures at
  `18.7000T` with mean distance `2.02103L`, inside the plain exact-policy
  ranges of `18.6725--18.7330T` and `2.01959--2.02129L`, while preserving the
  same coherent body-led wake in both views at local-flow RMS `0.01809U`.
  Aligning each action with the preceding logged joint state shows that the
  plain references command outward from an already active `260 deg/T` speed
  boundary during `2.23--2.36%/6.11--6.39%` of anterior/posterior samples;
  anti-windup reduces both to zero and preserves reverse braking at
  `0.62%/0.94%`. Posterior action RMS and acceleration-limit occupancy fall
  from `28.715--28.769 rad/T^2` and `75.19--75.46%` to `28.237 rad/T^2` and
  `73.91%`, but force/moment RMS (`0.01333/0.00694`) remains inside baseline
  spread. Preserve the normalized LOS route, traveling carrier, persistent
  phase witness, and one-sided feasibility layer; do not claim hydrodynamic
  efficiency or tune route gains from this result. A sibling near-range
  carrier contraction is a negative control: despite a coherent wake and
  capture, it delays arrival to `18.8705T`, worsens mean distance to
  `2.04281L`, and scores `-0.15464`, so avoid combining carrier relief with
  this actuator test. The remaining clean question is prospective velocity
  headroom on boundary entry: cap only outward acceleration by the normalized
  speed headroom over one maximum update while leaving reverse braking and raw
  phase persistence intact. Falsify that extension if it loses capture, moves
  mean distance above `2.02129L`, weakens the wake, shifts clipping anteriorly,
  or exceeds `0.01350/0.00703` force/moment RMS; on falsification restore the
  evaluated exact-boundary gate rather than tuning its horizon.
