...
- Posterior phase recruitment is sensitive to response ownership, and carrier
  demodulation is now the only sampled physical-response change with a material
  effort result. The persistent same-side phase path captures at `18.6725T`
  with `76.11%` posterior limit occupancy and `0.01350/0.00703` force/moment
  RMS; inherited logs place an identical-hash repeat at `19.0080T`, so current
  arrival differences inside that `0.3355T` spread are unresolved. The assigned
  parent's stress-confirmed raw-moment amplitude relief captures at `18.7440T`
  with `75.44%` occupancy and `0.01343/0.00699` loads, establishing neither a
  speed nor an effort improvement. Raw helpful-moment amplitude relief is
  compatible and fastest at `18.6560T`, but still has `75.74%` occupancy and
  `0.01328/0.00691` loads. In contrast, subtracting the sampled two-joint
  carrier estimate before using moment to modulate phase captures at
  `18.7165T` while lowering posterior occupancy to `74.14%`, action RMS to
  `24.46/28.56 rad/T^2`, and loads to `0.01309/0.00682`; its higher mean
  distance (`2.02337L` versus `2.02115--2.02198L`) supports load shedding, not
  better route control. All four top-down and oblique sheets retain the same
  coherent body-led wake at `0.01807--0.01815U` local-flow RMS. Preserve the
  normalized LOS route and persistent phase actuator; avoid another raw-moment
  gate, command-deficit multiplier, global handoff, or gain-only retune. If the
  demodulated residual is reused, test it on a redundant channel such as
  half-cycle amplitude rather than giving it ownership of primary phase.
  Falsify that allocation if it loses capture, exceeds the replicated
  `19.052T` half-cycle bound, raises mean distance above `2.02337L`, fails to
  keep posterior occupancy below `75.74%` and force/moment RMS below
  `0.01328/0.00691`, or weakens either wake view. These bounds apply only to
  the sampled self-generated still-water wake and do not establish external-
  disturbance rejection.
- Once route capture is replicated, capture termination and final distance at
  the `0.75L` threshold are saturated diagnostics rather than evidence that a
  new response allocator is better. All four fully observed samples capture
  with scores `-0.13321-- -0.13693`, while two inherited step-26 children also
  capture but regress to `-0.14551` and `-0.16256`; the latter compact logs do
  not expose policy semantics, mean distance, action, loads, or wake views, so
  they support rejecting unexplained regression but not attributing a control
  cause. In this replicated-capture regime, preserve the normalized LOS route
  and proven carrier/phase path, restrict experiments to a redundant bounded
  allocation channel, and require full evidence to separate score, arrival,
  mean distance, clipping, and loads. Falsify an allocation improvement if it
  merely retains capture while leaving the sampled score band or if its claimed
  effort benefit lacks trajectory and both-view wake evidence; this selection
  rule does not apply before a mechanism has established repeatable capture.
