...
- Instantaneous physical-response allocation has no replicated advantage in
  ...
  rejection.
- One-sided joint-rate anti-windup is now an evaluated actuator-feasibility
  improvement, though not a route-speed improvement. Against two exact
  actuator-consistent captures at `18.6725--18.7330T`, mean distance
  `2.01959--2.02018L`, and outward-at-limit occupancy
  `2.82--3.00%/7.02--7.33%`, the exact-boundary projection captures at
  `18.7000T` and `2.02103L` while reducing anterior/posterior outward occupancy
  to `0.618%/0.941%`. It also reduces posterior acceleration clipping from
  `75.19--75.46%` to `73.91%`, speed-limit/clipping overlap from
  `2.26--2.39%` to `0.676%`, and posterior action RMS from
  `28.72--28.77` to `28.24 rad/T^2`. Its top-down and oblique sheets preserve
  the same target-closing arc and coherent body-led alternating wake, while
  force/moment RMS (`0.01333/0.00694`) and local-flow RMS (`0.01809U`) remain
  inside exact-policy spread. Preserve one-sided projection, reverse braking,
  the normalized LOS route, posterior-lagged carrier, and raw same-side demand
  as the phase-persistence witness; do not claim that lower infeasible effort
  alone improves the route. Because waiting for exact equality still leaves
  measurable outward action at the logged boundary, test an anticipatory
  normalized rate barrier rather than lowering carrier gain or globally
  rescaling both joints. Falsify that refinement if outward occupancy does not
  fall below `0.618%/0.941%`, braking or wake phase changes, capture leaves
  `18.6560--19.0520T`, mean distance exceeds `2.02129L`, or force/moment RMS
  exceeds `0.01350/0.00703`. This lesson applies to a hard speed limiter with
  observed joint rate and must be retested under stronger external flow.
