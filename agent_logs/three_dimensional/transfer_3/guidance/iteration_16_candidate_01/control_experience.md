...
- Distributed anterior steering is the sampled mechanism that converts the
  ...
  loads. Replication now sharpens the robustness boundary: two feasible
  response-gate evaluations capture at `19.2335--19.2830T`, but a later
  materialization verified byte-identical to the `19.2335T` policy misses at
  `1.845L` and exits left. The assigned parent's position/velocity-quadrature
  recoil observer preserves capture but worsens score from `-0.1822` to
  `-0.4546`, so neither executable identity nor a lower offline beat-residual
  establishes robust closed-loop route response. Prefer redundant normalized
  geometry cues over another observer fit or scalar gain edit, and require
  repeated capture before calling a route robust. This lesson applies to
  direct-uniform still water with a coherent carrier and small local flow;
  falsify cue redundancy if a fused response-demand/collision-course gate
  oversteers, increases loads, or repeats the `1.845L` high pass, and falsify
  the broader recruitment invariant if a held-out pose or flow condition makes
  both successful gates lose capture on the same side.
