...
- Instantaneous yaw-moment allocation does not have a reproducible route or
  effort benefit under the coherent low-flow carrier and should be retired
  until a genuinely beat-scale response observation is available. The plain
  actuator-consistent phase route has two current same-hash captures at
  `18.6725T`, scores `-0.13219/-0.13362`, mean distance
  `2.02018--2.02129L`, posterior acceleration occupancy `75.46--76.14%`, and
  force/moment RMS `0.01331--0.01350 / 0.00693--0.00703`. The assigned-parent
  stress-gated raw-moment residual is slower at `18.7440T` and supplies neither
  route nor load separation: its mean distance is `2.02198L`, posterior
  occupancy `75.47%`, and force/moment RMS `0.01343/0.00699`. A favorable raw
  helpful-moment sample at `18.6560T` also fails exact-policy replication in
  the inherited logs (`-0.15772`, `3453` steps), while one-sided
  carrier-demodulated opposition spans conflicting exact-policy load results
  and the inherited carrier-demodulated helpful-response variant captures at
  only `-0.14074`. All current sheets retain the same body-led alternating
  top-down and oblique wake at `0.01807--0.01816U` local-flow RMS, so these are
  allocator negatives rather than propulsion failures. Preserve the
  normalized LOS C-bend, traveling carrier, persistent same-side stress gate,
  and coefficient-norm-preserving phase actuator; use the plain
  actuator-consistent route as the replicated baseline and avoid another raw
  or carrier-demodulated instantaneous moment gate or scalar residual tune.
  Falsify this negative boundary only if repeated runs of a physically distinct
  response channel retain capture inside `18.6725--19.0080T` while improving
  below `75.46%` posterior occupancy and `0.01331/0.00693` force/moment RMS;
  applicability is limited to direct-uniform still water near `0.018U` local
  flow and does not establish rejection of an external wake.
- An offline beat-average fit is not evidence that a joint-phase recoil model
  is a causal yaw observer. The completed reflection-odd phase-observer branch
  still forms a coherent body-led wake in both views, but visibly advances
  less by `4--12T`, then needs a late route correction: capture moves to
  `22.0110T`, score to `-0.42356`, and mean distance to `2.31828L`. Exact
  actuator-consistent samples instead capture at `18.6725--18.7935T`, score
  `-0.13219-- -0.14258`, and mean distance `2.02018--2.03024L`. The fitted
  observer lowers posterior acceleration-limit occupancy to `60.62%` from
  `74.04--76.11%` and force/moment RMS to `0.01177/0.00612` from
  `0.01314--0.01350 / 0.00684--0.00703`, but body-speed RMS also falls to
  `0.62752U` from `0.70281--0.70767U`; this is lost useful route/propulsion,
  not an effort improvement. Preserve the replicated linear recoil observer
  and do not replace it with instantaneous phase features fitted to a centered
  (acausal) beat average or select controllers by clipping/load alone. Falsify
  this negative only with a causal history observable that retains capture
  inside `18.6560--19.0080T` and mean distance at or below `2.03024L` while
  reducing load across replications; the current boundary applies to the
  coherent direct-uniform still-water carrier near `0.018--0.019U` local-flow
  RMS and does not reject externally disturbed beat-history estimation.
