...
- Raw hydrodynamic-moment allocation is not yet resolved beyond replication
  ...
  local-flow RMS.
- Opposition-only carrier-demodulated phase recruitment extends the established
  load/route tradeoff; it does not remove it. The completed inherited rollout
  captures with the same coherent top-down and oblique wake at `18.8980T`, but
  its score `-0.14813` and mean distance `2.03633L` trail the sampled helpful-
  moment amplitude allocator's `18.6560T`, `-0.13321`, and `2.02115L` and the
  actuator-consistent `18.6725--18.7385T` timing band. The one-sided residual
  does materially lower action RMS to `24.42/28.63 rad/T^2`, posterior
  acceleration-limit occupancy to `74.19%`, and force/moment RMS to
  `0.01307/0.00680`, compared with `28.77--28.85 rad/T^2`, `75.58--76.11%`,
  and up to `0.01350/0.00703` for sampled raw-response routes. Treat it as a
  load-shedding option, not a faster route: avoid increasing instantaneous
  residual phase authority or moving demodulated residual onto half-cycle
  amplitude, whose inherited captures are slower still at `18.975--19.107T`.
  For score-focused continuation, replicate the unresolved helpful-moment
  allocator or change to genuinely beat-scale history. Falsify this boundary
  only if a replicated opposition-only policy enters the
  `18.6725--18.7385T` route band while retaining posterior occupancy below
  `74.44%`, force/moment RMS below `0.01311/0.00683`, and both wake views; the
  conclusion is limited to direct-uniform still water near `0.018U` local flow.
