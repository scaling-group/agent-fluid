...
- Distributed curvature becomes a successful pursuit mechanism only when its
  recruitment persists through the mid-approach bearing lull and still
  releases with observed route response. Bearing-only recruitment first
  improved the coherent LOS-rate carrier from a `3.369L` high miss to
  `1.897L`; two independent persistence translations then converted that
  topology into capture. Gating the same bounded `6 deg` anterior center from
  the already-bounded bearing-plus-LOS response captures at `19.585T`, while a
  range/closing/predicted-miss gate captures at `19.784T`. Both retain the
  alternating top-down street and compact oblique Lambda2 wake in direct
  uniform still water. The simpler response gate is the evidence-backed
  incumbent because it also lowers force RMS (`0.01261` versus `0.01314`),
  moment RMS (`0.00661` versus `0.00690`), and raw acceleration-envelope
  occupancy (`39.5%/71.6%` versus `41.5%/72.9%`). Preserve response-gated
  anterior recruitment and the traveling carrier before adding terminal
  mechanisms; do not reintroduce continuous full-residual allocation, which
  produced a lower exit, or more range-only damping, which preserved the high
  miss. This lesson applies when a coherent self-propelled route has small
  local-flow RMS (about `0.018--0.020U`) and transient bearing alignment is the
  release failure. Falsify it if reevaluation or a held-out pose loses capture,
  curls to a boundary, or materially raises clipping/loads; only then test a
  new response observable rather than scalar thresholds.
