...
- Posterior phase recruitment is sensitive to response semantics, not merely
  ...
  complementary handoff, and do not scalar-tune either negative control. The
  parent trace instead shows normalized hydrodynamic yaw moment opposing the
  requested correction in `65.6%` of strong-response samples, with opposing
  versus helping RMS `0.00742/0.00395`; test physical-response-conditioned
  release when the measured moment already helps. Falsify that implication if
  it loses capture, arrives later than about `18.931T`, weakens the alternating
  wake, or exceeds the parent `0.01350/0.00703` load, and do not extrapolate it
  beyond the sampled low-flow still-water regime without disturbance evidence.
