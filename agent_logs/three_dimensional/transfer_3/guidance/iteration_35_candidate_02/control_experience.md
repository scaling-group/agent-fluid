...
- Instantaneous physical-response allocation has no replicated advantage in
  ...
  evidence. The current sample sharpens that boundary with a visual and
  same-hash replication envelope: three exact actuator-consistent instances
  capture at `18.6725`, `18.6725`, and `18.7330T`, while their scores span
  `-0.13362-- -0.13142`, mean distance spans `2.01959--2.02129L`,
  anterior/posterior acceleration occupancy spans
  `40.4--42.2%/75.2--76.1%`, and force/moment RMS spans
  `0.01331--0.01350/0.00693--0.00703`. All three touch the `260 deg/T`
  joint-speed limit and show nearly indistinguishable coherent top-down
  reverse-vortex streets and oblique Lambda2 structures. Helpful-moment relief
  again reaches `18.6560T` with `75.8%` posterior occupancy and
  `0.01328/0.00691` loads, but its `0.0165T` edge is smaller than both the
  current baseline timing spread and its failed inherited repeat. One-sided
  demodulated-moment recruitment is slower at `18.6890T`,
  ...
  moment/LOS/residual allocation or scalar tuning. Treat the measured
  same-policy timing, saturation, and load ranges as a joint acceptance
  envelope: a new actuator mechanism must preserve capture and wake coherence
  while separating at least one physical metric outside that envelope, rather
  than winning by a one-run timing delta. `turn_rate_recent` and the bearing
  window cover only about `0.0385T`, so test genuinely beat-scale response only
  when a true beat-scale observable exists. This boundary is limited to
  direct-uniform still water and does not establish external-wake rejection.
