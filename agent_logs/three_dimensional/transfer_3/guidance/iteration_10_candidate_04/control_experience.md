...
- Heading alignment is not route alignment, but completed evidence now rejects
  ...
  Range-normalized inertial line-of-sight rotation is a positive but partial
  response signal: it retains the coherent wake, lowers target-station
  crossing from `y=13.718L` to `12.994L`, and improves closest approach from
  `4.158L` to `3.369L`, but the posterior-only loop still passes about `3.5L`
  high and exits left. The inherited geometry-gated same-sign C-bend is a
  complementary negative actuator result: delaying it until the target is
  abeam/behind worsens crossing to `y=14.072L`, reaches only `4.467L`, and
  exits left at final range `10.299L` despite retaining a wake. The completed
  opposed-coordinate redistribution establishes that actuator placement is
  high leverage but rejects a fixed share as the whole solution: it is the
  first inherited trajectory inside `3L` and improves minimum range from
  `3.369L` to `2.232L`, while overcorrecting the target-station crossing from
  `3.494L` high to `2.907L` low and ending at `11.487L`. Its coherent wake
  survives, but actual anterior acceleration-envelope occupancy rises from
  `57.7%` to `69.2%`, falsifying the dry replay expectation that occupancy
  would be unchanged. Conversely, range-only carrier damping lowers occupancy
  yet repeats the posterior-only route (`3.392L` minimum and `y=12.992L`
  crossing), so drive relief alone is not a terminal correction. Preserve the
  line-of-sight response and early opposed polarity, but do not increase the
  fixed share or resume scalar tuning of instantaneous course, posterior-only
  curvature, response-damped half-cycle strength, or late same-sign bursts.
  When opposed allocation has already brought the route near the target's
  lateral station around `4L` range, test a continuous geometry-conditioned
  handoff of steering authority; falsify a memoryless handoff if it loses entry
  into `3L`, fails to reduce anterior saturation, or retains a greater-than-`2L`
  high/low pass, then prefer a genuinely beat-scale estimator or different
  carrier.
