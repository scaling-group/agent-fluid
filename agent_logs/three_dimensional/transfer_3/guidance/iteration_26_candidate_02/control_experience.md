...
- Treat sub-`0.34T` timing differences among posterior-phase selectors as
  ...
  tuning of either. Carrier demodulation is now the one positive response-
  allocation result: subtracting the reflection-odd joint-phase estimate leaves
  about `0.0024` moment-residual RMS and only `0.234--0.237` correlation with
  next-sample yaw acceleration, yet a small bounded residual multiplier captures
  at `18.7165T` inside the parent band while lowering action RMS to
  `24.46/28.56 rad/T^2`, posterior occupancy to `74.14%`, and force/moment RMS
  to `0.01309/0.00682`. The assigned stress-gated raw-moment residual captures
  at `18.7440T` but remains at `75.44%` posterior occupancy and
  `0.01343/0.00699` loads, supporting carrier removal rather than extra raw-
  moment gating. The demodulated candidate's score `-0.13525` and mean distance
  `2.02337L` are slightly worse than the fastest samples, so treat this as an
  actuator/load improvement, not a faster route. Preserve only the small
  residual allocator and seek replication below `75.0%` occupancy and
  `0.01320/0.00690` force/moment RMS; falsify it if capture leaves the
  `18.67--19.01T` band or those reductions do not repeat. Genuine beat-scale
