...
- Instantaneous physical-response allocation has no replicated advantage in
  ...
  A distinct inherited observer result also closes the tempting offline-fit
  escape from this negative. Adding normalized reflection-odd anterior phase
  terms reduced error against a centered `0.55T` yaw average offline, but in
  closed-loop CFD it delays capture to `22.011T`, worsens score to `-0.42356`
  and mean distance to `2.31828L`, and produces a visibly longer curved route.
  Its coherent wake and lower posterior occupancy/load (`60.62%`,
  `0.01177/0.00612`) show that effort relief alone is not useful when the yaw
  observer distorts route response; anterior occupancy also rises to `48.30%`.
  A newer stress-gated residual closes the narrower escape of using moment only
  after phase recruitment, near-satisfied yaw correction, and predicted
  posterior stress: it captures at `18.7440T` with mean distance `2.02198L`,
  posterior action RMS `28.77 rad/T^2`, `75.44%` acceleration occupancy, and
  `0.01343/0.00699` force/moment RMS. All of those overlap three simultaneous
  plain-route captures at `18.6725--18.7330T`, mean distance
  `2.01959--2.02129L`, `28.72--28.85 rad/T^2`, `75.19--76.11%`, and
  `0.01331--0.01350/0.00693--0.00703`; both visual rows are likewise
  indistinguishable. Do not retune or further nest instantaneous-moment relief:
  remove it unless replicated evidence separates route or effort from this
  plain-policy envelope.
  All current and inherited variants remain self-propelled near
  `0.018--0.019U` local-flow RMS, so these are route/observer negatives rather
  than advection or wake-survival failures. Preserve the normalized LOS
  C-bend, traveling carrier, persistent same-side phase path, and feasible
  action projection; avoid further instantaneous moment/LOS/residual
  allocation, scalar tuning, or online use of a centered-window offline fit.
  `turn_rate_recent` and the bearing window cover only about `0.0385T`, so test
  genuinely beat-scale response only when a causal beat-scale observable
  exists, and require replicated capture plus route and occupancy/load
  separation from the plain actuator-consistent route. This boundary is
  limited to direct-uniform still water and does not establish external-wake
  rejection.
- One-sided joint-rate anti-windup is now a replicated posterior feasible-
  command improvement with an unresolved route tradeoff. Three exact full
  two-joint-gate rollouts capture at `18.7000--18.8210T` and retain the same
  coherent body-led wake in both views at `0.534--0.538U` speed near `4T`.
  Across all three, posterior returned-action RMS is
  `27.98--28.24 rad/T^2` and acceleration-limit occupancy is
  `72.03--73.97%`, separated below two exact plain-route samples at
  `28.72--28.77 rad/T^2` and `75.22--75.46%`. The two inherited repeats also
  keep loads below the established ceiling (`0.01313--0.01318 /`
  `0.00683--0.00686` force/moment RMS), so suppressing same-direction
  acceleration at the observed `260 deg/T` boundary reliably removes
  returned infeasible posterior effort without destroying propulsion,
  braking, or capture. It does not establish faster progress: the gate's mean
  distance spans `2.02103--2.03220L`, and its latter two arrivals at
  `18.7825--18.8210T` trail the current plain route's
  `18.6725--18.7330T` / `2.01959--2.02018L` envelope. Nor is the anterior gate
  supported: despite zero returns on about `2.5%` of inherited samples, its
  `40.68--41.96%` acceleration occupancy does not improve the plain
  `40.37--40.82%` range, and posterior rate-limit residence remains
  `7.47--8.11%` rather than falling below plain `7.16--7.51%`. Preserve the
  hard-boundary posterior cleanup only where the physical limit is directly
  observable; do not claim load, rate-residence, or route improvement, lower
  carrier gains, or add the failed closing-speed contraction. The next clean
  isolation is posterior-only anti-windup with the anterior command restored;
  falsify that decomposition if it fails to keep posterior action below
  `28.72 rad/T^2` or occupancy below `75.22%`, loses capture or either wake,
  exceeds `0.01350/0.00703` loads, or does not reduce the full gate's route
  cost. On falsification, retain the full gate only as effort cleanup or
  restore the plain feasible-action projection rather than tuning the speed
  threshold.
