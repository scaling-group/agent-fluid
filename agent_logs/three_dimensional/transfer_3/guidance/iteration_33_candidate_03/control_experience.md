...
- Instantaneous physical-response allocation has no replicated advantage in
  ...
  evidence. All current sampled variants retain coherent body-led wakes in
  both views near `0.018U` local-flow RMS, while stress-confirmed moment relief,
  phase-demodulated residual response, and LOS anterior release add route cost.
  Preserve the normalized LOS C-bend, traveling carrier, persistent same-side
  phase path, and feasible action projection; avoid further instantaneous
  moment/LOS allocation or scalar tuning. Test a genuinely beat-scale response
  only when observable, and require replicated capture plus full occupancy and
  load separation from the plain actuator-consistent route. This boundary is
  limited to direct-uniform still water and does not establish external-wake
  rejection.
- Carrier-correlated yaw recoil has a repeatable position quadrature that the
  existing velocity-only correction omits. Across two exact actuator-
  consistent rollouts and the helpful-moment and stress-confirmed controls,
  one-period offline high-pass fits of yaw rate to anterior angle plus both
  joint velocities are stable: coefficients span only `-1.48-- -1.55`,
  `-0.716-- -0.720`, and `-0.341-- -0.351`, with `R^2=0.99435--0.99447`.
  The current velocity-only observer leaves `0.397--0.406 rad/T` fast
  residual, whereas the joint-quadrature diagnostic leaves about
  `0.103 rad/T`. This supports testing a centered joint-state quadrature
  observer as a new feedback structure, not retuning an instantaneous
  allocator. It does not yet establish control improvement: preserve the
  evaluated LOS C-bend and traveling carrier, center the angle term on the
  bounded anterior redirect so desired mean curvature is not cancelled, and
  reject the observer if later CFD loses capture, leaves the
  `18.6725--19.0520T` route band, weakens either coherent wake, exceeds `76.2%`
  posterior acceleration occupancy or `0.01350/0.00703` force/moment RMS, or
  fails to reduce reconstructed within-beat residual. The available seven-
  interval `turn_rate_recent` remains only about `0.0385T` and must not be
  relabelled a beat average; these fitted coefficients are specific to the
  present low-flow carrier and require re-identification after material gait
