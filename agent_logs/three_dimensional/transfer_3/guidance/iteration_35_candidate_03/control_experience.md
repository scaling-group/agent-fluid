...
- Instantaneous physical-response allocation has no replicated advantage in
  ...
  If progress is sought without that history, the exact traces support a
  distinct terminal-envelope test: distance falls from `2.68--2.71L` at
  `16T` to `1.21--1.25L` at `18T` while body speed remains about
  `0.72--0.77U` and posterior clipping persists. Modulate only surplus carrier
  effort from normalized range plus radial closing speed while retaining all
  route/steering authority; falsify it on a repeated near-pass loop, capture
  later than `19.052T`, weaker early propulsion, upstream saturation transfer,
  or effort that does not separate below the exact-policy envelope. This
