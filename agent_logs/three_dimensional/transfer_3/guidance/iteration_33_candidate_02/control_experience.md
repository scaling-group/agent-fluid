...
- Instantaneous physical-response allocation and fitted carrier-conditioned
  route geometry have exhausted their present evidence budget. The plain
  actuator-consistent phase policy now has three directly available captures:
  two at `18.6725T` and one at `18.7275T`, with mean distance
  `2.02018--2.02493L`, posterior acceleration-limit occupancy
  `75.46--76.14%`, and force/moment RMS
  `0.01331--0.01350` / `0.00693--0.00703`. Raw helpful-moment relief reaches
  `18.6560T` once, but its exact inherited repeat takes about `18.9915T`, so
  the apparent lead is smaller than same-policy timing variation. Likewise,
  one-sided carrier-demodulated recruitment's original `74.24%` occupancy and
  `0.01306/0.00680` load advantage does not repeat: later captures at about
  `18.8045T` and `18.6890T` return to as much as `75.93%` occupancy and
  `0.01352/0.00704` loads. Demodulated helpful-response relief then captures
  at `18.7495T` with worse `2.02850L` mean distance, while stress-gated raw
  relief reaches only `18.7440T`; none separates route or effort from the
  plain policy's spread. More seriously, subtracting a fitted joint-phase
  projection from bearing loses capture: it reaches `1.6947L`, loops despite
  a coherent wake, and exits left at `32.5654T` with `10.7380L` final range.
  These top-down and oblique comparisons are route/allocation negatives, not
  advection or wake-collapse results. Preserve normalized bearing plus LOS-rate
  C-bend guidance, the traveling carrier, persistent same-side phase
  recruitment, and feasible action projection. Avoid further instantaneous
  moment/LOS residual allocation, fitted carrier subtraction from target
  geometry, and scalar tuning of those negative controls. The available
  `turn_rate_recent` and bearing window span only about `0.0385T`, not the
  `0.55T` beat; test response separation again only with a genuine beat-scale
  observable or a disturbance regime that distinguishes fluid response from
  recoil. Falsify the surviving baseline if a repeat loses capture, leaves the
  inherited `18.6725--19.0520T` capture band, weakens either wake view, exceeds
  `76.2%` posterior acceleration occupancy or `0.01350/0.00703` force/moment
  RMS. This boundary applies only to direct-uniform still water near `0.018U`
  local-flow RMS and does not establish external-wake rejection.
