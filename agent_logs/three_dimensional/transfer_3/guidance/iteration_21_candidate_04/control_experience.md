...
- Posterior phase recruitment is sensitive to actuator-side release semantics,
  not just its amount. On the same coherent LOS-rate carrier, half-cycle-only,
  current-demand, demand-lead, and actuator-consistent gates all capture, but
  order at `18.931T`, `18.799T`, `18.788T`, and `18.672T`, with mean score
  distance improving from `2.04175L` to `2.02129L`. The actuator-consistent
  case is already ahead at `7T` and remains ahead at `18T` (`1.205L` versus
  `1.301--1.386L`), while all combined top-down and oblique sheets retain the
  same body-led alternating wake; this is sustained route improvement, not
  passive advection or a terminal crossing artifact. In particular, a secant
  lead does not beat releasing phase whenever raw demand and previous feasible
  action oppose, so avoid anticipatory phase exposure or scalar phase gain as
  the next lever. Preserve same-side beat-reversal release, but note its cost:
  relative to the demand-lead parent, posterior `99%`-limit occupancy rises
  from `75.67%` to `76.41%` and force/moment RMS rises from
  `0.01333/0.00694` to `0.01350/0.00703`, despite essentially identical local
  flow near `0.0181U`. When loads matter, test whether phase can be limited to
  the positive same-side gap between raw demand and realized feasible action,
  rather than weakening the carrier or reallocating curvature. Falsify that
  refinement if it loses capture, exceeds the `18.931T` half-cycle time, or
  fails to reduce phase exposure/load without preserving the direct topology;
  its applicability beyond quiescent low-local-flow release is untested.
