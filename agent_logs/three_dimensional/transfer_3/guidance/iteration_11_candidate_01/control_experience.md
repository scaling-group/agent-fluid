...
- A rotation-invariant line-of-sight-rate response is the first sampled
  ...
  to `y=12.985L`, and the same left exit remains. A completed bearing-triggered
  distributed C-bend is the first positive actuator-allocation test: it keeps
  the long three-dimensional alternating wake, lowers center target-line
  crossing to about `y=11.35L`, and improves minimum range from `3.369L` to
  `1.897L`. It still misses capture and exits left at `29.10T`; anterior and
  posterior raw over-limit occupancy rise to `62.2%` and `76.4%`, and its
  geometry-held anterior center remains about `+4.4 deg` when reconstructed
  phase-conditioned yaw already exceeds demand by `0.468 rad/T`. Beat-window
  comparisons also show that lower anterior mean and higher posterior mean
  accompany more negative yaw, evidencing opposite joint-coordinate yaw
  polarities. Preserve LOS-rate response and large-error distributed authority,
  but avoid more scalar range damping, far-field drive, or a same-signed bend
  held by raw bearing demand. Couple opposite-polarity joint centers to one
  observed yaw residual so both release or reverse with response. This applies
  to the coherent low-flow high-pass topology (`local-flow RMS about 0.021U`);
  falsify it if coordinated response release cannot beat `1.897L` or capture
  without worse saturation/load histories, in which case beat-scale route
  history or wave-shape modulation is the next missing capability.
