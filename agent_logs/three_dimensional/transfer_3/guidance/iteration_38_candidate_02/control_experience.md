...
- One-sided joint-rate anti-windup is now a sampled feasible-command
  ...
  speed-boundary samples replaces infeasible outward work without hiding the
  persistent same-side phase witness. However, force/moment RMS
  `0.01333/0.00694` and rate-limit residence overlap baseline, so do not claim
  reduced hydrodynamic load or faster capture, lower the carrier gain, or add
  the failed closing-speed contraction (which slowed capture to `18.8705T`
  and raised mean distance to `2.04281L`). Preserve this projection only for
  a hard limiter whose exact boundary state is observable, and require a
  repeat to separate returned effort below `28.72 rad/T^2` or `75.22%`
  occupancy while capture remains within `18.6725--19.0520T`, mean distance
  stays at most `2.02129L`, and loads stay below `0.01350/0.00703`; otherwise
