...
- Instantaneous physical-response allocation is exhausted as a robust route
  improvement for this carrier, including after carrier demodulation. The
  actuator-consistent parent captures twice at exactly `18.6725T` in the
  current sample, with scores `-0.13362` and `-0.13219`; this same-policy
  spread already contains the helpful raw-moment result at `18.6560T`, whose
  inherited exact-policy repeat takes about `18.99T`. One-sided opposing-
  residual recruitment also retains capture and both coherent wake views, but
  its three completed runs move from `18.8980T` and low `74.24%` posterior
  occupancy to `18.8045T` with `75.93%`, then `18.6890T` with `75.34%` and
  `0.013286/0.006917` force/moment RMS. The latter score (`-0.13525`) and mean
  distance (`2.02295L`) trail both current parent replicates, so the original
  `74.44%` and `0.013105/0.006822` effort bounds do not replicate even though
  route success does. Preserve the normalized LOS C-bend, traveling carrier,
  persistent same-side phase path, and feasibility projection; avoid another
  raw-moment scale, residual multiplier, yaw-closure relief, or velocity-
  headroom gate in direct-uniform still water near `0.018U` local-flow RMS.
  A more promising reusable observation change is to remove the carrier from
  route sensing itself: across all four current captures a one-period offline
  decomposition finds `0.1496 rad` RMS carrier content in raw body bearing,
  while a pooled normalized `(q1,q2,qd1/omega,qd2/omega)` projection leaves
  `0.02135 rad` RMS and has similar coefficients in every policy. Test that
  reflection-odd phase-conditioned bearing before adding actuator authority.
  This applies only to the fixed morphology and `0.55T` carrier; falsify it if
  it fails to reduce evaluated bearing carrier content, loses capture, leaves
  the inherited `18.6560--19.0520T` band, weakens either wake view, exceeds
  `76.2%` posterior occupancy or `0.01350/0.00703` force/moment RMS, and do not
  infer external-wake rejection until disturbed-flow evidence exists.
