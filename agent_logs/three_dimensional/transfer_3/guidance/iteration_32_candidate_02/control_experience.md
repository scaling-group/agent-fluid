...
- Instantaneous yaw-moment allocation is not a replicated improvement, even
  after carrier demodulation. The actuator-consistent reference captures twice
  at `18.6725T`, with mean distance `2.02018--2.02129L`, posterior acceleration
  occupancy `75.46--76.11%`, and force/moment RMS
  `0.01331--0.01350` / `0.00693--0.00703`. In contrast, exact-policy raw
  helpful-moment relief spans `18.6560--18.9915T` and scores
  `-0.13321-- -0.15772`; the apparent fast-run advantage therefore does not
  survive its inherited repeat. One-sided carrier-demodulated opposition also
  spans `18.6890--18.8980T`: its initial `74.24%` and `0.01306/0.00680` effort
  result rebounds to `75.93%` and `0.01352/0.00704` on one inherited repeat,
  while the current sample gives an intermediate `75.34%` and
  `0.013286/0.006917`. All sampled views retain coherent, body-led alternating
  wakes at `0.01808--0.01816U` local-flow RMS, so these are allocator-variance
  results rather than advection, propulsion, or stability differences.
  Preserve the normalized LOS C-bend, traveling carrier, persistent phase
  path, and feasibility projection; avoid further scalar tuning of raw moment,
  opposing-residual recruitment, yaw-closure release, or velocity-headroom
  gates. If instantaneous physical response is tested once more, first remove
  its joint-phase carrier and let only a helpful residual relieve a strictly
  bounded redundant amplitude channel, leaving propulsion and phase steering
  intact. Falsify that final separation if it loses capture, arrives outside
  `18.6725--19.0080T`, exceeds mean distance `2.02129L`, or fails to reduce
  occupancy and loads below `75.46%` and `0.01331/0.00693`; after that, wait
  for a genuinely beat-scale response observation rather than inventing
  another memoryless moment gate. These bounds apply only to direct-uniform
  still water near `0.018U` local-flow RMS and do not establish disturbance
  rejection in an external wake.
