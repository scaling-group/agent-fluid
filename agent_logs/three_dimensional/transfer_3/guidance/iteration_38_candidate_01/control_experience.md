...
- Actuator-envelope cleanup does not survive replication as a route
  improvement in this low-flow lane. A first one-sided joint-rate anti-windup
  sample captured at `18.7000T` and lowered posterior action RMS/acceleration-
  limit occupancy to `28.24 rad/T^2` / `73.91%`, but two later evaluations of
  the exact same policy hash slowed to `18.7825T` and `19.0905T`, with mean
  distance rising to `2.03157L` and `2.05157L`. Its posterior occupancy spans
  `72.24--73.91%`, yet rate-limit residence grows as high as `8.27%` and
  force/moment RMS remains inside the plain route's overlap, so removing
  infeasible outward commands neither keeps the joint out of saturation nor
  robustly improves the route. Anticipatory rate taper/headroom variants also
  capture only at `18.9365--19.1235T` with mean distance
  `2.04236--2.05212L`; closing-speed contraction and radial damping likewise
  slow to `18.8320--18.8705T` and `2.04018--2.04281L`. All preserve coherent
  top-down and oblique wakes at about `0.0181--0.0185U` local-flow RMS, so this
  is progress traded for command cleanup rather than wake failure. The newest
  assigned-parent moment-residual variant is also non-separating: it captures
  at `18.7440T`, mean distance `2.02198L`, and
  `0.01343/0.00699` force/moment RMS, while three exact-route samples capture
  at `18.6725--18.7330T`, mean distance `2.01959--2.02129L`, and overlapping
  loads. Preserve the plain actuator-consistent LOS/phase route and its
  componentwise acceleration projection; avoid further terminal carrier
  withdrawal, rate-feasibility projection, or instantaneous self-wake
  residuals without a distinct replicated disturbance signature. Falsify
  this negative boundary only if repeated runs both stay inside the exact
  route band and separate posterior effort plus rate residence without a
  route, load, or wake penalty; it applies to the current `0.55T` carrier in
  direct-uniform still water, not to external-wake rejection.
