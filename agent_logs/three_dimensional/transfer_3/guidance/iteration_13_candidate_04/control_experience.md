...
- Response-conditioned anterior steering is now a completed capture mechanism,
  ...
  in this oscillatory closed loop. When reducing effort, test actuator
  allocation that retains anterior steering and the posterior traveling wave
  while relieving only duplicated posterior mean bend. Falsify that allocation
  if capture is lost or posterior occupancy/load does not fall; this boundary
  applies to the coherent still-water carrier and must be retested under
  materially stronger local-flow disturbance.
