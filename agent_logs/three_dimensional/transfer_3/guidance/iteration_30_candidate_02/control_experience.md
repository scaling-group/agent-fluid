...
- Carrier-demodulated hydrodynamic response has a replicated effort benefit,
  but it should allocate a redundant amplitude channel rather than multiply
  the established primary phase actuator. Two earlier identical demodulated-
  residual runs capture at `18.7165T` and `18.7605T` with posterior
  acceleration-limit occupancy `74.17--74.44%` and force/moment RMS
  `0.013093--0.013105` / `0.006816--0.006822`. The current four-way ablation
  sharpens the actuator boundary: actuator-consistent phase captures at
  `18.6725T`; raw helpful-moment amplitude relief captures at `18.6560T` on a
  `12.447L` path with `0.013277/0.006912` loads and `40.74%/75.77%` acceleration
  occupancy; stress-confirming that relief delays the assigned parent to
  `18.7440T`; and using an opposing carrier-demodulated residual to increase
  phase authority is worst at `18.8045T`, a `12.502L` path,
  `0.013519/0.007041` loads, and `42.62%/75.93%` occupancy. All four retain the
  same coherent body-led top-down and oblique wake near `0.018U` local-flow
  RMS, so the negative result is excess route/effort, not advection or lost
  propulsion. The available inherited optimizer logs likewise contain only
  captures (scores `-0.15658-- -0.13682`); without trajectory or load fields,
  those scalars support retaining the capture-class route but cannot justify
  response gains or overturn this actuator-level negative. Preserve the LOS
  C-bend, carrier, persistent phase path, and carrier subtraction; avoid
  another opposing-residual phase multiplier, stress product, or scalar gain
  edit. If demodulated response is reused, let only helpful residual alignment
  relax half-cycle amplitude after yaw-response closure while opposing response
  leaves the route actuator intact. Falsify that allocation if capture leaves
  `18.656--18.804T`, the path exceeds `12.502L`, posterior occupancy exceeds
  `76.14%`, force/moment RMS exceeds `0.013499/0.007028`, or the coherent wake
  is lost. This boundary applies only to direct-uniform still water near
  `0.018U` local-flow RMS.
