...
- Instantaneous physical-response allocation has no replicated advantage in
  ...
  rejection.
- Exact-boundary anti-windup is a replicated posterior feasibility improvement,
  not a route-speed improvement and not an equally supported anterior change.
  Two same-policy runs capture at `18.7000T` and `18.7825T`; posterior returned-
  action RMS and acceleration-limit occupancy fall to `27.99--28.24 rad/T^2`
  and `72.24--73.97%` from the plain route's `28.72--28.77 rad/T^2` and
  `75.22--75.46%`, while both wakes remain coherent. Their scores
  (`-0.13320`, `-0.14324`) and mean distances (`2.02103L`, `2.03157L`) do not
  establish faster or shorter routing, and anterior RMS overlaps baseline
  while occupancy spans `40.68--41.96%` versus baseline `40.37--40.82%`.
  Prospective speed-headroom projection should now be treated as a negative:
  two executions slightly lower posterior effort further but capture at
  `18.7330T` and `19.1235T`, with mean distance `2.02411L` and `2.05212L`, so
  its route cost does not separate favorably and one run leaves the established
  arrival band. Preserve the normalized LOS route, phase recruitment, and
  reverse braking; if anti-windup is retained, localize exact-boundary blocking
  to the posterior actuator and keep same-side raw demand as its phase-
  persistence witness. Avoid another projection-horizon, pre-limit taper, or
  scalar carrier edit. This applies only when an exact hard rate-limit state is
  observable; falsify posterior-only allocation if it loses capture, leaves
  `18.6725--19.0520T`, exceeds `2.03157L` mean distance or
  `0.01350/0.00703` force/moment RMS, changes wake/braking phase, or fails to
  keep posterior effort below the plain route.
