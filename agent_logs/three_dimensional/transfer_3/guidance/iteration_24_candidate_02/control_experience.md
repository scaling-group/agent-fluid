...
- Treat sub-`0.34T` timing differences among the current posterior-phase
  selectors as unresolved until replicated. The exact actuator-consistent
  policy hash captures at both `18.6725T` and `19.0080T` (scores `-0.13362`
  and `-0.15441`), a `0.3355T` spread larger than the differences to the
  complementary handoff (`18.7495T`), hydrodynamic-moment release
  (`18.7880T`), and error-conditioned release (`18.7715T`). All preserve the
  same coherent top-down and oblique wake at `0.01798--0.01828U` local-flow
  RMS, so single-run scalar ordering does not establish better propulsion or
  routing. A further raw-moment use is concretely negative: although moment
  predicts next-sample yaw acceleration with correlation `0.972`, feeding a
  bounded forecast into beat-side response slows capture to `18.9860T`, raises
  force/moment RMS to `0.01355/0.00705`, and leaves posterior limit occupancy
  at `75.87%`. Preserve the normalized LOS, response-reversing half-cycle, and
  actuator-consistent phase architecture while measuring repeat spread; avoid
  another memoryless raw-moment authority gate or forecast. If repeated
  actuator-consistent runs do not separate from the `18.67--19.01T` band,
  test phase-conditioned fluid-response residuals or genuine beat-scale
  history instead. This lesson applies only to the sampled self-generated
  still-water wake and must be revisited under stronger ambient disturbance.
