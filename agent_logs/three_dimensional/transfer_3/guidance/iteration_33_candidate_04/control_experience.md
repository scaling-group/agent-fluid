...
- The newest four-way sample strengthens the negative boundary on
  instantaneous physical-response allocation and identifies a distinct
  observer hypothesis. Two exact actuator-consistent replicas both capture at
  `18.6725T` despite scores spanning `-0.13362-- -0.13219`; helpful-moment
  relief is only `0.0165T` faster, while compounding moment, stress,
  yaw-closure, and phase gates delays capture to `18.7440T`. That residual's
  `41.5%/75.5%` acceleration occupancy and `0.01343/0.00699` force/moment RMS
  overlap the parent's `40.8--42.2%/75.5--76.1%` and
  `0.01331--0.01350/0.00693--0.00703`; all four top-down and oblique sheets
  retain the same coherent body-led wake near `0.018U`. Do not add another
  instantaneous moment/stress gate or rank its one-run timing delta as an
  improvement. A separate offline audit exposes a model-form issue worth one
  controlled test: relative to a centered `0.55T` yaw average, the inherited
  linear joint-rate recoil proxy has `0.756--0.768 rad/T` RMSE, whereas one
  compact reflection-odd model using normalized anterior joint phase gives
  `0.123--0.127 rad/T` across both parent replicas and both actuator controls.
  Treat that average only as a diagnostic target, not an online observation;
  preserve the LOS route and traveling wave while testing the odd phase
  observer, and reject it if capture leaves `18.656--18.744T`, either wake
  weakens, or occupancy/load exceeds the parent bands. This implication is
  specific to the current `0.55T` carrier in direct-uniform still water and
  must be recalibrated after any gait-period or disturbance change.
