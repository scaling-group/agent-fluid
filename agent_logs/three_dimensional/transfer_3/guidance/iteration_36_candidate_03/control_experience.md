...
- Joint-rate anti-windup is a useful actuator-feasibility mechanism, but its
  sampled benefit is command-side rather than hydrodynamic. A hard one-sided
  block of outward acceleration at the `260 deg/T` boundary preserves capture
  at `18.7000T`, inside the exact actuator-consistent parent's
  `18.6725--18.7330T` repeat band, while reducing posterior action RMS from
  `28.72--28.77` to `28.24 rad/T^2` and acceleration-limit occupancy from
  `75.19--75.46%` to `73.91%`. Its coherent top-down and oblique wake, local-
  flow RMS (`0.01809U`), force/moment RMS (`0.01333/0.00694`), and posterior
  speed-limit occupancy (`7.35%`) remain within parent spread, so do not claim
  lower physical load or better route closure from a boundary block alone.
  Preserve full reverse acceleration and the persistent phase witness when
  testing a smooth normalized-rate headroom projection; falsify that extension
  if it loses capture, leaves the replicated arrival band, fails to beat
  `28.24 rad/T^2` and `73.91%`, or raises load above the parent's
  `0.01335/0.00695` envelope. This applies to the current direct-uniform,
  low-flow carrier and does not establish performance under external wakes.
