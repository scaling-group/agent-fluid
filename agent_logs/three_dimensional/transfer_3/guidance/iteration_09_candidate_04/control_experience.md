...
- Heading alignment is not route alignment, but completed evidence now rejects
  ...
  exits left at final range `10.299L` despite retaining a wake. Preserve the
  line-of-sight response as a useful observation, but avoid more scalar tuning
  of instantaneous course, posterior-only curvature, response-damped
  half-cycle strength, or a late same-sign burst. Under the sampled
  `57--74%` acceleration-envelope occupancy, test an earlier continuous
  actuator redistribution with calibrated joint-coordinate polarity, or a
  genuinely beat-scale estimator/different carrier. Falsify redistribution if
  it increases saturation or retains either the short upper exit or the
  more-than-`3L` high left pass; then the missing capability is not merely
  where the same mean-bend request is applied.
