...
- Phase-gate arrival differences smaller than same-policy variability are not
  resolved improvements. A persistent same-side actuator gate captures once
  at `18.6725T`, but an inherited repeat of the exact policy hash captures at
  `19.0080T`; that `0.3355T` spread exceeds the whole current mechanism band.
  Within that band, complementary amplitude handoff captures at `18.7495T`,
  while a rising-demand lead and helpful-yaw-moment release both capture at
  `18.7880T`. All use direct uniform still water, retain coherent body-led
  top-down and oblique wakes, and have only `0.01798--0.01828U` local-flow RMS,
  so none has a distinct trajectory or wake-survival class. Compare physical
  realization instead: the fast single same-side sample has `76.41%`
  posterior acceleration-limit occupancy and `0.01350/0.00703` force/moment
  RMS; complementary handoff lowers these to `74.48%` and
  `0.01336/0.00696`, while helpful-moment release reaches `74.41%` and
  `0.01306/0.00679`. Strengthening same-side phase through a saturating
  persistence map is a concrete negative control: it captures at `18.8650T`
  with `74.61%` occupancy and `0.01315/0.00685` loads, not a resolved gain over
  physical-response release. Preserve capture, the LOS-rate C-bend, and
  response-selective phase semantics; avoid scalar-tuning a gate to optimize
  sub-replication arrival deltas or increasing phase exposure. When extending
  the mechanism, condition bounded phase assistance on an adverse measured
  fluid response and require a load/occupancy improvement as well as capture
  no later than the robust half-cycle bound near `19.052T`. Falsify that
  implication if it adds switching, loses wake coherence, or crosses the
  `76.41%` and `0.01350/0.00703` actuator-consistent bounds. These conclusions
  apply only to the low-flow still-water regime until stronger-disturbance
  evidence exists.
