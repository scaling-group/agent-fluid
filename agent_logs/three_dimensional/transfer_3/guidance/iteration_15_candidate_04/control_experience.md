...
- Treat physical clipping as a control-allocation observation, not extra
  authority. Two feasible-output replications of the response-triggered C-bend
  capture at `19.2335--19.2830T` with scores `-0.17657-- -0.18218`, while two
  algebraically equivalent unbounded-output replications capture at
  `19.5855--19.8880T` with scores near `-0.204`; because the episode clips both,
  this spread does not establish clamp causality. More diagnostic is replay of
  all four completed traces: desired posterior yaw-residual steering averages
  `17.7--18.0 rad/T^2`, but only `3.8--4.5 rad/T^2` remains incrementally after
  carrier saturation, with same-sign posterior clipping in roughly `65--69%`
  of rows. Do not respond by raising gains or removing posterior curvature.
  When this continuously closed C-bend is the baseline, test bounded,
  sign-coherent spillover of only the rejected correction into unused anterior
  redirect capacity, keeping the carrier and posterior loop intact; falsify
  that allocation if it loses capture, repeats an upper/lower pass, pushes the
  anterior joint toward `45 degrees`, weakens the alternating wake, or cannot
  improve beyond the replicated arrival/score spread.
