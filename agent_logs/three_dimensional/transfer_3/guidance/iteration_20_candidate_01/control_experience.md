...
- Response-triggered distributed C-bend steering is a semantic success but its
  ...
  while improving its `75.0--76.1%` occupancy, and the predictive cue is the
  stronger timing result; this supports actuator-state timing rather than
  phase rotation by itself. Conversely, demand-gated total-curvature
  ...
  predictive actuator-headroom phase recruitment. Avoid always-active phase
  rotation, curvature redistribution, terminal coasting, predicted-miss
  release, range-only allocation, or threshold-only gain edits. A further
  phase-timing mechanism should distinguish persistent same-side saturation
  from beat reversal using observed actuator state; falsify it if arrival
  exceeds `18.931T` without reducing load below the actuator-gated band, if
  posterior occupancy exceeds `76.4%`, or if force/moment RMS exceeds
