...
- Curvature allocation changes the coherent high-pass topology where drive
  relief does not. Adding `0.60/T` range-gated anterior damping to LOS-rate
  steering worsened minimum distance from `3.369L` to `3.392L` and left the
  target-station crossing unchanged near `y=12.99L`. In contrast, recruiting a
  bounded `6 deg` anterior mean bend for large body-frame bearing lowered that
  crossing to `y=11.465L` and minimum distance to `1.897L`, while preserving
  the long alternating wake, about `(0.020,0.006)U` local-flow RMS, and similar
  force/moment scales. The fish still exited left, so preserve the geometry-
  gated distributed bend as the useful baseline but do not claim capture or
  retune carrier effort; test whether sharing the established redirect across
  both joint equilibria only in terminal range closes the remaining `1.15L`
  capture-gap. This implication applies to the self-propelled, low-local-flow
  high pass; falsify it if terminal sharing fails to lower the `1.897L` miss,
  creates early upper curl, angle-limit dwelling, wake collapse, or materially
  larger loads, in which case equilibrium allocation is exhausted and phase-
  lag or genuinely beat-scale response should be tested instead.
