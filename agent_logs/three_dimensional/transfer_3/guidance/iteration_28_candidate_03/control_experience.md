...
- Posterior phase recruitment is sensitive to response semantics, not merely
  ...
  negative controls. The next sampled cohort resolves where physical response
  can enter: retaining the slow normalized LOS route and primary phase actuator
  while helpful yaw moment relieves only half-cycle amplitude after response
  closure captures at `18.6560T` with `40.74%/75.74%` acceleration-limit
  occupancy and `0.01328/0.00691` force/moment RMS, versus `18.6725T`,
  `42.15%/76.11%`, and `0.01350/0.00703` for its actuator-consistent parent.
  The timing edge is below the inherited `0.066T` same-hash spread and is not
  yet a robust speed gain, but compatible capture plus lower effort survives
  the evidence. Extra response semantics are negative: stress-product gates
  capture at `18.7440T` and `18.7825T`, carrier-demodulated amplitude relief at
  `19.1070T`, and demodulating the primary phase path trades lower
  `0.01309/0.00682` loads for a later `18.7165T` capture and worse
  `2.02337L` mean distance. Preserve the simple response-closure amplitude
  residual for replication; avoid another instantaneous-moment stress gate,
  carrier regression, or phase-path modulation. Falsify the residual benefit
  if a repeat leaves the `18.6560--18.7385T` replication band, loses capture or
  wake coherence, exceeds `40.74%/75.74%` occupancy or `0.01328/0.00691`
  loads, or cannot separate effort relief from parent variability. Do not
  extrapolate this self-generated low-flow response to disturbance rejection.
