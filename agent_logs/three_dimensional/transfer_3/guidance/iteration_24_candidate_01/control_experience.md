...
- Posterior phase recruitment is sensitive to response semantics, not merely
  ...
  the half-cycle-only path. Two inherited-log results sharpen the negative
  boundary. Multiplying the same-side gate by positive unrealized command
  ...
  `-0.14192`. All retain coherent body-led top-down and oblique wakes at only
  `0.01798--0.01839U` local-flow RMS, so the differences are route-control
  effects rather than advection or wake survival. Preserve the same-side
  response-reversing gate; avoid another command-deficit multiplier or full
  complementary handoff, and do not scalar-tune either negative control. A
  completed hydrodynamic-response ablation now also rejects switching phase
  recruitment off whenever normalized yaw moment already helps: it retains
  capture and lowers posterior occupancy from `76.41%` to `74.41%` and
  force/moment RMS from `0.01350/0.00703` to `0.01306/0.00679`, but delays
  arrival from `18.6725T` to `18.7880T` and worsens score from `-0.13362` to
  `-0.13976`. The wake remains coherent and local-flow RMS is `0.01809U`, so
  this is a route-authority trade rather than wake loss. Do not repeat a
  helping-moment authority-release gate. A response-error-conditioned version
  reinforces that boundary: it still captures at `18.7715T` and `-0.13713`
  with `75.39%` posterior occupancy and `0.01335/0.00695` force/moment RMS,
  but it remains slower than persistent phase recruitment. Together with the
  full complementary handoff's `18.7495T`, `-0.14192`, and `74.16%` posterior
  occupancy, the evidence rejects using observed moment to remove route
  authority even when the removal is conditioned or lowers load. The signal
  supports a narrower mechanism: across all four sampled traces, moment
  predicts the next yaw-rate change with correlation `0.9716--0.9723`, fitted
  slope `12.85--12.88 (rad/T)/moment-unit`, and change RMS
  `0.090--0.093 rad/T`. Test a bounded signed one-step anticipation only in
  beat-side response while leaving phase recruitment intact. Falsify that use
  if it loses capture, arrives later than `18.788T`, exceeds the fastest
  branch's `0.01350/0.00703` load or `76.11%` posterior occupancy, or simply
  reproduces a slower release trajectory. Inherited scalar-only logs add
  captures at scores `-0.14930-- -0.15441`, supporting capture as the semantic
  baseline but not a mechanism or load claim; do not let such records override
  current multimodal traces. This implication applies only to the sampled
  self-generated-wake, low-flow still-water regime until disturbance evidence
  extends it.
