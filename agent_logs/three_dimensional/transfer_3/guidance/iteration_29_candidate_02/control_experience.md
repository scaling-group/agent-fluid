...
- Hydrodynamic-moment allocation is now a closed negative branch for this
  low-flow still-water carrier. The no-moment actuator-consistent phase policy
  captures at `18.6725T`, mean distance `2.02129L`, with `76.14%` posterior
  limit occupancy and `0.01350/0.00703` force/moment RMS. Raw helpful-moment
  relief is nominally faster at `18.6560T` with `75.77%` occupancy and
  `0.01328/0.00691` loads, but its `0.0165T` edge is smaller than the inherited
  same-hash `18.6725--19.0080T` timing spread; stress-gated raw variants arrive
  later at `18.7440--18.7825T`. Earlier demodulated phase runs did lower
  posterior occupancy to `74.17--74.44%`, but the assigned-parent replication
  arrives at `18.8650T`, raises mean distance to `2.03488L`, and reaches only
  `74.69%` / `0.01325/0.00689`, missing its own `2.02337L`, `74.3%`, and
  `0.01317/0.00685` support bounds. An inherited one-sided opposing residual
  is slower at `18.8980T`, while demodulated-amplitude variants degrade further
  to `18.9750--19.1070T` and `2.04497--2.05106L`. All retain coherent
  top-down and oblique wakes near `0.018U` local-flow RMS, so these differences
  are allocator-induced route/effort tradeoffs rather than advection or wake
  collapse. Preserve the normalized LOS C-bend, persistent phase gate, and
  traveling carrier, but avoid another raw, gated, carrier-demodulated,
  one-sided, amplitude, or gain-tuned moment allocator. Test a non-moment
  geometry-response release or genuine beat-history signal instead. Reopen
  moment allocation only if repeated evaluations move both route timing/mean
  distance outside the no-moment replication band and load/occupancy below it
  without losing capture or either wake; none of this evidence establishes
  rejection under stronger external disturbance.
