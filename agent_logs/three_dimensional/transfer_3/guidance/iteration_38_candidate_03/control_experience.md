...
- Instantaneous physical-response allocation has no replicated advantage in
  ...
  rejection.
- The replicated route exposes an actuator-envelope mismatch that is distinct
  from the rejected fluid-response allocators. Three exact actuator-consistent
  copies capture at `18.6725--18.7330T`, score `-0.13362-- -0.13142`, and mean
  distance `2.01959--2.02129L`; the assigned-parent stress-gated yaw-moment
  residual has the same visible target-closing arc and coherent top-down and
  oblique wake, but captures at `18.7440T`, score `-0.13364`, and mean distance
  `2.02198L`. It therefore has no route or effort separation worth preserving,
  especially against inherited all-capture score spread as wide as
  `-0.13320-- -0.16366`. Across the four current traces, anterior/posterior
  joints occupy the `260 deg/T` speed limit for `2.85--3.00%/7.02--7.36%` of
  samples, while acceleration still points outward for
  `2.82--3.00%/7.02--7.33%` of the full rollout; opposite-sign braking at the
  boundary is only `0--0.03%`, and posterior rate saturation overlaps
  acceleration clipping for `2.26--2.39%`. Strip the unsupported moment
  residual before testing a one-sided state-feedback anti-windup projection:
  suppress only infeasible outward acceleration while preserving reverse
  braking, the normalized LOS route, and the posterior-lagged carrier. If
  previous feasible action recruits phase, continued same-side raw demand at
  the exact speed boundary must remain its causal persistence witness until
  braking begins; a projected zero must not silently disable the established
  phase path. Do not lower carrier gain, globally rescale both joints, or
  interpret reduced command effort alone as a route improvement. This applies
  only when a hard speed limiter reports the observed state at its exact
  boundary; falsify it if outward action does not fall, braking or wake phase
  changes, capture leaves `18.6725--19.0520T`, mean distance exceeds
  `2.02129L`, or force/moment RMS exceeds `0.01350/0.00703`. Until evaluated,
  it is an evidence-backed feasibility test rather than a claimed CFD benefit.
