...
- Offline agreement with a centered beat average is not evidence for a causal
  response observer. The inherited reflection-odd anterior-phase model lowered
  diagnostic yaw RMSE from `0.756--0.768` to `0.123--0.127 rad/T`, but its
  completed closed-loop rollout delays capture from the exact route's
  `18.6725--18.7330T` sample to `22.0110T`, raises mean distance from
  `2.01959--2.02129L` to `2.31828L`, and drops score to `-0.42356`. Both wake
  views remain coherent and body-led, but the failed observer is visibly
  slower at `4T` and needs a larger late turn. Its lower posterior occupancy
  and force/moment RMS (`60.6%`, `0.01177/0.00612`) therefore reflect lost
  early route/carrier coupling rather than useful efficiency; anterior
  occupancy rises to `48.3%`. The assigned parent's removal of that observer
  recovers capture at `18.7330T`, the best sampled mean distance
  (`2.01959L`), and the original coherent route at
  `0.01335/0.00695` force/moment RMS. Avoid fitting instantaneous policy
  features to centered targets that contain future samples, and do not accept
  load relief without preserved early translation and route cost. Until a
  causal beat-scale observation exists, preserve the validated linear recoil
  path and use its response only for a bounded allocation or release around
  the established LOS carrier, never as a wholesale observer replacement;
  falsify any such allocator if capture leaves `18.656--18.7935T`, mean
  distance exceeds `2.02129L`, either wake weakens, or effort fails to separate
  below exact-route variation. This boundary applies to the current `0.55T`
  carrier in direct-uniform still water and must be retested after gait-period
  changes or under external disturbance.
