...
- Treat sub-`0.34T` timing differences among the current posterior-phase
  selectors as unresolved, but distinguish timing from actuator/load evidence.
  The exact actuator-consistent policy hash captures at `18.6725T`,
  `18.7385T`, and `19.0080T`; its two current traces span `75.58--76.11%`
  posterior limit occupancy and `0.013270--0.013499/0.006908--0.007028`
  force/moment RMS. Carrier-demodulated moment residual feedback captures at
  `18.7165T`, inside that timing and mean-distance band, while lowering
  posterior occupancy to `74.14%`, posterior action RMS to `28.558 rad/T^2`,
  and force/moment RMS to `0.013093/0.006816`; both visual rows retain the
  coherent self-propelled wake at `0.01815U` local-flow RMS. This is the first
  sampled moment path with a material load/occupancy separation: raw helpful-
  moment and stress-gated raw-moment releases retain `75.74%` and `75.44%`
  occupancy with higher loads, while a raw-moment forecast remains the
  concrete negative result at `18.9860T`, `75.87%`, and
  `0.01355/0.00705`. Preserve the normalized LOS, response-reversing
  half-cycle, and actuator-consistent phase architecture; when moment is used,
  subtract its joint-phase carrier and keep only a bounded residual allocator.
  Replicate that exact residual policy before adding another mechanism, and
  falsify the load benefit if posterior occupancy returns to the parent band
  or capture/load robustness degrades. This lesson applies only to the sampled
  self-generated still-water wake and must be retested under stronger ambient
  disturbance; it does not justify a direct raw-moment yaw command or scalar-
  only gain tuning.
