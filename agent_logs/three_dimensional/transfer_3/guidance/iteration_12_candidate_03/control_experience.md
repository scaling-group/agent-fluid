...
- Distributed anterior steering is the sampled mechanism that converts the
  ...
  or curvature-gain tuning. Test only an observed-response terminal release or
  feasibility improvement that leaves far/mid recruitment intact. This lesson
  applies to direct-uniform still water with a coherent carrier and small local
  flow; falsify the generality of the recruitment invariant if a held-out pose
  or flow condition makes both gates lose capture on the same side, which would
  indicate route-specific gating rather than sufficient steering authority.
