...
- Instantaneous physical-response allocation has no replicated advantage in
  ...
  rejection.
- One-sided joint-rate anti-windup is now a sampled feasible-command
  improvement, but not yet a replicated load or speed improvement. Suppressing
  only same-direction acceleration at the observed `260 deg/T` boundary while
  preserving reverse braking captures at `18.7000T` with mean distance
  `2.02103L`, inside the plain actuator-consistent route's current
  `18.6725--18.7330T` / `2.01959--2.02018L` spread. Both visual views retain
  the same coherent body-led wake and `0.536U` speed at `4T`. Posterior
  returned-action RMS falls from the exact samples' `28.72--28.77` to
  `28.24 rad/T^2`, and acceleration-limit occupancy falls from
  `75.22--75.46%` to `73.97%`; zero return on `6.38%` of posterior
  samples overall replaces infeasible outward work without hiding the
  persistent same-side phase witness. The full two-joint projection also
  returns zero on `2.44%` of anterior samples, but anterior action RMS and
  acceleration occupancy (`24.71 rad/T^2`, `40.68%`) overlap the two exact
  references (`24.70--24.73 rad/T^2`, `40.37--40.82%`); current evidence thus
  supports posterior cleanup, not a separate anterior benefit. Force/moment
  RMS `0.01333/0.00694` and rate-limit residence likewise overlap baseline, so
  do not claim reduced hydrodynamic load or faster capture, lower the carrier
  gain, or add the failed closing-speed contraction (which slowed capture to
  `18.8705T` and raised mean distance to `2.04281L`). Preserve this projection
  only for a hard limiter whose exact boundary state is observable, and first
  require an exact full-policy repeat to separate posterior returned effort
  below `28.72 rad/T^2` or `75.22%` occupancy while capture remains within
  `18.6725--19.0520T`, mean distance stays at most `2.02129L`, and loads stay
  below `0.01350/0.00703`. If that separation fails, restore the plain
  feasible-action projection or test posterior-only anti-windup as a distinct
  architecture before tuning the physical speed threshold.
