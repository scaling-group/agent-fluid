...
- Response-triggered distributed C-bend steering is a semantic success but its
  ...
  band. Actuator-conditioned phase recruitment is different: a gate from
  previous realized posterior action captures at `18.8925T`, mean distance
  `2.0378L`, with `74.5%` posterior occupancy and `0.01312/0.00683`
  force/moment RMS, while a predictive unclipped-demand gate captures at
  `18.7990T`, mean distance `2.0311L`, with `74.2%` occupancy and
  `0.01315/0.00684` loads. Both arrive ahead of the replicated half-cycle band
  while improving its `75.0--76.1%` occupancy. A later signed-consistency gate
  that recruits phase only when current unclipped demand and the previous
  realized posterior action remain on the same actuator side is the strongest
  timing result: it captures at `18.6725T` and lowers mean distance to
  `2.0213L`. Its benefit is not load relief: posterior 99%-limit occupancy
  returns to `76.4%` and force/moment RMS rises to `0.01350/0.00703`, versus
  `74.2%` and `0.01315/0.00684` for predictive headroom alone. The same
  coherent top-down vortex street, compact oblique Lambda2 packets, and local-
  flow RMS near `0.018U` persist, so signed actuator-state consistency is a
  reusable phase-timing cue but must not be treated as a desaturation
  mechanism. Preserve it when arrival is limiting; when load is also limiting,
  test a sign-symmetric, energy-neutral normalization of the existing
  half-cycle modulation rather than another phase threshold or curvature
  allocator. Falsify that implication if normalization cannot reduce the
  `76.4%` occupancy or `0.01350/0.00703` loads while keeping arrival below
  `18.799T`. Conversely, demand-gated total-curvature
  reallocation lowers action/load yet slows capture to `19.2390T` and raises
  mean distance to `2.0610L`; the inherited conservative allocator (`20.020T`)
  and near-range allocator (`49.742T` after a `1.62L` pass and `7.35L` loop)
  reinforce that negative result. Preserve normalized LOS-rate feedback,
  response-reversing half-cycle authority, continuous two-joint closure, and
  predictive-headroom plus signed-consistency phase recruitment. Avoid always-
  active phase rotation, curvature redistribution, terminal coasting,
  predicted-miss release, range-only allocation, or threshold-only gain edits.
  A further timing or carrier-effort mechanism is falsified if arrival exceeds
  `18.931T` without reducing load below the actuator-gated band, if posterior
  occupancy exceeds `76.4%`, or if force/moment RMS exceeds
  `0.01574/0.00810`. These bounds apply to the coherent still-water carrier
  near `0.02U` local flow and require retesting under stronger disturbance.
