...
- Instantaneous moment and LOS-response allocation now have replicated negative
  boundaries in this low-flow lane. The no-moment actuator-consistent phase
  policy captures at `18.6725T`, mean distance `2.02129L`, posterior limit
  occupancy `76.11%`, and `0.01350/0.00703` force/moment RMS, but its exact-hash
  assigned-parent repeat captures at `18.9970T`, mean distance `2.04736L`,
  `75.01%` occupancy, and `0.01338/0.00697` loads. Treat changes inside that
  roughly `0.32T` same-policy arrival spread as unresolved. Carrier-demodulated
  phase allocation first reaches `18.7165T`, `2.02337L`, `74.14%`, and
  `0.01309/0.00682`, but its exact-hash repeat degrades to `18.8650T`,
  `2.03488L`, `74.64%`, and `0.01325/0.00689`, so the claimed route/load
  separation does not replicate. Demodulated-amplitude variants are worse at
  `18.9750--19.1070T` and `2.04497--2.05106L`; instantaneous anterior release
  similarly lowers effort but reaches only `18.9310T` and `2.04457L`. Both wake
  views remain coherent and local-flow RMS stays about `0.018U`, identifying
  route-useful authority withdrawal rather than wake collapse or advection.
  Preserve the normalized LOS route, distributed C-bend, persistent joint-state
  phase gate, and traveling carrier; stop assigning instantaneous moment or
  counter-LOS response another route or release role, and avoid a gain-only
  retune. When testing phase as the lower-effort actuator, arbitrate it against
  amplitude under the existing stress gate rather than stacking both paths;
  falsify that direction unless replicated capture stays within about
  `19.008T`, both wakes remain intact, posterior occupancy falls below `76.11%`,
  and force/moment RMS does not exceed `0.01350/0.00703`. None of these
  still-water results establishes external-disturbance rejection.
