...
- Response-reversing beat-side feedback survives replication, and the timing
  of posterior phase recruitment now has a controlled semantic result. The
  same-side actuator-consistency gate is the strongest sampled route: it
  captures at `18.6725T`, mean distance `2.02129L`, and score `-0.13362`, ahead
  of demand-lead, current-demand, and instantaneous-headroom gates at
  `18.7880--18.8320T` and `-0.14000-- -0.14664`. All retain coherent body-led
  wakes and low local-flow RMS (`0.01808--0.01837U`), so the difference is
  feedback timing rather than advection or wake survival. The faster gate does
  raise posterior near-limit occupancy and force/moment RMS to
  `76.14%` and `0.01350/0.00703`; suppressing it by an unrealized-demand
  deficit is a concrete negative result, delaying capture to `18.9805T` and
  worsening mean distance to `2.04403L`. Do not infer that a feasible action
  matching raw-demand magnitude means the phase correction is no longer
  useful. A complementary amplitude-to-phase handoff is the better load
  tradeoff: it lowers occupancy and load to `74.16%` and
  `0.01336/0.00696` while still capturing at `18.7495T`, but its worse mean
  distance (`2.02989L`) shows that releasing amplitude on both beat sides
  sacrifices sustained progress. Preserve same-side phase release; when
  reducing stacked authority, test beat-side-selective arbitration rather
  than another gain, demand-deficit gate, or wholesale curvature transfer.
  Falsify that implication if it cannot retain capture by about `18.79T` and
  mean distance at or below `2.02833L` while reducing either occupancy or
  load. This boundary applies only to coherent still-water rollouts near
  `0.018U` local-flow RMS; stronger-disturbance robustness remains untested.
