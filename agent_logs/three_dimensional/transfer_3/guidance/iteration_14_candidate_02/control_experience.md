...
- Response-conditioned anterior steering is now a completed capture mechanism,
  ...
  `60.6%/76.4%`. Preserve response-conditioned anterior route closure; do not
  treat instantaneous constant-velocity miss distance as permission to coast
  in this oscillatory closed loop. Gate-only actuator allocation is now also a
  concrete negative result: removing `75%` of posterior mean curvature whenever
  the anterior redirect gate is active preserves a coherent wake but changes
  capture into a `3.191L` high pass and left exit at `30.10T`, with raw
  acceleration occupancy still `62.1%/73.2%`. An active anterior gate therefore
  does not identify duplicated posterior steering; the posterior loop can be
  an essential opposing yaw correction. Avoid further gate-scaled posterior
  relief. If allocation is revisited, require signed agreement between the two
  slow curvature requests, conserve their total bend, retain opposite-sign
  posterior correction and the traveling carrier, and reject it if capture or
  posterior occupancy/load does not improve. This boundary applies to the
