...
- Carrier-demodulated hydrodynamic response has a replicated effort benefit,
  and allocating it only against opposing response extends that benefit once
  without improving route speed. Two identical bidirectional residual runs
  capture at `18.7165T` and `18.7605T` with posterior acceleration-limit
  occupancy `74.17--74.44%` and force/moment RMS
  `0.013093--0.013105` / `0.006816--0.006822`, below the actuator-consistent
  parent's `75.58--76.11%` and `0.01327--0.01350` / `0.00691--0.00703` bands.
  The assigned-parent one-sided policy then captures at `18.8980T` with the
  same coherent body-led wake at `0.01809U` local-flow RMS while lowering
  posterior occupancy to `74.24%`, action RMS to `24.42/28.63 rad/T^2`, and
  force/moment RMS to `0.01306/0.00680`; its mean distance rises to `2.03633L`,
  so this is a further load-allocation result, not faster pursuit. In contrast,
  raw helpful-moment relief's `18.6560T` lead remains inside same-hash spread;
  stress-gating raw relief delays capture to `18.7825T`; and withdrawing
  outward phase near the posterior velocity limit captures at `18.8100T` but
  raises velocity-limit occupancy to `8.10%`. The assigned four-way capture
  comparison sharpens the mechanism boundary: the bidirectional demodulated
  residual has the lowest sampled force/moment RMS (`0.013093/0.006816`),
  posterior acceleration occupancy (`74.17%`), and posterior action RMS
  (`28.56 rad/T^2`), but the worst mean distance (`2.02337L`) and a
  `18.7165T` arrival. The faster helpful-moment amplitude relief reaches
  `18.6560T` with `2.02115L` mean distance but raises force/moment RMS to
  `0.013277/0.006912` and posterior occupancy to `75.77%`; actuator-consistent
  phase without a residual and raw stress-gated response likewise capture at
  `18.6725T` and `18.7440T` without matching the demodulated load reduction.
  All four retain nearly identical coherent top-down and oblique wakes at
  `0.01807--0.01815U` local-flow RMS, so demodulation is an effort-allocation
  mechanism rather than evidence of wake survival or route acceleration.
  Preserve the LOS C-bend, traveling carrier, persistent phase path, and
  carrier subtraction. Replicate one-sided opposing-response recruitment
  before changing it: let opposing residual recruit the redundant phase path,
  but do not withdraw that path when the residual helps. Avoid scalar-tuning
  raw response, yaw-closure relief, or velocity-headroom gates.
  Falsify the reusable one-sided benefit if replication loses capture, leaves
  the inherited `18.6725--19.0080T` band, weakens either wake view, exceeds
  `74.44%` posterior acceleration occupancy or `0.013105/0.006822`
  force/moment RMS, or reaches `8.10%` posterior velocity occupancy; the
  evidence applies only to direct-uniform still water near `0.018U` local-flow
  RMS and does not establish rejection of an external wake.
