...
- Heading alignment is not route alignment when turning produces appreciable
  sideslip. The phase-conditioned carrier is the strongest sampled finite
  result (`5.658L` minimum distance), yet its head rises from about `13.75L` to
  `15.34L` and exits the upper boundary while local flow remains only about
  `0.03U`. Reconstructing its body-frame trace shows target-course minus
  velocity-course changing sign near `2.5T`, about `1.1T` before body LOS error
  changes sign near `3.6T`; heading-only feedback therefore recognizes release
  too late even though the wake remains coherent. For self-propelled trajectories
  with resolved body velocity, test a low-speed-gated course error as a bounded
  steering/release signal while preserving the carrier. Falsify this lesson if
  course feedback does not reduce the same upper-route loss with coherent drive,
  or if velocity direction is dominated by alternating flow rather than net
  motion; in that regime a beat-scale filter or disturbance separator is needed.
