...
- A rotation-invariant line-of-sight-rate response usefully changes the broad
  approach, but completed descendants show that the decisive mechanism is
  response-sustained actuator distribution rather than more posterior gain or
  range damping. The LOS-only carrier keeps a long coherent wake yet passes
  `3.369L` high and exits left; its range-damped child slightly worsens the
  minimum to `3.392L`, and a bearing-only distributed C-bend improves to
  `1.897L` but releases during the mid-approach bearing lull. Sustaining the
  same bounded `6 degree` anterior oscillator-center shift either from the
  already-bounded bearing-plus-LOS demand or from an unsafe constant-velocity
  closest-pass estimate produces independent captures at `19.585T` and
  `19.784T`. The demand gate is marginally stronger here: versus the predicted-
  miss gate it has lower raw acceleration occupancy (`39.5%/71.6%` versus
  `41.5%/72.9%`) and lower force/moment RMS (`0.01261/0.00661` versus
  `0.01314/0.00690`). Preserve the coherent traveling bend, phase-conditioned
  posterior response, and continuously released distributed C-bend when low
  local flow and a high-pass route failure recur; avoid another carrier-gain,
  posterior-limit, or range-only edit. This preference is not universal:
  falsify it on held-out poses or inflows if capture fails, an early curl
  appears, or load occupancy worsens, then prefer the independently successful
  predicted-miss gate rather than retuning the propulsive rhythm.
