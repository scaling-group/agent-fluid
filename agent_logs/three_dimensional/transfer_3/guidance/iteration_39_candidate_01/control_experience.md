...
- Instantaneous physical-response allocation has no replicated advantage in
  ...
  rejection.
- One-sided joint-rate anti-windup is a route-robust feasible-command cleanup,
  but its effort advantage is supported by only one detailed rollout. At the
  observed `260 deg/T` boundary, suppressing same-direction acceleration while
  ...
  `75.19--75.46%` to `73.91%`; zero return on about `6.4%` of posterior
  speed-boundary samples replaces infeasible outward work without hiding the
  persistent same-side phase witness. Two inherited completed anti-windup
  candidates also capture (scores `-0.14324` and `-0.13666`), so route survival
  replicates, but their compressed evidence cannot establish repeated effort
  or speed separation. Current force/moment RMS `0.01333/0.00694` and
  rate-limit residence still overlap baseline; do not claim lower
  hydrodynamic load or faster capture, lower the carrier gain, or add the
  failed closing-speed contraction, which slowed capture to `18.8705T` and
  raised mean distance to `2.04281L`. Here joint angle is not limiting
  (`|q2| <= 0.646 rad` versus `pi/4`), whereas outward posterior action remains
  active above `95%` of the speed limit for about `8.6--8.9%` of exact-route
  rows. A later constraint projection may therefore test normalized speed
  headroom, but must preserve full reverse braking and the raw-demand phase
  witness. Require capture within `18.6725--19.0520T`, mean distance at most
  `2.02129L`, loads below `0.01350/0.00703`, and separation in returned effort
  or speed-limit residence; otherwise keep hard-boundary anti-windup or restore
  plain feasible projection rather than tuning a threshold.
