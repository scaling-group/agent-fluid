...
- Heading alignment is not route alignment, but neither raw course nor recoil
  ...
  that failed actuator from response-reversing half-cycle steering. The
  assigned parent's high-demand, curvature-conserving allocator captures at
  `19.751T` and score `-0.21838`, nearly unchanged from its feasible-C-bend
  parent at `19.739T` and `-0.20624`; inherited logs therefore provide no
  reason for another instantaneous allocation gate. In contrast, scaling
  only the posterior traveling-wave component by joint-state beat side and
  phase-conditioned yaw error preserves the coherent wake, captures at
  `18.931T`, and improves score from the acceleration-feasible C-bend's
  `-0.17657` to `-0.15357`. Its action RMS (`25.22/28.86 rad/T^2`) and
  force/moment RMS (`0.01357/0.00707`) remain close to that baseline
  (`25.19/28.56`, `0.01322/0.00690`), so the arrival gain is not explained by
  gross effort. Preserve the projection as an observation transform and
  prefer bounded, state-inferred, error-reversing half-cycle modulation over
  another projected-course mean-curvature loop, a clocked imbalance, or an
  undamped course-to-half-cycle command. This implication applies to the
  completed LOS-rate C-bend in quiescent flow and currently has only one
  completed sample; falsify it if capture does not replicate, arrival exceeds
  the observed `19.234--19.739T` feasible-baseline band, loads materially
  increase, or stronger local-flow disturbance destroys the response
  advantage.
