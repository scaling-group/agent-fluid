...
- Response-triggered distributed C-bend steering is the first replicated
  semantic success in this lineage. The assigned parent bracketed its design:
  bearing-triggered anterior recruitment missed `1.897L` high, whereas a raw
  continuous two-joint residual missed `2.317L` low. Recruiting the bounded
  anterior redirect from combined bearing-plus-LOS route demand while
  retaining posterior mean curvature converts that bracket into four sampled
  captures at `19.2335--19.8880T` (scores `-0.17657-- -0.20397`) under direct
  uniform still-water initialization. Both top-down and oblique sheets retain
  a coherent self-generated alternating wake, and local-flow RMS remains only
  `0.01798--0.01879U`. Two explicit acceleration-feasible implementations are
  the fastest samples, but the episode already applies the same clamp and
  identical-policy spread is material, so do not attribute timing to the
  redundant clamp without paired replications. Preserve normalized LOS-rate
  feedback, continuous response-gated anterior recruitment, posterior route
  curvature, and feasible outputs; avoid gain-only authority increases,
  terminal coasting, or assigning the posterior joint to thrust alone.
  Inherited safe-intercept release and posterior-steering relief lost capture
  at `1.712L` and `3.191L`, respectively, showing that both joints must keep
  doing route work through capture. This applies to the coherent still-water
  carrier at local-flow scale near `0.02U`; falsify it if held-out geometry or
  inflow loses capture, breaks wake coherence, or materially worsens loads and
  limit occupancy, in which case test genuine beat-history response rather
  than another scalar or release threshold.
