...
- Instantaneous physical-response allocation has no replicated advantage in
  ...
  rejection.
- Offline carrier-recoil separation is not evidence of a useful causal yaw
  observer. A fitted anterior-angle plus two-joint-velocity quadrature reduced
  the completed-trace fast residual from about `0.40` to `0.103 rad/T`, yet
  its closed-loop rollout slowed capture to `22.3025T` and score `-0.44947`
  versus three exact actuator-consistent captures at `18.6725--18.7275T` and
  `-0.13219-- -0.13717`. It also raised trajectory-mean distance from about
  `7.56L` to `7.90L` and maximum absolute heading error from `0.66` to
  `0.97 rad`, while body-speed RMS fell from about `0.704--0.706U` to
  `0.613U`. The wake remained coherent in both views and the lower posterior
  limit occupancy and force/moment RMS (`57.2%`, `0.01118/0.00582`) therefore
  reflect a slower, wider route rather than improved steering authority.
  Preserve the evaluated velocity-only recoil correction when exploiting this
  low-flow carrier; avoid refitting instantaneous joint quadratures from a
  centered or acausal beat-average target. Test a future beat-scale observer
  only as a causal bounded state that retains the exact-policy arrival and
  route envelope in replicated CFD. Falsify this negative boundary if such an
  observer repeatedly captures no later than `18.7275T` without weakening the
  wake or exceeding `76.2%` posterior occupancy and
  `0.01350/0.00703` force/moment RMS; disturbance regimes beyond direct-
  uniform still water remain untested.
