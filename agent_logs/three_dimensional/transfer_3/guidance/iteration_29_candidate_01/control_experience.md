...
- Reduced acceleration clipping is not evidence of posterior velocity
  headroom. Across the current four direct-still-water captures, memoryless raw-
  moment relief and the actuator-consistent phase path retain exact posterior
  velocity-limit occupancy of about `7.29--7.51%` while capturing at
  `18.6560--18.7825T`. The assigned parent's later attempt to withdraw only
  incremental phase acceleration near the velocity envelope lowers posterior
  acceleration occupancy from the replicated demodulated parent's
  `74.17--74.44%` to `73.83%`, yet raises velocity occupancy from
  `7.73--7.83%` to `8.10%` and captures later at `18.8100T`. This falsifies
  incremental phase withdrawal as anti-windup: do not tune its activation or
  withdrawal scalars. If velocity protection is revisited, constrain the total
  persistent outward posterior command while leaving inward acceleration and
  the carrier-demodulated route allocator intact; reject that architecture too
  unless capture remains within `18.6725--19.0080T`, velocity occupancy falls
  below `7.5%`, posterior acceleration occupancy stays below `75.0%`, and
  force/moment RMS stays below `0.01320/0.00690`. This boundary applies only to
  the coherent self-generated wake near `0.018U` local-flow RMS and must be
  retested under stronger disturbance.
