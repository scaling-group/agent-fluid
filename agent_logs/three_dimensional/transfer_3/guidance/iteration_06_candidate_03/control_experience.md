...
- Heading alignment is not route alignment, but raw velocity course is also not
  a slow route signal under this carrier. Low-speed-gated course and slip
  candidates improved closest approach from the phase-conditioned reference's
  `5.658L` to `4.358L` and `4.158L`, yet both passed the target's x station and
  exited left at final distances `9.767L` and `9.037L`; their coherent wakes
  and small local flow rule out advection or lost propulsion. Across the four
  sampled traces, the fixed joint-phase projection
  `v_y_slow = v_y + 0.11*phi_dot1 - 0.035*phi_dot2` reduces body-lateral RMS
  from `0.269--0.385U` to `0.086--0.140U`, while preserving the slip
  candidate's late mean drift (`-0.0743U` raw versus `-0.0746U` projected).
  Therefore avoid another unfiltered course/slip controller: with the same
  coherent gait, first separate joint-correlated lateral recoil and steer on
  the residual. This applies only while carrier phase explains the fast
  component consistently; falsify it if projection does not suppress
  within-beat variation, erases persistent drift, or fails to improve the
  left-exit topology, in which case test a genuine history filter or a
  different steering actuator rather than retuning course gain.
