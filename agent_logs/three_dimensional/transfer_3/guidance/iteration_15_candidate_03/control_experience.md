...
- Response-conditioned anterior steering is a replicated capture mechanism,
  ...
  not speed-improving, mechanism. To recover the direct approach, test a smooth
  normalized-range restriction on allocation rather than weakening propulsion
  or opening steering. Falsify it if capture is lost, arrival exceeds `20.02T`,
  far-field wake coherence degrades, or effort does not fall; all load claims
  must be retested under materially stronger local-flow disturbance.
