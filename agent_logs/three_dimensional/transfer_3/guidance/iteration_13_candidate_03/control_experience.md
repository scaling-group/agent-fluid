...
- Distributed curvature is useful only when its recruitment semantics preserve
  route reversibility. Adding a bounded, large-bearing `6 deg` anterior-center
  shift to the coherent LOS-rate carrier improves minimum distance from
  `3.369L` to `1.897L` and lowers the head's target-station crossing from about
  `y=12.995L` to `11.465L` while retaining the long alternating top-down street
  and compact oblique Lambda2 structures. In contrast, distributing the full
  yaw residual across both oscillator centers reaches `2.317L` but drives on
  to a lower-domain exit with `11.311L` final range. The successful gate still
  releases from about `4.6 deg` at `12T` to `0.8 deg` at `14T` when
  instantaneous bearing briefly aligns, even though constant-velocity closest-
  pass distance remains about `0.54L`; by `16T` that estimate has regrown to
  `2.36L` and steering returns too late. Preserve the bounded large-error
  anterior branch, but test an observed collision-course or genuinely beat-
  scale persistence gate rather than spreading the residual continuously or
  raising curvature gains. This applies to coherent self-propulsion in small
  local flow where target-line overshoot dominates; falsify it if predictive
  persistence neither lowers the `11.465L` crossing nor beats `1.897L`, or if
  it recreates the sibling's lower exit, which would show that the apparent
  release gap is not the limiting cause.
- A single threshold capture is not sufficient evidence that an instantaneous
  route-demand gate is robust in this still-water carrier. One response-gated
  distributed C-bend captured at `19.585T`, but a later evaluation with the
  same executable control expressions (only comments and the inert version
  label changed) crossed the target station at `y=11.857L`, reached only
  `2.273L`, and exited left. A safe-intercept terminal-release child also
  missed high at `y=11.255L` with a `1.712L` minimum; both failures retained
  coherent propulsion while raw acceleration-envelope occupancy rose to about
  `61%/76%`, versus `40%/72%` in the captured response trace. Do not tune a
  terminal release around an unreplicated capture or treat a saturated
  bearing/LOS command as reliable lateral authority. Prefer an earlier,
  bounded body-frame collision-course cue that directly and reversibly recruits
  curvature, then require capture topology to survive repeat evaluation. This
  applies when divergence is already visible by `8--12T` in direct-uniform
  still water; falsify it if an independently repeated response-gated policy
  captures with comparable route and loads, or if direct predictive
  recruitment cannot improve the `1.712--2.273L` high-pass band without a
  lower curl or greater saturation.
