...
- Helpful yaw moment is compatible only as a residual allocator; additional
  ...
  allocators. The next distinct observation test is beat-recoil removal from
  the slow route itself: on the best rollout instantaneous body bearing swings
  from about `+0.99` to `-1.01 rad` along a visually smooth inertial path, while
  the established joint-state recoil projection substantially reduces selected
  route-bearing excursions. Falsify angle-level recoil conditioning if it
  loses capture, leaves the `18.656--19.008T` timing band, weakens the wake,
  exceeds `76.41%` posterior occupancy or `0.01350/0.00703` loads, or does not
  reduce route-command oscillation. This implication is limited to the sampled
  self-generated still-water wake and does not establish disturbance rejection.
