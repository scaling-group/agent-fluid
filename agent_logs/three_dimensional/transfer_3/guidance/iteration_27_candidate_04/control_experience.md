...
- Helpful yaw moment is compatible only as a residual allocator; additional
  instantaneous ownership logic does not resolve speed. Relieving half-cycle
  amplitude after yaw-response closure remains the best sampled finite result
  (`18.6560T`, score `-0.13321`, mean distance `2.02115L`) versus `18.6725T`
  for the actuator-consistent phase baseline. Requiring both phase recruitment
  and sign-coherent predicted stress delays capture to `18.7440T` without a
  material load reduction (`42.17%/75.67%` acceleration-limit occupancy and
  `0.01343/0.00699` force/moment RMS). Removing a fitted instantaneous
  joint-phase component from moment lowers occupancy to `41.73%/74.40%` and
  loads to `0.01309/0.00682`, but also delays capture to `18.7165T` and worsens
  mean distance to `2.02337L`. All four direct-uniform sheets retain coherent
  body-led top-down and oblique wakes at `0.01807--0.01815U` local-flow RMS.
  These timing differences remain inside the inherited identical-hash
  `18.6725--19.0080T` spread: preserve the carrier, LOS-rate path, persistent
  phase actuator, and bounded half-cycle residual, but avoid another raw-moment
  demodulator, stress multiplier, phase suppression, or scalar tuning of these
  allocators. The completed angle-level recoil test supplies a sharper negative
  result. Replacing raw target bearing with
  `bearing + 0.82*phi1 + 0.12*phi2` retains capture and lowers acceleration-limit
  occupancy to `38.04%/70.69%`, but slows arrival to `18.7825T`, worsens mean
  distance from the best sampled `2.02115L` to `2.05789L`, and raises
  force/moment RMS from `0.01328/0.00691` to `0.01391/0.00728`. Its top-down
  alternating street and oblique Lambda2 chain remain coherent at only
  `0.01781U` local-flow RMS, so the failure is route distortion rather than
  wake or thrust loss. Do not use the integral of a successful yaw-rate recoil
  projection as a replacement slow target angle, and do not scalar-tune those
  fixed coefficients to rescue it. Preserve raw normalized target geometry for
  the slow route; if joint-angle phase is revisited, restrict it to confirming
  a transient redirect while an independent LOS-response path remains active.
  Falsify that narrower use if it loses capture, exceeds `2.02337L` mean
  distance or `0.01350/0.00703` loads, or leaves redirect recruitment unchanged.
  This implication is limited to the sampled self-generated still-water wake
  and does not establish disturbance rejection.
