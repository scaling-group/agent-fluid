...
- Posterior phase recruitment is sensitive to response ownership, not merely
  exposure or actuator headroom. Persistent same-side raw demand and previous
  feasible action captures at `18.6725T` and `18.7385T` in the current
  identical-hash samples, while an inherited repeat takes `19.0080T`; this
  same-controller spread is larger than the current allocator timing gaps and
  prevents a scalar-only speed claim. Helpful yaw moment is nevertheless a
  compatible residual cue: relieving only half-cycle amplitude after
  yaw-response closure gives the best current finite result (`18.6560T`, score
  `-0.13321`, mean distance `2.02115L`) with `41.19%/75.97%` acceleration-limit
  occupancy and `0.01328/0.00691` force/moment RMS. Its `0.0165T` lead over the
  fastest same-hash phase sample is unresolved, so the positive lesson is
  allocator compatibility, not improved route speed. In contrast, using
  helpful moment to suppress the primary phase actuator remains a negative
  control even when conditioned on yaw-error closure: capture slows to
  `18.7715T`, with `41.25%/75.53%` occupancy and `0.01335/0.00695` loads.
  Earlier command-deficit multiplication, global amplitude-to-phase handoff,
  and unconditional moment release likewise arrive at `18.7495--18.9805T`.
  All current sheets retain coherent body-led top-down and oblique wakes at
  only `0.01806--0.01816U` local-flow RMS, so these differences concern
  self-generated route/effort allocation, not advection or wake survival.
  Preserve the slow normalized LOS route and persistent phase actuator; avoid
  another gain change, command-deficit multiplier, complementary handoff, or
  instantaneous-moment phase release. If physical response is reused, confine
  it to a bounded residual such as half-cycle asymmetry and require observable
  primary-channel recruitment plus sign-coherent posterior stress before
  declaring that residual redundant.
  Falsify this implication if repeated results lose capture, arrive later than
  about `18.931T`, weaken the alternating wake, exceed `76.41%` posterior
  occupancy or `0.01350/0.00703` force/moment RMS, or cannot separate the
  allocator from same-hash variation. Do not extrapolate this low-flow
  still-water result to external-wake rejection without disturbance evidence.
