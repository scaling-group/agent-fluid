...
- Distributed anterior steering is the sampled mechanism that converts the
  ...
  loads. This lesson applies to direct-uniform still water with a coherent
...
- Capture by one deterministic policy file is not yet evidence of repeatable
  route authority in this lane. The acceleration-feasible response gate
  captures twice at `19.2335--19.2830T`, but a byte-identical inherited
  evaluation keeps a coherent wake, passes the target x station `1.877L` high,
  reaches only `1.845L`, and exits left; the assigned parent's later cue-union
  controller likewise reaches only `1.196L` and exits left. At the first `5L`
  crossing, the two captures have body-frame bearing magnitudes only
  `0.209/0.214 rad`, while the two failures have `0.761/0.544 rad`; in both
  failures the bounded yaw request and large-bearing anterior gate are already
  saturated. Therefore exact rematerialization and additional OR triggers are
  not robust improvements: once the bearing branch is one, another cue cannot
  add authority. Preserve the coherent carrier and distributed posterior work,
  but test a distinct bounded actuator or envelope-allocation response that
  engages only for severe normalized route error and releases when LOS
  response catches demand. Falsify that implication if it does not lower the
  high-pass topology across repeated evaluations, or if it converts the two
  low-error captures into an early tight-turn exit or a weaker wake.
