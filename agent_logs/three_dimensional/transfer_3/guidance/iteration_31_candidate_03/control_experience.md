...
- Carrier-demodulated hydrodynamic response lowers posterior acceleration and
  load only when its phase-allocation semantics remain bidirectional; neither
  response half can yet be credited alone. Three same-hash bidirectional phase
  runs capture at `18.7165--18.8650T` with coherent body-led wakes, posterior
  acceleration occupancy `74.14--74.64%`, and force/moment RMS
  `0.013093--0.013252` / `0.006816--0.006894`. Those effort bands stay below
  the sampled actuator-consistent `76.11%` and `0.01350/0.00703`, while route
  timing remains inside same-hash variability: actuator-consistent repeats
  span `18.6725--18.9970T`, and raw helpful-moment amplitude relief itself
  spans `18.6560--18.9915T`. The previously recommended opposition-only phase
  allocator has now been replicated at `18.8045T` and `18.8980T`; its benefit
  does not replicate, spanning `74.19--75.93%` posterior occupancy and
  `0.013066--0.013519` / `0.006804--0.007041` loads. Moving the other response
  half to amplitude is a concrete negative result: demodulated helpful-
  amplitude relief captures only at `19.1510T`, lengthens the center path to
  `12.5915L`, and scores `-0.16481`, despite low `74.07%` occupancy and
  `0.013060/0.006819` loads. Preserve the LOS C-bend, carrier, persistent phase
  path, and carrier subtraction; avoid further gain tuning of opposition-only
  recruitment or helpful-amplitude relief. The missing controlled ablation is
  helpful-only *phase* relief with opposing residual neutral. Falsify that
  attribution if it loses capture, arrives later than `19.1510T`, exceeds the
  `12.5915L` path or `0.01350/0.00703` load bounds, or fails to keep posterior
  occupancy below `76.11%`; after that, stop splitting instantaneous moment and
  expose genuinely beat-scale response history, since the current seven-step
  turn window is only about `0.0385T`. These bounds apply only to direct-uniform
  still water at local-flow RMS about `0.0180--0.0184U` and do not establish
