...
- Response-triggered distributed C-bend steering establishes capture but is
  not by itself robust. The assigned parent bracketed it between a `1.897L`
  high miss from bearing-only anterior recruitment and a `2.317L` low miss
  from sharing the raw residual continuously. Combined bearing-plus-LOS
  anterior recruitment with posterior mean curvature then produced three
  unmodulated captures at `19.2335--19.8880T`, yet the exact acceleration-
  feasible hash that captured at `19.2335T` also made a coherent-wake `1.845L`
  high pass and exited left at `28.875T`. That failure used more applied-limit
  occupancy (`60.8%/76.4%` anterior/posterior) than its sampled capture
  (`44.9%/73.6%`) and higher force/moment RMS, so more clipped mean bend is not
  a robustness mechanism. Preserve normalized LOS-rate feedback, continuous
  two-joint route work, and feasible outputs; inherited safe-intercept release
  and posterior relief also lost capture at `1.712L` and `3.191L`.
  Response-reversing posterior half-cycle modulation is the first mechanism
  to recover from this counterexample: its sampled rollout captures at
  `18.9310T`, improves mean distance to `2.04175L` from the unmodulated
  captures' `2.06505--2.09373L`, and retains both wake coherence and lower
  anterior limit occupancy. Treat this as a replication target, not a robust
  result, because it has one completed sample. This applies to the coherent
  still-water carrier at local-flow scale near `0.02U`; falsify it if another
  run loses capture, arrives later than the conservative `20.020T` distributed-
  bend reference, repeats the greater-than-`1.845L` high pass, or worsens wake,
  loads, or posterior saturation. If falsified, test genuine beat-history or
  bounded phase-lag feedback rather than scalar gains or a range gate.
