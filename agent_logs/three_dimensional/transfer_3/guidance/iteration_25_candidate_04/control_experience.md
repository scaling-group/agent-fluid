...
- Normalized yaw moment is an allocation signal, not permission to suppress
  posterior phase steering. The current physical-response ablation preserves
  coherent body-led top-down and oblique wakes and capture in every branch at
  only `0.01806--0.01816U` local-flow RMS, but releasing phase whenever moment
  helps arrives at `18.7880T`, and restricting that release to near-closed yaw
  error still arrives at `18.7715T` (`-0.13713`). Both are slower than exact-
  hash actuator-consistent phase repeats at `18.6725T` and `18.7385T` and the
  inherited complementary-handoff result at `18.7495T`; the latter phase-
  release branch's `40.96%/75.39%` limit occupancy and
  `0.01335/0.00695` force/moment RMS show that lower effort alone does not
  recover route timing. Relieving only half-cycle amplitude while retaining
  phase captures at `18.6560T` with `40.74%/75.74%` occupancy and
  `0.01328/0.00691` loads, but its `0.0165T` edge over the faster baseline is
  smaller than the baselines' `0.0660T` same-hash spread, so it is evidence of
  safe actuator-role separation rather than resolved speedup. Preserve the
  persistent same-side phase gate; avoid further moment-gated phase release,
  command-deficit multipliers, or wholesale amplitude-to-phase handoff. If
  moment conditioning is revisited, restrict it to the smallest residual
  amplitude contribution and distinguish strengthened from attenuated beat
  sides; falsify that implication if repeated capture exceeds the replicated
  half-cycle bound near `18.931T`, weakens the wake, or exceeds the
  `0.01350/0.00703` baseline load. This boundary is evidenced only for the
  self-generated low-flow still-water carrier and needs disturbance retesting.
