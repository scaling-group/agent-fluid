...
- Posterior phase recruitment benefits from actuator-side persistence, but it
  is a route-speed mechanism rather than load relief. A direct-uniform four-
  way cohort preserves coherent top-down and oblique wakes and captures in
  every case, yet the signed product of current normalized posterior demand
  and previous feasible action is fastest at `18.6725T` (score `-0.13362`,
  mean distance `2.02129L`). It improves on the assigned-parent rising-flank
  secant lead (`18.7880T`, `2.02833L`), current-demand recruitment
  (`18.7990T`, `2.03111L`), and instantaneous headroom allocation
  (`18.8320T`, `2.03494L`). This revises the inherited assumption that any
  previous-action confirmation is necessarily harmful: sign coherence, not
  delayed magnitude alone, separates sustained actuator stress from a beat
  reversal. The faster route raises posterior occupancy to `76.11%` and
  force/moment RMS to `0.01350/0.00703`, so do not claim clipping relief or
  add demand prediction/headroom allocation to it. Preserve the normalized,
  reflection-equivariant same-side gate when speed is the objective, and
  falsify it if a same-hash repeat loses capture, leaves the inherited
  `18.931--19.052T` half-cycle band, breaks wake coherence, or exceeds the
  failed-branch `76.4%` posterior-occupancy and `0.01574/0.00810` load bounds;
  applicability above the observed `0.01809U` local-flow RMS remains untested.
