...
- Instantaneous physical-response allocation has no replicated advantage in
  ...
  rejection.
- The replicated route exposes one actuator-envelope mismatch that is distinct
  from the rejected fluid-response allocators. Across the newest four traces,
  the anterior/posterior joints occupy the `260 deg/T` speed limit for about
  `2.8--3.1%/7.0--7.4%` of samples, almost always while the returned
  acceleration still points in the direction of joint motion; opposite-sign
  braking at the boundary is only `0--0.03%`. Posterior rate-limit samples
  also coincide with acceleration clipping for `2.3--2.4%` of each rollout.
  Since further outward acceleration cannot change the feasible joint speed,
  test a one-sided state-feedback anti-windup projection that suppresses only
  that redundant direction while preserving reverse braking, the normalized
  LOS route, and the posterior-lagged carrier. If previous feasible action is
  a phase-recruitment input, use continued same-side raw demand at the exact
  speed boundary as its causal persistence witness until braking begins; do
  not let the zeroed return silently disable the established phase path. Do
  not lower carrier gain or globally rescale both joints. This implication
  applies only to a hard speed limiter with the observed state reported at its
  exact boundary; falsify it if outward action does not fall, braking or wake
  phase changes, capture leaves `18.6725--19.0520T`, mean distance exceeds
