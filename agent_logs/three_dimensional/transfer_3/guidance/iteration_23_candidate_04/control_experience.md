...
- Posterior phase recruitment benefits from actuator-side persistence, but
  sub-beat arrival rankings are not robust evidence of a route-speed
  mechanism. The signed product of current normalized posterior demand and
  previous feasible action captures once at `18.6725T` (score `-0.13362`,
  mean distance `2.02129L`) and again with the exact same policy at about
  `19.008T` (score `-0.15441`). That `0.3355T` same-hash spread is larger than
  the current architecture gaps: complementary amplitude release captures at
  `18.7495T`, hydrodynamic-moment phase release at `18.7880T`, and rising-
  flank demand lead at `18.7880T`; all retain coherent top-down and oblique
  wakes. The response-aware alternatives establish a useful load trade even
  without a resolved timing win. Relative to the fast actuator-consistent
  sample's `24.95/28.85 rad/T^2` action RMS, `76.41%` posterior occupancy, and
  `0.01350/0.00703` force/moment RMS, hydrodynamic release gives
  `24.41/28.56`, `74.41%`, and `0.01306/0.00679` at the same `0.01809U` local-
  flow RMS; complementary release gives `24.80/28.70`, `74.48%`, and
  `0.01336/0.00696`. Preserve normalized LOS-rate guidance, the coherent
  carrier, response-reversing half-cycle authority, and same-side actuator
  persistence. Do not declare a speed improvement or tune another timing
  scalar from differences below the observed repeat spread. When load relief
  is the objective, replicate the normalized moment-gated phase release and
  falsify it if capture or wake coherence is lost, arrival exceeds `19.052T`
  without material relief, posterior occupancy exceeds `76.4%`, or
  force/moment RMS exceeds `0.01574/0.00810`; applicability above the observed
  still-water local-flow level remains untested.
