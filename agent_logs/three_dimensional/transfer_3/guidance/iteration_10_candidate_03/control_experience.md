...
- A rotation-invariant line-of-sight-rate response is the first sampled
  ...
  sacrificing the coherent carrier. Relative to raw-slip pursuit, it improves
  closest approach from `4.158L` to `3.369L`, lowers target-line crossing from
  `y=13.735L` to `12.893L`, and retains a long alternating wake until `30.12T`;
  local-flow RMS is only `0.020U`. The completed normalized-range damping test
  now separates speed from route authority: closest-approach speed falls from
  `0.878U` to `0.820U`, anterior raw over-limit occupancy from `57.7%` to
  `34.2%`, force RMS from `0.01488` to `0.01258`, and moment RMS from `0.00770`
  to `0.00659`, yet minimum range slightly worsens to `3.392L`, crossing rises
  to `y=12.985L`, and the same left exit remains. Preserve LOS-rate response
  as the broad-approach baseline, but avoid further scalar range damping or
  far-field drive increases for this coherent low-flow high-pass topology;
  lower speed and effort alone do not repair it. Test a bounded steering-
  actuator change such as distributing reversible curvature across evidenced
  joint polarities or wave-shape modulation. Falsify the actuator-authority
  implication if such a change retains the more-than-`3L` high pass without
  weakening the wake, in which case genuinely beat-scale route history rather
  than drive or allocation is the next missing capability.
