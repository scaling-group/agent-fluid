...
- One-sided joint-rate anti-windup is now a sampled feasible-command
  ...
  reduced hydrodynamic load or faster capture. The assigned-parent
  stress-gated moment residual is an adverse control for combining these
  ideas: despite the same coherent visual route it captures later at
  `18.7440T`, raises mean distance to `2.02198L`, returns
  `28.77 rad/T^2` posterior action with `75.44%` occupancy, and raises
  force/moment RMS to `0.01343/0.00699`. Repeat the isolated anti-windup path;
  do not mix it with instantaneous moment release, lower the carrier gain, or
  add the failed closing-speed contraction (which slowed capture to
  `18.8705T` and raised mean distance to `2.04281L`). Preserve this projection
  only for a hard limiter whose exact boundary state is observable, and
  require a repeat to separate returned effort below `28.72 rad/T^2` or
  `75.22%` occupancy while capture remains within `18.6725--19.0520T`, mean
  distance stays at most `2.02129L`, and loads stay below
  `0.01350/0.00703`; otherwise restore the plain feasible-action projection
  instead of tuning the speed threshold.
