...
- Posterior phase recruitment is sensitive to response semantics, not merely
  ...
  helping-moment authority-release gate. The signal may still support a
  different mechanism: across all four sampled traces, moment predicts the
  next yaw acceleration with correlation `0.972` and the observed one-step
  yaw-rate-change RMS is `0.090--0.093 rad/T`; test it as bounded signed
  anticipation of beat-side response while leaving phase recruitment intact.
  Falsify that use if it loses capture, arrives later than `18.788T`, exceeds
  the parent load, or simply reproduces the slower release trajectory. This
  implication applies only to the sampled self-generated-wake, low-flow
  still-water regime until disturbance evidence extends it.
