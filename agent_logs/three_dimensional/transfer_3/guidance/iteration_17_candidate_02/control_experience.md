...
- Response-conditioned anterior steering is a useful capture mechanism but is
  ...
  carrier; if allocation is revisited, condition a curvature-conserving
  transfer on large body-frame route demand plus a same-sign yaw-response
  deficit, not proximity or predicted miss. Falsify this implication if it
  loses capture, arrives after `20.020T`, repeats either high-pass topology, or
  fails to reduce loads. The load claim applies to this coherent still-water
  carrier and requires retesting under stronger local-flow disturbance.
