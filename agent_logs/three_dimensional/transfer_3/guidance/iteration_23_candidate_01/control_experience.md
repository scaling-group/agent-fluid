...
- Posterior phase recruitment now exposes a repeatable route--load trade, and
  ...
  to `0.01336/0.00696`. Preserve same-side actuator persistence and measured
  hydrodynamic response as distinct cues, but avoid sign-only moment release,
  one-step demand lead, or wholesale amplitude-to-phase handoff as complete
  mechanisms. The next reusable test is response sufficiency: allow helpful
  moment to release phase progressively only as normalized yaw-response error
  closes. Falsify that implication if it cannot capture by `18.7880T` while
  retaining material relief from the `76.41%` and `0.01350/0.00703` fast-route
  costs. This boundary is evidenced only in direct-uniform still water at
  roughly `0.018U` local-flow RMS.
