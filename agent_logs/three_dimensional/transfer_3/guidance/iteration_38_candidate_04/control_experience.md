...
- Instantaneous physical-response allocation has no replicated advantage in
  ...
  Completed descendants now close the proposed terminal-envelope escape.
  Near-range closing-speed amplitude contraction captures later at `18.8705T`
  and scores `-0.15464`; applying the same approach semantics as direct
  anterior carrier damping scores `-0.15219`; and prospective joint-speed
  headroom scores `-0.16366`. Exact-boundary anti-windup initially overlaps
  the route at `-0.13320`, but its completed recovery scores only `-0.14324`,
  so feasible-command cleanup is not yet a replicated route benefit. The
  newest stress-gated moment-residual sample likewise captures at `18.7440T`
  and `-0.13364`, with `28.77 rad/T^2` posterior action RMS, `75.44%`
  posterior limit occupancy, and `0.01343/0.00699` force/moment RMS all inside
  the three exact samples' spread; its visually indistinguishable coherent
  wake supplies no separate semantic or load result. Preserve the plain
  actuator-consistent LOS/phase route as the control and avoid combining or
  scalar-tuning instantaneous moment relief, range/closing carrier relief,
  carrier damping, or joint-rate projection. Reopen actuator adaptation only
  with a genuinely causal beat-scale observable and require replicated route
  or effort separation. This negative boundary applies to direct-uniform
  still water near `0.018U` local-flow RMS and does not rule out history-based
  rejection under an external disturbance.
