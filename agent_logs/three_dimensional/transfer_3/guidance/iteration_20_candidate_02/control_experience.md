...
- Response-triggered distributed C-bend steering is a replicated semantic
  ...
  The failed-hash load and occupancy remain the bounds for subsequent phase or
  history mechanisms; this comparison applies to the coherent still-water
  carrier near `0.02U` local flow and must be retested under stronger
  disturbance.
- Posterior phase modulation is useful only when recruited by instantaneous
  actuator headroom in the sampled response-reversing carrier. All four
  assigned variants preserve coherent top-down vortex streets and oblique
  Lambda2 structures through capture at `18.799--18.997T`, but smooth
  recruitment from normalized predicted posterior acceleration is best
  (`18.799T`, score `-0.14330`, mean distance `2.0311L`). It outperforms the
  unmodulated half-cycle (`18.931T`, `-0.15357`) while lowering acceleration-
  limit occupancy from `44.77%/76.09%` to `41.54%/73.96%` and force/moment RMS
  from `0.01357/0.00707` to `0.01315/0.00684`. Always-on phase rotation is
  slowest (`18.997T`, `-0.15967`), and a hard gate from the previous applied
  action is intermediate (`18.892T`, `-0.14930`). Preserve coefficient-norm-
  conserving phase rotation behind a smooth predicted-demand gate; avoid
  always-on rotation or treating a single clipped previous action as beat
  history. Falsify this lesson if replication loses capture or exceeds the
  unmodulated occupancy/load bounds, because the inherited exact-hash
  capture/miss split shows that one finite run does not establish robustness.
