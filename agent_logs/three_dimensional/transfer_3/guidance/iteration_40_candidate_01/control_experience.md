...
- Instantaneous physical-response allocation has no replicated advantage in
  ...
  rejection.
- Joint-rate anti-windup is a completed feasibility success but not a route or
  hydrodynamic-load improvement. One-sided suppression of outward acceleration
  at the `260 deg/T` boundary retains the coherent top-down and oblique wakes
  and captures at `18.7000T`, inside the two-instance exact-policy plain-route
  band of `18.6725--18.7330T`; its score `-0.13320`, mean distance `2.02103L`,
  and force/moment RMS `0.01333/0.00694` do not separate from plain-route
  variation. It does lower posterior action RMS/acceleration occupancy to
  `28.24 rad/T^2 / 74.0%` from `28.72--28.77 rad/T^2 / 75.2--75.5%` and cuts
  sampled outward-at-rate-limit rows to `32` from `239--249`. Preserve reverse
  braking and the raw-demand persistence witness if feasibility is required,
  but do not repeat or scalar-tune the exact-boundary gate. Nor should a
  feasibility layer be combined with another instantaneous hydrodynamic
  allocator: the sampled stress-gated moment residual has no visible wake or
  route-topology advantage, captures later at `18.7440T`, raises mean distance
  to `2.02198L`, and raises posterior action/load to `28.77 rad/T^2` and
  `0.01343/0.00699`. The remaining narrow test is prospective: cap only
  same-direction acceleration by normalized joint-speed headroom over one
  maximum solver update, leaving the LOS route, carrier, phase recruitment,
  and braking unchanged. Do not combine it with near-range carrier contraction:
  that inherited control keeps a coherent wake but delays capture to
  `18.8705T`, worsens mean distance to `2.04281L`, and shifts effort anteriorly.
  Falsify prospective projection if it fails to improve command feasibility
  beyond exact-boundary anti-windup, changes wake phase or braking, moves effort
  upstream, leaves the `18.6725--19.0520T` capture band, exceeds `2.02129L`
  mean distance or `0.01350/0.00703` force/moment RMS; then stop elaborating
  speed-feasibility layers and retain plain acceleration projection for route
  performance or exact-boundary anti-windup only for command cleanup.
