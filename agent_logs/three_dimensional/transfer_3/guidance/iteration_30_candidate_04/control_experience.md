...
- Hydrodynamic-moment allocation is a closed negative branch for this low-flow
  still-water carrier, not an unresolved gain search. In the current four-way
  sample, the no-moment actuator-consistent phase policy captures at
  `18.6725T` with mean distance `2.02129L`, `42.2%/76.1%` anterior/posterior
  limit occupancy, and `0.01350/0.00703` force/moment RMS. Helpful-moment
  amplitude relief is nominally faster at `18.6560T`, but its `0.0165T` edge
  is smaller than inherited same-hash timing spread; stress-gated and opposing
  residuals instead slow to `18.7440T` and `18.8045T`. The opposing residual
  also raises mean distance to `2.02499L`, loads to `0.01352/0.00704`, and
  anterior occupancy to `42.6%`, while inherited completed branches extend the
  score regression to `-0.14654` and `-0.15658`. Earlier demodulated phase,
  amplitude, and one-sided variants likewise failed to move both route quality
  and effort beyond replication spread. All current views retain coherent
  alternating top-down and oblique wakes near `0.018U` local-flow RMS, so the
  differences are allocator-induced route/effort tradeoffs rather than
  advection, instability, or wake collapse. Preserve normalized LOS-rate
  guidance, the traveling carrier, and posterior phase recruitment; avoid
  another raw, gated, demodulated, one-sided, amplitude, or scalar-tuned moment
  allocator. The clean next test is a non-moment geometry-response release:
  recruit the anterior C-bend from target geometry, but unload it when observed
  recoil-conditioned yaw meets demand while the posterior wave stays active.
  Falsify that transfer if it loses capture or wake coherence, leaves the
  `18.66--18.80T` sampled arrival band, or fails to reduce anterior occupancy
  and loads. Reopen moment allocation only if repeated evaluations improve
  route timing/mean distance outside the no-moment replication band and lower
  load/occupancy; this rejection does not cover stronger external disturbance.
