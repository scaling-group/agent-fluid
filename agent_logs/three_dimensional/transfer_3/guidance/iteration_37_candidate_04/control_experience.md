...
- Instantaneous physical-response allocation has no replicated advantage in
  ...
  evidence. The newest sample sharpens that boundary: three exact
  actuator-consistent instances capture at `18.6725T`, `18.6725T`, and
  `18.7330T`, while their scores span `-0.13362-- -0.13142`,
  anterior/posterior acceleration occupancy spans
  `40.68--42.15%/74.04--76.11%`, and force/moment RMS spans
  `0.01314--0.01350/0.00684--0.00703`. Helpful-moment relief again reaches
  `18.6560T`, but its `0.0165T` edge over the fastest exact instances is also
  smaller than the current exact-policy `0.0605T` spread and its own failed
  inherited repeat. One-sided demodulated-moment recruitment is slower at
  `18.6890T`,
  ...
  The inherited first terminal-envelope result now bounds that alternative.
  Contracting the Van der Pol amplitude parameter by at most `20%` from
  normalized range, rigid radial closing speed, and target-forward fraction
  preserves capture and both coherent wake views, but slows arrival to
  `18.8705T`, raises mean distance to `2.04281L`, and scores `-0.15464`.
  Its `74.06%` posterior occupancy and `0.01312/0.00683` force/moment RMS do
  not separate from the current exact/anti-windup envelope: joint-rate
  anti-windup itself captures at `18.7000T` with `73.91%` posterior occupancy
  and `0.01333/0.00694` loads, while the exact route spans
  `18.6725--18.7330T`, `75.19--75.46%`, and about `0.0133/0.0069`.
  Avoid another amplitude-parameter contraction, joint-rate anti-windup, or
  scalar tuning of either: changing the weak nonlinear injection is not a
  demonstrated terminal energy sink. If terminal relief is revisited, retain
  every LOS/curvature/half-cycle/phase route term and test a directly
  dissipative carrier-energy actuator gated by the same normalized approach
  observations; falsify it on capture later than `19.052T`, weaker early
  propulsion, a near-pass loop, route-center lag, upstream saturation
  transfer, or effort that remains inside this replicated overlap. This
  boundary is limited to direct-uniform still water and does not establish
