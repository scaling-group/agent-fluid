...
- Hard boundary anti-windup is reproducible effort cleanup, but is not a
  route-neutral fix for posterior rate saturation. The same one-sided policy,
  which zeros only acceleration directed farther into the `260 deg/T` speed
  boundary, captures once at `18.7000T` and once at `19.0905T`; the latter
  lies beyond the inherited exact-route band and raises mean distance to
  `2.05157L` with score `-0.16317`. It does consistently lower posterior
  action RMS to `28.15--28.24 rad/T^2` and acceleration-limit occupancy to
  `73.00--73.91%`, compared with `28.72--28.77 rad/T^2` and
  `75.19--75.46%` for two current exact-route samples. Both wake views remain
  coherent and body-led near `0.0181--0.0184U` local-flow RMS, so the repeat
  exposes a route/actuator tradeoff rather than wake collapse. Crucially,
  posterior speed-limit residence does not fall (`7.32--7.89%` versus
  `7.02--7.36%`), while inherited closing-speed carrier contraction likewise
  buys lower effort by slowing capture to `18.8705T`. Do not treat post-limit
  zeroing or carrier withdrawal as solved saturation, combine them, or tune
  their thresholds for score. If the speed envelope is revisited, preserve
  the LOS route and traveling-bend norm while testing a bounded pre-boundary
  phase reversal; falsify it unless replicated capture stays no later than
  `19.0520T`, rate residence falls below `7.02%`, and effort/load improve
  without slower progress or upstream saturation. This boundary is limited
  to the current direct-uniform, low-flow still-water carrier.
