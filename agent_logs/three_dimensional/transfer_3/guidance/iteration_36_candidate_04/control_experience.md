...
- Instantaneous physical-response allocation has no replicated advantage in
  ...
  rejection.
- Exact-boundary joint-rate anti-windup is a useful feasibility projection,
  but not a route or load improvement. Against two actuator-consistent repeats
  capturing at `18.6725--18.7330T`, it preserves capture at `18.7000T`, keeps
  mean distance at `2.02103L`, and retains the same shallow approach and
  coherent body-led wake in both visual views at `0.01809U` local-flow RMS.
  Its causal actuator effect is well separated from repeat scatter: posterior
  outward-at-rate-limit action falls from `7.02--7.33%` to `0.94%`, action RMS
  from `28.72--28.77` to `28.24 rad/T^2`, and acceleration-limit occupancy
  from `75.19--75.46%` to `73.91%`. Force/moment RMS remains `0.01333/0.00694`,
  inside the baseline `0.01331--0.01335/0.00693--0.00695` spread, while
  posterior rate-limit occupancy remains `7.35%`; suppressing an infeasible
  command only after the state reaches the hard boundary therefore cannot
  prevent boundary residence. Preserve the one-sided projection and immediate
  reverse braking as an actuator-contract layer, but do not claim it as
  hydrodynamic improvement or scalar-tune it for arrival. If rate residence is
  the next target, test a smooth normalized-speed headroom envelope while
  leaving the LOS route and traveling bend intact; falsify it if posterior
  rate-limit occupancy does not fall below `7.0%`, capture leaves the inherited
  `18.6725--19.0520T` band, mean distance exceeds `2.02129L`, the alternating
  wake weakens, or force/moment RMS exceeds `0.01350/0.00703`. This implication
  applies to a reported hard joint-speed boundary and does not establish an
  efficiency advantage or external-disturbance robustness.
