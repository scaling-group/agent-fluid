...
- Posterior phase recruitment now exposes a repeatable route--load trade, and
  a helpful physical-response sign is not by itself a sufficiency test. The
  persistent same-side actuator gate is the fastest sampled capture at
  `18.6725T` and mean distance `2.0213L`, but uses `76.41%` posterior
  acceleration-limit occupancy with `0.01350/0.00703` force/moment RMS.
  Releasing phase whenever hydrodynamic moment helps preserves the coherent
  wake and capture while lowering occupancy to `74.41%` and loads to
  `0.01306/0.00679`, but slows arrival to `18.7880T`; demand lead reaches the
  same time with higher occupancy and loads, while complementary amplitude
  handoff reaches `18.7495T` but raises mean distance to `2.0299L` and loads
  to `0.01336/0.00696`. Multiplying helpful-moment release by normalized-error
  closure produces only another point on this trade: it captures at
  `18.7715T` with mean distance `2.0254L`, `75.53%` posterior occupancy, and
  `0.01335/0.00695` loads, dominating neither endpoint. Preserve same-side
  actuator persistence and measured hydrodynamic response as distinct cues,
  but avoid sign-only release, one-step demand lead, wholesale amplitude
  handoff, or low yaw error alone as a claimed response-sufficiency mechanism.
  A sharper reusable test is to release phase only when normalized physical
  assistance exceeds residual normalized response demand. Falsify that class
  unless it captures by `18.7715T` while staying at or below `75.53%`
  posterior occupancy and `0.01335/0.00695` force/moment RMS; reject it
  outright above the inherited `76.41%` occupancy or `0.01574/0.00810` load
  boundaries. These implications are evidenced only in direct-uniform still
  water at roughly `0.018U` local-flow RMS and need disturbance retesting.
