...
- Hydrodynamic yaw moment is more useful as a residual amplitude-allocation
  cue than as an owner of posterior phase recruitment. With the normalized LOS
  route and actuator-consistent phase path preserved, relieving only satisfied
  half-cycle asymmetry captures at `18.65600T` (score `-0.133208`) with
  `41.19%/75.97%` anterior/posterior limit occupancy and
  `0.013277/0.006912` force/moment RMS. The unrelieved policy's identical-hash
  samples capture at `18.67250T` and `18.73849T`; error-conditioned moment
  release of the phase actuator is slower at `18.77150T` and does not separate
  from the low-load baseline repeat. All four retain coherent alternating
  top-down and oblique wakes at only `0.01806--0.01816U` local-flow RMS. Keep
  slow body-frame target geometry and actuator-consistent phase feedback in
  charge of route closure; if measured moment is used, relieve only a small
  residual rhythmic channel after yaw response is already helping. The
  amplitude result's `0.01650T` lead is below the baseline's `0.06600T`
  same-policy spread, so require replication before calling it a timing gain.
  Falsify the allocation if it loses capture, arrives beyond the replicated
  half-cycle bound near `19.052T`, or exceeds `76.41%` posterior occupancy or
  `0.01574/0.00810` force/moment RMS; disturbance use remains untested beyond
  quiescent local flow near `0.018U`.
