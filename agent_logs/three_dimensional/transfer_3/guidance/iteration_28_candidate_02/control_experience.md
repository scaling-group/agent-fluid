...
- Treat sub-`0.34T` timing differences among posterior-phase selectors as
  ...
  `0.01355/0.00705`, and leaves posterior occupancy at `75.87%`. Preserve the
  normalized LOS, response-reversing half-cycle, and persistent same-side
  phase gate; avoid another memoryless raw-moment release, forecast, or scalar
  tuning of either. If fluid response is revisited, first subtract an odd
  joint-phase carrier estimate: across the current samples this leaves about
  `0.0024` moment-residual RMS and only `0.234--0.237` correlation with next-
  sample yaw acceleration, so it warrants at most a small bounded allocator,
  falsified by loss of the `18.67--19.01T` capture band, posterior occupancy
  above `76.1%`, or force/moment RMS above `0.01350/0.00703`. Genuine beat-scale
  history remains unavailable from the seven-sample (`about 0.0385T`)
  observation window. These bounds apply only to the sampled self-generated
  still-water wake and require retesting under stronger ambient disturbance.
- Actuator headroom must be enforced on the command that actually consumes
  it, not inferred from a desirable intermediate allocator. Carrier-
  demodulated moment residuals reproducibly reduce posterior action/load while
  preserving the coherent route wake (sampled `18.7165T`, `74.41%` posterior
  acceleration occupancy, `0.01309/0.00682` force/moment RMS), but raise time
  above `95%` posterior velocity from the helpful-moment baseline's `12.32%`
  to `12.70%`. The assigned parent's directional gate then withdrew only the
  incremental phase request and captured later at `18.8100T`, with mean
  distance `2.03220L`, high-velocity occupancy `13.04%`, and outward command
  during high velocity `9.68%`; both wake views remained coherent. Thus the
  negative result is an actuator-state mismatch, not wake or route failure:
  avoid another phase-target or residual-moment headroom gate. If velocity
  protection is revisited, apply a reflection-equivariant barrier to the final
  outward posterior acceleration while leaving braking and the established
  LOS/phase route intact; falsify it unless capture stays within the replicated
  `18.656--19.008T` band and high-velocity/outward-command occupancy falls
  below `12.0%/8.5%` without exceeding `0.01328/0.00692` load RMS. These
  thresholds describe the sampled still-water carrier and require new bounds
  under stronger ambient disturbance.
