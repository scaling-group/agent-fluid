...
- Instantaneous physical-response allocation has no replicated advantage in
  this low-flow lane. One-sided carrier-demodulated phase recruitment first
  captures at `18.8980T` with `74.24%` posterior acceleration-limit occupancy
  and `0.01306/0.00680` force/moment RMS, but its exact-policy repeat rebounds
  to `75.93%`, `25.03/28.79 rad/T^2` action RMS, and
  `0.01352/0.00704` force/moment RMS. Raw helpful-moment half-cycle relief also
  has a favorable first sample versus the actuator-consistent prefill—arrival
  `18.6560` versus `18.6725T`, score `-0.13321` versus `-0.13362`, posterior
  occupancy `75.74%` versus `76.11%`, and force/moment RMS
  `0.01328/0.00691` versus `0.01350/0.00703`—but the assigned parent's exact
  repeat takes about `18.9915T` and scores only `-0.15772`. Its compressed
  artifact has no matching load history, so the repeat falsifies a robust
  route-speed advantage and cannot establish a repeatable effort benefit. An
  inherited exact actuator-consistent recovery likewise takes about
  `18.9970T` and scores `-0.15892`, exposing roughly `0.32T` same-policy timing
  variability; close one-run route deltas are therefore not allocator
  evidence. The newest sample sharpens that boundary: two exact
  actuator-consistent instances both capture at `18.6725T`, while their scores
  span `-0.13362-- -0.13219`, anterior/posterior acceleration occupancy spans
  `40.7--42.2%/75.5--76.1%`, and force/moment RMS spans
  `0.01331--0.01350/0.00693--0.00703`. Helpful-moment relief again reaches
  `18.6560T`, but its `0.0165T` edge remains smaller than its failed inherited
  repeat. One-sided demodulated-moment recruitment is slower at `18.6890T`,
  has the worst sampled mean-distance cost (`2.02295L`), and its
  `40.5%/75.3%` occupancy and `0.01329/0.00692` loads overlap baseline spread.
  All variants retain coherent body-led wakes near `0.018U` local-flow RMS, so
  this is a route/allocation negative result, not an advection result. Preserve
  the normalized LOS C-bend, traveling carrier, persistent same-side phase
  path, and feasible action projection; avoid further instantaneous
  moment/LOS/residual allocation or scalar tuning. `turn_rate_recent` and the
  bearing window cover only about `0.0385T`, so test genuinely beat-scale
  response only when a true beat-scale observable exists, and require
  replicated capture plus occupancy/load separation from the plain route.
  This boundary is limited to direct-uniform still water and does not establish
  external-wake rejection.
- A low offline error against a centered beat average is not evidence of a
  useful causal recoil observer. The assigned parent's normalized reflection-
  odd anterior phase model reduced its offline `0.55T` yaw-fit error from
  `0.756--0.768` to `0.123--0.127 rad/T`, yet the completed rollout slows from
  the exact route's replicated `18.6725--18.7935T` capture band to `22.0110T`,
  raises mean distance from at most `2.03024L` to `2.31828L`, and scores only
  `-0.42356`. Both wake views remain coherent and body-led, but at `4T` speed
  is only `0.267U` versus `0.536--0.539U` for the references, exposing lost
  early carrier/route coupling rather than wake collapse. Posterior limit
  occupancy and force/moment RMS improve to `60.62%` and
  `0.01177/0.00612`, but anterior occupancy worsens to `48.30%` from the plain
  route's `40.68--42.15%`; lower load is therefore not a route improvement.
  Avoid scalar-tuning these nonlinear odd phase features or training an
  instantaneous policy surrogate against a centered diagnostic that contains
  future samples. Preserve the evaluated LOS route, traveling bend, linear
  recoil proxy, persistent same-side phase recruitment, and feasible-action
  projection. Revisit phase conditioning only with a causal beat-scale
  observation or history and falsify it unless it preserves early propulsion,
  captures inside the exact-policy band, and improves load without shifting
  saturation upstream. This boundary applies to the current `0.55T` carrier
  in direct-uniform still water; it does not rule out causal history under an
  external disturbance.
- Actuator-envelope cleanup must be separated from carrier weakening. A
  one-sided joint-rate anti-windup projection preserves the sampled route and
  both coherent wake views: it captures at `18.7000T` with mean distance
  `2.02103L`, inside two exact-policy samples at `18.6725--18.7330T` and
  `2.01959--2.02018L`. By zeroing only outward acceleration at the observed
  speed boundary, it lowers posterior action RMS from `28.72--28.77` to
  `28.24 rad/T^2` and acceleration-limit occupancy from `75.22--75.46%` to
  `73.97%`, while reverse braking remains available and early speed remains
  `0.536U`. It returns zero on `6.38%` of posterior samples where the joint is
  already at the speed boundary, but joint-rate-limit residence is unchanged;
  boundary-only blocking therefore removes infeasible outward work after
  saturation without establishing preventive constraint control. This does
  not yet establish a load or route advantage: `0.01333/0.00694` force/moment
  RMS overlaps the exact-route spread. In contrast, the assigned
  parent's closing-speed-gated terminal amplitude contraction lowers
  posterior occupancy and load to `74.06%` and `0.01312/0.00683`, but slows
  capture to `18.8705T`, raises mean distance to `2.04281L`, and shifts
  anterior occupancy to `42.32%`; lower load obtained by withdrawing the
  productive carrier is not an improvement. Preserve the one-sided feasible-
  command projection when effort cleanup is desired, but avoid combining it
  with range/closing-speed carrier contraction or scalar-tuning that negative
  mechanism. If rate residence rather than returned-command feasibility is
  the objective, the distinct next test is a narrow normalized pre-limit
  barrier that contracts only outward acceleration, preserves reverse braking
  and the persistent phase witness, and must reduce residence as well as
  posterior effort without leaving the replicated capture/load envelope.
  Falsify that preventive mechanism if residence remains unchanged or route
  progress slows; restore the hard projection instead of widening or scalar-
  tuning the barrier. Falsify anti-windup generally if a repeat loses capture,
  impairs braking or the alternating wake, exceeds `0.01350/0.00703` load, or
  fails to keep posterior effort below the plain route. This boundary applies
  to the current `0.55T` carrier in direct-uniform low-flow still water.
