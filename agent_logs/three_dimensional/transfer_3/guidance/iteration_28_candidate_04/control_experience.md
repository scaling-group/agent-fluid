...
- Carrier-demodulated hydrodynamic response has a replicated effort benefit,
  but instantaneous relief gates do not improve route or actuator headroom.
  Two identical demodulated-residual runs capture at `18.7165T` and
  `18.7605T` while holding posterior acceleration-limit occupancy to
  `74.17--74.44%` and force/moment RMS to
  ...
  The gain is load allocation, not route speed: raw helpful-moment relief's
  `18.6560T` lead remains inside same-hash spread, while stress-gating that
  relief by yaw closure delays capture to `18.7825T` and raises loads above
  the demodulated pair. More decisively, withdrawing only outward phase near
  the posterior velocity limit captures at `18.8100T` and lowers acceleration
  occupancy to `73.83%`, but increases measured velocity-limit occupancy from
  `7.73--7.83%` to `8.10%`; instantaneous anti-windup therefore does not create
  velocity headroom. Preserve the LOS C-bend, carrier, persistent phase path,
  and carrier subtraction. Avoid scalar-tuning raw response, yaw-closure
  relief, or velocity-headroom thresholds. If response is tested again, change
  its role: recruit a bounded residual only against opposing response without
  withdrawing the established route actuator when response helps. Falsify
  that one-sided implication if capture leaves the inherited
  `18.6725--19.0080T` band, coherent wake is lost, posterior acceleration
  occupancy exceeds `76.11%`, or force/moment RMS exceeds
  `0.01350/0.00703`; the evidence applies only to direct-uniform still water
  near `0.018U` local-flow RMS.
