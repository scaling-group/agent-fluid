...
- Response-triggered distributed C-bend steering is a replicated semantic
  success but not yet a robust one. Three current base-C-bend samples capture
  at `19.2335--19.8880T`, while a response-reversing posterior half-cycle
  sample is the first positive beat-phase result: it preserves the coherent
  top-down and oblique wake, captures at `18.9310T`, and improves mean distance
  from the explicitly feasible baseline's `2.0650L` to `2.0418L`. The change
  does not obtain speed by a large action/load increase: action RMS changes
  only from `25.19/28.56` to `25.22/28.86 rad/T^2`, force/moment RMS from
  `0.01322/0.00690` to `0.01357/0.00707`, and local-flow RMS from `0.0180U` to
  `0.0184U`. However, posterior acceleration-limit occupancy rises from
  `73.6%` to `76.1%`, nearly the `76.4%` of the assigned-parent rollout where
  the exact base hash misses `1.845L` high and exits left; that failure also
  reaches `60.8%` anterior occupancy and `0.01574/0.00810` force/moment RMS.
  Thus response-reversing beat-side authority is worth preserving, but one
  faster capture does not establish a widened basin and more clipped mean bend
  remains contraindicated. A sign-coherent, total-curvature-conserving
  allocation captures at `20.020T` with much lower action/load, whereas a
  memoryless near-range restriction first passes at `1.62L`, loops to `7.35L`,
  and captures only at `49.742T`. Preserve normalized LOS-rate feedback and
  continuous two-joint route closure; avoid gain-only authority, terminal
  coasting, predicted-miss release, or range-only allocation. Next test a
  bounded response-reversing phase or beat-history mechanism that changes
  timing rather than coefficient norm, and falsify it unless replicated
  captures stay at or below `76.4%` posterior occupancy and
  `0.01574/0.00810` force/moment RMS. These bounds apply to the coherent
  still-water carrier near `0.02U` local flow and require retesting under
  stronger disturbance.
- Saturation-recruited posterior phase timing improves the replicated
  response-reversing half-cycle without trading away its wake or capture
  semantics. Four sampled variants all capture with coherent alternating
  top-down and compact oblique wakes at `18.799--18.997T` in direct-uniform
  still water (`0.01816--0.01843U` local-flow RMS). Relative to the inherited
  half-cycle's `18.931T`, `2.04175L` mean distance, `44.77%/76.09%` limit
  occupancy, and `0.01357/0.00707` force/moment RMS, ungated phase rotation is
  slightly slower (`18.997T`), and gating from the previous clipped action
  reaches `18.892T`. Predicting the baseline posterior demand and recruiting
  phase as it approaches the envelope is both fastest (`18.799T`, `2.03111L`)
  and less clipped/loaded (`41.54%/73.96%`, `0.01315/0.00684`). Therefore
  preserve response-reversing phase as a constraint-aware allocation path;
  avoid ungated phase or delayed previous-action gating as the next scalar
  tuning surface. When refining it, require the phase proposal to open
  instantaneous acceleration headroom rather than merely detecting
  saturation. Falsify this implication if such allocation loses capture,
  leaves the sampled arrival band, breaks either wake row, or cannot improve
  occupancy/load without worsening mean distance. The conclusion applies to
  this coherent quiescent carrier and does not establish robustness under
  stronger inflow or disturbed wakes.
