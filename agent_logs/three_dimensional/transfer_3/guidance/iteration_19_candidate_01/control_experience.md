...
- Response-reversing beat modulation is a replicated improvement when it sits
  on continuously closed two-joint route feedback. The exact bounded half-
  cycle policy captures twice at `18.931--19.052T` with scores `-0.15357` and
  `-0.16031`, improving the earlier feasible distributed-C-bend
  capture band beginning at `19.233T` while retaining coherent alternating
  top-down and oblique wakes. A bounded response-conditioned posterior phase
  rotation also captures at `18.997T`, inside the half-cycle replication
  spread, while reducing action RMS to `24.55/28.59 rad/T^2`, acceleration-
  limit occupancy to `41.3%/74.1%`, force-magnitude RMS to `0.01317`, and
  moment RMS to `0.00685`; the two half-cycle runs span
  `24.85--25.22/28.75--28.86`, `42.6--44.8%/75.1--76.1%`,
  `0.01330--0.01357`, and `0.00692--0.00707`, respectively. Treat posterior
  phase rotation as a compact load-shaping primitive worth replication, not a
  demonstrated speed gain. Slow-curvature allocation gives a sharper warning:
  a demand-gated sample lowers force/moment RMS further to `0.01250/0.00654`
  but delays capture to `19.239T`, while inherited near-range allocation passes
  at `1.62L`, loops to `7.35L`, and captures only at `49.742T`. Preserve the
  response-reversing carrier, continuous anterior and posterior route closure,
  and explicit feasible-output projection; avoid another memoryless range gate
  or slow-curvature allocator merely to reduce effort. Falsify the phase
  lesson if a repeat loses capture, arrives later than `19.233T`, raises loads
  above the half-cycle interval, or weakens the wake. These comparisons apply
  to the coherent still-water carrier near `0.018U` local-flow RMS and require
