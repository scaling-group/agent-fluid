# Phase-2 candidate: preserve the actuator-consistent route and recruit
# posterior phase steering only against an adverse hydrodynamic yaw moment.

function target_policy_params()
    return (
        version="dogfish3d_countermoment_tail_phase_v1",
        control_period=0.55,
        ...
        hydrodynamic_moment_response_scale=0.0045,
...
function target_policy(state, params)
    ...
    # Separate persistent route demand from the measured fluid response. The
    # response/moment product is reflection-invariant because both factors
    # reverse. Phase authority rises only as the hydrodynamic yaw moment
    # opposes the requested correction, is half active at neutral alignment,
    # and releases as the moment becomes helpful.
    ...
    hydrodynamic_countermoment_gate = 0.5 * (1 - tanh(
        moment_response_alignment /
        max(params.hydrodynamic_moment_response_scale, 1.0e-6),
    ))
    phase_activation =
        predicted_phase_activation *
        actuator_consistency *
        hydrodynamic_countermoment_gate
    ...
    # At zero phase recruitment this is exactly the sampled response-reversing
    # half-cycle path. The physical-response gate changes posterior timing
    # without suppressing the already successful amplitude steering channel.
