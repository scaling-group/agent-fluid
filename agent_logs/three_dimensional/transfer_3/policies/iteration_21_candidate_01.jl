# Phase-2 candidate: preserve the best actuator-consistent half-cycle route and
# recruit posterior phase only for persistent, unrealized same-side demand.

function target_policy_params()
    return (
        version="dogfish3d_unrealized_demand_tail_phase_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Absolute normalized demand is reflection-invariant. The smooth gate
    # leaves the replicated half-cycle path unchanged below the envelope.
    ...
    # Retain the best sampled beat-reversal semantics, then distinguish
    # genuinely unrealized demand from a falling or already-realized flank.
    # All products and magnitudes are invariant when reflected accelerations
    # reverse together; no clock or hidden oscillator phase is introduced.
    ...
    predicted_demand_fraction =
        abs(baseline_posterior_accel) / acceleration_limit
    realized_action_fraction = clamp(
        abs(previous_posterior_action) / acceleration_limit,
        0.0,
        1.0,
    )
    unrealized_demand_fraction = clamp(
        predicted_demand_fraction - realized_action_fraction,
        0.0,
        1.0,
    )
    unrealized_demand_gate = tanh(
        unrealized_demand_fraction /
        max(params.tail_phase_deficit_scale, 1.0e-6),
    )
    phase_activation =
        predicted_phase_activation *
        actuator_consistency *
        unrealized_demand_gate
