...
# use physical yaw response only to relieve a stress-confirmed residual channel.

function target_policy_params()
    return (
        version="dogfish3d_stress_gated_moment_residual_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Preserve the primary phase actuator. A helpful physical yaw response may
    # relieve only half-cycle asymmetry, and only where persistent actuator
    # stress already recruited phase, the requested correction closes, and
    # that asymmetry points into the predicted posterior stress. Every signed
    # signal reverses with reflection; their alignments and allocation weights
    # are therefore invariant.
    ...
    helping_moment_release = tanh(
        max(moment_response_alignment, 0.0) /
        max(params.helpful_moment_release_scale, 1.0e-6),
    )
    correction_satisfaction = 1 - abs(response_direction)
    unrelieved_tail_wave_scale = 1 +
        asymmetry_limit * response_direction * modulated_tail_wave_phase
    unrelieved_posterior_target =
        guidance.mean_tail_curvature +
        unrelieved_tail_wave_scale * phase_modulated_wave_target
    unrelieved_posterior_accel =
        omega^2 * (unrelieved_posterior_target - q2) -
        2 * params.tail_damping * omega * qd2
    asymmetry_stress_alignment = clamp(
        response_direction * unrelieved_posterior_accel /
        acceleration_limit,
        0.0,
        1.0,
    )
    stress_confirmed_residual_release =
        phase_activation *
        correction_satisfaction *
        helping_moment_release *
        asymmetry_stress_alignment
    residual_asymmetry_limit =
        asymmetry_limit * (1 - stress_confirmed_residual_release)
