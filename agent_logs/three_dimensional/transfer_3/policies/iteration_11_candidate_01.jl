# Phase-2 candidate: collision-course-gated distributed C-bend on the coherent
# line-of-sight-rate controller. Bearing or a predicted unsafe closest pass
# recruits the same anterior mean bend while the traveling wave remains active.

function target_policy_params()
    return (
        version="dogfish3d_intercept_gated_distributed_cbend_v2",
        control_period=0.55,
...
function target_guidance(state, qd1, qd2, params)
    ...
    # Retain the parent's large-bearing trigger. Also estimate the closest-pass
    # distance under the current rigid velocity: |r x v| / |v|. During an
    # established closing approach, this collision-course gate bridges brief
    # bearing-alignment releases without using time or mutable history.
