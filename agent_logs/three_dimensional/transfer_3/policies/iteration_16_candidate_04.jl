# Phase-2 candidate: protect the traveling carrier while allocating only
# compatible slow curvature during the terminal approach.

function target_policy_params()
    return (
        version="dogfish3d_carrier_protected_near_allocation_v1",
        control_period=0.55,
        ...
        carrier_radius_floor=0.40,
        carrier_radius_width=0.10,
...
function target_guidance(state, q1, qd1, qd2, omega, params)
    ...
    # during a mid-approach bearing lull when bounded route demand stays high.
    ...
    # Conserve the inherited total slow bend by moving only same-sign posterior
    # curvature forward. Restrict that optional transfer to the terminal range.
    ...
    # The completed range-only allocator settled near a shifted equilibrium and
    # extinguished its wake. Measure the carrier around the unallocated route
    # center, and release only the optional transfer before amplitude vanishes.
    carrier_amplitude = max(params.oscillator_amplitude, 1.0e-6)
    base_centered_q1 = q1 - route_anterior_mean_curvature
    normalized_carrier_position = base_centered_q1 / carrier_amplitude
    normalized_carrier_velocity =
        qd1 / max(omega * carrier_amplitude, 1.0e-6)
    carrier_radius = hypot(
        normalized_carrier_position,
        normalized_carrier_velocity,
    )
    carrier_support_weight = 0.5 * (1 + tanh(
        (carrier_radius - params.carrier_radius_floor) /
        max(params.carrier_radius_width, 1.0e-6),
    ))
    allocation_weight =
        near_allocation_weight *
        sign_coherent_weight *
    ...
    return (
        ...
        base_centered_q1=base_centered_q1,
        normalized_carrier_position=normalized_carrier_position,
        normalized_carrier_velocity=normalized_carrier_velocity,
...
function target_policy(state, params)
    ...
    guidance = target_guidance(state, q1, qd1, qd2, omega, params)

    # Oscillate around the protected anterior route-response center. Expressing
    ...
    # Match the episode's componentwise physical actuator envelope here too.
