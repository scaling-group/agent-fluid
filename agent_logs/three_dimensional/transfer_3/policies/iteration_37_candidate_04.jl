# Phase-2 candidate: preserve the actuator-consistent LOS route and rotate the
# posterior wave toward braking as normalized joint-rate headroom closes.

function target_policy_params()
    return (
        version="dogfish3d_rate_headroom_phase_brake_v1",
        control_period=0.55,
        ...
        tail_rate_phase_limit=2.0 * pi / 180,
        tail_rate_activation_fraction=0.90,
        tail_rate_activation_width=0.04,
        tail_rate_outward_work_scale=0.12,
...
function target_policy(state, params)
    ...
    # steering response reverses if measured yaw outruns demand, while the
    # phase-gradient factor selects the rotation sign that adds the requested
    # curvature.
    phase_gradient =
    ...
    steering_phase_shift =
        phase_activation *
        phase_limit *
        response_direction *
        phase_gradient_direction

    # Begin turning the posterior wave toward braking before the hard speed
    # boundary. The gate requires both low normalized rate headroom and
    # same-direction acceleration. A small phase rotation changes the wave
    # target in proportion to phase_gradient, so the signed construction below
    # makes that first-order change oppose qd2. It therefore preserves the
    # coefficient norm and carrier instead of hard-zeroing saturated action.
    normalized_posterior_velocity = qd2 / velocity_limit
    posterior_rate_activation = 0.5 * (1 + tanh(
        (abs(normalized_posterior_velocity) -
            params.tail_rate_activation_fraction) /
        max(params.tail_rate_activation_width, 1.0e-6),
    ))
    normalized_outward_work = max(
        normalized_posterior_velocity *
        (baseline_posterior_accel / acceleration_limit),
        0.0,
    )
    outward_rate_activation = tanh(
        normalized_outward_work /
        max(params.tail_rate_outward_work_scale, 1.0e-6),
    )
    rate_phase_activation =
        posterior_rate_activation * outward_rate_activation
    rate_phase_limit = clamp(
        params.tail_rate_phase_limit,
        0.0,
        max(pi / 4 - phase_limit, 0.0),
    )
    posterior_velocity_direction = sign(normalized_posterior_velocity)
    rate_braking_phase_shift =
        -rate_phase_activation *
        rate_phase_limit *
        posterior_velocity_direction *
        phase_gradient_direction
    total_phase_limit = phase_limit + rate_phase_limit
    tail_phase_shift = clamp(
        steering_phase_shift + rate_braking_phase_shift,
        -total_phase_limit,
        total_phase_limit,
    )
