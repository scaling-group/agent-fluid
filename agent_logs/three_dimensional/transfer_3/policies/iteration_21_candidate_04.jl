# Phase-2 candidate: preserve the fast actuator-consistent route while
# saturating phase recruitment under persistent same-side posterior stress.

function target_policy_params()
    return (
        version="dogfish3d_sign_persistent_tail_phase_v1",
        control_period=0.55,
        ...
        tail_phase_consistency_scale=0.25,
...
function target_policy(state, params)
    ...
    # A beat reversal makes current raw demand and the observed previous
    # feasible action disagree in sign, releasing phase steering. Once they
    # persist on the same actuator side, a bounded saturating map treats that
    # persistence as a gate rather than attenuating phase by stress magnitude.
    # The signed product is reflection-invariant because both actions reverse.
    ...
    same_side_persistence = clamp(
        normalized_predicted_stress * normalized_realized_stress,
        0.0,
        1.0,
    )
    actuator_persistence_gate = tanh(
        same_side_persistence /
        max(params.tail_phase_consistency_scale, 1.0e-6),
    )
    phase_activation =
        predicted_phase_activation * actuator_persistence_gate
