# Phase-2 candidate: preserve the actuator-consistent route and replace its
# linear recoil proxy with a normalized, reflection-odd joint-phase observer.

function target_policy_params()
    return (
        version="dogfish3d_phase_conditioned_recoil_observer_v1",
        control_period=0.55,
        ...
        yaw_recoil_q1_gain=0.55,
        yaw_recoil_q2_gain=0.20,
        yaw_recoil_phase_position_gain=0.13,
        yaw_recoil_phase_velocity_gain=0.03,
...
function target_guidance(state, q1, qd1, qd2, omega, params)
    ...
    # Separate route-scale yaw from carrier recoil with a compact phase model.
    # The two nonlinear features are normalized by the observed oscillator
    # ellipse. Each is odd under reflection, so the inferred yaw response
    # reverses with the fish/target geometry without encoding a beat clock.
    measured_yaw_rate = _finite_or(state.heading_rate, 0.0)
    oscillator_amplitude = max(
        params.oscillator_amplitude,
        1.0e-6,
    )
    carrier_rate_scale = max(
        omega * oscillator_amplitude,
        1.0e-6,
    )
    normalized_recoil_q1 = q1 / oscillator_amplitude
    normalized_recoil_qd1 = qd1 / carrier_rate_scale
    phase_position_recoil =
        carrier_rate_scale *
        params.yaw_recoil_phase_position_gain *
        normalized_recoil_q1 *
        abs(normalized_recoil_qd1)
    phase_velocity_recoil =
        carrier_rate_scale *
        params.yaw_recoil_phase_velocity_gain *
        abs(normalized_recoil_q1) *
        normalized_recoil_qd1
    ...
    return (
        ...
        normalized_recoil_q1=normalized_recoil_q1,
        normalized_recoil_qd1=normalized_recoil_qd1,
        phase_position_recoil=phase_position_recoil,
        phase_velocity_recoil=phase_velocity_recoil,
