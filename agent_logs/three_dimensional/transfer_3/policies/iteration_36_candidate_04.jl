# Phase-2 candidate: preserve the actuator-consistent phase route and fade
# only outward acceleration across the final joint-speed headroom.

function target_policy_params()
    return (
        version="dogfish3d_smooth_rate_headroom_tail_phase_v1",
        control_period=0.55,
        ...
        joint_rate_projection_onset_fraction=0.90,
...
@inline function _project_outward_in_speed_headroom(
    acceleration,
    joint_velocity,
    velocity_limit,
    onset_fraction,
    ...
    # Use only normalized actuator state. Below the onset the evaluated
    # carrier is unchanged. Across the remaining headroom, a C1 smoothstep
    # anticipates the hard speed projection without weakening reverse braking.
    feasible_onset = clamp(onset_fraction, 0.0, 1.0 - eps(Float64))
    normalized_speed = abs(joint_velocity) / feasible_velocity_limit
    remaining_headroom = clamp(
        (1.0 - normalized_speed) / (1.0 - feasible_onset),
        0.0,
        1.0,
    )
    outward_scale =
        remaining_headroom^2 * (3.0 - 2.0 * remaining_headroom)
    return outward_scale * acceleration
end
...
function target_policy(state, params)
    ...
    # Speed-envelope projection returns zero while saturation makes outward
    # action infeasible. Preserve the evaluated persistence semantics by
    # treating an ongoing same-side raw demand at that exact boundary as the
    # missing feasible-action witness. When demand reverses, the witness
    # disappears and braking/phase release remain immediate.
    rate_limited_outward_persistence =
        abs(qd2) >= velocity_limit &&
        observed_previous_posterior_action == 0.0 &&
    ...
    # Anticipate the observed hard speed boundary through normalized actuator
    # headroom. Projection is one-sided: reverse acceleration remains fully
    # available to brake and preserve the traveling-wave phase. Under lateral
    # reflection velocity and acceleration both reverse, so the scale is
    # invariant and its output remains odd.
    antiwindup_anterior_accel = _project_outward_in_speed_headroom(
        feasible_anterior_accel,
        qd1,
        velocity_limit,
        params.joint_rate_projection_onset_fraction,
    )
    antiwindup_posterior_accel = _project_outward_in_speed_headroom(
    ...
    return (
        phi_ddot=(antiwindup_anterior_accel, antiwindup_posterior_accel),
    )
end
