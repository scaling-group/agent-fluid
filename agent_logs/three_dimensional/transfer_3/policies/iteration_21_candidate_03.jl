...
# fast steering authority from amplitude asymmetry to posterior phase.

function target_policy_params()
    return (
        version="dogfish3d_complementary_tail_phase_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Hand fast steering authority from amplitude asymmetry to phase instead
    # of stacking both mechanisms. At zero phase recruitment this is exactly
    # the sampled actuator-consistent half-cycle path; as phase authority rises
    # the complementary amplitude channel releases continuously.
    modulated_tail_wave_phase = tanh(
        phase_modulated_wave_target /
        max(params.oscillator_amplitude, 1.0e-6),
    )
    amplitude_authority_fraction = 1 - phase_activation
