# Phase-2 candidate: range-scheduled approach drive on the parent's coherent
# line-of-sight-rate controller. Far-field propulsion is preserved; normalized
# target range adds bounded carrier damping while steering remains active.

function target_policy_params()
    return (
        version="dogfish3d_los_rate_approach_drive_v1",
        control_period=0.55,
        ...
        approach_distance_L=5.5,
        approach_transition_width_L=1.0,
        approach_joint_damping=0.60,
    )
end
...
function target_guidance(state, qd1, qd2, params)
    ...
    # The shelf's approach-hold invariant is translated as a slow range-only
    # schedule. It cannot encode a route, clock, or target identity.
...
function target_policy(state, params)
    ...
    # Preserve the parent's far-field traveling bend, then add a slow bounded
    # damping schedule during approach. Posterior steering is not scaled.
    vdp_drive = params.oscillator_mu *
        (1 - (q1 / params.oscillator_amplitude)^2) * qd1
    approach_damping =
        params.approach_joint_damping * guidance.approach_weight
    anterior_accel = vdp_drive - omega^2 * q1 - approach_damping * qd1
