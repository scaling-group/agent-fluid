# Phase-2 candidate: preserve the successful response-triggered anterior
# C-bend while allocating the posterior joint primarily to traveling-wave
# propulsion instead of duplicating the recruited slow steering curvature.

function target_policy_params()
    return (
        version="dogfish3d_response_gated_steering_allocation_v1",
        control_period=0.55,
        ...
        posterior_steering_relief=0.75,
    )
end
...
function target_guidance(state, qd1, qd2, params)
    ...
    # The sampled capture is created by recruited anterior curvature, while
    # posterior raw acceleration remains the dominant envelope violation.
    # Relieve only duplicated slow steering at the posterior joint; its
    # anti-phase and velocity-lag carrier terms remain fully active.
    posterior_route_weight = clamp(
        1 - params.posterior_steering_relief * redirect_weight,
        0.0,
        1.0,
    )
    posterior_mean_curvature =
        posterior_route_weight * mean_tail_curvature
