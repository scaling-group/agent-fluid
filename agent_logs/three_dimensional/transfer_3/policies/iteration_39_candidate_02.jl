...
@inline function _project_outward_to_velocity_headroom(
    ...
    # Bound only outward action by the speed headroom remaining over the
    # episode's documented maximum update. Reverse braking stays immediate.
...
function target_policy(state, params)
    ...
    # Projection returns zero at the boundary and may limit the entry action.
    # Preserve the established persistence gate while raw outward demand
    # continues; a demand or action reversal releases it immediately.
    ...
    # Project only the outward component into the next update's available
    # joint-speed headroom. Under lateral reflection both velocity and
    # acceleration reverse, so the gate is invariant and the output remains
    # odd; opposite-sign braking is never reduced.
