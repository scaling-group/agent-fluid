# Phase-2 candidate: preserve the completed LOS-rate distributed C-bend while
# routing same-sign posterior steering lost to saturation into anterior headroom.

function target_policy_params()
    return (
        version="dogfish3d_saturation_aware_steering_spillover_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Split the posterior request into carrier and steering components. The
    # difference between feasible combined and carrier-only commands is the
    # steering authority that survives the physical acceleration limit.
    nominal_centered_q1 = q1 - guidance.anterior_mean_curvature
    nominal_posterior_carrier_target =
        -nominal_centered_q1 -
        params.tail_lag_gain * qd1 / max(omega, eps(Float64))
    nominal_posterior_carrier_accel =
        omega^2 * (nominal_posterior_carrier_target - q2) -
        2 * params.tail_damping * omega * qd2
    posterior_steering_accel = omega^2 * guidance.mean_tail_curvature
    feasible_nominal_carrier = clamp(
        nominal_posterior_carrier_accel,
        -acceleration_limit,
        acceleration_limit,
    )
    feasible_nominal_combined = clamp(
        nominal_posterior_carrier_accel + posterior_steering_accel,
        -acceleration_limit,
        acceleration_limit,
    )
    effective_posterior_steering =
        feasible_nominal_combined - feasible_nominal_carrier
    rejected_posterior_steering =
        posterior_steering_accel - effective_posterior_steering

    # Fill only unused capacity inside the existing anterior C-bend bound.
    # Sign coherence prevents a phase-conditioned posterior correction from
    # overriding an opposing body-frame route request at the anterior joint.
    coherent_steering_spillover = ifelse(
        rejected_posterior_steering * guidance.target_yaw_rate > 0.0,
        rejected_posterior_steering,
        0.0,
    )
    anterior_mean_curvature = clamp(
        guidance.anterior_mean_curvature +
        coherent_steering_spillover / max(omega^2, eps(Float64)),
        -params.anterior_redirect_limit,
        params.anterior_redirect_limit,
    )

    # Oscillate around the adjusted bounded anterior route-response center.
    # Expressing the posterior wave relative to it preserves the traveling bend.
    ...
    posterior_carrier_target =
        -centered_q1 -
        params.tail_lag_gain * qd1 / max(omega, eps(Float64))
    posterior_carrier_accel =
        omega^2 * (posterior_carrier_target - q2) -
        2 * params.tail_damping * omega * qd2
    posterior_accel = posterior_carrier_accel + posterior_steering_accel

    # Return only commands inside the physical envelope.
