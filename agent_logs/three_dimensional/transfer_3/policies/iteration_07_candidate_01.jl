# Phase-2 candidate: distribute a response-released course bend across both
# joints of the coherent state-feedback carrier. Joint rates remove locomotor
# recoil before normalized target geometry requests total body curvature.
...
function target_policy_params()
    return (
        ...
        total_body_bend_limit=10.0 * pi / 180,
        anterior_bend_share=0.35,
    )
end
...
function target_guidance(state, qd1, qd2, params)
    ...
    return (
        ...
        total_body_bend=total_body_bend,
    )
end
...
function target_policy(state, params)
    ...
    anterior_bias = params.anterior_bend_share * guidance.total_body_bend
    posterior_bias =
        (1.0 - params.anterior_bend_share) * guidance.total_body_bend

    # Preserve the sampled carrier around a bounded, response-released
    # anterior equilibrium. Centering the oscillator state keeps its amplitude
    # mechanism independent of the slow course bend.
    ...
    # Form the traveling posterior wave from the centered anterior motion, so
    # the two equilibrium shares sum to the requested total body curvature.
    posterior_target =
        posterior_bias - centered_q1 -
