# Phase-2 candidate: preserve the actuator-consistent phase route and taper
# only outward acceleration inside a narrow joint-speed feasibility barrier.

function target_policy_params()
    return (
        version="dogfish3d_anticipatory_rate_guard_tail_phase_v1",
        control_period=0.55,
        ...
        joint_velocity_guard_fraction=0.04,
...
@inline function _project_outward_velocity_guard(
    acceleration,
    joint_velocity,
    velocity_limit,
    guard_fraction,
    ...
    bounded_guard_fraction = clamp(guard_fraction, 0.0, 1.0)
    guard_width = bounded_guard_fraction * feasible_velocity_limit

    # The signed product is invariant under lateral reflection. Never alter
    # reverse acceleration, because it is the braking/phase-reversal command.
    accelerating_outward = acceleration * joint_velocity > 0
    accelerating_outward || return acceleration

    speed_headroom = feasible_velocity_limit - abs(joint_velocity)
    if guard_width <= eps(Float64)
        return speed_headroom <= 0 ? 0.0 : acceleration
    end
    speed_headroom >= guard_width && return acceleration

    normalized_headroom = clamp(speed_headroom / guard_width, 0.0, 1.0)
    smooth_authority =
        normalized_headroom^2 * (3 - 2 * normalized_headroom)
    return smooth_authority * acceleration
end
...
function target_policy(state, params)
    ...
    velocity_guard_fraction = clamp(
        params.joint_velocity_guard_fraction,
        0.0,
        1.0,
    )
    velocity_guard_start =
        velocity_limit * (1 - velocity_guard_fraction)
    ...
    # The speed projector attenuates the returned action before a hard limit.
    # Preserve the evaluated persistence semantics by using continuing raw
    # same-side demand as the actuator witness only while that projector is
    # active. A previous reverse command keeps its immediate release authority.
    guarded_outward_persistence =
        abs(qd2) >= velocity_guard_start &&
        baseline_posterior_accel * qd2 > 0 &&
        observed_previous_posterior_action * qd2 >= 0
    previous_posterior_action = guarded_outward_persistence ?
        clamp(
            baseline_posterior_accel,
            -acceleration_limit,
            acceleration_limit,
    ...
    # Begin withdrawing only outward acceleration shortly before the hard
    # speed boundary. The smooth state-feedback barrier is inactive away from
    # the limit and retains full opposite-sign braking at every speed.
    guarded_anterior_accel = _project_outward_velocity_guard(
        feasible_anterior_accel,
        qd1,
        velocity_limit,
        velocity_guard_fraction,
    )
    guarded_posterior_accel = _project_outward_velocity_guard(
    ...
    return (
        phi_ddot=(guarded_anterior_accel, guarded_posterior_accel),
    )
end
