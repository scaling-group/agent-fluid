...
# only outward acceleration that would exhaust joint-rate headroom in one step.

function target_policy_params()
    return (
        version="dogfish3d_rate_headroom_projected_tail_phase_v1",
        control_period=0.55,
...
@inline function _outward_acceleration_headroom(
    joint_velocity,
    velocity_limit,
    maximum_update,
)
    feasible_velocity_limit = max(velocity_limit, eps(Float64))
    feasible_update = max(maximum_update, eps(Float64))
    remaining_velocity = max(
        feasible_velocity_limit - abs(joint_velocity),
        0.0,
    )
    return remaining_velocity / feasible_update
end
...
@inline function _project_outward_rate_headroom(
    ...
    # Reverse action remains untouched so braking and beat reversal retain
    # their full authority. Reflection reverses both signed inputs, leaving
    # the gate invariant and its projected output odd.
    if acceleration * joint_velocity <= 0
        return acceleration
    end
    outward_headroom = _outward_acceleration_headroom(
        joint_velocity,
        velocity_limit,
        maximum_update,
    )
    return copysign(
        min(abs(acceleration), outward_headroom),
        acceleration,
    )
end
...
function target_policy(state, params)
    ...
    posterior_outward_headroom = _outward_acceleration_headroom(
        qd2,
        velocity_limit,
        maximum_update,
    )
    rate_headroom_constrained_persistence =
        baseline_posterior_accel * qd2 > 0 &&
        abs(baseline_posterior_accel) > posterior_outward_headroom &&
        observed_previous_posterior_action * baseline_posterior_accel >= 0
    # Projection can make the feasible previous action arbitrarily small near
    # the speed boundary. Preserve the evaluated persistent raw-demand witness
    # there; a genuinely reversed previous action still releases immediately.
    ...
    # Respect the observed joint-rate envelope prospectively over one maximum
    # solver update instead of waiting for the hard boundary. This is a
    # one-sided feasibility projection, not a change to route or carrier gains.
    projected_anterior_accel = _project_outward_rate_headroom(
        feasible_anterior_accel,
        qd1,
        velocity_limit,
        maximum_update,
    )
    projected_posterior_accel = _project_outward_rate_headroom(
    ...
    return (
        phi_ddot=(projected_anterior_accel, projected_posterior_accel),
    )
end
