# Phase-2 candidate: preserve the actuator-consistent posterior route and
# release only redundant anterior C-bend after helpful yaw response appears.

function target_policy_params()
    return (
        version="dogfish3d_response_releasing_anterior_c_bend_v1",
        control_period=0.55,
        ...
        anterior_response_relief_limit=0.35,
...
function target_guidance(state, qd1, qd2, params)
    ...
    # A C-start-like redirect should release after helpful yaw appears rather
    # than maintaining the same anterior mean bend through response closure.
    # The signed product is reflection-invariant; opposite response keeps full
    # redirect authority. Normalization by the established response scale
    # prevents small route requests from producing a large release.
    aligned_yaw_response = clamp(
        target_yaw_rate * phase_conditioned_yaw_rate /
        max(
            target_yaw_rate^2,
            params.yaw_rate_error_scale^2,
            eps(Float64),
        ),
        0.0,
        1.0,
    )
    anterior_response_relief = clamp(
        params.anterior_response_relief_limit,
        0.0,
        0.95,
    ) * aligned_yaw_response
    anterior_response_scale = 1 - anterior_response_relief
    anterior_mean_curvature =
        params.anterior_redirect_limit *
        redirect_weight *
        anterior_response_scale *
        tanh(
            target_yaw_rate /
    ...
    return (
        ...
        aligned_yaw_response=aligned_yaw_response,
        anterior_response_relief=anterior_response_relief,
        anterior_response_scale=anterior_response_scale,
