# Phase-2 candidate: signed collision-course C-bend on the coherent LOS-rate
# carrier. Bearing and predicted closest-pass error each issue a reversible,
# bounded anterior redirect while the traveling wave remains active.

function target_policy_params()
    return (
        version="dogfish3d_signed_intercept_cbend_v1",
        control_period=0.55,
        ...
        intercept_direction_scale=0.55,
...
function target_guidance(state, qd1, qd2, params)
    ...
    # Retain the parent's large-bearing request. Also estimate signed closest-
    # pass error under the current rigid velocity: (r x v) / |v|. During an
    # established closing approach, this gives collision-course geometry its
    # own reversible curvature request instead of merely gating the possibly
    # cancelled instantaneous yaw request.
    ...
    signed_predicted_miss_L = (
        target_y * velocity_x - target_x * velocity_y
    ) / max(rigid_speed, params.intercept_speed_floor)
    predicted_miss_L = abs(signed_predicted_miss_L)
    ...
    bearing_redirect_request =
        bearing_redirect_weight *
        tanh(
            target_yaw_rate /
            max(params.redirect_yaw_rate_scale, 1.0e-6),
        )
    intercept_redirect_request =
        intercept_redirect_weight *
        tanh(
            signed_predicted_miss_L /
            max(params.intercept_direction_scale, 1.0e-6),
        )

    # The predictive branch receives only the authority not already used by
    # the bearing branch. Same-sign requests smoothly reinforce up to the
    # unchanged bound; opposite-sign requests release or reverse the bend.
    redirect_request = clamp(
        bearing_redirect_request +
        (1 - abs(bearing_redirect_request)) *
        intercept_redirect_request,
        -1.0,
        1.0,
    )
    ...
    return (
        ...
        signed_predicted_miss_L=signed_predicted_miss_L,
        ...
        bearing_redirect_request=bearing_redirect_request,
        intercept_redirect_request=intercept_redirect_request,
...
function target_policy(state, params)
    ...
    # Oscillate around the bounded sensory redirect. Using the centered state
    # in both oscillators preserves their relative traveling bend while the two
    # joints share slow route curvature.
