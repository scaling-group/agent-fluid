# Phase-2 candidate: preserve the actuator-consistent phase route and release
# redundant anterior redirect when LOS motion confirms route rotation.

function target_policy_params()
    return (
        version="dogfish3d_los_response_anterior_release_v1",
        control_period=0.55,
        ...
        anterior_los_release_limit=0.50,
        anterior_los_release_rate_floor=0.08,
...
function target_guidance(state, qd1, qd2, params)
    ...
    # When LOS motion counters the bearing request, route rotation is already
    # developing. Release only redundant anterior C-bend authority while the
    # posterior mean/half-cycle/phase paths remain intact. The product of the
    # two odd yaw requests is reflection-invariant; startup and reinforcing
    # LOS motion retain full anterior recruitment.
    counter_los_alignment = max(
        -bearing_yaw_rate * response_yaw_rate,
        0.0,
    )
    release_rate_floor = max(
        params.anterior_los_release_rate_floor,
        1.0e-6,
    )
    counter_los_fraction = clamp(
        counter_los_alignment /
        max(bearing_yaw_rate^2, release_rate_floor^2),
        0.0,
        1.0,
    )
    smooth_counter_los_fraction =
        counter_los_fraction^2 * (3 - 2 * counter_los_fraction)
    anterior_release_limit = clamp(
        params.anterior_los_release_limit,
        0.0,
        1.0,
    )
    anterior_authority_fraction =
        1 - anterior_release_limit * smooth_counter_los_fraction
    ...
    return (
        ...
        counter_los_alignment=counter_los_alignment,
        counter_los_fraction=counter_los_fraction,
        smooth_counter_los_fraction=smooth_counter_los_fraction,
        anterior_authority_fraction=anterior_authority_fraction,
...
function target_policy(state, params)
    ...
    # At zero phase recruitment this is exactly the replicated half-cycle
    # path; at full recruitment it is the sampled response-reversing phase
    # path. Hydrodynamic moment has no allocator role in this candidate.
