# Phase-2 candidate: release posterior phase only when helpful hydrodynamic
# moment coincides with a substantially satisfied yaw-response correction.

function target_policy_params()
    return (
        version="dogfish3d_error_conditioned_moment_release_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # A helping moment is sufficient reason to release posterior phase only
    # as the requested recoil-conditioned yaw correction is being satisfied.
    # At large normalized yaw error the evaluated fast phase response remains
    # active; near closure the measured hydrodynamic contribution can replace
    # it smoothly. Both signed physical responses reverse under reflection,
    # while correction satisfaction and their product remain invariant.
    ...
    response_sufficiency_gate = 1 - satisfied_helping_release
    phase_activation =
