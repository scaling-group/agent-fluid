# Phase-2 candidate: preserve the replicated actuator-consistent route and
# suppress only infeasible outward acceleration at a hard joint-speed boundary.

function target_policy_params()
    return (
        version="dogfish3d_joint_rate_antiwindup_v2",
        control_period=0.55,
...
@inline function _one_sided_rate_projection(accel, joint_velocity, params)
    velocity_limit = max(params.joint_velocity_limit, eps(Float64))
    at_speed_boundary =
        abs(joint_velocity) >= velocity_limit - sqrt(eps(Float64))
    outward_acceleration = accel * joint_velocity > 0
    return at_speed_boundary && outward_acceleration ? 0.0 : accel
end
...
function target_policy(state, params)
    ...
    velocity_limit = max(params.joint_velocity_limit, eps(Float64))
    posterior_outward_demand_at_boundary =
        abs(qd2) >= velocity_limit - sqrt(eps(Float64)) &&
        baseline_posterior_accel * qd2 > 0
    antiwindup_zero_witness =
        posterior_outward_demand_at_boundary &&
        abs(previous_posterior_action) <= sqrt(eps(Float64))
    normalized_realized_stress = antiwindup_zero_witness ?
        normalized_predicted_stress :
        clamp(
    ...
    # The episode applies the same componentwise physical limits. First clamp
    # acceleration, then remove only outward work at an already active speed
    # boundary. Opposite-sign braking remains available without attenuation.
    clamped_anterior_accel = clamp(
        anterior_accel,
        -acceleration_limit,
        acceleration_limit,
    )
    clamped_posterior_accel = clamp(
        posterior_accel,
        -acceleration_limit,
        acceleration_limit,
    )
    feasible_anterior_accel = _one_sided_rate_projection(
        clamped_anterior_accel,
        qd1,
        params,
    )
    feasible_posterior_accel = _one_sided_rate_projection(
        clamped_posterior_accel,
        qd2,
        params,
    )
