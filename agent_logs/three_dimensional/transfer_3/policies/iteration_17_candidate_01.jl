# Phase-2 candidate: retain completed response-conditioned anterior steering,
# but reserve the posterior joint for the phase-lagged propulsive wave.

function target_policy_params()
    return (
        version="dogfish3d_anterior_route_posterior_wave_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Give the posterior joint one job: lag and emphasize the oscillatory wave.
    # Slow route curvature is already supplied by the anterior oscillator
    # center, so a second posterior mean bend would duplicate steering demand.
    posterior_target =
        -centered_q1 -
