...
# Phase-2 adaptation: consume the evaluator's normalized body-frame distance,
# target vector, velocity, and moment fields directly. The new control
# mechanism is the response-gated posterior yaw-arrest path below.

function target_policy_params()
    return (
        version="dogfish_target_control_v19_response_gated_yaw_arrest",
        drive_period=0.55,
        control_period=0.55,
        ...
        yaw_arrest_curvature_gain=7.0 * pi / 180,
        yaw_arrest_rate_scale=0.28,
...
function drive_module(
    ...
    yaw_arrest_command = _clamp_unit(_safe(yaw_arrest_request, 0.0))
    mean_tail_tangent =
        ...
        params.yaw_arrest_curvature_gain * yaw_arrest_command
    ...
    return (
        ...
        yaw_arrest_command=yaw_arrest_command,
...
function guidance_module(state, params)
    ...
    slip_y = _safe(state.velocity_body_U[2], 0.0)
    ...
    # Give excess measured yaw a direct mean-curvature brake. This path is
    # distinct from half-cycle steering and releases when the requested and
    # measured target-directed turn rates agree.
    yaw_arrest_request = tanh(
        (turn_rate - target_turn_rate) /
        max(params.yaw_arrest_rate_scale, 1.0e-6),
    )
    ...
    return (
        ...
        yaw_arrest_request=yaw_arrest_request,
        bearing=bearing,
...
function target_policy(state, params)
    ...
    drive = drive_module(
        ...
        guidance.yaw_arrest_request,
    )
    turn = turn_actuator_module(drive, guidance, params)
    angular_damping =
        -params.body_angular_damping_gain * _safe(state.moment_z_L2, 0.0)
    return (
        phi_ddot=(
