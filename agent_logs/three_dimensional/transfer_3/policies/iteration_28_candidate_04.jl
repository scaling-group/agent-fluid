# Phase-2 candidate: preserve the helpful-moment route and withdraw only final
# posterior acceleration that consumes measured velocity headroom.

function target_policy_params()
    return (
        version="dogfish3d_final_posterior_velocity_barrier_v1",
        control_period=0.55,
        ...
        posterior_velocity_activation_fraction=0.95,
        posterior_velocity_activation_width=0.025,
        posterior_outward_acceleration_scale=0.05,
...
function target_policy(state, params)
    ...
    # Enforce velocity headroom on the final command rather than changing the
    # phase target that produced it. Only acceleration that drives a near-limit
    # posterior joint farther outward is withdrawn; restoring acceleration is
    # unchanged. Velocity and acceleration both reverse under reflection, so
    # their alignment is invariant while the gated acceleration remains odd.
    velocity_limit = max(
    ...
    normalized_outward_acceleration = max(
        (qd2 / velocity_limit) *
        (posterior_accel / acceleration_limit),
        0.0,
    )
    posterior_outward_activation = tanh(
        normalized_outward_acceleration /
        max(params.posterior_outward_acceleration_scale, 1.0e-6),
    )
    posterior_headroom_weight = clamp(
        1 - posterior_velocity_activation * posterior_outward_activation,
        0.0,
        1.0,
    )
    headroom_posterior_accel =
        posterior_headroom_weight * posterior_accel
