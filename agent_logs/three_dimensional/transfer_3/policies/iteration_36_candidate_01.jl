# Phase-2 candidate: preserve the actuator-consistent phase route and smoothly
# withdraw only outward acceleration as normalized joint-rate headroom vanishes.

function target_policy_params()
    return (
        version="dogfish3d_rate_headroom_tail_phase_v1",
        control_period=0.55,
        ...
        joint_rate_headroom_fraction=0.04,
...
@inline function _project_outward_rate_headroom(
    acceleration,
    joint_velocity,
    velocity_limit,
    headroom_fraction,
)
    # The product is invariant under lateral reflection. Reverse acceleration
    # remains fully available to brake out of either signed speed boundary.
    acceleration * joint_velocity <= 0 && return acceleration
    feasible_velocity_limit = max(velocity_limit, eps(Float64))
    normalized_headroom = clamp(
        1 - abs(joint_velocity) / feasible_velocity_limit,
        0.0,
        1.0,
    )
    feasible_headroom_fraction = clamp(
        headroom_fraction,
        eps(Float64),
        1.0,
    )
    headroom_coordinate = clamp(
        normalized_headroom / feasible_headroom_fraction,
        0.0,
        1.0,
    )
    # Smoothstep avoids switching the emitted acceleration sharply at the
    # inner edge of the projection band.
    outward_authority =
        headroom_coordinate^2 * (3 - 2 * headroom_coordinate)
    return outward_authority * acceleration
end
...
function target_policy(state, params)
    ...
    # The headroom projection deliberately reduces the feasible previous
    # action. Preserve the evaluated persistence semantics with a raw-demand
    # witness only while the posterior is moving outward inside that band.
    normalized_posterior_headroom = clamp(
        1 - abs(qd2) / velocity_limit,
        0.0,
        1.0,
    )
    outward_posterior_rate_stress =
        baseline_posterior_accel * qd2 > 0 &&
        normalized_posterior_headroom <
            params.joint_rate_headroom_fraction
    previous_posterior_action = outward_posterior_rate_stress ?
        clamp(
            baseline_posterior_accel,
            -acceleration_limit,
            acceleration_limit,
    ...
    # Smoothly withdraw only outward acceleration in the final normalized
    # joint-rate headroom band. Unlike a hard boundary block this acts before
    # the episode clips velocity, while reverse acceleration and the carrier's
    # braking half-cycle remain untouched.
    headroom_anterior_accel = _project_outward_rate_headroom(
        feasible_anterior_accel,
        qd1,
        velocity_limit,
        params.joint_rate_headroom_fraction,
    )
    headroom_posterior_accel = _project_outward_rate_headroom(
