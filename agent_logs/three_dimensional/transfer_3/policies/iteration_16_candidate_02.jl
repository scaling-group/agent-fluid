# Phase-2 candidate: keep the completed LOS-rate distributed C-bend while a
# coupled actuator projection preserves the traveling-wave command direction.

function target_policy_params()
    return (
        version="dogfish3d_los_response_triggered_cbend_coupled_projection_v3",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Fit the complete two-joint command into the componentwise physical
    # envelope with one positive scale. This preserves the raw inter-joint
    # direction (posterior emphasis, phase relation, and steering share) where
    # independent clipping would reshape the traveling bend.
    ...
    command_scale = min(
        1.0,
        acceleration_limit / max(peak_acceleration, eps(Float64)),
    )
    feasible_anterior_accel = command_scale * anterior_accel
    feasible_posterior_accel = command_scale * posterior_accel
