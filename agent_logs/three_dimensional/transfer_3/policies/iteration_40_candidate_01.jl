# Phase-2 candidate: preserve the actuator-consistent route and apply exact-
# boundary anti-windup only to the posterior joint with evidenced excess effort.
...
@inline function _block_outward_at_speed_limit(
    ...
    normalized_joint_speed =
        abs(joint_velocity) / feasible_velocity_limit
    at_speed_limit = normalized_joint_speed >= 1.0
...
function target_policy(state, params)
    ...
    # The posterior projection returns zero while speed saturation makes
    # outward action infeasible. Preserve the established persistence
    # semantics by treating continued same-side raw demand at that exact
    # boundary as the missing feasible-action witness. Braking releases it.
    ...
    # The sampled effort reduction is posterior-specific. Suppress only
    # posterior acceleration that winds farther into the observed hard speed
    # boundary; retain anterior route actuation and all reverse braking.
    # Under lateral reflection velocity and acceleration reverse together, so
    # the gate is invariant and its output remains odd.
