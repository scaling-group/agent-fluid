# Phase-2 candidate: preserve the actuator-consistent phase route and move only
# stress-aligned half-cycle steering into persistent anterior actuator headroom.

function target_policy_params()
    return (
        version="dogfish3d_complementary_steering_headroom_v1",
        control_period=0.55,
        ...
        tail_steering_transfer_limit=0.35,
        tail_steering_alignment_fraction=0.10,
...
function target_policy(state, params)
    ...
    tail_steering_wave_target =
        asymmetry_limit *
        response_direction *
        modulated_tail_wave_phase *
        phase_modulated_wave_target
    unallocated_posterior_target =
        guidance.mean_tail_curvature +
        phase_modulated_wave_target +
        tail_steering_wave_target
    unallocated_posterior_accel =
        omega^2 * (unallocated_posterior_target - q2) -
        2 * params.tail_damping * omega * qd2

    # Reallocate only the steering-induced part of the posterior acceleration.
    # Persistent posterior stress comes from the evaluated phase gate above;
    # both predicted and previous feasible anterior actions must expose
    # headroom. The smooth signed alignment rejects a transfer that would
    # increase, rather than relieve, posterior stress.
    previous_anterior_action =
        _finite_or(state.previous_action[1], 0.0)
    predicted_anterior_headroom = clamp(
        1 - abs(anterior_accel) / acceleration_limit,
        0.0,
        1.0,
    )
    realized_anterior_headroom = clamp(
        1 - abs(previous_anterior_action) / acceleration_limit,
        0.0,
        1.0,
    )
    persistent_anterior_headroom = min(
        predicted_anterior_headroom,
        realized_anterior_headroom,
    )
    tail_steering_accel = omega^2 * tail_steering_wave_target
    alignment_scale = max(
        params.tail_steering_alignment_fraction * acceleration_limit,
        eps(Float64),
    )
    tail_steering_stress_alignment = clamp(
        tanh(tail_steering_accel / alignment_scale) *
        tanh(unallocated_posterior_accel / alignment_scale),
        0.0,
        1.0,
    )
    steering_transfer_limit = clamp(
        params.tail_steering_transfer_limit,
        0.0,
        1.0,
    )
    steering_transfer_fraction =
        steering_transfer_limit *
        phase_activation *
        persistent_anterior_headroom *
        tail_steering_stress_alignment
    transferred_steering_accel =
        steering_transfer_fraction * tail_steering_accel

    # Equal subtraction and addition conserve the pre-limit joint-sum steering
    # acceleration. The carrier, mean bend, and phase-modulated wave are not
    # transferred, and componentwise projection remains the final authority.
    anterior_accel += transferred_steering_accel
    posterior_target =
        unallocated_posterior_target -
        steering_transfer_fraction * tail_steering_wave_target
    posterior_accel =
