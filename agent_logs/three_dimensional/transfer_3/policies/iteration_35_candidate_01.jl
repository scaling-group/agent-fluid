# Phase-2 candidate: preserve the actuator-consistent phase route and add a
# bounded response-directed duty ratio to the anterior state-feedback carrier.

function target_policy_params()
    return (
        version="dogfish3d_response_directed_anterior_duty_v1",
        control_period=0.55,
        ...
        anterior_duty_ratio_limit=0.08,
...
function target_policy(state, params)
    ...
    # During an active redirect, use observed joint phase to dwell slightly
    # longer on the bend side that agrees with the requested yaw response and
    # pass more quickly through the opposite side. The product of the odd yaw
    # response and odd phase side is reflection-invariant, so the restoring
    # acceleration remains equivariant and strictly positive in both halves.
    centered_q1 = q1 - guidance.anterior_mean_curvature
    anterior_phase_side = tanh(
        centered_q1 / max(params.oscillator_amplitude, 1.0e-6),
    )
    duty_ratio_limit = clamp(
        params.anterior_duty_ratio_limit,
        0.0,
        0.25,
    )
    duty_ratio_recruitment =
        guidance.redirect_weight * abs(response_direction)
    anterior_restoring_scale = 1 -
        duty_ratio_limit *
        duty_ratio_recruitment *
        response_direction *
        anterior_phase_side
    ...
    anterior_accel =
        vdp_drive - anterior_restoring_scale * omega^2 * centered_q1
