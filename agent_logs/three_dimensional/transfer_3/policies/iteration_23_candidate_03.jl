# Phase-2 candidate: preserve the actuator-consistent phase route and use
# hydrodynamic yaw moment to anticipate the fast beat-side response.
...
function target_policy_params()
    return (
        ...
        yaw_moment_response_scale=0.0070,
        yaw_moment_response_limit=0.12,
...
function target_policy(state, params)
    ...
    # The normalized moment strongly predicts the next yaw-rate increment in
    # the sampled traces. Use a bounded prospective response rather than
    # switching phase authority off: helpful moment starts braking early and
    # adverse moment strengthens correction. Both moment and yaw error reverse
    # under reflection, so the anticipated response remains odd.
    hydrodynamic_yaw_moment =
        _finite_or(state.moment_z_L2, 0.0)
    anticipated_yaw_rate_increment =
        params.yaw_moment_response_limit *
        tanh(
            hydrodynamic_yaw_moment /
            max(params.yaw_moment_response_scale, 1.0e-6),
        )
    anticipated_yaw_rate_error =
        guidance.yaw_rate_error - anticipated_yaw_rate_increment
    ...
    # Preserve the fastest sampled policy's stacked amplitude and phase path.
    # The physical-response forecast changes only its fast steering direction;
    # it does not remove authority during a helping-moment interval.
