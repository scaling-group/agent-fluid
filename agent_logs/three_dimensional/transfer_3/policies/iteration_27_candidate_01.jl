...
# let carrier-demodulated helpful moment relieve only redundant half-cycle effort.

function target_policy_params()
    return (
        version="dogfish3d_demodulated_amplitude_relief_v1",
        control_period=0.55,
        ...
        fluid_residual_amplitude_fraction=0.20,
...
function target_policy(state, params)
    ...
    # Preserve the replicated phase path. Raw moment is carrier dominated, so
    # subtract its sampled reflection-odd joint-phase estimate before using
    # the remaining fluid response. A residual aligned with the requested yaw
    # may relieve only bounded half-cycle authority after yaw-error closure;
    # opposing residuals cannot amplify this redundant channel.
    ...
    helpful_residual_alignment = max(
        response_direction * normalized_residual_response,
        0.0,
    )
    correction_satisfaction = 1 - abs(response_direction)
    residual_amplitude_release =
        helpful_residual_alignment * correction_satisfaction
    residual_amplitude_fraction = clamp(
        params.fluid_residual_amplitude_fraction,
        0.0,
        0.5,
    )
    amplitude_authority_fraction =
        1 - residual_amplitude_fraction * residual_amplitude_release
