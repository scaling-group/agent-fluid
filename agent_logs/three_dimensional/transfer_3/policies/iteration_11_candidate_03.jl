# Phase-2 candidate: terminal whole-body allocation on the sampled distributed
# C-bend. Far-field propulsion and LOS-rate release are preserved; normalized
# range shares an established redirect with the posterior joint near capture.

function target_policy_params()
    return (
        version="dogfish3d_terminal_whole_body_cbend_v1",
        control_period=0.55,
        ...
        terminal_allocation_distance_L=5.0,
        terminal_allocation_width_L=0.8,
        posterior_redirect_share=1.0,
    )
end
...
function target_guidance(state, qd1, qd2, params)
    ...
    # Recruit an anterior mean bend only for a large observed route error.
    # The sign follows the bounded route-yaw request, and the smooth bearing
    # gate returns continuously to the unshifted carrier after alignment.
    ...
    # Once the evidenced C-bend enters terminal range, share its existing
    # redirect with the posterior equilibrium. This adds steering allocation,
    # not range-only drive relief, and vanishes with the route-error command.
    terminal_allocation_weight = 0.5 * (1 + tanh(
        (params.terminal_allocation_distance_L - distance) /
        max(params.terminal_allocation_width_L, 1.0e-6),
    ))
    posterior_redirect =
        params.posterior_redirect_share *
        terminal_allocation_weight *
        anterior_mean_curvature
    ...
    return (
        ...
        terminal_allocation_weight=terminal_allocation_weight,
        posterior_redirect=posterior_redirect,
    )
end
...
function target_policy(state, params)
    ...
    # Centering the carrier state preserves its sampled traveling-wave shape.
    ...
    posterior_target =
        guidance.mean_tail_curvature + guidance.posterior_redirect -
        centered_q1 -
