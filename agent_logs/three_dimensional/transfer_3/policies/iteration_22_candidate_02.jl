...
# amplitude steering only when phase supplies an aligned posterior response.

function target_policy_params()
    return (
        version="dogfish3d_aligned_phase_substitution_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Allocate in posterior joint-target space. Phase releases amplitude only
    # when its signed displacement from the carrier supplies the same response
    # as the stacked half-cycle increment. This retains full amplitude at zero
    # phase leverage or under opposed contributions, without a new gain.
    modulated_tail_wave_phase = tanh(
        phase_modulated_wave_target /
        max(params.oscillator_amplitude, 1.0e-6),
    )
    stacked_amplitude_increment =
        asymmetry_limit *
        response_direction *
        modulated_tail_wave_phase *
        phase_modulated_wave_target
    phase_substitution_increment =
        phase_modulated_wave_target - baseline_posterior_wave_target
    aligned_phase_supply = max(
        phase_substitution_increment * stacked_amplitude_increment,
        0.0,
    )
    amplitude_increment_energy = stacked_amplitude_increment^2
    phase_substitution_fraction = clamp(
        aligned_phase_supply /
        max(amplitude_increment_energy, eps(Float64)),
        0.0,
        1.0,
    )
    retained_amplitude_increment =
        (1 - phase_substitution_fraction) *
        stacked_amplitude_increment
    posterior_target =
        guidance.mean_tail_curvature +
        phase_modulated_wave_target +
