...
# let only helpful carrier-demodulated response relieve redundant amplitude.
...
function target_policy(state, params)
    ...
    # Preserve the primary phase actuator. Remove the sampled joint-phase
    # carrier from normalized yaw moment, then let only a helpful residual
    # relieve redundant half-cycle asymmetry as the requested correction
    # closes. Opposing residual response cannot recruit or withdraw the route
    # actuator. Each signed signal reverses under reflection, so its alignment
    # and the resulting allocation remain invariant.
    ...
    helpful_residual_release = tanh(
        helpful_residual_alignment /
        max(params.fluid_moment_residual_scale, 1.0e-6),
    )
    correction_satisfaction = 1 - abs(response_direction)
    satisfied_helpful_residual_release =
        helpful_residual_release * correction_satisfaction
    residual_asymmetry_limit = asymmetry_limit *
        (1 - satisfied_helpful_residual_release)
