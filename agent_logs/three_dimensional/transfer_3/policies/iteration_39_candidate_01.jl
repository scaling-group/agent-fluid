...
# only outward posterior acceleration as measured joint-speed headroom closes.

function target_policy_params()
    return (
        version="dogfish3d_posterior_speed_headroom_tail_phase_v1",
        control_period=0.55,
        ...
        posterior_speed_guard_start_fraction=0.95,
...
@inline function _speed_headroom_scale(
    joint_velocity,
    velocity_limit,
    guard_start_fraction,
)
    feasible_velocity_limit = max(velocity_limit, eps(Float64))
    bounded_start_fraction = clamp(
        guard_start_fraction,
        0.0,
        1.0 - eps(Float64),
    )
    guard_start_speed =
        bounded_start_fraction * feasible_velocity_limit
    normalized_headroom = clamp(
        (feasible_velocity_limit - abs(joint_velocity)) /
        max(feasible_velocity_limit - guard_start_speed, eps(Float64)),
        0.0,
        1.0,
    )
    # Smoothstep avoids a command jump at guard recruitment while reaching
    # exactly zero outward authority at the physical speed boundary.
    return normalized_headroom^2 * (3 - 2 * normalized_headroom)
end
...
function target_policy(state, params)
    ...
    posterior_speed_headroom_scale = _speed_headroom_scale(
        qd2,
        velocity_limit,
        params.posterior_speed_guard_start_fraction,
    )
    ...
    # The feasible return is deliberately tapered near the speed boundary.
    # Keep the established phase gate tied to persistent raw demand rather
    # than mistaking that projection for lost demand. Opposite-sign previous
    # action still marks a beat reversal and is never replaced.
    outward_speed_guard_active =
        posterior_speed_headroom_scale < 1.0 &&
        baseline_posterior_accel * qd2 > 0.0
    same_side_previous_demand =
        observed_previous_posterior_action *
        baseline_posterior_accel >= 0.0
    projected_persistence_witness =
        outward_speed_guard_active && same_side_previous_demand
    previous_posterior_action = projected_persistence_witness ?
        clamp(
            baseline_posterior_accel,
            -acceleration_limit,
            acceleration_limit,
    ...
    # Spend only the measured posterior speed headroom. Same-direction
    # acceleration tapers smoothly over the final part of the envelope; full
    # reverse acceleration remains available for braking and phase reversal.
    # Under lateral reflection qd2 and acceleration both reverse, so the gate
    # is invariant and the returned command remains odd.
    posterior_accelerating_outward =
        feasible_posterior_accel * qd2 > 0.0
    speed_guarded_posterior_accel = posterior_accelerating_outward ?
        posterior_speed_headroom_scale * feasible_posterior_accel :
        feasible_posterior_accel

    return (
        phi_ddot=(
            feasible_anterior_accel,
            speed_guarded_posterior_accel,
        ),
    )
end
