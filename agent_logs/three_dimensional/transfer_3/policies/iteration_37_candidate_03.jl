# Phase-2 candidate: preserve the actuator-consistent phase route and block
# only acceleration that would wind farther into an active joint-rate limit.

function target_policy_params()
    return (
        version="dogfish3d_joint_rate_antiwindup_tail_phase_v1",
        control_period=0.55,
...
@inline function _block_outward_at_speed_limit(
    acceleration,
    joint_velocity,
    velocity_limit,
)
    feasible_velocity_limit = max(velocity_limit, eps(Float64))
    at_speed_limit = abs(joint_velocity) >= feasible_velocity_limit
    accelerating_outward = acceleration * joint_velocity > 0
    return at_speed_limit && accelerating_outward ? 0.0 : acceleration
end
...
function target_policy(state, params)
    ...
    # Anti-windup returns zero while speed saturation makes outward action
    # infeasible. Preserve the evaluated persistence semantics by treating an
    # ongoing same-side raw demand at that exact boundary as the missing
    # feasible-action witness. When demand reverses, the witness disappears
    # and braking/phase release remain immediate.
    ...
    # Do not wind the command farther into the observed hard speed boundary.
    # Blocking is one-sided: reverse acceleration remains fully available to
    # brake and preserve the traveling-wave phase. Under lateral reflection
    # velocity and acceleration both reverse, so the gate is invariant and its
    # output remains odd.
    antiwindup_anterior_accel = _block_outward_at_speed_limit(
        feasible_anterior_accel,
        qd1,
        velocity_limit,
    )
    antiwindup_posterior_accel = _block_outward_at_speed_limit(
