# Phase-2 candidate: preserve the actuator-consistent traveling route while
# removing sampled joint-phase recoil from the slow body-frame bearing signal.

function target_policy_params()
    return (
        version="dogfish3d_phase_conditioned_route_bearing_v1",
        control_period=0.55,
        ...
        bearing_carrier_q1_gain=-0.36044,
        bearing_carrier_q2_gain=-0.03758,
        bearing_carrier_qd1_gain=-0.12342,
        bearing_carrier_qd2_gain=0.17536,
...
function target_guidance(state, q1, q2, qd1, qd2, params)
    ...
    # The instantaneous body bearing contains a repeatable carrier-frequency
    # component because the two-joint bend recoils the free body. Remove the
    # sampled joint-state projection before the slow route loop sees it. The
    # correction and bearing both reverse under lateral reflection.
    omega = 2 * pi / params.control_period
    normalized_qd1 = qd1 / max(omega, eps(Float64))
    normalized_qd2 = qd2 / max(omega, eps(Float64))
    bearing_carrier_projection =
        params.bearing_carrier_q1_gain * q1 +
        params.bearing_carrier_q2_gain * q2 +
        params.bearing_carrier_qd1_gain * normalized_qd1 +
        params.bearing_carrier_qd2_gain * normalized_qd2
    route_bearing = bearing - bearing_carrier_projection
    ...
    bearing_yaw_rate =
        ...
        tanh(route_bearing / max(params.bearing_error_scale, 1.0e-6))
    response_yaw_rate =
    ...
    bearing_redirect_weight = 0.5 * (1 + tanh(
        (abs(route_bearing) - params.redirect_bearing_threshold) /
    ...
    return (
        bearing=bearing,
        bearing_carrier_projection=bearing_carrier_projection,
