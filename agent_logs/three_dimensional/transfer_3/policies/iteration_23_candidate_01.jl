# Phase-2 candidate: preserve the actuator-consistent route and release
# posterior phase steering when hydrodynamic yaw moment already helps.

function target_policy_params()
    return (
        version="dogfish3d_hydrodynamic_opposition_tail_phase_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # The normalized hydrodynamic yaw moment is an observed physical response,
    # not an actuator-demand proxy. Retain the sampled phase timing while the
    # moment is adverse or neutral, and release it only when the moment already
    # supplies the signed yaw correction. The response/moment product is even
    # under reflection because both factors reverse.
    ...
    hydrodynamic_opposition_gate = 1 - helping_moment_release
    phase_activation =
        predicted_phase_activation *
        actuator_consistency *
        hydrodynamic_opposition_gate
