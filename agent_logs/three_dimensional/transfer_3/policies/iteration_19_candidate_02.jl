# Phase-2 candidate: preserve the replicated response-reversing posterior
# half-cycle, and rotate its wave phase only when actuator history shows that
# the posterior acceleration waveform is being clipped.

function target_policy_params()
    return (
        version="dogfish3d_saturation_gated_tail_phase_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Rotate the baseline posterior coefficients without changing their norm.
    # The yaw-error/phase-gradient product is invariant under reflection. A
    # C1 smoothstep makes this timing response exactly zero below the observed
    # clipping neighborhood and fully active at the owned physical bound.
    phase_response = tanh(
    ...
    normalized_posterior_action = clamp(
        abs(previous_posterior_action) /
        max(params.joint_acceleration_limit, eps(Float64)),
        0.0,
        1.0,
    )
    activation_start = clamp(
        params.tail_phase_activation_fraction,
        0.0,
        1.0 - eps(Float64),
    )
    activation_coordinate = clamp(
        (normalized_posterior_action - activation_start) /
        max(1.0 - activation_start, eps(Float64)),
        0.0,
        1.0,
    )
    phase_activation =
        activation_coordinate^2 * (3 - 2 * activation_coordinate)
    phase_limit = clamp(
    ...
    # Retain the replicated response-reversing half-cycle mechanism. Under
    # reflection both yaw error and wave target reverse, so the scale is
    # invariant while the scaled traveling wave remains odd.
    response_direction = phase_response
    tail_wave_phase = tanh(
        posterior_wave_target /
        max(params.oscillator_amplitude, 1.0e-6),
    )
    ...
    tail_wave_scale = 1 +
        asymmetry_limit * response_direction * tail_wave_phase
    posterior_target =
        guidance.mean_tail_curvature +
        tail_wave_scale * posterior_wave_target
    posterior_accel =
