# Phase-2 candidate: keep the response-reversing posterior wave, then conserve
# slow curvature while moving compatible yaw correction forward only under a
# large observed route demand.

function target_policy_params()
    return (
        version="dogfish3d_demand_allocated_response_reversing_tail_v1",
        control_period=0.55,
...
function target_guidance(state, qd1, qd2, params)
    ...
    # Move only same-sign slow curvature forward when the body-frame LOS
    # response request is large. Adding and subtracting the same amount keeps
    # total slow bend closed; opposing posterior correction remains intact.
    ...
    demand_gated_allocation_weight =
        response_redirect_weight * max(0.0, tanh(curvature_coherence))
    allocated_mean_curvature =
        demand_gated_allocation_weight * mean_tail_curvature
    ...
    return (
        ...
        demand_gated_allocation_weight=demand_gated_allocation_weight,
...
function target_policy(state, params)
    ...
    posterior_target =
        guidance.posterior_mean_curvature +
