# Phase-2 candidate: preserve the actuator-consistent phase route and relieve
# redundant half-cycle amplitude only under persistent actuator stress.

function target_policy_params()
    return (
        version="dogfish3d_stress_gated_moment_relief_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # requested recoil-conditioned yaw response is near closure and predicted
    # demand remains on the same actuator side as the previous feasible
    # action. It never suppresses the carrier or phase actuator. Both yaw
    # error and moment reverse under reflection, so their alignment and this
    # allocation are invariant.
    ...
    stress_gated_helping_release =
        phase_activation * satisfied_helping_release
    amplitude_authority_fraction = 1 - stress_gated_helping_release
