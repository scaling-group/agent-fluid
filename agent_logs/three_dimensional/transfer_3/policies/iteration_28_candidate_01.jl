...
# recruit residual authority only against opposing carrier-demodulated response.

function target_policy_params()
    return (
        version="dogfish3d_opposition_only_moment_residual_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # as a small one-sided allocator. An opposing residual recruits phase;
    # helpful response leaves the established route actuator intact instead of
    # withdrawing it. The product of odd response and residual directions is
    # reflection invariant, and clipping it preserves that invariance.
    ...
    opposing_residual_recruitment = max(
        residual_opposition,
        0.0,
    )
    ...
    residual_phase_multiplier =
        1 + residual_phase_fraction * opposing_residual_recruitment
