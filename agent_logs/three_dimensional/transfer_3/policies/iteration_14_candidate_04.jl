# Phase-2 candidate: preserve the completed LOS-rate C-bend while moving only
# same-sign slow curvature forward without changing its total route bend.

function target_policy_params()
    return (
        version="dogfish3d_sign_coherent_conservative_allocation_v1",
        control_period=0.55,
...
function target_guidance(state, qd1, qd2, params)
    ...
    # The inherited gate-only posterior relief lost capture because an active
    # anterior gate does not prove that the posterior yaw correction is
    # redundant. Allocate only when the two normalized slow bends agree in
    # sign, and conserve their sum. Opposite-sign correction remains entirely
    # posterior; the phase-lagged carrier below is not part of this allocation.
    ...
    anterior_allocation_weight = max(0.0, tanh(curvature_coherence))
    allocated_mean_curvature =
        anterior_allocation_weight * mean_tail_curvature
    ...
    return (
        ...
        anterior_allocation_weight=anterior_allocation_weight,
