# Phase-2 candidate: preserve actuator-consistent posterior phase steering and
# project only infeasible outward acceleration at the hard joint-rate boundary.

function target_policy_params()
    return (
        version="dogfish3d_joint_rate_antiwindup_v1",
        control_period=0.55,
...
@inline function _project_outward_rate_command(accel, velocity, velocity_limit)
    at_positive_limit = velocity >= velocity_limit
    at_negative_limit = velocity <= -velocity_limit
    drives_outward =
        (at_positive_limit && accel > 0) ||
        (at_negative_limit && accel < 0)
    return drives_outward ? 0.0 : accel
end
...
function target_policy(state, params)
    ...
    normalized_previous_stress = clamp(
        previous_posterior_action / acceleration_limit,
        -1.0,
        1.0,
    )

    # A projected zero at the hard speed boundary is not evidence that the
    # persistent raw demand vanished. Use continued outward demand there as
    # the causal witness until an inward braking command begins.
    velocity_limit = max(
        params.joint_velocity_limit,
        eps(Float64),
    )
    posterior_at_rate_limit = abs(qd2) >= velocity_limit
    posterior_demand_continues_outward =
        posterior_at_rate_limit && qd2 * baseline_posterior_accel > 0
    normalized_persistence_witness =
        posterior_demand_continues_outward ?
        normalized_predicted_stress : normalized_previous_stress
    actuator_consistency = clamp(
        normalized_predicted_stress * normalized_persistence_witness,
        0.0,
        1.0,
    )
    phase_activation =
    ...
    # The episode applies componentwise acceleration and speed limits. First
    # clip acceleration, then suppress only commands that push farther through
    # an observed hard speed boundary; reverse braking remains untouched.
    bounded_anterior_accel = clamp(
        anterior_accel,
        -acceleration_limit,
        acceleration_limit,
    )
    bounded_posterior_accel = clamp(
        posterior_accel,
        -acceleration_limit,
        acceleration_limit,
    )
    feasible_anterior_accel = _project_outward_rate_command(
        bounded_anterior_accel,
        qd1,
        velocity_limit,
    )
    feasible_posterior_accel = _project_outward_rate_command(
        bounded_posterior_accel,
        qd2,
        velocity_limit,
    )
