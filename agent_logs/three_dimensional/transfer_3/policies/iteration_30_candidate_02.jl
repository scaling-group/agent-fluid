...
# the anterior C-bend when recoil-conditioned yaw response meets demand.

function target_policy_params()
    return (
        version="dogfish3d_response_releasing_c_bend_v1",
        control_period=0.55,
...
function target_guidance(state, qd1, qd2, params)
    ...
    # C-start-like response release: geometry recruits the redirect, while
    # the unresolved yaw response sets its signed curvature. Unlike a held
    # demand bend, this unloads or reverses once measured response catches up.
...
function target_policy(state, params)
    ...
    # Preserve the sampled actuator-consistent phase path and its reversible
    # half-cycle response. The architecture deliberately adds no moment or
    # range allocator: the only new mechanism is anterior response release.
