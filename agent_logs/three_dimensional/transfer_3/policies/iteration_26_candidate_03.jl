# Phase-2 candidate: demodulate joint-phase recoil from the slow route bearing
# while preserving the actuator-consistent phase and moment-residual paths.

function target_policy_params()
    return (
        version="dogfish3d_recoil_conditioned_bearing_v1",
        control_period=0.55,
...
function target_guidance(state, q1, q2, qd1, qd2, params)
    ...
    # The same sampled projection that removes joint-correlated recoil from
    # yaw rate has an angle-level counterpart. Remove that beat-scale body
    # recoil before interpreting target geometry as a slow route error. The
    # correction and bearing both reverse under reflection.
