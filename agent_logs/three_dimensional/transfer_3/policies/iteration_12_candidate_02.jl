# Phase-2 candidate: preserve the captured LOS-rate traveling bend, but release
# supplemental distributed C-bend authority when observed yaw meets demand.

function target_policy_params()
    return (
        version="dogfish3d_response_released_cbend_v2",
        control_period=0.55,
        ...
        redirect_response_deficit_threshold=0.15,
        redirect_response_deficit_width=0.10,
...
function target_guidance(state, qd1, qd2, params)
    ...
    # Retain the evaluated large-bearing recruitment. The supplemental path is
    # now closed around directional response: recruit when phase-conditioned
    # yaw lags or opposes demand, and release when it meets or outruns demand.
    # Request weighting prevents a large residual at nearly zero route demand
    # from shifting the oscillator center.
    bearing_redirect_weight = 0.5 * (1 + tanh(
        (abs(bearing) - params.redirect_bearing_threshold) /
        max(params.redirect_bearing_width, 1.0e-6),
    ))
    request_direction = tanh(
        target_yaw_rate /
        max(params.redirect_yaw_rate_scale, 1.0e-6),
    )
    directional_response_deficit = request_direction * yaw_rate_error
    response_request_weight = abs(request_direction)
    response_deficit_weight = 0.5 * (1 + tanh(
        (directional_response_deficit -
         params.redirect_response_deficit_threshold) /
        max(params.redirect_response_deficit_width, 1.0e-6),
    ))
    response_redirect_weight =
        response_request_weight * response_deficit_weight
    redirect_weight = max(
    ...
    return (
        ...
        directional_response_deficit=directional_response_deficit,
        response_request_weight=response_request_weight,
        response_deficit_weight=response_deficit_weight,
