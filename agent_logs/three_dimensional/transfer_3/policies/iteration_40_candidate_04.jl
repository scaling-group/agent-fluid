...
# only posterior acceleration that would wind farther into its joint-rate limit.

function target_policy_params()
    return (
        version="dogfish3d_posterior_rate_antiwindup_tail_phase_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # A projected zero at the posterior speed boundary should not erase the
    # established persistent-demand witness. Continued raw outward demand
    # supplies that witness only until the requested acceleration reverses.
    ...
    # Replicated evidence separates feasible-command cleanup only posteriorly.
    # Suppress outward acceleration at that observed hard speed boundary while
    # retaining reverse braking and leaving the anterior carrier untouched.
    # Under reflection velocity and acceleration reverse together, so the gate
    # is invariant and its output remains odd.
    ...
    return (
        phi_ddot=(feasible_anterior_accel, antiwindup_posterior_accel),
    )
end
