# Phase-2 candidate: bearing-triggered distributed C-bend on the coherent
# line-of-sight-rate controller. Large body-frame route error recruits an
# anterior mean bend while the posterior traveling wave remains active.

function target_policy_params()
    return (
        version="dogfish3d_los_rate_distributed_cbend_v1",
        control_period=0.55,
...
function target_guidance(state, qd1, qd2, params)
    ...
    # Recruit a distributed mean bend only after a large body-frame route
    # error appears. The sign follows the bounded route-yaw request, while the
    # smooth bearing gate returns to the unshifted carrier after alignment.
