# Phase-2 candidate: preserve the actuator-consistent route and dissipate only
# surplus anterior carrier motion during an already-fast near approach.

function target_policy_params()
    return (
        version="dogfish3d_radial_approach_carrier_damping_v1",
        control_period=0.55,
        ...
        approach_range_L=2.40,
        approach_range_width_L=0.40,
        approach_closing_speed_U=0.68,
        approach_closing_speed_width_U=0.08,
        approach_carrier_damping_ratio=0.10,
...
function target_guidance(state, qd1, qd2, params)
    ...
    # Positive radial speed closes on a stationary target. The target-forward
    # factor prevents approach braking after a pass. These rotation-invariant
    # weights gate carrier damping only; no route or steering term is released.
    closing_speed = (
        target_x * velocity_x + target_y * velocity_y
    ) / target_norm
    approach_range_weight = 0.5 * (1 + tanh(
        (params.approach_range_L - distance) /
        max(params.approach_range_width_L, 1.0e-6),
    ))
    approach_closing_weight = 0.5 * (1 + tanh(
        (closing_speed - params.approach_closing_speed_U) /
        max(params.approach_closing_speed_width_U, 1.0e-6),
    ))
    approach_forward_weight = clamp(target_forward, 0.0, 1.0)
    approach_damping_weight =
        approach_range_weight *
        approach_closing_weight *
        approach_forward_weight
    ...
    return (
        ...
        approach_range_weight=approach_range_weight,
        approach_closing_weight=approach_closing_weight,
        approach_forward_weight=approach_forward_weight,
        approach_damping_weight=approach_damping_weight,
        los_rate=los_rate,
...
function target_policy(state, params)
    ...
    # The evaluated amplitude-envelope edit acted only through the weak Van
    # der Pol injection. Apply the terminal mechanism directly as a bounded
    # viscous sink instead. The route-center stiffness remains unchanged, so
    # the equilibrium steering request is retained while carrier energy falls.
    approach_damping_ratio = clamp(
        params.approach_carrier_damping_ratio,
        0.0,
        0.25,
    )
    approach_carrier_damping =
        2 * approach_damping_ratio * omega *
        guidance.approach_damping_weight * qd1
    anterior_accel =
        vdp_drive - omega^2 * centered_q1 - approach_carrier_damping
