# Phase-2 candidate: line-of-sight-response steering with opposed two-joint
# mean-bend allocation. The evidenced carrier and route signal are preserved;
# a small anterior share gives both joint-coordinate polarities the same yaw
# effect instead of relying on the frequently clipped posterior alone.

function target_policy_params()
    return (
        version="dogfish3d_los_response_opposed_bend_v1",
        control_period=0.55,
...
function target_guidance(state, qd1, qd2, params)
    ...
    # Positive posterior mean curvature and negative anterior center have the
    # same sampled yaw polarity. Split one bounded request between those
    # opposed joint coordinates; at zero request the carrier is unchanged.
    anterior_center =
        -params.anterior_steering_share * mean_tail_curvature
    posterior_center =
        (1.0 - params.anterior_steering_share) * mean_tail_curvature
...
function target_policy(state, params)
    ...
    # Center a small share of the same response-led steering request on the
    # anterior oscillator. The unsteered limit is exactly the sampled carrier.
