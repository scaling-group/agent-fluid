...
# let carrier-demodulated yaw response relieve only residual half-cycle effort.

function target_policy_params()
    return (
        version="dogfish3d_demodulated_moment_asymmetry_relief_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # The sampled raw moment is dominated by the traveling-bend carrier.
    # Subtract its reflection-odd joint-phase estimate before asking whether
    # fluid response makes residual half-cycle authority redundant. This
    # signal cannot alter the primary phase recruitment above.
    ...
    # Preserve the primary phase actuator. When the demodulated fluid residual
    # helps an already-closing yaw response, relieve only half-cycle amplitude
    # and only in proportion to existing phase recruitment. Response direction
    # and residual moment are both odd under reflection, so their alignment and
    # every allocation weight are invariant.
    modulated_tail_wave_phase = tanh(
        phase_modulated_wave_target /
        max(params.oscillator_amplitude, 1.0e-6),
    )
    residual_response_alignment =
        response_direction * fluid_moment_residual
    helping_residual_release = tanh(
        max(residual_response_alignment, 0.0) /
        max(params.fluid_moment_residual_scale, 1.0e-6),
    )
    correction_satisfaction = 1 - abs(response_direction)
    demodulated_residual_release =
        phase_activation *
        correction_satisfaction *
        helping_residual_release
    residual_asymmetry_limit =
        asymmetry_limit * (1 - demodulated_residual_release)
