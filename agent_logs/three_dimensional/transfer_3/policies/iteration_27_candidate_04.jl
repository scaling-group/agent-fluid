# Phase-2 candidate: preserve raw-bearing route ownership and use joint phase
# only to confirm transient anterior redirect recruitment.

function target_policy_params()
    return (
        version="dogfish3d_phase_consensus_redirect_v1",
        control_period=0.55,
...
function target_guidance(state, q1, q2, qd1, qd2, params)
    ...
    # A fixed joint-angle correction distorted the completed broad route when
    # it replaced target geometry. Keep raw bearing as the slow request and
    # use its phase-conditioned counterpart only as redirect confidence.
    bearing_recoil_correction =
        params.yaw_recoil_q1_gain * q1 +
        params.yaw_recoil_q2_gain * q2
    unwrapped_conditioned_bearing =
        raw_bearing + bearing_recoil_correction
    conditioned_bearing = atan(
        sin(unwrapped_conditioned_bearing),
        cos(unwrapped_conditioned_bearing),
    )
    bearing = raw_bearing
    ...
    raw_bearing_redirect_weight = 0.5 * (1 + tanh(
        (abs(raw_bearing) - params.redirect_bearing_threshold) /
        max(params.redirect_bearing_width, 1.0e-6),
    ))
    conditioned_bearing_redirect_weight = 0.5 * (1 + tanh(
        (abs(conditioned_bearing) - params.redirect_bearing_threshold) /
        max(params.redirect_bearing_width, 1.0e-6),
    ))

    # The positive product is reflection-invariant and exists only when raw
    # target geometry and the gait-conditioned view agree on turn side. Its
    # square root is their geometric-mean magnitude, so neither observation
    # can recruit the burst alone. The LOS response remains a continuous
    # fallback when this phase consensus releases.
    consensus_bearing_magnitude = sqrt(max(
        raw_bearing * conditioned_bearing,
        0.0,
    ))
    bearing_redirect_weight = 0.5 * (1 + tanh(
        (consensus_bearing_magnitude - params.redirect_bearing_threshold) /
        max(params.redirect_bearing_width, 1.0e-6),
    ))
    ...
    return (
        ...
        bearing_recoil_correction=bearing_recoil_correction,
        conditioned_bearing=conditioned_bearing,
        bearing=bearing,
        ...
        raw_bearing_redirect_weight=raw_bearing_redirect_weight,
        conditioned_bearing_redirect_weight=conditioned_bearing_redirect_weight,
        consensus_bearing_magnitude=consensus_bearing_magnitude,
...
function target_policy(state, params)
    ...
    # Preserve the sampled actuator-consistent phase path. A helping physical
    # yaw moment may relieve only the residual half-cycle amplitude when the
    # requested recoil-conditioned yaw response is near closure; it never
    # suppresses the carrier or phase actuator. Both yaw error and moment
    # reverse under reflection, so their alignment and this allocation are
    # invariant.
    ...
    correction_satisfaction = 1 - abs(response_direction)
    satisfied_helping_release =
        helping_moment_release * correction_satisfaction
    amplitude_authority_fraction = 1 - satisfied_helping_release
    recruited_asymmetry_limit =
        asymmetry_limit * amplitude_authority_fraction
    modulated_tail_wave_scale = 1 +
        recruited_asymmetry_limit *
        response_direction *
        modulated_tail_wave_phase
    posterior_target =
        guidance.mean_tail_curvature +
