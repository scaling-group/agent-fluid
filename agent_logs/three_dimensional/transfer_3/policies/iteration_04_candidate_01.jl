# Phase-2 candidate: a coherent joint-state traveling bend with
# response-released, sign-preserving posterior mean curvature.

function target_policy_params()
    return (
        version="dogfish3d_response_released_curvature_v1",
        control_period=0.55,
...
function target_guidance(state, params)
    ...
    # The available bearing trend spans much less than a tail beat, so it is
    # not a reliable predicted angle or yaw-rate target. It may only release
    # steering while the geometric LOS magnitude is already closing. This
    # gate cannot reverse the curvature sign before LOS itself crosses zero.
    ...
    safe_rate_limit = max(params.bearing_rate_limit, 1.0e-6)
    bounded_bearing_rate =
        safe_rate_limit * tanh(bearing_rate / safe_rate_limit)
    alignment_rate = max(-sign(los_error) * bounded_bearing_rate, 0.0)
    alignment_release = 1.0 - tanh(
        alignment_rate / max(params.alignment_rate_scale, 1.0e-6),
    )
    ...
    mean_tail_curvature =
        params.mean_tail_curvature_limit * turn_command * alignment_release
    return (
        los_error=los_error,
        bearing_rate=bearing_rate,
        bounded_bearing_rate=bounded_bearing_rate,
        alignment_rate=alignment_rate,
        alignment_release=alignment_release,
