# Phase-2 candidate: preserve phase steering and relieve only the strengthened
# posterior half-cycle when yaw moment and measured response confirm the route.

function target_policy_params()
    return (
        version="dogfish3d_route_confirmed_phase_selective_relief_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Preserve the sampled actuator-consistent phase path. Classify physical
    # yaw moment against the slow route request rather than the fast error
    # sign, and require measured recoil-conditioned yaw to be progressing in
    # that route direction near response closure. Each signed quantity
    # reverses under reflection, so all three confirmation gates are invariant.
    modulated_tail_wave_phase = tanh(
        phase_modulated_wave_target /
        max(params.oscillator_amplitude, 1.0e-6),
    )
    route_direction = tanh(
        guidance.target_yaw_rate /
        max(params.yaw_rate_error_scale, 1.0e-6),
    )
    measured_response_direction = tanh(
        guidance.phase_conditioned_yaw_rate /
        max(params.yaw_rate_error_scale, 1.0e-6),
    )
    route_response_progress = clamp(
        route_direction * measured_response_direction,
        0.0,
        1.0,
    )
    hydrodynamic_yaw_moment =
        _finite_or(state.moment_z_L2, 0.0)
    helping_route_moment = max(
        route_direction * hydrodynamic_yaw_moment,
        0.0,
    )
    helping_moment_release = tanh(
        helping_route_moment /
        max(params.helpful_moment_release_scale, 1.0e-6),
    )
    correction_satisfaction = 1 - abs(response_direction)
    confirmed_helping_release =
        helping_moment_release *
        route_response_progress *
        correction_satisfaction

    # Relieve only positive amplification on the useful tail half-cycle.
    # Keeping the negative branch unchanged avoids filling in the attenuated
    # half-cycle and retains a bounded response-reversing asymmetry.
    directed_tail_wave_phase =
        response_direction * modulated_tail_wave_phase
    amplified_tail_half_cycle = max(directed_tail_wave_phase, 0.0)
    attenuated_tail_half_cycle = min(directed_tail_wave_phase, 0.0)
    effective_directed_tail_wave_phase =
        (1 - confirmed_helping_release) * amplified_tail_half_cycle +
        attenuated_tail_half_cycle
    modulated_tail_wave_scale = 1 +
        asymmetry_limit * effective_directed_tail_wave_phase
    posterior_target =
        guidance.mean_tail_curvature +
