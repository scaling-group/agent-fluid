# Phase-2 candidate: preserve the replicated response-reversing half-cycle
# route and lead posterior phase recruitment on rising actuator-demand flanks.

function target_policy_params()
    return (
        version="dogfish3d_demand_lead_tail_phase_v1",
        control_period=0.55,
        ...
        tail_phase_prediction_gain=0.50,
...
function target_policy(state, params)
    ...
    # A one-step secant lead from the previous feasible action anticipates only
    # a rising demand flank: max magnitude retains current demand on a falling
    # flank. Absolute demand and this selection are reflection-invariant.
    ...
    previous_posterior_action =
        _finite_or(state.previous_action[2], baseline_posterior_accel)
    demand_trend =
        baseline_posterior_accel - previous_posterior_action
    prediction_gain = clamp(
        params.tail_phase_prediction_gain,
        0.0,
        1.0,
    )
    projected_posterior_accel =
        baseline_posterior_accel + prediction_gain * demand_trend
    recruited_posterior_demand = max(
        abs(baseline_posterior_accel),
        abs(projected_posterior_accel),
    )
    predicted_acceleration_fraction =
        recruited_posterior_demand / acceleration_limit
