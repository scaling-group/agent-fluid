# Phase-2 candidate: preserve the actuator-consistent phase route and hand
# overlapping steering authority by observed posterior beat side.

function target_policy_params()
    return (
        version="dogfish3d_side_selective_phase_handoff_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Recruit phase timing only as the evaluated amplitude path approaches the
    # posterior constraint. Absolute normalized demand is reflection-invariant.
    ...
    # product is positive only when raw demand remains on the same actuator
    # side as the observed previous feasible action; it is invariant because
    # both accelerations reverse under reflection.
    ...
    # Arbitrate overlapping fast steering only on the aligned stress flank.
    # When carrier acceleration already points with the requested response,
    # recruited phase releases amplitude stacking that would otherwise clip.
    # On the opposing flank, amplitude remains available to reduce opposition.
    # The stress/response product is invariant when reflection reverses both.
    modulated_tail_wave_phase = tanh(
        phase_modulated_wave_target /
        max(params.oscillator_amplitude, 1.0e-6),
    )
    aligned_steering_stress = clamp(
        normalized_predicted_stress * response_direction,
        0.0,
        1.0,
    )
    amplitude_handoff =
        phase_activation * aligned_steering_stress
    recruited_asymmetry_limit =
        asymmetry_limit * (1 - amplitude_handoff)
