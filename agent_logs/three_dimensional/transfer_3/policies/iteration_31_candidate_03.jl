# Phase-2 candidate: arbitrate between response-reversing half-cycle amplitude
# and coefficient-norm-preserving phase steering under persistent actuator stress.

function target_policy_params()
    return (
        version="dogfish3d_stress_arbitrated_tail_phase_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Arbitrate rather than stack the two rhythmic steering paths. At zero
    # phase recruitment this is exactly the replicated half-cycle path. As
    # persistent same-side stress recruits coefficient-norm-preserving phase
    # rotation, withdraw the same fraction of amplitude asymmetry; at full
    # recruitment the phase-steered carrier has no extra amplitude multiplier.
    modulated_tail_wave_phase = tanh(
        phase_modulated_wave_target /
        max(params.oscillator_amplitude, 1.0e-6),
    )
    amplitude_authority = 1 - phase_activation
