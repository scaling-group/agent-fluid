# Phase-2 candidate: preserve actuator-consistent posterior phase timing while
# making the response-reversing half-cycle modulation energy-neutral.

function target_policy_params()
    return (
        version="dogfish3d_energy_neutral_asymmetry_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Equal and opposite response/phase products retain the same strong-to-weak
    # ratio as `1 + u`, but this normalization makes their mean squared scale
    # exactly one. Thus steering contrast does not inflate carrier intensity.
    baseline_tail_wave_modulation =
        asymmetry_limit * response_direction * baseline_tail_wave_phase
    baseline_tail_wave_scale =
        (1 + baseline_tail_wave_modulation) /
        sqrt(1 + baseline_tail_wave_modulation^2)
    ...
    modulated_tail_wave_modulation =
        asymmetry_limit * response_direction * modulated_tail_wave_phase
    modulated_tail_wave_scale =
        (1 + modulated_tail_wave_modulation) /
        sqrt(1 + modulated_tail_wave_modulation^2)
    posterior_target =
        guidance.mean_tail_curvature +
