# Phase-2 candidate: preserve the actuator-consistent phase route and project
# only outward acceleration into the next update's joint-speed headroom.

function target_policy_params()
    return (
        version="dogfish3d_joint_speed_headroom_tail_phase_v1",
        control_period=0.55,
        ...
        joint_velocity_projection_horizon=0.0055,
...
@inline function _project_outward_to_velocity_headroom(
    acceleration,
    joint_velocity,
    velocity_limit,
    projection_horizon,
)
    feasible_velocity_limit = max(velocity_limit, eps(Float64))
    accelerating_outward = acceleration * joint_velocity > 0
    accelerating_outward || return acceleration

    # The episode advances joint speed with explicit acceleration and then
    # applies the hard velocity clamp. Bound only the outward command by the
    # remaining speed headroom over the documented maximum update horizon.
    # Reverse acceleration is never projected, so braking remains immediate.
    feasible_projection_horizon = max(projection_horizon, eps(Float64))
    velocity_headroom = max(
        feasible_velocity_limit - abs(joint_velocity),
        0.0,
    )
    headroom_acceleration = velocity_headroom / feasible_projection_horizon
    return sign(acceleration) * min(abs(acceleration), headroom_acceleration)
end
...
function target_policy(state, params)
    ...
    observed_previous_posterior_action =
        _finite_or(state.previous_action[2], 0.0)
    # Headroom projection returns zero at the speed boundary and can return a
    # smaller same-side action on entry. Preserve the evaluated persistence
    # semantics by treating either as the feasible-action witness while raw
    # outward demand continues. When demand or the previous action reverses,
    # the witness disappears and braking/phase release remain immediate.
    rate_limited_outward_persistence =
        abs(qd2) >= velocity_limit &&
        baseline_posterior_accel * qd2 > 0 &&
        observed_previous_posterior_action * qd2 >= 0
    previous_posterior_action = rate_limited_outward_persistence ?
        clamp(
            baseline_posterior_accel,
            -acceleration_limit,
            acceleration_limit,
    ...
    phase_activation =
        predicted_phase_activation * actuator_consistency
    ...
    # The episode applies the same componentwise acceleration limiter. Project
    # at the policy boundary so returned commands are feasible by construction.
    ...
    # Do not ask the integrator to step beyond the hard speed boundary.
    # Projection is one-sided: reverse acceleration remains fully available
    # to brake and preserve the traveling-wave phase. Under lateral reflection
    # velocity and acceleration both reverse, so the weight is invariant and
    # its output remains odd.
    headroom_anterior_accel = _project_outward_to_velocity_headroom(
        feasible_anterior_accel,
        qd1,
        velocity_limit,
        params.joint_velocity_projection_horizon,
    )
    headroom_posterior_accel = _project_outward_to_velocity_headroom(
        feasible_posterior_accel,
        qd2,
        velocity_limit,
        params.joint_velocity_projection_horizon,
    )
    ...
    return (
        phi_ddot=(headroom_anterior_accel, headroom_posterior_accel),
    )
end
