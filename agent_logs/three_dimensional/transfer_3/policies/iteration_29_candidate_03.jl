# Phase-2 candidate: preserve carrier-demodulated posterior phase steering and
# project only persistent outward tail acceleration near the velocity envelope.

function target_policy_params()
    return (
        version="dogfish3d_persistent_tail_velocity_envelope_v1",
        control_period=0.55,
        ...
        posterior_velocity_activation_width=0.04,
        persistent_outward_drive_scale=0.04,
...
function target_policy(state, params)
    ...
    # Raw moment is dominated by the observed traveling-bend carrier. Remove
    # that joint-phase contribution before using the remaining fluid response
    # as a small bidirectional allocator: an opposing residual recruits phase,
    # while a helping residual releases the same bounded fraction. The product
    # of the odd response direction and odd residual is reflection invariant.
    ...
    # The assigned parent's incremental phase withdrawal did not create
    # posterior velocity headroom. Instead, apply a smooth tangent-envelope
    # projection to the complete command, and only after current and previous
    # accelerations both confirm persistent outward drive near the limit. At
    # full activation the outward component approaches zero; inward braking is
    # never weakened or augmented. All signed terms reverse under reflection.
    velocity_limit = max(
    ...
    current_outward_drive = max(
        (qd2 / velocity_limit) *
        (posterior_accel / acceleration_limit),
        0.0,
    )
    previous_outward_drive = max(
        (qd2 / velocity_limit) *
        (previous_posterior_action / acceleration_limit),
        0.0,
    )
    outward_drive_scale = max(
        params.persistent_outward_drive_scale,
        1.0e-6,
    )
    persistent_outward_weight =
        tanh(current_outward_drive / outward_drive_scale) *
        tanh(previous_outward_drive / outward_drive_scale)
    velocity_envelope_weight = clamp(
        posterior_velocity_activation * persistent_outward_weight,
        0.0,
        1.0,
    )
    posterior_velocity_direction = sign(qd2)
    outward_accel = max(
        posterior_velocity_direction * posterior_accel,
        0.0,
    )
    envelope_posterior_accel =
        posterior_accel -
        posterior_velocity_direction *
        velocity_envelope_weight *
        outward_accel
