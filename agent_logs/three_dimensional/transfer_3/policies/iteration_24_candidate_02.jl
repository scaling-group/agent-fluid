# Phase-2 candidate: preserve persistent actuator-consistent phase recruitment
# and anticipate the next beat-side yaw response from bounded moment feedback.

function target_policy_params()
    return (
        version="dogfish3d_moment_anticipated_tail_phase_v1",
        control_period=0.55,
        ...
        moment_prediction_scale=0.0070,
        moment_prediction_yaw_rate_limit=0.090,
...
function target_policy(state, params)
    ...
    # Preserve the evaluated phase-recruitment gate and use normalized yaw
    # moment only as bounded one-step response anticipation. The sampled
    # moment-to-next-yaw-change slope is about 12.86, matching the small-signal
    # ratio 0.090/0.007 here. Moment and yaw error both reverse under reflection.
    hydrodynamic_yaw_moment =
        _finite_or(state.moment_z_L2, 0.0)
    moment_yaw_rate_prediction =
        params.moment_prediction_yaw_rate_limit *
        tanh(
            hydrodynamic_yaw_moment /
            max(params.moment_prediction_scale, 1.0e-6),
        )
    anticipated_yaw_rate_error =
        guidance.yaw_rate_error - moment_yaw_rate_prediction
    anticipated_response_direction = tanh(
        anticipated_yaw_rate_error /
        max(params.yaw_rate_error_scale, 1.0e-6),
    )

    # Rotate the baseline wave coefficients in observed joint phase. The
    # response reverses if measured plus predicted yaw outruns demand, while
    # the phase-gradient factor selects the rotation sign that adds curvature.
    phase_gradient =
    ...
    tail_phase_shift =
        phase_activation *
        phase_limit *
        anticipated_response_direction *
    ...
    # Phase recruitment still follows the replicated persistent-stress gate;
    # only its beat-side direction and the compatible half-cycle scale use the
    # bounded anticipated response.
