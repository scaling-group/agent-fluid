# Phase-2 candidate: preserve the completed response-reversing C-bend and
# allocate saturation-recruited posterior phase only when it opens headroom.

function target_policy_params()
    return (
        version="dogfish3d_headroom_allocated_tail_phase_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Recruit phase timing only near a predicted posterior constraint. This
    # preserves the evaluated response-reversing half-cycle below the envelope.
    ...
    phase_activation = 0.5 * (1 + tanh(
        (predicted_acceleration_fraction -
            params.tail_phase_activation_fraction) /
        max(params.tail_phase_activation_width, 1.0e-6),
    ))

    # Rotate the baseline wave coefficients without changing their norm. The
    # yaw-response/phase-gradient product is reflection-invariant, and the
    # response reverses smoothly when measured yaw outruns route demand.
    phase_gradient =
    ...
    phase_posterior_target =
        guidance.mean_tail_curvature +
        modulated_tail_wave_scale * phase_modulated_wave_target
    phase_posterior_accel =
        omega^2 * (phase_posterior_target - q2) -
        2 * params.tail_damping * omega * qd2

    # Accept the phase proposal only to the degree that it reduces the
    # instantaneous command magnitude. Normalizing by the proposal delta makes
    # the allocator scale-free; smoothstep avoids a new gain and retains a
    # continuous fallback to the completed half-cycle command.
    opened_headroom = max(
        abs(baseline_posterior_accel) - abs(phase_posterior_accel),
        0.0,
    )
    proposal_accel_delta = abs(
        phase_posterior_accel - baseline_posterior_accel,
    )
    headroom_coordinate = clamp(
        opened_headroom / max(proposal_accel_delta, eps(Float64)),
        0.0,
        1.0,
    )
    phase_acceptance =
    ...
    posterior_target =
        baseline_posterior_target +
        phase_acceptance *
        (phase_posterior_target - baseline_posterior_target)
    posterior_accel =
