# Phase-2 candidate: retain the completed distributed C-bend while separating
# slow yaw response from both quadratures of periodic joint-driven recoil.

function target_policy_params()
    return (
        version="dogfish3d_joint_phase_recoil_observer_v1",
        control_period=0.55,
...
function target_guidance(state, q1, q2, qd1, qd2, params)
    ...
    # A periodic recoil projection needs both oscillator quadratures. Center
    # q1 by its commanded route bend, then scale joint positions by the carrier
    # frequency so position and velocity terms share yaw-rate units. This is
    # joint-state phase feedback, not an external clock.
...
function target_policy(state, params)
    ...
    # Match the episode's physical acceleration projection at the policy
    # boundary, preserving the applied-action contract of the assigned parent.
