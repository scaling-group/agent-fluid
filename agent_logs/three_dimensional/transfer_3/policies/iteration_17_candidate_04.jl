# Phase-2 candidate: retain the completed LOS-rate distributed C-bend, but
# reallocate the anterior motion envelope when route error remains severe.

function target_policy_params()
    return (
        version="dogfish3d_response_released_envelope_redirect_v1",
        control_period=0.55,
        ...
        burst_bearing_threshold=0.55,
        burst_bearing_width=0.15,
        burst_anterior_shift_limit=8.0 * pi / 180,
...
function target_guidance(state, qd1, qd2, params)
    ...
    # When large body-frame route error persists after the ordinary redirect
    # gate has saturated, trade part of the counter-turn half-cycle for extra
    # mean curvature. Removing the same amount from the centered amplitude
    # preserves the evidenced same-side peak angle. The LOS response inside
    # target_yaw_rate releases or reverses the shift when turn response catches
    # the demand, avoiding a fixed bias or an undamped half-cycle command.
    burst_weight = 0.5 * (1 + tanh(
        (abs(bearing) - params.burst_bearing_threshold) /
        max(params.burst_bearing_width, 1.0e-6),
    ))
    burst_mean_curvature =
        params.burst_anterior_shift_limit *
        burst_weight *
        tanh(
            target_yaw_rate /
            max(params.redirect_yaw_rate_scale, 1.0e-6),
    ...
    anterior_mean_curvature =
        base_anterior_mean_curvature + burst_mean_curvature
    effective_oscillator_amplitude = max(
        params.oscillator_amplitude - abs(burst_mean_curvature),
        eps(Float64),
    )
    ...
    return (
        ...
        base_anterior_mean_curvature=base_anterior_mean_curvature,
        burst_weight=burst_weight,
        burst_mean_curvature=burst_mean_curvature,
        anterior_mean_curvature=anterior_mean_curvature,
        effective_oscillator_amplitude=effective_oscillator_amplitude,
    )
end
...
function target_policy(state, params)
    ...
    # Oscillate around the bounded anterior route-response center. Severe route
    # error shifts existing amplitude into the center instead of enlarging the
    # same-side envelope. The posterior target remains relative to this center.
    centered_q1 = q1 - guidance.anterior_mean_curvature
    vdp_drive = params.oscillator_mu *
        (
            1 -
            (centered_q1 / guidance.effective_oscillator_amplitude)^2
        ) * qd1
