# Phase-2 candidate: preserve the actuator-consistent route and contract only
# surplus carrier amplitude during an already-fast, target-forward approach.

function target_policy_params()
    return (
        version="dogfish3d_closing_speed_carrier_envelope_v1",
        control_period=0.55,
        ...
        approach_carrier_relief_limit=0.20,
...
function target_guidance(state, qd1, qd2, params)
    ...
    # Positive radial speed closes on a stationary target. Recruit a bounded
    # near-approach envelope only while the target remains in front; steering
    # and posterior phase authority never depend on this gate.
    closing_speed = (
    ...
    approach_carrier_relief =
        clamp(params.approach_carrier_relief_limit, 0.0, 0.5) *
        approach_range_weight *
        approach_closing_weight *
        approach_forward_weight
    carrier_amplitude_scale = 1 - approach_carrier_relief
    ...
    return (
        ...
        approach_carrier_relief=approach_carrier_relief,
        carrier_amplitude_scale=carrier_amplitude_scale,
        los_rate=los_rate,
...
function target_policy(state, params)
    ...
    scheduled_oscillator_amplitude = max(
        params.oscillator_amplitude * guidance.carrier_amplitude_scale,
        1.0e-6,
    )
    vdp_drive = params.oscillator_mu *
        (1 - (centered_q1 / scheduled_oscillator_amplitude)^2) * qd1
