# Phase-2 candidate: preserve the actuator-consistent phase route while a
# joint-state quadrature observer separates slow yaw from carrier recoil.

function target_policy_params()
    return (
        version="dogfish3d_joint_quadrature_yaw_observer_v1",
        control_period=0.55,
        ...
        yaw_recoil_q1_position_gain=1.50,
        yaw_recoil_q1_velocity_gain=0.72,
        yaw_recoil_q2_velocity_gain=0.35,
...
function target_guidance(state, q1, qd1, qd2, params)
    ...
    # Completed traces show that carrier recoil occupies both the position and
    # velocity quadratures. Center the position term on the bounded geometric
    # redirect so the observer does not erase the requested mean C-bend, then
    # cancel the fitted joint-correlated component before steering responds.
    ...
    phase_conditioned_yaw_rate =
        ...
        params.yaw_recoil_q1_velocity_gain * qd1 +
        params.yaw_recoil_q2_velocity_gain * qd2
...
function target_policy(state, params)
    ...
    guidance = target_guidance(state, q1, qd1, qd2, params)
