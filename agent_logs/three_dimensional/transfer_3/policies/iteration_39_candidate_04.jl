# Phase-2 candidate: preserve the actuator-consistent phase route and use a
# one-sided rate barrier to prevent acceleration into the joint-speed envelope.

function target_policy_params()
    return (
        version="dogfish3d_joint_rate_barrier_tail_phase_v1",
        control_period=0.55,
...
@inline function _project_outward_rate_barrier(
    acceleration,
    joint_velocity,
    velocity_limit,
    barrier_rate,
    ...
    speed = abs(joint_velocity)

    # The barrier acts only on acceleration that increases speed. Inward
    # acceleration remains untouched so reversal and braking stay immediate.
    if acceleration * joint_velocity <= 0
        return acceleration
    elseif speed >= feasible_velocity_limit
        return 0.0
    end

    remaining_squared_speed = max(
        feasible_velocity_limit^2 - speed^2,
        0.0,
    )
    outward_acceleration_limit =
        max(barrier_rate, 0.0) * remaining_squared_speed /
        max(2 * speed, eps(Float64))
    return sign(acceleration) * min(
        abs(acceleration),
        outward_acceleration_limit,
    )
end
...
function target_policy(state, params)
    ...
    rate_barrier = max(params.joint_rate_barrier_gain, 0.0) * omega
    ...
    barrier_feasible_baseline_posterior_accel =
        _project_outward_rate_barrier(
            baseline_posterior_accel,
            qd2,
            velocity_limit,
            rate_barrier,
    ...
    # A projected action becomes small before the speed boundary. Preserve
    # the evaluated phase gate's causal persistence witness with same-side raw
    # demand whenever the rate barrier is active; release it as soon as demand
    # reverses so braking and phase reversal remain immediate.
    rate_barrier_limits_outward =
        baseline_posterior_accel * qd2 > 0 &&
        abs(barrier_feasible_baseline_posterior_accel) <
            abs(baseline_posterior_accel)
    previous_posterior_action = rate_barrier_limits_outward ?
        clamp(
            baseline_posterior_accel,
            -acceleration_limit,
            acceleration_limit,
    ...
    # Enforce a continuous one-sided speed barrier. The squared-speed margin
    # anticipates the hard boundary instead of waiting for exact equality;
    # reverse acceleration is never attenuated. Under reflection, velocity and
    # acceleration reverse together, so the gate is invariant and its output
    # remains odd.
    barrier_anterior_accel = _project_outward_rate_barrier(
        feasible_anterior_accel,
        qd1,
        velocity_limit,
        rate_barrier,
    )
    barrier_posterior_accel = _project_outward_rate_barrier(
    ...
    return (
        phi_ddot=(barrier_anterior_accel, barrier_posterior_accel),
    )
end
