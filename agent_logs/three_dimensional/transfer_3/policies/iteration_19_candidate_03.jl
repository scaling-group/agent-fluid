# Phase-2 candidate: keep the completed LOS-rate distributed C-bend and its
# response-reversing tail half-cycle, then add bounded posterior phase steering.

function target_policy_params()
    return (
        version="dogfish3d_response_reversing_tail_phase_v2",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Rotate the two baseline wave coefficients without changing their norm.
    # The local phase gradient tells which rotation sign adds the requested
    # curvature at this observed joint phase. Multiplying it by reversible yaw
    # error makes the phase shift reflection-invariant and response-reversing.
    phase_response = tanh(
    ...
    tail_phase_shift =
        phase_limit * phase_response * phase_gradient_direction
    ...
    # Retain the sampled response-reversing half-cycle authority. Its scale is
    # odd with the posterior wave and therefore preserves reflection symmetry;
    # when measured yaw outruns demand, the helpful half-cycle reverses.
    ...
    tail_wave_scale =
        1 + asymmetry_limit * phase_response * tail_wave_phase
    posterior_target =
