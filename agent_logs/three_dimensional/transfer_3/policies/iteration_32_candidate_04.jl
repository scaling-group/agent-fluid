# Phase-2 candidate: preserve actuator-consistent phase steering and relieve
# only redundant half-cycle amplitude under helpful demodulated fluid response.

function target_policy_params()
    return (
        version="dogfish3d_demodulated_helpful_amplitude_relief_v1",
        control_period=0.55,
        ...
        helpful_residual_relief_limit=0.30,
...
function target_policy(state, params)
    ...
    # Remove the joint-phase carrier from measured yaw moment before deciding
    # whether fluid response duplicates the established route correction. A
    # helping residual may relieve only bounded half-cycle amplitude near yaw
    # closure; it never suppresses the carrier, curvature, or phase actuator.
    # Both residual moment and response direction reverse under reflection, so
    # their product and the resulting authority fraction remain invariant.
    ...
    normalized_fluid_response = tanh(
        fluid_moment_residual /
        max(params.fluid_moment_residual_scale, 1.0e-6),
    )
    helping_residual_alignment = clamp(
        response_direction * normalized_fluid_response,
        0.0,
        1.0,
    )
    correction_satisfaction = 1 - abs(response_direction)
    residual_relief_limit = clamp(
        params.helpful_residual_relief_limit,
        0.0,
        0.5,
    )
    helpful_residual_relief =
        residual_relief_limit *
        helping_residual_alignment *
        correction_satisfaction
    recruited_asymmetry_limit =
        asymmetry_limit * (1 - helpful_residual_relief)
