# Phase-2 candidate: preserve the completed LOS-rate C-bend, then conserve its
# slow curvature while moving same-sign yaw correction forward only under a
# large observed route demand.

function target_policy_params()
    return (
        version="dogfish3d_high_demand_response_deficit_allocation_v1",
        control_period=0.55,
...
function target_guidance(state, qd1, qd2, params)
    ...
    # Reallocate slow bend only when the large LOS response request and the
    # phase-conditioned yaw deficit agree in sign. The response-specific gate
    # keeps early allocation negligible and avoids the falsified range gate.
    # Adding and subtracting the same bend preserves total slow curvature;
    # an opposing posterior correction is never removed.
