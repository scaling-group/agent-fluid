# Phase-2 candidate: preserve the successful LOS-rate distributed C-bend, then
# release mean steering continuously when the observed course is already a
# safe closing intercept while retaining the propulsive traveling carrier.

function target_policy_params()
    return (
        version="dogfish3d_safe_intercept_release_v1",
        control_period=0.55,
        ...
        intercept_release_start_range=2.0,
        intercept_release_range_width=0.30,
        intercept_release_closing_speed=0.35,
        intercept_release_closing_width=0.10,
        intercept_release_miss_limit=0.55,
        intercept_release_miss_width=0.12,
        intercept_release_speed_floor=0.30,
    )
end
...
function target_guidance(state, qd1, qd2, params)
    ...
    base_target_yaw_rate = clamp(
        bearing_yaw_rate + response_yaw_rate,
        -params.combined_yaw_rate_limit,
        params.combined_yaw_rate_limit,
    )

    # Once the instantaneous course is closing through an inner capture
    # corridor, release route curvature into the unsteered carrier. Each soft
    # condition is memoryless, so steering returns if the predicted miss grows.
    # The cross-product ratio is the closest distance to the target under the
    # current rigid velocity, expressed in L.
    closing_speed = (
        target_x * velocity_x + target_y * velocity_y
    ) / target_norm
    predicted_miss_L = abs(
        target_y * velocity_x - target_x * velocity_y
    ) / max(rigid_speed, params.intercept_release_speed_floor)
    intercept_range_weight = 0.5 * (1 + tanh(
        (params.intercept_release_start_range - distance) /
        max(params.intercept_release_range_width, 1.0e-6),
    ))
    intercept_closing_weight = 0.5 * (1 + tanh(
        (closing_speed - params.intercept_release_closing_speed) /
        max(params.intercept_release_closing_width, 1.0e-6),
    ))
    intercept_miss_weight = 0.5 * (1 + tanh(
        (params.intercept_release_miss_limit - predicted_miss_L) /
        max(params.intercept_release_miss_width, 1.0e-6),
    ))
    intercept_release_weight =
        intercept_range_weight *
        intercept_closing_weight *
        intercept_miss_weight
    target_yaw_rate =
        (1 - intercept_release_weight) * base_target_yaw_rate
    ...
    yaw_rate_error =
        (1 - intercept_release_weight) *
        (base_target_yaw_rate - phase_conditioned_yaw_rate)
    mean_tail_curvature =
    ...
    return (
        ...
        base_target_yaw_rate=base_target_yaw_rate,
        ...
        intercept_release_weight=intercept_release_weight,
