# Phase-2 candidate: preserve the actuator-consistent route and let only a
# helpful carrier-demodulated response relieve incremental posterior phase.

function target_policy_params()
    return (
        version="dogfish3d_helpful_residual_phase_relief_v1",
        control_period=0.55,
...
function target_policy(state, params)
    ...
    # Raw moment is dominated by the traveling-bend carrier. Subtract its
    # observed joint-phase estimate, then use only a helpful residual to relieve
    # a bounded fraction of incremental phase recruitment. Opposing response
    # cannot add authority, while carrier, mean bend, and half-cycle steering
    # remain intact. The product of the two reflection-odd signals is invariant.
    ...
    helpful_phase_relief =
        residual_phase_fraction * helpful_residual_alignment
    phase_activation = clamp(
        predicted_phase_activation *
        actuator_consistency *
        (1 - helpful_phase_relief),
        0.0,
        1.0,
    )
    ...
    # full recruitment it is the sampled response-reversing phase path. The
    # residual allocator above changes phase recruitment only, never amplitude.
