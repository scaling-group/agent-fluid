# Phase-balanced traveling wave with a response-released anterior turn burst.
...
function target_policy_params()
    return (
        ...
        maximum_terminal_turn_acceleration=10.0,
...
function target_policy(state, params)
    ...
    # A tangential course can follow target-ray rotation without closing. Ask
    # for a bounded additional turn, compare it with measured yaw, and apply
    # the missing response directly at the anterior actuator. Unlike a static
    # bend shift or velocity-selected pulse, this maneuver acts from the quiet
    # C-bend and vanishes continuously once the requested yaw appears.
    ...
    terminal_turn_weight = target_behind_weight^2 *
        terminal_response_weight
    terminal_turn_acceleration =
        params.maximum_terminal_turn_acceleration * terminal_turn_weight *
            tanh(terminal_turn_error / params.terminal_turn_error_scale)
