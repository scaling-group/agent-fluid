# Yaw-response burst around the evidenced terminal course-hold C-turn scaffold.
...
function target_policy_params()
    return (
        ...
        burst_yaw_response_threshold=0.35,
        burst_yaw_response_scale=0.12,
...
function target_policy(state, params)
    ...
    # A close target-behind recovery can retain speed and command reserve while
    # producing too little requested-sign yaw. Add bounded anterior authority
    # only during the existing terminal course hold, then release it as the
    # measured yaw response appears. The posterior wave law stays unchanged.
    requested_sign_yaw = -response_direction * heading_rate
    yaw_response_deficit_weight = 0.5 * (1 + tanh(
        (params.burst_yaw_response_threshold - requested_sign_yaw) /
            params.burst_yaw_response_scale,
    ))
    terminal_burst_weight = target_behind_weight^2 *
        terminal_response_weight * yaw_response_deficit_weight
    terminal_burst_acceleration =
        -params.maximum_terminal_burst_acceleration * terminal_burst_weight *
            response_direction
