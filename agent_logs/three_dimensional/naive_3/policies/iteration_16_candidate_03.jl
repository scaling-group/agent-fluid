# Approach-localized course-error redirect around the evidenced response-gated
...
function target_policy(state, params)
    ...
    # Translational course is a slower geometric response than beat-locked yaw.
    # Compare its direction with the full target ray, and request an additional
    # bounded redirect only on the evidenced near approach. Normalizing both
    # vectors makes the angle scale-free and keeps speed out of its sign.
    ...
    curvature_limit = params.maximum_mean_curvature +
    ...
    # Preserve the sampled posterior brake: it still attenuates only the
    # evidenced wrong-way response half-cycle. The course redirect changes the
    # shared body bend but adds no separate tail counterstroke or residual.
