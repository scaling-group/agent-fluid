# Receding-overshoot whole-wave hold around the evidenced posterior phase-lag
...
function target_policy_params()
    return (
        ...
        hold_longitudinal_transition=0.08,
        hold_receding_speed_threshold=0.05,
        hold_receding_speed_transition=0.04,
        closing_speed_limit=2.0,
        maximum_hold_damping=4.0,
        command_limit=28.0,
    )
end
...
function target_policy(state, params)
    ...
    # A hold requires three pieces of semantic evidence: the target has passed
    # behind the head, distance is increasing, and the miss is still local.
    # These normalized body-frame gates keep the inbound carrier unchanged and
    # release continuously on recovery, without a timer or mutable stage.
    longitudinal_fraction = target_x / distance
    behind_weight = 0.5 * (1 + tanh(
        longitudinal_fraction / params.hold_longitudinal_transition,
    ))
    closing_speed = clamp(
    ...
    receding_speed = max(-closing_speed, 0.0)
    receding_weight = 0.5 * (1 + tanh(
        (receding_speed - params.hold_receding_speed_threshold) /
            params.hold_receding_speed_transition,
    ))
    hold_distance_weight = 1 / (1 +
        (distance / params.hold_distance_scale)^params.hold_distance_power)
    hold_weight = behind_weight * receding_weight * hold_distance_weight
    hold_damping = params.maximum_hold_damping * hold_weight

    # Preserve the sampled anterior equilibrium and phase carrier. Once a
    # receding overshoot is confirmed, dissipate measured anterior joint motion
    # while retaining the target-directed mean curvature for reorientation.
    centered_q1 = q1 - mean_curvature
    vdp_drive = params.oscillator_mu *
        (1 - (centered_q1 / amp)^2) * qd1
    raw_a1 = vdp_drive - omega^2 * centered_q1 - hold_damping * qd1
    ...
    # Preserve the evidenced state-phased lag modulation on the inbound pass.
    # The course-direction/velocity product is reflection invariant: mirrored
    # geometry changes both signs while the posterior acceleration mirrors.
    ...
    # Dissipate the posterior follower at the same semantic hold gate. Unlike
    # rescaling its target, joint-rate damping avoids an abrupt phase-target
    # jump; both measured joints decay together around their steering means.
    raw_a2 = omega^2 * (phase_lag_target - q2) -
        2 * params.tail_damping * omega * qd2 - hold_damping * qd2
