# Harmful-impulse posterior damping around the phase-balanced terminal C-turn.
...
function target_policy_params()
    return (
        ...
        terminal_force_response_scale=0.0035,
        terminal_force_velocity_scale=0.08,
        maximum_terminal_force_damping_acceleration=1.25,
...
function target_policy(state, params)
    ...
    # Force is a fast response selector, not a route command. Positive
    # course_turn_force means the measured impulse rotates translational course
    # toward the target ray. During a harmful impulse only, oppose the measured
    # posterior motion: unlike the completed force-selected reinforcement, this
    # removes energy without moving mean bend or nominal posterior phase.
    force_x = Float64(state.force_body_L[1])
    force_y = Float64(state.force_body_L[2])
    course_turn_force = -course_direction *
        (velocity_unit_x * force_y - velocity_unit_y * force_x)
    harmful_force_weight = max(-tanh(
        course_turn_force / params.terminal_force_response_scale,
    ), 0.0)^2
    normalized_posterior_velocity = qd2 /
        max(omega * amp, eps(Float64))
    harmful_impulse_damping =
        -params.maximum_terminal_force_damping_acceleration *
            target_behind_weight^2 * terminal_response_weight *
            harmful_force_weight * tanh(
                normalized_posterior_velocity /
                    params.terminal_force_velocity_scale,
            )
    raw_a2 = omega^2 * (phase_lag_target - q2) -
        2 * params.tail_damping * omega * qd2 + harmful_impulse_damping
