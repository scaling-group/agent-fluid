# Response-selected anterior phase brake around the evidenced posterior brake.
...
function target_policy_params()
    return (
        ...
        anterior_velocity_scale=3.0,
        maximum_anterior_phase_brake=10.0,
        command_limit=28.0,
    )
end
...
function target_policy(state, params)
    ...
    # equilibrium. No clock, world coordinate, route, or identity enters.
    centered_q1 = q1 - mean_curvature
    vdp_drive = params.oscillator_mu *
        (1 - (centered_q1 / amp)^2) * qd1
    carrier_a1 = vdp_drive - omega^2 * centered_q1
    ...
    # acute bearing. During approach, measured yaw selects only intervals that
    # grow the signed direction error; corrective yaw leaves the carrier intact.
    ...
    # Heading response is phase-locked to anterior joint velocity in the
    # completed traces. Oppose that velocity only on the selected wrong-way
    # interval, producing a bounded state-feedback phase brake rather than a
    # clocked reset or another posterior counterstroke.
    anterior_phase_brake = -params.maximum_anterior_phase_brake *
        response_weight * tanh(qd1 / params.anterior_velocity_scale)
    raw_a1 = carrier_a1 + anterior_phase_brake

    # Retain the best sampled posterior response brake exactly: weaken only the
    # error-growing wave interval while preserving posterior mean curvature.
    propulsion_scale = 1 -
    ...
    phase_lag_target = tail_mean + propulsion_scale *
