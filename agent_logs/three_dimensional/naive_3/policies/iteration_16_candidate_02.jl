# Gait-residualized course feedback around the evidenced measured-yaw-brake
...
function target_policy_params()
    return (
        ...
        course_yaw_joint_rate_coupling=0.44,
...
function target_policy(state, params)
    ...
    # Raw yaw in the sampled carrier is almost entirely anticorrelated with
    # anterior joint rate near the target. Remove that fitted rhythmic
    # component before yaw damps the slow target-relative course request.
    bearing = clamp(
    ...
    course_heading_rate = clamp(
        raw_heading_rate +
            params.course_yaw_joint_rate_coupling * qd1,
        -params.heading_rate_limit,
        params.heading_rate_limit,
    )
    steering_error = bearing -
        params.heading_rate_damping * course_heading_rate
    ...
    # Preserve raw measured yaw as the evidenced beat-local selector. During
    # approach, weaken only the posterior wave half-cycle whose measured yaw
    # grows the signed full-direction error; corrective yaw is unchanged.
