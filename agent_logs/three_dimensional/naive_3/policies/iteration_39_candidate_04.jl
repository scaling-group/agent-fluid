# Force-selected terminal course conversion around the active-wave C-turn.
# Oscillation phase and hydrodynamic response remain entirely in measured state.
...
function target_policy_params()
    return (
        ...
        capture_force_distance_scale=1.35,
        capture_force_distance_power=16.0,
        course_force_response_threshold=0.0010,
        course_force_response_scale=0.00035,
        posterior_force_energy_velocity_scale=0.12,
        maximum_force_selective_posterior_energy=1.5,
...
function target_policy(state, params)
    ...
    # Preserve the completed moving C-turn equilibrium. The evidenced
    # two-sided velocity-odd regulator makes a low-activity anterior bend
    # locally unstable without changing its mean, handedness, or beat phase.
    ...
    # The nominal lag is deliberately unchanged by the terminal mechanism.
    ...
    # Body yaw alone did not contract the sampled tangential orbit. Select a
    # different posterior role from measured hydrodynamic response: only very
    # near capture, reinforce posterior motion during phases whose body-frame
    # force is already rotating translational velocity toward the target.
    # The force threshold rejects weak/opposite phases, and velocity-odd action
    # adds no static bend or prescribed phase.
    force_course_turn = (velocity_x * force_y - velocity_y * force_x) /
        max(speed, eps(Float64))
    inward_force_response = -course_direction * force_course_turn
    useful_force_weight = 0.5 * (1 + tanh(
        (inward_force_response -
            params.course_force_response_threshold) /
            params.course_force_response_scale,
    ))
    capture_force_distance_weight = 1 / (1 +
        (distance / params.capture_force_distance_scale)^
            params.capture_force_distance_power)
    posterior_effective_amp = max(redirect_wave_scale * amp, eps(Float64))
    posterior_normalized_velocity = qd2 /
        max(omega * posterior_effective_amp, eps(Float64))
    force_energy_weight = target_behind_weight^2 *
        capture_force_distance_weight * response_speed_weight *
        terminal_course_weight * useful_force_weight
    force_selected_posterior_energy =
        params.maximum_force_selective_posterior_energy *
            force_energy_weight * tanh(
                posterior_normalized_velocity /
                    params.posterior_force_energy_velocity_scale,
            )
