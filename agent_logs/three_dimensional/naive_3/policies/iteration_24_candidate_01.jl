# Response-released C-turn recovery around the evidenced posterior phase-lag
...
function target_policy(state, params)
    ...
    # head. Full target direction selects recovery onset, and course response
    # below determines whether a barely-ahead target is ready for release.
    ...
    # Crossing to the ahead side does not complete a recovery while the target
    # ray is still lateral and translational course remains tangential. Hold the
    # same bounded C-turn until that measured response aligns, with zero-speed
    # course removed continuously and no hidden maneuver state.
