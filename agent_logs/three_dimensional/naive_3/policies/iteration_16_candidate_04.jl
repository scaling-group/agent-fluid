# Geometry-persistent anterior redirect around the evidenced response-gated
...
function target_policy_params()
    return (
        ...
        maximum_anterior_redirect=6.0 * pi / 180,
        command_limit=28.0,
    )
end
...
function target_policy(state, params)
    ...
    # Preserve the sampled cruise loop: acute target bearing requests bounded
    # average curvature and measured yaw releases it as the body responds.
    bearing = clamp(
    ...
    # Full direction preserves ahead/behind information hidden by acute
    # bearing. Distance and lateral geometry sustain an anterior-only redirect
    # across alternating gait yaw, then release it continuously after recovery.
    ...
    anterior_redirect = params.maximum_anterior_redirect *
        approach_weight * lateral_weight * direction_sign
    anterior_curvature = cruise_curvature + anterior_redirect

    # Shift only the anterior oscillator equilibrium. The posterior mean below
    # remains on the evidenced cruise curvature, avoiding another tail
    # equilibrium reallocation while retaining the traveling joint wave.
    centered_q1 = q1 - anterior_curvature
    vdp_drive = params.oscillator_mu *
        (1 - (centered_q1 / amp)^2) * qd1
    raw_a1 = vdp_drive - omega^2 * centered_q1

    # Gross misalignment attenuates posterior thrust without removing its
    # cruise mean. Alignment continuously restores the full lagged wave.
    ...
    # Preserve the strongest sampled terminal selector: only error-growing
    # measured-yaw intervals attenuate the posterior wave during approach.
