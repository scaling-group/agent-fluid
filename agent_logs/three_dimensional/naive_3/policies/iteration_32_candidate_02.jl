# Terminal half-cycle energy around the evidenced course-hold C-turn scaffold.
...
function target_policy_params()
    return (
        ...
        terminal_halfcycle_velocity_scale=0.12,
        maximum_terminal_halfcycle_acceleration=2.0,
...
function target_policy(state, params)
    ...
    # Static equilibrium shifts and a fixed-sign restart both collapsed the
    # sampled terminal wave into a parked C-bend. Instead, add energy only on
    # the measured anterior half-cycle already moving toward the requested
    # turn. Velocity alignment makes the pulse exactly zero at a parked bend;
    # its smooth saturation bounds the action while the unchanged posterior
    # lag propagates the strengthened bend as a traveling wave.
    normalized_turn_velocity = -response_direction * qd1 /
        max(omega * effective_amp, eps(Float64))
    useful_halfcycle_weight = tanh(
        max(normalized_turn_velocity, 0.0) /
            params.terminal_halfcycle_velocity_scale,
    )
    normalized_wave_velocity = qd1 /
        max(
            params.terminal_halfcycle_velocity_scale * omega * effective_amp,
            eps(Float64),
        )
    terminal_halfcycle_weight = target_behind_weight^2 *
        terminal_response_weight * useful_halfcycle_weight
    halfcycle_acceleration =
        params.maximum_terminal_halfcycle_acceleration *
            terminal_halfcycle_weight * tanh(normalized_wave_velocity)
