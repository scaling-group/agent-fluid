# Terminal stroke-energy recovery around the course-hold C-turn scaffold.
...
function target_policy_params()
    return (
        ...
        stroke_energy_activity_threshold=0.12,
        stroke_energy_activity_scale=0.02,
        stroke_energy_velocity_scale=0.01,
        maximum_stroke_energy_acceleration=1.0,
...
function target_policy(state, params)
    ...
    # A course-signed restart can replace a quiet C-bend with another
    # one-sided action. Instead, detect weak phase-plane activity and add
    # energy with the measured stroke direction. This bounded odd feedback
    # changes neither the commanded mean bend nor the zero-velocity state;
    # the existing posterior lag turns recovered anterior motion into a
    # traveling bend.
    ...
    low_wave_activity_weight = 0.5 * (1 + tanh(
        (params.stroke_energy_activity_threshold - wave_activity) /
            params.stroke_energy_activity_scale,
    ))
    stroke_velocity_direction = tanh(
        normalized_wave_velocity / params.stroke_energy_velocity_scale,
    )
    stroke_energy_weight = target_behind_weight^2 *
        terminal_response_weight * low_wave_activity_weight
    stroke_energy_acceleration =
        params.maximum_stroke_energy_acceleration * stroke_energy_weight *
            stroke_velocity_direction
