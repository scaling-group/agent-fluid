# Response-selective anterior-wave brake around the evidenced
...
function target_policy(state, params)
    ...
    # Persistent target error asks for bounded mean curvature. Measured yaw
    ...
    # acute bearing. Proximity, material lateral error, and wrong-way measured
    # yaw must agree before anterior rhythmic authority is reduced.
    ...
    anterior_wave_authority = 1 -
        (1 - params.anterior_wave_floor) * brake_weight

    # Scale acceleration about the requested steering equilibrium. The
    # equilibrium and reflection symmetry are retained while the wrong-way
    # anterior response half-cycle loses rhythmic authority.
    centered_q1 = q1 - mean_curvature
    vdp_drive = params.oscillator_mu *
        (1 - (centered_q1 / amp)^2) * qd1
    centered_a1 = vdp_drive - omega^2 * centered_q1
    raw_a1 = anterior_wave_authority * centered_a1

    # Preserve the posterior steering share and lagged traveling wave. Unlike
    # the sampled terminal holds, the response gate does not attenuate or bias
    # the thrust-dominant posterior target.
