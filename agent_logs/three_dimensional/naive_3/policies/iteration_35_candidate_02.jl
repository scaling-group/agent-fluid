# Phase-balanced terminal energy around the course-hold C-turn scaffold.
...
function target_policy_params()
    return (
        ...
        terminal_activity_target=0.55,
        maximum_terminal_energy_gain=4.0,
...
function target_policy(state, params)
    ...
    # Restore deficient phase-plane motion without shifting the course-hold
    # equilibrium or selecting a fixed half-cycle. Positive velocity feedback
    # injects power on either measured sign, and its normalized radial deficit
    # releases continuously when the terminal wave regains useful activity.
    ...
    activity_deficit = clamp(
        1 - wave_activity / params.terminal_activity_target,
        0.0,
        1.0,
    )
    terminal_energy_acceleration =
        params.maximum_terminal_energy_gain * terminal_response_weight *
            activity_deficit * qd1
