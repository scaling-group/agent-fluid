# Target-ray response matching around the evidenced recovery C-turn.
...
function target_policy_params()
    return (
        ...
        ray_rate_deficit_scale=0.35,
        maximum_ray_rate_curvature=6.0 * pi / 180,
        terminal_distance_scale=1.8,
        terminal_distance_power=16.0,
        terminal_lateral_threshold=0.85,
        terminal_lateral_scale=0.08,
        terminal_course_threshold=0.90,
        terminal_course_scale=0.15,
...
function target_policy(state, params)
    ...
    # head. Full target direction selects the sampled C-turn after the pass.
    ...
    # Preserve the sampled response-selected same-sign curvature reserve. It
    # activates after the target passes and radial course response is poor;
    # positive closure removes only the reserve, never the sampled redirect.
    ...
    # A target-behind miss can remain tangential even with the static response
    # reserve active. Compare the inertial target-ray sweep with useful yaw and
    # add bounded authority only while the body turns too slowly to catch that
    # ray. The behind-target gate protects the initial ahead-side approach, and
    # measured response releases this residual without a hidden maneuver phase.
    target_ray_rate = -course_cross * speed / distance
    requested_ray_sweep = -response_direction * target_ray_rate
    useful_yaw_response = -response_direction * heading_rate
    ray_rate_deficit = max(
        requested_ray_sweep - useful_yaw_response,
        0.0,
    )
    ray_rate_deficit_weight = tanh(
        ray_rate_deficit / params.ray_rate_deficit_scale,
    )
    ray_rate_response_weight = redirect_weight^2 *
        response_distance_weight * response_speed_weight *
        ray_rate_deficit_weight
    ray_rate_curvature = -params.maximum_ray_rate_curvature *
        ray_rate_response_weight * response_direction
    redirect_curvature = base_redirect_curvature + response_curvature +
        ray_rate_curvature

    # The successful reserve otherwise releases at a barely-ahead lateral pass.
    # Bridge only that terminal gap: inside the evidenced sub-2L regime, retain
    # a bounded reserve until translational course aligns. The old behind-target
    # response continuously takes ownership as redirect_weight rises.
    terminal_distance_weight = 1 / (1 +
        (distance / params.terminal_distance_scale)^params.terminal_distance_power)
    terminal_lateral_weight = 0.5 * (1 + tanh(
        (abs(target_unit_y) - params.terminal_lateral_threshold) /
            params.terminal_lateral_scale,
    ))
    terminal_course_weight = 0.5 * (1 + tanh(
        (abs(course_error) - params.terminal_course_threshold) /
            params.terminal_course_scale,
    ))
    terminal_ahead_weight = 1 - redirect_weight
    terminal_response_weight = terminal_ahead_weight *
        terminal_distance_weight * terminal_lateral_weight *
        terminal_course_weight * response_speed_weight
    terminal_response_curvature = -params.maximum_response_curvature *
        terminal_response_weight * response_direction

    anterior_mean = (1 - redirect_weight) * mean_curvature +
        redirect_weight * redirect_curvature + terminal_response_curvature
    redirect_wave_scale = 1 -
        (1 - params.redirect_wave_floor) * redirect_weight

    # Preserve the sampled carrier on approach. During recovery, move its
    # equilibrium and envelope into the bounded C-turn without stopping the
    # traveling wave; the terminal bridge changes equilibrium only.
    ...
    # Preserve the evidenced joint-state phase modulation and blend both
    # response reserves into the same-curvature posterior equilibrium.
    ...
    tail_mean = (1 - redirect_weight) *
        ...
        params.redirect_tail_share * terminal_response_curvature
