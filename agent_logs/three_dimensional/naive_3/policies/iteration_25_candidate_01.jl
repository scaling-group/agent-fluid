# Response-selected C-bend/counterbend recovery around the evidenced horizon
# scaffold. Oscillation and maneuver response remain in measured state.
...
function target_policy_params()
    return (
        ...
        redirect_response_lateral_threshold=0.85,
        redirect_response_lateral_scale=0.08,
        redirect_response_course_threshold=0.55,
        redirect_response_course_scale=0.15,
        redirect_response_speed_scale=0.25,
        maximum_response_hold=0.60,
        maximum_redirect_curvature=18.0 * pi / 180,
        redirect_tail_share=1.05,
        redirect_wave_floor=0.45,
        counterbend_yaw_threshold=0.90,
        counterbend_yaw_scale=0.35,
        maximum_posterior_counterbend=6.0 * pi / 180,
        command_limit=28.0,
    )
end
...
function target_policy(state, params)
    ...
    # but cannot distinguish a lateral target from one behind the head. Full
    # target direction selects recovery onset, and course response holds the
    # turn across a barely-ahead lateral pass.
    ...
    response_lateral_weight = 0.5 * (1 + tanh(
        (abs(target_unit_y) - params.redirect_response_lateral_threshold) /
            params.redirect_response_lateral_scale,
    ))
    response_course_weight = 0.5 * (1 + tanh(
        (abs(course_error) - params.redirect_response_course_threshold) /
            params.redirect_response_course_scale,
    ))
    response_speed_ratio = speed /
        (speed + params.redirect_response_speed_scale)
    response_speed_weight = response_speed_ratio^2
    response_hold_weight = params.maximum_response_hold *
        response_lateral_weight * response_course_weight *
        response_speed_weight
    redirect_weight = 1 -
        (1 - target_behind_weight) * (1 - response_hold_weight)

    # The sampled persistent C-bend survives but keeps translation tangential.
    # Once same-sign measured yaw is established, select a posterior
    # counterbend component and restore the traveling-wave envelope. The
    # signed yaw product is reflection invariant and the component mirrors.
    redirect_direction = tanh(
        target_unit_y / params.redirect_lateral_scale,
    )
    redirect_curvature = -params.maximum_redirect_curvature *
        redirect_direction
    harmful_yaw_response = course_direction * heading_rate
    yaw_response_weight = 0.5 * (1 + tanh(
        (harmful_yaw_response - params.counterbend_yaw_threshold) /
            params.counterbend_yaw_scale,
    ))
    counterbend_weight = redirect_weight * phase_approach_weight *
        response_course_weight * yaw_response_weight
    anterior_mean = (1 - redirect_weight) * mean_curvature +
        redirect_weight * redirect_curvature
    base_redirect_wave_scale = 1 -
        (1 - params.redirect_wave_floor) * redirect_weight
    recovery_wave_scale = base_redirect_wave_scale +
        (1 - base_redirect_wave_scale) * counterbend_weight

    # Preserve the sampled anterior C-bend on approach. During recovery, the
    # measured yaw response releases its envelope toward the propulsive wave;
    # the posterior joint below supplies the counterbend from available reserve.
    centered_q1 = q1 - anterior_mean
    effective_amp = max(recovery_wave_scale * amp, eps(Float64))
    ...
    # Preserve the evidenced joint-state phase modulation. Add the opposite
    # posterior component only after harmful yaw establishes the response; this
    # uses posterior reserve instead of commanding a discontinuous two-joint
    ...
    tail_mean = (1 - redirect_weight) *
        ...
        counterbend_weight * params.maximum_posterior_counterbend *
            course_direction
    posterior_wave = -centered_q1 -
        modulated_tail_lag * qd1 / max(omega, eps(omega))
    phase_lag_target = tail_mean +
        propulsion_scale * posterior_wave_authority * recovery_wave_scale *
            posterior_wave
