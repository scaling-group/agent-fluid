# Closing-miss curvature bridge around the terminal course-hold C-turn.
...
function target_policy_params()
    return (
        ...
        closing_miss_course_threshold=0.20,
        closing_miss_course_scale=0.10,
        closing_miss_distance_threshold=0.90,
        closing_miss_distance_scale=0.15,
...
function target_policy(state, params)
    ...
    # A closing trajectory can already have a large constant-course miss while
    # the target is behind, before either the nonclosing response or terminal
    # hold has fully engaged. Anticipate that handoff from normalized relative
    # geometry, reusing the established response curvature without increasing
    # its authority or changing the propulsive wave.
    projected_miss_distance = distance * abs(course_cross)
    closing_course_weight = 0.5 * (1 + tanh(
        (course_dot - params.closing_miss_course_threshold) /
            params.closing_miss_course_scale,
    ))
    large_miss_weight = 0.5 * (1 + tanh(
        (projected_miss_distance -
            params.closing_miss_distance_threshold) /
            params.closing_miss_distance_scale,
    ))
    closing_miss_weight = target_behind_weight^2 *
        terminal_distance_weight * response_speed_weight *
        closing_course_weight * large_miss_weight
    response_weight = 1 -
        (1 - base_response_weight) * (1 - terminal_response_weight) *
            (1 - closing_miss_weight)
    redirect_weight = 1 -
