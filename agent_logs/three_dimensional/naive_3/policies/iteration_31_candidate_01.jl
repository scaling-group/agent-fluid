# Closing-course phase reversal around the evidenced terminal C-turn scaffold.
...
function target_policy_params()
    return (
        ...
        terminal_closing_course_threshold=0.15,
        terminal_closing_course_scale=0.08,
        terminal_reverse_lag_gain=0.80,
...
function target_policy(state, params)
    ...
    # On a close, fully-behind, still-closing return, reverse only the posterior
    # derivative lag. This tests active traveling-wave braking; it does not add
    # curvature or attenuate the oscillator, and releases on tangent/recession.
    ...
    closing_course_weight = 0.5 * (1 + tanh(
        (course_dot - params.terminal_closing_course_threshold) /
            params.terminal_closing_course_scale,
    ))
    terminal_brake_weight = target_behind_weight^4 *
        terminal_distance_weight * closing_course_weight
    effective_tail_lag = (1 - terminal_brake_weight) * modulated_tail_lag -
        terminal_brake_weight * params.terminal_reverse_lag_gain
