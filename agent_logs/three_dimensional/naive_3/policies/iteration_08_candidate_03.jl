# Response-gated posterior redirect around the evidenced alignment-gated
...
function target_policy(state, params)
    ...
    # acute bearing. Large misalignment and inadequate target-ray closure must
    # agree before the less-saturated posterior equilibrium receives a burst.
    # The approach envelope excludes the evidenced early-course failures, and
    # a correct-sign recent turn releases it without a clock or controller mode.
    ...
    recent_turn_rate = clamp(
        Float64(state.turn_rate_recent),
        -params.heading_rate_limit,
        params.heading_rate_limit,
    )
    corrective_turn_rate = max(
        -redirect_direction * recent_turn_rate,
        0.0,
    )
