# Phase-balanced anterior and posterior energy around the terminal C-turn.
...
function target_policy_params()
    return (
        ...
        posterior_energy_activity_scale=0.07,
        posterior_energy_velocity_scale=0.20,
        maximum_posterior_energy_acceleration=2.0,
...
function target_policy(state, params)
    ...
    # Preserve the sampled carrier and its moving C-turn equilibrium. Measure
    # anterior phase-plane activity about that equilibrium and add odd velocity
    # feedback on both half-cycles only when activity is deficient. This keeps
    # a quiet C-bend locally unstable without changing mean curvature.
    ...
    # Give the posterior joint a distinct energetic role without changing its
    # mean, nominal lag, or wave target. Deficient posterior phase-plane
    # activity activates bounded velocity-odd feedback on both half-cycles;
    # activity itself releases the term continuously.
    effective_posterior_amp = max(
        posterior_wave_authority * redirect_wave_scale * amp,
        eps(Float64),
    )
    normalized_posterior_position =
        (q2 - tail_mean) / effective_posterior_amp
    normalized_posterior_velocity = qd2 /
        max(omega * effective_posterior_amp, eps(Float64))
    posterior_activity = hypot(
        normalized_posterior_position,
        normalized_posterior_velocity,
    )
    low_posterior_activity_weight = 0.5 * (1 + tanh(
        (params.posterior_energy_activity_target - posterior_activity) /
            params.posterior_energy_activity_scale,
    ))
    posterior_energy_weight = target_behind_weight^2 *
        terminal_response_weight * low_posterior_activity_weight
    phase_balanced_posterior_energy =
