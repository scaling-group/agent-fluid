# Measured-response anterior half-cycle relief around the evidenced
# target-response posterior brake. Phase remains entirely in observed state.
...
function target_policy_params()
    return (
        ...
        course_relief_distance_scale=3.6,
        course_relief_distance_power=12.0,
...
function target_policy(state, params)
    ...
    # Preserve the evidenced cruise curvature and measured-yaw damping.
    bearing = clamp(
    ...
    # Translational course exposes the persistent lateral near miss that yaw
    # alone hides. All directions are normalized in the instantaneous body
    # frame, with speed gating protecting the undefined zero-speed limit.
    ...
    relief_approach_weight = 1 / (1 +
        (distance / params.course_relief_distance_scale)^
            params.course_relief_distance_power)
    speed_ratio = speed / (speed + params.course_speed_scale)
    relief_speed_weight = speed_ratio^2
    course_error_weight = 0.5 * (1 + tanh(
        (abs(course_error) - params.course_error_threshold) /
            params.course_error_transition,
    ))
    course_direction = tanh(course_error / params.course_error_scale)
    wrong_way_course_yaw = max(course_direction * heading_rate, 0.0)
    course_yaw_weight = 0.5 * (1 + tanh(
        (wrong_way_course_yaw - params.wrong_way_yaw_threshold) /
            params.wrong_way_yaw_scale,
    ))
    anterior_relief_weight = relief_approach_weight * relief_speed_weight *
        course_error_weight * course_yaw_weight
    anterior_wave_scale = 1 -
        (1 - params.anterior_wave_floor) * anterior_relief_weight

    # Preserve the mean-curvature acceleration while withholding only the
    # oscillatory anterior component during measured wrong-way yaw. This asks
    # for steering reserve without adding to the already clipped command.
    centered_q1 = q1 - mean_curvature
    vdp_drive = params.oscillator_mu *
        (1 - (centered_q1 / amp)^2) * qd1
    oscillatory_a1 = vdp_drive - omega^2 * q1
    steering_a1 = omega^2 * mean_curvature
    raw_a1 = anterior_wave_scale * oscillatory_a1 + steering_a1

    # Retain the strongest sampled posterior response brake. It weakens only
    # a wrong-way measured-yaw interval and leaves corrective yaw unmodified.
