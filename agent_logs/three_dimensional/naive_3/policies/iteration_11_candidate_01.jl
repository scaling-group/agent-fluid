# Cross-track/closure-gated posterior approach hold around the evidenced
...
function target_policy_params()
    return (
        ...
        cross_track_threshold=0.40,
        cross_track_scale=0.12,
        closing_threshold=0.30,
        closing_scale=0.10,
        target_ray_speed_limit=2.0,
        posterior_hold_floor=0.15,
        command_limit=28.0,
    )
end
...
function target_policy(state, params)
    ...
    # Alignment retains a nonzero posterior wave during broad redirection.
    ...
    # Near a lateral miss, separate posterior propulsion from steering. Target
    # ray projection is rotation invariant; the cross-track magnitude is also
    # reflection invariant. Poor closure and high cross-track speed must agree
    # before only the oscillatory posterior wave is attenuated.
    ...
    closing_speed = clamp(
        ...
        -params.target_ray_speed_limit,
        params.target_ray_speed_limit,
    )
    cross_track_speed = clamp(
        abs(target_x * velocity_y - target_y * velocity_x) / distance,
        0.0,
        params.target_ray_speed_limit,
    )
    approach_weight = 1 / (1 +
        (distance / params.approach_distance_scale)^params.approach_distance_power)
    cross_track_weight = 0.5 * (1 + tanh(
        (cross_track_speed - params.cross_track_threshold) /
            params.cross_track_scale,
    ))
    closing_deficit = 0.5 * (1 + tanh(
        (params.closing_threshold - closing_speed) /
            params.closing_scale,
    ))
    hold_weight = approach_weight * cross_track_weight * closing_deficit
    propulsion_scale = 1 -
        (1 - params.posterior_hold_floor) * hold_weight

    # The mean posterior curvature remains outside the hold so the fish can
    # turn while the lagged thrust-producing wave is continuously reduced.
