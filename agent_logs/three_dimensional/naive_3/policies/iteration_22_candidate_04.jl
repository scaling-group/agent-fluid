# Course-selected one-sided posterior phase reset around the evidenced
...
function target_policy(state, params)
    ...
    # The sampled symmetric phase action improves closest approach, but its
    # negative lag shift is the phase present at that best minimum. Preserve
    # full posterior-wave amplitude and rectify timing so only that lag-reducing
    # half-cycle can reset the wave; the opposite half-cycle keeps carrier lag.
    # Mirroring flips course direction and joint velocity together, leaving the
    # selector invariant and the acceleration reflection equivariant.
    ...
    modulated_tail_lag = params.tail_lag_gain +
        params.maximum_tail_lag_shift * phase_weight * lag_reduction_phase
