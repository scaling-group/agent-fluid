# Phase-balanced terminal energy with posterior turn-response modulation.
...
function target_policy_params()
    return (
        ...
        terminal_phase_velocity_scale=0.35,
        maximum_terminal_lag_shift=0.25,
...
function target_policy(state, params)
    ...
    # Preserve the evidenced approach phase modulation. At terminal range,
    # matching target-ray rate alone would retain a tangential orbit, so ask
    # for an additional bounded course-closing turn and compare it with
    # measured yaw. Joint phase translates only the missing response into
    # posterior lag; anterior energy and both equilibria remain unchanged.
    ...
    terminal_normalized_joint_velocity = qd1 / max(
        params.terminal_phase_velocity_scale * omega * amp,
        eps(Float64),
    )
    terminal_velocity_phase = tanh(
        terminal_turn_deficit_direction *
            terminal_normalized_joint_velocity,
    )
    terminal_phase_response_weight = target_behind_weight^2 *
        terminal_response_weight
    terminal_lag_shift = params.maximum_terminal_lag_shift *
        terminal_phase_response_weight * terminal_velocity_phase
