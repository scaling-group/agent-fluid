# Gait-synchronous yaw cancellation around the evidenced alignment-gated
...
function target_policy(state, params)
    ...
    # Remove the rollout-evidenced joint-rate-synchronous yaw component before
    # damping that request, so beat motion is not mistaken for course response.
    bearing = clamp(
    ...
    course_heading_rate = clamp(
        Float64(state.heading_rate) +
            params.gait_yaw_rate_coupling * qd1,
