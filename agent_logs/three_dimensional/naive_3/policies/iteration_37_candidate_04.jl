# Turn-rate-deficit reserve around the phase-balanced terminal C-turn scaffold.
...
function target_policy_params()
    return (
        ...
        maximum_terminal_turn_curvature=10.0 * pi / 180,
...
function target_policy(state, params)
    ...
    # Tangential motion can rotate the target ray faster than the body turns,
    # preserving a powered orbit even under the existing course hold. Compare
    # the measured target-ray rate plus a bounded inward alignment margin with
    # measured yaw. Feed only the remaining response deficit into the C-turn
    # equilibrium, and release it continuously when yaw catches the request.
    ...
    terminal_turn_deficit_curvature =
        params.maximum_terminal_turn_curvature *
    ...
    # Preserve the sampled carrier and its moving C-turn equilibrium. When
    # phase-plane activity becomes deficient, bounded velocity-odd feedback on
    # both half-cycles keeps the quiet bend locally unstable without moving
    # the requested mean curvature.
