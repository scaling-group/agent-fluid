# Response-gated posterior wave around the phase-balanced course-hold scaffold.
...
function target_policy_params()
    return (
        ...
        terminal_useful_yaw_scale=0.35,
        maximum_terminal_posterior_wave_boost=0.45,
...
function target_policy(state, params)
    ...
    # Preserve the evidenced moving equilibrium and keep its rhythm unstable
    # on both half-cycles. Activity is measured about that moving equilibrium;
    # odd velocity feedback restores a deficient wave without selecting a
    # static turn side.
    ...
    # Heading rate is used only as a measured gait-response selector. Boost
    # the posterior wave during yaw half-cycles already reducing the persistent
    # course error, while leaving the other half-cycle and both means intact.
    useful_terminal_yaw = max(-response_direction * heading_rate, 0.0)
    useful_yaw_weight = tanh(
        useful_terminal_yaw / params.terminal_useful_yaw_scale,
    )
    terminal_posterior_wave_scale = 1 +
        params.maximum_terminal_posterior_wave_boost *
    ...
    phase_lag_target = tail_mean +
        propulsion_scale * posterior_wave_authority * redirect_wave_scale *
            terminal_posterior_wave_scale * posterior_wave
