# Terminal target-ray-rate response around the course-hold C-turn scaffold.
...
function target_policy_params()
    return (
        ...
        terminal_rate_error_scale=0.35,
        maximum_terminal_rate_acceleration=2.0,
...
function target_policy(state, params)
    ...
    target_ray_rate_error = target_ray_rate - heading_rate
    ...
    # The closest sampled miss has coherent propulsion but yaws much more
    # slowly than its target ray rotates. Track that geometric response rather
    # than joint activity: the residual is odd under lateral reflection and
    # releases continuously when measured yaw follows the line of sight. Keep
    # it local to the existing target-behind terminal selector and leave the
    # posterior traveling-wave target unchanged.
    terminal_rate_weight = target_behind_weight^2 *
        terminal_response_weight
    terminal_rate_acceleration =
        params.maximum_terminal_rate_acceleration * terminal_rate_weight *
            tanh(target_ray_rate_error / params.terminal_rate_error_scale)
