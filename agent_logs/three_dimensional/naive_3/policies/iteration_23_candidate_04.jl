# Response-released rapid C-bend around the evidenced posterior-phase carrier.
...
function target_policy_params()
    return (
        ...
        rapid_course_threshold=0.55,
        rapid_course_transition=0.18,
        rapid_lateral_threshold=0.55,
        rapid_lateral_transition=0.12,
        maximum_rapid_curvature=16.0 * pi / 180,
        rapid_carrier_floor=0.25,
        rapid_damping_ratio=0.10,
        command_limit=28.0,
    )
end
...
function target_policy(state, params)
    ...
    # yaw, this direction error persists across the two gait half-cycles. Its
    # speed gate removes the undefined zero-velocity direction continuously.
    ...
    # A close lateral pass calls for a nonsteady redirect rather than another
    # small equilibrium reallocation. Large persistent course error and
    # normalized lateral geometry move both joint means into the same C-bend.
    # Alignment loss removes the request continuously, providing a memoryless
    # response release back into the traveling wave.
    rapid_course_weight = 0.5 * (1 + tanh(
        (abs(course_error) - params.rapid_course_threshold) /
            params.rapid_course_transition,
    ))
    lateral_target_fraction = abs(target_unit_y)
    rapid_lateral_weight = 0.5 * (1 + tanh(
        (lateral_target_fraction - params.rapid_lateral_threshold) /
            params.rapid_lateral_transition,
    ))
    rapid_weight = phase_approach_weight * phase_speed_weight *
        rapid_course_weight * rapid_lateral_weight
    rapid_curvature = params.maximum_rapid_curvature * rapid_weight *
        course_direction
    steering_limit = params.maximum_mean_curvature +
        params.maximum_rapid_curvature
    steering_curvature = clamp(
        mean_curvature + rapid_curvature,
        -steering_limit,
        steering_limit,
    )
    carrier_authority = 1 -
        (1 - params.rapid_carrier_floor) * rapid_weight
    centered_q1 = q1 - steering_curvature
    vdp_drive = carrier_authority * params.oscillator_mu *
        (1 - (centered_q1 / amp)^2) * qd1
    rapid_damping = 2 * params.rapid_damping_ratio * omega *
        (1 - carrier_authority) * qd1
    raw_a1 = vdp_drive - rapid_damping - omega^2 * centered_q1
    ...
    # Preserve the sampled phase-lag action on the remaining carrier. The
    # direction/velocity product is reflection invariant, so lag modulation
    # mirrors normally without a clock or a world-frame route.
    ...
    # During the redirect, the posterior equilibrium shares the anterior sign
    # while the traveling component is attenuated. As course error falls, both
    # changes vanish and the evidenced posteriorly lagged carrier is restored.
    tail_mean = params.tail_curvature_share * mean_curvature + rapid_curvature
    posterior_wave = -centered_q1 -
        modulated_tail_lag * qd1 / max(omega, eps(omega))
    phase_lag_target = tail_mean +
        carrier_authority * propulsion_scale * posterior_wave_authority *
            posterior_wave
