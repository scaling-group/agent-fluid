# Terminal posterior response around the evidenced course-hold C-turn scaffold.
...
function target_policy_params()
    return (
        ...
        terminal_turn_course_dot_threshold=0.10,
        terminal_turn_course_dot_scale=0.15,
        maximum_terminal_turn_accel=6.0,
...
function target_policy(state, params)
    ...
    # Static course-hold curvature contracts the sampled orbit but leaves a
    # finite-speed tangential pass. Select unused posterior dynamic authority
    # only when terminal course response is tangent or receding. The residual
    # releases continuously as radial closure appears.
    terminal_turn_course_weight = 0.5 * (1 + tanh(
        (params.terminal_turn_course_dot_threshold - course_dot) /
            params.terminal_turn_course_dot_scale,
    ))
    terminal_turn_weight = terminal_distance_weight *
        response_speed_weight * terminal_turn_course_weight
    response_weight = 1 -
    ...
    phase_lag_target = tail_mean +
        propulsion_scale * posterior_wave_authority * redirect_wave_scale *
            posterior_wave
    # On the sampled scaffold, positive posterior acceleration precedes
    # negative yaw acceleration. response_direction mirrors with geometry, so
    # this bounded residual turns the translational course toward the target
    # without changing the established equilibrium or traveling-wave carrier.
    terminal_turn_residual = params.maximum_terminal_turn_accel *
        terminal_turn_weight * response_direction
    raw_a2 = omega^2 * (phase_lag_target - q2) -
        2 * params.tail_damping * omega * qd2 + terminal_turn_residual
