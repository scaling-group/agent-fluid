# Counterphase wave propagation around the phase-balanced terminal C-turn.
...
function target_policy_params()
    return (
        ...
        maximum_counterphase_acceleration=2.0,
...
function target_policy(state, params)
    ...
    # Preserve the sampled carrier and its moving C-turn equilibrium. The
    # signed half-cycle pulse collapsed into a parked bend because its work
    # vanished near zero speed and favored only one side of the rhythm.
    # Phase-plane activity instead gates odd velocity feedback on both
    # half-cycles, changing local damping without moving the equilibrium.
    ...
    # A C-start-like redirect propagates only when the anterior is already
    # bending into the requested course turn while the posterior is releasing
    # in counterphase. Require both measured motions, then add a bounded
    # equal-and-opposite pair parallel to them. This term is zero at either
    # parked joint and cannot introduce a static curvature or hidden stage.
    normalized_posterior_velocity = qd2 /
        max(omega * effective_amp, eps(Float64))
    anterior_turn_motion = max(
        -response_direction * normalized_wave_velocity,
        0.0,
    )
    posterior_release_motion = max(
        response_direction * normalized_posterior_velocity,
        0.0,
    )
    anterior_counterphase_weight = tanh(
        anterior_turn_motion / params.terminal_energy_velocity_scale,
    )
    posterior_counterphase_weight = tanh(
        posterior_release_motion / params.terminal_energy_velocity_scale,
    )
    counterphase_weight = target_behind_weight^2 *
        terminal_response_weight * anterior_counterphase_weight *
        posterior_counterphase_weight
    counterphase_burst = params.maximum_counterphase_acceleration *
        counterphase_weight
    raw_a1 = vdp_drive - omega^2 * centered_q1 + phase_balanced_energy -
        response_direction * counterphase_burst
