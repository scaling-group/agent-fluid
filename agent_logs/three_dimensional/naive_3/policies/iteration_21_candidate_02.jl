# Forming-miss-gated posterior phase-lag modulation around the evidenced
...
function target_policy_params()
    return (
        ...
        phase_course_scale=0.60,
        phase_lateral_scale=0.55,
        phase_closing_threshold=0.40,
        phase_closing_transition=0.15,
        phase_closing_limit=1.5,
...
function target_policy(state, params)
    ...
    lateral_target_fraction = target_y / distance
    phase_lateral_weight = tanh(
        abs(lateral_target_fraction) / params.phase_lateral_scale,
    )^2
    closing_speed = clamp(
        Float64(state.closing_speed_L),
        -params.phase_closing_limit,
        params.phase_closing_limit,
    )
    phase_closing_weight = 0.5 * (1 + tanh(
        (params.phase_closing_threshold - closing_speed) /
            params.phase_closing_transition,
    ))
    phase_weight = phase_approach_weight * phase_speed_weight *
        phase_lateral_weight * phase_closing_weight
    course_direction = tanh(course_error / params.phase_course_scale)

    # Preserve the sampled anterior state-feedback carrier and its equilibrium.
    # No clock, world coordinate, route, or case identity enters.
    ...
    # Use persistent target-ray/course sign only to choose phase polarity. The
    # action turns on when normalized lateral geometry and falling closure say
    # that a miss is forming. Joint velocity supplies beat phase, so posterior
    # lag changes oppositely over the two half-cycles without a static bend.
