# Target-response posterior counterstroke around the evidenced alignment-gated
...
function target_policy_params()
    return (
        ...
        posterior_reallocation_fraction=0.65,
        command_limit=28.0,
    )
end
...
function target_policy(state, params)
    ...
    # Reuse the strongest sampled terminal selector. Full target direction
    # retains ahead/behind information; measured yaw selects only response
    # intervals that grow the signed direction error on close approach.
    ...
    posterior_drive = posterior_wave_authority * posterior_wave

    # The best sampled policy discarded 65% of this selected posterior wave.
    # Put that same bounded authority on the target-directed side instead. A
    # wave already pointing corrective is nearly unchanged; a wrong-side wave
    # becomes a smaller counterstroke rather than a passive brake.
    reallocated_fraction = params.posterior_reallocation_fraction *
        response_weight
    retained_drive = (1 - reallocated_fraction) * posterior_drive
    counterstroke = -direction_sign * reallocated_fraction *
        abs(posterior_drive)
    phase_lag_target = tail_mean + retained_drive + counterstroke
