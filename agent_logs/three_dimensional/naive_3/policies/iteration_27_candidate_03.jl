# Terminal line-of-sight response around the evidenced course-hold scaffold.
...
function target_policy_params()
    return (
        ...
        terminal_navigation_rate_gain=1.5,
        terminal_navigation_yaw_scale=0.50,
        maximum_response_curvature=8.0 * pi / 180,
        maximum_terminal_navigation_curvature=8.0 * pi / 180,
...
function target_policy(state, params)
    ...
    # At the closest completed pass the held C-turn is nearly static while the
    # target line still rotates faster than the body. A collision-course
    # response asks yaw to follow that inertial line rate. Desired-minus-
    # measured yaw can reinforce a stalled turn or oppose wrong-way overshoot;
    # the terminal selector makes it vanish during cruise and alignment.
    target_line_rate = -speed * course_cross / distance
    desired_navigation_yaw = params.terminal_navigation_rate_gain *
        target_line_rate
    navigation_yaw_error = desired_navigation_yaw - heading_rate
    navigation_direction = tanh(
        navigation_yaw_error / params.terminal_navigation_yaw_scale,
    )
    terminal_navigation_curvature =
        params.maximum_terminal_navigation_curvature *
        terminal_response_weight * navigation_direction
    ...
    # terminal line-rate response into the same-curvature posterior mean.
    ...
    tail_mean = (1 - redirect_weight) *
        ...
        params.redirect_tail_share * terminal_navigation_curvature
