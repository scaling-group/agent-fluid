# Target-crossing-gated posterior response around the course-hold C-turn.
...
function target_policy_params()
    return (
        ...
        posterior_response_course_dot_threshold=0.10,
        posterior_response_course_dot_scale=0.15,
        maximum_posterior_response_accel=6.0,
...
function target_policy(state, params)
    ...
    # The three completed ungated terminal residuals all reversed the first
    # recovery despite negligible frozen-trace action there. Dynamic posterior
    # authority is therefore allowed only after full target geometry confirms
    # that the target has crossed behind, and only for tangent/receding course.
    posterior_response_course_weight = 0.5 * (1 + tanh(
        (params.posterior_response_course_dot_threshold - course_dot) /
            params.posterior_response_course_dot_scale,
    ))
    posterior_response_weight = target_behind_weight^2 *
        terminal_distance_weight * response_speed_weight *
        posterior_response_course_weight
    response_weight = 1 -
    ...
    # During recovery the established terminal hold and traveling wave remain
    # active; the new mechanism does not change anterior equilibrium posture.
    ...
    phase_lag_target = tail_mean +
        propulsion_scale * posterior_wave_authority * redirect_wave_scale *
            posterior_wave
    # Frozen evidence calibrates positive posterior acceleration to subsequent
    # negative yaw. response_direction supplies the mirrored course-error sign;
    # closure and an ahead-side target release this bounded residual.
    posterior_response_residual = params.maximum_posterior_response_accel *
        posterior_response_weight * response_direction
    raw_a2 = omega^2 * (phase_lag_target - q2) -
        2 * params.tail_damping * omega * qd2 + posterior_response_residual
