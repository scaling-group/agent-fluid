# Approach-localized gait-yaw residual around the evidenced response-gated
...
function target_policy(state, params)
    ...
    # Raw yaw is phase-locked to the anterior joint rate in the sampled near
    # miss. Residualize that gait component only on approach before using yaw
    # as slow mean-curvature damping. The posterior selector below deliberately
    # retains raw measured yaw because its half-cycle role is evidenced.
    ...
    course_heading_rate = clamp(
        raw_heading_rate + approach_weight *
            params.gait_yaw_joint_rate_gain * qd1,
        -params.heading_rate_limit,
        params.heading_rate_limit,
    )
