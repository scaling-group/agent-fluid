# Constant-authority posterior phase rotation around the evidenced
...
function target_policy_params()
    return (
        ...
        maximum_tail_phase_rotation=0.30,
        command_limit=28.0,
    )
end
...
function target_policy(state, params)
    ...
    # Rotate the posterior angle/velocity quadrature instead of changing only
    # the lag coefficient. This preserves the base coefficient norm, isolating
    # phase steering from accidental tail-amplitude scaling. The
    # course-direction/velocity product is reflection invariant: both factors
    # reverse in a mirrored state, while both joint commands mirror normally.
    ...
    phase_rotation = params.maximum_tail_phase_rotation * phase_weight *
        velocity_phase
    rotation_cos = cos(phase_rotation)
    rotation_sin = sin(phase_rotation)
    angle_coefficient = -rotation_cos +
        params.tail_lag_gain * rotation_sin
    velocity_coefficient = -rotation_sin -
        params.tail_lag_gain * rotation_cos
    tail_mean = params.tail_curvature_share * mean_curvature
    posterior_wave = angle_coefficient * centered_q1 +
        velocity_coefficient * qd1 / max(omega, eps(omega))
