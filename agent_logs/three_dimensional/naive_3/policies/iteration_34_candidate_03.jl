# Phase-balanced asymmetric terminal energy around the course-hold C-turn.
...
function target_policy_params()
    return (
        ...
        terminal_energy_halfcycle_scale=0.20,
        terminal_energy_halfcycle_bias=0.75,
...
function target_policy(state, params)
    ...
    # Preserve the evidenced moving equilibrium and keep it rhythmically
    # unstable on both half-cycles when phase-plane activity becomes small.
    # A bounded duty asymmetry then strengthens only measured motion already
    # advancing the body-frame course correction; it adds no static bend or
    # fixed-sign restart, and the posterior lag remains the wave propagator.
    ...
    useful_turn_velocity = -response_direction * normalized_wave_velocity
    useful_halfcycle_weight = tanh(
        max(useful_turn_velocity, 0.0) /
            params.terminal_energy_halfcycle_scale,
    )
    halfcycle_energy_scale = 1 +
        params.terminal_energy_halfcycle_bias * useful_halfcycle_weight
    phase_balanced_asymmetric_energy =
        params.maximum_terminal_energy_acceleration *
            terminal_energy_weight * halfcycle_energy_scale * tanh(
                normalized_wave_velocity /
