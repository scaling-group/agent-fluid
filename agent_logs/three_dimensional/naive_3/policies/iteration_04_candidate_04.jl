# Alignment-gated posterior propulsion around the evidenced bounded-curvature
...
function target_policy(state, params)
    ...
    # Persistent target error asks for bounded average curvature. The head
    # points along body -x, so positive bearing is reduced by negative theta
    # rate. Only that toward-target yaw releases the request.
    bearing = clamp(
    ...
    steering_error = bearing + params.yaw_response_damping * heading_rate
