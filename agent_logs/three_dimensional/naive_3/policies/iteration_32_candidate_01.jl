# Active-wave terminal half-cycle around the course-hold C-turn scaffold.
...
function target_policy(state, params)
    ...
    # Add terminal steering only to the half-cycle already moving into the
    # course-requested bend. This motion gate is exactly zero for a parked
    # C-bend and for the opposite half-cycle, so it cannot create or maintain
    # the static equilibrium seen in the failed descendants.
    ...
    terminal_halfcycle_weight = target_behind_weight^2 *
        terminal_response_weight * halfcycle_motion_weight
