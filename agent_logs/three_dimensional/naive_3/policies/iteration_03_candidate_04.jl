# Alignment-gated posterior propulsion with state-phased return braking around
# the evidenced bounded-curvature carrier. Oscillation phase remains entirely
# in measured joint state.
...
function target_policy_params()
    return (
        ...
        return_half_cycle_brake_gain=2.5,
        command_limit=28.0,
    )
end
...
function target_policy(state, params)
    ...
    # Bias beat duty without adding static bend: dissipate only motion away
    # from the requested curvature. The construction is reflection-equivariant
    # and vanishes quadratically as the target returns to the body axis.
    away_half_cycle_speed = min(turn_request * qd1, 0.0)
    return_half_cycle_brake = -params.return_half_cycle_brake_gain *
        turn_request * away_half_cycle_speed
    raw_a1 = vdp_drive - omega^2 * centered_q1 + return_half_cycle_brake
