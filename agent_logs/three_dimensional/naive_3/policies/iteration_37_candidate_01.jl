# Response-released traveling wave around the phase-balanced terminal C-turn.
...
function target_policy_params()
    return (
        ...
        turn_response_onset=0.08,
        turn_response_scale=0.04,
        maximum_redirect_release=0.03,
...
function target_policy(state, params)
    ...
    # The assigned-parent counterphase burst produced much more useful yaw but
    # did not rotate translational course inward. Treat that response as the
    # release signal for a nonsteady bend-to-wave transition: after the strong
    # terminal C-turn is visibly turning the body in the requested direction,
    # unwind only a bounded fraction of its moving equilibrium. This gate is
    # body-frame and response based; at zero or wrong-sign yaw it remains off.
    useful_turn_rate = max(-response_direction * heading_rate, 0.0)
    turn_response_weight = 0.5 * (1 + tanh(
        (useful_turn_rate - params.turn_response_onset) /
            params.turn_response_scale,
    ))
    redirect_release = params.maximum_redirect_release *
        target_behind_weight^2 * terminal_distance_weight *
        terminal_course_weight * turn_response_weight
    redirect_weight = base_redirect_weight * (1 - redirect_release)
    ...
    # Preserve the evidenced phase-balanced carrier about the moving C-turn
    # equilibrium. When terminal activity is deficient, odd velocity feedback
    # energizes both measured half-cycles without shifting the mean bend. As
    # redirect_weight releases above, the same state-feedback wave envelope is
    # restored instead of adding another signed acceleration burst.
