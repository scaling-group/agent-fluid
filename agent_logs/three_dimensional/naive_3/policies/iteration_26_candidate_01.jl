# Terminal propulsive release around the evidenced course-response C-turn.
# Oscillation and release selection remain entirely in measured state.
...
function target_policy_params()
    return (
        ...
        terminal_distance_scale=2.0,
        terminal_distance_power=12.0,
        terminal_closing_threshold=0.15,
        terminal_closing_scale=0.12,
        terminal_activity_threshold=0.12,
        terminal_activity_scale=0.04,
        maximum_terminal_release=0.45,
        command_limit=28.0,
    )
end
...
function target_policy(state, params)
    ...
    # head. The full target direction therefore selects a separate recovery
    ...
    # The sampled C-turn survives but remains on a powered tangential orbit.
    # Add curvature, rather than removing carrier or redirect authority, only
    # when a nearby target is fully behind and normalized course is nonclosing.
    # A positive radial course response continuously removes just this reserve.
    ...
    # The improved trace later approaches inside 2L with useful inward course
    # but almost no joint motion. Select a partial release only in that
    # near/behind/closing/quiet state: straighten the equilibrium and restore
    # the posterior traveling wave without a clock or hidden maneuver stage.
    ...
    terminal_closing_weight = 0.5 * (1 + tanh(
        (course_dot - params.terminal_closing_threshold) /
            params.terminal_closing_scale,
    ))
    joint_activity = hypot(qd1, qd2) /
        max(omega * amp, eps(Float64))
    terminal_quiet_weight = 0.5 * (1 + tanh(
        (params.terminal_activity_threshold - joint_activity) /
            params.terminal_activity_scale,
    ))
    terminal_release_weight = params.maximum_terminal_release *
        terminal_distance_weight * redirect_weight *
        terminal_closing_weight * terminal_quiet_weight
    released_redirect_curvature =
        (1 - terminal_release_weight) * redirect_curvature
    anterior_mean = (1 - redirect_weight) * mean_curvature +
        redirect_weight * released_redirect_curvature
    base_redirect_wave_scale = 1 -
        (1 - params.redirect_wave_floor) * redirect_weight
    redirect_wave_scale = base_redirect_wave_scale +
        (1 - base_redirect_wave_scale) * terminal_release_weight
    ...
    tail_mean = (1 - redirect_weight) *
        ...
        redirect_weight * params.redirect_tail_share *
            released_redirect_curvature
