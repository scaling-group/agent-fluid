# Phase-balanced anterior carrier with load-selected terminal tail compliance.
# Slow route geometry and fast hydrodynamic timing remain separate feedback.
...
function target_policy_params()
    return (
        ...
        helpful_lateral_force_scale=8.0e-4,
        helpful_lateral_force_threshold=0.50,
        helpful_lateral_force_transition=0.20,
        maximum_tail_compliance=0.35,
...
function target_policy(state, params)
    ...
    lateral_force = Float64(state.force_body_L[2])
    ...
    # Preserve the evidenced C-turn equilibrium. When phase-plane activity is
    # deficient, bounded velocity-odd energy on both half-cycles keeps the
    # anterior bend moving without shifting its mean or choosing a turn side.
    ...
    # Body-frame lateral force is a fast gait/load cue, never a route command.
    # Yield some posterior tracking only when its signed half-cycle is associated
    # with inward course rotation in the sampled histories. The slow course
    # error still selects direction, and the nominal lag target is unchanged.
    normalized_helpful_load = course_direction * lateral_force /
        max(params.helpful_lateral_force_scale, eps(Float64))
    helpful_load_weight = 0.5 * (1 + tanh(
        (normalized_helpful_load -
            params.helpful_lateral_force_threshold) /
            params.helpful_lateral_force_transition,
    ))
    tail_compliance_weight = target_behind_weight^2 *
        terminal_response_weight * helpful_load_weight
    tail_tracking_scale = 1 -
        params.maximum_tail_compliance * tail_compliance_weight
    raw_a2 = tail_tracking_scale * (
