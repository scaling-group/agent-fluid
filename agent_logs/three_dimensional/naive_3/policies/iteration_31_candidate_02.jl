# Target-behind wave restart around the terminal course-hold C-turn scaffold.
...
function target_policy_params()
    return (
        ...
        wave_restart_activity_threshold=0.12,
        wave_restart_activity_scale=0.02,
        maximum_wave_restart_acceleration=3.0,
...
function target_policy(state, params)
    ...
    # the already commanded equilibrium. Only after a full target crossing,
    # nudge an abnormally weak rhythm in the course-selected turn direction;
    # release the nudge as activity returns and let the unchanged posterior lag
    ...
    low_wave_activity_weight = 0.5 * (1 + tanh(
        (params.wave_restart_activity_threshold - wave_activity) /
            params.wave_restart_activity_scale,
    ))
    wave_restart_weight = target_behind_weight^2 *
        terminal_response_weight * low_wave_activity_weight
    wave_restart_acceleration =
        -params.maximum_wave_restart_acceleration * wave_restart_weight *
            response_direction
