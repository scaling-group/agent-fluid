# Gait-stall release around the evidenced terminal course-hold C-turn.
...
function target_policy_params()
    return (
        ...
        stall_joint_activity_threshold=0.025,
        stall_joint_activity_scale=0.008,
        stall_course_error_threshold=1.25,
        stall_course_error_scale=0.20,
...
function target_policy(state, params)
    ...
    nominal_response_weight = 1 -
        (1 - base_response_weight) * (1 - terminal_response_weight)
    redirect_weight = 1 -
        (1 - target_behind_weight) * (1 - terminal_response_weight)

    # A fully-behind terminal miss can park both joints in a nearly static
    # C-bend. Detect collapse relative to the carrier's natural joint-speed
    # scale and smoothly release only the added course-response reserve. The
    # retained base redirect moves the equilibrium enough to restart the
    # state-feedback wave; healthy joint motion suppresses this release.
    joint_activity = hypot(qd1, qd2) /
        max(omega * amp, eps(Float64))
    low_joint_activity_weight = 0.5 * (1 + tanh(
        (params.stall_joint_activity_threshold - joint_activity) /
            params.stall_joint_activity_scale,
    ))
    large_course_error_weight = 0.5 * (1 + tanh(
        (abs(course_error) - params.stall_course_error_threshold) /
            params.stall_course_error_scale,
    ))
    gait_stall_release_weight = target_behind_weight^2 *
        terminal_distance_weight * large_course_error_weight *
        low_joint_activity_weight
    response_weight = nominal_response_weight *
        (1 - gait_stall_release_weight)
