# Response-released C-bend burst around the evidenced target-response brake.
...
function target_policy_params()
    return (
        ...
        burst_distance_scale=4.0,
        burst_distance_power=12.0,
        burst_speed_scale=0.20,
        burst_error_threshold=0.75,
        burst_error_transition=0.20,
        burst_error_scale=0.75,
        maximum_burst_curvature=7.0 * pi / 180,
        burst_tail_share=0.60,
        burst_wave_floor=0.60,
        burst_anterior_damping=0.05,
        command_limit=28.0,
    )
end
...
function target_policy(state, params)
    ...
    # The target-ray/course mismatch is slow relative to gait yaw. Finite speed
    # makes the course direction well-defined; distance localizes the maneuver
    # to the evidenced loss of closure. Every quantity is body-relative.
    ...
    burst_approach_weight = 1 / (1 +
        (distance / params.burst_distance_scale)^params.burst_distance_power)
    burst_speed_weight = speed / (speed + params.burst_speed_scale)
    burst_error_weight = 0.5 * (1 + tanh(
        (abs(course_error) - params.burst_error_threshold) /
            params.burst_error_transition,
    ))
    burst_weight = burst_approach_weight * burst_speed_weight *
        burst_error_weight
    burst_direction = tanh(course_error / params.burst_error_scale)
    burst_curvature = params.maximum_burst_curvature * burst_weight *
        burst_direction

    # Large persistent mismatch temporarily displaces both joint equilibria to
    # the same bend side. Extra state damping makes this a C-bend preparation,
    # rather than another static offset riding on an unchanged oscillation.
    # Translational course correction continuously releases the ordinary wave.
    anterior_mean = mean_curvature + burst_curvature
    centered_q1 = q1 - anterior_mean
    vdp_drive = params.oscillator_mu *
        (1 - (centered_q1 / amp)^2) * qd1
    burst_damping = 2 * params.burst_anterior_damping * omega *
        burst_weight * qd1
    raw_a1 = vdp_drive - omega^2 * centered_q1 - burst_damping
    ...
    # During preparation the posterior joint joins the same-sign C-bend and
    # the oscillatory wave is attenuated. As course error falls, the burst
    # weight vanishes and the original posterior mean, lag, and wave return.
    tail_mean = params.tail_curvature_share * mean_curvature +
        params.burst_tail_share * burst_curvature
    burst_wave_authority = 1 -
        (1 - params.burst_wave_floor) * burst_weight
    posterior_wave = -centered_q1 -
        params.tail_lag_gain * qd1 / max(omega, eps(omega))
    phase_lag_target = tail_mean +
        burst_wave_authority * propulsion_scale *
