# Velocity-ray intercept guidance around the evidenced geometry-released
# C-turn scaffold. Oscillation phase remains entirely in measured joint state.
...
function target_policy_params()
    return (
        ...
        intercept_distance_scale=4.0,
        intercept_distance_power=8.0,
        intercept_speed_scale=0.20,
        intercept_forward_threshold=0.05,
        intercept_forward_scale=0.12,
        intercept_closure_threshold=0.20,
        intercept_closure_scale=0.10,
        intercept_miss_scale=0.75,
        maximum_intercept_curvature=16.0 * pi / 180,
...
function target_policy(state, params)
    ...
    # A straight continuation of the measured velocity predicts the signed
    # transverse miss distance. Correct that collision-course error only while
    # the target is ahead and closing inside a terminal envelope. This is not a
    # raw sideslip term: the velocity is projected against the current target
    # ray, and zero speed removes the correction continuously.
    target_forward = -target_unit_x
    direction_error = atan(target_y, -target_x)
    signed_miss_distance = distance * course_cross
    intercept_direction = tanh(
        signed_miss_distance / params.intercept_miss_scale,
    )
    intercept_miss_weight = intercept_direction^2
    intercept_distance_weight = 1 / (1 +
        (distance / params.intercept_distance_scale)^params.intercept_distance_power)
    intercept_forward_weight = 0.5 * (1 + tanh(
        (target_forward - params.intercept_forward_threshold) /
            params.intercept_forward_scale,
    ))
    intercept_closure_weight = 0.5 * (1 + tanh(
        (course_dot - params.intercept_closure_threshold) /
            params.intercept_closure_scale,
    ))
    intercept_speed_ratio = speed / (speed + params.intercept_speed_scale)
    intercept_speed_weight = intercept_speed_ratio^2
    intercept_weight = intercept_distance_weight *
        intercept_forward_weight * intercept_closure_weight *
        intercept_speed_weight * intercept_miss_weight
    intercept_curvature = params.maximum_intercept_curvature *
        intercept_direction
    approach_curvature = (1 - intercept_weight) * mean_curvature +
        intercept_weight * intercept_curvature

    # Acute bearing cannot distinguish a lateral target from one behind the
    # head. Preserve the sampled full-direction C-turn only after normalized
    # forward projection becomes negative; the intercept is then released.
    ...
    redirect_curvature = -params.maximum_redirect_curvature *
        redirect_direction
    anterior_mean = (1 - redirect_weight) * approach_curvature +
    ...
    # whose measured yaw grows that signed direction error.
    direction_sign = tanh(
    ...
    # Modulate posterior lag with joint velocity phase instead of adding
    # another equilibrium bend. The course-direction/velocity product is
    # reflection invariant: mirrored geometry changes both signs, while the
    # resulting posterior target and acceleration mirror normally. The two
    # half-cycles receive opposite lag changes, producing a transient
    # opposite-sign S-bend only while the near-target course error persists.
    ...
    tail_mean = (1 - redirect_weight) *
        params.tail_curvature_share * approach_curvature +
