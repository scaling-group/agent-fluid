# Response-selected posterior counterstroke around the evidenced
...
function target_policy(state, params)
    ...
    # Persistent body-frame target error asks for bounded mean curvature.
    # Measured yaw releases that request as the body responds.
    bearing = clamp(
    ...
    # steering curvature. Alignment restores the full lagged traveling wave.
    ...
    # Full direction preserves ahead/behind information hidden by the acute
    # bearing. Near the target, signed direction and measured error-growing yaw
    # select the same half-cycle that produced the best sampled brake.
    ...
    approach_weight = 1 / (1 +
        (distance / params.counterstroke_distance_scale)^
            params.counterstroke_distance_power)
    response_weight = approach_weight * lateral_weight * wrong_way_weight

    # Reallocate instead of discard the authority removed by the brake. At a
    # full response gate the multiplier is 2*retained-1; with the evidenced
    # retained fraction this is a bounded -0.30 counterstroke. The smooth map
    # is exactly +1 outside the gate and never exceeds the cruise magnitude.
    wave_multiplier = 1 - 2 *
        (1 - params.posterior_retained_fraction) * response_weight
    phase_lag_target = tail_mean + wave_multiplier *
