# Receding-overshoot posterior wave-direction reversal around the evidenced
# phase-lag carrier. Oscillation phase remains entirely in measured joint state.
...
function target_policy_params()
    return (
        ...
        reversal_distance_scale=6.0,
        reversal_distance_power=8.0,
        reversal_longitudinal_threshold=0.15,
        reversal_longitudinal_transition=0.05,
        reversal_receding_speed_threshold=0.05,
        reversal_receding_speed_transition=0.04,
        maximum_lag_reversal_fraction=0.75,
...
function target_policy(state, params)
    ...
    # Reverse the posterior phase order only after a geometrically confirmed
    # overshoot: the target is clearly behind, nearby, and distance is growing.
    # The smooth selector restores the original lag on renewed closure without
    # a timer or mutable recovery stage.
    longitudinal_fraction = target_x / distance
    behind_weight = 0.5 * (1 + tanh(
        (longitudinal_fraction - params.reversal_longitudinal_threshold) /
            params.reversal_longitudinal_transition,
    ))
    closing_speed = clamp(
    ...
    receding_weight = 0.5 * (1 + tanh(
        (receding_speed - params.reversal_receding_speed_threshold) /
            params.reversal_receding_speed_transition,
    ))
    reversal_distance_weight = 1 / (1 +
        (distance / params.reversal_distance_scale)^params.reversal_distance_power)
    reversal_weight = behind_weight * receding_weight *
        reversal_distance_weight
    ...
    # Retain the evidenced state-phased lag modulation on the inbound pass.
    # Once overshoot is confirmed, the bounded negative lag reverses phase
    # order without removing the anterior rhythm or target-directed mean bend.
    ...
    lag_direction = 1 - 2 * params.maximum_lag_reversal_fraction *
        reversal_weight
    tail_mean = params.tail_curvature_share * mean_curvature
    posterior_wave = -centered_q1 -
        lag_direction * modulated_tail_lag * qd1 /
            max(omega, eps(omega))
