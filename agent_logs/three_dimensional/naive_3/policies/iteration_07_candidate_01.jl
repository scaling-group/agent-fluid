# Wrong-side course guard around the evidenced alignment-gated carrier. The
# joint-state traveling wave is unchanged; only posterior mean bend is added.
...
function target_policy_params()
    return (
        ...
        maximum_posterior_course_bias=5.0 * pi / 180,
        command_limit=28.0,
    )
end
...
function target_policy(state, params)
    ...
    # Add posterior bias only while measured lateral translation opposes the
    # normalized target side. This sign-safe guard is zero at rest, releases
    # for corrective motion, and mirrors exactly with target and velocity.
