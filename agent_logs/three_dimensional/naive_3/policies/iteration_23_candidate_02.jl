# One-sided posterior phase reset around the evidenced target-response brake.
...
function target_policy_params()
    return (
        ...
        maximum_tail_lag_reduction=0.50,
        command_limit=28.0,
    )
end
...
function target_policy(state, params)
    ...
    # Compare normalized body-frame target and translational-course directions.
    # Distance and speed gates keep the phase action local to a finite approach
    # with a defined course, while course error supplies its corrective sign.
    ...
    # Joint state selects posterior timing without a clock. Mirroring flips
    # both course error and qd1, so their product and the scalar lag remain
    # invariant while the joint commands negate normally. Retain only the
    # lag-reducing half-cycle evidenced at the best sampled approach; the
    # opposite half-cycle keeps the carrier lag and full wave amplitude.
    ...
    lag_reduction_phase = min(velocity_phase, 0.0)
    modulated_tail_lag = params.tail_lag_gain +
        params.maximum_tail_lag_reduction * phase_weight * lag_reduction_phase

    # Keep the carrier's posterior mean exactly outside the brake and phase
    # reset. The mechanism changes timing, not equilibrium or wave amplitude.
