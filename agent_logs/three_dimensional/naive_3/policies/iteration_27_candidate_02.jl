# Terminal line-of-sight-rate feedback around the evidenced course-response
# C-turn. Oscillation phase remains entirely in measured joint state.
...
function target_policy_params()
    return (
        ...
        terminal_rate_scale=0.30,
        maximum_terminal_rate_curvature=4.0 * pi / 180,
...
function target_policy(state, params)
    ...
    # Preserve the sampled response-selected turn reserve: add bounded
    # curvature only for a nearby, fully-behind target on a nonclosing course.
    # Positive radial response removes only this reserve, never the C-turn.
    ...
    # Static curvature persistence produced only tighter terminal orbits. In
    # the terminal envelope, instead regulate body turn rate against inertial
    # line-of-sight rotation. The cross product is frame invariant; its signed
    # residual vanishes when measured yaw follows the target ray and reverses
    # naturally if yaw overshoots.
    line_of_sight_rate = (target_y * velocity_x - target_x * velocity_y) /
    ...
    terminal_rate_error = line_of_sight_rate - heading_rate
    terminal_rate_curvature = params.maximum_terminal_rate_curvature *
        terminal_distance_weight * response_speed_weight *
        tanh(terminal_rate_error / params.terminal_rate_scale)
    anterior_mean = (1 - redirect_weight) * mean_curvature +
        redirect_weight * redirect_curvature + terminal_rate_curvature
    ...
    tail_mean = (1 - redirect_weight) *
        ...
        params.redirect_tail_share * terminal_rate_curvature
