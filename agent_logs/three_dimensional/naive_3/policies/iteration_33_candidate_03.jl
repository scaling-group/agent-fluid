# Terminal alternating-energy hold around the course-response C-turn scaffold.
...
function target_policy_params()
    return (
        ...
        wave_energy_activity_threshold=0.18,
        wave_energy_activity_scale=0.03,
        wave_energy_velocity_scale=0.12,
        maximum_wave_energy_acceleration=2.0,
...
function target_policy(state, params)
    ...
    # A terminal C-turn must retain alternating stroke energy rather than park
    # at its steering equilibrium. Measure phase-plane activity about that
    # unchanged equilibrium and, only under the existing terminal response
    # gate, add bounded acceleration in the direction of measured joint
    # velocity. This amplifies whichever half-cycle already exists; it neither
    # chooses a turn side nor applies a one-sided restart kick.
    ...
    low_wave_activity_weight = 0.5 * (1 + tanh(
        (params.wave_energy_activity_threshold - wave_activity) /
            params.wave_energy_activity_scale,
    ))
    wave_energy_weight = target_behind_weight^2 *
        terminal_response_weight * low_wave_activity_weight
    wave_energy_acceleration = params.maximum_wave_energy_acceleration *
        wave_energy_weight * tanh(
            normalized_wave_velocity / params.wave_energy_velocity_scale,
        )
    raw_a1 = vdp_drive - omega^2 * centered_q1 +
        wave_energy_acceleration
