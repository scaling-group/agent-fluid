# Course-relative steering around the evidenced bounded-curvature carrier.
...
function target_policy(state, params)
    ...
    # The spine's forward axis is negative body x. Full target direction avoids
    # the acute-bearing alias after a pass. Once translation is established,
    # compare that direction with measured velocity course so tailbeat yaw does
    # not masquerade as a navigation-scale turn response.
    ...
    course_direction = atan(velocity_y, -velocity_x)
    course_error = atan(
    ...
    course_confidence = speed_squared /
        (speed_squared + params.course_speed_scale^2)
    steering_error = (1 - course_confidence) * bounded_direction +
    ...
    # Full-direction misalignment attenuates posterior thrust without removing
    # mean steering curvature, including when the target has passed behind.
    normalized_direction =
        target_direction / params.alignment_direction_scale
