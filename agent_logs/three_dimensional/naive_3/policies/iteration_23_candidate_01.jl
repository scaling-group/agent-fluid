# Course-selected yaw-moment rejection around the evidenced posterior
...
function target_policy_params()
    return (
        ...
        moment_rejection_scale=0.004,
        maximum_moment_counterbend=5.0 * pi / 180,
        command_limit=28.0,
    )
end
...
function target_policy(state, params)
    ...
    # Measured yaw moment is an earlier hydrodynamic response than accumulated
    # yaw. Reject only the sign that grows the persistent course error. The
    # signed product is reflection invariant, while the opposite posterior
    # displacement mirrors with the target/course geometry. Useful moment and
    # the far-field traveling wave are left unchanged.
    yaw_moment = Float64(state.moment_z_L2)
    harmful_moment = max(course_direction * yaw_moment, 0.0)
    moment_rejection_weight = tanh(
        harmful_moment / params.moment_rejection_scale,
    )^2
    moment_counterbend = -params.maximum_moment_counterbend *
        course_direction * phase_weight * moment_rejection_weight

    tail_mean = params.tail_curvature_share * mean_curvature
    posterior_wave = -centered_q1 -
        modulated_tail_lag * qd1 / max(omega, eps(omega))
    phase_lag_target = tail_mean + moment_counterbend +
