# Behind-only target-ray-rate hold around the evidenced terminal C-turn.
...
function target_policy_params()
    return (
        ...
        ray_rate_distance_scale=1.65,
        ray_rate_distance_power=12.0,
        ray_rate_behind_power=4.0,
        ray_rate_error_scale=0.30,
...
function target_policy(state, params)
    ...
    # At the strongest sampled late miss, the target ray rotates clockwise
    # substantially faster than the body. Convert that measured rate deficit
    # into a small equilibrium reserve. The high-order behind gate makes this
    # mechanism effectively absent on the ahead-side first recovery.
    target_ray_rate = (target_y * velocity_x - target_x * velocity_y) /
        target_norm^2
    ray_rate_error = target_ray_rate - heading_rate
    ray_rate_distance_weight = 1 / (1 +
        (distance / params.ray_rate_distance_scale)^params.ray_rate_distance_power)
    ray_rate_behind_weight =
        target_behind_weight^params.ray_rate_behind_power
    ray_rate_curvature = params.maximum_ray_rate_curvature *
        ray_rate_distance_weight * ray_rate_behind_weight *
        tanh(ray_rate_error / params.ray_rate_error_scale)
    held_redirect_curvature = redirect_curvature + ray_rate_curvature
    anterior_mean = (1 - redirect_weight) * mean_curvature +
        redirect_weight * held_redirect_curvature
    ...
    # sampled terminal hold plus behind-only rate reserve into the posterior
    # equilibrium without changing the traveling-wave carrier.
    ...
    tail_mean = (1 - redirect_weight) *
        ...
        redirect_weight * params.redirect_tail_share * held_redirect_curvature
