# Phase-balanced terminal energy with response-released anterior redirect.
...
function target_policy_params()
    return (
        ...
        terminal_alignment_turn_rate=0.35,
        terminal_turn_rate_limit=1.4,
        terminal_turn_error_scale=0.35,
        maximum_terminal_burst_acceleration=6.0,
...
function target_policy(state, params)
    ...
    # A tangential orbit can match the target-ray rate without reducing course
    # error. Ask for a bounded additional closing turn and measure the missing
    # response directly in body yaw. This supplies the release condition for a
    # nonsteady redirect without a clock or maneuver stage.
    target_ray_rate = -speed * course_cross / distance
    desired_terminal_heading_rate = clamp(
        target_ray_rate -
            params.terminal_alignment_turn_rate * course_direction,
        -params.terminal_turn_rate_limit,
        params.terminal_turn_rate_limit,
    )
    terminal_turn_error = desired_terminal_heading_rate - heading_rate
    terminal_turn_deficit_direction = tanh(
        -terminal_turn_error / params.terminal_turn_error_scale,
    )
    ...
    # half-cycles. This keeps the quiet bend locally unstable without moving
    # the mean curvature or selecting a lateral sign.
    ...
    # Put the response-released C-start role on the anterior joint. The sign
    # bends toward the requested course-closing yaw; the term vanishes as the
    # measured turn catches the request. It does not alter the moving mean,
    # prescribe a half-cycle, or disturb the evidenced symmetric energy term.
    terminal_burst_weight = target_behind_weight^2 * terminal_response_weight
    terminal_burst_acceleration =
        -params.maximum_terminal_burst_acceleration *
            terminal_burst_weight * terminal_turn_deficit_direction
    ...
    # Preserve the evidenced approach phase modulation and the lagged
    # posterior traveling-wave role. Terminal yaw response is handled directly
    # at the anterior joint rather than perturbing posterior phase propagation.
