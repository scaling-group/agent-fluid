# Closing-responsive harmful-half-cycle notch around the sampled posterior
# phase-lag scaffold. Oscillation phase remains entirely in joint state.
...
function target_policy_params()
    return (
        ...
        notch_lateral_threshold=0.70,
        notch_lateral_scale=0.15,
        notch_closing_threshold=0.20,
        notch_closing_scale=0.12,
        notch_phase_scale=0.25,
        posterior_notch_floor=0.25,
        command_limit=28.0,
    )
end
...
function target_policy(state, params)
    ...
    # The sampled phase controller still carries finite speed through a large
    # lateral miss. Joint velocity and yaw are strongly anticorrelated in that
    # trace, so -direction*qd1 selects the half-cycle that yaws away from the
    # lateral target without introducing a clock. Suppress only that posterior
    # wave when the target is lateral and instantaneous closing response has
    # fallen; the opposite half-cycle retains full propulsive authority.
    lateral_fraction = abs(target_y) / target_norm
    notch_lateral_weight = 0.5 * (1 + tanh(
        (lateral_fraction - params.notch_lateral_threshold) /
            params.notch_lateral_scale,
    ))
    closing_speed = Float64(state.closing_speed_L)
    notch_closing_weight = 0.5 * (1 + tanh(
        (params.notch_closing_threshold - closing_speed) /
            params.notch_closing_scale,
    ))
    harmful_phase = -direction_sign * normalized_joint_velocity
    notch_phase_weight = 0.5 * (1 + tanh(
        harmful_phase / params.notch_phase_scale,
    ))
    notch_weight = phase_approach_weight * notch_lateral_weight *
        notch_closing_weight * notch_phase_weight
    posterior_notch_scale = 1 -
        (1 - params.posterior_notch_floor) * notch_weight
    ...
    phase_lag_target = tail_mean +
        propulsion_scale * posterior_notch_scale * posterior_wave_authority *
            posterior_wave
