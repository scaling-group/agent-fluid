# Measured-response posterior counterbend around the evidenced half-cycle
...
function target_policy_params()
    return (
        ...
        maximum_response_counterbend=5.0 * pi / 180,
        command_limit=28.0,
    )
end
...
function target_policy(state, params)
    ...
    # acute bearing. During approach, measured yaw selects only the response
    # intervals that grow the signed direction error. Joint-wave phase is not
    # treated as a proxy for body response.
    ...
    # Preserve the parent's successful response-selective brake. Reallocate
    # part of only the removed wrong-way wave authority into an opposite-sign
    # posterior equilibrium shift; corrective yaw releases both mechanisms.
    propulsion_scale = 1 -
        (1 - params.posterior_brake_floor) * response_weight
    response_counterbend = params.maximum_response_counterbend *
        response_weight * direction_sign
    tail_mean = params.tail_curvature_share * mean_curvature -
        response_counterbend
