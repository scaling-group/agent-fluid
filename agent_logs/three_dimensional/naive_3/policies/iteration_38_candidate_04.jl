# Force-selected posterior impulse around the phase-balanced terminal C-turn.
...
function target_policy_params()
    return (
        ...
        maximum_terminal_force_energy_acceleration=1.25,
...
function target_policy(state, params)
    ...
    # Preserve the evidenced moving C-turn equilibrium and restore deficient
    # anterior activity with velocity-odd energy on both measured half-cycles.
    # This is the exact phase-balanced scaffold that produced the 1.175L pass.
    ...
    # The phase-balanced scaffold still passes tangentially. Treat measured
    # body force as a fast response selector, not a route command: positive
    # course_turn_force means the current hydrodynamic impulse is rotating the
    # translational course toward the body-frame target ray. Only then, and
    # only under the established target-behind terminal gate, add bounded
    # velocity-odd energy to the posterior joint. Mean bend and nominal lag do
    # not move, and unhelpful force or zero posterior motion gives zero work.
    ...
    force_selected_posterior_energy =
        params.maximum_terminal_force_energy_acceleration *
            target_behind_weight^2 * terminal_response_weight *
            helpful_force_weight * tanh(
