...
function target_policy_params()
    return (
        ...
        posterior_counterstroke_fraction=1.0,
        command_limit=28.0,
    )
end
...
function target_policy(state, params)
    ...
    # acute bearing. During approach, measured error-growing yaw selects the
    # posterior response half-cycle; corrective yaw leaves the carrier intact.
    ...
    # The best sampled intervention only discarded the selected authority.
    # Reverse that removed portion into a bounded counterstroke. At a fully
    # active gate this changes the posterior wave coefficient from +0.35 to
    # -0.30; outside the gate the evidenced traveling wave is exactly retained.
    removed_wave_scale = 1 - propulsion_scale
    counterstroke_scale = params.posterior_counterstroke_fraction *
        removed_wave_scale
    selected_wave_scale = propulsion_scale - counterstroke_scale

    phase_lag_target = tail_mean + selected_wave_scale *
