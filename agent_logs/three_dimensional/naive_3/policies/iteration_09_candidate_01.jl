# Target-ray velocity-deficit counter-bend around the evidenced alignment-gated
...
function target_policy_params()
    return (
        ...
        lateral_speed_deficit_scale=0.25,
...
function target_policy(state, params)
    ...
    # Ask for lateral translation along the normalized target ray. A bounded
    # posterior S-bend persists while measured lateral velocity is inadequate
    # on that side, including the broadside terminal miss where the prior
    # wrong-side-only guard faded. The smooth side factor suppresses far-field
    # centerline intervention and makes the construction reflection-equivariant.
    ...
    lateral_speed_deficit = max(
        0.0,
        target_side * (desired_lateral_velocity - lateral_velocity),
    )
    counterbend_guard = target_side * tanh(
        lateral_speed_deficit / params.lateral_speed_deficit_scale,
    )^2
