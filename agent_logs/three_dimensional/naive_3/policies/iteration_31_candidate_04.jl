# Phase-restart guard around the terminal rear-crossing C-turn scaffold.
...
function target_policy_params()
    return (
        ...
        restart_phase_radius_threshold=0.04,
        restart_phase_radius_scale=0.012,
        restart_turn_response_threshold=0.45,
        restart_turn_response_scale=0.12,
        maximum_restart_acceleration=6.0,
...
function target_policy(state, params)
    ...
    # Lateral target side becomes ambiguous as a target behind the head crosses
    # the rear centerline. Only under the existing terminal hold, blend the
    # base C-turn direction toward the persistent target-ray/course error.
    # This changes maneuver selection without adding curvature or drive.
    course_consistent_redirect_direction = redirect_direction +
        terminal_response_weight * (response_direction - redirect_direction)
    base_redirect_curvature = -params.maximum_redirect_curvature *
        course_consistent_redirect_direction
    ...
    # Several sampled redirect descendants coast at finite body speed after
    # both joints collapse near the state-dependent C-bend equilibrium.  Seed
    # the existing oscillator only in that measured low-phase-radius state.
    # Target-behind/nonclosing geometry chooses the impulse sign, while joint
    # motion and useful yaw release it; neither equilibrium nor envelope moves.
    normalized_phase_position = centered_q1 / effective_amp
    normalized_phase_velocity = qd1 /
        max(omega * effective_amp, eps(Float64))
    anterior_phase_radius = hypot(
        normalized_phase_position,
        normalized_phase_velocity,
    )
    collapsed_phase_weight = 0.5 * (1 + tanh(
        (params.restart_phase_radius_threshold - anterior_phase_radius) /
            params.restart_phase_radius_scale,
    ))
    useful_turn_response = -response_direction * heading_rate
    weak_turn_response_weight = 0.5 * (1 + tanh(
        (params.restart_turn_response_threshold - useful_turn_response) /
            params.restart_turn_response_scale,
    ))
    restart_weight = base_response_weight * collapsed_phase_weight *
        weak_turn_response_weight
    restart_acceleration = -params.maximum_restart_acceleration *
        restart_weight * response_direction
    raw_a1 = vdp_drive - omega^2 * centered_q1 + restart_acceleration
