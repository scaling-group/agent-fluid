# Course-selected posterior half-cycle authority around the evidenced
...
function target_policy_params()
    return (
        ...
        posterior_halfcycle_floor=0.55,
        posterior_halfcycle_phase_scale=0.35,
        command_limit=28.0,
    )
end
...
function target_policy(state, params)
    ...
    # yaw, this route error persists across gait half-cycles. The speed weight
    # continuously removes the undefined zero-velocity direction.
    ...
    # Preserve the sampled phase-lag modulation. Joint-1 velocity supplies
    # state-only beat phase, and the reflection-invariant product changes lag
    # oppositely on the two half-cycles without a clock.
    ...
    # Attenuate only the posterior wave bend whose sign agrees with persistent
    # course error. Mirroring flips both signs and leaves the scale unchanged.
    # The opposite half-cycle and posterior mean retain full authority, while
    # this new action can never boost the raw wave or add a static redirect.
    tail_mean = params.tail_curvature_share * mean_curvature
    raw_posterior_wave = -centered_q1 -
        modulated_tail_lag * qd1 / max(omega, eps(omega))
    harmful_halfcycle = 0.5 * (1 + tanh(
        course_direction * raw_posterior_wave /
            (params.posterior_halfcycle_phase_scale * amp),
    ))
    posterior_halfcycle_scale = 1 -
        (1 - params.posterior_halfcycle_floor) * phase_weight *
            harmful_halfcycle
    posterior_wave = posterior_halfcycle_scale * raw_posterior_wave
