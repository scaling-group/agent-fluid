# Closure-windowed active half-cycle around the course-hold C-turn scaffold.
...
function target_policy(state, params)
    ...
    # course-requested bend. Positive target-ray closure defines a response
    # window: the pulse releases continuously when the pass becomes tangential
    # or receding, while the terminal hold and traveling carrier remain.
    ...
    positive_closure_weight = 0.5 * (1 + tanh(
        course_dot / params.terminal_course_dot_scale,
    ))
    terminal_halfcycle_weight = target_behind_weight^2 *
        terminal_response_weight * positive_closure_weight *
