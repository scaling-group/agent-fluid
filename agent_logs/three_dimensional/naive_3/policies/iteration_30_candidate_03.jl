# Target-behind turn-rate phase residual around the course-hold C-turn.
...
function target_policy_params()
    return (
        ...
        terminal_turn_rate_limit=1.2,
        terminal_turn_rate_error_scale=0.25,
        terminal_joint_phase_scale=0.10,
        maximum_terminal_lag_shift=0.45,
...
function target_policy(state, params)
    ...
    # Compare measured yaw with a target-crossing-time turn request. Only after
    # the target is behind and the terminal hold is active, convert that rate
    # error into a joint-phase-signed lag residual. The product of the two odd
    # signals is reflection-even, so neither joint equilibrium is displaced.
    ...
    requested_turn_rate = -response_direction * min(
        speed / distance,
        params.terminal_turn_rate_limit,
    )
    turn_rate_error = requested_turn_rate - heading_rate
    terminal_rate_gate = target_behind_weight^2 * terminal_response_weight
    terminal_rate_phase =
        tanh(turn_rate_error / params.terminal_turn_rate_error_scale) *
        tanh(normalized_joint_velocity / params.terminal_joint_phase_scale)
    modulated_tail_lag = params.tail_lag_gain +
        params.maximum_tail_lag_shift * phase_weight * velocity_phase +
        params.maximum_terminal_lag_shift * terminal_rate_gate *
            terminal_rate_phase
