# Geometry-released C-to-S recovery around the evidenced posterior phase-lag
# scaffold. Oscillation phase remains entirely in measured joint state.
...
function target_policy_params()
    return (
        ...
        counterstroke_forward_threshold=0.70,
        counterstroke_forward_scale=0.12,
        maximum_counterstroke_acceleration=8.0,
        command_limit=28.0,
    )
end
...
function target_policy(state, params)
    ...
    # head. Full target direction therefore selects recovery only after the
    # normalized forward projection becomes negative.
    ...
    # anterior equilibrium and envelope into a bounded C-turn without stopping
    # the traveling wave or introducing a hidden stage variable.
    ...
    # The horizon rollout held the C-bend while the target stayed deeply behind
    # and settled into a wide powered orbit. Once backward geometry and signed
    # course mismatch agree that recovery persists, add a bounded posterior
    # counterstroke opposite the anterior C-bend. Mirroring changes both
    # direction signs and mirrors the resulting posterior acceleration.
    counterstroke_geometry = 0.5 * (1 + tanh(
        (-target_forward - params.counterstroke_forward_threshold) /
            params.counterstroke_forward_scale,
    ))
    recovery_mismatch = max(
        redirect_direction * course_direction,
        0.0,
    )^2
    counterstroke_weight = redirect_weight * counterstroke_geometry *
        recovery_mismatch
    c_tail_mean = params.redirect_tail_share * redirect_curvature
    tail_mean = (1 - redirect_weight) *
        params.tail_curvature_share * mean_curvature +
        redirect_weight * c_tail_mean
    counterstroke_acceleration = params.maximum_counterstroke_acceleration *
        course_direction * counterstroke_weight
    ...
    raw_a2 = omega^2 * (phase_lag_target - q2) -
        2 * params.tail_damping * omega * qd2 + counterstroke_acceleration
