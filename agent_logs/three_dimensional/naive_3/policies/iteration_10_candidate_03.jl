# Differential target-ray course couple around the evidenced alignment-gated
...
function target_policy_params()
    return (
        ...
        maximum_anterior_course_couple=3.0 * pi / 180,
        command_limit=28.0,
    )
end
...
function target_policy(state, params)
    ...
    # Normalize the target ray before forming translational course error.
    # This keeps the mechanism independent of target coordinates and grid
    # resolution, while preserving the forward/behind distinction.
    ...
    # Retain the evidenced posterior S-shaped component when translation is
    # instantaneously opposite the target side.
    target_side = tanh(
    ...
    # Same-side translation can still cross the target ray. Envelope its
    # signed perpendicular velocity continuously so far-field propulsion and
    # the useful release route remain effectively unchanged.
    target_ray_course_residual =
        longitudinal_target_fraction * lateral_velocity -
        lateral_target_fraction * forward_velocity
    approach_ratio = distance / params.terminal_approach_distance_scale
    terminal_approach_weight =
        1 / (1 + approach_ratio^params.terminal_approach_power)
    terminal_course_guard = terminal_approach_weight * tanh(
        target_ray_course_residual / params.terminal_course_residual_scale,
    )

    # The inherited posterior-only residual improved closest approach but
    # remained saturated in the same lower-exit topology. Apply a weaker copy
    # to anterior equilibrium, opposite the posterior shift, to create a
    # bounded differential steering couple rather than another posterior gain.
    anterior_course_couple =
        params.maximum_anterior_course_couple * terminal_course_guard
    anterior_equilibrium = mean_curvature + anterior_course_couple
