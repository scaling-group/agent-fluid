# Closure-adaptive posterior phase-lag modulation around the evidenced
# target-response brake. Oscillation phase remains entirely in joint state.
...
function target_policy_params()
    return (
        ...
        response_lateral_threshold=0.60,
        response_lateral_scale=0.12,
        response_closure_threshold=0.55,
        response_closure_scale=0.15,
        phase_response_boost=1.0,
...
function target_policy(state, params)
    ...
    base_phase_weight = phase_approach_weight * phase_speed_weight *
        phase_error_weight
    course_direction = tanh(course_error / params.phase_error_scale)

    # A forming lateral miss is distinct from useful inbound sway only when
    # lateral target geometry and lost closure agree. Boosting the existing
    # phase action under that conjunction preserves its sign and the cruise
    # carrier; the bounded windowed response avoids an explicit stage or clock.
    lateral_target_fraction = abs(target_y) / target_norm
    lateral_response_weight = 0.5 * (1 + tanh(
        (lateral_target_fraction - params.response_lateral_threshold) /
            params.response_lateral_scale,
    ))
    closure_efficiency = Float64(state.window_closing_speed_L) /
        max(speed, params.phase_speed_scale)
    closure_loss_weight = 0.5 * (1 + tanh(
        (params.response_closure_threshold - closure_efficiency) /
            params.response_closure_scale,
    ))
    phase_response_weight = lateral_response_weight * closure_loss_weight
    phase_weight = clamp(
        base_phase_weight *
            (1 + params.phase_response_boost * phase_response_weight),
        0.0,
        1.0,
    )
