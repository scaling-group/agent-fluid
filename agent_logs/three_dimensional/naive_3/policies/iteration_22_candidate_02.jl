# Geometry-persistent posterior counterbend around the evidenced phase-lag
...
function target_policy_params()
    return (
        ...
        counterbend_distance_scale=4.0,
        counterbend_distance_power=8.0,
        counterbend_lateral_scale=0.55,
        maximum_posterior_counterbend=12.0 * pi / 180,
        counterbend_command_margin=1.0,
        command_limit=28.0,
    )
end
...
function target_policy(state, params)
    ...
    # Translational course is slower than beat-locked yaw. Its signed error
    # selects the already-evidenced posterior phase modulation, while speed
    # continuously removes the undefined stopped-course limit.
    ...
    # Persistent normalized lateral geometry, rather than instantaneous yaw or
    # beat side, owns the new posterior-only mean action. The odd signed weight
    # is reflection equivariant and releases continuously as geometry corrects.
    counterbend_approach_weight = 1 / (1 +
        (distance / params.counterbend_distance_scale)^
            params.counterbend_distance_power)
    normalized_lateral_target = target_y /
        (distance * params.counterbend_lateral_scale)
    lateral_counter_direction = tanh(normalized_lateral_target)
    lateral_counter_weight = lateral_counter_direction *
        abs(lateral_counter_direction)
    counterbend_weight = counterbend_approach_weight * phase_speed_weight *
        lateral_counter_weight
    ...
    # Preserve the sampled phase-lag modulation. Joint velocity supplies beat
    # phase; mirrored geometry and joint state produce mirrored acceleration.
    ...
    # Apply the new request only to posterior mean curvature, opposite the
    # persistent lateral error. Realize it through available command reserve
    # so the new action cannot create additional clamp residence on a replayed
    # state; the traveling-wave target itself remains unchanged.
    posterior_counterbend = params.maximum_posterior_counterbend *
        counterbend_weight
    ...
    base_raw_a2 = omega^2 * (phase_lag_target - q2) -
        2 * params.tail_damping * omega * qd2
    desired_counterbend_accel = -omega^2 * posterior_counterbend
    counterbend_limit = max(
        params.command_limit - params.counterbend_command_margin,
        0.0,
    )
    counterbend_reserve = desired_counterbend_accel >= 0 ?
        max(counterbend_limit - base_raw_a2, 0.0) :
        max(base_raw_a2 + counterbend_limit, 0.0)
    applied_counterbend_accel = clamp(
        desired_counterbend_accel,
        -counterbend_reserve,
        counterbend_reserve,
    )
    raw_a2 = base_raw_a2 + applied_counterbend_accel
