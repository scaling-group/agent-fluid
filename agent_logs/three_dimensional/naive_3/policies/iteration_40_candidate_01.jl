# Phase-coupled anterior release around the phase-balanced terminal C-turn.
...
function target_policy_params()
    return (
        ...
        recovery_phase_difference_scale=0.08,
        maximum_recovery_phase_coupling_acceleration=3.0,
...
function target_policy(state, params)
    ...
    # The sampled broad-loop failures converge to a quiet common C-bend before
    # the narrow terminal-energy gate becomes authoritative. Coupled-oscillator
    # phase propagation provides a distinct release: during an evidenced
    # target-behind, nonclosing stall, the measured anterior/posterior split
    # selects a mirror-equivariant anterior half-cycle. Resumed phase-plane
    # activity removes the coupling instead of moving the C-turn equilibrium.
    normalized_joint_phase_split = (q1 - q2) / effective_amp
    phase_coupled_release =
        params.maximum_recovery_phase_coupling_acceleration *
            base_response_weight * low_activity_weight * tanh(
                normalized_joint_phase_split /
                    params.recovery_phase_difference_scale,
            )
