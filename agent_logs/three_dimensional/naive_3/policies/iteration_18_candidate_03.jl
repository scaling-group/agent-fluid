# Course-selected anterior duty asymmetry around the evidenced target-response
...
function target_policy_params()
    return (
        ...
        duty_distance_scale=3.6,
        duty_distance_power=12.0,
        duty_speed_scale=0.25,
        duty_error_threshold=0.45,
        duty_error_transition=0.20,
        duty_error_scale=0.60,
        duty_phase_scale=0.35,
        maximum_restoring_asymmetry=0.25,
        command_limit=28.0,
    )
end
...
function target_policy(state, params)
    ...
    duty_approach_weight = 1 / (1 +
        (distance / params.duty_distance_scale)^params.duty_distance_power)
    speed_ratio = speed / (speed + params.duty_speed_scale)
    duty_speed_weight = speed_ratio^2
    duty_error_weight = 0.5 * (1 + tanh(
        (abs(course_error) - params.duty_error_threshold) /
            params.duty_error_transition,
    ))
    duty_weight = duty_approach_weight * duty_speed_weight * duty_error_weight
    course_direction = tanh(course_error / params.duty_error_scale)

    # Preserve the carrier equilibrium. Instead, slow the restoring passage on
    # the corrective bend side and hasten the opposite side. The product of
    # signed course direction and signed bend is reflection invariant, so a
    # mirrored target produces mirrored accelerations without a world route.
    centered_q1 = q1 - mean_curvature
    bend_side = tanh(
        course_direction * centered_q1 /
            (params.duty_phase_scale * amp),
    )
    restoring_scale = 1 -
        params.maximum_restoring_asymmetry * duty_weight * bend_side
    vdp_drive = params.oscillator_mu *
        (1 - (centered_q1 / amp)^2) * qd1
    raw_a1 = vdp_drive - restoring_scale * omega^2 * centered_q1
    ...
    # Keep the carrier's posterior mean exactly outside the brake. The new
    # mechanism changes only oscillatory authority on the evidenced wrong-way
    # response half-cycle; it adds no equilibrium redirect.
