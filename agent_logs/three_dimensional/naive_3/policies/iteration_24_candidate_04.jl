# Radial-response-released C-turn around the evidenced posterior phase-lag
...
function target_policy_params()
    return (
        ...
        redirect_closing_threshold=0.0,
...
function target_policy(state, params)
    ...
    # Body-behind geometry requests the evidenced recovery turn. Large sideslip
    # can make translation close on the target before the body rotates to put
    # it ahead, so normalized radial response releases rather than reinforces
    # that turn. At zero speed course_dot is neutral and retains half of the
    # geometrically selected recovery rather than inventing a course direction.
    target_forward = -target_unit_x
    direction_error = atan(target_y, -target_x)
    redirect_geometry_weight = 0.5 * (1 + tanh(
        (-target_forward - params.redirect_forward_threshold) /
            params.redirect_forward_scale,
    ))
    redirect_response_weight = 0.5 * (1 + tanh(
        (params.redirect_closing_threshold - course_dot) /
            params.redirect_closing_scale,
    ))
    redirect_weight = redirect_geometry_weight * redirect_response_weight
