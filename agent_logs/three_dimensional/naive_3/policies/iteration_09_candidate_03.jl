# Terminal target-geometry posterior counterbend around the evidenced
# alignment-gated carrier. Phase remains entirely in measured joint state.
...
function target_policy_params()
    return (
        ...
        terminal_target_side_scale=0.20,
        terminal_lateral_error_scale=0.45,
        terminal_distance_scale=3.2,
        terminal_distance_power=8.0,
        maximum_terminal_counterbend=7.0 * pi / 180,
        command_limit=28.0,
    )
end
...
function target_policy(state, params)
    ...
    # The sampled velocity-gated S-bend improved closest approach but released
    # on corrective portions of individual beats while the target was still
    # strongly lateral. Keep its opposite-sign posterior allocation, but make
    # persistence depend on normalized target geometry. A steep continuous
    # approach envelope leaves the evidenced far-field carrier unchanged.
    target_y = Float64(state.target_body_L[2])
    distance = max(Float64(state.distance_L), eps(Float64))
    lateral_target_fraction = clamp(target_y / distance, -1.0, 1.0)
    target_side = tanh(
        lateral_target_fraction / params.terminal_target_side_scale,
    )
    lateral_error_weight = tanh(
        abs(lateral_target_fraction) / params.terminal_lateral_error_scale,
    )^2
    approach_weight = 1 / (1 +
        (distance / params.terminal_distance_scale)^params.terminal_distance_power)
    terminal_counterbend = params.maximum_terminal_counterbend *
        approach_weight * lateral_error_weight * target_side

    tail_mean = params.tail_curvature_share * mean_curvature -
        terminal_counterbend
