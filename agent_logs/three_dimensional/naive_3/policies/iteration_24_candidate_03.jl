# Closing-response contraction around the evidenced geometry-released C-turn.
...
function target_policy_params()
    return (
        ...
        recovery_distance_scale=4.5,
        recovery_distance_power=8.0,
        recovery_speed_scale=0.15,
        recovery_closure_threshold=0.05,
        recovery_closure_scale=0.12,
        nonclosing_redirect_wave_floor=0.16,
        command_limit=28.0,
    )
end
...
function target_policy(state, params)
    ...
    # The sampled redirect survived but orbited at finite speed. Contract the
    # wave further only for a nearby, target-behind, nonclosing response. The
    # curvature equilibrium remains active, and positive closure continuously
    # restores the sampled recovery carrier.
    closing_speed = Float64(state.closing_speed_L)
    normalized_closure = clamp(
        closing_speed / (speed + params.recovery_speed_scale),
        -1.0,
        1.0,
    )
    recovery_distance_weight = 1 / (1 +
        (distance / params.recovery_distance_scale)^params.recovery_distance_power)
    nonclosing_weight = 0.5 * (1 + tanh(
        (params.recovery_closure_threshold - normalized_closure) /
            params.recovery_closure_scale,
    ))
    recovery_contraction = redirect_weight * recovery_distance_weight *
        nonclosing_weight
    recovery_wave_scale = base_redirect_wave_scale -
        (base_redirect_wave_scale - params.nonclosing_redirect_wave_floor) *
            recovery_contraction

    # Preserve the sampled carrier on approach. During recovery, move both its
    # equilibrium and limit-cycle envelope into the bounded response-selected
    # C-turn without stopping the traveling wave.
    ...
    # Preserve the evidenced joint-state phase modulation and blend the
    # posterior equilibrium into the same-curvature C-turn. Only the wave
    # envelope, never the redirect mean, receives the response contraction.
