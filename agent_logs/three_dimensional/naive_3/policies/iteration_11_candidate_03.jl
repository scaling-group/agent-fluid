# Post-pass-gated approach hold around the evidenced alignment-gated
...
function target_policy(state, params)
    ...
    # Retain the evidenced S-shaped response while body translation opposes the
    # target side. A target-ray course residual also responds to same-side
    # cross-track motion. Their shared clamp bounds the posterior equilibrium.
    ...
    # Preserve full posterior propulsion throughout the inbound pass. Only
    # once normalized body geometry puts the target behind may failed closure
    # and cross-track motion reduce the oscillatory wave. The mean S-bend is
    # outside this gate and therefore retains steering authority.
    target_ray_closing_speed =
        longitudinal_target_fraction * forward_velocity +
        lateral_target_fraction * lateral_velocity
    ...
    hold_closing_weight = 0.5 * (1 + tanh(
        (params.hold_closing_threshold - target_ray_closing_speed) /
            params.hold_closing_scale,
    ))
    hold_behind_weight = tanh(
        max(longitudinal_target_fraction, 0.0) / params.hold_behind_scale,
    )^2
    hold_weight = hold_behind_weight * hold_approach_weight *
        hold_course_weight * hold_closing_weight
