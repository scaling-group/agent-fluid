# Response-selected posterior phase-lag modulation around the evidenced brake.
...
function target_policy(state, params)
    ...
    # acute bearing. Near the target, weaken only the posterior wave interval
    # whose measured yaw grows the signed direction error.
    ...
    # Rotate the remaining posterior traveling wave on the selected interval.
    # Normalizing the joint-position/rate mixture preserves its nominal
    # sinusoidal amplitude, so the new mechanism changes lag rather than drive.
    selected_tail_lag = params.tail_lag_gain -
        params.maximum_tail_lag_shift * response_weight
    lag_amplitude_scale = sqrt(
        (1 + params.tail_lag_gain^2) / (1 + selected_tail_lag^2),
    )
    posterior_wave = lag_amplitude_scale *
        (-centered_q1 - selected_tail_lag *
