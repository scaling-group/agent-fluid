# Target-ray-drift lead around the evidenced alignment-gated bounded-curvature
...
function target_policy_params()
    return (
        ...
        target_ray_drift_limit=0.5,
        target_ray_drift_scale=0.12,
        target_ray_lead_gain=0.25,
...
function target_policy(state, params)
    ...
    # retains the strongest sampled turn response; all geometry is body-relative.
    bearing = clamp(
    ...
    # Bearing and yaw rates are measured over the same short history window.
    # Their difference cancels beat-scale body rotation and isolates the
    # translational sweep of the target ray. Lead only this bounded residual;
    # startup observations make it exactly zero without a clock or mode state.
    target_ray_drift = clamp(
        Float64(state.bearing_window_rate) -
            Float64(state.turn_rate_recent),
        -params.target_ray_drift_limit,
        params.target_ray_drift_limit,
    )
    target_ray_lead = params.target_ray_lead_gain *
        tanh(target_ray_drift / params.target_ray_drift_scale)
