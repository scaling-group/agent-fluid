# Response-gated posterior authority reallocation around the evidenced
# alignment-gated carrier. Oscillation phase remains in measured state.
...
function target_policy_params()
    return (
        ...
        maximum_posterior_reallocation=8.0 * pi / 180,
        command_limit=28.0,
    )
end
...
function target_policy(state, params)
    ...
    # acute bearing. During approach, measured wrong-way yaw selects the
    # posterior half-cycle whose authority should be redirected.
    ...
    reallocation_weight = approach_weight * lateral_weight * wrong_way_weight
    retained_wave_scale = 1 -
        (1 - params.posterior_brake_floor) * reallocation_weight

    # Preserve the brake's response selection, but do not discard its removed
    # traveling-wave authority. Reallocate that measured joint-state amplitude
    # to the corrective side, with a hard target bound and no added phase clock.
    tail_mean = params.tail_curvature_share * mean_curvature
    posterior_wave = -centered_q1 -
        params.tail_lag_gain * qd1 / max(omega, eps(omega))
    removed_wave = (1 - retained_wave_scale) *
        posterior_wave_authority * abs(posterior_wave)
    corrective_reallocation = -direction_sign * min(
        removed_wave,
        params.maximum_posterior_reallocation,
    )
    phase_lag_target = tail_mean +
        retained_wave_scale * posterior_wave_authority * posterior_wave +
        corrective_reallocation
