# Terminal relative-phase tracking around the phase-balanced C-turn scaffold.
...
function target_policy_params()
    return (
        ...
        posterior_phase_velocity_scale=0.10,
        maximum_posterior_phase_acceleration=1.5,
...
function target_policy(state, params)
    ...
    # The posterior spring follows a moving lag target, so damping only absolute
    # qd2 can erase propagation. For the harmonic anterior carrier, the measured-
    # state derivative of its lagged wave is -qd1 + lag*omega*centered_q1.
    # Compare qd2 with that bounded moving-wave velocity only in the existing
    # terminal maneuver; this corrects relative phase without moving a mean bend
    # or injecting phase-independent energy.
    posterior_wave_scale = propulsion_scale * posterior_wave_authority *
        redirect_wave_scale
    desired_posterior_velocity = posterior_wave_scale *
        (-qd1 + modulated_tail_lag * omega * centered_q1)
    normalized_posterior_phase_error =
        (desired_posterior_velocity - qd2) /
            max(omega * effective_amp, eps(Float64))
    posterior_phase_tracking =
        params.maximum_posterior_phase_acceleration *
            target_behind_weight^2 * terminal_response_weight * tanh(
                normalized_posterior_phase_error /
                    params.posterior_phase_velocity_scale,
            )
    raw_a2 = omega^2 * (phase_lag_target - q2) -
        2 * params.tail_damping * omega * qd2 + posterior_phase_tracking
