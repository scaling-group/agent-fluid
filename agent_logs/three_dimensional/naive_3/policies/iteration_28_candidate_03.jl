# Joint-state terminal stroke around the evidenced course-hold C-turn.
# Oscillation phase and maneuver transitions remain entirely in measured state.
...
function target_policy_params()
    return (
        ...
        stroke_bend_threshold=0.34,
        stroke_bend_scale=0.04,
        stroke_tail_mean_share=0.85,
        stroke_posterior_wave_scale=0.58,
...
function target_policy(state, params)
    ...
    # A persistent tangential miss can leave both joints parked in the same
    # C-bend. Once measured joint state confirms that the requested bend is
    # established, shift the posterior target smoothly from static curvature
    # into traveling-wave authority. As the tail unbends this state gate
    # releases, allowing the C-turn to rebuild without a clocked maneuver.
    signed_c_bend = -response_direction * 0.5 * (q1 + q2)
    bend_ready_weight = 0.5 * (1 + tanh(
        (signed_c_bend - params.stroke_bend_threshold) /
            params.stroke_bend_scale,
    ))
    terminal_stroke_weight = terminal_response_weight * bend_ready_weight
    ...
    # Preserve the evidenced joint-state phase modulation. The terminal stroke
    # does not reverse posterior mean curvature: it trades part of the
    # established C-bend for a bounded increase in posterior wave authority.
    ...
    effective_tail_redirect_share = params.redirect_tail_share +
        terminal_stroke_weight *
            (params.stroke_tail_mean_share - params.redirect_tail_share)
    tail_mean = (1 - redirect_weight) *
        params.tail_curvature_share * mean_curvature +
        redirect_weight * effective_tail_redirect_share * redirect_curvature
    posterior_wave = -centered_q1 -
        modulated_tail_lag * qd1 / max(omega, eps(omega))
    stroke_wave_target = max(
        params.stroke_posterior_wave_scale,
        redirect_wave_scale,
    )
    posterior_wave_scale = redirect_wave_scale + terminal_stroke_weight *
        (stroke_wave_target - redirect_wave_scale)
    phase_lag_target = tail_mean +
        propulsion_scale * posterior_wave_authority * posterior_wave_scale *
            posterior_wave
