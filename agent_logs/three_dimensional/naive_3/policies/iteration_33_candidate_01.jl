# Terminal wave-energy regulation around the course-hold C-turn scaffold.
...
function target_policy_params()
    return (
        ...
        terminal_activity_floor=0.50,
        terminal_activity_scale=0.12,
        maximum_terminal_energy_gain=3.0,
...
function target_policy(state, params)
    ...
    # The latest one-sided terminal pulse still decayed into the nearly static
    # C-bend seen in earlier descendants. Regulate phase-plane activity without
    # moving that equilibrium: when the target is behind and the existing
    # terminal selector is active, add bounded energy parallel to measured
    # anterior motion on both half-cycles. The term is exactly zero at rest and
    # releases continuously when the evidenced carrier activity is restored.
    ...
    low_activity_weight = 0.5 * (1 + tanh(
        (params.terminal_activity_floor - wave_activity) /
            params.terminal_activity_scale,
    ))
    terminal_energy_weight = target_behind_weight^2 *
        terminal_response_weight * low_activity_weight
    terminal_energy_acceleration =
        params.maximum_terminal_energy_gain * terminal_energy_weight * qd1
