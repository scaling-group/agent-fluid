# Terminal-retained posterior counter-bend around the evidenced
...
function target_policy_params()
    return (
        ...
        terminal_direction_scale=0.70,
        terminal_closing_reference=0.35,
        terminal_closing_scale=0.20,
...
function target_policy(state, params)
    ...
    # Preserve the sampled S-shaped component while rigid-body translation
    # opposes the normalized target side. Unlike the failed same-sign posterior
    # bend, this redirects through the less-saturated joint without adding
    # anterior curvature.
    ...
    # Retain that same bounded counter-bend when full target direction and
    # inadequate target-ray closure agree that a late pass remains unresolved.
    # Alignment or restored closing response releases it continuously. The
    # probabilistic union cannot exceed the already sampled counter-bend cap.
    target_direction = atan(target_y, -target_x)
    direction_weight = tanh(
        abs(target_direction) / params.terminal_direction_scale,
    )^2
    forward_velocity = Float64(state.velocity_body_U[1])
    projected_closing_speed =
        (target_x * forward_velocity + target_y * lateral_velocity) / distance
    closing_deficit = max(
        0.0,
        params.terminal_closing_reference - projected_closing_speed,
    )
    closing_weight = tanh(
        closing_deficit / params.terminal_closing_scale,
    )^2
    terminal_weight = direction_weight * closing_weight
    counterbend_weight = 1 - (1 - slip_weight) * (1 - terminal_weight)
    counterbend_guard = target_side * counterbend_weight
