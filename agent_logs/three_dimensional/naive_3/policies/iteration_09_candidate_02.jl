# Approach-course posterior counter-bend around the evidenced alignment-gated
...
function target_policy(state, params)
    ...
    # Retain the evidenced S-shaped component while rigid-body translation
    # opposes the normalized target side. This preserves the parent's useful
    # early correction without adding anterior effort.
    ...
    # Same-side translation can still cut across the target ray. During the
    # terminal approach, use the signed velocity component perpendicular to
    # that normalized ray as an additional posterior counter-bend request.
    # A continuous distance envelope leaves the evidenced far-field route
    # essentially unchanged; the shared clamp does not raise steering authority.
