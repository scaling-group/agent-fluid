# Response-selected posterior counterstroke after the evidenced C-turn.
# Oscillation and counterstroke phase remain entirely in measured joint state.
...
function target_policy_params()
    return (
        ...
        maximum_response_tail_stroke=18.0 * pi / 180,
...
function target_policy(state, params)
    ...
    # target direction selects the sampled C-turn only after the target passes.
    ...
    redirect_weight = target_behind_weight
    ...
    # Crossing back to the ahead side does not complete recovery while the
    # target ray remains lateral and translational course remains tangential.
    # Reuse that evidenced response boundary for a distinct posterior stroke,
    # rather than prolonging or weakening the same-curvature C-turn.
    ...
    response_completion_weight = (1 - target_behind_weight) *
        response_lateral_weight * response_course_weight *
        response_speed_weight

    # Preserve the sampled carrier on approach. Body-behind recovery moves its
    # equilibrium and envelope into the bounded C-turn without stopping the
    # traveling wave.
    centered_q1 = q1 - anterior_mean
    effective_amp = max(base_redirect_wave_scale * amp, eps(Float64))
    ...
    # Preserve the evidenced joint-state phase modulation. On the barely-ahead
    # recovery boundary, the target-ray/course sign and measured anterior
    # joint velocity select the targetward half-cycle of a bounded posterior
    # counterstroke. Mirroring changes the stroke sign while leaving its phase
    # selector invariant.
    ...
    targetward_phase_weight = 0.5 * (1 + velocity_phase)
    response_tail_stroke = params.maximum_response_tail_stroke *
        response_completion_weight * course_direction *
        targetward_phase_weight
    posterior_wave = -centered_q1 -
        modulated_tail_lag * qd1 / max(omega, eps(omega))
    phase_lag_target = tail_mean + response_tail_stroke +
        propulsion_scale * posterior_wave_authority * base_redirect_wave_scale *
            posterior_wave
