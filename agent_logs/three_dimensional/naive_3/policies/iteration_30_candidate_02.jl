# Phase-selected anterior stroke around the evidenced course-hold C-turn.
...
function target_policy_params()
    return (
        ...
        stroke_course_error_threshold=1.0,
        stroke_course_error_scale=0.20,
        stroke_turn_response_threshold=0.55,
        stroke_turn_response_scale=0.15,
        stroke_phase_velocity_scale=0.08,
        maximum_anterior_halfcycle_acceleration=6.0,
...
function target_policy(state, params)
    ...
    # The strongest sampled loop enters the terminal region but remains
    # tangential. Hold the same bounded reserve until measured course alignment
    # appears, including a brief crossing to the target-ahead side.
    ...
    # Large terminal course error with weak requested-sign yaw identifies a
    # transient turn-response deficit. The target-side gate isolates the late
    # fully-behind miss from the ahead-side first recovery.
    large_course_error_weight = 0.5 * (1 + tanh(
        (abs(course_error) - params.stroke_course_error_threshold) /
            params.stroke_course_error_scale,
    ))
    useful_turn_response = -response_direction * heading_rate
    turn_response_deficit_weight = 0.5 * (1 + tanh(
        (params.stroke_turn_response_threshold - useful_turn_response) /
            params.stroke_turn_response_scale,
    ))
    ...
    # Preserve both equilibria. Add energy only while measured anterior joint
    # velocity already moves toward the requested bend; the return half-cycle
    # and zero-velocity state receive no residual acceleration.
    centered_q1 = q1 - anterior_mean
    effective_amp = max(redirect_wave_scale * amp, eps(Float64))
    turn_direction = -response_direction
    normalized_turn_velocity = turn_direction * qd1 / max(
        params.stroke_phase_velocity_scale * omega * amp,
        eps(Float64),
    )
    toward_halfcycle_weight = max(tanh(normalized_turn_velocity), 0.0)
    stroke_weight = target_behind_weight^2 * terminal_response_weight *
        large_course_error_weight * turn_response_deficit_weight *
        toward_halfcycle_weight
    halfcycle_acceleration =
        params.maximum_anterior_halfcycle_acceleration * stroke_weight *
        turn_direction
    vdp_drive = params.oscillator_mu *
        (1 - (centered_q1 / effective_amp)^2) * qd1
    raw_a1 = vdp_drive - omega^2 * centered_q1 + halfcycle_acceleration
    ...
    # terminal-hold equilibrium; the new stroke does not change the lag target.
