# Terminal course-selective posterior thrust around the phase-balanced C-turn.
...
function target_policy_params()
    return (
        ...
        terminal_thrust_course_threshold=0.20,
        terminal_thrust_course_scale=0.10,
        terminal_thrust_wave_floor=0.50,
...
function target_policy(state, params)
    ...
    # Preserve the completed phase-balanced carrier and its moving C-turn
    # equilibrium. Measured activity about that equilibrium gates bounded,
    # velocity-odd energy on both half-cycles, so a quiet bend remains locally
    # unstable without adding static curvature or a preferred lateral sign.
    ...
    # At close range, poor measured course alignment selects a thrust/steering
    # role split. Attenuate only the oscillatory posterior wave while retaining
    # the anterior rhythm and both mean curvatures; alignment continuously
    # restores full posterior propulsion, and the floor prevents coasting.
    terminal_thrust_course_weight = 0.5 * (1 + tanh(
        (params.terminal_thrust_course_threshold - course_dot) /
            params.terminal_thrust_course_scale,
    ))
    terminal_thrust_weight = target_behind_weight^2 *
        terminal_distance_weight * response_speed_weight *
        terminal_thrust_course_weight
    terminal_thrust_scale = 1 -
        (1 - params.terminal_thrust_wave_floor) * terminal_thrust_weight
    phase_lag_target = tail_mean +
        propulsion_scale * posterior_wave_authority * redirect_wave_scale *
            terminal_thrust_scale * posterior_wave
