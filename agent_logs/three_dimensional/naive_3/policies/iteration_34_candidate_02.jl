# Realized-closure active half-cycle around the course-hold C-turn scaffold.
...
function target_policy_params()
    return (
        ...
        terminal_halfcycle_velocity_scale=0.06,
        terminal_closing_speed_scale=0.04,
        maximum_terminal_halfcycle_acceleration=1.5,
...
function target_policy(state, params)
    ...
    # Strengthen only the anterior half-cycle already moving into the
    # course-requested bend. Finite-window range closure, rather than nominal
    # translational course, preserves the pulse through a tangential approach
    # and releases it once measured head-to-target range actually opens.
    normalized_anterior_velocity = qd1 /
        max(omega * amp, eps(Float64))
    useful_halfcycle_velocity = max(
        -response_direction * normalized_anterior_velocity,
        0.0,
    )
    halfcycle_motion_weight = tanh(
        useful_halfcycle_velocity /
            params.terminal_halfcycle_velocity_scale,
    )
    window_closing_speed = Float64(state.window_closing_speed_L)
    closing_response_weight = 0.5 * (1 + tanh(
        window_closing_speed / params.terminal_closing_speed_scale,
    ))
    terminal_halfcycle_weight = target_behind_weight^2 *
        terminal_response_weight * closing_response_weight *
        halfcycle_motion_weight
    terminal_halfcycle_acceleration =
        -params.maximum_terminal_halfcycle_acceleration *
            terminal_halfcycle_weight * response_direction
