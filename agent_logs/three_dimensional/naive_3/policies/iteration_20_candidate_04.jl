# Closure-cone-selected posterior phase modulation around the evidenced
# target-response brake. Oscillation phase remains in measured joint state.
...
function target_policy_params()
    return (
        ...
        phase_lateral_threshold=0.35,
        phase_lateral_scale=0.12,
        phase_closure_threshold=0.75,
        phase_closure_scale=0.10,
...
function target_policy(state, params)
    ...
    # A persistent lateral target ray and degraded normalized closure identify
    # miss formation without a fixed route or approach radius. Their
    # conjunction rejects far-field beat-scale sway with a longitudinal target.
    lateral_geometry_weight = 0.5 * (1 + tanh(
        (abs(target_unit_y) - params.phase_lateral_threshold) /
            params.phase_lateral_scale,
    ))
    closure_loss_weight = 0.5 * (1 + tanh(
        (params.phase_closure_threshold - course_dot) /
            params.phase_closure_scale,
    ))
    ...
    phase_weight = lateral_geometry_weight * closure_loss_weight *
        phase_speed_weight * phase_error_weight
    ...
    # Modulate posterior lag with measured joint phase. Unlike the sampled
    # fixed-distance envelope, the closure cone can engage while a lateral miss
    # is forming and releases continuously with geometry, closure, or speed.
    ...
    # Keep the carrier's posterior mean and response-selected brake. The new
    # mechanism changes posterior wave shape rather than adding an equilibrium.
