# Course-conditioned approach hold around the evidenced alignment-gated
...
function target_policy(state, params)
    ...
    # Retain the sampled S-shaped response while body translation opposes the
    # normalized target side. A terminal target-ray residual also responds when
    # same-side motion is still cross-track. Their shared clamp preserves the
    # evidenced posterior equilibrium envelope.
    ...
    # Near a failed approach, separate posterior propulsion from steering.
    # Only the oscillatory wave is reduced when measured motion is substantially
    # cross-track and target-ray closure is inadequate; the mean S-bend remains
    # active, the anterior carrier keeps moving, and the far field is unchanged.
