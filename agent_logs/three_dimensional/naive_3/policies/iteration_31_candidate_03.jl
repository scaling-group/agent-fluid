# Joint-state terminal C-bend release around the course-hold C-turn scaffold.
...
function target_policy_params()
    return (
        ...
        terminal_bend_completion_threshold=0.70,
        terminal_bend_completion_scale=0.10,
        terminal_quiet_speed_scale=0.10,
...
function target_policy(state, params)
    ...
    # A terminal C-turn can become a stable parked bend instead of releasing
    # into a traveling stroke. Detect completion from requested-sign common
    # curvature and low normalized joint speed, then move only the anterior
    # equilibrium toward unbending. Joint motion continuously turns this gate
    # back off, so no clock, hidden maneuver stage, or persistent counterbend
    # is required.
    normalized_common_bend = -response_direction *
        (q1 + q2) / max(2 * amp, eps(Float64))
    bend_completion_weight = 0.5 * (1 + tanh(
        (normalized_common_bend -
            params.terminal_bend_completion_threshold) /
            params.terminal_bend_completion_scale,
    ))
    normalized_joint_speed = hypot(qd1, qd2) /
        max(omega * amp, eps(Float64))
    quiet_bend_weight = 1 / (1 +
        (normalized_joint_speed / params.terminal_quiet_speed_scale)^4)
    bend_release_weight = target_behind_weight^2 *
        terminal_response_weight * bend_completion_weight * quiet_bend_weight
    bend_release_curvature = params.maximum_response_curvature *
        bend_release_weight * response_direction
    anterior_mean = (1 - redirect_weight) * mean_curvature +
        redirect_weight * redirect_curvature + bend_release_curvature
    redirect_wave_scale = 1 -
        (1 - params.redirect_wave_floor) * redirect_weight

    # Preserve the sampled carrier on approach. During recovery, retain its
    # bounded C-turn envelope while the joint-state release prevents a quiet
    # terminal equilibrium from replacing the traveling wave.
