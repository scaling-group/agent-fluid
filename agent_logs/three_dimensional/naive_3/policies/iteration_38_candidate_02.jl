# Course-response recovery of a deficient posterior traveling-wave rhythm.
...
function target_policy_params()
    return (
        ...
        terminal_energy_activity_target=0.25,
        terminal_energy_activity_scale=0.05,
        terminal_energy_velocity_scale=0.20,
        maximum_terminal_energy_acceleration=2.0,
        posterior_energy_activity_target=0.25,
        posterior_energy_activity_scale=0.05,
        posterior_energy_velocity_scale=0.04,
        maximum_posterior_energy_acceleration=4.0,
...
function target_policy(state, params)
    ...
    # Preserve the completed moving C-turn equilibrium. When its phase-plane
    # activity becomes deficient, add bounded velocity-odd energy on both
    # half-cycles. This keeps the quiet anterior bend locally unstable without
    # moving the mean curvature or selecting a lateral sign.
    ...
    low_activity_weight = 0.5 * (1 + tanh(
        (params.terminal_energy_activity_target - wave_activity) /
            params.terminal_energy_activity_scale,
    ))
    terminal_energy_weight = target_behind_weight^2 *
        terminal_response_weight * low_activity_weight
    phase_balanced_energy =
        params.maximum_terminal_energy_acceleration *
            terminal_energy_weight * tanh(
                normalized_wave_velocity /
                    params.terminal_energy_velocity_scale,
            )
    raw_a1 = vdp_drive - omega^2 * centered_q1 + phase_balanced_energy
    ...
    # Preserve the evidenced approach phase modulation and blend both the
    # sampled redirect and terminal course hold into the posterior equilibrium.
    # The sampled terminal course-response lag residual is deliberately absent:
    # its completed combination parked both joints and broadened the return.
    ...
    # The lagged posterior tracker is strongly damped and can become the quiet
    # end of a common C-bend even while the vehicle keeps moving. Give it a
    # separate phase-balanced role: when measured posterior activity about its
    # moving lag target is low during a target-behind nonclosing response, add
    # bounded acceleration parallel to measured velocity on both half-cycles.
    # This changes rhythm stability, not mean curvature or prescribed phase;
    # the lag target still organizes propagation. Unlike a terminal-distance
    # shell, the response gate can destabilize an evidenced broad parked return.
    posterior_effective_amp = max(redirect_wave_scale * amp, eps(Float64))
    posterior_wave_position = (q2 - phase_lag_target) / posterior_effective_amp
    posterior_wave_velocity = qd2 /
        max(omega * posterior_effective_amp, eps(Float64))
    posterior_wave_activity = hypot(
        posterior_wave_position,
        posterior_wave_velocity,
    )
    posterior_low_activity_weight = 0.5 * (1 + tanh(
        (params.posterior_energy_activity_target - posterior_wave_activity) /
            params.posterior_energy_activity_scale,
    ))
    posterior_energy_weight = base_response_weight *
        posterior_low_activity_weight
    posterior_phase_balanced_energy =
        params.maximum_posterior_energy_acceleration *
            posterior_energy_weight * tanh(
                posterior_wave_velocity /
                    params.posterior_energy_velocity_scale,
            )
