# Phase-balanced anterior carrier with terminal posterior-wave reversal.
...
function target_policy_params()
    return (
        ...
        reverse_distance_scale=1.50,
        reverse_distance_power=12.0,
        reverse_course_dot_threshold=0.0,
        reverse_course_dot_scale=0.08,
        maximum_lag_reversal=2.0,
...
function target_policy(state, params)
    ...
    # When close target-behind geometry is nonclosing, reverse only the
    # quadrature lag. This changes traveling-wave direction without changing
    # the anterior carrier, either moving mean, or scalar wave authority.
    ...
    reverse_distance_weight = 1 / (1 +
        (distance / params.reverse_distance_scale)^params.reverse_distance_power)
    reverse_nonclosing_weight = 0.5 * (1 + tanh(
        (params.reverse_course_dot_threshold - course_dot) /
            params.reverse_course_dot_scale,
    ))
    reverse_wave_weight = target_behind_weight^2 * reverse_distance_weight *
        response_speed_ratio * reverse_nonclosing_weight
    lag_direction = 1 -
        params.maximum_lag_reversal * reverse_wave_weight
    effective_tail_lag = lag_direction * modulated_tail_lag
