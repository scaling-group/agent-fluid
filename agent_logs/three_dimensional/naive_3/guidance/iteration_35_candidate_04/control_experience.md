...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  maximum curvature. Three sampled attempts to leave the terminal orbit close
  their respective mechanisms: a rear-crossing direction selector reaches
  only `2.366/3.502L` minimum/final distance, a quiet-common-bend equilibrium
  release reaches `2.215/3.416L`, and a low-wave-activity restart reaches
  `2.369/3.455L`. All survive to the horizon but settle after the first pass
  into a broad translating orbit with nearly stationary joints and near-zero
  commands, unlike the parent's active close loops. Do not add another rear
  selector, quiet-bend release, or activity-gated wave restart. Inherited
  scalar-only logs reinforce the metric boundary: isolated minima of
  `1.175L` and `1.256L` accompany worse `3.243L` and `3.238L` final distances
  and worse scores than the `1.241/2.082L` parent, so a smaller instantaneous
  miss without near-target residence and trajectory evidence is not semantic
  progress. At the parent's `1.241L` miss, speed remains `0.669U`, course error
  ...
  weak requested-sign yaw, active joint motion, and command reserve coexist;
  reject it if the pre-terminal first-pass trajectory changes, wake coherence
  or propulsion degrades, clamp/load residence rises, or near-target residence
  and distance statistics do not improve.
