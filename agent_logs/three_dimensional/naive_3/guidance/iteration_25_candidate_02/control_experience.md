...
- Preserve the full-direction C-turn as the first evidenced semantic recovery
  scaffold, but do not strengthen or merely prolong its anterior equilibrium.
  It changed the powered lower exit into coherent `100T` survival at
  `2.346/4.714/3.902L` minimum/mean/final distance. A response hold improved
  mean distance to `4.458L` while reaching `2.377L`, whereas the assigned
  parent's target-behind/nonclosing wave contraction reached
  `2.484/4.545L` minimum/mean and radial-response release reached
  `2.532/4.780L`; all remained noncapturing broad orbits in both visual rows.
  On the strongest response-held trace, speed stayed `0.65--0.69U` on close
  passes while target-ray/course mismatch remained `1.02--1.39 rad` and
  same-sign yaw `1.35--2.22 rad/T`; anterior/posterior reserve residence was
  about `0.727/0.101`. Thus this horizon topology has established excessive
  tangential rotation with posterior reserve, not absent propulsion or too
  little persistent turn. Test response-selected posterior counterbend or
  propulsive release before another distance gate, wave contraction, or
  stronger C-bend, and require capture, an inward return leg, lower mean and
  minimum distance, or reduced saturation. This implication applies to a
  coherent finite-speed horizon orbit with same-sign yaw; it is falsified if a
  release policy loses the horizon class/wake or if a later scalar hold or
  contraction produces capture with acceptable loads.
