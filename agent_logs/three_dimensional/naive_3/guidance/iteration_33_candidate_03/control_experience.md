...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  parked equilibrium it was intended to escape. The assigned parent's distinct
  velocity-aligned half-cycle energy test also fails this boundary: it retains
  a coherent horizon wake and improves mean distance from `4.157L` to
  `3.921L`, but worsens minimum/final distance from `1.241/2.082L` to
  `1.702/3.574L`, never enters `1.5L`, and collapses mean absolute
  anterior/posterior velocity inside `2L` from about `1.057/0.409` to
  `0.028/0.028 rad/T`. Thus a frozen-state pulse that is localized in distance
  and target side does not establish rhythmic preservation; its requested-side
  half-cycle gating can still move the coupled system onto the same quiet
  C-bend attractor. Preserve the continuous course hold, but avoid more static
  bend shifts, selector replacement, posterior residuals, constant signed
  restarts, or retuning that asymmetric pulse. If terminal energy regulation is
  tested again, require an odd, phase-balanced feedback that is linear near
  zero velocity and gated by phase-plane activity about the moving equilibrium,
  not by a preferred half-cycle. This remaining branch is applicable only
  while the course-hold scaffold retains its coherent return and command/load
  reserve, and is falsified by a changed first return, another parked bend,
  degraded wake coherence, higher clamp/load residence, or no improvement in
  near-target and final-distance statistics.
