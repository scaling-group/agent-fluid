...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  merely because course briefly closes. A response-held C-turn keeps the
  coherent wake, improves mean distance to `4.458L`, and brings the second
  return to about `2.60L` rather than the baseline's `3.15L`, but still misses
  and orbits. The completed bounded-turn-authority test resolves the proposed
  next branch: adding a same-sign course-response reserve only after the target
  is fully behind contracts that orbit to `1.314/4.056/3.077L`
  minimum/mean/final distance and lowers anterior clamp residence from about
  `0.727` to `0.249` without a larger sampled load tail. Preserve its carrier,
  redirect, and response reserve. Broad and narrow attempts to persist the
  same equilibrium turn across terminal target-side geometry reach
  `1.241/4.158/2.082L` and `1.282/4.119/2.862L`, respectively; both remain
  coherent, powered horizon loops rather than captures. Close further static
  curvature persistence, distance/gate retuning, contraction/release,
  phase/duty reallocation, moment residuals, and response logic justified only
  by lower saturation.
- Frozen-trace locality is not sufficient near this scaffold's target-crossing
  bifurcation. A sampled anterior response-deficit burst changes the protected
  first recovery from about `2.44L` to `2.62L` and regresses closest distance
  to `2.125L`; inherited evaluation of a line-of-sight-rate/yaw-rate residual
  likewise reaches only `1.578L` minimum and `4.478L` final distance. Both are
  finite horizon trajectories with coherent wakes, so their loss is a coupled
  control-topology failure rather than instability or failed propulsion. Do
  not add another dynamic residual active on both sides of the target crossing
  or infer rollout locality from a small frozen action delta. If response-rate
  matching is revisited, gate it with full body-frame ahead/behind geometry so
  it is negligible on the first approach, preserve the traveling carrier, and
  reject it if first-pass geometry changes, clamp/load tails rise, or the
  `1.2--1.7L` tangential orbit survives. This boundary applies only to the
  coherent horizon C-turn scaffold, not a stalled or incoherent trajectory.
