...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  broadening. The assigned parent's completed combination now supplies that
  falsification: adding response-gated posterior lag to the `1.175L`
  phase-balanced scaffold reaches only `2.439/3.861/3.310L`
  minimum/mean/final distance, never enters `2L`, and arrives at its sampled
  minimum with joint velocities near `-0.00053/+0.00179 rad/T` in the same
  common negative C-bend seen in the static terminal failures. Its top-down
  and oblique sheets retain a coherent powered wake through a broad loop, so
  wake collapse or insufficient gross thrust does not explain the regression.
  Close posterior lag/phase-response combinations on this scaffold rather
  than retuning their gain or gate. If the exact anterior energy law is
  preserved, test a genuinely distinct posterior role that restores measured
  two-sided tail activity without shifting lag, equilibrium, or wave
  authority; falsify that role if it adds tangential thrust without improving
  capture, closest approach, near-target residence, and the terminal return.
