...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  only about `0.13 rad/T` while both joint commands are small and the anterior
  wave remains active (`phi_dot_1` about `-0.260 rad/T`). The proposed
  response-burst branch is now closed: an anterior burst reaches only
  `2.125/3.901/3.601L` minimum/mean/final distance, a target-behind turn-rate
  ...
  late minima all three descendants have nearly stationary joints in a common
  C-bend (`abs(phi_dot_1) <= 0.006 rad/T`), unlike the parent's closer active
  pass. In this finite-speed, coherent terminal-orbit regime, avoid more static
  bend, posterior phase/stroke authority, or direction-selector replacement
  that can stabilize a noncapturing quiescent C-bend. The assigned parent's
  normalized joint-state unbend now closes the proposed static release branch:
  although it improves mean distance to `3.859L` and earns the best sampled
  scalar score, it reaches only `2.215L`, never enters `1.5L`, ends at
  `3.416L`, and replaces the scaffold's tight late return with the same broad
  powered loop. At its minimum both joints are again nearly parked
  (`phi_dot_1/phi_dot_2` about `0.0027/0.0020 rad/T`). Together with the
  `2.369L` low-activity wave restart, this shows that frozen-trace locality and
  a joint-state release condition do not establish coupled maneuver locality;
  avoid more bend-completion thresholds, equilibrium unbends, or oscillator
  restart nudges on this scaffold. A remaining rhythmic test is distinct only
  if it acts on an already moving, course-requested half-cycle, is identically
  zero at a parked bend, and preserves the posterior traveling-wave law. It is
  falsified if the `1.241L` active-wave return, its `0.47T` residence inside
  `1.25L`, wake coherence, or clamp/load margins regress, even if mean distance
  or scalar score improves.
