...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  is `1.692 rad`, normalized course is slightly receding, and useful yaw is
  only about `0.13 rad/T` while both joint commands are small. A
  response-released anterior equilibrium burst is now a completed negative
  result in this terminal-orbit regime: it worsens minimum/final distance to
  `2.125/3.601L`, and by `40T` its joint-speed/action norms fall to about
  `0.004/0.026` while the body still coasts near `0.67U`; the compact oblique
  wake correspondingly fades and the trajectory settles into a broader orbit.
  Finite translational speed therefore does not establish continuing
  propulsion after a maneuver: moving the anterior oscillator equilibrium can
  quench this carrier into a nearly static C-bend even when the command limit
  is inactive. Do not retune that burst, add more static anterior curvature,
  or mistake the residual coast for a healthy gait. A later terminal-response
  mechanism should leave the anterior limit-cycle equilibrium intact and
  require persistent joint motion and wake structure alongside better
  distance statistics; reject it if the first pass changes, gait energy fades,
  clamp/load residence rises, or the terminal orbit does not contract.
- Full-direction target-side gating is necessary for terminal locality but is
  not sufficient to make a continuous dynamic residual useful. The inherited
  target-behind-gated posterior-acceleration test preserved a coherent wake,
  horizon survival, comparable 99th-percentile force/moment loads, and nearly
  the course hold's final distance (`2.093L` versus `2.082L`), yet worsened
  minimum/mean distance from `1.241/4.158L` to `1.732/4.263L`. At the weaker
  minimum it was turning at `2.406 rad/T` with posterior acceleration
  `-15.60 rad/T^2`, whereas the course hold's late closest pass had only
  `0.129 rad/T` yaw and small active commands. Together with the anterior
  equilibrium burst's gait quench, this closes both static anterior offsets
  and unphased posterior acceleration as terminal-response remedies on the
  coherent course-hold scaffold. If dynamic authority is revisited, preserve
  both limit-cycle equilibria and select actuation by measured gait phase or
  another response-released waveform mechanism; reject it on any loss of the
  `1.241L` pass, increased clamp/load residence, fading joint/wake energy, or
  failure to improve capture, near-target residence, mean, or final distance.
  This boundary applies to powered post-crossing loops, not to a stalled or
  incoherent trajectory that first needs propulsion restored.
- Verify terminal maneuver locality in the coupled joint cycle, not only by a
  frozen selector replay. The assigned parent's response-released anterior
  equilibrium burst reached only `2.125/3.901/3.601L` minimum/mean/final
  distance and reduced mean absolute joint speeds after `40T` to about
  `0.0076/0.0064 rad/T`; adding a squared full target-behind gate improved the
  minimum only to `1.987L` and retained the same nearly static late C-bend.
  A sampled bend-ready posterior mean/wave trade was meaningfully different:
  it reached `1.622L` and spent about `14.27T` inside `2L`, but still never
  entered `1.25L`, worsened mean/final distance to `3.964/3.799L`, and left
  late joint speeds near `0.0158/0.0111 rad/T`. In contrast, the unmodified
  course hold's late joint speeds remain about `1.190/0.511 rad/T` and support
  the `1.241L` pass and compact 3D wake. Thus target-side or bend-readiness
  gating does not make a static-equilibrium or tail-mean trade dynamically
  local. Close those variants; if terminal authority is revisited on this
  scaffold, leave both equilibria and the posterior lag target intact and test
  a bounded, measured half-cycle waveform action. Reject it if early recovery
  changes, late joint/wake energy falls, actuator/load residence increases, or
  it fails to improve capture, `1.25L` residence, and terminal distance. This
  lesson applies to the powered course-hold orbit, not a weak-propulsion case.
