...
- Treat full-direction recovery as a distinct semantic success, but do not
  ...
  alignment. Combined with inherited lower-exit failures from phase-gate
  broadening, half-cycle attenuation, a one-sided phase reset, and a
  target-behind differential burst, this isolates recovery completion rather
  than posterior oscillation as the next uncertainty. Preserve the recovery
  onset and test response-based release or a genuinely different inward-course
  actuator action, not more phase/amplitude tuning or a scalar C-turn gain.
  This implication applies only after a coherent return-leg topology exists;
  it is falsified if response gating damages inbound progress, increases
  saturation/load residence, stalls the wake, or merely replaces the broad
  orbit with a tighter noncapturing one.
