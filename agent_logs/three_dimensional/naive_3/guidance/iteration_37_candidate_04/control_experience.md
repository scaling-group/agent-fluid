...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  posterior residuals, or constant signed restart. The inherited completed
  rollouts now also close the simplest one-joint rhythmic escape: a
  requested-sign anterior half-cycle pulse reaches only
  `1.702/3.921/3.574L` minimum/mean/final distance and parks both joints, while
  adding a requested-half-cycle duty bias to an active carrier regresses to a
  broad `2.362L` orbit. In contrast, phase-balanced low-activity feedback on
  both anterior half-cycles is the first rhythmic semantic improvement: it
  reaches `1.175L`, remains inside `1.25L` for about `2.35T`, preserves
  near-target mean absolute joint velocity around `0.775/0.360 rad/T`, and
  lowers clamp/load residence relative to the `1.241L` course hold. A stronger
  radial activity regulator (`1.366/3.047L` minimum/final) and a posterior
  phase-response branch (`1.276/2.881L`) still do not capture, so preserve the
  phase-balanced moving-equilibrium scaffold but close scalar energy,
  one-joint duty-asymmetry, and simple posterior-lag retuning. The completed
  paired-propagation test now closes the apparent counterphase opportunity at
  the `1.175L` minimum: equal-and-opposite acceleration conditioned on both
  measured joint velocities reaches only `1.192L`, shortens residence inside
  `1.25L` from about `2.35T` to `1.24T`, and slightly worsens mean distance
  from about `4.041L` to `4.043L`; its `0.021L` final-distance gain does not
  offset those regressions. Avoid retuning that paired burst or inferring
  hydrodynamic locality from a favorable frozen-state phase transition. The
  two active traces instead expose a response deficit at closest approach:
  the phase-balanced fish is already receding at about `0.076L/T` while its
  target ray rotates at `-0.578 rad/T` and measured yaw is only
  `-0.061 rad/T`; the paired burst increases yaw to `-0.335 rad/T`, but the ray
  still rotates at `-0.568 rad/T` and the fish recedes at `0.071L/T`. This does
  not reopen static yaw feedback, which prior evidence rejects. It supports a
  distinct terminal test only when the phase-balanced carrier and course hold
  are active: compare bounded target-ray-plus-inward alignment rate with
  measured yaw, feed the signed deficit into the C-turn equilibrium, and
  release it when yaw response appears. Reject that mechanism if it parks a
  joint, increases joint-limit/load residence, overshoots into another
  noncapturing orbit, broadens the first approach, or fails to improve closest
  approach, near-target residence, mean, and final distance together.
