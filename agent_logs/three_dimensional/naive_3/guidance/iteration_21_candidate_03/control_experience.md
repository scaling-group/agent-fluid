...
- Preserve posterior phase modulation only as a provisional approach baseline,
  ...
  distance to `2.499/9.291L`. Avoid more command relief, equilibrium bursts,
  or post-overshoot damping. If phase activation is revisited, use normalized
  lateral target geometry together with closing response to distinguish a
  forming miss from far-field gait sway, rather than retuning only its radius
  or amplitude. Require capture, a target-return leg, a useful termination
  class, or material improvement below `2.326L` with retained mean distance.
  This implication applies only to coherent lateral near misses with stable
  course mismatch; it is falsified if phase control degrades cruise progress,
  breaks wake coherence, curls tightly, increases clamp/load residence, or
  again leaves only a small scalar shift in the same lower-exit class.
