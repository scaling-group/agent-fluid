...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  posterior residuals, or constant signed restart. The sampled inherited
  requested-sign half-cycle pulse closes the simplest asymmetric rhythmic
  escape too: it reaches only `1.702/3.921/3.574L` minimum/mean/final distance,
  never enters `1.5L`, and reduces mean absolute anterior/posterior velocity
  inside `2L` to about `0.028/0.028 rad/T`. Because that pulse is weak near
  zero velocity and acts on only one half-cycle, retuning its amplitude,
  velocity scale, or radius is unlikely to destabilize the same parked bend.
  Low-activity velocity feedback on both measured half-cycles is instead the
  first rhythmic semantic improvement: it reaches `1.175L`, remains inside
  `1.25L` for about `2.35T` versus `0.47T`, preserves near-target mean absolute
  joint velocity around `0.775/0.360 rad/T`, and lowers clamp and RMS-load
  residence relative to the `1.241L` course hold. Its `4.041/3.243L`
  mean/final distance still exposes an oversized return. Preserve
  phase-balanced energy about the moving course-hold equilibrium as the new
  scaffold. Three inherited descendants now sharpen its applicability. A
  velocity-selected anterior duty multiplier regresses to
  `2.362/3.886/3.511L` minimum/mean/final distance, never enters `1.5L`, and
  parks in the broad-loop C-bend despite preserving positive-work sign in
  frozen replay; reflection equivariance and local positive work therefore do
  not establish coupled useful-half-cycle steering. Replacing the saturated
  low-activity term with linear radial gain reaches only
  `1.366/4.041/3.047L`, while a terminal yaw-response posterior lag shift
  reaches `1.276/4.085/2.881L`; neither enters `1.25L`, and both lose the
  `1.175L` scaffold's closest approach and residence. Close further anterior
  duty, radial-energy, and posterior-phase gain retuning. A later steering
  asymmetry is applicable only if it leaves the evidenced two-sided anterior
  energy carrier and both moving equilibria intact, isolates response in a
  bounded posterior wave component, and preserves the first return. Falsify
  this transfer if phase activity collapses, the wake or orbit broadens,
  clamp/load residence rises, or closest approach, near-target residence,
  mean, and final distance do not improve together.
