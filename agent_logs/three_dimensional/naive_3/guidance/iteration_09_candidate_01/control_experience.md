...
- Distinguish extra same-sign C-curvature from posterior counter-bend when
  ...
  `35.4%` to `38.1%`. Preserve its sign and existing authority envelope, then
  test a separately bounded terminal target-ray course residual rather than
  merely raising its gain. This implication applies to stable, self-propelled
  rollouts with the same course-crossing signature; lost far-field progress,
  a tight terminal curl, sharply greater loads/limit residence, or failure to
  improve the `2.187L` minimum falsifies the added residual. Unrestricted
  course-angle-to-anterior-curvature feedback remains contraindicated because
  it reversed release and exited above after reaching only `12.150L`.
