...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  low-activity kick, or half-cycle gain. That proposed symmetric activity
  regulator is now completed negative evidence: velocity-parallel anterior
  energy on both half-cycles raises anterior speed to `0.927 rad/T` at its
  minimum but reaches only `1.615/4.089/2.811L` minimum/mean/final distance,
  never enters `1.25L`, and produces wrong-sign yaw of about `+0.374 rad/T`;
  two inherited variants likewise stop at `1.631L` and `1.491L`. Low activity
  diagnoses weak descendants but is not itself a steering error, so close
  anterior energy floors as well as one-sided restarts. Directly accelerating
  the anterior joint from target-ray-rate minus measured-yaw residual is also
  closed: the assigned-parent descendant regresses to
  `2.201/3.887/3.584L` and returns to a quiet broad loop. In a near-tangential
  orbit, matching target-ray angular rate can preserve rather than contract
  course error. A later test may leave anterior equilibrium and energy
  untouched and place bounded response authority in a posterior
  traveling-wave parameter, but reject it unless course error contracts while
  the ahead-side return, clamp/load margins, and coherent wake remain intact.
