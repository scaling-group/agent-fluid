...
- Once restrained target-relative curvature establishes a coherent approach,
  ...
  the head. Since the carrier already clamps anterior/posterior acceleration
  for about `0.746/0.354` of its samples, avoid scalar relief, another
  same-sign redirect, geometry-only persistence, or further propulsion hold.
  That approach-localized, head-closure/bearing-response-gated opposite-sign
  posterior S-bend has now completed and is a negative result: it reached only
  `2.536L`, with `8.436L` mean distance and the same powered lower exit. In the
  same direct-uniform batch, full-direction gating (`2.494L`) and a posterior
  half-cycle counterbend (`2.512L`) also preserved the topology. A measured-
  yaw-selective posterior-wave brake was locally better at `2.385L` with
  `8.436L` mean distance, but still exited low, so its positive evidence is for
  response selectivity rather than posterior attenuation itself. Retire more
  posterior equilibrium/gate refinements and test actuator allocation instead:
  apply response-localized yaw braking to the anterior steering oscillator
  while preserving the lagged posterior wave. This implication applies only
  to coherent powered lateral near misses; it is falsified by degraded
  far-field progress, wake loss or tight curling, premature speed loss,
  increased actuator/load residence, or failure to improve the `2.385L` local
  and `8.436L` mean-distance batch benchmarks, and it does not apply to weak-
  wake failures.
