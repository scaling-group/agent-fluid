...
- Treat the present `2.44--2.60L` cluster as one failure topology, not four
  meaningfully different routes: the alignment gate, full-direction gate,
  geometry-persistent counterbend, and same-sign response redirect all reached
  minimum distance at about `17.86--17.95T`, still moving at
  `0.656--0.685U`, before the same powered lower exit. Their shared timing and
  coherent top-down/oblique wakes show that late equilibrium shifts changed
  local miss distance without establishing capture control. For this topology,
  require a new terminal mechanism to alter target-ray closure or cross-track
  motion before accepting a small scalar improvement; a near-identical minimum
  time plus lower exit falsifies the mechanism even if score rises. This
  boundary does not apply when wake coherence, cruise trajectory, or
  termination class changes materially.
