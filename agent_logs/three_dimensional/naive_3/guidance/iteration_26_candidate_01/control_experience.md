...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  to course mismatch. For this coherent still-water topology, test a bounded
  response-conditioned contraction or release of the recovery carrier while
  retaining redirect curvature; avoid another moment residual, phase/duty
  reallocation, or scalar curvature boost. Falsify that implication if the
  first pass changes, the wake or turn stalls, limit/load residence rises, or
  the orbit and horizon distance statistics remain materially unchanged.
- The next four completed horizon samples separate carrier, release, and turn
  response inside that recovery topology. Nonclosing wave contraction is a
  concrete negative result: it worsens minimum/mean/final distance to
  `2.484/4.544/4.673L` without reducing full-trace mean speed (`0.655U`) and
  retains about `0.712/0.103` anterior/posterior clamp residence. Extending
  the C-turn across barely-ahead lateral geometry (`2.377L`) and adding a
  later-yaw posterior counterbend (`2.249L`) preserve coherent horizon wakes
  but remain broad noncapturing loops. In contrast, adding same-sign curvature
  reserve only for a nearby, target-behind, tangent-or-receding course improves
  minimum/mean/final distance to `1.314/4.056/3.077L` and lowers clamp
  residence to about `0.249/0.103`; preserve that course-response scaffold
  rather than returning to drive contraction, response-hold retuning, or a
  posterior counterbend. Its closest pass still occurs at finite `0.652U`
  with `1.672 rad` target-ray/course error and only `0.137 rad/T` useful yaw,
  so a bounded response-deficit redirect that uses available anterior reserve
  is a justified next mechanism. This implication applies only after the
  target-behind course reserve has produced the closer horizon trajectory;
  falsify it if cruise or the first pass changes, useful yaw fails to develop,
  clamp/load residence grows, or the same near-tangential orbit persists.
