...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  mean heading feedback. Signed target-ray to translational-course error
  persists on every sampled state inside `3L` (median about `1.53 rad`) while
  speed remains finite, but the completed rhythmic tests now bound what that
  slower geometry can accomplish through small continuous modulation. Against
  the yaw-selected brake's `2.385/8.436L` minimum/mean distance, course-selected
  anterior duty asymmetry reaches `2.433/8.434L`, response-gated posterior
  S-bending reaches `2.536/8.436L`, and posterior phase-lag modulation gives a
  narrow improvement to `2.326/8.424L`; all retain the coherent wake, downward
  pass, and powered lower exit. Treat phase lag as a useful trim result, not a
  recovery mechanism, and close another mild duty/lag/wave-shape iteration
  unless it introduces a distinct maneuver-and-release mode. The next bounded
  rhythmic test should use the slow course error to enter a genuinely nonsteady
  redirect and use observed course response to release it, while preserving the
  evidenced brake and cruise carrier. This implication applies only to coherent
  lateral near misses with a stable course mismatch; it is falsified if the
  selector changes sign at beat scale, cruise wake/progress degrades, actuator
  residence rises, or a distinct maneuver still leaves the same minimum and
  lower-exit class.
