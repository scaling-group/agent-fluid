...
- Distinguish a useful terminal selector from an effective terminal action.
  ...
  lower exit. The completed course-selected differential continuation now
  closes the remaining equilibrium-allocation hypothesis: adding the course
  request anteriorly and counterbending the posterior joint reached only
  `2.469L`, retained about `0.690U` speed and `1.12 rad` target-ray/course
  mismatch at its minimum, and again powered into the lower boundary. Its
  anterior/posterior clamp fractions (`0.748/0.356`) were effectively the same
  as the brake's (`0.747/0.356`), so neither extra saturation nor inadequate
  gate activation explains the unchanged class. Do not retune the shared or
  differential course bend's amplitude, distance envelope, or joint split:
  persistent geometric error is a useful selector, but both common-mode and
  differential equilibrium curvature have failed as terminal actions on this
  carrier. A later test should change the action's semantic regime or
  non-equilibrium mechanism and require capture, recovery toward the target,
  or a useful new termination class—not another small minimum-distance shift.
  This negative lesson applies to coherent, powered lateral near misses on the
  same cruise scaffold; it is falsified by a later course-equilibrium capture
  or useful termination change, and does not justify global sideslip damping
  (which previously caused an upper exit at `12.150L`).
- Treat target-behind geometry as a useful semantic selector, but do not equate
  gait quenching with either translational braking or recovery. The assigned
  parent's target-behind damping hold improved the first-pass minimum from the
  response-selected brake's `2.385L` to `2.293L`, yet worsened mean distance
  from `8.436L` to `8.487L` and retained the same final distance and powered
  lower exit. The hold rose only after the target crossed behind the head near
  `18T`; over `20--22T` it reduced anterior joint-rate RMS from `2.978` to
  `0.393 rad/T` and mean action norm from `28.234` to `4.673 rad/T^2`, while
  mean speed remained higher (`0.684U` versus `0.640U`) and mean yaw response
  fell (`0.091` versus `0.207 rad/T`). This separates a successful actuator
  state change from a useful vehicle state change: hydrodynamic momentum
  carried the nearly straight lower-boundary departure after the beat stopped.
  Avoid retuning the hold damping, behind threshold, or distance envelope as a
  stand-alone terminal action. A later test may preserve the behind selector
  but must add active reorientation or another genuinely different recovery
  action, and should require capture, a target-return trajectory, or a useful
  new termination class rather than another first-pass minimum shift. This
  lesson applies to coherent, fast lateral overshoots; it is falsified by a
  later hold-only recovery and does not imply that damping is ineffective for
  already slow or unstable approaches.
