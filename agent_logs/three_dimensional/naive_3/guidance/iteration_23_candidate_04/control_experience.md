...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  but equilibrium and posterior-reallocation variants all retain the lower
  exit: geometry-persistent posterior shift reaches `2.484L`, response-gated
  equilibrium S-bend `2.536L`, posterior polarity reversal `2.406L`,
  gait-yaw residualization `2.541L`, closure-gated anterior redirect `2.569L`,
  and course-selected differential S-bend `2.469L`. The assigned parent's
  course-selected anterior duty action likewise reaches only `2.433/8.434L`.
  Do not retune those scalar brake thresholds, static C/S-bend allocations,
  posterior counterstrokes, anterior duty envelopes, or beat-locked yaw as
  mean heading feedback.
- Preserve posterior phase modulation only as a provisional approach baseline,
  not as established recovery. Across the four current direct-uniform samples,
  signed target-ray-to-course error remains about `1.53 rad` inside `3L` while
  speed stays finite. Moving that slow selector from anterior duty to bounded
  joint-state-selected posterior lag improves minimum/scored mean distance from
  the yaw brake's `2.385/8.436L` to `2.326/8.424L`, and the top-down and oblique
  sheets retain the long coherent three-dimensional wake. Yet at the best
  minimum the fish still travels at `0.687U` with `1.088 rad` course error; its
  anterior/posterior clamps remain occupied for about `0.749/0.355` of samples,
  and it powers to the same lower exit at `9.213L` final distance. Inherited
  post-overshoot repairs do not close that gap: extra anterior damping reached
  a smaller `2.293L` first-pass minimum but worsened mean distance and exited
  earlier, while a target-behind differential burst degraded minimum/final
  distance to `2.499/9.291L`. Nor does better selector semantics supply the
  missing authority. The assigned parent's normalized lateral-geometry plus
  closure forming-miss gate regressed to `2.684L`, and its next slow-course/
  fast-yaw-moment posterior brake reached only `2.635L` despite moment's
  `0.956--0.959` approach correlation with next-step yaw acceleration. Other
  inherited posterior variants reached `2.619L` for a harmful-half-cycle
  notch, `2.738L` for amplitude attenuation, and `2.457L` for a one-sided
  lag reset; every case retained the coherent powered lower exit near
  `9.19--9.20L` final distance. Thus activation locality and predictive signal
  correlation are not evidence of useful steering. Avoid more command relief,
  equilibrium bursts, post-overshoot damping, proximity/closure gate edits,
  moment-triggered posterior braking, or posterior half-cycle timing/amplitude
  cuts on this scaffold. A later test should change the actuator-shape or
  interception mechanism rather than another scalar envelope, and must require
  capture, a target-return leg, a useful termination class, or material
  improvement below `2.326L` with retained mean distance. This implication
  applies to coherent, finite-speed lateral near misses with stable course
  mismatch; it does not apply when propulsion or wake coherence is itself
  absent, and it is falsified by evidence that a genuinely different mechanism
  changes the trajectory class without increasing clamp/load residence.
