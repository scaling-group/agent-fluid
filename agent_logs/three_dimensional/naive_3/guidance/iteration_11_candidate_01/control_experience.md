...
- Once restrained target-relative curvature establishes a coherent approach,
  ...
  The remaining clean test is an approach-localized opposite-sign posterior
  S-bend activated by measured head-distance closure and released only when a
  normalized bearing trend shows geometric correction. This implication
  applies to coherent powered lateral near misses; it is falsified by degraded
  far-field progress, a short-wake curl, increased actuator/load residence, or
  the same lower exit without improving the `2.011L` local benchmark or the
  associated distance integral, and it does not apply to weak-wake failures.
