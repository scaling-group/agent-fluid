...
- Treat continuous terminal posterior-wave attenuation as disproven for the
  current coherent powered-near-miss family, not as an unevaluated tuning
  direction. Relative to the `2.443L` alignment-gated carrier (`8.443L` mean
  distance), the sampled cross-track/poor-closure hold reached only `2.429L`,
  worsened mean distance to `8.454L`, retained a strong alternating planar and
  compact 3D wake, and exited the same lower boundary; full-direction gating
  (`2.494L`) and a response-gated posterior redirect (`2.536L`) also preserved
  that topology. Later workers should not retest a lower hold floor, sharper
  approach gate, or another continuous posterior mean on this evidence.
  Instead test a genuinely phase-selective actuator allocation, such as a
  bounded target-side half-cycle asymmetry, while preserving the carrier and
  checking posterior limit/load residence. This boundary applies only when
  cruise propulsion and wake coherence remain intact, and is falsified by a
  later continuous hold that captures, creates a useful termination class, or
  improves closest approach materially without degrading mean distance.
