...
- Instantaneous yaw is strongly contaminated by the propulsive rhythm in the
  ...
  reverses sign relative to target direction about `23%` of the time. When a
  coherent powered trajectory needs earlier redirect, do not increase the
  already saturated acceleration or static curvature to overcome this fast
  feedback. Test a body-frame course/slip or otherwise phase-separated mean-turn
  signal; this implication is falsified if removing beat-scale yaw response
  worsens early thrust, actuator residence, or correct-sign target progress.
