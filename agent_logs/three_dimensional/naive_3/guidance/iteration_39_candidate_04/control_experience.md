...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  posterior residuals, or constant signed restart. The inherited rhythmic
  sequence now closes the proposed one-sided half-cycle branch: a requested-
  sign pulse reaches only `1.702L` and parks both joints, while requested-side
  energy asymmetry regresses to `2.362L`. In contrast, low-activity feedback
  parallel to measured anterior velocity on both half-cycles is a semantic
  improvement: it reaches `1.175/4.041/3.243L`, remains inside `1.25L` for
  `2.35T`, preserves mean near-target `|phi_dot|` near `0.775/0.360 rad/T`, and
  lowers clamp residence to about `0.227/0.103`. Preserve this phase-balanced
  regulator as the active-wave terminal scaffold rather than strengthening or
  rescaling it: a repeated stronger radial regulator reaches only `1.366L`.
  A subsequent equal-and-opposite counterphase burst also fails its causal
  test. It increases useful yaw rate near closest approach from about `0.061`
  to `0.335 rad/T`, but reaches only `1.192/4.043/3.222L` and cuts residence
  inside `1.25L` to `1.24T`; coordinated yaw therefore does not by itself
  rotate the translational course inward. Do not retune burst magnitude,
  joint-phase gates, or scalar activity targets. A later terminal mechanism is
  applicable only if it preserves the two-sided regulator and explicitly tests
  conversion of measured body-turn response into inward course; falsify it on
  a broader loop, parked joint, lost wake coherence, higher clamp/load
  residence, or failure to improve closest approach and residence together.
- In a coherent, fast terminal C-turn, body yaw and translational-course
  rotation are separate control outcomes. Four sampled horizon rollouts with
  joint-state release, rear-centerline selection, posterior energy, or
  posterior turn-response phase modulation all settle into nearly the same
  `2.215--2.439L` orbit: near minimum they retain about `0.67U` speed,
  `1.70 rad` course error, `course_dot` near `-0.13`, and `+0.50U` body-frame
  lateral velocity despite useful yaw near `-0.25 rad/T`. The inherited
  phase-balanced anterior regulator remains the terminal scaffold because it
  reaches `1.175L` with `2.35T` inside `1.25L`, while a counterphase burst that
  raises useful yaw reaches only `1.192L` and halves close residence. For this
  topology, do not infer capture authority from yaw or add another
  geometry-only phase command. A genuinely new posterior role may instead use
  normalized hydrodynamic force as a gait-phase selector: in the four sampled
  traces, inward-signed force phases coincide with mean inward course rotation
  of about `0.31--0.34 rad/T`, whereas opposite-force phases rotate outward.
  This implication applies only after a coherent first return with active
  anterior motion; falsify any force-selected residual if it changes that
  return, increases clamp/load residence, or fails to improve `course_dot`,
  closest approach, and near-target residence together.
