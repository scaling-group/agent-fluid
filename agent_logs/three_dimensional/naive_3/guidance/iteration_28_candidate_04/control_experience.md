...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  a coherent `2.346L` first pass, reaches the `100T` horizon, and forms repeated
  returns instead of the powered lower exit. Preserve that nonsteady recovery
  topology, but the completed descendants now close both scalar contraction
  and more same-sign equilibrium persistence as terminal fixes. Nonclosing
  carrier contraction regressed to `2.484/4.545/4.673L`
  minimum/mean/final distance without reducing post-pass speed; merely holding
  redirect curvature reached `2.377L`. Moving response feedback into a
  course-selected curvature reserve was useful (`1.314/4.056/3.077L` with
  about `0.249/0.103` anterior/posterior clamp residence), and extending that
  reserve with a narrow terminal course hold produced the best sampled
  closest/final distance (`1.241/2.082L`) while retaining a coherent wake.
  However, the hold worsened mean distance to `4.158L`, made its best pass only
  at `97.092T`, and still had `0.669U` speed, `1.692 rad` target-ray/course
  error, and a slightly receding course while both joints occupied a
  `-0.396/-0.418 rad` C-bend. An ahead-only lateral bridge reached only
  `1.371L`, and the assigned parent's added anterior response-deficit bend is a
  stronger negative result: it regressed to `2.125/3.901/3.601L` and at the
  closest pass settled near `-0.366/-0.379 rad` with almost zero acceleration.
  Thus preserve the course-hold scaffold but avoid another terminal radius
  retune, scalar carrier contraction, or additive equilibrium-curvature burst.
  The evidence supports testing a bounded, joint-state-selected transition
  from an already established C-bend into dynamic stroke authority rather than
  demanding more static bend. Falsify that implication if the first pass or
  wake changes, posterior clamp/load residence rises, or closest approach,
  near-target residence, and horizon distance remain in the same class.
