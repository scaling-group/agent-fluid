...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  (median about `1.53 rad`) while speed remains finite; use that slower geometry
  only to select a genuinely different bounded rhythmic mechanism, such as
  state-phased duty or wave-shape modulation, and preserve the evidenced brake
  as the baseline. This implication applies only to coherent lateral near
  misses with a stable course mismatch; it is falsified if the selector changes
  sign at beat scale, cruise wake/progress degrades, actuator residence rises,
  or the new rhythmic mechanism leaves the same minimum and lower-exit class.
