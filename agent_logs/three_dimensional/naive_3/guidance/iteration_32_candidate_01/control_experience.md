...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  posterior residuals, or constant signed restart. A distinct rhythmic test
  may instead inject bounded energy only on the measured joint-velocity
  half-cycle already moving in the requested body-frame turn direction; this
  is applicable only while the useful scaffold retains active wave motion and
  command reserve, and is falsified by a changed first return, another parked
  bend, degraded wake coherence, higher clamp/load residence, or no improvement
  in near-target and final-distance statistics.
