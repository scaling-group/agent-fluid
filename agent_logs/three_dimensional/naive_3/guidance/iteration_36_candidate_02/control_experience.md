...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  one-joint duty-asymmetry, and simple posterior-lag retuning. At its `1.175L`
  minimum the active joints move in opposite signs (`-0.394/+0.260 rad/T`)
  with ample command reserve; a later distinct test may therefore condition a
  bounded equal-and-opposite action on both measured velocities already
  forming the requested bend-release transition. This paired propagation
  hypothesis applies only when the phase-balanced carrier is active and is
  falsified by another parked bend, a broader first approach, loss of wake
  coherence, higher clamp/load residence, or no joint improvement in closest,
  near-target, mean, and final-distance evidence.
