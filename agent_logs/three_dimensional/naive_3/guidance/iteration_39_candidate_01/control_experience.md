...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  posterior residuals, or constant signed restart. Completed inherited results
  now delimit the rhythmic branch as well. A requested-side pulse reaches only
  `1.702L`, whereas velocity-odd anterior feedback on both half-cycles is the
  first rhythmic semantic improvement: it reaches `1.175L`, stays inside
  `1.25L` for about `2.35T`, and retains active joint motion. Preserve that
  exact two-sided anterior law rather than changing its mean, phase balance, or
  activity target. Anterior duty asymmetry (`2.362L`), a stronger radial law
  (`1.366L`), and a paired counterphase burst (`1.192L`, only `1.24T` inside
  `1.25L`) fail to improve it. Response-triggered equilibrium release likewise
  reaches `1.177L` but worsens mean distance to `4.087L` and shortens close
  residence; extra yaw is not inward course redirection. Finally, adding a
  two-sided posterior activity floor regresses to
  `2.173/3.868/3.547L` minimum/mean/final distance and again parks both joints
  near `0.001/0.003 rad/T`. Close scalar activity, duty, counterphase-burst,
  equilibrium-release, and posterior-energy retuning, and distrust frozen
  action locality as evidence of coupled locality. A genuinely different
  actuator role may be tested only if measured hydrodynamic or course response
  selects it while the exact mean bends, nominal lag, and phase-balanced
  anterior carrier remain unchanged; reject it on a changed first return,
  another parked bend, yaw without inward course rotation, degraded wake or
  load margins, or failure to improve near-target residence and final return.
- Inspect the seed rollout, diagnostics, available observations, and inherited
  evidence to determine what capability is missing. Preserve behavior that the
  evidence shows is useful.
- Treat body-frame lateral force as a beat-scale load cue, not a slow course
  estimate, in the coherent terminal-loop topology. The four current
  direct-uniform samples all reach the horizon with coherent wakes yet miss at
  `2.215--2.439L` and finish `3.310--3.502L` away; their equilibrium release,
  rear-centerline selector, posterior phase response, and two-joint activity
  floor therefore do not repair capture. Within `3L`, course-signed lateral
  force is consistently associated with better absolute course alignment
  `0.25T` later (`r=0.432--0.458` in each trace), but that association reverses
  at `0.5T` (`r=-0.230---0.285`). This recurring sign reversal rules out using
  instantaneous force as heading or persistent mean-curvature feedback. It may
  instead gate a brief bounded compliant action while target-ray/course error
  retains route authority. Apply this only when the first return, active
  anterior carrier, coherent wake, and load margin are preserved; a changed
  first return, persistent bend, added load/clamp residence, yaw without inward
  course rotation, or no near-target-residence/final-return improvement
  falsifies the translation.
