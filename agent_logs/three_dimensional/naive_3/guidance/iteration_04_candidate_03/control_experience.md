...
- Once restrained target-relative curvature establishes a coherent approach,
  ...
  coherent wake and survival to `31.097T`. At that minimum the target was about
  `1.42 rad` off the full body axis while forward speed remained about
  `0.669 U`, after which the fish receded to a lower-boundary exit. A
  distance-only energy envelope (`2.845L`), full-direction posterior gating
  alone (`2.494L`), and away-half-cycle braking (`2.501L`) all preserved that
  termination topology; the boosted policy also clamped both accelerations for
  roughly three quarters of its rollout. For this powered lateral-near-miss
  class, avoid more scalar carrier relief or isolated half-cycle effort and do
  not treat repair of the acute-bearing alias as sufficient. Test a bounded
  redirect conditioned jointly on full body-frame direction and inadequate
  closing response while retaining the evidenced far-field carrier. This
  implication does not apply to weak-wake or wrong-sign release failures, and
  is falsified by degraded early progress, a short-wake tight curl, unchanged
  lower exit, or increased actuator-limit residence.
