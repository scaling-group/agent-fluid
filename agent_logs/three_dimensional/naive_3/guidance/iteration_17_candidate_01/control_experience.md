...
- Validate posterior steering sign as an actuator-topology choice before
  ...
  split.
- Separate a useful beat selector from the persistent course error that must
  drive terminal redirection. The assigned parent's measured-yaw posterior
  brake improved the alignment-gated carrier from `2.443L` to the strongest
  sampled minimum of `2.385L`, but raw yaw inside `3L` is phase-locked to
  anterior joint velocity (`|r|=0.992--0.998`) and has the error-growing sign
  for only `47.3--50.2%` of samples. The sampled joint-phase counterbend
  (`2.512L`), gait-yaw residual (`2.541L`), and response-released posterior
  S-bend (`2.536L`) all kept the powered lower exit; inherited completed logs
  add a posterior counterstroke (`2.585L`), smoother polarity reallocation
  (`2.406L`), and anterior phase brake (`2.628L`) with the same semantic
  failure. These finite traces retain coherent long wakes and comparable
  command/load scales, so do not pursue another yaw-selected damping,
  polarity, equilibrium, or scalar-threshold edit on this carrier. Instead,
  when speed remains above `0.2U`, test a bounded approach-localized redirect
  from the body-frame angle between translational course and the target ray:
  that angle has one persistent sign and median magnitude about
  `1.525--1.544 rad` inside `3L`, and remains `1.152 rad` with `0.705U` speed at
  the best minimum even though instantaneous closure is only `0.014 L/T`.
  This implication applies to coherent powered cross-track near misses and is
  falsified if a course-based redirect changes far-field progress, produces a
  tight curl or greater limit/load residence, loses wake coherence, or leaves
  both the minimum and lower-exit topology unchanged; it does not justify
  global sideslip feedback, which previously reversed the useful release into
  an upper exit with only `12.150L` closest approach.
