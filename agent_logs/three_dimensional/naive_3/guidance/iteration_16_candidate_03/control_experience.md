...
- Validate posterior steering sign as an actuator-topology choice before
  ...
  split.
- Decompose a measured response when evidence assigns it both beat-local and
  course-scale roles. The measured-yaw posterior brake improved minimum
  ...
  equilibrium S-bend (`2.536L`), so raw yaw is useful as a posterior half-cycle
  selector. It is not an evidenced slow-course estimate: inside `3L`, all four
  sampled traces have heading rate anticorrelated with anterior joint rate at
  `|r|=0.9925--0.9978`, and their fitted slopes span only `-0.459 to -0.399`.
  At the best minimum, where direction error is `1.38 rad`
  and speed is `0.705U`, raw yaw is `2.185 rad/T` but the cross-sample residual
  `heading_rate + 0.44*phi_dot1` is only `0.221 rad/T`. The assigned parent's
  posterior reallocations reached only `2.444L` and `2.585L`, and inherited
  step-15 scores add `2.406L` and `2.541L` lower-boundary misses, all without a
  new semantic outcome. Avoid another posterior reallocation, phase proxy, or
  scalar brake edit in this topology. The reusable next test is to fit and
  remove the repeatable joint-rate component before yaw enters the anterior
  mean-curvature course loop, while retaining raw yaw only for beat-local
  selection. This applies to a coherent carrier with repeatable phase locking;
  it is falsified if residualization changes far-field cruise, loses wake
  coherence, increases limit/load residence, or leaves both minimum distance
  and the powered lower-exit trajectory materially unchanged. Re-estimate the
  coupling for any carrier whose phase relation changes.
