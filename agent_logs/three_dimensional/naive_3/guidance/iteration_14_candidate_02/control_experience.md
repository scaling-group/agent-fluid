...
- Use instantaneous measured yaw as a terminal half-cycle selector, not as an
  ...
  speed `0.705U`; it and later inherited score-log minima of about `2.444L` and
  `2.633L` retained the powered lower-exit class. Thus the brake establishes a
  useful response-selected beat interval, not completed navigation: avoid
  another response-threshold or scalar brake-strength edit, and test a new
  within-beat allocation such as converting only the discarded posterior
  authority into a bounded counterstroke. This implication applies to coherent
  lateral near misses with the same strong yaw/joint-rate phase lock; it is
  falsified by useful slow yaw information under another carrier, or if the
  new allocation degrades cruise/wake coherence or leaves both the minimum and
  lower-exit topology materially unchanged.
