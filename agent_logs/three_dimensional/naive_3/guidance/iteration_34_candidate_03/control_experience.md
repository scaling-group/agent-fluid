...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  only about `0.13 rad/T`; the joint commands are small but the traveling bend
  remains active (`qdot1=-0.260`, `qdot2=0.108`). Completed descendants close
  the proposed signed-burst and selector branch: inherited score logs show
  ungated and target-behind anterior bursts reaching only `2.125/3.601L` and
  `1.987/3.522L` minimum/final distance, and the sampled joint-state unbend,
  course-signed low-activity restart, and rear-crossing selector regress to
  `2.215/3.416L`, `2.369/3.455L`, and `2.366/3.502L`; every run remains a
  horizon noncapture. At the three sampled descendant minima, speed remains
  about `0.677U`, yet both joints are nearly parked in a common negative
  C-bend (`abs(qdot1) <= 0.0027`) with near-zero commands. The restart's lower
  clamp residence (`0.134/0.099` versus `0.329/0.102` for the useful course
  hold) rules out global command saturation as the cause, while its regression
  shows that low activity is only a diagnostic, not evidence for another
  one-sided kick. Avoid further signed bursts, equilibrium release, rear-side
  selection, or restart threshold/sign tuning on this scaffold. A distinct
  gait-recovery test may reinforce both measured anterior velocity half-cycles
  about the unchanged steering equilibrium and release on recovered normalized
  phase-plane activity. This implication applies only after target-behind
  terminal geometry, finite speed, weak rhythmic activity, and command reserve
  coexist; reject it if the ahead-side return changes, the quiet bend persists,
  the posterior traveling wave degrades, orbit size or clamp/load residence
  rises, or near-target and final-distance statistics do not improve.
