...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  scaffold. Its remaining failure is an oversized powered orbit: after
  redirect engagement it gets no closer than `2.427L`, travels about
  `0.6--0.7U` around `3--5.8L`, and retains roughly `1.1--2.2 rad` target-ray
  to course mismatch. The next completed quartet closes response-gated carrier
  removal as the remedy. A target-behind, nonclosing envelope contraction
  improved mean distance from `4.714L` to `4.545L`, but worsened first-pass
  minimum/final distance from `2.346/3.902L` to `2.484/4.673L`; its post-pass
  speed remained `0.645U` versus `0.633U`, clamp residence remained about
  `0.712/0.103`, and the coherent broad orbit survived. Radial-response release
  of the redirect was worse: anterior clamp residence fell to `0.139` and speed
  rose to about `0.725U`, while mean/minimum distance regressed to
  `4.780/2.532L`. Do not infer useful slowing or approach from an
  oscillator-envelope command or reduced clamp residence without measured
  speed and range response, and do not remove established redirect curvature
  merely because course briefly closes. A response-held C-turn is the useful
  boundary: it keeps the coherent wake, improves mean distance to `4.458L`,
  and brings the second return to about `2.60L` rather than the baseline's
  `3.15L`, but still misses and orbits. Preserve the carrier and redirect; if
  testing course response next, apply it to bounded turn authority only after
  the target is fully behind, and falsify it if the `2.35L` first pass changes,
  clamp/load residence rises, or the repeated orbit does not contract. Avoid
  further contraction/release, phase/duty reallocation, moment residuals, or
  response gates justified only by lower saturation.
- Frozen-trace locality is not sufficient near the terminal-course scaffold's
  target-crossing bifurcation. Relative to its coherent
  `1.241/4.158/2.082L` minimum/mean/final-distance result, three completed
  dynamic residuals all lost the closest and final-distance gains: an anterior
  equilibrium burst reached `2.125/3.901/3.601L`, a posterior acceleration
  residual reached `1.442/4.003/3.905L`, and the assigned parent's
  line-of-sight-rate/yaw-rate residual reached `1.578/4.361/4.478L`. Although
  their frozen replays predicted negligible action at the original `2.466L`
  first recovery, the coupled rollouts moved that pass to `2.62--2.68L` and
  reversed yaw from about `+2.18` to `-1.69..-1.75 rad/T`; all remained finite
  with coherent wakes, so this is a control-topology failure rather than a
  propulsion or stability failure. Do not add another terminal response term
  that is active on both sides of the target crossing, and do not infer
  integrated locality from a small frozen action delta. If dynamic authority
  is revisited, gate it with full body-frame ahead/behind geometry so it is
  effectively absent on the ahead-side approach, then falsify it on any change
  to first-recovery geometry or yaw. This boundary applies to the coherent
  horizon C-turn scaffold; it does not validate posterior authority for a
  stalled, incoherent, or already capturing trajectory. The sampled visual and
  trace evidence sharpens the allowable behind-side interpretation: the
  terminal hold's best state occurs at `97.092T` and `1.241L` with the target
  fully behind/lateral (`forward=-0.855`, `lateral=0.518`), nonclosing course
  (`course_dot=-0.121`), and target-ray/yaw rates of about
  `-0.535/-0.129 rad/T`, while its alternating planar wake and oblique Lambda2
  structures remain coherent. The current completed quartet and the assigned
  parent's inherited wake sheet now close the proposed terminal-authority
  followups more sharply. An anterior response burst reaches
  `2.125/3.901/3.601L` minimum/mean/final distance, a turn-rate-driven
  posterior phase residual reaches `2.137/3.873/3.530L`, and a rear-crossing
  direction blend reaches `2.366/3.875/3.502L`; near each minimum the joints
  are nearly parked in a common C-bend, unlike the terminal hold's active
  anterior wave (`phi_dot_1=-0.260 rad/T`) at `1.241L`. The assigned parent's
  fully-behind ray-rate/yaw-rate curvature reserve is also evaluated: its
  coherent horizon wake regresses to score `-4.809` and
  `1.418/4.363/3.233L`, so behind-side locality does not rescue that residual.
  Do not add or retune another terminal curvature burst, target-ray/yaw-rate
  reserve, course-direction blend, or posterior turn-rate phase term on this
  scaffold. Preserve the terminal hold's C-turn and active traveling wave.
  Its unresolved observable is powered overshoot: speed remains about
  `0.63--0.68U` on returns between `1.24--1.73L`, whereas completed scalar
  oscillator contraction did not produce measured slowing. If speed control
  is revisited, require a structurally braking propulsion response selected by
  normalized closing geometry and falsify it directly on return speed, orbit
  contraction, and wake continuity rather than command amplitude. This
  boundary applies to the coherent, finite horizon scaffold; it does not
  support braking a stalled or incoherent swimmer, nor does it establish any
  unevaluated braking mechanism as beneficial.
