...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  only about `0.13 rad/T` while both joint commands are small. Three completed
  descendants close the proposed burst branch: an ungated anterior response
  burst (`2.125/3.901/3.601L` minimum/mean/final), the same burst gated to
  target-behind geometry (`1.987/3.887/3.522L`), and a joint-state posterior
  stroke (`1.622/3.964/3.799L`) all retained coherent self-propulsion and the
  `100T` horizon but displaced the parent's useful `1.241/4.158/2.082L` close
  return. At the descendant minima both joints were nearly parked in a common
  negative C-bend, whereas the parent arrived with an active anterior wave and
  unclipped commands. Avoid another anterior response burst, target-behind
  gate, posterior stroke release, or frozen-delta claim of trajectory locality
  on this scaffold. The proposed rear-crossing direction selector is now also
  closed: blending lateral target side toward persistent target-ray/course sign
  regressed the parent's `1.241/4.158/2.082L` minimum/mean/final result to
  `2.366/3.875/3.502L`. A target-behind yaw-rate error applied through posterior
  phase lag likewise reached only `2.137/3.873/3.530L`; both retained coherent
  wakes and the `100T` horizon, so neither another rear-side selector nor
  yaw-rate lag modulation is supported. Across the current completed quartet,
  normalized anterior phase-plane activity inside `3L` has median `0.699` for
  the `1.241L` course hold but only `0.011`, `0.004`, and `0.003` for the burst,
  yaw-rate-phase, and rear-selector failures; those failures spend about
  `86--92%` of their close samples below `0.15` and approach their minima near a
  stationary common C-bend. Later completed evidence makes that discriminator
  noncausal by itself: the assigned parent's course-signed low-activity restart
  reaches only `2.369/3.869/3.455L`, and a velocity-odd activity regulator
  reaches `1.631/3.937/3.654L`; both retain noncapturing powered loops. Avoid
  another parked-state kick, two-sided activity-deficit term, or claim that
  restoring phase-plane radius alone restores a traveling wave. A moving,
  course-signed anterior half-cycle is conditionally different: the inherited
  `1.5 rad/T^2` variant improves closest approach and residence inside `1.25L`
  from `1.241L/0.473T` to `1.228L/0.808T` while retaining active joint motion,
  although its final distance regresses to `3.368L`; a sibling parameterization
  reaches only `1.702L`, so this is not evidence for monotonic pulse-gain
  tuning. Its completed release test separates course orientation from realized
  response. Restricting the pulse to positive instantaneous target-ray/course
  dot improves final distance to `2.484L` but regresses the minimum to `1.274L`.
  At the `1.241L` scaffold and `1.274L` release-test minima, course dot is
  already about `-0.121/-0.099` while the finite distance window still reports
  about `+0.007 L/T` closure; after the latter pass, median window closure
  inside `2L` is `-0.266 L/T`. For this powered tangential-return topology,
  treat finite-window range closure as the next response-release observation,
  not another course-dot threshold or pulse magnitude. Reject that distinction
  if it opens the close arc, fails to remove post-pass action, parks the wave,
  raises clamp/load residence, or does not jointly improve near-target and
