...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  the course hold's `1.241/2.082L` closest/final distances. Treat posterior
  phase as a provisional complementary steering-response role, not a proven
  improvement. It is applicable only while mean bends and the exact
  phase-balanced anterior law remain unchanged, and is falsified unless a
  combined evaluation improves closest approach or capture, near-target
  residence, and terminal return without parking, saturation, or wake
  broadening. That combination is now falsified: adding the response-gated
  posterior phase role to the exact phase-balanced carrier reaches only
  `2.439/3.861/3.310L` minimum/mean/final distance, never enters `2L`, and
  arrives at its minimum with anterior/posterior joint velocities of only
  about `-0.00053/+0.00179 rad/T`. Two later attempts show that the failure is
  not specific to a phase-dependent tail action. Bounded two-sided posterior
  phase-plane energy improves that parked cluster only to
  `2.173/3.868/3.547L` and still has joint velocities near
  `+0.001/+0.003 rad/T` at minimum; a phase-independent, response-gated
  anterior acceleration reaches `2.077/3.890/3.512L` but retains the same
  broad, visually rigid C-bend loop. None is a semantic improvement over the
  active `1.175L` carrier, and the last result shows that merely making a
  correction nonzero at a frozen parked state is insufficient. Close
  posterior phase response, posterior anti-parking energy, and direct
  anterior turn-response residuals on this scaffold. Preserve the exact
  symmetric anterior regulator as the evidence control; later mechanisms must
  demonstrate that they retain the alternating joint motion and undulating
  wake, not just boundedness, reflection symmetry, or a locally nonzero action.
