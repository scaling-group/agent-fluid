...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  only while signed yaw grows the target-direction error improves the alignment
  carrier from `2.443/8.443L` to `2.385/8.436L` minimum/mean distance, but the
  sampled equilibrium and rhythmic reallocations all retain the powered lower
  exit: geometry-persistent posterior shift reaches `2.484L`, posterior
  polarity reversal `2.406L`, gait-yaw residualization `2.541L`,
  closure-gated anterior redirect `2.569L`, differential S-bend `2.469L`, and
  anterior duty `2.433/8.434L`. Do not retune scalar brake thresholds, static
  C/S-bends, counterstrokes, anterior duty, or beat-locked yaw as a mean-course
  estimate.
- Preserve proximity-localized posterior phase modulation only as a provisional
  scaffold, not as established recovery. Across the four direct-uniform
  samples, target-ray/course error remains about `1.53 rad` inside `3L` while
  speed stays finite. Moving that slow selector from anterior duty to bounded
  joint-state-selected posterior lag improves the yaw brake's `2.385/8.436L`
  minimum/mean distance to `2.326/8.424L`; both top-down vorticity and oblique
  Lambda2 sheets retain the long coherent three-dimensional wake. Yet at the
  best minimum the fish still travels at `0.687U` with `1.088 rad` course
  error, total anterior/posterior acceleration-clamp residence remains about
  `0.749/0.355`, and it powers to the same lower exit at `9.213L` final
  distance. Two later inherited score-only completions also remain
  `left_domain`, reaching only `2.390L` and `2.684L` with final distance near
  `9.2L`; because their compact logs lack trajectories, they establish no
  semantic improvement but cannot identify an actuator cause. Avoid retuning
  phase radius/amplitude, command relief, another equilibrium burst, or
  post-overshoot damping. A next mechanism-level test may instead use
  normalized lateral target geometry and closing response to allocate
  posterior oscillatory authority selectively across gait half-cycles, and
  must yield capture, a target-return leg, a useful termination class, or a
  material improvement below `2.326L` without worse mean distance. This lesson
  applies only to coherent lateral near misses with stable course mismatch; it
  is falsified if selective phase action degrades cruise wake/progress, curls
  tightly, raises actuator or load residence, or again leaves only a small
  scalar shift in the same lower-exit class.
