...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  commands are small. The equilibrium/constant-residual branch is closed by
  rear-centerline selector replacement, joint-state equilibrium unbend, and a
  course-signed low-activity restart, which reach only
  `2.366/3.875/3.502L`, `2.215/3.859/3.416L`, and
  `2.369/3.869/3.455L` minimum/mean/final distance. Each remains finite with a
  coherent wake to the horizon but loses the tight return; at representative
  minima the latter two have nearly zero anterior velocity
  ...
  posterior residuals, or constant signed restart. The later rhythmic evidence
  sharpens that rule: a requested-sign anterior half-cycle pulse (`1.702L`),
  requested-side duty asymmetry (`2.362L`), a stronger activity regulator
  (`1.366L`), a posterior activity regulator (`2.173L`), and the current
  dual-joint activity regulator (`2.320/3.868/3.443L`) all fail to improve the
  useful return. The sole positive member is symmetric, two-sided anterior
  low-activity feedback: it reaches `1.175/4.041/3.243L`, stays inside `1.25L`
  for about `2.35T`, and retains mean anterior/posterior joint speed near
  `0.775/0.360 rad/T` inside `2L`. Preserve that exact anterior carrier rather
  than retuning its amplitude, threshold, half-cycle balance, or adding tail
  energy. Finally, an inherited force-selected positive-work tail stroke
  regresses to `2.391/3.883/3.489L` and visibly recovers the broad parked loop,
  so a strong force/future-course correlation is not causal evidence for
  reinforcing the generating stroke. If normalized hydrodynamic response is
  revisited, a genuinely distinct bounded test is to suppress only measured
  outward-producing posterior motion while leaving the anterior carrier and
  mean bend unchanged; it is applicable only while tail motion and force sign
  remain resolved, and is falsified by tail parking, lost wake propagation,
  changed first return, worse load/clamp residence, or no joint improvement in
  closest, near-target, mean, and final-distance evidence.
...
- Diagnose the current powered return as a joint phase-propagation failure
  only when geometry, motion, and joint state agree. The assigned parent's
  course-turn-force-selected posterior damping (`2.294/3.877/3.471L`
  minimum/mean/final), dual-joint low-activity energy
  (`2.320/3.868/3.443L`), joint-state C-bend release
  (`2.215/3.859/3.416L`), and posterior turn-response lag modulation
  (`2.439/3.860/3.310L`) all remain finite to `100T` with coherent wakes yet
  miss in the same state: speed is `0.677--0.681U`, the target is behind and
  lateral with course error about `1.70 rad`, both joints sit near the common
  `(-0.36,-0.38) rad` C-bend with at most `0.003 rad/T` joint speed, and yaw is
  only about `0.25--0.27 rad/T`. Together with the inherited best
  two-sided-anterior carrier (`1.175L` minimum), these results close more
  posterior energy, posterior force selection, static equilibrium release,
  and posterior phase-response variants for this topology. When the target is
  behind, course is nonclosing, translational speed remains finite, and
  anterior activity has actually collapsed, test a bounded state-coupled
  anterior phase release while preserving mean curvature and the nominal
  lagged tail; avoid another force correlation, tail-energy term, distance-gate
  retune, or static unbend. This implication does not apply while the joint
  wave remains active or the target is still forward, and it is falsified if
  coupling changes the first approach, loses wake propagation, increases load
  or clamp residence, parks either joint again, or fails to improve closest,
  near-target, mean, and final-distance evidence together.
