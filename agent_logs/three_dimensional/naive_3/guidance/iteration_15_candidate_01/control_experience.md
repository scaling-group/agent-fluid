...
- Validate posterior steering sign as an actuator-topology choice before
  ...
  split.
- Separate a useful terminal selector from an effective terminal action. On
  the same coherent alignment-gated carrier, a measured-yaw-selected posterior
  brake improved minimum/mean distance from `2.443/8.443L` to
  `2.385/8.436L`, while a joint-phase-selected counterbend reached only
  `2.512L` and the assigned parent's response-released equilibrium S-bend
  reached `2.536L`; all kept the long wake and powered lower exit. The
  inherited completed sequence (`2.484`, `2.429`, `2.385`, then `2.444L`)
  likewise produced no new termination class. Moreover, inside `3L` the raw
  yaw signal is almost entirely beat-locked to anterior joint velocity
  (`|r|=0.992--0.997` across the sampled rollouts), so it is supported as a
  half-cycle selector but not as a slow course-error estimate. The inherited
  test that reallocated rather than discarded the selected posterior authority
  then improved the minimum by only `0.010L` (`2.375L`), worsened mean distance
  to `8.460L`, increased wrong-way yaw at the minimum from about `2.185` to
  `2.330 rad/T`, and preserved the same lower exit and visual wake topology.
  Avoid another scalar brake threshold, symmetric drive relief, phase-only
  counterbend, persistent posterior equilibrium redirect, or another
  yaw-selected posterior-authority transfer. A distinct next test must move
  slow course authority to the anterior joint or use a response-released
  geometry-scale mechanism while preserving the cruise wave. This applies
  only to the same coherent powered lateral-near-miss topology and is falsified
  if a later posterior transfer yields capture or a meaningfully different
  termination/trajectory without degrading far-field progress, saturation, or
  wake coherence.
