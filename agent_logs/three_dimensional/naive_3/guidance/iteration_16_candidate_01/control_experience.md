...
- Validate posterior steering sign as an actuator-topology choice before
  ...
  split.
- Use instantaneous measured yaw as a terminal half-cycle selector, not as an
  estimate of slow course response, under this carrier. The assigned parent's
  approach-localized brake used signed full target direction and heading rate
  to weaken only error-growing posterior-wave intervals; it improved minimum
  distance from the inherited cross-track/closure hold's `2.429L` to `2.385L`
  and beat a joint-wave-phase counterbend (`2.512L`), a full-direction gate
  (`2.494L`), and a response-released equilibrium S-bend (`2.536L`). Yet across
  all four sampled traces inside `3L`, heading rate is phase-locked to anterior
  joint velocity (`|r|=0.992--0.998`) and has the target-error-growing sign for
  only `47.3--50.2%` of samples. At the best minimum the direction error still
  measured `1.38 rad`, wrong-way yaw `2.19 rad/T`, closure `0.014 L/T`, and
  speed `0.705U`. Completed follow-ons now reject several plausible uses of
  that interval: converting discarded posterior authority into a counterstroke
  reached `2.585L`, a smooth posterior polarity reallocation reached `2.406L`,
  the assigned parent's anterior phase brake reached `2.628L`, and
  gait-yaw residualization reached `2.541L`; all retained the coherent wake and
  powered lower exit. The sampled joint-phase counterbend likewise reached
  only `2.512L`. Thus avoid another response-threshold or scalar brake edit,
  posterior equilibrium/polarity reallocation, anterior damping impulse, or
  fitted yaw/joint-rate residual unless a new semantic trajectory first changes
  the diagnosis. A distinct bounded test is to modulate posterior wave lag
  while normalizing nominal wave amplitude and preserving the evidenced brake,
  because none of these failures isolated bend timing from drive, equilibrium,
  or polarity. This implication applies to coherent lateral near misses with
  the same strong yaw/joint-rate phase lock; phase-lag modulation is falsified
  if it changes cruise or load residence, or leaves both closest approach and
  the lower-exit topology materially unchanged.
