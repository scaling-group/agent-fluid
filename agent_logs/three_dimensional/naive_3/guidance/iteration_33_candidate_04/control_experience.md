...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  stationary common C-bend. Treat this as evidence for a distinct gait-state
  test, not proof that low activity caused the miss. The inherited completed
  tests now separate parked restart from active half-cycle shaping.
  Odd-in-velocity energy gated by low phase-plane activity reaches only
  `1.631/3.937/3.654L` minimum/mean/final distance, while a broader
  useful-half-cycle parameterization reaches only `1.702/3.921/3.574L`; both
  preserve the horizon and coherent wake but lose the close final return. In
  contrast, a narrower active anterior half-cycle pulse is the first gait-state
  descendant to improve the course hold's near statistics: it reaches
  `1.228L`, increases residence inside `1.25L` from `0.473T` to `0.808T`,
  retains moving joints at the minimum, and keeps posterior clamp and `95%`
  force/moment magnitudes comparable. This small closest-approach gain is not
  capture and is coupled to worse mean/final distance (`4.045/3.368L`), so it
  does not establish a monotonic pulse-gain trend. Preserve the measured-motion
  half-cycle topology only as a provisional scaffold; close another
  parked-state restart, equilibrium shift, posterior stroke, or scalar pulse
  retune. Its next falsifiable extension should release extra half-cycle action
  from measured target response when normalized closure becomes nonpositive,
  while leaving the course hold and traveling carrier active. Reject that
  extension if it loses the `1.228L` pass or active wave, fails to improve
  near-target residence or the post-pass loop, or worsens clamp/load margins.
