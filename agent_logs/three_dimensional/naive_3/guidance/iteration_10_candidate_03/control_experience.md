...
- Distinguish extra same-sign C-curvature from posterior counter-bend when
  body pointing is near the target but translation remains cross-track. A
  wrong-side-only `5 deg` posterior mean bias worsened the alignment-gated
  carrier's minimum from `2.443L` to `2.697L`, whereas a bounded S-shaped
  posterior counter-bend improved it to `2.187L` and reduced mean distance
  from `6.781L` to `6.683L` while retaining a coherent wake and comparable
  peak loads. The counter-bend is useful but incomplete: it still powered
  through a lower exit and increased posterior command-clamp residence from
  `35.4%` to `38.1%`. A later distance-enveloped target-ray cross-velocity
  residual in that posterior channel improved the minimum again to `2.011L`
  with comparable peak lateral force/yaw moment (`0.0261/0.0150`) and only
  `35.0%` posterior clamp residence, confirming that normalized course error
  contains useful information beyond instantaneous wrong-side slip. It did
  not produce a semantic improvement: the same powered lower exit remained,
  mean distance worsened from `8.446L` to `8.740L`, and final distance grew
  to `9.836L`. Preserve the residual sign and far-field carrier, but do not
  retry a larger or more persistent posterior equilibrium residual; test a
  separately bounded actuator allocation or response-conditioned redirect
  and demand capture or a different useful trajectory, not only another
  smaller minimum. This implication applies to stable, self-propelled
  cross-track passes with coherent wakes and comparable loads. Altered
  release, lost propulsion, a tight curl, sharply greater limit/load
  residence, or failure to beat `2.011L` falsifies a new allocation.
  Unrestricted course-angle-to-anterior-curvature feedback remains
  contraindicated because it reversed release and exited above after reaching
  only `12.150L`.
