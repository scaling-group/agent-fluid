...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  only about `0.13 rad/T` while both joint commands are small. In this coherent
  terminal-orbit regime, the next distinct mechanism may test a bounded,
  response-released anterior turn burst without changing the posterior wave.
  This implication applies only when course-hold improvement, finite speed,
  weak requested-sign yaw, and command reserve coexist. The proposed anterior
  burst family is now closed by completed descendants: ungated and
  target-behind equilibrium bursts reach only `2.125L` and `1.987L`, a
  joint-state unbend reaches `2.215L`, a fixed-sign low-activity restart reaches
  `2.369L`, and the latest velocity-aligned useful-half-cycle pulse reaches
  `1.702L` while regressing mean/final distance to `3.921/3.574L`; all remain
  horizon noncaptures. The half-cycle pulse preserves a coherent wake but its
  normalized anterior phase-plane activity inside `1.8L` collapses to
  `0.004/0.011/0.015` at the 10th/50th/90th percentiles, versus median `0.523`
  for the `1.241L` course-hold scaffold, and both joints are nearly stationary
  at its minimum. Do not add another signed burst, equilibrium release,
  low-activity kick, or half-cycle gain. The proposed symmetric activity test
  is now completed and sharply bounded. A phase-balanced regulator that adds
  energy on both measured anterior-velocity half-cycles improves closest
  approach to `1.175L`, remains inside `1.25L` for about `2.35T`, preserves
  active terminal joints, and lowers clamp/load residence, but its
  `4.041/3.243L` mean/final distance remains worse than the course hold's
  `4.158/2.082L`; independent symmetric variants reach only
  `1.615/4.089/2.811L` and `1.366/4.041/3.047L`. A useful-side activity
  asymmetry then regresses to `2.362L`. Low phase-plane activity is therefore
  a valid diagnostic for the parked-bend descendants, but restoring activity
  is not itself steering and another energy floor, duty asymmetry, or drive
  scalar is unsupported. Keep route-response and carrier-amplitude coordinates
  separate: direct anterior target-ray-rate residual control reaches only
  `2.201/3.888/3.584L`, while response-driven posterior phase alone reaches
  `1.276/4.085/2.881L` and does not beat the `1.241L` course hold. A small
  compound test is applicable only when active joints coexist with the same
  tangential, weak-yaw terminal miss; it should preserve the evidenced
  phase-balanced carrier and put bounded response authority in posterior phase,
  not add curvature or direct acceleration. Falsify that separation if the
  first return changes, activity or requested-sign yaw does not improve, the
  orbit/wake expands, clamp/load residence rises, or near-target, mean, and
  final-distance statistics retain the noncapturing class.
