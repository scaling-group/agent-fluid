...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  response mechanism is evidenced. One calibrated candidate signal is
  normalized yaw moment: inside `4L` on the `2.326L` trace it spans about
  `-0.0121--0.0109` and correlates `0.927` with heading-rate derivative, so a
  slow course request may gate a small harmful-sign hydrodynamic residual
  rather than another equilibrium or duty bias. That correlation is not
  causal proof and external wakes may contain useful moment; reject this route
  if it damps both signs, damages cruise wake/progress, raises clamp/load
  residence, or again leaves the same minimum and lower-exit class. This
  implication applies only to coherent lateral near misses with a persistent
  course mismatch and a separately calibrated fast moment response.
