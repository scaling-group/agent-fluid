...
- Distinguish a useful terminal selector from an effective terminal action.
  ...
  rollout retained a long alternating 3D wake and powered lower exit. Inherited
  completed notes add a response-gated equilibrium residual at about `2.444L`
  with no new termination class. Inside `3L`, yaw rate is almost entirely
  beat-locked to anterior joint velocity (`|r|=0.992--0.998` across the four
  samples), so it is supported as a half-cycle selector but not a slow
  course-response estimate. Reconstructing the best brake's selector and
  posterior target gives 397 active gated samples inside `3.2L`; in 288
  (about `72%`) the traveling-wave target points with the growing full-direction
  error. Thus the brake's discarded authority is a concrete actuator resource,
  not evidence for more suppression. Avoid another scalar brake edit,
  joint-phase-only bias, response residualization, or equilibrium redirect;
  the distinct actuator test is to retain the measured-yaw selector and
  reallocate its removed posterior-wave magnitude into a bounded
  target-directed counterstroke, leaving an already corrective selected wave
  unchanged. This applies only to the same powered lateral-near-miss topology
  and is falsified by lost far-field progress or wake coherence, increased
  posterior limit/load residence, a minimum no better than `2.385L`, or an
  unchanged lower-exit trajectory after reallocation.
