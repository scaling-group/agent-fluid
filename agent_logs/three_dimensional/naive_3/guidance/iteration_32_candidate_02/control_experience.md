...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  on this scaffold. Three newer completed samples close the corresponding
  release/selector branch: rear-centerline direction blending reaches
  `2.366/3.875/3.502L` minimum/mean/final distance, a low-activity anterior
  wave restart reaches `2.369/3.868/3.455L`, and the assigned parent's
  quiet-C-bend anterior release reaches `2.215/3.859/3.416L`. All three retain
  coherent self-propulsion and the `100T` horizon but settle into broad loops
  with nearly stationary common negative bends, so do not add another
  rear-crossing selector, quiet-bend release, wave restart, or frozen-locality
  refinement. The useful scaffold instead exposes a timing boundary on later
  returns: while the target is already behind it is still closing at
  `1.643L` and `1.730L` with course dot `0.575` and `0.501`, yet its
  constant-course miss estimate remains `1.344L` and `1.497L`; the existing
  nonclosing response is then off and the terminal hold is only partly active.
  A distinct test may preserve carrier and maximum curvature while engaging
  that same response from normalized closing-miss geometry before closest
  approach. Apply this only when target-behind, finite-speed closing and a
  large projected miss coexist; reject it if the target-ahead return changes,
  wake/load margins worsen, or coupled near-target and final statistics do not
  improve.
