...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  weaker in the current sample (`1.282/2.862L` minimum/final distance) and
  never enters `1.25L`, so preserve the continuous course-dot hold rather than
  retuning lateral thresholds, radius, drive, or maximum curvature. At the
  `1.241L` miss, speed remains `0.669U`, course error is `1.692 rad`, normalized
  course is slightly receding, and useful yaw is only about `0.13 rad/T` while
  both joint commands are small.
- Frozen-trace locality is not sufficient around this course-hold scaffold's
  target-crossing bifurcation. Relative to its coherent
  `1.241/4.158/2.082L` minimum/mean/final-distance result, an ungated anterior
  response burst reaches only `2.125/3.901/3.601L`; inherited line-of-sight-rate
  and posterior-acceleration residuals reach `1.578/4.361/4.478L` and
  `1.442/4.003/3.905L`. The first three retain coherent wakes but move the
  original `2.466L` recovery to `2.62--2.68L` and reverse yaw despite nominally
  negligible replay actions there. Multiplying posterior acceleration by the
  full-direction target-behind selector isolates the late action but is not a
  remedy by itself: it approximately preserves the first recovery
  (`2.442L`, yaw `2.168 rad/T`, versus `2.466L` and `2.176 rad/T`) yet still
  regresses to `1.732/4.263/2.093L`. Avoid more ungated terminal dynamics,
  another posterior acceleration residual, or claims of integrated locality
  based only on frozen action deltas. The assigned parent's fully-behind-gated
  anterior equilibrium burst closes the remaining locality argument: it
  reaches only `1.987/3.887/3.522L`, and after `40T` its mean joint-speed/action
  norms fall to `0.012/0.046` with every sample below `0.05 rad/T` joint-speed
  norm. The sampled ungated burst similarly falls to `0.010/0.049`, while a
  joint-state posterior-stroke release reaches `1.622/3.964/3.799L` but still
  falls to `0.021/0.121` and is below that activity threshold for about `95%`
  of late samples. By contrast, the `1.241L` scaffold retains late norms of
  `1.372/15.829` and is below the threshold for only about `0.1%`. Thus finite
  body speed near `0.67U` and a persistent wake trail do not establish active
  propulsion after a maneuver; joint motion and action history expose the
  nearly static C-bend. Avoid more static anterior curvature, posterior-stroke
  release, or acceleration residuals in this topology. If a distinct stall
  recovery is tested, keep it effectively absent before the target is fully
  behind and require it to preserve the base C-turn while restoring joint-wave
  activity; reject it on any change to first-recovery geometry or yaw, higher
  clamp/load residence, continued gait collapse, or failure to improve
  near-target residence and distance statistics. This boundary applies to the
  finite-speed horizon loop; it does not establish the same selector for an
  incoherent, externally advected, or already capturing trajectory.
