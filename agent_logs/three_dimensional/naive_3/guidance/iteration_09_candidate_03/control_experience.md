...
- Once restrained target-relative curvature establishes a coherent approach,
  ...
  (`2.501L`) all preserved that topology, while the best plain gated carrier
  already clamped anterior/posterior acceleration for about `0.746/0.354` of
  its samples. Slip-gated posterior phase rotation worsened closest approach to
  `2.822L`, whereas a target-side, wrong-slip-gated posterior counter-bend kept
  comparable mean distance and loads, improved the minimum to `2.187L`, and
  changed yaw at that minimum from wrong-side `+2.04 rad/T` to corrective
  `-0.33 rad/T`. That counter-bend is useful actuator-allocation evidence, not
  a solved controller: it still powered into the same lower exit, and its slip
  weight had fallen to about `0.041` at minimum despite `1.49 rad` full target
  error and essentially zero center-velocity projection toward the target.
  For this coherent powered lateral-near-miss class, preserve the bounded
  posterior S-shape but test release on recovered full-direction alignment or
  target-ray closure rather than small lateral slip alone. Avoid more scalar
  carrier relief, proximity-only scheduling, direct course-angle replacement,
  or isolated phase/half-cycle changes. This implication does not apply to a
  weak wake or wrong-side initial release, and is falsified by degraded early
  progress, a short-wake curl, materially higher posterior clamp/load
  residence, failure to beat `2.187L`, or an unchanged powered lower exit.
