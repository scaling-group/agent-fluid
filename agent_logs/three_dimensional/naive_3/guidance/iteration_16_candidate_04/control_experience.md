...
- Distinguish a useful terminal selector from an effective terminal action.
  ...
  however, the signed angle from target ray to translational course never
  changes corrective sign inside `3L` at speeds above `0.2U` and has median
  magnitude `1.525--1.544 rad`, while raw yaw remains beat-locked to anterior
  joint velocity. This supports testing a distance-localized geometric course
  redirect as a distinct actuator request, not global sideslip damping (which
  previously caused an upper exit at `12.150L`). The implication applies only
  to powered lateral near misses with coherent wakes and persistent target-ray
  course mismatch; it is falsified by degraded far-field progress, a tight
  curl, higher limit/load residence, a minimum no better than `2.385L`, or an
  unchanged lower-exit class.
