...
- Treat instantaneous yaw during this powered near miss as beat-scale response,
  not course response. A posterior-wave brake selected by signed target error
  and raw heading rate improved the alignment-gated carrier's minimum from
  `2.443L` to `2.385L`, but mean distance changed only from `8.443L` to
  `8.436L` and the fish retained the coherent powered lower exit. Two inherited
  completed tests show that its raw-yaw gate is not a useful foundation for
  finer posterior allocation: reallocating removed wave authority into a
  bounded counterbend returned to `2.444/8.446L`, while relaxing only the
  posterior quadrature lag regressed to `2.633/8.462L`; both kept the same
  termination. Across the four current samples inside `3L`, heading rate is
  anticorrelated with anterior joint rate at `|r|=0.992--0.998`, with fitted
  slopes `-0.40` to `-0.46`, and the target-error-growing sign occupies about
  half the samples. Thus avoid another raw-yaw-gated brake, counterbend,
  phase-lag edit, or threshold conjunction. A genuinely distinct course test
  must first remove the repeatable gait-synchronous response (for example by a
  bounded joint-state-correlated residual) before damping target steering.
  This implication applies to coherent approaches with the same strong
  heading-rate/joint-rate coupling; it is falsified by a later capture or
  useful termination from raw-yaw-gated posterior allocation, by lost cruise
  after dephasing, or by another gait whose yaw is not phase-locked.
