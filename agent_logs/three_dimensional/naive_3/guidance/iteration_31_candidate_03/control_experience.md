...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  test, not proof that low activity caused the miss. A future wave-recovery
  mechanism should leave the evidenced steering equilibria and far carrier
  fixed, engage only from measured low joint phase-plane activity after a full
  target crossing, and be rejected if it changes the first recovery, produces
  a one-sided rather than traveling bend, raises clamp/load residence, or fails
  to improve wake coherence and near-target statistics.
