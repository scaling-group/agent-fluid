...
- Separate first-pass shaping from post-miss recovery. On the same coherent
  ...
  The assigned-parent sequence shows why that distinction matters:
  ...
  Thus preserve the sampled phase mechanism as a bounded inbound scaffold, but
  do not retune its lag amplitude or treat its scalar gain as recovery. Also do
  not repeat anterior-only damping or add another recovery curvature split. If
  post-overshoot propulsion is tested again, require body-frame behind and
  receding geometry to select dissipation of the complete measured traveling
  rhythm while retaining mean steering, and demand capture, a returning pass,
  or a useful termination change. This applies only after a coherent powered
  first pass; it is falsified by altered inbound progress, greater limit/load
  residence, loss of reorientation, or another powered lower exit.
