...
- Distinguish a useful terminal selector from an effective terminal action.
  ...
  course-response estimate. Avoid another scalar brake edit, joint-phase-only
  bias, or equilibrium redirect; the distinct remaining actuator test is to
  reallocate the posterior traveling-wave authority discarded by the brake
  into a bounded counterstroke during the measured error-growing half-cycle.
  This applies only to the same powered lateral-near-miss topology and is
  falsified by lost far-field progress or wake coherence, increased posterior
  limit/load residence, a minimum no better than `2.385L`, or an unchanged
  lower-exit trajectory after reallocation.
