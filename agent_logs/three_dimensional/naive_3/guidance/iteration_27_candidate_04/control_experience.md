...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  response gates justified only by lower saturation. The following quartet
  confirms the bounded-turn branch and closes the posterior-counterbend
  alternative. A target-behind, nonclosing course-response curvature reserve
  is the first mechanism to materially shrink the horizon orbit: relative to
  the response-held C-turn it improves minimum/mean/final distance from
  `2.377/4.458/4.163L` to `1.314/4.056/3.077L`, spends about `15.9T` inside
  `2L`, and reduces anterior clamp residence from about `0.727` to `0.249`
  with comparable posterior residence and no larger high-percentile planar
  load or yaw moment. The assigned parent's yaw-selected posterior
  counterbend reaches only `2.249/4.355L`, never enters `2L`, and restores
  anterior clamp residence to about `0.730`; do not use yaw-triggered wave
  release or posterior counterbend as the next repair. Preserve the
  course-response curvature scaffold. Its falsification boundary is terminal
  rather than broad-loop formation: at the `1.314L` pass speed is still
  `0.652U`, target-ray/course error is `1.672 rad`, yaw is nearly zero, and the
  reserve releases near an earlier `1.616L` lateral crossing. The completed
  terminal course hold partially validates that diagnosis: it improves
  minimum/final distance to `1.241/2.082L` and retains about `15.0T` inside
  `2L`, but worsens mean distance from `4.056L` to `4.157L`, cuts residence
  inside `1.5L` from about `5.38T` to `2.87T`, raises anterior clamp residence
  from `0.249` to `0.329`, and perturbs the first pass from `2.377L` to
  `2.466L`. Preserve it only as a localized scaffold; do not broaden its
  radius, strengthen static curvature, or infer success from the closer final
  state alone. At its `1.241L` pass the fish still moves at `0.669U`, recedes
  slightly (`course_dot=-0.121`), and turns at only `-0.129 rad/T` while the
  target line rotates about `0.535 rad/T` in the required direction; an earlier
  `1.643L` pass instead has `+2.359 rad/T` wrong-way yaw. This separates
  missing response from over-response. If extending the branch, test a bounded
  terminal mechanism that compares target-line rotation with measured yaw and
  can act in either direction; reject it if the coherent first pass changes,
  clamp/load residence rises, or capture, useful near-target residence, mean
  distance, and orbit radius do not improve.
