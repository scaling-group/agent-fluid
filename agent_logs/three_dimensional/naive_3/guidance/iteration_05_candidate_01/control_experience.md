...
- Once restrained target-relative curvature establishes a coherent approach,
  ...
  its samples. For this powered lateral-near-miss class, avoid more scalar
  carrier relief, proximity-only scheduling, or isolated half-cycle effort and
  do not treat repair of the acute-bearing alias as sufficient. Test one
  bounded redirect only when full body-frame direction and inadequate closing
  response agree, release it on corrective turn response, and retain the
  evidenced far-field carrier. This implication does not apply to weak-wake or
  wrong-sign release failures, and is falsified by degraded early progress, a
  short-wake tight curl, unchanged lower exit, or increased actuator-limit
  residence.
