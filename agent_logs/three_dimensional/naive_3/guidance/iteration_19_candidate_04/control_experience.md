...
- Distinguish a useful terminal selector from an effective terminal action.
  ...
  trajectory. Across the sampled traces, the signed angle from target ray to
  translational course never changes corrective sign inside `3L` at speeds
  above `0.2U` and has median magnitude `1.525--1.544 rad`, while raw yaw
  remains beat-locked to anterior joint velocity. Completed evidence now shows
  that this persistent geometric signal is a useful selector but not, by
  itself, an effective inbound action: shared course curvature reached
  `2.398L`, differential course curvature `2.469L`, measured-response anterior
  half-cycle relief `2.478L`, and course-selected anterior duty asymmetry
  `2.433L`; all retained the coherent wake and powered lower exit instead of
  improving on the `2.385L` brake. Do not retune those equilibrium, half-cycle,
  or duty allocations, and do not revive global sideslip damping (which caused
  an upper exit at `12.150L`). A target-behind selector also isolates the
  post-overshoot regime, but its completed passive terminal hold supplies a
  separate negative result: despite a `2.293L` first-pass minimum and activation
  near `18.66T` at `2.375L`, anterior rhythm damping produced no return arc and
  exited low earlier at `29.76T` rather than roughly `31.2T`. Thus later tests
  should require an active, geometry-released post-overshoot reorientation and
  judge it by target return, capture, or a new useful termination class—not a
  small minimum-distance shift. This lesson applies to powered lateral near
  misses with coherent wakes and evidenced behind-and-receding geometry; it is
  falsified by a later passive-hold or inbound course-action capture, and does
  not apply to weak-propulsion or wake-collapse failures.
