...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  response mechanism is evidenced. The calibrated normalized-yaw-moment route
  is now closed: despite its `0.927` same-step correlation with heading-rate
  derivative, harmful-sign posterior residual rejection worsened minimum
  distance from `2.326L` to `2.539L` and retained the powered lower exit near
  `31.4T`. Correlation was not useful control authority here; avoid another
  moment residual unless new evidence separates a repeatable disturbance from
  the gait response and predicts a different termination mechanism.
- Preserve full-direction target-behind C-turn recovery as the first proven
  termination-class scaffold, but do not confuse horizon survival with
  capture. It changed the inherited coherent powered lower exits into repeated
  finite orbits through `100T`. In the current completed samples, plain
  geometry release gives the best first-pass minimum (`2.346L`, mean
  `4.714L`); holding the opposite-sign recovery into ahead-but-lateral states
  improves mean distance to `4.458L` but worsens the minimum to `2.377L`;
  closing-selected carrier contraction gives `2.484/4.545L`; and radial
  recovery release gives `2.532/4.780L`. Thus close another recovery-release
  gate or scalar carrier contraction as the immediate capture test while
  retaining the target-behind topology. Across the plain and response-hold
  traces, the first approach enters `4L` with only `0.18--0.19 rad` course
  error and about `0.70--0.76L` signed velocity-ray miss, but reaches minimum
  distance with `1.02--1.12 rad` course error and about `2.0--2.1L` predicted
  miss before the target passes behind. This supports testing a bounded,
  ahead-and-closing target-relative collision-corridor correction before
  recovery rather than raw sideslip or another post-pass edit. Applicability is
  limited to coherent self-propelled approaches with finite closure; reject
  the signal if it changes far cruise, reproduces the earlier wrong-sign
  sideslip exit, increases clamp/load residence, loses horizon-safe recovery,
  or fails to beat the `2.346L` first-pass boundary.
