...
- Validate posterior steering sign as an actuator-topology choice before
  ...
  split.
- Treat instantaneous yaw under this carrier as a beat selector, not as a slow
  course-response estimate. The sampled measured-yaw posterior brake improved
  the alignment-gated carrier from `2.443/8.443L` minimum/mean distance to
  `2.385/8.436L`, but the assigned parent's approach-localized attempt to
  cancel inferred gait yaw inside the mean-curvature loop worsened those
  values to `2.541/8.449L`; an approach-localized response-released posterior
  equilibrium S-bend likewise reached only `2.536L`. All remained stable,
  self-propelled, and ended in the same powered lower exit, while inherited
  traces put the yaw/anterior-joint-rate phase lock inside `3L` at
  `|r|=0.992--0.998`. The assigned parent's subsequent closure-gated
  anterior-equilibrium burst also failed: despite using approach, full lateral
  direction, lost closure, and event-local command reserve, it worsened the
  minimum from `2.385L` to `2.569L`, kept final distance nearly unchanged
  (`9.195L` versus `9.190L`), and retained the same coherent powered lower
  exit. A sampled course-selected differential bend reached only `2.469L`.
  Do not add another yaw threshold, joint-rate yaw cancellation, static
  anterior or posterior equilibrium residual, course-error bend, or
  short-window target-ray lead to this topology. The surviving distinction is
  between mean shape and rhythmic authority: at the `2.385L` minimum the
  target was about `1.38 rad` lateral, speed remained `0.705U`, error-growing
  yaw was `2.19 rad/T`, anterior rate was about `-4.46 rad/T`, and anterior
  acceleration still had event-local reserve. A genuinely different test may
  therefore reuse the evidenced target/yaw selector to damp only the
  error-growing anterior beat half-cycle while leaving mean curvature,
  corrective rhythm, and the posterior brake unchanged. This implication is
  limited to a coherent powered near miss with strong yaw/joint-rate phase lock;
  it is falsified by altered far-field progress, damping of corrective
  half-cycles, a tight curl, lost wake coherence or speed, increased
  limit/load residence, or no useful improvement over the powered lower exit.
