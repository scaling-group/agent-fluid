...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  energy rather than retuning their gains or gates. More generally, a
  velocity-odd residual is structurally absent at the observed parked state,
  so frozen-trace positive work does not show that it can escape the coupled
  equilibrium. A later mechanism should preserve the exact symmetric
  anterior carrier and produce bounded action from an unresolved measured
  body-response error even when joint velocity is zero, then release when that
  response appears; falsify it on a changed first return, wrong response sign,
  renewed low-activity bend, saturation/load growth, or failure to improve
  closest approach, near-target residence, and final return together.
