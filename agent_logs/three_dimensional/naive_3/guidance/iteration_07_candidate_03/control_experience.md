...
- Once the restrained `7 deg` mean-curvature carrier produces a coherent
  ...
  do not keep varying equilibrium curvature, drive gates, or the already
  tested beat-shape channels. The alignment-gated parent reached `2.443L`,
  ...
  Unrestricted course-angle feedback in the primary mean-curvature map is a
  sharper negative boundary: a speed-qualified `0.35` sideslip fraction
  reversed the useful route, reached only `12.150L`, and exited the upper
  boundary at `9.295T`. If course feedback is revisited on this carrier,
  isolate a sign-restricted residual in the less-saturated posterior channel
  and require the parent route to survive before interpreting closest-distance
  changes. This implication applies to the finite, coherent direct-uniform
  `7 deg` carrier; it does not reject differently calibrated signals after a
  new propulsion topology, and a capture or meaningfully different finite
  route would falsify the plateau.
