...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  that can stabilize a noncapturing quiescent C-bend. A distinct next test may
  use normalized joint state to release a completed bend while preserving the
  posterior traveling-wave formulation; this boundary is falsified if joint
  motion remains active yet first-return geometry, near-target residence, and
  distance statistics still regress, or if release increases clamp/load
  residence or disrupts wake coherence.
