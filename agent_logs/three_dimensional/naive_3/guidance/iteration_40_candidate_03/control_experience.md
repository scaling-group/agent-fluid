...
- Diagnose the repeated return as a tangential orbital equilibrium when the
  wake remains coherent, the target is behind/lateral, speed stays finite, yaw
  nearly follows target-ray rotation, and course fails to close.  The four
  current horizon samples meet that signature at their minima: speed is about
  `0.677--0.681U`, course error `1.698--1.704 rad`, course dot
  `-0.127-- -0.133`, and yaw magnitude only `0.253--0.269 rad/T`.  Their
  response-lag (`2.439L`), independent posterior-energy (`2.320L`), fixed-sign
  restart (`2.369L`), and joint-state unbend (`2.215L`) variants all retain the
  broad top-down/oblique loop; the first three also reach the closest region
  with only about `0.003--0.005 rad/T` mean absolute anterior velocity inside
  `2.5L`.  These results are weaker than the inherited symmetric
  phase-balanced carrier's `1.175L` minimum and `2.35T` residence inside
  `1.25L`, while the inherited optimizer log's separate `1.444L` near miss
  confirms that terminal entry is attainable.  Preserve the symmetric
  anterior energy mechanism and stop adding posterior residual, posterior
  energy, fixed-sign restart, or static unbend roles.  When this orbital
  signature recurs, test a genuinely nonsteady, response-released steering
  role whose measured yaw deficit vanishes as the requested turn appears;
  falsify that direction if it changes the first return, parks the joints,
  broadens the loop/wake, raises limit residence, or fails to improve both
  terminal residence and closest approach.  This implication does not apply
  when low speed, incoherent propulsion, or a closing course—not insufficient
  yaw response—explains the miss.
- Do not use instantaneous self-wake force as a terminal steering selector in
  this direct-uniform still-water orbit merely because a signed impulse can be
  formed.  The newly sampled harmful-impulse posterior damping remains finite
  with a coherent top-down and oblique wake, but reaches only
  `2.294/3.877/3.471L` minimum/mean/final distance and arrives at its minimum
  in the same common C-bend (`q≈(-0.359,-0.377) rad`) with joint speeds near
  zero and speed about `0.680U`.  Its result lies inside the same narrow failure
  family as the sampled response-lag, independent-posterior-energy, and
  joint-state-unbend variants (`2.439`, `2.320`, and `2.215L` minima), while
  the assigned parent's score-only inherited near miss (`1.724L`, horizon,
  `3.610L` final) supplies no force-event evidence and must not be treated as a
  causal counterexample.  Preserve the symmetric anterior energy scaffold and
  close more force-sign or damping transforms for this quiescent topology.
  Revisit a force or relative-flow residual only when repeated resolved events
  visibly precede yaw loss and the feedback both rejects those events and
  preserves the first return; this lesson does not exclude such evidence in a
  genuinely imposed or cylinder-generated wake.
