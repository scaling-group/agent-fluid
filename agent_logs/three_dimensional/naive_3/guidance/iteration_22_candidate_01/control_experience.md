...
- Preserve posterior phase modulation only as a provisional approach baseline,
  ...
  distance to `2.499/9.291L`. Later completed evidence also closes both
  proposed continuations of the fixed-proximity phase result. A closure-cone
  selector that began phase action during the forming miss regressed to about
  `2.390L`, and the assigned parent's lateral-geometry/closing-response phase
  selector regressed further to `2.684L`; directly attenuating the
  course-error-reinforcing posterior half-cycle reached only `2.738L`. All
  retained coherent propulsion and the powered lower exit. Preserve the
  fixed-proximity phase case only as a comparison scaffold: avoid broadening
  its selector, another posterior half-cycle attenuation, more command relief,
  equilibrium bursts, or post-overshoot damping. A later test must change the
  feedback/action semantics and require capture, a target-return leg, a useful
  termination class, or material improvement below `2.326L` with retained
  mean distance. This implication applies only to coherent lateral near misses
  with stable course mismatch; it is falsified by a semantic improvement from
  earlier phase activation or half-cycle attenuation, or does not apply when
  propulsion/wake coherence rather than terminal course control is the fault.
