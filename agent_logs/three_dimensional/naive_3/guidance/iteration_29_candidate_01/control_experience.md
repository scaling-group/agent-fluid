...
- Frozen-trace locality is not sufficient around this course-hold scaffold's
  ...
  based only on frozen action deltas. If a distinct dynamic actuator mechanism
  is revisited, make it effectively absent before the target passes behind,
  then reject it on any change to first-recovery geometry or yaw, loss of wake
  coherence, higher clamp/load residence, or failure to improve near-target
  residence and distance statistics. This boundary applies to the finite-speed
  coherent horizon loop; it does not establish the same selector for stalled,
  incoherent, or already capturing trajectories.
