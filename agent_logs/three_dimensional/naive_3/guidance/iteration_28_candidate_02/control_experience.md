...
- After a full-direction C-turn converts the powered lower exit into coherent
  ...
  Later terminal samples sharpen that boundary. Extending the reserve with an
  ahead/lateral bridge reached only `1.371/4.121/3.303L` and spent `5.616T`
  inside `2L`. A broader course-selected hold did move a late return to
  `1.241L` and finish at `2.082L`, but worsened mean distance to `4.158L`,
  entered `1.25L` for only `0.473T` near `97T`, and raised anterior clamp
  residence from `0.249` to `0.329`. Adding a response-released anterior
  equilibrium burst to that hold then erased all sub-`2L` residence and
  regressed minimum distance to `2.125L`, despite a coherent powered wake and
  lower clamp residence. Thus the static terminal-persistence and anterior-
  burst branches are closed: do not retune their distance envelopes, add more
  held curvature, or interpret lower command residence as better navigation.
  The inherited posterior-residual proposal remains an unvalidated topology,
  not a positive result; if tested, it must preserve the carrier and static
  scaffold, act only on finite-speed tangential/receding course, release on
  radial closure, and earn capture, a pass below `1.241L`, longer near-target
  residence, or clear inward recovery without added posterior clamps or loads.
  This lesson applies to coherent powered horizon orbits with persistent
  target-ray/course mismatch; it is falsified if a later anterior burst yields
  semantic improvement, and it does not authorize more curvature or a dynamic
  residual for stalled, unstable, nonpropulsive, or already radially closing
  trajectories.
