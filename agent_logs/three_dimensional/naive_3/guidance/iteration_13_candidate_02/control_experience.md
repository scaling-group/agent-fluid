...
- Treat the current family of terminal posterior-only interventions as a
  concrete negative result, not evidence that another posterior threshold will
  capture.  Under the same coherent alignment-gated cruise, the assigned
  parent's posterior half-cycle counterbend reached `2.512L`; sampled
  full-direction gating and a response-released posterior S-bend reached
  `2.494L` and `2.536L`.  A yaw-selected posterior half-cycle attenuation was
  locally best at `2.385L` with mean distance `8.436L`, but at its minimum the
  fish still moved about `0.705U` with essentially zero closure, `1.379 rad`
  full body-frame direction error, and `2.185 rad/T` heading response before
  the same powered lower exit.  Together with inherited step-10--12 results
  (`2.484L`, `2.429L`, and lower exits), this is three consecutive completed
  generations without a semantic improvement.  Near `3L`, anterior
  acceleration was already clamped for roughly `0.73--0.75` of samples while
  posterior acceleration was not clamped, so neither extra raw effort nor more
  posterior equilibrium/gating is the supported discriminator.  Preserve the
  lagged posterior wave as the thrust carrier and next test a different joint
  role, such as bounded response-released anterior steering; reject that role
  if cruise or wake coherence degrades, joint-limit residence rises, the fish
  curls, or the lower-exit topology persists without material distance
  improvement.  This implication is limited to the self-propelled,
  wake-coherent lateral near miss and does not apply to weak propulsion or a
  wrong-sign initial release.
