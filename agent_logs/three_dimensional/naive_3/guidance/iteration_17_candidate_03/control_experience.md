...
- Treat instantaneous yaw under this carrier as a beat selector, not as a slow
  ...
  `|r|=0.992--0.998`. Do not add another yaw threshold, joint-rate yaw
  cancellation, posterior static residual, or short-window target-ray lead to
  this topology. At the `2.385L` minimum, the target was still about `1.38 rad`
  lateral, closure was only `0.014 L/T`, speed remained `0.705U`, and anterior
  acceleration was about `7.71 rad/T^2` under a `28 rad/T^2` command bound;
  moreover, inherited median closure fell from `0.586--0.606 L/T` at `3--4L`
  to `0.131--0.163 L/T` inside `2.7L`. A distinct evidence-backed test is thus
  bounded anterior equilibrium authority gated jointly by approach, full
  body-frame lateral error, and lost closure while retaining raw yaw only for
  posterior half-cycle selection. This implication applies only to a coherent,
  powered approach with this closure-stall signature and available event-local
  anterior reserve; it is falsified by changed far-field progress, a tight
  curl, lost wake coherence or speed, increased limit/load residence, or no
  useful improvement over the powered lower-exit trajectory.
