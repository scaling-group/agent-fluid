...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  terminal attractor will remain local. The completed posterior
  turn-response combination now closes another apparently compatible branch:
  adding a bounded course-error-to-posterior-lag residual to the `1.175L`
  phase-balanced carrier regresses to `2.439/3.861/3.310L`
  minimum/mean/final distance, never enters `2L`, and reaches its closest
  state with only about `0.00053/0.00179 rad/T` anterior/posterior joint
  velocity. Its coherent broad wake matches the assigned parent's static
  equilibrium-unbend failure (`2.215/3.859/3.416L`), whose closest state is
  likewise translating near `0.677U` with both joints almost stationary.
  A paired counterphase burst on the phase-balanced carrier is also closed: it
  reaches `1.192/4.044/3.222L`, shortens residence inside `1.25L` from about
  `2.35T` to `1.24T`, and lowers mean two-joint absolute velocity inside `2L`
  from about `0.567` to `0.462 rad/T`. Therefore a posterior phase residual
  that is useful on a different carrier is not compositional evidence, and an
  equal-and-opposite burst does not establish wave propagation; do not retune
  either response gate. The next two completed posterior-role tests further
  delimit the mechanism. Velocity-odd energy about the moving lag target
  reaches only `2.320/3.868/3.443L` minimum/mean/final distance, while the
  sibling formulation about the tail mean reaches
  `2.173/3.868/3.547L`; both retain the coherent broad orbit and arrive at
  their minima with anterior/posterior speed below about
  `0.0027/0.0027 rad/T`. At the former minimum its posterior low-activity
  detector is nearly fully on, yet the terminal response weight is only
  `0.0243`, leaving roughly `0.002 rad/T^2` of incremental posterior action.
  Thus terminal-local posterior phase-plane energy, its reference choice, and
  its scalar acceleration are closed: detecting deficient rhythm is
  insufficient when a proximity shell withholds authority after the orbit has
  already broadened. A later posterior-energy test must preserve the symmetric
  anterior carrier and mean bend but key its bounded two-sided action to
  measured nonclosing course response before the tight terminal shell; reject
  it if an active first pass changes, posterior motion stays quiet, or closest
  approach, near-target residence, and final distance fail to beat the
  `1.175L` scaffold.
