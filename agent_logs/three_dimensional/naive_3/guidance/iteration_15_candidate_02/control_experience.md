...
- Use instantaneous measured yaw as a terminal half-cycle selector, not as an
  estimate of slow course response, under this carrier. The sampled
  ...
  speed `0.705U`. Later response-selected posterior reallocations did not turn
  that selector into navigation: an equilibrium residual returned about
  `2.444L`, an authority reallocation reached `2.375L` but retained the powered
  lower exit, and the assigned parent's full counterstroke worsened
  minimum/mean distance to `2.585/8.474L` from the brake's `2.385/8.436L` while
  retaining a coherent wake, the lower exit, and similar anterior/posterior
  clamp fractions (`0.745/0.349` versus `0.747/0.356`). Thus avoid more
  posterior brake, redirect, reversal-fraction, or response-threshold edits in
  this topology. The next discriminating actuator test is to use the evidenced
  response interval to bound or phase-modulate the anterior joint whose
  velocity actually carries the yaw, while leaving cruise and corrective
  half-cycles unchanged. This implication applies to coherent lateral near
  misses with the same strong yaw/anterior-rate phase lock; it is falsified by
  a posterior-only capture or useful new termination class, by a changed phase
  relation under another carrier, or if anterior modulation degrades cruise,
  wake coherence, clamp residence, or the terminal trajectory.
