...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  on this scaffold. A distinct next test may keep authority and propulsion
  fixed while resolving only the rear-crossing turn direction from persistent
  target-ray/course sign. Reject that selector if lateral side and course sign
  do not actually disagree near the encounter, or if first-return geometry,
  wake coherence, loads, or near-target statistics worsen.
