...
- Validate posterior steering sign as an actuator-topology choice before
  ...
  split.
- Prefer measured body response over a joint-wave phase proxy when selecting
  a terminal half-cycle intervention. The assigned parent's approach-localized
  brake used signed full target direction and measured heading rate to weaken
  only wrong-way posterior-wave intervals; it improved minimum distance from
  the inherited cross-track/closure hold's `2.429L` to `2.385L` and also beat
  a joint-wave-phase posterior counterbend (`2.512L`), a full-direction gate
  (`2.494L`), and a response-gated equilibrium S-bend (`2.536L`). At its
  minimum, however, target-direction error remained `1.38 rad`, wrong-way yaw
  remained `2.19 rad/T`, closure was only `0.014 L/T`, and speed was `0.705U`;
  all current samples retained the coherent wake and powered lower exit.
  Therefore this evidence supports measured response as the selector, not
  terminal braking as a completed solution: avoid another phase-only
  counterbend or scalar brake-strength edit, and next test whether the same
  response gate can reallocate removed posterior-wave authority into bounded
  corrective actuation. This implication is limited to coherent lateral near
  misses with repeatable wrong-way yaw and is falsified if a phase-selected
  policy captures, if measured-response selection degrades cruise or wake
  coherence, or if the reallocation leaves the minimum and lower-exit topology
