...
- Separate first-pass shaping from post-miss recovery. On the same coherent
  response-selected brake, posterior phase-lag modulation improved minimum
  distance from `2.385L` to `2.326L`, reduced the recorded trajectory mean
  from `6.772L` to `6.725L`, and delayed the near-target target-behind crossing
  from `17.963T` to `18.650T`, without materially changing the already high
  anterior/posterior clamp residence (`0.749/0.355` versus `0.747/0.356`). It
  still receded from `2.331L` at `18T` to `4.777L` at `24T` and exited low.
  The inherited sequence shows why that distinction matters:
  course-selected differential equilibrium bending reached `2.469L`; an
  anterior-only target-behind damping hold reached `2.293L` but exited earlier
  while post-miss speed increased; and a target-behind differential burst with
  posterior-wave attenuation reached `2.499L`; all retained `left_domain`.
  The assigned parent's completed whole-wave damping test now closes the
  remaining passive branch: both joint rates were near zero by `21T`, yet the
  fish receded from `3.47L` at `21T` to `5.52L` at `24T`, regressed to a
  `2.484L` minimum, and exited low earlier at `29.21T`. The separately inherited
  closure-cone phase selector also regressed the best phase scaffold from
  `2.326L` to `2.390L` without changing termination. Thus preserve the sampled
  phase mechanism inbound, but do not retune its activation cone, add another
  recovery curvature split, or equate joint-motion damping with translational
  braking. The next distinct recovery class must actively oppose the measured
  post-miss momentum while behind-and-receding body geometry persists; a
  bounded posterior wave-direction reversal is one falsifiable test, not an
  established lesson. Demand capture, a returning pass, or a useful termination
  change rather than another scalar shift. This applies only after a coherent
  powered first pass and is falsified by altered inbound progress, greater
  limit/load residence, a one-sided/collapsed wake, failure to reverse recession,
  or another powered lower exit.
