...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  is fully behind contracts the response-held orbit from `2.377L` minimum and
  `4.458L` mean distance to `1.314/4.056L`, improves final distance from
  `4.163L` to `3.077L`, and lowers anterior clamp residence from about `0.727`
  to `0.249` without larger sampled load tails. Preserve that carrier,
  redirect, and response reserve. However, two attempts to persist the same
  equilibrium curvature across terminal target-side geometry do not complete
  capture: a broad course hold reaches `1.241/4.158/2.082L`
  minimum/mean/final distance, while a narrow barely-ahead bridge regresses to
  `1.371/4.121/3.303L`; both retain coherent powered loops through the horizon.
  Close further static-curvature persistence, distance/gate retuning,
  contraction/release, phase/duty reallocation, and response logic justified
  only by lower saturation. A next terminal test should change the regulated
  response (for example, bounded target-ray-rate versus yaw-rate matching),
  stay negligible at the `2.35L` first pass, and be rejected if wake/load
  margins degrade or the `1.2--1.7L` noncapturing orbit survives.
