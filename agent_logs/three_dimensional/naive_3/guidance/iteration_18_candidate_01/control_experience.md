...
- Validate posterior steering sign as an actuator-topology choice before
  ...
  split.
- Distinguish a useful terminal selector, a smaller angular error, and an
  effective capture action. On the coherent alignment-gated carrier, the
  measured-yaw-selected posterior brake remains the strongest sampled terminal
  action at `2.385/8.436L` minimum/mean distance; joint-phase counterbending,
  response-released equilibrium S-bending, reallocated posterior
  counterstrokes, and a fitted yaw/joint-rate residual all retained the powered
  lower exit at `2.406--2.541L`. The assigned parent's later geometric-course
  redirect also failed this boundary: differentially adding as much as `6 deg`
  to anterior equilibrium while counterbending the posterior reduced the
  instantaneous course error at minimum from `1.152` to `1.119 rad`, but
  worsened the minimum from `2.385` to `2.469L`, left mean distance essentially
  unchanged (`8.436` versus `8.439L`), and again exited low with the same long
  alternating 3D wake. Inside `3L`, its target-ray-to-course error never changed
  sign and averaged `1.542 rad`; anterior acceleration was already clamped for
  about `0.736` of samples while posterior acceleration was unclamped. Thus a
  modest heading/course-angle change is not evidence of better interception,
  and another additive equilibrium redirect or its gain/threshold retune
  should be avoided in this topology. The evidence-backed next actuator test
  is to expose anterior steering reserve by selectively withholding its
  oscillatory component during measured wrong-way response while preserving
  bounded mean-curvature steering and the propulsive posterior wave. This
  implication applies only to powered lateral near misses with coherent wakes,
  persistent one-sign course mismatch, and anterior—not posterior—command
  saturation; it is falsified by degraded far-field progress, a short-wake
  curl, higher limit/load residence, a minimum no better than `2.385L`, or the
  same lower-exit class.
