...
- Separate a useful terminal selector from an effective terminal action. On
  ...
  half-cycle selector but not as a slow course-error estimate. Avoid another
  scalar brake threshold, symmetric drive relief, phase-only counterbend, or
  persistent equilibrium redirect; the distinct actuator test is to
  reallocate rather than discard posterior authority during the measured
  wrong-way half-cycle. This applies only to the same powered lateral-near-miss
  topology and is falsified by lost far-field progress or wake coherence,
  increased limit residence, a minimum no better than `2.385L`, or an
