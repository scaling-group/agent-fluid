...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  yaw-rate lag modulation is supported. Low anterior phase-plane activity is
  now a diagnostic, not an established remedy. It separated the `1.241L`
  course hold (median `0.699` inside `3L`) from three earlier descendants
  (`0.003--0.011`), but the completed course-signed low-activity restart still
  regressed to `2.369/3.868/3.455L` minimum/mean/final distance. At its minimum
  both joints remained nearly stationary in a common negative C-bend and its
  commands were near zero, so the additive nudge neither restored a traveling
  wave nor preserved the useful close return. Joint-state equilibrium release
  similarly reached only `2.215/3.859/3.416L`. Both retained coherent wakes
  and the `100T` horizon, and the restart's apparently local frozen-trace edit
  still displaced the coupled trajectory; therefore do not infer CFD
  trajectory locality from replay deltas, treat low activity as causal, or
  retune the restart sign, threshold, authority, bend-release equilibrium, or
  rear selector. Across the current four sampled traces, the useful `1.241L`
  course hold spends only about `1.4%` of its inside-`3L` samples below `0.12`
  normalized anterior phase-plane activity (median `0.699`), whereas the
  rear-selector, signed-restart, and equilibrium-release descendants spend
  about `92.0%`, `88.4%`, and `85.4%` there. This makes low activity a robust
  topology classifier but still not a causal trigger: the fixed-sign restart
  and equilibrium shift did not restore the useful return. A distinct later
  gait-energy test must therefore leave the evidenced steering equilibria,
  far carrier, and posterior traveling-wave law fixed, act only along an
  already measured joint-velocity half-cycle, and be identically zero at an
  exactly parked bend. It applies only under the existing terminal response
  geometry with finite motion and command reserve; reject it if the quiet bend
  persists, the first recovery changes, the anterior-to-posterior wave or wake
  coherence degrades, clamp/load residence rises, or near-target and final
