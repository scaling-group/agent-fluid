...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  only about `0.13 rad/T` while its joint-speed norm remains `0.281`. Three
  sampled descendants now expose a shared carrier-collapse failure rather than
  three independent near misses. The response-released anterior-equilibrium
  burst reaches `2.125/3.601L` minimum/final distance, the assigned parent's
  target-behind turn-rate phase residual reaches `2.137/3.530L`, and the
  prefilled rear-crossing direction selector reaches `2.366/3.502L`; all
  survive to `100T`, but after `35T` their mean joint-speed norms fall to only
  `0.011`, `0.009`, and `0.007` while body speed remains about `0.67U`. Their
  oblique sheets likewise change from an organized compact wake at `24T` to a
  faint wake behind an almost fixed common C-bend by `52--100T`. Close the
  static anterior-burst, target-behind yaw-rate phase residual, and continuous
  rear-crossing direction-blend branches in their tested forms; their lower
  mean distance (`3.875--3.901L` versus the active scaffold's `4.157L`) is a
  coasting-orbit artifact, not useful terminal control. A later response
  mechanism must preserve the active limit-cycle equilibrium and traveling
  wake. If a state-triggered restart is tested, gate it on measured collapse
  of normalized joint phase plus persistent body-frame nonclosure, and release
  it as joint motion or useful yaw returns; reject it if first-pass geometry
  changes, the wake still fades, clamp/load residence rises, or minimum/final
  distance and repeated-return topology do not improve.
