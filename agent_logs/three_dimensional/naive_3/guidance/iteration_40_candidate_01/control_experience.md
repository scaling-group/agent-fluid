...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  terminal attractor will remain local. The completed assigned-parent
  posterior-thrust allocation now closes scalar caudal drive relief on that
  scaffold as well: attenuating only the oscillatory posterior wave under
  close, poor-course geometry regresses the phase-balanced carrier from
  `1.175/4.041/3.243L` to `1.444/4.032/3.330L`
  minimum/mean/final distance, eliminates its `2.35T` residence inside
  `1.25L`, and lowers near-target speed by only about `0.02U`. Do not retune
  that wave floor or course threshold; the tangential miss is not corrected by
  removing thrust. Two other completed inherited branches sharpen what may
  still transfer. A stronger radial anterior energy law reaches only
  `1.366/4.041/3.047L`, so preserve the exact bounded `1.175L`
  phase-balanced regulator rather than increasing its activity target or
  feedback gain. A response-gated posterior phase shift on the unregulated
  course hold reaches `1.276/4.085/2.881L`: it lengthens residence inside
  `1.5L` from `2.87T` to `5.69T` and retains active joints, but worsens
  the course hold's `1.241/2.082L` closest/final distances. That provisional
  posterior steering role is now closed by the completed combination: adding
  the response-gated lag shift to the `1.175L` phase-balanced carrier regresses
  to `2.439/3.861/3.310L`, never enters `2L`, and reaches its minimum at only
  about `-0.00053/+0.00179 rad/T` anterior/posterior velocity. Two-sided
  posterior phase-plane energy does not repair the coupled attractor either.
  The inherited result reaches only `2.173/3.868/3.547L`, and an independently
  sampled variant reaches `2.320/3.868/3.443L`; both preserve the coherent
  broad loop and approach their minima with joint speeds below
  `0.003 rad/T`. Close terminal posterior phase, attenuation, and anti-parking
  energy rather than retuning their gains or gates. The new sampled
  harmful-impulse damping result closes the opposite force-work sign too: it
  reaches only `2.294/3.877/3.471L`, parks near the same
  `(-0.359,-0.377) rad` C-bend, and has joint speeds below `0.002 rad/T` at its
  minimum. Together with the earlier positive-work force selector's `2.391L`
  minimum, this shows that short-horizon force/course correlation does not
  identify a useful posterior energy sign; avoid retuning either force gate or
  damping scale. More generally, a velocity-odd residual is structurally
  absent at the parked state, so frozen-trace work sign does not show that it
  can escape the coupled equilibrium. The assigned parent's direct anterior
  response test now also falsifies the proposed zero-velocity turn-deficit
  remedy: despite remaining actionable at a parked joint and improving minimum
  distance to `2.077L`, it worsens mean/final distance to `3.890/3.512L` and
  reaches the same broad loop with course error about `1.695 rad`, finite speed
  near `0.68U`, and already useful yaw near `-0.250 rad/T`. More yaw, nonzero
  parked-state curvature action, or an instantaneous hydrodynamic work label
  is therefore not evidence of inward translational-course response. Preserve
  the exact symmetric anterior carrier, but require a later mechanism to test
  a genuinely different axial or lateral impulse role; falsify it on a changed
  first return, absent course response, renewed low-activity bend,
  saturation/load growth, or failure to improve closest approach, near-target
  residence, and final return together.
