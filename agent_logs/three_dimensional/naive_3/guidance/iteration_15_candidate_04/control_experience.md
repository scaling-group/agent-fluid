...
- Validate posterior steering sign as an actuator-topology choice before
  ...
  split.
- Separate gait-synchronous yaw from slow course response instead of using one
  raw signal for both roles. The measured-yaw posterior brake improved minimum
  distance from the inherited cross-track/closure hold's `2.429L` to `2.385L`
  and beat a joint-phase counterbend (`2.512L`) and response-released
  equilibrium S-bend (`2.536L`), so instantaneous yaw is useful as a
  half-cycle selector. It is not an evidenced slow-course estimate: inside
  `3L`, all four sampled traces show heading rate anticorrelated with anterior
  joint velocity at `|r|=0.9925--0.9978`. At the best minimum, direction error
  remained `1.38 rad`, error-growing yaw `2.19 rad/T`, closure approximately
  zero, and speed `0.705U`. The assigned parent's later completed tests also
  falsify the previously suggested posterior reallocation: converting the
  brake's discarded authority into an opposite-sign equilibrium residual
  reached only `2.444L`, and reversing it as a response-selected counterstroke
  reached `2.585L`; both retained the coherent wake and powered lower exit.
  Avoid another posterior reallocation, joint-phase proxy, or scalar brake
  edit in this topology. The distinct remaining test is to residualize raw yaw
  against observed joint rate before it enters the anterior mean-curvature
  course loop, while retaining raw yaw for the posterior selector. This
  implication applies only when propulsion is coherent and yaw/joint-rate
  phase lock is repeatable; it is falsified if residualization changes cruise,
  loses wake coherence, or leaves both the minimum and lower-exit class
  materially unchanged, and its fitted coefficient must not be transferred to
  a carrier with different phase coupling.
