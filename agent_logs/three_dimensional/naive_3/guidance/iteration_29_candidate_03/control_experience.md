...
- Frozen-trace locality is not sufficient near the terminal-course scaffold's
  ...
  structures remain coherent. By contrast, the anterior response burst keeps
  a coherent wake but broadens the visible orbit and reaches only
  `2.125/3.601L` minimum/final distance. Thus a ray-rate mismatch is evidence
  of a late turn-response deficit only after full behind-side conditioning;
  it is not a generic near-target steering signal. A future behind-only test
  must remain negligible at the `2.466L` ahead-side recovery and be rejected
  if that pass moves, the `1.241L` minimum worsens, or the repeated powered
  orbit does not contract toward capture. The newest inherited optimizer log
  does not overturn this boundary: it remains an uncaptured horizon result at
  score `-4.481` with `1.732/2.093L` minimum/final distance. Since that log has
  no matching trajectory or wake sheet in the inherited artifact, use it to
  close claims of success but not to infer which late-turn mechanism caused
  its scalar outcome.
