...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  low-activity kick, or half-cycle gain. Symmetric activity regulation is now
  also a completed negative family rather than the next remedy: three
  velocity-parallel energy variants preserve finite coherent wakes but reach
  only `1.491--1.631L`, all remain horizon noncaptures, and finish at
  `2.811--3.654L`. The assigned-parent variant restores strong anterior motion
  (`phi_dot_1=-0.927 rad/T`) at its `1.615L` minimum yet yaws in the wrong
  direction (`+0.374 rad/T`), whereas the `1.241L` scaffold has weaker but
  useful-sign yaw (`-0.129 rad/T`). Low phase-plane activity can diagnose a
  quiet failed bend, but it is not itself a terminal control objective; close
  the energy-floor and two-sided velocity-gain branch. A distinct response
  test may instead compare the normalized inertial target-ray rate
  `-speed*course_cross/distance` with measured yaw: at the scaffold's best miss
  these are about `-0.535` and `-0.129 rad/T`, respectively. Apply this only
  when the course-hold return, finite speed, coherent wake, and command reserve
  coexist; reject it if gait-scale yaw causes chatter, the ahead-side return
  moves, the residual or orbit does not shrink, a quiet bend reappears, or
  load, clamp, and distance statistics worsen.
