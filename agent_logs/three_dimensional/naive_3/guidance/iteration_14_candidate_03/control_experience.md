...
- Treat instantaneous heading rate as a beat-response selector, not a proven
  slow course-error signal, in this powered lateral near miss. A posterior-wave
  brake gated by the error-growing yaw half-cycle improved the alignment-gated
  carrier's minimum/mean distance only from about `2.443/8.443L` to
  `2.385/8.436L` and retained the same powered lower exit near `31T`; sampled
  posterior S-bend and joint-wave-phase counterbend variants were worse at
  `2.536L` and `2.512L`. In inherited trace analysis inside `3L`, heading rate
  correlates `-0.992-- -0.997` with anterior joint velocity and takes the
  error-growing sign on about half the beat, while anterior/posterior
  acceleration is clamped for about `0.747/0.356` of the strongest trace.
  Therefore do not interpret another raw-yaw gate, posterior-only residual, or
  global drive reduction as a new navigation mechanism. A useful next test
  must change actuator allocation on the selected half-cycle or replace raw
  yaw with a slower target-relative response measure, while preserving the
  coherent cruise carrier. This applies only to the self-propelled,
  wake-coherent below-target topology and is falsified by capture, a useful
  termination change, or a material distance gain from posterior-only
  response modulation; reject new allocations on altered cruise, wake
  collapse, premature coasting, a tight curl, or higher limit/load residence.
