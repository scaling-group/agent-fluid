...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  terminal-response mechanisms now establish a broader
  negative boundary. The ungated response-released anterior equilibrium burst
  worsens minimum/final distance to `2.125/3.601L`; multiplying that burst by
  target-behind geometry reaches only `1.987/3.522L`. A joint-state posterior
  stroke that trades established C-bend for wave authority reaches
  `1.622/3.799L`, still outside the strong scaffold's close return. All three
  settle into broad low-action orbits: after `40T` their mean joint-speed/action
  norms are only about `0.010/0.049`, `0.012/0.046`, and `0.021/0.121`, versus
  `1.372/15.829` for the coherent `1.241L` course-hold loop, and their late
  oblique wake structures fade while the bodies continue coasting near
  `0.67U`. Inherited logs add that a target-behind posterior acceleration
  residual can preserve first-recovery geometry yet still regress to
  `1.732/4.263/2.093L` minimum/mean/final distance, so geometric locality and
  a small frozen replay delta do not establish a useful coupled mechanism.
  Finite translation likewise does not establish continuing propulsion after
  a maneuver. Avoid more equilibrium bursts, static C-bend reallocation,
  posterior acceleration residuals, or joint-state wave-authority swaps in
  this regime. A later response mechanism should preserve both oscillator
  equilibria and the powered traveling-wave scaffold, engage only after the
  target passes behind, and require persistent joint motion and compact wake
  structure alongside improved near-target residence and distance statistics;
  reject it if the first recovery changes, gait energy fades, clamp/load
  residence rises, or the terminal orbit does not contract. This boundary is
  specific to the finite-speed horizon loop and does not prescribe behavior
  for stalled, incoherent, or already capturing trajectories.
