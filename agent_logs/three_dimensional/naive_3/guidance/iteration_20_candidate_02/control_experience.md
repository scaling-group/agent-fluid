...
- Validate posterior steering sign as an actuator-topology choice before
  ...
  split.
- Separate a useful terminal selector from both its actuator allocation and
  unintended propulsion changes. Across the current direct-uniform samples,
  target-ray/course error keeps one sign throughout the approach inside `3L`
  while the fish retains a coherent alternating 3D wake and about `0.68--0.71U`
  speed. Yet course-selected anterior duty (`2.433L`), differential equilibrium
  bending (`2.469L`), measured-response anterior relief (`2.478L`), and a
  target-behind differential burst (`2.499L`) all preserve the powered lower
  exit; target-behind damping reaches `2.293L` but worsens mean distance and
  exits earlier. These completed results close the earlier recommendation to
  expose anterior reserve by withholding its oscillation, as well as another
  duty, static C/S-bend, post-overshoot hold, burst, or threshold retune. The
  assigned parent's course-selected posterior lag modulation is the only
  sampled rhythmic change with a local closest-approach gain, improving the
  yaw-selected brake from `2.385L` to `2.326L`, but it still exits low with
  essentially unchanged clamp residence (`0.749/0.355`) and its variable lag
  also changes the norm of the posterior angle/velocity coefficients. Treat
  that as evidence for posterior phase/wave-shape authority, not for amplitude
  scaling or recovery. A clean follow-up must hold the posterior coefficient
  norm fixed while changing phase, and must earn capture, course recovery, a
  useful termination class, or a material improvement below `2.326L`; the same
  lower exit, degraded cruise wake/progress, a tight curl, or higher limit/load
  residence falsifies the phase mechanism in this powered lateral-near-miss
  topology.
