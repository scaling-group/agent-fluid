...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  commands are small. Three completed descendants close the proposed
  ...
  posterior residuals, or constant signed restart. Inherited completed evidence
  now also closes the proposed useful-half-cycle energy branch: a requested-side
  pulse and its phase-balanced duty variant reach only `1.702L` and `2.362L`
  and park the wave. In contrast, velocity-odd energy on both anterior
  half-cycles is the surviving rhythmic scaffold: it reaches
  `1.175/4.041/3.243L`, remains inside `1.25L` for about `2.35T`, preserves
  near-target mean `|phi_dot|` near `0.775/0.360 rad/T`, and lowers clamp/load
  residence relative to the `1.241L` course hold. Preserve that two-sided
  anterior activity law, but do not confuse limit-cycle amplitude with useful
  inter-joint propagation. A stronger radial activity target (`1.366L`), paired
  counterphase burst (`1.192L`), response-triggered equilibrium release
  (`1.177L` with worse `4.087L` mean distance), direct turn-rate residual
  (`2.077L`), assigned-parent posterior activity regulator
  (`2.320/3.868/3.443L`), and helpful/harmful force-selected posterior work
  (`2.391L`/`2.294L`) all fail to improve the complete near-target evidence and
  mostly recover the broad parked loop. Avoid retuning their energy, force,
  rate, or release thresholds. At the `1.175L` minimum, both joints remain
  active but measured posterior velocity (`+0.260 rad/T`) disagrees with the
  approximate velocity of its moving lag target (`-0.025 rad/T`); this supports
  separating posterior relative-phase tracking from another scalar energy
  floor. Such a test applies only to the active phase-balanced scaffold and
  must leave mean bend and route gates unchanged; a changed first return,
  parked joint, yaw-only response, broader wake/orbit, higher clamp/load
  residence, or no joint improvement in closest, residence, mean, and final
  distance falsifies the mechanism.
