...
- Treat measured yaw as a gait-half-cycle selector, not a slow course estimate,
  ...
  course-response curvature scaffold. Its falsification boundary is now
  terminal rather than broad-loop formation: at the `1.314L` pass speed is
  still `0.652U`, target-ray/course error is `1.672 rad`, yaw is nearly zero,
  and the reserve releases near an earlier `1.616L` lateral crossing. If
  extending this branch, hold bounded course-relative turn authority only in
  that close, still-tangential regime; reject it if the coherent `2.38L` first
  pass changes, clamp/load residence rises, or capture/near-target residence
  and orbit radius do not improve.
