...
- Common-envelope half-cycle redistribution has a conditional distance-
  integral benefit but has now crossed its predefined semantic falsification
  boundary. Three current executable-identical samples retain the coherent
  target-bending top-down street and compact caudal Lambda2 structures,
  capture at `18.6505--18.8815T`, and have mean distance
  `2.08855--2.09222L`; their acceleration and rate contact remain structural
  at about `60.85--61.00%`/`72.97--73.27%` and
  `11.03--11.07%`/`14.91--15.07%`. However, the inherited step-31 result has
  the exact same executable SHA and reached only `1.25093L` before exiting
  left at `9.94426L` with score `-10.84833`. Together with the earlier exact-
  policy `0.81206L` near miss and left exit, that second miss overturns the
  prior status of redistribution as the default performance candidate: do not
  replicate it again or stack another envelope, distance, alignment,
  response-release, posterior-lag, or recovery compound around it merely
  because some realizations improve the integral. Use the non-redistributed
  geometry-scheduled envelope as the robustness control; three completed
  samples captured at `18.6505--18.7550T` and `2.09340--2.09542L`. Reopen
  phase-to-envelope coupling only if a distinct mechanism produces repeated
  capture, both coherent wake rows, and a route/integral improvement outside
  that control's spread without another left-exit topology. The exact-policy
  traces also preserve an activation warning: maximum absolute body-lateral
  target fraction spans about `0.438--0.940`, so threshold activation and
  arrival order must be audited per repeat rather than credited from a dormant
  or unusually fast realization.
- Route recovery must activate before the observed divergence and must not
  ...
  demonstrably active before either replicated miss diverges and must first
  beat the geometry-scheduled control's capture, two-view wake, and
  `2.09340--2.09542L` band without increasing actuator/load contact; otherwise
  preserve that simpler non-redistributed envelope.
