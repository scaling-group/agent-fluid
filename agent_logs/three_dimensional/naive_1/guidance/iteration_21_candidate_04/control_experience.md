...
- Common half-cycle envelope redistribution now has replicated, narrowly
  positive evidence, but not demand or arrival evidence. Two direct-uniform
  evaluations of the exact same executable policy use anterior displacement
  phase to apply less of the existing common amplitude relief on the target-
  aligned half and more on the opposed half; both retain the coherent
  target-directed top-down street and compact caudal Lambda2 structures,
  capture, and improve mean scored distance to `2.088545--2.088964L` and
  score to `-0.201056-- -0.200406`, versus `2.093400--2.094046L` and
  `-0.206060-- -0.205783` for the two sampled geometry-scheduled parents.
  They arrive later (`18.8265--18.8815T` versus `18.6505--18.6835T`) and their
  acceleration/rate contacts overlap the parent, so preserve this as a
  far/middle-progress mechanism rather than claiming faster or cheaper
  swimming. Its boundary remains fragile: inherited logs report a semantic-
  equivalent near miss followed by left exit, distance-based release loses
  capture at `2.2485L`, and forward-alignment release worsens mean distance to
  `2.099614L` without demand relief. Replicate the plain common-envelope
  architecture before promotion, and avoid stacking approach-release gates,
  posterior-only allocation, velocity phase, or terminal residuals; falsify
  it on loss of capture/either wake row or failure to stay below the parent
  mean-distance band, not on arrival alone.
