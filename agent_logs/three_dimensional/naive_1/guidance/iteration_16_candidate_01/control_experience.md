...
- Body-frame misalignment scheduling is now a replicated capture-class gait-
  envelope mechanism with a sharper allocation boundary. Three executable-
  identical direct-uniform samples use absolute lateral target fraction to
  reduce the whole rhythmic envelope by at most `15%`, while preserving
  differential mean curvature, displacement-only phase, non-inverting
  response release, posterior lag, and final projection. All three retain a
  coherent target-directed top-down street and compact caudal 3D structures,
  capture at `18.6505--18.7550T`, and hold mean distance to
  `2.09340--2.09542L`. Multiplying the same envelope schedule by the
  correcting-yaw response gate also captures at `18.6615T` and `2.09362L`,
  inside the geometry-only repeat band, so that extra coupling is not
  attributable; prefer geometry alone as the sufficient gait observation.
  This remains route evidence rather than demand relief: the repeats contact
  joint-rate limits on about `10.9--11.2%`/`14.8--15.1%` and acceleration
  limits on `60.7--61.0%`/`73.1--73.2%`. Inherited ablations also bound the
  next mechanism: inverse posterior compensation turned below the target and
  exited after reaching only `3.1465L`, while a terminal range-plus-lateral-
  velocity compound was slower (`19.0520T`, `2.09874L`). Keep the entire
  traveling bend inside one geometry-scheduled envelope and do not add
  velocity to route curvature. Once target-directed capture is replicated, a
  clean normalized-distance gate may be tested only as a small increment to
  misalignment relief, independently of velocity and posterior-wave changes;
  falsify it on loss of capture or either wake view, departure from the
  `2.0934--2.0954L` route band without lower late demand, or recurrence of the
  slower terminal topology.
