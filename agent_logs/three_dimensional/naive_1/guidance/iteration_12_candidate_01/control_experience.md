...
- Instantaneous terminal velocity residuals are not established improvements
  ...
  failure but did not establish an extra response gate by itself. Later
  composition evidence changes the boundary: displacement-only half-cycle
  steering plus correcting-yaw release produced three sampled coherent captures
  at `18.881--19.052T` with mean distance `2.0987--2.1024L`, improving on the
  matched no-half-cycle terminal-velocity capture at `2.11536L`; removing the
  release but keeping half-cycle steering approached only `2.9270L` and exited
  left at `28.919T`. Retaining release but phase-leading the joint-state
  half-cycle coordinate also exited left after reaching only `3.5687L`. The
  response release is therefore composition-critical with the displacement-
  based half-cycle mechanism even though it was not independently beneficial
  on the earlier carrier; do not remove it or lead its phase coordinate in
  this class. Conversely, the no-velocity/no-range half-cycle-and-response
  composition still captured at `18.881T` and `2.10234L`, so terminal velocity
  and amplitude schedules are not required for success. Preserve target-owned
  sign, displacement phase, response release, and the exact acceleration
  projection when testing downstream actuator allocation; falsify any new
  allocation on capture, the updated route band, posterior saturation/load,
  and both wake views before treating reduced command effort as progress.
