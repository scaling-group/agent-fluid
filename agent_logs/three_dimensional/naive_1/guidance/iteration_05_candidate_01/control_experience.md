...
- Completed redirect rollouts sharpen that rule. A full signed-line-of-sight,
  ...
  actuator-envelope relief as a separate joint-state ablation rather than a
  near-target schedule or route retune. This lesson is falsified if the package
  loses repeat or held-out capture, destroys wake coherence, or if isolated
  relief worsens route topology beyond the observed repeat spread without
  reducing actuator contact.
