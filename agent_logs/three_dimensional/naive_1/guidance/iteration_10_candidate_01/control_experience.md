...
- Terminal feedback must preserve target-side authority as well as the carrier.
  Among the sampled projected captures, a sign-preserving raw body-lateral
  velocity lead reaches `0.7470L` at `19.019T` with mean distance `2.1154L`,
  compared with `0.7464L`, `19.135T`, and `2.1220L` for the projected base and
  `0.7492L`, `19.168T`, and `2.1259L` for a yaw-plus-bearing-progress-qualified
  release. Their top-down streets and caudal Lambda2 packets remain coherent,
  and their acceleration/rate contact is materially unchanged, so raw lateral
  velocity is supported as a capture-safe, normalized terminal modulator but
  its small timing advantage is not isolated beyond repeat variability. In
  contrast, inherited evidence for an instantaneous line-of-sight transverse
  velocity residual records a `0.9490L` near miss followed by a left-domain
  exit at `32.065T` and `8.7907L`. Do not replace geometry-owned sign with an
  instantaneous course projection. Joint-phase-qualifying only the small
  terminal lateral-velocity release is also rejected: the assigned-parent
  rollout retained a coherent wake and capture-family peak planar force/moment,
  but missed at `1.1146L` near `20.0145T`, then produced a straight departing
  street and exited left at `32.0705T` and `8.9017L`. Avoid gating this terminal
  release by instantaneous anterior displacement; preserving the oscillator is
  not enough when target-side authority is lost. In contrast, placing the
  joint-phase modulation on both shares of the persistent geometry-owned
  differential curvature captured at `0.7486L` and `19.0080T`, with the best
  sampled score (`-0.21164`) and mean distance (`2.09994L`) while preserving the
  same top-down street and caudal Lambda2 packets. This single completed result
  supports the feedback location, not a transferable phase gain: retain a
  positive bounded scale and target-owned sign, and falsify the distinction on
  repeated capture, route topology, wake coherence, and load/saturation before
  accepting a scalar advantage.
