...
- Common-envelope half-cycle relief redistribution is provisionally useful
  only before final approach. One completed sampled rollout preserved the
  target-directed top-down street and compact caudal 3D structures, retained
  capture, lowered mean distance to `2.08855L`, and crossed the `2--10L`
  distance bands about `0.027--0.068T` earlier than three geometry-scheduled
  or response-coupled captures at `2.09340--2.09405L`. This is the first
  evidence that redistribution shared by both joints can improve the route
  where inherited joint-specific allocations instead kept coherent wakes but
  exited left. It is not demand relief: acceleration contact remained
  `60.85%/73.27%` and rate contact `11.07%/14.93%`, inside the comparison
  band. Nor is it uniformly beneficial: after retaining a `0.044T` lead at
  `2L`, the redistributed carrier crossed `1L` later, drove the head below
  target height, ended with body-lateral target fraction `0.887`, and captured
  at `18.8265T` versus `18.6505--18.6835T` for the comparisons. For this
  topology, preserve common rather than joint-specific allocation and test a
  continuous normalized-distance release of the added redistribution before
  changing its magnitude. Falsify the provisional lesson if replication does
  not preserve both wake views and the far/middle progress lead, or if an
  approach release fails to retain capture and recover arrival outside finite
  repeat variability; do not count unchanged saturation as relief.
