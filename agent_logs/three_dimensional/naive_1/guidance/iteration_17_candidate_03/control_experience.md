...
- Target-lateral-error gait scheduling is the first sampled improvement beyond
  ...
  compact caudal Lambda2 structures in three executable replications (their
  only differences are comments): they captured at `18.6505--18.7550T` with
  mean distance `2.09340--2.09542L`, versus the assigned carrier's `18.8815T`
  and `2.10234L`. Coupling the same envelope schedule to the existing
  correcting-yaw response gate captured at `18.6615T`, `2.09362L`, inside
  that replication spread; the shared geometry schedule is supported but the
  extra response coupling is not. Treat the roughly `0.1045T` arrival and
  `0.00202L` mean-distance spread as the current local repeatability floor: do
  not promote a secondary gate whose effect remains inside it. A terminal-only
  ...
  the `2.0934--2.0954L` repeat band, and require separately measured
  saturation reduction before calling it actuator relief.
- State-observed cadence modulation is a concrete negative around the
  geometry-scheduled carrier. A `6%` boost derived from low normalized body
  speed and attenuated by target misalignment used one shared instantaneous
  frequency in both joints, yet the initially coherent street rotated into a
  downward route, approached only `3.1735L`, and exited left at `29.0895T`
  with final distance `9.1868L`. Its lower rate contact (about
  `10.1%/12.8%`, versus `11%/15%` in captures) is loss of capture-class
  authority, not demand relief. Do not use instantaneous speed to change this
  carrier's cadence, even with shared joint phase and alignment gating;
  falsify this boundary only if a distinct cadence mechanism first retains
  capture, both coherent wake views, and the `2.0934--2.0954L` route band
  before comparing onset speed or saturation.
