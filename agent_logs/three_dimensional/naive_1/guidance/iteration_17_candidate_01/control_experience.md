...
- Target-lateral gait-envelope scheduling is the first sampled improvement
  beyond the clean half-cycle carrier. Reducing only oscillator amplitude by
  up to `15%` with absolute normalized body-frame lateral target fraction,
  while preserving differential curvature, displacement-only phase,
  correcting-response release, and posterior lag, captured at `18.6505T`
  with mean distance `2.09340L`; an executable comment-only repeat captured at
  `18.6835T` and `2.09405L`. Coupling the schedule to correcting yaw also fell
  inside that repeat band (`18.6615T`, `2.09362L`), so the shared geometry
  schedule is supported but the extra response coupling is not. A terminal
  compound of range relief and lateral-velocity lead was weaker (`19.0520T`,
  `2.09874L`) despite retaining capture, and it took a larger visible vertical
  excursion while leaving acceleration/rate contact essentially unchanged.
  Preserve the simpler geometry schedule when testing a distinct mechanism;
  avoid stacking terminal velocity/range compounds, and do not call the
  schedule demand relief unless a later rollout reduces the current roughly
  `61%/73%` acceleration and `11%/15%` rate contact while retaining capture,
  both coherent wake views, and the replicated `2.0934--2.0955L` mean-distance
  band. Three completed gait-allocation tests sharpen that boundary. Scheduling
  the common oscillator rate down with target misalignment retained capture
  but regressed to `19.3875T` and `2.14184L`; its modest acceleration-contact
  reduction to `59.2%/70.1%` was not useful relief. Boosting that common rate
  only at low observed body speed preserved an energetic wake yet missed at
  `3.1735L` and exited left at `29.0895T`, while response-gated relief of the
  zero-mean posterior wave missed at `3.4260L` and exited left at `27.5055T`.
  Around this carrier, avoid further target- or speed-scheduled cadence and
  posterior-wave allocation: wake coherence and lower isolated contact rates
  do not establish route preservation. Recover the geometry-only amplitude
  schedule before testing a genuinely different observation-to-actuation
  mechanism; falsify this negative boundary only with capture, both wake
  views, and a route metric outside duplicate variation.
