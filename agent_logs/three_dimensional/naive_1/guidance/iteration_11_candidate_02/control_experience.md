...
- Instantaneous terminal velocity residuals are not established improvements
  ...
  steering while retaining the response-release gate: three unique
  compositions produced four coherent captures at `18.865--19.052T` with mean
  distance `2.0987--2.1047L`, improving the prior sampled route band without
  disrupting the top-down street or compact caudal Lambda2 structures. That
  establishes half-cycle redistribution as compatible with capture, but not
  the terminal additions. Byte-identical lateral-velocity versions span
  `18.865--19.008T`, the no-velocity composition reaches at `18.881T`, and an
  added `20%` approach-amplitude relief reaches at `19.052T`; meanwhile all
  variants remain on the acceleration envelope for about `60.7--61.1%` /
  `73.1--73.4%` of anterior/posterior rows and retain rate contact. Treat those
  differences as repeatability-scale tradeoffs, not support for a terminal
  residual, amplitude scalar, or load benefit. For this capture class, next
  isolate geometry-signed half-cycle steering from velocity and response
  qualifiers while retaining the projected envelope and static curvature
  sign; falsify the ablation on capture, the `18.865--19.052T` and
  `2.0987--2.1047L` route band, saturation, and both wake views.
