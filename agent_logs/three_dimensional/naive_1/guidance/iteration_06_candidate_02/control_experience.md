...
- Completed redirect evidence supports a geometry-authoritative steering
  ...
  `10.6--10.8%`/`14.1--14.4%`. Two later isolated outward-rate guards beginning
  at `0.80` and `0.85` of the envelope eliminate rate contact and preserve
  energetic 2D/3D wakes, but both destroy the captured route: they exit after
  reaching only `5.3386L` and `5.0277L`. This falsifies pre-limit velocity
  tapering as route-neutral demand relief for this carrier; do not repeat it or
  treat wake coherence and zero saturation as sufficient. A final hard
  projection identical to the episode's acceleration clamp is the appropriate
  next contract-cleanup ablation because it leaves applied action unchanged,
  but it cannot claim lower rate contact or physical effort. Any actual demand
  relief must separately demonstrate preserved carrier phase, capture, arrival,
  distance integral, and load history.
