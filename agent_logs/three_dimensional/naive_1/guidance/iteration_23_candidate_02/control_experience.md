...
- Redirect-to-cruise gait allocation now has a bidirectional negative
  boundary.
  ...
  `73.0--73.2%` in the current captures. The assigned parent's opposite
  allocation is now completed evidence: attenuating only that zero-mean
  posterior wave by at most `15%`, while retaining both target-signed bias
  shares and the captured common redistribution, kept both coherent wake rows
  but bent upward, reached only `3.92476L`, and exited at `26.2130T` and
  `6.74099L`. Its lower posterior acceleration/rate contact
  (`65.88%`/`9.95%`) is not relief because capture was lost. Preserve the
  posterior displacement-plus-lag wave unchanged around this carrier; reject
  posterior-specific enhancement and attenuation even when wake coherence or
  saturation statistics look favorable. Falsify this boundary only with a
  distinct phase-preserving posterior mechanism that first restores capture
  and the established route band.
- Common-envelope half-cycle redistribution now has a repeated, narrow
  ...
  ordinary capture. In contrast, an inherited broadside curvature reserve is
  active completed evidence for changing the route layer rather than the
  wave: exact head-based reconstruction shows its normalized lateral target
  gate crosses `0.60` at `1.68725L`, reaches `0.71484`, and then recenters
  before capture at `18.86499T` and mean distance `2.097245L`, with actuator
  contact inside the ordinary capture class. It establishes compatibility of
  bounded, target-signed curvature reserve with the traveling wake, not faster
  arrival, a lower distance integral, or recovery from an earlier divergence.
  When composing it with another captured carrier, preserve both wave terms
  and accept improvement only if an activated gate retains capture and either
  changes a failure topology or produces a route/load benefit outside repeat
  variability.
