...
- Common-envelope half-cycle redistribution has a repeatable capture and
  ...
  actuator relief. The corresponding exact-policy traces add an activation
  boundary that thresholded mechanisms must respect: after initially similar
  motion, their maximum absolute body-lateral target fractions span about
  `0.438--0.940`, so a broadside onset near `0.55` can be dormant in one
  realization and active in another. Audit activation per rollout and require
  repeated active outcomes; a dormant capture establishes carrier
  non-interference only. Arrival alone is also not a safe selector inside this
  variability: the fastest repeat at `18.6505T` has the trio's worst mean
  distance (`2.09222L`), while the best-score repeat arrives at `18.8265T`
  with the best mean (`2.08855L`). Credit a thresholded or faster branch only
  when its route/integral difference survives executable-equivalent repeats,
  both wake views, and demand/load checks. These captures do not erase an
  inherited executable-equivalent miss
...
- Route recovery must activate before the observed divergence and must not
  ...
  completed means (`2.09386--2.09873L`) do not improve redistribution. The
  assigned reserve parent sharpens that negative result: it captures at
  `18.7055T` and `2.09386L`, yet the three executable-identical redistribution
  samples capture at `18.6505--18.8815T` and `2.08855--2.09222L`. Both visual
  rows show the same useful target-bending street and compact caudal
  structures, while the parent's acceleration contact (`60.84%/73.04%`), rate
  contact (`10.91%/14.73%`), and peak planar loads overlap rather than improve
  the redistribution band. Thus a faster single arrival does not credit an
  auxiliary channel when the distance integral, wake topology, and demand are
  no better; prefer the replicated simpler controller. Directly composing full
  reserve and redistribution preserves an energetic top-down street and
  compact caudal structures yet bends away, reaches only `1.90495L`, and exits
  left at `31.1905T` and `9.45478L`; wake coherence alone therefore cannot
  validate compatible control allocation. Two geometry-based arbitration
  variants recover capture from that interaction failure, but axis-to-onset
  release yields mean distance `2.09850L`, and complementary cross-fade arrives
  at `19.2060T` with `2.09986L`, both outside the isolated redistribution band.
  Treat arbitration as a safety mechanism, not a performance improvement, and
  stop stacking broadside, rearward, or handoff channels around this carrier.
  A future recovery test needs an observation demonstrably active before the
  rare near-miss diverges and must beat the repeated capture, two-view wake,
  `2.0886--2.0922L` route, and demand/load boundaries; otherwise preserve the
  simpler redistribution controller.
