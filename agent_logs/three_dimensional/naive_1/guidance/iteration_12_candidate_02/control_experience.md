...
- Instantaneous terminal velocity residuals are not established improvements
  ...
  instantaneous velocity directly add or subtract route curvature. The
  assigned parent's next response-qualification test made target geometry
  retain sign and required both correcting yaw and improving recent bearing
  before releasing curvature. It preserved the same coherent wake and
  `capture`, but at `19.1675T`, mean scored distance `2.12587L`, and score
  `-0.23738` it did not improve the sampled terminal-slip carrier at
  `19.0190T`, `2.11536L`, and `-0.22671`; rate contact also rose slightly from
  `10.76%/14.05%` to `10.85%/14.55%`. Thus the inherited eight-row
  reconstruction—only `6/1624` yaw/bearing disagreements in a capture versus
  `1092/2826` in the course-residual exit—identified misleading yaw in that
  failure but did not establish a useful extra gate on the already successful
  route. The next sampled package added bounded target-side half-cycle
  steering while retaining the response-release gate. The selected matched
  comparison sharpens the reusable result: the terminal-velocity carrier
  without half-cycle steering captures at `19.0190T` with mean distance
  `2.11536L`, whereas adding the joint-state half-cycle factor to that same
  composition captures at `19.0080T` and `2.09994L`. A half-cycle version with
  no velocity residual also captures at `18.8815T` and `2.10234L`, and the
  version with `20%` approach-amplitude relief reaches at `19.0520T` and
  `2.09874L`. All retain the target-directed top-down street and compact caudal
  Lambda2 structures. Thus the common positive mechanism is geometry-signed
  half-cycle redistribution, while instantaneous velocity is not necessary
  for capture and range relief remains unproven. None establishes load relief:
  all four contact projected acceleration on about `60.7--61.5%` /
  `72.0--73.4%` of anterior/posterior rows and retain rate contact. Every
  sampled half-cycle version still shares the yaw-response release gate, so
  half-cycle steering alone remains unisolated; test that ablation before
  stacking another terminal schedule. Two inherited step-11 scalar-only
  `left_domain` logs (closest approaches `2.927L` and `3.569L`) lack sampled
  policy and wake evidence, so they establish recurrence of a failure class,
  not which qualifier caused it. Falsify the clean ablation on capture, the
  `18.881--19.052T` and `2.0987--2.1024L` half-cycle route band, saturation,
  and both wake views.
