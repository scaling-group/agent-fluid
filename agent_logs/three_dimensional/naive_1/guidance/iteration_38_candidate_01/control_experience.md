...
- Completed redirect evidence supports a geometry-authoritative steering
  ...
  than an isolated outlier. If a coupled demand allocator is tested, make it
  a separate bounded ablation that preserves target-signed curvature and
...
- Full-vector radial acceleration allocation is now a concrete negative
  result, not an open coupled-demand hypothesis. An inherited completed worker
  replaced independent final clipping with one positive scale on the entire
  anterior/posterior command vector. In direct-uniform still water it reduced
  acceleration contact from the captured carrier's roughly
  `60.9%/73.0%` to `19.5%/42.9%` and eliminated rate contact, but reached only
  `11.5298L`, swept heading from `0.645` to `-1.104 rad`, and exited
  upward-left at `9.713T` and `11.7998L`. Its top-down wake remains short and
  weak relative to sampled captures and the oblique structures never develop
  the same compact target-directed trail, so the lower contact is propulsion
  starvation rather than relief. Do not scale the whole traveling-bend pair
  merely to preserve instantaneous command ratio. A future allocator must
  protect the independently projected propulsive carrier and, if tested,
  coordinate only a bounded target-feedback residual; reject it on lost
  capture, either wake row, or route/load statistics outside the completed
  carrier spread even if saturation counts fall.
...
- Common-envelope half-cycle redistribution has a repeatable capture and
  ...
  retain the `2.0886--2.0922L` band without another near miss. Three subsequent
  ...
  six completed captures without a known miss and revising its exact-policy
  range to `18.6010--18.9585T` and `2.09042--2.09898L`. Their inspected
  top-down streets and oblique caudal structures remain coherent, but
  acceleration contact (`60.78--61.15%`/`72.95--73.09%`), rate contact
  (`10.88--11.05%`/`14.84--14.96%`), peak planar force
  (`0.03197--0.03259`), and peak moment (`0.01638--0.01656`) overlap the
  redistribution captures. The newest clean repeat arrives earlier than the
  current executable-identical redistribution pair (`18.8265--18.8815T`) but
  has mean distance only `0.00146--0.00188L` higher, so that tradeoff is too
  small relative to exact-policy spread to establish performance dominance.
  Separating displacement-phase steering from drive-envelope phase modulation
  remains the better semantic-robustness candidate, while redistribution is
  only the lower-distance-integral comparator; neither is actuator relief.
  Falsify that preference on the clean carrier's first executable-equivalent
  miss or loss of either wake view, and do not restore redistribution merely
  for a score change inside repeat spread.
