...
- Capture-compatible redirect and gait-allocation mechanisms are not safely
  additive without a shared curvature budget. The geometry-scheduled
  broadside reserve captures at `18.7055T` and the half-cycle redistribution
  captures twice at `18.8265--18.8815T`, all with target-bending top-down
  streets and compact caudal Lambda2 structures. Yet their inherited direct
  composition retained both energetic wake views while reaching only
  `1.90495L`, passing broadside, and exiting left at `31.1905T` with final
  distance `9.45478L`. Trace reconstruction gives the actionable boundary:
  the successful reserve's combined normalized curvature request peaked near
  `1.012`, whereas the failed composition reached about `1.196` near `2L` and
  remained about `1.170` at closest approach. Do not infer composability from
  separate captures or wake coherence, and do not stack an additive redirect
  onto a phase-redistributed gait without explicitly budgeting their combined
  mean-curvature request. A later projected composition is only a hypothesis;
  falsify the budget lesson if independent evaluations preserve capture, both
  wake views, and a useful route change while intentionally exceeding the
  established request without extra contact/load or the same left-exit
  topology.
- Behind-target route-argument feedback has not demonstrated recovery and is
  ...
  argument and do not call a target-ahead capture recovery evidence. This
  boundary survives the current sample: the ordinary geometry carrier, two
  executable-identical redistribution policies, and the dormant rearward
  multiplier all capture from direct-uniform water at `18.6505--18.9640T`,
  while the inspected carrier and redistribution sheets show the same
  target-directed top-down street and compact caudal Lambda2 structures.
  Their success therefore does not identify recovery authority. Likewise,
  inherited compact score logs span capture and a `left_domain` failure that
  reached only `3.9248L`, but omit the executable controller and wake evidence;
  use them as a robustness warning, not as mechanism attribution. A distinct
  test may gate a small mean-curvature reserve directly from broadside
  body-frame target direction before overshoot, outside the saturated route
  argument. This remains a hypothesis, not a positive lesson: preserve
  target-owned sign, correcting-yaw release, the two-joint traveling wave,
  ordinary-capture behavior, both wake views, and the established
  actuator/load boundary; falsify it on any lost capture, repeated left-exit
  topology, or extra contact/load without semantic route benefit.
