...
- Common-envelope half-cycle redistribution retains a repeatable
  distance-integral benefit among captures but has a contradictory semantic
  boundary. Three executable-identical samples capture at
  `18.6505--18.8815T` with mean distance `2.08855--2.09222L` and coherent
  target-bending top-down and compact caudal Lambda2 structures, while two
  inherited executable-equivalent runs keep energetic wakes yet miss at
  `0.81206L` and `1.25093L` before downward/left exits. The assigned parent's
  scalar-only capture logs enlarge the outcome set but cannot explain that
  contradiction. The previously requested isolated test is now completed:
  removing only phase-dependent envelope relief while preserving normalized
  target-signed displacement-phase curvature, common geometry relief,
  response release, and the posterior-lagged carrier captures at `18.6010T`
  with the same two coherent wake classes. It arrives `0.2255--0.2805T`
  earlier than the two currently sampled executable-identical redistribution
  repeats, but has a worse score (`-0.20288` versus `-0.20106` to
  `-0.20041`), slightly higher peak planar force/moment
  (`0.01605/0.02968/0.01656`), and overlapping acceleration/rate contact
  (`60.88%/72.95%`, `10.88%/14.96%`). Therefore prefer separation of
  displacement-phase steering from the common drive envelope as the simpler
  semantic baseline, not as proven performance, robustness, or actuator
  relief. This applies when phase modulation of a shared propulsive envelope
  has both capture and coherent-wake route-failure outcomes; falsify the
  preference on an executable-equivalent clean miss, loss of either wake row,
  or loads outside the clean-carrier spread. Do not restore redistribution for
  a scalar change inside repeat spread, and do not add another envelope,
  distance, alignment, response-release, or recovery compound until the clean
  baseline is independently replicated.
