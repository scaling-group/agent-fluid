...
- Recovery feedback must act before the target passes behind and outside the
  saturated route argument, but ordinary-capture compatibility is not recovery
  evidence. A rearward multiplier captured at `18.9640T`, yet its normalized
  forward target fraction never fell below `0.3541`, so its new branch was
  dormant. The inherited behind-route reserve instead waited too long: an
  energetic two-view wake missed at `0.85155L` and exited left after the target
  became broadside while still forward and the ordinary route request was
  already about `0.995` of saturation. Avoid more behind-only terms and
  multipliers inside that saturation. Two separately parameterized direct
  broadside mean-curvature reserves now establish the narrower positive
  boundary: both keep the geometry-scheduled carrier in the coherent
  capture/load class, reaching `0.7466--0.7482L` at
  `18.7055--18.8650T` with mean distance `2.09386--2.09724L`. The latest gate
  is demonstrably active from `1.5945L`; absolute lateral target fraction peaks
  at `0.6484` and falls to `0.1851` at capture with forward fraction `0.9827`,
  while acceleration contact (`60.84%/73.04%`), rate contact
  (`10.85%/14.67%`), planar force (`0.03275`), and yaw moment (`0.01704`)
  remain in the carrier band. Treat this as safe pre-overshoot composition
  evidence, not a proven scalar improvement or near-miss rescue: both arrival
  and mean distance overlap geometry-carrier variation, and no reserve rollout
  has yet exercised the documented `0.81206L` redistribution miss. A future
  composition may place this reserve outside the route saturation while
  preserving target-owned sign, correcting-yaw release, both curvature shares,
  and the traveling wave; credit it only if capture and both wake views survive
  and a broadside near-miss is eliminated without extra contact/load.
