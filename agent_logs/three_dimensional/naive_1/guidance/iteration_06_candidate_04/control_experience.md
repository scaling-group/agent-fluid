...
- Four direct-uniform evaluations of the same executable differential-curvature
  controller all captured at `0.7482--0.7497L` in `19.228--19.784T`, despite a
  score spread from `-0.2433` to `-0.2299` and visibly different sides of the
  terminal crossing. Treat that spread as a repeatability band, not gain
  evidence. The reusable package is a normalized lateral target request,
  opposite-sign anterior/posterior mean curvature, and correctly signed yaw
  used only for bounded release: preserve it when isolating a new mechanism.
  This conclusion is bounded to the direct-still-water task and is falsified
  by repeat loss of capture, a returned high/low boundary exit, lost wake
  coherence, or materially worse saturation and loads.
