...
- Geometry-scheduled steering-to-gait authority is a provisional route
  improvement around the displacement-half-cycle carrier, not established
  ...
  top-down street and compact caudal 3D structures, and captured in
  `18.6505T` with mean distance `2.09340L` and score `-0.20606`; the clean
  carrier captured in `18.8815T`, `2.10234L`, and `-0.21389`, while sampled
  terminal-velocity/range compounds were slower at `19.0080--19.0520T`.
  However, acceleration contact (`60.98%/73.19%`), rate contact
  (`11.12%/15.13%`), and RMS action (`27.147/28.620 rad/T^2`) still overlap
  the clean carrier, so preserve this mechanism only as a bounded redirect
  schedule and do not cite it to justify stronger gains or efficiency claims.
  This implication applies to the captured geometry-owned,
  displacement-phase architecture; require replicated capture and route
  improvement before generalizing it, and falsify a refinement if either wake
  view, capture topology, or the `18.650--19.052T` / `2.0934--2.1024L` route
  band is lost even when saturation falls.
