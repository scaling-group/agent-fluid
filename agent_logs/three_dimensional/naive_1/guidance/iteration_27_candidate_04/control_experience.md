...
- Common-envelope half-cycle redistribution has a repeatable distance-integral
  ...
  mean distance `2.08855--2.09222L`, overlapping but mostly below five
  geometry-scheduled captures' `2.09340--2.09542L` band. Their terminal
  geometry is not identical: one never reaches
  `|lateral_fraction|=0.60` and captures with forward/lateral direction
  fractions `0.9843/-0.1766`, while two first become broadside only at
  `1.2746--1.2861L` and capture with forward fraction `0.3420--0.4612`.
  Thus executable identity can span distinct useful terminal trajectories;
  neither broadside motion nor one faster arrival is itself evidence for a
  recovery term. Sampled demand remains inside the carrier's roughly
  `61%/73%` acceleration-contact and `11%/15%` rate-contact bands, so relief
  is not established. An inherited optimizer reports an executable-equivalent
...
- Behind-target route-argument feedback has not demonstrated recovery and is
  ...
  boundary survives the current sample: three executable-identical
  redistribution policies and the dormant rearward multiplier all capture
  from direct-uniform water at `18.6505--18.9640T`, while their inspected
  sheets show the same
  target-directed top-down street and compact caudal Lambda2 structures.
  Their success therefore does not identify recovery authority. Likewise,
  inherited compact score logs span captures and a `left_domain` failure that
  reached only `1.90495L` and finished at `9.45478L`, but omit the executable
  controller and wake evidence;
  ...
  actuator/load boundary. The sampled captures sharpen the admissible test:
  an early reserve must be zero throughout the successful terminal broadside
  band (no later than about `1.35L`) and use positive forward target fraction
  to exclude behind-only action. This is a non-interference boundary, not
  evidence that a reserve works. Falsify it on any lost capture, repeated
  left-exit topology, or extra contact/load without semantic route benefit.
