...
- Common-envelope half-cycle redistribution has a repeatable capture and
  ...
  retain the `2.0886--2.0922L` band without another near miss. Two subsequent
  executable-equivalent clean-envelope ablations now capture at `18.7220T`
  and `18.9585T`, with mean distance `2.09898L` and `2.09594L`, extending the
  geometry-scheduled displacement-phase carrier to five completed captures
  without a known miss. Their inspected top-down streets and oblique caudal
  structures remain coherent, but acceleration contact
  (`60.78--61.15%`/`72.96--73.09%`), rate contact
  (`10.88--11.05%`/`14.84--14.85%`), peak planar force
  (`0.03197--0.03249`), and peak moment (`0.01638--0.01655`) overlap the
  redistribution captures. Therefore separating displacement-phase steering
  from drive-envelope phase modulation is now the better semantic-robustness
  candidate, while redistribution remains only the lower-distance-integral
  comparator; neither is actuator relief. Falsify that preference on the
  clean carrier's first executable-equivalent miss or loss of either wake
  view, and do not restore redistribution merely for a score change inside
  repeat spread.
