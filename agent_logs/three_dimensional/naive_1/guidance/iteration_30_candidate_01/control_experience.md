...
- Common-envelope half-cycle redistribution has a completed negative
  robustness boundary despite an allocation-local distance-integral benefit.
  Three current executable-identical samples retain the coherent
  target-bending top-down street and compact caudal Lambda2 structures,
  capture at `18.6505--18.8815T`, and give mean distance
  `2.08855--2.09222L`, overlapping or improving on five geometry-scheduled
  captures' `2.09340--2.09542L` band. But the same executable now has two
  direct-uniform failures across inherited evaluations. One misses at
  `0.81206L`, exits left at `34.2320T`, and finishes at `10.6779L`; the
  assigned-parent repeat misses at `1.28476L`, exits left at `32.7195T`, and
  finishes at `9.94538L`. In the latter, both energetic wake views and the
  capture-class peak planar force/moment survive, while lower rate contact
  (`9.40%/10.69%`) than a matched capture (`11.07%/14.93%`) merely accompanies
  the lost route. Treat wake coherence, lower contact, and the lower captured
  integral as insufficient when a phase-dependent envelope can bifurcate into
  this down-left exit. Prefer phase-independent geometry-scheduled common
  relief for reliability, while retaining displacement-only half-cycle
  steering as the single beat-side channel. Avoid another envelope, distance,
  alignment, or response-release compound: prior distance release exited after
  reaching `2.2485L`, while alignment release captured at `18.7880T` but
  worsened mean distance to `2.09961L` without relieving contact. Falsify this
  boundary only if redistribution captures repeat across independent
  evaluation allocations without another near miss and retain both wake views,
  the lower mean-distance band, and the semantic route class.
- Behind-target and broadside recovery channels now have a completed negative
  robustness boundary. A sampled rearward multiplier captures at `18.9640T`
  and `2.09072L` mean distance, but its normalized forward target fraction
  remains positive, so the branch is dormant and establishes only the clean
  redistribution carrier. A forward-qualified broadside reserve has one
  active capture at `18.7055T` and `2.09386L`, but an executable-identical
  active replication retains its energetic target-bending top-down street and
  compact caudal Lambda2 packets while stalling at `2.46559L`, bending away,
  and exiting left at `31.718T` with final distance `10.32467L`. The reserve's
  clean composition with redistribution also exited after reaching only
  `1.90495L`, while two smooth arbitration attempts captured with worse
  `2.09850--2.09986L` means. Therefore the isolated reserve is not repeatably
  capture-safe, and coherent wake production is not evidence of recovery.
  Remove dormant rearward logic and prefer the simpler redistribution carrier;
  do not stack another rearward, broadside, terminal, velocity, posterior, or
  arbitration modifier unless a new observation is demonstrably active before
  divergence and preserves capture, both wake views, and the established
  `2.08855--2.09072L` integral band.
- Coordinated saturation allocation is not established demand relief around
  this carrier. A sampled allocator that moved only `15%` toward preserving a
  common two-joint rhythmic carrier while prioritizing the steering residual
  still retained an energetic alternating top-down street and compact oblique
  caudal structures, yet stalled at `2.43664L`, exited left, and finished at
  `9.22177L`. Reject lower or more coordinated command demand if it changes the
  route class before demonstrating capture; avoid further allocation blends
  until a distinct design preserves the clean controller's phase-dependent
  curvature, two-view wake, capture, and distance-integral band.
