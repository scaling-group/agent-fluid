...
- Completed redirect evidence supports a geometry-authoritative steering
  ...
  ablation; a prior posterior-only plus pointwise-`tanh` compression failure at
  `9.576T` is confounded and does not falsify relief around the successful
  allocation. Reject relief that loses capture or wake coherence, preserves
  rate contact, worsens loads, or materially degrades arrival and distance
  integral.
