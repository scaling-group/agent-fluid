...
- Keep the captured carrier's cadence and zero-mean posterior wave fixed when
  testing the next mechanism. Against the current geometry-scheduled capture
  band (`18.6615--18.7550T`, mean distance `2.09362--2.09542L`), a `15%`
  misalignment-conditioned cadence reduction retained capture but delayed it
  to `19.3875T` with score `-0.25289`; a speed-conditioned `6%` onset cadence
  boost was worse, remaining self-propelled yet reaching only `3.1735L` before
  a `left_domain` exit at `29.0895T`. Posterior allocation is likewise
  falsified in both directions: the inherited enhancement failed near
  `3.1465L`, while a `15%` response-gated relief kept energetic top-down and
  oblique structures but curved down after roughly `16T`, reached only
  `3.4260L`, and exited at `27.5055T`. For this carrier, coherent vortices do
  not validate route control; avoid further shared-frequency or posterior-wave
  schedules. A later onset test should alter only a phase-preserving envelope
  and must recover the exact parent gait with observed speed, then be rejected
  on lost capture, route-band departure, persistent onset saturation, or
  worsened planar loads.
- Redistributing the existing common gait-envelope relief across observed
  displacement half-cycles is a provisional route-integral improvement, not
  actuator relief. The one completed variant differs from the replicated
  geometry-only schedule only by applying less relief on the target-aligned
  anterior half-cycle and more on the opposed half-cycle; it preserves the
  target-signed curvature shares, cadence, and posterior lag. Both wake views
  remain coherent and it captures at `18.8265T`, with score `-0.20041` and
  mean distance `2.08855L`, versus `18.6505--18.6835T`, `-0.20606--
  -0.20578`, and `2.09340--2.09405L` for executable geometry-only repeats.
  The later arrival makes the integral gain a tradeoff, and acceleration
  (`60.85%/73.27%`) and rate (`11.07%/14.93%`) contact still overlap the
  parent band. Around this captured displacement-phase carrier, replicate the
  exact common-envelope mechanism before composing another residual; accept
  it only if capture and both wake structures persist and the score or
  mean-distance lead repeats without worse loads. If a replay falls inside
  the geometry-only band, classify the first lead as finite variation rather
  than tuning the redistribution fraction; posterior-only redistribution is
  already falsified by coherent-wake downward exits.
