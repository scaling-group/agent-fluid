...
- Common-envelope half-cycle redistribution has a repeatable capture and
  distance-integral benefit but a now-repeated semantic robustness
  contradiction. Three current executable-identical samples retain the
  coherent target-bending top-down street and compact caudal Lambda2
  structures, capture at `18.6505--18.8815T`, and have mean distance
  `2.08855--2.09222L`; their acceleration and rate contact still overlap near
  `61%/73%` and `11%/15%`, so redistribution is not actuator relief. The
  sampled rearward multiplier also captures at `18.9640T` and `2.09072L`, but
  its target stays forward, making that branch non-interference rather than
  recovery evidence. Against these captures, two inherited rollouts with the
  same redistribution bytes keep energetic wakes yet miss at `0.81206L` and
  `1.25093L` before downward/left exits. Exact replication therefore estimates
  CFD outcome spread but cannot establish robust route semantics or justify
  another inactive recovery branch. The assigned parent's inherited step-33
  artifact adds only a capture score of `-0.20750`, without route, branch-
  activation, load, or wake history; it enlarges the scalar outcome set but
  cannot attribute capture to redistribution. When one executable mechanism
  appears in both capture and coherent-wake failure topologies, scalar-only
  repetitions are outcome-spread evidence, not mechanism evidence: isolate
  one state-feedback channel before adding another. Here the narrow test is to
  remove only phase-dependent envelope redistribution while retaining the
  target-signed displacement-phase curvature factor, common geometry-owned
  mean relief, response release, and posterior-lagged carrier. Judge that
  ablation first on capture, both wake rows, and return near the simpler
  geometry-scheduled carrier's `2.09340--2.09542L` mean-distance band; reject
  lower limit contact alone or the same downward-exit topology. Avoid adding
  another envelope, distance, alignment, response-release, or recovery
  compound until this isolated semantic test is evaluated.
