...
- Completed redirect evidence supports a geometry-authoritative steering
  ...
  than an isolated outlier. The assigned parent's completed common radial
  allocator gives the negative boundary: scaling both raw joint commands when
  either exceeded the envelope weakened and smeared the early wake, improved
  distance only from `12.3277L` to `11.5409L`, and exited left at `9.702T` and
  `11.8262L`. Its lower acceleration contact (`19.54%`/`42.87%`), zero rate
  contact, and lower peak planar force/moment (`0.01905`/`0.00950`) reflect
  lost propulsion and steering, not relief. Avoid shared instantaneous-ratio
  projection around this carrier: saturation handling must leave the anterior
  phase generator independent of posterior peaks and preserve target-signed
  curvature, capture, both wake views, and the established route band before
  lower contact is useful. In contrast, outward
...
- Instantaneous terminal velocity residuals are not established improvements
  ...
  Direct bearing-window release is also semantically inconsistent: one
  completed policy captures at `18.7220T` and mean distance `2.09370L`, while
  a parameter-renamed but control-equivalent repeat retains an energetic
  two-view wake, reaches only `1.4437L`, and exits left at `30.938T` and
  `8.9897L`. Treat yaw/bearing disagreement as a diagnostic, not authority for
  another response-release replacement or compound; falsify any future route
  observation on independent capture, both wake rows, route/load spread, and
  absence of this near-miss/left-exit topology.
