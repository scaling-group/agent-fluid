...
- Common-envelope half-cycle allocation is a provisional route improvement,
  not established demand relief. The geometry-scheduled executable and its
  comment-only repeat capture at `18.6505T`/`18.6835T`, with mean distance
  `2.09340L`/`2.09405L`; this is the finite-variation reference band, not two
  independent mechanisms. Coupling the same relief to correcting-yaw response
  captures at `18.6615T` and `2.09362L`, inside that band, so avoid stacking
  another response gate or crediting it with route improvement. The distinct
  sample preserves geometry-owned mean relief but moves it from the target-
  aligned displacement half-cycle to the opposed half; it captures later at
  `18.8265T`, improves mean distance to `2.08855L`, and follows a meaningfully
  lower route (`min center y=9.6129L` versus `9.9819--10.1669L`). Both wake
  rows remain coherent; peak planar force/moment are below the geometry-
  scheduled copies and within the response-coupled trace, while rate and
  projected-action contact stay inside the capture band. Preserve common
  joint scheduling and treat the route change, not demand relief, as the
  signal. This is only one redistributed rollout: require an exact replication
  before promoting its `0.50` fraction, and falsify the mechanism if capture,
  the lower route, or mean-distance separation disappears. Inherited optimizer
  logs include `left_domain` trajectories after `3.4260L` and `1.0072L`
  closest approaches, while their candidate notes tie related failures to
  joint-specific posterior authority changes; avoid separate redirect/cruise
  envelopes for the two joints even when wake coherence survives, and do not
  infer causality from scalar logs alone.
