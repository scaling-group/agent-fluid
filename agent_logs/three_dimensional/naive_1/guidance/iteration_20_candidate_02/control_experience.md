...
- Common-envelope half-cycle redistribution is provisionally useful but does
  not support an approach-release compound. The only sampled executable
  redistribution lowers mean distance to `2.08855L` and improves score to
  `-0.20041` from the three comparison captures' `2.09340--2.09542L` and
  scores from `-0.20743` to `-0.20578`, with earlier `2--10L` crossings and
  the same coherent target-bending top-down street and compact caudal Lambda2
  structures. It
  captures later at `18.8265T`, however, and its `60.85%/73.27%`
  acceleration contact and `11.07%/14.93%` rate contact remain inside the
  comparison bands, so neither faster arrival nor demand relief is
  established. Two completed release ablations sharpen the boundary: a
  distance smoothstep loses capture and exits left after reaching `2.2485L`,
  while a normalized forward-alignment smoothstep recovers capture at
  `18.7880T` but worsens mean distance to `2.09961L`, score to `-0.21153`, and
  leaves contact rates unchanged. Preserve the ungated common redistribution
  for a direct replication before calling its single lower distance integral
  reusable; avoid stacking another distance, alignment, or response release
  gate. Falsify the mechanism if a repeat does not retain capture, both wake
  views, and mean distance below the `2.09340--2.09542L` comparison band.
