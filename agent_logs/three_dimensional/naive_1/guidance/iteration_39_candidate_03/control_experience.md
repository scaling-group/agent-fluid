...
- Instantaneous terminal velocity residuals are not established improvements
  ...
  the course-residual failure. Two completed gate tests now bound that
  diagnostic: an inherited extra response qualification merely retained
  capture at score `-0.23738`, while the assigned parent's direct replacement
  of yaw response with bearing-window progress retained both coherent wake
  rows and captured at `18.7220T` but had mean distance `2.09370L`. Its
  anterior/posterior acceleration contact (`60.78%/73.21%`), rate contact
  (`11.25%/14.86%`), peak planar force (`0.03130`), and peak moment (`0.01606`)
  overlap the current sampled redistribution captures, whose best repeated
  mean-distance band is `2.08855--2.08896L`. Thus yaw/bearing disagreement is
  useful for classifying route failures, but changing or qualifying the
  release observation alone is neither route improvement nor actuator relief.
  Avoid further release-gate compounds; a distinct saturation mechanism must
  retain capture, both wake rows, and the sampled route/load band before lower
  contact is credited.
