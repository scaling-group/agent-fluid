...
- Completed redirect evidence supports a geometry-authoritative steering
  ...
  gives a concrete negative boundary: blending only `15%` toward a common
  carrier scale lowered anterior/posterior acceleration contact to
  `39.23%`/`57.05%` and kept an energetic alternating top-down street and
  compact caudal Lambda2 structures, yet reached only `2.43664L`, then exited
  left at `29.7110T` and `9.22177L`. Rate contact stayed near
  `10.22%`/`14.55%` and peak planar loads remained comparable, so neither wake
  coherence nor lower clipping validates an allocator that changes the
  evidenced carrier/steering balance. Avoid further common carrier scaling or
  residual-priority blends around this controller; falsify this boundary only
  with capture, both wake views, and the established route band. In contrast, outward
...
- Common-envelope half-cycle redistribution has a repeatable capture and
  ...
  contact. One-period detrending across the four current captures also shows
  that rigid heading and normalized body-lateral target residuals are both
  tightly locked to anterior displacement: their slopes are about
  `-0.65 rad/rad` and `-0.64-- -0.65`, with absolute correlations above
  `0.95`. Therefore raw instantaneous body-lateral geometry is not purely a
  slow route signal around this gait. A bounded displacement-conditioned
  target-axis correction is a legitimate observation ablation only if it
  preserves the existing carrier and mean-curvature allocation; the
  correlation alone is not proof of better control. Falsify redistribution's
  current status on another executable-equivalent miss or loss of either wake
  row, and reject any phase correction that loses capture, increases route
  oscillation or loads, or leaves the `2.0886--2.0922L` band without a distinct
  robustness benefit.
