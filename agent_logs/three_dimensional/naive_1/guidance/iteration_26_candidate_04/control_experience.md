...
- Common-envelope half-cycle redistribution has a repeatable distance-integral
  ...
  distance to `2.09961L` without relieving contact. The proposed normalized
  body-frame handoff now has one completed inherited result: smoothly releasing
  redistribution before the broadside reserve activates retained semantic
  capture at `0.749397L` (score `-0.210898`) after the ungated composition had
  exited left. The compact parent score does not include wake, route, demand,
  arrival, or integral diagnostics, so this establishes capture compatibility,
  not recovery or efficiency. A separately completed unit-curvature-budgeted
  reserve also retained capture at `0.749975L` (score `-0.211698`), supporting
  the projection's non-interference in isolation but not its composition with
  the handoff. Test that small combination as an explicit robustness boundary:
  falsify it on any lost capture, repeated left-exit topology, lost wake
  coherence, load/contact growth, or failure to retain either a distinct route
  benefit or the isolated redistribution's lower mean-distance band.
