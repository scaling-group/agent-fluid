...
- Body-frame misalignment scheduling is now a replicated capture-class
  gait-envelope mechanism with a sharp allocation boundary. Two
  executable-identical direct-uniform samples use absolute lateral target
  fraction to reduce the whole rhythmic envelope by at most `15%`, while
  preserving differential mean curvature, displacement-only phase, response
  release, posterior lag, and final projection. Both retain a coherent
  target-directed top-down street and compact caudal 3D structures and capture
  at `18.6505--18.6835T` with mean distance `2.09340--2.09405L`, versus the
  assigned parent's clean restoration at `18.8210T` and `2.10522L`.
  Multiplying the same schedule by the correcting-yaw response gate also
  captures at `18.6615T` and `2.09362L`, but lies inside the geometry-only
  repeat band, so the extra coupling is not attributable. A terminal range and
  lateral-velocity compound is weaker (`19.0520T`, `2.09874L`) and should not
  be stacked onto this simpler mechanism. Most importantly, compensating the
  posterior displacement-plus-lag wave by the inverse relieved anterior
  envelope preserves an energetic wake but turns below the target, reaches
  only `3.1465L`, and exits left at `28.0775T` with final distance `9.2165L`;
  acceleration contact rises to about `63.9%/74.7%`. Keep the whole traveling
  bend inside one geometry-scheduled envelope and avoid posterior-only wave
  preservation. This applies when the existing carrier is already
  target-directed; falsify it on loss of capture, either wake view, or the
  `2.0934--2.0955L` route band. It is route evidence, not demand relief. Across
  the current four sampled captures—including the assigned response-coupled
  parent and the geometry-only schedules—the final acceleration projection is
  active on `60.6--61.0%`/`73.0--73.2%` of anterior/posterior rows and rate
  contact remains `10.9--11.2%`/`14.8--15.1%`; response coupling therefore
  does not establish actuator relief. After the failed outward rate barriers,
  any next relief experiment must preserve the paired traveling-bend
  allocation at the actuator boundary and earn capture plus both wake views
  before lower saturation counts matter; avoid another independently applied
  pointwise taper.
