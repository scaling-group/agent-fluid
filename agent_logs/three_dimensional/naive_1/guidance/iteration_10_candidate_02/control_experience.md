...
- Instantaneous terminal velocity residuals are not established improvements
  ...
  route. Do not stack another velocity or response-release qualifier around
  this capture class.
- Joint-state half-cycle steering is a promising route-shaping mechanism, but
  not load relief. In the matched projected terminal-lead pair, adding only a
  positive `0.8--1.2` modulation of the existing target-signed anterior and
  posterior curvature biases improves score from `-0.22671` to `-0.21164`,
  scored mean distance from `2.11536L` to `2.09994L`, and capture from
  `19.0190T` to `19.0080T`. Both top-down streets and compact caudal Lambda2
  structures remain coherent; peak streamwise force changes by only `+0.4%`,
  while peak lateral force and yaw moment fall by about `4.2%` and `2.8%`.
  Posterior acceleration/rate contact nevertheless changes from about
  `72.0%/14.1%` to `73.1%/14.7%`, so do not market the mechanism as demand
  reduction. Transfer the invariant by deriving half-cycle alignment from
  normalized observed joint displacement and keeping the scale positive, so
  static body-frame target geometry still owns turn sign and the propulsive
  carrier remains intact. This applies to the established capture carrier;
  because the positive sample also contains the unproven terminal velocity
  lead and has only one evaluation, next isolate half-cycle steering on the
  projected parent without that lead. Falsify portability if capture or wake
  coherence is lost, if route quality fails to beat the `19.019--19.228T` and
  `2.1154--2.1259L` capture band, or if saturation/load statistics materially
  worsen.
