...
- Promote a recovery channel only when evidence shows that it activated and
  changed the failure topology; ordinary capture establishes at most
  compatibility. The sampled rearward-multiplier policy captures at
  `18.9640T`, but its normalized forward target fraction never falls below
  `0.3541`, so the new branch is dormant. The inherited behind-route reserve
  instead waits too late: its energetic two-view wake misses at `0.85155L`
  and exits left after becoming broadside while the target is still forward
  and the ordinary route request is already about `0.995` saturated. Avoid
  more behind-only branches and multipliers inside route saturation. A
  separate broadside mean-curvature channel supplies the narrower exercised
  boundary: it first activates at `1.5945L`, peaks at a bounded `0.03851`
  curvature share while the target remains ahead, preserves the coherent
  target-bending top-down street and compact caudal Lambda2 structures, and
  captures at `18.7055T` with mean distance `2.09386L`. Its acceleration/rate
  contact (`60.84%/73.04%`, `10.91%/14.73%`) overlaps the other three sampled
  captures, while peak planar force/moment (`0.03275/0.01704`) sit just above
  their `0.03066--0.03171`/`0.01603--0.01640` ranges. The two
  executable-identical common-envelope samples remain separately useful:
  both capture with the same wake class and reproduce a lower
  `2.08855--2.08896L` mean-distance band, but an inherited equivalent misses
  at `0.81206L`. Thus a pre-overshoot reserve outside route saturation may be
  composed with that carrier as a bounded robustness test only if it preserves
  target-owned sign, both curvature shares, displacement-only phase,
  correcting-yaw release, and the traveling wave. Do not call an ordinary
  capture a rescue; falsify the composition on lost capture, the same
  broadside/left-exit topology, damaged two-view wake coherence, or a material
  load/contact increase.
