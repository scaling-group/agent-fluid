...
- Common-envelope half-cycle redistribution has a repeatable distance-integral
  benefit among captures but does not satisfy the semantic robustness boundary.
  ...
  actuator relief and is contradicted by two executable-equivalent misses. The
  earlier one reaches `0.81206L` before exiting left at `34.2320T` and
  `10.6779L`; the later one bends below the target, reaches only `1.25093L`,
  and exits left at `32.6370T` and `9.94426L`. Both retain energetic two-view
  wakes, and the later failure's acceleration/rate contact
  (`64.27%/70.48%`, `9.22%/10.57%`) and finite planar loads do not explain away
  the route failure. A completed clean ablation now establishes the positive
  reusable boundary: removing only phase-dependent relief redistribution while
  preserving displacement-phase steering, common geometry-owned mean relief,
  response release, and posterior lag retains both wake rows and captures at
  `18.6395T` with mean distance `2.09383L`, inside the prior
  `2.09340--2.09542L` geometry-scheduled band. Its acceleration/rate contact
  (`60.81%/73.12%`, `11.12%/15.05%`) and peak force/moment remain within the
  redistribution spread, so this ablation is a validated semantic carrier but
  not demand relief or a distance-integral improvement. Prefer it as the
  robustness baseline pending independent repeats. Avoid another
  ...
  contact. Falsify the clean carrier on an independent miss, loss of either wake
  row, or departure from its geometry-scheduled distance/load band; reopen
  redistribution as robust only if independent repeats retain its lower
  integral without another near miss.
- Cruise-aligned posterior-lag enhancement is not supported around this
  carrier. A bounded `6%` boost preserves capture at `18.7825T` and both wake
  rows but worsens mean distance to `2.10021L`, while acceleration contact
  (`60.79%/73.35%`), rate contact (`10.95%/14.73%`), peak planar force
  (`0.01635/0.0280`), and peak moment (`0.01625`) remain comparable. Avoid
  further aligned lag increases as either performance or demand relief;
  falsify this negative result only with a distinct semantic robustness gain
  plus capture, coherent wakes, and distance/load results outside repeat spread.
- Route recovery must activate before the observed divergence and must not
  ...
  actuator/load contact; otherwise preserve the clean geometry-scheduled
  half-cycle controller.
