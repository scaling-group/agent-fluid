...
- Do not promote half-cycle *envelope-relief redistribution* from a single
  scalar lead. One sampled variant preserved the target-directed top-down
  street and compact caudal structures, captured at `18.8265T`, and improved
  mean distance to `2.08855L`, but a direct-uniform inherited repeat with
  identical executable text after comment/whitespace removal missed at
  `0.81206L`, then exited left at `34.2320T` and `10.67789L` despite retaining
  an energetic wake and no worse peak planar load. Its target was still
  forward and the existing turn request was saturated at closest approach;
  only after the miss did the target become rearward while lateral direction
  cosine decayed from `0.361` to `0.128`, weakening the lateral-only route
  request. Classify the envelope mechanism as non-repeatable or numerically
  fragile until executable-equivalent capture repeats separate it from CFD
  variability. In this failure topology, do not weaken the proven pre-capture
  carrier or stack another phase/envelope term. The first completed rearward-
  geometry multiplier preserved capture at `18.6505T`, mean distance
  `2.09634L`, and both coherent wake views, but reconstructed forward target
  fraction stayed `0.85627--1.00000` on every row; the multiplier was exactly
  inactive, so this is non-interference evidence and not recovery evidence.
  Do not promote another multiplier from an ordinary capture. A later recovery
  test must show rearward activation followed by renewed approach; when lateral
  target magnitude is decaying, test a bounded smooth signed turn reserve
  rather than another phase/envelope term, while lateral target sign continues
  to own direction. Falsify it if target-ahead trajectories change, the
  directly-astern ambiguity chatters, demand or loads leave the capture band,
  or the same decaying-turn exit remains.
