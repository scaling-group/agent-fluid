...
- Redirect-to-cruise gait allocation has both a narrow common-envelope benefit
  and a sharp release boundary. Three geometry-scheduled samples capture at
  `18.6505--18.7550T` with mean distance `2.09340--2.09542L`; coupling the same
  anterior relief to correcting yaw remains inside that band. In contrast,
  redistributing the existing common relief by displacement half-cycle keeps
  the coherent target-bending top-down street and compact caudal Lambda2
  structures, reaches the `2--10L` thresholds earlier, lowers mean distance to
  `2.08855L`, and improves score to `-0.20041`, although capture is delayed to
  `18.8265T` and acceleration/rate contact is unchanged. Preserve common
  anterior/posterior allocation when building on this qualified improvement:
  posterior-specific enhancement during anterior relief previously kept an
  energetic wake but turned below the target, reached only `3.1465L`, and
  exited left at `9.2165L` final distance with higher posterior sweep and
  demand. Proximity alone is also an invalid release signal. A completed
  smooth distance schedule that retained redistribution above `3L` and removed
  it below `1L` lost capture before the nominal terminal regime, approached
  only `2.2485L`, then exited left at `9.8890L` and score `-10.93255`. Avoid
  further distance-only release or joint-specific wave allocation around this
  carrier. If release is retested, it must preserve target-signed curvature,
  displacement-only phase, response release, posterior lag, and projection,
  and must remain active while normalized body-frame geometry still shows the
  target longitudinally ahead; falsify it first on capture and route topology,
  then on arrival, mean distance, demand, loads, and both wake views.
