...
- Common-envelope half-cycle allocation is a provisional route improvement,
  not established demand relief. Three current direct-uniform captures using
  the geometry-scheduled envelope or its response-gated composition finish at
  `18.6505--18.6835T` with mean distance `2.09340--2.09405L`; the distinct
  sample that preserves mean relief but moves relief from the target-aligned
  displacement half-cycle to the opposed half captures at `18.8265T`, improves
  mean distance to `2.08855L`, and follows a meaningfully lower route
  (`min center y=9.6129L` versus `9.9819--10.1669L`). Both wake rows remain
  coherent; peak planar force/moment are below the geometry-scheduled copies
  and within the response-coupled trace, while rate and projected-action
  contact stay inside the capture band. Preserve common
  joint scheduling and treat the observed route change as the signal. This is
  only one redistributed rollout: replicate before promoting its `0.50`
  fraction to a durable gain, and falsify the mechanism if capture, the lower
  route, or the mean-distance separation disappears. The inherited step-16
  and step-17 score logs include `left_domain` trajectories after `3.4260L`
  and `1.0072L` closest approaches, while sampled guidance ties related
  failures to joint-specific posterior authority changes; avoid assigning
  separate redirect/cruise envelopes to the two joints even if wake coherence
  survives, and do not infer causality from the scalar logs alone.
