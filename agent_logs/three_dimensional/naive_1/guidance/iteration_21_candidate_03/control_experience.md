...
- Half-cycle *envelope-relief redistribution* remains a fragile carrier
  ...
  decayed from `0.361` to `0.128`. Classify redistribution as non-repeatable
  until executable-equivalent captures separate it from CFD variability. For
  this failure topology, preserve target-ahead carrier, half-cycle phase, and
  response release; test normalized rearward body-longitudinal geometry only
  as a bounded multiplier of the lateral-signed post-miss route request, not as
  a new turn sign or another phase/envelope term. Falsify recovery if it changes
  the current capture topology, chatters at the directly-astern ambiguity,
