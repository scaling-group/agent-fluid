...
- Treat the `15%` body-frame target-misalignment gait schedule as replicated
  redirect control, not as actuator-demand relief. Two completed ungated
  executions with identical control expressions retain the target-directed
  top-down street and compact caudal Lambda2 structures and capture in
  `18.6505--18.6835T`, ahead of the inherited clean displacement-half-cycle
  carrier at `18.8815T`; multiplying the same schedule by correcting-yaw
  response is visually and semantically neutral at `18.6615T`. Across the two
  ungated traces, reconstructed target-aligned anterior half-cycles accompany
  correcting yaw on `57.1--57.2%` of rows, versus only `10.2--11.1%` for the
  opposed half-cycle, yet posterior acceleration contact remains
  `74.8--76.3%` on opposed rows and whole-rollout rate/acceleration contact
  remains about `11%/15%` and `61%/73%`. Preserve the ungated geometry schedule
  when testing a new actuator mechanism; if seeking effort relief, test a
  bounded displacement-phase redistribution on the weak-response half-cycle
  while leaving target sign, aligned-half-cycle authority, posterior lag, and
  final projection intact. This implication applies only to the stable
  captured traveling-bend family; reject it if capture, route/wake class, or
  load history worsens, and do not count lower demand alone as improvement.
