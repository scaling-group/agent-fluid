...
- Joint-state phase is provisionally useful when it redistributes the existing
  geometry-owned curvature, but not when it merely qualifies an instantaneous
  velocity residual. Multiplying both anterior and posterior bias shares by
  one positive bounded half-cycle factor preserved the broad target-directed
  street and compact caudal Lambda2 structures, and produced the best current
  finite sample: capture at `19.0080T`, mean distance `2.09994L`, and score
  `-0.211642`, versus `19.0190T`, `2.11536L`, and `-0.226715` for the otherwise
  matched projected terminal-lateral-velocity carrier. Overall rate contact
  and peak planar load remain comparable, so this is a route-integral result,
  not demand relief, and still needs replication. In contrast, using anterior
  phase only to gate terminal velocity release retained a coherent wake but
  missed at `1.1146L`, then exited left at `32.0705T` and `8.9017L`. On this
  stable carrier, preserve target geometry as sign authority and phase-modulate
  both curvature shares together; do not infer that phase can rehabilitate a
  weak instantaneous course residual. Falsify the positive result if repeats
  lose capture, fail to retain the lower distance integral, change the wake
  class, or worsen rate/load statistics beyond the successful family.
- Range-only Van der Pol amplitude relief is not an established terminal-demand
  mechanism on the phase-steered carrier. A `20%` envelope reduction from
  `2.5L` to `1.5L` preserved the target-directed top-down street, compact
  oblique caudal structures, and capture at `19.0520T`, but below `1.5L` its
  mean speed was `0.8863L/T`, acceleration contact was `65.60%/77.98%`, and
  rate contact was `11.93%/11.01%`. These remain inside the matched carrier's
  repeat ranges of `0.8724--0.8971L/T`, `60.00--66.83%/76.24--78.24%`, and
  `8.91--10.00%/11.88--16.47%` (apart from a small anterior-rate shift), while
  maximum joint angle rose to `35.35 deg` from `31.78--34.93 deg`. The scalar
  score improvement to `-0.210566` therefore does not rescue the demand-relief
  hypothesis. In a roughly two-period terminal interval, changing the
  amplitude appearing only in the oscillator's nonlinear energy term need not
  promptly reduce its harmonic restoring or posterior tracking rate. Do not
  retune this amplitude fraction as demand relief; test a distinct continuous
  gait-rate or phase-preserving propulsion mechanism, and accept it only if
  capture and both wake views survive while terminal speed and saturation move
  outside repeat variability.
