...
- Joint velocity is not interchangeable with a useful beat-phase estimate on
  this carrier.  The inherited candidate that replaced displacement-only
  half-cycle phase with `phi1 + 0.35*phi1_dot/omega` retained an alternating
  wake and bounded public commands, but its closest approach was only
  `3.5687L`; it exited left at `28.650T` and `9.1929L`.  By contrast, the four
  currently sampled displacement-phase variants all capture at
  `18.6505--19.0520T` with mean distance `2.09340--2.10234L`, coherent
  top-down streets, and compact caudal Lambda2 structures.  Keep target
  geometry as route-sign owner and centered displacement as the half-cycle
  coordinate.  If joint speed is reused for actuator relief, confine it to a
  bounded shared gait-envelope parameter that leaves the phase coordinate and
  mean-curvature shares unchanged; falsify that separation first on capture
  and route topology, then require rate contact and RMS demand to improve
  together without larger force or moment peaks.
