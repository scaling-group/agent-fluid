...
- Completed redirect evidence supports a geometry-authoritative steering
  ...
  than an isolated outlier. The assigned parent's common-scale allocator now
  gives a concrete negative boundary: preserving the instantaneous raw
  acceleration ratio by scaling both joints whenever either exceeded the
  limit weakened and smeared the early wake, improved distance only from
  `12.3277L` to `11.5298L`, and exited left at `9.713T` and `11.7998L`.
  Its lower acceleration contact (`19.54%`/`42.87%`), zero rate contact, and
  lower peak planar force/moment (`0.01905`/`0.00950`) reflect lost propulsion
  and steering, not demand relief. Avoid global instantaneous-ratio scaling
  around this carrier; future relief must preserve the evolving joint-state
  traveling bend, target-signed curvature, capture, both wake rows, and the
  established route band rather than command ratio alone. In contrast, outward
...
- Common-envelope half-cycle redistribution has a repeatable capture and
  ...
  redistribution captures. Neither architecture therefore has established
  performance dominance. Separating displacement-phase steering from
  drive-envelope phase modulation remains the better semantic-robustness
  candidate because its seven completed captures have no known miss, while
  redistribution remains only the lower-distance-integral comparator and has
  inherited executable-equivalent misses; neither is actuator relief.
