...
- Completed redirect evidence supports a geometry-authoritative steering
  ...
  than an isolated outlier. Completed evidence now rejects broad coupled
  projection as the remedy. Scaling both raw accelerations whenever either
  exceeded the envelope reduced anterior/posterior contact to
  `19.60%/42.78%` and removed rate contact, but it also reduced joint ranges to
  `0.3425/0.4635 rad`, prevented the detached alternating top-down street and
  compact caudal Lambda2 structures from developing, reached only `11.5435L`,
  and exited left at `9.7075T` and `11.8349L`. The common scale throttled an
  in-envelope anterior carrier during posterior-only excess; lower contact was
  therefore propulsion-and-steering collapse, not relief. Avoid any allocator
  that attenuates the other joint on single-joint overdrive. If coordinated
  projection is revisited, isolate it to states where both raw commands are
  already out of envelope—the current clean captures show simultaneous
  contact on about `39.7--39.9%` of rows—and still require capture, both wake
  views, preserved joint range/rate, and the `2.08855--2.09266L` route band
  before crediting lower corner contact. In contrast, outward
