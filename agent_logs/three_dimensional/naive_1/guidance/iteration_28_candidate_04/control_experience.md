...
- Common-envelope half-cycle redistribution has a repeatable distance-integral
  ...
  boundary across inherited evaluations. Two executable-identical current
  samples retain the coherent target-bending top-down street and compact
  caudal Lambda2 structures, capture at `18.8265--18.8815T`, and reproduce
  mean distance `2.08855--2.08896L`, below five geometry-scheduled captures'
  `2.09340--2.09542L` band. They capture later, however, and sampled demand
  remains inside the carrier's roughly `61%/73%` acceleration-contact and
  `11%/15%` rate-contact bands, so faster arrival and relief are not
  established. An inherited optimizer reports an executable-equivalent
  ...
  near-miss topology. Prefer the geometry-scheduled carrier when capture
  reliability is the objective, and avoid stacking another envelope,
  distance, alignment, or response-release term: prior distance release exited
  after reaching `2.2485L`, while alignment release captured at `18.7880T`
  but worsened mean distance to `2.09961L` without relieving contact. Falsify
  this reliability boundary only with executable-equivalent redistribution
  captures across independent evaluation allocations that retain both wake
  views and the lower mean-distance band without another near miss.
...
- Geometry arbitration between capture-compatible gait modifiers has a
  completed negative boundary and a narrower next test. Full simultaneous
  ...
  repeat band (`2.09386--2.09873L`). Therefore avoid both full superposition
  and releasing redistribution across the whole ordinary target sector. If
  combination is tested again, use the reserve's actual normalized
  body-lateral activation as a complementary partition: keep redistribution
  intact before broadside and cross-fade it only as reserve authority opens.
  Falsify that narrower arbitration if capture or either wake view is lost, if
  the same left-exit topology returns, or if mean distance does not recover
  below the broadside-only band without worse actuator/load contact.
- Beat-phase curvature is necessary somewhere on the redistribution route,
  while reducing all route authority near alignment is safe but not yet
  beneficial. The completed phase-separated ablation retained displacement-
  phase envelope redistribution and an energetic target-bending top-down
  street with compact caudal Lambda2 structures, but removing phase scaling
  from both mean-curvature shares produced a `0.95507L` near miss and
  `left_domain` exit at `34.2815T`, final/mean distances
  `10.15052/8.98475L`. Its lower `8.62%/9.43%` rate contact is not relief:
  acceleration contact remained `64.88%/68.47%` and capture was lost. Retain
  displacement-phase curvature rather than repeating this full separation.
  A sibling smooth route-authority gate confined to the observed
  `|lateral_fraction| < 0.05` chatter sector did retain capture at `18.6670T`
  and both wake views, but its `2.09620L` mean distance was worse than the
  executable rollback's `2.09418L` and the sampled redistribution repeats'
  `2.08855--2.09222L`; suppressing the whole weak-error route is therefore
  capture-compatible, not an established improvement. If alignment
  qualification is revisited, preserve base target-signed mean curvature and
  localize the test to phase-dependent amplification; falsify it on lost
  capture, either degraded wake row, departure from the sampled integral band
  without a distinct semantic benefit, or worse actuator/load contact.
