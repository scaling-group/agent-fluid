...
- Do not promote half-cycle *envelope-relief redistribution* from a single
  ...
  carrier or stack another phase/envelope term; test rearward body-longitudinal
  geometry as a bounded post-miss recovery qualifier while lateral target sign
  continues to own turn direction. Falsify that recovery idea if it changes
  target-ahead capture trajectories, chatters at the directly-astern sign
  ambiguity, or reproduces the same decaying-turn exit.
