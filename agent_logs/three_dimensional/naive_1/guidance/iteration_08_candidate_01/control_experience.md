...
- Completed terminal-feedback ablations separate semantic normalization from
  compatibility with the captured first crossing. A raw body-lateral-velocity
  lead inside `2.5L` captured at `19.129T` and mean distance `2.1245L`, inside
  the unmodified controller's `19.228--19.321T` and `2.1185--2.1279L` repeat
  band. Replacing it with the geometrically cleaner target-line transverse
  velocity `target_x*velocity_y-target_y*velocity_x`, using the same bounded
  `0.12` residual and terminal gate, then lost capture: it passed about `0.90L`
  above the target, reached only `0.9490L` at `20.014T`, and exited left at
  `32.065T` with final distance `8.7907L`. Its alternating top-down street and
  compact 3D caudal wake remained coherent. Therefore neither a favorable
  transverse-speed scalar nor body-frame geometric validity licenses terminal
  velocity modulation on this carrier; later workers should leave the
  replicated closing path unmodified unless a separately isolated residual
  preserves first crossing and improves beyond repeat variability. By
  contrast, hard-projecting only completed public accelerations at
  `1800 deg/T^2` retained capture (`0.7464L` at `19.135T`) while eliminating
  over-envelope returns, but left rate contact essentially unchanged
  (`10.81%/14.46%`); use it for interface ownership, not actuator relief. The
  `80%/85%` outward-rate tapers remain a separate negative boundary: both kept
  coherent wakes yet exited after only `5.34L/5.03L` closest approaches.
  Reconsider rate-aware shaping only after it preserves carrier phase, target
  turn, and capture; falsify the projection lesson if its integrated action
  differs from the episode's identical clamp for the same finite state.
