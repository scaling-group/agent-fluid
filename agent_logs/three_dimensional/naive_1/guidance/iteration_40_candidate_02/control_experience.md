...
- Common-envelope half-cycle redistribution has a repeatable capture and
  ...
  record, but a sampled higher-Elo optimizer log supplies the falsifying
  ...
  but on `1376/3033` in the inherited clean near-miss/exit, including `315`
  rows inside `3L`. Do not treat correcting yaw alone as route improvement
  when releasing curvature. Two completed target-progress replacements retain
  capture at scores `-0.20612` and `-0.20751`, but those summaries contain no
  two-view wake, distance-integral, saturation, or load evidence and are worse
  than the current sampled scores (`-0.20288` to `-0.20041`). Treat another
  signed bearing-window-progress run as evidence completion or robustness
  replication, not a performance advance. Credit it only if multimodal repeats
  preserve the established route/load band and eliminate the below-target miss
  topology; reject it if it merely repeats an ordinary capture or score change,
