...
- Target-lateral-error gait scheduling is the first sampled improvement beyond
  that clean half-cycle carrier. Reducing only the oscillator envelope by up
  to `15%` with absolute body-frame lateral target fraction, while preserving
  differential mean curvature, displacement-only phase, response release,
  and posterior lag, retained the coherent target-directed top-down street and
  compact caudal Lambda2 structures in two executable duplicates (their only
  differences are comments): they captured at `18.6505T`/`18.6835T` with mean
  distance `2.09340L`/`2.09405L`, versus the assigned carrier's `18.8815T` and
  `2.10234L`. Coupling the same envelope schedule to the existing correcting-
  yaw response gate captured at `18.6615T`, `2.09362L`, inside that duplicate
  spread; the shared geometry schedule is supported but the extra response
  coupling is not. Treat the duplicate's roughly `0.033T` arrival and
  `0.00065L` mean-distance spread as a local repeatability floor: do not
  promote a secondary gate whose effect remains inside it. A terminal-only
  `20%` range schedule compounded with lateral-velocity lead was weaker
  (`19.0520T`, `2.09874L`) and took a larger vertical excursion; do not stack
  that compound onto the simpler mechanism. This is route/arrival evidence,
  not demand relief: reconstructed rate contact stayed near `11%/15%` and
  acceleration contact near `61%/73%` for both parent and strongest scheduled
  capture. Prefer the geometry-only schedule while this carrier already has
  target-directed motion; falsify it on loss of capture, wake coherence, or
  the `2.0934--2.0941L` repeat band, and require separately measured
  saturation reduction before calling it actuator relief.
- Posterior-specific reallocation is now a repeated negative boundary around
  that geometry-scheduled carrier, even when both wake views remain coherent.
  Compensating the posterior traveling-wave coordinate for anterior envelope
  relief preserved an energetic street but missed the target, approached only
  `3.1465L`, and exited left at `28.0775T`. Independently releasing posterior
  mean curvature more strongly than anterior curvature during correcting yaw
  also retained compact caudal structures but passed high, approached only
  `4.5218L`, and exited left at `24.8215T`; its lower `10.2%/13.3%` rate
  contact and `0.828L/T` peak speed are loss of route/propulsion authority,
  not actuator relief. Around the current carrier, preserve the common
  response gate and evidenced anterior/posterior curvature ratio rather than
  assigning redirect and cruise recovery to separate joints. Falsify this
  boundary only with a joint-role mechanism that first retains capture, the
  `2.0934--2.0941L` route band, and both wake views before comparing demand.
