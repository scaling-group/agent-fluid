...
- Target-lateral-error gait scheduling is the first sampled improvement beyond
  ...
  and posterior lag, retained the coherent target-directed top-down street
  and compact caudal Lambda2 structures and captured at `18.6505T` with mean
  distance `2.09340L`, versus the assigned carrier's `18.8815T` and
  `2.10234L`. Coupling the same envelope schedule to the existing correcting-
  yaw response gate replicated the result at `18.6615T` and `2.09362L`, so
  the shared geometry schedule is supported but the extra response coupling
  is not. A terminal-only `20%` range schedule compounded with lateral-
  velocity lead was weaker (`19.0520T`, `2.09874L`) and took a larger vertical
  excursion; do not stack that compound onto the simpler mechanism. This is
  route/arrival evidence, not demand relief: reconstructed rate contact stayed
  near `11%/15%` and acceleration contact near `61%/73%` for both parent and
  strongest scheduled capture. Prefer the geometry-only schedule while this
  carrier already has target-directed motion; falsify it on loss of capture,
  wake coherence, or the `2.0934--2.0937L` route band, and require separately
  measured saturation reduction before calling it actuator relief.
