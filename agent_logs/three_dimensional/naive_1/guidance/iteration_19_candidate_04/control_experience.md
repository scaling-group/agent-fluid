...
- Redirect-to-cruise allocation now has a repeated joint-specific negative
  boundary. Three geometry-scheduled samples capture at
  `18.6505--18.7550T` with mean distance `2.09340--2.09542L`, and coupling the
  schedule to correcting-yaw response remains inside that repeat band. In
  contrast, preserving extra posterior displacement-plus-lag authority during
  anterior relief kept both wakes coherent but reached only `3.1465L` before
  `left_domain`; sampled optimizer guidance independently reports that
  releasing posterior mean curvature more strongly than anterior curvature
  passed high, reached only `4.5218L`, and also exited. Its lower rate contact
  was therefore lost authority, not relief. Both inherited step-16 score logs
  remain in the same failure class (`3.1735L` and `3.4260L` closest approach,
  then `left_domain`), although scalar logs alone cannot assign their precise
  mechanisms. Retire posterior-specific reallocation around this carrier:
  preserve the common response gate, target-signed curvature ratio,
  displacement-only phase, and posterior lag. A demand-relief experiment is
  evidence-compatible only when it changes the common rhythmic envelope
  without assigning separate redirect/cruise roles to the joints, and it must
  first retain capture, both wake views, and the replicated route band before
  lower saturation can count as improvement. Later repeatability evidence now
  rejects displacement-half-cycle envelope redistribution as that relief
  mechanism: the same executable policy produced captures at `18.8265T` and
  `19.0520T` with mean distance changing from `2.08855L` to `2.09871L`, but a
  third repeat missed at `0.81206L` and exited with final distance `10.6779L`.
  Its response-gated variant was worse, approaching only `2.47664L` before
  exit. All retained coherent top-down streets and compact caudal Lambda2
  structures, so neither wake appearance nor a one-run integral lead makes
  beat-scale envelope allocation reusable. Retire that redistribution and do
  not stack further response gates onto it. For a geometry-carrier rollout
  that reaches roughly `1L` with a strongly lateral target while yaw is already
  correcting, test a distinct continuous approach regime that restores but
  never reverses target-signed mean curvature; falsify it on loss of the
  replicated far-route band, capture, wake coherence, or bounded load/contact
  behavior.
