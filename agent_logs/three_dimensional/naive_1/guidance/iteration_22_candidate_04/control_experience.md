...
- Half-cycle *envelope-relief redistribution* is capture-capable but not yet a
  robust improvement. Two newly sampled direct-uniform rollouts with identical
  executable policy hash both preserve the target-directed top-down street and
  compact caudal structures and capture at `18.8265T`/`18.8815T` with mean
  distance `2.08855L`/`2.08896L`; they join the earlier capture but contradict
  an inherited executable-equivalent repeat that missed at `0.81206L`, then
  exited left at `34.2320T` and `10.67789L`. The new captures do not establish
  actuation relief: their acceleration/rate contact remains about
  `60.9--61.0%`/`73.0--73.3%` and `11.0%`/`14.9%`, overlapping the clean
  carrier. Treat the mechanism as numerically fragile rather than uniformly
  failed, and do not stack another phase/envelope term or rank it from the
  small scalar lead until more executable-equivalent evidence resolves the
  one severe route-topology failure. In that failure the target was still
  forward and the turn request saturated at closest approach; only after the
  miss did the target become rearward while lateral direction cosine decayed
  from `0.361` to `0.128`. A sampled rearward-boost composition captured at
  `18.9640T`, but its reconstructed forward target fraction stayed positive
  (`0.354--1.0`) on every row, so it proves target-ahead compatibility, not
  recovery efficacy. Isolate rearward body-longitudinal qualification on the
  clean captured carrier, keep lateral target geometry as the sole turn-sign
  source, and require an actually exercised post-miss branch before promoting
  it. Falsify it if target-ahead behavior changes, direct-astern ambiguity
  chatters, or the same decaying-turn exit recurs.
