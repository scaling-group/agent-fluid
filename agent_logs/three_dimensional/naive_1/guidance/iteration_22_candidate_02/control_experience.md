...
- Half-cycle *envelope-relief redistribution* remains a fragile carrier
  modifier rather than an established improvement. The four current
  direct-uniform samples all show coherent target-directed top-down streets
  and compact caudal structures: two clean displacement-half-cycle policies
  capture at `18.6505--18.6835T` with mean distance
  `2.09340--2.09405L`, while two redistributed policies capture at
  `18.8265--18.8815T` with mean distance `2.08855--2.08896L`. That small
  integral lead is not semantic evidence for changing target-ahead propulsion.
  The assigned parent's inherited logs also contain a `0.74811L` capture and
  a near-miss/left exit that reached `0.85155L` before ending at `10.33926L`;
  separately, an executable-equivalent redistributed repeat missed at
  `0.81206L`, then exited left at `34.2320T` and `10.67789L` despite retaining
  an energetic wake and no worse peak planar load. In the latter, the target
  was still forward and route request saturated at closest approach; only
  after the miss did the target become rearward while lateral direction cosine
  decayed from `0.361` to `0.128`. The newly sampled rearward-multiplier plus
  redistribution policy captures at `18.9640T` with mean distance `2.09072L`
  and the same two-view coherent wake, but reconstructed body geometry keeps
  its forward direction cosine in `0.354--1.000`; its multiplier therefore
  never activates. Count that rollout as target-ahead compatibility only, not
  recovery evidence. Conversely, the additive rearward reserve becomes active
  after its `0.85155L` miss and still exits left, so avoid signed additive
  recovery authority around this carrier. Classify redistribution as
  non-repeatable until executable-equivalent captures separate it from CFD
  variability. For this failure topology, preserve the clean target-ahead
  carrier, displacement half-cycle phase, and response release; isolate
  normalized rearward body-longitudinal geometry only as a bounded multiplier
  of the lateral-signed post-miss route request, not as a new turn sign or
  another phase/envelope term. Falsify recovery if it changes target-ahead
  actions or capture topology, chatters at the directly-astern ambiguity,
  destroys either coherent wake view, or repeats the decaying-turn left exit;
  require at least one successful rollout with actual rearward activation
  before calling the mechanism a recovery improvement.
