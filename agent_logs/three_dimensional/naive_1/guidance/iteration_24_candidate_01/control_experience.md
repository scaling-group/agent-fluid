...
- Common-envelope half-cycle redistribution now has two executable-identical
  direct-uniform captures, so its far/middle route benefit survives direct
  replication. Both retain the coherent target-bending top-down street and
  compact caudal Lambda2 structures, capture at `18.8265--18.8815T`, and
  reproduce mean distance `2.08855--2.08896L` and score
  from `-0.20106` to `-0.20041`, below the geometry carrier's replicated
  `2.09340--2.09542L` band. The benefit is not arrival or demand relief:
  acceleration contact remains `60.85--61.00%` anterior and
  `72.97--73.27%` posterior, rate contact remains `11.04--11.07%` and
  `14.88--14.93%`, and the geometry carrier still captures sooner. Preserve
  the common redistribution as the route frontier, but keep the negative
  approach boundary: a distance release lost capture at `2.2485L`, and a
  normalized forward-alignment release captured while worsening mean distance
  to `2.09961L` without demand relief. Recovery branches must also demonstrate
  activation before receiving credit. A sampled rearward-route multiplier
  captured at `18.9640T`, but forward target fraction stayed positive on every
  row (`0.3541--1.0`), so it did not test recovery. A separate broadside
  curvature reserve did activate and retained capture at `18.7055T`, bounded
  absolute lateral target fraction to `0.6484` and kept forward fraction above
  `0.7613`, but its `2.09386L` mean distance and `-0.20576` score remained in
  the geometry-carrier band. Thus it establishes ordinary-capture safety and
  a less-broadside terminal geometry, not an independent performance gain.
  Test such a reserve only as a separable composition that leaves the common
  envelope and both target-signed curvature shares intact; falsify it on lost
  capture, loss of either coherent wake row, departure from the replicated
  redistribution distance band without a meaningful arrival/geometry gain,
  or increased actuator/load contact. Avoid further distance, alignment,
  response-release, dormant rearward, or posterior-wave compounds around this
  carrier.
