...
- Common-envelope half-cycle redistribution has a repeatable capture and
  ...
  releasing curvature. Test target-relative response as a replacement
  observation while preserving the non-inverting gate and carrier; because an
  earlier stacked response qualification merely retained capture at a worse
  score, credit the replacement only if it retains both wake views and the
  established route/load band and eliminates the below-target miss topology
  across independent runs. Reject it if it merely changes scalar score,
  repeats the miss, or increases limit contact or planar loads.
