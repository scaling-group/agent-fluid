...
- Late-route recovery authority must be allocated before overshoot and should
  not be superposed indiscriminately with another gait modifier. The sampled
  rearward-route multiplier captured at `18.9640T`, but reconstructed forward
  target fraction remained positive (minimum `0.3541`), so its recovery term
  never activated and establishes only non-interference. In contrast, a
  forward-qualified broadside mean-curvature reserve was active before
  overshoot and retained capture at `18.7055T`; it limited maximum absolute
  lateral target fraction to `0.6484`, versus `0.8873--0.9397` for the sampled
  redistribution/rearward policies, while acceleration and rate contact stayed
  in their approximate `61%/73%` and `11%/15%` bands. Its mean distance was
  worse at `2.09386L`, so this establishes redirect compatibility rather than
  a scalar or robustness improvement. The inherited full simultaneous
  composition of reserve and redistribution kept energetic top-down and
  compact caudal structures but reached only `1.90495L` and exited left at
  `31.1905T`; coherent wakes do not rescue conflicting two-joint allocation.
  When testing these mechanisms together, use normalized body-frame geometry
  to hand authority between them continuously so their full actions are
  mutually exclusive; apply this only while the target is broadside and still
  ahead. Falsify the architecture on lost capture, the same left-exit topology,
  lost two-view coherence, contact/load growth, or failure to improve a late-
  route diagnostic outside direct-uniform repeat variation.
