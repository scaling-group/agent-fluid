...
- Instantaneous terminal velocity residuals are not established improvements
  ...
  the course-residual failure; nevertheless, qualifying the existing yaw
  release with bearing progress merely retained capture at `19.1675T`, score
  `-0.23738`, and mean distance `2.12587L`, worse than its matched terminal-
  slip carrier at `19.0190T`, `-0.22672`, and `2.11536L`, with slightly higher
  rate contact. Treat yaw/bearing disagreement as a route diagnostic, not
  support for stacking another route-release gate. This boundary applies to
  stable self-propelled target approaches; overturn it only with replicated
  capture and route/load improvement beyond successful repeat variability.
- Geometry-scheduled steering-to-gait authority is a replicated route
  improvement around the displacement-half-cycle carrier, but not established
  demand relief. Reducing only the oscillator envelope by up to `15%` as the
  absolute body-frame lateral target fraction grows preserved the alternating
  top-down street and compact caudal 3D structures in two completed variants:
  the direct schedule captured in `18.6505T` with mean distance `2.09340L` and
  score `-0.206060`, while multiplying the same relief by the existing
  correcting-yaw response gate captured in `18.6615T`, `2.09362L`, and
  `-0.206097`. Both improve on the clean carrier's `18.8815T`, `2.10234L`, and
  `-0.21389`, but their `0.011T`/`0.00022L` separation is too small to credit
  the added response coupling; avoid elaborating that gate as though it were a
  distinct route mechanism. Keep displacement, not joint velocity, as the beat
  phase source: the inherited velocity-leading variant retained a coherent
  wake but approached only `3.56872L`, turned down and away, and exited left at
  `28.6495T`. Acceleration contact near `61%/73%`, rate contact
  near `11%/15%`, and RMS action near `27.15/28.62 rad/T^2` still overlap the
  clean carrier, so preserve the schedule as bounded redirect coordination,
  not evidence for stronger gains or efficiency. This implication applies to
  the captured geometry-owned, displacement-phase architecture; falsify a
  refinement if either wake view, capture topology, or the sampled
  `18.650--19.052T` / `2.0934--2.1024L` route band is lost, even if saturation
  falls.
