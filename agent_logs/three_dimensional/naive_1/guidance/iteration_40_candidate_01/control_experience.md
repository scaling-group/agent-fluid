...
- Common-envelope half-cycle redistribution has a repeatable capture and
  ...
  releasing curvature. Four completed policies replace yaw response with
  signed bearing-window progress while preserving the non-inverting gate and
  redistribution carrier. Three capture, including a newly sampled
  `18.6835T` result with coherent target-bending top-down and compact caudal 3D
  wakes, but its mean distance is `2.09750L`, outside the current
  redistribution captures' `2.08855--2.08896L` band. The fourth
  executable-equivalent policy preserves an energetic two-view wake yet
  passes below the target, reaches only `1.44371L`, and exits left at
  `30.9375T` and `8.98965L`. Its lower anterior/posterior acceleration contact
  (`58.90%/62.92%`) and rate contact (`6.77%/9.32%`) are not relief: peak
  planar force/moment rise to `0.03507/0.01792`, versus `0.03330/0.01726` in
  the sampled progress-gate capture, while semantic success is lost. Therefore
  target-bearing progress is capture-compatible but is neither a performance
  improvement nor a robust replacement for yaw response. Avoid further
  executable-equivalent progress-gate replications and do not credit lower
  limit contact in an escaping route. A different response mechanism must
  retain the target-signed non-inverting curvature, both wake rows, capture,
  and the established route/load band; falsify it on another below-target exit
  before comparing small scalar-score changes.
