...
- Do not assume that the same mean-offset sign has the same yaw effect at both
  ...
  The subsequent opposite-sign differential turn-rate servo did preserve the
  longest coherent wake and improved the sampled minimum to `4.9765L`, but its
  closest head position was `(9.427,14.458)L`, still about `5L` laterally from
  the target, before an upper-boundary exit. It also exceeded the raw
  acceleration envelope on `99.0%` of rows and touched the velocity limit on
  `35.9%`, versus `72.0%` and `5.9%` for the posterior-only rollout. Thus
  differential allocation driven by a short-window yaw-rate error is a
  concrete negative result: beat-scale yaw must not own or repeatedly reverse
  the route-steering sign merely because streamwise propulsion improves.
  Preserve posterior lag, let normalized target geometry own the sign, and
  test monotone response release on one yaw-calibrated joint before another
  differential servo. This implication applies when a coherent wake coexists
  with yaw-rate oscillations much larger than the requested mean turn; falsify
  it if one-way release fails to shift the closest approach laterally, reduce
  actuator contact, or change the same upper-exit topology.
