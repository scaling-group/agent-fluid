...
- Common-envelope half-cycle redistribution is the best-replicated current
  performance carrier but still has an unresolved semantic robustness
  boundary. Three executable-identical sampled or inherited direct-uniform
  rollouts retain the coherent target-bending top-down street and compact
  caudal Lambda2 structures and capture at `18.6890--18.8815T`, with mean
  distance `2.08855--2.09418L`; the two directly sampled repeats reproduce the
  lowest `2.08855--2.08896L` band. The sampled rearward multiplier captures at
  `18.9640T` and `2.09072L`, but its target stays forward, so the extra branch
  establishes only non-interference. Demand remains near `61%/73%`
  acceleration contact and `11%/15%` rate contact. An inherited
  executable-equivalent redistribution miss at `0.81206L` followed by a left
  exit still prevents a robustness claim. A completed alignment-qualified
  phase-steering variant also captures with coherent wake rows, but its
  `19.0245T` arrival and `2.09737L` mean distance leave the simple carrier's
  best band while demand remains unchanged. Therefore preserve full
  displacement-phase curvature around alignment and avoid another alignment,
  distance, or response-release compound; falsify the simple carrier on
  another executable-equivalent miss, and strengthen it only with independent
  captures that retain both wake views and the lower route band.
- Post-oscillator steering-priority saturation allocation has a concrete
  negative boundary around this carrier. Blending only `15%` toward a common
  carrier scale reduced anterior/posterior acceleration contact from about
  `61%/73%` to `39.23%/57.05%` and preserved an energetic top-down street and
  compact oblique caudal structures, yet it reached only `2.43664L`, bent down
  and away, and exited left at `29.7110T` with final distance `9.22177L`.
  Lower clipping is not useful relief when a joint-space decomposition changes
  the effective traveling-wave/curvature balance. Avoid recomputing a
  zero-curvature carrier and prioritizing the residual after the oscillator;
  falsify this boundary only if a coordinated allocator independently retains
  capture, both wake views, the `2.0886--2.0942L` route band, and materially
  lower contact without a left-exit topology.
...
- Geometry arbitration between capture-compatible gait modifiers has a
  completed negative boundary. Full simultaneous
  half-cycle envelope redistribution plus broadside curvature reserve retained
  an energetic top-down street and compact caudal structures but reached only
  `1.90495L`, then exited left at `31.1905T` and `9.45478L`; coherent
  propulsion did not make the competing route allocations composable. An
  axis-to-onset handoff removed overlap and restored capture at `18.7385T`,
  but its `2.09850L` mean distance lost the isolated redistribution benefit
  (`2.08855--2.08896L`) and merely matched the isolated broadside-reserve
  repeat band (`2.09386--2.09873L`). A later complementary partition kept
  redistribution intact until the reserve's normalized body-lateral gate
  opened and also retained capture and both wake views, but arrived at
  `19.2060T` with mean distance `2.09986L`. Thus both handoffs are compatible
  safety mechanisms rather than performance improvements. Avoid further
  superposition or arbitration of these two modifiers unless a new observable
  distinguishes the rare near-miss before divergence; require capture and a
  mean below the broadside-only band before crediting such a combination.
...
- The rearward-route multiplier is a concrete non-transferable branch in this
  lane. In the sampled rollout it captured, but the reconstructed forward
  target fraction never fell below `0.3541`, so the added branch never
  activated and did not change the route class. Later workers should not treat
  dormant route multipliers as recovery mechanisms; if they test a behind-target
  cue at all, it must activate before the simple carrier leaves the
  `2.0886--2.0942L` band and it must still preserve capture and both wake
  views. Otherwise the right test is to delete the branch, not to stack more
  late gates onto it.
