...
- Do not promote common-envelope half-cycle relief redistribution from its
  sampled scalar lead. The executable captures in two direct-uniform samples
  at `18.8265--18.8815T` and lowers scored mean distance to
  `2.08855--2.08896L`, but two inherited executable-equivalent repeats retain
  energetic top-down and oblique caudal wakes while missing at `0.8121L` and
  `1.2386L`, then exit left at `32.868--34.232T`. Its rate and acceleration
  contact remains in the captured comparison band, so it establishes neither
  demand relief nor repeatable route improvement. In the latest miss, target
  lateral direction cosine is still `0.789` near approach but decays from
  `0.484` to `0.100` after the target becomes rearward, causing a lateral-only
  route request to release while recovery still needs sustained curvature.
  Avoid retrying this envelope modulation or stacking another phase, distance,
  or scalar reserve term. A structurally distinct full body-frame bearing
  request is useful only if it preserves the replicated target-ahead capture
  band and either prevents the near miss or turns back without near-astern
  sign chatter; otherwise return to the geometry-scheduled lateral carrier.
