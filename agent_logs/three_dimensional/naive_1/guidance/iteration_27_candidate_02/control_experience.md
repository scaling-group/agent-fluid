...
- Common-envelope half-cycle redistribution has a repeatable distance-integral
  ...
  mean distance `2.08855--2.09222L`, below five geometry-scheduled captures'
  `2.09340--2.09542L` band. Their sampled demand stays tightly grouped at
  `60.85--61.00%`/`72.97--73.27%` acceleration contact and
  `11.01--11.07%`/`14.88--15.07%` rate contact, so faster arrival and relief
  are not established. Reconstructing all four current capture traces,
  including the dormant rearward-route variant, reveals `46--47` body-lateral
  target-sign crossings and `9.3--9.6%` of rows inside
  `|lateral_fraction| < 0.05`; this is a repeatable small-error route-chatter
  signature, not yet a demonstrated cause of score or demand. If route
  persistence is tested, isolate it to that normalized alignment sector and
  recover the captured request exactly at larger error; falsify it on lost
  capture, either degraded wake row, departure from the repeat distance band
  without a distinct benefit, or worse contact/load. An inherited optimizer
  reports an executable-equivalent
