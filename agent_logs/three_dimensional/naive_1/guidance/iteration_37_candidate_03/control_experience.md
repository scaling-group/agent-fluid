...
- Joint-state half-cycle steering is capture-class only within a now-sharper
  ...
  band, then judge demand and both wake views. A later equal blend of centered
  anterior and sign-adjusted posterior displacement gives a subtler negative
  boundary: it retains capture at `18.8485T` and both coherent wake rows, but
  worsens mean distance to `2.10603L`, outside six completed anterior-only
  clean captures (`2.09042--2.09898L`) and the current redistribution samples
  (`2.08855--2.09222L`). Its lower rate contact (`10.39%/14.30%`) and
  posterior acceleration contact (`72.05%`) are not useful relief because the
  route integral regresses. Keep centered anterior displacement as the
  steering-phase authority and posterior state in its evidenced lagged-thrust
  role; reject posterior-informed phase coordinates unless they improve route
  outside exact-policy spread while retaining capture, both views, and load
  bounds. A later observation-filtering ablation sharpens this boundary:
  rotating the normalized target vector by
...
- Common-envelope half-cycle redistribution has a repeatable capture and
  ...
  retain the `2.0886--2.0922L` band without another near miss. Later
  executable-equivalent clean-envelope evidence removes its proposed
  semantic-robustness advantage. Two additional clean executions capture at
  `18.6010T`/`2.09042L` mean distance and `19.0795T`/`2.10426L`, but the
  assigned-parent execution reaches only `1.16972L` at `19.1620T`, turns down
  to heading `1.2640 rad`, and exits left at `33.0825T` and `10.05586L`. Its
  top-down street and compact oblique caudal structures remain energetic,
  while its acceleration contact (`64.46%`/`70.41%`) and peak planar
  force/moment (`0.03067`/`0.01590`) overlap capture-class demand; wake
  coherence, ordinary loads, and lower rate contact therefore do not rescue
  the miss. Both clean separation and redistribution now have
  executable-equivalent misses, so neither is proven robust; the current three
  sampled redistribution captures at `18.7165--18.8815T` and
  `2.08855--2.09266L` support it only as the better replicated performance
  comparator. Keep envelope, terminal, and recovery compounds closed. A
  coupled demand allocator remains a distinct open ablation only if it
  preserves target-signed curvature and the instantaneous
  anterior/posterior command direction; reject lower independent clipping or
  rate contact unless capture, both wake views, the redistribution route band,
  and planar-load bounds also survive.
