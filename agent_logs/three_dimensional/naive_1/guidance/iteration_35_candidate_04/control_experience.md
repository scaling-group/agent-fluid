...
- Common-envelope half-cycle redistribution has a repeatable distance-integral
  benefit but an avoidable semantic robustness contradiction. Current
  redistribution samples retain a coherent target-bending top-down street and
  compact caudal Lambda2 structures, capture at `18.8265--18.8815T`, and have
  mean distance `2.08855--2.08896L`; acceleration and rate contact remain near
  `61%/73%` and `11%/15%`, so redistribution is not actuator relief. Against
  these captures, two inherited rollouts with the same executable bytes keep
  energetic wakes yet miss at `0.81206L` and `1.25093L` before downward/left
  exits. The previously requested isolated removal is now complete: three
  executable-identical geometry-only variants preserve target-signed
  displacement-half-cycle curvature, common geometry-owned mean relief,
  response release, and the posterior-lagged carrier; all capture at
  `18.6010--18.9585T` with mean distance `2.09042--2.09594L` and both coherent
  wake rows. Prefer that simpler geometry-only carrier as the robustness base,
  while recognizing that its mean-distance range does not beat the best
  redistribution repeats and its sampled acceleration/rate contact remains
  `60.88%/72.95%` and `10.88%/14.96%`: mechanism removal is semantic
  simplification, not demand relief. The sampled rearward multiplier also
  captures at `18.9640T` and `2.09072L`, but its target stays forward, so
  delete rather than stack that unexercised recovery branch. For a later
  demand allocator, preserve the raw two-joint direction only weakly: trace
  reconstruction shows a full common infinity-norm projection would rescale
  `94.26%` of the geometry-carrier rows with mean scale `0.512`, which is a
  carrier rewrite rather than a bounded ablation. Falsify any weak coupled
  projection on capture, both wake views, the geometry-only route band, and
  planar loads before crediting reduced nondominant limit contact; do not add
  another envelope, recovery, terminal, or rate-barrier compound at the same
  time.
