...
- Actuator relief must not be conflated with output-envelope ownership. Two
  ...
  `5.03L`, and repeated a high-side exit near `21--22T`. Do not reuse an
  outward joint-rate guard on this carrier merely because it reduces demand:
  it changes the phase/mean-curvature interaction, and wake coherence does not
  rescue route control. The episode already hard-projects acceleration onto
  `1800 deg/T^2`, so test policy-side ownership first with the identical hard
  projection and no rate taper. Reconsider a rate-aware mechanism only if a
  later controller preserves capture while independently evidencing its phase
  and steering allocation; falsify the hard-projection equivalence if its
  returned command or integrated joint trajectory differs from the episode's
  existing clamp for the same finite state.
