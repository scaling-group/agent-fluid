...
- Redirect-to-cruise gait allocation has an asymmetric negative boundary.
  ...
  left at `27.506T`, and finished at `7.2511L`. A second completed posterior-
  relief evaluation repeats the semantic failure: despite coherent structures
  in both views, it reaches only `3.9248L`, exits left at `26.2130T`, and ends
  at `6.7410L`; its lower `10.5%/10.3%` rate contact and
  `63.8%/65.9%` acceleration contact are not useful relief. Therefore reject
  posterior-specific traveling-wave allocation in either direction around
  this carrier; lower demand is not useful unless common anterior/posterior
  coupling, capture, both wake views, and the established arrival/mean-
  distance band all survive.
...
- Broadside-gated mean-curvature reserve has an activation-tested compatibility
  boundary, but recovery and performance improvement remain unproved. On the
  reliable geometry carrier, a smooth body-lateral gate directly scaled both
  target-signed curvature shares together; it activated while the target was
  still forward at about `1.687L`, retained the coherent target-directed
  top-down street and compact oblique caudal structures, and captured at
  `18.8650T` and `0.74820L`. Its `61.1%/72.7%` acceleration contact,
  `11.1%/14.9%` rate contact, `0.03329` peak planar load, and `0.01740` peak
  yaw moment remain in the carrier class. Mean distance `2.09724L` does not
  improve the geometry-carrier band, and an ordinary capture does not show
  recovery from the inherited broadside near miss. Prefer this direct,
  bounded, same-sign scaling over rearward-only gates or another multiplier
  inside an already saturated route request when testing pre-overshoot
  authority. Falsify transfer to another carrier on lost capture, loss of
  either wake row, material demand/load growth, or recurrence of the same
  activated broadside left-exit topology.
