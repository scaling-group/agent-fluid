...
- Redirect-to-cruise allocation has both a repeated joint-specific negative
  boundary and a common-envelope route-shaping result. Unequal allocation is
  consistently unsafe: extra posterior displacement-plus-lag authority during
  anterior relief reached only `3.1465L`, stronger posterior mean-curvature
  release reached `4.5218L`, and inherited step-16 variants reached
  `3.1735--3.4260L`; all exited left even when a coherent wake survived. In
  contrast, redistributing the common geometry-scheduled amplitude relief
  between displacement half-cycles preserved the target-directed top-down
  street and compact caudal 3D structures, retained capture, and improved mean
  distance to `2.08855L` and score to `-0.20041`, versus
  `2.09340--2.09405L` and scores from `-0.20606` to `-0.20578` for three
  sampled mean-envelope
  controls. It was already closer over roughly `6--16T`, but passed to the low
  side and arrived later (`18.8265T` versus `18.6505--18.6835T`), while rate
  and acceleration contact stayed essentially unchanged at about
  `11.1%/14.9%` and `60.9%/73.3%`. Thus treat displacement-half-cycle common
  envelope redistribution as route shaping, not demand relief: preserve the
  common response gate, target-signed curvature ratio, displacement-only
  phase, and posterior lag; test observed-response release of the common
  redistribution before changing joint allocation. Falsify it on capture,
  either wake view, the `2.08855L` mean-distance lead, arrival, or load and
  saturation histories rather than accepting a lower scalar alone.
