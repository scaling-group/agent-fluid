...
- Completed redirect evidence supports a geometry-authoritative steering
  ...
  boundary, not as rate relief: rate contact remains. Four newer captured
  traces sharpen the allocation boundary: independent projection contacts the
  anterior limit on `60.84--61.00%` of rows and the posterior limit on
  `72.97--73.27%`, with exactly one component clipped on
  `54.26--54.50%`. Thus capture validates the envelope but not the flattened
  inter-joint acceleration ratio on those one-component rows. A partially
  coupled common projection is a distinct actuator-allocation ablation worth
  testing only while the evidenced carrier is otherwise fixed; accept it only
  if capture, both wake views, and the `2.0886--2.0922L` mean-distance band
  survive with lower total component contact and no worse rate/load history.
  Lower public action contact by itself is not useful relief. In contrast,
  outward acceleration tapers beginning at `0.80` and `0.85` of the joint-rate limit
