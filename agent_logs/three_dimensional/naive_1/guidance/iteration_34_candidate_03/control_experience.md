...
- Common-envelope half-cycle redistribution has a repeatable distance-integral
  ...
  band and the third remains just below the geometry-scheduled carrier's
  completed spread. This wider replication establishes that
  ...
  Two assigned-parent optimizer logs now complete the clean ablation that this
  guidance previously treated as unevaluated. They preserve the target-bending
  top-down street and compact caudal structures and capture at
  `18.6395T`/`2.09383L` mean distance and `18.7220T`/`2.09898L`, extending the
  prior three clean captures to five results spanning `18.6395--18.7550T` and
  `2.09340--2.09898L`. The latter result falsifies the earlier narrower
  repeatability band. Their acceleration contact
  (`60.78--60.81%`/`73.09--73.12%`), rate contact
  (`11.05--11.09%`/`14.84--15.05%`), and peak planar force/moment remain in
  the established carrier range, so removal of redistribution is repeatedly
  capture-compatible but provides neither demand nor load relief. Prefer this
  simpler geometry-scheduled carrier as the semantic baseline while the two
  redistribution misses remain unresolved, but do not call five captures a
  proof of robustness. Avoid another
  ...
  contact. Falsify the clean baseline if an independent repeat loses capture
  or either coherent wake row; reopen redistribution only if independent
  repeats retain its lower integral without another near miss. Judge future
  mechanisms against the revised clean-carrier band rather than the obsolete
  narrower one.
...
- Route recovery must activate before the observed divergence and must not
  ...
  late: the sampled rearward multiplier captures at `18.9640T`, but its target
  remains forward, while an inherited behind-route reserve misses at
  `0.85155L` and exits left. A separate forward-qualified broadside curvature
  reserve is genuinely exercised and capture-compatible in isolation, but its
  completed means (`2.09386--2.09873L`) do not improve redistribution. Directly
