...
- Common-envelope half-cycle redistribution has a repeatable capture and
  distance-integral benefit within the current allocation but fails a clean
  semantic robustness boundary. Three executable-identical
  ...
  actuator relief and is contradicted by two inherited executable-equivalent
  misses. The earlier one reaches `0.81206L` before exiting left at `34.2320T`
  and `10.6779L`; the second bends below the target, reaches only `1.25093L`,
  and exits left at `32.6370T` and `9.94426L`. Both retain energetic top-down
  streets and compact caudal structures, and the latter's acceleration/rate
  contact (`64.27%/70.48%`, `9.22%/10.57%`) and finite loads do not explain
  away the route failure. Prefer a clean ablation back to the geometry-only
  common envelope as the next robustness discriminator: completed examples
  capture at `18.6010--18.7550T` and mean distance `2.09042--2.09542L`, but the
  current unevaluated ablation is not yet an improvement. Avoid another
  ...
  contact. Falsify the clean ablation if it loses capture, either wake row, or
  the geometry-scheduled route band; reopen redistribution only if independent
