...
- Outside-saturation broadside curvature is capture-compatible but not yet an
  attributable recovery improvement. The additive-reserve sample activates on
  47 of 3401 rows beginning at `1.5945L`, peaks at a `0.1925` smooth gate while
  the normalized target remains forward/broadside (`0.7613/-0.6484`), and
  captures at `18.7055T`, `2.09386L` mean distance, with terminal direction
  `0.9827/-0.1851`. A multiplicative reserve also captures, but later at
  `18.8650T` and worse `2.09724L`; more importantly, the exact no-reserve
  redistribution execution independently captures at `18.7110T` with nearly
  the same terminal direction (`0.9877/-0.1562`). All three demand bands
  overlap. Preserve direct target-owned curvature outside the saturated route
  argument as a bounded hypothesis, but do not increase its gain or claim that
  it caused terminal re-alignment. Falsify or establish it only with matched
  executions or a small composition that changes capture robustness, route
  topology, or distance integral beyond this variation while retaining both
  wake views and the actuator/load boundary.
