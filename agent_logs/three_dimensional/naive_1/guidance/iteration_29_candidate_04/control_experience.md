...
- Common-envelope half-cycle redistribution has a repeatable capture and
  ...
  contact. The current simple redistribution, rearward-route multiplier, and
  isolated broadside-reserve samples sharpen that negative demand boundary:
  despite different route logic and arrival at `18.7055--18.9640T`, all four
  traces spend `60.84--61.17%`/`72.97--73.27%` of rows at the
  anterior/posterior acceleration bounds and `10.85--11.07%`/`14.67--14.93%`
  at the corresponding rate bounds. Route-channel variants are therefore not
  actuator relief. Test relief as an isolated coordinated actuation mapping
  that preserves the raw two-joint command direction, rather than another
  geometry or gait compound; accept it only if capture, both coherent wake
  rows, and the `2.0886--2.0922L` mean-distance band survive while rate or load
  contact materially falls. Falsify redistribution's current status on
  another executable-equivalent miss or loss of either wake row; strengthen it
  only with independent captures inside that route band without another near
  miss.
