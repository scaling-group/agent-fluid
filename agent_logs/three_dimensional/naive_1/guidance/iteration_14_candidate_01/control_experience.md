...
- Body-frame misalignment scheduling is a capture-repeatable gait-envelope
  mechanism, but its route advantage is variable and it is not demand relief.
  With displacement-only phase, non-inverting response release,
  differential-curvature shares, posterior lag, and final action projection
  fixed, two independently completed copies of the exact executable `15%`
  lateral-fraction schedule preserve the coherent target-directed top-down
  street and compact caudal 3D structures and capture at `18.6505T` and
  `18.7990T`, with mean distances `2.09340L` and `2.10189L`. The clean carrier
  captures at `18.8815T` and `2.10234L`, so capture preservation is established
  but the second copy's route margin is inside finite variability. Coupling
  relief to correcting-yaw response also preserves the class at `18.6615T`
  and `2.09362L`, without materially changing acceleration contact
  (`60.9%/73.0%` versus `61.0%/73.2%`) or rate contact (`10.9%/14.9%` versus
  `11.0%/15.1%`). Reuse this mechanism when persistent normalized target
  misalignment should increase curvature authority relative to anterior gait,
  but require both capture and the `2.0934--2.1024L` route band before judging
  a downstream propulsion allocation; falsify it on route-class loss, either
  wake view, or worse posterior saturation and planar loads. Do not claim
  efficiency from the envelope alone, conflate it with unsuccessful range-only
  relief, or alter displacement phase: velocity-led prediction kept its wake
  but exited on the wrong route.
