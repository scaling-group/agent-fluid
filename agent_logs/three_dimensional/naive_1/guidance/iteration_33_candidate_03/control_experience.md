...
- Common-envelope half-cycle redistribution has a repeatable capture and
  ...
  The immediately inherited plateau supplies a reusable mechanism-selection
  boundary rather than support for further scalar search. A `15%`
  steering-residual-priority allocator cut anterior/posterior acceleration
  contact from roughly `60.9--61.2%`/`73.0--73.3%` to
  `39.23%`/`57.05%`, but kept an energetic two-view wake while reaching only
  `2.43664L` and exiting left. Joint-displacement target-axis compensation
  then kept capture but slowed arrival to `19.5745T`, worsened mean distance
  to `2.14814L`, and raised peak planar force/moment to
  `0.03296`/`0.01733`, outside the clean-repeat bands. Later clean selections
  and removal of an inactive rearward branch produced no new success,
  termination, route, or demand class. After this pattern, reconsulting the
  mechanism shelf does not oblige adoption: preserve the clean carrier rather
  than repeating clipping coordination, observation filtering, or gain-only
  variants. Test a future primitive only if one body-frame signal is shown to
  activate before the inherited `0.81206L` near-miss divergence and the
  ablation can independently retain capture, both wake rows, and the
  `2.0886--2.0922L` band; otherwise another clean rollout is replication, not
