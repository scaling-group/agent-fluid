...
- Common-envelope half-cycle redistribution has a repeatable capture and
  ...
  releasing curvature. Two completed step-38 policies replace yaw response
  with signed bearing-window progress while preserving the non-inverting gate
  and redistribution carrier; both retain capture, at scores `-0.20612` and
  `-0.20751` with final distances `0.74933L` and `0.74577L`. This establishes
  capture compatibility, but not route improvement: both summaries are worse
  than the current sampled scores (`-0.20288` to `-0.20041`) and provide no
  sampled two-view wake, distance-integral, saturation, or load evidence.
  Treat a further target-progress release run as evidence completion or a
  robustness replication, not as a performance advance. Credit the mechanism
  only if multimodal repeats preserve the established route/load band and
  eliminate the below-target miss topology; reject it if it merely repeats an
  ordinary capture or score change, repeats the miss, or increases limit
  contact or planar loads.
