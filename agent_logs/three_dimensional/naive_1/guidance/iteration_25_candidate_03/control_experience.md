...
- Redirect-to-cruise gait allocation now has a bidirectional posterior-wave
  negative boundary.
  ...
  `73.0--73.2%` in the current captures. The completed opposite allocation is
  more damaging: attenuating only that posterior displacement-plus-lag wave
  during redirect leaves an energetic two-view wake, yet bends below and away
  from the target, reaches only `3.92476L`, and exits left at `26.2130T` with
  final distance `6.74099L` and score `-8.24598`. Therefore preserve the
  posterior lagged wave unchanged and reject posterior-specific authority
  allocation in either direction around this carrier; lower demand is not an
  improvement unless capture and route topology survive.
- Common-envelope half-cycle redistribution is capture-class but no longer has
  a reproducible distance-integral advantage. Two executable-identical sampled
  runs preserve the coherent target-bending top-down street and compact caudal
  Lambda2 structures, capture at `18.8265--18.8815T`, and give mean distance
  `2.088545--2.088964L`; an inherited third execution of the exact same policy
  captures sooner at `18.7110T` but regresses to `2.096216L` and score
  `-0.20805`, outside both that apparent advantage and the geometry carrier's
  `2.09340--2.09542L` band. Its `60.82%/73.07%` acceleration contact and
  `11.05%/14.73%` rate contact still overlap the carrier, while another
  executable-equivalent inherited rollout misses at `0.81206L`. Therefore do
  not call redistribution faster, lower-demand, or robustly lower-integral,
  and avoid scalar tuning or another distance/alignment/response-release
  compound: distance release exits after reaching `2.2485L`, alignment release
  captures but worsens mean distance to `2.09961L`, and a rearward multiplier
  remains inactive throughout its capture.
- Outside-saturation broadside curvature has a compatibility boundary, not an
  attributable standalone advantage. The directly additive reserve activates
  on 47 of 3401 rows beginning at `1.5945L`, while the target is still forward,
  and preserves the alternating target-bending top-down street and compact
  oblique caudal structures through capture at `18.7055T`; however, its
  `2.09386L` mean distance is worse than the two current executable-identical
  redistribution captures (`2.08855--2.08896L`). Its acceleration contact
  (`60.84%/73.04%`) and rate contact (`10.85%/14.67%`) overlap those captures,
  while peak planar force (`0.03275`) and yaw moment (`0.01704`) sit at the
  upper edge, so neither demand relief nor a faster route is established. A
  current rearward-route multiplier is also only a dormant capture: its target
  remains ahead, so it cannot rebut the inherited behind-target left exit or
  the executable-equivalent redistribution miss at `0.81206L`. Two inherited
  step-24 logs also remain in the `left_domain` class after approaching only
  `1.71215L` and `1.90495L`, so the current all-capture sample must not be read
  as elimination of that semantic failure. Preserve the small direct reserve
  only as a pre-overshoot reliability mechanism composed outside route
  saturation with the unchanged common traveling wave and target-owned sign;
  do not answer the inherited misses with a larger reserve, another
  saturated-argument multiplier, or a behind-only branch. Establish the
  composition only if it retains capture, both wake views, and the sampled
  load/contact boundary while eliminating a broadside near miss or changing
  route topology beyond direct-uniform repeat variation; otherwise record the
  composition as non-beneficial rather than tuning it.
