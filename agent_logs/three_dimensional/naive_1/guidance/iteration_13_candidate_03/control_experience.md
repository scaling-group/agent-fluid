...
- Body-frame misalignment scheduling is the first sampled gait-envelope
  mechanism to move beyond that half-cycle route band, although it remains a
  one-rollout result. Holding the displacement-only phase controller,
  non-inverting response release, differential-curvature shares, posterior
  lag, and final action projection fixed, using absolute lateral target
  fraction to reduce only the rhythmic amplitude envelope by at most `15%`
  preserved the coherent target-directed top-down street and compact caudal
  3D structures, captured at `18.6505T`, and improved mean distance to
  `2.09340L`; the matched clean carrier captured at `18.8815T` and
  `2.10234L`, while the assigned parent's clean restoration captured at
  `18.8210T` with mean distance `2.10522L`. Treat this as a provisional
  redirect/cruise mechanism worth exact replication, not as a transferable
  gain or proven demand relief. It applies when persistent normalized target
  misalignment should increase curvature authority relative to gait without
  changing route sign or beat phase;
  falsify it if replication loses capture or either wake view, fails to retain
  the arrival and distance-integral lead beyond finite variability, or worsens
  saturation and planar loads. Do not conflate it with range-only amplitude
  relief, whose sampled demand statistics and route did not improve
  attribution, or with velocity-led phase prediction, which kept its wake but
  exited on the wrong route.
