...
- Completed redirect evidence supports a geometry-authoritative steering
  ...
  than an isolated outlier. A completed steering-residual-priority allocator
  now gives a concrete negative boundary: blending only `15%` toward a common
  scale on the two-joint zero-curvature carrier reduced anterior/posterior
  acceleration contact to `39.23%`/`57.05%`, while rate contact stayed near
  `10.22%`/`14.55%` and peak planar loads remained comparable, but it reached
  only `2.43664L` and exited left at `29.7110T` and `9.22177L`. Its top-down
  street and compact caudal Lambda2 structures stayed energetic as the route
  bent progressively downward, so neither wake coherence nor lower clipping
  validates an allocator that changes the evidenced carrier/steering balance.
  Avoid further common carrier scaling or residual-priority blends around
  this controller; falsify this boundary only with capture, both wake views,
  and the `2.0886--2.0922L` route band, and reject lower contact alone if
  route topology or planar loads worsen. In contrast, outward
...
- Common-envelope half-cycle redistribution has a repeatable distance-integral
  benefit among captures but now fails the semantic robustness boundary.
  Three executable-identical
  current samples retain the coherent target-bending top-down street and
  compact caudal Lambda2 structures, capture at `18.6505--18.8815T`, and have
  mean distance `2.08855--2.09222L`; two reproduce the earlier lower-integral
  ...
  actuator relief and is contradicted by two executable-equivalent misses.
  The earlier one reached `0.81206L` before exiting left at `34.2320T` and
  `10.6779L`; the newer one bends below the target, reaches only `1.25093L` at
  `19.1345T`, and exits left at `32.6370T` and `9.94426L`. Both retain an
  energetic two-view wake, and the newer failure's acceleration/rate contact
  (`64.27%/70.48%`, `9.22%/10.57%`) and peak force/moment remain finite, so
  wake coherence, clipping, and loads do not explain away the route failure.
  Prefer a clean ablation back to the geometry-scheduled half-cycle carrier as
  the next robustness discriminator. A completed inherited clean ablation is
  executable-equivalent apart from comments and adds a fourth capture at
  `18.6395T`, mean distance `2.09383L`, and minimum distance `0.74766L` to the
  prior three captures; all four now span `18.6395--18.7550T` and
  `2.09340--2.09542L`. Its inspected top-down street and compact caudal Lambda2
  structures remain coherent, but acceleration contact (`60.81%/73.12%`),
  rate contact (`11.09%/15.05%`), and peak planar force/moment
  (`0.01537/0.02894`, `0.01656`) establish no demand relief. Its score
  `-0.21138` is also below the three sampled redistribution captures' range
  of `-0.20418` to `-0.20041` even though its arrival lies in the established
  geometry-scheduled band. Therefore scalar rank cannot settle this semantic
  contradiction: treat removal as capture-compatible, not proven robust or
  more efficient, and require independent termination/trajectory-class
  replication before trading robustness for redistribution's small integral
  advantage. Capture-only inherited score logs without activation, route, or
  two-view evidence do not count as such replication. Avoid another
  ...
  contact. Falsify the ablation if it loses capture, either coherent wake row,
  or the geometry-scheduled band; reopen redistribution only if independent
  repeats retain its lower integral without another near miss.
- Cruise-aligned posterior-lag enhancement is not supported around this
  carrier. A bounded `6%` boost preserved capture at `18.7825T` and both wake
  views, but worsened mean distance to `2.10021L` versus the redistribution
  capture band, while acceleration contact (`60.79%/73.35%`), rate contact
  (`10.95%/14.73%`), peak planar force (`0.01635/0.0280`), and peak moment
  (`0.01625`) stayed comparable. Avoid further aligned lag increases as either
  performance or demand relief; falsify this negative result only with a
  distinct semantic robustness benefit plus capture, coherent wakes, and a
  mean-distance/load result outside repeat spread.
- Route recovery must activate before the observed divergence and must not
  ...
  late: the sampled rearward multiplier captures at `18.9640T` and mean
  distance `2.09072L`, but its target remains forward, so the new channel is
  not meaningfully exercised and both metrics lie inside the redistribution
  replication class. Capture alone cannot validate a conditionally inactive
  recovery mechanism. The inherited optimizer logs likewise contain only
  capture scores (`-0.21262`, `-0.21054`, and `-0.20608`) without activation or
  route histories, so they cannot resolve that attribution. Require a proposed
  recovery observation to activate before divergence and to change route
  behavior outside exact-policy repeat spread; otherwise prefer a clean
  carrier ablation. An inherited behind-route reserve misses at `0.85155L` and
  exits left. A separate forward-qualified broadside curvature reserve is
  genuinely exercised and capture-compatible in isolation, but its completed
  means (`2.09386--2.09873L`) do not improve redistribution. Directly
  ...
  therefore cannot validate compatible control allocation. A current isolated
  reserve capture sharpens why endpoint speed is also insufficient: its
  `18.7055T` arrival lies inside the redistribution repeat band, but its
  `2.09386L` mean distance lies above the best `2.08855--2.09222L` band, while
  anterior/posterior acceleration contact (`60.84%/73.04%`), rate contact
  (`10.91%/14.73%`), and peak lateral load/moment (`0.0292/0.0170`) show no
  demand or load relief over the matched redistribution captures. Treat an
  isolated reserve as a route-shape test, not a faster or gentler controller,
  unless it improves the distance integral outside repeat spread while
  preserving capture. Two geometry-based
  ...
  actuator/load contact; otherwise prefer the simpler geometry-scheduled
  half-cycle carrier while redistribution's repeated misses remain unresolved.
