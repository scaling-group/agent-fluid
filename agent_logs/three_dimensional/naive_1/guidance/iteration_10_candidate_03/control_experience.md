...
- The successful direct-uniform family bounds both a steering package and an
  interface distinction. Base, terminal body-lateral-velocity-lead, and
  final-command-projected variants all retain the target-directed alternating
  top-down street and compact caudal Lambda2 structures, capture at
  `0.7464--0.7493L` in `19.129--19.228T`, and keep mean distance within
  `2.1185--2.1277L`; the terminal lead's small timing change is therefore not
  isolated improvement beyond repeat variability. Preserve the normalized
  lateral target request, opposite-sign anterior/posterior mean curvature,
  and correctly signed yaw used only for bounded release. Exact final
  projection at the plant's existing `1800 deg/T^2` envelope is separately
  supported by two captures: it bounds public commands at
  `31.416 rad/T^2` without changing the successful route class, but joint-rate
  contact remains near `10.7--10.8%`/`14.5--14.6%`. Use that projection as
  interface ownership, not evidence of actuator relief; falsify this package
  on repeated loss of capture, a returned high/low exit, lost wake coherence,
  or materially worse rate contact and loads.
- Half-cycle steering is allocation-sensitive around this captured carrier.
  A sampled variant retained the terminal lateral-velocity lead and scaled the
  opposite-sign anterior/posterior mean-curvature pair together by a positive
  `[0.8,1.2]` factor from normalized anterior displacement. It preserved the
  coherent top-down street and compact caudal Lambda2 structures, captured at
  `19.008T`, and improved score/mean distance to `-0.21164`/`2.09994L` from
  the assigned parent's `-0.22671`/`2.11536L` at `19.019T`; peak planar force
  and moment did not increase. This is a positive trajectory signal, not
  actuator relief or replicated causality: anterior/posterior rate contact was
  `11.20%/14.99%` versus `11.08%/14.29%`. In contrast, the assigned parent's
  posterior-only phase modulation removed the terminal lead, missed capture at
  `1.219L`, and exited left at `31.570T` and `8.882L` despite an energetic
  alternating wake. Preserve target-owned sign and the coordinated curvature
  allocation when testing beat-phase steering; do not generalize the result to
  posterior-only modulation or gain escalation. Falsify the positive signal
  if repeated evaluations lose capture, leave the successful wake/route class,
  or fail to retain the distance/load improvement beyond rollout variability.
