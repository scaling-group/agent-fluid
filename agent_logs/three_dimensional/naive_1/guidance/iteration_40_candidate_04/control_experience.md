...
- Common-envelope half-cycle redistribution has a repeatable capture and
  ...
  contact. Later clean-envelope captures at `18.6010--19.0795T` and mean
  distance `2.09042--2.10426L` initially suggested a stronger robustness
  record, but a sampled lower-Elo optimizer log now supplies the falsifying
  executable-equivalent clean result: it retains an energetic two-view wake,
  passes below the target at `0.96285L`, and exits left at `33.979T` and
  `10.44465L`. The current batch simultaneously adds two redistribution
  captures at `18.8265--18.8815T` and `2.08855--2.08896L`, one clean capture
  at `18.6010T` and `2.09042L`, and only non-interference from a rearward
  branch whose target stays forward. Their overlapping acceleration/rate
  contact and planar loads still establish no actuator relief. Therefore
  neither common-envelope choice is semantically robust, and their small
  arrival/integral tradeoff is not evidence for another envelope edit. A
  seven-row reconstruction gives a reusable response boundary instead: body
  yaw has the nominal correcting sign while target bearing is non-improving on
  only `8/1603` yaw-correcting rows in the best current redistribution capture
  but on `1376/3033` in the inherited clean near-miss/exit, including 315 rows
  inside `3L`. Do not treat correcting yaw alone as route improvement when
  releasing curvature. Completed target-relative response evidence now closes
  the proposed replacement as a negative robustness result. Three policies
  with the same signed bearing-window release semantics, differing only in
  names and comments, produce two captures at `18.6835--18.7220T` and mean
  distance `2.09370--2.09750L`, but also one `left_domain` exit after reaching
  only `1.44371L`; that failure ends at `30.9375T` and `8.98965L`. Its wake is
  energetic and compact through approach, then stretches into paired
  top-down bands and loses compact oblique structure only after bypass, so it
  is route divergence rather than initial propulsion collapse or instability.
  Its peak planar force/moment (`0.03507`/`0.01792`) also exceed the current
  redistribution capture bands (`0.03066--0.03259`/`0.01603--0.01656`). Thus
  bearing progress is a useful diagnostic but neither yaw-sign nor bearing-
  progress release is semantically robust enough to deserve another
  substitution or compound, and a capture alone does not validate either.
  Preserve the simpler replicated redistribution policy as the performance
  candidate and remove inactive recovery branches; revisit response release
  only if a distinct observation is demonstrably active before divergence and
  independent multimodal repeats eliminate both miss topologies while
  retaining the `2.08855--2.08896L` route band and load envelope.
