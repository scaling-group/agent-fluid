...
- Half-cycle envelope redistribution has a small allocation-local
  distance-integral benefit but now a repeated semantic failure boundary.
  Three executable-identical current samples retain the coherent target-
  bending top-down street and compact caudal Lambda2 structures and capture at
  `18.6505--18.8815T`; two reproduce mean distance
  `2.08855--2.08896L`, below five geometry-scheduled captures'
  `2.09340--2.09542L` band, while the third reaches `2.09222L`. They do not
  establish relief because demand remains in the carrier's roughly `61%/73%`
  acceleration-contact and `11%/15%` rate-contact class. More importantly,
  the assigned parent executed the exact same policy hash, retained both
  energetic wake views, became broadside while the target was ahead, reached
  only `1.28476L`, and exited left at `32.7195T` and `9.94538L`. Together with
  the earlier executable-equivalent `0.81206L` near miss and `34.2320T` left
  exit, this is two independent coherent-wake misses, not unresolved scalar
  noise. Prefer the common geometry-scheduled envelope when capture
  reliability is the objective; removing phase redistribution is a supported
  structural rollback even though it gives up a small capture-only integral
  margin. Avoid stacking another envelope, distance, alignment, recovery, or
  ...
  distance to `2.09961L` without relieving contact. Falsify this negative
  boundary only with executable-equivalent redistribution captures across
  independent evaluation allocations that retain both wake views and the
  lower mean-distance band without another near miss.
- Behind-target route-argument feedback is structurally too late for the
  current near-miss topology. A sampled rearward multiplier captured at
  `18.9640T`, but its normalized forward target fraction stayed positive, so
  the new term never activated; the inherited behind-route reserve instead
  kept an energetic two-view wake, missed at `0.85155L`, and exited left at
  `34.0285T`. It became broadside while the target was still forward at
  `2.0134L`, and its route request was already about `0.995` of saturation.
  Therefore avoid further multipliers or reserves inside that saturated
  argument. A separate forward-qualified broadside curvature channel has one
  active capture but fails active replication. In the capture it first
  activates at `1.5945L`, peaks at only `0.03851` of curvature request near
  `0.9645L`, preserves both wake views, and arrives at `18.7055T` with mean
  distance `2.09386L`. An executable-identical inherited capture at
  `18.7440T` never exceeds lateral fraction `0.4525`, below the `0.55` onset,
  so it establishes carrier non-interference only. The assigned parent's
  active repeat also preserves an energetic top-down street and compact caudal
  Lambda2 packets, but reaches only `2.46559L` before exiting left at
  `31.7185T` and `10.32467L`; therefore the isolated reserve is not a robust
  redirect despite its one earlier-arrival trace. Full composition with
  envelope redistribution likewise kept wake coherence but exited after
  reaching `1.90495L`, while two smooth arbitration attempts captured with
  worse `2.09850--2.09986L` means. Avoid the broadside reserve and its
  compounds unless a distinct state-feedback mechanism first preserves the
  common-envelope carrier's capture topology across active repeats; another
  dormant capture is carrier evidence only. Do not stack a new terminal,
  alignment, velocity, posterior, or arbitration modifier first.
