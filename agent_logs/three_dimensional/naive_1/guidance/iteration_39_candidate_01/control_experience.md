...
- Completed redirect evidence supports a geometry-authoritative steering
  ...
  than an isolated outlier. A completed common radial allocator now falsifies
  instantaneous raw-acceleration ratio as a proxy for traveling-bend
  coordination: scaling both commands whenever either exceeded the limit cut
  anterior amplitude from `0.527` to `0.342 rad` and mean speed from `0.6874`
  to `0.2721U`. Acceleration contact fell from `60.85%/73.27%` to
  `19.54%/42.87%` and rate contact vanished, but the two-view wake became weak
  and smeared, distance improved only from `12.3277L` to `11.5298L`, and the
  fish turned off route and exited left at `9.713T` and `11.7998L`. Preserve
  the independent final projections around this carrier; do not retry global
  two-joint scaling or credit its lower contact, which is propulsion and route
  loss. Any different demand-relief mechanism must retain anterior phase-source
  amplitude, capture, both wake rows, and the established route/load band. In
  contrast, outward
...
- Instantaneous terminal velocity residuals are not established improvements
  ...
  release gate. The assigned parent's direct replacement of correcting-yaw
  response with bearing-window progress now supplies the missing isolated
  test: it retains both coherent wake views and captures at `18.7660T`, but
  worsens mean distance to `2.09589L` versus the current executable-identical
  yaw-release redistribution repeats' `2.08855--2.08896L`, lowers mean speed
  to `0.6840U` versus `0.6874--0.6897U`, and raises peak moment to `0.01697`,
  above the four current samples' `0.01603--0.01656` range. Therefore
  bearing-progress disagreement remains a diagnostic only: do not replace or
  qualify the established non-inverting yaw-response release with it unless a
  new mechanism first produces a semantic route improvement outside repeat
  spread while preserving capture, both wake rows, and the load band.
...
- Redirect-to-cruise gait allocation has an asymmetric negative boundary.
  ...
  do not treat aligned lag modulation as performance or demand relief. The
  once-distinct posterior-state phase-observation test is now also negative:
  equally blending centered anterior displacement with sign-adjusted centered
  posterior displacement retained capture and both coherent wake rows at
  `18.8485T`, but worsened mean distance to `2.10603L`, outside the clean
  anterior-phase carrier's `2.09042--2.09898L` band. Its headings at
  `4T/8T/12T/16T` (`0.499/0.294/0.359/0.367 rad`) repeatedly differ from a
  clean capture (`0.337/0.513/0.045/0.671 rad`), showing route retiming rather
  than wake failure. The blend's lower rate contact (`10.33%/14.30%` versus
  `11.00%/14.96%`) is not useful relief because the route integral worsens.
  Preserve anterior displacement as steering phase and do not let the lagged
  posterior joint vote on that phase unless a genuinely different bounded
  coordination mechanism retains capture, both wake views, and the clean
  route/load band. The opposite allocation now has completed sampled
...
- Common-envelope half-cycle redistribution has a repeatable capture and
  ...
  retain the `2.0886--2.0922L` band without another near miss. Earlier clean-
  envelope evidence had six captures at `18.6010--18.9585T` and
  `2.09042--2.09898L`, but the predicted falsification has now occurred. The
  assigned parent's exact clean policy captures more slowly at `19.0795T` and
  mean distance `2.10426L`, outside that band, while another inherited exact
  clean rollout keeps an energetic two-view wake yet reaches only `1.16972L`,
  turns down past the target, and exits left at `33.0825T` and `10.05586L`.
  Its lower rate contact (`9.06%/10.46%`) is not useful relief. In the current
  sampled batch, three executable-equivalent redistribution rollouts capture
  at `18.7165--18.8815T` and `2.08855--2.09266L`, while one clean rollout
  captures at `18.6010T` and `2.09042L`; all retain coherent top-down streets
  and compact caudal structures. Thus separating steering from envelope phase
  is no longer a semantic-robustness default, and neither envelope is proven
  robust. Preserve redistribution only as the better-replicated current
  performance carrier, and never select either envelope from one capture or a
  small score change. Both remain actuator-heavy: representative
  redistribution/clean captures overlap at about `60.85%/73.27%` and
  `60.88%/72.95%` acceleration contact, `11.07%/14.93%` and
  `10.88%/14.96%` rate contact, and peak planar force/moment
  `0.03171/0.01632` and `0.03259/0.01656`. The failed common allocator above
  rules out raw-command ratio projection as the next relief test; a future
  mechanism must instead preserve the observed joint-space traveling bend and
  cannot be credited unless capture, both wake rows, the current route band,
  and a material rate-contact or load improvement all survive.
