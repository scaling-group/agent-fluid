# Coordinated-projection half-cycle policy for the moving-window lane.
# Target geometry owns turn sign and mean relief; observed displacement phase
# redistributes effort, and a common projection preserves the two-joint command
# direction at the acceleration envelope.
...
function target_policy(state, params)
    ...
    # Desaturate the two-joint command with one common scale. Unlike independent
    # clipping, this preserves its instantaneous anterior/posterior ratio and
    # signs while placing the dominant component exactly on the same episode
    # envelope. The evidenced gait and steering gains remain unchanged.
    ...
    a1 = raw_a1 * projection_scale
    a2 = raw_a2 * projection_scale
