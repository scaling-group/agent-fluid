# Cruise-lag modulation policy for the moving-window lane.
# Target geometry owns turn sign and mean relief; observed displacement phase
# redistributes the envelope, while target alignment gates a small posterior
# lag increase without a clock.
...
function target_policy_params()
    return (
        ...
        cruise_alignment_lateral_band=0.35,
        cruise_tail_lag_boost_fraction=0.06,
...
function target_policy(state, params)
    ...
    # The inherited negative posterior-allocation evidence applies during
    # redirect. Keep that regime exactly at the parent lag, and admit a small
    # smooth lag increase only while the normalized target vector is aligned.
    # This tests posterior traveling-wave authority without changing target
    # sign, mean-curvature shares, oscillator frequency, or route memory.
    cruise_alignment_coordinate = clamp(
        1.0 - abs(lateral_fraction) / max(
            params.cruise_alignment_lateral_band,
            eps(params.cruise_alignment_lateral_band),
        ),
        0.0,
        1.0,
    )
    cruise_alignment_gate = cruise_alignment_coordinate^2 *
        (3.0 - 2.0 * cruise_alignment_coordinate)
    effective_tail_lag_gain = params.tail_lag_gain * (
        1.0 + params.cruise_tail_lag_boost_fraction * cruise_alignment_gate
    )
