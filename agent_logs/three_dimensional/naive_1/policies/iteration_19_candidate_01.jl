# Behind-aware redirect/cruise policy for the moving-window lane.
# Target geometry owns turn sign; rearward geometry preserves recovery
# authority while useful yaw response releases curvature and gait relief.
...
function target_policy(state, params)
    ...
    # A bounded lateral target direction cosine owns route sign. When a miss
    # leaves the target behind, preserve off-axis recovery authority as the
    # lateral fraction shrinks. This factor is exactly one while target-ahead,
    # and contains no world-frame heading, case identity, or memorized route.
    ...
    recovery_scale = 1.0 + clamp(
        params.behind_target_turn_boost,
        0.0,
        1.0,
    ) * behind_fraction
    route_request = tanh(
        recovery_scale * lateral_fraction /
    ...
    # Preserve the parent's gait relief while lateral error is large and yaw
    # has not responded. As correcting yaw releases curvature through the
    # one-sided gate, use that same gate to restore the rhythmic envelope
    # toward cruise without adding route authority or changing beat phase.
    amplitude_relief = clamp(
        params.turn_amplitude_relief_fraction,
        0.0,
        1.0,
    ) * abs(lateral_fraction) * response_gate
