# Coupled-projection half-cycle redistribution policy for the moving-window
# lane. Target geometry and observed displacement retain the prefilled carrier;
# one common acceleration scale preserves its raw two-joint command direction.
...
function target_policy(state, params)
    ...
    # Project the two-joint command with one positive infinity-norm scale.
    # Unlike independent clipping, this keeps the raw acceleration ratio and
    # therefore the instantaneous traveling-bend allocation while satisfying
    # the same per-joint actuator envelope owned by the policy parameters.
    command_peak = max(abs(raw_a1), abs(raw_a2))
    projection_scale = command_peak > params.acceleration_limit ?
        params.acceleration_limit / command_peak : 1.0
    a1 = projection_scale * raw_a1
    a2 = projection_scale * raw_a2
