# Error-qualified response-release policy for the moving-window lane.
# Target geometry owns turn sign and mean relief; large body-frame route error
# retains bounded curvature until an aligning yaw response can release it.
...
function target_policy_params()
    return (
        ...
        response_alignment_error_scale=1.0,
        turn_rate_limit=2.5,
...
function target_policy(state, params)
    ...
    # target error. Large lateral error retains the base curvature; release
    # grows continuously only as target alignment appears. Unlike an additive
    # recovery channel, this bounded factor cannot exceed or reverse the
    # geometry-owned turn request.
    turn_rate = clamp(
    ...
    route_error_fraction = clamp(
        ...
        max(
            params.response_alignment_error_scale,
            eps(params.response_alignment_error_scale),
        ),
        0.0,
        1.0,
    )
    alignment_release = 1.0 - route_error_fraction
    response_gate = 1.0 -
