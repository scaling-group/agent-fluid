# Target-progress-gated half-cycle redistribution for the moving-window lane.
# Body-frame geometry owns turn sign; observed bearing progress releases
# curvature while joint displacement redistributes the useful beat half.
...
function target_policy_params()
    return (
        ...
        bearing_progress_scale=1.0,
        response_release_fraction=0.35,
        bearing_progress_rate_limit=2.5,
...
function target_policy(state, params)
    ...
    # Release a bounded fraction only when the short-window body-frame bearing
    # is moving toward zero. This target-relative observation distinguishes
    # useful route response from correcting-sign yaw that does not improve the
    # bearing. Neither response can reverse the geometry-owned route sign.
    bearing_progress_rate = clamp(
        Float64(state.bearing_window_rate),
        -params.bearing_progress_rate_limit,
        params.bearing_progress_rate_limit,
    )
    correcting_response = clamp(
        -route_request * bearing_progress_rate /
        max(params.bearing_progress_scale, eps(params.bearing_progress_scale)),
        0.0,
        1.0,
    )
    response_gate = 1.0 -
