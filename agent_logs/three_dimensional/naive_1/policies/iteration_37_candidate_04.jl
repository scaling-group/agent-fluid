# Selective-corner half-cycle redistribution policy for the moving-window lane.
# Target geometry owns turn sign; joint coupling is preserved only where both
# raw acceleration demands would otherwise collapse onto a saturation corner.
...
function target_policy(state, params)
    ...
    # Preserve the proven independent projection whenever only one joint is
    # over demand: broad common scaling suppressed the anterior carrier in the
    # inherited failure. Couple only a genuine two-joint saturation corner,
    # where one positive scale preserves the raw command signs and ratio.
    both_over = abs(raw_a1) > params.acceleration_limit &&
        abs(raw_a2) > params.acceleration_limit
    corner_scale = if both_over
        params.acceleration_limit / max(abs(raw_a1), abs(raw_a2))
    else
        1.0
    end
    allocated_a1 = corner_scale * raw_a1
    allocated_a2 = corner_scale * raw_a2

    # Keep the exact componentwise guard as the public actuator contract and
    # as the unchanged behavior for all interior and single-overdrive states.
    a1 = clamp(
        allocated_a1,
