# Coupled-allocation redistribution policy for the moving-window lane.
# Target geometry and observed displacement retain the sampled route carrier;
# one final scale preserves its two-joint request direction under saturation.
...
function target_policy(state, params)
    ...
    # Project the raw two-joint request along one ray onto the same episode
    # envelope.  Unlike independent component clipping, the common positive
    # scale preserves both signs and the instantaneous anterior/posterior ratio
    # of the observed traveling-bend carrier whenever either joint saturates.
    acceleration_limit = max(
        params.acceleration_limit,
        eps(params.acceleration_limit),
    )
    peak_raw_acceleration = max(abs(raw_a1), abs(raw_a2))
    allocation_scale = min(
        1.0,
        acceleration_limit /
        max(peak_raw_acceleration, eps(peak_raw_acceleration)),
    )
    a1 = allocation_scale * raw_a1
    a2 = allocation_scale * raw_a2
