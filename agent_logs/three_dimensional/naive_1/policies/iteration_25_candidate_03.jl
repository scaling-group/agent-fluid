# Cruise-to-redirect arbitration policy for the moving-window lane.
# Normalized target geometry hands common-envelope allocation to a bounded
# broadside curvature reserve without superposing the two gait modifiers.
...
function target_policy(state, params)
    ...
    # Preserve the geometry-owned mean relief, but admit the evidenced
    # half-cycle redistribution only in the forward-aligned cruise sector.
    # It releases smoothly to zero at the broadside onset, before the separate
    # curvature reserve can activate, so both modifiers never compete for the
    # same two-joint rhythm.
    ...
    redirect_handoff = handoff_phase^2 * (3.0 - 2.0 * handoff_phase)
    cruise_allocation_gate = 1.0 - redirect_handoff
    ...
    amplitude_relief = clamp(
        ...
        (1.0 - relief_redistribution * cruise_allocation_gate * phase_alignment),
        0.0,
        1.0,
    )
