# Approach-released envelope-redistribution policy for the moving-window lane.
# Target geometry owns turn sign and mean relief; normalized target distance
# smoothly returns the added half-cycle redistribution to the proven carrier.
...
function target_policy_params()
    return (
        ...
        approach_redistribution_near_L=1.0,
        approach_redistribution_far_L=3.0,
...
function target_policy(state, params)
    ...
    # less relief and the opposed half gets more. Release only this added
    # redistribution through a distance-normalized approach smoothstep: full
    # above the far boundary and exactly the sampled carrier below the near
    # boundary. This changes neither mean-curvature shares nor posterior lag.
    ...
    approach_width = max(
        params.approach_redistribution_far_L -
            params.approach_redistribution_near_L,
        eps(Float64),
    )
    approach_coordinate = clamp(
        (distance - params.approach_redistribution_near_L) / approach_width,
        0.0,
        1.0,
    )
    approach_gate = approach_coordinate^2 * (3.0 - 2.0 * approach_coordinate)
    effective_redistribution = relief_redistribution * approach_gate
    amplitude_relief = clamp(
        mean_amplitude_relief *
        (1.0 - effective_redistribution * phase_alignment),
        0.0,
        1.0,
    )
