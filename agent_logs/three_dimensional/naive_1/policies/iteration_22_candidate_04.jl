# Broadside-reserve redirect/cruise policy for the moving-window lane.
# Target geometry owns turn sign and gates bounded recovery curvature; observed
# displacement phase preserves the clock-free traveling-bend carrier.
...
function target_policy_params()
    return (
        ...
        broadside_curvature_onset=0.60,
        broadside_curvature_reserve=0.25,
...
function target_policy(state, params)
    ...
    # Once target direction becomes broadside, the route nonlinearity is
    # already saturated. Add a smooth bounded reserve to the curvature shares
    # themselves so recovery authority appears before an overshoot. The
    # reserve is exactly inactive in the sampled ordinary-capture sector, and
    # the existing response gate still releases it when yaw is correcting.
    broadside_onset = clamp(
        params.broadside_curvature_onset,
        0.0,
        1.0,
    )
    broadside_phase = clamp(
        (abs(lateral_fraction) - broadside_onset) /
        max(1.0 - broadside_onset, eps(Float64)),
        0.0,
        1.0,
    )
    broadside_weight = broadside_phase^2 * (3.0 - 2.0 * broadside_phase)
    curvature_scale = 1.0 + clamp(
        params.broadside_curvature_reserve,
        0.0,
        1.0,
    ) * broadside_weight
    ...
    base_head_bias = -params.head_bias_limit * turn_request * curvature_scale
    base_tail_bias = params.tail_bias_limit * turn_request * curvature_scale
