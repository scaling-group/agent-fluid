# Broadside-reserve policy for the moving-window lane. Target geometry owns
# turn sign and mean relief; observed displacement phase moves rhythmic effort
# toward the useful beat half, while an early broadside error receives one
# small, releaseable curvature reserve outside the saturated route argument.
...
function target_policy_params()
    return (
        ...
        broadside_lateral_start=0.60,
        broadside_lateral_span=0.25,
        broadside_distance_floor=1.35,
        broadside_distance_span=0.65,
        broadside_head_reserve=1.5 * pi / 180,
        broadside_tail_reserve=3.75 * pi / 180,
...
function target_policy(state, params)
    ...
    # This adapter's body x-axis points aft, so the negative first component
    # is the normalized forward target fraction.
    ...
    # Give only an early, target-ahead broadside error a small reserve outside
    # the already saturated route argument. It fades out before the broadside
    # sector occupied by sampled captures and shares their non-inverting
    # correcting-response release. No elapsed time or world route is used.
    broadside_gate = clamp(
        (abs(lateral_fraction) - params.broadside_lateral_start) /
        max(params.broadside_lateral_span, eps(params.broadside_lateral_span)),
        0.0,
        1.0,
    )
    middle_distance_gate = clamp(
        (distance - params.broadside_distance_floor) /
        max(params.broadside_distance_span, eps(params.broadside_distance_span)),
        0.0,
        1.0,
    )
    target_ahead_gate = clamp(forward_fraction, 0.0, 1.0)
    reserve_request = sign(lateral_fraction) * broadside_gate *
        middle_distance_gate * target_ahead_gate * response_gate
    ...
    base_head_bias = -params.head_bias_limit * turn_request -
        params.broadside_head_reserve * reserve_request
    base_tail_bias = params.tail_bias_limit * turn_request +
        params.broadside_tail_reserve * reserve_request
