# Coupled-projection half-cycle steering policy for the moving-window lane.
# Target geometry owns turn sign and common amplitude relief; a weak shared
# projection preserves more of the raw two-joint command direction.
...
function target_policy_params()
    return (
        ...
        coupled_projection_fraction=0.15,
...
function target_policy(state, params)
    ...
    # Apply the same target-geometry relief on both observed beat halves. This
    # keeps displacement phase in steering rather than in the drive envelope.
    ...
    # Blend the validated independent projection with a weak common projection.
    # Both endpoints satisfy the same envelope. The common endpoint preserves
    # the raw two-joint command direction, while the small convex step avoids
    # replacing the captured carrier with full-vector rescaling.
    independent_a1 = clamp(
        raw_a1,
        -params.acceleration_limit,
        params.acceleration_limit,
    )
    independent_a2 = clamp(
        raw_a2,
        -params.acceleration_limit,
        params.acceleration_limit,
    )
    raw_peak = max(abs(raw_a1), abs(raw_a2), eps(params.acceleration_limit))
    coupled_scale = min(1.0, params.acceleration_limit / raw_peak)
    coupled_a1 = raw_a1 * coupled_scale
    coupled_a2 = raw_a2 * coupled_scale
    coupled_fraction = clamp(params.coupled_projection_fraction, 0.0, 1.0)
    a1 = independent_a1 + coupled_fraction * (coupled_a1 - independent_a1)
    a2 = independent_a2 + coupled_fraction * (coupled_a2 - independent_a2)
