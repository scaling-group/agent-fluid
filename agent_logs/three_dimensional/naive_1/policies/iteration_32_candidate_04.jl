# Course-qualified approach-recovery policy for the moving-window lane.
# Target geometry owns turn sign and mean relief. Observed displacement phase
# redistributes rhythmic effort until poor approach alignment smoothly returns
# the envelope toward the phase-neutral geometry carrier.
...
function target_policy_params()
    return (
        ...
        recovery_distance_onset_L=3.0,
        recovery_distance_transition_L=1.0,
        recovery_course_alignment_reference=0.75,
        recovery_course_alignment_width=0.10,
...
function target_policy(state, params)
    ...
    # Course alignment compares translational velocity with the target vector
    # in the same normalized body frame. It is used only to withdraw the
    # optional envelope redistribution on a poor near approach; it never adds
    # a velocity residual to target curvature.
    ...
    course_alignment = clamp(
        (
            Float64(state.target_body_L[1]) * velocity_x +
            Float64(state.target_body_L[2]) * velocity_y
        ) / (distance * max(velocity_norm, eps(Float64))),
        -1.0,
        1.0,
    )
    approach_coordinate = clamp(
        (params.recovery_distance_onset_L - distance) /
        max(
            params.recovery_distance_transition_L,
            eps(params.recovery_distance_transition_L),
        ),
        0.0,
        1.0,
    )
    approach_gate = approach_coordinate^2 * (3.0 - 2.0 * approach_coordinate)
    course_deficit_coordinate = clamp(
        (
            params.recovery_course_alignment_reference +
            0.5 * params.recovery_course_alignment_width -
            course_alignment
        ) / max(
            params.recovery_course_alignment_width,
            eps(params.recovery_course_alignment_width),
        ),
        0.0,
        1.0,
    )
    course_deficit_gate = course_deficit_coordinate^2 *
        (3.0 - 2.0 * course_deficit_coordinate)
    recovery_gate = approach_gate * course_deficit_gate

    # Preserve geometry-owned mean relief, but redistribute it between
    # observed displacement half-cycles only while the approach is healthy.
    # Recovery returns continuously toward the already capture-class neutral
    # envelope without changing mean curvature, posterior lag, or beat phase.
