...
# Target geometry owns turn sign; a bounded pre-overshoot curvature reserve
# composes with the observed-displacement gait without changing its tail wave.
...
function target_policy(state, params)
    ...
    # Supply a separate reserve before the target passes behind, where the
    # ordinary lateral route tanh is already saturated. The smooth gate is
    # inactive for ordinary forward alignment, and target geometry retains
    # sign ownership. Correcting yaw releases but cannot invert the reserve.
    broadside_span = max(
