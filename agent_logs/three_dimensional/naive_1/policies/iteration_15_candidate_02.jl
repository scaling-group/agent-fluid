# Joint-role-selective redirect/cruise policy for the moving-window lane.
# Target geometry owns turn sign and the gait envelope; observed correcting
# yaw releases posterior redirect bias sooner than anterior steering.
...
function target_policy(state, params)
    ...
    tail_response_gate = 1.0 -
        params.tail_response_release_fraction * correcting_response
    head_turn_request = route_request * head_response_gate
    tail_turn_request = route_request * tail_response_gate

    # Geometry still owns both bias signs. Once yaw is correcting, release the
    # larger posterior redirect share sooner so the caudal joint returns toward
    # a symmetric propulsive beat while anterior steering remains available.
