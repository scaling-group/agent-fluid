# Phase-gated differential-curvature redirect for the moving-window lane.
# Target geometry owns turn sign; joint state allocates a small extra bend to
# the useful half-cycle without changing the terminal capture controller.
...
function target_policy_params()
    return (
        ...
        halfcycle_full_distance_L=3.5,
        halfcycle_phase_velocity_scale=1.0,
        halfcycle_head_bias_limit=2.0 * pi / 180,
        halfcycle_tail_bias_limit=4.0 * pi / 180,
...
function target_policy(state, params)
    ...
    # Infer the target-supporting half-cycle from joint state rather than a
    # clock. Positive route request asks q1 to move negative; negative request
    # asks it to move positive. The extra curvature fades out before the
    # evidenced terminal lateral-velocity controller becomes active.
    phase_velocity_scale = max(
        omega * amp * params.halfcycle_phase_velocity_scale,
        eps(omega * amp),
    )
    toward_turn_phase = clamp(
        -sign(turn_request) * qd1 / phase_velocity_scale,
        0.0,
        1.0,
    )
    halfcycle_width = max(
        params.halfcycle_full_distance_L -
        params.lateral_velocity_activation_distance_L,
        eps(params.halfcycle_full_distance_L),
    )
    halfcycle_distance_gate = clamp(
        (distance - params.lateral_velocity_activation_distance_L) /
        halfcycle_width,
        0.0,
        1.0,
    )
    halfcycle_request =
        turn_request * toward_turn_phase * halfcycle_distance_gate

    # The proven opposite-sign mean curvature remains active continuously.
    # A smaller phase-gated share strengthens only the useful half-cycle while
    # retaining posterior emphasis in the thrust-producing traveling bend.
    head_bias = -params.head_bias_limit * turn_request -
        params.halfcycle_head_bias_limit * halfcycle_request
    tail_bias = params.tail_bias_limit * turn_request +
        params.halfcycle_tail_bias_limit * halfcycle_request
