# Steering-priority redistribution policy for the moving-window lane.
# Target geometry owns turn sign; bounded residual allocation preserves a
# coordinated share of the traveling-wave carrier at the acceleration limit.
...
function target_policy_params()
    return (
        ...
        steering_priority_fraction=0.15,
...
function target_policy(state, params)
    ...
    # Separate the target-curvature residual from the same envelope-modulated
    # zero-curvature carrier. This decomposition adds no route observation and
    # leaves the evidenced steering and oscillator equations unchanged.
    carrier_centered_q1 = q1
    carrier_drive = params.oscillator_mu *
        (1 - (carrier_centered_q1 / effective_amp)^2) * qd1
    carrier_a1 = carrier_drive - omega^2 * carrier_centered_q1
    carrier_phase_lag_target = -carrier_centered_q1 -
        params.tail_lag_gain * qd1 / max(omega, eps(omega))
    carrier_a2 = omega^2 * (carrier_phase_lag_target - q2) -
        2 * params.tail_damping * omega * qd2

    limit = params.acceleration_limit
    steering_a1 = clamp(raw_a1 - carrier_a1, -limit, limit)
    steering_a2 = clamp(raw_a2 - carrier_a2, -limit, limit)

    # Use one positive scale for both carrier accelerations so their direction
    # is preserved while bounded steering keeps priority inside the box limit.
    carrier_scale = 1.0
    if carrier_a1 > 0.0
        carrier_scale = min(carrier_scale, (limit - steering_a1) / carrier_a1)
    elseif carrier_a1 < 0.0
        carrier_scale = min(carrier_scale, (-limit - steering_a1) / carrier_a1)
    end
    if carrier_a2 > 0.0
        carrier_scale = min(carrier_scale, (limit - steering_a2) / carrier_a2)
    elseif carrier_a2 < 0.0
        carrier_scale = min(carrier_scale, (-limit - steering_a2) / carrier_a2)
    end
    carrier_scale = clamp(carrier_scale, 0.0, 1.0)

    priority_a1 = clamp(
        steering_a1 + carrier_scale * carrier_a1,
        -limit,
        limit,
    )
    priority_a2 = clamp(
        steering_a2 + carrier_scale * carrier_a2,
        -limit,
        limit,
    )

    # Retain most of the validated independent hard projection while making
    # the coordinated allocator a bounded, measurable architectural ablation.
    projected_a1 = clamp(raw_a1, -limit, limit)
    projected_a2 = clamp(raw_a2, -limit, limit)
    priority_fraction = clamp(params.steering_priority_fraction, 0.0, 1.0)
    a1 = clamp(
        (1.0 - priority_fraction) * projected_a1 +
        priority_fraction * priority_a1,
        -limit,
        limit,
    )
    a2 = clamp(
        (1.0 - priority_fraction) * projected_a2 +
        priority_fraction * priority_a2,
        -limit,
        limit,
    )
