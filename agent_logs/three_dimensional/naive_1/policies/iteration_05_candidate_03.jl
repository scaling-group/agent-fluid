# Actuator-compatible differential-curvature redirect for the moving-window
# lane. Body-frame target geometry still owns turn sign; one smooth output
# shoulder prevents the evidenced carrier from relying on downstream clipping.
...
function target_policy(state, params)
    ...
    # A high-order generalized soft clip is nearly transparent below the
    # actuator envelope and approaches it continuously above the knee. Apply
    # it after propulsion and steering are combined so neither layer can
    # bypass the same physical command contract.
    accel_limit = params.acceleration_limit
    soft_power = params.soft_limit_power
    soft_limit(a) = a / (
        1.0 + (abs(a) / max(accel_limit, eps(accel_limit)))^soft_power
    )^(1.0 / soft_power)

    return (phi_ddot=(soft_limit(a1), soft_limit(a2)),)
end
