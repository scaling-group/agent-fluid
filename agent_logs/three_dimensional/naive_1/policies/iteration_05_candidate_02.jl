...
function target_policy(state, params)
    ...
    # Smooth the final commands inside the policy-owned actuator envelope.
    # The high-order knee closely preserves sub-limit carrier dynamics while
    # replacing the rollout's persistent downstream hard clipping.
    acceleration_limit = max(
        params.command_acceleration_limit,
        eps(params.command_acceleration_limit),
    )
    softness = max(params.command_softness, 2.0)
    a1 = raw_a1 / (
        1 + (abs(raw_a1) / acceleration_limit)^softness
    )^(1 / softness)
    a2 = raw_a2 / (
        1 + (abs(raw_a2) / acceleration_limit)^softness
    )^(1 / softness)
