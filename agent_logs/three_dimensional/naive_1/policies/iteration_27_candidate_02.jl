# Alignment-persistent redistribution policy for the moving-window lane.
# Target geometry owns turn sign, while a narrow smooth deadband keeps weak
# beat-scale sign crossings from receiving full mean-curvature authority.
...
function target_policy(state, params)
    ...
    # Reconstructed capture traces cross the target axis 46--47 times, while
    # substantive target error is already capture-capable. Suppress only the
    # weak alignment neighborhood and recover the inherited request exactly
    # outside it. The smooth positive gate cannot invent or invert route sign.
    alignment_phase = clamp(
        abs(lateral_fraction) /
        max(params.route_alignment_deadband, eps(params.route_alignment_deadband)),
        0.0,
        1.0,
    )
    route_authority = alignment_phase^2 * (3.0 - 2.0 * alignment_phase)
    route_request = raw_route_request * route_authority
