# Geometry-authoritative redirect-allocation policy for the moving-window lane.
# Normalized target direction hands authority from cruise redistribution to a
# bounded broadside reserve while observed joint displacement supplies phase.
...
function target_policy(state, params)
    ...
    # is normalized forward target distance.
    ...
    # A separate target-signed reserve starts only after the cruise-sector
    # envelope redistribution has fully released. This bypasses route tanh
    # saturation without superposing two gait modifiers on the same joints.
    broadside_span = max(
    ...
    # Keep redirect authority inside the nominal mean-curvature allocation.
    # The reserve can use authority released by response feedback, but cannot
    # enlarge both established joint-bias shares beyond the carrier budget.
    curvature_limit = max(
        params.curvature_request_limit,
        eps(params.curvature_request_limit),
    )
    curvature_request = clamp(
        turn_request + broadside_turn,
        -curvature_limit,
        curvature_limit,
    )
    ...
    # Normalized body-frame geometry smoothly releases redistribution before
    # the broadside reserve begins, making cruise allocation and redirect
    # authority mutually exclusive without time or a hidden stage counter.
    cruise_phase = clamp(
        abs(lateral_fraction) /
        max(params.broadside_onset_fraction, eps(params.broadside_onset_fraction)),
        0.0,
        1.0,
    )
    cruise_gate = 1.0 - cruise_phase^2 * (3.0 - 2.0 * cruise_phase)
    ...
    amplitude_relief = clamp(
        ...
        (1.0 - relief_redistribution * cruise_gate * phase_alignment),
        0.0,
        1.0,
    )
