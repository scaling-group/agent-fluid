# Alignment-sector route-release policy for the moving-window lane.
# Target geometry owns turn sign and mean relief; a narrow smooth deadband
# keeps beat-scale target-axis crossings from commanding mean-curvature flips.
...
function target_policy_params()
    return (
        ...
        tail_lag_gain=0.8,
        tail_damping=0.65,
        route_alignment_deadband_fraction=0.03,
        route_alignment_full_fraction=0.08,
...
function target_policy(state, params)
    ...
    # Release mean-curvature steering only in the small alignment sector seen
    # repeatedly in the sampled captures.  The smooth gate recovers the
    # inherited route request exactly at larger body-frame target error, so it
    # neither adds a second steering channel nor retunes propulsion.
    alignment_span = max(
        params.route_alignment_full_fraction -
            params.route_alignment_deadband_fraction,
        eps(params.route_alignment_full_fraction),
    )
    alignment_phase = clamp(
        (abs(lateral_fraction) -
            params.route_alignment_deadband_fraction) / alignment_span,
        0.0,
        1.0,
    )
    alignment_gate = alignment_phase^2 * (3.0 - 2.0 * alignment_phase)
    inherited_route_request = tanh(
        lateral_fraction /
        max(params.target_lateral_scale, eps(params.target_lateral_scale)),
    )
    route_request = inherited_route_request * alignment_gate
