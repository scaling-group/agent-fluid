# Target-progress response-release policy for the moving-window lane.
# Target geometry owns turn sign and mean relief; observed displacement phase
# redistributes the useful beat, while bearing progress qualifies turn release.
...
function target_policy_params()
    return (
        ...
        bearing_response_rate_scale=1.0,
        response_release_fraction=0.35,
        bearing_response_rate_limit=2.5,
...
function target_policy(state, params)
    ...
    # Release steering only when target-relative bearing improves over the
    # observation window. Body yaw alone is insufficient because sway can make
    # bearing worse during nominally correcting rotation. The one-sided gate
    # can reduce target-signed curvature but cannot reverse it.
    bearing_response_rate = clamp(
        Float64(state.bearing_window_rate),
        -params.bearing_response_rate_limit,
        params.bearing_response_rate_limit,
    )
    correcting_response = clamp(
        -route_request * bearing_response_rate /
        max(
            params.bearing_response_rate_scale,
            eps(params.bearing_response_rate_scale),
        ),
        0.0,
        1.0,
    )
