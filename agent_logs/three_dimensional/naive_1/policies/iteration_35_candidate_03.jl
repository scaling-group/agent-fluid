# Two-joint displacement-phase steering policy for the moving-window lane.
# Target geometry owns turn sign and common amplitude relief; the observed
# traveling bend times steering without redistributing the propulsive wave.
...
function target_policy(state, params)
    ...
    # Coordinate steering with the full observed traveling bend. The carrier
    # drives posterior displacement opposite anterior displacement, so reverse
    # its sign before an equal normalized blend. Centering each joint on its
    # unmodulated target bias prevents route curvature from masquerading as
    # beat phase. The blend is displacement-only: it adds neither a clock nor
    # the velocity phase prediction that failed in inherited rollouts.
    base_centered_q1 = q1 - base_head_bias
    base_centered_q2 = q2 - base_tail_bias
    posterior_phase_fraction = clamp(
        params.posterior_phase_fraction,
        0.0,
        1.0,
    )
    bend_phase_displacement =
        (1.0 - posterior_phase_fraction) * base_centered_q1 -
        posterior_phase_fraction * base_centered_q2
    desired_bend_side = sign(base_head_bias)
    phase_alignment = clamp(
        desired_bend_side * bend_phase_displacement /
        max(amp, eps(amp)),
        -1.0,
        1.0,
    )
    ...
    # separates the evidenced steering modulation from the propulsive envelope.
