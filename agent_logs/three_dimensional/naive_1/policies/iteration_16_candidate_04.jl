# State-observed onset plus geometry-scheduled redirect/cruise policy.
# A coupled cadence boost vanishes with body speed; target geometry continues
# to own turn sign and the gait envelope without changing posterior phase.
...
function target_policy_params()
    return (
        ...
        oscillator_mu=0.35,
        onset_frequency_boost_fraction=0.06,
        onset_speed_scale=0.35,
        tail_lag_gain=0.8,
        tail_damping=0.65,
...
function target_policy(state, params)
    base_omega = 2 * pi / params.control_period
    ...
    # The direct-uniform fish must generate its own onset flow. Give a slow,
    # nearly aligned body a small cadence boost, then recover the evidenced
    # carrier exactly as normalized speed reaches the owned scale. The shared
    # omega preserves anterior/posterior phase coordination without a clock.
    body_speed = hypot(
    ...
    speed_deficit = clamp(
        1.0 - body_speed /
        max(params.onset_speed_scale, eps(params.onset_speed_scale)),
        0.0,
        1.0,
    )
    alignment_weight = clamp(1.0 - abs(lateral_fraction), 0.0, 1.0)
    frequency_boost = clamp(
        params.onset_frequency_boost_fraction,
        0.0,
        1.0,
    ) * speed_deficit * alignment_weight
    omega = base_omega * (1.0 + frequency_boost)
    turn_rate = clamp(
    ...
    # Persistent target misalignment mildly reduces the whole rhythmic
    # envelope while preserving absolute curvature shares. Keep this schedule
    # independent of the response gate: completed repeats support geometry as
    # its sufficient observation, while posterior-only compensation failed.
