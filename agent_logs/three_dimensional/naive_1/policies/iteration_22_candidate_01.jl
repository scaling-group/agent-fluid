# Posterior-relief half-cycle policy for the moving-window lane.
# Target geometry owns turn sign and mean curvature; observed lateral error
# mildly relieves only the zero-mean caudal wave during redirect.
...
function target_policy(state, params)
    ...
    # Separate target-signed posterior mean curvature from the zero-mean
    # displacement-plus-lag wave.  During redirect, attenuate only that wave;
    # the evidenced bias share and its sign remain unchanged.
    posterior_wave_target = -centered_q1 -
        params.tail_lag_gain * qd1 / max(omega, eps(omega))
    posterior_redirect_relief = clamp(
        params.posterior_redirect_relief_fraction,
        0.0,
        1.0,
    ) * abs(lateral_fraction)
    posterior_wave_scale = 1.0 - posterior_redirect_relief
    tail_centered_q2 = q2 - tail_bias
    raw_a2 = omega^2 * (
        posterior_wave_scale * posterior_wave_target - tail_centered_q2
    ) -
