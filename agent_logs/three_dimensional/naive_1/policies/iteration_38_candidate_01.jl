# Target-response-gated redistribution policy for the moving-window lane.
# Target geometry owns turn sign and mean relief; observed displacement phase
# allocates rhythmic effort, and target-bearing progress releases steering.
...
function target_policy(state, params)
    ...
    # Release curvature only when the signed target bearing actually improves
    # over the observation window. Body yaw alone is insufficient because
    # translation and sway can still make target-relative bearing worse. The
    # bounded one-sided response cannot reverse the geometry-owned route sign.
    bearing_response_rate = clamp(
        Float64(state.bearing_window_rate),
        -params.response_rate_limit,
        params.response_rate_limit,
    )
