# Near-field curvature-priority policy for the moving-window lane.
# Target geometry owns turn sign and the gait envelope; normalized distance
# adds only a bounded final-approach emphasis without changing beat phase.
...
function target_policy_params()
    return (
        ...
        approach_distance_scale_L=2.50,
        approach_turn_relief_fraction=0.10,
...
function target_policy(state, params)
    ...
    # envelope while preserving absolute curvature shares. During final
    # approach, add a small continuous increment only while that lateral
    # error remains; an aligned approach therefore retains the full gait.
    approach_gate = clamp(
        1.0 - distance / max(
            params.approach_distance_scale_L,
            eps(params.approach_distance_scale_L),
        ),
        0.0,
        1.0,
    )
    relief_fraction = clamp(
        ...
        params.approach_turn_relief_fraction,
        0.0,
        1.0,
    ) * approach_gate
    amplitude_relief = clamp(relief_fraction, 0.0, 1.0) *
