...
# A bounded amplitude boost vanishes with body speed; target geometry keeps
# ownership of turn sign without changing cadence or posterior phase lag.
...
function target_policy_params()
    return (
        ...
        oscillator_mu=0.35,
        onset_amplitude_boost_fraction=0.06,
...
function target_policy(state, params)
    ...
    # Direct-uniform release requires self-propelled onset. Give a slow,
    # nearly aligned fish a small gait-envelope boost, then recover the exact
    # parent envelope as normalized body speed rises. This cannot alter route
    # sign, oscillator cadence, displacement phase, or posterior lag.
    body_speed = hypot(
    ...
    onset_amplitude_boost = clamp(
        params.onset_amplitude_boost_fraction,
        0.0,
        1.0,
    ...
    # Persistent target misalignment mildly reduces the rhythmic envelope
    # while preserving absolute curvature shares. Slow-speed onset modulation
    # acts only on that envelope; it is exactly absent at the owned speed scale.
    ...
    effective_amp = max(
        amp * (1.0 - amplitude_relief) *
        (1.0 + onset_amplitude_boost),
        eps(amp),
    )
