# Geometry-rate-scheduled redirect/cruise policy for the moving-window lane.
# Target geometry owns turn sign and slowly schedules the gait envelope and
# rate; observed displacement phase redistributes curvature without a clock.
...
function target_policy_params()
    return (
        ...
        turn_frequency_relief_fraction=0.15,
...
function target_policy(state, params)
    ...
    # shares while slowly reducing the rhythmic envelope and natural rate.
    # Every omega-dependent term below uses the same scheduled rate, preserving
    # dimensionless posterior phase instead of clipping individual half-cycles.
    ...
    frequency_relief = clamp(
        params.turn_frequency_relief_fraction,
        0.0,
        1.0,
    ) * abs(lateral_fraction)
    effective_omega = max(
        base_omega * (1.0 - frequency_relief),
        eps(base_omega),
    )
    ...
    raw_a1 = vdp_drive - effective_omega^2 * centered_q1

    phase_lag_target = tail_bias - centered_q1 -
        params.tail_lag_gain * qd1 /
        max(effective_omega, eps(effective_omega))
    raw_a2 = effective_omega^2 * (phase_lag_target - q2) -
        2 * params.tail_damping * effective_omega * qd2
