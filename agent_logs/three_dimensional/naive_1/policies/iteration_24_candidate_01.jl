# Half-cycle redistribution plus broadside curvature reserve.
# Target geometry owns turn sign; observed displacement allocates the common
# rhythmic envelope, while a forward-target gate bounds extra redirect bias.
...
function target_policy(state, params)
    ...
    # The observation adapter's body x-axis points aft, so its negative first
    # component is normalized forward target distance.
    ...
    # The ordinary route command is already nearly saturated when the target
    # becomes broadside. Add a separate smooth reserve to mean curvature only
    # in that observed geometry. Target side still owns its sign; a rearward
    # target or correcting yaw continuously releases the reserve.
    broadside_span = max(
