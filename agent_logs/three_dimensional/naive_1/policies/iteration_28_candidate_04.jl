# Alignment-qualified phase-steering policy for the moving-window lane.
# Target geometry owns persistent curvature; observed displacement phase adds
# beat-side curvature only outside the weak alignment sector.
...
function target_policy_params()
    return (
        ...
        phase_steering_alignment_scale=0.05,
...
function target_policy(state, params)
    ...
    # Full removal of phase-dependent curvature retained an energetic wake but
    # lost capture.  Conversely, a completed route-gate rollout showed that
    # the narrow alignment sector tolerates reduced steering authority. Keep
    # the base target-owned curvature there and smoothly qualify only its
    # beat-side amplification; recover the captured carrier exactly outside.
    alignment_phase = clamp(
        abs(lateral_fraction) / max(
            params.phase_steering_alignment_scale,
            eps(params.phase_steering_alignment_scale),
        ),
        0.0,
        1.0,
    )
    phase_steering_gate = alignment_phase^2 * (3.0 - 2.0 * alignment_phase)
    phase_steering_scale = 1.0 +
        half_cycle_fraction * phase_steering_gate * phase_alignment
