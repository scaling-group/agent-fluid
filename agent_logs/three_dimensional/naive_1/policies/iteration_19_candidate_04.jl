# Approach-authority policy for the moving-window lane.
# Target geometry owns turn sign and mean relief; normalized distance restores
# non-inverting curvature authority near the target without a clock.
...
function target_policy_params()
    return (
        ...
        approach_distance_scale=1.5,
        approach_response_recovery_fraction=1.0,
        turn_rate_limit=2.5,
...
function target_policy(state, params)
    ...
    # Inside a body-length-scaled approach region, continuously restore the
    # target-signed mean curvature that correcting yaw would otherwise release.
    # The reciprocal schedule begins at approach_distance_scale and reaches
    # full configured recovery at half that distance. It never changes sign.
    approach_weight = clamp(
        params.approach_distance_scale / distance - 1.0,
        0.0,
        1.0,
    )
    response_recovery = clamp(
        params.approach_response_recovery_fraction,
        0.0,
        1.0,
    ) * approach_weight
    effective_response_release = clamp(
        params.response_release_fraction * (1.0 - response_recovery),
        0.0,
        1.0,
    )
    response_gate = 1.0 - effective_response_release * correcting_response
    ...
    # Preserve the replicated geometry-owned common rhythmic relief. Approach
    # recovery acts only on mean curvature and cannot reallocate the carrier
    # between joints or displacement half-cycles.
