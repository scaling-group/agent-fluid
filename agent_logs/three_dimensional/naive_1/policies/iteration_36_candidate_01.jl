# Coupled-demand half-cycle redistribution policy for the moving-window lane.
# Target geometry owns turn sign; a common acceleration projection preserves
# the coordinated two-joint traveling bend when raw demand reaches the envelope.
...
function target_policy(state, params)
    ...
    # Project the two raw commands with one positive scale rather than clipping
    # them independently onto a saturation corner.  This is inactive inside
    # the envelope and preserves command sign and interjoint ratio outside it,
    # so the posterior-lagged traveling bend remains coordinated.  Keep the
    # exact componentwise guard for the public actuator contract and roundoff.
    peak_raw_demand = max(abs(raw_a1), abs(raw_a2))
    demand_scale = min(
        1.0,
        params.acceleration_limit /
        max(peak_raw_demand, eps(params.acceleration_limit)),
    )
    allocated_a1 = demand_scale * raw_a1
    allocated_a2 = demand_scale * raw_a2
    a1 = clamp(
        allocated_a1,
        -params.acceleration_limit,
        params.acceleration_limit,
