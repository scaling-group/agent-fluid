# Coupled-allocation half-cycle policy for the moving-window lane.
# Target geometry and observed displacement retain the established route;
# one common actuator projection preserves the two-joint traveling bend.
...
function target_policy(state, params)
    ...
    # Project the two-joint demand with one positive scale.  Unlike independent
    # component clipping, this preserves the instantaneous anterior/posterior
    # ratio and therefore the directed traveling-bend shape under saturation.
    peak_demand = max(
        abs(raw_a1),
        abs(raw_a2),
        eps(params.acceleration_limit),
    )
    allocation_scale = clamp(
        params.acceleration_limit / peak_demand,
        0.0,
        1.0,
    )
