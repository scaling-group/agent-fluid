# Phase-selective redirect/cruise policy for the moving-window lane.
# Target geometry owns turn sign; observed displacement preserves the
# replicated aligned half-cycle and relieves only opposed rhythmic effort.
...
function target_policy_params()
    return (
        ...
        opposed_gait_relief_fraction=0.05,
...
function target_policy(state, params)
    ...
    # Preserve the replicated geometry-scheduled relief over the whole beat.
    # Add only a bounded displacement-phase relief on the target-opposed
    # half-cycle, where inherited and sampled trajectories show little useful
    # correcting yaw and persistent posterior acceleration contact. The
    # target-aligned half-cycle remains exactly on the evidenced carrier.
    base_amplitude_relief = clamp(
        params.turn_amplitude_relief_fraction,
        0.0,
        1.0,
    ) * abs(lateral_fraction)
    opposed_phase = clamp(-phase_alignment, 0.0, 1.0)
    opposed_amplitude_relief = clamp(
        params.opposed_gait_relief_fraction,
        0.0,
        1.0,
    ) * abs(lateral_fraction) * opposed_phase
    amplitude_relief = clamp(
        base_amplitude_relief + opposed_amplitude_relief,
        0.0,
        1.0 - eps(Float64),
    )
