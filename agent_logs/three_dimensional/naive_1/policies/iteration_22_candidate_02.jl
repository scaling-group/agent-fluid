# Geometry-scheduled redirect/recovery policy for the moving-window lane.
# Target geometry owns turn sign and post-miss authority; observed displacement
# phase redistributes bounded curvature without a clock or velocity-led phase.
...
function target_policy(state, params)
    ...
    # is the normalized forward target direction. Preserve the proven
    # target-ahead carrier exactly, but keep lateral route authority from
    # fading after an overshoot when the target moves behind the fish. The
    # lateral component remains the sole sign source, so this smooth product
    # is still zero at the directly-astern ambiguity.
    forward_fraction = clamp(
        -Float64(state.target_body_L[1]) / distance,
        -1.0,
        1.0,
    )
    rearward_fraction = clamp(-forward_fraction, 0.0, 1.0)
    recovery_scale = 1.0 +
        clamp(params.rearward_route_boost, 0.0, 1.0) * rearward_fraction
    route_request = tanh(
        lateral_fraction * recovery_scale /
