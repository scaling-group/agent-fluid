# Phase-leading half-cycle steering for the 3D moving-window lane.
# Target geometry owns turn sign; normalized anterior joint state anticipates
# the useful half-cycle while preserving the traveling-bend carrier.
...
function target_policy(state, params)
    ...
    # A bounded lateral direction cosine owns route sign; it contains no
    # world-frame heading, case identity, or memorized route.
    ...
    # Correcting yaw can release, but never invert, geometry-owned curvature.
    turn_rate = clamp(
    ...
    # Predict a short-horizon bend coordinate from normalized oscillator
    # position and velocity. This is a joint-state phase observation, not an
    # external clock. Unit normalization keeps the phase coordinate bounded
    # independently of the chosen lead fraction.
    base_centered_q1 = q1 - base_head_bias
    desired_bend_side = sign(base_head_bias)
    phase_lead = clamp(params.phase_lead_fraction, 0.0, 1.0)
    phase_coordinate = (
        base_centered_q1 / max(amp, eps(amp)) +
        phase_lead * qd1 / max(omega * amp, eps(omega * amp))
    ) / sqrt(1.0 + phase_lead^2)
    phase_alignment = clamp(
        desired_bend_side * phase_coordinate,
        -1.0,
        1.0,
    )
