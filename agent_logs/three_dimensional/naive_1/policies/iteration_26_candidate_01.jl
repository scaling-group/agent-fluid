# Complementary cruise-to-redirect policy for the moving-window lane.
# One normalized body-frame gate cross-fades useful half-cycle envelope
# allocation into bounded broadside curvature authority.
...
function target_policy(state, params)
    ...
    # Preserve the geometry-owned mean relief and the evidenced half-cycle
    # envelope redistribution throughout the ordinary target sector. Once
    # the broadside reserve actually opens, its own smooth gate cross-fades
    # redistribution away. The two modifiers therefore cannot both reach
    # full authority, while cruise allocation is not released prematurely.
    mean_amplitude_relief = clamp(
        params.turn_amplitude_relief_fraction,
        0.0,
        1.0,
    ) * abs(lateral_fraction)
    cruise_allocation_gate = 1.0 - broadside_gate
