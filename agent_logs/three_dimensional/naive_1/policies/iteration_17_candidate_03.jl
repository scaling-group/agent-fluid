# Bend-budget redirect/cruise policy for the moving-window lane.
# Target geometry trades a bounded part of rhythmic excursion for matched
# mean curvature while preserving the captured traveling-bend carrier.
...
function target_policy_params()
    return (
        ...
        redirect_curvature_reallocation_fraction=0.35,
...
function target_policy(state, params)
    ...
    # Geometry alone owns this schedule: sampled response coupling remained
    # inside the executable-replication band.
    ...
    # Allocate a bounded fraction of the relieved anterior excursion to both
    # target-signed mean-curvature shares. One common scale preserves the
    # evidenced anterior/posterior ratio and recovers exactly at alignment.
    freed_amplitude = max(amp - effective_amp, 0.0)
    redirect_scale = 1.0 + clamp(
        params.redirect_curvature_reallocation_fraction,
        0.0,
        1.0,
    ) * freed_amplitude /
        max(params.head_bias_limit, eps(params.head_bias_limit))
    base_head_bias = -params.head_bias_limit * turn_request * redirect_scale
    base_tail_bias = params.tail_bias_limit * turn_request * redirect_scale
