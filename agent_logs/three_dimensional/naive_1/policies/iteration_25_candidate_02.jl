# Curvature-budgeted broadside redirect for the moving-window lane.
# Target geometry owns turn sign and the gait envelope; broadside authority
# may fill, but never exceed, the established normalized curvature budget.
...
function target_policy(state, params)
    ...
    # This adapter's body x-axis points aft, so negate its first component to
    # obtain the normalized forward target fraction used by the redirect gate.
    ...
    # A persistent broadside target can need authority outside the saturated
    # route argument. Gate a target-signed reserve while the target is ahead,
    # and pass it through the same non-inverting correcting-response release.
    broadside_span = max(
    ...
    # Projecting the combined request prevents the additive reserve from
    # recreating the inherited over-budget composition failure.
    curvature_limit = max(
    ...
    # Use the sampled geometry schedule rather than composing the redirect
    # with the inherited fragile half-cycle envelope redistribution. Large
    # lateral error mildly reduces the common rhythmic envelope while leaving
    # both curvature shares and the posterior traveling-wave lag intact.
