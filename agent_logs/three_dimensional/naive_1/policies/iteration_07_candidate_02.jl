# Projected terminal lateral-motion redirect for the moving-window lane.
# Target geometry owns turn sign; target-relative lateral motion modulates only
# terminal bend magnitude, and completed commands own the physical envelope.
...
function target_policy(state, params)
    ...
    # Target side owns the route sign. Near the target, helpful lateral motion
    # releases some curvature and adverse motion restores it without inversion.
    ...
    # Correctly signed yaw may release only part of the target-owned request.
    # Beat-scale yaw cannot invert steering while the target stays on one side.
    turn_rate = clamp(
    ...
    # This exact projection already occurs in the joint integrator. Owning it
    # here bounds the public command without rate-dependent phase distortion.
