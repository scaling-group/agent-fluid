# Active-envelope-normalized redirect/cruise policy for the moving-window
# lane. Target geometry owns turn sign and gait relief; observed displacement
# phase is normalized by that scheduled envelope without velocity-led phase.
...
function target_policy(state, params)
    ...
    # Preserve the parent's response-coupled gait relief: redirect authority
    # takes priority while lateral error persists, and the rhythmic envelope
    # returns toward cruise as useful correcting yaw appears.
    ...
    # Infer beat side from displacement alone. Normalizing by the active gait
    # envelope keeps half-cycle steering authority coherent during relief,
    # without adding a clock, velocity lead, or another route signal.
    base_centered_q1 = q1 - base_head_bias
    desired_bend_side = sign(base_head_bias)
    phase_alignment = clamp(
        desired_bend_side * base_centered_q1 / effective_amp,
        -1.0,
        1.0,
    )
