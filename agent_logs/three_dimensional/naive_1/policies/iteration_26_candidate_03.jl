# Broadside handoff policy for the moving-window lane. Target geometry owns
# turn sign and continuously hands authority from half-cycle envelope
# redistribution to a bounded redirect reserve without a clock.
...
function target_policy(state, params)
    ...
    # The adapter's body x-axis points aft, so negate its longitudinal target
    # component to obtain the normalized forward target direction.
    ...
    # Smoothly select the sampled broadside-redirect architecture only while
    # the target is both lateral and still ahead. The same gate will remove
    # envelope redistribution below, avoiding the inherited failed full stack.
    broadside_span = max(
    ...
    redirect_handoff = broadside_gate * target_ahead_gate
    ...
    broadside_turn = broadside_direction *
        params.broadside_curvature_reserve_fraction * redirect_handoff *
        response_gate
