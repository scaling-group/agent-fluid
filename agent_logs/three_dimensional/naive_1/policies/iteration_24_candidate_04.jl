# Broadside-reserve plus half-cycle redistribution policy.
# Target geometry owns turn sign; a forward-qualified reserve adds redirect
# authority outside route saturation while joint phase allocates common effort.
...
function target_policy(state, params)
    ...
    # obtain a normalized forward target fraction.
    ...
    # Add a separate bounded reserve only as the forward target becomes
    # broadside. It bypasses the saturated route argument but retains target
    # sign and the same non-inverting response release.
    broadside_span = max(
