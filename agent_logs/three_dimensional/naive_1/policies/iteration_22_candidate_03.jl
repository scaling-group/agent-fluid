# Rearward-aware redirect/cruise policy for the moving-window lane.
# Lateral target geometry owns turn sign; rearward geometry can preserve route
# magnitude after a miss without changing target-ahead gait or beat phase.
...
function target_policy(state, params)
    ...
    # no world-frame heading, case identity, or memorized route. This adapter's
    # body x-axis points aft, so the negative first component is target-forward.
    ...
    # Preserve target-ahead actions exactly. After an overshoot, rearward
    # geometry can at most double the pre-tanh lateral route argument, while
    # the smooth product remains zero at the directly-astern ambiguity.
    rearward_fraction = clamp(-forward_fraction, 0.0, 1.0)
    recovery_scale = 1.0 + clamp(
        params.rearward_route_boost,
        0.0,
        1.0,
    ) * rearward_fraction
    route_request = tanh(
