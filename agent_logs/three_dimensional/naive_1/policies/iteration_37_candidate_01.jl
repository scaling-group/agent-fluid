...
# lane. Target geometry owns turn sign while one common bounded allocation
# preserves the raw anterior/posterior command direction.
...
function target_policy(state, params)
    ...
    # Radially project the two-joint command into the episode's infinity-norm
    # envelope. A common scale preserves the raw acceleration ratio, so the
    # anterior steering and posterior-lag carrier are not independently clipped.
    ...
    a1 = raw_a1 * allocation_scale
    a2 = raw_a2 * allocation_scale
