# Full-bearing redirect/cruise policy for the moving-window lane.
# Body-frame target direction owns bounded mean curvature and the gait
# envelope; observed displacement phase organizes steering without a clock.
...
function target_policy(state, params)
    ...
    # Full signed body-frame bearing owns route sign and magnitude. Unlike a
    # lateral direction cosine alone, bearing retains recovery authority if a
    # near miss leaves the target rearward while its lateral cosine shrinks.
    # This contains no world-frame heading, case identity, or memorized route.
    ...
    bearing_scale = max(
        params.target_bearing_scale,
        eps(params.target_bearing_scale),
    )
    bearing_magnitude = atan(abs(lateral_fraction), forward_fraction)
    rearward_fraction = max(-forward_fraction, 0.0)
    astern_direction = abs(tanh(lateral_fraction / bearing_scale))
    bearing_authority = 1.0 -
        rearward_fraction * (1.0 - astern_direction)
    target_bearing = sign(lateral_fraction) *
        bearing_magnitude * bearing_authority
    route_request = tanh(
        target_bearing / bearing_scale,
    )
