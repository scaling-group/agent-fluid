...
# Target geometry owns turn sign.  The final projection matches the plant's
# physical acceleration clamp without disturbing the captured carrier phase.
...
function target_policy(state, params)
    ...
    # The episode applies this same hard envelope before joint integration.
    # Projecting here removes unrealizable returned demand while preserving the
    # exact applied action of the sampled capture controller.
    acceleration_limit = max(
        params.joint_acceleration_limit,
        eps(params.joint_acceleration_limit),
    )
    a1 = clamp(raw_a1, -acceleration_limit, acceleration_limit)
    a2 = clamp(raw_a2, -acceleration_limit, acceleration_limit)
