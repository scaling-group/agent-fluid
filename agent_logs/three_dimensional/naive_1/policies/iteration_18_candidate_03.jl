# Response-released half-cycle envelope policy for the moving-window lane.
# Target geometry owns turn sign and mean relief; observed displacement phase
# moves rhythmic effort toward the useful beat half until correcting yaw
# releases that redistribution without a clock.
...
function target_policy(state, params)
    ...
    # less relief and the opposed half gets more. Once target-signed yaw is
    # already responding, the same non-inverting response gate releases this
    # extra asymmetry without changing mean-curvature shares or posterior lag.
    ...
    relief_redistribution = clamp(
        params.half_cycle_relief_redistribution * response_gate,
        0.0,
        1.0,
    )
