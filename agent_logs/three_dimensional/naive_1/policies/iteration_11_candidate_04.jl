# Approach-rate-relieved half-cycle redirect for the moving-window lane.
# Target geometry owns turn sign; observed joint phase redistributes bounded
# curvature while normalized range continuously slows the terminal gait.
...
function target_policy_params()
    return (
        ...
        approach_cycle_rate_relief_fraction=0.10,
...
function target_policy(state, params)
    ...
    cycle_rate_relief = clamp(
        params.approach_cycle_rate_relief_fraction,
        0.0,
        1.0,
    ) * approach_gate
    omega = max(base_omega * (1.0 - cycle_rate_relief), eps(base_omega))
    ...
    # Use the same range-scheduled rate in the lag normalization and tail
    # tracker. This slows one continuous traveling bend rather than clipping
    # individual high-rate samples or introducing a clocked terminal mode.
