# Energy-regulated redistribution policy for the moving-window lane.
# Target geometry and displacement phase retain the sampled route carrier;
# normalized joint-state energy regulates the anterior rhythmic injection.
...
function target_policy(state, params)
    ...
    # Regulate the carrier from its complete normalized phase-plane state.
    # Unlike displacement-only Van-der-Pol injection, this term stops adding
    # energy when joint speed already makes the observed cycle energetic.
    # Velocity does not choose a steering half-cycle or alter target sign.
    energy_target = max(
        params.oscillator_energy_target,
        eps(params.oscillator_energy_target),
    )
    normalized_displacement = centered_q1 / effective_amp
    normalized_velocity = qd1 / max(omega * effective_amp, eps(effective_amp))
    phase_energy = normalized_displacement^2 + normalized_velocity^2
    energy_drive = params.oscillator_mu *
        (energy_target - phase_energy) * qd1
    raw_a1 = energy_drive - omega^2 * centered_q1
