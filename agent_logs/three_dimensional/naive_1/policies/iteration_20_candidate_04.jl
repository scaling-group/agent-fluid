# Behind-reserve redirect/cruise policy for the moving-window lane.
# Target geometry owns turn sign; a smooth rearward reserve maintains recovery
# authority without changing target-ahead gait, beat phase, or posterior lag.
...
function target_policy_params()
    return (
        ...
        behind_target_turn_reserve=0.30,
        behind_target_direction_scale=0.20,
...
function target_policy(state, params)
    ...
    # A bounded lateral target direction cosine owns route sign. If a miss
    # leaves the target behind, add a smooth signed reserve so turn authority
    # does not remain proportional only to a shrinking lateral fraction. The
    # reserve is exactly zero while the target is ahead and vanishes
    # continuously at the directly-astern ambiguity.
    ...
    behind_fraction = max(-forward_fraction, 0.0)
    recovery_direction = tanh(
        lateral_fraction /
        max(
            params.behind_target_direction_scale,
            eps(params.behind_target_direction_scale),
        ),
    )
    route_argument = lateral_fraction +
        clamp(params.behind_target_turn_reserve, 0.0, 1.0) *
        behind_fraction * recovery_direction
    route_request = tanh(
        route_argument /
