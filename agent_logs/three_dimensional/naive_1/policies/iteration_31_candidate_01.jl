# Phase-preserving acceleration-allocation policy for the moving-window lane.
# Preserve the captured half-cycle redistribution carrier while partially
# coupling final two-joint projection toward its raw acceleration ratio.
...
function target_policy_params()
    return (
        ...
        coupled_projection_fraction=0.25,
    )
end
...
function target_policy(state, params)
    ...
    # Independent clipping is capture-compatible but changes the requested
    # inter-joint ratio whenever only one component saturates. A common scale
    # preserves that ratio. Blend the projections conservatively so this
    # actuator-allocation ablation does not replace the evidenced carrier.
    ...
    raw_peak = max(abs(raw_a1), abs(raw_a2))
    common_scale = min(
        1.0,
        params.acceleration_limit /
        max(raw_peak, eps(params.acceleration_limit)),
    )
    coupled_a1 = common_scale * raw_a1
    coupled_a2 = common_scale * raw_a2
    coupled_fraction = clamp(
        params.coupled_projection_fraction,
        0.0,
        1.0,
    )
    a1 = (1.0 - coupled_fraction) * independent_a1 +
        coupled_fraction * coupled_a1
    a2 = (1.0 - coupled_fraction) * independent_a2 +
        coupled_fraction * coupled_a2
