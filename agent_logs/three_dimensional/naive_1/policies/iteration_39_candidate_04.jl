# Target-progress-release policy for the moving-window lane.
# Target geometry owns turn sign and mean relief; observed displacement phase
# moves rhythmic effort toward the useful beat half without a clock, while
# body-frame bearing progress releases steering only after route response.
...
function target_policy(state, params)
    ...
    # Release steering only when the observed body-frame target bearing is
    # moving toward zero. Body yaw alone can look corrective while lateral
    # translation worsens the route. The one-sided release remains bounded and
    # cannot reverse the target-owned turn sign.
    bearing_rate = clamp(
