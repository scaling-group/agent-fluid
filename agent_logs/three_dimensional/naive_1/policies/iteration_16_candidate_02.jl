# Response-gated redirect/cruise policy for the moving-window lane.
# Target geometry owns turn sign; observed yaw response restores the posterior
# traveling wave after redirect without changing beat phase or mean curvature.
...
function target_policy_params()
    return (
        ...
        posterior_redirect_relief_fraction=0.15,
...
function target_policy(state, params)
    ...
    # During redirect, de-emphasize only the zero-mean posterior wave while
    # preserving both target-signed mean-curvature shares. Correcting yaw
    # continuously restores the caudal wave toward cruise. This is the bounded
    # opposite of the inherited posterior-wave enhancement that kept a strong
    # wake but lost the route.
    posterior_relief = clamp(
        params.posterior_redirect_relief_fraction,
        0.0,
        1.0,
    ) * abs(lateral_fraction) * response_gate
    posterior_wave_scale = 1.0 - posterior_relief
    posterior_wave_target = -centered_q1 -
        params.tail_lag_gain * qd1 / max(omega, eps(omega))
    phase_lag_target = tail_bias +
        posterior_wave_scale * posterior_wave_target
