...
- Treat sending/receiving reserve gates as joint-quality safeguards, not a
  ...
  jointly preserved margin and load quality. Two completed alternative
  representations now separate a useful safeguard from a failed actuator.
  Projecting directional angle reserve by one oscillator-normalized joint-
  velocity horizon improved miss to `0.421L`, eliminated both joints' angle
  dwell, and lowered peak normalized force/moment to `0.03628/0.01798` while
  preserving the compact wake, but slowed capture to `15.5850T/-0.02349`.
  Replacing the cubic residual with an envelope-sharing posterior wave target
  lowered loads further to `0.03506/0.01724` but widened miss to `0.725L`,
  retained `0.174%` posterior dwell, and delayed capture to
  `15.8431T/-0.02283`; do not tune posterior offset or amplitude contraction
  further on this carrier. New completed evidence now also falsifies the
  tempting distance-scheduled projection: activating the same joint-motion
  look-ahead only through the inherited near-target gate recovered arrival to
  `15.2650T/-0.01575` and kept loads at `0.03781/0.01864`, but widened miss
  from the fixed projection's `0.421L` to `0.529L` and restored `0.360%`
  posterior dwell instead of zero. Its direct route and compact alternating
  wake were unchanged. Thus distance scheduling exchanges reserve protection
  for translation; it does not combine their benefits, and more distance-gate
  or projection-gain tuning is not supported. If the cubic residual is kept,
  test a distinct response-coupled completion signal that credits observed
  corrective motion rather than another joint-capacity convention, and retain
  it only if capture, compact wake, miss at or below `0.461L`, posterior dwell
  at or below `0.681%`, roughly `0.040/0.019` peak loads, and the
  `15.14--15.34T` arrival class improve together; otherwise remove the cubic
  posterior correction rather than continue allocator edits.
