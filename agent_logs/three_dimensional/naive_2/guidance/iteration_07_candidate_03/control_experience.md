...
- Distinguish body pointing from translational course when diagnosing a fast
  near miss. At the full-vector parent's `2.319L` closest approach, its head-to-
  target vector was `(0.511,-2.262)L` while velocity was
  `(-0.956,-0.579)L/T`; in its body frame the target angle was about
  `-0.722 rad` but the velocity-course angle was about `+0.526 rad`, even as
  yaw rate reached `-2.581 rad/T`. Thus more yaw authority or a posture-only
  burst release can coexist with motion across the wrong side of the target.
  The assigned parent's later scalar-only log reached `2.211L` yet retained
  `left_domain` termination and provides no policy or visual trace, so a closer
  scalar pass alone does not establish a reusable mechanism. Inside an
  approach band and only at resolved speed, test the wrapped angle between
  normalized body-frame target and velocity directions to retain steering
  reserve until actual course responds; keep far-field propulsion unchanged.
  Falsify this implication if course conditioning fails to improve closest
  approach or termination, increases load/rate-limit occupancy, or harms the
  coherent alternating wake.
