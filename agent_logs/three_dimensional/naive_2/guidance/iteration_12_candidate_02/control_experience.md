...
- Phase-compensating body yaw before response feedback is the first sampled
  ...
  and wake coherence.
- A smaller miss and quieter joints do not establish a valid terminal hold,
  and correcting its static curvature sign is still not a capture mechanism.
  After course-residual steering reached `1.276L`, a closing-speed-gated
  damped-curvature handoff narrowed the miss to `1.008L`, removed all dwell
  above `40 deg`, and limited peak normalized planar force/moment to about
  `0.028/0.018`; nevertheless it crossed at `1.011L/T` and escaped lower-left.
  Its `+7.6 deg` mean bend produced a wrong-side `+0.201 rad/T`
  carrier-separated yaw residual. Reversing that terminal bend changed the
  recovery topology to a nearly target-height left exit (`y=9.614L`) and
  retained the cruise wake, but worsened the closest pass to `1.106L`; there,
  both joints had settled near `-8.1 deg`, heading response was already
  correct-sign, yet the target remained `0.99L` above the head and course
  error was near `pi/2` as distance began increasing. Avoid further static
  bend-sign, magnitude, or coasting-hold edits: they act after cross-target
  momentum has fixed the miss. Test a body-frame course/intercept gate that
  releases a bounded redirect when the predicted pass enters the capture
  corridor, and require capture or a closer pass plus improved termination;
  falsify it if wake, joint reserve, loads, or far-field translation regress.
