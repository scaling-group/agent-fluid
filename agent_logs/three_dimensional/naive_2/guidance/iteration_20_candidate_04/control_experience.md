...
- Promote body-frame constant-course predicted miss with response-released
  mean curvature, rather than a posterior pulse, as the first replicated
  capture mechanism on this carrier.  The sampled phase-separated pursuit
  prefill retained a coherent alternating wake but climbed to `15.202L`,
  missed by `4.650L`, and exited left; replacing that navigation structure
  produced two byte-identical captures at `15.983--15.994T` and a
  response-released-pulse capture at `16.016T`.  Inherited evidence also
  records a no-pulse sibling capture at `16.011T`, following earlier
  `0.96311L` and `0.81002L` predicted-miss near passes.  The capture family
  retained a compact alternating 3D wake, zero `>40 deg` angle dwell, peak
  normalized planar force/moment below `0.035/0.018`, and similar rate-limit
  occupancy.  Preserve normalized target/velocity time-to-closest and signed
  miss, the small bounded carrier mean, and carrier-separated measured-response
  handoff; remove the pulse when isolating the reusable mechanism and avoid
  pulse-gain iteration unless repeated ablations establish a benefit.  Falsify
  this lesson under reflected or perturbed geometry, loss of repeat capture,
  a changed trajectory class, wake degradation, or joint/load growth.
