...
- Preserve joint-state allocation on the direct compact-wake carrier, but
  define capacity from the incremental residual rather than the total carrier
  action. Tail-only cubic redirect captured at `15.1403T/-0.01094` with
  `0.651L` head-relative predicted miss and `1.269%` posterior `>40 deg` dwell;
  absolute tail-reserve transfer centered the course to `0.362L` but slowed to
  `15.4464T` and exchanged limit exposure (`0.285/0.996%` anterior/posterior
  dwell). Signed residual-direction capacity was the best sampled compromise:
  it retained capture at `15.3385T/-0.01502`, reduced miss to `0.461L`, removed
  anterior dwell, lowered posterior dwell to `0.681%`, and kept peak normalized
  planar force/moment at `0.03959/0.01889`. By contrast, gating that same
  residual with previous total-action phase widened miss to `0.631L` and raised
  posterior dwell to `1.878%`; the additive residual multiplies
  `abs(carrier)`, so previous carrier direction is not its acceleration sign.
  The assigned parent's absolute-tail/signed-receiver spillover eliminated
  both joints' dwell but widened miss to `0.516L` at `15.2823T/-0.01727` and
  comparable `0.03988/0.01906` loads, showing that safe shedding alone gives
  back centered authority. Avoid further previous-action phase gates, absolute
  receiver gates, and scalar residual tuning on this carrier. Test predictive
  signed reserve from phase-normalized joint state (for example
  `q + qdot/omega`) and retain it only if capture, the direct coherent wake,
  miss `<=0.461L`, zero anterior dwell, posterior dwell `<0.681%`, and the
  approximately `0.040/0.019` load class improve together; otherwise preserve
  the instantaneous signed allocator or change the actuator mechanism.
