...
- Keep carrier-phase separation in the response gate and require measured
  geometry to agree before releasing rhythmic redirect authority. The assigned
  parent released shared half-cycle steering on corrective yaw alone while
  geometry-qualified only its posterior pulse; it captured at `16.02701T`,
  scored `-0.02335`, and integrated `1.90579L` of distance. Applying the same
  response-plus-predicted-miss consensus to both rhythmic channels preserved
  the compact alternating 3D wake and comparable joint/load quality, but
  captured `0.526T` earlier with the best sampled score (`-0.02109`) and
  distance integral (`1.90236L`). Its peak normalized planar force/moment was
  `0.0365/0.0180`, neither joint dwelled beyond `40 deg`, and near-rate
  occupancy remained about `17.8/17.0%`. An always-on posterior pulse and a
  raw lateral-corridor release with that pulse also captured at
  `15.983/15.921T`, but scored worse; the corridor route crossed lower and
  introduced `0.276%` posterior angle dwell. Thus response-plus-geometry
  consensus is reusable on this established carrier, while extra retained
  authority or a particular residual is not automatically better. Inherited
  related executions still missed at `1.01--1.22L`, and replacing measured
  geometry with an angle-domain carrier estimate missed at `0.8293L` before
  exiting left. Preserve raw normalized body-frame target/course geometry;
  test actuator-matched completion residuals without gain changes, and retain
  them only if repeated capture or closest-pass termination, route, wake,
  joint reserve, and loads improve together.
