...
- Preserve measured target-versus-course error as the strongest evidenced
  ...
  Together with the sampled `2.595L` full-circle controller that pinned both
  joints at `-45 deg`, this is a reusable negative boundary: preserve the
  course-responsive traveling carrier, but do not stack more mean-bend gain,
  static hold, or scalar drive relief to solve the last `0.25--0.42L`.
  Instead test a distinct state-feedback semantic such as bounded body-frame
  velocity compensation in the terminal aim or response-gated dynamic
  redirection. Falsify that direction if it loses the `1.033L` inherited
  approach, repeats the lower-left exit, or increases dwell/load without
  capture.
