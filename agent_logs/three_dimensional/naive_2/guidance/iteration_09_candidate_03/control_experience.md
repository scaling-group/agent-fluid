...
- Phase-compensating body yaw before response feedback is the first sampled
  ...
  and wake coherence.
- The sampled terminal schedulers show that approach relief must preserve the
  carrier-to-steering authority ratio and must be judged by interception, not
  score alone. Bearing-gated carrier-only relief made the closest pass
  (`2.319L`) but reached target x `2.595L` high, pinned both joints near their
  angle limits for `11.7%` of samples, and exited the upper boundary. Releasing
  that relief from accumulated bend eliminated simultaneous pinning and lowered
  peak planar force from `0.688` to `0.488`, yet worsened the pass to `2.664L`
  and retained the same exit; continuous target-plane hold pinned both joints
  for `18.8%` and reached only `2.703L`. Joint-angle phase separation then
  improved scalar score and final distance while degrading closest approach to
  `4.650L`, crossing target x at `y=14.555L`, and repeating the exit. Avoid
  scaling only the restorative carrier or accepting phase compensation from
  score; on this traveling-bend carrier, test body-frame rotation invariants
  such as target/velocity course error and co-scale the complete rhythmic
  command for a near miss. Retain that implication only if it improves capture
  geometry or termination without sacrificing far-field translation, wake
  coherence, actuator reserve, or load quality; a different carrier or an
  unreliable near-zero velocity invalidates the course-error assumption.
...
- Rotation-invariant target/velocity course error produced the first inherited
  sub-`2L` pass on this carrier: `1.276L` versus `2.664--4.650L` for the current
  sampled policies, and it changed the crossing from high to below the target.
  It was still a near miss followed by left-domain exit and `10.936L` final
  distance. Replacing it with an aligned whole-gait hold then regressed the pass
  to `3.401L` with the same termination, so do not treat broad approach braking
  as a substitute for course feedback. At the `1.276L` pass the target-line
  rate had grown to about `-0.77 rad/T` while one angle and the other joint rate
  approached their limits; for terminal refinement, test a bounded body-frame
  target/velocity line-of-sight-rate residual that leaves the far-field carrier
  intact. Retain it only if capture geometry or termination improves without
  sacrificing translation, wake coherence, actuator reserve, or load quality;
  unreliable velocity or a moving target invalidates the stationary-target
  rate construction.
