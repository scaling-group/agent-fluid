...
- Treat a capture with rapidly rotating target line as a tangential-margin
  failure even when the route and wake look unchanged. Relative to the unified
  handoff's `15.5008T/-0.02109/0.179L` arrival, score, and terminal predicted
  miss, active carrier-yaw arrest, course-confirmed release, centered amplitude
  relief, terminal duty skew, and unused-authority hydrodynamic-moment rejection
  all retained the direct compact-wake capture but ended with `0.679--0.747L`
  miss and scores from `-0.02175` to `-0.02329`; duty skew also restored
  `0.138%` posterior `>40 deg` dwell, while moment rejection raised peak
  normalized planar force slightly to `0.03746`. The centered sample ended
  near `0.411/T` absolute inertial target-line rate, whereas the duty-skew,
  yaw-arrest, and amplitude-relief samples ended near `1.57--1.70/T`. This
  rejects more scalar or gate refinements of static bend, pulse phase, yaw
  residual, drive envelope, duty ratio, or moment residual on this carrier.
  A later worker may test a distinct bounded target-relative dynamic signal,
  but retain it only if capture and terminal course margin improve together
  without losing arrival, route, wake coherence, joint reserve, or load class;
  the target-line-rate separation is diagnostic evidence, not yet causal proof.
- Do not promote a faster capture produced by extra posterior rhythmic
  authority unless terminal margin and tail reserve survive. Against the
  persistent-line parent's `15.7829T/-0.02023/0.747L` arrival, score, and
  predicted miss, a cubed predicted-miss tail residual reached capture at
  `15.1403T` and improved score to `-0.01094`, but miss remained `0.651L`,
  above its own `0.590L` boundary; peak normalized planar force rose from
  `0.03553` to `0.04041`, posterior `>40 deg` dwell rose from zero to `1.2695%`,
  and the terminal posterior rate pinned at `260 deg/T`. The unchanged direct
  route and compact wake identify actuator allocation, not propulsion, as the
  remaining issue. Avoid strengthening or scalar-tuning this tail residual on
  the present carrier. A later worker may test anterior-steering/posterior-
  thrust role separation, but retain it only if capture miss drops below
  `0.590L` and joint/load quality improves without sacrificing arrival or wake.
