...
- Treat terminal reserve as a current joint phase-space prediction problem,
  not an action-sign proxy or another spillover threshold. Same-carrier,
  direct-quiescent captures retain visually comparable direct routes and
  compact alternating 3D wakes while separating a real geometry/reserve
  trade: tail-only cubic predicted miss arrived at `15.1403T` with `0.651L`
  head-relative course miss and `1.269%` posterior `>40 deg` dwell; fully
  directional reserve reached `15.3385T/0.461L` with `0/0.681%` dwell and
  `0.03959/0.01889` peak normalized planar force/moment. Delayed-action gating
  widened miss to `0.631L`, raised posterior dwell to `1.878%`, and increased
  peak moment to `0.01922`. Receiver-safe shedding then produced `0.516L` once
  but repeated at `0.619L` with higher `0.04087/0.01968` loads; analytic
  carrier-phase gating also stayed at `0.619L` and worsened posterior dwell to
  `2.233%`. Avoid more action-scale, carrier-sign, sender/receiver threshold,
  or absolute-tail hybrid tuning on this carrier.

  Projecting observed angle by oscillator-normalized velocity is the first
  reserve mechanism to improve terminal course, angle dwell, and loads
  together: the inherited linear phase-space projection captured with
  `0.421L` miss, zero `>40 deg` dwell in both joints, and
  `0.03628/0.01798` loads. Its boundary is translation: speed at the `3L`
  approach fell from `1.379U` for directional reserve to `1.256U`, delaying
  capture to `15.5850T`. A distinct envelope-preserving posterior wave target
  lowered loads but instead widened miss to `0.725L` and slowed capture to
  `15.8431T`, so do not combine or retune that actuator. Preserve projected
  directional reserve while testing how its forecast transports state; retain
  a new projection only if the direct compact-wake capture keeps miss at or
  below `0.421L`, prevents anterior dwell and limits posterior dwell to
  `0.681%`, stays near the `0.040/0.019` load class, and recovers arrival toward
  `15.14--15.34T`. Re-evaluate under a different carrier if projection no
  longer predicts the actual joint orbit.
