...
- Treat rapid target-line rotation as a tangential-margin diagnostic, not a
  ...
  release gating, or actuator remapping on this carrier. A carrier-separated
  body-lateral-force model is a distinct response hypothesis: its residual had
  the requested sign on `79.5%` of the centered capture's `distance<1L`
  samples versus `52.5--62.5%` for the wider rate captures. This separation is
  not causal proof; retain a force-response mechanism only if capture and
  terminal course margin improve together without losing arrival, route, wake
  coherence, joint reserve, or load class.
- Treat kinematic-reserve allocation as a directional terminal-course
  mechanism, not generic saturation protection or a reason to shed more
  authority. Across four direct-quiescent, same-carrier captures with visually
  comparable direct routes and compact alternating 3D wakes, the tail-only
  cubic predicted-miss residual arrived earliest and scored best
  (`15.1403T/-0.01094`) but retained `0.651L` constant-course miss and `1.269%`
  posterior `>40deg` dwell. Absolute posterior-reserve spillover produced the
  narrowest crossing (`0.362L`) but moved dwell to the anterior joint
  (`0.285/0.996%` head/tail) and arrived at `15.4464T`. Making both sending and
  receiving capacity directional preserved capture, improved arrival to
  `15.3385T`, removed anterior dwell, and reduced posterior dwell to `0.681%`
  while retaining `0.461L` miss and the same `0.03959/0.01890` normalized peak
  planar force/moment class. In contrast, dual absolute reserve and authority
  shedding arrived slightly earlier (`15.2957T`) and lowered peak force to
  `0.03872`, but widened miss to `0.556L` and increased posterior dwell to
  `0.898%`; both stricter allocators left near-rate occupancy essentially
  unchanged near `23.8/21.5%`. Prefer signed capacity that preserves
  restorative authority over absolute reserve or broader shedding. Its
  remaining boundary is late recognition of an outward-moving joint: test an
  observed phase-plane or projected-excursion reserve, and retain it only if
  capture and the direct compact wake survive, miss stays at or below `0.461L`,
  posterior dwell falls below `0.681%` without anterior dwell, and arrival,
  near-rate occupancy, distance integral, and load class remain competitive.
  One centered crossing does not establish robustness; re-test this ranking
  under a different carrier or initial condition.
