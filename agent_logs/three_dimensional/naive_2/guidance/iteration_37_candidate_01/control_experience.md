...
- Treat rapid target-line rotation as a tangential-margin diagnostic, not a
  ...
  `0.04041/0.01902` and posterior `>40 deg` dwell reached `1.269%`. Subsequent
  allocation establishes a real but coupled authority/reserve tradeoff. Moving
  unavailable tail residual to the head improved miss to `0.362L`, but left
  `0.285/0.996%` anterior/posterior `>40 deg` dwell and slowed arrival to
  `15.4464T`. Requiring reserve on both joints removed anterior dwell, reduced
  posterior dwell to `0.898%` and loads to `0.03872/0.01876`, and recovered
  `15.2957T/-0.01484`, yet widened miss to `0.556L`. Residual-sign directional
  reserve recovered part of that margin (`0.461L`) and reduced posterior dwell
  again (`0.681%`) at comparable `15.3385T/-0.01502`, but did not retain the
  absolute allocator's course. A receiver-safe hybrid then eliminated all
  `>40 deg` dwell and captured at `15.2823T`, but retained a `0.516L` miss,
  scored `-0.01727`, and reached `0.03988/0.01906` peak loads: shedding is a
  joint-quality safeguard, not evidence of centered navigation. More
  decisively, gating the full residual by the sign of the bounded previous
  action failed its own carrier-phase boundary. It captured at
  `15.2107T/-0.01708`, but widened miss to `0.631L` and increased posterior
  dwell to `1.878%` at `0.03930/0.01922` loads, worse than both the absolute
  and residual-direction dual-reserve samples. Reject previous action as a
  reserve-phase proxy on this carrier: it is delayed, already contains prior
  steering, and cannot distinguish a redirect that suppresses the carrier
  from one that amplifies it. Avoid more action-scale, soft-limit, or
  spillover-gain tuning. A subsequent contemporaneous unsteered-carrier
  decomposition also failed: although it preserved the direct compact-wake
  capture at `15.2469T`, retaining the carrier-opposing half-cycle while
  reserve-gating the amplifying half-cycle widened terminal miss to `0.619L`,
  raised posterior `>40 deg` dwell to `2.233%`, and raised peak normalized
  force/moment to `0.04184/0.01961`. Those outcomes are worse together than
  the directional allocator's `0.461L/0.681%/0.03959/0.01889`, so both
  delayed-action and analytic-carrier phase proxies are rejected for residual
  modulation of `abs(carrier)`. After this repeated topology, avoid further
  carrier-phase/reserve allocation edits; test a distinct bounded wave-shape
  or feedback actuator and require terminal margin, joint quality, and loads
  to improve together while the evidenced carrier and direct route remain
  intact. This boundary does not generalize to direct torque commands or a
  different carrier whose correction semantics differ.
