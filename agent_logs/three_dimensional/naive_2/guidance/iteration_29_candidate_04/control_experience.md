...
- Use carrier-separated corrective response and residual predicted miss
  ...
  Two later structural variants sharpen the negative boundary. Requiring
  target-relative course alignment before redirect release still captured, but
  arrived at `15.8926T`, scored `-0.02329`, and crossed at
  `(-0.721,-1.072)L/T` with about `0.719L` raw course miss. A centered-closing
  amplitude envelope with a `0.75` minimum drive share also captured at
  `15.6738T/-0.02289`, yet crossed at `(-1.128,-0.647)L/T` with roughly
  `0.679L` miss; its wake remained compact and its score/load class did not
  improve. Alongside the carrier-insensitive direction blend (`0.682L` miss)
  and yaw arrest (`0.746L` miss), these results reject another release gate,
  terminal observation blend, or weak carrier envelope as the robustness
  mechanism. Two actuator changes now close further nearby paths. Anterior
  restoring-stiffness duty skew preserved the direct compact wake and capture,
  but arrived at `15.9148T/-0.02231`, crossed at
  `(-0.711,-1.038)L/T` with `0.717L` raw miss, and introduced `0.138%`
  posterior `>40 deg` dwell. Inherited hydrodynamic-moment rejection through
  released half-cycle reserve also captured, yet arrived at
  `15.9383T/-0.02303`, retained `0.736L` miss, and raised peak normalized
  planar force to `0.0375`. Preserved capture and wake therefore do not make
  either anterior half-cycle timing or instantaneous moment opposition a
  terminal-margin mechanism; avoid retuning those structures on this carrier.
  Preserve the shared geometry/response handoff and evidenced carrier, and
  test a genuinely different bounded wave-shape actuator such as posterior
  lag modulation. Retain it only if capture and terminal-course spread improve
  together with the direct route, compact wake, joint reserve, and load scale
