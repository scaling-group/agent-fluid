...
- Phase-compensating body yaw before response feedback is the first sampled
  ...
  and wake coherence.
- For terminal interception, a rotation-invariant target/course residual and a
  smooth target-bearing mean-curvature handoff form a stronger mechanism chain
  than bearing/slip release gates. The inherited course-residual carrier moved
  the pass from the sampled `2.595--4.650L` high-side class to `1.276L` below
  the target, and adding an `8 deg` terminal carrier-equilibrium shift improved
  it to `1.033L` with zero joint dwell above `40 deg` and only `0.034/0.017`
  peak normalized planar force/moment. A later proximity/positive-closing/
  course-misalignment-gated damped hold is a concrete negative result: it
  retained low `0.036/0.018` loads but improved the pass by only `0.025L` to
  `1.008L`, still moved down-left at `1.011L/T` while the target lay up-left,
  and exited the lower boundary at `9.228L` final distance. Stronger terminal
  common-mode braking is also unsupported because a related inherited trial
  regressed to `2.167L`. Preserve the far-field course residual and smooth
  mean-bend handoff, but do not keep increasing a closing-speed-gated brake:
  radial closing naturally collapses at the tangent point while cross-track
  momentum remains. Before testing a terminal course-to-curvature channel,
  remove any repeatable beat-correlated lateral component using joint state
  and require that the residual be consistent across multiple near-miss
  traces. Falsify this implication if phase separation does not make the
  terminal request more persistent, loses the `1.033L` approach or coherent
  wake, or increases joint/load occupancy without capture or a better
  termination class.
