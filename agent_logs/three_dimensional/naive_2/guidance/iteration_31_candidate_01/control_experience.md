...
- Treat rapid target-line rotation as a tangential-margin failure, but do not
  infer it from one carrier-phase termination row. Relative to the unified
  handoff's `15.5008T/-0.02109/0.179L` arrival, score, and predicted miss,
  active carrier-yaw arrest, course-confirmed release, centered amplitude
  relief, duty skew, and unused-authority moment rejection retained the direct
  compact-wake capture but ended with `0.679--0.747L` misses and scores from
  `-0.02175` to `-0.02329`; duty skew restored `0.138%` posterior `>40 deg`
  dwell and moment rejection raised peak normalized planar force to `0.03746`.
  Transferring released rhythmic authority directly to instantaneous
  target-line-rate steering then improved score and distance integral to
  `-0.01965/1.90130L`, but crossed at `15.6893T` with `0.674L` miss and
  `(-1.112,-0.671)L/T` velocity, so its scalar benefit did not establish
  terminal-margin control. More importantly, the centered capture's apparent
  `0.411/T` final instantaneous line rate is `1.761/T` over the available
  eight-observation inertial bearing-plus-turn window; across `distance<3L`
  those estimates agree in sign only `41.4%`, versus `49.4--62.9%` for the
  wider-course captures. Requiring same-sign instantaneous/windowed persistence
  did not rescue the signal: it still captured with good scalar score
  (`15.7829T/-0.02023`) and the same compact wake/load class, but widened the
  projected miss to `0.747L`, beyond the instantaneous variant's `0.674L`, and
  crossed with slightly receding radial velocity (`-0.113L/T`). This falsifies
  persistence consensus as a terminal-margin mechanism; avoid further
  target-line-rate gains, thresholds, windows, or actuator transfers on this
  carrier. If phase separation is revisited, use it to clean the predictive
  course observation rather than create another steering request: across the
  four sampled histories, a two-joint-rate carrier model explains about `92%`
  of body-frame lateral-speed variance and separates the centered final course
  estimate from the three `0.674--0.747L` crossings. Retain that observation
  mechanism only if capture and course margin improve together without losing
  arrival, direct routing, wake coherence, joint reserve, or load class; the
  current worker's unevaluated candidate is not evidence that it succeeds.
