...
- Treat rapid target-line rotation as a tangential-margin diagnostic, not a
  ...
  release gating, or actuator remapping on this carrier. The later
  carrier-separated body-lateral-force residual converted its promising sign
  separation (`79.5%` correct near the centered capture versus
  `52.5--62.5%` for wider captures) into a bounded low-load capture
  (`15.7384T`, `0.03563/0.01764` peak normalized force/moment), but terminal
  course miss remained `0.744L`, far outside the inherited `0.179L` centered
  sample and in the same tangential class as line-rate control. The correlation
  was not a causal steering signal; avoid more force-model gains, thresholds,
  or force-triggered release on this carrier unless a different actuator or
  carrier first changes the response map.
- Preserve reserve-aware redistribution as a terminal-margin mechanism, but do
  not equate stricter reserve protection with better capture geometry. Across
  four direct compact-wake captures, the tail-only predicted-miss residual was
  earliest and score-best (`15.1403T/-0.01094`) but crossed with `0.651L`
  reconstructed miss and `1.269%` posterior `>40 deg` dwell. Absolute
  posterior allocation improved miss to `0.362L` but retained `0.996%` tail
  dwell and introduced `0.285%` head dwell; absolute dual-joint and directional
  dual-joint protection removed head dwell and reduced peak load slightly, but
  gave back margin to `0.556L` and `0.461L`. All variants still spent roughly
  `22--24%` of samples in the near-rate band. This supports redistributing a
  rhythmic residual, while falsifying more soft-limit threshold or allocation-
  gain tuning as a complete solution. Next test a distinct state-dependent
  actuation phase, such as steering only on a braking/restoring half-cycle, and
  retain it only if capture margin, arrival, near-rate occupancy, dwell, loads,
  translation, and wake coherence improve together.
