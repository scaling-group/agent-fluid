...
- Use carrier-separated corrective response and residual predicted miss
  ...
  normalized planar force/moment. Three distinct terminal refinements now
  sharpen the negative boundary without changing that ranking. Transferring
  released authority to raw-yaw arrest captured at `15.8061T/-0.02175` but
  crossed laterally at `(-0.774,-1.012)L/T`; centered-approach drive relief
  captured at `15.6738T/-0.02289` and `(-1.128,-0.647)L/T`; and anterior
  duty-duration steering captured at `15.9148T/-0.02231` but crossed at
  `(-0.711,-1.038)L/T`, versus the unified sample's
  `15.5008T/-0.02109` and `(-1.286,-0.014)L/T`. All retained the direct route
  and compact wake, so preserved capture alone does not justify perturbing
  carrier restoring dynamics or repurposing released authority; roll back such
  changes when terminal course, arrival/score, loads, and joint reserve do not
  improve together. Inherited identical-controller repeats still expose
  `1.2164L` and `1.0117L` left-exit misses, while angle-domain subtraction
  lowered effort but missed at `0.8293L`. Require repeated capture or
  simultaneous closest-pass and termination improvement with the direct route
  and compact wake intact; a single threshold capture, lower effort, or score
  improvement alone does not establish robustness. Revisit terminal carrier
  deformation only with a different carrier or an evidenced physical-response
  signal that predicts the failure before the lateral crossing.
