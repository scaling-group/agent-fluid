...
- Treat the sending-joint gate, receiving-joint gate, and carrier-phase proxy as
  separate decisions in terminal residual allocation; lower limit dwell or a
  quicker capture alone does not validate the allocation. Five same-carrier,
  direct-quiescent captures have visually comparable direct routes and compact
  alternating 3D wakes, yet separate a real geometry/reserve trade. A tail-only
  cubic predicted-miss residual
  arrived at `15.1403T` with `0.651L` head-relative course miss and `1.269%`
  posterior `>40 deg` dwell. Absolute posterior reserve with unconditional
  anterior spillover uniquely centered the course to `0.362L`, but slowed
  arrival to `15.4464T` and exchanged some exposure into `0.285/0.996%`
  anterior/posterior dwell. Requiring absolute reserve at both the sending and
  receiving joints eliminated anterior dwell and arrived at `15.2957T`, but
  widened miss to `0.556L` with `0.898%` posterior dwell. Making both capacities
  directional improved that compromise to `15.3385T/0.461L` and `0/0.681%`
  dwell, but still surrendered `0.099L` of the centered margin. Peak normalized
  planar force/moment stayed in the narrow `0.03872--0.04041/0.01876--0.01902`
  class for those four. Gating both joints from the sign of bounded
  `previous_action` then captured sooner at `15.2107T`, but widened miss to
  `0.631L`, raised posterior dwell to `1.878%`, increased peak moment to
  `0.01922`, and worsened score to `-0.01708`; its compact wake remained
  visually unchanged, so the action proxy did not identify usable reserve.
  Avoid action-scale tuning or further previous-action phase gates on this
  carrier. Sampled scores and inherited optimizer logs confirm all five
  remained captures, so neither wake breakup nor termination explains the
  ranking. Preserve the absolute posterior gate associated with margin, make
  only the receiving spillover direction-aware, and shed authority no joint can accept;
  retain that hybrid only if it preserves capture and the compact wake, keeps
  miss competitive with `0.362L`, eliminates anterior dwell, does not increase
  posterior dwell/load, and avoids a material arrival penalty. Re-evaluate the
  sender/receiver split under a different carrier or initial condition rather
  than treating these reserve conventions as universal.
