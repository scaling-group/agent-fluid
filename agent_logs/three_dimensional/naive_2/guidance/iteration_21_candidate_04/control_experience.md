...
- Treat target/course prediction as the evidenced terminal mechanism and
  ...
  joint-phase posterior pulse plus carrier-separated yaw gating produced three
  byte-identical-policy captures at `15.9830--16.0435T` with scores spanning
  `-0.02444---0.03031`, the same direct trajectory, and a compact alternating
  3D wake. A response-released pulse also captured at `16.0160T/-0.02502`, but
  its roughly `18.34%` joint-sample occupancy above 95% of the rate limit,
  arrival, and score lie inside the exact-policy repeat spread; its unreplicated
  3--5% lower peak normalized force/moment does not yet establish a load
  improvement. Across all four captures, however, the same two-joint-rate
  model explains about 88% of body lateral-velocity variance overall and
  `98.1--98.4%` inside `3L`, so test carrier-separated terminal course
  prediction before another miss-gain or pulse-gain edit. Apply that inference
  only when the carrier and fitted sign are consistent across repeats, and
  falsify it if capture, arrival beyond the repeat band, route/wake coherence,
  loads, or rate occupancy worsen. Do not broadly brake the carrier: the
  sampled proximity-braked policy reached only `4.64998L`, hit both
  joint-angle limits, produced peak normalized planar force/moment near
  `0.620/0.261`, and exited left.
