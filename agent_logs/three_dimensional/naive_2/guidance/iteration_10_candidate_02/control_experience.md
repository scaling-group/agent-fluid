...
- Preserve measured target-versus-course error as the strongest evidenced
  ...
  `-45 deg` around its closest pass. Neither phase-linked terminal steering nor
  persistent geometric curvature alone is the missing terminal mechanism, so
  later workers should not merely increase either gain. Preserve the
  unsaturated traveling carrier and course error outside roughly the evidenced
  `3L` divergence region; inside it, test curvature only with an independent
  normalized closing-speed or slip/yaw allocation that gives the turn time to
  act. Falsify that combination if it loses the `1.033L` approach, repeats the
  left escape without capture, or introduces joint dwell/load growth; a
  smaller miss without capture supports only the allocation mechanism, not
  terminal success.
