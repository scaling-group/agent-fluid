...
- Use carrier-separated corrective response and residual predicted miss
  ...
  Transferring released shared-half-cycle authority to instantaneous residual-
  yaw countersteer also retained capture, zero `>40 deg` dwell, the direct
  compact wake, and low `0.0351/0.0176` peak normalized force/moment, but it
  slowed arrival to `15.8061T`, worsened score to `-0.02175`, and widened raw
  head-relative terminal course miss to `0.746L` with strongly lateral
  `(-0.774,-1.012)L/T` velocity, versus `0.179L` and nearly horizontal travel
  in the best unified sample. Do not treat carrier-separated body yaw alone as
  a redirect-completion reference or harden that countersteer. Qualifying the
  same handoff with target-relative course did not repair it: the inherited
  course-confirmed release captured later at `15.8926T`, scored `-0.02329`,
  retained `0.719L` raw course miss and crossed at
  `(-0.721,-1.072)L/T`. Replacing the terminal observation with a common-
  rotation-invariant request likewise captured later with `0.682L` miss and
  `0.418%` angle dwell above `40 deg`; a centered-closing `0.75` minimum-drive
  envelope then scored `-0.02289` without reducing terminal speed
  (`1.300L/T` versus `1.286L/T`) or peak command. These are concrete negative
  results for further request blending, redirect-release qualification, and
  mild limit-cycle relief on this carrier. Three newly completed mechanisms
  close the remaining shared-half-cycle variants. Hydrodynamic-moment
  rejection captured at `15.9383T/-0.02303` but retained `0.736L` course miss,
  restored `0.173%` posterior `>40 deg` dwell, and raised peak normalized
  planar force to `0.03746`; terminal duty skew captured at
  `15.9148T/-0.02231` with `0.717L` miss and `0.138%` posterior angle dwell.
  Target-line-rate steering produced the best scalar score and distance
  integral (`-0.01965/1.90130L`) yet still crossed later than the unified
  sample, at `15.6893T`, with lateral `(-1.112,-0.671)L/T` velocity and
  `0.674L` miss. Do not harden or retune moment, yaw, duty, target-line, or
  release signals on the shared asymmetry channel: capture and one improved
  scalar did not reduce the replicated `0.179--0.590L` margin spread. Preserve
  the strongest response-plus-miss handoff and isolate a different bounded
  actuator/physical-response pairing; retain it only with capture and reduced
  terminal-course spread while the direct route, compact wake, joint reserve,
  and roughly `0.037/0.019` force/moment scale remain intact. In particular,
  target-normal relative flow was about `0.288U` at the centered crossing but
  `1.145--1.252U` in four lateral variants, so it is a testable response scale
  for a posterior wave-shape residual, not yet evidence of causality or a gain
  target. Repeat capture, lower distance integral, or one centered score alone
  does not establish margin robustness.
