...
- Keep carrier-phase separation in the response gate; do not let a joint-angle
  carrier estimate replace measured terminal target geometry. Four current
  sampled predicted-miss rollouts captured at `15.983--16.044T` behind the
  same compact wake, but inherited repeats expose threshold sensitivity: the
  always-pulse controller also missed at `1.2164L`, and the response-released
  controller both captured at `0.7486L` and missed at `1.0117L`. Subtracting
  the angle-domain carrier estimate from terminal pursuit reduced near-rate
  occupancy to about `10.0/10.5%`, yet its rollout passed low at `0.8293L`
  and exited left while retaining low `0.0351/0.0176` normalized planar
  force/moment. Fewer request crossings, lower effort, and a closer miss are
  therefore not semantic improvement. Preserve raw normalized body-frame
  pursuit/course geometry, and test steering release only when both
  carrier-separated response and remaining predicted miss agree; retain that
  qualification only if capture or closest pass and termination improve
  together without degrading the direct route, wake, joint reserve, or loads.
