...
- Preserve joint-state allocation as the first sampled mechanism to create
  substantial terminal course margin on the direct compact-wake carrier, but
  make reserve directional before adding more relief or release logic. The
  tail-only cubic residual captured at `15.1403T/-0.01094` with `0.651L`
  head-relative predicted miss and `1.269%` posterior `>40 deg` dwell;
  miss-conditioned posterior drive relief remained in essentially that class
  (`15.1939T/-0.01891/0.635L`, `1.229%` dwell), while response-releasing the
  residual reached `0.573L` miss but increased posterior dwell to `1.551%`.
  Allocating the same residual away from absolute posterior angle/rate limits
  preserved capture and reduced miss to `0.362L`, a meaningfully centered
  course, but slowed arrival to `15.4464T`, retained `0.996%` posterior dwell,
  and introduced `0.285%` anterior dwell at comparable `0.03970/0.01889` peak
  normalized planar force/moment. Thus absolute reserve transfer can exchange
  one joint's limit exposure for the other's even when geometry improves.
  On this carrier, test signed angle/rate capacity that retains restorative
  redirect authority, transfers only outward demand to an admissible joint,
  and otherwise returns to cruise; retain it only if the `0.362L` margin
  survives with lower two-joint dwell/load and without a material arrival or
  wake penalty. Revisit broad drive relief or response-only release only under
  a different carrier or terminal failure topology.
