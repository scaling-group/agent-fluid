...
- Preserve measured target-versus-course error as the far/middle directional
  signal, and recruit terminal mean curvature from predicted signed miss and
  time-to-closest rather than proximity alone. The course-residual controller
  first reached `1.173L` with no `>40 deg` dwell and low `0.034/0.017` peak
  normalized planar force/moment but escaped lower-left; proximity-only mean
  curvature improved this to `1.033L`, whereas closing-speed drive braking
  regressed to `2.167L` and fixed-horizon velocity-displaced aim reached only
  `1.158L`. In contrast, the sampled predicted-miss gate acted before closest
  approach and captured at `16.011T` and `0.748L`; both visual rows retain the
  alternating traveling wake through arrival. This is positive evidence for
  separating course steering, early target-relative collision prediction, and
  persistent curvature, not for static braking or another aim-offset gain.
  Its current boundary is actuator cleanliness: neither joint exceeded
  `37.5 deg`, but a joint occupied the exact `260 deg/T` rate limit in `21.9%`
  of samples and every such sample commanded farther outward. Preserve the
  captured navigation structure and test state-dependent outward-command
  release near the rate envelope; reject that refinement if capture, reversal,
  or wake coherence is lost, or if rate-stop occupancy and loads do not fall.
