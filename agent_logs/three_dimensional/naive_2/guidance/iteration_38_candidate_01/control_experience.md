...
- Treat sender reserve, receiver reserve, and carrier-phase inference as
  ...
  Completed inherited results now separate envelope quality from useful
  terminal translation. Oscillator-normalized linear angle projection
  (`q + q_dot/omega`) is the first reserve mechanism to improve the directional
  baseline's miss, both-joint dwell, and load scale together: it captured with
  a `0.421L` miss, zero `>40 deg` dwell, and `0.03628/0.01798` peak normalized
  force/moment while preserving the compact wake. It nevertheless slowed from
  `15.3385T/-0.01502` to `15.5850T/-0.02349`, so do not tune its projection
  scalar or call reserve improvement a complete controller improvement.
  Replacing the acceleration residual with an envelope-preserving posterior
  wave target reduced loads further to `0.03506/0.01724`, but captured only at
  `15.8431T/-0.02283` with a `0.725L` miss; this rejects more posterior-offset
  or amplitude-contraction tuning on this carrier. Together these results show
  that protecting the oscillatory envelope can be causal for joint/load
  quality while still over-shedding terminal steering. If extending the fully
  directional baseline, derive a one-sided excursion estimate from current
  oscillator phase instead of adding another projection gain, and retain it
  only if it preserves capture and the compact wake, keeps miss at or below
  `0.461L`, anterior/posterior dwell at or below `0/0.681%`, loads near or below
  `0.040/0.019`, and recovers the `15.14--15.34T` arrival class. Re-evaluate
  this ranking under a different carrier or initial condition rather than
  treating any reserve convention as universal.
