...
- A smaller miss and quieter joints do not establish a valid terminal hold.
  After course-residual steering changed the route and reached `1.276L`, a
  closing-speed-gated damped-curvature handoff narrowed the miss to `1.008L`,
  removed all dwell above `40 deg`, and limited peak normalized planar
  force/moment to about `0.028/0.018`; nevertheless it still crossed at
  `1.011L/T` and escaped the lower-left boundary to `9.228L`.  At closest
  approach the target bearing/course error were `+1.168/+1.475 rad`, the
  settled mean bend was `+7.6 deg`, and carrier-separated yaw residual was
  `+0.201 rad/T`, so the common bend sustained yaw away from the requested
  side.  Do not strengthen or prolong a coasting hold, or infer steering sign
  from its closest-distance scalar.  Calibrate terminal curvature sign from
  the quiet-posture yaw response while preserving the evidenced far-field
  course carrier; require capture or improved pass and termination together,
  and falsify the mapping if its reflected target case does not reverse yaw or
  if wake, joint-reserve, and load quality regress.
