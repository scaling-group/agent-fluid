...
- Treat target/course prediction as the evidenced terminal mechanism and
  ...
  rotation-invariant time-to-closest/signed-miss geometry; if reducing the
  roughly 18% near-rate-limit occupancy, release only the terminal steering
  transient when measured carrier-separated yaw is already corrective. Do not
  broadly brake the carrier: the sampled proximity-braked policy reached only
  `4.64998L`, hit both joint-angle limits, produced peak normalized planar
  force/moment near `0.620/0.261`, and exited left. Falsify response release if
  it loses capture, slows beyond the no-pulse reference, raises loads or rate
  occupancy, or disrupts the direct route and wake under repeated, reflected,
  or perturbed trials.
