...
- For the tight capture radius, predict interception from body-frame target and
  ...
  event. On the established traveling carrier, blending pursuit with course
  residual and using signed constant-course miss to recruit a bounded `8 deg`
  terminal mean bend produced the first sampled capture at `16.011T` and
  `0.748L`, with a compact alternating top-down/3D wake. By contrast, continuous
  approach drive relief reacted into a broad post-pass curl (`2.703L` closest,
  left exit), and inherited position-led late corrections passed as close as
  `0.963L` and `0.810L` but still exited left with final distances above
  `9.6L`. The reusable mechanism is anticipatory cross-track curvature while
  closing, not the sampled numerical gains: apply it only after coherent
  self-propulsion exists and the target/velocity vectors share a calibrated
  body frame; falsify it if capture, pre-approach translation, wake coherence,
  or load history degrades, or if noisy/near-zero speed makes the predicted
  miss unreliable.
