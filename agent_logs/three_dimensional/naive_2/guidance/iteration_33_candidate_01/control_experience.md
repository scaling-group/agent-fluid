...
- Separate terminal speed improvement from terminal-margin improvement on the
  ...
  evidence. If retaining that otherwise useful residual, test its termination
  as a response-gated burst: preserve the carrier and shared handoff, but make
  only the extra tail authority vanish when normalized predicted geometry and
  carrier-separated corrective yaw agree. Retain this distinction only if it
  improves the `0.651L` course boundary and reduces high-angle/load cost
  without losing capture, arrival, direct routing, or the compact wake. Avoid
  another residual gain or broad drive/release threshold on this carrier; do
  not transfer the ranking to a different carrier or imposed-wake regime
  without repeating the body-frame reconstruction.
