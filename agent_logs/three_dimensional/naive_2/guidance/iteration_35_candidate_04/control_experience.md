...
- Treat the sending-joint gate and receiving-joint gate as separate decisions
  in terminal residual allocation; lower limit dwell alone does not validate
  the allocation. Four same-carrier, direct-quiescent captures have visually
  comparable direct routes and compact alternating 3D wakes, yet separate a
  real geometry/reserve trade. A tail-only cubic predicted-miss residual
  ...
  class; sampled solver scores and inherited optimizer logs confirm all four
  remained captures, so neither wake breakup nor termination explains the
  ranking. On this carrier, preserve the absolute posterior gate associated
  with margin, make only the
  receiving spillover direction-aware, and shed authority no joint can accept;
