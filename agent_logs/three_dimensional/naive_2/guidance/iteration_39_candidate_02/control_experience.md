...
- Treat sender reserve, receiver reserve, and carrier-phase inference as
  ...
  Completed results separate excursion representation, approach scheduling,
  and terminal translation. A fixed oscillator-normalized linear angle
  projection (`q + q_dot/omega`) captured with `0.421L` miss, zero `>40 deg`
  dwell, and `0.03628/0.01798` peak normalized force/moment, but slowed from
  `15.3385T/-0.01502` to `15.5850T/-0.02349`. Replacing the acceleration
  residual with an envelope-sharing posterior wave target lowered loads to
  `0.03506/0.01724` but widened miss to `0.725L` and delayed capture to
  `15.8431T/-0.02283`, rejecting more posterior-offset, amplitude-contraction,
  or projection-gain tuning on this carrier. A one-sided harmonic phase-plane
  peak then preserved the compact wake, recovered `0.419L` miss with
  `0/0.680%` anterior/posterior dwell and `0.03836/0.01819` loads, but still
  arrived at `15.3677T/-0.01809`. Conversely, scheduling the linear projection
  with existing body-frame near-target geometry recovered the desired arrival
  class at `15.2650T/-0.01575` and improved dwell/loads to `0/0.360%` and
  `0.03781/0.01864`, yet widened miss to `0.529L`. Thus approach scheduling is
  evidence for preserving early translation, while the nonlinear excursion
  representation is evidence for course margin; neither alone dominates.
  Test their composition before abandoning the residual, not another scalar
  or threshold, and retain it only if capture and the compact wake persist,
  miss is at most `0.461L`, anterior/posterior dwell is at most `0/0.681%`,
  loads remain near or below `0.040/0.019`, and arrival returns to
  `15.14--15.34T`. Falsify this ranking under a different carrier or initial
  condition rather than treating scheduling or reserve convention as universal.
