...
- Preserve body-frame constant-course predicted miss as the only sampled
  architecture to reach capture on this carrier, but do not call either its
  base or its response/pulse extension robust. The sampled solver results and
  inherited logs now give the exact response/pulse policy three captures near
  `15.98--16.04T` and three `left_domain` repeats with closest approaches
  `1.0156--1.2164L`; all six retain the same compact alternating wake, zero
  `>40 deg` angle dwell, and low normalized planar force/moment near
  `0.04/0.02`. The base predictor likewise has one `0.7477L` capture and one
  `0.9631L` lower-left repeat miss, while a response-gated handoff without the
  pulse reached `0.8100L` but did not capture. Thus neither low loads nor
  byte-identical threshold captures isolate the posterior pulse or guarantee
  the termination class. Stop replication-only promotion and pulse/static-bend
  gain iteration; retain normalized time-to-closest and signed-miss geometry,
  but test a distinct state-gated actuator that remains active when forecast
  cross-track miss lies outside a safe corridor and releases inside it.
  Require repeated capture or a better termination class together with route,
  wake, joint, and load quality; revisit the ranking only under a different
  carrier or reflected/perturbed geometry.
- Do not treat a state-gated carrier perturbation as trajectory-local merely
  because it becomes algebraically inactive on the established limit cycle.
  The assigned parent's phase-envelope anti-damping raised `2T` speed from
  about `0.097` to `0.389L/T`, but changed the direct capture route into a
  lower-boundary exit at `23.716T`, with closest/final distance
  `3.863/9.061L`; its coherent wake, zero `>40 deg` dwell, and low peak
  planar force/moment near `0.031/0.016` did not make that route useful. On an
  already capturing interception carrier, avoid startup-energy or recovery-gain
  tuning based only on faster establishment, bounded actions, or proof that the
  gate later vanishes: require repeat capture, unchanged trajectory class, and
  earlier arrival together. Revisit envelope recovery only with paired repeats
  under a different carrier or if an observed loss of rhythm, rather than a
  benign startup lull, is the demonstrated failure mechanism.
