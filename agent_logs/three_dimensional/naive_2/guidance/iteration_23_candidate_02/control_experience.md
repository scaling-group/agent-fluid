...
- Preserve body-frame constant-course predicted miss as the first repeatable
  ...
  tuning. An offline replay had suggested angle-domain phase separation by
  reducing terminal request variation about one third, but two byte-identical
  CFD evaluations of that edit missed at `0.82926L` and `1.10362L` and exited
  left. A fitted joint-rate sway subtraction and a lagged-posterior phase
  extension likewise retained the direct low-load approach yet missed at
  `1.02506L` and `0.92803L`. Offline de-chattering therefore did not transfer
  to capture, and preserving wake/load quality is not enough when termination
  is unchanged. Avoid more fitted joint-state carrier subtraction or pulse
  phase-envelope variants on this carrier. Test release against a bounded
  target-relative response signal instead, leaving far-field route, carrier,
  and course prediction intact; retain it only if closest pass and termination
  improve together with wake, joint, load, and trajectory quality.
