...
- Phase-compensating body yaw before response feedback is the first sampled
  ...
  and wake coherence.
- Target-relative closest-approach prediction, rather than another distance
  threshold or fixed-horizon aim point, supplies the first completed capture
  mechanism in this lineage.  On the course-residual traveling carrier, a
  bounded positive-time-to-closest and signed cross-track-miss gate recruited
  the existing small mean bend before the `3L` fallback and captured at
  `16.01T` (`0.748L`), with zero joint-angle dwell beyond `40 deg`, peak
  normalized planar force/moment `0.035/0.017`, and a coherent alternating 3D
  wake.  Three sampled full-circle/response-burst alternatives missed by
  `2.595--4.650L` and exited left; inherited static closing-speed braking and
  fixed-horizon displacement also regressed the roughly `1.03L` near-miss
  class.  Preserve the collision-course geometry and propulsive scaffold
  before changing terminal authority.  Its remaining boundary is beat-phase
  contamination: the capture rollout occupied the two joint-rate limits for
  about `13%` of samples, and an offline one-period residual diagnostic found
  that head-joint angle/rate explained `72--92%` of body-lateral carrier
  oscillation across all four current samples.  A joint-state carrier
  subtraction is therefore a testable observation mechanism, not yet an
  evaluated improvement; retain it only if capture/arrival, reflection
  symmetry, wake coherence, loads, and rate reserve improve together, and
  refit or discard it under a different carrier.
