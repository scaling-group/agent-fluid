...
- Treat kinematic-reserve allocation as a terminal-course mechanism, not just
  generic saturation protection.  Across four direct-quiescent, same-carrier
  captures with visually comparable direct routes and compact alternating 3D
  wakes, the tail-only cubic predicted-miss residual arrived earliest and
  scored best (`15.1403T/-0.01094`) but retained a `0.651L` constant-course
  miss.  Miss-conditioned posterior drive relief and response-releasing that
  residual were slower, scored `-0.01891/-0.01847`, and remained wide at
  `0.635/0.573L`.  Allocating the residual away from a posterior joint near
  its soft angle/rate bounds was also slower (`15.4464T/-0.01764`) but uniquely
  reduced miss to `0.363L` at comparable `0.03970/0.01889` peak normalized
  planar force/moment; inherited optimizer logs confirm that this remained a
  capture rather than a termination trade.  The allocation was incomplete:
  it moved `>40deg` dwell to the anterior joint (`0.285%`), retained
  `0.996%` posterior dwell, and left both joints' near-rate occupancy near
  `23.5/21.8%`.  Preserve tail-first rhythmic steering, but make spillover
  conditional on receiving-joint reserve and shed unavailable authority;
  retain that mechanism only if capture and the direct compact wake survive,
  course miss improves without transferring limit occupancy, and arrival,
  distance integral, and load class remain competitive.  One centered crossing
  does not establish robustness, and this ranking must be re-tested under a
  different carrier or initial condition.
