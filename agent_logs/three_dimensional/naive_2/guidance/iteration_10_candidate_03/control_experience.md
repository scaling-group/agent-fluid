...
- Course residual is an evidenced navigation signal, but neither replacing
  pursuit with its beat-scale angle nor handing terminal mean curvature to
  pursuit alone is a capture mechanism. The four detailed pursuit/approach
  samples all retain coherent self-propulsion yet pass high and exit at
  `y=15.20L` (`2.595--4.650L` closest). The assigned parent's pure
  target-versus-velocity controller changes that topology to a lower-boundary
  exit, proving real directional authority, but regresses to a `3.176L` pass.
  A co-scaled course controller then reaches `1.276L`, and a terminal
  pursuit-mean handoff reaches `1.033L` with zero dwell above `40 deg` and only
  `0.034/0.017` peak normalized planar force/moment; both still escape
  lower-left with final distance above `10L`. At the stronger handoff's first
  `2.50L` crossing, pursuit has the wrong sign (`-0.093 rad`) while course
  residual already has the correcting sign (`+0.712 rad`). Avoid another
  scalar distance/release edit or pursuit-only terminal bend. The remaining
  falsifiable transfer is to route persistent normalized course error into a
  slowly varying mean-curvature channel while retaining the opposed traveling
  wave; keep it only if capture or return topology improves below the `1.033L`
  reference without losing the low loads, joint reserve, or coherent wake.
