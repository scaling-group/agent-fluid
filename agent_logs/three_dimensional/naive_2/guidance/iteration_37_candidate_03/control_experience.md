...
- Treat rapid target-line rotation as a tangential-margin diagnostic, not a
  ...
  absolute allocator's course. Preserve dual-joint shedding as an evidenced
  load/limit safeguard, but do not equate the target-relative residual sign
  with actual actuator direction when it modulates `abs(carrier)`. Avoid more
  soft-limit threshold or spillover-gain tuning; test reserve against observed
  carrier phase, and retain it only if capture margin and both-joint quality
  improve together. This implication applies to rhythmic residual allocation,
  not direct torque commands or a different carrier whose modulation can
  reverse acceleration sign.
- Carrier-phase awareness does not rescue the cubic `abs(carrier)` redirect as
  a reserve allocator. A delayed previous-action proxy captured with
  `0.631L` terminal miss, `1.878%` posterior `>40 deg` dwell, and
  `0.03930/0.01922` peak normalized force/moment; replacing it with the
  contemporaneous analytic carrier retained the same direct compact wake and
  captured at `15.2469T`, but still missed by `0.619L` while worsening dwell
  and loads to `2.233%` and `0.04184/0.01961`. Even receiver-safe absolute-tail
  allocation was not a stable margin solution: identical-controller captures
  varied from `0.516L/0%/0.03988/0.01906` to
  `0.619L/0.397%/0.04087/0.01968` in miss, posterior dwell, force, and moment.
  Avoid more phase-softness, reserve-threshold, or spillover-gain work on this
  carrier-magnitude residual. Test a distinct normalized waveform-level
  actuator, such as response-released posterior lag shaping, and retain it
  only if repeated capture margin, posterior reserve, and load class improve
  together without changing the far-field route or alternating 3D wake. This
  boundary does not reject phase feedback for a different actuator whose
  command is kinematically interpretable rather than multiplied by
  `abs(carrier)`.
