...
- Body-frame constant-course prediction is the first mechanism in this
  lineage to cross from closer passes into sampled capture. Three current
  predicted-miss policies preserve the full traveling-bend carrier, recruit a
  small bounded mean bend from time-to-closest and signed cross-track miss,
  and capture at `15.983--16.016T` and `0.7472--0.7500L` behind a compact
  alternating 3D wake; the inherited physical audit reports zero joint-angle
  dwell beyond `40 deg` and peak normalized planar force/moment near
  `0.034--0.037/0.017--0.019`. This supports course prediction and early
  interception rather than proximity-only bearing, broad carrier braking, or
  stronger propulsion. The boundary is repeat robustness: related inherited
  executions missed at `0.9631L`, `0.8100L`, and `1.2164L`, while adding a
  phase-envelope startup-recovery channel regressed to
  `3.8633L/left_domain`. Preserve the full carrier and test response-released
  joint-phase steering as a terminal refinement, but require repeat capture
  under reflected or perturbed geometry before treating curvature sign,
  prediction horizon, or threshold-level nominal capture as general; reject
  additions that change the direct route, wake/load class, or termination
  without improving capture margin.
- Do not use a plausible response decomposition as a proxy for terminal
  success on the predicted-miss carrier. In the assigned parent, releasing the
  posterior pulse when carrier-separated yaw appeared corrective preserved the
  direct compact-wake approach and low `0.033/0.016` peak normalized
  force/moment, yet missed at `1.01175L` and exited left. A subsequent predictor
  that subtracted a joint-rate sway model fitted to explain over `92%` of the
  sampled far-field lateral-velocity variance also retained low
  `0.035/0.017` loads but missed at `1.02506L` with the same termination.
  Together with three ungated-pulse captures at `0.7472--0.7500L`, this rejects
  both yaw-based pulse release and same-case fitted sway subtraction as robust
  refinements despite clean offline diagnostics. Keep the ungated pulse as the
  nominal baseline and test a distinct normalized phase allocation or
  target-response actuator; retain it only if capture margin or repeat
  termination improves without sacrificing the direct wake, joint reserve, or
  load class.
