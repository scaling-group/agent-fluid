...
- Phase-compensating body yaw before response feedback is the first sampled
  ...
  and wake coherence.
- Posterior mid-stroke wave shaping is a supported terminal actuator on the
  predictive-intercept carrier, but not yet evidence of robust capture. The
  identical-configuration predicted-miss repeat missed at `0.96311L`, and a
  yaw-confirmed half-cycle handoff recovered the next pass to `0.81002L` but
  still escaped lower-left. Adding a bounded, target-signed posterior pulse
  that vanishes at beat reversal and after closing then produced a sampled
  capture at `0.749982L` and `15.983T`, with the same compact body-connected
  alternating wake and zero `>40 deg` joint-angle dwell. Preserve the
  predictive intercept, response gate, and phase-gated posterior channel on
  this carrier instead of reverting to static bend or broad braking. The
  capture crossed the lower-right edge with head velocity about
  `(-1.052,-0.515)L/T`; another sampled capture similarly crossed at
  `(-1.093,-0.408)L/T`. Treat that transverse-course signature and the prior
  repeat miss as the remaining fragility: test a normalized near-target course
  or slip residual through the posterior channel, and retain it only on repeat
  capture or more central approach without posterior pinning, joint/load
  growth, far-field route loss, or wake degradation. Revisit the actuator sign
  and handoff if a different carrier changes the curvature/yaw response map.
