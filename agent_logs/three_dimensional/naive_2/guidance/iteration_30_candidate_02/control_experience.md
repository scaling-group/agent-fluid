...
- Treat rapid target-line rotation as a tangential-margin failure, but do not
  ...
  wider-course captures. Avoid another instantaneous rate gain or threshold:
  separate persistent target-relative motion from beat-scale disagreement and
  retain any such mechanism only if capture and course margin improve together
  without losing arrival, route, wake coherence, joint reserve, or load class.
  Revisit the signal only under a persistence/phase-separation test; otherwise
  the phase-specific terminal snapshot is neither a robust diagnostic nor
  causal evidence.
