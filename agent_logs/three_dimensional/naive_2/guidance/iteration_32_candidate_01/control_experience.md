...
- Use carrier-separated corrective response and residual predicted miss
  ...
  Subsequent actuator and response tests close several tempting replays:
  posterior phase-lag shaping, window-consensus line-rate steering, and
  carrier-separated force-deficit steering all captured with wider
  `0.727--0.747L` terminal misses. A cubed predicted-miss tail residual instead
  produced the best sampled score and earliest arrival
  (`-0.01094/15.1403T`), establishing a real posterior rhythmic
  speed/progress benefit, but improved miss only to `0.651L` while raising
  target-line rate to `1.745/T`, peak normalized force/moment to
  `0.0404/0.0190`, posterior `>40 deg` dwell to `1.269%`, and the terminal raw
  heading-rate sample to `-4.637 rad/T`. Do not retune line-rate scale, replay
  phase-lag/force gates, or treat an unqualified tail residual as robust
  completion. If retaining its speed benefit, require an observed-response
  release to restore terminal miss, joint reserve, and load class without
  losing capture or arrival. Inherited identical-controller repeats still
  expose `1.2164L` and `1.0117L` left-exit misses, while angle-domain
  subtraction lowered effort but missed at `0.8293L`. Preserve the shared
  geometry/response handoff, but require both capture and reduced terminal-
  course spread with the direct route, compact wake, joint reserve, and load
  scale intact; repeat capture, lower effort, or a better score alone does not
