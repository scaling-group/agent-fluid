...
- Separate indiscriminate static bend from closing-geometry-triggered mean
  curvature. After inherited near misses of `0.810--1.121L` still exited left,
  body-frame constant-course time/miss gating on the established traveling
  carrier produced a `0.7477L` capture at `16.01T`; adding carrier-separated
  yaw release and a posterior mid-stroke pulse then captured twice at
  `15.98--15.99T` (`0.7500/0.7472L`) with compact alternating wakes, no
  `>40 deg` joint dwell, and only `0.034--0.037/0.017--0.019` peak normalized
  planar force/moment. By contrast, symmetric approach-drive relief on the
  same carrier locked both joints at `-45 deg`, raised those peaks to
  `0.713/0.300`, missed by `2.703L`, and exited left. Preserve propulsion and
  recruit curvature/pulse authority only from normalized target/course
  geometry (including predicted miss and closing alignment), joint phase, and
  measured response; falsify this lesson
  under a different carrier, or if capture, trajectory topology, wake
  coherence, and load/joint quality do not improve together.
- Treat a threshold-grazing capture as an execution-robustness concern when
  an exact policy replay is available. The response-gated predicted-miss policy
  hash captured twice at `0.7500/0.7472L`, but its assigned-parent replay
  missed at `1.216L` and then exited the lower boundary at `27.19T`, despite
  preserving the same compact wake, sub-`39 deg` joints, and roughly
  `0.035/0.018` peak normalized planar force/moment. The replay diverged
  gradually and lost its posterior pulse when closing alignment vanished, so
  neither two successes nor clean loads establish terminal robustness. After
  this pattern, preserve pre-pass propulsion and test an observation-gated
  near-miss re-acquisition actuator that persists only while proximity and
  stalled/reversed closing agree; reject it if it perturbs the successful
  approach, repeats the post-miss boundary topology, or worsens wake, joint,
  and load quality. This boundary applies to capture thresholds approached at
  full drive and should be rechecked if the carrier or success geometry changes.
