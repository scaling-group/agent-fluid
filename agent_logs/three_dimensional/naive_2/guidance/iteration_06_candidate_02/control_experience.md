...
- Treat terminal carrier relief and steering release as one authority-allocation
  problem. Relative to the coherent full-carrier pass (`3.174L` closest, later
  left-boundary exit), continuous target-plane carrier relief improved closest
  approach to `2.703L` but left half-cycle steering dominant, produced a sharp
  curl, and changed termination to the upper boundary. A full-circle,
  misalignment-gated carrier restored translation when the nose aligned and
  improved the miss to `2.319L`, yet its still-unrelieved slip steering also
  curled upward after center y reached only `12.064L`. Together with the
  inherited broad-braking failure, this shows that neither more braking nor
  retained steering alone is a terminal solution: after an evidenced redirect,
  test a smooth body-frame alignment transition that restores the propulsive
  carrier while releasing steering, and retain it only if closest approach and
  termination topology improve without worse wake, load, or limit histories.
  Falsify this implication for carriers whose aligned thrust does not follow
  the body axis or when the approach miss is caused by weak propulsion rather
  than persistent yaw/slip.
