...
- Preserve body-frame constant-course predicted miss as a useful direct-route
  architecture, but treat its terminal composite as threshold-sensitive.
  Three byte-identical sampled runs captured at `15.983--16.044T` with a
  compact alternating 3D wake, zero `>40 deg` angle dwell, and peak normalized
  planar force/moment below `0.037/0.019`; a response-released pulse sibling
  also captured at `16.016T`. Later inherited evaluations of the same composite
  nevertheless missed at `1.01564L`, `1.12140L`, and `1.21644L` before exiting
  left, while a no-pulse sibling captured, so neither pulse presence nor its
  response release explains reliability. Preserve normalized time-to-closest/
  signed-miss geometry, the small bounded carrier mean, and measured-response
  handoff; avoid more pulse, static-bend, or carrier-gain tuning.
  Raw near-target bearing flipped five times and the blended request six times
  during the final `2.5T` of the captures, but reducing that variance offline
  was not a valid control surrogate: two completed joint-angle carrier-yaw
  subtraction policies retained the direct compact-wake approach yet missed at
  `1.07282L` and `1.21754L`, curled into a near-vertical escape, and exited
  left at about `27.5T`. Do not reuse that phase subtraction without a newly
  evidenced carrier model. A distinct next test may let rotation-invariant
  signed miss own the reliable closing interval and restore raw pursuit only
  when course alignment or speed is lost; retain it only if repeated capture
  or closest-pass/termination improves without changing the far route, wake,
  joint reserve, or load quality.
