...
- Preserve body-frame constant-course predicted miss as a useful direct-route
  ...
  In the evaluated captures, raw near-target bearing flipped sign five times
  and the blended terminal request six times during the final `2.5T`; an
  offline joint-angle carrier-yaw subtraction reduced request deviation by
  about one third and left two brief crossings. Treat that replay only as
  evidence of beat-sensitive terminal geometry, not as CFD proof of a fix:
  test a bounded phase-separated terminal observation while leaving the
  far-field route intact, and retain it only on repeated capture plus preserved
  trajectory, wake, joint, and load quality. Revisit the pulse only through
  paired repeated ablation; falsify phase separation if it changes the route
  class, fails to improve the `1.0--1.22L` miss band, or degrades those physical
  diagnostics.
