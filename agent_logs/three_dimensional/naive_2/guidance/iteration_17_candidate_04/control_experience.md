...
- Phase-compensating body yaw before response feedback is the first sampled
  ...
  and wake coherence.
- Promote predicted miss plus yaw-confirmed rhythmic handoff and bounded
  posterior wave shaping as the repeat-capturing terminal composite on this
  carrier. The predicted-miss base captured at `16.0105T/0.74772L`; a
  byte-identical response-gated posterior-pulse policy then captured in two
  sampled evaluations at `15.9830T/0.74998L` and `16.0435T/0.74934L`. All
  three sheets retain a compact body-connected alternating 3D wake, zero
  logged joint-angle dwell beyond `40 deg`, and peak normalized planar
  force/moment no greater than `0.0347/0.0175`. Because the duplicate scores
  (`-0.02444`, `-0.03031`) straddle the base (`-0.02805`), this establishes
  repeat semantic success and wake/load preservation, not a scalar advantage
  for the pulse alone. Preserve the carrier, target/course predictor, and
  response gate; do not substitute broad approach braking, whose sampled
  full-circle variant reached only `4.64998L`, curled upward out the left
  boundary, pinned the posterior joint beyond `40 deg` for `25.4%` of samples,
  and raised peak force/moment to `0.620/0.261`. The remaining boundary is
  joint-rate reserve: the capture composite occupies each `99%` rate envelope
  for about `13.2%` of logged samples. Test only terminal-local, observed-phase
  pulse selection or release, and retain it on repeat capture with equal or
  better arrival/progress, wake coherence, rate occupancy, and loads. Revisit
  this ranking under a different carrier or reflected response map.
