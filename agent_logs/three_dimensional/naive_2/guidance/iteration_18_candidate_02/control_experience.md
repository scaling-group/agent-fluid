...
- Predicted miss, rather than proximity alone, is the first repeatably sampled
  ...
  `0.74772L/16.0105T`; more decisively, the exact response-gated version with
  a joint-phase posterior pulse captured in two independent samples at
  `0.74998L/15.9830T` and `0.74723L/15.9940T`. Both repeats preserved the
  direct targetward trajectory, compact alternating 3D wake, zero `>40 deg`
  joint dwell, and low peak normalized planar force/moment
  (`0.0342--0.0367/0.0173--0.0185`). Promote the mechanism ahead of distance
  holds, full-circle pursuit, or another static-bend/gain variant: predict
  ...
  observed joint phase. The replication strengthens the composite, but does
  not erase its tight capture margin or the inherited sequence in which a
  base-policy repeat missed at `0.96311L` and response gating alone recovered
  only to `0.81002L`; the successful composite also used the two rate
  envelopes for about `17%` of samples. Falsify or revise it if reflected or
  perturbed runs lose capture, repeat the lower-left escape, or increase rate
  occupancy/loads. If rate reserve is addressed, require a selective
  allocation mechanism to preserve capture and translation; broad carrier
  braking already removed saturation while worsening both progress and
  termination.
