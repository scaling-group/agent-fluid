...
- Phase-compensating body yaw before response feedback is the first sampled
  ...
  and wake coherence.
- Approach schedulers and maneuver-release gates can improve one diagnostic
  while preserving the same high-pass failure. On the phase-compensated
  carrier, continuous hold reached `2.703L` but pinned both joints beyond
  `40 deg` for `5.349T`; signed mean-bend release removed simultaneous pinning
  and reached `2.664L`, yet left the tail beyond `40 deg` for `6.352T` and
  repeated the upper return arc. Tail-bend release cut tail dwell to `0.929T`
  but regressed to `3.312L` and raised force/moment peaks from `0.488/0.220` to
  `0.890/0.414`. Actuator reserve therefore does not validate a release gate;
  require closer geometry and no load tradeoff as well as less limit dwell.
  The attempted joint-angle phase correction is also a negative result: despite
  a coherent wake, it reached only `4.650L`, exited high at `20.03T`, and kept
  the tail beyond `40 deg` for `5.081T`. Do not integrate the useful joint-rate
  yaw residual model as a target-attitude estimate. To remove body-yaw phase
  without such integration, test a rotation-invariant target-versus-velocity
  course residual on the full carrier; falsify it if it cannot retain at least
  the `2.664L` approach, lower the trajectory, and preserve load quality.
