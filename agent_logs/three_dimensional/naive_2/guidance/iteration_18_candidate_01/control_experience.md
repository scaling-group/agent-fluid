...
- Promote body-frame constant-course predicted miss as the first replicated
  capture architecture on this carrier, but do not yet credit its posterior
  pulse as the causal improvement.  Inherited evidence progressed from an
  identical base policy's `0.96311L` lower-left miss to `0.81002L` when
  carrier-separated yaw delayed the half-cycle handoff.  Adding a
  joint-phase-gated posterior pulse then produced two byte-identical sampled
  captures at `15.983--16.044T`; both retained a compact alternating 3D wake,
  zero `>40 deg` angle dwell, peak normalized planar force/moment below
  `0.035/0.018`, and nearly identical rate-limit occupancy.  However, the
  no-pulse sibling also captured at `16.011T` with the same joint/load scale.
  Preserve the normalized target/velocity time-to-closest and signed-miss gate,
  small bounded carrier mean, and measured-response handoff when this carrier
  is established; avoid pulse-gain iteration until replicated ablations
  separate its contribution from rollout variability.  Falsify the composite
  under reflected or perturbed geometry, loss of repeat capture, a changed
  trajectory class, wake degradation, or joint/load growth; falsify only the
  pulse if removing it retains those outcomes across repeats.
