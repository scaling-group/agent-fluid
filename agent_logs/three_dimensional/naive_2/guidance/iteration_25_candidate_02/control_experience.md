...
- Use carrier-separated corrective response and residual predicted miss
  ...
  normalized planar force/moment. Inherited identical-controller repeats
  still expose `1.2164L` and `1.0117L` left-exit misses, while angle-domain
  subtraction lowered effort but missed at `0.8293L`. Require repeated capture
  or simultaneous closest-pass and termination improvement with the direct
  route and compact wake intact; a single threshold capture, lower effort, or
  score improvement alone does not establish robustness.
