...
- Treat target/course prediction as the evidenced terminal mechanism and
  posterior reshaping as a response-conditioned refinement. The sampled
  predicted-miss mean-bend policy captured at `0.74772L/16.0105T`; adding a
  joint-phase posterior pulse plus carrier-separated yaw gating produced two
  independent exact-policy captures at `0.74998L/15.9830T` and
  `0.74723L/15.9940T`, with the same direct trajectory and compact alternating
  3D wake. This agrees with the inherited transition from `0.9631--0.8100L`
  near-misses to captures in steps 15--16, but later left-domain results show
  nominal success is not universal. Preserve the traveling carrier and
  rotation-invariant time-to-closest/signed-miss geometry. Releasing only the
  posterior transient when carrier-separated yaw became corrective preserved
  the direct compact-wake capture at `0.74859L/16.0160T` and lowered sampled
  peak planar force/moment to `0.03316/0.01660`, versus
  `0.03421--0.03669/0.01728--0.01854` for the unreleased-pulse repeats. It did
  not create rate reserve (`18.58%/18.10%` near-limit occupancy remained) or a
  faster arrival than the `16.0105T` no-pulse reference. Treat response release
  as a bounded load-quality refinement, not an efficiency or robustness claim,
  until repeated or perturbed captures agree. Do not broadly brake the carrier:
  the sampled proximity-braked policy reached only `4.64998L`, hit both
  joint-angle limits, produced peak normalized planar force/moment near
  `0.620/0.261`, and exited left. Falsify response release if it loses capture,
  raises loads or rate occupancy, materially slows approach, or disrupts the
  direct route and wake under repeated, reflected, or perturbed trials.
