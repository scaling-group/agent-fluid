...
- Preserve constant-course predicted miss and carrier-separated response as a
  repeatable direct-route capture architecture, but stop iterating terminal
  request blends, pulse releases, and yaw-arrest handoffs on this carrier. All
  four current direct-quiescent samples captured at `15.5008--16.0270T` behind
  the same compact alternating 3D wake and in the same load class. The unified
  response-plus-miss prefill remained best at `15.5008T`, score `-0.02109`,
  integral `1.90236L`, and terminal velocity `(-1.286,-0.014)U`. Replacing its
  raw terminal pursuit with common-rotation-invariant interception still
  captured, but was slower and worse-scoring (`15.7735T/-0.02195`) and ended
  laterally at `(-1.016,-0.691)U`; active yaw arrest likewise captured at
  `15.8061T/-0.02175` with `(-0.774,-1.012)U`. Neither mechanism reduced the
  terminal phase spread, the roughly `17--18%` per-joint near-rate occupancy,
  or acceleration-cap contact. Together with inherited identical-controller
  `1.01--1.22L` left-exit misses, this is a concrete negative result for more
  observation substitution or release tuning, not evidence to strengthen the
  carrier. A distinct approach-hold test may smoothly relieve rhythmic drive
  only when proximity, reliable closing, and small predicted miss agree, and
  must restore full drive off-course. Retain it only if repeated capture or a
  simultaneously closer pass and better termination comes with lower terminal
  speed/rate occupancy and no loss of far-field translation, direct routing,
  wake coherence, or compact loads; reject mere effort reduction or a slower
  threshold capture without added margin.
- A proximity/closing/centered amplitude envelope acting only through the Van
  der Pol nonlinear damping is not an effective approach hold on this
  carrier. Its 25% nominal amplitude relief preserved the direct compact-wake
  capture, but versus the unified response-plus-miss controller it slowed
  arrival from `15.5008T` to `15.6738T`, worsened score from `-0.02109` to
  `-0.02289` and raw terminal course miss from `0.179L` to `0.679L`, while
  terminal speed rose from `1.286U` to `1.300U`, near-rate occupancy remained
  about `19.2/17.6%` versus `19.1/18.1%`, and peak normalized force/moment
  stayed in the same `0.0366/0.0182` class. Avoid further minimum-drive-share
  or consensus-gate gain tuning: with harmonic restoring and posterior
  tracking untouched, changing the nonlinear amplitude regulator did not
  produce measured terminal gait relief. Under this carrier, test a distinct
  normalized dissipative actuator that directly opposes joint motion only
  during a centered closing approach, and retain it only if capture or margin
  improves together with terminal speed/rate reserve; revisit amplitude
  scheduling only for a carrier whose commanded envelope actually controls
  its short-horizon oscillation energy.
