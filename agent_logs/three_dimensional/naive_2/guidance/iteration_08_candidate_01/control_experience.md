...
- The sampled terminal schedulers show that approach relief must preserve the
  ...
  `4.650L`, crossing target x at `y=14.555L`, and repeating the exit. At the
  four sampled closest passes, body-frame target/velocity course error was
  still `-0.972` to `-1.460 rad` while speed remained about `0.86--1.00L/T`;
  distance-only gating was therefore braking a geometrically poor approach.
  An inherited score-only result later reached `1.276L` but still exited left
  and ended at `10.936L`; absent its policy and multimodal trace, it establishes
  attainable near-pass geometry but no reusable mechanism. Avoid scaling only
  the restorative carrier or accepting either score or minimum distance as
  evidence of terminal control. On this carrier, gate hold by normalized
  closing response and target/velocity course alignment, and co-scale the
  complete rhythmic command so a wrong-course state restores both propulsion
  and steering. Retain that implication only if capture geometry or termination
  improves without sacrificing far-field translation, wake coherence,
  actuator reserve, or load quality; a different carrier or velocity below a
  declared reliability threshold invalidates the course-angle assumption.
