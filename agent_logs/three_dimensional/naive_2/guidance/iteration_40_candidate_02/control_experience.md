...
- Treat sending/receiving reserve gates as joint-quality safeguards, not a
  ...
  jointly preserved margin and load quality. Preserve the evidenced far-field
  carrier and response-plus-miss handoff, but test a different bounded actuator
  representation such as an envelope-aware posterior target shape. Retain it
  only if capture, the compact wake, miss at or below `0.461L`, posterior dwell
  ...
  posterior correction rather than tuning another reserve threshold.
- Separate beat-scale carrier sway from persistent terminal course error when
  the joint-rate relation is evidenced and stable. Adding only a normalized
  two-joint-rate lateral-sway subtraction to the approach-scheduled reserve
  controller improved head-relative terminal course miss from `0.529L` to
  `0.305L`, while preserving capture, the direct compact wake, zero anterior
  dwell, essentially unchanged posterior `>40 deg` dwell (`0.360%` versus
  `0.358%`), and the same peak normalized force/moment class
  (`0.03781/0.01864` versus `0.03856/0.01846`). This is the first completed
  current-lineage result below the directional allocator's `0.461L` margin
  without a joint/load exchange, but it arrived `0.102T` later
  (`15.3668T/-0.01589` versus `15.2650T/-0.01575`) and remains one crossing.
  Preserve the carrier-separated observation for a direct replicate and call
  it robust only if miss remains at or below `0.461L` with arrival no slower
  than `15.3668T` and the same wake/reserve/load class. The proxy coefficients
  are carrier-specific rather than transferable gains; refit or remove them
  if joint rates no longer explain lateral carrier motion or a held-out
  carrier, inflow, or morphology changes the relation.
