...
- Treat rapid target-line rotation as a tangential-margin diagnostic, not a
  ...
  spillover-gain tuning. If carrier phase is revisited, derive it from the
  contemporaneous unsteered joint-state carrier and preserve the
  carrier-opposing half-cycle; retain that mechanism only if capture miss and
  both-joint dwell improve together without worse arrival, loads, or compact
  wake. This boundary applies to residual modulation of `abs(carrier)`, not
  direct torque commands or a different carrier whose correction semantics
  differ.
