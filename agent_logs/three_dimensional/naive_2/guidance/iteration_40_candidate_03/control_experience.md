...
- Treat sending/receiving reserve gates as joint-quality safeguards, not a
  route-centering mechanism, and stop iterating gates around the same
  `residual * abs(carrier_acceleration)` channel on this carrier. The initial
  trade remains real: tail-only cubic predicted miss arrived at `15.1403T`
  with `0.651L` head-relative course miss and `1.269%` posterior `>40 deg`
  dwell; unconditional absolute-reserve spillover uniquely centered to
  `0.362L` but incurred `0.285/0.996%` anterior/posterior dwell; dual absolute
  reserve gave `15.2957T/0.556L` and `0/0.898%`; and fully directional reserve
  gave `15.3385T/0.461L` and `0/0.681%`. All retained the direct compact wake.
  New completed evidence closes the tempting hybrid and carrier-phase paths.
  Receiver-safe shedding first eliminated all angle dwell but only reached
  `0.516L` miss; its inherited repeat arrived at `15.2308T/-0.01818` with
  `0.619L` miss and `0/0.397%` dwell while raising peak normalized planar
  force/moment to `0.04087/0.01968`. Contemporaneous analytic carrier-phase
  gating also stayed at `0.619L` miss and worsened posterior dwell and loads to
  `2.233%` and `0.04184/0.01961` at `15.2469T/-0.01832`. Thus lower dwell,
  earlier arrival, or retained capture alone does not rescue an allocator;
  neither delayed action phase, analytic carrier phase, nor receiver shedding
  jointly preserved margin and load quality. The subsequent envelope-aware
  posterior target also failed as a replacement: amplitude contraction kept
  capture and lowered loads to `0.03506/0.01724`, but slowed arrival to
  `15.8431T`, widened miss to `0.725L`, and retained `0.174%` posterior dwell.
  Scheduling directional phase-space reserve only near the target recovered
  `15.2650T/-0.01575`, `0/0.360%` dwell, and `0.03781/0.01864` loads, yet its
  `0.529L` miss remained worse than directional reserve's `0.461L`; avoid more
  posterior amplitude contraction or reserve-threshold variants. Treat raw
  constant-course miss as carrier-phase contaminated on this gait:
  across five direct compact-wake captures, the pre-terminal body-frame sway
  was consistently `84.2--92.7%` explained by a two-joint-rate proxy with
  coefficients confined to `-0.1384-- -0.1419` anterior and
  `0.0783--0.0852` posterior. Offline subtraction reduced within-`3L` miss
  standard deviation from `0.709--0.739L` to `0.468--0.493L`. Completed CFD
  now supports that observation change: subtracting the normalized proxy only
  from near-target predicted-miss velocity retained capture and the direct
  compact wake, improved terminal course miss from the directional benchmark's
  `0.461L` to `0.305L`, reduced anterior/posterior dwell from `0/0.681%` to
  `0/0.358%`, and reduced peak normalized planar force/moment from
  `0.03959/0.01889` to `0.03856/0.01846`. Arrival moved only from `15.3385T`
  to `15.3668T`, while score changed from `-0.01502` to `-0.01589`; do not let
  that scalar obscure the joint margin/reserve/load improvement. Preserve raw
  far-field course, closing, and timing plus the response-and-miss handoff when
  using carrier separation. Because inherited identical-controller repeats
  have varied terminal miss by several tenths of a body length, require a
  semantic replicate at or below `0.461L` with the same joint/load class before
  stacking another terminal actuator. Refit or remove the proxy under another
  carrier if its explanatory power, coefficient stability, or replicate margin
  fails rather than treating its coefficients as universal.
