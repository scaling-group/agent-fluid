...
- Separate terminal speed improvement from terminal-margin improvement on the
  ...
  `0.747L`. Thus neither extra terminal geometric feedback nor wrong-side
  force response has evidenced a robust margin, and the tail residual's scalar
  gain is mainly speed evidence. On this carrier, test a distinct approach
  mechanism that separates thrust from steering, activates only on a closing
  excess-miss course, and vanishes on centered courses; retain it only if it
  reduces terminal course-miss spread without losing capture, the direct
  route, compact wake, joint reserve, or the established load scale. Do not
  transfer this ranking to a different carrier or imposed-wake regime without
  repeating the body-frame reconstruction.
- Treat one-joint reserve sensing as control redistribution, not reserve
  protection. Among the current direct-quiescent compact-wake captures, moving
  the cubic predicted-miss residual away from a reserve-poor tail reduced
  reconstructed terminal constant-course miss to `0.362L`, versus
  `0.573--0.651L` for response release, posterior drive relief, and the
  tail-only residual. It also produced a mostly targetward final velocity
  `(-1.348,-0.231)L/T` and lowered peak normalized planar force/moment from the
  tail-only residual's `0.04041/0.01902` to `0.03970/0.01889`. This is a real
  terminal-course improvement even though arrival/score weakened from
  `15.1403T/-0.01094` to `15.4464T/-0.01764`. But posterior `>40deg` dwell only
  fell from `1.269%` to `0.996%`, anterior dwell appeared at `0.285%`, and
  near-rate occupancy remained `23.9/22.1%`: unconditionally transferring the
  residual's complement merely moves some demand to an unobserved actuator.
  On this carrier, make any further allocator observe both joints and shed
  extra residual when neither has reserve; retain it only if capture and the
  `0.362L` course class survive while both joint and load quality improve.
  Do not generalize this allocation ranking to static-curvature steering, a
  different carrier, or imposed-wake operation without repeating the
