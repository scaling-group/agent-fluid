...
- Once target-versus-velocity course feedback established a useful approach,
  ...
  A byte-identical inherited repeat retained that coherent low-load wake but
  passed below the disk at `0.9387L` and escaped lower-left; at closest
  approach its joint-phase-compensated yaw residual was `0.4200 rad/T` versus
  `0.0045 rad/T` in the capture.  Thus one threshold crossing does not
  establish repeat margin even when code, carrier, and peak loads agree:
  preserve the measured course/miss geometry, but test a bounded terminal-only
  response residual and require repeat capture or a closer pass without loss
  of pre-approach translation.  A separate inherited iteration that removed
  an instantaneous joint-state estimate of carrier lateral velocity from the
  predictor also regressed to a `0.9016L` near miss and lower-left exit despite
  lower rate occupancy.  Do not treat cleaner latent signals or smaller
  non-capture minima as better than capture, and do not retry predictor-side
  carrier subtraction without repeated wake-crossflow evidence.  These bounds
  are established only for direct-uniform still water; under actual wakes,
  reject instantaneous response feedback if alternating crossflow causes
  wrong-side correction, and then test a history-based slow residual while
  requiring capture, wake coherence, joint reserve, and low loads together.
