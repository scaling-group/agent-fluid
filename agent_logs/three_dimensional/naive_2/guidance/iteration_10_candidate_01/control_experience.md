...
- For terminal interception, a rotation-invariant target/course residual and a
  ...
  the pass from the repeated `2.595--4.650L` high-side class to `1.276L` below
  ...
  peak normalized planar force/moment. That result still exited left: at its
  closest point the fish moved down-left at `1.067L/T` while the target lay
  up-left, even though its head had begun to rotate correctly. Preserve the
  far-field course residual and smooth mean-bend handoff; do not interpret the
  remaining miss as a request for more global asymmetry or a larger static
  bend. Test drive relief or a damped posture only when normalized proximity,
  positive closing speed, and course misalignment coincide. Falsify this
  implication if such conditional hold loses the `1.033L` approach or coherent
  far-field wake, coasts on an aligned approach, or increases limit/load
  occupancy without capture or a better termination class.
