...
- Treat sender reserve, receiver reserve, and carrier-phase inference as
  ...
  limit dwell does not establish a robust combination. Eight same-carrier,
  ...
  carrier. If the fully directional baseline is extended, test a genuinely
  predictive joint-state reserve such as oscillator-normalized phase-space
  excursion rather than another instantaneous sign proxy; retain it only if it
  preserves capture and the compact wake, keeps miss at or below `0.461L`,
  anterior/posterior dwell at or below `0/0.681%`, loads near or below
  `0.040/0.019`, and arrival within the `15.14--15.34T` class. Re-evaluate this
  ranking under a different carrier or initial condition rather than treating
  any reserve convention as universal.
- Separate carrier-contaminated terminal observations before inventing another
  actuator allocator, and do not treat angular-envelope sharing as sufficient
  protection when the steering center itself moves. The assigned parent's
  reserve-blocked anterior wave-center shift retained capture, early arrival
  (`15.2745T`), the direct compact wake, and low `0.03894/0.01874` peak
  normalized force/moment, but widened head-relative course miss to `0.632L`
  and renewed `0.648/1.044%` anterior/posterior `>40 deg` dwell. This falsifies
  that center shift as a remedy for additive spillover; avoid combining it with
  more reserve thresholds, projection scalars, or gain tuning on this carrier.
  In contrast, a sampled carrier-separated predictor improved its scheduled-
  reserve parent's miss from `0.529L` to `0.305L` while retaining capture,
  `0/0.358%` dwell, `0.03856/0.01846` loads, and a `15.3668T` arrival. The
  mechanism subtracts normalized two-joint-rate-correlated sway only from the
  near-target predicted-miss observation, leaving raw velocity in far-field
  course, closing, and timing. Treat this as evidence for phase-clean terminal
  observation, not universal fitted coefficients: its pre-terminal regression
  was stable only across the same carrier (`R^2=0.842--0.927`). Preserve or
  refit the proxy only when that explanatory power survives a changed carrier
  or condition, and require a repeat capture with miss at or below `0.461L`,
  posterior dwell at or below `0.681%`, loads near `0.040/0.019`, and the
  direct compact wake because identical-controller terminal margins have
  previously varied.
