...
- Preserve measured target-versus-course error as the strongest evidenced
  ...
  does not establish capture by itself. Preserve its unsaturated traveling
  carrier outside the terminal region, and test a continuous handoff inside
  roughly the evidenced `3L` divergence region to a persistent body-frame
  geometric actuator such as bounded mean curvature. The handoff hypothesis
  is falsified if it loses the `1.173L` approach, repeats the lower-left escape,
  or introduces joint dwell/load growth without capture; do not claim it as a
  positive terminal lesson until later CFD confirms the new candidate.
