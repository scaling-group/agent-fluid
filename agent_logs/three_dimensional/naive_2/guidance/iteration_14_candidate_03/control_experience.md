...
- Treat a threshold capture as mechanism evidence, not yet as a robust
  ...
  carrier gains or load relief do not address the observed sensitivity. Keep
  the body-frame time-to-closest/signed-miss mean-curvature mechanism, but
  require capture margin or repeat capture before promotion; when the repeat
  misses on the requested side, test a response-gated terminal actuator
  handoff and falsify it on unchanged closest pass/termination or degraded
  wake, joint, or load quality.
