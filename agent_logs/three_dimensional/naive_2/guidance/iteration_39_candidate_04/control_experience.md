...
- Treat sending/receiving reserve gates as joint-quality safeguards, not a
  ...
  posterior amplitude contraction or reserve-threshold variants. Instead,
  treat raw constant-course miss as carrier-phase contaminated on this gait:
  ...
  standard deviation from `0.709--0.739L` to `0.468--0.493L` for all sampled
  allocators, but is not counterfactual CFD evidence. Test carrier-separated
  miss only as a normalized observation change while preserving raw far-field
  course and the response-plus-miss handoff; retain it only if capture, compact
  wake, miss at or below `0.461L`, posterior dwell at or below `0.681%`, about
  `0.040/0.019` peak loads, and `15.14--15.34T` arrival improve together.
  Refit or remove the proxy under another carrier if its explanatory power or
  coefficient stability fails rather than treating these values as universal.
