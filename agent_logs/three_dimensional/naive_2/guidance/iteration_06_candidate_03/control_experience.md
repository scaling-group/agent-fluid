...
- Phase-compensating body yaw before response feedback is the first sampled
  ...
  and wake coherence.
- The first sampled approach schedulers improve closest approach but expose
  carrier-phase contamination in terminal target geometry. On the same
  phase-compensated carrier, ungated steering reached `3.174L`, distance-only
  relief reached `3.600L`, continuous distance/target-plane hold reached
  `2.703L`, and full-circle misalignment-gated relief reached `2.319L`; the
  latter two then made sharp return arcs and exited the upper boundary, with
  force peaks `0.478--0.512` versus `0.031` for the ungated rollout. Do not
  stack stronger braking or turning on those near-misses. The full-circle
  trace instead shows raw pursuit reversing with beat yaw while the target
  remains on one side (for example `+0.20` to `-0.80 rad` over half a period),
  whereas subtracting the joint-predicted carrier yaw preserves a correct-side
  residual (`-0.27` to `-0.55 rad`). This supports testing phase-separated
  target angle as the input to a bounded approach redirect. Retain it only if
  closest approach beats `2.319L` or termination improves without losing the
  far-field wake, translation, actuator margin, or load quality.
