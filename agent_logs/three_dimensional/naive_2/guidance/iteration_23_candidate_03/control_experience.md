...
- Preserve body-frame constant-course prediction as the first repeatable
  direct-route capture architecture, but treat the terminal pulse as
  phase-sensitive rather than a proven source of success. Three byte-identical
  always-pulse samples capture at `15.983--16.044T` and a response-released
  sibling captures at `16.016T`, all at the `0.7472--0.7500L` first-crossing
  threshold behind a compact alternating 3D wake, with zero joint-angle dwell
  beyond `40 deg`, about `17%` occupancy per joint within the last `10 deg/T`
  of the rate envelope, and peak normalized planar force/moment near
  `0.033--0.037/0.017--0.019`. Yet a byte-identical response-released execution
  missed at `1.01175L`; subtracting a fitted joint-rate sway carrier from the
  predictor missed at `1.02506L`; extending the redirect through the
  target-opposed posterior return missed at `0.92803L`; and removing an
  angle-domain carrier component from terminal bearing missed at `1.10362L`.
  All four inherited variants then exited left while retaining low loads and
  an initially organized wake. Thus pulse presence, response release, and
  extra joint-state phase correction are neither necessary nor sufficient for
  capture. Avoid further pulse/static-bend gain tuning and do not extend
  steering into the target-opposed posterior stroke; if testing a distinct
  actuator allocation, preserve the full carrier and gate the bounded redirect
  on the observed target-signed stroke. Retain it only on repeat capture under
  reflected or perturbed geometry with the same direct route, wake, joint, and
  load class. Because evaluation terminates on the first `0.75L` crossing,
  final distance cannot measure extra capture margin; use repeat semantics and
  arrival/trajectory quality instead.
