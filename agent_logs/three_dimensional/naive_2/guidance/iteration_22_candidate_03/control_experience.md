...
- Preserve body-frame constant-course predicted miss as a useful direct-route
  architecture, but demote its current terminal composite from replicated
  capture to threshold-sensitive. Two byte-identical sampled runs captured at
  `15.983--15.994T` with a compact alternating 3D wake, zero `>40 deg` angle
  dwell, and peak normalized planar force/moment below `0.037/0.019`; later
  inherited evaluations of that exact response/pulse policy instead missed at
  `1.01564L`, `1.12140L`, and `1.21644L` before exiting left. Both a
  no-pulse sibling and a response-released-pulse sibling captured at
  `16.011--16.016T`. Response release left each joint above `95%` of its
  rate envelope for about `18%` of samples and peak normalized planar
  force/moment at `0.0332/0.0166`, essentially the full-pulse capture range.
  Thus pulse presence or response release explains neither repeat reliability
  nor rate reserve; preserve the normalized time-to-closest/signed-miss gate,
  small bounded carrier mean, and measured-response handoff, but avoid more
  pulse, release-gate, or static-bend gain tuning.
  Later CFD falsified terminal observation cleanup as the missing mechanism:
  joint-angle bearing phase separation still exited left at `1.21754L`, while
  recovery-pulse, response-released-pulse, and joint-rate sway-residual
  variants also exited left at `0.9952--1.0251L` despite retaining the direct
  route and roughly `0.033--0.035/0.016--0.018` peak planar force/moment.
  Moreover, the three sampled captures crossed at anterior angle
  `0.636--0.661 rad`, near its positive extreme, whereas those near misses
  passed at `0.338--0.504 rad` with an already near-saturated terminal request.
  Treat this as beat-phase dependence of capture reach, not a reason for more
  bearing, pulse, residual-fit, or curvature gain tuning. The next distinct
  test should change terminal actuation while preserving the posterior wave—for
  example, a bounded target-signed duty-ratio hold that prolongs a useful
  anterior corrective half-cycle—and retain it only if capture margin and
  termination improve without `>40 deg` dwell, wake loss, or load growth.
  Revisit observation-phase cleanup only if held-out evidence shows that it
  changes the slow route rather than merely the instantaneous request.
