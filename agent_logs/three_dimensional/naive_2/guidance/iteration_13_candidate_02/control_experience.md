...
- Treat normalized constant-course miss and time-to-closest as an established
  terminal mechanism in this direct-uniform still-water lane, not merely a
  proposed lead cue. After course-residual steering reached `1.173L`,
  distance-only mean curvature reached `1.033L`, closing-speed braking
  regressed to `2.167L`, and fixed-horizon aim reached `1.158L`, the
  predicted-miss gate produced the first sampled capture at `16.011T` and
  `0.748L` with score `-0.028`. Both wake views retain the alternating
  propulsive structure, maximum joint angles remain `37.4/36.6 deg`, and peak
  normalized planar force/moment remain only `0.030/0.017`. Preserve its
  traveling carrier, course steering, and anticipatory curvature before
  testing terminal refinements; do not revert to static posture, scalar drive
  relief, or distance-only/fixed-lead steering. This lesson applies when
  target and velocity share a normalized body frame and a stable course is
  already established. It is falsified outside the evidenced lane if the
  prediction activates on non-closing motion or beat-scale slip, or if a
  refinement loses capture, delays arrival without a physical-quality gain,
  creates joint/load occupancy, or disrupts the coherent wake.
