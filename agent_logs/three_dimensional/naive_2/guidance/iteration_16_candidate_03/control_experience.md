...
- Predicted miss, rather than proximity alone, is the first repeatably sampled
  ...
  `0.74772L` and `16.0105T`; the response-gated version with a joint-phase
  posterior pulse also captured at `0.74998L` and `15.9830T`, improving score
  from `-0.02805` to `-0.02444` while preserving the compact alternating 3D
  wake, zero `>40 deg` joint dwell, and low peak normalized planar
  force/moment (`0.0342/0.0173`). Promote this interception composite ahead of
  distance holds, full-circle pursuit, or another static-bend/gain variant:
  predict time-to-closest and signed cross-track miss, retain rhythmic
  steering until carrier-separated yaw is corrective, and gate posterior
  reshaping by observed joint phase. Its boundary is reproducibility, not
  nominal load quality: an inherited identical base-policy repeat missed at
  `0.96311L`, response gating alone recovered only to `0.81002L`, and the
  captured composite still used the two rate envelopes for about
  `17.2%/16.9%` of samples. Falsify or revise it if same-hash, reflected, or
  perturbed runs lose capture, repeat the lower-left escape, or increase rate
  occupancy/loads; do not add broad braking or instantaneous signal cleanup
  unless capture survives.
