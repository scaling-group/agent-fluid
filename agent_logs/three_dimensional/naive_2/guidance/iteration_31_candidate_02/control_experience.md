...
- Use carrier-separated corrective response and residual predicted miss
  ...
  in the best unified sample. A centered-approach amplitude envelope likewise
  retained capture but crossed laterally with `0.679L` miss at `15.6738T`, and
  anterior terminal duty skew slowed arrival to `15.9148T`, crossed at
  `(-0.711,-1.038)L/T` with `0.717L` miss, and introduced `0.138%` posterior
  `>40 deg` dwell. Do not treat carrier-separated body yaw alone as a redirect-
  completion reference, harden its countersteer, brake this carrier, or retry
  beat-residence skew for robustness: all preserve capture and the compact wake
  without narrowing the repeated lateral course class. Preserve the shared
  response-plus-miss handoff and test a distinct normalized physical-response
  actuator only if it leaves route authority and carrier drive intact; retain
  it only with both capture and reduced terminal-course spread while the direct
  route, compact wake, joint reserve, and `0.037/0.019` load scale remain
  intact. Repeat capture, lower effort, or one centered score alone does not
  establish margin robustness.
...
- Treat the unified response-plus-miss release as a control-allocation
  boundary, not spare authority that must be reassigned. On the same compact-
  wake carrier, the best unified sample captured at `15.5008T` with `0.179L`
  raw terminal course miss. Reassigning released shared steering to
  instantaneous target-line rate (`0.674L`), a two-estimate persistence
  consensus (`0.747L`), posterior phase-lag modulation (`0.727L`), or the
  assigned parent's normalized yaw-moment rejection (`0.738L`) all retained
  capture but crossed later and laterally; their peak normalized planar
  force/moment stayed in the benign `0.0354--0.0371/0.0174--0.0183` band, so
  low loads and capture do not rescue the mechanism. Avoid further sign,
  scale, persistence, or actuator variants that spend authority after this
  release. If testing physical response on this carrier, leave route authority
  and the common handoff intact, gate a distinct corrective actuator only
  while the redirect remains unmet, and require reduced replicated terminal-
  course spread as well as capture; revisit post-release allocation only with
  a different carrier or a signal that separates centered and lateral
  histories before termination.
