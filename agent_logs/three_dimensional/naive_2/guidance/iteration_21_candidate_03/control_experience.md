...
- Body-frame constant-course prediction is the first mechanism in this
  ...
  `4L` range and flips with beat phase near `2L`. Preserve measured velocity
  for broad navigation, but test terminal interception on carrier-residual
  course before adding more curvature or gain; retain that separation only if
  its rate-to-sway relation survives held-out carrier/geometry data and repeat
  capture margin improves without changing the direct route, wake/load class,
  or rate occupancy. Related inherited executions still missed at `0.9631L`,
  `0.8100L`, and `1.2164L`, and a phase-envelope startup-recovery channel
  regressed to `3.8633L/left_domain`, so threshold-level nominal capture or a
  same-case fitted residual is not yet general evidence.
