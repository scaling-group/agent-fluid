...
- Body-frame constant-course prediction is the first mechanism in this
  lineage to cross from closer passes into sampled capture. Four sampled
  executions spanning ungated and response-released posterior redirects
  preserve the full traveling-bend carrier, recruit a small bounded mean bend
  from time-to-closest and signed cross-track miss, and capture at
  `15.983--16.044T` and `0.7472--0.7500L` behind a compact alternating 3D
  wake; joint angles remain below `37.9 deg` and peak normalized planar
  force/moment stays near `0.033--0.037/0.017--0.019`. This supports course
  prediction and early interception rather than proximity-only bearing, broad
  carrier braking, or stronger propulsion. It also exposes a terminal
  estimator boundary: across all four trajectories, a two-joint-rate carrier
  model explains `92.1--92.5%` of far-field body-frame lateral-velocity
  variance, while raw instantaneous predicted miss varies by about `1L` near
  `4L` range and flips with beat phase near `2L`. The assigned parent now
  falsifies the same-case fitted-residual proposal: subtracting the evidenced
  `-0.13*phi_dot[1] + 0.0685*phi_dot[2]` sway only inside the terminal
  predictor still produced `1.0251L/left_domain`, no semantic improvement over
  the preceding response-released result at `1.0117L/left_domain`. Recovery
  pulse and related inherited executions also missed at `0.9952L`, `1.2164L`,
  `0.9631L`, and `0.8100L`; a phase-envelope startup-recovery channel regressed
  to `3.8633L/left_domain`. Preserve measured velocity for broad navigation,
  but do not reuse a trajectory-fitted joint-rate sway model as terminal
  robustness evidence. A distinct head-centered geometry-rate channel is
  justified because capture is a head crossing: test inertial line-of-sight
  rate under positive closure, and retain it only if capture margin and
  termination improve without changing the direct route, compact wake/load
  class, reflection behavior, or rate occupancy.
