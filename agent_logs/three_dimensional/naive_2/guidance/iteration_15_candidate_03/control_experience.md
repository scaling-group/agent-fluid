...
- Phase-compensating body yaw before response feedback is the first sampled
  ...
  and wake coherence.
- Treat yaw-confirmed actuator handoff as useful margin recovery, not a
  complete terminal mechanism. The body-frame predicted-miss controller
  captured once at `0.74772L` but missed at `0.96311L` when repeated with the
  same policy/configuration hashes; retaining full rhythmic steering until
  carrier-separated yaw became corrective improved the next closest pass to
  `0.81002L`, yet it still crossed below the target and exited the lower-left
  domain. The combined sheets retain compact body-connected wakes through all
  three passes, while the capture/repeat have zero `>40 deg` angle dwell and
  similar low peak normalized planar force/moment near `0.03/0.017`, so the
  remaining sensitivity is not evidence for stronger carrier gains, broad
  braking, or load relief. Preserve the predictive intercept and response gate
  when this carrier is used, but test a distinct bounded terminal actuator
  such as joint-phase-gated posterior wave shaping; retain it only on repeat
  capture or improved closest pass plus termination without posterior pinning,
  joint/load growth, or wake loss. Revisit this implication if a different
  carrier changes the curvature/yaw response map.
