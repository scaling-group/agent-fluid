...
- Keep carrier-phase separation in the response gate and retain raw measured
  terminal geometry. Geometry-qualifying a response-released posterior pulse
  reproduced capture with the best current sampled score (`-0.02335`) and
  lowest distance integral (`1.90579L`), while preserving zero `>40 deg`
  joint dwell, about `17%` near-rate occupancy per joint, and the compact
  `0.0348/0.0175` peak normalized force/moment class. Qualifying shared-joint
  release with a lateral corridor also captured and arrived `0.106T` earlier,
  but scored worse, crossed lower, and introduced `0.28%` posterior angle
  dwell; retaining more redirect authority is therefore compatible with
  success but not automatically a margin or efficiency gain. Inherited
  identical-controller repeats still missed at `1.2164L` (always pulse) and
  `1.0117L` (response release), while replacing target geometry with an
  angle-domain carrier estimate lowered near-rate occupancy to about
  `10.0/10.5%` yet missed at `0.8293L` and exited left. Treat conjunction of
  corrective response and small remaining body-frame miss as a reusable
  handoff condition, not proof of repeat robustness or permission to increase
  gains; retain it for another steering channel only if capture/closest pass
  and termination improve together without degrading route, wake, joint
  reserve, or loads. Two follow-on mechanisms preserved the direct compact
  wake and `capture` termination but failed that margin test. Transferring
  released shared-joint authority to active yaw arrest arrived at `15.806T`
  with zero `>40 deg` dwell and comparable loads, yet widened raw terminal
  constant-course miss to `0.751L`, beyond the unified handoff's evidenced
  `0.179--0.590L` repeat range. Replacing terminal pursuit direction with
  instantaneous rotation-invariant predicted miss arrived at `15.774T`,
  widened that miss to `0.678L`, and introduced `0.42%` posterior `>40 deg`
  dwell; both also worsened score and distance integral relative to the best
  unified sample. Do not hand released authority to yaw arrest or replace raw
  terminal direction with instantaneous predicted miss on this carrier merely
  because either remains a capture. Retain the unified response-plus-small-miss
  handoff as the baseline unless a distinct mechanism improves the terminal
  course boundary or termination together with route, wake, reserve, and
  loads; revisit either rejected translation only with a different carrier or
  temporally robust response observation.
