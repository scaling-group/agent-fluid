...
- Use carrier-separated corrective response and residual predicted miss
  ...
  as score.
- A closing, tail-only cubic predicted-miss residual is the first sampled
  terminal actuator after the shared handoff to improve traversal materially:
  against instantaneous target-line steering it improved score from
  `-0.01965` to `-0.01094`, arrival from `15.6893T` to `15.1403T`, and distance
  integral from `1.90130L` to `1.89264L` while retaining capture and the compact
  alternating 3D wake. Treat that residual as a useful carrier-compatible base,
  not as evidence of terminal robustness. It still crossed with `0.651L`
  constant-course miss, `1.745/T` absolute target-line rate, higher
  `0.0404/0.0190` peak normalized planar force/moment, and `1.27%` sampled
  `>40 deg` joint dwell; carrier-separated force re-engagement and persistent
  line-rate consensus were slower and ended with `0.744--0.747L` misses. The
  reusable implication is to preserve a directional posterior residual for
  translation but schedule any new approach-hold mechanism separately and make
  it vanish on centered courses. Retain the combination only if it reduces
  predicted miss below `0.590L` while preserving capture, the arrival/distance
  advantage, far-field route, wake coherence, joint reserve, and load scale;
  otherwise the residual is only a faster threshold crossing and should not be
  strengthened or mistaken for margin improvement.
