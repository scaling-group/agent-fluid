...
- Separate beat-scale yaw from directional response before feeding yaw back to
  ...
  approach turn radius, not weak propulsion. Preserve the phase separation;
  after broad target-directed motion works, test normalized distance plus the
  full body-frame target vector to trade symmetric drive for turning only
  during approach misalignment. Do not obtain this reserve with a general rate
  brake: the inherited barrier reduced mean speed to `0.279L/T`, regressed
  closest approach to `11.643L`, and repeated the upper exit. This lesson is
  falsified if conditional approach relief loses the alternating wake or fails
  to make a tighter or returning target arc than the `3.174L` pass.
