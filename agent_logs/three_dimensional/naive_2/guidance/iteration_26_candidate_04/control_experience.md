...
- Promote the unified carrier-separated-response plus raw predicted-miss
  handoff over response-only release on this carrier. Semantically identical
  unified code captured twice at `15.501T` and `15.663T`, versus `15.983T` for
  the response-only prefill, while preserving the direct compact wake, zero
  `>=40 deg` joint dwell, roughly `17--18%` near-rate occupancy, and at most
  `0.0366/0.0181` peak normalized planar force/moment. This converts an earlier
  single threshold crossing into repeat semantic evidence: measured corrective
  yaw may release both rhythmic steering channels only as raw body-frame
  predicted miss becomes small. It does not establish deterministic pose or
  capture margin—the two repeats crossed at visibly different beat phases,
  and earlier response-only repeats missed at `1.012--1.216L`. Do not replace
  measured geometry with a joint-angle carrier estimate: that branch lowered
  rate occupancy but missed at `0.829--1.104L` before exiting left. Also avoid
  another scalar or phase gate on the posterior pulse under this carrier; a
  carrier-acceleration-aligned pulse still captured but worsened score and
  distance integral to `-0.02503/1.90677L` without improving reserve or loads.
  Revisit pulse phase selection only if a different carrier exposes destructive
  half-cycle interference, and require margin, trajectory, reserve, or load
  improvement rather than another threshold-level capture.
