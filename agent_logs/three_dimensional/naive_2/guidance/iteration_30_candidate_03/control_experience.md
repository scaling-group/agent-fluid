...
- Use carrier-separated corrective response and residual predicted miss
  ...
  a redirect-completion reference or harden that countersteer. Preserve the
  shared response-plus-miss handoff and test whether target-relative course
  alignment should qualify its return to cruise; retain such a mechanism only
  with both capture and reduced terminal-course spread while the direct route,
  compact wake, joint reserve, and load scale remain intact. Repeat capture,
  lower effort, or one centered score alone does not establish margin
  robustness.
- Stop iterating on redirect-release qualifiers or hydrodynamic-yaw-moment
  rejection for this carrier. Requiring target-relative course agreement in
  the response-plus-miss release still captured only at `15.8926T`, crossed
  with `(-0.721,-1.072)L/T` velocity and about `0.719L` raw course miss, and
  scored `-0.02329`. Two evaluations of the exact same normalized-moment
  residual also remained lateral at `(-0.792,-0.954)` and
  `(-0.721,-1.081)L/T`, while scores spread from `-0.02303` to `-0.02716`
  despite compact wakes, bounded `0.0354--0.0375/0.0175--0.0181` peak planar
  force/moment, and capture in both runs. This rejects another release gate,
  yaw/moment sign change, or moment-gain edit as a robustness step and shows
  that capture plus one scalar score cannot rank margin under identical-policy
  variability. If the unified carrier is retained, isolate a different
  rhythmic actuator channel and require reduced terminal-course spread across
  repeat evidence without worsening arrival, wake, joint reserve, or loads;
  do not promote a new channel from one centered crossing alone.
