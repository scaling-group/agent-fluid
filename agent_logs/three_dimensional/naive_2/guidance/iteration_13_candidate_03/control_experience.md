...
- Do not infer a universal terminal common-curvature sign from one quiet
  ...
  peak normalized force/moment as high as `0.890/0.414`.  In contrast, gating
  the same carrier's small mean bend by normalized body-frame time-to-closest
  and signed constant-course miss captured at `0.74772L` in `16.01T`, retained
  the coherent alternating 3D wake, had zero `>40 deg` joint dwell, and limited
  peak normalized force components/moment to `0.024/0.030/0.017`.  Prefer
  predictive cross-track recruitment over more static-bend sign, gain, or
  release-threshold iteration when an established course would otherwise miss;
  keep both rhythmic half-cycles active.  Falsify this implication if an aligned
  approach redirects early, capture/wake/load quality is lost, or reflected or
  perturbed initial geometry shows the learned curvature sign or bounded
