...
- In the direct-quiescent L64 seed rollout, a coherent self-generated wake and
  ...
  and the fish exited the upper boundary at 8.60 T. Across four later samples
  with the same carrier, steering by two-joint bearing/slip half-cycle
  asymmetry preserved the alternating 3D wake and achieved the strongest
  targetward displacement (`2.041L` left; `11.347L` closest distance), while
  posterior static mean curvature moved only `1.056L` left and reached
  `12.056L`; tail-only half-cycle variants were intermediate at
  `11.670--11.867L`. Prefer bounded beat asymmetry over persistent curvature
  when this carrier already propels, but do not call the mechanism sufficient
  until termination topology also changes: every sample still drifted about
  `1.20L` upward, hit the joint-rate envelope, and exited the upper boundary
  near `9T`. Falsify the ranking under a different carrier or if half-cycle
  control loses wake coherence, increases load/limit occupancy, or fails to
  improve closest approach and termination together.
