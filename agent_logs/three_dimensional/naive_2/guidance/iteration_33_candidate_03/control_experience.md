...
- Treat rapid target-line rotation as a tangential-margin diagnostic, not a
  ...
  line-rate release gates, or actuator remapping on this carrier. The proposed
  carrier-separated body-force discriminator is now also falsified: despite
  its residual having the requested sign on `79.5%` of the centered capture's
  `distance<1L` samples versus `52.5--62.5%` for wider captures, feedback on
  the wrong-sign residual crossed at `15.7384T/-0.01874` with `0.744L`
  predicted miss, effectively the same wide class as line-rate consensus,
  while only preserving the existing `0.0356/0.0176` load class. A tail-only
  cubic predicted-miss residual improved arrival, score, and mean distance to
  `15.1403T/-0.01094/1.89264L`, but its `0.651L` miss still exceeded the
  inherited `0.590L` repeat boundary and it raised peak normalized
  force/moment to `0.0404/0.0190` plus posterior `>40 deg` dwell to `1.269%`.
  Do not scale either additive residual: scalar speed/progress can improve
  while terminal margin and actuator quality worsen. The inherited
  predicted-capture-corridor release also rejects broad return-to-carrier as
  the remedy. Simultaneously releasing mean bend, posterior pulse, and shared
  rhythmic steering inside a closing `0.5L` predicted corridor retained the
  compact-wake capture, but slowed arrival to `15.9109T`, scored `-0.02184`,
  crossed with `0.7155L` miss and velocity `(-0.702,-1.049)L/T`, raised peak
  normalized force/moment to `0.03765/0.01844`, and introduced `0.104%`
  posterior `>40 deg` dwell. A predicted safe crossing therefore does not
  imply that every target-correction channel is dispensable. Avoid fanning one
  corridor gate across persistent offsets, posterior pulses, and rhythmic
  course authority; test channel-selective release that preserves the
  response-gated half-cycle correction, and retain it only if capture and
  course margin improve together with route, wake, joint reserve, and the
  lower load class intact.
