...
- Treat residual representation, sending-joint reserve, and receiving-joint
  reserve as one terminal mechanism; lower dwell or retained capture alone
  does not validate any allocator. Four sampled direct-quiescent captures have
  visually comparable direct routes and compact alternating 3D wakes, but a
  tail-only cubic predicted-miss residual trades its earliest arrival
  (`15.1403T`) for `0.651L` head-relative course miss, `1.269%` posterior
  `>40 deg` dwell, and `0.04041/0.01902` peak normalized planar force/moment.
  Dual absolute reserve improves loads to `0.03872/0.01876` but leaves
  `0.556L/0.898%` miss/dwell; fully directional reserve improves the compromise
  to `15.3385T/0.461L`, zero/`0.681%` anterior/posterior dwell, and
  `0.03959/0.01889` loads. Delayed previous action is not a useful
  carrier-direction proxy: it regresses to `0.631L` miss and `1.878%`
  posterior dwell at comparable wake topology.

  Completed inherited evidence closes the tempting absolute-tail,
  receiver-safe hybrid: identical-policy evaluations varied from `0.516L` to
  `0.619L` miss, and the repeat raised peak loads to `0.04087/0.01968` despite
  reducing dwell. Analytic carrier-phase gating likewise retained `0.619L`
  miss while worsening posterior dwell to `2.233%`; do not tune more action
  scales, carrier-sign proxies, or receiver-spill thresholds around
  `residual * abs(carrier_acceleration)`. Moving the residual into an
  amplitude-contracted posterior wave target is a different but still negative
  result: it lowered loads/dwell to `0.03506/0.01724` and `0.174%`, yet widened
  miss to `0.725L`, delayed arrival to `15.8431T`, and scored `-0.02283`.

  A fixed oscillator-normalized phase-space projection is the first allocator
  in this group to improve terminal geometry, angle reserve, and loads together
  (`0.421L` miss, zero `>40 deg` dwell, `0.03628/0.01798`), but its
  `15.5850T/-0.02349` arrival/score shows that a linear look-ahead can shed
  useful authority too early. Preserve anticipatory current-state reserve as
  the reusable implication, not that projection horizon: test a dynamically
  scaled reserve such as directional stopping margin, and retain it only if it
  keeps capture, the direct compact wake, miss no worse than `0.421L`, zero
  anterior and at most `0.681%` posterior dwell, the lower load class, and an
  arrival meaningfully nearer `15.14--15.34T`. Re-evaluate these boundaries
  under another carrier or initial condition rather than treating the reserve
