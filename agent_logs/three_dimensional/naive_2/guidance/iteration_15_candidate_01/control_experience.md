...
- Phase-compensating body yaw before response feedback is the first sampled
  ...
  and wake coherence.
- Treat a threshold capture as mechanism evidence, not yet as a robust
  incumbent. The predicted-miss policy under the same policy and config hashes
  captured once at `0.74772L`, but its assigned-parent repeat missed at
  `0.96311L` and exited lower-left. Both retained compact alternating wakes,
  zero `>40 deg` joint dwell, and nearly identical low peak normalized
  planar-force/moment scales (`0.030/0.017` versus `0.029/0.017`), so stronger
  carrier gains or load relief do not address the observed sensitivity. A
  later response-gated handoff that retained rhythmic steering until
  carrier-separated yaw became corrective improved the repeat closest pass to
  `0.81002L` and reduced near-rate-limit occupancy from `9.40%/10.02%` to
  `8.59%/8.98%`, while preserving zero large-angle dwell and the low
  `0.0348/0.0174` peak force/moment scale. It nevertheless crossed below the
  target at `1.117L/T` with wrong-sign residual yaw and exited the lower
  boundary. Keep the body-frame time-to-closest/signed-miss mean-curvature and
  response-gated release mechanisms, but do not promote them or tune the fade
  alone: test a distinct phase-selective actuator for wrong-sign terminal yaw,
  and require capture or a better closest pass and termination together
  without degraded wake, joint, or load quality.
