...
- For the tight capture radius, predict interception from body-frame target and
  translational velocity instead of waiting for proximity or a passed-target
  event. On the established traveling carrier, pursuit/course blending and
  signed constant-course miss recruited bounded terminal curvature and first
  captured at `16.011T`; however, an assigned-parent rollout with identical
  policy, configuration, body, and geometry hashes missed at `1.034L` and
  exited left. Treat one threshold crossing as support for the architecture,
  not repeatability. A yaw-response-gated handoff improved an inherited repeat
  miss to `0.810L` without capture; adding a closing, anterior-phase-gated
  posterior wave-shape pulse then produced the best sampled result, capture at
  `15.983T` and score `-0.02444`, while the compact alternating top-down/3D
  wake persisted. By contrast, continuous approach drive relief formed a broad
  post-pass curl (`2.703L` closest, left exit). The reusable mechanism is early
  cross-track interception plus brief phase-gated posterior authority, not the
  sampled gains: apply it only after coherent self-propulsion exists and the
  target/velocity vectors share a calibrated body frame. Require replicated
  capture before calling it robust, and falsify it if another miss, lost
  pre-approach translation, posterior pinning, joint/load growth, incoherent
  wake, or noisy/near-zero-speed miss estimates recur.
