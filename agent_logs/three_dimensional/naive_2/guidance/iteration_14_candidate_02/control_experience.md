...
- Do not infer a universal terminal common-curvature sign from one quiet
  closest-pass state, but distinguish proximity/bend-threshold curvature from
  course-predicted interception. A closing-speed hold reached `1.00784L` and
  reversing only its curvature sign worsened the pass to `1.106L`; sampled
  mean/posterior bend releases likewise missed at `2.664--3.312L` and raised
  peak normalized force/moment as high as `0.890/0.414`. Gating the same
  carrier's small mean bend by normalized body-frame time-to-closest and signed
  constant-course miss produced the only sampled capture (`0.74772L` in
  `16.01T`), a coherent alternating 3D wake, zero `>40 deg` joint dwell, and
  peak normalized force components/moment of only `0.024/0.030/0.017`.
  However, a byte-identical repeat missed at `0.96311L` and exited left; an
  actuator-aware sibling reduced outward near-rate-limit commands from about
  `12%` to `2.7%` per joint but also missed at `0.87559L` with the same exit
  topology. Preserve predictive cross-track recruitment, but treat its first
  capture as a narrow-margin mechanism rather than robust success. Do not
  substitute rate-reserve protection, another static-bend sign, or broad drive
  relief for terminal geometry; when the mean-bend handoff suppresses the
  faster steering channel while predicted miss remains outside a capture
  corridor, test a bounded geometry- and response-time-gated authority reserve
  and require repeat capture without increased limits, loads, or wake damage.
  Falsify this implication if aligned approaches redirect early or reflected,
  perturbed, or repeated rollouts show the corridor, curvature sign, or
  prediction horizon is case-specific.
