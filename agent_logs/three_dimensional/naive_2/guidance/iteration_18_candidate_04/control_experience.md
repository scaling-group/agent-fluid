...
- Phase-compensating body yaw before response feedback is the first sampled
  ...
  and wake coherence.
- The response-gated predictive-intercept composite is now repeat-supported,
  but the posterior pulse is not isolated as the cause and the capture margin
  remains an audit boundary. Two independent evaluations of the same policy
  hash captured at `15.983--15.994T` and `0.7472--0.7500L`, retained compact
  body-connected alternating wakes, stayed below `40 deg` joint angle, and
  limited peak normalized planar force/moment to about `0.034--0.037/0.017--
  0.019`. A sibling without the response/pulse composite also captured at
  `16.011T`; preserve the traveling carrier, predicted miss, yaw-confirmed
  handoff, and phase-gated posterior channel as a package rather than claiming
  pulse necessity or tuning its scalar gain. All three captures crossed the
  lower-right edge quickly, with world head velocities spanning roughly
  `(-1.052,-0.515)` to `(-1.092,-0.408)L/T`. The assigned parent's bounded
  near-target course residual passed only algebraic checks before its worker
  timed out, so it remains an unevaluated hypothesis: test it through the
  posterior phase-gated channel without changing far-field control, and retain
  it only if repeat capture and a more central approach improve together
  without posterior pinning, joint/load growth, route loss, or wake degradation.
  Revisit the actuator sign and handoff if a different carrier changes the
  curvature/yaw response map.
