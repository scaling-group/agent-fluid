...
- Use carrier-separated corrective response and residual predicted miss
  ...
  mild limit-cycle relief on this carrier. Preserve the strongest shared
  response-plus-miss handoff and test a different normalized physical-response
  residual within unused steering authority; retain it only with capture and
  reduced terminal-course spread while the direct route, compact wake, joint
  reserve, and roughly `0.037/0.019` force/moment scale remain intact. Repeat
  capture, lower effort, or one centered score alone does not establish margin
  robustness.
