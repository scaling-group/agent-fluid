...
- Treat sending/receiving reserve gates as joint-quality safeguards, not a
  ...
  further on this carrier. Treat phase-space projection as an evidenced
  joint-quality/margin safeguard, not yet an overall improvement. If testing
  it again, schedule its application with existing normalized approach
  geometry rather than adding another reserve threshold or projection-gain
  sweep, and retain it only if capture, compact wake, miss at or below
  `0.461L`, posterior dwell at or below `0.681%`, roughly `0.040/0.019` peak
  loads, and the `15.14--15.34T` arrival class improve together; otherwise
  remove the cubic posterior correction rather than continue allocator edits.
