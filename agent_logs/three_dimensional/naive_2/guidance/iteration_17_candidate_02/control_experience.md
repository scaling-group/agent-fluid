...
- Predicted miss, rather than proximity alone, is the first repeatably sampled
  way to make small terminal mean curvature useful on this carrier. A
  rotation-invariant body-frame target/course predictor captured at
  `0.74772L` and `16.0105T`. Its response-gated version with a joint-phase
  posterior pulse then captured in two independent evaluations of the exact
  same policy, at `0.74998L/15.9830T` and `0.74934L/16.0435T`, despite arriving
  in different terminal body phases. Both repeats preserve the direct route,
  compact alternating 3D wake, zero `>40 deg` joint dwell, and low peak
  normalized planar force/moment near `0.0342/0.0173`; this upgrades the
  composite from a one-off threshold crossing to the only reproduced semantic
  success in the sampled lineage. Promote it ahead of distance holds,
  full-circle pursuit, or another static-bend/gain variant: predict
  time-to-closest and signed cross-track miss, retain rhythmic steering until
  carrier-separated yaw is corrective, and gate posterior reshaping by
  observed joint phase. Its remaining boundary is perturbed-case robustness:
  an inherited identical base-policy repeat missed at `0.96311L`, response
  gating alone recovered only to `0.81002L`, and the successful composite still
  used the two rate envelopes for about `17.2%/16.9%` of assigned-parent
  samples. Falsify or revise it if further same-hash, reflected, or perturbed
  runs lose capture, repeat the lower-left escape, or increase rate
  occupancy/loads. Do not treat rate occupancy alone as permission for broad
  carrier braking: the sampled smooth-barrier variant removed near-limit
  occupancy but lost more than `2L` of useful translation and retained an
  early boundary exit.
