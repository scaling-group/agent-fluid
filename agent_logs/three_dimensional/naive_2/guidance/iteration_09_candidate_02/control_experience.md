...
- Do not promote an approach scheduler or thresholded bend release from
  closest distance alone. Across the four detailed direct-uniform samples,
  continuous hold, signed mean-bend release, posterior-bend release, and
  joint-angle phase correction all retained coherent self-propelled wakes but
  exited the same upper boundary at `y=15.20L`; their closest distances span
  `2.664--4.650L`, while angle-limit occupancy, peak planar force, and peak
  moment trade against one another rather than improve together. The inherited
  scalar logs make the boundary sharper: later `1.276L` and `1.173L` near
  misses still returned to `10.936L` and `10.560L` final distance with the same
  termination. Treat those as a concrete negative result for more scalar
  approach thresholds. On this traveling-bend carrier, next test a normalized,
  rotation-invariant target-versus-velocity course residual while preserving
  an opposed propulsive half-cycle; retain it only if it lowers the high pass
  and improves termination or return geometry without losing the coherent
  wake, useful translation, or load quality.
