...
- Treat sender reserve, receiver reserve, and carrier-phase inference as
  separate terminal-allocation decisions; a single centered pass or lower
  limit dwell does not establish a robust combination. Ten same-carrier,
  direct-quiescent captures have visually comparable direct routes and compact
  alternating 3D wakes, yet expose a geometry/reserve trade. Tail-only cubic
  redirection arrived at `15.1403T` with `0.651L` head-relative course miss and
  `1.269%` posterior `>40 deg` dwell. Absolute posterior reserve with
  unconditional anterior spillover uniquely produced `0.362L` miss, but slowed
  arrival to `15.4464T` and exchanged exposure into `0.285/0.996%`
  anterior/posterior dwell. Requiring absolute reserve at both joints gave
  `15.2957T/0.556L` and `0/0.898%`; making both gates directional improved that
  sampled compromise to `15.3385T/0.461L` and `0/0.681%`. The latter four stayed
  within a narrow `0.03872--0.04041/0.01876--0.01902` peak normalized
  force/moment class.

  Delayed or carrier-sign phase proxies do not recover the centered course.
  Bounded `previous_action` gating captured at `15.2107T` but widened miss to
  `0.631L`, raised posterior dwell to `1.878%`, and raised peak moment to
  `0.01922`. More decisively, two evaluations of the identical absolute-tail,
  directional-receiver policy varied from `15.2823T/0.516L`, zero dwell, and
  `0.03988/0.01906` loads to `15.2308T/0.619L`, `0/0.397%` dwell, and
  `0.04087/0.01968`; the apparent joint-quality improvement did not survive its
  repeat and neither pass met the `0.461L` directional margin. Replacing the
  delayed action with contemporaneous analytic carrier sign was worse again:
  `15.2469T/0.619L`, `0/2.233%` dwell, and `0.04184/0.01961`. Because all retain
  capture and the same compact wake, these are allocation failures rather than
  propulsion failures. Avoid more action-scale, analytic-carrier-sign,
  absolute-tail/directional-receiver, or associated scalar tuning on this
  carrier.

  Predictive reserve is also a bounded trade rather than a universal repair.
  A fixed oscillator-normalized phase-space projection improved the fully
  directional baseline to `0.421L` miss, zero angle dwell, and
  `0.03628/0.01798` peak normalized force/moment, but delayed capture to
  `15.5850T/-0.02349`. Scheduling that same look-ahead with normalized approach
  distance recovered the fast class at `15.2650T/-0.01575`, preserved the
  direct compact wake, and kept good `0/0.360%` dwell and `0.03781/0.01864`
  loads, yet widened miss to `0.529L`; it beat tail-only allocation quality but
  not the directional baseline's `0.461L` course or the fixed projection's
  `0.421L`. Thus distance scheduling can recover translation lost to an
  anticipatory safeguard, but it does not establish course robustness. Do not
  iterate projection magnitude or distance-gate scalars on this carrier.
  Instead change the representation of the evidenced course-centering
  authority, and retain it only if capture and the compact wake coexist with
  miss at or below `0.461L`, anterior/posterior dwell at or below
  `0/0.681%`, loads near or below `0.040/0.019`, and arrival within the
  `15.14--15.34T` class. Re-evaluate this ranking under a different carrier or
  initial condition rather than treating any reserve convention as universal.
- Geometric amplitude sharing around a steering-shifted oscillator center is
  not a dynamic joint-envelope safeguard on this carrier. Replacing
  reserve-blocked anterior acceleration spillover with a wave-center shift and
  contracting excursion by the same offset retained the direct compact wake
  and captured at `15.2745T`, but widened head-relative course miss to
  `0.632L`, created `0.648/1.044%` anterior/posterior `>40 deg` dwell, and
  scored `-0.01525`. The sampled directional acceleration allocator was
  slightly slower at `15.3385T` and marginally higher-load
  (`0.03959/0.01889` versus `0.03894/0.01874` peak normalized planar
  force/moment), yet kept `0.461L` miss, `0/0.681%` dwell, and a better
  `-0.01502` score. Visually indistinguishable self-propelled routing and wake
  coherence isolate this as a terminal actuator-representation failure, not a
  propulsion failure. Avoid more wave-center, posterior-target, or coupled
  offset/amplitude tuning with this carrier; test a response-gated rhythmic
  actuator that releases rather than shifts the carrier, and retain it only if
  capture, course, joint histories, loads, arrival, and wake quality improve
  together. Revisit center shifts only under a different carrier or with an
  independently evidenced dynamic envelope constraint.
