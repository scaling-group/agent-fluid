...
- Once target-versus-velocity course feedback established a useful approach,
  ...
  Byte-identical base repeats retained that low-load wake but passed below the
  disk at roughly `0.94--0.96L` and escaped lower-left, so a threshold crossing
  alone did not establish margin. Retaining rhythmic steering until
  carrier-separated yaw became corrective improved a repeat to `0.8100L` but
  still missed; adding a terminal, joint-phase-gated posterior pulse produced
  two sampled captures of the exact composite at `15.9830T/0.749982L` and
  `16.0435T/0.749337L`. Both kept the compact alternating wake, zero `>40 deg`
  angle dwell, about `17%` near-rate-limit occupancy, and only
  `0.0342--0.0344/0.0173--0.0174` peak force/moment. Prefer this small
  response-gated actuator composite over stronger gains, but do not attribute
  robustness to either component alone or treat near-threshold captures as
  broad margin. A separate inherited iteration that removed an instantaneous
  joint-state estimate of carrier lateral velocity from the predictor
  regressed to a `0.9016L` miss and lower-left exit despite lower rate
  occupancy; do not retry predictor-side carrier subtraction without repeated
  wake-crossflow evidence. These bounds are established only for
  direct-uniform still water. Under actual wakes, reject the instantaneous yaw
  handoff if alternating crossflow causes wrong-side correction, then test a
  slow/history-based residual while requiring repeated capture, wake
  coherence, joint reserve, and low loads together.
