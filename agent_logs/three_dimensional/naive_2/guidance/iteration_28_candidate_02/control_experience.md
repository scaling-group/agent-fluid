...
- Preserve constant-course predicted miss and carrier-separated response as a
  repeatable direct-route capture architecture, but treat terminal beat phase
  as its remaining robustness boundary. All four current direct-quiescent
  samples captured at `15.5008--16.0270T` behind the same compact alternating
  3D wake; the byte-identical common response-plus-miss release captured twice
  and supplied the best sampled score/integral (`-0.02109/1.90236L`). Yet its
  two captures ended with markedly different headings (`0.259/0.765rad`) and
  velocity directions, while all variants occupied the near-rate band for
  roughly `17--18%` and reached the acceleration cap. Geometry-qualified
  posterior release and an always-active posterior pulse also captured, and
  inherited identical-controller executions still missed in the
  `1.01--1.22L` band before left exit; pulse presence or release choice is
  therefore neither necessary nor sufficient for repeat capture. Do not tune
  carrier, pulse, bend, or release gains from these threshold outcomes. Test a
  bounded terminal observation that rejects common carrier rotation while
  preserving raw body-frame pursuit as a low-speed/non-closing fallback, and
  retain it only on repeated capture or a simultaneously better closest pass
  and termination with the direct route, wake coherence, joint reserve, and
  compact force/moment class intact. Falsify phase separation if it changes
  the route topology or merely lowers effort while retaining the miss class.
- Stop extending the terminal redirect-release consensus or shallow carrier
  relief on this architecture without a new actuator mechanism. Requiring
  course alignment in addition to corrective yaw and small predicted miss
  still captured, but slowed arrival to `15.8926T`, scored `-0.02329`, and
  crossed with about `0.719L` raw constant-course miss; direct yaw-arrest had
  already crossed with about `0.746L` miss. The centered-closing amplitude
  envelope likewise slowed the best sampled `15.5008T/-0.02109` capture to
  `15.6738T/-0.02289`, while terminal speed stayed near `1.30L/T` and joint-rate
  occupancy did not materially fall. These are concrete negative results for
  more release gates and weak approach envelopes, even though all preserved
  capture, the direct route, and compact loads/wake. After this pattern, test a
  different reflection-equivariant actuator such as joint-phase-conditioned
  posterior wave shape, and retain it only if terminal miss or repeat capture
  improves without sacrificing arrival, wake coherence, reserve, or loads;
  revisit release/relief only if a stronger schedule measurably changes speed
  or response rather than merely the command algebra.
