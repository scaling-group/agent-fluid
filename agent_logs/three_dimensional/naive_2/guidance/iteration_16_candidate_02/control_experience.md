...
- Phase-compensating body yaw before response feedback is the first sampled
  ...
  and wake coherence.
- Once target-versus-velocity course feedback established a useful approach,
  a bounded constant-course predicted-miss handoff to terminal mean curvature
  produced the first semantic success: capture at `16.0105T` and `0.7477L`, a
  compact body-connected alternating 3D wake, zero logged joint-angle dwell
  beyond `40 deg`, and only `0.0347/0.0172` peak normalized planar force/moment.
  Preserve that measured course/miss geometry despite its beat-scale jitter.
  A later small combination—carrier-separated yaw gating of the overlapping
  half-cycle channel plus a posterior mid-stroke pulse—also captured, slightly
  earlier at `15.9830T` with score `-0.02444` rather than `16.0105T` and
  `-0.02805`, retained the compact wake, and reduced terminal measured yaw rate
  from `1.642` to `0.694 rad/T`. Treat those two additions as compatible with
  the predictor, not as independently proven improvements: they entered
  together and arrival changed by only `0.0275T`, so isolate one at a time and
  require capture, low loads, joint reserve, and wake coherence before keeping
  either.
  The next inherited iteration subtracted an instantaneous joint-state model
  of carrier lateral velocity only inside the predictor; although loads stayed
  low and rate-limit occupancy fell, capture regressed to a `0.9016L` near miss
  followed by a lower-left exit at `10.604L` final distance.  Do not treat a
  cleaner latent signal or a smaller non-capture minimum as better than a
  capture, and avoid carrier subtraction in the successful predictor unless
  actual wake-crossflow evidence first shows a repeated semantic failure. Two
  assigned-parent continuations reinforce that boundary: `0.939L` and `1.120L`
  closest passes both became left-domain exits at `10.558L` and `9.409L`, so a
  near-miss scalar improvement is not a substitute for the captured topology.
  This result is established only for direct-uniform still water; under real
  cylinder wakes, test a history-based slow course estimate or small bounded
  disturbance residual only if it preserves capture, wake coherence, joint
  reserve, and low loads together.
