# Predicted-miss pursuit with stroke-local near-miss re-acquisition.
# The established traveling bend and closing approach remain intact; loss of
# closure near the target recruits one bounded posterior recovery pulse.
...
function target_policy_params()
    return (
        ...
        recovery_closing_speed=0.15,
        recovery_closing_width=0.10,
        recovery_error_scale=0.5,
        maximum_recovery_tail_pulse=6.0 * pi / 180,
...
function target_policy(state, params)
    ...
    closing_speed = isfinite(state.closing_speed_L) ?
        ...
        params.recovery_closing_speed + 4 * params.recovery_closing_width
    ...
    # Preserve the inherited 3L proximity handoff as a fallback. Predict the
    # constant-course closest approach from normalized target and velocity; a
    # positive projection means the target remains ahead along that course.
    ...
    # Smoothly unite ordinary proximity with the predictive gate. The
    # predictive request controls only the authority it recruited; current
    ...
    # Keep rhythmic steering available until carrier-separated yaw confirms a
    # corrective terminal response. Wrong-side response retains the full
    # half-cycle channel without braking the propulsive carrier.
    ...
    # The inherited posterior pulse acts during force-producing mid-stroke on
    # a still-closing intercept. It vanishes at beat reversal.
    ...
    terminal_tail_pulse = closing_alignment * terminal_gate *
        midstroke_gate * abs(params.maximum_terminal_tail_pulse) *
        terminal_turn_request

    # Near-miss re-acquisition is a separate response state. It remains
    # dormant while distance closes normally, then recruits a target-signed
    # posterior pulse if proximity, lost closure, and line-of-sight error all
    # agree. Closing recovery releases it continuously.
    closing_width = max(abs(params.recovery_closing_width), eps(Float64))
    closure_loss_gate = 0.5 * (
        1 - tanh(
            (closing_speed - params.recovery_closing_speed) / closing_width,
        )
    )
    recovery_error_gate = tanh(
        abs(bounded_pursuit) /
        max(abs(params.recovery_error_scale), eps(Float64)),
    )
    recovery_gate = distance_gate * closure_loss_gate * recovery_error_gate
    recovery_tail_pulse = recovery_gate * midstroke_gate *
        abs(params.maximum_recovery_tail_pulse) *
        terminal_pursuit_request

    # Center the oscillator and lagged posterior target on the slow mean bend.
    # Opposed traveling motion and posterior lag remain intact around it.
    q1_centered = q1 - terminal_mean_bend
    vdp_drive = params.oscillator_mu *
        (1 - (q1_centered / amp)^2) * qd1
    carrier_a1 = vdp_drive - omega^2 * q1_centered
    phase_lag_target = 2 * terminal_mean_bend + terminal_tail_pulse +
        recovery_tail_pulse - q1 -
    ...
    # to persistent mean curvature after a measured corrective response.
    redirect_gate = tanh(
