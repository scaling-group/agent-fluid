# Unified predicted-miss handoff with harmonic-peak joint reserve.
# A one-sided oscillator phase-plane envelope anticipates angular excursion
# without the double counting of a linear angle-plus-rate projection.
...
function target_policy(state, params)
    ...
    # allocation. During motion in the requested direction, approach the
    # harmonic phase-plane peak hypot(q, qdot/omega); during inward motion,
    # retain the current signed angle. This anticipates angular excursion
    # without the linear projection's double counting when angle and rate are
    # both outward. The existing rate width makes the phase transition smooth,
    # and the direct rate guard still protects high outward joint speed.
    ...
    directed_tail_angle = redirect_direction * q2
    directed_tail_rate = redirect_direction * qd2
    tail_outward_phase = tanh(max(directed_tail_rate, 0.0) / rate_width)
    tail_harmonic_peak = hypot(q2, qd2 / max(omega, eps(omega)))
    predicted_tail_peak = directed_tail_angle + tail_outward_phase *
        (tail_harmonic_peak - directed_tail_angle)

    tail_angle_capacity = 0.5 * (
        1 - tanh((predicted_tail_peak - soft_angle) / angle_width)
    )
    tail_rate_capacity = 0.5 * (
        1 - tanh((directed_tail_rate - soft_rate) / rate_width)
    )
    tail_capacity = tail_angle_capacity * tail_rate_capacity

    directed_head_angle = redirect_direction * q1
    directed_head_rate = redirect_direction * qd1
    head_outward_phase = tanh(max(directed_head_rate, 0.0) / rate_width)
    head_harmonic_peak = hypot(q1, qd1 / max(omega, eps(omega)))
    predicted_head_peak = directed_head_angle + head_outward_phase *
        (head_harmonic_peak - directed_head_angle)

    head_angle_capacity = 0.5 * (
        1 - tanh((predicted_head_peak - soft_angle) / angle_width)
    )
    head_rate_capacity = 0.5 * (
        1 - tanh((directed_head_rate - soft_rate) / rate_width)
    )
