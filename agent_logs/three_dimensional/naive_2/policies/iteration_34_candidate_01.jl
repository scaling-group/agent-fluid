# Unified predicted-miss handoff with recipient-reserve-gated allocation.
# Large closing misses recruit bounded rhythmic redirect authority, which can
# leave the tail only while the receiving anterior joint has kinematic reserve.
...
function target_policy(state, params)
    ...
    # it vanish rapidly on centered courses. Keep that residual tail-dominant
    # while the posterior has angle and rate reserve; as either soft boundary
    # is approached, transfer authority only while the anterior joint also has
    # reserve. If neither path has reserve the extra redirect sheds smoothly
    # instead of transferring saturation. Absolute joint state keeps the
    # allocation reflection-neutral while predicted miss supplies direction.
    ...
    posterior_angle_reserve = 0.5 * (
        1 - tanh((abs(q2) - soft_angle) / angle_width)
    )
    posterior_rate_reserve = 0.5 * (
        1 - tanh((abs(qd2) - soft_rate) / rate_width)
    )
    posterior_reserve = posterior_angle_reserve * posterior_rate_reserve
    anterior_angle_reserve = 0.5 * (
        1 - tanh((abs(q1) - soft_angle) / angle_width)
    )
    anterior_rate_reserve = 0.5 * (
        1 - tanh((abs(qd1) - soft_rate) / rate_width)
    )
    anterior_reserve = anterior_angle_reserve * anterior_rate_reserve
    tail_redirect_residual = posterior_reserve * posterior_miss_residual
    head_redirect_residual = params.head_asymmetry_share *
        (1 - posterior_reserve) * anterior_reserve * posterior_miss_residual
