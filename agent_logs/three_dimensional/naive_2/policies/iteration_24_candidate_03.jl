# Closing-course terminal steering with geometry-qualified pulse release.
# Near the target, observable course error replaces beat-sensitive pursuit;
# the traveling carrier and the far-field predicted-miss route stay intact.
...
function target_policy(state, params)
    ...
    blended_terminal_request = request_weight > eps(Float64) ? clamp(
        (
    ...
    # While a positive closing course is observable, use its rotation-
    # invariant angular error near the target. Unlike prediction_gate, this
    # selector does not vanish when its own error is small and therefore does
    # not hand control back to beat-sensitive pursuit on a centered course.
    # The inherited blend remains the fallback outside the near-target gate,
    # at low speed, and once closing alignment disappears.
    course_turn_request = tanh(
        bounded_course_error /
        max(abs(params.steering_softness), eps(Float64)),
    )
    closing_course_gate = distance_gate * course_weight * time_gate * tanh(
        closing_alignment /
        max(abs(params.steering_softness), eps(Float64)),
    )
    terminal_turn_request = clamp(
        (1 - closing_course_gate) * blended_terminal_request +
        closing_course_gate * course_turn_request,
        -1.0,
        1.0,
    )
