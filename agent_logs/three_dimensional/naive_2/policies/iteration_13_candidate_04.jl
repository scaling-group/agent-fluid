# Rate-reserved predicted-miss curvature on a course-residual traveling bend.
# Body-frame target and velocity recruit persistent mean bend early, while
# observed joint rate preserves the carrier's inward reversal channel near the
# actuator envelope.
...
function target_policy_params()
    return (
        ...
        joint_rate_reserve_width=8.0 * pi / 180,
...
function target_policy(state, params)
    ...
    # Preserve a narrow rate reserve without weakening the reversal that
    # sustains the traveling bend. Only total acceleration pointing in the
    # same direction as the observed near-limit rate is attenuated; an inward
    # command passes unchanged. This prevents useful steering and propulsion
    # components from winding up against the episode's hard rate constraint.
    rate_limit = max(abs(params.maximum_joint_rate), eps(Float64))
    rate_width = max(abs(params.joint_rate_reserve_width), eps(Float64))
    rate_margin1 = max(rate_limit - abs(qd1), 0.0)
    rate_margin2 = max(rate_limit - abs(qd2), 0.0)
    outward_scale1 = tanh(rate_margin1 / rate_width)
    outward_scale2 = tanh(rate_margin2 / rate_width)
    reserved_a1 = raw_a1 * qd1 > 0 ? outward_scale1 * raw_a1 : raw_a1
    reserved_a2 = raw_a2 * qd2 > 0 ? outward_scale2 * raw_a2 : raw_a2
