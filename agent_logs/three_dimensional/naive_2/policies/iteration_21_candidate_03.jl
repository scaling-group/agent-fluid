# Phase-separated terminal pursuit on predicted-miss curvature.
# Body-frame interception recruits a persistent mean bend; joint phase removes
# carrier yaw from terminal geometry without changing the far-field route.
...
function target_policy(state, params)
    ...
    # The angle-domain integral of the carrier-yaw-rate model removes
    # beat-correlated body yaw from terminal bearing. Keep this residual out of
    # far-field pursuit and course prediction so the evidenced route is intact.
    carrier_phase_yaw =
        -params.carrier_yaw_head_rate_gain * q1 -
        params.carrier_yaw_tail_rate_gain * q2
    terminal_pursuit_error = atan(
        sin(pursuit_error - carrier_phase_yaw),
        cos(pursuit_error - carrier_phase_yaw),
    )
    bounded_terminal_pursuit = clamp(
        terminal_pursuit_error,
        -maximum_error,
        maximum_error,
    )
