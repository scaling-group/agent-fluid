# Response-gated predicted-miss curvature on a course-residual traveling bend.
# Joint state sustains propulsion; body-frame target and velocity recruit a
# persistent mean bend before closest approach, then observed same-sign yaw
# releases a bounded share after the predicted course enters the corridor.
...
function target_policy_params()
    return (
        ...
        terminal_response_distance=1.5,
        terminal_response_width=0.25,
        terminal_yaw_rate_scale=1.0,
        maximum_terminal_response_release=0.3,
...
function target_policy(state, params)
    ...
    # Once the predicted course is inside the miss corridor and yaw is already
    # responding in the requested direction, release only a bounded share of
    # persistent curvature. This acts near capture, never reverses the request,
    # and leaves both propulsive/course carrier channels untouched.
    response_width = max(
        abs(params.terminal_response_width),
        eps(Float64),
    )
    response_distance_gate = 0.5 * (
        1 - tanh(
            (distance - params.terminal_response_distance) / response_width,
        )
    )
    corridor_gate = clamp(1 - abs(prediction_request), 0.0, 1.0)
    yaw_rate_scale = max(
        abs(params.terminal_yaw_rate_scale),
        eps(Float64),
    )
    aligned_yaw_response = clamp(
        terminal_turn_request * tanh(heading_rate / yaw_rate_scale),
        0.0,
        1.0,
    )
    maximum_response_release = clamp(
        params.maximum_terminal_response_release,
        0.0,
        1.0,
    )
    response_release = maximum_response_release * response_distance_gate *
        corridor_gate * aligned_yaw_response
    released_terminal_turn_request =
        (1 - response_release) * terminal_turn_request
    terminal_mean_bend = terminal_gate *
        abs(params.maximum_terminal_mean_bend) *
        released_terminal_turn_request
