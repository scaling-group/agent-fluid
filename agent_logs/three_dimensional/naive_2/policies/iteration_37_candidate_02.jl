# Unified predicted-miss handoff with an envelope-preserving posterior target.
# Large closing misses shift the lagged wave center while contracting its
# rhythmic excursion, rather than amplifying an already large acceleration.
...
function target_policy_params()
    return (
        ...
        maximum_terminal_wave_offset=4.0 * pi / 180,
...
function target_policy(state, params)
    ...
    # Move the cubic predicted-miss correction from acceleration asymmetry to
    # the lagged posterior wave target. The signed center shift is paired with
    # an equal fractional contraction of rhythmic excursion, so redirect and
    # carrier share the inherited nominal angle envelope. The mechanism
    # vanishes rapidly on centered courses and cannot alter far-field drive.
    terminal_wave_offset = terminal_gate * closing_alignment *
        abs(params.maximum_terminal_wave_offset) * prediction_request^3
    posterior_wave_scale = clamp(
        1 - abs(terminal_wave_offset) / max(amp, eps(Float64)),
        0.0,
        1.0,
    )
    ...
    phase_lag_target = terminal_mean_bend + terminal_tail_pulse +
        terminal_wave_offset - posterior_wave_scale * (
            q1_centered +
