# Response-gated predicted-miss curvature on a traveling-bend carrier.
# Body-frame interception recruits a persistent mean bend; carrier-separated
# yaw releases overlapping rhythmic steering only after corrective response.
...
function target_policy(state, params)
    ...
    # Predict constant-course closest approach from normalized target geometry
    # and measured body-frame velocity, while retaining a 3L proximity fallback.
    ...
    # Smoothly unite proximity and predictive recruitment. Mean curvature uses
    # the request of the gate that recruited it, with pursuit as the near-field
    # fallback after closing alignment disappears.
    ...
    # Keep rhythmic steering until carrier-separated yaw confirms corrective
    # response. This isolates response-gated authority from the sampled
    # posterior-pulse combination.
    ...
    # the opposed traveling component remains intact around that mean.
    ...
    # to persistent curvature after measured corrective response.
    redirect_gate = tanh(
