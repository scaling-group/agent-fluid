# Unified predicted-miss handoff with terminal posterior phase-lag steering.
# Course error reshapes posterior timing across opposing bend half-cycles while
# the anterior state-feedback oscillator remains continuously propulsive.
...
function target_policy_params()
    return (
        ...
        maximum_terminal_tail_lag_fraction=0.30,
...
function target_policy(state, params)
    ...
    # Signed course error times centered joint phase is reflection-even, so it
    # can increase posterior lag on one requested half-cycle and decrease it
    # on the other without imposing a static bend or an external phase clock.
    ...
    maximum_lag_fraction = clamp(
        abs(params.maximum_terminal_tail_lag_fraction),
        0.0,
        0.75,
    )
    lag_shape_request = terminal_gate * course_weight * course_share *
        sin(bounded_course_error) * centered_head_phase
    effective_tail_lag_gain = params.tail_lag_gain *
        (1 + maximum_lag_fraction * lag_shape_request)
