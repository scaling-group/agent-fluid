# Unified predicted-miss handoff with an anterior course residual.
# Large closing misses recruit bounded head-joint rhythmic steering while the
# posterior joint preserves the evidenced lagged propulsive wave.
...
function target_policy_params()
    return (
        ...
        maximum_terminal_head_residual=0.35,
...
function target_policy(state, params)
    ...
    # The sampled tail residual improved arrival but drove the posterior joint
    # to its rate limit and raised planar load. Preserve the centered shared
    # redirect while moving only its large-miss residual to the anterior wave
    # generator. Cubing makes this smooth, reflection-equivariant authority
    # vanish rapidly on centered courses and outside a closing terminal pass.
    anterior_miss_residual = terminal_gate * closing_alignment *
        abs(params.maximum_terminal_head_residual) * prediction_request^3
