# Unified predicted-miss handoff with a response-released posterior redirect.
# Large closing misses recruit bounded tail-only rhythmic authority, which
# returns to the carrier as geometry and corrective yaw confirm the response.
...
function target_policy(state, params)
    ...
    # Treat the evidenced tail-only residual as a redirect burst, not a
    # persistent terminal bias. Large closing misses still recruit it, while
    # the shared geometry-plus-yaw consensus returns it continuously to the
    # carrier after a corrective response. Cubing the miss keeps the residual
    # rapidly vanishing and reflection equivariant on centered courses.
    posterior_miss_residual = terminal_gate * closing_alignment *
        unmet_release_gate * abs(params.maximum_terminal_tail_residual) *
