# Phase-separated terminal bearing on response-gated predicted-miss curvature.
# Body-frame interception preserves the established route while joint state
# removes beat-correlated yaw from the near-target steering fallback.
...
function target_policy(state, params)
    ...
    # Raw bearing oscillates with the propulsive body yaw even on an otherwise
    # direct course. Use the angle-domain integral of the carrier-yaw-rate
    # model only for terminal pursuit, leaving far-field course prediction
    # unchanged. This makes persistent target geometry set the fallback sign.
    carrier_phase_yaw =
        -params.carrier_yaw_head_rate_gain * q1 -
        params.carrier_yaw_tail_rate_gain * q2
    phase_separated_pursuit = atan(
        sin(pursuit_error - carrier_phase_yaw),
        cos(pursuit_error - carrier_phase_yaw),
    )
    bounded_terminal_pursuit = clamp(
        phase_separated_pursuit,
        -maximum_error,
        maximum_error,
    )
    terminal_pursuit_request = tanh(
        bounded_terminal_pursuit /
