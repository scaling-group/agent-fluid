# Unified predicted-miss handoff with projected directional reserve.
# A bounded terminal residual remains rhythmic and target-relative while each
# joint anticipates whether its observed motion leaves compatible reserve.
...
function target_policy(state, params)
    ...
    # projected directional reserve. q + qdot/omega looks one oscillator
    # radian ahead without a clock: restoring residuals remain admissible near
    # a boundary, while outward residuals are withheld before observed motion
    # carries a joint into it. Tail authority remains primary; admissible head
    # reserve receives only unavailable tail share, and any remainder is shed.
    ...
    phase_rate = max(abs(omega), eps(Float64))
    projected_q1 = q1 + qd1 / phase_rate
    projected_q2 = q2 + qd2 / phase_rate
