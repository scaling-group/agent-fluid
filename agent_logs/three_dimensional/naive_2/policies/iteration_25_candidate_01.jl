# Predicted-miss curvature with carrier-separated line-of-sight damping.
# A near-target inertial bearing rate has its joint-rate carrier removed before
# it makes a bounded correction to the established terminal intercept.
...
function target_policy_params()
    return (
        ...
        line_of_sight_carrier_head_rate_gain=-2.11,
        line_of_sight_carrier_tail_rate_gain=0.16,
        terminal_line_of_sight_rate_scale=0.8,
        terminal_line_of_sight_share=0.25,
...
function target_policy(state, params)
    ...
    turn_rate_recent = isfinite(state.turn_rate_recent) ?
        Float64(state.turn_rate_recent) : heading_rate
    ...
    geometric_terminal_request = request_weight > eps(Float64) ? clamp(
        (
    ...
    # The body-frame bearing rate contains both inertial line-of-sight motion
    # and the rhythmic body yaw. Adding matched-window yaw first restores the
    # inertial rate; the joint-rate model then removes the evidenced carrier.
    # Its bounded residual is used only near the target, so the successful
    # far-field course and traveling-wave propulsion remain unchanged.
    inertial_line_of_sight_rate = bearing_window_rate + turn_rate_recent
    carrier_line_of_sight_rate =
        params.line_of_sight_carrier_head_rate_gain * qd1 +
        params.line_of_sight_carrier_tail_rate_gain * qd2
    phase_separated_line_of_sight_rate =
        inertial_line_of_sight_rate - carrier_line_of_sight_rate
    line_of_sight_turn_request = -tanh(
        phase_separated_line_of_sight_rate /
        max(abs(params.terminal_line_of_sight_rate_scale), eps(Float64)),
    )
    line_of_sight_share = distance_gate * clamp(
        params.terminal_line_of_sight_share,
        0.0,
        1.0,
    )
    terminal_turn_request =
        (1 - line_of_sight_share) * geometric_terminal_request +
        line_of_sight_share * line_of_sight_turn_request
