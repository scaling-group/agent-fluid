# Predicted capture funnel on a course-residual traveling bend.
# Joint state sustains propulsion; body-frame target and velocity recruit a
# persistent mean bend plus a bounded anterior rhythmic residual while the
# measured course still forecasts a miss outside the capture corridor.
...
function target_policy_params()
    return (
        ...
        funnel_miss_corridor=0.70,
        funnel_miss_width=0.20,
        maximum_funnel_head_asymmetry=0.18,
...
function target_policy(state, params)
    ...
    # A safe-corridor deficit recruits a separate anterior half-cycle
    # residual. Unlike another static mean-bend or posterior pulse gain, this
    # channel is exactly inactive when the measured course already forecasts
    # a pass inside the corridor. It also releases after closest approach.
    funnel_corridor = abs(params.funnel_miss_corridor)
    funnel_width = max(abs(params.funnel_miss_width), eps(Float64))
    funnel_excess_miss = max(
        abs(signed_predicted_miss) - funnel_corridor,
        0.0,
    )
    funnel_gate = course_weight * closing_alignment * time_gate * tanh(
        funnel_excess_miss / funnel_width,
    )
    funnel_turn_request = funnel_gate * prediction_request
    ...
    funnel_head_asymmetry =
        abs(params.maximum_funnel_head_asymmetry) * funnel_turn_request
    raw_a1 = carrier_a1 +
        params.head_asymmetry_share * asymmetry * abs(carrier_a1) +
        funnel_head_asymmetry * abs(carrier_a1)
    raw_a2 = carrier_a2 +
