# Unified predicted-miss handoff with projected directional joint reserve.
# A short oscillator-normalized phase-space look-ahead anticipates outward
# joint motion while preserving the rhythmic, target-relative residual.
...
function target_policy(state, params)
    ...
    # directional allocation. Project angle by a short oscillator-normalized
    # velocity horizon: outward momentum consumes angle reserve early, whereas
    # inward motion restores reserve without a delayed action or carrier-sign
    # proxy. The direct rate guard still protects high outward joint speed.
    ...
    projection = abs(params.joint_phase_projection) /
        max(omega, eps(omega))
