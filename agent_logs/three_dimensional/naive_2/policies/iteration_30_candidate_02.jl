# Unified predicted-miss handoff with target-line-rate phase-lag steering.
# Released redirect authority reshapes the posterior traveling wave while the
# anterior state-feedback carrier and minimum target steering remain active.
...
function target_policy_params()
    return (
        ...
        maximum_terminal_lag_modulation=0.5,
...
function target_policy(state, params)
    ...
    # A target-relative course can still rotate around the target after body
    # yaw looks corrective. Measure that rotation during closing with the
    # body-frame target/velocity cross product; it rejects beat-scale body
    # orientation without introducing a world route or clock.
    target_line_rate = -course_cross /
        max(target_norm^2, eps(Float64))
    line_rate_scale = max(
        abs(params.target_line_rate_scale),
        eps(Float64),
    )
    target_line_rate_request = -tanh(target_line_rate / line_rate_scale)
    ...
    # The released target-line-rate authority changes posterior phase only on
    # phase-supporting wave quadrants. Request, centered joint phase, and the
    # resulting lag term are jointly odd under reflection; no static bend is
    # added and the anterior carrier remains unchanged.
    q1_centered = q1 - terminal_mean_bend
    centered_head_phase = tanh(
        q1_centered / max(abs(amp), eps(Float64)),
    )
    maximum_lag_modulation = clamp(
        abs(params.maximum_terminal_lag_modulation),
        0.0,
        0.95,
    )
    terminal_lag_scale = 1 + maximum_lag_modulation * line_rate_share *
        target_line_rate_request * centered_head_phase
    ...
    phase_lag_target = 2 * terminal_mean_bend + terminal_tail_pulse - q1 -
        terminal_lag_scale * params.tail_lag_gain * qd1 /
        max(omega, eps(omega))
