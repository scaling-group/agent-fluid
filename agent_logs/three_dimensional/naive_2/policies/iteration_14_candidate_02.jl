# Rate-reserve predicted-miss curvature on a course-residual traveling bend.
# Body-frame target and velocity recruit the evidenced terminal turn; observed
# joint rate retains an inward recovery channel near the actuator envelope.
...
function target_policy_params()
    return (
        ...
        maximum_joint_rate=260.0 * pi / 180,
        joint_rate_reserve_width=30.0 * pi / 180,
...
function target_policy(state, params)
    ...
    # Preserve the captured parent's 3L fallback while predicting the
    # constant-course closest approach from normalized target and velocity.
    ...
    # Unite proximity and prediction continuously. Predicted miss controls
    # only the authority it recruits; pursuit remains the fallback when
    # closing alignment disappears.
    ...
    # Spend a controller-owned rate reserve only on outward acceleration.
    # Cubic smoothstep is exactly one below the reserve and exactly zero at
    # the rate envelope; inward acceleration remains available for reversal.
    rate_limit = max(abs(params.maximum_joint_rate), eps(Float64))
    reserve_width = max(
        abs(params.joint_rate_reserve_width),
        eps(Float64),
    )
    function outward_rate_scale(rate, acceleration)
        reserve = clamp(
            (rate_limit - abs(rate)) / reserve_width,
            0.0,
            1.0,
        )
        smooth_reserve = reserve^2 * (3 - 2 * reserve)
        return rate * acceleration > 0 ? smooth_reserve : 1.0
    end
    reserved_a1 = outward_rate_scale(qd1, raw_a1) * raw_a1
    reserved_a2 = outward_rate_scale(qd2, raw_a2) * raw_a2

    limit = abs(params.maximum_joint_acceleration)
    bounded_a1 = limit * tanh(reserved_a1 / max(limit, eps(Float64)))
    bounded_a2 = limit * tanh(reserved_a2 / max(limit, eps(Float64)))
