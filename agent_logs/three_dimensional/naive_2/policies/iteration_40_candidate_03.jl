# Unified predicted-miss handoff with carrier-separated course prediction.
# The productive traveling carrier remains intact while its joint-rate-linked
# lateral sway is removed from the terminal miss observation.
...
function target_policy_params()
    return (
        ...
        joint_phase_projection=1.0,
        joint_soft_rate=235.0 * pi / 180,
        joint_rate_width=20.0 * pi / 180,
        carrier_sway_head_scale=0.79,
        carrier_sway_tail_scale=0.47,
...
function target_policy(state, params)
    ...
    # Joint-rate-correlated sway is part of the productive carrier rather than
    # a persistent course error. Normalize its two-joint proxy by the owned
    # carrier rate and remove it only from near-target closest-approach
    # prediction; raw velocity still owns broad course, closing, and timing.
    carrier_rate = max(omega * amp, eps(Float64))
    normalized_head_rate = qd1 / carrier_rate
    normalized_tail_rate = qd2 / carrier_rate
    carrier_sway_y =
        -params.carrier_sway_head_scale * normalized_head_rate +
        params.carrier_sway_tail_scale * normalized_tail_rate
    prediction_velocity_y = velocity_y - distance_gate * carrier_sway_y
    prediction_speed = hypot(velocity_x, prediction_velocity_y)
    prediction_course_cross =
        target_x * prediction_velocity_y - target_y * velocity_x
    signed_predicted_miss = prediction_speed > prediction_floor ?
        prediction_course_cross / prediction_speed : 0.0
    ...
    # same geometry/response consensus releases it toward the base carrier.
    ...
    # Preserve the sampled cubic predicted-miss response and tail-first
    # directional allocation. During the final approach, smoothly project
    # angle by an oscillator-normalized velocity horizon: outward momentum
    # consumes reserve early, whereas inward motion restores it. Before the
    # near-target gate engages, projection vanishes. The direct rate guard
    # remains active throughout, and unavailable authority returns to carrier.
    redirect_residual = terminal_gate * closing_alignment *
        abs(params.maximum_terminal_redirect_residual) * prediction_request^3
    redirect_direction = sign(prediction_request)
    soft_angle = abs(params.joint_soft_angle)
    angle_width = max(abs(params.joint_angle_width), eps(Float64))
    projection = distance_gate * abs(params.joint_phase_projection) /
        max(omega, eps(omega))
    projected_q1 = q1 + projection * qd1
    projected_q2 = q2 + projection * qd2
    soft_rate = abs(params.joint_soft_rate)
    rate_width = max(abs(params.joint_rate_width), eps(Float64))

    tail_angle_capacity = 0.5 * (
        1 - tanh(
            (redirect_direction * projected_q2 - soft_angle) / angle_width,
        )
    )
    tail_rate_capacity = 0.5 * (
        1 - tanh((redirect_direction * qd2 - soft_rate) / rate_width)
    )
    tail_capacity = tail_angle_capacity * tail_rate_capacity

    head_angle_capacity = 0.5 * (
        1 - tanh(
            (redirect_direction * projected_q1 - soft_angle) / angle_width,
        )
    )
    head_rate_capacity = 0.5 * (
        1 - tanh((redirect_direction * qd1 - soft_rate) / rate_width)
    )
    head_capacity = head_angle_capacity * head_rate_capacity

    tail_redirect_residual = tail_capacity * redirect_residual
    head_redirect_residual = params.head_asymmetry_share *
        (1 - tail_capacity) * head_capacity * redirect_residual
