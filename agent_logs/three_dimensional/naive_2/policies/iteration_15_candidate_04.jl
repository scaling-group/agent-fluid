# Phase-selective counter-yaw on response-gated predicted-miss curvature.
# Body-frame interception preserves the traveling bend; wrong-sign terminal
# yaw recruits only the corrective anterior carrier half-cycle.
...
function target_policy_params()
    return (
        ...
        maximum_head_counter_yaw_acceleration=8.0,
        head_counter_yaw_phase_scale=8.0,
...
function target_policy(state, params)
    ...
    # The repeated identical-policy rollout missed below the capture circle
    # after the terminal handoff had already faded half-cycle steering. Remove
    # beat-correlated yaw before judging the slow directional response. Keep
    # the rhythmic steering channel fully available while residual yaw is
    # neutral or wrong-side, and release it only once yaw opposes the active
    # terminal request. Mean curvature remains active during this handoff.
    ...
    # Retaining the rhythmic steering channel improved the repeated closest
    # pass, but carrier-separated yaw was still wrong-sign at the miss while
    # posterior acceleration was already near its limit. Recruit a distinct
    # anterior counter-yaw channel only on wrong-sign terminal response and
    # only while the unmodified carrier already accelerates the head in the
    # corrective direction. This leaves the opposed half-cycle and posterior
    # traveling wave unchanged, and releases continuously on corrective yaw.
    wrong_yaw = max(terminal_turn_request * yaw_residual, 0.0)
    wrong_response_gate = terminal_gate * abs(terminal_turn_request) * tanh(
        wrong_yaw /
        max(abs(params.corrective_response_scale), eps(Float64)),
    )
    corrective_head_phase = max(
        -terminal_turn_request * carrier_a1,
        0.0,
    )
    head_phase_gate = tanh(
        corrective_head_phase /
        max(abs(params.head_counter_yaw_phase_scale), eps(Float64)),
    )
    head_counter_yaw = -abs(
        params.maximum_head_counter_yaw_acceleration,
    ) * terminal_turn_request * wrong_response_gate * head_phase_gate
