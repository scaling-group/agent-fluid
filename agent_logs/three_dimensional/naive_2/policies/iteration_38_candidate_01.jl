# Unified predicted-miss handoff with stopping-distance joint reserve.
# A dynamic braking margin admits tail-first rhythmic redirect authority while
# preserving the carrier and shedding demand that neither joint can stop.
...
function target_policy(state, params)
    ...
    # allocation, but measure dynamic rather than instantaneous joint reserve.
    # Outward velocity consumes angle reserve by its stopping distance under
    # the policy-owned acceleration envelope; inward velocity consumes none.
    # A direct directional rate guard remains, and authority that neither
    # joint can stop safely returns smoothly to the unmodified carrier.
    ...
    braking_acceleration = max(
        abs(params.maximum_joint_acceleration),
        eps(Float64),
    )

    tail_outward_rate = max(redirect_direction * qd2, 0.0)
    tail_stopping_position = redirect_direction * q2 +
        tail_outward_rate^2 / (2 * braking_acceleration)
    tail_angle_capacity = 0.5 * (
        1 - tanh((tail_stopping_position - soft_angle) / angle_width)
    )
    tail_rate_capacity = 0.5 * (
        1 - tanh(
            (redirect_direction * qd2 - soft_rate) / rate_width,
        )
    )
    tail_capacity = tail_angle_capacity * tail_rate_capacity

    head_outward_rate = max(redirect_direction * qd1, 0.0)
    head_stopping_position = redirect_direction * q1 +
        head_outward_rate^2 / (2 * braking_acceleration)
    head_angle_capacity = 0.5 * (
        1 - tanh((head_stopping_position - soft_angle) / angle_width)
    )
    head_rate_capacity = 0.5 * (
        1 - tanh(
            (redirect_direction * qd1 - soft_rate) / rate_width,
        )
    )
