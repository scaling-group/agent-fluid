# Full-circle burst redirect with alignment-triggered propulsive release.
# Joint state carries phase; near-target geometry trades shared half-cycle
# steering back to the traveling-bend carrier once the body axis is aimed.
...
function target_policy(state, params)
    ...
    # Preserve the sampled traveling-bend carrier and posterior lag. Approach
    # allocation is applied only when composing the final action.
    ...
    # The adapter's restricted bearing folds fore and aft together. Retain
    # that distinction with the normalized target vector: body forward is -x.
    ...
    # A near, badly misaligned target receives the sampled burst redirect:
    # only symmetric carrier authority is relieved, and it returns smoothly as
    # alignment recovers. This avoids broad braking of the propulsive scaffold.
    ...
    # New aim-then-translate allocation: while near and aligned, suppress the
    # residual half-cycle turn instead of feeding body-lateral skid. The same
    # alignment simultaneously restores carrier drive above, converting the
    # achieved target-facing pose into translation. Far or misaligned steering
    # is identical to the sampled full-circle redirect.
    alignment_translation = approach_proximity *
        (1 - approach_misalignment)
    steering_scale = 1 - alignment_translation
    allocated_asymmetry = steering_scale * asymmetry

    # Signed absolute carrier acceleration retains both propulsive half-cycles;
    # smooth saturation remains inside the acceleration envelope.
    raw_a1 = carrier_scale * carrier_a1 +
        params.head_asymmetry_share * allocated_asymmetry * abs(carrier_a1)
    raw_a2 = carrier_scale * carrier_a2 +
        params.tail_asymmetry_share * allocated_asymmetry * abs(carrier_a2)
