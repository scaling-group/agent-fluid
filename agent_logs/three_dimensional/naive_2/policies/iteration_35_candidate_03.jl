# Unified predicted-miss handoff with braking-half-cycle redirect allocation.
# A bounded target-relative residual prolongs the useful beat side without
# accelerating either joint farther outward.
...
function target_policy_params()
    return (
        ...
        redirect_braking_rate_width=0.15,
...
function target_policy(state, params)
    ...
    # Preserve the sampled cubic predicted-miss response, but admit it only on
    # a braking half-cycle. When a joint moves against the requested bend, the
    # residual decelerates that return stroke and prolongs the useful beat side
    # without adding outward kinetic energy. Directional angle capacity lets
    # the carrier restore a joint that is already beyond its soft boundary.
    # Unavailable tail authority moves to an admissible braking head channel;
    # any remainder returns smoothly to the carrier.
    ...
    carrier_rate_scale = max(omega * amp, eps(Float64))
    braking_width = max(
        abs(params.redirect_braking_rate_width),
        eps(Float64),
    )
    ...
    tail_braking_capacity = 0.5 * (
        1 - tanh(
            redirect_direction * qd2 / carrier_rate_scale / braking_width,
        )
    )
    tail_capacity = tail_angle_capacity * tail_braking_capacity

    head_angle_capacity = 0.5 * (
        1 - tanh((redirect_direction * q1 - soft_angle) / angle_width)
    )
    head_braking_capacity = 0.5 * (
        1 - tanh(
            redirect_direction * qd1 / carrier_rate_scale / braking_width,
        )
    )
    head_capacity = head_angle_capacity * head_braking_capacity
