# Unified predicted-miss handoff with dual-reserve-aware redirect allocation.
# Large closing misses recruit tail-first rhythmic authority; spillover uses
# the anterior joint only while both observed angle and rate reserve remain.
...
function target_policy(state, params)
    ...
    # this residual vanish rapidly on centered courses. Tail authority remains
    # primary; only the share excluded by posterior kinematic reserve may
    # spill into an anterior joint that itself has angle and rate reserve.
    # Authority that neither joint can accept is shed instead of saturating.
    ...
    head_angle_reserve = 0.5 * (
        1 - tanh(
            (abs(q1) - abs(params.joint_soft_angle)) / angle_width,
        )
    )
    head_rate_reserve = 0.5 * (
        1 - tanh(
            (abs(qd1) - abs(params.joint_soft_rate)) / rate_width,
        )
    )
    tail_angle_reserve = 0.5 * (
        1 - tanh(
            (abs(q2) - abs(params.joint_soft_angle)) / angle_width,
        )
    )
    tail_rate_reserve = 0.5 * (
        1 - tanh(
            (abs(qd2) - abs(params.joint_soft_rate)) / rate_width,
        )
    )
    head_reserve = head_angle_reserve * head_rate_reserve
    tail_reserve = tail_angle_reserve * tail_rate_reserve
    tail_redirect_residual = tail_reserve * posterior_miss_residual
    head_redirect_residual = params.head_asymmetry_share * head_reserve *
        (1 - tail_reserve) * posterior_miss_residual
