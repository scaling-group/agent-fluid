# Unified predicted-miss handoff with a carrier-supportive posterior pulse.
# The terminal pulse reinforces only the requested-side carrier half-cycle,
# preserving the traveling bend while refining the centered intercept.
...
function target_policy_params()
    return (
        ...
        tail_phase_alignment_softness=0.15,
...
function target_policy(state, params)
    ...
    # miss also becomes small, keeping their handoff geometrically coherent.
    ...
    # mid-stroke of a still-closing terminal intercept. Infer phase from the
    # unpulsed posterior carrier: admit the pulse smoothly when that carrier
    # acceleration agrees with the requested turn, rather than braking the
    # opposite half-cycle. No clock or stored phase enters the feedback.
    ...
    base_phase_lag_target = 2 * terminal_mean_bend - q1 -
        params.tail_lag_gain * qd1 / max(omega, eps(omega))
    base_carrier_a2 = omega^2 * (base_phase_lag_target - q2) -
        2 * params.tail_damping * omega * qd2
    tail_acceleration_scale = max(
        abs(params.maximum_joint_acceleration),
        eps(Float64),
    )
    normalized_phase_agreement = terminal_turn_request * base_carrier_a2 /
        tail_acceleration_scale
    supportive_half_cycle_gate = 0.5 * (
        1 + tanh(
            normalized_phase_agreement /
            max(abs(params.tail_phase_alignment_softness), eps(Float64)),
        )
    )
    terminal_tail_pulse = closing_alignment * terminal_gate *
        midstroke_gate * unmet_release_gate * supportive_half_cycle_gate *
    ...
    phase_lag_target = base_phase_lag_target + terminal_tail_pulse
