# Unified predicted-miss handoff with adverse-lateral-force response.
# A bounded posterior pulse helps an unmet redirect oppose measured lateral
# loading without reallocating released route authority or changing the carrier.
...
function target_policy_params()
    return (
        ...
        lateral_force_scale=0.016,
        maximum_adverse_force_tail_pulse=1.5 * pi / 180,
...
function target_policy(state, params)
    ...
    lateral_force = isfinite(state.force_body_L[2]) ?
    ...
    # The same terminal request supplies direction to a small physical-response
    # component. Only lateral force opposing that request is adverse. It can
    # strengthen the posterior wave while the redirect remains unmet, but it
    # vanishes after release and never enters the shared route actuator.
    adverse_lateral_force = max(
        -terminal_turn_request * lateral_force,
        0.0,
    )
    adverse_force_gate = tanh(
        adverse_lateral_force /
        max(abs(params.lateral_force_scale), eps(Float64)),
    )
    force_tail_pulse = closing_alignment * terminal_gate * midstroke_gate *
        unmet_release_gate * adverse_force_gate *
        abs(params.maximum_adverse_force_tail_pulse) * terminal_turn_request
