# Unified predicted-miss steering with terminal course-to-phase modulation.
# Reliable target-relative course error changes posterior wave timing through
# observed joint phase while the state-feedback traveling carrier stays active.
...
function target_policy(state, params)
    ...
    # Translate reliable target-relative course error through observed carrier
    # phase into posterior wave timing. Course request and anterior bend side
    # are both reflection-odd, so their product is a reflection-even lag shift
    # and the resulting posterior target remains reflection-odd. Far, slow, or
    # non-closing motion continuously recovers the established base lag.
    terminal_course_request = tanh(
        sin(bounded_course_error) /
        max(abs(params.steering_softness), eps(Float64)),
    )
    phase_scale = max(
        abs(params.phase_state_softness) * amp,
        eps(Float64),
    )
    phase_side = tanh(q1 / phase_scale)
    course_response_gate = terminal_gate * course_weight * closing_alignment
    terminal_lag_shift = course_response_gate *
        abs(params.maximum_terminal_lag_shift) *
        terminal_course_request * phase_side
    effective_tail_lag_gain = params.tail_lag_gain + terminal_lag_shift

    # Center the oscillator and lagged posterior target on the slow mean bend.
    # The opposed traveling component remains active around that mean; only
    # posterior timing carries the new terminal course correction.
