# Unified predicted-miss handoff with active redirect-to-cruise yaw arrest.
# Released shared half-cycle authority countersteers residual body response
# while the state-feedback traveling bend remains continuously propulsive.
...
function target_policy(state, params)
    ...
    # becomes small. The released share actively arrests carrier-separated
    # yaw instead of leaving residual turn to decay through the crossing.
    ...
    # Retain a minimum target-turn share and transfer only released authority
    # to bounded countersteer against residual yaw. Both propulsive carrier
    # half-cycles remain active; the request transfer changes their asymmetry.
    redirect_gate = tanh(
    ...
    active_turn_request = clamp(
        ...
        released_course_share * yaw_arrest_request,
        -1.0,
        1.0,
    )
