# Phase-compensated terminal course on a traveling-bend carrier.
# Far-field course feedback preserves translation; near the target, joint-rate
# phase separation gives persistent translational miss its own mean-bend channel.
...
function target_policy_params()
    return (
        ...
        lateral_carrier_head_rate_gain=0.90,
        lateral_carrier_tail_rate_gain=0.47,
...
function target_policy(state, params)
    ...
    # The two near-miss traces share a repeatable joint-rate-correlated lateral
    # carrier. Remove it only for the terminal mean-turn signal; the far-field
    # course controller and physical traveling bend remain unchanged.
    lateral_carrier_velocity =
        -abs(params.lateral_carrier_head_rate_gain) * qd1 /
        max(omega, eps(Float64)) +
        abs(params.lateral_carrier_tail_rate_gain) * qd2 /
        max(omega, eps(Float64))
    corrected_velocity_y = velocity_y - lateral_carrier_velocity
    corrected_speed = hypot(velocity_x, corrected_velocity_y)
    corrected_course_cross =
        target_x * corrected_velocity_y - target_y * velocity_x
    corrected_course_dot =
        target_x * velocity_x + target_y * corrected_velocity_y
    corrected_course_scale = target_norm * corrected_speed
    corrected_antiparallel = corrected_course_dot < 0 &&
        abs(corrected_course_cross) <= sqrt(eps(Float64)) *
        max(corrected_course_scale, eps(Float64))
    corrected_course_error = if corrected_speed <= eps(Float64) ||
        target_norm <= eps(Float64)
        pursuit_error
    elseif corrected_antiparallel
        pursuit_error
    else
        atan(corrected_course_cross, corrected_course_dot)
    end
    bounded_corrected_course = clamp(
        corrected_course_error,
        -maximum_error,
        maximum_error,
    )
    corrected_course_weight = corrected_speed^2 /
        (corrected_speed^2 + speed_scale^2)
    terminal_navigation_error =
        (1 - corrected_course_weight) * bounded_pursuit +
        corrected_course_weight * bounded_corrected_course
    ...
    # Center the evidenced state-feedback carrier on the terminal mean. The
    # posterior target retains its opposed traveling component and phase lag.
    ...
    # Course misalignment recruits the evidenced half-cycle steering while
    # translation is observable. The terminal handoff fades, but does not
    # switch off, this phase-sensitive channel.
    redirect_gate = tanh(
