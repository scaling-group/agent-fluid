# Unified predicted-miss handoff with posterior phase-quadrant steering.
# Terminal course error reshapes posterior lag inside the traveling carrier
# instead of amplifying carrier acceleration or transferring residual torque.
...
function target_policy_params()
    return (
        ...
        maximum_terminal_lag_modulation=0.35,
...
function target_policy(state, params)
    ...
    # Center the oscillator and posterior target on the slow mean bend. A
    # target-relative cubic miss then changes posterior lag by anterior bend
    # quadrant. Miss and centered bend are both reflection-odd, so their
    # product is a reflection-even lag coefficient; multiplying by head rate
    # keeps the posterior target reflection-odd. This redistributes wave timing
    # without a static bend or carrier-magnitude amplification, and the common
    # response-plus-miss consensus releases the modulation back to cruise.
    q1_centered = q1 - terminal_mean_bend
    normalized_head_bend = q1_centered / max(amp, eps(Float64))
    head_bend_quadrant = tanh(normalized_head_bend)
    terminal_lag_modulation = terminal_gate * closing_alignment *
        unmet_release_gate *
        abs(params.maximum_terminal_lag_modulation) *
        prediction_request^3 * head_bend_quadrant
    effective_tail_lag_gain = params.tail_lag_gain +
        terminal_lag_modulation
