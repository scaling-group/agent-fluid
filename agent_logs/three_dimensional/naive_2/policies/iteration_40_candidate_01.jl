# Unified predicted-miss handoff with a counter-bend phase-spring redirect.
# Tail-first directional reserve admits a bounded redirect only on the part of
# each rhythmic excursion opposite the requested turn.
...
function target_policy(state, params)
    ...
    # Preserve the evidenced cubic predicted-miss request and tail-first
    # directional reserve, but change its actuator representation. A requested
    # redirect remains admissible near an absolute boundary if the joint state
    # points inward; excluded tail authority reaches only an anterior joint
    # that can accept the same direction. Unavailable authority is shed.
    ...
    # A compact phase spring acts only while a receiving joint is bent
    # opposite the requested redirect. It vanishes continuously at center and
    # leaves the requested-side excursion and rhythmic restoring half-cycle
    # unchanged. Oscillator stiffness supplies the owned acceleration scale.
    redirect_magnitude = abs(redirect_residual)
    tail_counter_bend = max(-redirect_direction * q2, 0.0)
    head_counter_bend = max(-redirect_direction * q1, 0.0)
    tail_phase_spring = redirect_direction * tail_capacity *
        redirect_magnitude * omega^2 * tail_counter_bend
    head_phase_spring = redirect_direction *
        params.head_asymmetry_share * (1 - tail_capacity) * head_capacity *
        redirect_magnitude * omega^2 * head_counter_bend
    ...
    # Keep the sampled carrier-scaled directional residual in the outer
    # predictive regime. The already-owned smooth 3L approach gate hands its
    # authority to the counter-bend spring only near capture.
    tail_redirect_acceleration =
        (1 - distance_gate) * tail_redirect_residual * abs(carrier_a2) +
        distance_gate * tail_phase_spring
    head_redirect_acceleration =
        (1 - distance_gate) * head_redirect_residual * abs(carrier_a1) +
        distance_gate * head_phase_spring
