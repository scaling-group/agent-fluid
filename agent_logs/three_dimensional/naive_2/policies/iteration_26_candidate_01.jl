# Carrier-insensitive interception with a unified response-and-geometry handoff.
# Reliable closing motion transfers terminal steering from beat-sensitive body
# bearing to rotation-invariant predicted miss without changing the carrier.
...
function target_policy(state, params)
    ...
    # When speed and closing alignment make the constant-course estimate
    # reliable, use its rotation-invariant cross-track miss for terminal
    # direction. Raw pursuit remains the low-speed/non-closing fallback.
    ...
    interception_weight = course_weight * closing_alignment
    terminal_turn_request = clamp(
        (1 - interception_weight) * terminal_pursuit_request +
        interception_weight * prediction_request,
        -1.0,
        1.0,
    )
