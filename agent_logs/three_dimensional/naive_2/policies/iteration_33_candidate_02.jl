# Unified predicted-miss handoff with selective capture-corridor release.
# A safe short-horizon intercept retires terminal offsets while preserving
# target-relative rhythmic steering around the state-feedback carrier.
...
function target_policy_params()
    return (
        ...
        capture_corridor_half_width=0.5,
        capture_corridor_width=0.12,
        capture_release_horizon=0.85,
        capture_release_time_width=0.2,
...
function target_policy(state, params)
    ...
    # A closing short-horizon course whose predicted miss is safely inside a
    # body-scaled corridor no longer needs slow bend or posterior-pulse bias.
    # Keep this release separate from rhythmic course steering: inherited
    # evidence shows that returning every correction to the carrier together
    # preserves capture but widens and slows the terminal crossing.
    corridor_width = max(
    ...
    corridor_gate = 0.5 * (
        1 - tanh(
            (
                abs(signed_predicted_miss) -
                abs(params.capture_corridor_half_width)
            ) / corridor_width,
        )
    )
    release_time_width = max(
        abs(params.capture_release_time_width),
        eps(Float64),
    )
    release_time_gate = 0.5 * (
        1 - tanh(
            (
                time_to_closest -
                abs(params.capture_release_horizon)
            ) / release_time_width,
        )
    )
    capture_release_gate = terminal_gate * course_weight *
        closing_alignment * corridor_gate * release_time_gate
    offset_correction_share = 1 - capture_release_gate

    terminal_mean_bend = offset_correction_share * terminal_gate *
    ...
    unmet_release_gate = offset_correction_share * (1 - release_gate)
    ...
    active_turn_request = clamp(course_share * turn_request, -1.0, 1.0)
