# Response-gated interception with a transverse-course posterior pulse.
# Body-frame prediction recruits a terminal bend; measured response, course,
# and joint phase preserve rhythmic authority while refining the capture pass.
...
function target_policy_params()
    return (
        ...
        terminal_course_residual_scale=0.35,
        terminal_course_residual_share=0.35,
...
function target_policy(state, params)
    ...
    # Predicted miss contracts with target distance even when the measured
    # course still crosses the line of sight sharply. Preserve a bounded share
    # of that dimensionless transverse-course residual only for the posterior
    # terminal pulse; far-field steering and persistent curvature are intact.
    course_residual_scale = max(
        abs(params.terminal_course_residual_scale),
        eps(Float64),
    )
    terminal_course_residual = tanh(
        sin(bounded_course_error) / course_residual_scale,
    )
    course_residual_weight = distance_gate * clamp(
        params.terminal_course_residual_share,
        0.0,
        1.0,
    )
    posterior_pulse_request =
        (1 - course_residual_weight) * terminal_turn_request +
        course_residual_weight * terminal_course_residual
