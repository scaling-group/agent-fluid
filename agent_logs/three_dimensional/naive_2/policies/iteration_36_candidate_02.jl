# Unified predicted-miss handoff with receiver-safe redirect spillover.
# Absolute posterior reserve retains the centered tail-first response, while
# excluded authority reaches the head only in a direction it can accept.
...
function target_policy(state, params)
    ...
    # Preserve the sampled cubic predicted-miss response and the absolute
    # posterior reserve associated with the centered course. Only its excluded
    # share may spill into the head, and then only while signed head angle and
    # rate state can accept the requested direction. Unavailable authority is
    # shed rather than exchanged for anterior limit exposure.
    ...
    tail_angle_reserve = 0.5 * (
        1 - tanh(
            (abs(q2) - soft_angle) / angle_width,
        )
    )
    tail_rate_reserve = 0.5 * (
        1 - tanh(
            (abs(qd2) - soft_rate) / rate_width,
        )
    )
    ...
    head_angle_capacity = 0.5 * (
        1 - tanh(
            (redirect_direction * q1 - soft_angle) / angle_width,
        )
    )
    ...
    head_redirect_residual = params.head_asymmetry_share *
        (1 - tail_reserve) * head_capacity * redirect_residual
