# Predicted-miss curvature with lagged-wave terminal follow-through.
# The posterior pulse follows observed two-joint phase through anterior
# reversal while the full traveling-bend carrier remains active.
...
function target_policy(state, params)
    ...
    # Carry the posterior wave-shape pulse through the lagged active stroke.
    # The inherited head-speed gate is retained; target-opposed posterior
    # motion adds follow-through when the head reverses. Their maximum avoids
    # double authority and remains reflection-equivariant without a clock.
    normalized_head_speed = abs(qd1) /
        max(omega * amp, eps(Float64))
    normalized_target_opposed_tail_speed = max(
        -terminal_turn_request * qd2,
        0.0,
    ) / max(omega * amp, eps(Float64))
    lagged_wave_phase_speed = max(
        normalized_head_speed,
        normalized_target_opposed_tail_speed,
    )
    lagged_wave_phase_gate = tanh(
        lagged_wave_phase_speed /
        max(abs(params.tail_pulse_rate_scale), eps(Float64)),
    )
    terminal_tail_pulse = closing_alignment * terminal_gate *
        lagged_wave_phase_gate * abs(params.maximum_terminal_tail_pulse) *
