# Unified predicted-miss handoff with harmonic phase-space joint reserve.
# A bounded oscillator-state rotation anticipates outward joint motion while
# preserving the rhythmic, target-relative residual and carrier envelope.
...
function target_policy(state, params)
    ...
    # directional allocation. Rotate normalized angle/rate state along the
    # inherited oscillator before testing angle capacity; unlike a linear
    # extrapolation, this bounded forecast preserves the carrier phase-space
    # envelope. The direct rate guard still protects high outward joint speed.
    # Any share neither joint can accept returns smoothly to the carrier.
    ...
    projection_phase = abs(params.joint_phase_projection)
    phase_cosine = cos(projection_phase)
    phase_sine = sin(projection_phase)
    normalized_qd1 = qd1 / max(omega, eps(omega))
    normalized_qd2 = qd2 / max(omega, eps(omega))
    projected_q1 = phase_cosine * q1 + phase_sine * normalized_qd1
    projected_q2 = phase_cosine * q2 + phase_sine * normalized_qd2
