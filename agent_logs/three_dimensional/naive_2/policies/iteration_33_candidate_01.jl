# Posterior predicted-miss residual with miss-conditioned drive relief.
# Excess closing miss recruits tail-only rhythmic steering while smoothly
# reducing symmetric posterior thrust around the state-feedback carrier.
...
function target_policy_params()
    return (
        ...
        maximum_terminal_tail_residual=0.35,
...
function target_policy(state, params)
    ...
    # Preserve the sampled tail-only actuator that improved traversal. Cubing
    # makes its directional half-cycle residual vanish rapidly on a centered
    # course while retaining bounded authority for the sampled large-miss
    # terminal class.
    posterior_miss_residual = terminal_gate * closing_alignment *
        abs(params.maximum_terminal_tail_residual) * prediction_request^3

    # The sampled family reaches the capture edge with excess lateral course.
    # Reduce only the symmetric posterior traveling component while close,
    # closing, and beyond a miss margin. Full posterior drive is restored on
    # centered courses; anterior rhythm and steering authority are untouched.
    miss_excess = max(
    ...
    relief_support_coordinate = max(
        (
            params.terminal_distance + 4 * terminal_width - distance
        ) / terminal_width,
        0.0,
    )
    compact_distance_support = tanh(relief_support_coordinate)^2
    drive_relief_gate = compact_distance_support * distance_gate *
        course_weight * closing_alignment * miss_relief_gate
    ...
    # Relief scales the opposed traveling component and lag around that mean,
    # not the mean bend, pulse, or directional posterior residual.
