# Predicted-miss curvature with actuator-specific redirect release.
# Corrective yaw releases each rhythmic steering channel only after its
# body-frame geometry residual also indicates a centered intercept.
...
function target_policy_params()
    return (
        ...
        terminal_corridor_half_width=0.55,
        terminal_corridor_softness=0.15,
...
function target_policy(state, params)
    ...
    # Corrective yaw can release shared half-cycle steering only after the
    # residual lateral target geometry lies inside the capture corridor.
    corridor_width = max(
        abs(params.terminal_corridor_softness),
        eps(Float64),
    )
    outside_corridor_gate = 0.5 * (
        1 + tanh(
            (abs(target_y) - abs(params.terminal_corridor_half_width)) /
            corridor_width,
        )
    )
    half_cycle_release_gate = corrective_response_gate *
        (1 - outside_corridor_gate)
    course_share = 1 - terminal_gate * half_cycle_release_gate *
    ...
    # direction and absolute joint speed supplies phase without a clock. The
    # pulse is released only when corrective response and predicted course
    # centering agree, rather than on a transient body response alone.
    ...
    prediction_ready_gate = 1 - abs(prediction_request)
    pulse_release_gate = corrective_response_gate * prediction_ready_gate
    unmet_pulse_release_gate = 1 - pulse_release_gate
    terminal_tail_pulse = closing_alignment * terminal_gate *
        midstroke_gate * unmet_pulse_release_gate *
