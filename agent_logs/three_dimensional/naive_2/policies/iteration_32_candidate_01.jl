# Unified predicted-miss handoff with miss-conditioned posterior drive relief.
# Wrong-side residual force retains rhythmic steering while excess near-field
# course miss temporarily reduces only the posterior thrust-producing wave.
...
function target_policy_params()
    return (
        ...
        carrier_force_head_rate_gain=0.0015,
        carrier_force_tail_rate_gain=-0.0037,
        maximum_lateral_force_residual=0.02,
        force_deficit_scale=0.007,
        maximum_terminal_tail_pulse=4.0 * pi / 180,
        tail_pulse_rate_scale=0.6,
        maximum_posterior_drive_relief=0.35,
        drive_relief_miss_margin=0.5,
        drive_relief_miss_width=0.4,
...
function target_policy(state, params)
    ...
    lateral_force = hasproperty(state, :force_body_L) &&
        isfinite(state.force_body_L[2]) ?
    ...
    released_course_share = terminal_gate * release_gate *
        (1 - minimum_course_share)
    course_share = 1 - released_course_share

    # Joint-rate regression across the sampled near fields identifies the
    # repeatable carrier component of normalized body-lateral force. Do not
    # cancel that beat-scale loading. Instead, use only a bounded residual
    # whose sign opposes the terminal request as evidence that the physical
    # redirect response is still deficient. This re-engages existing rhythmic
    # authority without enlarging its envelope or commanding from force alone.
    carrier_lateral_force =
        params.carrier_force_head_rate_gain * qd1 +
        params.carrier_force_tail_rate_gain * qd2
    force_residual_limit = max(
        abs(params.maximum_lateral_force_residual),
        eps(Float64),
    )
    lateral_force_residual = clamp(
        lateral_force - carrier_lateral_force,
        -force_residual_limit,
        force_residual_limit,
    )
    wrong_side_force = max(
        -terminal_turn_request * lateral_force_residual,
        0.0,
    )
    force_deficit_gate = tanh(
        wrong_side_force /
        max(abs(params.force_deficit_scale), eps(Float64)),
    )
    force_reengagement_share = released_course_share * closing_alignment *
        force_deficit_gate
    ...
    # The sampled family reaches the capture edge with a large transverse
    # constant-course miss.  Give the inherited steering another corrective
    # beat by reducing only the posterior traveling-wave target when that miss
    # exceeds a normalized margin.  The close-range and closing gates protect
    # far-field translation; exact zero excess restores the full carrier on an
    # already-centered course without weakening anterior steering authority.
    miss_excess = max(
        abs(signed_predicted_miss) - abs(params.drive_relief_miss_margin),
        0.0,
    )
    miss_relief_gate = tanh(
        miss_excess /
        max(abs(params.drive_relief_miss_width), eps(Float64)),
    )
    maximum_drive_relief = clamp(
        params.maximum_posterior_drive_relief,
        0.0,
        1.0,
    )
    drive_relief_gate = distance_gate * course_weight * closing_alignment *
        miss_relief_gate
    posterior_drive_scale = 1 - maximum_drive_relief * drive_relief_gate

    # Center the oscillator and lagged posterior target on the slow mean bend.
    # Relief scales only the traveling component around that mean, not the
    # posterior pulse or the evidenced response/geometry steering handoff.
    q1_centered = q1 - terminal_mean_bend
    vdp_drive = params.oscillator_mu *
        (1 - (q1_centered / amp)^2) * qd1
    carrier_a1 = vdp_drive - omega^2 * q1_centered
    phase_lag_target = terminal_mean_bend + terminal_tail_pulse -
        posterior_drive_scale * (
            q1_centered +
    ...
    active_turn_request = clamp(
        course_share * turn_request +
        force_reengagement_share * terminal_turn_request,
        -1.0,
        1.0,
    )
    asymmetry = asymmetry_limit * active_turn_request
