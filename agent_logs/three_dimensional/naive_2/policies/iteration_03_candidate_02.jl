# State-feedback traveling bend with response-gated half-cycle redirect.
# Joint state carries phase; normalized body-frame target geometry and lateral
# response steer without a clock, static curvature, or world-frame route.
...
function target_policy(state, params)
    ...
    # the current target side.  Their product is reflection invariant, so it
    # can gate a stronger C-start-like redirect while the signed request stays
    # reflection equivariant.  Authority remains below one, preserving both
    # half-cycles instead of replacing the traveling bend with bang-bang yaw.
    wrong_side_slip = max(
