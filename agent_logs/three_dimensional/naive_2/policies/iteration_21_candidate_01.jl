# Carrier-residual predicted-miss curvature with a response-released redirect.
# Joint-rate feedback separates beat-induced sway from the terminal course
# estimate while preserving the full traveling-bend carrier.
...
function target_policy_params()
    return (
        ...
        carrier_sway_head_rate_gain=-0.13,
        carrier_sway_tail_rate_gain=0.0685,
        maximum_carrier_sway=0.8,
...
function target_policy(state, params)
    ...
    # Preserve the inherited 3L proximity handoff as a fallback. For the
    # constant-course predictor only, subtract the repeatable beat-induced
    # sway evidenced across the sampled trajectories. Broad navigation above
    # still uses measured velocity, so this does not replace productive
    # lateral motion with carrier braking.
    terminal_width = max(abs(params.terminal_distance_width), eps(Float64))
    distance_gate = 0.5 * (
        1 - tanh((distance - params.terminal_distance) / terminal_width)
    )
    raw_carrier_sway =
        params.carrier_sway_head_rate_gain * qd1 +
        params.carrier_sway_tail_rate_gain * qd2
    carrier_sway_limit = max(
        abs(params.maximum_carrier_sway),
        eps(Float64),
    )
    carrier_sway = clamp(
        raw_carrier_sway,
    ...
    prediction_velocity_y = velocity_y - carrier_sway
    prediction_speed = hypot(velocity_x, prediction_velocity_y)
    prediction_cross =
        target_x * prediction_velocity_y - target_y * velocity_x
    prediction_dot =
        target_x * velocity_x + target_y * prediction_velocity_y
    prediction_course_scale = target_norm * prediction_speed
    prediction_weight = prediction_speed^2 /
        (prediction_speed^2 + speed_scale^2)
    prediction_floor = max(
        abs(params.prediction_speed_floor),
        eps(Float64),
    )
    safe_prediction_speed_squared = max(
        prediction_speed^2,
        prediction_floor^2,
    )
    raw_time_to_closest = prediction_dot / safe_prediction_speed_squared
    ...
    closing_alignment = prediction_course_scale > eps(Float64) ?
        clamp(
            prediction_dot / prediction_course_scale,
            0.0,
            1.0,
        ) : 0.0
    signed_predicted_miss = prediction_speed > prediction_floor ?
        prediction_cross / prediction_speed : 0.0
    miss_scale = max(abs(params.prediction_miss_scale), eps(Float64))
    prediction_request = tanh(signed_predicted_miss / miss_scale)
    prediction_gate = prediction_weight * closing_alignment * time_gate *
    ...
    # direction; absolute joint speed supplies phase without a clock. Release
    # the redirect as soon as carrier-separated yaw is already corrective,
    # rather than braking the propulsive carrier.
