# Unified predicted-miss handoff with predicted-capture corridor release.
# A short-horizon narrow intercept returns continuously to the autonomous
# carrier instead of recruiting another terminal corrective residual.
...
function target_policy(state, params)
    ...
    # Constant-course geometry can also identify when correction is no longer
    # useful. Only a closing, short-horizon intercept whose predicted miss is
    # safely inside a body-scaled corridor releases the target modulation.
    # The smooth conjunction cannot trigger at rest, while receding, or on a
    # wide pass, and it introduces neither an external phase nor a route.
    corridor_width = max(
    ...
    capture_correction_share = 1 - capture_release_gate

    terminal_mean_bend = capture_correction_share * terminal_gate *
    ...
    unmet_release_gate = capture_correction_share * (1 - release_gate)
    response_released_course_share = terminal_gate * release_gate *
        (1 - minimum_course_share)
    course_share = capture_correction_share *
        (1 - response_released_course_share)
