# Carrier-separated predicted-miss curvature on a traveling bend.
# Joint state sustains propulsion and removes beat-correlated lateral motion
# before predicted closest approach recruits the terminal mean bend.
...
function target_policy_params()
    return (
        ...
        carrier_lateral_head_angle_gain=-0.20,
        carrier_lateral_head_rate_gain=-0.065,
        maximum_carrier_lateral_velocity=0.75,
...
function target_policy(state, params)
    ...
    # The sampled carrier creates a repeatable beat-scale lateral velocity.
    # Its bounded joint-state prediction will be removed only from terminal
    # closest-approach geometry.  Ordinary course steering, measured speed,
    # and the physical carrier remain unchanged.  The normalized gains have
    # the same reflection parity as lateral velocity, so left/right target
    # symmetry is preserved.
    maximum_carrier_lateral_velocity = max(
        abs(params.maximum_carrier_lateral_velocity),
        eps(Float64),
    )
    carrier_lateral_velocity = clamp(
        params.carrier_lateral_head_angle_gain * q1 +
        params.carrier_lateral_head_rate_gain * qd1,
        -maximum_carrier_lateral_velocity,
        maximum_carrier_lateral_velocity,
    )
    prediction_velocity_y = velocity_y - carrier_lateral_velocity
    ...
    prediction_cross = target_x * prediction_velocity_y -
        target_y * velocity_x
    prediction_dot = target_x * velocity_x +
        target_y * prediction_velocity_y
    prediction_scale = target_norm * prediction_speed
    safe_speed_squared = max(prediction_speed^2, prediction_floor^2)
    raw_time_to_closest = prediction_dot / safe_speed_squared
    ...
    closing_alignment = prediction_scale > eps(Float64) ?
        clamp(prediction_dot / prediction_scale, 0.0, 1.0) : 0.0
