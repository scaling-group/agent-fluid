# Unified predicted-miss handoff with course-confirmed redirect release.
# Rhythmic steering returns toward cruise only when corrective response, small
# predicted miss, and target-relative translational alignment all agree.
...
function target_policy(state, params)
    ...
    # A correct-sign yaw or small perpendicular miss can occur while the fish
    # still travels laterally across the target. Release both rhythmic steering
    # channels only when response, miss, and target-relative course agree.
    geometry_ready_gate = 1 - abs(prediction_request)
    course_ready_gate = 1 - abs(sin(bounded_course_error))
    ...
    # common response, miss, and course gate releases the pulse and shared
    # redirect together rather than handing off on transient body yaw.
