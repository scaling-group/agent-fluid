# Closing-course terminal handoff on response-gated predicted-miss curvature.
# Rotation-invariant course geometry owns the closing intercept; raw pursuit
# returns only when course reliability or closing alignment is lost.
...
function target_policy(state, params)
    ...
    # While a reliable course is still closing, do not let beat-correlated raw
    # bearing compete with the rotation-invariant signed-miss request. Restore
    # pursuit continuously at low speed or as closing alignment is lost, so a
    # missed pass retains recovery authority without changing the far route.
    pursuit_weight = distance_gate *
        (1 - course_weight * closing_alignment)
    request_weight = pursuit_weight + prediction_gate
    terminal_turn_request = request_weight > eps(Float64) ? clamp(
        (
            pursuit_weight * terminal_pursuit_request +
