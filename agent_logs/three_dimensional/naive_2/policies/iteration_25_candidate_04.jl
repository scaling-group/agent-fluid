# Actuator-specific response-and-geometry handoffs on predicted-miss curvature.
# Shared rhythmic steering releases inside the raw lateral capture corridor;
# the posterior pulse releases when the predicted course miss is small.
...
function target_policy(state, params)
    ...
    # Keep shared rhythmic steering until carrier-separated yaw confirms a
    # corrective response and the raw lateral target offset is inside the
    # capture corridor. This response residual matches the actuator's role:
    # shared half-cycle asymmetry moves the whole lateral route.
    ...
    # miss is already small. This separate response residual matches the
    # posterior actuator's role in shaping the terminal course.
