# Unified predicted-miss handoff with directional joint-reserve allocation.
# A bounded terminal residual remains rhythmic and target-relative while each
# joint accepts only the share whose direction preserves kinematic reserve.
...
function target_policy(state, params)
    ...
    # directional rather than absolute reserve. A requested acceleration that
    # returns a joint toward center remains admissible near an absolute soft
    # boundary. Outward tail demand moves to the head only while that joint can
    # accept the same direction; any remainder returns smoothly to the carrier.
