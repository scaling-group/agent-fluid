# Predicted-miss interception with a corrective anterior half-cycle hold.
# The sampled traveling carrier is preserved; target geometry and observed
# joint state briefly prolong a useful terminal bend without braking the tail.
...
function target_policy_params()
    return (
        ...
        terminal_head_dwell_angle_scale=12.0 * pi / 180,
        terminal_head_dwell_rate_scale=0.35,
        maximum_terminal_head_dwell_acceleration=8.0,
...
function target_policy(state, params)
    ...
    # Extend only the target-signed part of the anterior half-cycle when the
    # joint is already on that useful side but is departing it. This is a
    # state-based duty-ratio change: it fades on the opposite side, at stroke
    # reversal, outside the near-target gate, or after closing alignment is
    # lost. The posterior traveling-wave target is not scaled or held.
    dwell_angle_scale = max(
        abs(params.terminal_head_dwell_angle_scale),
        eps(Float64),
    )
    useful_bend_gate = tanh(max(
        terminal_turn_request * q1 / dwell_angle_scale,
        0.0,
    ))
    normalized_departure = max(
        -terminal_turn_request * qd1 /
        max(omega * amp, eps(Float64)),
        0.0,
    )
    departing_bend_gate = tanh(
        normalized_departure /
        max(abs(params.terminal_head_dwell_rate_scale), eps(Float64)),
    )
    terminal_head_dwell = distance_gate * closing_alignment *
        abs(terminal_turn_request) * useful_bend_gate * departing_bend_gate *
        abs(params.maximum_terminal_head_dwell_acceleration) *
        terminal_turn_request
