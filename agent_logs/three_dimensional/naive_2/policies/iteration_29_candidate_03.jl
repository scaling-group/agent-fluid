# Unified predicted-miss handoff with target-line-rate steering.
# Released redirect authority remains target-relative while the state-feedback
# traveling wave and its response-plus-geometry handoff remain propulsive.
...
function target_policy(state, params)
    ...
    # yaw looks corrective. Transfer only the already-released route share to
    # bounded inertial target-line-rate feedback during closing. Target and
    # velocity are in the same normalized body frame, so their cross product
    # rejects beat-scale body orientation without a world route or clock.
