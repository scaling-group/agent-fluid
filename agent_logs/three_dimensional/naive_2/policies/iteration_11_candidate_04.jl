# Response-gated posterior braking on a course-residual traveling bend.
# Far-field course feedback preserves propulsion; a near wrong-sign yaw
# selectively damps tail thrust without arresting the anterior steering beat.
...
function target_policy_params()
    return (
        ...
        posterior_brake_distance=2.75,
        posterior_brake_distance_width=0.45,
        posterior_brake_closing_speed_scale=0.35,
        posterior_brake_course_scale=0.45,
        posterior_brake_yaw_rate_scale=0.40,
        maximum_posterior_brake=0.85,
        posterior_hold_frequency_ratio=0.40,
        posterior_hold_damping=1.0,
...
function target_policy(state, params)
    ...
    # Body forward is -x. Target and velocity share the body frame, so their
    # angular mismatch is independent of instantaneous body orientation.
    # Resolve the exactly-aft atan branch without imposing a turn direction.
    ...
    # In the region where the inherited course carrier starts to diverge,
    # center its traveling bend on persistent target geometry. Retain a share
    # of course-linked half-cycle authority during the handoff.
    ...
    # Preserve the evidenced state-feedback oscillator and posterior lag.
    # Centering both joint targets on the terminal mean changes slow curvature
    # without replacing joint-state phase with a clock.
    ...
    # Selective posterior braking is recruited only when four observed facts
    # agree: the target is near, motion is closing, the course predicts a miss,
    # and measured yaw is responding opposite to the requested turn. In this
    # sign convention positive turn request requires negative body yaw, so the
    # invariant product identifies a counterproductive response on either side.
    radial_closing_speed = target_norm > eps(Float64) ?
        course_dot / target_norm : 0.0
    brake_width = max(
        abs(params.posterior_brake_distance_width),
        eps(Float64),
    )
    brake_proximity = 0.5 * (
        1 - tanh(
            (distance - params.posterior_brake_distance) / brake_width,
        )
    )
    closing_gate = tanh(
        max(radial_closing_speed, 0.0) /
        max(
            abs(params.posterior_brake_closing_speed_scale),
            eps(Float64),
        ),
    )
    miss_gate = tanh(
        abs(sin(bounded_course_error)) /
        max(abs(params.posterior_brake_course_scale), eps(Float64)),
    )
    wrong_sign_yaw = max(turn_request * heading_rate, 0.0)
    yaw_gate = tanh(
        wrong_sign_yaw /
        max(abs(params.posterior_brake_yaw_rate_scale), eps(Float64)),
    )
    posterior_brake = clamp(params.maximum_posterior_brake, 0.0, 1.0) *
        brake_proximity * closing_gate * miss_gate * yaw_gate

    # Dampen only the posterior joint toward the target-signed mean. The
    # anterior joint keeps the course-responsive rhythm, and correct-sign yaw
    # immediately restores the posterior traveling wave and thrust.
    posterior_hold_omega = omega * clamp(
        abs(params.posterior_hold_frequency_ratio),
        eps(Float64),
        1.0,
    )
    posterior_hold_damping = max(abs(params.posterior_hold_damping), 0.0)
    posterior_hold_a2 = posterior_hold_omega^2 *
        (terminal_mean_bend - q2) -
        2 * posterior_hold_damping * posterior_hold_omega * qd2
    raw_a1 = rhythmic_a1
    raw_a2 = (1 - posterior_brake) * rhythmic_a2 +
        posterior_brake * posterior_hold_a2
