# Carrier-envelope recovery with response-gated predictive interception.
# Joint phase-plane deficit accelerates establishment of the propulsive rhythm;
# the evidenced target-feedback composite is unchanged at full carrier amplitude.
...
function target_policy_params()
    return (
        ...
        oscillator_mu=0.35,
        minimum_carrier_phase_radius=0.75,
        maximum_carrier_recovery_gain=1.2,
        tail_lag_gain=0.8,
        tail_damping=0.65,
...
function target_policy(state, params)
    ...
    # A phase-plane amplitude deficit recruits temporary state-only
    # anti-damping during startup or rhythm loss. The recovery term is exactly
    # zero above its envelope, preserving the evidenced established carrier.
    q1_centered = q1 - terminal_mean_bend
    normalized_phase_radius = hypot(
        q1_centered / amp,
        qd1 / max(omega * amp, eps(Float64)),
    )
    recovery_threshold = max(
        abs(params.minimum_carrier_phase_radius),
        eps(Float64),
    )
    amplitude_deficit = clamp(
        (recovery_threshold - normalized_phase_radius) /
            recovery_threshold,
        0.0,
        1.0,
    )
    carrier_energy_gain = params.oscillator_mu *
        (1 - (q1_centered / amp)^2) +
        abs(params.maximum_carrier_recovery_gain) * amplitude_deficit
    vdp_drive = carrier_energy_gain * qd1
