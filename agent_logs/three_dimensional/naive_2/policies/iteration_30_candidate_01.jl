# Unified predicted-miss handoff with persistent target-line steering.
# Released redirect authority responds only when instantaneous and windowed
# target-line motion agree around the state-feedback traveling carrier.
...
function target_policy(state, params)
    ...
    # Release both inherited rhythmic steering channels only as the remaining
    # predicted miss also becomes small, keeping their handoff coherent.
    ...
    # Body yaw and lateral translation oscillate over the propulsive cycle, so
    # a single target/velocity sample is not sufficient evidence of persistent
    # tangential approach. Compare its inertial target-line rate with the
    # independently observed bearing-window rate plus recent body turn rate.
    # Only smooth same-sign consensus receives the already-released steering
    # share; disagreement returns to the evidenced unified handoff.
    instantaneous_line_rate = -course_cross /
        max(target_norm^2, eps(Float64))
    windowed_line_rate = bearing_window_rate + turn_rate_recent
    line_rate_scale = max(
        abs(params.target_line_rate_scale),
        eps(Float64),
    )
    signed_rate_product = instantaneous_line_rate * windowed_line_rate
    positive_rate_product = max(signed_rate_product, 0.0)
    persistence_gate = positive_rate_product / (
        positive_rate_product + line_rate_scale^2
    )
    persistent_line_rate = 0.5 * (
        instantaneous_line_rate + windowed_line_rate
    ) * persistence_gate
    target_line_rate_request = -tanh(
        persistent_line_rate / line_rate_scale,
    )
    line_rate_share = released_course_share * closing_alignment
    ...
    active_turn_request = clamp(
        ...
        line_rate_share * target_line_rate_request,
        -1.0,
        1.0,
    )
