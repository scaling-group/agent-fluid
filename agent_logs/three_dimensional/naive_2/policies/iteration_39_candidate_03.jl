# Unified predicted-miss handoff with envelope-sharing anterior spillover.
# Posterior reserve loss shifts the anterior carrier center while contracting
# its excursion, instead of adding acceleration on top of the carrier.
...
function target_policy(state, params)
    ...
    # Preserve the sampled cubic predicted-miss response and absolute
    # posterior reserve. The tail retains the evidenced acceleration residual.
    # Reserve-blocked authority becomes an anterior wave-center displacement,
    # rather than another acceleration residual on top of the carrier.
    ...
    head_wave_offset = amp * params.head_asymmetry_share *
        (1 - posterior_reserve) * posterior_miss_residual

    # Share the inherited anterior angular envelope between steering center
    # and rhythmic excursion. Removing the offset from the posterior lag input
    # prevents the anterior spillover from cancelling posterior thrust.
    head_wave_amplitude = max(amp - abs(head_wave_offset), eps(Float64))
    q1_centered = q1 - terminal_mean_bend - head_wave_offset
    vdp_drive = params.oscillator_mu *
        (1 - (q1_centered / head_wave_amplitude)^2) * qd1
