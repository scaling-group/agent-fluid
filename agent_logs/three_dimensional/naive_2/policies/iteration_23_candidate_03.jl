# Predicted-miss curvature with target-response redirect release.
# Body-frame interception recruits a persistent mean bend; rhythmic steering
# fades only when the target-bearing response is already corrective.
...
function target_policy_params()
    return (
        ...
        maximum_bearing_response_rate=0.5,
...
function target_policy(state, params)
    ...
    # Keep the rhythmic steering channel available until target-relative
    # geometry confirms a corrective response. Unlike modeled carrier yaw,
    # the short-window bearing response cannot release the redirect while the
    # signed target request is still opening.
    bounded_bearing_response_rate = clamp(
        bearing_window_rate,
        -abs(params.maximum_bearing_response_rate),
        abs(params.maximum_bearing_response_rate),
    )
    corrective_bearing_response = max(
        -terminal_turn_request * bounded_bearing_response_rate,
        0.0,
    )
    corrective_response_gate = tanh(
        corrective_bearing_response /
    ...
    # the redirect only after the target-bearing response is corrective,
