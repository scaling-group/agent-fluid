# Predicted-miss curvature with a target-signed posterior-stroke redirect.
# Body-frame interception recruits a persistent mean bend; posterior joint
# phase confines the bounded redirect to the requested traveling-wave stroke.
...
function target_policy(state, params)
    ...
    # Add a posterior-only redirect while the lagged joint is moving in the
    # requested bend direction. The reflection-invariant request/rate product
    # supplies phase without a clock, releasing the pulse on the returning
    # target-opposed stroke and after closest approach.
    target_signed_posterior_speed = max(
        terminal_turn_request * qd2,
        0.0,
    ) / max(omega * amp, eps(Float64))
    posterior_stroke_gate = tanh(
        target_signed_posterior_speed /
        max(abs(params.tail_pulse_rate_scale), eps(Float64)),
    )
    terminal_tail_pulse = closing_alignment * terminal_gate *
        posterior_stroke_gate * abs(params.maximum_terminal_tail_pulse) *
