# Unified predicted-miss handoff with current-command margin allocation.
# Large closing misses recruit tail-first rhythmic authority only while the
# combined command retains restoring capacity near each joint's soft limits.
...
function target_policy_params()
    return (
        ...
        joint_soft_angle=38.0 * pi / 180,
        joint_angle_width=3.0 * pi / 180,
        joint_soft_rate=235.0 * pi / 180,
        joint_rate_width=20.0 * pi / 180,
...
function target_policy(state, params)
    ...
    # direction; absolute joint speed supplies phase without a clock. A
    ...
    # Evaluate the present carrier plus the evidenced common redirect before
    # allocating the cubic terminal residual. This avoids treating the prior
    # action as the current carrier phase and exposes the actual restoring
    # acceleration margin available on this state-feedback step.
    base_a1 = carrier_a1 + params.head_asymmetry_share * asymmetry *
        abs(carrier_a1)
    base_a2 = carrier_a2 + params.tail_asymmetry_share * asymmetry *
        abs(carrier_a2)

    # Large predicted misses retain the sampled tail-first rhythmic actuator.
    # At a signed angle or rate boundary, however, admit only the share that
    # fits inside the current base command's restoring margin. Thus an
    # outward-signed increment may survive a strongly restoring carrier, but
    # cannot reverse the combined command outward. Unavailable tail authority
    # reaches the head under the same test; any remainder returns to cruise.
    redirect_residual = terminal_gate * closing_alignment *
        abs(params.maximum_terminal_redirect_residual) * prediction_request^3
    redirect_direction = sign(redirect_residual)
    soft_angle = abs(params.joint_soft_angle)
    angle_width = max(abs(params.joint_angle_width), eps(Float64))
    soft_rate = abs(params.joint_soft_rate)
    rate_width = max(abs(params.joint_rate_width), eps(Float64))

    tail_angle_pressure = 0.5 * (
        1 + tanh((redirect_direction * q2 - soft_angle) / angle_width)
    )
    tail_rate_pressure = 0.5 * (
        1 + tanh((redirect_direction * qd2 - soft_rate) / rate_width)
    )
    tail_limit_pressure = 1 -
        (1 - tail_angle_pressure) * (1 - tail_rate_pressure)
    tail_carrier_scale = max(abs(carrier_a2), eps(Float64))
    tail_base_coefficient = base_a2 / tail_carrier_scale
    tail_restoring_margin = max(
        -redirect_direction * tail_base_coefficient,
        0.0,
    )
    tail_margin_share = clamp(
        tail_restoring_margin /
        max(abs(redirect_residual), eps(Float64)),
        0.0,
        1.0,
    )
    tail_capacity = 1 - tail_limit_pressure * (1 - tail_margin_share)
    tail_redirect_residual = tail_capacity * redirect_residual

    unallocated_tail_residual = (1 - tail_capacity) * redirect_residual
    requested_head_residual = params.head_asymmetry_share *
        unallocated_tail_residual
    head_angle_pressure = 0.5 * (
        1 + tanh((redirect_direction * q1 - soft_angle) / angle_width)
    )
    head_rate_pressure = 0.5 * (
        1 + tanh((redirect_direction * qd1 - soft_rate) / rate_width)
    )
    head_limit_pressure = 1 -
        (1 - head_angle_pressure) * (1 - head_rate_pressure)
    head_carrier_scale = max(abs(carrier_a1), eps(Float64))
    head_base_coefficient = base_a1 / head_carrier_scale
    head_restoring_margin = max(
        -redirect_direction * head_base_coefficient,
        0.0,
    )
    head_margin_share = clamp(
        head_restoring_margin /
        max(abs(requested_head_residual), eps(Float64)),
        0.0,
        1.0,
    )
    head_capacity = 1 - head_limit_pressure * (1 - head_margin_share)
    head_redirect_residual = head_capacity * requested_head_residual

    raw_a1 = base_a1 + head_redirect_residual * abs(carrier_a1)
    raw_a2 = base_a2 + tail_redirect_residual * abs(carrier_a2)
