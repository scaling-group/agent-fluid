# Carrier-separated predicted-miss curvature with a posterior mid-stroke pulse.
# Body-frame interception removes joint-rate-correlated lateral carrier sway
# from terminal course prediction while preserving the propulsive rhythm.
...
function target_policy_params()
    return (
        ...
        carrier_sway_tail_rate_gain=0.07,
        maximum_carrier_sway=1.0,
...
function target_policy(state, params)
    ...
    # The captured rollouts show that joint-rate-correlated carrier motion
    # dominates instantaneous lateral velocity near the target. Remove only
    # that bounded fast component for closest-pass prediction; the proven
    # far-field course controller above continues to use measured velocity.
    raw_carrier_sway =
    ...
    # pulse vanishes at beat reversal and after closest approach.
