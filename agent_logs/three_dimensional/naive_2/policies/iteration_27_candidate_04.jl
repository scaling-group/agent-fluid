# Unified response-and-geometry steering with a terminal amplitude envelope.
# Centered closing motion smoothly relieves excess drive, while an off-course
# or non-closing intercept retains the full state-feedback traveling carrier.
...
function target_policy_params()
    return (
        ...
        minimum_terminal_drive_share=0.75,
...
function target_policy(state, params)
    ...
    # Only a reliably closing, centered near-field intercept earns drive
    # relief. Normalize closing speed by an existing observation scale; this
    # keeps full propulsion for weak, receding, or off-course motion.
    closing_speed = target_norm > eps(Float64) ?
        max(course_dot / target_norm, 0.0) : 0.0
    closing_speed_gate = closing_speed^2 /
        (closing_speed^2 + speed_scale^2)
    approach_relief_gate = distance_gate * course_weight *
        closing_alignment * closing_speed_gate * geometry_ready_gate
    ...
    # A smooth, bounded amplitude envelope reduces only the limit-cycle drive;
    # restoring dynamics and posterior lag remain intact, so the controller
    # never coasts and immediately recovers full propulsion if geometry opens.
    minimum_drive_share = clamp(
        params.minimum_terminal_drive_share,
        0.0,
        1.0,
    )
    drive_share = 1 - approach_relief_gate * (1 - minimum_drive_share)
    drive_amplitude = max(amp * drive_share, eps(Float64))
    q1_centered = q1 - terminal_mean_bend
    vdp_drive = params.oscillator_mu *
        (1 - (q1_centered / drive_amplitude)^2) * qd1
