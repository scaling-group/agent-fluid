# Unified response-and-geometry steering with physical-moment rejection.
# Once the terminal redirect is centered, unused half-cycle authority opposes
# measured yaw moment without changing the propulsive carrier or route command.
...
function target_policy_params()
    return (
        ...
        terminal_moment_scale=0.012,
        maximum_terminal_moment_rejection_share=0.5,
...
function target_policy(state, params)
    ...
    moment_z = isfinite(state.moment_z_L2) ?
    ...
    # A correct-sign yaw can be a transient carrier response. Release both
    # rhythmic steering channels only when response and remaining predicted
    # miss agree that the intercept is centered.
    ...
    # common response-and-geometry gate releases the pulse and shared redirect
    # together rather than handing off on transient yaw alone.
    ...
    route_asymmetry = course_share * asymmetry_limit * turn_request

    # The carrier-separated yaw used by the release gate is a completion cue,
    # not a course reference. After that response and predicted miss agree,
    # reject measured hydrodynamic yaw moment using only otherwise-unused
    # half-cycle authority. This preserves the established route command and
    # cannot enlarge its existing asymmetry envelope.
    moment_scale = max(abs(params.terminal_moment_scale), eps(Float64))
    moment_rejection_request = -tanh(moment_z / moment_scale)
    moment_rejection_gate = terminal_gate * release_gate * course_weight *
        closing_alignment
    rejection_share = clamp(
        params.maximum_terminal_moment_rejection_share,
        0.0,
        1.0,
    )
    asymmetry_reserve = max(asymmetry_limit - abs(route_asymmetry), 0.0)
    moment_asymmetry = moment_rejection_gate * rejection_share *
        asymmetry_reserve * moment_rejection_request
    asymmetry = route_asymmetry + moment_asymmetry
