# Response-gated predicted-miss curvature with a restoring-half-cycle tail pulse.
# Body-frame interception recruits a persistent mean bend; measured yaw and
# posterior joint direction preserve rhythmic authority and terminal rate reserve.
...
function target_policy(state, params)
    ...
    # Apply the posterior wave-shape pulse only while its requested bend
    # opposes measured tail motion. This state-based restoring half-cycle
    # gate avoids reinforcing a tail already moving toward its rate envelope;
    # it vanishes at beat reversal and after closest approach.
    normalized_restoring_tail_speed = max(
        -terminal_turn_request * qd2,
        0.0,
    ) /
        max(omega * amp, eps(Float64))
    restoring_half_cycle_gate = tanh(
        normalized_restoring_tail_speed /
        max(abs(params.tail_restoring_rate_scale), eps(Float64)),
    )
    terminal_tail_pulse = closing_alignment * terminal_gate *
        restoring_half_cycle_gate * abs(params.maximum_terminal_tail_pulse) *
