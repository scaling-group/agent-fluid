# Unified predicted-miss handoff with scheduled harmonic-excursion reserve.
# Near-target geometry smoothly recruits a one-sided phase-plane predictor
# without changing the earlier directional allocator or propulsive carrier.
...
function target_policy(state, params)
    ...
    # directional allocation. As near-target geometry engages, interpolate
    # from current directed angle toward the joint's one-sided harmonic peak.
    # This anticipates outward angular excursion without the double counting
    # of a linear angle-plus-rate projection. Earlier predictive steering uses
    # the faster directional allocator, while the direct rate guard remains
    # active throughout and unavailable authority returns to the carrier.
    ...
    tail_outward_phase = distance_gate * tanh(
        max(directed_tail_rate, 0.0) / rate_width,
    )
    ...
    head_outward_phase = distance_gate * tanh(
        max(directed_head_rate, 0.0) / rate_width,
    )
