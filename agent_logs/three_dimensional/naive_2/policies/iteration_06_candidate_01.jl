# State-feedback traveling bend with phase-compensated burst redirects.
# Full body-frame target geometry trades symmetric drive for steering during
# near misalignment; accumulated signed bend releases the carrier before pinning.
...
function target_policy_params()
    return (
        ...
        burst_release_mean_bend=32.0 * pi / 180,
...
function target_policy(state, params)
    ...
    # retaining signed half-cycle steering. The parent kept this relief active
    # after both joints accumulated the requested bend and consequently pinned
    # them at the same angle limit. Use reflection-invariant signed mean bend
    # to release the relief before that state: the symmetric carrier then
    # restores the reverse half-cycle while steering remains target-directed.
    ...
    signed_mean_bend = turn_request * 0.5 * (q1 + q2)
    ...
    bend_reserve = 0.5 * (
        1 - tanh(
            (signed_mean_bend - params.burst_release_mean_bend) /
