# Capture-corridor authority reserve on predicted-miss terminal curvature.
# A traveling bend supplies propulsion; body-frame interception geometry keeps
# half-cycle steering available until the measured course enters a corridor.
...
function target_policy_params()
    return (
        ...
        capture_corridor=0.6,
        capture_corridor_width=0.15,
        minimum_response_time=0.75,
        response_time_width=0.25,
...
function target_policy(state, params)
    ...
    # frame, so their angular mismatch rejects beat-scale body yaw. Exact
    # aft-centerline geometry remains reflection-unbiased.
    ...
    # Predict a constant-course closest approach from normalized target and
    # velocity. Positive time means the target is still ahead along the
    # measured course; cross/speed is the signed perpendicular miss.
    ...
    # Smoothly unite ordinary near-target pursuit with predictive curvature.
    # The predictive request controls only the authority it recruited.
    ...
    # A repeat of the captured parent missed while the fixed terminal handoff
    # held course steering at its minimum share. Restore that faster channel
    # only when the predicted miss remains outside a normalized corridor and
    # there is still time for a response. Proximity prevents far-field course
    # oscillations from recruiting this terminal reserve.
    corridor_width = max(
        abs(params.capture_corridor_width),
        eps(Float64),
    )
    miss_excess_gate = 0.5 * (
        1 + tanh(
            (abs(signed_predicted_miss) - abs(params.capture_corridor)) /
            corridor_width,
        )
    )
    response_width = max(abs(params.response_time_width), eps(Float64))
    response_time_gate = 0.5 * (
        1 + tanh(
            (time_to_closest - abs(params.minimum_response_time)) /
            response_width,
        )
    )
    authority_reserve = distance_gate * course_weight * closing_alignment *
        time_gate * response_time_gate * miss_excess_gate
    ...
    # The opposed traveling component and posterior lag remain intact.
    ...
    # Half-cycle steering keeps both carrier half-cycles active. The new
    # reserve changes only its geometrically allocated terminal share.
    redirect_gate = tanh(
