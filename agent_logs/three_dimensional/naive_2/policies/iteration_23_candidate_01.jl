# Corridor-gated redirect release on predicted-miss curvature.
# Corrective yaw reduces rhythmic steering only after residual body-frame
# target geometry is already consistent with the terminal capture corridor.
...
function target_policy(state, params)
    ...
    # A corrective yaw response is not sufficient to finish the redirect.
    # Permit it to release half-cycle steering only after the target's
    # normalized lateral offset is also inside the evidenced capture corridor.
    # Outside the corridor, both carrier half-cycles remain fully driven and
    # the established target/course request continues to provide turn sign.
    corridor_width = max(
    ...
    redirect_release_gate = corrective_response_gate *
        (1 - outside_corridor_gate)
    course_share = 1 - terminal_gate * redirect_release_gate *
