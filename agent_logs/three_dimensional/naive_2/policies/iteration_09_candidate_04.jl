# Terminal mean-curvature handoff on a course-residual traveling bend.
# Far-field translation steers through measured course error; near the target,
# body-frame bearing shifts the mean of both joint oscillations without a clock.
...
function target_policy(state, params)
    ...
    # No reflection-equivariant controller can choose a side on the exactly
    # aft centerline; leave that measure-zero geometry unbiased.
    ...
    # The inherited course residual gives the best approach but its
    # phase-sensitive steering does not establish a terminal mean bend. Hand
    # authority smoothly to persistent target geometry only inside the region
    # where that rollout began to diverge.
    ...
    # The 2*mean posterior target preserves an oppositely signed traveling
    # component around that same mean: q2_target-mean = -(q1-mean)-lag.
    ...
    # abruptly switch off, this phase-sensitive channel.
    redirect_gate = tanh(
