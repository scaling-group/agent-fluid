# Unified predicted-miss handoff with carrier-separated response preview.
# Observed correct-sign yaw reduces only the portion of the constant-course
# miss that the current turn can cover, then releases toward the base carrier.
...
function target_policy(state, params)
    ...
    # Separate beat-scale carrier yaw before using body response in terminal
    # guidance. This evidenced residual is shared by the response preview and
    # the inherited redirect-release gate below.
    ...
    raw_prediction_request = tanh(signed_predicted_miss / miss_scale)

    # Give credit only to yaw that is correcting the current miss. A
    # constant-rate kinematic preview converts that carrier-separated response
    # into lateral sweep over the remaining bounded approach. The existing
    # miss scale bounds the credit, while the multiplicative relief cannot
    # reverse the requested turn if the local approximation over-predicts.
    preview_time = min(
        time_to_closest,
        abs(params.maximum_prediction_horizon),
    )
    corrective_prediction_yaw = time_gate * max(
        -raw_prediction_request * yaw_residual,
        0.0,
    )
    raw_response_sweep = 0.5 * speed * corrective_prediction_yaw *
        preview_time^2
    bounded_response_sweep = miss_scale * tanh(
        raw_response_sweep / miss_scale,
    )
    response_relief_fraction = tanh(
        bounded_response_sweep / max(abs(signed_predicted_miss), miss_scale),
    )
    response_previewed_miss = signed_predicted_miss *
        (1 - response_relief_fraction)
    prediction_request = tanh(response_previewed_miss / miss_scale)
    ...
    # Keep rhythmic steering available until the same carrier-separated yaw
    # confirms a corrective terminal response. This preserves the inherited
    # handoff while making the predictor and release use one response signal.
