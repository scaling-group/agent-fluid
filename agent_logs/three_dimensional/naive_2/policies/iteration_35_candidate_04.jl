# Unified predicted-miss handoff with carrier-phase reserve allocation.
# Large closing misses recruit tail-first rhythmic authority only while each
# joint's observed carrier command has directional kinematic reserve.
...
function target_policy(state, params)
    ...
    qd1 = state.phi_dot[1]
    qd2 = state.phi_dot[2]
    previous_a1 = isfinite(state.previous_action[1]) ?
        Float64(state.previous_action[1]) : 0.0
    previous_a2 = isfinite(state.previous_action[2]) ?
        Float64(state.previous_action[2]) : 0.0
    ...
    # observed carrier-command phase rather than by the residual sign. The
    # residual modulates abs(carrier), so its sign is not the joint's actual
    # acceleration direction on both half-cycles. Bounded previous action is a
    # one-step observation of that direction: restoring phases retain
    # authority near a limit, while outward phases release it. Tail authority
    # remains primary; admissible head capacity receives only unavailable tail
    # share, and any remainder returns continuously to the cruise carrier.
    ...
    head_carrier_direction = tanh(previous_a1 / action_scale)
    tail_carrier_direction = tanh(previous_a2 / action_scale)
