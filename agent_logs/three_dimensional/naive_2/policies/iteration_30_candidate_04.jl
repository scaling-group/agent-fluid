# Unified predicted-miss handoff with a released posterior course pulse.
# Shared redirect authority returns toward cruise after measured response;
# residual target-course error then shapes one corrective posterior half-cycle.
...
function target_policy_params()
    return (
        ...
        maximum_released_tail_course_pulse=3.0 * pi / 180,
...
function target_policy(state, params)
    ...
    # Release rhythmic steering only as the remaining predicted miss also
    # becomes small. Keep the released share explicit so a different,
    # posterior-only wave-shape channel can carry residual course correction.
    ...
    # Once the shared redirect is released, apply remaining target-course
    # feedback through one corrective posterior half-cycle. The product of
    # signed course request and signed observed joint motion is reflection-even
    # and supplies phase without a clock; the resulting pulse is reflection-odd.
    released_tail_request = tanh(
        sin(bounded_course_error) /
        max(abs(params.steering_softness), eps(Float64)),
    )
    signed_head_motion = tanh(
        qd1 /
        max(
            omega * amp * abs(params.tail_pulse_rate_scale),
            eps(Float64),
        ),
    )
    corrective_half_cycle_gate = max(
        released_tail_request * signed_head_motion,
        0.0,
    )
    released_tail_course_pulse = released_course_share * closing_alignment *
        corrective_half_cycle_gate *
        abs(params.maximum_released_tail_course_pulse) *
        released_tail_request
    ...
    phase_lag_target = 2 * terminal_mean_bend + terminal_tail_pulse +
        released_tail_course_pulse - q1 -
        params.tail_lag_gain * qd1 / max(omega, eps(omega))
    carrier_a2 = omega^2 * (phase_lag_target - q2) -
        2 * params.tail_damping * omega * qd2

    # Retain a minimum target-turn share in the evidenced shared half-cycle
    # actuator. Released authority is handled only by the small posterior pulse
    # above, leaving both propulsive carrier half-cycles continuously active.
    redirect_gate = tanh(
