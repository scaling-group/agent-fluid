# Unified predicted-miss handoff with a response-gated posterior residual.
# Large closing misses recruit tail-only rhythmic authority only until the
# carrier-separated body response becomes corrective.
...
function target_policy(state, params)
    ...
    # A transient correct-sign yaw is not enough to complete the shared
    # redirect. Release both base rhythmic steering channels only as the
    # remaining predicted miss also becomes small.
    ...
    # The sampled ungated posterior residual accelerated capture, but remained
    # active into a high-load, high-tail-angle terminal response. Preserve its
    # large-miss rhythmic authority only while carrier-separated yaw has not
    # responded in the requested direction. This is a state-triggered burst
    # release, not another amplitude threshold or persistent static bend.
    response_deficit_gate = 1 - corrective_response_gate
    posterior_miss_residual = terminal_gate * closing_alignment *
        response_deficit_gate *
