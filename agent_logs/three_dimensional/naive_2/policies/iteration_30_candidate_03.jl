# Unified predicted-miss handoff with posterior relative-flow rejection.
# Released target-turn pulse authority opposes target-normal relative flow
# while the state-feedback traveling wave remains continuously propulsive.
...
function target_policy_params()
    return (
        ...
        terminal_relative_flow_scale=0.5,
...
function target_policy(state, params)
    ...
    relative_flow_x = isfinite(state.relative_flow_velocity_body_U[1]) ?
        Float64(state.relative_flow_velocity_body_U[1]) : 0.0
    relative_flow_y = isfinite(state.relative_flow_velocity_body_U[2]) ?
    ...
    # Relative flow is measured in the same normalized body frame as the
    # target. Its target-normal projection distinguishes residual lateral
    # translation from body yaw without relying on a world route. Use it only
    # for the pulse authority that the response-plus-miss consensus releases.
    target_normal_relative_flow = target_norm > eps(Float64) ?
        (
            target_x * relative_flow_y -
            target_y * relative_flow_x
        ) / target_norm : 0.0
    relative_flow_scale = max(
        abs(params.terminal_relative_flow_scale),
        eps(Float64),
    )
    relative_flow_request = -tanh(
        target_normal_relative_flow / relative_flow_scale,
    )

    # During the force-producing mid-stroke, transfer the released share of
    # the target-turn pulse to relative-flow rejection. The convex request
    # keeps the original pulse envelope, and joint speed supplies phase without
    # a clock or mutable oscillator state.
    ...
    terminal_pulse_request = clamp(
        unmet_release_gate * terminal_turn_request +
        release_gate * relative_flow_request,
        -1.0,
        1.0,
    )
    terminal_tail_pulse = closing_alignment * terminal_gate * midstroke_gate *
        abs(params.maximum_terminal_tail_pulse) * terminal_pulse_request
    ...
    # Retain course-residual half-cycle steering while both propulsive carrier
    # half-cycles remain active.
    redirect_gate = tanh(
