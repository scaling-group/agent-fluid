# Predicted-miss curvature with corridor-consensus redirect release.
# Both rhythmic redirects remain active until measured yaw is corrective, the
# predicted course is centered, and lateral target geometry is capture-ready.
...
function target_policy(state, params)
    ...
    # A transient correct-sign yaw and centered instantaneous course are not
    # enough to complete the redirect if that course remains laterally offset.
    # Release both rhythmic channels only inside the body-frame capture
    # corridor, retaining a single geometric consensus handoff.
    ...
    corridor_ready_gate = 1 - outside_corridor_gate
