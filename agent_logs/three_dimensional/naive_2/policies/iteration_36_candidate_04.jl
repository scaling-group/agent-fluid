# Unified predicted-miss handoff with phase-decomposed reserve allocation.
# Carrier-suppressing redirect is retained; only carrier-amplifying authority
# is admitted through directional angle/rate reserve.
...
function target_policy(state, params)
    ...
    # Build the unsteered instantaneous carrier before allocating the cubic
    # redirect. This is the current joint-state phase, not a one-step delayed
    # action that already contains steering. The opposed traveling component
    # and posterior lag remain intact around the slow mean bend.
    ...
    # Preserve the sampled cubic predicted-miss response while distinguishing
    # what it does on each carrier half-cycle. A redirect opposed to the
    # carrier suppresses acceleration and is retained. An aligned redirect
    # amplifies that carrier phase and therefore passes only through signed
    # angle/rate reserve. Excluded tail share reaches the head under the same
    # rule; authority unavailable to both joints is shed back to cruise.
    redirect_residual = terminal_gate * closing_alignment *
        abs(params.maximum_terminal_redirect_residual) * prediction_request^3
    action_scale = max(abs(params.carrier_action_scale), eps(Float64))
    redirect_direction = sign(prediction_request)
    head_carrier_phase = tanh(carrier_a1 / action_scale)
    tail_carrier_phase = tanh(carrier_a2 / action_scale)
    head_amplifying_share = 0.5 * (
        1 + redirect_direction * head_carrier_phase
    )
    tail_amplifying_share = 0.5 * (
        1 + redirect_direction * tail_carrier_phase
    )
    angle_width = max(abs(params.joint_angle_width), eps(Float64))
    rate_width = max(abs(params.joint_rate_width), eps(Float64))
    head_directional_angle_capacity = 0.5 * (
        1 - tanh(
            (
                redirect_direction * q1 -
                abs(params.joint_soft_angle)
            ) / angle_width,
        )
    )
    head_directional_rate_capacity = 0.5 * (
        1 - tanh(
            (
                redirect_direction * qd1 -
                abs(params.joint_soft_rate)
            ) / rate_width,
        )
    )
    tail_directional_angle_capacity = 0.5 * (
        1 - tanh(
            (
                redirect_direction * q2 -
                abs(params.joint_soft_angle)
            ) / angle_width,
        )
    )
    tail_directional_rate_capacity = 0.5 * (
        1 - tanh(
            (
                redirect_direction * qd2 -
                abs(params.joint_soft_rate)
            ) / rate_width,
        )
    )
    head_directional_capacity = head_directional_angle_capacity *
        head_directional_rate_capacity
    tail_directional_capacity = tail_directional_angle_capacity *
        tail_directional_rate_capacity
    head_capacity = 1 - head_amplifying_share *
        (1 - head_directional_capacity)
    tail_capacity = 1 - tail_amplifying_share *
        (1 - tail_directional_capacity)
    tail_redirect_residual = tail_capacity * redirect_residual
    head_redirect_residual = params.head_asymmetry_share * head_capacity *
        (1 - tail_capacity) * redirect_residual
