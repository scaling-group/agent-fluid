# Unified predicted-miss handoff with directional phase-plane allocation.
# Large closing misses recruit tail-first rhythmic authority; outward joint
# rate consumes projected angle reserve before redirect spillover is assigned.
...
function target_policy(state, params)
    ...
    # Preserve the centered shared redirect while giving large predicted
    # misses a different rhythmic actuator. Cubing the normalized miss makes
    # this residual vanish rapidly on centered courses. Predict each joint's
    # directional excursion by adding only outward rate, normalized by the
    # carrier frequency, to signed angle. This phase-plane gate anticipates an
    # outward soft-boundary crossing without suppressing inward motion.
    ...
    carrier_rate_scale = max(abs(omega), eps(Float64))
    head_projected_excursion = redirect_direction * q1 +
        max(redirect_direction * qd1, 0.0) / carrier_rate_scale
    tail_projected_excursion = redirect_direction * q2 +
        max(redirect_direction * qd2, 0.0) / carrier_rate_scale
    head_angle_reserve = 0.5 * (
        1 - tanh(
            (head_projected_excursion - abs(params.joint_soft_angle)) /
            angle_width,
        )
    )
    head_rate_reserve = 0.5 * (
        1 - tanh(
            (redirect_direction * qd1 - abs(params.joint_soft_rate)) /
            rate_width,
        )
    )
    tail_angle_reserve = 0.5 * (
        1 - tanh(
            (tail_projected_excursion - abs(params.joint_soft_angle)) /
            angle_width,
        )
    )
    tail_rate_reserve = 0.5 * (
        1 - tanh(
            (redirect_direction * qd2 - abs(params.joint_soft_rate)) /
            rate_width,
        )
    )
    head_reserve = head_angle_reserve * head_rate_reserve
    tail_reserve = tail_angle_reserve * tail_rate_reserve
    tail_redirect_residual = tail_reserve * redirect_residual
    head_redirect_residual = params.head_asymmetry_share * head_reserve *
        (1 - tail_reserve) * redirect_residual
