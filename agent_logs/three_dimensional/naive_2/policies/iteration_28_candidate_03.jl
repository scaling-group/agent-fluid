# Unified predicted-miss handoff with terminal duty-skew steering.
# Course error changes opposing bend residence times while the state-feedback
...
function target_policy_params()
    return (
        ...
        maximum_terminal_duty_skew=0.20,
...
function target_policy(state, params)
    ...
    # Near a reliable terminal intercept, course error lengthens the requested
    # bend half-cycle and shortens its opposite by smoothly skewing restoring
    # stiffness. Joint state supplies phase; no clock or route is introduced.
    q1_centered = q1 - terminal_mean_bend
    centered_phase = tanh(
        q1_centered / max(abs(amp), eps(Float64)),
    )
    terminal_course_request = terminal_gate * course_weight *
        sin(bounded_course_error)
    maximum_duty_skew = clamp(
        abs(params.maximum_terminal_duty_skew),
        0.0,
        0.95,
    )
    restoring_stiffness_scale = 1 - maximum_duty_skew *
        terminal_course_request * centered_phase
    vdp_drive = params.oscillator_mu *
        (1 - (q1_centered / amp)^2) * qd1
    carrier_a1 = vdp_drive - omega^2 * restoring_stiffness_scale *
        q1_centered
