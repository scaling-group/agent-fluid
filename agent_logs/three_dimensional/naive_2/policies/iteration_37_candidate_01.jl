# Unified predicted-miss handoff with posterior-lag modulation.
# Unresolved closing misses reshape the traveling wave without adding a
# one-sided posterior acceleration residual or changing far-field propulsion.
...
function target_policy_params()
    return (
        ...
        maximum_terminal_lag_increment=0.2,
...
function target_policy(state, params)
    ...
    # Express unresolved miss authority through posterior wave timing rather
    # than a target-signed acceleration bias. The magnitude-only modulation is
    # reflection even; steering direction remains in the shared half-cycle
    # channel. It vanishes on centered courses and after the response-plus-miss
    # consensus releases the redirect back to the base traveling carrier.
    terminal_lag_increment = terminal_gate * closing_alignment *
        unmet_release_gate * abs(params.maximum_terminal_lag_increment) *
        abs(prediction_request)^3
    ...
    phase_lag_target = 2 * terminal_mean_bend + terminal_tail_pulse - q1 -
        (params.tail_lag_gain + terminal_lag_increment) * qd1 /
        max(omega, eps(omega))
