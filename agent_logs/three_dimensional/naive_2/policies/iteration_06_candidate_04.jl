# State-feedback traveling bend with course-aware approach redirects.
# Joint state carries propulsive phase; full body-frame target geometry and
# measured course alignment gate near-target drive-to-steering reallocation.
...
function target_policy_params()
    return (
        ...
        minimum_course_speed=0.25,
        approach_course_scale=0.5,
...
function target_policy(state, params)
    ...
    # Body yaw can momentarily align the pursuit angle even while inertia still
    # carries the fish across the target line. The normalized cross product of
    # target and velocity is rotation invariant and changes sign under lateral
    # reflection; its magnitude therefore measures course misalignment without
    # introducing a world-frame route. Suppress it at low speed, where course
    # direction is not informative.
    target_norm = hypot(target_x, target_y)
    course_speed = hypot(longitudinal_velocity, lateral_velocity)
    course_crossflow = if course_speed > params.minimum_course_speed &&
            target_norm > eps(Float64)
        clamp(
            (target_x * lateral_velocity -
                target_y * longitudinal_velocity) /
            (target_norm * course_speed),
            -1.0,
            1.0,
        )
    else
    ...
    # Near a target, retain drive relief until both the body and its actual
    # velocity course align. The smooth union changes no far-field behavior,
    # preserves signed half-cycle steering, and avoids a persistent curvature
    # equilibrium or general rate brake.
    ...
    course_misalignment = tanh(
        abs(course_crossflow) /
        max(params.approach_course_scale, eps(Float64)),
    )
    combined_misalignment = 1 -
        (1 - approach_misalignment) * (1 - course_misalignment)
    approach_redirect = approach_proximity * combined_misalignment
    carrier_scale = 1 -
