# Course-error steering with terminal line-of-sight-rate damping.
# Rotation-invariant target/velocity feedback steers the evidenced traveling
# bend, while a bounded late residual damps rapid target-line sweep.
...
function target_policy_params()
    return (
        ...
        terminal_los_distance=3.0,
        terminal_los_width=0.5,
        los_rate_radius_floor=0.75,
        los_rate_scale=0.5,
        maximum_los_correction=0.55,
...
function target_policy(state, params)
    ...
    # only on their dot and cross products, so carrier yaw cannot corrupt it.
    # At low speed there is no reliable course and the blend falls back
    # continuously to target bearing.
    ...
    base_navigation_error = (1 - course_weight) * bounded_bearing +
        course_weight * bounded_course_turn

    # For a stationary target, v x r / |r|^2 is its inertial line-of-sight
    # angular rate. Computing it from two body-frame vectors preserves that
    # rotation invariant. Recruit it only in the terminal region, where it
    # opposes the inherited controller's rapidly growing late course request.
    los_radius = max(distance, params.los_rate_radius_floor)
    los_rate = course_cross / max(los_radius^2, eps(Float64))
    terminal_width = max(params.terminal_los_width, eps(Float64))
    terminal_proximity = 0.5 * (
        1 - tanh((distance - params.terminal_los_distance) / terminal_width)
    )
    los_correction = params.maximum_los_correction * terminal_proximity *
        tanh(los_rate / max(params.los_rate_scale, eps(Float64)))
    navigation_error = base_navigation_error + los_correction
    ...
    turn_signal = navigation_error +
        params.yaw_residual_gain * yaw_residual
    turn_request = tanh(
        turn_signal / max(params.steering_softness, eps(Float64)),
    )

    # Preserve response-gated stronger redirection, but evaluate it against
    # phase-robust navigation error. Course misalignment adds authority when
    # the measured trajectory, rather than body pose alone, misses the target.
    wrong_side_slip = max(
    ...
    # If the measured course still misses a nearby target, scale the complete
    # rhythmic command. This preserves carrier-to-steering authority instead
    # of allowing one-sided steering to dominate a relieved restorative beat.
