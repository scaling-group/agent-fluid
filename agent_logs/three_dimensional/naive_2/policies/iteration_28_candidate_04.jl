# Unified response-and-miss steering with centered-approach joint dissipation.
# Direct virtual damping removes beat energy only during a reliably closing,
# centered intercept; off-course motion retains the full traveling carrier.
...
function target_policy_params()
    return (
        ...
        maximum_approach_damping_ratio=0.12,
...
function target_policy(state, params)
    ...
    # A dissipative gait transition is earned only by a reliably closing,
    # centered near-field intercept. Normalized closing speed prevents damping
    # weak or receding motion; an opening miss restores full drive smoothly.
    ...
    approach_damping_gate = distance_gate * course_weight *
    ...
    # Unlike the ineffective nonlinear amplitude envelope, this virtual
    # damping directly opposes observed joint motion. It is continuous,
    # reflection-equivariant, and cannot become a static steering bias.
    damping_ratio = max(abs(params.maximum_approach_damping_ratio), 0.0)
    damping_gain = 2 * damping_ratio * omega * approach_damping_gate
    raw_a1 = carrier_a1 +
