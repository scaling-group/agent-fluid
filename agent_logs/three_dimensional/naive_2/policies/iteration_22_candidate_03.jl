# Head-centered collision-course interception with a response-released redirect.
# Broad course prediction preserves the sampled traveling-bend route; terminal
# steering opposes head-target line-of-sight rotation while closure remains.
...
function target_policy_params()
    return (
        ...
        collision_course_distance=4.0,
        collision_course_width=0.5,
        collision_closing_speed_scale=0.25,
        collision_los_distance_floor=0.75,
        maximum_collision_los_rate=2.0,
        collision_los_rate_softness=0.3,
...
function target_policy(state, params)
    ...
    target_rate_x = isfinite(state.target_body_rate_L[1]) ?
        Float64(state.target_body_rate_L[1]) : 0.0
    target_rate_y = isfinite(state.target_body_rate_L[2]) ?
        Float64(state.target_body_rate_L[2]) : 0.0
    ...
    closing_speed = isfinite(state.closing_speed_L) ?
        max(Float64(state.closing_speed_L), 0.0) : 0.0

    # Body forward is -x. Target and center velocity share the body frame, so
    # their angular mismatch supplies broad pursuit and course feedback without
    # a world-frame route. Exact aft-centerline geometry stays unbiased.
    ...
    navigation_request = tanh(
        navigation_error / max(abs(params.steering_softness), eps(Float64)),
    )

    # Preserve the sampled constant-course predictor as the middle-distance
    # intercept. Positive projection means the target is still ahead along the
    # measured course; cross/speed is its signed closest-approach miss.
    ...
    # The capture condition is a head crossing, whereas the broad predictor
    # uses center velocity. The normalized derivative of the head-target body
    # vector already contains frame rotation; adding measured heading rate
    # cancels it and yields inertial line-of-sight rotation. Under positive
    # closure, opposing that rotation drives a constant-bearing collision
    # course. Smooth distance and closure gates keep this channel terminal.
    los_distance_floor = max(
        abs(params.collision_los_distance_floor),
        eps(Float64),
    )
    los_denominator = max(target_norm^2, los_distance_floor^2)
    body_los_rate = (
        target_x * target_rate_y - target_y * target_rate_x
    ) / los_denominator
    inertial_los_rate = body_los_rate + heading_rate
    maximum_los_rate = max(
        abs(params.maximum_collision_los_rate),
        eps(Float64),
    )
    bounded_los_rate = clamp(
        inertial_los_rate,
        -maximum_los_rate,
        maximum_los_rate,
    )
    collision_request = tanh(
        -bounded_los_rate /
        max(abs(params.collision_los_rate_softness), eps(Float64)),
    )
    collision_width = max(
        abs(params.collision_course_width),
        eps(Float64),
    )
    collision_distance_gate = 0.5 * (
        1 - tanh(
            (distance - params.collision_course_distance) / collision_width,
        )
    )
    closing_scale = max(
        abs(params.collision_closing_speed_scale),
        eps(Float64),
    )
    collision_closure_gate = closing_speed^2 /
        (closing_speed^2 + closing_scale^2)
    collision_gate = collision_distance_gate * collision_closure_gate

    # Add the collision-course request as a bounded residual rather than
    # replacing proven navigation. The residual uses only available command
    # headroom, so agreeing signals strengthen an early redirect while an
    # established constant-bearing intercept leaves the inherited route intact.
    collision_residual = collision_gate * collision_request
    steering_request = clamp(
        navigation_request +
        collision_residual * (1 - abs(navigation_request)),
        -1.0,
        1.0,
    )
    ...
    legacy_request_weight = distance_gate + prediction_gate
    legacy_terminal_request = legacy_request_weight > eps(Float64) ? clamp(
        (
            distance_gate * terminal_pursuit_request +
            prediction_gate * prediction_request
        ) / legacy_request_weight,
        -1.0,
        1.0,
    ) : 0.0
    terminal_turn_request = clamp(
        legacy_terminal_request +
        collision_residual * (1 - abs(legacy_terminal_request)),
        -1.0,
        1.0,
    )
    collision_authority = abs(collision_residual)
    terminal_gate = 1 - (1 - distance_gate) * (1 - prediction_gate) *
        (1 - collision_authority)
    ...
    # Remove joint-phase-correlated yaw before deciding that a directional
    # response is corrective. A measured response releases redundant rhythmic
    # steering but never brakes the propulsive carrier.
    ...
    course_share = 1 - terminal_gate * corrective_response_gate *
        (1 - minimum_course_share)

    # Apply a small posterior wave-shape pulse only during an observed active
    # stroke, a still-closing intercept, and unmet directional response.
    ...
    unmet_response_gate = 1 - corrective_response_gate
    terminal_tail_pulse = closing_alignment * terminal_gate *
        midstroke_gate * unmet_response_gate *
        abs(params.maximum_terminal_tail_pulse) * terminal_turn_request

    # Center the oscillator and posterior target on the slow mean bend. The
    # opposed traveling component and posterior lag remain intact around it.
    ...
    # Retain both propulsive half-cycles. Only the steering request changes
    # from body-center course to head-centered collision course near capture.
    redirect_gate = tanh(
    ...
    asymmetry = course_share * asymmetry_limit * steering_request
