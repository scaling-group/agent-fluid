# Unified predicted-miss handoff with dual-reserve redirect allocation.
# A bounded terminal residual remains rhythmic and target-relative while its
# two-joint distribution responds continuously to both actuators' reserve.
...
function target_policy_params()
    return (
        ...
        anterior_soft_angle=38.0 * pi / 180,
        anterior_angle_width=3.0 * pi / 180,
        anterior_soft_rate=235.0 * pi / 180,
        anterior_rate_width=20.0 * pi / 180,
...
function target_policy(state, params)
    ...
    # observed joint reserve rather than always burdening the tail. The
    # residual remains tail-dominant away from the posterior soft boundaries;
    # as either boundary is approached its complement moves smoothly to the
    # anterior rhythmic channel only while that joint also has reserve. If
    # neither actuator can accept the extra redirect, the residual fades while
    # the carrier and shared steering remain intact. Absolute joint state makes
    # the allocation reflection-neutral while the residual supplies direction.
    ...
    anterior_angle_reserve = 0.5 * (
        1 - tanh(
            (abs(q1) - abs(params.anterior_soft_angle)) /
            max(abs(params.anterior_angle_width), eps(Float64)),
        )
    )
    anterior_rate_reserve = 0.5 * (
        1 - tanh(
            (abs(qd1) - abs(params.anterior_soft_rate)) /
            max(abs(params.anterior_rate_width), eps(Float64)),
        )
    )
    ...
    head_redirect_residual = params.head_asymmetry_share * anterior_reserve *
