# State-feedback traveling bend with reverse-half-cycle reserve.
# Full-circle target geometry recruits a near redirect while a structural
# asymmetry cap preserves restoring acceleration on both propulsive half-cycles.
...
function target_policy_params()
    return (
        ...
        minimum_reverse_half_cycle_fraction=0.25,
...
function target_policy(state, params)
    ...
    # Near a badly misaligned target, trade symmetric drive for steering.
    ...
    # A signed abs(carrier) steering term can erase the opposed half-cycle
    # whenever its joint share times asymmetry exceeds the live carrier scale.
    # Cap that ratio directly, leaving a parameter-owned restoring fraction on
    # both joints. Unlike bend-threshold release, this has no beat-state switch.
    requested_asymmetry = asymmetry_limit * turn_request
    reverse_fraction = clamp(
        params.minimum_reverse_half_cycle_fraction,
        0.0,
        1.0,
    )
    maximum_steering_share = max(
        abs(params.head_asymmetry_share),
        abs(params.tail_asymmetry_share),
        eps(Float64),
    )
    asymmetry_capacity = carrier_scale *
        (1 - reverse_fraction) / maximum_steering_share
    asymmetry = clamp(
        requested_asymmetry,
        -asymmetry_capacity,
        asymmetry_capacity,
