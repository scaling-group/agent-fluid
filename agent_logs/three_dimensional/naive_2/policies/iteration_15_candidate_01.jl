# Phase-separated terminal response on a course-residual traveling bend.
# Body-frame interception geometry preserves the evidenced approach; measured
# yaw recruits extra terminal curvature only after carrier phase is removed.
...
function target_policy_params()
    return (
        ...
        maximum_terminal_yaw_residual=0.5,
        terminal_yaw_residual_gain=0.6,
...
function target_policy(state, params)
    ...
    # course residual rejects beat-scale body yaw without a world-frame route.
    ...
    # Predict constant-course closest approach. Positive course projection
    # means the target remains ahead; cross/speed is signed transverse miss.
    terminal_width = max(abs(params.terminal_distance_width), eps(Float64))
    distance_gate = 0.5 * (
        1 - tanh((distance - params.terminal_distance) / terminal_width)
    )
    prediction_floor = max(abs(params.prediction_speed_floor), eps(Float64))
    safe_speed_squared = max(speed^2, prediction_floor^2)
    time_to_closest = max(course_dot / safe_speed_squared, 0.0)
    time_gate = 0.5 * (
        1 - tanh(
    ...
    # A two-joint rate model removes carrier-correlated yaw. Applied only in
    # the terminal handoff, the residual releases a correct response and
    # reinforces the requested bend when yaw persists to the wrong side.
    carrier_phase_yaw = -params.carrier_yaw_head_rate_gain * qd1 -
        params.carrier_yaw_tail_rate_gain * qd2
    maximum_yaw_residual = max(
        abs(params.maximum_terminal_yaw_residual),
        eps(Float64),
    )
    yaw_residual = clamp(
        heading_rate - carrier_phase_yaw,
        -maximum_yaw_residual,
        maximum_yaw_residual,
    )
    response_corrected_request = clamp(
        terminal_turn_request +
        params.terminal_yaw_residual_gain * yaw_residual,
        -1.0,
        1.0,
    )
    terminal_mean_bend = terminal_gate *
        abs(params.maximum_terminal_mean_bend) * response_corrected_request
    ...
    # Center the traveling bend on the terminal mean curvature while retaining
    # the evidenced posterior lag and both propulsive half-cycles.
