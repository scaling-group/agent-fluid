# Unified predicted-miss handoff with carrier-separated course geometry.
# Raw velocity retains far-field navigation; joint-rate carrier crossflow is
# removed smoothly only from the terminal redirect-to-cruise decision.
...
function target_policy_params()
    return (
        ...
        carrier_lateral_head_rate_gain=0.15,
        carrier_lateral_tail_rate_gain=0.105,
        maximum_carrier_lateral_velocity=1.2,
...
function target_policy(state, params)
    ...
    # Preserve the inherited 3L proximity handoff as a fallback. Joint-rate
    # phase explains most sampled beat-scale lateral speed, so smoothly remove
    # its bounded estimate inside this handoff before predicting closest
    # approach. Raw velocity above still owns far-field course navigation.
    terminal_width = max(abs(params.terminal_distance_width), eps(Float64))
    distance_gate = 0.5 * (
        1 - tanh((distance - params.terminal_distance) / terminal_width)
    )
    carrier_lateral_limit = max(
        abs(params.maximum_carrier_lateral_velocity),
        eps(Float64),
    )
    raw_carrier_lateral_velocity =
        -params.carrier_lateral_head_rate_gain * qd1 +
        params.carrier_lateral_tail_rate_gain * qd2
    carrier_lateral_velocity = clamp(
        raw_carrier_lateral_velocity,
        -carrier_lateral_limit,
        carrier_lateral_limit,
    )
    prediction_velocity_y = velocity_y -
        distance_gate * carrier_lateral_velocity
    ...
    prediction_course_dot =
        target_x * velocity_x + target_y * prediction_velocity_y
    prediction_course_scale = target_norm * prediction_speed

    # A positive projection means the target is still ahead along the
    # carrier-separated course; cross/speed is its signed perpendicular miss.
    prediction_floor = max(
        abs(params.prediction_speed_floor),
        eps(Float64),
    )
    safe_speed_squared = max(prediction_speed^2, prediction_floor^2)
    raw_time_to_closest = prediction_course_dot / safe_speed_squared
    ...
    closing_alignment = prediction_course_scale > eps(Float64) ?
        clamp(
            prediction_course_dot / prediction_course_scale,
            0.0,
            1.0,
        ) : 0.0
