...
- On the exactly reproduced `137.357`-unit scaffold, do not treat temporary
  route improvement, proximity, or spare posterior headroom as permission to
  remove established feedback or amplify the whole tail wave. Gating the
  direct moment residual only when absolute bearing is growing still captures
  but regresses to `149.490` units, `4.428L` mean distance, `97418` energy,
  and `16.38/318.46` RMS force/moment. More severely, reducing bearing-owned
  turn during a closing near approach gets within `1.796L` before folding
  away and exiting the top boundary at `243.447` units with `161191` energy;
  the full route command returning after closure is lost does not undo the
  wake-sensitive trajectory bifurcation. A same-sign posterior half-cycle
  multiplier also creates broad loops and delays capture to `279.439` units,
  with `6.982L` mean distance and `182836` energy. Preserve continuous target
  bearing, progress qualification, direct moment feedback, and the base
  posterior lag on this topology. If posterior coordination is tested, change
  the relative curvature distribution rather than amplifying anterior and
  oppositely phased posterior targets together; falsify it unless it retains
  the alternating wave and baseline capture while improving arrival/distance
  and effort/load together under the same wake snapshot.
