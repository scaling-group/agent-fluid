...
- Direction-response refinements on the fixed `0.72`-period half-cycle and
  ...
  `31.42 rad/time^2` limit). Capture survival alone is not enough to promote a
  route/load refinement: the assigned parent's other completed step-7 sibling
  still reaches the target, but at `149.85` with `4.423L` mean distance,
  `671.58` mean command energy, and `15.58/312.62` RMS force/moment, all worse
  than the progress-qualified sample's `137.36`, `4.184L`, `656.89`, and
  `14.75/303.02`. Inherited mechanism records associate this regression class
  with extra heading-rate damping or gating the moment residual by bearing.
  Avoid those couplings merely to lower a scalar load; test a separate bounded
  route observation while retaining closing-progress qualification and direct
  moment rejection, and require arrival, distance, loads, and acceleration
  margin to improve together.
