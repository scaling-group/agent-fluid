...
- On the `137.357`-unit progress-qualified scaffold, neither another bounded
  route observation nor unqualified steering recruitment at the posterior
  actuator is supported. An oppositely signed relative-crossflow residual
  delays capture to `154.110` units, raises mean distance from `4.184L` to
  ...
  (`0.12955` to `0.13005`); a lateral-target residual similarly reaches at
  `159.302` with `4.722L` mean distance. Bearing-dependent posterior-lag relief
  reaches at `158.147`, and applying a small same-sign requested-half-cycle
  multiplier to the posterior wave is much worse: keyframes show broad loops,
  arrival nearly doubles to `279.439`, mean distance rises to `6.982L`, total
  energy to `182836`, mean upstream body speed halves from `-0.0791` to
  `-0.0389`, and RMS crossflow/moment rise to `0.1355/316.29`. Joint-2 angle
  and acceleration remain below their caps, so spare headroom and eventual
  capture do not validate posterior steering authority. Preserve the posterior
  lag as propulsion on this scaffold; avoid more unconditioned additive flow/
  route residuals or route-synchronous tail scaling until a distinct adverse
  response signature is evidenced. Prefer a bounded regime- or response-
  conditioned schedule of existing anterior route authority, and falsify this
  boundary only if a one-change alternative preserves the `137.357` topology
  while improving distance and load/effort together under the same snapshot.
- Two newer response schedules tighten that boundary. Relieving the existing
  route request by up to `30%` during a close, positively-closing approach
  changes the replicated `137.357` capture into a `243.447`-unit top-boundary
  exit: after reaching only `1.796L`, keyframes show repeated right-side
  switchbacks, mean/final distance rise to `7.938L/7.185L`, upstream
  displacement falls to `-7.710L`, and energy rises to `161191`. Separately,
  gating the direct yaw-moment residual off whenever absolute bearing is not
  growing still captures, but at `149.490` units with worse mean distance and
  energy (`4.428L/97418`) and higher RMS force/moment (`16.38/318.46` versus
  `14.75/303.02`). Therefore positive closure before first crossing does not
  justify terminal steering relief, and a coarse route-response signature
  cannot replace the small continuously available moment loop. Preserve both
  loops on this tight-capture scaffold. If response scheduling is tested next,
  prefer bounded recovery only when measured distance is increasing, and
  falsify it on any loss or delay of capture, route-topology change, extra cap
  contact, or increase in distance, effort, or loads.
