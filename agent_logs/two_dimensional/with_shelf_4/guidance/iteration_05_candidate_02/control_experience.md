...
- Normalized yaw-moment rejection now has a narrow positive operating point and
  a sharp negative boundary.  Adding a small direct `moment_z_L2` residual to
  the otherwise identical `0.72`-period half-cycle controller improves target
  arrival from `179.22` to `149.57` units, mean distance from `5.04L` to
  `4.38L`, and RMS moment from `338.91` to `314.99`, while retaining about
  `-10.92L` upstream travel; mean command energy rises from `653.20` to
  `681.91`.  In contrast, the inherited variant that simultaneously slows the
  gait to `0.76`, raises moment gain, and applies the residual only through
  `1 - abs(route_turn)` loops above the wake and exits the top boundary after
  `126.43` units with only `-1.12L` upstream travel, despite a similar
  `317.27` RMS moment and actuator-feasible joint maxima.  Thus a moderate load
  scalar does not validate navigation, and route-headroom gating must not be
  presumed to separate route and disturbance authority: it can concentrate
  load-driven steering precisely when bearing is small. Preserve the verified
  small direct residual as the baseline and test later damping, gating, gain,
  or period changes one mechanism at a time. This lesson is falsified only by
  a controlled ablation that preserves capture and upstream translation while
  improving arrival/load behavior; the bundled failed rollout cannot identify
  which of its three changes caused the exit.
- A controlled follow-on isolates windowed target-bearing-rate damping from
  that verified direct-residual baseline. Three repeated baseline samples
  reach in `149.572` units with `4.384L` mean distance; adding only the bounded
  rate term retains the same broad targetward keyframe route and capture at
  `149.605`, while mean distance falls to `4.358L`, RMS relative crossflow to
  `0.1321` from `0.1362`, RMS force/moment to `15.49/308.48` from
  `16.22/314.99`, total command energy to `96933` from `101995`, and mean power
  proxy to `46.94` from `49.63`. The late trajectory is also visibly less
  jagged. Thus, after half-cycle steering has already established upstream
  target motion, the observed convergence rate of normalized body-frame
  bearing is a supported slow-loop damping signal; prefer it over reopening
  gait or moment-residual gains. The improvement is modest and has one distinct
  evaluated realization, so preserve its far-field gain and test any stronger
  or distance-conditioned damping as a separate mechanism. Falsify the lesson
  if replication loses capture/upstream translation, materially delays
  arrival, or fails to improve path/load behavior under changed wake phase.
