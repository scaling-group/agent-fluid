...
- Treat previous-action response as a phase-sensitive actuator mechanism, not
  generic low-pass smoothing. An anterior-only response captures in `135.019`
  units with `3.685L` mean distance but raises energy and RMS crossflow/force/
  moment to `110448` and `0.1535/18.53/362.61`; an unguarded two-joint response
  reaches in `130.729` but raises them further to `115750` and
  `0.1543/18.30/359.97`. Guarding each two-joint history term by its normalized
  signed added power instead reaches in `123.018` with `3.594L` mean distance,
  `95084` energy, and `0.1429/17.03/334.45` RMS crossflow/force/moment. Four
  semantically identical sampled materializations reproduce those metrics
  exactly, supporting a deterministic phase-safe continuity effect under the
  shared snapshot rather than a cosmetic policy difference. Preserve the raw
  posterior-lagged wave and bypass history when it keeps adding kinetic energy
  after current state feedback requests less drive. The guarded result still
  exceeds the older `90228` energy and `0.1296/14.75/303.02` load scaffold, so
  regard it as a speed/distance improvement with partial load mitigation, not
  proof that action smoothing reduces absolute effort. This lesson is limited
  to the present gait and wake snapshot; falsify it if a one-change actuator
  coordination loses capture or alternating propulsion, or if held-out wakes
  do not preserve its joint improvement over the unguarded response.
- Couple the accept/reject decision for previous-action history across the
  traveling-wave pair. Replacing the two per-joint positive-power gates with
  one gate on total normalized added power improves the completed `123.018`-
  unit result to `118.024`, lowers mean distance/energy from `3.594L/95084`
  to `3.560L/89079`, and lowers RMS crossflow/force/moment from
  `0.1429/17.03/334.45` to `0.1383/16.74/329.67`, while the keyframes retain
  the alternating posterior-lagged bend and compact upstream capture path.
  This supports coherent joint coordination, not a scalar gait increase: the
  anterior acceleration still reaches its cap. A separate response-qualified
  large-bearing burst widens the midcourse fold and regresses capture to
  `130.053` with `3.758L/99377`, so body-frame bearing magnitude plus early
  response absence does not justify extra steering authority on this
  scaffold. Preserve the ordinary route loop and test further actuator-history
  mechanisms against the shared-gate result; falsify this boundary if a
  one-change mechanism improves capture topology, distance, effort, and load
  together without weakening upstream translation or posterior lag.
