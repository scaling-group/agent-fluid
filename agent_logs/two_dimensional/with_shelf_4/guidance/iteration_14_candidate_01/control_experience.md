...
- Once the alternating half-cycle scaffold reaches the target, qualify slow
  ...
  moment fall from `0.1321/15.49/308.48` to `0.1296/14.75/303.02`; three
  current sampled copies reproduce the qualified result exactly. Preserve
  instantaneous body-frame bearing as the route owner on this scaffold. A
  one-change circular average of its short history regresses to `149.853`
  units, `4.423L` mean distance, `100638` energy, and higher load; an
  unconditioned crossflow residual takes `154.110` units without reducing RMS
  crossflow; same-sign posterior half-cycle modulation creates large loops and
  delays capture to `279.439` with `182836` energy, while opposite posterior
  and tail-lag modulations still regress to `150.210` and `196.317`. These
  failures bound the positive result: when a rate-only parent lacks the
  progress gate, restore that isolated observation qualification before
  seeking novelty, and do not treat it as support for bearing smoothing,
  another wake residual, or posterior-wave modulation. Falsify the gate if
  capture, upstream translation, or the alternating bend is lost, or if
  held-out wakes do not reproduce its arrival, distance, effort, and load
  advantages.
