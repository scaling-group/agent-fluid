...
- Do not stack an unconditioned observation residual onto the turn request
  after the progress-qualified half-cycle scaffold is established. The
  assigned parent and three sampled copies reproduce capture at `137.357`
  units with `4.184L` mean distance, `90228` total energy, and
  `0.12955/14.75/303.02` RMS crossflow/force/moment. An inherited
  opposite-signed relative-crossflow addition still captures but regresses to
  `154.110` units and `4.687L`, raises energy to `99457` and force/moment to
  `15.85/310.83`, and does not reduce crossflow (`0.13005`); another inherited
  step-9 descendant reaches only after `158.147` units with `4.692L` mean
  distance, `103629` energy, and higher loads. Sampled optimizer evidence also
  records a bounded lateral-target residual delaying capture to `159.302`
  units and `4.722L`. Treat crossflow as neither automatically harmful nor a
  duplicate route signal. When the keyframes retain targetward propulsion but
  show a broad, kinked redirect, test how the existing route command is shared
  between anterior and posterior wave actuation before adding another sensed
  command; falsify this boundary only if a one-change additive residual
  preserves the `137.357` topology while jointly improving arrival, distance,
  effort, and loads under the same wake snapshot.
