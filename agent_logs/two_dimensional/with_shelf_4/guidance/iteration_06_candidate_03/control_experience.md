...
- On the otherwise identical `0.72`-period half-cycle controller with a small
  ...
  causal component.
- Do not gate the proven direct yaw-moment residual using the sign of bearing
  error times bearing rate alone.  On the otherwise identical rate-damped
  success, that one-mechanism gate still captures slightly sooner (`148.70`
  versus `149.61` units) and lowers mean command energy (`645.37` versus
  `647.93`) and RMS moment (`306.15` versus `308.48`), but its keyframes show a
  wider midcourse excursion, mean distance worsens from `4.358L` to `4.578L`,
  and score falls from `-2.408` to `-2.629`.  Bearing convergence mixes body
  rotation with target-relative translation, so it is not a reliable label
  for whether an instantaneous wake load is helpful. Preserve the direct
  residual when extending this scaffold, and test a separately bounded direct
  yaw-response signal one mechanism at a time; revise this boundary only if a
  replicated comparison improves the distance integral without losing capture
  or upstream translation.
