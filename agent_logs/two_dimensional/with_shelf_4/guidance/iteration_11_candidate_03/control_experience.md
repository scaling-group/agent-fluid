...
- Once zero-mean half-cycle routing works, a small normalized yaw-moment
  ...
  helpful or harmful. Positive targetward translation is not a valid relief
  gate either: the assigned parent's smooth half-relief of the same moment
  residual during positive windowed closing speed retains semantic capture
  but delays it from the reproduced `137.357` baseline to `152.526`, widens
  mean distance from `4.184L` to `4.658L`, raises effort from `90228` to
  `99467`, and raises RMS force/moment from `14.75/303.02` to
  `15.62/309.29`. Keep the small direct moment residual at full authority
  during both stalled and targetward motion on this scaffold. Retest response
  gating only after a changed-wake failure shows that load rejection is
  cancelling useful advection; falsify it if capture, upstream translation,
  route width, loads, and effort do not improve together.
