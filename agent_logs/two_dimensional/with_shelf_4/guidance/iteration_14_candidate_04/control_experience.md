...
- Two later one-change actuator-coordination attempts preserve semantic
  capture but decisively regress from that same scaffold. Composing the raw
  route drive and moment residual inside one smooth authority envelope delays
  arrival to `223.746` units, with `6.429L` mean distance, `144686` total
  energy, and `0.13399/16.79/324.37` RMS crossflow/force/moment. Adding
  route-driven posterior half-cycle asymmetry instead arrives at `196.317`,
  with `5.718L` mean distance, `131294` energy, and
  `0.13498/15.48/310.21` RMS crossflow/force/moment. Both retain roughly
  `-11L` upstream displacement, so neither smoother command saturation nor
  unused posterior acceleration headroom establishes useful steering
  coordination. Avoid folding the route and disturbance residual into a new
  shared saturation and avoid adding route bias to the posterior wave on this
  topology; preserve their evidenced roles unless a fast physical response
  signature can distinguish useful from adverse load. A third inherited
  response test supplies a sharper negative boundary: attenuating the direct
  moment residual when normalized yaw moment and heading rate indicate passive
  rotational damping still captures, but arrival regresses to `205.519`, mean
  distance to `6.209L`, energy to `134893`, and RMS crossflow/force/moment to
  `0.13320/16.53/317.07`; mean upstream speed also falls from `-0.0791` to
  `-0.0531`. Thus neither slow bearing divergence nor instantaneous
  moment-times-yaw-rate sign identifies permission to gate the evidenced
  direct moment residual. Preserve that residual while testing target-relative
  response elsewhere in the route loop, and require any future torque gate to
  preserve the `137.357` capture while improving distance and effort/load
  together, not merely retaining success. This boundary applies to attenuation
  of the current direct residual; it does not rule out a separately evidenced
  fast load mechanism under a different wake or actuator topology.
