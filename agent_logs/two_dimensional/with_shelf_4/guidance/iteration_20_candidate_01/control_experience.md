...
- Treat previous-action response as a phase/amplitude mechanism, not as an
  effort smoother. Relative to the replicated direct-action scaffold, an
  unguarded two-joint response advances capture from `137.357` to `130.729`
  and mean distance from `4.184L` to `3.754L`, but raises energy and RMS
  crossflow/force/moment from `90228` and `0.1296/14.75/303.02` to `115750`
  and `0.1543/18.30/359.97`. Three completed policies that guard each joint's
  lag response independently reproduce capture at `123.018` with `3.594L`
  mean distance, `95084` energy, and `0.1429/17.03/334.45` RMS loads. A
  one-change shared guard based on the two joints' summed signed lag power
  improves all of these to `118.024`, `3.560L`, `89079`, and
  `0.1383/16.74/329.67`; its keyframes retain the alternating wave and show a
  straighter middle approach. This supports treating action history as one
  coupled traveling-wave actuator: preserve a common response weight rather
  than independently shearing the posterior phase relation. It does not prove
  that cross-joint cancellation of positive and negative lag power is useful;
  a later one-change test may rectify joint-level injection before applying a
  common weight. Falsify that refinement if it loses the `118.024` capture
  topology, upstream translation, or alternating bend, or if distance,
  cap contact, effort, and force/moment fail to improve coherently. The shared
  result still has higher RMS loads than the older direct-action scaffold, so
  it is a route/coordination improvement rather than evidence that response
  smoothing minimizes absolute load.
