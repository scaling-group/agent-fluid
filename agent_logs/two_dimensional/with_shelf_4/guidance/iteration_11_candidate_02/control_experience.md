...
- Steering placement, rather than another gait scalar, changes the observed
  ...
  trades away thrust. Do not assume that the same asymmetry should be copied
  into the posterior target: inherited one-change posterior half-cycle shares
  of `0.10` and `0.12` retain capture but delay it to `172.095` and `279.439`
  units, with the former widening mean distance to `5.816L` and raising
  force/moment to `16.39/322.27`. Preserve the posterior traveling-wave owner
  unless a later rollout isolates phase-specific posterior failure; reject
  sharing that widens the redirect or fails to improve arrival and load
  together.
- Once zero-mean half-cycle routing works, a small normalized yaw-moment
  ...
  distance. The assigned parent's isolated positive-progress relief of half
  the moment residual also regresses the `137.357`-unit baseline to `152.526`
  units, widens mean distance from `4.184L` to `4.658L`, and raises energy and
  force/moment from `90228` and `14.75/303.02` to `99467` and
  `15.62/309.29`. Thus neither bearing convergence nor scalar radial closure
  certifies that instantaneous wake yaw is helpful; keep the full direct
  residual on this scaffold. Reconsider response gating only with a more
  specific adverse phase/signature, and falsify it if it loses capture or
