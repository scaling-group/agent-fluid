...
- Previous-action continuity is useful on this rhythmic controller only when
  the two joints are treated as one propulsive wave and raw state feedback can
  recover authority during the wrong energetic phase. Filtering both joints
  without qualification captures in `130.729` units but costs `115750`
  command energy and `0.15435/18.30/359.97` RMS crossflow/force/moment;
  filtering only the anterior joint takes `135.019` units with `110448`
  energy and even higher force/moment (`18.53/362.61`). Independent guards
  that withdraw each history term when it adds positive normalized joint power
  improve capture to `123.018`, mean distance to `3.594L`, energy to `95084`,
  and RMS crossflow/force/moment to `0.14287/17.03/334.45`. Replacing those
  guards with one total-power gate preserves the visible route and alternating
  bend while improving all of the same metrics together: `118.024` arrival,
  `3.560L` mean distance, `89079` energy, and
  `0.13833/16.74/329.67` RMS crossflow/force/moment. Therefore do not assume
  positive lag power at either joint is independently harmful: simultaneous
  energy-adding and energy-removing history can preserve a useful inter-joint
  phase relationship, so qualify response at the normalized pair level before
  attenuating its components. The inherited response-qualified C-start test
  confirms that the remaining initial fold is not by itself evidence for more
  steering authority; it deepens the lower fold and regresses to `130.053`
  units, `3.758L`, and `99377` energy. This lesson applies to state-observed
  acceleration-command history on the common wake snapshot, not arbitrary
  torque limiting or a universal power scale; the shared gate still touches
  the anterior acceleration cap. Falsify it if held-out wakes lose capture,
  upstream translation, or alternating propulsion, or if distance and
  effort/load improvements separate into a tradeoff.
