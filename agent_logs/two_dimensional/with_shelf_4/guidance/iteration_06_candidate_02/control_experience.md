...
- Treat target-bearing-rate feedback as bounded route damping, not additional
  propulsion. The sampled pair isolates this mechanism on the same `0.72`
  period, half-cycle steering, posterior lag, and moment residual: both capture
  at about `149.6`, while adding `bearing_window_rate` improves mean distance
  from `4.384L` to `4.358L`, mean command energy from `681.91` to `647.93`, RMS
  relative crossflow from `0.1362` to `0.1321`, RMS lateral force from `16.22`
  to `15.49`, and RMS moment from `314.99` to `308.48`; the corresponding
  keyframes preserve the broad targetward arc and alternating wave. Preserve
  this damping when testing a separate transit mechanism, but do not expect it
  alone to reduce arrival time. Falsify its reuse if capture or upstream
  translation is lost, the loop/domain-exit topology returns, or the load and
  effort reductions fail to reproduce; do not bundle it with period, residual
  gain, or authority-gating changes because the inherited three-change bundle
  exited the top boundary and cannot assign causality.
