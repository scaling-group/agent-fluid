...
- Do not replace the progress-qualified controller's instantaneous bearing
  owner with an unconditional short-history circular mean. That one-mechanism
  sampled child still captures, but it blunts the decisive initial redirect:
  arrival regresses from `137.357` to `149.853`, mean distance from `4.184L`
  to `4.423L`, total command energy from `90228` to `100638`, RMS relative
  crossflow from `0.1296` to `0.1339`, and RMS force/moment from
  `14.75/303.02` to `15.58/312.62`. Its keyframes show a wider turn and later
  wake-corridor entry, so lower-frequency route estimation is not inherently
  smoother control when the target begins behind or abeam. Preserve immediate
  body-frame bearing authority for large redirects. If history is retested,
  admit it only through an observed geometry/progress condition that leaves
  far-field response unchanged; falsify the condition if it delays entry,
  widens the route, or fails to improve effort and load without losing capture.
