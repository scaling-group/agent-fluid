...
- On the exactly reproduced `137.357`-unit scaffold, do not infer useful
  actuator redistribution or arbitration from temporary route improvement,
  proximity, posterior headroom, or removal of a hard command clamp. Earlier
  tests already showed that gating direct moment feedback delays capture to
  `149.490`, closing-qualified near-target route relief folds away into a top
  exit, and same-sign posterior amplification loops until `279.439`. Three
  newer one-change results close the remaining suggested variants: opposite
  posterior half-cycle redistribution captures at `150.210` with `4.617L`
  mean distance and `99669` energy; applying route asymmetry only to the lag
  component captures at `196.317` with `5.718L` and `131294`; and combining
  route plus moment residual inside one smooth envelope captures at `223.746`
  with `6.429L` and `144686`. All three reduce mean upstream speed from the
  baseline's `-0.07905` to `-0.07404`, `-0.05601`, or `-0.04851`, and raise
  RMS force/moment above `14.75/303.02` to at least `15.18/306.96`. Preserve
  continuous target geometry, progress qualification, direct additive moment
  feedback with its clamp, anterior half-cycle steering, and the unmodified
  posterior lag on this trajectory topology. Revisit their allocation only
  if a distinct failure exposes which actuator or saturation is causal; under
  the common wake snapshot, falsify any replacement unless it retains capture,
  alternating propulsion, and upstream speed while improving arrival/distance
  and effort/load together.
