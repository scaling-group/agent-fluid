...
- Once the alternating half-cycle scaffold reaches the target, qualify slow
  ...
  moment fall from `0.1321/15.49/308.48` to `0.1296/14.75/303.02`. Apply this
  qualification when bearing convergence can be body spin without approach;
  falsify it if capture, upstream translation, or the alternating bend is lost,
  or if held-out wakes do not reproduce the arrival, distance, and load gains.
- Preserve the successful anterior half-cycle steering allocation until a new
  mechanism beats the progress-qualified baseline. Inherited one-change tests
  that circularly average bearing, append lateral-target or relative-crossflow
  residuals, or suppress posterior lag all retain capture but delay it to
  `149.853`--`159.302` units. More decisively, two independent attempts to put
  target-signed half-cycle steering into the posterior bend delay arrival to
  `172.095` and `199.980` units; the latter raises mean distance from `4.184L`
  to `6.189L`. Avoid more posterior steering allocation or unconditioned
  additive observations on this scaffold. Retest only with a distinct
  response-conditioned mechanism that preserves the incumbent wake-crossing
  gait, and reject it if the `137.357`-unit capture, distance integral, loads,
  or upstream translation regress.
