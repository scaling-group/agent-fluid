...
- Windowed bearing-rate feedback is now a replicated-baseline refinement, not
  evidence for a new route. Against three physically identical runs of the
  direct-moment-residual controller, adding one bounded
  `bearing_window_rate` term preserves target capture and essentially the
  same arrival (`149.61` versus `149.57`) while improving score from
  `-2.4344` to `-2.4084`, mean distance from `4.384L` to `4.358L`,
  RMS relative crossflow from `0.1362` to `0.1321`, and RMS lateral
  force/moment from `16.22/314.99` to `15.49/308.48`. Mean command energy
  and power proxy also fall from `681.91/49.63` to `647.93/46.94`, and
  the keyframes show the same upstream wake-crossing topology with terminal
  vertical displacement closer to the target (`-4.465L` versus
  `-4.145L`). Preserve convergence-rate damping as a path/load smoother, but
  do not claim it accelerates transit; extensions should keep the proven
  far-field route and test terminal approach continuously. Falsify it if
  capture or upstream displacement is lost, arrival slows materially, or the
  load, effort, or actuator maxima rise.
