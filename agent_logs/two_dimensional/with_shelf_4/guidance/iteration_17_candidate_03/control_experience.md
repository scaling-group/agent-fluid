...
- Treat previous-action filtering on the regulated oscillator as a phase and
  amplitude change, not presumed actuator or load smoothing.  Against the
  inherited direct-action calibration (`137.357` arrival, `4.184L` mean
  distance, `90228` energy, and `0.12955/14.75/303.02` RMS crossflow/force/
  moment), filtering both joints improves arrival and distance to
  `130.729/3.754L` but raises energy to `115750`, loads to
  `0.15435/18.30/359.97`, anterior bend from `0.404` to `0.469` rad, and
  anterior acceleration to the cap.  Filtering only the anterior joint
  retains a smaller route benefit (`135.019/3.685L`) and reduces energy and
  posterior extrema relative to the both-joint filter, but still costs
  `110448` energy, reaches the anterior cap, and leaves loads high at
  `0.15352/18.53/362.61`.  By contrast, two exact copies of an unfiltered
  body-course correction capture at `137.247/4.077L` with only `91401`
  energy and `0.13358/14.91/306.50` loads.  Preserve direct posterior wave
  tracking and do not stack a fixed filter as an efficiency repair; test a
  bounded load- or phase-conditioned anterior allocation, and falsify it
  unless it keeps capture and a material route benefit while reducing cap
  contact, effort, and loads together.  These same-snapshot results do not
