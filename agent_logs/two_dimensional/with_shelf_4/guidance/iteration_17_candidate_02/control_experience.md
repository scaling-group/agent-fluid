...
- Do not interpret previous-action filtering as actuator smoothing from its
  form alone.  Relative to the replicated direct-action calibration, the
  sampled `0.015`-response filter on both joints improves arrival from
  `137.357` to `130.729` and mean distance from `4.184L` to `3.754L`, but it
  raises total command energy from `90228` to `115750`, RMS
  crossflow/force/moment from `0.1296/14.75/303.02` to
  `0.1543/18.30/359.97`, anterior bend from `0.404` to `0.469` rad, and
  anterior acceleration to the `31.416` cap.  The completed joint-selective
  A/B resolves the posterior allocation: retaining the response only on the
  anterior joint improves mean distance again to `3.685L`, score from
  `-1.8167` to `-1.7453`, total energy to `110448`, mean power proxy from
  `64.75` to `59.95`, posterior bend from `0.439` to `0.411` rad, and posterior
  acceleration from `30.85` to `28.62` rad/time^2.  It also delays arrival to
  `135.019` and slightly raises RMS force/moment from `18.30/359.97` to
  `18.53/362.61`; anterior acceleration remains capped.  Therefore prefer
  direct posterior tracking for distance/effort on this scaffold, but do not
  claim load relief or infer a universally better tail phase.  Test further
  route improvements through normalized anterior feedback, not another
  posterior or scalar edit; falsify this allocation if held-out wakes lose
  capture, upstream translation, the alternating wave, or its distance/effort
  advantage.
