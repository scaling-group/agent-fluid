...
- Treat the current scaffold as a trajectory-level regression oracle for the
  certified wake snapshot, not as evidence of broad robustness. All four
  sampled evaluations, spanning two policy files that differ only in comments,
  exactly reproduce target capture at `137.357`, `4.184L` mean distance,
  `90228` command energy, and `0.12955/14.75/303.02` RMS
  crossflow/force/moment; the inherited completed `5cc...` result repeats the
  same physical tuple. By contrast, another inherited completed sibling still
  reaches the target but only after `279.439` units, with `6.982L` mean
  distance, `182836` energy, and higher `0.13554/15.54/316.29` RMS metrics.
  Preserve the alternating traveling bend and target-bearing route ownership,
  change one feedback topology at a time, and require a new mechanism to match
  capture while improving arrival/distance and effort/load together. The
  exact baseline is an applicable guard only under this shared prewarm and
  wake seed; it becomes a robustness claim only if materially different wake
  phases or layouts reproduce the ordering. The score-only sibling log does
  not identify its policy mechanism, so do not use that regression to condemn
  a particular observation or gain.
- Smooth response gates are not sufficient evidence for removing an
  established feedback authority. On the replicated `137.357`-unit scaffold,
  the assigned parent's one-change policy admits normalized moment rejection
  only while absolute bearing is growing; it still captures, but its deeper
  lower excursion delays arrival to `149.490`, raises mean distance from
  `4.184L` to `4.428L`, energy from `90228` to `97418`, and RMS
  crossflow/force/moment from `0.12955/14.75/303.02` to
  `0.13148/16.38/318.46`. A separate inherited policy smoothly relieves route
  authority only near the target and during positive closure, yet approaches
  no closer than `1.796L`, loops upward, reaches the anterior acceleration cap,
  and exits the domain after `243.447` units with `7.185L` final distance and
  `161191` energy. In this shared wake, neither worsening alignment nor useful
  closure identifies an interval in which the corresponding moment or route
  loop is dispensable. Preserve the direct loops when testing their
  interaction; prefer bounded command arbitration or actuator coordination to
  deactivating one loop. This boundary is falsified if a one-change gate under
  a replicated or held-out wake preserves capture and the traveling bend while
  improving arrival, distance, effort, and load together.
- Translational course error is informative on the successful scaffold, but
  neither additive course steering nor actuator smoothing is a free route
  improvement. Relative to the replicated `137.357`-unit, `4.18356L`, `90228`
  baseline, closing-qualified bearing-minus-velocity alignment preserves
  capture, visibly smooths the midcourse arc, and lowers mean distance to
  `4.07666L`, but improves arrival by only `0.110` units while raising energy
  to `91401` and RMS crossflow/force/moment from
  `0.12955/14.75/303.02` to `0.13358/14.91/306.50`. A separate short common
  filter of both joint commands reaches sooner at `130.729` with `3.75391L`
  mean distance, yet raises energy to `115750`, RMS metrics to
  `0.15435/18.30/359.97`, and drives anterior acceleration to the hard cap.
  An inherited smooth composition of the direct route and moment commands also
  retains capture only after `223.746` units, with `6.429L` mean distance and
  `144686` energy. Therefore do not strengthen the additive course term, stack
  the sampled actuator filter from scalar score alone, or recombine the two
  established direct loops. Preserve the bearing and moment loops and test
  course response through bounded auxiliary-command arbitration that cannot
  reverse persistent route feedback; this boundary is falsified only if a
  one-change mechanism retains capture and the traveling bend while improving
  distance/arrival and effort/load together against both the course-aligned
  parent and replicated baseline under the same wake.
