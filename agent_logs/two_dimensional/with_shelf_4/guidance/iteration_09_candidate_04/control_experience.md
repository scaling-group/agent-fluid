...
- Do not assume that a normalized, bounded observation is useful merely because
  target capture survives when it is added to the incumbent turn sum. Against
  the `137.357`-unit, `4.184L` progress-qualified baseline, a lateral-target
  residual delays capture to `159.302` and raises mean distance to `4.722L`,
  while an oppositely signed relative-crossflow residual takes `154.110`, raises
  mean distance to `4.687L`, and leaves RMS relative crossflow essentially
  unchanged (`0.13005` versus `0.12955`) while force/moment rise to
  `15.85/310.83`. Together with the slower circular-bearing filter, these
  results bound further parallel observation stacking on the shared
  half-cycle command: preserve the route owner and test a distinct actuator
  placement or isolated authority allocation instead. Revisit a residual only
  if a changed-wake rollout identifies a repeatable route bias or slip event
  and an ablation shows that its signal improves progress rather than merely
  changing a still-successful trajectory.
