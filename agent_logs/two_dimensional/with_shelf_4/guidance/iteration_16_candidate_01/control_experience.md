...
- Do not interpret previous-action filtering as actuator smoothing from its
  ...
  anterior acceleration to the `31.416` cap.  Treat this as an evidenced
  phase/amplitude and trajectory change, not load relief.  Because inherited
  posterior half-cycle steering produces a looping `279.439`-unit route with
  `182836` energy, test joint-selective placement that preserves direct
  posterior wave tracking before stacking more posterior or scalar changes.
  Falsify that allocation if it cannot retain materially earlier arrival and
  lower mean distance while reducing the both-joint filter's effort and loads;
  the present snapshot does not identify which joint caused the speed-up.
