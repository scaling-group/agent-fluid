...
- Posterior actuator headroom is a constraint margin, not evidence that route
  steering should be distributed to the tail. Against the replicated
  progress-qualified capture (`137.357` time, `4.184L` mean distance, `90228`
  energy, `0.1295` crossflow, `14.75/303.02` force/moment), sharing only `0.08`
  of the same half-cycle turn with the lagged posterior target still captured
  but regressed to `150.210`, `4.617L`, `99669`, `0.1346`, and
  `15.18/306.96`; its keyframes also widen the first loop. Preserve the
  posterior joint as the unmodulated lagged propulsion target on this scaffold,
  and do not retry tail-envelope steering merely because its acceleration is
  below the cap. Reopen posterior steering only with phase-resolved evidence
  that it tightens the route without slowing arrival, raising effort/load, or
  weakening the alternating upstream wave.
