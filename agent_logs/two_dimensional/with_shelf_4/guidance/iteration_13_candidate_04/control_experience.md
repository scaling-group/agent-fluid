...
- Slow route divergence is also a poor gate for the existing fast yaw-load
  residual on this scaffold. Four sampled files replicate the direct-residual
  baseline at `137.357` units and `4.184L` mean distance; an inherited
  one-change policy instead admits moment rejection only when
  `bearing * bearing_window_rate > 0`. It still captures, but keyframes show
  a deeper lower-wake excursion, arrival regresses to `149.490` units, mean
  distance and total energy rise to `4.428L/97418`, and RMS
  crossflow/force/moment rise from `0.12955/14.75/303.02` to
  `0.13148/16.38/318.46`. Do not equate growth of body-frame bearing with an
  adverse instantaneous wake moment: the windowed route signature is too slow
  or aliased to decide which fast load corrections are useful. Preserve the
  direct residual until a phase- or actuator-response-conditioned alternative
  improves arrival, distance, effort, and loads together; meanwhile prefer a
  separately falsifiable coordination change that leaves the replicated route
  and disturbance observations in their evidenced roles.
- Two later one-change rollouts now bound that coordination direction. Sharing
  the slow route continuously through posterior half-cycle asymmetry preserves
  capture but produces several visible loops, delaying arrival from `137.357`
  to `196.317` units while mean distance and total energy rise from
  `4.184L/90228` to `5.718L/131294` and RMS crossflow/force/moment rise from
  `0.12955/14.75/303.02` to `0.13498/15.48/310.21`. Combining the route and
  moment residual inside one smooth saturation envelope is worse: the looping
  trajectory arrives at `223.746` units with `6.429L` mean distance, `144686`
  energy, and `0.13399/16.79/324.37` RMS crossflow/force/moment. Preserve the
  successful hard route-plus-direct-residual arbitration, and do not give the
  posterior wave a persistent slow-route bias merely because it has observed
  acceleration headroom. If posterior redirect assistance is tested again,
  make its release contingent on measured useful response so the ordinary
  traveling wave recovers; falsify that boundary if response-qualified sharing
  still loops, delays capture, or fails to improve distance, effort, and loads
  together under the common wake snapshot.
