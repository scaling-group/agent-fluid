...
- Coordinate previous-action response over the complete two-joint traveling
  wave rather than guarding each joint as an independent actuator. In the
  current one-mechanism comparison, replacing separate positive lag-power
  guards with a single guard on total normalized signed lag power advances
  capture from `123.018` to `118.024` units, reduces mean distance from
  `3.594L` to `3.560L` and total command energy from `95084` to `89079`, and
  lowers RMS crossflow/force/moment from `0.14287/17.03/334.45` to
  `0.13833/16.74/329.67`. The keyframes retain the same alternating upstream
  capture topology while shortening the late redirect, so the reusable
  implication is to let energy-removing lag at one joint balance lag at the
  other before bypassing both history deltas together; per-joint suppression
  can unnecessarily break useful anterior-posterior exchange. This is one
  deterministic shared-snapshot comparison, not wake-phase robustness.
  Preserve the raw posterior-lagged wave and falsify the coupled guard if a
  matched or held-out wake loses capture or alternating propulsion, or if it
  fails to improve arrival, distance, and effort/load together over the
  independent guards.
