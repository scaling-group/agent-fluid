...
- Treat previous-action response as a phase/amplitude mechanism, not actuator
  ...
  `135.019` and leaves force/moment at `18.53/362.61`. A normalized signed-
  power guard is the first completed response conditioner to improve route,
  ...
  `0.1429/17.03/334.45` RMS crossflow/force/moment. Four sampled directories
  reproduce that result with byte-identical released sheets, and both-joint
  bend and acceleration peaks fall relative to the unguarded response,
  although anterior acceleration still touches its cap. Prefer phase- or
  energy-aware command shaping over unconditional history or scalar changes
  on this scaffold; falsify the guard if held-out wakes lose capture, upstream
...
- Feedback mechanisms that help separately are not necessarily additive.
  ...
  mechanisms continuously on the current scaffold or trying to rescue the
  combination by scalar-only tuning; retest compatibility only with a distinct
  phase- or response-conditioned arbitration signal. This boundary is limited
  to the certified wake snapshot and is falsified by a one-change combination
  that preserves the faster capture topology while improving distance and
  effort/load jointly.
