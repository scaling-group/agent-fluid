...
- On the otherwise identical `0.72`-period half-cycle controller with a small
  ...
  causal component.
- Response qualification is a causal route variable, not just another gain.
  Relative to the otherwise identical bearing-rate controller, multiplying
  only its rate damping by positive normalized closing speed shortens capture
  from `149.61` to `137.36` units, improves mean distance from `4.358L` to
  `4.184L` and score from `-2.408` to `-2.242`, and lowers RMS crossflow,
  force, and moment from `0.1321/15.49/308.48` to
  `0.1295/14.75/303.02`; its slightly higher mean command energy (`656.89`
  versus `647.93`) means the supported benefit is quicker targetward response,
  not generic effort reduction. In the complementary assigned-parent test,
  adding only unconditional bounded heading-rate damping lowers mean command
  energy and RMS crossflow/force/moment further to
  `624.50` and `0.1275/14.71/298.03`, but delays capture to `150.83`, worsens
  mean distance to `4.387L`, and lowers score to `-2.437`. Therefore do not
  apply angular-response damping throughout a target redirect: test it only
  when body-frame alignment and positive closing corroborate that yaw is a
  disturbance. This boundary is falsified if the progress-qualified result
  does not reproduce, or if conditional yaw damping still delays capture or
  worsens distance despite retaining the load benefit.
