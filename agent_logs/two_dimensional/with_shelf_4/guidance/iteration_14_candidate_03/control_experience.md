...
- Completed one-change descendants also bound how the existing signed moment
  residual may be coordinated. Gating it by growth of absolute bearing delays
  capture to `149.490` units and raises mean distance/energy and RMS
  crossflow/force/moment from the replicated baseline's
  `4.184L/90228/0.12955/14.75/303.02` to
  `4.428L/97418/0.13148/16.38/318.46`; putting route and moment inside one
  smooth saturation envelope is worse at
  `223.746/6.429L/144686/0.13399/16.79/324.37`. Moving extra route asymmetry
  to the posterior actuator likewise captures only after `196.317` units with
  `5.718L` mean distance and `131294` energy. Thus neither slow route-response
  gates, smooth command arbitration, nor observed posterior acceleration
  headroom justify weakening the direct moment path or reallocating steering
  to the tail. Preserve its sign, anterior half-cycle location, and the
  unmodified posterior traveling wave when testing a distinct load mechanism;
  test load-conditioned compliance as a common gait envelope, not another
  route residual. This boundary is falsified only by a one-change controller
  that retains the `137.357` capture topology while improving arrival or
  distance and effort/load together under the certified wake snapshot.
