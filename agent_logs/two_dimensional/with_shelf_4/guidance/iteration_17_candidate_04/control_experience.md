...
- Previous-action response on this half-cycle scaffold is a route/phase
  mechanism, not an evidenced load smoother.  Filtering both accelerations
  captures in `130.729` units at `3.754L` mean distance but costs `115750`
  energy and `0.1543/18.30/359.97` RMS crossflow/force/moment.  Restricting the
  same response to the anterior joint lowers energy to `110448`, lowers the
  posterior peak bend/acceleration from `0.439/30.851` to `0.411/28.624`, and
  improves mean distance to `3.685L`, but capture slows to `135.019` and force
  and moment remain `18.53/362.61`.  In contrast, duplicated direct-action
  course feedback captures in `137.247` at `4.077L` mean distance with only
  `91401` energy and `0.1336/14.91/306.50` loads.  Preserve direct posterior
  tracking; if the anterior history effect is retained for its route benefit,
  qualify it continuously with normalized target geometry and measured
  progress rather than applying it through every beat.  Reject that strategy
  if it loses capture or the alternating wave, regresses beyond the direct
  route, or fails to reduce effort and loads materially below the persistent
  anterior-response result.  This comparison is same-snapshot evidence and
  remains falsifiable under held-out wake phase, inflow, and resolution.
