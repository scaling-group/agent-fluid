...
- Three completed response-conditioning/actuator-sharing extensions establish
  a sharper negative boundary around the `137.357`-unit scaffold. Gating the
  direct yaw-moment residual by growth of absolute bearing delays capture to
  `149.490` units and raises mean distance/energy to `4.428L/97418`; using
  posterior acceleration headroom for route-driven tail half-cycle asymmetry
  delays capture to `196.317` with `5.718L/131294`; and attenuating moment
  rejection when moment opposes body yaw rate delays capture to `205.519` with
  `6.209L/134893`. Their keyframes preserve alternating propulsion but show
  deeper or prolonged midcourse detours, while RMS force/moment also exceed
  the direct-residual baseline. Thus neither slow bearing divergence,
  instantaneous yaw-power sign, nor unused posterior acceleration is evidence
  that disturbance rejection should be withdrawn or route steering moved into
  the tail. After this topology recurs, restore the direct moment residual and
  unmodified posterior traveling wave; avoid another attenuation or
  actuator-sharing heuristic unless a calibrated phase/response signature
  predicts when it should act. This boundary is limited to the common wake
  snapshot and is falsified by a one-change mechanism that preserves the
  `137.357` capture topology while improving distance and effort/load jointly.
...
- Treat exact repeats of the progress-qualified scaffold as a calibrated
  control, not as evidence for additional mechanisms. The four assigned solver
  samples have identical prewarm/released keyframe sheets and exactly reproduce
  `137.357`-unit capture, `4.184L` mean distance, `90228` command energy, and
  `0.12955/14.75/303.02` RMS crossflow/force/moment; the assigned parent's
  completed step-14 control reproduces the same values after the inherited
  response-gating and actuator-sharing regressions. This repeatability makes a
  one-change comparison unusually sharp: require any extension to preserve
  capture and the alternating wave while improving beyond those matched
  arrival/distance/load values, and do not count duplicated samples as
  independent architectural wins. The remaining baseline keyframe feature is
  a broad initial hook with `4.305L` maximum lateral target offset. Given the
  logged failures of extra additive observations, disturbance attenuation, and
  posterior route sharing, the next distinct test should modulate the existing
  anterior phase-dependent steering from measured response, such as a bounded
  large-bearing/non-closing burst that releases on positive closing, rather
  than add another residual or alter the cruise wave. This test is falsified by
  the same or deeper hook, delayed capture, greater saturation/load/effort, or
  any loss of target capture or alternating propulsion.
