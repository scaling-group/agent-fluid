...
- Treat previous-action response as a phase/amplitude mechanism, not as an
  ...
  stacking another route residual. That condition now has positive completed
  evidence: continuously bypassing each joint's history response only when its
  lag adds positive normalized joint power advances capture from the best
  unguarded response's `130.729` to `123.018`, lowers mean distance from
  `3.754L` to `3.594L` and energy from `115750` to `95084`, and lowers
  RMS crossflow/force/moment from `0.1543/18.30/359.97` to
  `0.1429/17.03/334.45`; it also beats the anterior-only response on all of
  those measures. Preserve the raw traveling-wave command as the phase owner
  and let continuity yield when it postpones braking. The four current sample
  artifacts differ only cosmetically and have identical keyframes and metrics,
  so they count as repeated materialization of one CFD result, not robustness
  across wake phases. This boundary is falsified if a held-out wake loses
  capture or the alternating upstream wave, or if a further response mechanism
  cannot improve route without worsening bend peaks, cap contact, effort, and
  force/moment as a coherent tradeoff.
