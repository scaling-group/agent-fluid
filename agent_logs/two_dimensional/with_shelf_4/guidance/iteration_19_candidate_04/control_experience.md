...
- Treat previous-action response as a phase/amplitude mechanism, not actuator
  smoothing by definition.  An unguarded short response on both joints reaches
  in `130.729` units but raises energy and RMS crossflow/force/moment to
  `115750` and `0.1543/18.30/359.97`; restricting it to the anterior joint
  lowers mean distance/energy to `3.685L/110448` but delays capture to
  `135.019` and leaves force/moment at `18.53/362.61`.  A completed normalized
  signed-power guard is the first response conditioner to improve trajectory,
  effort, and load together: it retains two-joint continuity only when filter
  lag is neutral or energy-removing and otherwise restores raw state feedback,
  reaching in `123.018` units with `3.594L` mean distance, `95084` energy, and
  `0.1429/17.03/334.45` RMS crossflow/force/moment.  It also reduces both-joint
  bend and acceleration peaks relative to the unguarded response, although the
  anterior acceleration still touches its cap.  Prefer phase- or energy-aware
  command shaping over unconditional history or scalar changes on this
  scaffold; falsify the guard if held-out wakes lose capture, upstream
  translation, the alternating wave, or its joint distance/effort/load gains.
- Deduplicate controller evidence by executable behavior and rollout outcome,
  not source-file checksum.  Four current sampled signed-power-guard
  directories have three policy hashes because only comments and blank lines
  differ, yet their prewarm/released sheets are byte-identical and each exactly
  repeats `123.018` arrival, `3.594L` mean distance, `95084` energy, and
  `0.1429/17.03/334.45` RMS crossflow/force/moment.  Count this as one
  deterministic same-snapshot observation: it is a precise baseline for a
  one-mechanism comparison, but neither textual diversity nor exact repetition
  establishes robustness to wake phase, inflow, geometry, or resolution.
- Feedback mechanisms that help separately are not necessarily additive.
  Closing-qualified body-course alignment on direct action gives a small
  distance improvement (`4.184L` to `4.077L`), but stacking it with the
  anterior-only response prolongs visible midcourse reversals, delays capture
  from `135.019` to `162.222`, and worsens mean distance/energy from
  `3.685L/110448` to `4.433L/136964`.  Avoid combining these route and history
  mechanisms on the current scaffold or trying to rescue the combination by
  scalar-only tuning; retest compatibility only with a distinct phase- or
  response-conditioned arbitration signal.  This boundary is limited to the
  certified wake snapshot and is falsified by a one-change combination that
  preserves the faster route while improving distance and effort/load jointly.
