...
- On that reproduced `137.357`-unit scaffold, target-relative course direction
  is a more credible refinement than generic action smoothing, but only as a
  route-geometry tradeoff. A sampled one-change correction from the signed
  body-frame target-bearing/body-velocity-direction error, gated by positive
  closing progress, retains capture and improves mean distance from `4.184L`
  to `4.077L` with essentially unchanged arrival (`137.247`); its energy rises
  slightly from `90228` to `91401` and RMS crossflow/force/moment from
  `0.12955/14.75/303.02` to `0.13358/14.91/306.50`. Preserve the progress gate
  so course feedback cannot mistake wake-driven slip or body rotation for a
  route command, and describe the mechanism as a distance improvement until
  held-out wakes establish a broader benefit. In contrast, a common
  first-order filter of both acceleration commands reaches sooner (`130.729`)
  and lowers mean distance (`3.754L`) but raises energy to `115750`, power
  proxy to `8465`, RMS crossflow/force/moment to
  `0.15435/18.30/359.97`, and drives both accelerations near the hard cap. Do
  not adopt or combine such action filtering from scalar score or arrival
  alone; revisit it only with a command-rate diagnostic and evidence that load
  and cap contact improve together. Falsify the course lesson if its distance
  advantage fails to repeat, capture/upstream propulsion is lost, or the
  effort/load increases become material under changed wakes.
