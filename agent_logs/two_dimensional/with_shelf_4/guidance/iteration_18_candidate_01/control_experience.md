...
- Previous-action response on this half-cycle scaffold is a route/phase
  mechanism whose qualification must remain local to the rhythmic state.
  Unqualified filtering of both accelerations captures in `130.729` units at
  `3.754L` mean distance but costs `115750` energy and
  `0.1543/18.30/359.97` RMS crossflow/force/moment; anterior-only filtering
  captures in `135.019` at `3.685L` and lowers energy to `110448`, but force
  and moment remain `18.53/362.61`.  Qualifying both response residuals by the
  normalized positive signed power added by command lag is the current
  positive result: its sheet retains the early redirect and alternating wave,
  capture improves to `123.018`, mean distance to `3.594L`, energy to `95084`,
  and loads to `0.1429/17.03/334.45`.  In contrast, broadly releasing anterior
  history from bearing/closing progress regresses to `177.260/4.869L` and
  `121791` energy, while stacking course alignment on persistent anterior
  response regresses to `162.222/4.433L`, `136964` energy, and `0.1577` RMS
  crossflow.  Avoid geometry-wide history gates and combinations of separately
  useful route residuals until their interaction is isolated; prefer a
  phase-local energy guard that preserves both-joint coherence.  This guard
  still reaches the anterior acceleration cap and remains higher-load than the
  direct-action course policy (`91401` energy and
  `0.1336/14.91/306.50` loads), so reject it if held-out wakes lose capture or
  its arrival/distance advantage, and do not describe it as complete load
  smoothing until cap contact and the load gap also fall.
