...
- Previous-action continuity is useful on this rhythmic two-joint controller
  only when the raw state-feedback command can recover authority during the
  wrong energetic phase. Filtering both joints without qualification captures
  in `130.729` units but costs `115750` command energy and
  `0.15435/18.30/359.97` RMS crossflow/force/moment; filtering only the
  anterior joint takes `135.019` units with `110448` energy and even higher
  force/moment (`18.53/362.61`). A completed two-joint variant that preserves
  the response except when its lag adds positive normalized joint power
  improves capture to `123.018`, mean distance to `3.594L`, energy to `95084`,
  and RMS crossflow/force/moment to `0.14287/17.03/334.45`. Thus preserve
  inter-joint response coherence, but continuously return authority to the raw
  oscillator when command history would keep energizing a joint after the
  requested phase has changed. This lesson applies to state-observed action
  filtering, not arbitrary torque limiting, and does not identify a universal
  time constant or power scale. Falsify it if held-out wakes lose capture,
  upstream translation, or alternating propulsion, or if the distance and
  effort/load advantages do not remain joint rather than trading off.
