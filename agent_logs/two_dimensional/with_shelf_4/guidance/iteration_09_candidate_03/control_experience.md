...
- Once closing-progress qualification produces the replicated `137.357`-unit
  capture (`4.18356L` mean distance, `14.75/303.02` RMS force/moment), three
  observation-only additions all preserve success but degrade the same route:
  circular bearing-history smoothing reaches in `149.853` with `4.42329L`
  mean distance, a body-frame lateral-target residual reaches in `159.302`
  with `4.72194L`, and direct relative-crossflow rejection reaches in `154.110`
  with `4.68696L` while raising RMS force/moment to `15.85/310.83`. Their
  keyframes show wider or delayed wake-corridor entry rather than useful
  smoothing. On this progress-qualified half-cycle scaffold, avoid treating
  visible zigzags as sufficient evidence for another route/history/slip signal
  or for scalar tuning of those residuals; preserve the instantaneous-bearing
  route loop and test a distinct actuation mechanism when joint headroom
  supports it. This boundary is specific to the tested signs, scales, common
  snapshot, and direct additive forms; overturn it only with an isolated
  residual that improves capture and distance without worse load or with
  evidence that changed wake phase exposes a robustness benefit.
