...
- Later mechanism-level descendants sharpen the preservation boundary around
  that reproduced `137.357`-unit scaffold. Gating the direct moment residual
  by adverse bearing response delays capture to `149.490` and raises RMS
  force/moment to `16.38/318.46`; isolated posterior lag-wave asymmetry delays
  capture to `196.317` with `5.718L` mean distance and `131294` energy; and
  composing route plus moment under one smooth saturation delays capture to
  `223.746` with `6.429L` mean distance and `144686` energy. Opposite and
  same-sign full-tail half-cycle multipliers also regress to `150.210` and
  `279.439`, respectively. Thus neither apparent posterior headroom nor a
  smoother arbitration formula is evidence that established actuator roles
  should change. Preserve the direct moment residual, hard bounded route/load
  composition, and unmodified posterior wave until a new result improves the
  trajectory. If response damping is extended, keep it dormant throughout the
  rearward redirect and qualify it with normalized forward target geometry or
  translation; falsify it against capture, arrival, mean distance, effort,
  load, and the alternating upstream trajectory together.
- Treat exact trajectory reproduction as a possible inactive-code result, not
  automatic controller validation. An inherited attempt to add forward-target
  qualification to the `137.357`-unit scaffold reproduced its keyframes and
  every physical metric exactly because it clamped positive
  `target_body_L[1] / distance_L`; in this body convention forward target
  distance is `-target_body_L[1]`, so the proposed gate was zero when the
  target was ahead. Use the exposed `forward_distance_L` (or the explicit
  negative projection), and verify that a new bounded observation can be
  active on the evidenced trajectory before interpreting an unchanged CFD
  result. This sign lesson applies to body-frame longitudinal geometry and is
  falsified only if the observation convention changes or recorded state
  histories show the candidate term was materially active.
