...
- On the successful direct-moment-residual scaffold, adding only bounded
  `bearing_window_rate` feedback is a replicated causal improvement in route
  damping, not speed: capture remains essentially `149.6` units, while mean
  distance improves from `4.384L` to `4.358L`, total command energy from
  `101995` to `96933`, and RMS force/moment from `16.22/314.99` to
  `15.49/308.48`; peak joint rates and accelerations also fall. Preserve this
  rate term when testing a new mechanism, and falsify it by loss of capture,
  upstream translation, or those distance/load benefits. An inherited
  response-direction gate on the moment residual is a useful boundary: it
  still captures slightly earlier (`148.70`) and lowers RMS moment to
  `306.15`, but worsens mean distance to `4.578L` and score to `-2.629`.
  Therefore do not infer a better route from arrival or load alone, and avoid
  attenuating the evidenced direct residual from the sign of bearing response
  unless a later causal comparison also improves distance topology.
- A later one-change comparison establishes a useful qualification for that
  bearing-rate term: multiplying it continuously by positive normalized
  closing progress, so it cannot relax steering while distance is stagnant or
  growing, advances capture from `149.61` to `137.36`, improves mean distance
  from `4.358L` to `4.184L`, lowers relative-crossflow RMS from `0.1321` to
  `0.1295`, and lowers force/moment RMS from `15.49/308.48` to
  `14.75/303.02`. Total command energy falls from `96933` to `90228`, although
  its per-time mean rises slightly from `647.93` to `656.89`; interpret this as
  a faster, cleaner route rather than a general efficiency gain. Preserve the
  closing qualification when adding a new disturbance mechanism. It is
  falsified if a held-out wake loses capture/upstream translation or if the
  gate merely suppresses corrective damping during poor progress without
  improving distance topology.
