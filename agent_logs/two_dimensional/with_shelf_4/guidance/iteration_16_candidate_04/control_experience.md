...
- A short first-order tracker over both complete joint-acceleration commands
  improves the replicated scaffold's arrival from `137.357` to `130.729`
  units and mean distance from `4.184L` to `3.754L`, but it does so by
  enlarging the gait rather than relieving the actuator. Total command energy
  rises from `90228` to `115750`; joint-1 acceleration reaches the `31.416`
  cap, joint-2 acceleration rises from `25.552` to `30.851`, joint-angle
  extrema rise from `0.402/0.346` to `0.469/0.439` rad, and RMS
  crossflow/force/moment rise from `0.12955/14.75/303.02` to
  `0.15435/18.30/359.97`. Therefore a nominally smooth command is not a
  realized load-reduction mechanism when delay sits inside a nonlinear
  state-feedback oscillator; do not repeat full-command low-pass tuning or
  claim lower effort without checking the realized envelope. The inherited
  full-tail steering and lag-modulation regressions further rule out changing
  the posterior target merely because it has actuator headroom. If response
  dynamics are tested again, keep anterior radial/route regulation direct and
  isolate the unchanged posterior propulsive interface; falsify that split if
  it cannot retain capture and some arrival/distance benefit while moving cap
  contact, gait envelope, effort, and loads back toward the unfiltered
  scaffold. A separate body-velocity course-alignment sample only improves
  mean distance to `4.077L` and arrival to `137.247` with `91401` energy, so
  it is a modest route-response contrast, not evidence that another additive
  steering residual explains the faster filtered trajectory.
