function target_policy_params()
    return (
        ...
        lateral_target_scale=2.0,
        lateral_target_gain=0.15,
...
function target_policy(state, params)
    ...
    # At equal bearing, absolute lateral displacement is larger in the far
    # field than near capture. This bounded body-frame residual strengthens
    # broad route correction while fading naturally on final approach.
    lateral_target = params.lateral_target_gain * tanh(
        Float64(state.target_body_L[2]) / params.lateral_target_scale,
    )
    route_turn = tanh(
        bearing_error + lateral_target +
