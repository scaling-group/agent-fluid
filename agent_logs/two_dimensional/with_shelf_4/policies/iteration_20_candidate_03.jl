...
function target_policy(state, params)
    ...
    # Treat the short actuator response as one traveling-wave pair. Project its
    # lag onto the current raw command direction so history can smooth command
    # magnitude without rotating the anterior/posterior acceleration ratio.
    # A gait-scaled regularizer restores raw feedback continuously when the
    # current command pair vanishes. This uses observed action and dt, not a
    # clock or mutable controller state.
    ...
    lag_a1 = filtered_a1 - raw_a1
    lag_a2 = filtered_a2 - raw_a2
    gait_acceleration_scale = max(omega^2 * amp, eps(Float64))
    raw_command_norm_squared = raw_a1^2 + raw_a2^2
    projection_floor_squared = (
        params.command_projection_floor * gait_acceleration_scale
    )^2
    parallel_fraction = (lag_a1 * raw_a1 + lag_a2 * raw_a2) /
        (raw_command_norm_squared + projection_floor_squared)
    coherent_lag_a1 = parallel_fraction * raw_a1
    coherent_lag_a2 = parallel_fraction * raw_a2

    # Retain the evidenced shared signed-power guard after the projection. One
    # soft gate releases both joints toward raw feedback together whenever the
    # remaining history component injects net kinetic energy.
    gait_power_scale = max(omega^3 * amp^2, eps(Float64))
    lag_power = (
        qd1 * coherent_lag_a1 + qd2 * coherent_lag_a2
    ) / gait_power_scale
    excess_power = max(lag_power, 0.0) / params.lag_power_softness
    response_weight = inv(1 + excess_power^2)
    a1 = raw_a1 + response_weight * coherent_lag_a1
    a2 = raw_a2 + response_weight * coherent_lag_a2
