function target_policy_params()
    return (
        ...
        phase_softness=0.35,
        redirect_error_ratio=2.0,
        counterturn_suppression=0.18,
        tail_lag_gain=0.65,
        tail_damping=0.7,
        moment_scale=0.08,
...
function target_policy(state, params)
    ...
    incumbent_amplitude = amp * (
        1 + params.half_cycle_asymmetry * turn_request * beat_side
    )

    # A large route error without measured approach calls for redirect rather
    # than more near-cap positive-side actuation. Attenuate only the half-cycle
    # opposing the persistent route turn, and release the attenuation as
    # closing progress confirms useful translation. The strengthened side and
    # the incumbent wake-crossing gait are unchanged.
    large_redirect = tanh(
        (abs(bearing_error) / params.redirect_error_ratio)^2,
    )
    counterturn_side = max(-route_turn * beat_side, 0.0)
    redirect_attenuation = 1 - params.counterturn_suppression *
        large_redirect * (1 - closing_progress) * counterturn_side
    half_cycle_amplitude = incumbent_amplitude * redirect_attenuation
