...
function target_policy(state, params)
    ...
    # Persistent body-frame bearing owns the route. Keep the sampled radial
    # progress qualification, then distinguish a useful aligning response from
    # radial approach that still widens the lateral target error. Diverging
    # bearing keeps its existing rate correction; only the aligning portion is
    # released in proportion to measured lateral convergence. This uses no
    # hidden mode, world-frame route, or external wake phase.
    ...
    target_distance = max(Float64(state.distance_L), eps(Float64))
    lateral_target_direction = clamp(
        Float64(state.target_body_L[2]) / target_distance,
        -1.0,
        1.0,
    )
    lateral_target_rate = Float64(state.target_body_window_rate_L[2])
    lateral_closing_speed = max(
        -lateral_target_direction * lateral_target_rate,
        0.0,
    )
    lateral_progress = tanh(
        lateral_closing_speed / params.closing_speed_scale,
    )
    aligning_response = max(-tanh(bearing_error) * bearing_rate, 0.0)
    aligning_release = 1 - aligning_response * (1 - lateral_progress)
    route_turn = tanh(
        bearing_error +
        params.bearing_rate_damping * closing_progress *
        aligning_release * bearing_rate,
    )
