...
function target_policy(state, params)
    ...
    # Coordinate the posterior wave with the same requested half-cycle. This
    # recruits the sampled joint-2 headroom for turning without a static bend:
    # the positive bounded scale returns to one whenever turn or phase vanishes.
    tail_half_cycle_scale = 1 +
        params.posterior_steering_gain * turn_request * beat_side
    tail_wave = -q1 -
        params.tail_lag_gain * qd1 / max(omega, eps(omega))
    tail_target = tail_half_cycle_scale * tail_wave
