...
function target_policy(state, params)
    ...
    # Keep anterior radial regulation and route steering prompt: filtering this
    # loop enlarged the sampled limit cycle and drove joint 1 to its hard
    # acceleration cap. Retain the short response only at the posterior
    # propulsive interface, without changing its lagged traveling-wave target.
    # The actual normalized observation interval and previous applied action
    # add neither a clock nor mutable policy state.
    ...
    a1 = raw_a1
    a2 = previous_a2 + response_fraction * (raw_a2 - previous_a2)
