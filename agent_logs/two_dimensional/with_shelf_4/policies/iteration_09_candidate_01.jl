...
function target_policy(state, params)
    ...
    # Infer the active beat side from joint state. During a large persistent
    # route turn, reduce only the symmetric cruise envelope while retaining the
    # signed half-cycle difference. This favors redirect over translation
    # without adding anterior acceleration or letting a fast wake-load residual
    # suppress propulsion. Full cruise returns continuously near alignment.
    phase_coordinate = (q1 + qd1 / max(omega, eps(omega))) / amp
    beat_side = tanh(phase_coordinate / params.phase_softness)
    cruise_fraction = 1 - params.turn_drive_relief * abs(route_turn)
