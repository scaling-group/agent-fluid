...
function target_policy(state, params)
    ...
    # target bearing after distance history confirms targetward motion. Treat
    # this auxiliary observation only as course damping: it may reduce an
    # overshooting route drive, but may neither add curvature nor reverse the
    # persistent target-bearing command.
    ...
    raw_course_correction = params.course_alignment_gain * closing_progress *
        tanh(course_error / params.bearing_scale)
    route_drive =
        bearing_error +
        params.bearing_rate_damping * closing_progress * bearing_rate
    opposing_course = route_drive > 0.0 ?
        min(raw_course_correction, 0.0) :
        max(raw_course_correction, 0.0)
    max_course_brake = params.course_brake_fraction * abs(route_drive)
    course_brake = clamp(
        opposing_course,
        -max_course_brake,
        max_course_brake,
    )
    route_turn = tanh(route_drive + course_brake)
