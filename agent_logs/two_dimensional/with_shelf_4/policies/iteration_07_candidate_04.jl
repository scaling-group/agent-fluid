...
function target_policy(state, params)
    ...
    # Retain the best sampled policy's normalized yaw-load residual, then add
    # low-authority damping on the measured body response. Moment feedback
    # reacts to the disturbance while yaw-rate feedback opposes the resulting
    # rotation; neither term assumes a vortex phase or changes the mean bend.
    moment_turn = params.moment_rejection_gain * tanh(
        Float64(state.moment_z_L2) / params.moment_scale,
    )
    yaw_rate_turn = params.yaw_rate_damping * tanh(
        Float64(state.heading_rate) / params.yaw_rate_scale,
    )
    wake_turn = moment_turn + yaw_rate_turn
