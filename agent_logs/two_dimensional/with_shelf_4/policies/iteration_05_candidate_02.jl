function target_policy_params()
    return (
        ...
        approach_distance_L=2.0,
        approach_width_L=0.4,
        approach_bearing_rate_damping=0.2,
...
function target_policy(state, params)
    ...
    # bearing is diverging. Smooth distance feedback strengthens only this
    # damping mechanism during final approach, without a hidden mode or clock.
    ...
    approach_weight = 0.5 * (1 - tanh(
        (Float64(state.distance_L) - params.approach_distance_L) /
        params.approach_width_L,
    ))
    rate_damping = params.bearing_rate_damping +
        params.approach_bearing_rate_damping * approach_weight
    route_turn = tanh(bearing_error + rate_damping * bearing_rate)

    # Retain the sampled small, direct normalized yaw-load residual. It trims
    # fast wake disturbances without concentrating its authority near zero
