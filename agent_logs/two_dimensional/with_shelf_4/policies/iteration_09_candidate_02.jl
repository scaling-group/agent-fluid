function target_policy_params()
    return (
        ...
        phase_softness=0.35,
        tail_lag_gain=0.65,
        redirect_bearing_scale=90.0 * pi / 180,
        tail_lag_relief=0.45,
        tail_damping=0.7,
        moment_scale=0.08,
...
function target_policy(state, params)
    ...
    # During a very large redirect, relieve only the posterior phase-lag
    # component so the anterior half-cycle asymmetry can turn without full
    # target-misaligned propulsive thrust. Restore the complete evidenced lag
    # continuously as instantaneous body-frame bearing returns to alignment.
    redirect_level = tanh(
        abs(bearing) / params.redirect_bearing_scale,
    )^2
    tail_wave_gate = 1 - params.tail_lag_relief * redirect_level
    tail_target = -q1 -
        tail_wave_gate * params.tail_lag_gain *
