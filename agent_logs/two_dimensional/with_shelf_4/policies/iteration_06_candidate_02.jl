...
function target_policy(state, params)
    ...
    # Share a small amount of the same target-relative half-cycle envelope with
    # the lagged posterior wave. This uses the tail's remaining actuation
    # headroom for turning without adding a static bend: the multiplier returns
    # to one when either route/load request or the inferred beat side vanishes.
    posterior_envelope = 1 +
        params.posterior_steering_share * turn_request * beat_side
    tail_target = posterior_envelope * (
