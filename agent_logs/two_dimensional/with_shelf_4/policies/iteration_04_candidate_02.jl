...
function target_policy(state, params)
    ...
    # Persistent body-frame bearing owns the route. A smaller normalized yaw-
    # load residual trims fast disturbances without introducing a static bend,
    # clock, or assumed vortex phase. Positive bend produces the negative-yaw
    # response needed to oppose a positive applied moment for this body map.
    ...
    # the successful alternating propulsion while both feedback terms vanish.
    ...
    # Retain a posterior-lagged traveling bend. The load residual acts only
    # through the anterior envelope, so it cannot replace the tail's alternating
    # propulsive target with a separate steering offset.
    tail_target = -q1 -
