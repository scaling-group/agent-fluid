...
function target_policy(state, params)
    ...
    # Persistent body-frame bearing owns the route.  Circularly average the
    # supplied short history so beat-scale body yaw does not masquerade as a
    # new route direction; this uses passive observations, not hidden state.
    bearing_sin = 0.0
    bearing_cos = 0.0
    for bearing_sample in state.bearing_history
        bearing_sin += sin(Float64(bearing_sample))
        bearing_cos += cos(Float64(bearing_sample))
    end
    persistent_bearing = atan(bearing_sin, bearing_cos)
    bearing_error = persistent_bearing / params.bearing_scale

    # Qualify windowed-rate damping with measured target-distance progress:
    # alignment change alone can be body spin, while positive closing speed
    # confirms useful response. This keeps the redirect firm until it produces
    # translation, then restores the sampled damping continuously.
    bearing_rate = tanh(
