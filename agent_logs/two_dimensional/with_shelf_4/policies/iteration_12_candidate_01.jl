...
function target_policy(state, params)
    ...
    # Let the persistent target geometry and the faster yaw-load residual
    # share one smooth authority envelope. With zero measured moment this is
    # exactly the sampled route request. Near a large route drive, tanh's slope
    # attenuates the residual continuously instead of letting an additive term
    # flatten at a hard clamp or subtract its full authority from the route.
    wake_residual = params.moment_rejection_gain * tanh(
        Float64(state.moment_z_L2) / params.moment_scale,
    )
    turn_request = tanh(route_drive + wake_residual)
