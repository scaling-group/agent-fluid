function target_policy_params()
    return (
        ...
        tail_lag_gain=0.65,
        tail_damping=0.7,
        posterior_countersteer_gain=0.08,
        moment_scale=0.08,
...
function target_policy(state, params)
    ...
    # Redistribute a small amount of half-cycle authority across the traveling
    # bend. When the anterior joint strengthens the requested side, ease the
    # oppositely phased posterior target instead of amplifying both together;
    # this increases distributed curvature without adding a static tail bias.
    # The positive multiplier returns to one with zero turn or beat phase.
    posterior_half_cycle_scale = 1 -
        params.posterior_countersteer_gain * turn_request * beat_side
    tail_wave = -q1 -
        params.tail_lag_gain * qd1 / max(omega, eps(omega))
    tail_target = posterior_half_cycle_scale * tail_wave
