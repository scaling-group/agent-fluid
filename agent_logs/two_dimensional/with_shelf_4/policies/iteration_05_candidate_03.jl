function target_policy_params()
    return (
        ...
        response_divergence_scale=0.2,
        minimum_moment_authority=0.5,
...
function target_policy(state, params)
    ...
    # Treat a same-sign bearing error and bearing rate as a harmful response,
    # while an opposite-sign pair means that alignment is already improving.
    # Preserve a nonzero rejection floor, but avoid cancelling all beneficial
    # wake-assisted yaw solely because an instantaneous load is large.
    bearing_direction = tanh(bearing_error)
    response_divergence = bearing_direction * bearing_rate
    divergence_gate = 0.5 * (1 + tanh(
        response_divergence / params.response_divergence_scale,
    ))
    moment_authority = params.minimum_moment_authority +
        (1 - params.minimum_moment_authority) * divergence_gate

    # Keep the proven small normalized yaw-load residual as a disturbance
    # correction, not a route owner or an assumed external vortex phase.
    wake_turn = moment_authority * params.moment_rejection_gain * tanh(
