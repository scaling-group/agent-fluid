...
function target_policy(state, params)
    ...
    # The assigned parent's unconditional yaw damping reduced load and effort
    # but also delayed the route. Treat measured rotation as a disturbance only
    # after body-frame alignment and target-distance progress corroborate that
    # it is not the purposeful redirect. Both gates are normalized, bounded,
    # and vanish continuously when the fish is stalled or far off bearing.
    alignment_gate = 1 - tanh(abs(bearing_error))
    response_gate = closing_progress * alignment_gate
    yaw_damping_turn = params.yaw_rate_damping * response_gate * tanh(
