...
function target_policy(state, params)
    ...
    # Spend yaw-load rejection only when the observed route response is
    # adverse. The product below is positive exactly when the magnitude of
    # body-frame bearing is growing; a helpful or neutral wake-induced yaw
    # therefore cannot consume more half-cycle steering authority.
    bearing_divergence = tanh(
        max(bearing_error * bearing_rate, 0.0) /
        params.bearing_divergence_scale,
    )
    wake_turn = params.moment_rejection_gain * bearing_divergence * tanh(
