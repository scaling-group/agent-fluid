...
function target_policy(state, params)
    ...
    # Keep the evidenced yaw-load residual at full authority while target
    # distance is stalled or growing. When measured translation is already
    # targetward, relieve only half of this fast correction so useful wake yaw
    # is not cancelled indiscriminately; route bearing remains the slow owner.
    moment_gate = 1 - params.moment_progress_relief * closing_progress
    wake_turn = moment_gate * params.moment_rejection_gain * tanh(
