...
function target_policy(state, params)
    ...
    # The sampled two-joint action response produced a faster trajectory but
    # increased gait amplitude, effort, and load. Keep its stateful continuity
    # only at the route-owning anterior joint. The posterior joint directly
    # tracks the current lagged target so command history cannot add a second
    # phase delay to the thrust-producing traveling wave.
