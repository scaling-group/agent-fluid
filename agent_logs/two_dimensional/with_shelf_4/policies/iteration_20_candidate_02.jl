...
function target_policy(state, params)
    ...
    # but treat that response as a shared traveling-wave actuator. The total
    # signed power added by filter lag is normalized by the gait's natural
    # velocity-acceleration scale. One soft gate retains coherent continuity
    # for neutral or energy-removing lag and restores both raw phase commands
    # together when history injects net kinetic energy. This uses observed
    # action and dt, not a clock or mutable controller state.
    ...
    lag_power = (
        qd1 * (filtered_a1 - raw_a1) +
        qd2 * (filtered_a2 - raw_a2)
    ) / gait_power_scale
    excess_power = max(lag_power, 0.0) / params.lag_power_softness
    response_weight = inv(1 + excess_power^2)
    a1 = raw_a1 + response_weight * (filtered_a1 - raw_a1)
    a2 = raw_a2 + response_weight * (filtered_a2 - raw_a2)
