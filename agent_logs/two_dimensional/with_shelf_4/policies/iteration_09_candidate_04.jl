...
function target_policy(state, params)
    ...
    # Preserve the posterior-lagged traveling bend while distributing a small
    # share of the already bounded turn request to its useful half-cycle. This
    # adds posterior steering placement without a static-curvature offset or a
    # second route/disturbance residual, and returns exactly to the base target
    # when either the requested turn or the inferred beat side vanishes.
    base_tail_target = -q1 -
        params.tail_lag_gain * qd1 / max(omega, eps(omega))
    posterior_envelope = 1 +
        params.posterior_steering_share * turn_request * beat_side
    tail_target = posterior_envelope * base_tail_target
