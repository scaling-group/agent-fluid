...
function target_policy(state, params)
    ...
    # Preserve the firm large-error redirect, then release continuously into
    # stronger directional-response damping as instantaneous alignment returns.
    # This acts only while measured translation is targetward and does not
    # filter the route observation or infer a wake phase.
    alignment_gate = exp(-bearing_error^2)
    response_damping = params.bearing_rate_damping +
        params.alignment_rate_damping * alignment_gate
    route_turn = tanh(
        bearing_error +
        response_damping * closing_progress * bearing_rate,
    )
