...
function target_policy(state, params)
    ...
    # Coordinate slow route steering with the posterior actuator without
    # adding a static tail offset or weakening the traveling-wave lag. Infer
    # the desired tail-tangent half-cycle from joint state, then strengthen
    # only its route-consistent side. Fast moment rejection remains confined
    # to the evidenced anterior half-cycle envelope.
    ...
    tail_half_cycle_gain = 1 +
        params.tail_route_asymmetry * route_turn * tail_side
    tail_target = -q1 + tail_half_cycle_gain * tail_wave
