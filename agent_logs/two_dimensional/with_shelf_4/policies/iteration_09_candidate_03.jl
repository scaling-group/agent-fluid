function target_policy_params()
    return (
        ...
        tail_lag_gain=0.65,
        tail_damping=0.7,
        tail_half_cycle_asymmetry=0.12,
        moment_scale=0.08,
...
function target_policy(state, params)
    ...
    # Preserve the posterior-lagged traveling bend, but let the posterior joint
    # share a small amount of the proven target-signed half-cycle steering. The
    # envelope is inferred from the lagged tail state target, remains positive,
    # and is exactly neutral when no turn is requested.
    tail_wave = -q1 -
        params.tail_lag_gain * qd1 / max(omega, eps(omega))
    tail_side = tanh(
        tail_wave / (amp * params.phase_softness),
    )
    tail_half_cycle_envelope = 1 +
        params.tail_half_cycle_asymmetry * turn_request * tail_side
    tail_target = tail_wave * tail_half_cycle_envelope
