function target_policy_params()
    return (
        ...
        approach_amplitude_relief=0.25,
...
function target_policy(state, params)
    ...
    # Relieve propulsion only during a measured close approach.  Normalized
    # distance supplies a continuous proximity signal, while positive closing
    # progress prevents premature coasting when wake motion stalls the route.
    # The far-field scaffold is recovered continuously without a timed mode or
    # a fixed capture-radius threshold.
    distance_ratio = max(Float64(state.distance_L), 0.0) /
        max(params.approach_distance_scale, eps(Float64))
    approach_proximity = inv(1 + distance_ratio^2)
    approach_relief = params.approach_amplitude_relief *
        approach_proximity * closing_progress
    amp = cruise_amp * (1 - approach_relief)
    ...
    # Keep the sampled signed-power guard calibrated to cruise gait scale so
    # approach relief changes the oscillator envelope, not the guard threshold.
    gait_power_scale = max(omega^3 * cruise_amp^2, eps(Float64))
