...
function target_policy(state, params)
    ...
    wake_turn = params.moment_rejection_gain * moment_signal
    turn_request = clamp(route_turn + wake_turn, -1.0, 1.0)

    # Infer the active beat side from joint state and strengthen only the
    # requested half-cycle. Yield a bounded fraction of common gait amplitude
    # to strong measured yaw load instead of adding another disturbance route
    # term. This preserves zero mean, the signed direct moment correction, and
    # the target-directed asymmetry while reducing active work during a large
    # hydrodynamic torque.
    phase_coordinate = (q1 + qd1 / max(omega, eps(omega))) / amp
    beat_side = tanh(phase_coordinate / params.phase_softness)
    load_compliance = clamp(params.load_compliance, 0.0, 0.5)
    compliant_amplitude = amp * (1 - load_compliance * abs(moment_signal))
    half_cycle_amplitude = compliant_amplitude * (
