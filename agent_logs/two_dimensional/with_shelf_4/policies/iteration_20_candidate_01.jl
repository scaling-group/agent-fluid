...
function target_policy(state, params)
    ...
    # but treat that response as a shared traveling-wave actuator. Observe
    # positive lag-added power at each joint before aggregation: dissipation at
    # one joint must not conceal stale-history injection at the other. One soft
    # gate then restores both raw phase commands together, preserving their
    # posterior relationship instead of assigning unequal response weights.
    ...
    positive_lag_power = max(lag_power1, 0.0) + max(lag_power2, 0.0)
    excess_power = positive_lag_power / params.lag_power_softness
