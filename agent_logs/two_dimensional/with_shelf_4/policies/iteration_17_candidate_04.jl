...
function target_policy(state, params)
    ...
    # Use the sampled anterior action continuity only as a redirect mechanism.
    # Large target misalignment with no measured closing progress retains its
    # useful history-shaped response; alignment or established translation
    # releases the joint continuously to the direct oscillator acceleration.
    # Posterior tracking remains direct so this gate cannot add a tail phase.
    ...
    history_shaped_a1 = previous_a1 +
        response_fraction * (raw_a1 - previous_a1)
    redirect_gate = tanh(abs(bearing_error)) * (1 - closing_progress)
    a1 = raw_a1 + redirect_gate * (history_shaped_a1 - raw_a1)
