...
function target_policy(state, params)
    ...
    moment_z = Float64(state.moment_z_L2)
    wake_turn = params.moment_rejection_gain * tanh(
        moment_z / params.moment_scale,
    )
    ...
    # Fixed command memory changed the oscillator phase and increased gait
    # amplitude and load in the sampled rollouts. Keep continuity when yaw
    # load is small, but release it smoothly toward the current oscillator
    # command during strong measured wake/turn events. The posterior joint
    # continues to track its lagged target directly, so neither branch adds a
    # second phase delay to the thrust-producing traveling wave.
    history_dt = max(Float64(state.history_dt), 0.0)
    base_response_fraction = history_dt > 0.0 ?
        -expm1(-history_dt / params.actuator_response_time) : 1.0
    load_release = tanh(
        abs(moment_z) / max(params.filter_load_scale, eps(Float64)),
    )
    response_fraction = base_response_fraction +
        load_release * (1 - base_response_fraction)
    previous_a1 = Float64(state.previous_action[1])
    a1 = previous_a1 + response_fraction * (raw_a1 - previous_a1)
