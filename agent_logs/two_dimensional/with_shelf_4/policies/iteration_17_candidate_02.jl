...
function target_policy(state, params)
    ...
    # The sampled course-alignment residual improved distance on the same
    # direct-action route scaffold.  Compare target bearing with the observed
    # body-translation direction, and admit this correction only after
    # positive closing speed confirms that the motion represents approach.
    # This is normalized body-frame feedback, not a world-frame route.
    ...
    # Retain the evidenced stateful continuity only at the route-owning
    # anterior joint.  The posterior acceleration directly tracks the current
    # lagged target, avoiding a second history-dependent phase change in the
    # thrust-producing traveling wave.
