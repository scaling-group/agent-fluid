...
function target_policy(state, params)
    ...
    # Increase only that convergence damping on approach; the smooth normalized
    # distance schedule is weak far away and introduces no target-specific
    # threshold or route.
    ...
    approach_weight = params.approach_distance_scale_L / max(
        params.approach_distance_scale_L + distance_L,
        eps(params.approach_distance_scale_L),
    )
    bearing_rate_weight = params.bearing_rate_damping +
        params.approach_rate_boost * approach_weight
    route_turn = tanh(
        bearing_error + bearing_rate_weight * bearing_rate,
    )
