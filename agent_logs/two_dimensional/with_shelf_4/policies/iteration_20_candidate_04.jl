...
function target_policy(state, params)
    ...
    # treating the response as a coupled traveling-wave actuator. Normalize the
    # residual and joint velocity by their gait scales, then remove only the
    # residual component that adds positive total joint power. The orthogonal
    # component preserves phase-shaping continuity without adding instantaneous
    # kinetic power. This uses observed action and dt, not a clock or mutable
    # controller state.
    ...
    velocity_scale = max(omega * amp, eps(Float64))
    acceleration_scale = max(omega^2 * amp, eps(Float64))
    velocity1 = qd1 / velocity_scale
    velocity2 = qd2 / velocity_scale
    residual1 = (filtered_a1 - raw_a1) / acceleration_scale
    residual2 = (filtered_a2 - raw_a2) / acceleration_scale

    lag_power = velocity1 * residual1 + velocity2 * residual2
    positive_power = max(lag_power, 0.0)
    removal_fraction = positive_power^2 / (
        params.lag_power_softness^2 + positive_power^2
    )
    velocity_squared = velocity1^2 + velocity2^2
    projection_gain = removal_fraction * positive_power /
        max(velocity_squared, eps(Float64))
    safe_residual1 = residual1 - projection_gain * velocity1
    safe_residual2 = residual2 - projection_gain * velocity2

    a1 = raw_a1 + acceleration_scale * safe_residual1
    a2 = raw_a2 + acceleration_scale * safe_residual2
