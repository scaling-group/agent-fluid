...
function target_policy(state, params)
    ...
    closing_progress = tanh(
        max(closing_speed, 0.0) / params.closing_speed_scale,
    )

    # Preserve the complete sampled route request during useful closure. Only
    # measured distance regression recruits a little more of the existing
    # anterior bearing authority, and the boost vanishes smoothly as soon as
    # neutral or positive targetward translation returns.
    retreat_response = tanh(
        max(-closing_speed, 0.0) / params.closing_speed_scale,
    )
    route_error_gain = 1 + params.retreat_route_boost * retreat_response
    route_turn = tanh(
        route_error_gain * bearing_error +
