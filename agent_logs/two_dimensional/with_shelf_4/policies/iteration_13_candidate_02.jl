...
function target_policy(state, params)
    ...
    # Persistent body-frame target geometry owns the route. Unlike the
    # testbed's compact bearing, this signed two-argument angle retains whether
    # the target is ahead or behind after a wake-driven rotation; negative body
    # x is the fish's forward direction. Qualify the established windowed-rate
    ...
    target_bearing = atan(target_lateral, target_forward)
    bearing_error = target_bearing / params.bearing_scale
    bearing_rate = tanh(
