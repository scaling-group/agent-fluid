...
function target_policy(state, params)
    ...
    # Retain full direct load rejection when hydrodynamic torque feeds the
    # observed body spin. If torque already opposes the spin, it is passive yaw
    # damping; discount only that portion rather than rejecting useful wake
    # work. The bounded power signature uses no external vortex phase.
    moment_signal = tanh(
        Float64(state.moment_z_L2) / params.moment_scale,
    )
    yaw_rate_signal = tanh(
        Float64(state.heading_rate) /
        max(params.yaw_rate_scale, eps(Float64)),
    )
    dissipative_yaw_power = max(-moment_signal * yaw_rate_signal, 0.0)
    passive_discount = clamp(params.passive_damping_discount, 0.0, 1.0)
    moment_authority = 1 - passive_discount * dissipative_yaw_power
    wake_turn = params.moment_rejection_gain * moment_authority * moment_signal
