...
function target_policy(state, params)
    ...
    closing_progress = tanh(
        max(window_closing_speed, 0.0) / params.closing_speed_scale,
    )
    route_turn = tanh(
        bearing_error +
    ...
    # requested half-cycle. If a large body-frame bearing is accompanied by
    # measured opening rather than closing, add a small route-only burst to the
    # existing asymmetry. The burst vanishes continuously once translation is
    # useful, and it does not attenuate the direct wake residual or move route
    # authority into the posterior joint.
    phase_coordinate = (q1 + qd1 / max(omega, eps(omega))) / amp
    beat_side = tanh(phase_coordinate / params.phase_softness)
    route_misalignment = abs(tanh(bearing_error))
    opening_progress = tanh(
        max(-window_closing_speed, 0.0) / params.closing_speed_scale,
    )
    steering_asymmetry = params.half_cycle_asymmetry * (
        turn_request +
        params.redirect_asymmetry_boost *
        route_misalignment * opening_progress * route_turn
    )
    half_cycle_amplitude = amp * (
        1 + steering_asymmetry * beat_side
    )
