...
function target_policy(state, params)
    ...
    # Persistent body-frame bearing owns the route. Admit its windowed-rate
    # damping only when target-distance closure and correctly signed forward
    # alignment agree. Bearing convergence alone can be body spin, while
    # closure with a lateral/aft target still calls for a firm redirect.
    ...
    response_gate = closing_progress * forward_alignment
    route_turn = tanh(
        bearing_error +
