function target_policy_params()
    return (
        ...
        approach_distance_scale_L=2.0,
        approach_route_relief=0.3,
...
function target_policy(state, params)
    ...
    unrelieved_route_turn = tanh(
        bearing_error +
        params.bearing_rate_damping * closing_progress * bearing_rate,
    )

    # Relieve target-route authority only during a confirmed near approach.
    # Proximity cannot create a coast mode: if target-distance closure stops,
    # the full instantaneous-bearing request returns continuously. Keep the
    # faster yaw-load residual outside this terminal route schedule.
    distance_ratio = max(Float64(state.distance_L), 0.0) /
        params.approach_distance_scale_L
    approach_proximity = 1 / (1 + distance_ratio^2)
    approach_relief = params.approach_route_relief *
        closing_progress * approach_proximity
    route_turn = (1 - approach_relief) * unrelieved_route_turn
