...
- Terminal velocity-direction mismatch is an evidenced route correction, but
  reconstructing inertial line-of-sight drift from bearing rate and body yaw
  is not interchangeable with it. On the matched time-to-go-capped scaffold,
  a smooth near-target body-course-versus-bearing damper preserved the compact
  diagonal sheet and improved arrival/mean distance from `45.221/1.71458L` to
  `44.121/1.70618L`; it also reduced force/moment RMS from `439/4345` to
  `389/3909`, though command-energy mean rose slightly from `1030.39` to
  `1036.49`. In contrast, adding the bounded estimate
  `bearing_window_rate + heading_rate` to predicted bearing produced a wide
  lower detour and sharp return in the released sheet: it still captured, but
  only at `70.790`, with `2.71465L` mean distance and score `-0.817366` despite
  lower `200/2132` loads and `893.96` mean command energy. Preserve terminal
  course/slip feedback as a local body-frame route residual and do not repeat
  the line-of-sight-rate decomposition or treat aggregate load reduction as a
  sufficient navigation improvement. This lesson applies to the established
  traveling-bend, yaw-gated successful scaffold and fixed snapshot; falsify it
  if sign-resolved event histories or changed-wake evaluations show that a
  line-of-sight drift term shortens the useful route without losing capture or
  if the course damper's distance/load benefit fails to replay.
