...
- Treat posterior propulsion earned by target progress as a small
  ...
  test body-frame course convergence before amplifying it further. This lesson
  applies only after route alignment, upstream propulsion, and yaw-gated target
  success exist; the course-convergence extension remains a hypothesis until
  a later rollout improves arrival or distance history without worse topology,
  saturation, or load.
- Gate a course-conditioned posterior residual asymmetrically: actuator and
  disturbance feedback may withdraw optional positive amplification, but must
  not erase negative course correction. From the ungated successful controller
  (`45.61`, `1.7222L` mean distance, `46916` energy, `453/4406` force/moment
  RMS), positive yaw-only allocation reached at `45.10` with `1.7191L`, `46677`,
  and `416/4183`; compounding yaw and oscillator-normalized joint-rate headroom
  produced the strongest sampled result at `45.26`, `1.7153L`, score `0.165009`,
  `46642`, and `439/4342`. In contrast, replacing yaw separation with a
  near-target distance gate reached only at `45.61` and raised loads to
  `475/4653`, while whole-signed yaw gates repeated the weaker `45.84`,
  `1.7300L`, `460/4433` class. Preserve yaw separation and full negative
  suppression; do not treat proximity as actuator headroom. A further gate is
  justified only by a distinct normalized precursor such as body-relative
  disturbance flow, and it should act on the positive optional branch rather
  than the base wave. This applies only after upstream propulsion and target
  steering are proven, and is falsified if capture, diagonal topology, or the
  distance/load tradeoff regresses under replay or changed wake conditions.
