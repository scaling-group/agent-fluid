...
- Once the traveling bend and separated steering/propulsion authorities already
  capture the target, cap heading-rate extrapolation by remaining normalized
  approach time instead of projecting the present turn past the target. The
  distinct sampled parent without this cap reached at `45.2595` with score
  `0.165009`, mean distance `1.71529L`, command-energy mean `1030.54`, and
  `438.82/4341.50` force/moment RMS. Three exact capped-policy materializations
  preserved its visible diagonal trajectory and improved arrival, score, and
  mean distance to `45.2210`, `0.165860`, and `1.71458L`, with essentially
  unchanged effort (`1030.39`) and slightly higher `439.16/4344.77` loads.
  Preserve this as a terminal route-response scheduler, not a tail-motion
  distance gate: the inherited distance-gated propulsion variant instead
  returned to about `45.61/1.721L` and raised loads to `475/4653`. This lesson
  applies only after reliable target capture and is falsified if a genuinely
  changed wake or matched policy comparison loses capture or the small distance
  benefit without compensating effort/load reduction; fixed-snapshot duplicates
  do not establish robustness or a load benefit.
