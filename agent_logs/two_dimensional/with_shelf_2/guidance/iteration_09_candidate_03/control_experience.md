...
- Alignment-conditioned posterior wave emphasis is a positive propulsion
  ...
  missed below and collided. This lesson applies only after target success,
  a coherent traveling wave, and yaw-load steering moderation are present;
  falsify it if the closure-efficiency advantage fails to reproduce, or if a
  phase/headroom follow-up raises saturation or load without earlier capture
  or lower distance history.
