...
- Instantaneous heading-rate lookahead on that distributed scaffold is a mixed
  ...
  load. When the distributed gait already provides upstream travel, preserve
  its response-aided route command and next test a smooth normalized
  force/moment-magnitude gate on steering only; reject that gate if it loses the
  `4.62L` approach baseline or fails to reduce load and the downward-exit
  topology, then require sign-resolved evidence before trying a directional
