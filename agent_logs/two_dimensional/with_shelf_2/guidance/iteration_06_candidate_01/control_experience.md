...
- Exact repeated outcomes from functionally identical candidates establish a
  materialization baseline, not wake robustness. Three sampled workers used
  the same response-aware, yaw-load-gated distributed half-cycle law (one file
  differed only in comments) and each reproduced `target_reached` at `51.47`,
  `0.748L` final/minimum and `1.82L` mean distance, `-11.28/-4.94L` head
  displacement, and `426/4084` force/moment RMS. Together with the inherited
  notes, this supports preserving that controller as the deterministic
  reference instead of treating duplicate samples as evidence for another
  scalar gain or signed disturbance edit. This lesson applies when policy
  expressions and initial wake snapshots are functionally identical; it is
  falsified as a robustness claim until a changed wake phase, inflow, geometry,
  target, or genuinely different policy output also succeeds.
