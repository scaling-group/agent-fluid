...
- A floor-bounded hydrodynamic-load gate can rescue response-aware half-cycle
  steering when the ungated controller reaches the target neighborhood but
  then amplifies a wake/body yaw event. `solver_a84fba8bf04f` used heading-rate
  prediction to approach within `1.65L`, yet folded into a lower exit after
  `75.09` units with `-13.23L` lateral displacement and force/moment RMS
  `487/4680`. Keeping that route controller but attenuating only its steering
  residual with `abs(moment_z_L2)` in `solver_e6805fe0c5bc` produced the
  lineage's first target capture after `51.47` units, improved minimum distance
  to `0.748L` and progress to `0.940`, limited lateral displacement to
  `-4.94L`, and reduced loads to `426/4084`. Preserve the zero-centered wave,
  response-aware route request, nonzero steering floor, and load separation;
  do not replace this success with the sampled posterior-curvature design,
  which lowered loads to `116/1278` but missed at `2.43L` on a wide oscillatory
  path. The success still reached both joint-velocity caps and about `0.72 rad`
  anterior bend, so a complementary normalized force/moment gate is a valid
  next test only if capture and upstream displacement are retained. If it loses
  capture or fails to reduce cap/load symptoms, revert to moment-only gating
  and require directional time-history evidence before adding a signed wake
  residual.
