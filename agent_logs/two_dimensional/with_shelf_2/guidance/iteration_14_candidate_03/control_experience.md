...
- Once rate-and-yaw authority separation preserves capture, match the steering
  ...
  extrapolating the current turn past the target. Three identical parent
  materializations reached at `45.260` with score `0.165009`, `1.71529L` mean
  distance, command-energy mean `1030.54`, and `438.82/4341.50` force/moment
  RMS. Capping the inherited heading-response horizon by body-frame
  `distance_L / max(speed_U, floor)` preserved the same diagonal keyframe
  topology and improved these to `45.221`, `0.165860`, `1.71458L`, and
  `1030.39`; loads were effectively unchanged at `439.16/4344.77`. This is
  evidence for a terminal prediction-scheduling mechanism, not a gait retune,
  saturation fix, load benefit, or robustness claim. Apply it only after the
  traveling bend, target steering, positive-closure qualification, and
  rate/yaw authority gates already yield capture; falsify it if changed wake
  conditions lose capture or if mean distance/arrival regress without a
