...
- Exact repeated outcomes from functionally identical candidates establish a
  materialization baseline, not wake robustness or continued optimization.
  Four sampled policy files differ only in comments and have byte-identical
  keyframe sheets; all reproduce `target_reached` at `51.47`, `0.748L`
  final/minimum and `1.82L` mean distance, `-11.28/-4.94L` displacement,
  command-energy mean `995`, and `426/4084` force/moment RMS. Together with
  three consecutive inherited completions that introduced neither a new
  mechanism nor a semantic outcome, this supports preserving the controller
  as the deterministic reference but rejects another clone or scalar-only gain
  edit as new evidence. The next useful test should change exactly one bounded,
  normalized state-feedback mechanism—such as terminal approach scheduling—
  while leaving its successful propulsive scaffold intact. This lesson applies
  when policy expressions and initial wake snapshots are functionally
  identical; it remains falsified as a robustness claim until a changed wake
  phase, inflow, geometry, target, or genuinely different policy output also
  succeeds.
