...
- Once a time-to-go-capped heading predictor preserves the successful diagonal
  ...
  refinements reject four seemingly compatible extensions: sharing the
  ...
  forecast reaches at `45.331/1.72584L`. More broadly, an independent signed
  line-of-sight-rate forecast creates a long down-then-up loop, delaying
  capture to `70.790`, worsening mean distance to `2.71465L` and score to
  `-0.817366`; its lower `200/2132` force/moment RMS is not a useful trade for
  the corrupted route. Preserve the fixed independently bounded course
  residual; do not strengthen it from closure, merge it into a common angular
  budget, arbitrate away yaw prediction merely because two observed responses
  oppose, or add a parallel signed target-rate predictor without event-level
  directional evidence. This applies only after upstream
...
- Replacing the independently distance-gated raw course-angle mismatch with
  its sine is a concrete negative representation result on the successful
  scaffold. Against four byte-identical raw-angle replays at
  `44.121/1.70618L/0.173450` arrival, mean distance, and score with `389/3909`
  force/moment RMS, the assigned parent's sine variant preserved capture and
  the broad diagonal keyframe topology but regressed to
  `46.074/1.73655L/0.144279`, traveled farther downward (`-4.574L` versus
  `-4.263L`), and raised loads to `407/4002`; the only benefit was a small
  mean-command-energy reduction from `1036.49` to `1028.72`. Preserve the raw
  bounded angle as the evidenced terminal course residual instead of
  attenuating large slip through a sine or another weaker representation.
  This applies to the same independent `2L` distance gate and successful
  route scaffold; falsify it if a different slip representation improves
  arrival or mean distance, or materially reduces load/effort without losing
  capture or deepening the terminal excursion.
