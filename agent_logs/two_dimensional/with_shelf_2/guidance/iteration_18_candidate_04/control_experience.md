...
- After the terminal time-to-go cap is established, normalized body-course
  ...
  Do not subsequently force the heading-rate and course responses through one
  symmetric angular budget. Relative to the independently additive course
  damper, that matched shared-cap variant still captured but worsened
  arrival/mean-distance/score from `44.121/1.70618L/0.173450` to
  `45.727/1.72692L/0.153609` and raised force/moment RMS from `389/3909` to
  `449/4408`; its lower mean command energy (`1025.91` versus `1036.49`) did
  not offset the deeper terminal excursion. Preserve aligned cue authority. If
  route-response interaction is tested again, confine it to opposing cues and
  leave the additive aligned branch intact. This negative boundary applies to
  the same distance-gated course damper and fixed prewarm snapshot; falsify it
  if a matched shared-budget design reduces loads or effort without regressing
  capture, arrival, mean distance, or the diagonal topology.
- Once the additive near-target body-course damper is established, neither
  closure-based gain scheduling nor one-sided heading/course arbitration is an
  evidenced refinement. Relative to the assigned parent's
  `44.121/1.70618L/0.173450` arrival, mean distance, and score with `389/3909`
  force/moment RMS, strengthening only the course residual under positive
  normalized closure regressed to `45.689/1.71923L/0.161543` with `396/3917`
  loads. Attenuating extrapolated heading response only when it opposed the
  measured course response regressed to `45.331/1.72584L/0.154397` with
  `413/4089` loads. Both still captured and retained the diagonal wake-entry
  topology, but their keyframe sheets show a deeper terminal trough, so
  neither lower mean command energy (`1031.20` and `1034.31` versus `1036.49`)
  compensates for worse tracking. Preserve the independent additive cue
  responses and avoid further authority or arbitration variants without
  sign-resolved histories. Test actuator-side residual allocation separately;
  all three policies touch the exact `4.537856` rate cap on both joints, which
  is evidence for a headroom hypothesis but not proof that steering owns the
  saturation. This boundary applies to the same successful scaffold and fixed
  prewarm snapshot; falsify it if a matched cue-interaction design improves
  arrival, mean distance, or loads without deepening the terminal trajectory.
