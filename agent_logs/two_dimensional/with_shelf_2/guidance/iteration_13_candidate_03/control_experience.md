...
- Treat posterior propulsion earned by target progress as a small
  ...
  residual rather than making stalls or reversals earn more tail motion, and
  qualify any further posterior amplification by body-frame course convergence.
  This lesson applies only after route alignment, upstream propulsion, and
  yaw-gated target success exist.
- Once course convergence qualifies the small positive-closure tail residual,
  preserve its negative branch and gate only positive amplification when
  actuator usage or yaw response is already large. The unconditioned course
  residual reached at `45.61` with `1.7222L` mean distance and `453/4406`
  force/moment RMS; yaw-only positive gating reached at `45.11`, `1.7191L`, and
  `416/4183`; joint-rate-headroom plus yaw gating produced the strongest sample
  at `45.26`, `1.7153L`, `439/4342`, and score `0.1650`. Adding a near-distance
  gate instead delayed capture to `45.61`, regressed to `1.7208L`, and raised
  load to `475/4653`. Prefer asymmetric rate-and-yaw headroom allocation over
  whole-signed or distance-gated attenuation; falsify this ranking if a changed
  wake, geometry, target, or replay loses capture or reverses its distance/load
  advantage.
- Do not add an inferred translation component of bearing rate directly to the
  target-steering request merely because it reduces hydrodynamic load. The
  inherited course-over-ground look-ahead retained capture and lowered
  force/moment RMS from `453/4406` to `272/2896`, but it delayed arrival from
  `45.61` to `45.72`, worsened mean distance from `1.7222L` to `1.7666L`, and
  reduced score from `0.1588` to `0.1142`. Treat line-of-sight drift as an
  uncalibrated mixture of translation and wake-driven motion until sign-resolved
  event histories show otherwise; future use should test it as a bounded
  authority gate, not an additive turn direction. This boundary is falsified
  if a direct drift term improves distance history or termination semantics
  without sacrificing the proven diagonal topology.
