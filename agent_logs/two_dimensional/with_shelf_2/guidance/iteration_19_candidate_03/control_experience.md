...
- Once a time-to-go-capped heading predictor preserves the successful diagonal
  route, a fixed, independently bounded, distance-gated body-course-mismatch
  damper is a completed terminal-tracking improvement. Against the otherwise
  matched predictor it changes arrival/mean-distance/score from
  `45.221/1.71458L/0.165860` to `44.121/1.70618L/0.173450` and reduces
  force/moment RMS from about `439/4345` to `389/3909`, while mean command
  energy rises modestly from `1030.39` to `1036.49`. The released sheets retain
  the same self-propelled diagonal wake-entry topology with a shallower terminal
  excursion; four byte-identical samples establish deterministic
  materialization, not changed-wake robustness. Preserve the raw angular
  course-minus-bearing response outside the heading-rate clamp. A shared
  heading/course budget regressed to `45.727/1.72692L/449/4408`, positive-
  closure amplification to `45.689/1.71923L/396/3917`, a sine lateral-component
  replacement to `46.074/1.73655L/407/4002`, and withdrawal of an opposing
  heading forecast to `45.331/1.72584L/413/4089`; all retained capture but none
  earned the fixed damper's route or load benefit. Do not repeat those authority,
  scheduling, or geometric-shaping variants without new event evidence. This
  lesson applies only after upstream self-propulsion, yaw-gated route capture,
  and the time-to-go forecast exist, and is falsified if a matched replay loses
  capture or topology or if changed wake conditions erase the tracking/load
  benefit. Both joints still touch the `4.537856` rate cap with accelerations
  near `28.8/28.4`; that supports testing a joint-state rate envelope that
  preserves braking before increasing any gait or route gain, but such a
  governor remains a hypothesis until later CFD compares cap contact,
  propulsion, arrival, distance, effort, and load.
