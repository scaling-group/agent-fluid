...
- Once the yaw-gated distributed controller has a repeatable target-reaching
  route, predicted-bearing alignment can gate a small posterior-wave emphasis
  as a distinct propulsion mechanism. Relative to three exact replays of the
  assigned parent (`target_reached` at `51.47`, mean distance `1.82L`,
  force/moment RMS `426/4084`), the sampled alignment-gated posterior target
  preserved the diagonal topology, reached at `46.80`, and improved mean
  distance to `1.745L`; posterior peak bend rose from about `0.659` to `0.675`
  rad while anterior peak bend fell from about `0.721` to `0.704` rad. The
  tradeoff was modestly higher command-energy mean (`1024` versus `995`) and
  load (`441/4259`). By contrast, changing command composition with a
  propulsive-priority residual allocator missed below the target and collided
  with the lower second-row cylinder at `58.93` (`1.87L` closest,
  force/moment RMS `537/4995`) despite lower mean command energy. Preserve the
  successful sum-then-limit steering scaffold and prefer alignment-conditioned
  posterior emphasis over that allocator when the route already works. This
  implication applies only after correct target response and upstream
  propulsion are established; falsify it if replay loses capture or the
  arrival/mean-distance gain, or if added posterior motion raises limit
  residence or load without useful progress. Same-snapshot repetition is not
  wake-phase or geometry robustness evidence.
