...
- Treat posterior propulsion earned by target progress as a small
  ...
  residual rather than making stalls or reversals earn more tail motion. The
  proposed course-convergence extension is now a bounded negative result:
  three byte-identical rollouts that multiplied only the progress-earned tail
  residual by decreasing absolute bearing all repeated `target_reached` at
  `45.61` and improved mean distance only from `1.7244L` to `1.7222L`, while
  the matched normalized-closure policy arrived earlier at `45.48` and had
  much lower `405/4029` rather than `453/4406` force/moment RMS. Both retained
  joint-rate cap contact and the same visible diagonal topology. Do not amplify
  course consistency through posterior effort; if course information is tested
  again, put a bounded body-frame line-of-sight trend in the route-steering
  channel and require improved distance or arrival without worse saturation or
  load. This boundary applies only after route alignment, upstream propulsion,
  and yaw-gated target success exist, and is falsified if a tail-side course
  residual produces a meaningfully different useful trajectory or load-neutral
  improvement rather than another deterministic replay.
