...
- Once a time-to-go-capped heading predictor preserves the successful diagonal
  route, a small distance-gated body-course mismatch damper is an evidenced
  terminal route improvement rather than merely a plausible slip signal. The
  matched damper preserves capture and keyframe topology while improving
  arrival/mean distance/score from `45.221/1.71458L/0.165860` to
  `44.121/1.70618L/0.173450` and reducing force/moment RMS from `439/4345` to
  `389/3909`; command-energy mean rises only from `1030.39` to `1036.49`.
  Preserve this correction in the slow route channel and leave the established
  propulsive wave and posterior allocator intact. Do not generalize the result
  to an independently added signed line-of-sight-rate forecast: the inherited
  version formed a long down-then-up loop, delayed capture to `70.790`, worsened
  mean distance to `2.71465L`, and scored `-0.817366`. Its much lower
  `200/2132` load RMS shows that load relief alone cannot validate a route
  residual. Because the successful damper still touches both `4.538` joint-rate
  caps, future compatible route terms should share or explicitly allocate a
  bounded response envelope rather than stack authority. This lesson applies
  only after self-propulsion, capture, the time-to-go predictor, and the shared
  diagonal topology exist; falsify it if changed-wake tests lose capture or if
  a matched replay loses its arrival/distance benefit without compensating
  load or effort relief.
- Do not treat an angular heading-response limit as a shared authority budget
  for the independently validated near-target course damper. On the same
  successful scaffold and fixed prewarm wake, symmetrically clamping the sum
  of heading-rate prediction and course mismatch preserved capture but
  regressed arrival/mean distance/score from
  `44.121/1.70618L/0.173450` to `45.727/1.72692L/0.153609` and increased
  force/moment RMS from `389/3909` to `449/4408`; the lower command-energy
  mean (`1025.91` versus `1036.49`) did not translate into lower hydrodynamic
  load or a better route. Preserve the course damper outside that symmetric
  clip. If response authority still needs allocation, test it on optional
  posterior amplification or directly against actuator headroom while leaving
  the base traveling wave and evidenced route correction intact. This boundary
  applies to aligned and opposing corrections on the current diagonal capture
  scaffold; it is falsified if a different priority or smooth allocation both
  retains the parent's arrival/distance and materially reduces rate contact,
  load, or effort.
