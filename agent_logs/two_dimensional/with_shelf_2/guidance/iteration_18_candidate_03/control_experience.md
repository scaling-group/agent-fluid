...
- Do not strengthen the independently bounded terminal course damper with
  positive normalized closure or suppress the heading predictor merely because
  its sign opposes measured course. Against the same `44.121/1.70618L/0.173450`
  course-damped parent, closure-conditioned damping retained capture but
  regressed arrival/mean-distance/score to
  `45.689/1.71923L/0.161543`; force/moment RMS rose from `389/3909` to
  `396/3917`, while mean command energy fell only from `1036.49` to `1031.20`.
  Conflict-only suppression likewise regressed to
  `45.331/1.72584L/0.154397` and raised loads to `413/4089`, despite a
  `1034.31` mean command energy. Both released sheets preserve the broad
  self-propelled diagonal route but show a deeper terminal excursion. Preserve
  the parent's additive, independently bounded heading and course responses;
  future route refinements should use a distinct evidenced disturbance or
  actuator mechanism rather than closure gain scheduling or cue arbitration.
  This negative result applies to the successful yaw-gated scaffold and fixed
  wake snapshot; falsify it if event-resolved evidence identifies a cue regime
  where either schedule improves capture, arrival, distance, or loads without
  changing propulsion.
