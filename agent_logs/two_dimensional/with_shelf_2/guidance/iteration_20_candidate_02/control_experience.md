...
- Once a time-to-go-capped heading predictor preserves the successful diagonal
  ...
  forecast reaches at `45.331/1.72584L`. Two later matched representation and
  scheduling changes tighten this negative boundary. Compressing the angular
  course mismatch to its sine delays capture to `46.074`, worsens mean distance
  to `1.73655L`, and raises loads to `407/4002` despite lowering mean command
  energy to `1028.72`; replacing the fixed range gate with a body-speed-derived
  time-to-go onset reaches at `45.975/1.73033L` with still higher `455/4405`
  loads. Both retain the same diagonal capture and the joint-rate caps rather
  than introducing a useful trajectory. Preserve the raw angular mismatch,
  fixed distance onset, and independently bounded course residual; do not
  strengthen it from closure, nonlinear-compress it, speed-schedule it, merge
  it into a common angular budget, or arbitrate away yaw prediction merely
  because the two observed responses oppose. After these repeated same-topology
  regressions, test a distinct actuator-side state-feedback mechanism before
  another course-residual representation or gain edit. This applies only after
  upstream self-propulsion, target capture, the time-to-go predictor, and the
  established wake-entry topology exist; falsify it if a changed-condition
  replay loses capture, or if a different course representation produces a
  meaningful new trajectory, load reduction, or robustness gain without losing
  capture. Fixed-prewarm replication is not wake robustness.
