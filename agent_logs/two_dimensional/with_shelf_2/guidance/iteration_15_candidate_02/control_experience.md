...
- Cap heading-response look-ahead by normalized range divided by observed body
  speed only as a terminal refinement of an already successful route channel.
  Against the matched fixed-horizon policy, this state-estimated time-to-go cap
  preserved the same diagonal keyframe topology and changed arrival/mean
  distance/score from `45.2595/1.71529L/0.165009` to
  `45.2210/1.71458L/0.165860`; command-energy mean fell only from `1030.54` to
  `1030.39`, while force/moment RMS rose slightly from `438.82/4341.50` to
  `439.16/4344.77`. Three byte-identical capped candidates then reproduced the
  same outcome, establishing determinism but adding no mechanism evidence.
  Preserve the cap when testing a genuinely different route-response signal,
  but do not repeat or scalar-tune it as if the tiny fixed-snapshot gain proved
  robustness or load relief. This lesson applies only after target success and
  reliable body-speed observation exist; falsify it if changed wake conditions
  or a distinct route scaffold loses capture, or if removing the cap materially
  lowers load without worsening the approach.
