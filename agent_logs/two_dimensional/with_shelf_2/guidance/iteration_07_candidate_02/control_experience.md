...
- Separate the propulsive wave from hydrodynamic-load gating once distributed
  route steering works. Relative to the inherited ungated heading-response
  controller—which approached to `1.65L` but folded into a lower exit at
  `75.09` with force/moment RMS `487/4680`—the absolute-yaw-moment-gated
  distributed controller crossed the `0.75L` target radius at `51.47`, held a
  `1.82L` mean distance, and reduced RMS loads to `426/4084`. Four independent
  same-seed samples reproduce its trajectory and scalar metrics exactly, so
  the result supports a smooth `abs(moment_z_L2)` gate on only the steering
  residual and establishes deterministic materialization, not robustness to
  wake phase, inflow, geometry, or target changes. Do not infer a signed
  disturbance residual from aggregate RMS evidence; require event-level sign
  correlation first. Because the successful samples still reach both
  `4.5379 rad/time` joint-speed caps and nearly exhaust their acceleration soft
  limit, refine them with actuator-state-aware authority allocation rather
  than larger scalar gains; falsify such a refinement if capture or the direct
  diagonal approach is lost without reducing cap contact or loads.
