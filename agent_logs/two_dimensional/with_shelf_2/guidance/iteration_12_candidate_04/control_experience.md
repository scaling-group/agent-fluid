...
- Closure-conditioned posterior phase is not interchangeable with the proven
  closure-conditioned amplitude residual. On the same normalized-positive-
  closure scaffold, a small aligned/efficient-closure lag increase with
  analytic harmonic-amplitude compensation preserved `target_reached` but
  regressed arrival/mean distance/score from `45.48/1.724L/0.156386` to
  `46.22/1.735L/0.146573`; force/moment RMS rose from `405/4029` to
  `424/4253`, mean command energy remained essentially unchanged (`1034` to
  `1036`), and both variants still touched both joint-rate caps. Treat this as
  a concrete negative result for further lag-only tuning after the diagonal
  capture topology already works: preserve the posterior timing unless
  beat-resolved or sign-resolved evidence identifies a timing deficit, and
  test bounded authority separation or load gating instead. This boundary is
  falsified only by a timing change that improves arrival or distance without
  losing capture or increasing saturation/load burden; a different gain alone
  is not a new mechanism.
- Feedback gates on a small course residual are not interchangeable: the gate
  signal must match the authority being protected. Relative to the bounded
  course-convergence controller (`45.61`, score `0.158830`, `1.72215L` mean
  distance, and `453/4406` force/moment RMS), reusing its yaw-moment steering
  gate on the course residual retained capture but regressed to `45.84`, score
  `0.150759`, `1.72999L`, and higher `460/4433` loads. Gating only positive
  course amplification by joint-rate headroom instead preserved the visible
  diagonal topology, reached fastest at `45.10`, held mean distance to
  `1.72267L`, and reduced loads to `414/4104`. Keep worsening-course
  suppression intact and use normalized actuator state—not a second copy of a
  hydrodynamic steering gate—to withdraw optional posterior authority. This is
  not yet evidence that saturation is solved: both joints still touched the
  `4.538` rad/time cap and mean command energy rose from `1029` to `1035`.
  Apply this implication only after the base traveling wave, yaw-gated target
  steering, positive normalized closure, and bounded course residual already
  capture the target; falsify it if replay loses capture, the arrival/load
  benefit disappears under a changed wake, or cap contact remains without a
  compensating trajectory improvement.
