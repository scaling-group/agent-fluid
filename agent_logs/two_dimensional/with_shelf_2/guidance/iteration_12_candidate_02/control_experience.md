...
- A smooth normalized yaw-moment-magnitude gate applied only to distributed
  ...
  exit at `75.09` with force/moment RMS `487/4680`; the floor-bounded steering
  gate changed that topology to `target_reached` at `51.47`, `1.82L` mean
  distance, `-11.28/-4.94L` displacement, and `426/4084` RMS load. A
  posterior-curvature alternative was much lower-load (`116/1278`) yet exited
  above the target after approaching only `2.43L`, so low aggregate load is not
  itself a route controller. Preserve the zero-centered traveling bend and
  magnitude-gate only its steering residual after upstream propulsion and
  correct initial response exist. Falsify this implication if replay loses
  capture or the diagonal topology; require sign-resolved event evidence before
  adding directional moment rejection or claiming a universal moment scale.
- On that successful yaw-gated scaffold, joint-state headroom can schedule a
  small positive course-progress residual, but it does not solve saturation.
  The sampled bearing-convergence controller reaches at `45.61` with score
  `0.158830`, mean distance `1.72215L`, total/mean command energy
  `46916/1028.61`, and force/moment RMS `453/4406`. Smoothly withdrawing only
  positive course amplification as maximum joint-rate usage rises preserves
  the visible diagonal capture, reaches earlier at `45.10`, holds mean distance
  to `1.72267L`, and reduces total energy and load to `46677` and `414/4104`,
  although score falls slightly to `0.157432` and mean command energy rises to
  `1034.97`. Both variants still touch both `4.538` joint-rate caps; interpret
  the gate as route/load shaping, not rate-limit relief. Applying the existing
  yaw-moment gate to the entire course residual is a negative contrast: it
  regresses score/arrival/mean distance to `0.150759/45.84/1.72999L` and raises
  load to `460/4433`. Keep worsening-course suppression, the base wave, and
  steering intact; gate only optional positive amplification with normalized
  joint state. This applies after normalized positive closure and target
  capture already exist, and is falsified if the simpler `1.724L` closure
  baseline is lost or the load benefit disappears without an arrival gain.
