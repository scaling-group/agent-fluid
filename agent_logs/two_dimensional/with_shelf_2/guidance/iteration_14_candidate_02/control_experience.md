...
- When yaw load gates a signed course-consistency residual, preserve negative
  ...
  body-frame route. The current sampled comparison further separates yaw
  response from actuator headroom. The assigned yaw-only positive gate reached
  at `45.105`, with score `0.161024`, `1.719L` mean distance, command-energy
  mean `1035`, and `416/4183` force/moment RMS. Requiring both low yaw load and
  oscillator-normalized joint-rate headroom preserved the same diagonal
  keyframe topology and improved score/mean distance/effort to
  `0.165009/1.715L/1031`, while reaching at `45.260` with higher `439/4342`
  loads. Against the ungated residual it improved arrival, distance, and load,
  but against yaw-only gating it is a route/effort trade rather than a load
  win. A rate-headroom-plus-distance gate without yaw conditioning reached
  only at `45.611`, with `1.721L` mean distance and worse `475/4653` loads, so
  proximity scheduling is not a supported replacement for response separation.
  Apply the combined gate only to positive amplification on a small
  progress-earned residual after the traveling-bend and yaw-gated steering
  scaffold exists; do not interpret it as removal of saturation because both
  joint rates still touch their caps. This lesson is falsified if capture or
  upstream propulsion is lost, if the mean-distance benefit disappears without
  compensating load reduction, or if changed wake conditions do not preserve
  the route topology.
- On the successful yaw/rate-gated scaffold, terminal heading look-ahead is a
  route-response channel distinct from posterior propulsion allocation. Capping
  the existing heading-rate projection by normalized distance divided by total
  observed body speed changed the uncapped reference from arrival `45.260`,
  mean distance `1.715290L`, score `0.165009`, and command-energy mean `1030.54`
  to `45.221`, `1.714578L`, `0.165860`, and `1030.39`; the assigned parent's
  inherited rollout exactly replayed the latter result. Force/moment RMS was
  essentially unchanged (`439/4342` versus `439/4345`), both policies retained
  the same diagonal capture topology, and both joint rates still touched their
  caps, so this supports a small terminal prediction refinement rather than a
  load or saturation claim. Replacing total body speed with positive windowed
  closing speed retained capture but regressed arrival/mean distance/score to
  `45.243/1.715373L/0.164890` with unchanged `439/4343` loads. Preserve the
  total-speed horizon cap and avoid further denominator or scalar-horizon
  tuning; a later terminal experiment should instead test a distinct bounded
  course/slip observable inside that response channel while leaving the base
  wave and propulsion residual unchanged. This lesson applies only after the
  established target-reaching route exists and is falsified if changed wake
  conditions reverse the small path benefit or a different denominator yields
  a material load, topology, or semantic improvement.
