...
- Alignment-conditioned posterior wave emphasis is a positive propulsion
  mechanism after the yaw-gated diagonal route is established, and four
  progress schedules show that the feedback invariant matters more than a
  scalar increase. Fixed `10%` emphasis reached at `46.80` with `1.745L` mean
  distance and `441/4259` force/moment RMS; signed absolute-closing modulation
  reached at `46.66` with `1.731L` and `453/4371`; progress-deficit emphasis
  reached at `46.31` with `1.736L` and `393/3878`; and adding up to `5%` only
  as windowed closing speed became efficient relative to body speed reached
  earliest at `45.48` with `1.724L` and `405/4029`. The last variant also cut
  total command energy from the fixed parent's `47931` to `47033`, despite a
  higher per-time mean caused by its shorter episode. Preserve alignment and
  normalized useful closure as the extra-envelope gates; do not infer that
  more amplitude or weaker steering is the cause, because every variant still
  touched both joint-rate caps and an inherited propulsive-priority allocator
  missed below and collided. One inherited mechanism follow-up now closes the
  phase part of that boundary: adding an alignment-and-closure-conditioned
  `0.10` posterior lag shift with analytic harmonic-amplitude compensation
  preserved capture but regressed arrival/mean distance/score from
  `45.48/1.724L/0.156386` to `46.22/1.735L/0.146573`, raised mean command
  energy from `1034` to `1036` and loads from `405/4029` to `424/4253`, and
  still touched both rate caps. Do not revisit posterior timing on this
  tracker merely because ideal sinusoidal amplitude is compensated: clipping
  and wake-loaded tracking prevent that algebra from isolating phase. Test a
  scale-free wake-event allocation on only the optional progress residual, or
  first obtain phase/headroom histories that demonstrate an unclipped timing
  opportunity. This lesson applies only after target success, a coherent
  traveling wave, and yaw-load steering moderation are present; falsify it if
  a timing policy with measured actuator headroom improves arrival or distance
  history without worse saturation, effort, load, or trajectory topology.
