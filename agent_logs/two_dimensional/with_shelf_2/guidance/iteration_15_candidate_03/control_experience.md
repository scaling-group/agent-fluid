...
- Once a time-to-go-capped heading predictor preserves the successful diagonal
  route, target bearing and measured body-course angle separate residual
  terminal translation from body yaw. In the strongest sampled capture they
  differ by `0.291/0.879` rad at `2.05L`, `0.031/-0.368` at `1.15L`, and
  `-0.051/0.283` at `0.753L`, even though the run reaches at `45.221` with
  `1.7146L` mean distance. Treat this alternating mismatch as evidence for a
  small distance-gated route-response damper, not another posterior-propulsion
  gate: both joint rates already touch their caps, loads remain `439/4345`, and
  the inherited broad-propulsion alternative passed below capture and collided.
  This observation applies only after self-propulsion, target capture, and the
  established wake-entry topology exist; falsify its usefulness if a matched
  velocity-course residual fails to reduce terminal crossing or loses capture,
  arrival, mean-distance, load, or effort performance. Do not claim the residual
  is beneficial until its later CFD evaluation supplies that comparison.
