...
- Do not force independently evidenced yaw prediction and terminal course-slip
  damping into one shared angular-response clamp on this scaffold. Relative to
  the separate bounded terms, the shared-budget variant still reached the
  target but regressed arrival/mean distance from `44.121/1.70618L` to
  `45.727/1.72692L`, raised force/moment RMS from `389/3909` to `449/4408`,
  and retained the same `4.538` joint-rate cap and near-`28.8` acceleration
  maxima; its lower command-energy mean (`1025.91` versus `1036.49`) was not a
  control improvement. The keyframes preserve the broad diagonal route but
  show a deeper terminal excursion, so lower effort alone does not justify
  merging slow response channels or reallocating their authority. Preserve
  their separate bounds and test terminal relief on the optional propulsion or
  another isolated channel. This boundary applies to the successful
  time-to-go-plus-course-damper policy under the fixed wake; falsify it only if
  a matched shared allocator reduces cap contact or load without worsening
