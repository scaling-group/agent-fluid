...
- Course-consistency is more promising as route feedback than as additional
  propulsion or posterior phase. Multiplying only the small closure-earned
  posterior envelope by a bounded bearing-trend term preserved capture and
  improved score/mean distance from `0.156386/1.7244L` to
  `0.158830/1.7222L`, but arrival slowed from `45.48` to `45.61` and
  force/moment RMS rose from `405/4029` to `453/4406`; do not amplify that
  tail residual from the scalar improvement alone. In a sibling comparison,
  adding an amplitude-compensated posterior lag shift during aligned closure
  also preserved capture but regressed score, arrival, and mean distance to
  `0.146573`, `46.22`, and `1.7346L`, while load rose to `424/4253` versus the
  same unshifted baseline. Preserve the established posterior timing and test
  bounded body-frame course trend in the steering request instead. This
  implication applies to the successful yaw-gated traveling-bend scaffold;
  falsify it if sign-resolved route evidence shows that a lag change improves
  closure without added saturation/load, or if steering-trend feedback merely
  follows beat-scale wake oscillation rather than tightening the target path.
