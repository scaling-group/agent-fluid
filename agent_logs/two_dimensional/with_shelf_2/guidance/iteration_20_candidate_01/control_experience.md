...
- Exact repeated outcomes from functionally identical candidates establish a
  materialization baseline, not wake robustness. The earlier four-way replay
  reproduced `target_reached` at `51.47`, `0.748L` final/minimum and `1.82L`
  mean distance under one prewarmed wake. The current four sampled policies,
  released sheets, and prewarm sheets are byte-identical and likewise
  reproduce the stronger course-damped outcome exactly: capture at `44.121`,
  `1.70618L` mean distance, `-10.910/-4.263L` head displacement, and `389/3909`
  force/moment RMS. Their joint-rate maxima still touch the `4.538` hard cap,
  so cap contact alone is not evidence that weakening the proven base wave is
  beneficial. Conversely, the inherited score-only rollout at `45.975` and
  `1.73033L` mean distance lowers mean command energy from `1036.49` to
  `1024.12` while raising force/moment RMS to `455/4405`; because this workspace
  contains neither its policy nor its keyframes, it rejects promotion of that
  outcome but cannot support a causal claim about its controller mechanism.
  Preserve the deterministic traveling-bend/course-damping reference and make
  mechanism-level steering tests without treating duplicate samples, a lower
  effort scalar, or provenance-poor aggregate loads as evidence for base-wave
  relief or signed disturbance feedback. This implication applies only to
  functionally identical policies and the fixed snapshot; revise it when a
  genuinely different, fully attributed policy preserves capture and route
  topology while reducing cap contact or load, and do not call it robustness
  until wake phase, inflow, geometry, or target also changes.
...
- After the terminal time-to-go cap is established, normalized body-course
  mismatch is a stronger approach residual than forecasting inertial
  line-of-sight rate. On the matched successful scaffold, smoothly damping
  `body_course - bearing` only near the target improved the prefill's
  arrival/mean-distance/score from `45.221/1.71458L/0.165860` to
  `44.121/1.70618L/0.173450` and lowered force/moment RMS from `439/4345` to
  `389/3909`; mean command energy rose modestly from `1030.39` to `1036.49`.
  The released sheet retained the same self-propelled diagonal wake-entry
  topology. In contrast, adding a bounded `bearing_window_rate + heading_rate`
  forecast produced a large downward detour and hooked return: it reached only
  at `70.790` with `2.71465L` mean distance despite much lower `200/2132` load
  RMS. Prefer a speed-confident, distance-gated velocity-direction residual
  over extrapolating noisy relative-bearing decomposition, and do not treat
  aggregate load reduction as route improvement. This applies only after
  target-directed propulsion, yaw gating, and the time-to-go cap already give
  reliable capture; falsify it if the benefit disappears on a matched replay
  or under changed wake phase without a compensating load or effort advantage.
- Unsigned hydrodynamic magnitudes do not by themselves identify a wake-driven
  yaw event on the successful course-damped scaffold. Replacing its yaw-moment
  gate with a product of absolute relative-crossflow and yaw-load ratios
  preserved capture and broad wake entry, but deepened the terminal path,
  delayed arrival from `44.121` to `45.150`, worsened mean distance from
  `1.70618L` to `1.71930L`, and raised force/moment RMS from `389/3909` to
  `460/4421`; mean command energy fell only from `1036.49` to `1024.87`.
  Multiplying two large alternating cues therefore confounds commanded gait
  load with disturbance causality and is not an evidenced steering-withdrawal
  trigger. Preserve the simpler yaw-magnitude gate and avoid another unsigned
  coincidence scalar unless sign-resolved event histories or a changed-wake
  comparison establish when the second cue adds information. This boundary
  applies to the established target-reaching diagonal route and fixed prewarm
  snapshot; falsify it if a matched or changed-condition coincidence detector
  reduces route error or load without sacrificing capture, arrival, distance,
  or effort.
