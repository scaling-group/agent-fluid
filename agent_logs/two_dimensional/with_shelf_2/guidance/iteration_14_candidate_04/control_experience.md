...
- Terminal heading-response scheduling has reached a denominator-edit plateau
  on the successful yaw/rate-gated allocator. The unscheduled prefill repeated
  capture at `45.260`, `1.71529L` mean distance, score `0.165009`, and
  `439/4342` force/moment RMS; capping response horizon by distance divided by
  total observed body speed changed this only to `45.221`, `1.71458L`,
  `0.165860`, and `439/4345`, while substituting positive range-closure speed
  regressed slightly to `45.243`, `1.71537L`, `0.164890`, and `439/4343`.
  All retained the same visible diagonal transit and broad late correction.
  Preserve the total-speed terminal cap as the best finite reference, but do
  not spend another iteration changing its denominator or scalar horizon
  without a new trajectory class or load separation. The next route-response
  test should add one independently observable target-relative component, such
  as bounded translational line-of-sight drift, while leaving the base wave and
  allocation intact. This boundary is falsified if a future denominator-only
  edit produces a material arrival, mean-distance, load, or termination-class
