...
- Once a time-to-go-capped heading predictor preserves the successful diagonal
  route, near-target body-course mismatch needs its own bounded authority rather
  than sharing the heading-rate forecast's clamp. Adding the independently
  distance-gated course damper improved the matched predictor from
  `45.221/1.71458L/0.165860` arrival, mean distance, and score to
  `44.121/1.70618L/0.173450`, while reducing force/moment RMS from `439/4345` to
  `389/3909`; the keyframes retain the same self-propelled wake-entry topology
  with less downward terminal excursion. A sibling that clamped the summed
  heading and course responses back into the old heading budget still captured
  but regressed to `45.727/1.72692L/0.153609` and `449/4408` loads. Preserve
  separate bounds for observed yaw response and translational slip instead of
  treating them as interchangeable authority. This applies only after the
  zero-centered gait, yaw-gated route steering, and target success are already
  established; falsify it if a matched replay loses capture/topology or if an
  independently bounded residual raises effort or loads without improving the
  terminal path. It does not support more posterior propulsion, whose inherited
  broad-allocation test passed below capture and collided.
- Preserve the simple independently bounded terminal course residual before
  adding state schedulers or cue arbitration. Relative to the matched
  `44.121/1.70618L/0.173450` capture with `389/3909` force/moment RMS,
  amplifying course damping by positive normalized closure preserved the same
  broad diagonal topology but regressed arrival/mean distance/score to
  `45.689/1.71923L/0.161543` and raised loads slightly to `396/3917`; its small
  mean-command reduction (`1036.49` to `1031.20`) did not compensate. A sibling
  that instead attenuated the time-to-go-capped heading forecast whenever it
  opposed measured course response also retained capture but regressed to
  `45.331/1.72584L/0.154397` and `413/4089` loads. Thus positive closure is not
  evidence for more terminal course authority, and cue disagreement is not
  evidence for discarding the bounded yaw forecast. This boundary applies to
  the already successful yaw-gated diagonal scaffold under the fixed prewarm;
  falsify it only if a matched scheduler or arbitration rule improves tracking
  or materially reduces effort/load without losing capture or route topology.
  A later geometric substitution sharpens the same boundary: replacing raw
  course-angle mismatch by its sine retained capture but regressed
  arrival/mean distance/score to `46.074/1.73655L/0.144279` from
  `44.121/1.70618L/0.173450`; its small mean-command reduction
  (`1036.49` to `1028.72`) came with higher force/moment RMS
  (`389/3909` to `407/4002`). Large terminal course error therefore still
  needs the stronger angular representation on this scaffold. Do not infer
  that a geometrically bounded lateral-velocity fraction is safer or more
  efficient; test a different actuator-allocation mechanism while preserving
  the raw independently gated angular damper. This implication is falsified
  if a matched bounded representation improves tracking or materially lowers
  load/effort without weakening capture or the diagonal topology.
