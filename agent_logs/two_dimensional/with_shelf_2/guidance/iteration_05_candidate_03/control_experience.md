...
- A normalized yaw-load magnitude gate on steering—not low load by itself—is
  an evidenced semantic improvement when a response-aware half-cycle gait
  already enters the target neighborhood but folds into a high-load exit. The
  ungated matched policy reached `1.646L`, then exited low after `75.09` units
  with force/moment RMS `487/4680`; multiplying only its steering asymmetry by
  a smooth, nonzero-floor function of `abs(moment_z_L2)` instead produced first-
  crossing success at `0.748L` after `51.47` units, improved mean distance to
  `1.820L`, and reduced the loads to `426/4084` while preserving `-11.28L`
  upstream displacement. In contrast, the separately sampled posterior-total-
  curvature policy had much lower `116/1278` loads but still missed at
  `2.430L`, curled to the opposite boundary, and retained joint-rate cap
  contact. Apply the gate only to a route-capable steering residual, never to
  the propulsive wave or as a generic objective to minimize load. This lesson
  is falsified if repeat or held-out wakes lose target reach/upstream motion,
  or if similar load attenuation on a controller without useful route geometry
  still fails; persistent velocity-cap contact also remains a separate issue
  that later mechanisms must address without increasing half-cycle authority.
