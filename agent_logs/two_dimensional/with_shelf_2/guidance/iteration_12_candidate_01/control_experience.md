...
- Treat posterior propulsion earned by target progress as a small
  ...
  saturation, or load.
- Gate a course-conditioned posterior residual asymmetrically: actuator-state
  feedback can withdraw optional positive amplification, but a fast-response
  gate must not also erase negative course correction. Starting from the
  successful course-convergence controller (`45.61`, `1.7222L` mean distance,
  `453/4406` force/moment RMS), oscillator-normalized joint-rate headroom on
  only its positive branch retained capture, reached at `45.10`, held mean
  distance to `1.7227L`, reduced command energy from `46916` to `46677`, and
  lowered loads to `414/4104`. Two functionally equivalent yaw-load gates on
  the whole signed residual instead repeated `45.84`, `1.7300L`, and
  `460/4433`; a route-drift steering lead reached at `45.72` but further
  regressed mean distance to `1.7666L` despite much lower `272/2896` loads.
  Preserve negative residual suppression and use normalized headroom only on
  extra progress-earned motion; test any additional yaw separation on that
  positive branch alone. This lesson applies only to the small posterior
  residual after the proven traveling wave and target steering exist, and is
  falsified if rate-gated replays lose capture or the distance topology under
  changed wake conditions.
