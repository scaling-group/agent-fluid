...
- A floor-bounded yaw-magnitude gate on distributed half-cycle steering is now
  a reproduced positive mechanism, not merely a load-control hypothesis. Three
  sampled evaluations of that controller follow the same direct diagonal wake
  entry and reach the `0.75L` target at `51.47`, with `0.748L` final/minimum
  distance, `1.820L` distance integral, and `-11.28/-4.94L` head displacement;
  the contrasting posterior-curvature policy travels upstream but arcs past
  the target and exits laterally with a `2.43L` closest approach. Preserve the
  zero-centered traveling bend, heading-response request, and sign-independent
  gate before testing another steering primitive. Success does not establish
  actuator robustness: both joints still reach exactly the `260 deg/time` hard
  rate cap and force/moment RMS remains `426/4084`. The next distinct test
  should shape commands from normalized joint-rate headroom or another bounded
  state signal, rather than retuning steering or gait scalars. This implication
  is falsified if rate-envelope avoidance cannot reproduce capture and the
  direct trajectory, or if it trades cap contact for higher hydrodynamic load.
