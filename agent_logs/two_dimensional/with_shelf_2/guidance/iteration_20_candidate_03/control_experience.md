...
- After the terminal time-to-go cap is established, normalized body-course
  ...
  line-of-sight rate, and its independent authority is part of the mechanism.
  On the matched successful scaffold, smoothly damping `body_course - bearing`
  only near the target improved arrival/mean-distance/score from
  `45.221/1.71458L/0.165860` to `44.121/1.70618L/0.173450` and lowered
  force/moment RMS from `439/4345` to `389/3909`; mean command energy rose
  modestly from `1030.39` to `1036.49`. Four byte-identical sampled policies
  and released sheets reproduce that diagonal wake-entry topology and result,
  which confirms deterministic materialization under the fixed prewarm but not
  changed-wake robustness. Inherited matched refinements sharpen the negative
  boundary: sharing the heading-response cap delayed capture to `45.727` with
  `1.72692L` mean distance and `449/4408` loads; multiplying course damping by
  positive normalized closure reached at `45.689/1.71923L`; suppressing an
  opposing heading forecast reached at `45.331/1.72584L`; and replacing the
  yaw gate with unsigned crossflow-times-yaw coincidence reached at
  `45.150/1.71930L` while raising loads to `460/4421`. A parallel bounded
  `bearing_window_rate + heading_rate` forecast was worse still, producing a
  large downward detour and hooked return at `70.790/2.71465L` despite much
  lower `200/2132` load RMS. Preserve the speed-confident distance-gated course
  residual as a fixed, separately bounded route-response term; do not merge it
  into a common angular budget, strengthen it from closure, arbitrate it from
  opposing observed responses, or add unsigned wake coincidence without
  sign-resolved event evidence. Aggregate load reduction alone is not route
  improvement. This applies only after target-directed propulsion, yaw gating,
  and the time-to-go cap already give reliable capture; falsify it if a
  changed-condition replay loses the benefit or one rejected coupling improves
  capture/route without a compensating load or effort regression.
