...
function target_policy(state, params)
    ...
    # Near capture, bearing alignment alone can award extra tail drive while
    # the measured translation is crossing the line of sight. Preserve the
    # base traveling wave and withdraw only its optional posterior residual.
    # Far from the target (or at low speed) this gate tends to one.
    posterior_course_alignment = 1 / (
        1 + (
            course_mismatch /
            max(params.posterior_course_scale, eps(Float64))
        )^2
    )
    posterior_course_gate = 1 - approach_gate * course_confidence *
        (1 - posterior_course_alignment)
    posterior_wave_scale = 1 + route_alignment * posterior_course_gate * (
