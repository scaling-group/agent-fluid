...
function target_policy(state, params)
    ...
    # A body-frame bearing rate contains both body yaw and inertial motion of
    # the target line of sight. Recover the latter with the established sign
    # convention, then forecast it over the same bounded response horizon.
    # This avoids assuming a stationary line of sight during diagonal transit.
    ...
    line_of_sight_rate = bearing_window_rate + heading_rate
    line_of_sight_response = clamp(
        heading_response_horizon * line_of_sight_rate,
        -params.heading_response_limit,
        params.heading_response_limit,
    )
    predicted_bearing = bearing - heading_response + line_of_sight_response
    route_turn = tanh(
