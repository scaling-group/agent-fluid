...
function target_policy(state, params)
    ...
    # Preserve the established unsigned yaw-load gate for optional posterior
    # effort. For steering, a large normalized yaw moment warrants withdrawal
    # only when the observed heading response is already aligned with the
    # remaining route request. Retain authority under an opposing response
    # without assigning a direction to the moment itself. The propulsive wave
    # remains intact.
    yaw_load_ratio = abs(Float64(state.moment_z_L2)) /
        max(params.yaw_moment_scale, eps(Float64))
    yaw_load_gate = params.minimum_steering_fraction +
        (1 - params.minimum_steering_fraction) /
        (1 + yaw_load_ratio * yaw_load_ratio)
    response_assistance = clamp(
        route_turn * heading_response /
            max(params.heading_response_limit, eps(Float64)),
        0.0,
        1.0,
    )
    steering_gate = params.minimum_steering_fraction +
        (1 - params.minimum_steering_fraction) /
        (1 + yaw_load_ratio * yaw_load_ratio * response_assistance)
    ...
    positive_course_gate = course_headroom_gate * yaw_load_gate
