...
function target_policy(state, params)
    ...
    # already carries the fish across the line of sight. Dampen that measured
    # body-course mismatch without disturbing the established far/middle route.
    body_course = atan(
        velocity_body_y,
    ...
    course_response = params.approach_course_damping * approach_gate *
        course_confidence * course_mismatch

    # A matched shared cap degraded arrival, distance, and loads by clipping
    # aligned corrections. Preserve aligned addition. Only when extrapolated
    # yaw and measured translation oppose, smoothly give the measured course
    # priority according to their scale-free balance.
    response_opposition = max(-heading_response * course_response, 0.0)
    response_magnitude = abs(heading_response) + abs(course_response)
    response_conflict_fraction = clamp(
        4 * response_opposition /
        max(response_magnitude^2, eps(Float64)),
        0.0,
        1.0,
    )
    selected_heading_response =
        (1 - response_conflict_fraction) * heading_response
    predicted_bearing = bearing - selected_heading_response - course_response
    route_turn = tanh(
