function target_policy_params()
    return (
        ...
        rate_soft_limit=230.0 * pi / 180,
        rate_transition_width=15.0 * pi / 180,
        rate_alignment_sharpness=8.0,
    )
end
...
function target_policy(state, params)
    ...
    # The successful sampled controller reached the hard rate cap at both
    # joints. Preserve acceleration that reverses each traveling-wave segment,
    # but smoothly remove same-direction drive as normalized rate headroom
    # closes. This is an actuator-state mechanism, not a change to gait gains.
    rate_limit = params.rate_soft_limit
    rate_width = max(params.rate_transition_width, eps(Float64))
    rate_scale = max(rate_limit, eps(Float64))
    accel_scale = max(limit, eps(Float64))

    headroom1 = 0.5 * (
        1 - tanh((abs(qd1) - rate_limit) / rate_width)
    )
    headroom2 = 0.5 * (
        1 - tanh((abs(qd2) - rate_limit) / rate_width)
    )
    increasing1 = 0.5 * (
        1 + tanh(
            params.rate_alignment_sharpness * soft_a1 * qd1 /
            (accel_scale * rate_scale),
        )
    )
    increasing2 = 0.5 * (
        1 + tanh(
            params.rate_alignment_sharpness * soft_a2 * qd2 /
            (accel_scale * rate_scale),
        )
    )
    a1 = soft_a1 * (1 - increasing1 * (1 - headroom1))
    a2 = soft_a2 * (1 - increasing2 * (1 - headroom2))
