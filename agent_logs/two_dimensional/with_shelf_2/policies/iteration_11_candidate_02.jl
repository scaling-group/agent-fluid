function target_policy_params()
    return (
        ...
        translation_drift_horizon=2.0,
        translation_drift_limit=0.12,
...
function target_policy(state, params)
    ...
    history_window_dt = hasproperty(state, :history_window_dt) ?
        Float64(getproperty(state, :history_window_dt)) : 0.0

    # Bearing rate contains the effects of both body yaw and translation.
    # Adding heading rate approximately isolates course-over-ground drift of
    # the target line of sight. Lead that bounded drift in the route request,
    # but keep the release call identical until observation history exists.
    translation_bearing_rate = history_window_dt > 0.0 ?
        bearing_window_rate + heading_rate : 0.0
    translation_response = clamp(
        params.translation_drift_horizon * translation_bearing_rate,
        -params.translation_drift_limit,
        params.translation_drift_limit,
    )
    heading_predicted_bearing = bearing - heading_response
    route_bearing = heading_predicted_bearing + translation_response
    route_turn = tanh(
        route_bearing / max(params.bearing_scale, eps(Float64)),
    )
    ...
    # rather than a dimensional closing-speed threshold. Keep this envelope
    # on the parent's heading-only bearing so the new drift term changes only
    # route steering, not posterior propulsion allocation.
    route_alignment = 1 /
        (1 + (heading_predicted_bearing /
            max(params.bearing_scale, eps(Float64)))^2)
