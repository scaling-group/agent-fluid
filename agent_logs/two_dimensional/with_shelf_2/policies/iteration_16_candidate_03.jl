function target_policy_params()
    return (
        ...
        joint_rate_reference_ratio=1.25,
        yaw_moment_scale=1.0,
        minimum_steering_fraction=0.20,
        acceleration_soft_limit=29.0,
        outward_rate_guard_strength=0.65,
        outward_rate_guard_power=12,
    )
end
...
function target_policy(state, params)
    ...
    rate_reference = params.joint_rate_reference_ratio * omega * amp
    ...
    soft_a1 = limit * tanh(asymmetric_a1 / limit)
    soft_a2 = limit * tanh(asymmetric_a2 / limit)

    # The rollout still reaches the joint-rate envelope. Withdraw only the
    # acceleration component that would push an already fast joint farther
    # outward; retain all braking authority and leave ordinary-rate propulsion
    # effectively unchanged. The reference remains tied to the candidate's
    # own oscillator frequency and amplitude rather than an environment cap.
    rate_power = params.outward_rate_guard_power
    rate_usage1 = abs(qd1) / max(rate_reference, eps(Float64))
    rate_usage2 = abs(qd2) / max(rate_reference, eps(Float64))
    rate_gate1 = rate_usage1^rate_power / (1 + rate_usage1^rate_power)
    rate_gate2 = rate_usage2^rate_power / (1 + rate_usage2^rate_power)
    direction1 = sign(qd1)
    direction2 = sign(qd2)
    outward_a1 = max(direction1 * soft_a1, 0.0)
    outward_a2 = max(direction2 * soft_a2, 0.0)
    a1 = soft_a1 - params.outward_rate_guard_strength *
        rate_gate1 * outward_a1 * direction1
    a2 = soft_a2 - params.outward_rate_guard_strength *
        rate_gate2 * outward_a2 * direction2
