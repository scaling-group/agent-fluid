...
function target_policy(state, params)
    ...
    # Use the same evidenced progress permission for a small posterior phase
    # change. Normalize the harmonic lag basis so this tests wave timing
    # without quietly adding another amplitude envelope.
    effective_tail_lag = params.tail_lag_gain +
        params.progress_phase_lag * route_alignment * closure_efficiency
    lag_amplitude_normalization = sqrt(1 + params.tail_lag_gain^2) /
        sqrt(1 + effective_tail_lag^2)
    tail_target = posterior_wave_scale * lag_amplitude_normalization * (
