function target_policy_params()
    return (
        ...
        steering_rate_reference_ratio=1.25,
        steering_rate_guard_power=4.0,
...
function target_policy(state, params)
    ...
    steering_rate_reference = params.steering_rate_reference_ratio * omega * amp
    steering_residual_a1 =
        params.half_cycle_asymmetry * turn_fraction * abs(symmetric_a1)
    steering_rate_usage1 = abs(qd1) /
        max(steering_rate_reference, eps(Float64))
    steering_rate_gate1 = qd1 * steering_residual_a1 > 0 ?
        1 / (1 + steering_rate_usage1^params.steering_rate_guard_power) : 1.0
    asymmetric_a1 = symmetric_a1 + steering_rate_gate1 * steering_residual_a1
    ...
    # requested turn through an equal anti-phase bias. For either joint, use
    # oscillator-normalized rate headroom only when the steering residual would
    # compound current motion; never weaken steering that brakes a fast joint.
    steering_residual_a2 = params.posterior_asymmetry_share *
        params.half_cycle_asymmetry * turn_fraction * abs(symmetric_a2)
    steering_rate_usage2 = abs(qd2) /
        max(steering_rate_reference, eps(Float64))
    steering_rate_gate2 = qd2 * steering_residual_a2 > 0 ?
        1 / (1 + steering_rate_usage2^params.steering_rate_guard_power) : 1.0
    asymmetric_a2 = symmetric_a2 + steering_rate_gate2 * steering_residual_a2
