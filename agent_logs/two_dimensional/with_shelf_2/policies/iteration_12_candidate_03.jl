...
function target_policy(state, params)
    ...
    # observed body-frame bearing magnitude is shrinking or growing. Preserve
    # the full reduction when the course worsens, but use the existing yaw-load
    # gate to withhold optional amplification when the wake/body interaction is
    # already turning strongly. The bounded response is neutral before
    # observation history is available.
    ...
    course_consistency = tanh(-normalized_course_trend)
    allocated_course_consistency = min(course_consistency, 0.0) +
        steering_gate * max(course_consistency, 0.0)
    course_consistency_scale = 1 +
        params.course_consistency_modulation * allocated_course_consistency
