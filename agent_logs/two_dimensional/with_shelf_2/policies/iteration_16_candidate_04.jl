...
function target_policy(state, params)
    ...
    # Heading response and translational slip describe compatible parts of one
    # route forecast. Share the established angular-response budget so aligned
    # corrections cannot stack into excess steering, while opposing responses
    # can still cancel before the target-bearing command is formed.
    route_response = clamp(
        heading_response + course_response,
        -params.heading_response_limit,
        params.heading_response_limit,
    )
    predicted_bearing = bearing - route_response
    route_turn = tanh(
