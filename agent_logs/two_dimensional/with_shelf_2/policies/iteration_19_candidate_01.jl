...
function target_policy(state, params)
    ...
    course_angle_error = clamp(body_course - bearing, -pi / 2, pi / 2)
    # Use the target-relative lateral component rather than the raw angle. It
    # retains small-error course damping but naturally bounds the response to
    # large wake-induced slip without adding another authority channel.
    course_mismatch = sin(course_angle_error)
