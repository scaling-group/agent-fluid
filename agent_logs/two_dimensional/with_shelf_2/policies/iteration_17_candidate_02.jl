function target_policy_params()
    return (
        ...
        approach_slip_propulsion_relief=1.0,
...
function target_policy(state, params)
    ...
    course_mismatch_ratio = course_mismatch /
        max(params.bearing_scale, eps(Float64))
    course_slip_gate = course_mismatch_ratio^2 /
        (1 + course_mismatch_ratio^2)
    ...
    # Near capture, positive closure can coexist with velocity crossing the
    # line of sight. Withdraw only the optional tail increment in proportion
    # to that observed slip; the base traveling wave remains available.
    optional_propulsion_gate = clamp(
        1 - params.approach_slip_propulsion_relief *
            approach_gate * course_slip_gate,
        0.0,
        1.0,
    )
    posterior_wave_scale = 1 + optional_propulsion_gate * route_alignment * (
