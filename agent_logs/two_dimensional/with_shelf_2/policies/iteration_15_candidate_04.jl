function target_policy_params()
    return (
        ...
        minimum_heading_prediction_trust=0.25,
...
function target_policy(state, params)
    ...
    heading_response_horizon = if hasproperty(state, :distance_L)
    ...
    # In crossflow, body yaw is not sufficient evidence that the line of sight
    # is improving. Trust an apparently corrective heading response fully on a
    # converging or steady course, but smoothly withdraw that trust when the
    # recent body-frame bearing instead diverges. Preserve wrong-sign response,
    # which already increases the requested correction.
    course_divergence = max(tanh(normalized_course_trend), 0.0)
    heading_prediction_trust = 1 -
        (1 - params.minimum_heading_prediction_trust) * course_divergence
    bearing_sign = sign(bearing)
    corrective_heading_response = bearing_sign * max(
        bearing_sign * heading_response,
        0.0,
    )
    noncorrective_heading_response = heading_response -
        corrective_heading_response
    trusted_heading_response = noncorrective_heading_response +
        heading_prediction_trust * corrective_heading_response
    predicted_bearing = bearing - trusted_heading_response
    route_turn = tanh(
