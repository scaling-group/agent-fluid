function target_policy_params()
    return (
        ...
        posterior_phase_lag_shift=0.10,
...
function target_policy(state, params)
    ...
    # During efficient aligned travel, rotate the posterior harmonic toward a
    # slightly larger lag. Compensate its ideal sinusoidal magnitude relative
    # to the baseline so this tests wave timing rather than another amplitude
    # increase; outside that regime the prefilled tail target is exact.
    effective_tail_lag = params.tail_lag_gain +
        params.posterior_phase_lag_shift * route_alignment * closure_efficiency
    tail_lag_amplitude_compensation = sqrt(1 + params.tail_lag_gain^2) /
        sqrt(1 + effective_tail_lag^2)
    tail_target = posterior_wave_scale * tail_lag_amplitude_compensation * (
