...
function target_policy(state, params)
    ...
    yaw_predicted_bearing = bearing - heading_response

    # Instantaneous yaw does not reveal whether translation and wake-induced
    # slip make the whole route converge. Retain the yaw projection far from
    # capture; as the existing time-to-target cap shortens the horizon, blend
    # toward the observed windowed bearing trend. Both projections share the
    # same response bound, so the terminal mechanism cannot add steering range.
    ...
    course_response = clamp(
        heading_response_horizon * bearing_window_rate,
        -params.heading_response_limit,
        params.heading_response_limit,
    )
    course_predicted_bearing = bearing + course_response
    terminal_course_fraction = clamp(
        1 - heading_response_horizon /
            max(params.heading_response_horizon, eps(Float64)),
        0.0,
        1.0,
    )
    predicted_bearing =
        (1 - terminal_course_fraction) * yaw_predicted_bearing +
        terminal_course_fraction * course_predicted_bearing
    route_turn = tanh(
