function target_policy_params()
    return (
        ...
        course_approach_distance_L=1.0,
...
function target_policy(state, params)
    ...
    # Scalar range closure can accompany an off-course pass. Keep the sampled
    # course-convergence correction on the small progress-earned tail residual,
    # but protect its positive amplification when either joint is already using
    # most of the oscillator's candidate-owned rate scale. A worsening course
    # still suppresses the residual fully; high rate usage never earns thrust.
    ...
    # Close to capture, withdraw only extra positive course amplification so a
    # converging route does not earn a late tail surge. Negative course trend
    # retains its full suppressive authority, and the base wave, closure bonus,
    # and target steering are unchanged. Missing distance observations leave
    # this scheduler neutral for the compact compatibility contract.
    approach_course_gate = if hasproperty(state, :distance_L)
        distance_L = max(Float64(getproperty(state, :distance_L)), 0.0)
        approach_scale = max(
            params.course_approach_distance_L,
            eps(Float64),
        )
        distance_L^2 / (distance_L^2 + approach_scale^2)
    else
        1.0
    end
    positive_course_gate = course_headroom_gate * approach_course_gate
    headroom_conditioned_course = min(course_direction, 0.0) +
        positive_course_gate * max(course_direction, 0.0)
    course_consistency_scale = 1 +
        params.course_consistency_modulation * headroom_conditioned_course
