...
function target_policy(state, params)
    ...
    # motion only when the predicted body-frame route is already aligned.
    # Modulate that residual around the successful parent's startup value with
    # windowed normalized closure: confirmed progress earns more propulsion,
    # while adverse progress relaxes it without weakening the base wave.
    route_alignment = 1 /
    ...
    closing_response = tanh(
        window_closing_speed /
        max(params.closing_speed_scale, eps(Float64)),
    )
    progress_emphasis_scale = 1 +
        params.closing_speed_modulation * closing_response
    posterior_wave_scale = 1 +
        params.aligned_posterior_emphasis * route_alignment *
        progress_emphasis_scale
