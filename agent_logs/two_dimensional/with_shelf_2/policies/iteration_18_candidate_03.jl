...
function target_policy(state, params)
    ...
    # Separate a coupled wake-driven yaw event from large moment generated by
    # the commanded gait alone. Reduce only the target-steering residual when
    # normalized yaw load and local relative crossflow are both large. The
    # compact-state fallback recovers the established moment-only gate.
    yaw_load_ratio = abs(Float64(state.moment_z_L2)) /
        max(params.yaw_moment_scale, eps(Float64))
    relative_crossflow_ratio = if hasproperty(
        state,
        :relative_flow_velocity_body_U,
    )
        relative_flow_body = getproperty(
            state,
            :relative_flow_velocity_body_U,
        )
        abs(Float64(relative_flow_body[2])) /
            max(params.relative_crossflow_scale, eps(Float64))
    else
        1.0
    end
    wake_yaw_ratio = yaw_load_ratio * relative_crossflow_ratio
    steering_gate = params.minimum_steering_fraction +
        (1 - params.minimum_steering_fraction) /
        (1 + wake_yaw_ratio * wake_yaw_ratio)
