...
function target_policy(state, params)
    ...
    # body-course angle with target bearing and damp only that mismatch. Gate
    # the residual by observed time-to-go so faster motion receives correction
    # earlier in range; speed confidence suppresses a noisy course near rest.
    body_course = atan(
        velocity_body_y,
    ...
    approach_time_to_go = distance_L /
        max(body_speed, params.body_speed_floor)
    approach_ratio = approach_time_to_go /
        max(params.approach_time_scale, eps(Float64))
