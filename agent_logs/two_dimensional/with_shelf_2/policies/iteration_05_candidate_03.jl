...
function target_policy(state, params)
    ...
    # Release steering that the observed heading response is already supplying.
    # The fallback keeps the normalized public-policy smoke contract callable
    # while the formal multi-wake state provides heading rate.
    ...
    # Large normalized yaw load means the wake/body interaction is already
    # turning the fish. Reduce only the steering residual, retain a nonzero
    # steering floor, and never suppress the symmetric propulsive wave.
    ...
    # Retain the lagged posterior traveling bend and distribute only a smaller
    # share of the same gated half-cycle steering residual to joint 2.
    tail_target = -q1 -
