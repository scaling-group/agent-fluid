...
function target_policy(state, params)
    ...
    rate_headroom_gate = 1 / (1 + joint_rate_usage^4)
    positive_course_gate = rate_headroom_gate * steering_gate
    conditioned_course = min(course_direction, 0.0) +
        positive_course_gate * max(course_direction, 0.0)
    course_posterior_adjustment =
        params.progress_posterior_emphasis * closure_efficiency *
        params.course_consistency_modulation * conditioned_course

    # The reference rollout reaches both joint-rate caps. Preserve the base
    # lagged wave, but smoothly withdraw optional aligned/progress emphasis
    # when the joint state has no rate headroom. Keep a nonzero floor so this
    # allocation does not abruptly erase the evidenced posterior contribution;
    # the full negative course adjustment remains outside the headroom gate.
    posterior_headroom_gate = params.posterior_headroom_floor +
        (1 - params.posterior_headroom_floor) * rate_headroom_gate
    base_posterior_emphasis = params.aligned_posterior_emphasis +
        params.progress_posterior_emphasis * closure_efficiency
    posterior_wave_scale = 1 + route_alignment * (
        posterior_headroom_gate * base_posterior_emphasis +
        course_posterior_adjustment
    )
