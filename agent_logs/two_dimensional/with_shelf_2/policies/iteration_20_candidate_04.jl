function target_policy_params()
    return (
        ...
        outward_rate_guard_floor=0.35,
        outward_rate_guard_exponent=8.0,
...
function target_policy(state, params)
    ...
    # Both sampled joint rates reach their envelope even though the useful
    # route is already established. Preserve braking and beat reversal, but
    # smoothly withdraw acceleration that would drive an already-fast joint
    # farther outward. The oscillator-normalized reference keeps this guard
    # independent of resolution and dimensional actuator settings.
    guard_exponent = max(
        params.outward_rate_guard_exponent,
        eps(Float64),
    )
    rate_usage1 = abs(qd1) / max(rate_reference, eps(Float64))
    rate_usage2 = abs(qd2) / max(rate_reference, eps(Float64))
    outward_gate1 = params.outward_rate_guard_floor +
        (1 - params.outward_rate_guard_floor) /
        (1 + rate_usage1^guard_exponent)
    outward_gate2 = params.outward_rate_guard_floor +
        (1 - params.outward_rate_guard_floor) /
        (1 + rate_usage2^guard_exponent)
    outward_a1 = qd1 * asymmetric_a1 > 0 ? asymmetric_a1 : 0.0
    outward_a2 = qd2 * asymmetric_a2 > 0 ? asymmetric_a2 : 0.0
    guarded_a1 = asymmetric_a1 + (outward_gate1 - 1) * outward_a1
    guarded_a2 = asymmetric_a2 + (outward_gate2 - 1) * outward_a2

    # Smooth candidate-owned limiting avoids a persistent hard-clipped command.
    limit = params.acceleration_soft_limit
    a1 = limit * tanh(guarded_a1 / limit)
    a2 = limit * tanh(guarded_a2 / limit)
