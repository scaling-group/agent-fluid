function target_policy_params()
    return (
        ...
        rate_barrier_start=4.0,
        rate_barrier_width=0.45,
    )
end
...
function target_policy(state, params)
    ...
    # Smooth candidate-owned acceleration limiting keeps the command inside the
    # harder episode clip.
    limit = params.acceleration_soft_limit
    candidate_a1 = limit * tanh(asymmetric_a1 / limit)
    candidate_a2 = limit * tanh(asymmetric_a2 / limit)

    # The successful parent repeatedly reached the hard joint-speed cap. Once
    # a joint enters the soft boundary layer, attenuate only acceleration that
    # would increase its speed. Opposite-sign braking and the low-speed
    # traveling wave are unchanged.
    rate_excess1 = max(0.0, abs(qd1) - params.rate_barrier_start)
    rate_excess2 = max(0.0, abs(qd2) - params.rate_barrier_start)
    rate_gate1 = 1 / (1 + (rate_excess1 / params.rate_barrier_width)^2)
    rate_gate2 = 1 / (1 + (rate_excess2 / params.rate_barrier_width)^2)
    outward_a1 = qd1 * candidate_a1 > 0 ? candidate_a1 : 0.0
    outward_a2 = qd2 * candidate_a2 > 0 ? candidate_a2 : 0.0
    a1 = candidate_a1 - (1 - rate_gate1) * outward_a1
    a2 = candidate_a2 - (1 - rate_gate2) * outward_a2
