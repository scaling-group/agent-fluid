...
function target_policy(state, params)
    ...
    # Give the posterior joint one role: follow the already-steered anterior
    # state with a velocity lag so it preserves a traveling thrust wave. The
    # anterior asymmetry propagates through q1 without a second direct steering
    # injection that could distort posterior propulsion.
    tail_target = -q1 -
        params.tail_lag_gain * qd1 / max(omega, eps(omega))
    symmetric_a2 = omega^2 * (tail_target - q2) -
        2 * params.tail_damping * omega * qd2

    # Smooth candidate-owned limiting avoids persistent hard-clipped commands.
