...
function target_policy(state, params)
    ...
    # Smooth candidate-owned acceleration limiting avoids a persistent
    # hard-clipped acceleration command.
    limit = params.acceleration_soft_limit
    soft_a1 = limit * tanh(asymmetric_a1 / limit)
    soft_a2 = limit * tanh(asymmetric_a2 / limit)

    # Both joints reached the episode rate cap in the retained rollout. Form a
    # smooth compact-support envelope from the gait's own expected rate and
    # withdraw only acceleration that would push a fast joint farther outward.
    # Opposing acceleration remains untouched so braking and beat reversal keep
    # their full authority. This acts after route and propulsion allocation and
    # therefore does not reinterpret either feedback signal.
    rate_power1 = (
        abs(qd1) / max(rate_reference, eps(Float64))
    )^params.rate_governor_exponent
    rate_power2 = (
        abs(qd2) / max(rate_reference, eps(Float64))
    )^params.rate_governor_exponent
    outward_gate1 = rate_power1 < 1 ?
        exp(-rate_power1 / max(1 - rate_power1, eps(Float64))) : 0.0
    outward_gate2 = rate_power2 < 1 ?
        exp(-rate_power2 / max(1 - rate_power2, eps(Float64))) : 0.0
    a1 = soft_a1 * (soft_a1 * qd1 > 0 ? outward_gate1 : 1.0)
    a2 = soft_a2 * (soft_a2 * qd2 > 0 ? outward_gate2 : 1.0)
