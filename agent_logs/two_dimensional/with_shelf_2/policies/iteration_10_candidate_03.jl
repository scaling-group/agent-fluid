function target_policy_params()
    return (
        ...
        bearing_trend_horizon=2.0,
        bearing_trend_limit=0.12,
...
function target_policy(state, params)
    ...
    # Add a small line-of-sight trend lead to the established heading
    # response. Converging bearing releases a turn that is already working;
    # diverging bearing receives bounded extra route authority. The smooth cap
    # keeps beat-scale wake motion subordinate to current target geometry.
    bearing_trend_response = params.bearing_trend_limit * tanh(
        params.bearing_trend_horizon * bearing_window_rate /
        max(params.bearing_trend_limit, eps(Float64)),
    )
    predicted_bearing = bearing - heading_response + bearing_trend_response
    route_turn = tanh(
