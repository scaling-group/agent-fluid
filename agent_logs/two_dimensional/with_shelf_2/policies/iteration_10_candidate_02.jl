function target_policy_params()
    return (
        ...
        minimum_course_feedback_fraction=0.20,
...
function target_policy(state, params)
    ...
    # Range closure can accompany an off-course pass, so use body-frame
    # bearing trend to reallocate only the progress-earned tail residual.
    # During strong yaw loading, smoothly fade this optional reallocation
    # toward a nonzero floor; the closure bonus and base traveling wave remain
    # intact instead of fighting a fast wake/body turn with more tail motion.
    ...
    course_feedback_gate = params.minimum_course_feedback_fraction +
        (1 - params.minimum_course_feedback_fraction) /
        (1 + yaw_load_ratio * yaw_load_ratio)
    course_consistency_scale = 1 +
        params.course_consistency_modulation * course_feedback_gate *
        tanh(-normalized_course_trend)
