...
function target_policy(state, params)
    ...
    # Moment and lateral force expose complementary rotational and translational
    # wake loads. Their sign-independent normalized norm smoothly reduces only
    # the steering residual, without assuming a directional load convention or
    # suppressing the propulsive wave. The fallback preserves the compact
    # normalized-policy smoke contract, which does not supply force_body_L.
    ...
    lateral_force = hasproperty(state, :force_body_L) ?
        Float64(getproperty(state, :force_body_L)[2]) : 0.0
    lateral_load_ratio = abs(lateral_force) /
        max(params.lateral_force_scale, eps(Float64))
    hydrodynamic_load_ratio = hypot(yaw_load_ratio, lateral_load_ratio)
    steering_gate = params.minimum_steering_fraction +
        (1 - params.minimum_steering_fraction) /
        (1 + hydrodynamic_load_ratio * hydrodynamic_load_ratio)
