function target_policy_params()
    return (
        ...
        minimum_rate_steering_fraction=0.65,
...
function target_policy(state, params)
    ...
    # Preserve full half-cycle steering while each joint has rate headroom,
    # then release only that optional residual as the observed joint response
    # consumes the oscillator-scaled rate envelope. Independent gates avoid
    # making one busy joint suppress useful authority at the other, and the
    # floor leaves the base target response available throughout the beat.
    ...
    steering_rate_usage2 = abs(qd2) /
        max(steering_rate_reference, eps(Float64))
    steering_rate_gate1 = params.minimum_rate_steering_fraction +
        (1 - params.minimum_rate_steering_fraction) /
        (1 + steering_rate_usage1^4)
    steering_rate_gate2 = params.minimum_rate_steering_fraction +
        (1 - params.minimum_rate_steering_fraction) /
        (1 + steering_rate_usage2^4)
    asymmetric_a1 = symmetric_a1 +
        params.half_cycle_asymmetry * turn_fraction *
        steering_rate_gate1 * abs(symmetric_a1)
    ...
    asymmetric_a2 = symmetric_a2 + params.posterior_asymmetry_share *
        params.half_cycle_asymmetry * turn_fraction *
        steering_rate_gate2 * abs(symmetric_a2)
