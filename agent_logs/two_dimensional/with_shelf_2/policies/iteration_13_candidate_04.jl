function target_policy_params()
    return (
        ...
        relative_flow_speed_floor=0.05,
...
function target_policy(state, params)
    ...
    # Relative crossflow is used only as disturbance headroom, not as a signed
    # steering command. Withhold optional positive course amplification when
    # the observed body-relative flow is predominantly lateral, while leaving
    # the base wave and negative course correction unchanged.
    crossflow_headroom_gate = if hasproperty(
        state,
        :relative_flow_velocity_body_U,
    )
        relative_flow = getproperty(state, :relative_flow_velocity_body_U)
        relative_streamwise = Float64(relative_flow[1])
        relative_crossflow = Float64(relative_flow[2])
        relative_flow_speed = hypot(relative_streamwise, relative_crossflow)
        crossflow_fraction = clamp(
            abs(relative_crossflow) / max(
                relative_flow_speed,
                params.relative_flow_speed_floor,
            ),
            0.0,
            1.0,
        )
        1 - crossflow_fraction
    else
        1.0
    end
    positive_course_gate = course_headroom_gate * steering_gate *
        crossflow_headroom_gate
