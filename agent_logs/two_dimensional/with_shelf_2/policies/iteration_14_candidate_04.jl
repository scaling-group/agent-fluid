function target_policy_params()
    return (
        ...
        translation_response_fraction=0.35,
        translation_response_limit=0.18,
...
function target_policy(state, params)
    ...
    # Bearing rate contains a yaw contribution of approximately -heading_rate.
    # Their sum therefore isolates target-line-of-sight motion caused by body
    # translation through the wake. Predict a bounded fraction of that drift
    # only after the observation window is populated; this avoids counting yaw
    # twice and leaves the established heading predictor intact.
    translational_los_rate = history_window_dt > eps(Float64) ?
        bearing_window_rate + heading_rate : 0.0
    translation_response = clamp(
        params.translation_response_fraction * heading_response_horizon *
            translational_los_rate,
        -params.translation_response_limit,
        params.translation_response_limit,
    )
    predicted_bearing = bearing - heading_response + translation_response
    route_turn = tanh(
