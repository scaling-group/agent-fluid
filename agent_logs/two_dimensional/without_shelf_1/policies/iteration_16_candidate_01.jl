function target_policy_params()
    return (
        ...
        close_lateral_start_distance_L=3.0,
        close_lateral_full_distance_L=1.5,
        lateral_offset_scale_L=2.0,
...
function target_policy(state, params)
    ...
    angular_steering_command = tanh(state.bearing / params.bearing_scale)
    lateral_steering_command = tanh(
        state.target_body_L[2] / params.lateral_offset_scale_L,
    )
    close_lateral_coordinate = clamp(
        (params.close_lateral_start_distance_L - state.distance_L) /
            max(
                params.close_lateral_start_distance_L -
                    params.close_lateral_full_distance_L,
                eps(params.close_lateral_start_distance_L),
            ),
        0.0,
        1.0,
    )
    close_lateral_gate = close_lateral_coordinate^2 *
        (3 - 2 * close_lateral_coordinate)
    target_direction_command =
        (1 - close_lateral_gate) * angular_steering_command +
        close_lateral_gate * lateral_steering_command
    steering_command = target_direction_command -
    ...
    # unchanged. At 3L and beyond this is exactly the sampled 0.50 anchor. The
    # close blend changes only target-direction conditioning, while phase gain
    # remains in [1, 1.50] and cannot attenuate the static request.
