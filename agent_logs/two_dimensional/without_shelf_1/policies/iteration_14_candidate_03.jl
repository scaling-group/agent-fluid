function target_policy_params()
    return (
        ...
        behind_reversal_distance_L=0.75,
...
function target_policy(state, params)
    ...
    # Preserve the best sampled phase-headroom anchor exactly until the target
    # passes behind the fish. Bearing retains lateral sign but intentionally
    # uses absolute forward separation, so correct its direction only after a
    # measured longitudinal overshoot. Unlike the failed symmetric fore-aft
    # gate, this one-sided transition never attenuates an ahead-of-fish request.
    bearing_drive = tanh(state.bearing / params.bearing_scale)
    behind_distance_L = max(Float64(state.target_body_L[1]), 0.0)
    behind_coordinate = clamp(
        behind_distance_L /
            max(
                params.behind_reversal_distance_L,
                eps(params.behind_reversal_distance_L),
            ),
        0.0,
        1.0,
    )
    behind_gate = behind_coordinate^2 * (3 - 2 * behind_coordinate)
    bearing_direction = 1.0 - 2.0 * behind_gate
    steering_command = bearing_direction * bearing_drive -
