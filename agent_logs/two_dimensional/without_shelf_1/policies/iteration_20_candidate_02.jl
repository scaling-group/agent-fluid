function target_policy_params()
    return (
        ...
        cross_track_far_gain=0.20,
        cross_track_far_blend_start_distance_L=3.0,
        cross_track_far_blend_full_distance_L=6.0,
...
function target_policy(state, params)
    ...
    # Preserve the sampled bearing and direct-body-rate anchor. Keep the
    # successful cross-track gain through the close approach, while using the
    # failed 0.20 branch's stronger upstream approach only in the far field.
    ...
    far_blend_coordinate = clamp(
        (state.distance_L - params.cross_track_far_blend_start_distance_L) /
            max(
                params.cross_track_far_blend_full_distance_L -
                    params.cross_track_far_blend_start_distance_L,
                eps(params.cross_track_far_blend_full_distance_L),
            ),
        0.0,
        1.0,
    )
    far_blend = far_blend_coordinate^2 * (3 - 2 * far_blend_coordinate)
    scheduled_cross_track_gain = params.cross_track_gain +
        (params.cross_track_far_gain - params.cross_track_gain) * far_blend
    steering_command = bearing_command +
        scheduled_cross_track_gain * cross_track_command -
