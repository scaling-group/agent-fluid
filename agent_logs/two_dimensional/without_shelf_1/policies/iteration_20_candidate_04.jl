...
function target_policy(state, params)
    ...
    # Preserve the sampled bearing and direct-body-rate anchor. Add the
    # successful bounded body-frame cross-track request so a large lateral
    # miss starts correcting before angular bearing grows near a pass.
    ...
    # Fade angular steering inside the final body length, where bearing is
    # poorly conditioned and first-crossing capture will terminate anyway.
    ...
    # Preserve the sampled command exactly outside the fade band. Inside it,
    # restore only the portion of the bounded cross-track request removed by
    # the common fade. Unlike bearing, this body-frame lateral error remains a
    # direct geometric capture signal and tends to zero at the target.
    faded_steering_command = steering_fade * bounded_steering_command
    cross_track_residual = (1 - steering_fade) *
        params.cross_track_gain * cross_track_command
    approach_steering_command = clamp(
        faded_steering_command + cross_track_residual,
        -1.0,
        1.0,
    )
    ...
    base_steering_bias = params.steering_bias_limit *
        approach_steering_command
    ...
    # unchanged. The phase gain is in [1, 1.50] and cannot attenuate the
    # sampled static anchor.
