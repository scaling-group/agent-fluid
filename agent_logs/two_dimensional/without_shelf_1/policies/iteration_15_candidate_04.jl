function target_policy_params()
    return (
        ...
        close_opposition_boost=0.05,
        close_boost_start_distance_L=2.75,
        close_boost_full_distance_L=2.0,
...
function target_policy(state, params)
    ...
    close_coordinate = clamp(
        (params.close_boost_start_distance_L - state.distance_L) /
            max(
                params.close_boost_start_distance_L -
                    params.close_boost_full_distance_L,
                eps(params.close_boost_start_distance_L),
            ),
        0.0,
        1.0,
    )
    close_gate = close_coordinate^2 * (3 - 2 * close_coordinate)
    active_opposition_boost = params.steering_opposition_boost +
        params.close_opposition_boost * close_gate
    phase_gain = 1.0 +
        active_opposition_boost * opposing_headroom
    steering_bias = base_steering_bias * phase_gain

    # Steering remains posterior-only; the anterior propulsion oscillator is
    # unchanged. At 2.75L and beyond this exactly preserves the sampled 0.50
    # anchor. Only the newly demonstrated close region can smoothly continue
    # toward 0.55, and no phase of the static request can be attenuated.
