...
function target_policy(state, params)
    ...
    # Keep the evidence-best static bias and direct-body-rate settings. Reject
    # a small amount of measured hydrodynamic yaw moment before it accumulates
    # into the sampled late curl. The moment observation is normalized by L^2;
    # sampled RMS values are order one on that scale, and tanh prevents this
    # new load channel from overriding the bearing command.
    steering_command = tanh(state.bearing / params.bearing_scale) -
        ...
        ) - params.yaw_moment_rejection * tanh(
            state.moment_z_L2 / params.yaw_moment_scale,
        )
