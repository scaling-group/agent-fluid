...
function target_policy(state, params)
    ...
    # Preserve the strongest sampled upstream-capable oscillator unchanged.
    # Its phase remains encoded in joint state rather than elapsed time.
    vdp_drive = params.oscillator_mu * (1 - (q1 / amp)^2) * qd1
    raw_a1 = vdp_drive - omega^2 * q1

    # Retain the gain-0.70 body-turn feedback that gave the best sampled
    # progress. Unlike failed target-bearing-rate brakes, this signal does not
    # mix target-vector translation into the rotational damping channel.
    ...
    # Isolate a smaller static posterior ceiling: the sampled 10-degree
    # ceiling retained a near-limit mean bend even with stronger turn damping.
