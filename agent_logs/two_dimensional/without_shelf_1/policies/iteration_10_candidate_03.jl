function target_policy_params()
    return (
        ...
        tail_lag_gain=0.8,
        tail_damping=0.65,
        posterior_rate_soft_limit=220.0 * pi / 180,
        posterior_rate_brake_gain=10.0,
...
function target_policy(state, params)
    ...
    # Restore the evidence-best bearing and direct body-rate anchor. Neither
    # term reacts to target-vector translation as failed rate variants did.
    ...
    nominal_a2 = omega^2 * (q2_target - q2) -
        2 * params.tail_damping * omega * qd2

    # Brake only the measured posterior rate above the soft threshold. Unlike
    # failed oscillator-phase allocation, this leaves the desired traveling
    # bend and steering request intact through most of each tailbeat.
    posterior_rate_excess = max(
        abs(qd2) - params.posterior_rate_soft_limit,
        0.0,
    )
    posterior_rate_brake = params.posterior_rate_brake_gain *
        posterior_rate_excess * sign(qd2)
    raw_a2 = nominal_a2 - posterior_rate_brake
