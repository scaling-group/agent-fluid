function target_policy_params()
    return (
        ...
        receding_speed_deadband_L=0.01,
        receding_speed_scale_L=0.05,
        recovery_bearing_scale=30.0 * pi / 180,
        recovery_gain=1.20,
...
function target_policy(state, params)
    ...
    # Keep the evidence-best bearing and direct body-rate controller unchanged
    # while the fish is closing on the target.
    bearing_command = tanh(state.bearing / params.bearing_scale)

    # Once the history-window trend shows sustained recession while the target
    # is off-axis, smoothly unload and briefly reverse only the posterior
    # steering request.  Initial padded history and positive closing speed
    # leave this gate exactly zero.
    receding_excess = max(
        -state.window_closing_speed_L - params.receding_speed_deadband_L,
        0.0,
    )
    receding_gate = tanh(
        receding_excess /
            max(params.receding_speed_scale_L, eps(params.receding_speed_scale_L)),
    )
    off_axis_gate = tanh(
        abs(state.bearing) / params.recovery_bearing_scale,
    )^2
    recovery_command = params.recovery_gain * receding_gate *
        off_axis_gate * bearing_command

    steering_command = bearing_command - recovery_command -
