...
function target_policy(state, params)
    ...
    # Preserve the evaluated positive bearing-to-tail sign, but lead the
    # geometric error with a smoothed bearing rate. A converging bearing backs
    # off the turn before yaw carries it through the target line; a diverging
    # bearing reinforces it. The independent clamp prevents wake noise from
    # dominating target geometry.
    ...
    # The bias fades inside the final body length, where bearing is poorly
    # conditioned and the first-crossing capture will terminate anyway.
