...
function target_policy(state, params)
    ...
    # Preserve the strongest sampled upstream-capable oscillator unchanged so
    # this candidate isolates the residual posterior steering ceiling. Its
    # phase remains encoded in joint state rather than time.
    vdp_drive = params.oscillator_mu * (1 - (q1 / amp)^2) * qd1
    raw_a1 = vdp_drive - omega^2 * q1

    # Keep the evidence-backed positive bearing sign and strongest sampled
    # heading-rate gain. The lower static ceiling isolates the mean posterior
    # request that remains after turn-rate damping subsides; target-rate and
    # body-lateral-velocity channels both worsened sampled behavior.
