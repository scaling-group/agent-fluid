...
function target_policy(state, params)
    ...
    # Keep the evidence-best static bias and direct-body-rate settings. Apply
    # one orthogonal bounded correction to measured body-frame lateral motion;
    # unlike failed bearing-rate feedback it does not differentiate the moving
    # target vector, and unlike failed approach relief it leaves zero-slip
    # far-field propulsion unchanged.
    steering_command = tanh(state.bearing / params.bearing_scale) -
        ...
        ) - params.lateral_velocity_damping * tanh(
            state.velocity_body_U[2] / params.lateral_velocity_scale,
        )
