...
function target_policy(state, params)
    ...
    # Keep the evidence-backed positive target-to-posterior sign without the
    # aggressive small-error gain of the sampled tanh(bearing / 25 deg) term.
    # The lateral component of the body-frame target unit vector is bounded,
    # coordinate-free, and grows smoothly as alignment worsens.
    lateral_direction = clamp(
        state.target_body_L[2] /
            max(state.distance_L, eps(state.distance_L)),
        -1.0,
        1.0,
    )
    steering_command = params.lateral_direction_gain * lateral_direction -
