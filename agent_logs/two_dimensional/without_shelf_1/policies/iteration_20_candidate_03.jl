...
function target_policy(state, params)
    ...
    # Preserve the sampled bearing and direct-body-rate anchor. The additive,
    # bounded body-frame cross-track request is the only sampled mechanism
    # that turns the upstream approach into a diagonal target capture.
    ...
    # Preserve the demonstrated far-field route while retaining slightly more
    # steering only in the terminal 1.4L. The request still fades to zero at
    # the first-crossing boundary, where bearing is poorly conditioned.
