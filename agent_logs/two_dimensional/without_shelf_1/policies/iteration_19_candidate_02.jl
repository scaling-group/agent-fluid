function target_policy_params()
    return (
        ...
        joint_rate_brake_start=220.0 * pi / 180,
        joint_rate_brake_full=255.0 * pi / 180,
        acceleration_command_limit=1650.0 * pi / 180,
    )
end

function rate_headroom_brake(raw_acceleration, joint_rate, params)
    rate_coordinate = clamp(
        (abs(joint_rate) - params.joint_rate_brake_start) /
            max(
                params.joint_rate_brake_full - params.joint_rate_brake_start,
                eps(params.joint_rate_brake_full),
            ),
        0.0,
        1.0,
    )
    outward_fade = rate_coordinate^2 * (3 - 2 * rate_coordinate)
    outward_scale = raw_acceleration * joint_rate > 0.0 ?
        1.0 - outward_fade : 1.0
    return outward_scale * raw_acceleration
end
...
function target_policy(state, params)
    ...
    # Preserve the sampled bearing and direct-body-rate anchor. Add a small,
    # bounded body-frame cross-track request so a large lateral miss starts
    # correcting before the angular bearing grows near a streamwise pass.
    ...
    # unchanged. The restored sampled phase gain is in [1, 1.50] and cannot
    # attenuate the base request.
    q2_target = unsteered_q2_target + steering_bias
    raw_a2 = omega^2 * (q2_target - q2) -
        2 * params.tail_damping * omega * qd2

    # The sampled arrival controller contacts both hard joint-rate limits.
    # Preserve commands below 220 deg/time and every braking command, but
    # smoothly remove acceleration that would drive either joint farther
    # outward as it approaches the 260 deg/time envelope.
    headroom_a1 = rate_headroom_brake(raw_a1, qd1, params)
    headroom_a2 = rate_headroom_brake(raw_a2, qd2, params)

    limit = params.acceleration_command_limit
    return (
        phi_ddot=(
            clamp(headroom_a1, -limit, limit),
            clamp(headroom_a2, -limit, limit),
        ),
    )
end
