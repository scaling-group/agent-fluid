...
function target_policy(state, params)
    ...
    # Restore the complete evidence-best geometric and direct-rate anchor.
    # Static bias, bearing sensitivity, and stronger rate damping all failed
    # to change the upper-loop topology when varied independently.
    ...
    unbounded_q2_target = tail_tangent_target - q1

    # Keep the posterior servo from pursuing a target outside its physical
    # angle envelope. The margin acts only on the combined extreme target, so
    # mid-range traveling-wave phase and geometric steering remain unchanged.
    q2_target = clamp(
        unbounded_q2_target,
