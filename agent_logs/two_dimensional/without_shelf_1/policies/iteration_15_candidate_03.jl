...
function target_policy(state, params)
    ...
    # Preserve the sampled positive steering sign without allowing shrinking
    # forward separation to amplify the request. Body-lateral target error is
    # normalized by L, retains the target side, and stays well conditioned
    # through a streamwise pass.
    lateral_drive = tanh(
        state.target_body_L[2] /
            max(params.lateral_error_scale_L, eps(params.lateral_error_scale_L)),
    )
    steering_command = lateral_drive -
