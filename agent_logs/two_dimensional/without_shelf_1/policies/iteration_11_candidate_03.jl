function target_policy_params()
    return (
        ...
        lateral_speed_floor=0.05,
        lateral_velocity_damping=0.30,
...
function target_policy(state, params)
    ...
    # Dampen measured lateral translation separately: the dimensionless slip
    # fraction is scale-free, bounded, and zero before cross-track motion
    # develops. Unlike bearing rate, it does not mix target-vector rotation
    # with translation.
    body_velocity_x = Float64(state.velocity_body_U[1])
    body_velocity_y = Float64(state.velocity_body_U[2])
    body_speed = hypot(body_velocity_x, body_velocity_y)
    lateral_slip = clamp(
        body_velocity_y /
            max(params.lateral_speed_floor, body_speed),
        -1.0,
        1.0,
    )
    lateral_motion_feedback = params.lateral_velocity_damping * tanh(
        lateral_slip / params.lateral_slip_scale,
    )
