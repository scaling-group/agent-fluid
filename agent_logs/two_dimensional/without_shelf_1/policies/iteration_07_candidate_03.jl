...
function target_policy(state, params)
    ...
    # Preserve the sampled upstream-capable oscillator unchanged so this
    # candidate brackets only the heading-rate gain. Its phase remains encoded
    # in joint state rather than time.
    vdp_drive = params.oscillator_mu * (1 - (q1 / amp)^2) * qd1
    raw_a1 = vdp_drive - omega^2 * q1

    # Keep the evidence-backed positive bearing sign and direct body-rate
    # feedback. Gain 0.70 was the best finite sample, while 1.05 slightly
    # relieved posterior angle but regressed progress; 0.80 brackets that
    # tradeoff without reintroducing failed target-bearing-rate feedback.
