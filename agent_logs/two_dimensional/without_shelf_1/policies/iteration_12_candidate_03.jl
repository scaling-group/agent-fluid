...
function target_policy(state, params)
    ...
    # Preserve it exactly while the fish is closing on the target. Once the
    # target distance is measurably increasing, smoothly reverse half of the
    # geometric bearing drive so the policy can recover instead of repeating
    # the terminal upper hook. Radial closing speed is unaffected by body
    # rotation, unlike the failed bearing-rate feedback.
    ...
    recovery_drive = params.recovery_bearing_reversal * recovery_gate *
        bearing_drive
