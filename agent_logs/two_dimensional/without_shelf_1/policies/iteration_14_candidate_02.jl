...
function target_policy(state, params)
    ...
    # Restore the complete evidence-best phase-headroom anchor. Sampled
    # receding and longitudinal gates all lost approach without changing the
    # upper-exit topology, so geometric steering is not staged here.
    bearing_drive = tanh(state.bearing / params.bearing_scale)
    steering_command = bearing_drive - params.turn_rate_damping * tanh(
    ...
    # Steering remains posterior-only and the anterior propulsion oscillator
    # is unchanged. Bound the desired posterior angle before the PD law so an
    # extreme traveling-wave target cannot keep pushing toward the 45 degree
    # hard stop. This does not attenuate any in-envelope phase.
    unbounded_q2_target = unsteered_q2_target + steering_bias
    q2_target = clamp(
        unbounded_q2_target,
