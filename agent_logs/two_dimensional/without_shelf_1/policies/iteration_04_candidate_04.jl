...
function target_policy(state, params)
    ...
    # Restore the only sampled gait with strong finite target approach. Joint
    # state carries oscillator phase; target feedback never shifts this
    # anterior propulsion equilibrium.
    vdp_drive = params.oscillator_mu * (1 - (q1 / amp)^2) * qd1
    raw_a1 = vdp_drive - omega^2 * q1

    # Retain the evidence-backed positive bearing sign, but isolate a modestly
    # lower static ceiling. Both sampled bearing-rate signs degraded the finite
    # proportional anchor, so no rate term enters this candidate.
