function target_policy_params()
    return (
        ...
        bearing_reverse_width_L=1.0,
        bearing_reverse_gain=0.50,
...
function target_policy(state, params)
    ...
    # Bearing folds the fore-aft target projection, so its sign alone cannot
    # distinguish an ahead target from one behind the fish. Preserve the
    # evidence-best drive exactly while the target is ahead, then smoothly
    # reverse only the bearing contribution after it moves behind. The sampled
    # positive-floor gate could weaken, but not unwind, the terminal loop.
    forward_target_L = -state.target_body_L[1]
    behind_coordinate = clamp(
        -forward_target_L /
            max(
                params.bearing_reverse_width_L,
                eps(params.bearing_reverse_width_L),
            ),
        0.0,
        1.0,
    )
    ...
    bearing_multiplier = 1 -
        (1 + params.bearing_reverse_gain) * behind_gate
    bearing_drive = bearing_multiplier *
