function target_policy_params()
    return (
        ...
        recovery_activation_distance_L=4.0,
        recovery_transition_width_L=1.0,
...
function target_policy(state, params)
    ...
    # Preserve the strongest sampled phase-opposition policy in the far field.
    # Only after a close approach may receding distance reverse the geometric
    # bearing drive. This retains the demonstrated upstream route while keeping
    # the parent's lower-excursion recovery mechanism local to its useful zone.
    ...
    recovery_distance_coordinate = clamp(
        (params.recovery_activation_distance_L - state.distance_L) /
            max(
                params.recovery_transition_width_L,
                eps(params.recovery_transition_width_L),
            ),
        0.0,
        1.0,
    )
    ...
    recovery_drive = params.recovery_bearing_reversal *
        recovery_distance_gate * recovery_receding_gate * bearing_drive
    steering_command = bearing_drive - recovery_drive -
    ...
    # Retain the evidence-best phase-opposition boost. Extra steering authority
    # is available only when the posterior target opposes the bounded request,
    # so it uses measured phase headroom rather than increasing outward bend.
    ...
    # Propulsion remains state-phased and steering remains posterior-only.
