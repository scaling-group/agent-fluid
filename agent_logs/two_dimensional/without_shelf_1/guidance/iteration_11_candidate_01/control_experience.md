...
- Retain the positive bearing-to-posterior-tail sign as the current finite
  ...
  route; do not continue that scalar sweep. Restore `10 deg` and test bounded
  phase-selective relief only when the steering bias reinforces the nominal
  posterior traveling-wave excursion. That implication is falsified if the
  phase allocation loses upstream progress like the global `8 deg` reduction,
  or repeats the upper exit without reducing posterior saturation or load; in
  that case retain the static anchor and test a different posterior-servo
  saturation mechanism, not bearing-rate lead, anterior steering, or another
  simultaneous weakening of all gait gains.
- Distance-only steering relief and measured near-limit braking are now
  negative controls for the persistent upper exit. Relative to the static
  `11 deg` best approach (`-7.89L` upstream head travel, `4.87L` closest
  approach, `0.424` progress, loads `406/4113`), fading the bias to a `0.70`
  floor inside `6L` still followed the same visible upper curl and retained the
  posterior angle/rate/command limits, while regressing to `-7.19L`, `4.93L`,
  and `0.393`; its lower `386/3879` load did not repair the route. On the
  static `10 deg` anchor, blending to an inward brake only during measured
  `40--45 deg` outward posterior motion was later still: it also retained the
  rate/command-cap hits and upper exit, worsened upstream travel and closest
  approach to `-6.88L` and `5.77L`, and bought only lower `357/3779` load.
  Avoid distance as a phase surrogate and avoid another late joint-boundary
  brake. A clean next test may anticipate saturation from the nominal
  posterior traveling-wave target and reduce bias only on the half-cycle where
  its sign reinforces that target, while preserving the static anchor on the
  opposing half-cycle. This implication is falsified if posterior peak/load do
  not fall, the same upper-exit topology survives, or upstream progress falls
  to the distance-relief regime; then restore the unmodified static anchor and
  abandon this phase-gating direction.
