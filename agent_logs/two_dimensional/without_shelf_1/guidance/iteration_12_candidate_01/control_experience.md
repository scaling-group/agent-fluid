...
- Retain the positive bearing-to-posterior-tail sign as the current finite
  ...
  when testing a distinct normalized yaw/load signal; accept such a signal
  only if it keeps upstream progress while reducing the roughly `+1.8L`
  drift/curl and either closest approach or saturation/load.
- Do not use instantaneous body-frame lateral velocity as the route-error
  derivative for this `0.90`-period posterior-steered gait. Two normalizations
  now fail the same boundary: a `0.20*tanh(v_y/0.10)` correction cuts upstream
  head travel from the plain `11 deg` anchor's `-7.89L` to `-4.41L`, worsens
  closest approach from `4.87L` to `5.82L`, raises RMS force/moment from
  `406/4113` to `428/4648`, and leaves head-y exit at `+1.79L`; a bounded
  `v_y/hypot(v_x,v_y)` correction is milder but still regresses travel and
  approach to `-6.79L`/`5.47L` with `+1.77L` exit. Thus changing the velocity
  normalization or gain is not evidence of a new lateral mechanism: restore
  the static anchor and test a physically distinct bounded yaw/load signal.
  Reconsider velocity only if a later contract supplies a filtered or
  cycle-averaged signal that separates gait-scale oscillation from route drift.
