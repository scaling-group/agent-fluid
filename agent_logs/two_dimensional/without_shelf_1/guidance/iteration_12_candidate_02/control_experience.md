...
- Retain the positive bearing-to-posterior-tail sign as the current finite
  ...
  damping or its near-zero sensitivity. Because the inherited isolated bias
  decrease from `10` to `8 deg` also collapsed progress to `0.095`, the next
  orthogonal static-steering probe should move upward by only a small increment
  while preserving the gait and rate anchor. That direction is falsified if
  posterior saturation or load increases without reducing the upper drift or
  improving closest approach; then return to `10 deg` and test phase-aware
  posterior steering allocation, not bearing-rate lead, anterior steering, or
  another simultaneous weakening of all gait gains.
- Do not use instantaneous body-lateral velocity as a damping proxy for the
  persistent upper-exit turn. Two independently scaled posterior-only tests
  fail the same boundary: direct `gain/scale=0.20/0.10` feedback cuts the
  `11 deg` anchor's upstream travel/progress from `-7.89L/0.424` to
  `-4.41L/0.239`, worsens closest approach from `4.87L` to `5.82L`, raises RMS
  force/moment from `406/4113` to `428/4648`, and leaves head-y drift at
  `+1.79L`; speed-normalized slip feedback at `0.30/0.35` likewise regresses
  travel/progress to `-6.79L/0.375` and closest approach to `5.47L` while
  retaining `+1.77L` drift. This rules out further lateral-velocity gain or
  normalization sweeps for this static-posterior architecture. The sampled
  fore-aft gate sets a narrower alternative boundary: weakening bearing drive
  to a same-sign `0.15` floor preserves the best approach (`4.89L`) but not the
  upper exit, so a later recovery test must preserve full ahead-target drive
  and change sign only after the target is observably behind; reject that
  mechanism too if the gate never activates or if it changes pre-approach
  propulsion, switching, saturation, or load without changing the route.
