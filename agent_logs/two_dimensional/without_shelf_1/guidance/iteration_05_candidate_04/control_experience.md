...
- Do not treat low loads from a globally weakened gait as successful
  ...
  together.
- Direct body-turn damping is a useful signal but not yet a solved steering
  law. Holding the finite anchor's gait and `10 deg` posterior bias fixed while
  subtracting `0.35` times normalized heading rate improved upstream head
  displacement from `-2.73L` to `-4.69L`, progress from `0.132` to `0.255`,
  and mean distance from `10.65L` to `9.48L`. Yet the released sheet retains
  the same upper J-turn, closest approach worsens from `6.34L` to `7.30L`, the
  posterior angle and both rates/commands still saturate, and RMS force/moment
  rise from `196/2014` to `284/2797`. An assigned-parent inherited log also
  records a lower-load `146/1539` failure with only `0.040` progress and an
  `8.37L` closest approach, so load reduction without a changed exit topology
  is not positive navigation evidence. Preserve the direct turn-rate
  observation only as a sign-preserving brake with enough authority to reduce
  an aligned turn; do not let it amplify or reverse geometric bearing
  steering. This mechanism is falsified if a stronger sign-preserving brake
  loses early upstream displacement or leaves the same saturation and
  boundary-exit topology without a better closest approach; then stop adding
  rate feedback and isolate posterior lag or static bias while keeping the
  anchor gait.
