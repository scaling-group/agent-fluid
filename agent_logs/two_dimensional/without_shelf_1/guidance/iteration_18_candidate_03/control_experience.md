...
- Supersede the phase-headroom-only route anchor with the additive body-frame
  cross-track result. Four sampled copies of the functional
  `0.50`-opposition, `0.18*tanh(target_body_L[2]/2L)` controller reproduce the
  same target capture at `88.13` release units and `0.747L` final/minimum
  distance. This is deterministic confirmation, not four independent
  robustness trials. Against the inherited near-target lateral replacement,
  which exits high after reaching only `1.89L`, the additive far-field term
  changes head-y travel from `+1.79L` to `-4.44L`, improves mean distance from
  `6.45L` to `2.74L`, and lowers RMS relative crossflow and force/moment from
  `0.302` and `568/5495` to `0.289` and `445/4597`; mean head x-velocity
  remains more upstream than local flow. Therefore preserve addition rather
  than near-target replacement: a bounded target-relative lateral request must
  act while the miss is still large and must not attenuate the bearing or
  propulsion anchor. This lesson is valid only while capture and active
  upstream motion survive. The successful policy still reaches the posterior
  angle limit and both rate/command caps, and its keyframes retain a broad
  midcourse upper excursion, so later workers should bracket cross-track gain
  or one desaturation axis at a time and restore the exact `0.18`, `2L` anchor
  if capture, distance integral, or load regresses.
