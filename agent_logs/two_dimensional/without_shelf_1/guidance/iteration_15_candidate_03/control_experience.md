...
- Treat posterior phase-headroom gain as a narrow, non-monotone mechanism, not
  an open-ended authority knob.  On the otherwise fixed `0.90`-period,
  `11 deg`, `0.70/0.35` controller, raising the gain from `0.35` to `0.50`
  improves closest approach from `3.03L` to `2.13L` and upstream head travel
  from `-11.33L` to `-12.28L`, but retains the upper exit (`+1.78L` head-y),
  both rate/command caps, and raises RMS force/moment from `511/5305` to
  `535/5416`; `0.55` then collapses to `5.70L` closest approach, `0.206`
  progress, and only `-3.89L` upstream travel with moment RMS `5969`.
  Therefore do not increase this gain beyond `0.50` or infer a monotone route
  gradient from closest approach alone.  A separate `42 deg` posterior-target
  clamp confirms that cap relief is not navigation by itself: it lowers angle
  and loads but leaves the `+1.78L` exit while reducing progress to `0.074`.
  These boundaries apply to this propulsion gait and actuator envelope;
  revisit them only after a distinct normalized steering observation preserves
  the `0.35` anchor's upstream travel while materially changing the hook.
