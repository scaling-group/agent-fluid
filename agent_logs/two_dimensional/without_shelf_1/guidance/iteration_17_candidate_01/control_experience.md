...
- Do not attenuate the static posterior request using gait phase, approach
  ...
  lateral change.
- Use measured body-frame lateral target offset as a small additive correction
  to bearing, not as a close-range bearing replacement. On the complete
  `0.50` opposing-headroom anchor, adding
  `0.18*tanh(target_body_L[2]/2L)` changes the sampled upper-domain failure
  into capture at `0.747L` after `88.13` release units: head-y displacement
  changes from `+1.78L` to `-4.44L`, progress rises from `0.509` to `0.940`,
  and RMS relative crossflow/force/moment fall from `0.306/535/5416` to
  `0.289/445/4597` despite retained joint-envelope contact. This is a genuine
  route correction because head motion remains upstream of mean local flow.
  By contrast, blending bearing into lateral offset only below `3L` still
  exits high at `+1.79L`, reaches only `1.89L`, and raises loads to
  `568/5495`; do not continue near-target replacement thresholds. Preserve
  the full bearing, rate-damping, and propulsion anchor when tuning the
  additive term, and reject stronger continuations if they lose capture,
  undershoot laterally, or raise load without improving arrival/distance.
