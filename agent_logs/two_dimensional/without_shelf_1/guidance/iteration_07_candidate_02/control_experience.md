...
- Retain the positive bearing-to-posterior-tail sign as the current finite
  ...
  lateral and bearing-rate feedback out of the next continuation and vary only
  bounded direct turn-rate gain or sensitivity while preserving the gain-
  `0.70` gait. The isolated `1.05` gain continuation now supplies the
  falsification boundary: it preserves the same roughly `+1.20L` upper exit,
  ...
  gain response as turning over at or before `0.70`: do not use a gain above
  `0.70` as another loop repair. A sensitivity-only test may retain that
  bounded ceiling and change `turn_rate_scale`, but it is useful only if it
  preserves the `0.70` upstream progress and reduces the upper displacement or
  loads. If it repeats the same exit or loses progress, abandon this
  heading-rate continuation rather than lowering the scale again, adding a
  second observed rate, or weakening propulsion.
