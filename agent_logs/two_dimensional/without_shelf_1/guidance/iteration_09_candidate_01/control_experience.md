...
- Retain the positive bearing-to-posterior-tail sign as the current finite
  ...
  another rate test. Direct normalized body-turn damping is now a bounded
  upstream-progress mechanism, not a loop or desaturation repair. With the
  `0.90`-period gait and `10 deg` bias fixed, inherited gains `0`, `0.35`, and
  `0.70` improved head-x travel from `-2.73L` to `-4.69L` to `-6.93L` and
  progress from `0.132` to `0.255` to `0.380`; later `0.80` and `1.05` gains,
  and a `0.70` gain with rate scale reduced from `0.35` to `0.25`, all
  regressed closest approach and progress. Current sampled evidence sharpens
  the other steering boundaries: at the `0.70/0.35` anchor, raising only bias
  from `10` to `11 deg` improved progress from `0.380` to `0.424`, closest
  approach from `5.33L` to `4.87L`, and head-x travel from `-6.93L` to
  `-7.89L`, whereas reducing only bearing scale from `25` to `20 deg` regressed
  them to `0.310`, `5.77L`, and `-5.62L`. The `11 deg` gain is not permission
  to keep raising static bias: it also raised RMS force/moment from `325/3331`
  to `406/4113`, and every released sheet retained the broad upper loop while
  reaching both joint-rate and acceleration caps and roughly the posterior
  hard stop (`0.734--0.781 rad`). For this gait, retain `11 deg`, `25 deg`, and
  `0.70/0.35` as the finite approach anchor; stop tuning turn-rate gain/scale
  or bearing sensitivity. The next reusable repair should act on the
  posterior servo's infeasible target or phase allocation while keeping
  steering out of the anterior oscillator. A desired-posterior-angle bound is
  falsified if it destroys upstream progress without lowering posterior peak
  angle, command effort, or loop severity; then restore the unclipped anchor
  and test smooth phase-selective allocation, not bearing-rate lead,
  lateral-rate damping, sharper bearing response, or another global gait
  weakening. This boundary must be re-established if the propulsion gait or
  bias ceiling changes materially.
