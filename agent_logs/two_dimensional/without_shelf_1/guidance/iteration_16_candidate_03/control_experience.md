...
- Close gating does not rescue the memoryless receding reversal, so retire that
  mechanism rather than moving its distance thresholds again.  Relative to the
  unclipped phase-headroom anchor (`-11.33L` head-x travel, `3.03L` closest
  approach, `0.517` progress), smooth reversal gates spanning `4--3L` and
  `3.75--2.75L` regress to `-10.15/-10.83L`, `3.13/3.10L`, and
  `0.495/0.507`.  Both retain the visible upper hook, roughly `+1.79L` head-y
  exit, and exact joint-rate/command-cap hits; neither produces the required
  second approach.  The inherited globally active receding relief is worse
  again (`4.93L`, `0.444`), while a signed body-frame fore-aft gate repeats the
  hook at `3.45L` and `0.449`.  Thus neither receding distance nor longitudinal
  target sign is a demonstrated recovery-state surrogate in this rollout.
  Restore the full phase anchor for future structural tests; falsify this
  negative boundary only with a controller that preserves its upstream leg and
  changes the route topology, not with a marginal closest-approach change.
- The opposing-posterior-headroom sweep has a sharp upper boundary at `0.50`;
  retire further strength increases rather than localizing them with another
  distance gate.  Raising the `0.35` phase anchor to `0.50` improves head-x
  travel and closest approach from `-11.33L/3.03L` to `-12.28L/2.13L`, but
  retains the upper hook, about `+1.78L` head-y exit, exact joint-rate/command
  cap hits, and increases RMS force/moment from `511/5305` to `535/5416`.
  The inherited globally active `0.55` branch had already collapsed to
  `-3.89L` travel, `5.70L` closest approach, and `0.206` progress.  The sampled
  close-only continuation from `0.50` toward `0.55` now also fails: despite
  reaching `-12.66L`, it worsens closest approach to `2.21L`, preserves the
  same visible upper exit and saturation maxima, and raises RMS force/moment
  to `566/5660`.  Restore the complete unclipped `0.50` policy as the approach
  anchor for structurally different observations.  Falsify this boundary only
  if a later headroom-strength change produces capture or a changed lateral
  route without sacrificing the upstream leg; extra x travel on the same hook
