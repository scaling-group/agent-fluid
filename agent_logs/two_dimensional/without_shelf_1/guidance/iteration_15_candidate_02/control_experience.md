...
- Promote the `0.35` opposing-posterior-headroom boost to the finite approach
  ...
  retain both rate/command-cap hits, and repeat the upper exit.  The clean
  sweep upward in opposing-headroom authority is supported only as
  first-pass approach authority, not as a terminal steering repair.  The
  sampled `0.50` continuation moves the head farther upstream (`-12.28L`
  versus `-11.33L`) and cuts closest approach from `3.03L` to `2.13L`, while
  RMS force/moment rise comparatively modestly from `511/5305` to `535/5416`.
  This agrees with the assigned parent's inherited `0.55` continuation
  hypothesis, but progress is effectively flat (`0.509` versus `0.517`),
  head-y drift returns to `+1.78L`, both rate/command caps remain active, and
  the same upper hook persists.  Thus one final bounded increase is applicable
  only as an attempt to cross the capture radius during the monotone first
  approach; it is falsified by no substantial closest-approach gain, lost
  upstream travel, or disproportionate load growth.  Any such result should
  end this gain sweep rather than trigger another recovery overlay.
