...
- Make the sampled additive body-frame cross-track term the finite arrival
  anchor, while keeping its evidence boundary narrow. Adding
  `0.18*tanh(target_body_L[2]/2L)` to the complete `0.50` bearing/body-rate
  controller changes the repeated upper-hook topology into a diagonal descent
  and reaches the `0.75L` target after `88.13` release units. It improves
  minimum/mean distance and progress from `2.13L/6.46L/0.509` for the isolated
  `0.50` policy to `0.747L/2.736L/0.940`, changes head-y displacement from
  `+1.78L` to `-4.44L`, and lowers RMS force/moment from `535/5416` to
  `445/4597`. The result is specifically evidence for a small additive
  correction active while lateral miss is large: blending bearing toward
  lateral offset only inside `3L` reaches `1.89L` but repeats the upper exit,
  so do not treat late representation replacement as equivalent. Preserve the
  successful gait and cross-track term as the reference for later effort or
  robustness tests; the posterior angle and both joint rate/command caps still
  activate, so accept desaturation only if capture and the diagonal topology
  survive. Four later sampled artifacts reproduce exactly the same keyframe
  sheet and every physical metric (`88.13` arrival, `2.736L` mean distance,
  `0.940` progress, and `445/4597` RMS force/moment), but their policies differ
  only in comments. Count this as deterministic reproduction at one snapshot,
  not four independent control or robustness samples. It supports isolating a
  conservative acceleration-ceiling test while preserving all route feedback;
  falsify a lower ceiling if capture, diagonal topology, or approach time is
  lost without a load benefit, then restore `1650 deg/time^2` rather than
  stacking a previously failed clipping, recovery, velocity, or phase overlay.
  One wake phase still does not establish robustness: reopen cross-track
  gain/scale only if a held-out phase or layout misses capture, and test that
  axis directly.
