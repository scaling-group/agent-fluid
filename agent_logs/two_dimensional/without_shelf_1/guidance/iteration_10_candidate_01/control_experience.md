...
- Retain the positive bearing-to-posterior-tail sign as the current finite
  ...
  continue phase-allocation strength or nominal-headroom sweeps. A distinct
  posterior-servo test may act only on measured near-limit outward angle/rate
  while preserving the request elsewhere; reject it too if upstream progress
  falls materially or the upper exit persists without reduced saturation.
