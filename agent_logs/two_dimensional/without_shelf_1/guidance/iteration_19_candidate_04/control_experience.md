...
- Make the additive body-frame cross-track term the finite arrival anchor,
  ...
  sweeps. An above-anchor cap is only an unproven navigation-authority bracket,
  applicable because both successful commands contact `1650` and the episode
  gives effort zero score weight; reject it if capture, mean distance, or route
  topology regresses, or if loads rise without an arrival benefit.
