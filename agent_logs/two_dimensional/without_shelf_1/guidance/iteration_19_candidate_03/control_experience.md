...
- Treat `0.18*tanh(target_body_L[2]/2L)` added to bearing as the current
  capture anchor, not as a monotone gain direction. Four sampled copies reach
  the `0.75L` circle deterministically at `88.13` release units with
  `(-10.92,-4.44)L` head travel, `2.736L` mean distance, and `445/4597` RMS
  force/moment. The assigned parent's isolated `0.20` gain passes as close as
  `1.123L` but then restores the nearly vertical upper hook, exits after
  `76.17` units at `+1.78L` head-y travel, and raises loads to `490/4800` while
  retaining the same angle, rate, and command caps. Therefore close upward
  gain tuning and do not mistake a closer failed pass for a route improvement;
  test only the lower side in isolation, and retain it only if semantic capture
  survives with better arrival, distance integral, or load.
