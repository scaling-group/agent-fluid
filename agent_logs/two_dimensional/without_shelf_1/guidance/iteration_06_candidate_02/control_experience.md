...
- Retain the positive bearing-to-posterior-tail sign as the current finite
  ...
  `0.70` gait. A stronger gain is useful only if it retains upstream progress
  and bends away from the upper boundary; unchanged upper exit, rapid
  downstream motion, or worse loads without improved clearance falsifies the
  continuation and calls for bracketing the gain/scale rather than adding a
  second observed signal or weakening propulsion.
