...
- Do not relieve posterior steering according to anterior gait phase throughout
  ...
  safety bound. Preserve the full far-field wave rather than spending another
  candidate on posterior relief. Inherited follow-ups also close instantaneous
  body-lateral velocity as the missing orthogonal signal: absolute damping
  reduced upstream travel from `-7.89L` to `-4.41L` and raised loads to
  `428/4648`, while speed-normalized slip still regressed travel/approach to
  `-6.79L/5.47L`; both retained about `+1.8L` head-y drift and the upper exit.
- Keep the `0.35` posterior phase-headroom boost as the current finite approach
  anchor, but do not call it a lateral steering solution. Relative to the
  static `11 deg` policy it improves upstream head travel from `-7.89L` to
  `-11.33L`, closest approach from `4.87L` to `3.03L`, progress from `0.424`
  to `0.517`, and mean distance from `7.52L` to `6.44L`; however, the visual
  route still ends in the same upper hook, head-y exit remains `+1.69L`, both
  rate/command caps remain active, and RMS force/moment rise to `511/5305`.
  Neither sampled reactive substitute supplies the missing route correction:
  a `1.5` receding-distance bearing reversal reaches `2.67L` but cuts travel
  to `-7.45L`, worsens mean/final distance to `7.59/7.39L`, and still exits at
  `+1.78L`; `0.15` instantaneous normalized-yaw-moment rejection retains the
  curl, reaches only `4.99L`, and raises loads over the static anchor to
  `439/4516`. Thus preserve phase-headroom allocation while the target is
  ahead, and avoid switching on receding distance, instantaneous slip, or
  instantaneous load alone. A signed normalized body-frame forward target
  projection remains a distinct overshoot discriminator because the supplied
  bearing discards that sign; it is useful only if a later rollout preserves
  the `3.03L`-class approach and changes the terminal lateral route.
