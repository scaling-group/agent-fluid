...
- Make the additive body-frame cross-track term the finite arrival anchor, but
  do not count duplicated deterministic rollouts as robustness evidence.
  Adding `0.18*tanh(target_body_L[2]/2L)` to the complete `0.50`
  bearing/body-rate controller changes the inherited upper hook into the
  visually confirmed diagonal descent and reaches the `0.75L` target after
  `88.13` release units. Relative to the `0.50` near-miss it improves
  closest/mean distance and progress from `2.13L/6.46L/0.509` to
  `0.747L/2.736L/0.940`, changes head-y travel from `+1.78L` to `-4.44L`, and
  lowers RMS force/moment from `535/5416` to `445/4597`. The four sampled
  policies have identical executable code and rollout evidence apart from
  comments, so they establish one fixed-snapshot result, not wake-phase or
  layout generality. They also retain the exact posterior angle limit and both
  joint rate/command caps. Preserve the complete arrival controller while
  isolating any actuator refinement. Two inherited descendants that reverted
  to about `+1.78L` head-y travel left the domain despite passing within
  `1.12--1.56L`; because their implementations are unavailable here, use them
  only as a retention boundary, not a mechanism attribution. Reject any later
  refinement that loses the negative-y route or first crossing even if it
  reduces effort or produces a close pass.
