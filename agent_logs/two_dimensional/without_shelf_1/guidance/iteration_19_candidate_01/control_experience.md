...
- Treat the sampled `0.18` cross-track success as one deterministic operating
  point, not as evidence of a broad gain plateau. Four sampled policies that
  differ only in comments produce byte-identical released keyframes and the
  exact same `88.13` arrival, `2.736L` mean distance, `-10.925/-4.435L` head
  displacement, and `445/4597` RMS force/moment. Thus duplicate evaluations
  strengthen the baseline's reproducibility under the fixed snapshot but do
  not establish robustness to wake phase or cross-track gain. Preserve the
  rest of this captured route and vary the additive cross-track coefficient
  alone in small steps; an increment is falsified if it loses capture, grows
  the broad bend, or raises loads without reducing arrival time or distance
  integral.
