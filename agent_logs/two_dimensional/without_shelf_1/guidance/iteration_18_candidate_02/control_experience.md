...
- Make the additive body-frame cross-track controller the finite arrival
  anchor, but do not mistake replication for a gain sweep.  Relative to the
  isolated `0.50` opposing-headroom policy, which travels `-12.28L` upstream
  yet passes no closer than `2.13L` and exits with about `+1.78L` y drift,
  adding `0.18*tanh(target_body_L[2]/2L)` changes the visible upper-hook route
  into an early diagonal descent and target capture.  The resulting controller
  reaches `0.747L` after `88.129` release units with `0.940` progress and head
  displacement `(-10.92,-4.44)L`; it also lowers RMS force/moment from the
  isolated anchor's `535/5416` to `445/4597`.  Relative to the close-only
  continuation, RMS crossflow likewise falls from `0.306` to `0.289`.  Four
  current samples reproduce those metrics exactly, but their policies are
  functionally identical, so they establish deterministic repeatability only.
  The inherited close-only `0.55` continuation instead reaches `2.21L`, keeps
  the upper exit, and raises load to `566/5660`, showing that lateral geometry
  must act while the miss is large rather than being added as another late
  phase repair.  Preserve the `0.18/2L` route controller as the reference; its
  posterior angle and joint rate/command caps still activate, so test at most
  one effort axis at a time and accept desaturation only if capture and the
  diagonal topology survive.  Reopen cross-track gain or scale only when a
  distinct wake phase or layout fails, because the present duplicates provide
  no parameter sensitivity on that axis.
