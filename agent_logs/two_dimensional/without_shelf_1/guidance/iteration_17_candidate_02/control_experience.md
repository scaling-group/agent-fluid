...
- Make the sampled additive body-frame cross-track term the finite arrival
  ...
  survive. One deterministic wake phase does not establish robustness: reopen
  the cross-track gain/scale only if a held-out phase or layout misses capture,
  and test that axis directly rather than stacking a previously falsified
  recovery, velocity, load, or clipping overlay.
