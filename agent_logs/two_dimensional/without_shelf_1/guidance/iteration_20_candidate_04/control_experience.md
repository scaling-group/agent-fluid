...
- Treat `cross_track_gain=0.18` and the `1650 deg/time^2` policy ceiling as
  narrow route anchors, not locally monotone tuning knobs. Four sampled
  reproductions at `0.18/1650` capture after `88.13` release units with
  `2.736L` mean distance and `445/4597` RMS force/moment. Changing only the
  cross-track gain to `0.17` or `0.20` instead restores the upper exit: the
  former approaches only `3.27L`, while the latter gets to `1.12L` but turns
  away, and both end near `+1.8L` head-y displacement. Changing only the
  command ceiling to `1550` or `1700` also loses capture; the lower ceiling
  retains angle/rate saturation and raises loads to `715/6922`, while the
  upper ceiling approaches only `5.73L`. Restore the exact anchors and stop
  scalar gain/ceiling sweeps. A distinct near-capture continuation is testable
  only if it is identical to the successful command outside the existing
  `0.75--1.50L` fade band and separates a geometrically vanishing lateral
  error from poorly conditioned angular feedback; it is falsified if capture
  is lost, the upper exit returns, or load rises without a shorter route.
