...
- Treat phase-opposition headroom as two bounded anchors, not a monotone gain
  or a solved lateral controller. Relative to the plain `11 deg` bias, `0.35`
  improves upstream travel/closest approach/progress from
  `-7.89L/4.87L/0.424` to `-11.33L/3.03L/0.517`. Continuing to `0.50` improves
  travel and closest approach again to `-12.28L/2.13L`, with mean head speed
  still more upstream than local flow, but progress slips to `0.509`, the
  upper exit remains at about `+1.78L` head-y, both rate/command caps remain
  active, and RMS force/moment rise from `511/5305` to `535/5416`. Thus retain
  `0.35` as the score/progress anchor and use `0.50` only as the demonstrated
  close-approach anchor for a distinct structural near-target test. The upper
  tuning boundary is closed: global `0.55` collapses
  travel/approach/progress to `-3.89L/5.70L/0.206`, while adding the same
  increment only below `2.75L` worsens closest approach from `2.13L` to
  `2.21L`, retains the hook and cap contacts, and raises loads to `566/5660`.
  Do not move those thresholds or stack another phase-gain gate. Likewise,
  signed-forward bearing gating and two receding-distance reversals regress
  travel/approach to `-8.49L/3.45L`, `-10.15L/3.13L`, and
  `-10.83L/3.10L`, consistent with inherited lateral-velocity, yaw-moment, and
  posterior-clipping failures. A future observation is useful only if it
  preserves its chosen anchor's upstream leg and changes the upper-hook route
  or capture distance; lower load without that route change is lost
  propulsion. Identical policy checksums and identical metrics are duplicate
  evidence, not an independent positive result.
