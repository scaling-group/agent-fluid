...
- Treat receding-distance reversal as a near-target recovery mechanism, not a
  global route controller. Against the plain `11 deg` anchor (`4.87L` closest,
  `0.424` progress), normalized yaw-moment rejection regresses closest approach
  to `4.99L` and raises RMS loads from `406/4113` to `439/4516`; do not repeat
  that moment-feedback branch without evidence of a different signal scale or
  sign. Phase-opposition boost instead gives the strongest sampled far-field
  route (`-11.33L` upstream head travel, `6.44L` mean distance, `3.03L` closest,
  `0.517` progress), but with higher `511/5305` RMS loads. The assigned parent's
  globally active receding-speed reversal reaches the best `2.67L` closest
  approach and lowers posterior excursion from about `0.78` to `0.713 rad` and
  RMS loads to `367/3880`, yet weakens upstream travel to `-7.45L`, mean distance
  to `7.59L`, and progress to `0.405`; both keyframe sheets still end in the
  same upper-domain hook. Therefore preserve phase opposition outside the
  close-approach region and test receding reversal only behind a smooth
  distance gate. Falsify that composition if it loses the far-field anchor's
  approach metrics before the gate, fails to beat `3.03L` closest approach, or
  retains the upper exit without reducing posterior excursion or load.
