...
- Retain the positive bearing-to-posterior-tail sign as the current finite
  ...
  another rate test. Direct normalized body-turn damping has a bounded useful
  range, but is not a loop or desaturation solution. With every anchor
  parameter fixed, gains `0`, `0.35`, and `0.70` improve head displacement
  monotonically from `-2.73L` to `-4.69L` to `-6.93L` and progress from
  `0.132` to `0.255` to `0.380`; gain `0.70` also reaches `5.33L`. The clean
  bracket closes further gain/sensitivity tuning: `damping/scale=0.80/0.35`,
  `1.05/0.35`, and `0.70/0.25` regress to `-6.35/-6.67/-5.30L` upstream
  travel, `5.66/5.64/6.41L` closest approach, and `0.351/0.369/0.291`
  progress. Reducing only bearing scale from `25` to `20 deg` at the
  `0.70/0.35` anchor also regresses upstream travel from `-6.93L` to `-5.62L`
  and progress from `0.380` to `0.310`. All keyframe sheets retain the broad
  upper loop, roughly `+1.8L` head-y drift, and both joint-rate/command-cap
  hits. For this gait, keep `0.70/0.35` body-rate feedback and `25 deg` bearing
  scale; do not raise damping or near-zero sensitivity.
  ...
  `7.73L`, and upstream travel falls from `-4.69L` to `-3.40L`; keep lateral
  and bearing-rate feedback excluded. Raising only static posterior bias from
  `10` to `11 deg` does improve upstream travel from `-6.93L` to `-7.89L`,
  closest approach from `5.33L` to `4.87L`, and progress from `0.380` to
  `0.424`, confirming that posterior steering still controls useful approach.
  But it leaves the same upper exit and `+1.8L` head-y drift, moves posterior
  peak angle from `0.770` to `0.781 rad` (within `0.005 rad` of the hard
  limit), and raises RMS force/moment from `325/3331` to `406/4113`. Thus the
  `11 deg` static continuation fails the topology/load boundary despite its
  better score: retain `10 deg` and test phase-aware posterior allocation that
  removes bias only when it compounds the unsteered tail target. Such
  allocation is reusable only if it preserves upstream progress and closest
  approach while reducing upper drift, posterior saturation, or load; losing
  progress with the same loop falsifies it and calls for restoring the static
  `10 deg` anchor, not increasing bias or reopening the rate sweep.
