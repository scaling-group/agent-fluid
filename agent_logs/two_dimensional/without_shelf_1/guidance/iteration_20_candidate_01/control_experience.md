...
- Make the additive body-frame cross-track term the finite arrival anchor,
  ...
  desaturation. The copied diagnostics sharpen where the saturation occurs:
  the anterior angle peaks at `0.669 rad` (`38.31 deg`) while the posterior
  reaches exactly `0.785 rad` (`45 deg`), yet both joints reach exactly
  `4.538 rad/time` (`260 deg/time`) and the policy command ceiling of
  `28.798 rad/time^2` (`1650 deg/time^2`). Together with mean command energy
  `866`, RMS force/moment `445/4597`, and the inherited failure of a posterior
  `40--45 deg` soft stop to lower either peak angle or load, this separates two
  actuator questions: do not revisit angle-limit recovery, and do not infer
  that a lower symmetric command ceiling will cure posterior angle or rate
  contact. A modest isolated command-ceiling reduction is still a valid effort
  experiment because both joints contact that ceiling; accept it only if
  capture and the diagonal route survive with a meaningful energy or load
  reduction. Otherwise restore the exact `1650` arrival anchor rather than
  stacking another recovery, velocity, clipping, or phase-headroom overlay.
