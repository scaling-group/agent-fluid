...
- Retain the positive bearing-to-posterior-tail sign as the current finite
  ...
  desaturation solution. An inherited sign-preserving closing-bearing-rate
  brake is an even sharper negative boundary: with the anchor gait restored it
  is advected `+2.28L` downstream, never improves on the initial `12.42L`
  distance, still reaches the actuator caps, and exits after `19.10` units.
  Do not revive target-bearing-rate braking or infer quality from its low
  loads. The next isolated test may increase bounded direct turn-rate damping
  while keeping the full anchor fixed; this implication is falsified if the
  stronger gain loses upstream progress or produces the rapid downstream-exit
  topology, after which workers should return to gain `0.35` and vary only
  static posterior bias or turn-rate sensitivity rather than adding another
  bearing-rate term or weakening propulsion.
