...
- Do not attenuate the static posterior request using gait phase, approach
  ...
  lateral change.
- Treat `0.50` as the bounded opposing-posterior-headroom approach anchor and
  close further increases on that axis. Relative to `0.35`, the sampled
  `0.50` policy improves upstream head travel/closest approach from
  `-11.33L/3.03L` to `-12.28L/2.13L`, but retains the upper hook, both
  rate/command caps, about `+1.78L` terminal head-y drift, and raises RMS
  force/moment from `511/5305` to `535/5416`. The inherited global `0.55`
  policy collapses to `-3.89L/5.70L/0.206` travel/approach/progress with RMS
  moment `5969`; localizing the same increment below `2.75L` does not rescue
  it, because the sampled close-gated policy regresses approach to `2.21L`,
  repeats the upper exit, and raises loads again to `566/5660`. Restore the
  ungated `0.50` controller before testing an observation orthogonal to phase
  headroom. Reopen this boundary only if a future continuation both beats the
  `2.13L` approach and changes lateral topology without disproportionate load,
  not merely if it adds upstream travel.
- Make the additive body-frame cross-track term the finite arrival anchor,
  while keeping its evidence boundary narrow. Adding
  `0.18*tanh(target_body_L[2]/2L)` to the complete `0.50` bearing/body-rate
  controller changes the inherited upper-hook topology into a diagonal descent
  and reaches the `0.75L` target after `88.13` release units. Against the
  isolated/close-gated phase-headroom failures, it improves closest/mean
  distance and progress from `2.13-2.21L/6.46L/about 0.51` to
  `0.747L/2.736L/0.940`, changes head-y travel from about `+1.7L` to `-4.44L`,
  and lowers RMS force/moment from `535-566/5416-5660` to `445/4597`. Four
  current sampled copies with only comment differences reproduce every metric
  exactly, so use the complete controller as the deterministic fixed-snapshot
  reference rather than reopening cross-track gain or scale. This does not
  establish wake-phase or layout robustness, and it does not establish
  desaturation: the posterior angle and both joint rate/command caps still
  activate. Accept an effort refinement only if capture and the diagonal route
  survive; otherwise restore the exact arrival anchor and test one actuator
  mechanism at a time instead of stacking a falsified recovery, velocity,
  clipping, or phase-headroom overlay.
  The assigned parent's isolated `1650` to `1550 deg/time^2` command-ceiling
  reduction now closes simple cap lowering as that refinement: despite reducing
  mean command energy from `866` to `792`, it changed capture into an upper
  domain exit, worsened closest/mean distance and progress to
  `1.560L/8.005L/0.386`, and raised RMS force/moment to `715/6922`. Posterior
  angle and both joint rates still reached their exact limits, so a lower
  command cap neither desaturated the controller nor reduced physical load.
  Restore `1650` before any distinct test and avoid further downward cap
  sweeps. The inherited `1700` opposite-side bracket also loses capture: it
  exits upward after `58.34` units, never comes within `5.73L`, and regresses
  mean distance/progress to `7.52L/0.433` despite slightly lower RMS loads
  (`430/4342`). Close simple command-ceiling tuning on both sides of `1650`;
  neither reduced effort nor reduced episode-averaged load rescues a failed
  route. Reopen this axis only with evidence that separately identifies which
  joint's cap contact disrupts capture.
- Treat the successful `0.18` body-frame cross-track gain as an isolated route
  value, not a local monotone gradient. Against four deterministic `0.18`
  copies that capture after `88.13` units with `2.736L` mean distance and
  `-4.44L` head-y travel, the inherited `0.20` bracket approaches to `1.123L`
  but then restores the upper hook (`+1.781L` head-y, `6.365L` mean distance),
  while `0.17` misses at `3.267L` after a larger loop (`+1.800L` head-y,
  `7.775L` mean distance). Both exit the upper boundary and raise RMS loads
  above the `445/4597` anchor. Restore exactly `0.18` and its `2L` scale; do
  not continue either gain direction. A next refinement is applicable only if
  it leaves the established far-field command unchanged and acts in a bounded
  terminal or actuator-specific regime; reject it if capture is lost or the
  approximately `+1.8L` upper-exit topology returns.
