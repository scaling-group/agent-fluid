...
- Receding distance is not a reliable recovery event for this route, even when
  restricted to the close-approach region. Against the plain `11 deg` anchor
  (`4.87L` closest, `0.424` progress), normalized yaw-moment rejection regresses
  closest approach to `4.99L` and raises RMS loads from `406/4113` to
  `439/4516`; do not repeat that moment branch without evidence of a different
  scale or sign. Phase-opposition boost instead supplies the strongest sampled
  far-field route (`-11.33L` upstream head travel, `6.44L` mean distance,
  `3.03L` closest, `0.517` progress), albeit at `511/5305` RMS loads. The
  assigned parent's globally active receding reversal once reached `2.67L` but
  sacrificed travel/progress to `-7.45L/0.405` and retained the upper exit.
  Its two sampled distance-gated descendants now falsify the proposed repair:
  `4--3L` and `3.75--2.75L` activation worsen closest approach to
  `3.13L/3.10L`, travel to `-10.15L/-10.83L`, and progress to `0.495/0.507`;
  both retain about `+1.8L` head-y drift, the upper hook, and all joint caps.
  Trajectory geometry explains the mismatch: at the phase anchor's `3.03L`
  radial minimum the target is still `2.35L` forward and `1.91L` lateral in
  the body frame, so increasing distance fires before longitudinal overshoot
  and reverses a still-valid first-pass request. Do not broaden, strengthen, or
  retune radial-receding recovery. A symmetric signed-fore-aft gate is also
  closed: attenuation while the target is still ahead regresses travel,
  closest approach, and progress to `-8.49L/3.45L/0.449` and preserves the
  same exit. The remaining geometric distinction is one-sided longitudinal
  overshoot: preserve the phase anchor exactly for every ahead-of-fish state,
  and test a bounded bearing-sign correction only after the target is measured
  behind. Falsify that mechanism if the first pass differs from the anchor or
  if post-overshoot countersteering fails to create a second approach, capture,
  or a material reduction in the terminal hook and saturation/load.
