...
- Treat `oscillator_energy_gain=2.1` as a qualified closure/propulsion anchor,
  not evidence that restoration gain is monotone or load-regulating. Against
  the duplicated `2.0` result, the isolated `2.1` rollout retained the compact
  useful-wake route and improved score from `-0.6617` to `-0.5806`, arrival
  from `74.23` to `73.86`, mean distance from `2.561L` to `2.480L`, and
  upstream-relative x speed from `0.06433` to `0.06565`. It also raised command
  energy from `50940` to `51797`, RMS force/moment from `22.39/393.08` to
  `24.94/410.68`, and posterior peak speed from `3.225` to `3.323`. Reuse the
  exact evaluated gain when closure is the objective, but do not extrapolate it
  or combine it with the failed guard/damping changes without isolating the
  mechanism. This boundary applies to the common deterministic wake; falsify
  it if held-out wake phase or geometry loses compact capture, or if the effort
  and load increase persists without a distance benefit.
