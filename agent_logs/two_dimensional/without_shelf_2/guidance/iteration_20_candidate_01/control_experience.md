...
- Localizing posterior damping to normalized peak-speed states can improve
  target closure without reproducing the failed constant-damping route shift.
  The inherited isolated increase from `tail_damping=0.65` to `0.675` lowered
  posterior peak speed but took a visibly deeper, kinked route and regressed to
  `91.50` arrival, `2.788L` mean distance, `60660` energy, and `42.13/581.11`
  RMS force/moment while moving motion to the anterior joint. In contrast, the
  sampled smooth gate that retained base `0.65` and added at most `0.025` only
  as `abs(qd2)/(omega*amplitude)` crossed `1.0` improved the exact static
  posterior-share anchor from `72.457` to `70.823` arrival, `2.4541L` to
  `2.3854L` mean distance, `0.06620` to `0.06972` upstream-relative x speed,
  `51842` to `50756` energy, and `23.85/408.89` to `22.72/397.24` loads while
  reducing posterior peak speed from `3.293` to `3.260`. Use that evaluated
  gate as the same-snapshot damping anchor; do not widen it or lower its
  threshold from aggregate metrics alone, because anterior peak speed rose
  from `3.086` to `3.112` and both commands still touched the guard. A
  composition with another independently positive bounded schedule is a new
  interaction test, not evidence of additive gains. This lesson applies to the
  certified wake snapshot; falsify it if time-resolved or held-out wake tests
  show deeper routing, anterior motion transfer, lost capture, or no joint
  improvement in compactness, effort, and load.
- Stop static posterior steering-share extrapolation at the replicated `0.70`
  same-snapshot anchor. The sampled isolated `0.65` policy was shallower and
  fastest (`-4.030L` center-y displacement, `72.160` arrival, `0.06732`
  upstream-relative x speed, and `51284` command energy), but its `2.4638L`
  mean distance and `25.32/420.32` RMS force/moment were worse than the three
  identical `0.70` results (`-4.285L`, `72.457`, `0.06620`, `51842`,
  `2.4541L`, and `23.85/408.89`). More decisively, three inherited evaluations
  of the isolated `0.75` continuation reproduced a visibly deeper lower return
  and regressed to `-4.590L` center-y displacement, `2.5093L` mean distance,
  score `-0.6110`, and `52496` energy despite slightly faster arrival and lower
  RMS force/moment (`72.275`, `22.60/394.40`); posterior peak speed also rose
  from `3.293` at `0.70` to `3.370` while anterior peak speed fell. Thus lower
  aggregate load at higher posterior share can mask motion redistribution and
  a worse target-relative route. Avoid another constant step above `0.70`; if
  posterior sharing is revisited, test a bounded state-dependent allocation of
  the evaluated `0.65--0.70` range and require route compactness, arrival,
  upstream-relative propulsion, effort, both joint peaks, and load to agree.
  This boundary is specific to the certified wake snapshot; falsify it if
  held-out wake phases or geometries make a value above `0.70` reproducibly
  compact without shifting motion or effort between joints.
