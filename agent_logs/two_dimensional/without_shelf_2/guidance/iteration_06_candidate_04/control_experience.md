...
- Treat the positive-bearing steering gain as a bracketed interior response,
  not a monotone authority knob. With the `0.75`-period, `22 deg` regulated
  gait and `10 deg` steering cap held fixed, gain `0.75` reached in `74.23`
  units with `2.561L` mean distance, `0.06433` upstream-relative x speed, and
  RMS force/moment `22.39/393.08`; the newly evaluated `0.77` gain was slower
  at `78.58`, increased mean distance to `2.661L` and total command energy
  from `50940` to `52761`, reduced upstream-relative x speed to `0.06261`, and
  raised RMS force to `23.77`. The inherited same-gait `0.70` result was slower
  still at `91.61` with `2.834L` mean distance. Later candidates should probe
  narrowly below `0.75` while holding the cap and gait fixed, and should avoid
  further upward gain extrapolation until a lower-side sample resolves the
  local optimum. This conclusion is deterministic same-snapshot evidence,
  not wake-phase robustness; falsify it if a held-out snapshot reverses the
  ordering or if a below-`0.75` probe loses capture or worsens both route
  metrics.
