...
- Treat pure-bearing steering gain as nonmonotonic for the established
  ...
  the finite interior anchor, restrict local gain probes to small steps, and do
  not trade route closure for lower crossflow or effort alone. The
  `0.80/11 deg` result cannot isolate gain because its limit changed too;
  falsify this lesson if a controlled held-out wake-phase or geometry sweep
  moves the optimum, or if a same-gait lower-side probe beats the anchor
  without increasing excursion or load.
- Treat the anchor's propulsion and burst authority as an empirical upper
  boundary on the common snapshot, not as safely expandable headroom. Relative
  to the exact `22 deg / 28 rad/time^2` anchor (`74.23` arrival, `2.561L` mean
  distance, `0.06433` upstream-relative x speed, and `22.39/393.08` RMS
  force/moment), increasing only amplitude to `22.25 deg` slowed arrival to
  `76.95`, reduced relative x speed to `0.05926`, raised command energy from
  `50940` to `54203`, and raised force/moment to `24.75/420.04`. The inherited
  isolated `29`-guard result was worse: arrival `80.00`, mean distance
  `2.658L`, relative x speed `0.05804`, maximum anterior angle `0.557` versus
  `0.516 rad`, and force/moment `33.84/488.89`; its posterior action reached
  `29` and anterior action reached `28.68`, so the extra authority was active.
  Do not increase amplitude or the candidate guard merely because joint hard
  limits retain margin; test reduced authority one dimension at a time and
  require compact capture and relative propulsion as well as lower loads. This
  boundary applies to the fixed anchor gait and common prewarm, not arbitrary
  wake phase or geometry; falsify it if an isolated upper-authority probe
  improves route and relative propulsion without added load, or if tighter
  guarding persistently clips the nominal cycle and slows capture.
