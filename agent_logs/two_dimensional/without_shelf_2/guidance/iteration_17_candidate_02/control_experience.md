...
- Treat posterior sharing of the bounded bearing curvature as a route-shaping
  control, not a monotone speed knob. With period, oscillator, anterior
  steering, lag, damping, and `28/28` guard fixed, increasing
  `tail_steering_gain` from `0.60` to `0.65` improved arrival
  `73.859 -> 72.160`, mean distance `2.4800L -> 2.4638L`, relative x speed
  `0.06565 -> 0.06732`, and energy `51797 -> 51284`, but increased RMS
  crossflow and force/moment. The isolated `0.70` continuation, reproduced by
  three current samples, improved score and mean distance again to `-0.55577`
  and `2.4541L` and reduced crossflow and force/moment to
  `0.13063` and `23.85/408.89`; it also slowed arrival slightly to `72.457`
  and raised energy to `51842`. Use exact `0.70` as the current same-snapshot
  Pareto anchor and vary posterior share only in isolated bounded steps;
  require compact visual capture, distance, relative propulsion, effort, and
  load to agree before continuing. The replications establish determinism, not
  robustness or improvement beyond `0.70`; falsify this lesson if held-out
  wakes reverse the ordering, or if the next isolated step selects the deeper
  route branches already seen under gain, guard, and damping interpolation.
