...
- Treat gait-amplitude and static acceleration-authority changes around the
  anchor as negative same-snapshot results, not as automatic exploitation of
  apparent joint-state headroom. Relative to the duplicated `22 deg`,
  guard-`28/28` anchor (`74.23` arrival, `2.561L` mean distance, `0.06433`
  upstream-relative x speed, command energy `50940`, and RMS force/moment
  `22.39/393.08`), changing only amplitude to `22.25 deg` regressed to
  `76.95`, `2.579L`, `0.05926`, and `24.75/420.04`; changing the common guard
  to `29 rad/time^2` regressed further to `80.00`, `2.658L`, `0.05804`, and
  `33.84/488.89`. The newly sampled isolated posterior reduction to `27`
  likewise slowed capture to `76.44`, reduced upstream-relative x speed to
  `0.06199`, raised command energy to `53200`, and raised RMS force/moment to
  `24.69/417.71`, despite its slightly better scalar score and `2.555L` mean
  distance. All probes still captured but took a visibly deeper lower
  correction. Preserve the anchor's anterior gait and `28/28` peak authority;
  if probing posterior transients, retain that peak and isolate damping or
  time-local smoothing instead of lowering the static cap. This boundary is
  specific to the common prewarm and coupled two-joint controller; the upper
  posterior direction remains confounded by the common-guard test. Falsify it
  if a controlled held-out wake phase or an isolated posterior-response test
  improves approach and relative propulsion without increasing load or losing
  capture.
