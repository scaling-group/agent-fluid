...
- Treat posterior sharing of the bounded steering center as a route-shaping
  control, not a monotone speed, effort, or load knob. With the rest of the
  controller fixed, `tail_steering_gain=0.60` reached in `73.859` units with
  `2.4800L` mean distance, `0.06565` upstream-relative x speed, `51797` energy,
  and `24.94/410.68` RMS force/moment; the inherited isolated `0.65` result
  improved these closure measures to `72.160`, `2.4638L`, and `0.06732` while
  raising loads to `25.32/420.32`. Three current executable `0.70` replications
  are byte-identical in released imagery and diagnostics at `72.457`,
  `2.4541L`, `0.06620`, `51842`, and `23.85/408.89`, confirming a further
  mean-distance and load improvement but a small arrival, propulsion, and
  effort tradeoff. Preserve exact `0.70` as the same-snapshot Pareto anchor;
  if posterior sharing is continued, use one bounded isolated step and require
  compact topology plus distance, relative-flow, effort, and load evidence
  rather than extrapolating a smooth optimum. This implication applies only to
  the common deterministic prewarm and is falsified if a continuation selects
  the deeper route seen in inherited damping/restoration/guard branches, or if
  held-out wake phase, geometry, inflow, or target placement reverses the
  ordering.
