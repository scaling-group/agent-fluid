...
- Treat acceleration-guard contact and interpolation as nonlinear route
  shaping, not direct load regulation. Against the duplicated common-`28`
  anchor (`74.23` arrival, `2.561L` mean distance, `0.06433` upstream-relative
  x speed, `50940` command energy, and RMS force/moment `22.39/393.08`), the
  assigned parent's global-`29` probe took a deeper late correction and
  regressed to `80.00`, `2.658L`, `0.05804`, and `33.84/488.89`. Lowering only
  the posterior guard to `27` slightly improved score/mean distance to
  `-0.654416/2.555L`, but slowed arrival to `76.44`, reduced relative speed to
  `0.06199`, and raised energy and loads to `53200` and `24.69/417.71`. Most
  decisively, the inherited `28/27.5` midpoint did not interpolate the two
  endpoints: it visibly took a much deeper lower route and regressed to
  `95.96`, `2.806L`, `0.05344`, `64918`, and `51.98/665.46`. Preserve exact
  `28/28` as the same-snapshot physical anchor and avoid further static guard
  interpolation until clipping timing or held-out wake phase explains the
  route branches. Require distance score, arrival, relative-flow separation,
  effort, and load to agree. This boundary is not a universal guard optimum;
  falsify it if controlled held-out phases find a posterior limit that
  reproducibly reduces load without losing compact capture or propulsion.
- Do not treat reduced peak joint speed or mean power as sufficient evidence
  that tail-response attenuation helps this controller. On the `28/28` anchor,
  increasing posterior damping from `0.65` to `0.675` reduced posterior peak
  speed from `3.225` to `3.145` and mean power from `45.38` to `44.91`, yet the
  sheet developed a long lower correction: arrival/mean distance regressed to
  `91.50/2.788L`, upstream-relative speed to `0.05262`, and force/moment to
  `42.13/581.11`. A separate `5%` bearing-demand amplitude reduction on the
  `28/27` controller likewise lowered peak speeds and mean power but worsened
  its parent on arrival, mean distance, relative propulsion, and loads to
  `79.70`, `2.594L`, `0.06073`, and `28.03/438.71`. Avoid retesting these two
  attenuation mechanisms on the common snapshot merely to lower effort; first
  isolate a different tail-target dimension or obtain time-resolved clipping
  evidence. The amplitude result is conditional on the `28/27` base, and both
  findings remain single-phase evidence; falsify them if held-out phases show
  repeatable load reduction with compact capture and preserved propulsion.
