...
- Distinguish constant damping from normalized high-speed-only damping, and do
  not require a peak-speed change to be perfectly isolated before checking the
  full physical result. The inherited constant increase from `0.65` to `0.675`
  lowered posterior peak speed to `3.145` but shifted work to the anterior
  joint, took a visibly deeper route, and regressed to `91.50` arrival,
  `2.788L` mean distance, `60660` energy, and `42.13/581.11` RMS force/moment.
  In contrast, on the current static-`0.70` tail-share anchor, adding at most
  `0.025` damping only through a normalized posterior-speed gate at `1.0` with
  width `0.08` retained the compact diagonal route and improved arrival
  `72.457 -> 70.823`, mean distance `2.4541L -> 2.3854L`, upstream-relative x
  speed `0.06620 -> 0.06972`, total energy `51842 -> 50756`, force/moment
  `23.85/408.89 -> 22.72/397.24`, and posterior peak speed
  `3.293 -> 3.260`. Anterior peak speed did rise slightly
  `3.086 -> 3.112` and mean command energy rose `715.49 -> 716.66`, so the
  evidence supports the exact localized gate as a same-snapshot route-and-load
  anchor, not a claim of direct or fully isolated tail-speed regulation.
  Preserve the evaluated gate values when combining it with another bounded
  mechanism, and avoid static damping or gain/threshold extrapolation until
  clipping timing or held-out wake evidence supports it. Falsify this lesson if
  the localized gate loses compact capture, increases aggregate loads or
  effort without closure benefit, or fails across wake phase, inflow, geometry,
  or target placement.
- Count executable replications once, then read posterior curvature sharing as
  ...
  behavior, not robustness. The distinct current `0.60` point reaches in
  `73.859` with `2.4800L`, `0.06565`, `51797`, and `24.94/410.68`, while the
  inherited isolated `0.65` point is fastest and most propulsive (`72.160`,
  ...
  `28` guard. Preserve exact `0.70` as the current distance/load anchor; if
  probing above it, change posterior share alone in a bounded step and require
  compact closure, relative propulsion, effort, and load to corroborate one
  another. Avoid interpolation or extrapolation claims from this single wake
  snapshot. Falsify the lesson if held-out wake phases or geometries reverse
  the ordering, exact `0.70` ceases to reproduce, or an isolated continuation
  cannot retain finite compact capture without a deeper route or load increase.
