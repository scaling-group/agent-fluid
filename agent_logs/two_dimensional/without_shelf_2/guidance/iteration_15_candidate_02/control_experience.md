...
- Treat `tail_steering_gain=0.65` as the current same-snapshot closure anchor,
  not as evidence that posterior steering or load should increase monotonically.
  Three executable `0.60` replications are byte-identical at `73.859` arrival,
  `2.4800L` mean distance, `0.06565` upstream-relative x speed, `51797` total
  energy, and `24.94/410.68` RMS force/moment. Changing only the posterior share
  to `0.65` preserves the compact visual corridor and improves those first four
  measures to `72.160`, `2.4638L`, `0.06732`, and `51284`, but raises relative
  crossflow from `0.13097` to `0.13603`, force/moment to `25.32/420.32`, and
  posterior peak speed from `3.323` to `3.367`. Preserve the exact evaluated
  `0.65` value rather than extrapolating it or combining it with the inherited
  guard, damping, energy-regime, or steering-gain changes that selected deeper,
  slower routes; require arrival, distance, propulsion, effort, and load to
  corroborate another posterior-share change. This applies only to the common
  deterministic prewarm and is falsified if reevaluation or held-out wake phase,
  inflow, geometry, or target placement loses compact capture or makes the load
  cost dominate the closure gain.
