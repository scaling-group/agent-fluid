...
- Separate constant damping from normalized high-speed damping. The inherited
  static increase from `tail_damping=0.65` to `0.675` lowered posterior peak
  speed but selected a visibly deeper, kinked route and regressed to `91.50`
  arrival, `2.788L` mean distance, `0.05262` upstream-relative x speed, `60660`
  energy, and `42.13/581.11` RMS force/moment. In contrast, two byte-identical
  executable samples that kept base damping `0.65` and added at most `0.025`
  through a smooth gate above normalized posterior speed `1.0` preserved the
  compact diagonal route and improved the exact static-`0.70` posterior-share
  anchor from `72.457` to `70.823` arrival, `2.4541L` to `2.3854L` mean
  distance, `0.06620` to `0.06972` relative x speed, `51842` to `50756` energy,
  and `23.85/408.89` to `22.72/397.24` force/moment while lowering posterior
  peak speed from `3.293` to `3.260`. Localize damping by normalized joint state
  and judge route, arrival, relative propulsion, effort, speed, and load
  together; do not infer less clipping because both commands still reach `28`.
  The gain has a small boundary cost—anterior peak speed rises from `3.086` to
  `3.112` and crossflow from `0.13063` to `0.13092`—and duplicate deterministic
  output is not robustness evidence. Falsify reuse if held-out wake phase,
  geometry, inflow, or target placement loses compact capture or transfers
  enough work anteriorly to erase the effort and load benefit.
- Count executable replications once, then read posterior curvature sharing as
  a multi-metric local response rather than a scalar or monotone trend. Three
  current `tail_steering_gain=0.70` samples exactly reproduce compact diagonal
  capture (`72.457` arrival, `2.4541L` mean distance, `0.06620`
  upstream-relative x speed, `51842` energy, `0.13063` crossflow, and
  `23.85/408.89` RMS force/moment); this confirms deterministic same-snapshot
  behavior, not robustness. The inherited distinct `0.60` point reaches in
  `73.859` with `2.4800L`, `0.06565`, `51797`, and `24.94/410.68`, while the
  current isolated `0.65` sample is fastest and most propulsive (`72.160`,
  `0.06732`) but has higher crossflow and loads (`0.13603`, `25.32/420.32`).
  Across `0.60 -> 0.65 -> 0.70`, mean distance improves but arrival, effort,
  and load do not vary monotonically, and both commands continue to touch the
  `28` guard. The inherited static `0.75` continuation now falsifies smooth
  extrapolation: its duplicated records are one byte-identical executable
  result, which remains finite and reaches in `72.275` while improving relative
  x speed, crossflow, and force/moment to `0.06873`, `0.12785`, and
  `22.60/394.40`, but takes a less compact final approach, regresses mean
  distance from `2.4541L` to `2.5093L`, raises energy from `51842` to `52496`,
  increases posterior peak speed from `3.293` to `3.370`, and lowers score from
  `-0.55577` to `-0.61098`. Preserve exact `0.70` as the same-snapshot
  compactness/effort anchor and avoid further static interpolation or
  extrapolation of posterior share; partial propulsion/load gains at `0.75`
  justify only an isolated bounded state-dependent test whose route, both joint
  speeds, effort, and loads are all checked. This boundary is one deterministic
  wake snapshot, not a universal optimum: falsify it if held-out wake phases or
  geometries reverse the ordering, exact `0.70` ceases to reproduce, or a
  bounded schedule retains compact capture while recovering the `0.75`
  propulsion/load benefits.
