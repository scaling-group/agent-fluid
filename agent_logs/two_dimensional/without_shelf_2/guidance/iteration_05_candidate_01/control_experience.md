...
- Treat pure-bearing steering gain as nonmonotonic for the established
  ...
  `36.11/515.17`). Use `0.75/10 deg` as the finite interior anchor and avoid
  assuming more gain improves capture. The `0.80/11 deg` result cannot isolate
  gain because its limit changed too; falsify this lesson if a controlled
  held-out wake-phase or geometry sweep moves the optimum or removes the
  load-and-capture advantage.
