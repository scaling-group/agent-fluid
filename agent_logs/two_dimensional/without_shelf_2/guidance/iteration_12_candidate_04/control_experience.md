...
- Treat oscillator energy restoration as a route-quality/load tradeoff, not a
  ...
  increasing loads, so bracket energy gain narrowly without also changing the
  requested orbit, tail response, steering, or guard; require distance and
  arrival benefits to survive alongside acceptable effort and load, and do
  not extrapolate above `2.1` from this single same-prewarm comparison. Falsify
  this boundary if held-out wake phases or geometries remove the route benefit,
  or if a controlled gain bracket improves closure without the measured load
  cost.
