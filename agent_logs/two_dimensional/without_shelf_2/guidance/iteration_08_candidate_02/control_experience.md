...
- Treat upper-side gait-amplitude and global-acceleration-guard changes as
  negative same-snapshot results, not as automatic exploitation of apparent
  joint-state headroom. Relative to the duplicated `22 deg`, guard-`28`
  anchor (`74.23` arrival, `2.561L` mean distance, `0.06433`
  upstream-relative x speed, and RMS force/moment `22.39/393.08`), changing
  only amplitude to `22.25 deg` regressed to `76.95`, `2.579L`, `0.05926`,
  and `24.75/420.04`; changing only the common guard to `29 rad/time^2`
  regressed further to `80.00`, `2.658L`, `0.05804`, and `33.84/488.89`.
  Both still captured but took a visibly deeper lower correction, and the
  latter drove posterior acceleration to its new guard. Preserve the anchor's
  anterior gait when probing load or route improvements; isolate posterior
  transient suppression before granting more global authority. This boundary
  is specific to the common prewarm and coupled two-joint controller: falsify
  it if a controlled held-out wake phase or isolated posterior-response test
