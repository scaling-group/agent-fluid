...
- Treat posterior steering share as a bounded curvature-redistribution knob
  ...
  `0.13603` and `25.32/420.32`. The current isolated `0.70` sample preserved
  the compact visual route and improved score/mean distance to
  `-0.55577/2.4541L`, while lowering crossflow to `0.13063`, force/moment to
  `23.85/408.89`, and peak anterior/posterior speeds to `3.086/3.293`; however,
  arrival slowed to `72.457`, upstream-relative x speed fell to `0.06620`, and
  total effort rose from `51284` to `51842`. Use exact `0.70` as the present
  mean-distance/load anchor and retain `0.65` as the faster propulsion anchor;
  avoid extrapolating above `0.70` until a controlled wake-phase test shows a
  stable route response. This conclusion is limited to the certified common
  prewarm and is falsified if held-out wakes remove the `0.70` load/closure
  tradeoff or make its posterior share unstable or non-capturing.
