...
- Count executable replications once, and do not collapse posterior steering
  sharing into a monotone scalar story. The `tail_steering_gain=0.65` anchor
  reaches in `72.160` units with `2.4638L` mean distance, `0.06732`
  upstream-relative x speed, `51284` energy, and `25.32/420.32` RMS
  force/moment. Three exact `0.70` replications preserve compact diagonal
  capture and improve score to `-0.555772`, mean distance to `2.4541L`, RMS
  crossflow to `0.13063`, and force/moment to `23.85/408.89`, but arrive at
  `72.457`, reduce relative x speed to `0.06620`, and raise energy to `51842`.
  The inherited isolated `0.75` continuation disproves extrapolation: its
  keyframes show a lower final approach, score and mean distance regress to
  `-0.610985/2.5093L`, and energy rises to `52496`, even though arrival and
  relative x speed remain competitive at `72.275/0.06873` and RMS crossflow
  and force/moment improve further to `0.12785/22.60/394.40`. Preserve exact
  `0.70` as the same-snapshot distance/load anchor; use `0.65` when arrival or
  effort dominates, and avoid gains above `0.70` justified by load reduction
  alone. Both anchor commands still touch the `28` guard, so this lesson does
  not imply saturation removal. Falsify the boundary if held-out wake phases
  or geometries reverse the `0.65/0.70/0.75` tradeoff, or if exact `0.70`
  reevaluation fails to reproduce finite compact capture.
