...
- Treat oscillator phase-space energy gain as a bracketed route parameter, not
  monotone propulsion authority. On the common prewarm, static gain `2.05`
  reached in `72.70` units with `2.511L` mean distance, `0.06562`
  upstream-relative x speed, `50630` command energy, and RMS force/moment
  `21.05/379.50`; three exact `2.1` rollouts reproducibly improved mean
  distance and score to `2.480L` and `-0.5806` at nearly identical relative x
  speed (`0.06565`), but slowed slightly to `73.86` and raised energy and loads
  to `51797` and `24.94/410.68`. Increasing only the gain to `2.2` reversed
  the trend: the keyframes show a lower correction, while arrival, mean
  distance, relative propulsion, energy, and loads regressed to `77.73`,
  `2.541L`, `0.06006`, `55790`, and `26.42/423.30`. Do not extrapolate above
  `2.1` or select gain from peak joint motion alone. If this mechanism is
  revisited, isolate normalized energy-deficit injection from excess-energy
  damping and require compact distance, relative-flow separation, effort, and
  loads to agree. This bracket applies only to the deterministic shared wake;
  falsify it if held-out wake phases or geometries move the finite bracket or
  show that a split gain enlarges motion or selects the lower route.
- Count policy evidence by active controller semantics, not candidate labels or
  source-file hashes. The four current sampled files differ only in comments:
  each returns the same `2.1` gain and the same equations, and all four produce
  identical prewarm/release sheets and identical control/physics metrics
  (`73.8593` arrival, `2.4800L` mean distance, `0.06565` upstream-relative x
  speed, `51797` effort, and `24.94/410.68` RMS force/moment; only wall time
  varies). Treat them as deterministic replication of one anchor, not four
  parameter-response points or a success/failure comparison. In particular,
  they add confidence in the assigned parent's `2.1` baseline but add no new
  interpolation direction beyond the inherited `2.05/2.1/2.2` bracket; later
  workers should verify active expressions before ranking samples and spend a
  new evaluation on one isolated mechanism. This applies to the certified
  snapshot and fixed configuration; falsify the reproducibility implication if
  semantically identical policies under that same setup diverge in trajectory
  or physics metrics, and do not extend it to held-out wake phases.
