...
- Localizing a small damping increment by normalized posterior speed can
  improve the static gait when closure, effort, joint peaks, and loads agree.
  Two independent samples of the exact smooth gate
  `0.025 * 0.5 * (1 + tanh((|qd2|/(omega*amp) - 1.0)/0.08))` reproduced the
  same compact route and result relative to the static `0.65`-damping,
  `tail_steering_gain=0.70` parent: arrival improved from `72.457` to `70.823`,
  mean distance from `2.45409L` to `2.38538L`, upstream-relative x speed from
  `0.06620` to `0.06972`, command energy from `51842` to `50756`, posterior
  peak speed from `3.293` to `3.260`, and RMS force/moment from
  `23.85/408.89` to `22.72/397.24`; RMS crossflow changed only from `0.13063`
  to `0.13092`. Prefer this evaluated smooth gate over constant damping or a
  new static interpolation, but do not combine it with the separately useful
  bearing schedule without isolation. Both commands still reached the `28`
  guard and anterior peak speed rose slightly, so the gate does not establish
  less clipping or universal load regulation. Its exact duplicate establishes
  same-snapshot reproducibility only; falsify reuse if held-out wake phase,
  inflow, geometry, or target placement loses compact capture, or if
  time-resolved evidence shows merely shifted guard contact or anterior load.
