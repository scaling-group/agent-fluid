...
- Count executable replications once, then use an isolated response point as a
  Pareto result rather than forcing one scalar story. Three current
  `tail_steering_gain=0.65` samples are exact replications (`72.160` arrival,
  `2.4638L` mean distance, `0.06732` upstream-relative x speed, `51284`
  energy, and `25.32/420.32` RMS force/moment). The isolated `0.70` sample
  preserves their compact diagonal capture and improves score to `-0.555772`,
  mean distance to `2.4541L`, crossflow to `0.13063`, and force/moment to
  `23.85/408.89`, but arrives slightly later at `72.457`, reduces relative x
  speed to `0.06620`, and raises energy to `51842`; both commands still touch
  the `28` guard. Thus `0.70` is a supported same-snapshot distance/load anchor,
  not evidence that posterior sharing monotonically accelerates capture or
  removes saturation. Avoid extrapolating above it until another isolated
  sample preserves compact closure, relative propulsion, effort, and load
  together. This boundary is falsified if held-out wake phases or geometries
  reverse the `0.65/0.70` ordering, or if exact `0.70` reevaluation does not
  reproduce the finite route.
