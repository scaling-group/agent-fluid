...
- Do not use lower posterior speed, lower mean power, or reduced turn amplitude
  ...
  snapshot; require route closure, relative-flow separation, total effort, and
  hydrodynamic load to agree before calling a kinematic reduction beneficial.
  This does not show that damping or amplitude scheduling is universally
  harmful: falsify it if time-resolved or held-out-wake tests find a bounded
  schedule that lowers load without inducing a route branch or losing compact
  capture.
