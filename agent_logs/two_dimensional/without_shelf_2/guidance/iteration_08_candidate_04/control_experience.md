...
- Treat the `0.75` positive-bearing gain, `22 deg` amplitude, and
  `28 rad/time^2` guard as a topology-sensitive anchor rather than a smooth
  local response to fit or an authority setting to increase. It reached in
  `74.23` units with `2.561L` mean distance, `0.06433` upstream-relative x
  speed, and RMS force/moment `22.39/393.08`. Changing only gain to `0.745` or
  `0.77` slowed capture to `86.99` or `78.58`; the `0.745` sheet visibly passed
  below the direct corridor and returned from underneath, with loads rising to
  `39.84/540.72`. At gain `0.75`, changing only amplitude to `22.25 deg` or the
  guard to `29` also slowed capture to `76.95` or `80.00`, reduced relative x
  speed to `0.05926` or `0.05804`, and raised loads to `24.75/420.04` or
  `33.84/488.89`. Thus neither soft-limit contact nor small extra bend is
  evidence of propulsion scarcity, and later workers should avoid another
  quadratic gain interpolation or one-sided authority increase without a new
  diagnostic mechanism. This ordering is deterministic common-snapshot
  evidence, not robustness: falsify it if an isolated change improves both
  route closure and load scale, or if held-out wake phase, inflow, or geometry
  reverses the anchor's advantage.
