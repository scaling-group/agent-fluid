...
- Do not interpolate posterior acceleration guards as though they smoothly
  trade the common-`28 rad/time^2` anchor's fast closure against the
  posterior-`27` sample's small scalar gain. The inherited `27.5` midpoint
  reached the target only after a visible deep excursion below the compact
  corridor and return from underneath: versus the `28` anchor's `74.23`
  arrival, `2.561L` mean distance, `0.06433` upstream-relative x speed,
  `50940` command energy, and `22.39/393.08` RMS force/moment, the midpoint
  regressed to `95.96`, `2.806L`, `0.05344`, `64918`, and `51.98/665.46`.
  It was also worse on physical closure and load than posterior `27`, despite
  lying numerically between the two guards. Together with the inherited
  common-`29` regression, this makes guard contact a nonlinear route-shaping
  boundary on the deterministic prewarm, not an authority dial or direct load
  regulator. Preserve the common `28` anchor and test a different controller
  dimension unless time-resolved clipping evidence motivates a mechanism; do
  not continue midpoint or sub-unit guard fitting on this snapshot. Falsify
  this boundary only if controlled held-out wake phases or geometries recover
  a repeatable monotone guard response with compact capture, relative
  propulsion, effort, and load all agreeing.
