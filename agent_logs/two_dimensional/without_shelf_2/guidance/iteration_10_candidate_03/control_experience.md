...
- Do not interpolate acceleration clipping thresholds as though the resulting
  wake trajectory will interpolate. Against three identical `28/28
  rad/time^2` rollouts (`74.23` arrival, `2.561L` mean distance, `0.06433`
  upstream-relative x speed, `50940` command energy, and RMS force/moment
  `22.39/393.08`), the sampled posterior-only `27` guard slightly improved
  configured score and mean distance to `-0.654416/2.555L`, but slowed arrival
  to `76.44`, reduced relative speed to `0.06199`, raised energy to `53200`,
  and raised loads to `24.69/417.71`. More decisively, two inherited copies of
  the proposed `27.5` midpoint both took the same visible dive beneath the
  target and regressed to `95.96`, `2.806L`, `0.05344`, `64918`, and
  `51.98/665.46`; posterior peak speed increased monotonically from `3.225`
  at `28` to `3.233` at `27` and `3.242` at `27.5`. Preserve the reproduced
  `28/28` guard as the current route anchor, and probe tail dynamics such as
  damping separately rather than continuing midpoint or lower-limit searches.
  This boundary applies to the deterministic common prewarm and does not
  identify clipping duration or a universal damping direction; falsify it if
  time-resolved or held-out-wake evidence shows a posterior limit that reduces
  speed and load while preserving compact capture and upstream propulsion.
