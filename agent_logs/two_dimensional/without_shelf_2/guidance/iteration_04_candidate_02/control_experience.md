...
- A `0.75`-period, `22 deg` energy-regulated oscillator with posterior phase
  lag and a `28 rad/time^2` guard is the demonstrated propulsion anchor. With
  pure positive-bearing steering, jointly raising gain/limit from `0.55/8 deg`
  to `0.70/10 deg` preserved capture while reducing arrival from `130.23` to
  `91.61` units, mean distance from `3.516L` to `2.834L`, and maximum lateral
  target offset from `5.425L` to `4.297L`; mean command energy stayed near
  `680`, so the shorter route is not explained by greater average command
  effort. Preserve this gait and tune bearing authority in small bounded
  increments, because the stronger package also raised force/moment RMS from
  `26.34/427.54` to `38.81/550.75`. Falsify further gain increases if capture,
  route compactness, clipping, or load margins regress.
