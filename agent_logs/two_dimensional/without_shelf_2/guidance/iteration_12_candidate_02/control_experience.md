...
- Treat acceleration-guard contact as nonlinear route shaping, not proof of
  ...
  speed to `0.06199`, and raised loads to `24.69/417.71`. Preserve anterior
  authority at `28`, avoid granting more global authority, and evaluate
  posterior-only changes on score, route closure, and load together rather than
  extrapolating either direction. This mixed boundary applies to the common
  deterministic prewarm; falsify it if controlled held-out wake phases make
  posterior authority monotone, or if a bounded value inside `27--28` improves
  both distance score and physical closure without raising load.
- Prefer the exact evaluated `2.1` oscillator-energy recovery on the common
  `28/28` guard over further static guard or posterior phase-response tuning.
  Relative to the duplicated gain-`2.0` anchor, raising only energy gain to
  `2.1` preserved the visibly compact diagonal wake entry while improving
  score from `-0.661705` to `-0.580586`, arrival from `74.23` to `73.86`, mean
  distance from `2.561L` to `2.480L`, and upstream-relative x speed from
  `0.06433` to `0.06565`; both guards were contacted, and the tradeoff was
  modestly higher command energy (`51797` versus `50940`) and RMS force/moment
  (`24.94/410.68` versus `22.39/393.08`). This corroborated distance, route,
  arrival, and relative-propulsion improvement is stronger evidence than the
  assigned gain-`2.0`, `28/27` parent's scalar-only guard benefit. Preserve
  tail lag `0.55`: the inherited isolated reduction to `0.525` visibly took a
  deeper lower route and regressed to `85.29` arrival, `2.688L` mean distance,
  `0.05793` upstream-relative x speed, and `40.09/558.58` force/moment despite
  slightly lower mean command and power. Do not extrapolate energy gain above
  `2.1` from this single positive step or infer universal optimality from the
  common deterministic prewarm; falsify the lesson if held-out wake phase or
  geometry reverses the closure gain, or if a controlled higher-gain probe
  improves the same metrics without persistent saturation or increased load.
