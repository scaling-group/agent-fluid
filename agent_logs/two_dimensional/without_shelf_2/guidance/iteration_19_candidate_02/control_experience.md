...
- Do not generalize the failed constant-damping increase to bounded high-speed
  damping. The inherited static `tail_damping=0.675` controller lowered
  ...
  `42.13/581.11` RMS force/moment. In contrast, the sampled controller that
  retained base damping `0.65` and smoothly added at most `0.025` only above
  normalized posterior speed `1.0` preserved the compact diagonal route and
  improved the exactly replicated static-`0.70` posterior-share anchor from
  `72.457` to `70.823` arrival, `2.4541L` to `2.3854L` mean distance,
  `0.06620` to `0.06972` upstream-relative x speed, `51842` to `50756` energy,
  and `23.85/408.89` to `22.72/397.24` force/moment while lowering posterior
  peak speed from `3.293` to `3.260`. The boundary is not cost-free: anterior
  peak speed rose from `3.086` to `3.112`, relative crossflow edged from
  `0.13063` to `0.13092`, and both accelerations still reached the `28` guard.
  Localize damping by normalized joint state rather than applying it across the
  orbit, and require route, arrival, relative propulsion, effort, posterior
  speed, and load to agree; do not infer reduced clipping or combine this gate
  with another schedule from this aggregate evidence alone. This positive
  result is one deterministic wake snapshot, not robustness evidence; falsify
  it if exact repetition or held-out wake phase, geometry, inflow, or target
  placement loses compact capture or transfers enough work anteriorly to erase
  the effort/load benefit.
