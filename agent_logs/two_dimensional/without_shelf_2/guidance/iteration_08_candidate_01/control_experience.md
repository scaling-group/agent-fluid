...
- Preserve the evaluated `0.75`-gain, `10 deg`-limited, `0.75`-period,
  `22 deg` gait as the local common-snapshot anchor while isolating new
  controller dimensions; nearby increases in either gait or transient action
  authority have now failed. Relative to the duplicated `28 rad/time^2`
  anchor's `74.23` capture, `2.561L` mean distance, `0.06433`
  upstream-relative x speed, and `22.39/393.08` RMS force/moment, raising only
  amplitude to `22.25 deg` regressed to `76.95`, `2.579L`, `0.05926`, and
  `24.75/420.04`, with total command energy rising from `50940` to `54203`.
  An inherited guard-only increase from `28` to `29 rad/time^2` visibly drove
  the path farther below the compact corridor and regressed to `80.00`,
  `2.658L`, `0.05804`, and `33.84/488.89`. Do not spend the anchor's apparent
  joint-state headroom by increasing amplitude or relaxing the soft action
  guard again on this wake snapshot; test a modestly tighter guard or a
  different isolated feedback dimension instead, requiring compact capture
  and relative propulsion as well as reduced load. This is a local negative
  result, not proof that lower authority is monotonic or universally better;
  falsify it if controlled wake-phase, geometry, or inflow changes make either
  authority increase reproducibly improve route closure without load growth.
