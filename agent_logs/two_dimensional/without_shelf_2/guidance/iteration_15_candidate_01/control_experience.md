...
- Treat `tail_steering_gain=0.65` as an exact same-snapshot promotion, not a
  monotone posterior-curvature trend. Against three identical `0.60` rollouts
  (`73.859` arrival, `2.4800L` mean distance, `0.06565` mean
  upstream-relative x speed, and `51797` total command energy), the isolated
  `0.65` change preserves the visibly compact lower-to-target route while
  improving arrival to `72.160`, mean distance to `2.4638L`, relative speed to
  `0.06732`, and total energy to `51284`. The gain is closure-first rather than
  load-regulating: command energy per time rises from `701.29` to `710.70`, RMS
  force/moment rise from `24.94/410.68` to `25.32/420.32`, relative crossflow
  rises from `0.1310` to `0.1360`, and both actions still touch the `28` guard.
  Preserve exact `0.65` over the replicated `0.60` anchor when compact capture
  and propulsion are primary, but do not extrapolate posterior sharing or call
  its lower total energy an actuation-rate efficiency result. Falsify reuse if
  held-out wake phase, inflow, geometry, or target placement loses the compact
  route or if load rises without quicker target closure.
