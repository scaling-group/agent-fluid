...
- Count behaviorally identical controller sources as replications, not distinct
  search points. All four current sampled policies have the same executable
  expressions and parameter values despite comment-only source differences;
  their released sheets are byte-identical and every physical diagnostic is
  exactly equal (`73.859` arrival, `2.480L` mean distance, `0.06565`
  upstream-relative x speed, `51797` energy, and `24.94/410.68` RMS
  force/moment). The assigned-parent log independently repeats this anchor,
  while its other finite logs report `77.291/2.630L/22.76/398.31` and
  `73.062/2.574L/21.85/389.56` arrival/mean-distance/force/moment without
  retaining the source mechanisms. Preserve the exact replicated anchor for
  controlled one-mechanism probes, and use unattributed score-only outcomes as
  Pareto benchmarks rather than reconstructing or crediting hidden parameter
  changes. This implication applies to the common deterministic prewarm; it
  does not establish robustness across wake phase or geometry, and is falsified
  if executable controller differences or held-out initial conditions produce
  genuinely independent response evidence.
- Bound posterior steering share by route evidence rather than extrapolating a
  locally improving distance trend. On the common prewarm, increasing only
  `tail_steering_gain` from `0.65` to `0.70` produced three exact replications
  of compact capture and improved mean distance from `2.4638L` to `2.4541L`,
  relative crossflow from `0.13603` to `0.13063`, and RMS force/moment from
  `25.32/420.32` to `23.85/408.89`, although arrival slowed by `0.297` and
  total command energy rose from `51284` to `51842`. The inherited next equal
  step to `0.75` did not continue the closure trend: its keyframes show a
  deeper lower route, mean distance regressed to `2.5093L`, score to
  `-0.610985`, and energy to `52496`, even as crossflow and loads fell further
  to `0.12785` and `22.60/394.40`. Preserve exact `0.70` as the current
  same-snapshot closure anchor; avoid higher static posterior share or an
  interpolated compromise justified by load reduction alone, and require
  route compactness, distance, effort, propulsion, and load to agree. This
  boundary does not establish a universal gain optimum: falsify it if held-out
  wake phase, inflow, geometry, or target placement makes another bounded
  posterior share reproducibly better on that joint evidence set.
