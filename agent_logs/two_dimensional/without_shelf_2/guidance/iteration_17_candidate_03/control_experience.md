...
- Treat posterior steering share as a bounded curvature-redistribution knob
  with a same-snapshot Pareto knee, not as a monotone arrival-time gain. The
  inherited `0.60 -> 0.65` change improved arrival from `73.859` to `72.160`,
  mean distance from `2.4800L` to `2.4638L`, and upstream-relative x speed from
  `0.06565` to `0.06732`, but raised crossflow and RMS force/moment to
  `0.13603` and `25.32/420.32`. Three current executable replications of exact
  `0.70` have byte-identical keyframe sheets and identical physical metrics:
  they preserve the compact route and improve score/mean distance to
  `-0.55577/2.4541L`, lower crossflow to `0.13063`, force/moment to
  `23.85/408.89`, and peak anterior/posterior speeds to `3.086/3.293`; however,
  arrival is `72.457`, upstream-relative x speed is `0.06620`, and effort is
  `51842`, versus `51284` at `0.65`. Count those replications once for search
  diversity but use them to treat exact `0.70` as a reproducible
  distance/load anchor and `0.65` as the faster propulsion/effort anchor. A
  next share probe should stay within the evaluated `[0.65,0.70]` envelope and
  use bounded body-frame conditioning rather than another static extrapolation;
  the conditioning itself remains unvalidated until later evaluation. This
  conclusion is limited to the certified common prewarm, not held-out wake
  robustness, and is falsified if varied wake phase or geometry reverses the
  tradeoff, or if either endpoint loses finite compact capture.
