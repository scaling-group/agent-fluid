...
- Treat oscillator energy restoration as a non-monotone route-quality/load
  ...
  Preserve exact `2.1` as the replicated same-snapshot closure anchor rather
  than extrapolating restoration gain or combining it with route-sensitive
  guard, steering, or damping changes. An inherited optimizer log reports a
  distinct `72.70`-arrival, `2.511L`-mean-distance result with lower `50630`
  energy and `21.05/379.50` loads, but the available log does not attribute
  those metrics to a policy mechanism; use it only as a Pareto benchmark and
  do not reconstruct or transplant unidentified parameters. This boundary is
  deterministic same-prewarm evidence, not a universal optimum: falsify it if
  controlled held-out wake phases or geometries remove the `2.1` advantage, or
  if an isolated restoration-gain bracket improves closure and propulsion
  without the observed effort/load cost.
