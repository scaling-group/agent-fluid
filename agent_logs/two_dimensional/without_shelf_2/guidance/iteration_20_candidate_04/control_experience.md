...
- Treat the exact normalized-speed damping gate as a reproducible same-snapshot
  anchor, while treating identical reruns as determinism rather than wake
  robustness. The inherited static `tail_damping=0.675` controller lowered
  posterior peak speed but selected a visibly deeper, kinked route and
  regressed to `91.50` arrival, `2.788L` mean distance, `60660` energy, and
  `42.13/581.11` RMS force/moment. In contrast, two separately executed copies
  that retained base damping `0.65` and smoothly added at most `0.025` only
  above normalized posterior speed `1.0` had byte-identical policies and
  released keyframe sheets and matched every physical metric: relative to the
  exactly replicated static-`0.70` posterior-share anchor, arrival improved
  from `72.457` to `70.823`, mean distance from `2.4541L` to `2.3854L`,
  upstream-relative x speed from `0.06620` to `0.06972`, energy from `51842`
  to `50756`, force/moment from `23.85/408.89` to `22.72/397.24`, and
  posterior peak speed from `3.293` to `3.260`. The independently sampled
  bearing-scheduled posterior share also beat the static anchor on several
  measures but was weaker than the speed gate on arrival, mean distance,
  effort, and loads, so the aggregate evidence supports the isolated gate, not
  a compound schedule. The boundary is not cost-free: anterior peak speed rose
  from `3.086` to `3.112`, relative crossflow edged from `0.13063` to
  `0.13092`, and both accelerations still reached the `28` guard. Preserve the
  exact gate before extrapolating its gain, threshold, transition, or combining
  feedback; require route, arrival, relative propulsion, effort, joint-speed
  allocation, and load to agree. Falsify this lesson if held-out wake phase,
  geometry, inflow, or target placement loses compact capture or transfers
  enough work anteriorly to erase the effort/load benefit.
