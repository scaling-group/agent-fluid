...
- Do not treat a lower posterior speed peak from either constant or
  speed-triggered damping as direct hydrodynamic-load regulation. Relative to
  the duplicated `tail_damping=0.65`, `28/28` anchor (`74.23` arrival,
  `2.561L` mean distance, `0.06433` upstream-relative x speed, `50940` energy,
  `22.39/393.08` RMS force/moment, and `3.225` posterior peak speed), the
  inherited isolated increase to `0.675` lowered posterior peak speed to
  `3.145` but took a visibly deeper, kinked route and regressed to `91.50`,
  `2.788L`, `0.05262`, `60660`, and `42.13/581.11`; anterior peak angle/speed
  also rose. The assigned parent's normalized schedule, which retained `0.65`
  below tail-speed ratio `0.95` and ramped to `0.675` only near the peak, also
  failed: the negligible peak reduction to `3.216` accompanied a lower
  correction, `77.29` arrival, `2.630L` mean distance, `53167` energy, and
  `22.76/398.31` loads. Preserve `0.65` as the same-snapshot anchor and avoid
  another posterior-speed-only damping change unless a distinct normalized
  trigger and time-resolved evidence predict route preservation; require
  arrival, relative-flow separation, effort, and load to corroborate any speed
  reduction. This does not establish that every state-dependent damping law is
  harmful; falsify it if a bounded law reduces posterior speed and load without
  shifting motion anteriorly or losing compact closure across held-out wakes.
- Treat `oscillator_energy_gain=2.1` as a qualified closure/propulsion anchor
  and the uniform energy-restoration response as non-monotone. Against the
  duplicated `2.0` result, three identical sampled `2.1` rollouts retained the
  compact useful-wake route and improved score from `-0.6617` to `-0.5806`,
  arrival from `74.23` to `73.86`, mean distance from `2.561L` to `2.480L`, and
  upstream-relative x speed from `0.06433` to `0.06565`, at the cost of energy
  rising from `50940` to `51797` and RMS force/moment from `22.39/393.08` to
  `24.94/410.68`. The isolated uniform increase to `2.2` disproved further
  extrapolation: its keyframes take a deeper lower route, arrival regresses to
  `77.73`, mean distance to `2.541L`, relative speed to `0.06006`, energy to
  `55790`, and force/moment to `26.42/423.30`. Although anterior peak
  angle/speed fell from `0.5259/3.1505` to `0.5156/3.1319`, that peak reduction
  did not regulate aggregate load or preserve the route. Reuse exact uniform
  `2.1` for closure and avoid another whole-cycle gain increase; if energy
  feedback is revisited, isolate below-orbit recovery from above-orbit damping
  and require route, distance, relative propulsion, effort, and loads to agree.
  This boundary applies to the common deterministic wake and does not establish
  optimal branch-specific gains; falsify it only if held-out wake phase or
  geometry restores a monotone uniform-gain benefit, or an isolated branch
  change improves closure without the measured route and load cost.
