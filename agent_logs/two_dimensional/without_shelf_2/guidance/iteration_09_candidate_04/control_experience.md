...
- Do not fit or interpolate the pure-bearing steering-gain response as though
  ...
  `23.77/393.44`. More decisively, the inherited proposed interpolation at
  gain `0.745` took a visibly deeper route below the target and regressed to
  `86.99`, `2.694L`, `0.05865`, and `39.84/540.72`. Retain exact gain `0.75`
  as the same-snapshot anchor and avoid further sub-percent gain interpolation
  until wake phase is varied or time-resolved evidence explains the route
  switch. This does not prove a universal optimum; falsify it if controlled
  held-out wake phases or geometries recover a smooth ordering or move the
  finite optimum.
- Do not treat a lower posterior hard-acceleration guard as a load suppressor
  in this traveling-bend controller. Against three deterministic `28/28`
  guard anchor rollouts (`74.23` arrival, `2.56125L` mean distance,
  `0.06433` upstream-relative x speed, `50940` command energy, and RMS
  crossflow/force/moment `0.12919/22.39/393.08`), changing only the posterior
  guard to `27 rad/time^2` improved score by `0.00729` and mean distance by
  `0.00582L`, but slowed arrival to `76.44`, reduced relative propulsion to
  `0.06199`, raised energy to `53200`, raised RMS crossflow/force/moment to
  `0.13246/24.69/417.71`, and produced a visibly deeper final correction while
  reaching the new guard. Preserve the scalar/route tradeoff as evidence, but
  reject the tested claim that harder tail clipping reduces load; do not lower
  this guard further on that premise. Test posterior damping or lag upstream
  of the restored guard if load suppression is the goal. This boundary is
  same-snapshot and controller-specific; falsify it if a held-out wake or a
  separately retuned traveling bend lowers both load and route cost under the
  tighter guard.
