function target_policy_params()
    return (
        ...
        tail_steering_gain_aligned=0.65,
        tail_steering_gain_turning=0.70,
        tail_lag_gain=0.55,
        tail_damping=0.65,
...
function target_policy(state, params)
    ...
    # Stay inside the two evaluated posterior-share endpoints. Retain 0.70 for
    # substantial bearing correction, then taper toward the faster 0.65 share
    # as the body aligns; the blend is bounded and coordinate independent.
    steering_fraction = min(
        abs(steering_center) / params.steering_limit,
        1.0,
    )
    tail_steering_gain = params.tail_steering_gain_aligned +
        (params.tail_steering_gain_turning -
         params.tail_steering_gain_aligned) * steering_fraction
    ...
    # Retain the reproducible common guard so the share schedule is the only
    # executable change from the assigned parent.
