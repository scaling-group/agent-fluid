...
function target_policy(state, params)
    ...
    # Preserve the evaluated static posterior share rather than combining the
    # successful damping gate with the dominated bearing-demand schedule.
    tail_target = params.tail_steering_gain * steering_center -
        oscillator_position -
        params.tail_lag_gain * qd1 / omega

    # Keep the successful high-speed-only damping mechanism and narrow its
    # smooth transition. This strengthens damping just above nominal orbit
    # speed while withdrawing it faster below the threshold, away from the
    # inherited failed constant-damping regime.
    ...
    # Retain the reproducible common guard so transition localization is the
    # only executable change from the evaluated speed-gated anchor.
