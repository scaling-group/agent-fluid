function target_policy_params()
    return (
        control_period=0.75,
        oscillator_amplitude=22.25 * pi / 180,
...
function target_policy(state, params)
    ...
    # The sampled pure-bearing sweep and the failed lower-side interpolation
    # select the exact 0.75/10 degree anchor without the failed rate lead.
    steering_center = params.steering_limit * tanh(
        params.steering_gain * state.bearing / params.steering_limit,
    )

    # Probe 0.25 degree more propulsion inside the anchor's observed joint-state
    # headroom. At unit energy, nominal speed and restoring acceleration remain
    # below the actuator limits and the candidate-side acceleration guard.
