function target_policy_params()
    return (
        ...
        bearing_rate_horizon=0.10,
        bearing_rate_limit=0.45,
...
function target_policy(state, params)
    ...
    # Preserve the evaluated successful positive-bearing curvature sign. A
    # small bounded window-rate lead reduces that curvature while bearing is
    # already closing, without allowing derivative noise to own the turn.
    bounded_bearing_rate = params.bearing_rate_limit * tanh(
        state.bearing_window_rate / params.bearing_rate_limit,
    )
    steering_error = state.bearing +
        params.bearing_rate_horizon * bounded_bearing_rate
    steering_center = params.steering_limit * tanh(
        params.steering_gain * steering_error / params.steering_limit,
    )
