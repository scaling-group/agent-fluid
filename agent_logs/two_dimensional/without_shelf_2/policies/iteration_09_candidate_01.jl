...
function target_policy(state, params)
    ...
    # Keep the independent authority demonstrated by the successful anchor.
    # A posterior slew guard just above the nominal traveling-wave jerk
    # suppresses only abrupt tracking/cap transitions; unlike a lower static
    # acceleration cap, it does not remove steady tail authority.
    accel_limit = params.acceleration_limit
    a1 = clamp(raw_a1, -accel_limit, accel_limit)
    bounded_a2 = clamp(raw_a2, -accel_limit, accel_limit)
    previous_a2 = clamp(state.previous_action[2], -accel_limit, accel_limit)
    max_a2_change = params.posterior_jerk_limit * state.history_dt
    a2 = clamp(
        bounded_a2,
        previous_a2 - max_a2_change,
        previous_a2 + max_a2_change,
    )
