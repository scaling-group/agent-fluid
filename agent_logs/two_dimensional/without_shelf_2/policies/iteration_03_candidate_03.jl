...
function target_policy(state, params)
    ...
    # Keep steering subordinate to gait startup. Positive bearing curvature is
    # the finite, stable sign established by the sampled target-reaching run;
    # the small smooth bound prevents a distant target from consuming the gait.
    steering_center = params.steering_limit * tanh(
        params.steering_gain * state.bearing / params.steering_limit,
    )

    # Regulate phase-space energy so the initial bend grows into a useful cycle
    # before downstream advection ends the episode. Phase remains encoded in
    # joint state rather than in a clock or a case-specific route.
