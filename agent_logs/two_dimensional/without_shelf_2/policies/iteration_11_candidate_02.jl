function target_policy_params()
    return (
        ...
        tail_lag_gain=0.55,
        tail_damping=0.65,
        tail_high_speed_damping=0.025,
        tail_damping_activation_ratio=0.95,
        tail_damping_ramp_ratio=0.05,
...
function target_policy(state, params)
    ...
    # Preserve the anchor damping through most of each beat. Apply the small
    # evaluated damping increment only near the normalized posterior-speed
    # peak, avoiding the persistent phase change of the failed 0.675 probe.
    tail_speed_ratio = abs(qd2) / (omega * amp)
    high_speed_fraction = clamp(
        (tail_speed_ratio - params.tail_damping_activation_ratio) /
            params.tail_damping_ramp_ratio,
        0.0,
        1.0,
    )
    effective_tail_damping = params.tail_damping +
        params.tail_high_speed_damping * high_speed_fraction^2
