...
function target_policy(state, params)
    ...
    # Compose the two positively evaluated bounded mechanisms without
    # extrapolating either range. Use the shallower 0.65 posterior share at
    # high steering demand, then recover the compact 0.70 share near alignment.
    ...
    # Retain the evaluated high-speed-only damping gate exactly. Constant
    # damping shifted the route, whereas this localized increment reduced
    # posterior speed, effort, and load while tightening target closure.
    ...
    # Preserve the reproducible common guard so the schedule composition is
    # the only executable change from the speed-gated prefill.
