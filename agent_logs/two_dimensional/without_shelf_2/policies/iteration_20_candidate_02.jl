...
function target_policy(state, params)
    ...
    # Combine the exact evaluated bearing schedule with the independently
    # successful speed gate below. Large bounded turns use the shallower 0.65
    # response; the compact 0.70 share is recovered smoothly near alignment.
    ...
    # The inherited constant 0.675 damping shifted the whole route and moved
    # work to the anterior joint. Apply the same maximum increment only near
    # the observed posterior speed peak, with a smooth self-limiting gate.
