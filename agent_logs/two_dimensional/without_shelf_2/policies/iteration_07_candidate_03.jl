function target_policy_params()
    return (
        control_period=0.75,
        oscillator_amplitude=22.5 * pi / 180,
...
function target_policy(state, params)
    ...
    # Keep the directly evaluated 0.75/10-degree positive-bearing response.
    # Both the 0.745 and 0.77 same-limit probes took longer, less direct routes;
    # the inherited window-rate lead is also excluded.
    steering_center = params.steering_limit * tanh(
        params.steering_gain * state.bearing / params.steering_limit,
    )

    # Increase only nominal oscillator amplitude by 0.5 degree to probe more
    # upstream-relative propulsion. Its nominal restoring peak remains below
    # the candidate action guard, without a clock or prescribed route.
