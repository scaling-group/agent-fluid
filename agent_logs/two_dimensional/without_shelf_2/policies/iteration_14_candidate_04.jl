function target_policy_params()
    return (
        ...
        oscillator_excess_damping_gain=2.2,
        steering_gain=0.75,
...
function target_policy(state, params)
    ...
    # Preserve the replicated anchor's recovery gain below the requested orbit,
    # but damp wake-driven excess energy without also increasing injection.
    oscillator_position = q1 - steering_center
    oscillator_energy = (oscillator_position / amp)^2 +
        (qd1 / (omega * amp))^2
    energy_error = 1 - oscillator_energy
    energy_gain = ifelse(
        energy_error >= 0,
        params.oscillator_energy_gain,
        params.oscillator_excess_damping_gain,
    )
    oscillator_drive = energy_gain * energy_error * qd1
