function target_policy_params()
    return (
        ...
        oscillator_recovery_gain=2.1,
        oscillator_excess_gain=2.05,
        steering_gain=0.75,
...
function target_policy(state, params)
    ...
    # Separate recovery from excess-energy dissipation using two evaluated
    # gains. The piecewise drive is continuous at the unit-energy orbit.
    oscillator_position = q1 - steering_center
    oscillator_energy = (oscillator_position / amp)^2 +
        (qd1 / (omega * amp))^2
    energy_gain = oscillator_energy < 1 ?
        params.oscillator_recovery_gain : params.oscillator_excess_gain
