function target_policy_params()
    return (
        ...
        oscillator_energy_gain_deficit=2.1,
        oscillator_energy_gain_excess=2.05,
        steering_gain=0.75,
...
function target_policy(state, params)
    ...
    # Use the successful positive-bearing sign without the window-rate lead
    # that converted the same gait into a downstream exit. Increase only the
    # small-error response while retaining the evaluated 10 degree bend bound.
    steering_center = params.steering_limit * tanh(
        params.steering_gain * state.bearing / params.steering_limit,
    )

    # Retain the score-leading recovery while the oscillator is below its
    # requested energy, then use the evaluated lower-load gain for excess-
    # energy damping. The drive remains continuous where energy equals one.
    oscillator_position = q1 - steering_center
    oscillator_energy = (oscillator_position / amp)^2 +
        (qd1 / (omega * amp))^2
    oscillator_energy_gain = ifelse(
        oscillator_energy <= one(oscillator_energy),
        params.oscillator_energy_gain_deficit,
        params.oscillator_energy_gain_excess,
    )
    oscillator_drive = oscillator_energy_gain *
    ...
    # the bound demonstrated by the successful anchor.
