...
- The same deep-rearward dual-opening selector does preserve the closing leg,
  but it does not make body-lateral velocity a useful recovery direction.
  Adding the previously tested `0.35` lateral-velocity subtraction capped at
  `2 deg` only under that selector retained the anchor's identical `6.609L`
  closest approach, yet also retained the visible nose-up exit and
  `+1.798L` head-y travel. It worsened score from `-11.149` to `-11.177`,
  head-x travel from `-4.676L` to `-4.637L`, mean/final range from
  `9.342/9.266L` to `9.365/9.295L`, and progress from `0.2542` to `0.2518`,
  while RMS force/moment stayed effectively unchanged near `75.6/984`.
  Treat the selector as an approach-preserving timing primitive, not evidence
  that a globally negative observation becomes corrective after gating; avoid
  further lateral-velocity gain or limit tuning in this far-field topology.
  Test a bounded terminal curvature objective that actually changes command
  sign, and reject it if it loses the `6.61L` approach, repeats either boundary
  return without better mean/final range, or raises loads. This boundary does
  not claim the selector will remain benign after wake entry.
