...
- Treat the `12 deg` ceiling and fixed `0.04` recent-turn damping as a sharp
  local anchor, not one point on a monotone scalar-gain trend. It produced the
  strongest finite approach (`-4.08L` head x, `6.71L` minimum range, RMS
  force/moment `66.5/958`). With the rest of that law fixed, `10 deg` and
  `14 deg` ceilings retained the upper-exit topology but cut head-x travel to
  `-1.43L` and `-2.46L` and worsened minimum range to `9.00L` and `8.85L`.
  Derivative interpolation was no safer: range-gating toward `0.06` reached
  only `8.59L`, uniform `0.045` exited upward after just `-0.15L` head-x
  travel, and uniform `0.05` made a lower flow-following return ending at
  `(+0.33,-13.31)L`. Before useful wake entry, stop interpolating either
  steering saturation or turn damping around this anchor; move controlled
  ablations to a distinct mechanism, and revisit course-rate feedback only if
  that mechanism reaches the wake corridor. This boundary is falsified by a
  later same-law scalar setting that preserves upstream-relative motion and
  avoids both boundary-return topologies.
- Separate anterior terminal protection from posterior propulsion when
  testing gait guards. The finite `12 deg`/`0.04` anchor kept posterior speed
  to `191.2 deg/time` but let anterior speed reach `249.9 deg/time`, close to
  the `260 deg/time` hard cap despite a `200 deg/time` guard; the unguarded
  folded-body failure reached `260 deg/time`, both action caps, and RMS
  force/moment `20024/314391`. A reusable next test is therefore damping only
  the above-guard anterior excess while leaving all sub-guard motion and the
  posterior guard unchanged. Apply this only to the present far-field
  near-cap regime: reject it if anterior rate falls without retaining the
  anchor's upstream approach, or if the same upper loop persists despite the
  localized rate reduction.
