...
- Prefer a deeply rearward, persistent-opening selector over an abeam-only
  ...
  `0.10` as the terminal anterior-allocation anchor and do not interpolate or
  extrapolate farther downward. An inherited strict-gate posterior test closes
  the other allocation branch: reducing the posterior fraction from `0.65` to
  `0.55` with anterior `0.10` repeats the upper exit, worsens score to
  `-11.213`, upstream travel to `-4.593L`, mean/final range to
  `9.393/9.332L`, and progress to `0.249`, while RMS force/moment stay nearly
  unchanged at `75.5/982`. Freeze the evaluated `0.10/0.65` terminal
  allocation rather than redistributing or reducing it. If testing a distinct
  sensed-motion correction, keep it identically inactive until the same strict
  deep-rearward dual-opening selector confirms the terminal regime; the prior
  always-active lateral-velocity term damaged the approach and does not test
  that localization. Reject such a test if it loses roughly `-4.68L` upstream
  travel, raises loads or joint excursions, or repeats a boundary return
  without better mean/final range. None of these allocation results establishes
  wake entry or `0.75L` capture.
