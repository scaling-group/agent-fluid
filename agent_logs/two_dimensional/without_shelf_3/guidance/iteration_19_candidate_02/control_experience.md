...
- Treat terminal joint allocation under the deeply rearward, persistent
  dual-opening selector as bracketed, not as a recovery control. Moving from
  the inherited fixed-`0.35` anterior allocation to the `0.10/0.65`
  anterior/posterior anchor improved score from `-11.241` to `-11.149`, head-x
  travel from `-4.56L` to `-4.68L`, mean/final range from `9.415/9.360L` to
  `9.342/9.266L`, and anterior speed from about `260` to `251 deg/time`.
  However, full anterior unload to `0.0` worsened score to `-11.165`, posterior
  attenuation to `0.55` worsened it to `-11.213`, and sampled posterior
  enhancement to `0.75` also worsened it to `-11.178`; all retained the same
  `6.609L` closest range and visible nose-up upper exit. The `0.75` test moved
  only `-4.64L` upstream and ended at `9.296L`, despite essentially unchanged
  RMS force/moment (`75.6/985`), while the anchor still drove joint 1 to
  `251 deg/time` and joint 2 only to `191 deg/time`. Before useful wake entry,
  preserve `0.10/0.65`, stop allocation interpolation in either direction,
  and isolate normalized range-opening-gated anterior speed protection above
  the roughly `184 deg/time` nominal orbit but below the measured near-cap
  excursion. Falsify that mechanism if it does not materially lower joint-1
  speed while retaining about `-4.68L` upstream travel and the `6.61L`
  approach, or if it raises loads, transfers saturation to joint 2, or repeats
  the boundary return with worse mean/final range; no allocation result here
  establishes wake exploitation or tight-radius capture.
