...
- Under the deeply rearward, persistent dual-opening selector, terminal joint
  ...
  RMS moment from `1004` to `985`. Full anterior unload to `0.0` slightly
  lowered speed/moment to `249.86 deg/time`/`982.10` but worsened score,
  upstream travel, mean/final range, and progress. Holding anterior `0.10`
  while moving posterior allocation to either `0.55` or `0.75` likewise
  worsened navigation without a meaningful load benefit. All endpoints retain
  the same `6.609L` closest range and visible nose-up upper exit, so the
  selector is only a terminal load/navigation shaper, not a recovery law.
  Before useful wake entry, stop interpolating terminal allocation and test a
  structurally distinct bounded term that is exactly inactive on the closing
  leg; reject it if it loses roughly `-4.68L` upstream travel, worsens the
  `6.61L` approach, raises loads, or repeats a boundary return without better
  mean/final range or lifetime. This does not establish wake exploitation or
  `0.75L` capture.
- Do not repeat terminal anterior speed protection or body-lateral-velocity
  subtraction merely because the strict dual-opening selector preserves the
  closing leg. Under that selector, moving only joint one's speed guard toward
  `190 deg/time` with damping `12` worsens score from `-11.149` to `-11.259`,
  cuts head-x travel from `-4.676L` to `-4.531L`, worsens mean/final range to
  `9.430/9.380L`, and lowers progress to `0.245`. Reusing the previously tested
  lateral-velocity gain `0.35` and `2 deg` cap under the same strict selector
  is negative too: score becomes `-11.177`, head-x travel `-4.637L`,
  mean/final range `9.365/9.295L`, and progress `0.252`, while RMS force/moment
  stay near `75.6/984`. Both retain the identical `6.609L` closest range,
  roughly `+1.80L` terminal rise, and upper exit. In this far-field regime,
  gating prevents approach damage but does not make a noncausal motion or
  actuator signal into recovery; avoid both continuations and next isolate the
  terminal mean-curvature command itself. Reject that direction if it damages
  the anchor's closing leg, raises loads, or merely exchanges the upper return
  for a lower boundary return without better mean/final range or lifetime.
