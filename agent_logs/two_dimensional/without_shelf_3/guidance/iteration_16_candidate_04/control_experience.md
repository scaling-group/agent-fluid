...
- Treat the guarded controller's `12 deg` saturated steering ceiling as locally
  ...
  force/moment rise from `68.8/947` to `76.8/1030`, so only a modest
  one-dimensional continuation of the behind-target fraction is supported.
...
- A deep-rearward, sustained-opening selector is the first sampled structural
  terminal change to improve the `-0.125` approach without retuning its useful
  closing leg. Requiring the target to be more than `1L` rearward and both
  instantaneous and windowed range rates to show opening above `0.02L/time`,
  then reducing only the anterior steering allocation from `0.35` to `0.10`
  while retaining the posterior `0.65`, improves score from `-11.2406` to
  `-11.1487`, head-x travel from `-4.55` to `-4.68L`, progress from `0.247` to
  `0.254`, and mean/final range from `9.42/9.36` to `9.34/9.27L`. It also
  moves anterior angle/speed from `34.24 deg`/`259.9 deg/time` to
  `33.23 deg`/`251.4 deg/time` and lowers RMS force/moment from `75.9/1004` to
  `75.6/985`. This is evidence for terminal mode isolation and anterior-only
  unloading, not recovery: closest range remains `6.61L`, the sheet still
  makes the upper return, and wake entry never occurs. A simple rearward-bearing
  continuation to `-0.15625` also improves progress but exits sooner and is
  worse than this gate on navigation metrics, so do not resume bearing
  interpolation as the recovery mechanism. Continue only a bounded
  one-dimensional anterior-allocation test behind the same three-factor gate,
  with the approach and posterior traveling bend fixed; reject the direction
  if it loses roughly `-4.6L` upstream travel or the `6.65L` approach, repeats
  the return without material navigation gain, approaches a joint cap, or
  raises loads above the sampled `76/1005` scale. This lesson applies before
  wake entry and does not establish a wake-phase or tight-capture controller.
