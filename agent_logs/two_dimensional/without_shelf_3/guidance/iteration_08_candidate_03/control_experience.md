...
- Stop interpolating the guarded angle-only controller's scalar turn settings
  in the far field. Its `0.04` recent-turn gain and `12 deg` ceiling provide the
  only finite propulsion anchor (`-4.08L` head x, `6.71L` minimum), whereas
  `0.045` nearly removes upstream travel, `0.05` exits lower/downstream, a
  range-gated `0.04--0.06` schedule regresses to `8.59L`, and `10/14 deg`
  ceilings retain the upper-exit topology. The shared scalar `bearing` also
  folds target-ahead and target-behind geometry through an absolute forward
  component, matching keyframes in which the target becomes aft/down-left
  during the upper U-turn. Preserve the `0.04/12 deg` anchor when testing a
  normalized `target_body_L` fore/aft-aware course signal; reject that signal
  if it loses the early upstream leg, causes tight looping or load growth, or
  repeats the same exit before useful wake entry.
