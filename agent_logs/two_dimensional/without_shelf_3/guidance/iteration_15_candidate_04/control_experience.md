...
- Treat rearward bearing fraction as an approach tradeoff, not a recovery
  mechanism, and stop interpolating it. The new `-0.125` midpoint is the best
  finite navigation anchor: versus `-0.25` it improves head-x travel from
  `-4.44L` to `-4.55L`, minimum/mean/final range from
  `6.63/9.45/9.43L` to `6.61/9.42/9.36L`, progress from `0.241` to `0.247`,
  and score from `-11.286` to `-11.241`, with slightly lower RMS force/moment
  (`75.9/1004` versus `76.8/1030`). Its sheet nevertheless makes the same
  near-vertical upper exit, lifetime falls from `74.48` to `69.85`, mean
  command effort rises from `714` to `723`, and anterior speed reaches the
  `260 deg/time` hard cap. Delaying an additional `-0.25` reversal until the
  target is more than `1L` aft does not rescue the timing hypothesis: it
  repeats the exit, cuts upstream travel to `-4.32L`, worsens
  minimum/mean/final range to `6.63/9.52/9.54L`, lowers progress to `0.232`,
  and raises RMS force/moment to `78.1/1072`. The inherited immediate `-0.50`
  continuation is worse (`-3.49L` upstream and `0.180` progress), while
  conditional overspeed damping, full-circle bearing, and opening-only
  attenuation also lose the approach without changing the failed topology.
  Use exact `-0.125` only as the current evidence-backed exploitation baseline;
  do not continue rearward-bearing, turn-damping, or speed-guard tuning. Before
  useful wake entry, a future structural recovery term must use a measured,
  bounded discriminator more specific than abeam crossing or an
  eight-observation opening sign and remain inactive through the demonstrated
  closing leg. Reject it if it loses roughly `-4.5L` upstream travel, worsens
  the `6.6L` approach, repeats either boundary return without material metric
  gain, or raises loads above the sampled baseline scale.
