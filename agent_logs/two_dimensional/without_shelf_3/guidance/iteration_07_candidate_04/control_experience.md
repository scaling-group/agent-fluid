...
- Far-field course has a sharp recent-turn-damping boundary, not a supported
  lower-bearing-gain or range-gating fix. With the same guarded gain-`0.60`,
  `12 deg` law, uniform damping `0.04` moved the head `(-4.08,+1.80)L` and
  reached `6.71L` before an upper U-turn. The inherited parent now closes the
  former interpolation at `0.05`: it reached `6.83L`, then made a lower U-turn
  and became almost flow-following in y (mean velocity/local flow
  `-0.142/-0.150`), finishing at `(+0.33,-13.31)L` with joint-one rate near
  its cap. Exact `0.06` likewise reversed to `(+2.62,-7.92)L` after a `5.41L`
  transient minimum. Raising `0.04` toward `0.06` below an `8L` range gate
  instead degraded the upper pass to `(-2.48,+1.77)L` with an `8.59L`
  minimum, while lowering bearing gain to `0.45` retained the upward exit but
  cut upstream displacement to `-0.73L`. Avoid the tested uniform
  `0.05--0.06` band, early distance-gated escalation, and lower bearing gain
  as far-field course fixes; only a tightly bracketed uniform interpolation in
  `(0.04,0.05)` remains supported. This lesson is limited to pre-wake entry
  and is falsified if an interior gain cannot retain negative x while shifting
  lateral travel, or if the apparent boundary moves with wake phase or cap
  contact.
