...
- Treat the guarded controller's `12 deg` saturated steering ceiling as locally
  ...
  the same upper exit and joint-one speed reaches the `260 deg/time` cap.
  Therefore reuse signed fore/aft projection to keep future terminal actions
  inactive on the useful target-ahead leg, but do not treat bearing shutoff as
  a cure by itself; test bounded active straightening after the abeam crossing.
  Reject such a test if it loses roughly `-4.2L` upstream travel, worsens the
  `6.7L` approach, repeats either full return without material metric gain,
  rides a joint cap, or raises loads. This boundary does not rule out motion
  feedback after useful wake entry.
