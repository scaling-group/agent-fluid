...
- Treat the guarded controller's `12 deg` saturated steering ceiling as locally
  ...
  from `65.66` to `69.37`, with similar loads. However, its sheet still shows
  the same upper exit and joint-one speed reaches the `260 deg/time` cap. A
  `-0.25` rearward fraction is a modestly better continuation: it improves
  head-x travel to `-4.44L`, progress to `0.241`, mean/final range to
  `9.45/9.43L`, and lifetime to `74.48`, while moving anterior speed just below
  the cap; it still makes the same upper exit and raises RMS force/moment to
  `76.8/1030`. Do not extrapolate that gain monotonically. The inherited
  `-0.50` fraction, activated by the same `0.5L` abeam transition, repeats the
  exit while cutting upstream travel to `-3.49L`, worsening progress to `0.180`
  and mean/final range to `10.12/10.18L`, and shortening lifetime to `65.44`.
  Strengthening only the rearward overspeed guard is negative too: adding
  damping gain `12` to the `-0.25` controller reaches a transient `6.60L` but
  reduces upstream travel to `-3.71L` and progress to `0.194`, with the same
  visible exit. Therefore reuse signed fore/aft projection to keep future
  terminal actions inactive on the useful target-ahead leg, preserve `-0.25`
  as the locally bracketed abeam law, and avoid stronger reversal or
  joint-speed damping beginning at that first crossing. A stronger action
  remains testable only behind a later normalized body-frame selector that
  demonstrably preserves roughly `-4.4L` upstream travel and the `6.6--6.7L`
  approach; reject it if the same full return persists without material metric
  gain, a joint cap is ridden, or loads rise. This boundary does not rule out
  delayed motion feedback after useful wake entry.
