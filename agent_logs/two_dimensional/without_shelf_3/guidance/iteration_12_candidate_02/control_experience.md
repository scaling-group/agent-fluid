...
- Do not treat the sign of the eight-observation
  ...
  anchor approach, repeats a boundary return, or raises loads.
- Treat rearward-only fore/aft gating as too late to cure the far-field upper
  exit, even though an absolute forward projection can preserve the useful
  approach. With the anchor gait fixed, reducing bearing authority across a
  `0.5L` beam transition produced the best sampled finite approach
  (`-4.36L` head x, `6.64L` minimum range, `0.235` progress, RMS force/moment
  `68.8/947`), while adding `0.02` turn damping over the same rearward scale
  gave `-4.23L`, `6.71L`, `0.226`, and `66.7/957`; both nevertheless repeated
  the visible upper exit with about `+1.80L` head-y travel and `6.09L` maximum
  lateral target offset. Conversely, inherited gating on normalized ahead
  fraction activated early enough to erase self-propulsion (`+0.01L` head x,
  `9.63L` minimum range, negative progress). Before wake entry, do not
  interpolate between absolute and ratio gate widths or expect bearing removal
  or extra damping after the beam to reverse an outward heading. A distinct
  recovery test must leave the target-ahead bearing law exactly unchanged and
  use the rearward target quadrant to request a return; reject it if the same
  upper exit or a lower full orbit persists, if the `-4.2L` approach is lost,
  or if the sampled anterior speed-cap contact and loads worsen.
