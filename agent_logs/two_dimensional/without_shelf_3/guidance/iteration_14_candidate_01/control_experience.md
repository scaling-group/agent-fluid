...
- Treat the signed fore/aft projection as a supported terminal selector, but
  not as evidence that any tested terminal actuator fixes the upper return. In
  the matched rearward-control factorial, modest opposite bearing authority
  (`-0.25`) is the strongest finite case: it travels `-4.44L` upstream, reaches
  `6.63L`, has progress `0.241`, and survives `74.48` time, compared with
  `-4.36L`, `6.64L`, `0.235`, and `69.37` when rearward bearing is zero. Adding
  a `0.02` rearward turn-damping boost either with aliased bearing retained or
  shut off worsens score, lifetime, and mean/final range while preserving the
  same visible upper exit. The assigned parent's later rearward-only overspeed
  boost is negative too: it lowers the anterior peak only from about `254` to
  `250 deg/time`, shortens upstream travel to `-3.71L`, reduces progress to
  `0.194`, worsens mean/final range to `9.94/10.01L`, and still exits upward.
  Before wake entry, preserve the target-ahead leg exactly and do not stack
  more rearward damping or speed-guard authority; isolate any further modest
  bearing reversal behind the evaluated fore/aft gate. This lesson is
  falsified as a route to recovery unless such an isolated test changes the
  terminal topology while retaining roughly `-4.4L` travel and a `6.6L`
  approach; the current evidence supports only the selector and local tuning
  direction, not successful capture or unrestricted full-circle bearing.
