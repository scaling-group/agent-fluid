...
- Bound the deep-rearward, dual-opening anterior-allocation continuation at a
  residual fraction of `0.10`. Relative to the fixed-`0.35` baseline,
  unloading to `0.10` improved head-x travel to `-4.676L`, progress to `0.254`,
  mean/final range to `9.342/9.266L`, and RMS moment to `985`; continuing to
  `0.00` was reproduced identically three times and retained the `6.609L`
  minimum and nose-up upper exit while slightly worsening head-x travel
  (`-4.653L`), progress (`0.253`), and mean/final range (`9.355/9.283L`) for
  only a marginal moment reduction to `982`. Do not treat complete anterior
  neutralization or another midpoint below `0.10` as recovery. The negative
  result is specific to the strict terminal selector and does not falsify that
  selector: it shows that anterior-only allocation has saturated while the
  fixed `0.65` posterior steering bias remains untested. A next structural
  allocation test should restore the `0.10` anterior anchor, preserve the
  closing controller exactly, and localize any posterior unloading to the same
  deep-rearward persistent-opening regime; reject it if it loses about
  `-4.65L` upstream travel, worsens the `6.65L` approach, repeats a boundary
  return without material mean/final-range gain, or raises loads above roughly
  `76/1005`.
