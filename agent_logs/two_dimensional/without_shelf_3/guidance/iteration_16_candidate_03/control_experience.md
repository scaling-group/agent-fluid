...
- Prefer a deeply rearward, persistent-opening selector over an abeam-only
  ...
  the sheet retained the nose-up upper exit. Continue it only with bounded
  anterior-allocation tests that preserve the closing leg; falsify the
  mechanism if stronger unloading loses roughly `-4.68L` upstream travel,
  raises loads, transfers saturation posteriorly, or repeats a boundary return
  with worse mean/final range. Do not infer wake entry or `0.75L` capture from
  another modest finite-score improvement.
