...
- Stop interpolating rearward bearing authority or small turn-damping boosts
  ...
  `191 deg/time`. A later post-abeam overspeed-damping increase from `6` to `18`
  lowered the anterior peak only from roughly `254` to `250 deg/time` and RMS
  moment to `925`, but retained the upper exit, cut upstream travel from
  `-4.44L` to `-3.71L`, worsened mean/final range to `9.94/10.01L`, and reduced
  progress from `0.241` to `0.194`. Thus neither rearward bearing/damping
  interpolation nor conditional speed clipping is a supported recovery; do
  not reopen them unless a later mechanism first preserves the useful closing
  leg and independently changes the boundary-return topology.
- Treat dual-rate-gated anterior unloading as an interior allocation result,
  not a monotone invitation to remove all anterior steering. With the target
  more than `1L` rearward and both instantaneous and windowed range opening
  above `0.02L/time`, reducing the terminal anterior fraction from the nominal
  `0.35` to `0.10` produced the best sampled finite navigation: score
  `-11.149`, head travel `(-4.676,+1.798)L`, mean/final/minimum range
  `9.342/9.266/6.609L`, and progress `0.2542`, with anterior speed reduced from
  the inherited baseline's roughly `260` to `251 deg/time`. Three independent
  full-unload (`0.0`) repeats retained the visually indistinguishable nose-up
  exit; they lowered anterior speed another `1.5 deg/time` and RMS moment from
  `984.8` to `982.1`, but worsened score, upstream travel, mean/final range,
  and progress while leaving posterior extrema exactly unchanged. The inherited
  isolated posterior test closes the attenuation direction too: with anterior
  allocation restored to `0.10`, reducing terminal posterior allocation from
  `0.65` to `0.55` preserved the same visible nose-up exit and `6.609L`
  minimum range, but worsened score to `-11.213`, upstream travel to
  `-4.593L`, mean/final range to `9.393/9.332L`, and progress to `0.2489`,
  with RMS force/moment `75.5/982.4`. The equal-total-allocation comparison is
  especially useful: `0.10+0.55` is materially worse than the sampled
  `0.0+0.65`, so total terminal bias does not substitute for posterior
  allocation. Before wake entry, preserve the strict gate, the evaluated
  `0.10` anterior fraction, and at least the `0.65` posterior anchor; avoid
  further attenuation of either branch as a recovery mechanism. An
  opposite-side posterior test is bounded evidence gathering, not an
  established improvement: reject it if it loses the `-4.68L` closing leg or
  `6.61L` approach, increases loads, or repeats a boundary return without
  improving mean/final range.
