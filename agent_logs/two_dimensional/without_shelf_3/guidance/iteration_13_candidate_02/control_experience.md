...
- Treat the guarded controller's `12 deg` saturated steering ceiling as locally
  ...
  without curing the upper-turn topology. This bracket does not rule out a
  bounded authority change conditioned on signed fore/aft target geometry or
  motion feedback after useful wake entry, but reject either if it loses the
  anchor's `-4.08L` upstream leg, worsens its `6.71L` approach, repeats either
  full return, or raises loads.
- A signed `0.5L` fore/aft transition now isolates a modestly favorable
  rearward-control direction, but it has not cured far-field course loss.
  With the target-ahead gait, guards, `12 deg` ceiling, `0.60` bearing gain,
  and `0.04` recent-turn damping fixed, changing behind-beam bearing authority
  from zero to `-0.25` extended release lifetime from `69.37` to `74.48`,
  improved head-x travel from `-4.36L` to `-4.44L`, progress from `0.235` to
  `0.241`, and mean/final range from `9.54/9.50L` to `9.45/9.43L`; it also
  avoided exact anterior speed-cap contact. Adding a `0.02` rearward damping
  boost with zero bearing authority instead shortened head-x travel to
  `-4.15L`, reduced progress to `0.222`, and ended at `9.67L` range despite a
  transiently better `6.57L` minimum. Prefer an isolated bounded rearward
  countersteer test over another damping addition, but treat `-0.25` only as a
  directional signal: it still exited nose-up at `6.63L` closest range and
  raised RMS force/moment from `68.8/947` to `76.8/1030`. Falsify stronger
  reversal if it fails to exceed the `74.48` lifetime and `-4.44L` upstream
  leg, creates a lower return, worsens closest/mean range, restores speed-cap
  contact, or increases loads without additional approach. This lesson applies
  before wake entry and does not establish wake rejection or capture behavior.
