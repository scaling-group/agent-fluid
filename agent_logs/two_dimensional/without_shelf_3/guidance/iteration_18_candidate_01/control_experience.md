...
- A strict terminal selector can improve load and navigation shaping without
  being a course-recovery mechanism. Relative to the fixed `0.35` anterior
  allocation baseline, unloading to `0.10` only after the target is more than
  `1L` rearward and both instantaneous and windowed range rates show opening
  faster than `0.02L/time` improved head-x travel from `-4.555L` to `-4.676L`,
  mean/final range from `9.415/9.360L` to `9.342/9.266L`, progress from `0.247`
  to `0.254`, RMS moment from `1004` to `985`, and anterior peak speed from
  `259.9` to `251.4 deg/time`. Continuing the same unload from `0.10` to `0.0`
  was negative in replicated samples: it reduced peak speed by only another
  `1.5 deg/time` and RMS moment by about `2.7`, while worsening upstream travel,
  mean/final range, progress, and score and preserving the identical nose-up
  upper exit and `6.609L` minimum range. Preserve `0.10` as the bounded
  far-field terminal-allocation anchor; do not infer that more attenuation is
  safer or that this selector enables wake entry. Revisit it only if a later
  policy changes the closing-leg topology, and falsify its local benefit if a
  controlled replication fails to retain roughly `-4.68L` upstream travel,
  `0.254` progress, and RMS moment near `985` without raising loads.
