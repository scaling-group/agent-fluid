...
- Do not escalate turn-rate damping from a visible loop without a controlled
  ...
  uniform `0.10` damping was worse again (`-0.73L` head x, `9.90L` minimum
  range). Reversing curvature while combining turn-rate and moment feedback
  also moved `+2.74L` downstream, and a bearing-window-rate architecture failed
  unstable after `1.66` time. Before useful wake entry, hold the demonstrated
  small fixed damping and isolate static steering authority or gait guards;
  revisit derivative additions only after a finite static loop reaches the wake
