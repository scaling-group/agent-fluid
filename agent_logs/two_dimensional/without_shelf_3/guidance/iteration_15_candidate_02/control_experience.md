...
- Treat the guarded controller's `12 deg` saturated steering ceiling as locally
  ...
  the exit. At most one conservative approach refinement between `-0.125` and
  `-0.25` remains supported, and it must both preserve roughly `-4.5L` upstream
  travel and a `6.65L` approach and move anterior speed off the cap or improve
  lifetime/mean range. If it repeats the upper return without material metric
  gain, stop fraction, damping, and speed-guard interpolation and test a
  structural terminal discriminator that remains inactive through the closing
  leg. Do not instead reconstruct a full-circle angle from `target_body_L`:
