...
- Stop interpolating rearward steering or treating the anterior speed peak as
  the causal handle for the far-field upper exit. Four sampled fore/aft
  variants spanning full, zero, and `-0.25` rearward bearing authority plus
  `0--0.02` added damping all make the same visible nose-up return, exit with
  about `+1.80L` head-y travel, move only `-4.15` to `-4.44L` upstream, and
  reach `6.57--6.71L`. The `-0.25` fixed-`0.04` variant is the best exploitation
  anchor by score, lifetime, progress, and mean/final range, but its `6.63L`
  closest range is followed by a `9.43L` final range and RMS force/moment
  `76.8/1030`, so it is not a recovery mechanism. Extending that fraction to
  `-0.50` closes the apparent positive gradient: it repeats the upper exit,
  cuts upstream travel to `-3.49L`, reaches only `6.76L`, worsens mean/final
  range to `10.12/10.18L`, and lowers progress to `0.180`. Conditional
  overspeed damping closes the prior speed-protection hypothesis as well:
  raising damping from `6` to `18` after abeam lowers the anterior peak only
  from about `254` to `250 deg/time`, retains the exit, cuts upstream travel to
  `-3.71L`, and worsens mean/final range to `9.94/10.01L`; its lower RMS moment
  `925` is not navigation recovery. Inherited full-circle target-vector bearing
  and short-window opening attenuation are stronger negatives, moving only
  `-0.57L` and `-2.27L` upstream respectively while losing the approach. Use
  the exact `-0.25` policy as an evidence-backed baseline, and avoid further
  local rearward-bearing, turn-damping, or speed-guard continuation. A future
  structural recovery term must remain inactive through the demonstrated
  closing leg and use a terminal discriminator more specific than abeam
  crossing or the sign of an eight-observation range window; reject it if it
  loses roughly `-4.4L` upstream travel, worsens the `6.6L` approach, repeats
  either boundary return, or raises loads above the sampled anchor scale.
