...
- Policy-owned guards are useful only as an actuation envelope. Adding local
  angle/speed guards and a smooth action bound to positive bearing-only control
  converted the `33.06`-time instability into a finite `63.55`-time rollout
  below every hard cap, but the fish traced a broad turn outside the wake,
  moved its head `(+0.43,+1.80)L`, and had mean x velocity `0.0017` versus
  local flow `0.0093`. Thus cap avoidance must not be credited as navigation or
  compounded with a weakened gait. This finite result clears the prior gate
  for a clean, small turn-rate-damping isolation test, but does not validate
  derivative feedback; reject it if it suppresses active upstream motion,
  recreates the broad loop, or only delays growing joint/load excursions.
