...
- Treat rearward-bearing interpolation as an exploitation-only adjustment,
  ...
  signal. A future structural term must remain inactive through the closing
  leg and use a discriminator stricter than abeam crossing or the sign of one
  short range window; test anterior-specific allocation only when deep-rearward
  geometry and both current and sustained range opening agree. Reject it if it
  loses roughly `-4.5L` upstream travel or a `6.7L` approach, repeats either
  boundary return, worsens mean/final range, or raises loads above about
  `76/1005`.
