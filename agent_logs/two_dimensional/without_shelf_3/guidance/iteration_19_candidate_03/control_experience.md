...
- Prefer a deeply rearward, persistent-opening selector over an abeam-only
  selector when localizing terminal corrections, but stop treating anterior
  allocation below `0.10` as an improvement direction. Relative to two
  identical evaluations of the fixed-`0.35` allocation,
  `behind_bearing_fraction=-0.125` baseline, unloading toward `0.10` only
  after the target was more than `1L` rearward and both instantaneous and
  eight-observation range rates exceeded `0.02L/time` improved score from
  `-11.241` to `-11.149`, head-x travel from `-4.555L` to `-4.676L`,
  mean/final range from `9.415/9.360L` to `9.342/9.266L`, and progress from
  `0.247` to `0.254`. It also reduced anterior angle/speed from about
  `34.24 deg`/`259.90 deg/time` to `33.23 deg`/`251.35 deg/time` and RMS
  moment from `1004` to `985`. Three current full-unload (`0.0`) evaluations
  then reproduce the same nose-up upper exit and `6.609L` minimum while
  worsening score to `-11.165`, head-x travel to `-4.653L`, mean/final range
  to `9.355/9.283L`, and progress to `0.253`; their small further reductions
  in anterior angle/speed (`33.11 deg`/`249.86 deg/time`) and RMS moment
  (`982`) are load shaping bought with navigation loss, not recovery. Preserve
  `0.10` as the terminal-allocation anchor and do not interpolate or extrapolate
  farther downward. If testing another correction, keep it inactive until the
  same strict selector confirms opening and change a distinct measured failure
  signal; abeam-gated damping and always-active motion feedback already damaged
  the approach. Reject it if it loses roughly `-4.68L` upstream travel, raises
  loads or joint excursions, or repeats a boundary return without better
  mean/final range. Treat posterior allocation as bracketed too: under the
  same strict selector, reducing its nominal `0.65` share to `0.55` worsens
  score to `-11.213`, upstream travel to `-4.59L`, mean/final range to
  `9.393/9.332L`, and progress to `0.249`, while increasing it to `0.75`
  worsens score to `-11.178`, upstream travel to `-4.64L`, mean/final range
  to `9.366/9.296L`, and progress to `0.252`; both preserve the `6.609L`
  minimum, nose-up exit, and essentially unchanged `76/985` load scale. A
  body-lateral-velocity subtraction with gain `0.35` and a `2 deg` bound is
  now negative even when localized by the strict deep-rearward dual-opening
  selector: it also repeats the same closest range and upper exit while
  worsening score to `-11.177`, head-x travel to `-4.64L`, mean/final range
  to `9.365/9.295L`, and progress to `0.252`, without useful load relief.
  Therefore preserve the evaluated `0.10/0.65` terminal allocation and stop
  tuning either joint share or treating body sideslip as a recovery signal;
  the selector can localize a correction but does not make its input causal.
  Test only a distinct measured terminal failure signal through that selector,
  and reject it if it loses roughly `-4.68L` upstream travel, raises loads or
  joint excursions, or repeats a boundary return without better mean/final
  range. None of these results establishes wake entry or `0.75L` capture.
