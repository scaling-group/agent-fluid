...
- Under the deeply rearward, persistent dual-opening selector, terminal joint
  ...
  `0.249`, with no meaningful load benefit. All retain the same `6.609L`
  closest range and visible nose-up upper exit, so this selector supports only
  terminal load/navigation shaping. Before useful wake entry, avoid further
  terminal allocation interpolation and require a structurally distinct term
  that is inactive on the demonstrated closing leg. The inherited first test
  of that requirement is negative: subtracting body-lateral velocity at gain
  `0.35`, capped at `2 deg`, only under the same strict selector preserves the
  `6.609L` closest range but repeats the visually identical nose-up exit and
  `+1.798L` rise, while worsening score from `-11.149` to `-11.177`, head-x
  travel from `-4.676L` to `-4.637L`, mean/final range from
  `9.342/9.266L` to `9.365/9.295L`, and progress from `0.2542` to `0.2518`;
  RMS loads and joint extrema are effectively unchanged. This closes both
  always-active and dual-opening-gated lateral-velocity correction at the
  tested `2 deg` authority: the selector protects the approach, but that
  motion signal does not brake the controlled terminal curvature. Next test a
  gated mechanism that can directly remove mean steering bias rather than
  increasing this lateral-motion gain; reject it if it loses roughly `-4.68L`
  upstream travel, worsens the `6.61L` approach, raises loads, or repeats a
  boundary return with worse mean/final range. This does not establish wake
  exploitation or `0.75L` capture, and it does not rule out differently
  scaled motion feedback after a policy first reaches the wake corridor.
