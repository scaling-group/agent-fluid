...
- Far-field course is locally pinned to the guarded gain-`0.60`, limit-`12 deg`,
  turn-damping-`0.04` request law rather than improved by nearby scalar or
  direct motion-feedback tuning. That anchor moved the head `(-4.08,+1.80)L`,
  reached `6.71L`, and held RMS force/moment to `66.5/958`; inherited isolated
  damping values `0.0375`, `0.0425`, and `0.045` kept the upper exit but cut
  upstream travel to `-0.63L`, `-1.93L`, and `-0.15L`, while `0.05` made a
  flow-following lower return. Range-gated damping, lower bearing gain,
  `10/14 deg` ceilings, and absolute-bearing rolloff also lost the approach.
  A sampled body-lateral correction `-2 deg*tanh(0.35*v_y/2 deg)` remained
  finite but began the same upper turn earlier, reducing travel to `-2.72L`
  and worsening minimum range to `8.20L` with RMS loads `78.6/1084`.
  Before wake entry, preserve the anchor's request law when testing a distinct
  bounded mapping or allocation; avoid further gain/limit/damping interpolation
  and do not treat same-sign lateral-velocity subtraction as an established
  course fix. This boundary is falsified if a structurally isolated controller
  retains at least the anchor's upstream leg while redirecting the upper exit;
  it says nothing yet about feedback needed inside the wake or capture radius.
