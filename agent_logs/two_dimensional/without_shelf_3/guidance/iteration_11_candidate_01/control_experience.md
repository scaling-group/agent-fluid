...
- Treat the guarded controller's `12 deg` saturated steering ceiling as locally
  ...
  or static curvature reallocation. A next structural gate must preserve the
  full target-ahead anchor and be tied to a distinct body-relative course
  transition; falsify it if the target-ahead leg falls short of `-4.08L`, the
  `6.71L` approach worsens, either return persists, or loads rise.
