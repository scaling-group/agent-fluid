...
- Treat the guarded controller's `12 deg` saturated steering ceiling as locally
  ...
  approach. Before wake/target entry, require a bounded structural term to be
  inactive during the demonstrated closing leg and selected by a terminal
  observation that has survived a controlled test. This boundary does not rule
  out motion feedback after useful wake entry, but reject such a test if it
  loses the anchor's `-4.08L` upstream leg, worsens its `6.71L` approach,
...
- Do not treat the sign of the eight-observation
  ...
  closing leg. Signed fore/aft target projection has now passed the early-leg
  selector test, while the choice of rearward action remains only locally
  supported. Relative to rearward extra damping with aliased bearing, shutting
  bearing off behind the beam improves released life from `65.66` to `69.37`,
  head-x travel from `-4.23L` to `-4.36L`, minimum range from `6.71L` to
  `6.64L`, and progress from `0.226` to `0.235` at similar loads. Applying a
  modest opposite bearing fraction of `-0.25` after the same beam crossing
  improves life again to `74.48`, head-x travel to `-4.44L`, progress to
  `0.241`, and mean/final range to `9.45/9.43L`; it also lowers anterior peak
  speed from the `260 deg/time` cap to `254 deg/time`. In contrast, combining
  bearing shutoff with `0.02` rearward damping loses travel (`-4.15L`) and
  progress (`0.222`) and exits sooner (`67.24`). Therefore retain the full
  target-ahead law and fixed `0.04` damping, use signed fore/aft projection to
  gate only the bearing term, and prefer bounded opposite bearing over extra
  rearward damping as the next terminal straightening direction. This is not
  evidence for unlimited gain: `-0.25` still exits upward and raises RMS
  force/moment from `68.8/947` to `76.8/1030`. Reject stronger authority if it
  damages roughly `-4.4L` upstream travel or the `6.7L` approach, tightens the
  upper loop, creates a lower return, rides a joint cap, or further raises
  loads.
