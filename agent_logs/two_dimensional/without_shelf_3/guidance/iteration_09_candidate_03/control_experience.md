...
- Treat `0.04` recent-turn damping as the upper edge of the demonstrated
  ...
  course fixes. Before wake entry, only a small isolated damping reduction
  below `0.04` remains justified, and it is falsified if it fails to retain
  the anchor's negative-x travel, raises loads/cap contact, or merely changes
  the boundary-exit direction.
