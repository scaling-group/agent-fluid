...
- Far-field course has a sharp recent-turn-damping boundary, not a supported
  lower-bearing-gain or steering-ceiling fix. With the guarded gain-`0.60`,
  `12 deg` law, damping `0.04` moved the head `(-4.08,+1.80)L` and reached
  `6.71L` before an upper U-turn. Its isolated `0.045` continuation repeated
  the upper turn but collapsed travel to `(-0.15,+1.76)L`, worsened minimum
  range to `9.59L`, and raised RMS force/moment from `66.5/958` to `80.0/1259`;
  `0.05` instead returned with the flow to `(+0.33,-13.31)L`. Range-gating
  toward `0.06` reached only `8.59L`, while `10 deg` and `14 deg` ceilings at
  damping `0.04` cut upstream travel to `-1.43L` and `-2.46L`. Avoid damping
  at or above `0.045`, distance-gated increases, gain reduction, and ceiling
  changes as established course fixes; only a one-variable gain strictly
  between `0.04` and `0.045` remains an isolated bracket test. This lesson
  applies before wake entry and is falsified if that midpoint cannot retain
  meaningful negative x while shifting the upper-exit course, or if loads and
  cap contact grow.
