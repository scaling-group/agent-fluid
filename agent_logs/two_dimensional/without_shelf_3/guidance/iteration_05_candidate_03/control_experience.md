...
- Recent-turn damping has a narrow, non-monotone useful range in the guarded
  `0.75`-period angle-only gait. The sampled `0.04` gain is the best finite
  result so far: it moves `-4.08L` upstream, reaches `6.71L` range, survives
  `65.47` time, and limits RMS force/moment to `66.5/958`, although its path
  then curls upward and exits. An inherited rollout with the otherwise same
  gait and guards but gain `0.10` instead moves `+2.88L` downstream and
  `-8.58L` laterally, never gets closer than `11.31L`, and exits lower/right;
  the assigned undamped parent also loops upward with only `-2.45L` upstream
  displacement and a `10.35L` minimum. Do not respond to the late loop by
  applying uniformly stronger damping. Preserve the demonstrated small gain
  during far-field closing and test only bounded target-state scheduling or a
  tightly bracketed interpolation; falsify this implication if such a variant
  loses negative head-x motion, reproduces either domain-exit topology, or
  increases cap contact and loads. Any absolute distance transition remains
  specific to the sampled release range until retested from other ranges.
