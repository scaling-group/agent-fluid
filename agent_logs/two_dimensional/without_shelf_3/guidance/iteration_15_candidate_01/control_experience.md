...
- Treat the guarded controller's `12 deg` saturated steering ceiling as locally
  ...
  bounded `-0.25` fraction of that existing bearing after abeam is the first
  active-straightening step with a positive sampled gradient: versus bearing
  shutoff it improves head-x travel from `-4.36L` to `-4.44L`, minimum range
  ...
  moving joint-one speed below the cap. The upper exit persists and RMS
  force/moment rise from `68.8/947` to `76.8/1030`. The inherited `-0.50`
  continuation disproves a monotone benefit beyond `-0.25`: it repeats the
  upper exit, cuts head-x travel to `-3.49L`, worsens minimum/mean/final range
  to `6.76/10.12/10.18L`, lowers progress to `0.180`, and shortens lifetime to
  `65.44`; its lower `65.5/901` loads accompany a lost useful leg rather than
  course recovery. Increasing overspeed damping from `6` to `18` only after
  abeam is negative too. It reduces anterior peak speed only from about `254`
  to `250 deg/time` and moment RMS from `1030` to `925`, while the same upper
  exit remains, head-x travel falls to `-3.71L`, progress to `0.194`, and
  mean/final range worsen to `9.94/10.01L`. The sampled `-0.125` midpoint then
  supplies the best approach metrics in this bracket: upstream head travel
  improves to `-4.55L`, minimum range to `6.61L`, progress to `0.247`, and
  mean/final range to `9.42/9.36L`, with slightly lower RMS force/moment
  `75.9/1004` than at `-0.25`. This is a real interior optimum for approach
  quality, but not a recovery: its sheet repeats the same upper exit, lifetime
  falls to `69.85`, and anterior speed returns to the `260 deg/time` cap.
  Delaying an added `-0.25` reversal until the target is more than `1L` aft is
  negative as well: it retains the exit, cuts head-x travel to `-4.32L`, lowers
  progress to `0.232`, worsens mean/final range to `9.52/9.54L`, and raises RMS
  force/moment to `78.1/1072`. Treat `-0.125` as the exploitation anchor and
  stop using rearward-bearing interpolation, stronger late reversal, or
  conditional speed damping as an upper-return mechanism. Cap proximity is not
  independently causal, but the anchor's anterior/posterior peaks of roughly
  `34.2/25.9 deg` and `260/191 deg/time` leave an evidence-backed one-factor
  test boundary for redistributing the existing bounded steering curvature
  posteriorly without changing its total. Reject that mechanism if it loses
  about `-4.4L` upstream travel or the `6.7L` approach, transfers saturation to
  the posterior joint, repeats the return without a navigation gain, or raises
  loads above the sampled `77/1030` scale. This conclusion applies before
  useful wake entry; it does not establish wake-phase control or capture.
  Do not instead reconstruct a full-circle angle from `target_body_L`: the
  inherited direct `atan` replacement turned upward immediately, moved only
  `-0.57L` upstream, reached `9.27L`, produced negative progress, and raised
  RMS force/moment to `77.4/1157`, showing that a plausible geometric formula
  did not preserve the runtime bearing convention. Reuse signed fore/aft
  projection only as the late selector and scale the demonstrated bearing; do
  not assume a replacement angle's sign or quadrant. Reject the continuation
  if it loses roughly `-4.3L` upstream travel, worsens the `6.7L` approach,
  repeats either full return without material metric gain, rides a joint cap,
  or raises loads materially above the sampled `77/1030` scale. This boundary
  does not rule out motion feedback after useful wake entry.
