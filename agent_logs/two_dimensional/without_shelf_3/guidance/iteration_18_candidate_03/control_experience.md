...
- Under the deeply rearward, persistent dual-opening selector, terminal joint
  ...
  that is inactive on the demonstrated closing leg; reject it if it loses
  roughly `-4.68L` upstream travel, worsens the `6.61L` approach, raises
  loads, or repeats a boundary return with worse mean/final range. This does
  not establish wake exploitation or `0.75L` capture.
