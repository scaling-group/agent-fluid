...
- A deeply rearward, persistent dual-opening selector localizes terminal
  changes without making attenuation a course-recovery mechanism. When the
  target is more than `1L` rearward and both instantaneous and eight-sample
  range rates open faster than `0.02L/time`, changing anterior allocation from
  fixed `0.35` to `0.10` improved score from `-11.241` to `-11.149`, head-x
  travel from `-4.555L` to `-4.676L`, mean/final range from
  `9.415/9.360L` to `9.342/9.266L`, progress from `0.247` to `0.254`, anterior
  peak speed from `259.9` to `251.4 deg/time`, and RMS moment from `1004` to
  `985`. Four current samples reproduce this anchor exactly, including its
  `6.609L` minimum and nose-up upper exit. Allocation is now bracketed around
  terminal `0.10/0.65`: full anterior unload to `0.0` and posterior changes to
  `0.55` or `0.75` all preserve the minimum and exit while worsening
  navigation, with at most a negligible load benefit. Two structurally
  different proportional corrections under the same strict selector are
  negative as well. Adding `0.04` recent-turn damping worsens score to
  `-11.257`, head-x travel to `-4.532L`, mean/final range to
  `9.429/9.377L`, progress to `0.245`, and RMS moment to `987`; a body-lateral-
  velocity subtraction capped at `2 deg` worsens score to `-11.177`, head-x
  travel to `-4.637L`, mean/final range to `9.365/9.295L`, and progress to
  `0.252`. Both retain the identical `6.609L` closest range, controlled upper
  return, and nearly unchanged joint extrema. Before useful wake entry, keep
  `0.10/0.65` only as a far-field shaping anchor and avoid further allocation,
  turn-damping, or same-sign lateral-motion attenuation under this selector;
  require a bounded test capable of changing terminal curvature topology while
  leaving the closing leg inactive. Reopen these branches only if a later law
  changes the approach topology or new diagnostics identify a distinct regime;
  reject any replacement that loses roughly `-4.68L` upstream travel, worsens
  the `6.61L` approach, raises loads, or substitutes another boundary return.
