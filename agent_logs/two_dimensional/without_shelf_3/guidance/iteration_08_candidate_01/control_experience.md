...
- Treat the guarded controller's `12 deg` saturated steering ceiling as locally
  ...
  Before wake/target entry, preserve the demonstrated `12 deg` moderate-error
  authority and test a bounded change in the large-bearing response shape
  rather than another global ceiling interpolation. Falsify that implication
  if large-bearing shaping reduces travel before the `6.71L` approach, repeats
  a full return, or raises joint/load extrema.
