...
- Treat the guarded controller's `12 deg` saturated steering ceiling as locally
  ...
  without curing the upper-turn topology. Always-active body-lateral-velocity
  subtraction is negative too: gain `0.35` capped at `2 deg` repeats the upper
  exit, cuts upstream travel to `-2.72L`, worsens minimum range to `8.20L`,
  leaves head-y exit travel at `+1.80L`, and raises RMS force/moment to
  `78.6/1084`. Conditioning attenuation only on smoothed range opening also
  fails: scaling bearing authority toward `0.35` over `0.02 L/time` produces
  the same upper exit, moves only `-2.27L` upstream, reaches `7.99L`, lowers
  progress to `0.099`, and raises loads to `117/1568`. A static anterior-bias
  allocation shift from `0.35` to `0.30` is no safer: it moves `+0.07L`
  downstream, reaches only `9.82L`, and has negative progress. Before useful
  wake entry, avoid another always-active motion term, range-only regime gate,
  or static curvature reallocation. Fore/aft gating now has a useful boundary:
  the assigned parent's cosine-like forward-projection/range gate with width
  `0.15` activated during reorientation, erased upstream travel (`+0.01L` head
  x), reached only `9.63L`, and produced negative progress, whereas a sharp
  gate on raw body-forward target distance over `0.5L` retained and slightly
  improved the demonstrated approach (`-4.36L` head x, `6.64L` minimum range,
  progress `0.235`, RMS force/moment `68.8/947`). The sharp gate still exited
  upward and joint one touched `260 deg/time`, so switching the aliased bearing
  completely off behind the beam is not a recovery law. Preserve the full
  target-ahead command, avoid broad orientation-fraction gates, and test a
  bounded unaliased rear-target heading or another explicit recovery signal;
  reject it if the `-4.2L`/`6.7L` approach is lost, either return persists, or
  joint/load extrema rise. This boundary applies before wake entry and does
  not yet establish wake-phase rejection or tight-radius capture.
