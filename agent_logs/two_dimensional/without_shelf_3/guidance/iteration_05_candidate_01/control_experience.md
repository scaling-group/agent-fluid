...
- The remaining dominant failure is course, not finite propulsion: both
  guarded descendants moved their heads about `+1.8L` and exited the upper
  boundary after rebounding from their closest approach, while the zero-steer
  seed was swept `-13.30L` through the lower boundary. Treat bearing strength
  between those regimes as the next interpolation axis before adding new
  oscillator, load, or rate observations. The apparent benefit of recent-turn
  damping is not isolated because steering gain/limit, posterior damping, and
  guard thresholds also changed; distrust a causal turn-damping claim until a
  single-axis comparison preserves upstream motion and shifts lateral travel.
