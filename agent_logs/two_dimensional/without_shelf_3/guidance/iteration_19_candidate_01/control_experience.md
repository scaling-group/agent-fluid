...
- Prefer a deeply rearward, persistent-opening selector over an abeam-only
  selector when localizing terminal anterior steering changes. Relative to two
  identical evaluations of the fixed-`0.35` anterior-allocation,
  `behind_bearing_fraction=-0.125` baseline, unloading the anterior allocation
  toward `0.10` only after the target was more than `1L` rearward and both
  instantaneous and eight-observation range rates exceeded `0.02L/time`
  improved score from `-11.241` to `-11.149`, head-x travel from `-4.555L` to
  `-4.676L`, mean/final range from `9.415/9.360L` to `9.342/9.266L`, and
  progress from `0.247` to `0.254`. It also reduced anterior angle/speed from
  about `34.24 deg`/`259.90 deg/time` to `33.23 deg`/`251.35 deg/time` and RMS
  moment from `1004` to `985` while posterior extrema stayed effectively
  fixed. This supports the selector and localized allocation as terminal
  shaping, not task recovery: the `6.609L` closest range did not improve and
  the sheet retained the nose-up upper exit. Terminal allocation is now
  bracketed on both joints. Complete anterior unload to `0.0` and posterior
  attenuation to `0.55` retained the same exit and closest range while
  worsening navigation; raising posterior allocation to `0.75` also kept the
  same topology and posterior extrema, and worsened score from `-11.149` to
  `-11.178`, head-x travel from `-4.676L` to `-4.642L`, mean/final range from
  `9.342/9.266L` to `9.366/9.296L`, and progress from `0.2542` to `0.2518`
  without a load benefit. Preserve `0.10/0.65` as the terminal allocation
  anchor and stop tuning those fractions under this selector; the next test
  should act on a structurally distinct observation while remaining inactive
  during the closing leg. Reopen allocation only if a new selector changes the
  return topology or later wake-entry evidence exposes a different regime, and
  do not infer wake entry or `0.75L` capture from another modest finite-score
  improvement.
