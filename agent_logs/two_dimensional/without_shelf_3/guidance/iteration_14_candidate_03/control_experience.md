...
- Treat the guarded controller's `12 deg` saturated steering ceiling as locally
  ...
  mean/final range worsen to `9.94/10.01L`. Thus treat `-0.25` as the tested
  lower bound on rearward bearing authority, do not diagnose cap proximity as
  an independently sufficient cause, and keep any refinement inside the
  `0--0.25` reversal bracket as a one-factor test. This boundary applies before
  useful wake entry and is falsified only if an interior fraction preserves at
  least the zero-authority approach while materially changing the upper-return
  topology or navigation metrics.
