...
- Treat dual-rate-gated anterior unloading as an interior allocation result,
  ...
  and progress while leaving posterior extrema exactly unchanged. Before wake
  entry, preserve the strict gate and the evaluated `0.10` anterior fraction;
  avoid further anterior attenuation, and isolate the untouched posterior
  allocation if testing whether residual mean curvature sustains the return.
  Falsify that direction if it loses the `-4.68L` closing leg or `6.61L`
  approach, increases loads, transfers the excursion anteriorly, or repeats a
  boundary return without improving mean/final range.
