...
- Treat the guarded controller's `12 deg` saturated steering ceiling as locally
  ...
  approach. Short-window range opening is not a sufficient terminal selector
  either: an inherited opening-gated attenuation shortened head-x travel to
  `-2.27L`, worsened minimum range to `7.99L`, and raised RMS force/moment to
  `117/1568` while retaining the upper exit. In contrast, gating only the
  bearing term with signed fore/aft target projection is the first late-mode
  discriminator to preserve the approach: against the rearward-damping prefill
  it improved head-x travel from `-4.23L` to `-4.36L`, minimum range from
  `6.71L` to `6.64L`, progress from `0.226` to `0.235`, and release lifetime
  from `65.66` to `69.37`, with similar loads. Its sheet still shows the same
  upper exit and joint-one speed reaches the `260 deg/time` cap. Retaining a
  bounded `-0.25` fraction of that existing bearing after abeam was the first
  active-straightening step with a positive sampled gradient: versus bearing
  shutoff it improved head-x travel from `-4.36L` to `-4.44L`, minimum range
  from `6.64L` to `6.63L`, progress from `0.235` to `0.241`, mean/final range
  from `9.54/9.50L` to `9.45/9.43L`, and lifetime from `69.37` to `74.48`, while
  moving joint-one speed below the cap. The newly evaluated interior `-0.125`
  fraction improves navigation again: head-x travel `-4.55L`, minimum range
  `6.61L`, progress `0.247`, mean/final range `9.42/9.36L`, and score
  `-11.241` are the best in the current bracket. However, it shortens lifetime
  to `69.85`, returns joint-one speed to the `260 deg/time` cap, and visibly
  repeats the upper exit with RMS force/moment `75.9/1004`. This establishes a
  narrow non-monotone approach tradeoff inside `0` to `-0.25`, not a recovery
  mechanism. A deep-rearward transition toward `-0.50` closes the adverse
  side: it preserves the same `6.63L` transient minimum but cuts head-x travel
  to `-4.32L`, progress to `0.232`, worsens mean/final range to `9.52/9.54L`,
  and raises loads to `78.1/1072`; inherited full-`-0.50` and rearward-only
  overspeed-damping tests likewise shortened the useful leg without changing
  the exit. The evaluated `-0.15625` interior fraction closes the remaining
  scalar bracket: versus `-0.125` it improves upstream travel from `-4.55L` to
  `-4.64L`, progress from `0.247` to `0.252`, mean/final range from
  `9.42/9.36L` to `9.38/9.29L`, and RMS force/moment from `75.9/1004` to
  `69.8/949`, but worsens closest range to `6.64L`, shortens lifetime from
  `69.85` to `67.74`, leaves anterior speed near the cap at about
  `258 deg/time`, and visibly repeats the upper exit. Stop rearward-fraction,
  damping, and speed-guard interpolation: they expose non-monotone approach
  tradeoffs, not recovery. A deep-rearward structural discriminator is more
  promising only when its action is localized. On the `-0.125` baseline, a
  conjunction of target more than `1L` rearward plus instantaneous and windowed
  opening above `0.02L/time`, used solely to unload anterior steering
  allocation from `0.35` to `0.10`, improves upstream travel to `-4.68L`,
  progress to `0.254`, mean/final range to `9.34/9.27L`, lifetime to `70.14`,
  and score to `-11.149`; it lowers anterior angle/speed to about
  `33.2 deg/251 deg/time` and RMS force/moment to `75.6/985` while preserving
  the `6.61L` closest approach. This positive gradient distinguishes
  late localized curvature allocation from the inherited negative
  opening-gated global attenuation, but the same upper exit remains. At most
  one bounded continuation toward zero anterior terminal allocation is
  supported; reject magnitude tuning if it erodes the roughly `-4.6L` closing
  leg, raises loads or joint excursion, or repeats the exit without material
  lifetime/mean-range gain, and then test a different late-mode structure.
  Do not instead reconstruct a full-circle angle from `target_body_L`:
  the inherited direct `atan` replacement turned upward immediately, moved
  only `-0.57L` upstream, reached `9.27L`, produced negative progress, and
  raised RMS force/moment to `77.4/1157`, showing that a plausible geometric
  formula did not preserve the runtime bearing convention. Reuse signed
  fore/aft projection only as the late selector and scale the demonstrated
  bearing; do not assume a replacement angle's sign or quadrant. This boundary
  applies before useful wake entry and does not rule out motion feedback after
  entry.
