...
- Stop interpolating rearward steering or treating anterior speed alone as the
  causal handle for the far-field upper exit. Inherited full, zero, `-0.25`,
  and `-0.50` rearward authority, added turn damping, full-circle target-vector
  bearing, short-window opening attenuation, and conditional overspeed damping
  all preserve an upper return or lose the useful approach; the damping test
  lowers anterior speed from about `254` to `250 deg/time` but cuts upstream
  travel to `-3.71L` and worsens mean/final range to `9.94/10.01L`, so cap
  proximity is not itself a recovery signal. A stricter discriminator does
  isolate a small positive allocation effect. Against two identical `-0.125`
  baselines, requiring the target to be more than `1L` rearward and requiring
  both current and windowed range opening before reducing anterior steering
  to `0.10` preserves the same `6.609L` minimum range while improving score
  from `-11.241` to `-11.149`, head-x travel from `-4.555` to `-4.676L`,
  mean/final range from `9.415/9.360` to `9.342/9.266L`, and progress from
  `0.2466` to `0.2542`. It also lowers anterior angle/speed peaks from
  `34.24 deg`/`259.90 deg/time` to `33.23 deg`/`251.35 deg/time` and RMS
  force/moment from `75.9/1004` to `75.6/985`. However, lifetime rises by only
  `0.29`, the keyframes still show the same nose-up upper exit before wake
  entry, and the assigned `-0.15625` parent remains lower-load (`69.8/949`)
  despite slightly worse navigation (`-11.197`, `9.384/9.290L` mean/final
  range). The reusable implication is narrow: a conjunctive deep-rearward plus
  dual-opening gate can keep a terminal anterior-allocation test inactive
  through the demonstrated closing leg, unlike abeam or one-window gates, but
  the sampled partial unload is exploitation evidence rather than recovery.
  Preserve the exact approach gait and vary only the gated anterior allocation;
  reject the direction if it loses roughly `-4.55L` upstream travel or the
  `6.7L` approach, raises loads above the exact baseline scale, or repeats the
  boundary return without a material lifetime and mean/final-range gain.
