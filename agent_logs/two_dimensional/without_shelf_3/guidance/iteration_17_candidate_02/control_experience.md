...
- Treat rearward-bearing interpolation as an exploitation-only adjustment,
  not a recovery mechanism for the far-field upper exit. The new `-0.125`
  midpoint modestly improves on `-0.25`: score `-11.241` versus `-11.286`,
  head-x travel `-4.55` versus `-4.44L`, minimum/mean/final range
  `6.61/9.42/9.36` versus `6.63/9.45/9.43L`, progress `0.247` versus `0.241`,
  and RMS force/moment `75.9/1004` versus `76.8/1030`. But it shortens lifetime
  from `74.48` to `69.85` and visibly repeats the same nose-up upper exit without
  wake entry. Delaying an extra `-0.25` reversal until the target is more than
  `1L` rearward does not rescue the idea: lifetime reaches `75.88`, but score
  falls to `-11.379`, head-x travel to `-4.32L`, mean/final range worsen to
  `9.52/9.54L`, progress falls to `0.232`, loads rise to `78.1/1072`, and the
  upper-return topology remains. Together with inherited full, zero, and
  `-0.50` rearward authority, full-circle target-vector bearing, added turn
  damping, and conditional overspeed damping, this closes further bearing,
  damping, or cap-proximity continuation. Anterior speed still reaches roughly
  `250--260 deg/time` while posterior speed remains near `191 deg/time`, but
  the failed damping test shows that cap proximity is not itself a recovery
  signal. The stricter structural test supports the selector but brackets
  magnitude-only anterior unloading. Relative to the fixed-`0.35` allocation
  `-0.125` baseline, targeting `0.10` only when the target is more than `1L`
  rearward and both current and sustained range opening exceed `0.02L/time`
  improves score from `-11.241` to `-11.149`, head-x travel from `-4.55L` to
  `-4.68L`, mean/final range from `9.42/9.36L` to `9.34/9.27L`, progress from
  `0.247` to `0.254`, anterior angle/speed from about
  `34.24 deg`/`259.90 deg/time` to `33.23 deg`/`251.35 deg/time`, and RMS
  moment from `1004` to `985`, while retaining the same `6.61L` closest range
  and upper exit. Three identical sampled evaluations of full neutralization
  at `0.0` then repeat that visible exit and unchanged minimum while slightly
  worsening score (`-11.165`), upstream travel (`-4.65L`), mean/final range
  (`9.36/9.28L`), and progress (`0.253`); their only gains are about `0.09`
  more lifetime and small reductions in anterior excursion and RMS moment.
  Therefore keep the deep-rearward dual-opening gate as a reusable localization
  device, but stop treating stronger same-sign unloading as monotone recovery.
  A sign-changing anterior allocation is a distinct bounded falsification test,
  not a demonstrated improvement; reject that channel entirely if it loses
  roughly `-4.5L` upstream travel or the `6.61L` approach, repeats either
  boundary return with worse mean/final range, transfers saturation posteriorly,
  or raises loads above about `76/1005`.
