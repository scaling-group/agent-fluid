function target_policy_params()
    return (
        ...
        terminal_anterior_steering_fraction=0.10,
        terminal_lateral_velocity_gain=0.35,
        terminal_lateral_velocity_limit=2.0 * pi / 180,
...
function target_policy(state, params)
    ...
    # All sampled policies close before making the same upper return, and
    # abeam-only bearing, damping, and speed-guard changes damage that useful
    # leg. Activate recovery only after the target is deeply rearward and both
    # instantaneous and windowed normalized range rates confirm opening.
    ...
    # The same motion correction damaged the sampled closing leg when it was
    # always active. Test it only in the persistent terminal-opening regime,
    # with a separate bound below the established steering ceiling.
    lateral_velocity_request = params.terminal_lateral_velocity_gain *
        Float64(state.velocity_body_U[2])
    lateral_velocity_correction = params.terminal_lateral_velocity_limit *
        tanh(
            lateral_velocity_request /
            max(
                params.terminal_lateral_velocity_limit,
                eps(params.terminal_lateral_velocity_limit),
            ),
        )
    steering_request -= recovery_gate * lateral_velocity_correction
    steering = params.steering_limit * tanh(
        steering_request /
        max(params.steering_limit, eps(params.steering_limit)),
    )

    # Retain the sampled partial anterior unload and fixed posterior allocation;
    # stronger anterior or posterior attenuation both worsened navigation.
