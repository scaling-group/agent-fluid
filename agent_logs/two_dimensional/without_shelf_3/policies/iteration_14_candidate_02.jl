function target_policy_params()
    return (
        ...
        deep_rearward_activation_L=1.0,
        deep_rearward_transition_L=0.5,
        deep_rearward_bearing_delta=-0.25,
...
function target_policy(state, params)
    ...
    # Preserve the best finite anchor through the approach and first rearward
    # body length. The compact bearing aliases ahead and behind because its
    # forward denominator is absolute, so reuse the evaluated signed gate.
    ...
    base_bearing_authority = params.behind_bearing_fraction +
        (1 - params.behind_bearing_fraction) * front_gate

    # The inherited -0.50 fraction was harmful when introduced at abeam.
    # Test its timing separately by adding the extra bounded reversal only
    # once the normalized target projection is more than one body length aft.
    deep_rearward_excess_L = max(
        -forward_target_L - params.deep_rearward_activation_L,
        0.0,
    )
    deep_rearward_gate = tanh(
        deep_rearward_excess_L /
        max(
            params.deep_rearward_transition_L,
            eps(params.deep_rearward_transition_L),
        ),
    )
    bearing_authority = base_bearing_authority +
        params.deep_rearward_bearing_delta * deep_rearward_gate
    steering_request =
