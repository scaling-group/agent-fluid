...
function target_policy(state, params)
    ...
    # Preserve the demonstrated target-ahead law exactly, but keep the full
    # target quadrant after a beam crossing. The supplied compact bearing uses
    # abs(forward_distance), which maps a target behind the fish back to an
    # acute angle and cannot command a shortest-direction return.
    forward_target_L = -Float64(state.target_body_L[1])
    lateral_target_L = Float64(state.target_body_L[2])
    target_heading_error = atan(lateral_target_L, forward_target_L)
    steering_request =
        params.steering_gain * target_heading_error -
