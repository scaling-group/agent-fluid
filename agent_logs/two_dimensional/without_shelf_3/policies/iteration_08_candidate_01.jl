function target_policy_params()
    return (
        ...
        bearing_rolloff_scale=45.0 * pi / 180,
        bearing_rolloff_power=4.0,
        bearing_rolloff_floor=0.25,
...
function target_policy(state, params)
    ...
    # Preserve the demonstrated moderate-error bearing authority, but smoothly
    # reduce only the proportional term when the target becomes far off-axis.
    # The sampled 10 deg ceiling weakened early travel, whereas 14 deg and
    # approach damping made the same saturated upper loop occur earlier.
    bearing_ratio =
        abs(state.bearing) /
        max(params.bearing_rolloff_scale, eps(params.bearing_rolloff_scale))
    bearing_authority = params.bearing_rolloff_floor +
        (1 - params.bearing_rolloff_floor) /
        (1 + bearing_ratio^params.bearing_rolloff_power)
    steering_request =
        params.steering_gain * bearing_authority * state.bearing -
