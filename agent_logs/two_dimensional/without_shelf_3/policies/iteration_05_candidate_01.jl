...
function target_policy(state, params)
    ...
    # Preserve the finite sample's upstream gait and opposing yaw damping. The
    # reduced proportional bearing gain is the sole experimental change: it
    # should recover some target-directed descent without restoring the seed's
    # uncontrolled downward sweep.
    steering_request =
    ...
    # Run the progress-producing angle-only oscillator about the moving
    # steering equilibrium; oscillator phase remains encoded in joint state.
    ...
    # velocity-dependent traveling lag associated with active upstream motion.
    ...
    # hard-cap contact and body folding in the unguarded rollout.
