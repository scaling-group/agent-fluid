...
function target_policy(state, params)
    ...
    # Preserve the only sampled steering direction and oscillator architecture
    # that produced active upstream motion. Oppose a sustained turn so the
    # finite parent does not repeat its broad loop away from the target. The
    # fallback keeps the compact normalized-observation contract usable by
    # callers that provide only the original bearing and joint fields.
    turn_rate = hasproperty(state, :turn_rate_recent) ?
        Float64(state.turn_rate_recent) : 0.0
    steering_request = params.steering_gain * Float64(state.bearing) -
        params.turn_rate_damping_gain * turn_rate
    ...
    # Keep angle-only Van der Pol drive around the moving steering equilibrium.
    # The restored 0.75 period retains the sole sampled upstream-producing gait;
    # full phase-radius regulators were finite but lost propulsion.
    ...
    # Local guards are inactive on the nominal gait and oppose only the
    # high-angle/high-speed route that preceded cap contact and body folding.
    ...
    # A high-order smooth bound is nearly identity at nominal demand while
    # keeping every policy command below the evaluator's hard envelope.
