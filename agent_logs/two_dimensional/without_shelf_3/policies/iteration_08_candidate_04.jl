function target_policy_params()
    return (
        ...
        anterior_overspeed_damping_boost=4.0,
...
function target_policy(state, params)
    ...
    # Restore the strongest finite sample's steering law. Isolated ceilings on
    # either side of 12 deg and nearby turn-damping gains all degraded it.
    steering_request =
    ...
    # Keep the progress-producing angle-only Van der Pol drive around the
    # moving equilibrium; sampled full-orbit regulators were flow-following.
    ...
    # Preserve the local guards that kept the strongest approach finite. Apply
    # extra damping only to the recurrent near-cap anterior rate excursion;
    # posterior overspeed protection and all sub-guard motion are unchanged.
    ...
    anterior_speed_damping = params.overspeed_damping +
        params.anterior_overspeed_damping_boost
    raw_a1 -= params.overangle_stiffness * angle_excess1 * sign(q1) +
        anterior_speed_damping * speed_excess1 * sign(qd1)
