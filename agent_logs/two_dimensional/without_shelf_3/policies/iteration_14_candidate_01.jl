...
function target_policy(state, params)
    ...
    # Preserve the strongest sampled controller exactly while the target is
    # ahead. The compact bearing aliases ahead and behind, so use signed,
    # normalized body-frame geometry only to reverse its authority after the
    # target passes abeam.
