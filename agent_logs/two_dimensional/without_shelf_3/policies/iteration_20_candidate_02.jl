function target_policy_params()
    return (
        ...
        terminal_steering_fraction=0.0,
...
function target_policy(state, params)
    ...
    # A bounded lateral-motion correction under this selector retained the
    # same terminal turn. Instead release all commanded mean curvature after
    # confirmed course loss while leaving the zero-gate approach unchanged.
    terminal_steering_scale = 1 +
        (params.terminal_steering_fraction - 1) * recovery_gate
    steering *= terminal_steering_scale

    # Retain the measured allocation anchor through the gate transition; at a
    # full recovery gate the common steering scale removes both mean biases.
