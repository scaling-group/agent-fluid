...
function target_policy(state, params)
    ...
    # allocation changes on either joint retain that failure. Activate any
    # recovery correction only after the target is deeply rearward and both
    ...
    # Always-active lateral-velocity feedback damaged the demonstrated
    # approach. Under confirmed course loss only, use the same measured scale
    # as a bounded motion brake before the existing steering saturation.
    lateral_velocity_limit = params.terminal_lateral_velocity_limit
    lateral_motion_correction = lateral_velocity_limit * tanh(
        params.terminal_lateral_velocity_gain *
        Float64(state.velocity_body_U[2]) /
        max(lateral_velocity_limit, eps(lateral_velocity_limit)),
    )
    steering = params.steering_limit * tanh(
        (steering_request - recovery_gate * lateral_motion_correction) /
        max(params.steering_limit, eps(params.steering_limit)),
    )

    # Preserve the terminal allocation bracket's best measured joint split.
