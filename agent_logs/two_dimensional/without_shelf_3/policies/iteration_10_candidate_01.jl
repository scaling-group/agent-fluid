function target_policy_params()
    return (
        ...
        anterior_steering_fraction=0.25,
...
function target_policy(state, params)
    ...
    # Preserve the only sampled rhythm and steering direction that produced
    # active upstream motion. A small opposing turn-rate term tests whether the
    # finite bearing-only controller's broad late turn can be arrested without
    # replacing its angle-only propulsion oscillator.
    steering_request =
    ...
    # Keep total requested curvature fixed while moving a small share of its
    # mean bias away from the near-guard anterior oscillator. The posterior
    # joint has substantially more observed angle and rate margin.
