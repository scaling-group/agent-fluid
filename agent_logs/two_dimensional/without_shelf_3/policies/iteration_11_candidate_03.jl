...
function target_policy(state, params)
    ...
    # Preserve the finite anchor while the target is ahead. The compact
    # bearing aliases ahead and behind because its forward denominator is
    # absolute, so release only the bearing contribution after an abeam
    # crossing while retaining the demonstrated recent-turn damping.
