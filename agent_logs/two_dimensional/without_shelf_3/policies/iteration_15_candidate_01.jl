...
function target_policy(state, params)
    ...
    # Preserve the demonstrated target-ahead law. The sampled rearward-bearing
    # response is non-monotone, so retain only a bounded fraction between the
    # evaluated zero and -0.25 controls once the target passes abeam.
    ...
    # Shift a bounded share of the unchanged total steering equilibrium to the
    # posterior joint, which has measured angle and speed headroom, while
    # preserving the state-encoded oscillator and demonstrated course law.
