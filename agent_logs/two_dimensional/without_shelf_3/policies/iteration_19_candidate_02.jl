function target_policy_params()
    return (
        ...
        terminal_anterior_speed_guard=190.0 * pi / 180,
        overangle_stiffness=30.0,
        overspeed_damping=6.0,
        terminal_anterior_overspeed_damping=12.0,
...
function target_policy(state, params)
    ...
    # The sampled dual-opening gate preserved the closing leg and reduced the
    # anterior excursion but retained the upper return. Once the target is
    # deeply rearward, require both instantaneous and windowed range opening
    # before applying the strongest measured terminal anterior allocation.
    ...
    # motion in the sampled far-field approaches.
    ...
    # Keep the nominal guards on the measured closing leg. Once the strict
    # terminal selector is active, strengthen only anterior speed protection;
    # sampled posterior allocation changes were negative and joint two retained
    # speed headroom.
    angle_excess1 = max(abs(q1) - params.joint_angle_guard, 0.0)
    angle_excess2 = max(abs(q2) - params.joint_angle_guard, 0.0)
    anterior_speed_guard = params.joint_speed_guard +
        (params.terminal_anterior_speed_guard - params.joint_speed_guard) *
        recovery_gate
    anterior_overspeed_damping = params.overspeed_damping +
        (params.terminal_anterior_overspeed_damping -
        params.overspeed_damping) * recovery_gate
    speed_excess1 = max(abs(qd1) - anterior_speed_guard, 0.0)
    speed_excess2 = max(abs(qd2) - params.joint_speed_guard, 0.0)
    raw_a1 -= params.overangle_stiffness * angle_excess1 * sign(q1) +
        anterior_overspeed_damping * speed_excess1 * sign(qd1)
