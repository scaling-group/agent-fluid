function target_policy_params()
    return (
        ...
        terminal_turn_rate_damping_gain=0.04,
...
function target_policy(state, params)
    ...
    # The dual-opening gate preserved the sampled closing leg. Require the
    # target to be deeply rearward and range to open on both measured scales
    # before changing either anterior allocation or turn-rate damping.
    ...
    # Terminal joint-allocation tests on both sides of the sampled 0.10/0.65
    # anchor retained the upper loop. Instead, oppose only the established
    # late angular motion; the approach keeps its evaluated 0.04 damping.
    turn_rate_damping_gain = params.turn_rate_damping_gain +
        params.terminal_turn_rate_damping_gain * recovery_gate
    steering_request = bearing_request -
        turn_rate_damping_gain * Float64(state.turn_rate_recent)
