function target_policy_params()
    return (
        ...
        target_ahead_gate_width=0.15,
        behind_bearing_floor=0.0,
...
function target_policy(state, params)
    ...
    # Preserve the strongest finite sample while the target is ahead. When the
    # target crosses the body-frame beam, smoothly release only the bearing
    # bias and retain measured turn-rate damping to oppose the upper pitch.
    distance_L = max(Float64(state.distance_L), eps(Float64))
    ahead_fraction = -Float64(state.target_body_L[1]) / distance_L
    ahead_gate = 0.5 * (1 + tanh(
        ahead_fraction /
        max(params.target_ahead_gate_width, eps(params.target_ahead_gate_width)),
    ))
    bearing_authority = params.behind_bearing_floor +
        (1 - params.behind_bearing_floor) * ahead_gate
    steering_request =
        params.steering_gain * bearing_authority * Float64(state.bearing) -
