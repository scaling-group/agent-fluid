function target_policy_params()
    return (
        ...
        rearward_overspeed_damping_boost=12.0,
...
function target_policy(state, params)
    ...
    # Preserve the best finite sample while the target is ahead. Its compact
    # bearing aliases ahead and behind, so transition only near the body-frame
    # beam and apply a modest opposite bias once the target is truly rearward.
    ...
    # Keep the evaluated guard unchanged on the useful target-ahead leg, then
    # strengthen only its overspeed damping after the target passes abeam. All
    # sampled rearward steering variants retained the same nose-up exit while
    # joint one approached the hard speed cap; joint two stayed below this
    # threshold and is therefore unaffected unless it develops its own excess.
    ...
    speed_guard_damping = params.overspeed_damping +
        params.rearward_overspeed_damping_boost * rearward_gate
    raw_a1 -= params.overangle_stiffness * angle_excess1 * sign(q1) +
        speed_guard_damping * speed_excess1 * sign(qd1)
    raw_a2 -= params.overangle_stiffness * angle_excess2 * sign(q2) +
        speed_guard_damping * speed_excess2 * sign(qd2)
