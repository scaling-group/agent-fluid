...
function target_policy(state, params)
    ...
    # Preserve the finite anchor while the target is ahead, but remove the
    # compact bearing's fore/aft alias after the fish turns past abeam. The
    # two-argument angle equals state.bearing whenever the target is at least
    # 0.25L ahead, then continues to encode a target behind the fish instead of
    # withdrawing target authority and leaving the upward heading unchecked.
    forward_target_L = -Float64(state.target_body_L[1])
    lateral_target_L = Float64(state.target_body_L[2])
    pursuit_bearing = atan(lateral_target_L, forward_target_L)
    steering_request =
        params.steering_gain * pursuit_bearing -
