function target_policy_params()
    return (
        ...
        approach_turn_damping_relief=0.02,
        approach_distance_L=8.0,
        approach_transition_L=0.75,
...
function target_policy(state, params)
    ...
    # Use the closer-approaching sample's turn damping in the far field, then
    # smoothly return to the strongest finite sample's 0.04 value near target.
    # This reverses the current rollout's harmful near-field damping increment.
    approach_gate = 0.5 * (1 - tanh(
        (Float64(state.distance_L) - params.approach_distance_L) /
        max(params.approach_transition_L, eps(params.approach_transition_L)),
    ))
    turn_damping = params.turn_rate_damping_gain -
        params.approach_turn_damping_relief * approach_gate
    steering_request =
