function target_policy_params()
    return (
        ...
        lateral_velocity_damping_gain=0.35,
        lateral_velocity_damping_limit=2.0 * pi / 180,
...
function target_policy(state, params)
    ...
    # Restore the strongest finite sample's static bearing and fixed turn-rate
    # terms. Add a small bounded correction from measured body-frame lateral
    # motion: the anchor's upper drift was controller-generated rather than
    # explained by its near-zero mean local crossflow.
    lateral_limit = params.lateral_velocity_damping_limit
    lateral_velocity_request = params.lateral_velocity_damping_gain *
        Float64(state.velocity_body_U[2])
    lateral_velocity_damping = lateral_limit * tanh(
        lateral_velocity_request /
        max(lateral_limit, eps(lateral_limit)),
    )
    steering_request =
