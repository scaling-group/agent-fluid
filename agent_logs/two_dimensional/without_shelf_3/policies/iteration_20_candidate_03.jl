function target_policy_params()
    return (
        ...
        terminal_steering_fraction=-0.5,
...
function target_policy(state, params)
    ...
    # Allocation, damping, and lateral-motion attenuation all preserved the
    # same terminal nose-up return. Once course loss is confirmed, smoothly
    # reverse the bounded mean curvature while retaining the evaluated closing
    # law exactly at zero gate.
    steering_fraction = 1 +
        (params.terminal_steering_fraction - 1) * recovery_gate
    terminal_steering = steering_fraction * steering
    ...
    anterior_bias = anterior_fraction * terminal_steering
    posterior_bias =
        (1 - params.anterior_steering_fraction) * terminal_steering
