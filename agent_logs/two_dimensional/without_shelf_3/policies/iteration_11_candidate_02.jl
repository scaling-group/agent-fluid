function target_policy_params()
    return (
        ...
        rearward_turn_damping_boost=0.02,
        rearward_transition_L=0.5,
...
function target_policy(state, params)
    ...
    # Preserve the only sampled rhythm and steering law that produced active
    # upstream motion while the target is forward. The scalar bearing discards
    # the sign of longitudinal target separation, so use the normalized signed
    # body-frame component only to identify the visibly target-rearward end of
    # the failed upper turn.
    forward_target_L = -Float64(state.target_body_L[1])
    rearward_distance_L = max(-forward_target_L, 0.0)
    rearward_gate = tanh((
        rearward_distance_L /
        max(params.rearward_transition_L, eps(params.rearward_transition_L))
    )^2)
    turn_damping = params.turn_rate_damping_gain +
        params.rearward_turn_damping_boost * rearward_gate
    steering_request =
        params.steering_gain * state.bearing -
        turn_damping * state.turn_rate_recent
