function target_policy_params()
    return (
        ...
        terminal_posterior_steering_fraction=0.0,
...
function target_policy(state, params)
    ...
    # Keep recovery inactive through the useful closing leg. Require the
    # target to be deeply rearward and range to be opening both now and over
    # the observation window before changing terminal steering allocation.
    ...
    # Full anterior unloading was reproducibly weaker than the sampled 0.10
    # residual and left the upper return unchanged. Restore that local anchor,
    # then use the same strict gate to test whether the still-active posterior
    # mean curvature sustains the return. The posterior traveling-gait terms
    # remain active even when its mean steering bias is unloaded.
    ...
    nominal_posterior_fraction = 1 - params.anterior_steering_fraction
    posterior_fraction = nominal_posterior_fraction +
        (params.terminal_posterior_steering_fraction -
        nominal_posterior_fraction) * recovery_gate
    anterior_bias = anterior_fraction * steering
    posterior_bias = posterior_fraction * steering
