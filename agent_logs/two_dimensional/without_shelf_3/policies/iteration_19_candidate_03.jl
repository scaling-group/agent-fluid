function target_policy_params()
    return (
        ...
        terminal_anterior_rate_damping=1.0,
...
function target_policy(state, params)
    ...
    # Unload only the near-cap anterior steering bias during recovery. Keep the
    # posterior allocation fixed so the demonstrated traveling bend and its
    # measured joint-speed headroom are not globally attenuated.
    ...
    # Brake the remaining near-cap anterior rhythm only after persistent range
    # opening is established. The strict selector leaves the sampled closing
    # leg unchanged, unlike the broader inherited speed-guard test.
    raw_a1 -= recovery_gate * params.terminal_anterior_rate_damping * qd1
