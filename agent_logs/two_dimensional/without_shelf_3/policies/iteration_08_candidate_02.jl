...
function target_policy(state, params)
    ...
    # Bisect the observed far-field course boundary: 0.04 retained upstream
    # propulsion before an upper turn, whereas 0.045 lost most of that travel.
    steering_request =
    ...
    # Keep the angle-only Van der Pol gait that demonstrated active upstream
    # motion while shifting its equilibrium by bounded target-relative bearing.
    centered_q1 = q1 - anterior_bias
    vdp_drive = params.oscillator_mu * (1 - (centered_q1 / amp)^2) * qd1
    raw_a1 = vdp_drive - omega^2 * centered_q1

    # Retain the demonstrated posterior traveling-bend lag and damping.
    phase_lag_target = posterior_bias - centered_q1 -
        params.tail_lag_gain * qd1 / max(omega, eps(omega))
    raw_a2 = omega^2 * (phase_lag_target - q2) -
        2 * params.tail_damping * omega * qd2

    # Local guards oppose the high-angle/high-speed path that previously led
    # to actuator-cap contact and folded-body instability.
