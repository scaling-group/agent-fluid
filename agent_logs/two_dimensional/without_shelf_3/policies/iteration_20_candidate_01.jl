function target_policy_params()
    return (
        ...
        terminal_steering_multiplier=-1.0,
...
function target_policy(state, params)
    ...
    # The strict selector demonstrably preserves the sampled closing leg.
    # Activate a structurally different command only after the target is
    # deeply rearward and both normalized range rates confirm opening.
    ...
    # Allocation and lateral-motion corrections retained the same nose-up
    # exit. Smoothly reverse the complete bounded request under confirmed
    # course loss, while leaving it algebraically unchanged at zero gate.
    steering_multiplier = 1 +
        (params.terminal_steering_multiplier - 1) * recovery_gate
    steering = params.steering_limit * tanh(
        steering_multiplier * steering_request /
