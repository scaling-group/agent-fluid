...
function target_policy(state, params)
    ...
    # Preserve the strongest finite sample's fixed turn damping and 12-degree
    # ceiling. Only the unsaturated bearing slope is raised: large far-field
    # errors keep the demonstrated bounded curvature, while small course errors
    # receive earlier target-relative correction during the late approach.
    steering_request =
    ...
    # Keep the angle-only Van der Pol drive around the moving equilibrium; the
    ...
    # cap contact and body folding in the progress-producing lineage.
    ...
    # Smoothly keep every policy command below the evaluator's hard envelope.
