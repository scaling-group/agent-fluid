...
function target_policy(state, params)
    ...
    # The task's scalar bearing folds fore/aft geometry together. Recover the
    # full signed body-frame target direction while preserving the anchor's
    # small-error response on the sampled far-forward approach.
    forward_target_L = -Float64(state.target_body_L[1])
    lateral_target_L = Float64(state.target_body_L[2])
    course_error = atan(lateral_target_L, forward_target_L)
    steering_request =
        params.steering_gain * course_error -
