...
function target_policy(state, params)
    ...
    # Release only part of the distributed mean curvature after the developed
    # gait is visibly rotating toward the target and its bearing is shrinking.
    # This one-sided response gate cannot add steering or attenuate the rhythm.
    ...
    # Distinguish coherent target closure from lateral target-vector motion.
    # Low confidence restores the sampled ungated residual; high confidence
    # permits yielding only when posterior state also shows gait-scale pressure.
