function target_policy_params()
    return (
        ...
        max_targetward_turn_relief=0.50,
        targetward_turn_rate_fraction=0.08,
    )
end
...
function target_policy(state, params)
    ...
    # Positive body-frame bearing requests positive bend, but this fish swims
    # along its negative body-x axis; the corresponding targetward heading
    # response therefore has the opposite sign. Preserve the full transient
    # boost until that response appears, then relieve only the extra half-cycle
    # gain. Mean curvature and the base traveling wave are never attenuated.
    heading_rate = hasproperty(state, :heading_rate) ?
        Float64(state.heading_rate) : 0.0
    targetward_turn_rate = max(0.0, -turn_activation * heading_rate)
    targetward_rate_scale = max(
        params.targetward_turn_rate_fraction * omega,
        eps(Float64),
    )
    response_relief = clamp(
        params.max_targetward_turn_relief,
        0.0,
        1.0,
    ) * tanh(targetward_turn_rate / targetward_rate_scale)
    posterior_wave_gain = 1 +
        clamp(params.max_helpful_half_cycle_gain, 0.0, 0.25) *
        abs(turn_activation) * helpful_half_cycle * (1 - response_relief)
