function target_policy_params()
    return (
        ...
        bearing_rate_horizon=0.08,
        max_bearing_rate_correction=5.0 * pi / 180,
...
function target_policy(state, params)
    ...
    # A recent error-growing trend adds limited recovery authority; a correcting
    # trend releases the mean bend early. The cap prevents fast wake motion from
    # replacing the persistent body-frame route request.
    ...
    rate_correction = clamp(
        ...
        -params.max_bearing_rate_correction,
        params.max_bearing_rate_correction,
    )
    steering_error = clamp(bearing + rate_correction, -pi / 2, pi / 2)
