function target_policy_params()
    return (
        ...
        max_balanced_half_cycle_gain=0.08,
...
function target_policy(state, params)
    ...
    # Infer posterior wave phase from joint state, not time. Apply a balanced
    # asymmetry: strengthen the target-helping half-cycle and complementarily
    # weaken the opposing one. The sampled symmetric traveling wave is recovered
    # exactly at alignment, without adding a positive cycle-average wave gain.
    ...
    helpful_half_cycle_bias = tanh(
        turn_activation * posterior_wave / half_cycle_width,
    )
    posterior_wave_gain = 1 +
        clamp(params.max_balanced_half_cycle_gain, 0.0, 0.25) *
        abs(turn_activation) * helpful_half_cycle_bias
