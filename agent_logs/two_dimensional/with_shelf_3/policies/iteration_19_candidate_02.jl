function target_policy_params()
    return (
        ...
        assisting_target_force_scale_L=1.0,
    )
end
...
function target_policy(state, params)
    ...
    # A measured fluid wrench is assistance only through its task-frame effect:
    # yaw moment must reinforce the persistent target turn, while translational
    # force must point along the instantaneous body-frame target vector. Let
    # either normalized cue yield only the optional posterior residual during
    # coherent closure. Opposing loads leave the evaluated supervisor unchanged.
    ...
    target_distance_L = hypot(
        Float64(target_body_L[1]),
        Float64(target_body_L[2]),
    )
    targetward_force_L = target_distance_L <= eps(Float64) ? 0.0 : (
        Float64(force_body_L[1]) * Float64(target_body_L[1]) +
        Float64(force_body_L[2]) * Float64(target_body_L[2])
    ) / target_distance_L
    target_force_scale = max(
        params.assisting_target_force_scale_L,
        eps(Float64),
    )
    assisting_target_force = max(
        tanh(targetward_force_L / target_force_scale),
        0.0,
    )
    assisting_task_wrench = max(
        assisting_yaw_load,
        assisting_target_force,
    )
