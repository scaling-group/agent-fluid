...
function target_policy(state, params)
    ...
    # Opposing loads leave the evaluated supervisor unchanged.
    ...
    # Preserve either evidenced cue on its own, but let simultaneous positive
    # yaw and lateral support provide bounded extra confidence. Unlike an
    # opposing-load veto, this cooperative union is monotone: an adverse or
    # absent component cannot cancel useful assistance from the other channel.
    assisting_planar_wrench = 1 -
        (1 - assisting_yaw_load) * (1 - assisting_lateral_load)
