...
function target_policy(state, params)
    ...
    # A measured yaw load that has the same sign as the persistent target turn
    # is hydrodynamic assistance, not an error to cancel. At the observed
    # dimensionless load scale, let that assistance participate in yielding
    # only the optional posterior residual during coherent closure. Opposing
    # moments leave the evaluated supervisor unchanged.
    ...
    optional_residual_gain =
        clamp(params.max_helpful_half_cycle_gain, 0.0, 0.25) *
        abs(turn_activation) * helpful_half_cycle * posterior_headroom

    # Both the evaluated moment cue and actuator-state pressure must agree
    # before touching the base traveling wave. This small reference governor
    # preserves phase while relieving only a clipping-bound, target-assisted
    # posterior component; redirection and unsupported propulsion stay at unit
    # gain even when either cue appears alone.
    assisted_wave_yield =
        clamp(params.max_assisted_wave_yield, 0.0, 0.10) *
        progress_confidence * assisting_yaw_load * actuator_pressure
    posterior_wave_gain = 1 - assisted_wave_yield + optional_residual_gain
