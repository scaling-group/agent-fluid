...
function target_policy(state, params)
    ...
    # Recognize an established, correct-sign redirect from body-frame history.
    # This confidence cannot add steering and falls to zero when observations
    # are absent, bearing is not shrinking, or heading is not responding.
    ...
    # Give verified redirect-and-release behavior priority over posterior
    # residual yielding. The complementary gate below prevents both optional
    # withdrawals from reaching full authority together.
    response_release =
    ...
    posterior_headroom = 1 -
        (1 - response_priority) * progress_confidence * posterior_pressure
