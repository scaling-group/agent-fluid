function target_policy_params()
    return (
        ...
        oscillator_mu=0.35,
        tail_lag_gain=0.8,
        max_turn_phase_shift=8.0 * pi / 180,
        tail_damping=0.65,
...
function target_policy(state, params)
    ...
    # Infer posterior wave phase from joint state, not time. During a persistent
    # turn, rotate the parent's angle/normalized-velocity coefficient pair by a
    # small bearing-conditioned phase shift. A true rotation conserves the
    # nominal harmonic envelope instead of buying phase with more amplitude,
    # and zero bearing recovers the sampled posterior wave exactly.
    turn_phase_shift = clamp(params.max_turn_phase_shift, 0.0, pi / 4) *
        abs(turn_activation)
    phase_cos = cos(turn_phase_shift)
    phase_sin = sin(turn_phase_shift)
    posterior_angle_coefficient =
        phase_cos - params.tail_lag_gain * phase_sin
    posterior_velocity_coefficient =
        phase_sin + params.tail_lag_gain * phase_cos
    posterior_wave =
        -posterior_angle_coefficient * q1_wave -
        posterior_velocity_coefficient * qd1 / max(omega, eps(omega))

    # Smoothly strengthen only the target-helping half-cycle while bearing
    # error is present; the sampled symmetric traveling wave is recovered
