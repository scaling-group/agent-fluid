...
function target_policy(state, params)
    ...
    # Release only bounded mean curvature after the developed gait has both a
    # correct-sign heading response and shrinking body-frame bearing. This is
    # response scheduling, not a bearing-rate path into the oscillator.
    ...
    max_response_release = clamp(
        params.max_response_curvature_release,
        0.0,
        0.35,
    )
    response_release =
        max_response_release * activity_gate * verified_response
    ...
    # The sampled additive composition regressed when curvature release and
    # posterior-residual withdrawal overlapped. Arbitrate the two optional
    # withdrawals: verified curvature release continuously restores the
    # residual, while zero release recovers the evaluated headroom parent.
    response_release_fraction = clamp(
        response_release / max(max_response_release, eps(Float64)),
        0.0,
        1.0,
    )
    residual_yield =
        (1 - response_release_fraction) *
        progress_confidence * posterior_pressure
    posterior_headroom = 1 - residual_yield
