...
function target_policy(state, params)
    ...
    response_occupancy = activity_gate * verified_response
    response_release =
    ...
    # Treat recent target-vector motion as useful only when it is coherent
    # closure, rather than redirection or lateral displacement in the wake.
    # Early padded history and adverse progress recover the evaluated residual.
    ...
    # Form a dimensionless request to yield only the optional posterior
    # asymmetry when observed posterior state/action already reinforce it.
    # The target curvature and unit-gain lagged traveling wave stay active.
    ...
    # Share one unit of modulation authority. A verified response-release call
    # consumes the same budget that could otherwise yield the posterior
    # residual, preventing the independently negative full-strength stack.
    headroom_request = progress_confidence * posterior_pressure
    remaining_yield_authority = 1 - response_occupancy
    posterior_headroom = 1 - headroom_request * remaining_yield_authority
