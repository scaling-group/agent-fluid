...
function target_policy(state, params)
    ...
    # A lateral force or yaw moment with the same sign as the persistent target
    # turn is hydrodynamic assistance, not an error to cancel. At their observed
    # normalized scales, let either component of that planar wrench participate
    # in yielding only the optional posterior residual during coherent closure.
    # Preserve their signs for the conservative adverse-load consensus below.
    ...
    signed_yaw_load = tanh(
        turn_activation * moment_z_L2 / moment_scale,
    )
    force_body_L = hasproperty(state, :force_body_L) ?
        state.force_body_L : (0.0, 0.0)
    lateral_force_L = Float64(force_body_L[2])
    lateral_force_scale = max(
        params.assisting_lateral_force_scale_L,
        eps(Float64),
    )
    signed_lateral_load = tanh(
        turn_activation * lateral_force_L / lateral_force_scale,
    )

    # Either target-aligned load is useful assistance, but treat opposition as
    # actionable only when both channels agree. This preserves the sampled
    # force-or-moment yield response while preventing one adverse component
    # from cancelling simultaneous help from the other component.
    signed_planar_support = max(signed_yaw_load, signed_lateral_load)
    assisting_planar_wrench = max(signed_planar_support, 0.0)
    opposing_planar_wrench = max(-signed_planar_support, 0.0)
    ...
    # Preserve helpful fluid-load yielding, while unanimous opposing planar
    # load can veto only actuator-state withdrawal. The veto restores at most
    # the already evaluated ungated residual and cannot touch mean curvature
    # or the unit-gain traveling wave.
    posterior_pressure = max(
        actuator_pressure,
        assisting_planar_wrench,
    ) * (1 - opposing_planar_wrench)
