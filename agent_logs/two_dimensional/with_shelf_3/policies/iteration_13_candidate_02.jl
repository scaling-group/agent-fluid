...
function target_policy(state, params)
    ...
    # curvature. This one-sided gate cannot add steering or attenuate either
    # component of the propulsive traveling wave.
    ...
    # Treat recent body-frame target-vector motion as useful only when it is
    # mostly coherent closure. Padded or adverse history therefore restores
    # the evaluated residual instead of weakening propulsion during redirect.
    ...
    # Yield only the optional posterior asymmetry when posterior state/action
    # already reinforce the requested wave. The target curvature, unit-gain
    # posterior wave, and lagged traveling-bend scaffold remain active.
