function target_policy_params()
    return (
        ...
        planar_assistance_concurrence_gain=1.0,
    )
end
...
function target_policy(state, params)
    ...
    # moment assists rotation; a target-signed lateral force assists translation
    # toward the route. At their observed normalized scales, either may yield
    # only the optional posterior residual during coherent closure. Opposing
    # loads leave the evaluated progress supervisor unchanged.
    ...
    assisting_lateral_load = max(
        tanh(turn_activation * lateral_force_L / lateral_force_scale),
        0.0,
    )
    primary_assistance = max(
        assisting_yaw_load,
        assisting_lateral_load,
    )
    concurrent_assistance = min(
        assisting_yaw_load,
        assisting_lateral_load,
    )

    # Preserve either evaluated assistance channel exactly when it acts alone.
    # When target-signed lateral force and yaw moment agree, their bounded
    # complement product supplies extra confidence that optional active turn
    # effort is redundant; it adds no authority and cannot exceed unit yield.
    concurrence_gain = clamp(
        params.planar_assistance_concurrence_gain,
        0.0,
        1.0,
    )
    hydrodynamic_assistance = clamp(
        primary_assistance +
        concurrence_gain * concurrent_assistance * (1 - primary_assistance),
        0.0,
        1.0,
    )
