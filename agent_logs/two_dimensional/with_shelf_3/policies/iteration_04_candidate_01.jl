...
function target_policy(state, params)
    ...
    # The low-pass route estimate can keep requesting curvature after the
    # observed target bearing is already converging. Add a bounded derivative
    # correction from the same target-relative history: negative trend releases
    # the turn, while positive trend restores authority when error is growing.
    bearing_window_rate = hasproperty(state, :bearing_window_rate) ?
        Float64(state.bearing_window_rate) : 0.0
    trend_limit = max(params.bearing_trend_limit, eps(params.bearing_trend_limit))
    bearing_trend = trend_limit * tanh(
        bearing_window_rate * params.control_period / trend_limit,
    )
    route_bearing = clamp(
        persistent_bearing + params.route_trend_gain * bearing_trend,
        -pi / 2,
        pi / 2,
    )

    # Apply the already-evidenced bounded total-curvature budget to the filtered
    # route signal; propulsion and the two-joint split remain unchanged.
