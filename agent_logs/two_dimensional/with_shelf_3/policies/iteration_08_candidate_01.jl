function target_policy_params()
    return (
        ...
        max_redirect_half_cycle_gain=0.04,
        redirect_release_scale=0.50,
    )
end
...
function target_policy(state, params)
    ...
    # Admit a small burst only until recent body-frame target geometry shows
    # the requested turn taking effect. The response term can remove this
    # optional boost but cannot weaken the evaluated eight-percent asymmetry,
    # keeping a bearing derivative out of the oscillator-center path.
    bearing_window_rate = hasproperty(state, :bearing_window_rate) ?
        Float64(state.bearing_window_rate) : 0.0
    normalized_alignment_response = max(
        0.0,
        -turn_activation * bearing_window_rate * params.control_period /
        max(params.bearing_scale, eps(Float64)),
    )
    redirect_release = exp(
        -normalized_alignment_response /
        max(params.redirect_release_scale, eps(Float64)),
    )
    redirect_gain =
        clamp(params.max_redirect_half_cycle_gain, 0.0, 0.12) *
        abs(turn_activation) * redirect_release
    posterior_wave_gain = 1 +
        (
            clamp(params.max_helpful_half_cycle_gain, 0.0, 0.25) *
            abs(turn_activation) + redirect_gain
        ) * helpful_half_cycle
