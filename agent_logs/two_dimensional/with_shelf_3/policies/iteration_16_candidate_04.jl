...
function target_policy(state, params)
    ...
    # A redirect is useful before it becomes mostly translational closure.
    # Admit that state only after the gait is developed. Use history-window
    # target geometry instead of instantaneous yaw, which is contaminated by
    # the beat-scale body oscillation. This signal supervises the same optional
    # posterior residual as closure confidence; it does not release mean
    # curvature or weaken the unit-gain base wave.
    gait_activity = hypot(
        q1_wave,
    ...
    activity_confidence = activity_z^2 * (3 - 2 * activity_z)

    target_window_rate = hasproperty(state, :target_body_window_rate_L) ?
        state.target_body_window_rate_L : (0.0, 0.0)
    signed_window_closing_speed = hasproperty(state, :window_closing_speed_L) ?
        Float64(state.window_closing_speed_L) : 0.0
    window_closing_speed = max(signed_window_closing_speed, 0.0)
    ...
    radial_motion_weight = clamp(
        0.5 * (
            1 + signed_window_closing_speed /
            max(target_window_rate_norm, eps(Float64))
        ),
        0.0,
        1.0,
    )
    response_confidence = activity_confidence *
        shrinking_bearing_response * radial_motion_weight
    ...
    posterior_headroom = 1 - residual_yield_confidence * posterior_pressure
