...
function target_policy(state, params)
    ...
    # only the target-helping half-cycle while bearing error is present. Large
    # normalized yaw load removes only this incremental asymmetry; it cannot
    # cancel the sampled symmetric traveling wave or the mean-curvature turn.
    ...
    moment_scale = max(params.yaw_moment_soft_scale, eps(Float64))
    moment_ratio = isfinite(yaw_moment) ?
        clamp(abs(yaw_moment) / moment_scale, 0.0, 20.0) : 20.0
    yaw_load_gate = inv(1 + moment_ratio^2)
    posterior_wave_gain = 1 +
        clamp(params.max_helpful_half_cycle_gain, 0.0, 0.25) *
        abs(turn_activation) * helpful_half_cycle * yaw_load_gate
