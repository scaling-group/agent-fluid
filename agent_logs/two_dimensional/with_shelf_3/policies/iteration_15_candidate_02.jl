...
function target_policy(state, params)
    ...
    # Admit that state only after the gait is developed and two independent
    # body-frame observations agree: heading turns toward the persistent
    # target bearing while the magnitude of that bearing shrinks. This signal
    # supervises the same optional posterior residual as closure confidence;
    # it does not release mean curvature or weaken the unit-gain base wave.
    gait_activity = hypot(
        q1_wave,
    ...
    response_confidence = activity_confidence * min(
