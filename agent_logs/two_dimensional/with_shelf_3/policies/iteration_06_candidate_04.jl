function target_policy_params()
    return (
        ...
        half_cycle_shift_modulation=0.25,
...
function target_policy(state, params)
    ...
    base_posterior_shift = clamp(params.max_posterior_bias_shift, 0.0, 1.0) * tanh(
    ...
    # Preserve the evaluated cycle-average allocation while moving a modest
    # share of it toward the posterior joint on the turn-congruent half-cycle.
    # The reference displacement uses the parent center, avoiding an implicit
    # solve after the phase-dependent center is applied. As bearing vanishes,
    # so does the scheduled shift and this gate becomes behaviorally inactive.
    parent_anterior_fraction = clamp(
        params.anterior_bias_fraction - base_posterior_shift,
        0.0,
        1.0,
    )
    q1_wave_reference = q1 - parent_anterior_fraction * turn_curvature
    half_cycle_scale = max(
        clamp(params.half_cycle_transition_fraction, 0.0, 1.0) * amp,
        eps(amp),
    )
    wave_side = tanh(q1_wave_reference / half_cycle_scale)
    turn_direction = sign(turn_curvature)
    shift_modulation = clamp(params.half_cycle_shift_modulation, 0.0, 1.0)
    half_cycle_multiplier = 1 + shift_modulation * turn_direction * wave_side
    posterior_shift = clamp(
        base_posterior_shift * half_cycle_multiplier,
        0.0,
        1.0,
    )
