...
function target_policy(state, params)
    ...
    # only the optional posterior residual during coherent closure. Preserve
    # the sign as well so opposing load can veto withdrawal below.
    ...
    assisting_yaw_load = max(signed_yaw_load, 0.0)
    opposing_yaw_load = max(-signed_yaw_load, 0.0)
    ...
    # Retain the evaluated yielding response to target-aligned yaw load, but
    # do not also withdraw the optional residual when measured yaw opposes the
    # requested turn. The opposing-load veto can only restore the already
    # evaluated ungated residual; it cannot add authority or touch the mean
    # curvature and unit-gain traveling wave.
