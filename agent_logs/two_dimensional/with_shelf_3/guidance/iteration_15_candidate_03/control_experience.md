...
- Do not assume that separately positive steering-withdrawal gates compose in
  this coupled two-joint gait. The assigned parent stacked response-confirmed
  mean-curvature release onto trajectory-supervised posterior headroom; two
  code-equivalent samples then captured at `32.4555`, slower than the
  headroom-only (`32.3400`) and response-release-only (`32.1365`) branches,
  and worsened mean distance to `1.64548L` from `1.63773/1.63696L`. It also
  exceeded both components in force/moment RMS (`67.83/914.09` versus
  `65.12/888.56` and `67.22/907.46`) and posterior excursion (`0.58180` versus
  about `0.5753 rad`) despite the same direct visible route and nearly equal
  relative crossflow. When two feedback paths alter the bend on different
  nominal layers, require a combined ablation before calling them compatible;
  otherwise retain the single branch with the preferred navigation/load
  trade. Revisit composition only with activation-overlap histories or a
  held-out wake that shows complementary regimes, and reject it again if it
  raises excursion or loads without earlier arrival or better distance.
- Treat byte-identical fixed-snapshot replays as determinism checks, not new
  controller or wake-robustness evidence. Three independently sampled copies
  of progress-supervised posterior headroom reproduced the same policy and
  keyframe hashes, `32.3400` capture, `1.63773L` mean distance, score
  `0.234663`, and `65.12/888.56` force/moment RMS; the inherited stacked-gate
  pair likewise reproduced its dominated outcome exactly. Once equality is
  established, additional copies should not be counted as separate support or
  used to tune another scalar. Change one observable-to-actuator mechanism or
  test a genuinely different wake phase, and falsify the new mechanism against
  the reproducible route, capture, distance, load, and saturation baselines.
