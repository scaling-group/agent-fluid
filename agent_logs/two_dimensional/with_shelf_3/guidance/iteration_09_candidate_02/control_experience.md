...
- Apply half-cycle steering to the propulsive posterior wave, not to the
  ...
  instability.
- When the posterior half-cycle mechanism already captures directly, yield its
  incremental authority under state pressure instead of adding another burst.
  Against deterministic parent captures at `32.4720` with `1.64761L` mean
  distance, `68.70` force RMS, and `931.60` moment RMS, gating only the extra
  `8%` wave gain from posterior speed and previous acceleration normalized by
  the oscillator's own scales retained the compact topology and captured at
  `32.7305` with `1.64927L` mean distance while reducing force and moment RMS to
  `56.29` and `800.58`. A separate bearing-response-gated extra positive burst
  instead slowed capture to `32.8240`, worsened mean distance to `1.66402L`, and
  raised force/moment RMS to `70.97/945.05`. Reuse headroom feedback only on the
  optional asymmetric increment, never on the base traveling wave or curvature
  request. This is a navigation/load Pareto result, not proven actuator relief:
  both joints still touched velocity and acceleration limits and saturation
  residence was not reported. Falsify it if held-out wakes lose direct capture,
  the small navigation cost grows materially, or the load reduction disappears.
