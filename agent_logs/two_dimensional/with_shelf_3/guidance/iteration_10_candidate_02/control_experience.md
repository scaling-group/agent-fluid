...
- Three code-equivalent sampled realizations of the ungated posterior half-
  cycle controller returned exactly the same `32.472` capture, `1.64761L` mean
  distance, and `68.70/931.60` force/moment RMS. The inherited posterior state-
  headroom gate then reproduced exactly across two evaluations: it preserved
  the same direct diagonal capture topology at `32.7305` and `1.64927L` mean
  distance while lowering force/moment RMS by `18.1%/14.1%` to
  `56.29/800.58`; posterior peak angle also fell from `0.5834` to `0.5684` rad.
  This supports gating only the optional target-helping posterior asymmetry
  when observed posterior speed or prior applied acceleration reinforces it
  near oscillator-owned gait scales; never gate the base traveling wave or
  target-signed mean curvature. Both variants still touch joint velocity and
  acceleration limits, so claim lower aggregate loads, not shorter saturation
  residence, and treat exact repeats only as fixed-snapshot reproducibility.
  Retain the gate while direct capture survives and the roughly `0.80%` arrival
  trade remains small relative to the load reduction; falsify it if a held-out
  wake changes the route, loses capture, or fails to retain meaningful load
  relief. In contrast, the sampled bearing-response extra burst arrived later
  at `32.824` and increased loads to `70.97/945.05`; do not add target-bearing
  trend merely to recover posterior authority.
