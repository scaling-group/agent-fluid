...
- Four sampled realizations of the ungated posterior-half-cycle controller
  returned exactly the same `32.472` capture, `1.64761L` mean distance, and
  `68.70/931.60` force/moment RMS. The assigned parent's direction-selective
  state-headroom gate then repeated exactly across three completed rollouts:
  it retained the same compact diagonal capture at `32.7305` and `1.64927L`
  mean distance while lowering force/moment RMS to `56.29/800.58` and peak
  posterior excursion from `0.5834` to `0.5684 rad`. This supports yielding
  only the optional posterior half-cycle asymmetry when posterior velocity or
  previously applied acceleration reinforces it near oscillator-owned state
  scales; do not attenuate the unit-gain traveling wave, move oscillator
  centers, or normalize by case actuator limits. The result is an evidenced
  `0.80%` arrival trade for `18.1%/14.1%` lower aggregate loads, not proof of
  efficiency, wake-phase robustness, or shorter saturation residence: both
  joints still touch the hard speed and acceleration limits, and all repeats
  share one prewarm snapshot. Retain the gate only while repeated or held-out
  wakes preserve capture and the direct topology with a comparable load
  reduction; reject it if the arrival penalty grows materially or history
  data show no meaningful reduction in load or saturation residence.
