...
- Once the filtered distributed-curvature plus `8%` posterior-half-cycle
  ...
  already captures. Directly stacking the two withdrawals has now been tested
  in two code-equivalent fixed-snapshot rollouts: capture remained, but arrival
  regressed to `32.4555`, mean distance to `1.64548L`, score to `0.226804`, and
  force/moment RMS to `67.83/914.09`. That is worse than the progress-headroom
  parent at `32.340/1.63773L/0.234663/65.12/888.56` and loses the response-
  release branch's `32.136` arrival without improving its `67.22/907.46`
  loads. Treat optional authority withdrawals as competing control allocations,
  not independently additive improvements: do not stack them at full effect.
  If composition is revisited, make measured activation of one continuously
  restore the authority withdrawn by the other while preserving the target-
  signed centers and unit-gain traveling wave. Falsify that arbitration if it
  loses direct capture, arrives later than `32.340`, exceeds `1.63773L` mean
  distance, or exceeds `65.12/888.56` loads without a compensating navigation
  gain.
