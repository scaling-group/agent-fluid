...
- At that same single residual locus, extend hydrodynamic yielding only with a
  narrow, monotone cue set. Adding target-signed lateral force beside yaw
  moment was reproduced by three functionally identical samples and improved
  the yaw-only branch from `31.5645` to `30.4865` arrival, `1.60525L` to
  `1.56766L` mean distance, `0.24105` to `0.23534` relative-crossflow RMS,
  and `66.32/887.63` to `65.80/872.96` force/moment RMS while preserving the
  compact self-propelled diagonal. Broadening the same gate with a direct
  targetward-force projection instead regressed slightly to `30.5580`,
  `1.56958L`, and `69.18/902.43`; allowing jointly adverse lateral/yaw signs
  to veto actuator-state withdrawal regressed more clearly to `31.3335` and
  `1.59773L`, despite lower `62.68/845.71` loads. Preserve positive yaw-or-
  lateral assistance without letting one adverse instantaneous component
  cancel the other, and do not assume that additional force projections are
  independent evidence. This is fixed-snapshot evidence, not wake-phase
  robustness; falsify it if a repeat or held-out wake loses capture or the
  direct route, or if narrower cue fusion ceases to dominate on both
  navigation and loads.
