...
- Closure-conditioned yielding of only the optional posterior residual moved
  ...
  integral.
- Once coherent-closure gating is established, a target-signed body-yaw moment
  can distinguish helpful hydrodynamic assistance from actuator pressure more
  usefully than another route-response condition. Adding the bounded assisting
  moment only at the optional posterior-residual gate preserved the same direct
  traveling-wave capture and improved release time from `32.3400-32.3180` to
  `31.5645`, mean distance from `1.63773-1.63741L` to `1.60525L`, and mean
  command energy from `1423.42-1423.00` to `1420.86`. Relative-crossflow RMS
  remained similar (`0.24105` versus `0.24372-0.24339`), force/moment RMS were
  `66.32/887.63`, and both joints still reached velocity and acceleration
  limits, so this is evidence for yielding redundant optional effort—not wake
  avoidance, desaturation, or efficiency. Reuse only a soft body-frame
  load/turn agreement under coherent closure; opposing load must recover the
  evaluated controller, and mean curvature plus the unit-gain traveling wave
  must remain untouched. Falsify on a held-out wake if the direct capture,
  arrival and distance-integral advantage, or propulsion topology disappears,
  or if load and saturation residence rise without navigation benefit.
