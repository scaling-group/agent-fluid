...
- Bearing-magnitude scheduling of the curvature allocation is a useful
  wave-shape mechanism beyond a constant posterior-weighted split. Against
  three equation-equivalent filtered `40/60` replays (`35.6895` capture,
  `1.76192L` mean distance), shifting at most five percentage points of the
  unchanged total-curvature budget posteriorly during large bearing error
  preserves the same compact diagonal capture topology while reaching at
  `35.0625` and lowering mean distance to `1.73388L`. It also lowers relative
  crossflow, force, and moment RMS from `0.24775/59.28/821.23` to
  `0.24674/56.57/793.76`, even though posterior peak excursion rises from
  `0.575` to `0.583 rad` and mean command energy rises slightly from `1425.38`
  to `1431.01`; both joint-rate and acceleration envelopes remain active.
  Reuse the invariant as bounded, target-relative redistribution of an already
  successful curvature budget, not as evidence for more total curvature or
  scalar-only posterior gain. Falsify it if changed wake phase or target
  geometry loses capture, if scheduling weakens the traveling wave as the
  inherited route-trend failure did, or if saturation/load rises without a
  navigation gain.
