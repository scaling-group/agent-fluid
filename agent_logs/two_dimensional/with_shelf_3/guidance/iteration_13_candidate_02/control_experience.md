...
- Four current sampled realizations of the ungated posterior-half-cycle
  controller return exactly the same `32.472` capture, `1.64761L` mean
  distance, `0.224538` score, and `68.70/931.60` force/moment RMS. Four
  consecutive assigned-parent evaluations of the direction-selective
  posterior-headroom gate likewise repeat exactly at `32.7305`, `1.64927L`,
  `0.223211`, and `56.29/800.58`. This establishes a deterministic
  fixed-snapshot trade: the gate reduces force and moment RMS by `18.1%/14.1%`
  for a `0.80%` arrival delay and lower navigation score; it does not establish
  wake-phase robustness, reduced saturation residence, or a new semantic
  improvement. Do not keep replaying that gate as a score optimization; retain
  it only as a lower-load branch when loads or held-out repeatability are the
  explicit objective. The complementary negatives bound the next mechanism:
  unrestricted bearing trend erased the traveling bend, while a response-
  triggered extra posterior burst arrived later and raised loads. If route-
  response feedback is tested again, let verified correct-sign heading and
  shrinking body-frame bearing withdraw only a bounded part of distributed
  mean curvature after gait activity develops; never use the response to add
  posterior authority or suppress the base wave. Reject it if initial redirect,
  direct capture, distance integral, or the compact topology worsens.
- On that same fixed prewarm, two different one-sided yielding mechanisms
  improved the ungated controller without changing its compact diagonal
  topology. Releasing at most `18%` of distributed mean curvature only after
  developed-gait, correct-sign heading and shrinking-bearing checks captured
  at `32.1365`, with `1.63696L` mean distance, score `0.234607`, and
  `67.22/907.46` force/moment RMS. Supervising actuator-state withdrawal of
  only the optional posterior half-cycle residual by coherent target closure
  captured at `32.3400`, scored `0.234663`, and lowered force/moment RMS to
  `65.12/888.56`. Relative crossflow remained within `0.6%` of the ungated
  branch, so the gains are not evidence of wake avoidance. Reuse the shared
  invariant: after the target-signed traveling gait is established, measured
  useful response may withdraw bounded optional steering authority; it must
  not add posterior authority, move oscillator centers from an unrestricted
  route derivative, or attenuate the unit-gain base wave. The fixed wake phase
  and unchanged peak-limit contacts leave robustness and saturation residence
  unresolved. A combined gate is falsified if it loses the direct capture,
  arrives later than `32.3400`, exceeds `1.63773L` mean distance, or raises
  loads above the response-release branch without a navigation benefit.
