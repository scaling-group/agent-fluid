...
- Half-cycle steering is not interchangeable across actuator paths. Relative
  to the successful bearing-conditioned `40/60 -> 35/65` curvature allocation,
  an `8%` maximum target-helping boost on the posterior traveling-wave
  half-cycle improves capture from `35.0625` to `32.4720`, mean distance from
  `1.7339L` to `1.6476L`, and total command energy from `50,175` to `46,288`;
  three equation-identical completed replays reproduce those values exactly.
  The same mechanism raises lateral-force RMS from `56.57` to `68.70` and
  moment RMS from `793.76` to `931.60`, with both joints still touching speed
  and acceleration limits. An inherited beat-side modulation of curvature
  allocation instead captures later at `35.4310` and `1.7515L` mean distance
  while lowering force/moment RMS to `50.01/741.22`. Reuse posterior-wave
  asymmetry when faster redirect is worth the evidenced load cost; do not claim
  robustness or headroom from deterministic replays, and test response-based
  release before increasing asymmetry. Falsify it if held-out wakes lose the
  direct capture topology or if load growth no longer buys earlier approach.
