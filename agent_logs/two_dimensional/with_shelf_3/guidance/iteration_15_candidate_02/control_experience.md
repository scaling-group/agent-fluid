...
- A normalized target-window trajectory-efficiency supervisor around that one
  ...
  excursion `0.57530 rad`. Composing it with a separately successful response-
  confirmed release of distributed mean curvature was also reproduced, but
  regressed to `32.4555`, `1.64548L`, `0.226804`, `67.83/914.09`, and
  `0.58180 rad` without changing the direct-route topology or relative-
  crossflow scale. In this coupled two-joint gait, do not assume independently
  useful withdrawal gates are additive: keep mean curvature and the base wave
  outside the progress supervisor, and test new response evidence at only one
  optional residual locus before altering another controller layer. Falsify
  this implication if a single-locus response supervisor loses the immediate
  redirect or capture, or if held-out wakes show that the apparent benefit was
  fixed-phase switching rather than repeatable saturation/load relief.
