...
- Once filtered bearing and a `40/60` anterior/posterior curvature split already
  produced deterministic `35.6895`-time captures, scheduling up to five
  percentage points of the same total curvature farther posterior as bearing
  magnitude grew preserved the visible diagonal traveling-wave trajectory and
  improved capture to `35.0625`, mean distance from `1.76192L` to `1.73388L`,
  and score from `0.112158` to `0.140088`. Command energy, lateral-force RMS,
  and moment RMS also fell from `50,871/59.28/821.23` to
  `50,175/56.57/793.76`, although both policies still contacted joint velocity
  and acceleration limits. This supports continuous body-frame error scheduling
  of a bounded allocation, rather than another fixed posterior scalar, when the
  aligned allocation and total curvature are already successful. Treat the
  result as common-wake-phase evidence only: falsify it if a repeat or held-out
  wake loses capture or the direct route, or if added posterior excursion and
  saturation residence rise without an arrival or distance benefit.
- A state-inferred, bearing-gated `8%` boost on only the target-helping
  posterior half-cycle improved the scheduled policy's capture from `35.0625`
  to `32.472`, mean distance from `1.73388L` to `1.64761L`, and score from
  `0.140088` to `0.224538`, while preserving the compact diagonal keyframe
  topology. Its shorter episode reduced total command energy from `50,175` to
  `46,288`, but mean command energy and power stayed essentially unchanged and
  lateral-force/moment RMS rose from `56.57/793.76` to `68.70/931.60`. A
  cycle-balanced phase modulation of the smaller curvature-allocation term
  instead remained a capture at `35.431` and reduced loads to `50.01/741.22`.
  Therefore treat joint-state half-cycle asymmetry as a validated rapid-turn
  mechanism, not as evidence for efficiency or unrestricted one-sided
  amplification. Load-oriented refinements should keep the completed maximum
  helpful impulse and direct route while balancing or releasing the opposing
  half-cycle; falsify them if capture is later than `32.472`, mean distance is
  worse than `1.64761L`, propulsion weakens, or force/moment fail to improve.
