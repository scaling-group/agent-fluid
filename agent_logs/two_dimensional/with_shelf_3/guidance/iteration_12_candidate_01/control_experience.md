...
- When that `8%` posterior half-cycle residual is already active, recent
  ...
  `70.97/945.05`, and visibly widened the late body wake. If load relief is
  worth a small navigation trade, gate only the incremental residual when
  posterior motion reinforces it, using the gait's own velocity and
  acceleration scales: that inherited variant captured at `32.7305` with
  score `0.223211` and reduced loads to `56.29/800.58`. Do not replace this
  direction-selective normalization with an undirected physical-limit gate;
  the latter delayed capture to `33.9405` and scored `0.181716`. Reuse this
  distinction only while the base traveling wave and distributed curvature
  still supply a direct capture; falsify either gate if a repeat loses that
  topology or its stated arrival/load trade. Four fixed-snapshot replays of
  the ungated residual reproduced `32.472/0.224538`, while three completed
  replays of the direction-selective gate reproduced
  `32.7305/0.223211` and `56.29/800.58` force/moment RMS without a new
  termination class or useful trajectory. Treat that exact repetition as a
  saturated load-versus-arrival branch, not evidence for another equivalent
  headroom threshold or normalization. Unless held-out wake evidence breaks
  the trade or load weighting becomes the objective, test a different bounded
  wave-shape mechanism while preserving the successful mean-curvature and
  traveling-wave scaffold.
