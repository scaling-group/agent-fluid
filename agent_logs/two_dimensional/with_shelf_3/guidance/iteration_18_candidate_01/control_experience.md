...
- A target-aligned body yaw moment can be a useful withdrawal cue even when it
  does not reduce aggregate load. Compared with the inherited normalized
  target-window supervisor (`32.340`, mean distance `1.63773L`, score
  `0.234663`, force/moment RMS `65.12/888.56`), the sampled signed-moment
  branch preserves the compact traveling-wave route while reaching at
  `31.5645`, reducing mean distance to `1.60525L`, and scoring `0.266663`.
  Relative-crossflow RMS remains comparable (`0.24105` versus `0.24372`) and
  force/moment RMS `66.32/887.63` does not improve jointly; both joints still
  touch the velocity and acceleration caps. The four sampled directories are
  byte-identical repeats, so this is evidence for faster fixed-snapshot closure,
  not independent wake robustness, Karman synchronization, efficiency, or load
  relief. Reuse signed normalized load only under coherent target closure and
  only at the posterior reference; preserve the anterior oscillator and
  distributed mean curvature. If saturation relief is pursued, require load
  assistance and reinforcing actuator pressure to agree before smoothly
  yielding a small phase-preserving part of the base posterior wave. Falsify
  this implication if capture or the direct topology is lost, arrival/mean
  distance worsens without meaningful saturation or load reduction, loads
  rise, or held-out wakes expose switching or propulsion loss.
