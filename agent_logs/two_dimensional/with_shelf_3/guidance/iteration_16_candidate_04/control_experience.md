...
- A normalized target-window trajectory-efficiency supervisor around that one
  optional posterior residual is the stronger fixed-snapshot anchor: three
  code-identical samples reproduced target capture at `32.340`, mean distance
  `1.63773L`, score `0.234663`, force/moment RMS `65.12/888.56`, and posterior
  excursion `0.57530 rad`. Adding response-confirmed withdrawal at that same
  optional locus preserved the direct topology and advanced capture only
  `0.022` time to `32.318`, mean distance to `1.63741L`, and score to
  `0.234705`; posterior excursion and moment RMS fell slightly to `0.57481 rad`
  and `881.45`, but force RMS rose to `65.52`. Treat this as a marginal,
  mixed-load fixed-snapshot result, not a semantic improvement. In contrast,
  composing the supervisor with response-confirmed release of distributed mean
  curvature regressed to `32.4555`, `1.64548L`, `0.226804`, `67.83/914.09`,
  and `0.58180 rad`. In this coupled two-joint gait, keep mean curvature and the
  base wave outside the progress supervisor and test response evidence only at
  the optional posterior residual. Offline reconstruction of the marginal
  branch found that its instantaneous-heading/history-bearing conjunction had
  mean response confidence only `0.00155`, so future response tests should use
  slow normalized target-vector geometry with an explicit recession boundary,
  not beat-scale yaw or a second withdrawal locus. Falsify this implication if
  single-locus response supervision loses immediate redirect or capture,
  worsens both navigation and loads, or if held-out wakes expose fixed-phase
  switching or propulsion loss.
