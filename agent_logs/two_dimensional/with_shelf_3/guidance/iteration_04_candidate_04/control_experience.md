...
- Do not equate a smoother target-bearing command with lower physical or
  actuator load. Gait-relative circular averaging improved capture time from
  `39.737` to `36.564` and mean distance from `1.934L` to `1.808L` against the
  otherwise identical instantaneous-bearing `45/55` policy, but raised mean
  command energy from `1279` to `1417`, RMS force from `53.7` to `66.2`, and
  RMS moment from `762` to `902`; both still touched velocity and acceleration
  limits. Preserve the filter when fast approach is valuable, but treat its
  removed beat-scale response as a separate damping hypothesis: test a small
  bounded yaw-response residual inside the same curvature budget, and reject
  it if load relief delays or loses capture.
