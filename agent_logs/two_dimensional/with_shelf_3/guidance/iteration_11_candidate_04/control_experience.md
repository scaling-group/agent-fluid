...
- Peak saturation alone does not justify withdrawing the target-helping
  posterior residual. Against the reproducible `32.472` capture (score
  `0.224538`, force/moment RMS `68.70/931.60`), an oscillator-scale,
  direction-selective velocity/previous-acceleration headroom gate kept the
  same compact route and reduced loads to `56.29/800.58`, but arrived at
  `32.7305` and scored `0.223211`; a blanket physical-envelope gate regressed
  further to `33.9405` and `0.181716` with `60.11/828.12` loads. Keep the
  directional gate only as an explicit low-load branch. For score-seeking
  descendants, avoid further headroom-threshold variants until saturation
  residence (not just maxima) identifies when residual authority is wasted;
  instead isolate a different route-response mechanism. This boundary is
  established only for the fixed prewarm snapshot and may change if held-out
  wakes or an objective with an explicit load term favor the slower trade.
