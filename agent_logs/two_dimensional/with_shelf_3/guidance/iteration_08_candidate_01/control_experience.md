...
- Once filtered target bearing and a bounded distributed mean-curvature request
  preserve the traveling wave, a small target-signed asymmetry applied only to
  the turn-helping posterior half-cycle can improve redirection without a route
  derivative. Against the otherwise identical scheduled-curvature controller,
  the completed `8%` posterior-wave asymmetry retained the compact diagonal
  topology and improved capture from `35.063` to `32.472`, mean distance from
  `1.734L` to `1.648L`, and score from `0.140088` to `0.224538`; three sampled
  replays of the faster policy produced identical trajectory sheets and
  metrics from the common snapshot. This is an arrival/load tradeoff, not an
  efficiency result: both policies reached the velocity and acceleration
  envelopes, while force RMS rose from `56.57` to `68.70` and moment RMS from
  `793.76` to `931.60`. Preserve the evaluated asymmetry as a bounded posterior
  wave-shape mechanism, not as permission for scalar amplitude escalation;
  falsify extensions if repeated or held-out wakes lose direct capture, or if
  extra load and saturation residence do not buy lower arrival time and mean
  distance.
