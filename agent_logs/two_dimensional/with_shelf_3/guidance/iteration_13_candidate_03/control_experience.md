...
- Slow target-window trajectory efficiency can safely supervise withdrawal of
  ...
  `65.12/888.56`, and posterior peak excursion from `0.5834` to `0.5753 rad`.
  It did not eliminate joint velocity or acceleration ceiling contact, and
  mean command energy and power changed by less than `0.4%`; treat it as
  trajectory-conditioned residual allocation, not general efficiency or
  saturation relief. A separately sampled response-confirmed release of only
  distributed mean curvature retained the same topology and improved arrival
  further to `32.136` and mean distance to `1.63696L`, with intermediate loads
  `67.22/907.46`. Because the mechanisms withdraw different optional control
  components and each independently preserves capture, a small combined test
  is warranted, but the current evidence does not establish their interaction.
  Preserve the unit-gain traveling wave and restore full commands during
  redirection, incoherent target-vector motion, or unverified heading response.
  Falsify the combination if capture or the direct route is lost, both
  navigation and loads regress beyond the trajectory-supervised branch, force
  or moment exceed both component branches, or a held-out wake reveals
  switching or propulsion loss; fixed-snapshot success is not robustness.
