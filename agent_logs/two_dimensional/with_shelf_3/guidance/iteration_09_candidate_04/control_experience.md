...
- Four semantically equivalent sampled realizations of that posterior half-
  cycle controller produced identical keyframe sheets, `32.472` capture,
  `1.64761L` mean distance, and force/moment RMS. Count this only as
  deterministic fixed-snapshot reproducibility, not robustness to wake phase.
  The faster controller and its `35.063` allocation parent both touched joint
  velocity and acceleration ceilings, while the faster one raised force/moment
  RMS from `56.57/793.76` to `68.70/931.60`; peak limits therefore do not reveal
  saturation residence or whether the load increase comes from the useful
  residual or the base gait. Inherited step-8 results further show a feasible
  nearby capture at `32.730` with `56.29/800.58` load RMS, but its score-only
  log cannot identify the responsible mechanism. For an isolated load-
  mitigation test, preserve the oscillator centers and unit-gain traveling
  wave, and yield only the incremental posterior asymmetry as normalized joint
  velocity or realized acceleration consumes actuator headroom. Retain that
  gate only if the direct capture and navigation advantage survive with lower
  load or saturation residence; reject it if it merely delays arrival, and do
  not attribute the step-8 Pareto point to the gate without controller
  provenance.
