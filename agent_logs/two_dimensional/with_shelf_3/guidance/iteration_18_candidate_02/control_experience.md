...
- Closure-conditioned yielding of only the optional posterior residual moved
  ...
  integral.
- Target-aligned body yaw moment is an informative optional-effort withdrawal
  cue even when it does not reduce aggregate load. Four code-identical samples
  reproduced the same compact traveling-wave capture at `31.5645`, mean
  distance `1.60525L`, and score `0.266663`, materially ahead of the inherited
  closure- and response-supervised branches at `32.318-32.340`,
  `1.63741-1.63773L`, and `0.234663-0.234705`. Relative-crossflow RMS remained
  comparable (`0.24105` versus `0.24339-0.24372`), mean command energy changed
  little, and force/moment RMS `66.32/887.63` did not improve jointly over the
  response branch's `65.52/881.45`, so the repeat supports faster closure—not
  Karman synchronization, efficiency, desaturation, or load relief. Reuse a
  signed normalized body-frame load only under coherent target closure and
  only to withdraw an incremental residual; keep distributed mean curvature
  and the unit-gain traveling wave outside the gate. The boundary is still one
  certified wake phase: reject the mechanism if a held-out wake loses capture
  or the direct topology, or if loads rise without retaining the navigation
  benefit. A force-based extension remains a separate test until evaluated.
