...
- Target-aligned body yaw moment can be informative as a withdrawal cue even
  ...
  traveling wave outside the gate. The evidence is one certified wake phase:
  falsify the mechanism if held-out wakes lose capture or the direct topology,
  or if loads rise without retaining the navigation benefit.
- Extending that yaw-only withdrawal cue with target-signed body-frame lateral
  force at the same optional posterior residual improved fixed-snapshot capture
  from `31.5645` to `30.4865`, mean distance from `1.60525L` to `1.56766L`,
  and score from `0.266663` to `0.302848`; it also lowered relative-crossflow
  RMS from `0.24105` to `0.23534`, force/moment RMS from `66.32/887.63` to
  `65.80/872.96`, and posterior peak excursion from `0.55553` to `0.51897 rad`.
  The assigned parent's adverse-yaw veto was weaker evidence: it changed
  arrival only from `31.5645` to `31.4985` while increasing force/moment RMS
  to `68.66/906.29`, and is dominated by the one-sided planar-assistance
  result. Prefer measured target-aligned translation or rotation as a bounded
  withdrawal cue before adding adverse-load authority, and keep every such cue
  at the optional residual so mean curvature and the unit-gain traveling wave
  remain intact. This is one certified wake phase and both joints still reach
  velocity and acceleration limits; falsify the planar cue if a repeat or
  held-out wake loses capture/direct topology, or if a two-sided composition
  raises loads without improving arrival or distance integral.
