...
- Treat half-cycle asymmetry as an actuator-placement mechanism, not a generic
  phase gain. Relative to the phase-invariant posterior-allocation parent, an
  `8%` bearing-gated amplification of the helpful posterior *wave* half-cycle
  improved capture from `35.0625` to `32.472` and mean distance from `1.73388L`
  to `1.64761L`, but raised lateral-force/moment RMS from `56.57/793.76` to
  `68.70/931.60`. An inherited alternative that moved the mean-curvature
  *center* allocation by half-cycle instead lowered loads to `50.01/741.22`
  while regressing to `35.431` capture and `1.75151L` mean distance. Preserve
  slow target geometry as the owner of bounded mean curvature and place any
  transient load response downstream on only the incremental posterior-wave
  asymmetry; falsify it if it cannot reduce the faster policy's loads while
  retaining capture no later than the `35.0625` phase-invariant parent. Three
  equation-identical replays of the faster policy establish deterministic
  repeatability at the shared wake snapshot, not robustness to held-out wakes.
