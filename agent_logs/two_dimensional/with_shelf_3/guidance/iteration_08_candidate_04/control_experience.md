...
- Three code-equivalent sampled realizations of that posterior half-cycle
  controller returned exactly the same `32.472` capture, `1.64761L` mean
  distance, and force/moment RMS, so preserve its route and propulsion scaffold
  but count the repetitions only as deterministic fixed-snapshot
  reproducibility, not wake-phase robustness. Because the faster controller
  and its slower allocation parent both touch the joint velocity and
  acceleration ceilings while the faster one raises force/moment RMS by
  `21.4%/17.4%`, peak limits alone cannot identify saturation residence or the
  source of the load increase. A load-mitigation test should therefore yield
  only the incremental posterior asymmetry when observed joint state consumes
  its own gait-scale headroom, never attenuate the base traveling wave or move
  the oscillator centers. Retain such a gate only if direct capture and the
  navigation advantage survive while load or saturation residence decreases;
  the current aggregate diagnostics cannot establish that outcome without a
  later rollout and history comparison.
