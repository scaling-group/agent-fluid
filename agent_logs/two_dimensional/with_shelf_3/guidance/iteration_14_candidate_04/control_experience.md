...
- Slow target-window trajectory efficiency can safely supervise withdrawal of
  an optional posterior residual in the sampled fixed wake. Relative to the
  ungated branch's `32.472` capture, `1.64761L` mean distance, and
  `68.70/931.60` force/moment RMS, the assigned parent's supervisor preserved
  the same direct traveling-bend topology while improving capture to `32.340`,
  mean distance to `1.63773L`, score from `0.224538` to `0.234663`, loads to
  `65.12/888.56`, and posterior excursion from `0.5834` to `0.5753 rad`. A
  separately sampled response-confirmed release of only distributed mean
  curvature retained that topology and improved arrival further to `32.136`
  and mean distance to `1.63696L`, with intermediate loads `67.22/907.46`.
  However, three independently materialized, code-equivalent simultaneous
  compositions reproduced the same regression: capture at `32.4555`, mean
  distance `1.64548L`, score `0.226804`, loads `67.83/914.09`, and increased
  posterior excursion `0.5818 rad`. The repeated composition remained a
  semantic success but was worse than both components in navigation and worse
  than the assigned parent in loads, so independent benefits from different
  command layers are not additive. Avoid concurrent full-confidence
  withdrawal of mean curvature and posterior residual. If their combination
  is revisited, use an observable state-dependent arbitration or test them in
  separate regimes while preserving target-signed curvature and the unit-gain
  traveling wave; falsify arbitration if it fails to beat the repeated
  composition in navigation without exceeding both component loads. These
  exact repeats establish a fixed-snapshot negative interaction, not its cause
  or held-out wake-phase generality, and neither component eliminates joint
  velocity or acceleration ceiling contact.
