...
- Once that yaw-only cue is active, target-signed lateral body force is useful
  as a complementary translation cue at the same optional posterior-residual
  locus. Three code-equivalent samples reproduced capture at `30.4865`, mean
  distance `1.56766L`, score `0.302848`, relative-crossflow RMS `0.23534`, and
  force/moment RMS `65.80/872.96`, improving the yaw-only branch's
  `31.5645/1.60525L/0.266663` while preserving its compact self-propelled
  diagonal. Keep the stronger positive target-signed lateral-force or yaw-
  moment cue and leave opposing loads inactive: a sampled unanimous-
  opposition veto also overrode independent actuator-pressure yielding and
  regressed to `31.3335/1.59773L/0.273753`, raised mean command energy from
  `1416.41` to `1420.90`, and increased peak joint excursions from
  `0.5094/0.5190` to `0.5169/0.5482 rad`; its lower `62.68/845.71` loads are a
  navigation trade, not desaturation because both branches still touched
  velocity and acceleration limits. An inherited direct target-force
  projection likewise reached slightly later at `30.5580` and raised loads to
  `69.18/902.43`, so do not broaden the cue to streamwise force without new
  temporal evidence. Reuse only the positive planar-wrench withdrawal at one
  bounded residual; keep actuator headroom, distributed mean curvature, and
  the unit-gain traveling wave independent. Falsify this lesson if a repeated
  or held-out wake loses capture or route topology, or if the navigation and
  aggregate-load advantage over yaw-only control does not survive.
