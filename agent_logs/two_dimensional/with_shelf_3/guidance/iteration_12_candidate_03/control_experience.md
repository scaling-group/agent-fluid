...
- Do not interpret small wake-response magnitudes as successful disturbance
  ...
  `0.045` RMS relative crossflow and `299` RMS moment, whereas earlier captures
  traversed the developed wake at `0.215-0.227` crossflow and `713-762` moment.
  More sharply, the ungated `32.472` capture and its actuator-headroom-gated
  `32.7305` child had nearly identical local/relative crossflow RMS
  (`0.29554/0.24511` versus `0.29506/0.24306`) even though the gate reduced
  force RMS by `18.1%` and moment RMS by `14.1%`. Thus aggregate crossflow
  magnitude explains neither capture nor that load reduction. Prefer
  gait-normalized joint-state headroom for regulating an optional steering
  residual; do not add magnitude-only flow/force cancellation or a
  response-gated extra burst without keyframes and histories showing a
  repeatable target-directed trajectory specifically lost during wake events.
  Falsify this preference if held-out event-level evidence ties a route loss
  to a signed disturbance and a bounded residual restores it without harming
  capture, upstream propulsion, or loads.
...
- Four code-equivalent sampled realizations of the ungated posterior half-cycle
  ...
  distance. Six unique completed inherited evaluations likewise reproduce a
  semantically equivalent speed/previous-acceleration headroom gate at exactly
  `32.7305` arrival and `1.64927L` mean distance. The gate yields only the
  incremental `8%` asymmetry when normalized posterior state reinforces it; it
  preserves the same direct topology while reducing lateral-force RMS from
  `68.70` to `56.29` (`18.1%`), moment RMS from `931.60` to `800.58` (`14.1%`),
  and posterior peak excursion from `0.5834` to `0.5684 rad`. Treat the ungated
  wave as the navigation benchmark and this gate as the low-load benchmark;
  another fixed-snapshot copy of either adds no robustness or mechanism
  evidence. Both still touch velocity and acceleration limits and their mean
  command energy differs by only about `0.4%`, so aggregate peaks and effort do
  not establish reduced saturation residence or identify why loads fall. A
  later residual gate should therefore isolate a more direct gait-normalized
  state such as base-wave tracking error, leave mean curvature and the unit-
  gain traveling wave untouched, and beat at least one benchmark on its stated
  trade axis. Falsify this guidance if a varied wake phase breaks the shared
  topology or load reduction, or event histories show that tracking coherence
  is unrelated to saturation residence and loads. A response-gated extra burst
  is not a substitute: it arrived at `32.824`, increased mean distance to
