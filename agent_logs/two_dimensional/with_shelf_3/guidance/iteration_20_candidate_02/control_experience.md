...
- Once target-signed yaw load was already withdrawing only the optional
  posterior residual, adding normalized target-signed lateral body force at
  that same locus produced identical outcomes from two functionally equivalent
  policies at `30.4865`, improving the yaw-only result from `31.5645` release
  time and `1.60525L` mean distance to `1.56766L`. Force/moment RMS fell from
  `66.32/887.63` to `65.80/872.96`,
  mean command energy fell from `1420.86` to `1416.41`, and anterior/posterior
  peak excursions fell from `0.5191/0.5555` to `0.5094/0.5190 rad`, although
  both joints still touched their velocity and acceleration limits. Released
  sheets preserve the compact self-propelled diagonal and late developed-wake
  crossing; this supports body-frame fluid-wrench agreement as a withdrawal
  cue, not load cancellation, desaturation, or wake avoidance. Reuse measured
  target-aligned translation and rotation only to arbitrate bounded optional
  authority at the single evidenced locus, leaving distributed mean curvature
  and the unit-gain traveling wave intact. The two exact repeats establish
  fixed-snapshot reproducibility, not changed-phase robustness; falsify the
  lesson if a held-out wake loses capture or propulsion, or if force, moment,
  switching, or saturation residence worsens without navigation benefit.
- Do not broaden that successful lateral-force/yaw assistance gate to every
  intuitively task-aligned planar load. Adding instantaneous total-force
  projection onto `target_body_L` retained capture and the compact route but
  slipped from `30.4865/1.56766L` arrival/mean distance to
  `30.5580/1.56958L`, while force/moment RMS rose from `65.80/872.96` to
  `69.18/902.43`. Conversely, using unanimous opposing lateral/yaw load to
  veto actuator-state withdrawal delayed capture to `31.3335`, raised mean
  distance to `1.59773L`, and increased anterior/posterior peak excursions
  from `0.50945/0.51897` to `0.51690/0.54820 rad`, despite reducing aggregate
  loads to `62.68/845.71`. In this direct-capture regime, retain positive
  target-signed lateral translation and yaw rotation as optional-effort yield
  cues, but do not assume streamwise target projection is helpful or adverse
  load demands restored actuation. Revisit either only if keyframes show a
  distinct propulsion or route-loss event, and require navigation benefit to
  justify any load trade.
