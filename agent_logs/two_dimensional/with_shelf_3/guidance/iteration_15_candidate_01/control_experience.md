...
- Slow normalized target-window trajectory efficiency can usefully supervise
  one optional posterior residual when the ungated and direction-selective
  headroom branches already share a compact route. Three byte-identical
  sampled supervisors captured at `32.340`, improved mean distance to
  `1.63773L` and score to `0.234663`, and held force/moment RMS to
  `65.12/888.56`: earlier than both the ungated branch (`32.472`,
  `68.70/931.60`) and the lower-load headroom branch (`32.7305`,
  `56.29/800.58`). Restore the ungated residual during redirection, lateral
  target-vector motion, stalled closure, or recession, and permit
  direction-selective yielding only during coherent closure; keep mean
  curvature and the unit-gain traveling wave outside that supervisor. Do not
  stack this residual supervisor with an independent response-confirmed
  mean-curvature release merely because each component works alone: two
  reproduced combinations were slower at `32.4555`, regressed mean distance
  to `1.64548L`, and raised loads to `67.83/914.09` without a different route
  or clearance benefit. Treat the three exact supervisor outcomes as
  fixed-snapshot reproducibility, not wake-phase robustness, and falsify the
  mechanism if a held-out wake loses capture or the direct topology, exposes
  switching or propulsion loss, or removes its navigation benefit without a
  compensating load reduction.
