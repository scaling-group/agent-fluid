...
- Treat independently useful gait-yield paths as competing control authority,
  not automatically compatible refinements. On the same prewarm snapshot, the
  response-triggered mean-curvature release captured at `32.1365` with
  `1.63696L` mean distance and `67.22/907.46` force/moment RMS, while posterior-
  residual headroom alone captured at `32.3400`, `1.63773L`, and lower
  `65.12/888.56` loads. Two independently materialized policies that stacked
  both mechanisms returned exactly the same worse `32.4555` capture,
  `1.64548L` mean distance, `0.226804` score, and `67.83/914.09` loads even
  though their keyframes retained the compact diagonal topology. All branches
  still touched the joint speed and acceleration caps, so the load difference
  is not evidence of reduced saturation residence. If these reductions are
  combined again, arbitrate them under one bounded authority budget and compare
  against both single branches; reject the combination if it is slower or more
  highly loaded than either constituent. This boundary is fixed-snapshot
  evidence and must be retested under a changed wake phase or with saturation
  histories before being generalized.
