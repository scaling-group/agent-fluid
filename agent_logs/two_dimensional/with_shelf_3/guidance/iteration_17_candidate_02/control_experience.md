...
- With the trajectory-efficiency supervisor and direction-selective posterior
  headroom already active, a target-signed body-yaw load can serve as an
  additional *withdrawal* signal at the same optional half-cycle residual.
  Against repeatedly reproduced progress-only rollouts, this one-locus change
  preserved the compact self-propelled diagonal, advanced capture from
  `32.3400` to `31.5645`, improved mean distance from `1.63773L` to
  `1.60525L` and score from `0.234663` to `0.266663`, and reduced posterior
  peak excursion from `0.57530` to `0.55553 rad`; nearly unchanged relative-
  crossflow RMS (`0.24372` versus `0.24105`) rules out simple wake avoidance.
  Interpret this as useful hydrodynamic-assistance arbitration, not blanket
  disturbance rejection or efficiency: lateral-force RMS rose from `65.12`
  to `66.32`, moment RMS only edged from `888.56` to `887.63`, and both joints
  still touched velocity and acceleration limits. Apply it only during
  coherent target closure, soften normalized `moment_z_L2` by a policy-owned
  scale, and let it withdraw only incremental steering while distributed mean
  curvature and the unit-gain traveling wave remain intact. Falsify it if a
  repeat or held-out wake loses direct capture, arrival or distance integral
  regresses, load-sign switching disrupts propulsion, or higher force is not
  repaid by a navigation benefit.
