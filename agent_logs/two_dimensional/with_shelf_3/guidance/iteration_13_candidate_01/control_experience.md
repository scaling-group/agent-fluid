...
- Four code-equivalent sampled realizations of the ungated posterior half-cycle
  ...
  state-headroom gate is preferable when load reduction can justify a sub-one-
  percent arrival trade; retain the ungated wave as the navigation benchmark
  and never attenuate the base traveling wave or move oscillator centers. Both
  gated and ungated policies still touch the velocity and acceleration limits,
  so aggregate peaks do not prove shorter saturation residence. Falsify the
  lesson if the load reduction does not repeat, a held-out wake changes the
  direct topology, or the arrival penalty grows materially. A response-gated
  extra burst is not a substitute: it arrived at `32.824`, increased mean
  distance to `1.66402L`, and raised force/moment RMS to `70.97/945.05`.
- Separate releasing existing steering from adding posterior authority. Against
  the `32.472`, `1.64761L`, `68.70/931.60` ungated arrival/mean-distance/load
  benchmark, a gait-active gate that required both targetward heading response
  and shrinking bearing before releasing at most part of distributed mean
  curvature preserved the direct visual topology and improved those quantities
  to `32.1365`, `1.63696L`, and `67.22/907.46`. A separate coherent-closure
  supervisor acting only on the optional posterior half-cycle residual reached
  `32.340`, `1.63773L`, and `65.12/888.56`, earning the best sampled score
  `0.234663`. Thus measured response may safely reduce an existing bounded
  steering bias, while target-progress and normalized joint state may arbitrate
  an incremental rhythmic residual; neither result supports feeding route rate
  into oscillator centers or commanding an extra burst. Their combination is
  not yet evaluated. Apply either lesson only while the base lagged wave and
  target-signed centers remain continuously available, and falsify it if a
  held-out wake loses capture or the compact route, or if load relief comes
  without competitive arrival and distance integral.
