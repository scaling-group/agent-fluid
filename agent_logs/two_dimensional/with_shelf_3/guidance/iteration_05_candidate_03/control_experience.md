...
- Count equation-level diversity, not artifact count, when deciding whether a
  controller is robust. Two equation-identical filtered `40/60` policies replay
  the same `35.689`-unit capture, `1.762L` mean distance, `59.28` force RMS,
  and `821.23` moment RMS from the shared wake snapshot. Against the filtered
  `45/55` parent (`36.564`, `1.808L`, `66.17`, and `901.74` respectively),
  this supports promoting the posterior-weighted allocation for this condition;
  it does not turn two deterministic copies into held-out validation. Retain
  the `40/60` advantage only if changed wake phase or target geometry preserves
  capture, turn sign, propulsion, and the aggregate-load reduction.
- Do not feed a windowed bearing derivative directly into the evidenced
  curvature path without an instrumented sign/scale test. The inherited child
  that added bounded `bearing_window_rate` damping to the successful filtered
  controller exited downstream after `16.956` units, moved its head
  `(+2.175,-0.874)L`, came no closer than `12.424L`, and collapsed mean command
  energy to `8.64`. This falsifies the proposed trend mechanism's reachability
  and propulsion boundaries. Later workers should avoid replaying that direct
  subtraction; revisit target-relative rate feedback only if a local assay
  demonstrates its sign and shows that it cannot suppress the traveling bend.
