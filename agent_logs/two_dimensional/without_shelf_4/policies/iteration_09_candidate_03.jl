function target_policy_params()
    return (
        ...
        bearing_scale=0.30,
        bearing_rate_lead_converging=0.25,
        bearing_rate_lead_diverging=0.20,
...
function target_policy(state, params)
    ...
    # Preserve the demonstrated lookahead while target-bearing error is
    # converging, but smoothly reduce rate amplification while it diverges.
    # The normalized nonnegative product makes the blend bounded and returns
    # exactly to the evaluated anchor whenever divergence ends.
    bearing_rate = clamp(
        state.bearing_window_rate,
        -params.bearing_rate_limit,
        params.bearing_rate_limit,
    )
    divergence = max(state.bearing * bearing_rate, zero(bearing_rate))
    divergence_scale = params.bearing_scale * params.bearing_rate_limit
    divergence_weight = tanh(
        divergence / max(divergence_scale, eps(divergence_scale)),
    )
    bearing_rate_lead = params.bearing_rate_lead_converging +
                        divergence_weight *
                        (params.bearing_rate_lead_diverging -
                         params.bearing_rate_lead_converging)
    predicted_bearing = state.bearing + bearing_rate_lead * bearing_rate
