function target_policy_params()
    return (
        ...
        closing_speed_scale=0.02,
        lateral_schedule_progress_fraction=0.75,
...
function target_policy(state, params)
    ...
    # Retain the sampled anchor's bounded bearing/rate response. The dominant
    # rolling-progress schedule gave the best route integral, while a smaller
    # contribution from drift magnitude tests its faster, lower-load transport.
    bearing_rate = clamp(
    ...
    progress_fraction = clamp(
        params.lateral_schedule_progress_fraction,
        zero(progress_loss_weight),
        one(progress_loss_weight),
    )
    schedule_weight = progress_fraction * progress_loss_weight +
                      (1 - progress_fraction) * away_drift_weight
    lateral_velocity_lookahead = params.lateral_velocity_lookahead_min +
        (params.lateral_velocity_lookahead_max -
         params.lateral_velocity_lookahead_min) * schedule_weight
