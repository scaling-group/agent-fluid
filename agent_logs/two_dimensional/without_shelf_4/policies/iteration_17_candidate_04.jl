...
function target_policy(state, params)
    ...
    # Retain the evaluated 0.015 closing transition, but select 0.08 more
    # decisively while receding. Targetward translation stays on-anchor.
    bearing_rate = clamp(
    ...
    progress_transition_scale = if state.window_closing_speed_L < 0
        params.receding_speed_scale
    else
        params.closing_speed_scale
    end
    progress_transition_scale = max(
        progress_transition_scale,
        eps(progress_transition_scale),
    )
    progress_loss_weight = 0.5 * (
        1 - tanh(
            state.window_closing_speed_L / progress_transition_scale,
        )
    )
