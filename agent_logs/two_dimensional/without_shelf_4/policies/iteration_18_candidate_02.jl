function target_policy_params()
    return (
        ...
        closing_speed_scale_closing=0.015,
        closing_speed_scale_receding=0.020,
...
function target_policy(state, params)
    ...
    # Retain the sampled anchor's bounded bearing/rate response and pure
    # rolling-progress selector. Preserve its sharper, route-improving
    # closing-side transition, but restore the parent's gentler receding-side
    # transition to isolate the sampled route/load tradeoff.
    bearing_rate = clamp(
    ...
    closing_speed_scale = ifelse(
        closing_speed > zero(closing_speed),
        params.closing_speed_scale_closing,
        params.closing_speed_scale_receding,
    )
    closing_speed_scale = max(
        closing_speed_scale,
        eps(closing_speed_scale),
    )
