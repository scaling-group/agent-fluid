...
function target_policy(state, params)
    ...
    # Retain the sampled anchor's bounded bearing/rate response. Use rolling
    # distance progress to select only inside the evaluated 0.07--0.08
    # counter-drift envelope: recover with 0.08 while receding, then ease
    # toward 0.07 while closing. Targetward translation stays on-anchor.
    bearing_rate = clamp(
