...
function target_policy(state, params)
    ...
    # Preserve the demonstrated bearing lookahead. Add bounded body-rotation
    # damping only for turn-away motion near alignment; the Gaussian gate
    # suppresses the inherited correction during large-error wake acquisition.
    bearing_rate = clamp(
    ...
    alignment_scale = max(
        params.heading_alignment_scale,
        eps(params.heading_alignment_scale),
    )
    alignment_weight = exp(-(state.bearing / alignment_scale)^2)
    predicted_bearing = state.bearing +
                        params.bearing_rate_lead * bearing_rate -
                        params.heading_rate_lead * alignment_weight *
                        turn_away_weight * heading_rate
