...
function target_policy(state, params)
    ...
    # Retain the sampled anchor's bounded bearing/rate response. Schedule the
    # counter-drift lookahead only inside the evaluated 0.07--0.08 envelope.
    # Strong target-away drift keeps the fast magnitude schedule; stalled or
    # receding windows also receive the upper retention value. Targetward
    # translation stays on the anchor exactly.
    bearing_rate = clamp(
    ...
    retention_weight = max(away_drift_weight, progress_loss_weight)
    lateral_velocity_lookahead = params.lateral_velocity_lookahead_min +
        (params.lateral_velocity_lookahead_max -
         params.lateral_velocity_lookahead_min) * retention_weight
