...
function target_policy(state, params)
    ...
    # Keep propulsion phase independent of steering. The 20-degree shell is a
    # single-axis increase from the sampled full-horizon 19-degree controller.
    ...
    # Preserve the successful bounded posterior steering sign. A slightly
    # longer, still rate-clamped lead tests earlier unwinding of the broad
    # release turns visible in the sampled target-reaching trajectory.
    bearing_rate = clamp(
    ...
    # bend. The local guard sits above the nominal anterior acceleration but
