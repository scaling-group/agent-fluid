...
function target_policy(state, params)
    ...
    # Retain bounded rate lookahead, but reduce it from the evaluated 0.25
    # anchor: the isolated 0.30 test added a large premature reversal and
    # delayed wake-corridor entry. No coordinate or external clock is used.
    bearing_rate = clamp(
