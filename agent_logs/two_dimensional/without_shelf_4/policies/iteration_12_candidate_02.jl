function target_policy_params()
    return (
        ...
        lateral_velocity_lookahead=0.10,
...
function target_policy(state, params)
    ...
    # oscillator on the demonstrated cap-feasible energy shell.
    ...
    # Preserve the sampled bearing/rate anchor. Apply a small body-frame
    # translation correction only while the fish is drifting away from the
    # target; targetward translation remains on the demonstrated response.
    bearing_rate = clamp(
    ...
    # The posterior joint supplies traveling-wave lag and mean steering bend.
    phase_lag_target = steering_bias - q1 -
                       params.tail_lag_gain * qd1 / max(omega, eps(omega))
    a2_raw = omega^2 * (phase_lag_target - q2) -
             2 * params.tail_damping * omega * qd2

    # Keep unexpected coupled transients below the demonstrated local guard.
