...
function target_policy(state, params)
    ...
    # Keep the replicated selector purely on rolling distance progress. Test a
    # smaller second narrowing inside the evaluated 0.07--0.08 envelope:
    # recover with 0.08 while receding, then ease toward 0.07 while closing.
    # Targetward translation stays on-anchor and no authority is added.
    bearing_rate = clamp(
