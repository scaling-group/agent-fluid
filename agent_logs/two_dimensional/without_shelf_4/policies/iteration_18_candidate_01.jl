function target_policy_params()
    return (
        ...
        receding_closing_speed_scale=0.015,
        approaching_closing_speed_scale=0.020,
...
function target_policy(state, params)
    ...
    # Keep progress as the sole schedule input. Retain the reproduced sharper
    # recovery branch while receding, but restore the smoother parent scale
    # while closing to test whether it reduces the anchor's load tradeoff.
    bearing_rate = clamp(
    ...
    closing_speed = state.window_closing_speed_L
    selected_closing_speed_scale = if closing_speed <= zero(closing_speed)
        params.receding_closing_speed_scale
    else
        params.approaching_closing_speed_scale
    end
    closing_speed_scale = max(
        selected_closing_speed_scale,
        eps(selected_closing_speed_scale),
    )
    progress_loss_weight = 0.5 * (
        1 - tanh(closing_speed / closing_speed_scale)
    )
