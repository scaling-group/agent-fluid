function target_policy_params()
    return (
        ...
        steering_turn_rate_lookahead=0.08,
        steering_turn_rate_limit=0.30,
...
function target_policy(state, params)
    ...
    # Preserve the sampled 0.30-scale/0.25-lead bearing response. A separately
    # bounded body-turn term damps the broad yaw reversals visible in the
    # successful route without weakening translational bearing correction.
    bearing_rate = clamp(
    ...
    turn_rate = clamp(
        ...
        -params.steering_turn_rate_limit,
        params.steering_turn_rate_limit,
    )
    steering_error = state.bearing +
                     params.steering_rate_lookahead * bearing_rate -
                     params.steering_turn_rate_lookahead * turn_rate
