function target_policy_params()
    return (
        ...
        crossflow_error_limit=1.0 * pi / 180,
        relative_crossflow_scale=0.15,
...
function target_policy(state, params)
    ...
    # Preserve the sampled 0.30-scale/0.25-lead bearing response. Both constant
    # lead directions and a separate heading-rate term lengthened the route.
    bearing_rate = clamp(
        state.bearing_window_rate,
        -params.steering_rate_limit,
        params.steering_rate_limit,
    )
    # Add only a small body-frame corridor-retention correction. It asks the
    # fish to crab against lateral relative flow, is smooth at zero, and is
    # capped at one degree so local wake fluctuations cannot replace bearing.
    relative_crossflow = state.relative_flow_velocity_body_U[2]
    crossflow_error = params.crossflow_error_limit * tanh(
        relative_crossflow / params.relative_crossflow_scale,
    )
