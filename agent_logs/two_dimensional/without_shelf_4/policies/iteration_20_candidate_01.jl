function target_policy_params()
    return (
        ...
        steering_rate_lookahead_max=0.30,
        steering_rate_convergence_scale=1.0,
        steering_rate_limit=0.30,
        lateral_velocity_lookahead_min=0.07,
        lateral_velocity_lookahead_max=0.08,
        lateral_velocity_limit=0.10,
        closing_speed_scale=0.015,
...
function target_policy(state, params)
    ...
    # Keep the score-leading selector purely on rolling distance progress. Both
    # evaluated sign splits lost route quality, so retain the symmetric 0.015
    # transition and its 0.07--0.08 envelope.
    bearing_rate = clamp(
        state.bearing_window_rate,
        -params.steering_rate_limit,
        params.steering_rate_limit,
    )
    # Constant 0.20 and 0.30 rate leads both regressed from 0.25. Isolate the
    # remaining convergence-phase hypothesis: anticipate only a shrinking
    # bearing error, while leaving stationary/diverging recovery on 0.25.
    convergence_scale = max(
        params.steering_rate_convergence_scale *
        params.steering_bearing_scale * params.steering_rate_limit,
        eps(params.steering_bearing_scale),
    )
    bearing_convergence = max(
        -state.bearing * bearing_rate,
        zero(bearing_rate),
    )
    convergence_weight = tanh(bearing_convergence / convergence_scale)
    steering_rate_lookahead = params.steering_rate_lookahead +
        (params.steering_rate_lookahead_max -
         params.steering_rate_lookahead) * convergence_weight
    ...
    closing_speed_scale = max(
        params.closing_speed_scale,
        eps(params.closing_speed_scale),
    )
    progress_loss_weight = 0.5 * (
        1 - tanh(state.window_closing_speed_L / closing_speed_scale)
    )
    lateral_velocity_lookahead = params.lateral_velocity_lookahead_min +
        (params.lateral_velocity_lookahead_max -
         params.lateral_velocity_lookahead_min) * progress_loss_weight
    steering_error = state.bearing +
                     steering_rate_lookahead * bearing_rate -
                     lateral_velocity_lookahead * away_drift_weight *
                     lateral_velocity
