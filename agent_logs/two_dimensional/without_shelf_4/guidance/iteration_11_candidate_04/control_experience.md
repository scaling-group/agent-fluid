...
- Treat `0.25` as the local constant bearing-rate-lead anchor for the
  ...
  transport to `0.00359`. Restricting that same `0.05` correction to measured
  turn-away rotation reversed the arrival penalty and captured at `238.557`,
  `5.990` earlier than the anchor, but it was not an objective improvement:
  score fell from `-4.439` to `-5.068`, mean distance rose from `6.452L` to
  `7.077L`, RMS force/crossflow rose to `18.354/0.13453`, and relative-upstream
  transport fell to `0.00476`; only RMS moment improved slightly to `360.851`.
  Thus the sign of `bearing * heading_rate` contains useful turn-timing
  information, but the evaluated smooth gate acts too broadly, and earlier
  capture alone must not justify it. Do not retry constant-lead interpolation,
  bearing-divergence gating, global heading damping, or the unchanged
  turn-away gate for this fixed-prewarm gait. A remaining bounded test may
  concentrate the same correction on only the strongest turn-away events
  while leaving weak, stationary, and targetward turns on the anchor; reject
  it unless capture is no later than `244.547` and mean distance is below
  `6.452L`, or loads fall materially without a route regression. This boundary
  remains local to the retained gait, static-bearing bundle, and wake phase.
