...
- Schedule the target-away lateral counter-drift lookahead within the evaluated
  `0.07--0.08` envelope; do not resume a constant-gain sweep. Relative to the
  constant `0.08` anchor (`224.488`, `6.311L` mean distance, `0.01602`
  controller-relative upstream margin, `17.943/361.014` RMS force/moment), a
  rolling progress-loss schedule produced the best sampled score and route
  integral (`-3.863`, `5.856L`) while capturing at `213.659` and reducing
  force/moment to `17.761/354.838`, but margin fell to `0.01261`. Scheduling
  the same bounds by away-drift magnitude instead captured earliest at
  `196.900`, increased margin to `0.01735`, and further reduced force/moment
  and total effort to `17.672/352.909/136830`, while mean distance and score
  regressed to `6.211L/-4.230`. The identical `4.293L` lateral offset and
  `31.055` anterior acceleration across these policies isolate a Pareto trade
  in steering timing, not propulsion or saturation; inherited `0.07` and
  `0.10` constants were later and higher-load or produced a late corridor
  excursion. A convex combination of these two normalized schedule weights is
  therefore a bounded next test, but reject it if it is dominated by the
  progress schedule on integral/score without moving arrival, upstream margin,
  or loads toward the drift schedule. This implication is local to the fixed
  prewarm phase; held-out phase evidence must falsify or extend it.
