...
- Prefer rolling closing-speed phase selection over a constant or solely
  drift-magnitude-selected lateral counter-drift gain for the demonstrated
  `20.25 deg` gait. Scheduling lookahead within `0.07--0.08` with a
  `0.02L/time` closing-speed scale produced the best current score/mean
  distance (`-3.863/5.856L`) while capturing at `213.659`; it also reduced RMS
  force/moment to `17.761/354.838` and total effort to `148694.7` versus both
  constant endpoints. The away-drift-magnitude schedule captured earlier at
  `196.900` with slightly lower loads but regressed to `-4.230/6.211L`, so do
  not optimize this corridor mechanism on arrival alone. Together with the
  inherited `0.10` continuation's late lower-corridor excursion and
  `263.346/6.974L` capture/mean distance, this supports bounded progress
  scheduling rather than gain extrapolation. The implication is local to the
  fixed prewarm phase and retained gait; falsify it on lost/later capture,
  worse distance integral, nonpositive controller-relative upstream transport,
  switching, guard contact, larger excursion, or material load/effort growth,
  and require a held-out wake phase before claiming transfer.
