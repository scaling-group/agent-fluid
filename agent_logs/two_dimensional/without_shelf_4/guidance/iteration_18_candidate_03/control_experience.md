...
- Treat `0.015L/time` as the replicated local transition-scale anchor for the
  ...
  switching. Three current samples with executable-equivalent policies and the
  common prewarm reproduce the same `210.370` capture, `-3.528` score,
  `5.519L` mean distance, `4.293L` excursion, `0.01672` controller-relative
  upstream speed, and `147846.5` effort. The current `0.020` comparator captures
  later at `213.659` with `5.856L` mean distance and `0.01261` upstream margin,
  while an inherited executable-equivalent `0.020` rollout instead approached
  to `0.960L`, rebounded to `2.904L`, and expanded excursion to `5.835L` despite
  unchanged `31.055` anterior acceleration and finite lower loads. This makes
  `0.015` the stronger fixed-phase repeatability anchor and identifies selector
  timing/corridor retention, not propulsion or saturation, as the supported
  exploration surface. Its route gain carries higher RMS force/moment
  (`18.426/363.057` versus the successful `0.020` run's `17.761/354.838`), so
  any narrower controlled probe must change only transition width and earn its
  load cost through better arrival or distance integral. Do not blend in
  away-drift magnitude: the inherited `75%/25%` blend missed at `3.689L` final
  distance. Falsify transfer on a near-entry rebound, later or lost capture,
  mean distance above `5.856L`, excursion beyond `4.293L`, nonpositive upstream
  margin, guard contact, switching, or material load/effort growth. Repetition
  under one certified wake phase still does not establish held-out robustness.
