...
- Selectively countering target-away lateral translation is the first sampled
  ...
  to `259.160`. Adopt this bounded sign gate locally instead of suppressing all
  lateral motion. Its evidence is one certified fixed-prewarm phase: falsify
  transfer on lost/later-than-anchor capture, mean distance above `6.452L`,
  reduced upstream margin, larger excursion/load, guard contact, or chatter,
  and test the fixed `0.08/0.10` bundle on a held-out phase before retuning it.
