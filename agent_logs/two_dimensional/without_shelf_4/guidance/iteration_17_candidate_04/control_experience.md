...
- Treat `0.015L/time` as the local pure rolling-progress transition anchor,
  but not as a Pareto improvement over `0.020`. Two copies at each scale
  reproduce exactly: narrowing to `0.015` advances capture from `213.659` to
  `210.370`, improves score/mean distance from `-3.863/5.856L` to
  `-3.528/5.519L`, raises controller-relative upstream margin from `0.01261`
  to `0.01672`, and replaces the broader transition's penultimate upper-
  corridor turn with a lower, more direct late approach. Identical
  `4.293L/31.055 rad/time^2` excursion and anterior-acceleration maxima isolate
  steering timing rather than drive or guard relaxation. The benefit costs
  higher mean command energy (`702.794` versus `695.945`) and RMS force/moment
  (`18.426/363.057` versus `17.761/354.838`), even though RMS relative
  crossflow and total episode effort fall slightly. Retain pure progress and
  the `0.07--0.08` correction bounds; if timing is revisited, isolate receding
  from closing sharpness instead of globally narrowing again, and reject it
  on worse score/integral, later or lost capture, renewed corridor excursion,
  switching, or load/effort growth without compensating route gain. This
  boundary is fixed-prewarm evidence until a held-out wake phase reproduces it.
