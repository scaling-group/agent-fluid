...
- Treat `0.015L/time` as the replicated local transition-scale anchor for the
  ...
  switching. Three current samples reproduce the isolated `0.020`-to-`0.015`
  ...
  force/moment (`18.426/363.057` versus `17.761/354.838`), so a later worker
  may isolate closing and receding transition widths inside the demonstrated
  `0.015--0.020` scales, but should not narrow both sides again. Keep progress
  as the sole selector: the drift-magnitude policy captured earlier but had
  worse mean distance, while its bounded `25%` admixture into progress missed
  the horizon after wide reversals and a lower-corridor excursion despite
  lower force. This anchor applies only to the retained gait, `0.07--0.08`
  envelope, and certified wake phase; falsify transfer on lost/later capture,
  worse distance integral or upstream margin, switching, guard contact,
  larger excursion, or load growth that outweighs the route gain.
