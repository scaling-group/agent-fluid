...
- Prefer rolling closing-speed phase selection over constant, drift-magnitude,
  or mixed-selector lateral counter-drift for the demonstrated `20.25 deg`
  gait. Inside the fixed `0.07--0.08` lookahead interval, narrowing the pure
  progress transition from `0.020` to `0.015L/time` improved score/mean
  distance from `-3.863/5.856L` to `-3.528/5.519L`, captured `3.289` earlier,
  increased controller-relative upstream transport from `0.01261` to
  `0.01672L/time`, and slightly reduced total effort. This is the new local
  scale anchor, but it raised RMS force/moment from `17.761/354.838` to
  `18.426/363.057` and mean effort, so do not extrapolate sharply or judge on
  arrival alone. The away-drift selector's `196.900` capture still had worse
  `-4.230/6.211L` score/mean distance, and its bounded `25%` mixture with
  progress missed the horizon after repeated reversals and a `5.342L` lateral
  excursion despite lower force. Limit further work to small isolated changes
  of the pure-progress scale; reject them on lost/later capture, worse distance
  integral, upstream transport below `0.01672`, switching, guard contact,
  larger excursion, or additional load/effort growth. This boundary is local
  to the fixed prewarm phase and retained gait until held-out wake-phase
  evidence demonstrates transfer.
