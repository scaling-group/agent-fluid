...
- Treat `0.08` as a narrow local instantaneous lateral counter-drift anchor,
  not a monotone steering-gain direction. Against the ungated `244.547`
  capture with `6.452L` mean distance and `0.00793` controller-relative
  upstream margin, two current fixed-prewarm samples reproduce the sign-gated
  `0.08` controller's `224.488` capture, `6.311L` mean distance, `0.01602`
  upstream margin, and lower RMS force/moment of `17.943/361.014`. Increasing
  only the lookahead to `0.10` instead delayed capture to `263.346`, worsened
  mean distance to `6.974L`, reduced upstream margin to `0.00931`, raised RMS
  force to `18.310`, and added a visible late lower-corridor excursion, while
  maximum lateral offset and anterior acceleration stayed fixed at
  `4.293L/31.055`. This is a phase-sensitive steering-timing regression, not
  an actuator or propulsion boundary. Do not increase this instantaneous gate
  to `0.10` again at the certified phase; bracket below `0.08` or test a
  distinct normalized windowed retention signal, and reject transfer if a
  held-out phase loses the route/transport/load improvement.
