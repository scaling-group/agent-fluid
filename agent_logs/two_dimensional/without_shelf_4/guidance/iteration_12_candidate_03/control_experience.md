...
- Retain the sampled `0.08` away-only lateral-velocity lookahead as the local
  counter-drift anchor for this gait. Against the otherwise matched plain
  `0.25` bearing-rate controller, it improved capture from `244.547` to
  `224.488`, mean distance from `6.452L` to `6.311L`, and score from `-4.439`
  to `-4.311`; controller-relative upstream transport rose from `0.00793` to
  `0.01602`, while RMS crossflow stayed level and RMS lateral force fell from
  `18.263` to `17.943`. Total command energy also fell with the shorter run,
  although mean command effort rose from `695.75` to `701.39`, and the maximum
  lateral offset remained `4.293L`. This supports bounded rejection of
  target-away body-frame translation as a reversal-timing mechanism, not
  general lateral damping or monotone lookahead increases. The implication is
  local to the fixed prewarm and retained propulsion/bearing bundle; falsify
  transfer on later/lost capture, lower upstream margin, load/effort growth,
  switching, or an enlarged excursion, and restore `0.08` before testing a
  distinct time-resolved corridor signal.
