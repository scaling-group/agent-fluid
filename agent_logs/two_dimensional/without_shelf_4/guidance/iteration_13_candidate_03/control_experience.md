...
- Selectively countering target-away lateral translation is the first sampled
  ...
  lateral force from `18.263` to `17.943`; moment fell from `362.214` to
  `361.014`, maximum lateral offset stayed `4.293L`, and the unchanged
  `31.055` anterior maximum confirms a steering-timing rather than propulsion
  gain. Its sheet preserves the useful wake-band crossings while reaching the
  central target approach sooner, unlike inherited bearing-divergence gating,
  which missed at `3.632L`, or global heading damping, which delayed capture
  to `259.160`. The direct higher-gain continuation establishes a local upper
  boundary: changing only the lookahead from `0.08` to `0.10` delayed capture
  to `263.346`, raised mean distance to `6.974L`, cut controller-relative
  upstream transport from `0.01602` to `0.00931`, and raised force/moment to
  `18.310/361.415`, despite slightly lower relative crossflow. Its sheet shows
  a longer jagged corridor transit, while unchanged `4.293L` maximum lateral
  offset and `31.055` anterior acceleration identify steering timing rather
  than propulsion or envelope. Adopt the bounded sign gate locally instead of
  suppressing all lateral motion, but do not increase its global lookahead
  above the demonstrated `0.08` anchor on this phase. If retuning, bracket the
  lower side with gait, clamp, and other steering fixed; reject it on lost or
  later-than-`224.488` capture, mean distance above `6.311L`, upstream margin
  below `0.01602`, larger excursion/load, guard contact, or chatter. A held-out
  wake phase can falsify this fixed-prewarm boundary.
