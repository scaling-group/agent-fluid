...
- Prefer pure rolling closing-speed phase selection over constant or blended
  lateral counter-drift scheduling for the demonstrated `20.25 deg` gait. Two
  current samples reproduce the `0.07--0.08`, `0.02L/time` progress schedule's
  `213.659` capture and best score/mean distance `-3.863/5.856L`; relative to
  constant `0.07`, it captures `31.790` earlier, lowers RMS force/moment from
  `18.399/363.454` to `17.761/354.838`, and cuts total effort from `169649.0`
  to `148694.7`. A drift-magnitude schedule captures earlier at `196.900` but
  regresses to `-4.230/6.211L`, so arrival alone is not the objective. More
  importantly, an inherited convex blend with only `0.25` drift-magnitude
  weight misses the horizon at final/minimum distance `3.689/3.381L`, expands
  maximum lateral offset from `4.293L` to `5.342L`, and roughly halves mean
  upstream head speed despite unchanged `31.055` anterior acceleration and no
  collision, exit, or instability. Do not interpolate successful steering
  schedules merely because their output remains inside evaluated gain bounds;
  their timing can change route topology. This lesson is local to the retained
  gait and certified wake phase: require held-out-phase capture, bounded
  excursion, positive controller-relative transport, and no integral/load or
  switching regression before claiming transfer.
