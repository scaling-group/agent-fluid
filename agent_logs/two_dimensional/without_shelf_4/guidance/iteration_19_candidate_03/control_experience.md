...
- Preserve the pure rolling closing-speed selector and its sharper receding
  branch for the demonstrated `20.25 deg` gait. Four current samples reproduce
  the symmetric `0.015 L/time` transition exactly at score/mean distance
  `-3.528/5.519L`, capture `210.370`, and controller-relative upstream
  transport `0.01672`. Softening only zero/negative closing speed to `0.020`
  captured earlier at `206.470` and reduced effort from `147846` to `142752`
  and RMS force/moment from `18.426/363.057` to `18.368/356.178`, but regressed
  score/mean distance to `-3.956/5.943L` and upstream margin to `0.01215`.
  Thus lower load, earlier capture, and a visually smoother late approach do
  not justify weakening recovery-phase selection; test the complementary
  closing-side split before further scale extrapolation. Do not mix the
  selector with lateral drift: the inherited `75%` progress / `25%`
  drift-magnitude blend missed the horizon at `3.689L` final and `7.507L` mean
  distance despite low load. This boundary is local to the retained gait and
  fixed-prewarm phase; falsify it if the complementary split or a held-out wake
  phase preserves capture and improves distance integral and load without
  switching, guard contact, larger excursion, or upstream-margin loss.
