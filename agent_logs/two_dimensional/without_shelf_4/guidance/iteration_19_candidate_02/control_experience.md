...
- Treat `0.015L/time` as the replicated local transition-scale anchor for the
  pure rolling closing-speed selector, not evidence for monotonically sharper
  switching. Four current samples reproduce the isolated `0.020`-to-`0.015`
  change and outcome exactly: capture improves from `213.659` to `210.370`,
  mean distance from `5.856L` to `5.519L`, score from `-3.863` to `-3.528`,
  controller-relative upstream transport from `0.01261` to `0.01672`, and
  total effort from `148695` to `147846`. Unchanged `4.293L` maximum lateral
  offset and `31.055 rad/time^2` anterior acceleration isolate selector timing
  rather than extra drive or saturation. The benefit costs higher RMS
  force/moment (`18.426/363.057` versus `17.761/354.838`). The inherited
  asymmetric test identifies which relaxation to avoid: keeping `0.015` while
  closing but using `0.020` while receding arrived earlier at `206.470` and
  reduced effort/force/moment to `142752/18.368/356.178`, yet worsened mean
  distance and score to `5.943L/-3.956` and reduced upstream transport to
  `0.01215`. Its finite sheet takes a different middle wake-band route, so
  earlier capture and lower loads do not validate softer recession; preserve
  the sharp receding scale unless the opposite half-isolation demonstrates
  otherwise, and do not repeat the falsified split. Keep progress as the sole
  selector: the drift-magnitude policy captured earlier but had worse mean
  distance, while its bounded `25%` admixture into progress missed the horizon
  after wide reversals and a lower-corridor excursion despite lower force.
  This anchor and asymmetric boundary apply only to the retained gait,
  `0.07--0.08` envelope, and certified wake phase; falsify transfer on
  lost/later capture, worse distance integral or upstream margin, switching,
  guard contact, larger excursion, or load growth that outweighs the route
  gain.
