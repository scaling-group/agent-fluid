...
- Selectively countering target-away lateral translation is the first sampled
  ...
  central target approach sooner. Constant gain around this point is
  non-monotone: the reproduced `0.07` lower bracket narrowly improved mean
  distance/score to `6.305L/-4.291`, but delayed capture to `245.449`, reduced
  controller-relative upstream transport from `0.01602` to `0.01284`, and
  raised force/moment to `18.399/363.454`; inherited `0.10` delayed capture to
  `263.346` and worsened mean distance to `6.974L`. Do not select `0.07` from
  its scalar score alone, repeat another constant bracket in the tested
  `0.07--0.10` interval, or suppress all lateral motion. The next local test
  should either hold `0.08` on a new wake phase or bound a magnitude/phase
  schedule within the sampled `0.07--0.08` envelope. Falsify it on lost or
  later capture, mean-distance regression, upstream margin below `0.01602`,
  larger excursion/load, guard contact, or chatter. This boundary is one
  certified fixed-prewarm phase; inherited bearing-divergence gating that
  missed at `3.632L` and global heading damping that delayed capture to
  `259.160` remain distinct negative controls.
