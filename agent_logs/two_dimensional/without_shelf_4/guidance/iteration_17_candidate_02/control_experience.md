...
- Prefer pure rolling closing-speed phase selection over constant or blended
  lateral counter-drift scheduling for the demonstrated `20.25 deg` gait, but
  require independent replication before calling one transition scale robust.
  The exact `0.02L/time` policy and byte-identical prewarm sheet produced two
  earlier `213.659` captures, then the assigned parent's latest rollout missed
  after approaching to `0.960L`: it rebounded to `2.904L`, expanded maximum
  lateral offset from `4.293L` to `5.835L`, and ended with `6.613L` mean
  distance despite positive `0.01639` controller-relative upstream speed,
  unchanged `31.055` anterior acceleration, and finite/lower loads. This is a
  corridor-retention sensitivity, not evidence for more drive or saturation.
  By contrast, two current samples of the otherwise matched `0.015L/time`
  transition capture exactly at `210.370`, improve score/mean distance to
  `-3.528/5.519L`, retain `4.293L` excursion and positive `0.01672` upstream
  margin, and use `147846.5` effort, though their `18.426/363.057` RMS
  force/moment exceed the successful `0.02` run. Retain `0.015` as the stronger
  local anchor; do not blend in away-drift magnitude, whose inherited `0.25`
  contribution missed at `3.689L` final distance. Falsify the anchor on an
  independent miss or near-entry rebound, mean distance above `5.856L`,
  excursion beyond `4.293L`, nonpositive upstream margin, guard contact, or
  material switching/load/effort growth. Same-step repetition cannot establish
  held-out wake-phase or later-run robustness.
