...
- Treat `0.015L/time` as the replicated local transition-scale anchor for the
  pure rolling closing-speed selector, and preserve it specifically while the
  fish is receding. Four current samples reproduce its `210.370` capture,
  `5.519L` mean distance, `0.01672` controller-relative upstream transport,
  `4.293L` maximum lateral offset, and `31.055 rad/time^2` anterior-acceleration
  maximum. An inherited controlled sign split kept `0.015` while closing but
  softened only recession to `0.020`; it crossed earlier at `206.470` and
  lowered command energy and RMS moment from `147846/363.057` to
  `142752/356.178`, yet worsened mean distance to `5.943L`, score to `-3.956`,
  and upstream transport to `0.01215`. Its visibly calmer middle path is thus
  not evidence of better navigation. Do not repeat a softened receding-side
  transition or rank the split by arrival/load alone; receding sharpness is
  part of corridor recovery, not merely a load source. If sign dependence is
  revisited, isolate the complementary closing-only softening while retaining
  `0.015` during recession, and reject it on worse distance integral, later or
  lost capture, reduced upstream margin, switching, guard contact, larger
  excursion, or load growth. This boundary applies only to the retained gait,
  pure progress signal, `0.07--0.08` envelope, and certified wake phase.
