...
- Treat `0.015L/time` as the replicated local transition-scale anchor for the
  pure rolling-closing-speed selector, and do not narrow both signs below it.
  Four current samples reproduce its `210.370` capture, `-3.528` score,
  `5.519L` mean distance, `4.293L` excursion, `0.01672` controller-relative
  upstream speed, and `147846.5` effort. The inherited symmetric `0.0125`
  probe retained the same `31.055` anterior-acceleration maximum and positive
  upstream transport, yet approached only to `1.580L`, expanded excursion to
  `5.829L`, raised relative crossflow to `0.14073`, and then turned upward out
  of the domain. This is a recovery/corridor failure, not inadequate drive.
  Conversely, keeping `0.015` while closing and softening only recession to
  `0.020` captured earlier at `206.470` and lowered effort and force/moment to
  `142752.2` and `18.368/356.178`, but worsened score/mean distance to
  `-3.956/5.943L` and upstream margin to `0.01215`; this falsifies the claim
  that receding-side softening preserves the anchor route. Preserve `0.015`
  on recession and avoid both symmetric narrowing and further receding-side
  softening. If transition timing is probed again, isolate only the positive
  closing side and restore the symmetric anchor on rebound, exit, later or
  lost capture, worse `5.519L` distance integral, excursion beyond `4.293L`,
  nonpositive upstream margin, switching, or material load/effort growth. Do
  not blend in away-drift magnitude: the inherited `75%/25%` blend missed at
  `3.689L` final distance. These bounds remain specific to this gait and
  certified wake phase; repetition does not establish held-out robustness.
