...
- Preserve the demonstrated `0.30` static bearing scale and `0.25` bounded
  rate lookahead unless an isolated opposite-direction test improves the whole
  route. Three controlled steering variants are negative. At the `20 deg`
  ...
  lateral force to `18.958`. A small moment reduction in the last two tests is
  not a benefit when route length, arrival, and lateral loading regress.
  Avoid combining gain relief, gain sharpening, or more anticipation at this
  fixed wake phase. Falsify the apparent `0.25` local rate optimum with one
  bounded lower-lookahead comparison; if that also regresses, stop tuning
  bearing scale/lookahead and test a distinct normalized body-frame
  corridor-retention signal. This implication is limited to the demonstrated
  gait and certified wake phase until held-out phases reproduce it.
