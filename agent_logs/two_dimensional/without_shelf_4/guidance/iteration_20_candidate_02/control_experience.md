...
- Keep the pure rolling closing-speed selector sign-symmetric near its
  replicated `0.015L/time` anchor; hard sign-dependent transition scales are a
  fragile route switch. Four current samples reproduce the symmetric anchor
  exactly: relative to symmetric `0.020`, capture improves from `213.659` to
  `210.370`, mean distance from `5.856L` to `5.519L`, score from `-3.863` to
  `-3.528`, controller-relative upstream transport from `0.01261` to
  `0.01672`, and effort from `148695` to `147846`, with unchanged `4.293L`
  maximum lateral offset and `31.055 rad/time^2` anterior acceleration. The
  gain costs higher RMS force/moment (`18.426/363.057` versus
  `17.761/354.838`), so it is not proof that arbitrarily sharper switching is
  better. Both one-sided relaxations nevertheless fail as repairs. Using
  `0.020` only while receding captures at `206.470` with lower load/effort but
  worsens mean distance/score to `5.943L/-3.956` and upstream transport to
  `0.01215`; using `0.020` only while closing makes a broad down/up loop and
  exits the top boundary at `214.528`, with score `-13.428`, minimum/final/mean
  distances `7.316/12.542/11.062L`, negative progress, and effort/moment
  `151816/368.309`. Thus neither visually smoother motion nor lower force
  validates asymmetric relaxation. Do not repeat either split or mix progress
  with lateral drift, whose `25%` admixture missed the horizon. If transition
  tuning continues, preserve sign symmetry and use a small isolated scale
  move; falsify it on changed route topology, lost/later capture, worse
  distance integral or upstream margin, switching, guard contact, larger
  excursion, or load growth that outweighs route gain. This boundary is local
  to the retained gait, `0.07--0.08` envelope, and certified wake phase.
