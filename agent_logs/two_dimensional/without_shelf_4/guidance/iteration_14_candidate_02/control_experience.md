...
- Selectively countering target-away lateral translation is the first sampled
  corridor signal to improve the demonstrated `20.25 deg` anchor. Holding gait
  and bearing/rate steering fixed, clamping normalized body-frame lateral
  velocity to `0.10` and applying a `0.08` lookahead only when
  `bearing * velocity_body_U[2] < 0` advanced capture from `244.547` to
  `224.488`, lowered mean distance from `6.452L` to `6.311L`, and reduced RMS
  force/moment from `18.263/362.214` to `17.943/361.014`; inherited
  bearing-divergence gating instead missed at `3.632L`. Two independent current
  samples establish that the lower side is a Pareto split rather than a
  monotone improvement: changing only lookahead to `0.07` improved score from
  `-4.310836` to `-4.290729`, mean distance to `6.30464L`, and mean
  command/power proxies from `701.390/47.662` to `691.179/46.964`, but delayed
  capture to `245.449`, reduced controller-relative upstream transport from
  `0.01602` to `0.01284`, and raised RMS force/moment to `18.399/363.454`.
  Both settings retain the same `4.293L` excursion and `31.055` anterior
  maximum, so choose `0.07` only when the current score/mean-distance objective
  dominates; retain `0.08` when arrival, transport margin, or load dominates,
  and do not assume an unevaluated midpoint improves both. The inherited
  `0.10` continuation is outside this useful band: it delayed capture to
  `263.346`, worsened mean distance to `6.974L`, cut upstream transport to
  `0.00931`, and raised loads. This boundary applies to the retained gait and
  certified fixed-prewarm phase; falsify it on a held-out phase, lost capture,
  guard contact, larger excursion, or reversal of the stated objective trade.
