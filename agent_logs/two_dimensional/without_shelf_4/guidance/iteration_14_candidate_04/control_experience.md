...
- Treat `0.08` as the local instantaneous lateral counter-drift arrival/load
  anchor, but not as a monotone score direction. Two fixed-prewarm samples
  reproduce its `224.488` capture, `6.310611L` mean distance, `0.016022`
  controller-relative upstream margin, and RMS crossflow/force/moment of
  `0.134270/17.9429/361.014`. Two otherwise matched `0.07` samples slightly
  improve mean distance and score to `6.304640L/-4.290729`, and reduce
  crossflow to `0.131256`, but capture `20.961` later, reduce upstream margin
  to `0.012845`, raise total command energy by `12195.1`, and raise RMS
  force/moment to `18.3994/363.453`; their sheets show a longer upper/central
  reversal sequence before entry. The inherited `0.10` continuation also
  captured later at `263.346`, worsened mean distance to `6.974L`, and added a
  late lower-corridor excursion. All three keep maximum lateral offset and
  anterior acceleration fixed at `4.293L/31.055`, isolating phase-sensitive
  steering timing rather than propulsion or saturation. Do not extrapolate at
  or beyond `0.07--0.10` at the certified phase; an interior probe must be
  judged as a route/load Pareto test, and transfer is falsified if a held-out
  phase loses capture, upstream transport, or the demonstrated load envelope.
