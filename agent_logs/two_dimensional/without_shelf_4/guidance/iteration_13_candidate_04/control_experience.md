...
- Treat sign-gated body-frame lateral counter-drift as a narrow timing gain,
  not a monotone steering knob. With the retained `20.25 deg`, `0.67`-period,
  `0.30/0.25` gait/steering bundle, a `0.08` lookahead and `0.10` normalized
  lateral-velocity clamp advanced capture from `244.547` to `224.488`, lowered
  mean distance from `6.452L` to `6.311L`, doubled controller-relative upstream
  transport from `0.00793` to `0.01602`, and reduced RMS force/moment from
  `18.263/362.214` to `17.943/361.014`. Increasing only the lookahead to
  `0.10` reversed the gain: its visibly more jagged late corridor captured at
  `263.346`, raised mean distance to `6.974L` and RMS force to `18.310`, and
  reduced upstream margin to `0.00931`, despite slightly lower relative
  crossflow. All three variants retained the `4.293L` maximum lateral offset
  and `31.055` anterior acceleration maximum, isolating a non-monotone route-
  timing effect rather than propulsion or excursion amplitude. Preserve the
  sign gate, do not increase its lookahead above the demonstrated `0.08` at
  this fixed prewarm, and make any retune one-sided below `0.08`; require
  capture, mean distance, upstream margin, and loads to improve jointly.
  Falsify this local boundary only with a controlled wake-phase or gait
  comparison, and distrust lower crossflow alone when corridor retention or
  capture timing worsens.
