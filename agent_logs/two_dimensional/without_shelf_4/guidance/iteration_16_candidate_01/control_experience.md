...
- Prefer pure rolling closing-speed phase selection over constant,
  drift-magnitude, or interpolated lateral counter-drift selectors for the
  demonstrated `20.25 deg` gait. Scheduling lookahead within `0.07--0.08`
  using only closing speed produced the best current score/mean distance
  (`-3.863/5.856L`) while capturing at `213.659`; it also reduced RMS
  force/moment to `17.761/354.838` and effort to `148694.7` versus both
  constant endpoints. The drift-magnitude selector captured earlier at
  `196.900` with slightly lower loads but regressed to `-4.230/6.211L`, and a
  bounded `75%` progress / `25%` drift-magnitude convex blend then missed the
  `300` horizon at minimum/final/mean distances `3.381/3.689/7.507L` with
  effort `213650.5`, despite low `17.475` RMS force. Thus route quality does
  not interpolate between these two individually finite selectors, and lower
  load or earlier endpoint timing cannot justify mixing them. Together with
  the inherited `0.10` continuation's late lower-corridor excursion, this
  supports testing only the pure progress selector's bounded transition
  shaping before another signal decomposition or gain extrapolation. This
  boundary is local to the fixed prewarm phase and retained gait; falsify it
  if a held-out wake phase or controlled blend sweep preserves capture while
  improving distance integral without switching, guard contact, larger
  excursion, nonpositive controller-relative upstream transport, or material
  load/effort growth.
