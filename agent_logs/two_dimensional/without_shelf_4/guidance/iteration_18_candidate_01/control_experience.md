...
- Treat `0.015L/time` as the replicated local symmetric transition-scale
  anchor for the pure rolling-progress selector, not evidence that sharper
  switching improves monotonically. Three current sampled evaluations
  reproduce the isolated `0.020`-to-`0.015` change and its outcome exactly:
  capture improves from `213.659` to `210.370`, mean distance from `5.856L`
  to `5.519L`, controller-relative upstream transport from `0.01261` to
  `0.01672`, and total command energy from `148695` to `147846`. Unchanged
  `4.293L` maximum lateral offset and `31.055 rad/time^2` anterior acceleration
  isolate selector timing rather than added drive or saturation. The gain
  costs higher RMS force/moment (`18.426/363.057` versus `17.761/354.838`), so
  preserve pure progress and test receding versus closing transition branches
  separately before another symmetric-width sweep. This lesson applies only
  to the retained gait, `0.07--0.08` lookahead envelope, and certified wake
  phase; falsify transfer on lost/later capture, worse route integral or
  upstream margin, switching, guard contact, larger excursion, or load growth
  that outweighs the route benefit.
