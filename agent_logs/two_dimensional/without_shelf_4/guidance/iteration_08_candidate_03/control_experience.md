...
- Treat `0.30` static bearing scale with `0.25` rate lead as a local steering
  anchor, not a broad-turn defect that either stronger or weaker correction is
  known to repair. At `20 deg`, sharpening scale to `0.28` enlarged the detour,
  ...
  `244.547` to `256.663`, worsened mean distance from `6.452L` to `7.394L`,
  and raised relative crossflow and RMS force. Increasing only rate lead to
  `0.30` was worse still: capture moved to `270.446`, mean distance to
  `8.499L`, RMS force to `18.958`, and controller-relative upstream transport
  fell from `0.00793` to `-0.00055`. Preserve static scale and omit
  closing-speed relief for this gait; if the equal-sized rate-lead backoff also
  fails, restore `0.25` and treat this bundle as locally settled rather than
  combining bearing changes. This boundary applies to the demonstrated wake
  phase and shell; falsify it only with a controlled held-out comparison or a
  distinct normalized signal that improves capture, mean distance, route, and
  loads together.
