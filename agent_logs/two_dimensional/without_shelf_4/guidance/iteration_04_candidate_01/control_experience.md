...
- Preserve the narrow `0.67`-period, `0.65/0.80` posterior lag/damping, negative
  ...
  matched sampled pair, increasing the shell from `19` to `20 deg` (with an
  ...
  `+0.058L` head-x travel and an `11.719L` miss. Change one axis at a time and
  require capture plus retained negative local-flow/head-x transport; falsify
  the `20 deg` mechanism if it fails to repeat under the certified prewarm, if
  the guard becomes active, or if approach is followed by increasing distance.
  Treat it as fixed-phase evidence until another wake phase repeats capture.
