...
- Schedule the target-away lateral counter-drift lookahead within the evaluated
  `0.07--0.08` envelope, but do not infer that successful endpoint schedules
  can be safely interpolated. The rolling progress-loss schedule remains the
  best sampled score/integral (`-3.863`, `5.856L`) with `213.659` capture,
  `0.01261` upstream margin, and `17.761/354.838` RMS force/moment; the
  away-drift schedule captures earliest at `196.900` with higher `0.01735`
  margin and lower `17.672/352.909` loads, but regresses to `6.211L/-4.230`
  mean distance/score. A `75/25` arithmetic combination of those normalized
  weights then missed the `300` horizon at `3.689L`, worsened mean distance to
  `7.507L`, expanded maximum lateral offset from `4.293L` to `5.342L`, and
  raised total effort to `213651`; its sheet shows large late reversals and
  lost central-corridor retention. The gait and `31.055` anterior acceleration
  maximum were unchanged, so the negative result is phase-sensitive steering
  timing rather than saturation or instability. Avoid further convex blends
  or constant-gain sweeps: if the two signals are combined, first test a
  one-sided selector that cannot weaken the demonstrated progress-loss
  recovery, and reject it on lost/later capture, worse integral, margin below
  `0.01261`, larger excursion, or load/effort growth. This nonconvex boundary
  is established only for the retained gait and certified fixed-prewarm phase;
  held-out wake phase must falsify or extend it.
