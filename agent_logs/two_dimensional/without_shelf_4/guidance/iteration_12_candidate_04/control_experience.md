...
- Treat `0.30` static bearing scale with `0.25` rate lead as a local steering
  ...
  the static scale/rate-lead bundle for this fixed shell and wake phase; alter
  it only through a separately bounded body-frame correction whose capture,
  mean distance, upstream margin, loads, and effort can be judged together.
  Falsify this boundary only with a controlled held-out wake phase or shell
  comparison.
- Prefer bearing-gated body-frame lateral-velocity rejection over direct
  heading-rate damping as the next local corridor-retention axis. On the same
  `20.25 deg`, `0.30/0.25` anchor, an away-drift-only term with `0.08`
  lookahead and a `0.10` velocity clamp (at most `0.008 rad` added steering
  error) advanced capture from the exactly replicated `244.547` to `224.488`,
  reduced mean distance from `6.452L` to `6.311L`, increased mean upstream
  velocity magnitude from `0.04443` to `0.04895`, and lowered RMS lateral force
  from `18.263` to `17.943`. The released sheet still shows a midcourse
  reversal, so exploit only by small bounded gain interpolation; mean command
  energy already rose from `695.746` to `701.390`. As a negative contrast, a
  `0.05` heading-rate lead retained the broad loop and delayed capture to
  `259.160` despite a lower RMS moment. This lesson applies only to the fixed
  shell and shared wake phase; reject stronger drift correction on lost/later
  capture, worse mean distance, renewed lateral ejection, or material force,
  moment, effort, or guard growth, and require held-out phase evidence before
  treating it as general wake rejection.
