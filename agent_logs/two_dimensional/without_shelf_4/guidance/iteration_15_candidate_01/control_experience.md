...
- Selectively countering target-away lateral translation is the first sampled
  corridor signal to improve the demonstrated `20.25 deg` anchor without a
  load trade. Holding gait and bearing/rate steering fixed, clamping normalized
  body-frame lateral velocity to `0.10` and applying a `0.08` lookahead only
  when `bearing * velocity_body_U[2] < 0` advanced capture from `244.547` to
  `224.488`, lowered mean distance from `6.452L` to `6.311L`, and reduced RMS
  force/moment from `18.263/362.214` to `17.943/361.014`. Constant gain is
  non-monotone: reproduced `0.07` delayed capture to `245.449` and raised loads
  to `18.399/363.454` despite a `0.006L` mean-distance gain, while inherited
  `0.10` delayed capture to `263.346` and worsened mean distance to `6.974L`.
  Two bounded schedules inside the evidenced `0.07--0.08` interval now show
  that controller state, rather than a finer constant bracket, is the useful
  selector. Weighting lookahead by target-away drift magnitude captured at
  `196.900`, with `6.211L` mean distance, `0.01735` controller-relative
  upstream transport, and `17.672/352.909` force/moment; scheduling `0.08`
  while receding toward `0.07` while closing captured at `213.659` but improved
  score/mean distance to `-3.863/5.856L` and loads to `17.761/354.838`. Their
  sheets preserve the productive wake crossings and remove late reversals,
  while the common `4.293L` lateral-offset and `31.055` anterior-acceleration
  maxima isolate steering timing from propulsion or saturation. Preserve the
  sign gate and sampled bounds; do not repeat constants in `0.07--0.10` or rank
  the two schedules by one metric. Prefer drift magnitude when arrival and
  upstream margin dominate, rolling progress when distance integral and load
  dominate, and test any combination only within their union so strong-drift
  recovery is not weakened. Falsify this rule on lost/later capture, worse
  mean distance, upstream margin below `0.01261`, excursion/load growth, guard
  contact, chatter, or a held-out wake phase. Inherited bearing-divergence and
  heading-damping failures remain distinct negative controls.
