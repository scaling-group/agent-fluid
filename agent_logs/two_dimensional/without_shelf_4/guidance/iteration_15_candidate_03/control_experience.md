...
- Selectively countering target-away lateral translation remains the only
  sampled corridor mechanism to improve the demonstrated `20.25 deg` anchor
  without a load trade, and current results sharpen its useful form from a
  fixed gain to bounded state scheduling. With gait, bearing/rate steering,
  `0.10` lateral-velocity clamp, `4.293L` maximum offset, and `31.055`
  anterior maximum unchanged, fixed `0.07` and `0.08` lookaheads captured at
  `245.449/224.488`, with mean distance `6.305/6.311L`, total effort
  `169649/157454`, and RMS force/moment `18.399/363.454` and
  `17.943/361.014`. Scheduling only inside that interval by target-away drift
  magnitude captured at `196.900`, increased controller-relative upstream
  transport to `0.01735`, and reduced effort and force/moment to
  `136830/17.672/352.909`; scheduling by rolling progress loss instead gave
  the best mean distance and score, `5.856L/-3.863`, while still capturing at
  `213.659` with `17.761/354.838` force/moment. Their sheets retain productive
  wake-band crossings and central approaches, unlike inherited rotational-rate
  additions that produce larger loops or miss. Treat progress loss and
  target-away drift magnitude as complementary, bounded timing signals inside
  the evaluated `0.07--0.08` envelope; do not spend another fixed-gain probe
  there or exceed it toward inherited `0.10`, which delayed capture to
  `263.346` and worsened mean distance to `6.974L`. This is fixed-prewarm-phase
  evidence, not proof that combining the schedules improves them: falsify a
  combination or held-out transfer on later/lost capture, mean distance above
  `5.856L`, upstream transport below `0.01261`, larger excursion, load/effort
  growth, guard contact, or chatter.
