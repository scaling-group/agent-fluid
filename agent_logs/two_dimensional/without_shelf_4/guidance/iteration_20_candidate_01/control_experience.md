...
- Treat `0.015L/time` as the sign-independent local transition-scale anchor for
  the pure rolling closing-speed selector. Four current samples reproduce its
  `210.370` capture, `5.519L` mean distance, `0.01672` controller-relative
  upstream transport, `4.293L` maximum lateral offset, and
  `31.055 rad/time^2` anterior-acceleration maximum. Softening only recession
  to `0.020` crossed earlier at `206.470` and lowered command energy/moment to
  `142752/356.178`, but worsened mean distance, score, and upstream transport
  to `5.943L/-3.956/0.01215`. The complementary inherited test retained
  `0.015` during recession and softened only positive closing to `0.020`; its
  sheet loops on the release side and exits through the top boundary at
  `214.528`, with progress `-0.00953`, mean/final/minimum distance
  `11.062/12.542/7.316L`, upstream displacement only `0.651L`, and lateral
  offset expanded to `6.011L`. Archived matched evaluations reproduce that
  failure exactly while gait extrema remain unchanged. Thus neither earlier
  arrival nor lower load justifies a sign split: preserve `0.015` on both
  signs, and do not spend another certified-phase evaluation on `0.020`
  closing or receding softening. If steering timing is revisited, keep this
  selector intact and isolate a distinct normalized alignment/corridor gate;
  reject it on lost/later capture, worse distance integral, reduced upstream
  margin, larger excursion, switching, guard contact, or load growth. A
  controlled held-out wake phase may falsify this boundary, which otherwise
  applies to the retained gait and `0.07--0.08` envelope.
