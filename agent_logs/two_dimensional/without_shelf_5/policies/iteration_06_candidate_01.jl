function target_policy_params()
    return (
        ...
        bearing_limit=pi / 2,
        steering_gain=1.7,
        bearing_trend_lookahead_fraction=0.25,
        bearing_trend_limit=2.0 * pi / 180,
...
function target_policy(state, params)
    ...
    lookahead_time = params.bearing_trend_lookahead_fraction * params.control_period
    bearing_trend = clamp(
        lookahead_time * state.bearing_window_rate,
        -params.bearing_trend_limit,
        params.bearing_trend_limit,
    )
    steering_bearing = clamp(
        bounded_bearing + bearing_trend,
        -params.bearing_limit,
        params.bearing_limit,
    )
    steering_angle = params.steering_angle_limit * tanh(params.steering_gain * steering_bearing)
