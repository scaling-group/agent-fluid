...
- Positive, bounded body-frame bearing curvature across both joints is the
  ...
  unstable at `2.807`. Across gains `1.7`, `1.725`, and `1.9`, posterior peak
  angle exceeds anterior peak angle by `0.0212`, `0.0297`, and `0.0243` rad,
  while the two higher gains increase both excursions and loads. A separately
  bounded distribution probe may therefore keep gain, total steering command,
  and gait fixed while transferring only a small steering bias between joints;
  credit it only if capture survives and posterior excursion, moment, or effort
  falls without moving the excess to the anterior joint. This is a test
  direction, not an established improvement. Exact replication establishes
  repeatability only for the certified wake phase/start pose; the lesson is
  falsified by loss of the `39.710/1.874L` navigation envelope or larger load,
  and it does not establish robustness to held-out wake phase or geometry.
