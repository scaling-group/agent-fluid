...
- Steering-center allocation is a coupled navigation/load axis, not a static
  ...
  `0.544/0.562` rad. Continuing downward to fraction `0.30` also fails: arrival
  and mean distance regress to `39.286/1.850L`, energy/power to
  `56145.5/4263.2`, crossflow and force/moment to `0.2447/42.06/662.67`, and
  the posterior peak to `0.583` rad. Thus retain `0.35`, avoid either further
  allocation direction, and distrust peak-gap balancing from the quasi-static
  formula. This bracket covers one certified wake phase/start pose and is
  falsified by a materially different repeat or held-out route ordering.
- Posterior phase-lag gain is locally closed at `0.75`, not a smooth
  interpolation axis. Three exact `0.75` repeats capture through the same
  visible self-propelled diagonal corridor at `38.049`, with mean distance
  `1.802L`, energy/power `52895/3965`, crossflow `0.2249`, loads
  `42.01/653.13`, and posterior peak `0.521` rad. Both outer probes regress:
  `0.80` reaches at `38.362` with `1.812L` and `53487/4007`, while `0.70`
  reaches at `39.605` with `1.872L`, `55461/4197`, crossflow `0.2429`, loads
  `43.11/675.66`, and posterior peak `0.570` rad. More decisively, inherited
  quadratic refinements `0.765` and `0.7675` also regress to arrivals
  `38.494/38.412`, mean distances `1.815/1.817L`, and posterior peaks
  `0.523/0.533` rad. Do not fit or test another phase-lag sub-step; retain exact
  `0.75` and move only to a separately bounded response axis. Because the
  anchor still touches both rate and acceleration caps, any such test must
  preserve capture and the route while improving excursion, cap contact, or
  effort without exceeding the anchor's navigation/crossflow/load envelope.
  This conclusion applies only to the certified wake phase/start pose and is
  falsified by a materially different exact repeat or held-out ordering.
