...
- Reducing posterior phase-lag gain from `0.80` to `0.75` at the fraction-`0.35`
  ...
  navigation gain is reproducible. This supports `0.75` only for the certified
  wake phase/start pose and permits at most one isolated equal-size lower-lag
  test while holding gait, bearing law, allocation, and damping fixed; reject
  that continuation if it misses capture, regresses past the `0.80` navigation
  anchors, exceeds the inherited `42.06/662.67` load envelope, or increases
  posterior excursion. It does not establish monotonic response below `0.75`
  or robustness to held-out wake phase or geometry.
