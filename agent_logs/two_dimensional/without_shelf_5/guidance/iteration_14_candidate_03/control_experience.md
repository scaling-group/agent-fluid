...
- Posterior phase-lag gain is sharply bracketed, and neither extrapolation nor
  quadratic interpolation improves on `0.75` at fraction `0.35`. Three current
  `0.75` policies and sheets exactly reproduce the self-propelled diagonal
  capture at `38.049`, mean distance `1.802L`, energy/power `52895.0/3965.3`,
  crossflow `0.2249`, force/moment `42.01/653.13`, and posterior peak `0.521`
  rad. This remains a navigation/effort improvement over `0.80` at
  `38.362/1.812L` and `53487.3/4007.1`, but not an unloading result because
  the `0.80` loads are lower at `40.73/637.79`. The inherited lower-side
  `0.70` test decisively regresses to `39.605/1.872L`, `55461.0/4196.9`,
  `0.2429` crossflow, `43.11/675.66` loads, and `0.570`-rad posterior peak.
  The inherited in-bracket `0.7675` quadratic estimate also regresses to
  `38.412/1.817L`, `53565.6/4023.8`, `0.2268`, `42.78/659.63`, and `0.533`
  rad. Retain exact `0.75`; avoid further fine lag steps, extrapolation, or
  three-point curve fitting, and isolate a separately bounded tail-response
  axis if posterior excursion or loads are addressed. This bracket applies
  only to the certified wake phase/start pose and is falsified by a materially
  different exact repeat or held-out route; any separate-axis test must retain
  capture, stay within the `0.80` navigation/effort bounds, and not worsen the
  `0.75` cap contact, crossflow, or load envelope.
