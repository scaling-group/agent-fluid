...
- Posterior acceleration capping is a replicated navigation/effort axis, not
  an unloading knob, but the latest isolated step does not show the previously
  suspected steepening load cost. Against four inherited `1800 deg/time^2`
  captures (`38.049` arrival, `1.802L` mean distance, `52895/3965`
  energy/power, and `0.2249/42.01/653.13` crossflow/force/moment RMS), `1750`
  improved to `35.750/1.7406L` and `48243/3614` with loads
  `0.2257/47.39/694.77`; three exact `1700` samples improved again to score
  `0.159531`, `34.331/1.7137L`, and `45153/3390`, with loads
  `0.2312/54.48/766.14`. Two exact `1650` samples replicate the safe,
  self-propelled diagonal capture at score `0.189303`, `33.027/1.6831L`, and
  `42310/3183`, with crossflow/loads `0.2358/63.93/859.31`. The current
  single `1600` test preserves the visible route and improves to score
  `0.217528`, `32.202/1.6541L`, and `40126/3010`; it also lowers crossflow to
  `0.2321` and joint excursions from `0.499/0.495` to `0.492/0.474` rad while
  force/moment change only to `64.77/860.25`. Conversely, the isolated `1675`
  test regresses to `33.605/1.6954L` and `43589/3272` despite lower loads,
  confirming that unloading alone is not a navigation gain. Treat exact
  `1600` as the best measured but not yet replicated anchor: repeat it before
  any `1550` extrapolation, change no other axis, and reject the mechanism if
  replication loses the route/capture, regresses beyond exact `1650`, or
  exceeds `0.25` crossflow, `75` force RMS, or `1000` moment RMS. Do not mix
  this test with the inherited failed lower-amplitude, damping, lag,
  allocation, gain, or auxiliary-feedback mechanisms. This applies only to
  the certified wake phase/start pose and is falsified by materially different
  exact-repeat behavior or held-out wake/geometry evidence.
