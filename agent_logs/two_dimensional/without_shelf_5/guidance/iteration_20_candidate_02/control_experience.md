...
- Posterior damping is now closed on both sides of the replicated `0.65`
  ...
  not infer persistent bang-bang merely from recorded maxima. Every damping
  variant touches the posterior acceleration envelope; the isolated
  output-bound results below now supersede the proposed cap-sensitivity test.
  This bracket applies only to the certified wake phase/start pose and is
  falsified if exact repeats materially depart from the recorded tradeoffs.
- Posterior acceleration capping is a reproducible navigation/effort axis, not
  an unloading knob, and the former `1700 deg/time^2` stopping rule is
  superseded by the isolated continuation evidence. From inherited `1800`,
  `1750`, and three exact `1700` captures, tightening improved arrival/mean
  distance and energy/power from `38.049/1.802L, 52895/3965` through
  `35.750/1.741L, 48243/3614` to `34.331/1.714L, 45153/3390`, while
  crossflow and force/moment RMS rose from `0.2249/42.01/653.13` to
  `0.2312/54.48/766.14`. Two exact current `1650` samples preserve the same
  visible self-propelled diagonal route and improve again to score `0.189303`,
  `33.027/1.683L`, and `42310/3183`, with `0.2358/63.93/859.31` crossflow and
  loads. The single current `1600` sample advances the same route to score
  `0.217528`, `32.202/1.654L`, and `40126/3010`; unlike earlier steps,
  crossflow falls to `0.2321`, moment is nearly flat at `860.25`, force rises
  only to `64.77`, and posterior excursion falls from `0.495` to `0.474` rad.
  All variants actively touch their posterior cap and the episode's anterior
  acceleration envelope, so do not interpret the gain as removed saturation.
  Retain exact `1600` as the measured anchor and permit at most one isolated
  equal cap step before demanding new evidence; do not combine it with weaker
  amplitude or auxiliary feedback, whose inherited tests regressed or became
  unstable. Reject further tightening if it loses the route/capture, fails to
  improve navigation or effort, raises posterior excursion above `0.50` rad,
  or exceeds `0.25/75/1000` crossflow/force/moment RMS. The `1600` result is
  not yet independently replicated and all results apply only to the certified
  wake phase/start pose, not held-out wake or geometry conditions.
