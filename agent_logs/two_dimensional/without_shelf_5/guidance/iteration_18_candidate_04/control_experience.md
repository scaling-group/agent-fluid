...
- A posterior acceleration output bound is a reproducible navigation/effort
  axis with an opposing wake-load cost, not an unloading knob. The inherited
  exact `1800 deg/time²` envelope captures reached at `38.049` with `1.802L`
  mean distance, `52895/3965` energy/power, `0.2249` relative crossflow,
  `42.01/653.13` force/moment RMS, and `0.521`-rad posterior excursion. The
  assigned `1750` parent preserves the same visible self-propelled diagonal
  route while improving those navigation/effort measures to
  `35.750/1.741L` and `48243/3614`; crossflow and loads rise to
  `0.2257/47.39/694.77`, while excursion falls to `0.500` rad. Three current
  `1700` candidates, released sheets, and physical metrics are byte-identical:
  the same safe route advances to `34.331/1.714L` and `45153/3390`, with an
  active `29.671 rad/time²` ceiling and `0.509`-rad excursion, but crossflow
  and loads rise again to `0.2312/54.48/766.14`. Replication therefore clears
  the prior repeat-before-continuation boundary and establishes a local
  `1800 -> 1750 -> 1700` direction for earlier, lower-effort capture, while
  the non-monotone excursion and monotone load growth forbid a general
  tighter-is-better or load-relief claim. The inherited 27-degree amplitude
  reduction is a concrete counterexample to weakening propulsion instead: its
  available sheet and controller regress to `38.214/1.812L` and `53078/3978`
  despite lower `40.03/630.70` loads. Keep gait, lag, damping, bearing law,
  allocation, and observations fixed; at most test one further equal cap step,
  and reject it if the route/capture changes, navigation or effort fails to
  improve, posterior excursion exceeds `0.521` rad, or crossflow and loads grow
  disproportionately beyond `0.240/65/850`. This local sequence applies only
  to the certified wake phase/start pose and is falsified by a materially
  different exact repeat or held-out trajectory.
