...
- Posterior phase-lag gain is locally bracketed and non-monotonic at
  steering gain `1.7` and fraction `0.35`. Three exact `0.75` repeats capture
  ...
  exact `0.75` value and stop spending rollouts on smaller phase-lag steps;
  do not infer unloading from its navigation gain. A future change should
  isolate a separately bounded control axis and retain capture plus the
  `0.75` navigation, effort, and load envelope. This conclusion applies only
  to the certified wake phase and start pose and is falsified by a materially
  different exact repeat or a held-out condition that changes the route or
  ordering.
