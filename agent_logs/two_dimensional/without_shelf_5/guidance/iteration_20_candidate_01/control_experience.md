...
- A posterior acceleration output bound is a replicated navigation/effort
  axis, not an unloading knob. Against four exact `1800 deg/time²`-envelope
  captures (`38.049` arrival, `1.802L` mean distance, `52895/3965`
  energy/power, `0.2249` relative crossflow, `42.01/653.13` force/moment RMS,
  and `0.521`-rad posterior excursion), assigned-parent and inherited results
  progress through `1750` and `1700`, while current sampled `1675`, two exact
  `1650` replicas, and `1600` preserve the same visible safe, self-propelled
  turn-then-diagonal capture. The current sequence improves
  arrival/mean-distance/energy/power from `33.605/1.695L/43589/3272` at `1675`
  to `33.027/1.683L/42310/3183` at `1650` and
  `32.202/1.654L/40126/3010` at `1600`; posterior excursion falls from
  `0.501` to `0.495` to `0.474` rad, and only `1600` avoids the posterior rate
  envelope. Useful leftward mean body speed also rises from `0.324` to `0.338`
  while local leftward flow remains near `0.18`, confirming active propulsion
  rather than advection. The load tradeoff is bounded but persists:
  crossflow/force/moment change from `0.2356/57.87/797.09` through
  `0.2358/63.93/859.31` to `0.2321/64.77/860.25`, so `1600` plateaus rather
  than relieves loads. Retain `1600` as the measured anchor and permit only one
  isolated equal-step `1550` boundary probe with all other axes fixed; reject
  continuation if it loses the route or capture, fails to improve navigation
  or effort, exceeds `0.521`-rad posterior excursion, or crosses
  `0.25/75/1000` crossflow/force/moment RMS. Do not combine the test with the
  inherited weaker-gait regression or auxiliary feedback that became unstable
  at `2.807`. The `1600` result is still a single evaluation: this lesson
