...
- Increasing posterior damping from `0.65` to `0.675` is a concrete negative
  ...
  recorded tradeoff.
- A small posterior acceleration ceiling is a distinct navigation/effort versus
  load tradeoff, not the unloading continuation suggested by the damping
  bracket. The assigned-parent hypothesis capped only raw posterior command at
  `1700 deg/time^2`, `5.6%` below the repeatedly contacted `1800` episode
  envelope. The sampled result preserves the visible self-propelled
  turn-then-diagonal capture and improves arrival/mean distance from the three
  exact damping-`0.65` anchors' `38.049/1.802L` to `34.331/1.714L`, mean
  velocity from `(-0.2853,-0.1198)` to `(-0.3164,-0.1322)`, energy/power from
  `52895/3965` to `45153/3390`, and posterior peak from `0.521` to `0.509` rad.
  However, relative crossflow rises from `0.2249` to `0.2312` and force/moment
  RMS jumps from `42.01/653.13` to `54.48/766.14`, exceeding even the failed
  damping-`0.625` load trade's `47.26/710.38`. Thus do not call posterior cap
  relief an all-metric improvement or continue to a stronger cap. At most one
  isolated midpoint toward the anchor is justified to test whether navigation
  and effort gains survive while loads move materially back toward the anchor;
  reject it if the route or capture changes, navigation is no better than the
  anchor, or loads fail to improve on `54.48/766.14`. This single rollout is
  specific to the certified wake phase/start pose and is falsified as a
  repeatable mechanism if an exact `1700` repeat materially departs from its
  recorded route, arrival, effort, excursion, or load envelope.
