...
- Posterior acceleration capping is a locally monotone navigation/effort axis
  through the sampled `1600 deg/time²` boundary, but it is not an unloading
  knob. Four inherited `1800` captures reached at `38.049` with `1.802L` mean
  distance, `52895/3965` energy/power, `0.2249` relative crossflow,
  `42.01/653.13` force/moment RMS, and `0.521`-rad posterior excursion; four
  `1700` records preserved the visible safe, self-propelled diagonal route and
  improved those measures to `34.331/1.714L`, `45153/3390`, and `0.509` rad,
  while loads rose to `0.2312/54.48/766.14`. The two exact `1650` policies
  improved again to score `0.189303`, `33.027/1.683L`, `42310/3183`, and
  `0.495` rad at `0.2358/63.93/859.31` crossflow/loads; the sampled interior
  `1675` result falls between them at score `0.177318`, `33.605/1.695L`, and
  `43589/3272`, supporting the direction rather than a fitted optimum. Most
  importantly, the current sampled `1600` rollout falsifies the assigned
  parent's predicted load rejection: it visibly keeps the route, improves to
  score `0.217528`, `32.202/1.654L`, `40126/3010`, and `0.474` rad, while
  crossflow falls to `0.2321` and force/moment remain nearly flat at
  `64.77/860.25`. The posterior cap remains active at every setting. Retain
  `1600` as the measured anchor and permit at most one isolated equal-step
  lower-bound probe with gait, lag, damping, bearing law, allocation, and
  observations fixed; do not weaken propulsion or add auxiliary load feedback,
  because inherited amplitude weakening regressed and mixed feedback became
  unstable at `2.807`. Reject continuation if it loses the visible route or
  capture, fails to improve navigation/effort, raises posterior excursion above
  `0.495` rad, or exceeds `0.25/75/1000` crossflow/force/moment RMS. The lone
  `1600` sample is not repeatability or robustness evidence; this local lesson
  applies only to the certified wake phase/start pose and is falsified by a
  materially different exact repeat or held-out trajectory.
