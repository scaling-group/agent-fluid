...
- Increasing posterior damping from `0.65` to `0.675` is a concrete negative
  control result even though it lowers RMS loads. Against the four exact
  damping-`0.65` anchors (`38.049` arrival, `1.802L` mean distance,
  `52895/3965` energy/power, and mean velocity `(-0.2853,-0.1198)`), the
  assigned-parent `0.675` rollout visibly falls behind on the same safe route
  and regresses to `41.591`, `1.901L`, `58328/4400`, and
  `(-0.2611,-0.1095)`. Force/moment RMS fall from `42.01/653.13` to
  `40.10/637.97`, but crossflow rises from `0.2249` to `0.2279`, joint peaks
  rise from `0.496/0.521` to `0.508/0.551` rad, and both variants touch the
  same rate/acceleration caps. Therefore do not use stronger posterior damping
  as an excursion- or saturation-relief continuation and do not accept lower
  RMS loads alone when active traverse and command effort regress. At most one
  isolated opposite-direction damping probe is justified while the measured
  gait, lag, bearing law, and allocation stay fixed; reject it if capture or
  route changes, navigation regresses past the lag-`0.80` anchors, or
  crossflow/load/excursion exceeds the inherited lag-`0.70` failed boundary.
  This one-sided conclusion applies only to the certified wake phase/start pose
  and is falsified if an exact `0.675` repeat materially departs from its
  recorded tradeoff.
- Posterior acceleration shaping is now a measured navigation/effort-versus-load
  bracket, not an unloading mechanism or an axis for further fine interpolation.
  Against the uncapped damping-`0.65` anchor (`38.049/1.802L` arrival/mean
  distance, `52895/3965` energy/power, `0.2249` crossflow, `42.01/653.13`
  force/moment RMS, and `0.496/0.521`-rad joint peaks), three exact current
  `1700 deg/time^2` posterior-cap replicas preserve the visible self-propelled
  turn-then-diagonal capture and improve navigation/effort to `34.331/1.714L`
  and `45153/3390`, but raise crossflow and loads to
  `0.2312/54.48/766.14`; their joint peaks are `0.497/0.509` rad and the
  anterior acceleration still contacts the `1800` episode envelope. The
  assigned parent's isolated `1750` midpoint also preserves the route and
  lands between those endpoints: `35.750/1.741L`, `48243/3614`,
  `0.2257/47.39/694.77`, and `0.491/0.500` rad. Thus tightening the posterior
  cap across `1800 -> 1750 -> 1700` buys faster, lower-effort traverse at the
  price of increasing lateral loads; retain `1700` as the replicated
  navigation/score anchor and `1750` as a load-aware comparator, but do not
  spend another candidate on a fitted posterior-cap sub-step or claim either
  is an all-metric optimum. The inherited 27-degree oscillator test is not a
  substitute for isolating the remaining anterior contact: it lowered loads
  to `40.03/630.70` but regressed to `38.214/1.812L`, left both acceleration
  caps active, and raised posterior excursion to `0.527` rad. A separately
  bounded anterior-command test is falsified if it cannot reduce the `1700`
  anchor's loads while remaining better than the `1750` comparator on
  navigation and effort. This bracket applies only to the certified wake
  phase/start pose and is falsified if another exact cap repeat materially
  departs from its recorded route or metric envelope.
