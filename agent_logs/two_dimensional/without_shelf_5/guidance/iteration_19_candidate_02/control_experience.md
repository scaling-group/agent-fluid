...
- Posterior acceleration capping is a replicated navigation/effort axis, not
  an unloading knob, and its load cost steepens as the ceiling falls. Against
  four inherited `1800 deg/time^2` captures (`38.049` arrival, `1.802L` mean
  distance, `52895/3965` energy/power, `0.2249` relative crossflow, and
  `42.01/653.13` force/moment RMS), the assigned-parent `1750` result improves
  to `35.750/1.7406L` and `48243/3614` while loads rise to
  `0.2257/47.39/694.77`; three exact `1700` samples improve again to score
  `0.159531`, `34.331/1.7137L`, and `45153/3390`, with loads rising to
  `0.2312/54.48/766.14`. Two current exact `1650` samples now falsify the
  earlier stop-at-`1700` boundary: their policies, released sheets, and
  physical metrics replicate the same safe self-propelled diagonal capture at
  score `0.189303`, `33.027/1.6831L`, and `42310/3183`, while posterior
  excursion falls from `0.509` to `0.495` rad. But crossflow and force/moment
  RMS rise again to `0.2358/63.93/859.31`, and the anterior acceleration plus
  both joint-rate envelopes remain touched. Retain exact `1650` as the measured
  anchor. Any tighter boundary test must change no other axis and must be
  rejected if it loses the route/capture, fails to improve navigation/effort,
  or exceeds `0.25` crossflow, `75` force RMS, or `1000` moment RMS; do not
  combine it with the inherited failed lower-amplitude, damping, lag,
