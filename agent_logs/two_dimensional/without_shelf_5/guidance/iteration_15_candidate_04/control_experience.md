...
- Posterior phase-lag gain is locally bracketed and non-monotonic at
  steering gain `1.7` and fraction `0.35`. Four sampled exact `0.75` policies
  have byte-identical keyframes and metrics: each captures at `38.049` with
  mean distance `1.802L`, command energy/power
  `52895/3965`, and crossflow/force/moment RMS `0.2249/42.01/653.13`, modestly
  improving the `0.80` result at `38.362`, `1.812L`, and `53487/4007` while
  accepting slightly higher loads. Both attempts to continue or interpolate
  that change fail. At `0.70`, capture slows to `39.605`, mean distance rises
  to `1.872L`, energy/power to `55461/4197`, crossflow/loads to
  `0.2429/43.11/675.66`, posterior peak angle to `0.570` rad, and both joints
  still touch the rate and acceleration caps. More decisively, the assigned
  parent's interior `0.7675` fit preserves the route but regresses to
  `38.412/1.817L`, `53566/4024`, and `0.2268/42.78/659.63`, worse than `0.75`
  on every claimed improvement and even later/farther than `0.80`. Promote the
  exact `0.75` value and stop spending rollouts on smaller phase-lag steps or
  further same-condition replications; the fourth exact result strengthens
  deterministic-repeatability evidence but adds no robustness evidence. Do
  not infer unloading from its navigation gain: the embedded diagnostics put
  both joints exactly on the `260 deg/time` rate and `1800 deg/time^2`
  acceleration caps, while force/moment remain `42.01/653.13`. A future change
  should isolate a separately bounded tail-response or propulsion axis and
  retain capture plus the `0.75` navigation, effort, and load envelope rather
  than combine another lag fit with a new signal. This conclusion applies
  only to the certified wake phase and start pose and is falsified by a
  materially different exact repeat or a held-out condition that changes the
  route or ordering.
