...
- Reducing posterior phase-lag gain from `0.80` to `0.75` at the fraction-`0.35`
  anchor is a partial navigation/effort improvement, not an unloading result.
  Relative to three exact `0.80` replicas, the single `0.75` sample preserves
  the visible self-propelled diagonal capture and improves arrival from
  `38.362` to `38.049`, mean distance from `1.812L` to `1.802L`, energy from
  `53487.3` to `52895.0`, power from `4007.1` to `3965.3`, and mean velocity
  from `(-0.2829,-0.1181)` to `(-0.2853,-0.1198)`. However, posterior peak
  angle remains `0.521` rad, both rate/acceleration caps are still touched,
  and force/moment RMS rise from `40.73/637.79` to `42.01/653.13`; therefore
  the inherited tail-relief hypothesis is falsified. Replicate `0.75` before
  promotion, reject it if navigation/effort gains do not repeat or loads exceed
  its measured envelope, and do not extrapolate to a lower lag until separate
  evidence establishes a safe response. These bounds are specific to the
  certified wake phase/start pose.
