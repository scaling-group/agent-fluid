...
- At the fraction-`0.35` anchor, reducing only posterior `tail_lag_gain` from
  `0.80` to `0.75` preserved the visible self-propelled turn, merged-wake entry,
  and safe capture while improving arrival `38.362 -> 38.049`, mean distance
  `1.812 -> 1.802L`, energy `53487 -> 52895`, power `4007 -> 3965`, and mean
  velocity `(-0.2829,-0.1181) -> (-0.2853,-0.1198)`. Three independently
  sampled `0.75` rollouts now reproduce the policy, released sheet, and all
  reported metrics exactly, so treat `0.75` as a deterministic
  navigation/effort anchor and stop spending candidates on replication.
  Crossflow remains nearly unchanged (`0.2244 -> 0.2249`), posterior peak
  stays `0.5208` rad, and force/moment rise about `3.1%/2.4%` to
  `42.01/653.13`; the result still falsifies unloading and does not establish
  monotonicity below `0.75`. Only an isolated, equally bounded lower-lag test
  is supported: reject continuation on lost capture, arrival no earlier than
  `38.049`, mean distance or effort regression, enlarged posterior excursion,
  or material growth beyond that crossflow/load envelope. All numeric gain,
  allocation, and tail-lag conclusions remain limited to the certified wake
