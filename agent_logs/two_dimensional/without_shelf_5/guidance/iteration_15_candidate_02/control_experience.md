...
- Posterior phase-lag gain is locally bracketed and non-monotonic at
  steering gain `1.7` and fraction `0.35`. Four assigned `0.75` samples have
  byte-identical policies, released keyframes, and metrics: they capture
  at `38.049` with mean distance `1.802L`, command energy/power
  ...
  accepting slightly higher loads. Continuing downward to `0.70` decisively
  reverses the gain: capture slows to `39.605`, mean distance rises to
  `1.872L`, energy/power to `55461/4197`, crossflow/loads to
  `0.2429/43.11/675.66`, posterior peak angle to `0.570` rad, and both joints
  still touch the rate and acceleration caps. Do not reduce phase lag below
  `0.75`, infer unloading from the `0.80 -> 0.75` navigation improvement, or
  spend another candidate on exact `0.75` promotion or a tiny phase-lag
  interpolation. The fourth exact result strengthens deterministic
  repeatability under the certified snapshot but supplies no new local
  gradient. Instead test a separately bounded, orthogonal posterior-response
  axis (or a held-out condition) and reject it if capture or the `0.75`
  navigation/effort/load envelope is lost. This
  bracket applies only to the certified wake phase and start pose and is
  falsified by a materially different exact repeat or a held-out condition
  that changes the route or ordering.
