...
- Positive, bounded body-frame bearing curvature across both joints is the
  ...
  A separately falsifiable next axis is to redistribute the unchanged steering
  center between joints, since the replicated anchor's posterior excursion is
  larger, and test whether that reduces posterior excursion and loads without
  degrading capture or arrival. Do not confound this with slower propulsion or
  unscaled velocity/moment terms: the inherited mixed-feedback slow controller
  became unstable at `2.807`. The numeric anchor and allocation implication are
