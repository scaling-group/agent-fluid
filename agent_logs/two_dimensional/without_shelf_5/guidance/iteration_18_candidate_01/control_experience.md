...
- Posterior acceleration capping is a replicated navigation/effort axis, not
  an unloading knob. Against four inherited `1800 deg/time^2` captures
  (`38.049` arrival, `1.802L` mean distance, `52895/3965` energy/power,
  `0.2249` relative crossflow, `42.01/653.13` force/moment RMS, and `0.521`-rad
  posterior excursion), the assigned-parent `1750` prefill preserves the
  visible active diagonal route while improving to score `0.133441`,
  `35.750/1.7406L`, and `48243/3614`; crossflow and loads rise to
  `0.2257/47.39/694.77`, though posterior excursion falls to `0.500` rad.
  Three current exact `1700` samples then produce byte-identical released
  sheets and metrics apart from wall time: the same safe self-propelled route
  reaches at `34.331` with score `0.159531`, mean distance `1.7137L`, and
  energy/power `45153/3390`, but crossflow and loads rise again to
  `0.2312/54.48/766.14` and posterior excursion rebounds to `0.509` rad. Both
  bounded policies still touch their posterior bound and the episode's
  anterior acceleration and joint-rate envelopes. Retain exact `1700` as the
  measured score/navigation/effort anchor, but do not continue to a tighter or
  interpolated cap: the load penalty is monotone and excursion is not. Reject
  it if an exact repeat loses the route/capture, regresses beyond the `1750`
  navigation/effort envelope, or materially exceeds the recorded `1700`
  crossflow/load envelope. This applies only to the certified wake phase/start
  pose and establishes no held-out wake or geometry robustness.
