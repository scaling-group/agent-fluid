...
- Posterior acceleration-envelope relief is not interchangeable with weaker
  propulsion or stronger damping. Against three exact uncapped `28`-degree
  replicas (`0.073801` score, `38.049/1.802L` arrival/mean distance,
  `52895/3965` energy/power, `0.496/0.521`-rad peaks, and `42.01/653.13`
  force/moment RMS), the isolated `1700 deg/time^2` posterior ceiling preserves
  the visible safe route while improving score to `0.159531`, arrival/mean
  distance to `34.331/1.714L`, energy/power to `45153/3390`, and posterior peak
  to `0.509` rad. Its active `29.671 rad/time^2` posterior limit confirms that
  the new envelope is causal at the command boundary, but crossflow and
  force/moment worsen to `0.2312/54.48/766.14`; treat `1700` as a measured
  navigation/effort anchor, not an unloading result. The assigned parent's
  `27`-degree amplitude reduction instead regresses score, arrival, mean
  distance, and effort to `0.063788`, `38.214`, `1.812L`, and `53078/3978`
  despite lowering loads, while the inherited higher-damping side likewise
  trades away traverse. Retain the `28`-degree gait with the exact `1700`
  posterior ceiling; do not combine it with amplitude reduction or claim that
  lower force/moment alone indicates better control. The ceiling currently has
  one independent CFD outcome and is falsified if an exact certified-condition
  repeat loses capture or materially departs from its arrival, distance,
  effort, posterior-excursion, or load envelope; it does not establish
  robustness to held-out wake phase or geometry.
