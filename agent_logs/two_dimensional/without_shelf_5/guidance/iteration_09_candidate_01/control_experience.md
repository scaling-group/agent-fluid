...
- Treat the anterior share of a fixed total steering center as a separate,
  wake-coupled distribution axis, not as a peak-angle balancing knob. With the
  `0.55`-period, 28-degree gait, gain `1.7`, and 12-degree total bound fixed,
  fractions `0.45`, `0.40`, and `0.35` improved arrival monotonically
  (`43.323`, `39.710`, `38.362`) and mean distance (`2.025L`, `1.874L`,
  `1.812L`) as the anterior share fell. The `0.35` policy also reduced total
  command energy, power, crossflow RMS, and both peak joint angles relative to
  `0.40`, but force/moment RMS rose from `38.40/618.59` to `40.73/637.79`;
  the failed `0.45` redistribution raised both excursions and loads. Therefore
  test only one bounded decrement at a time and require preserved capture plus
  a navigation gain without material load-envelope regression; neither more
  balanced joint peaks nor lower command effort alone establishes improvement.
  This direction is specific to the certified wake phase/start pose and is
  falsified if a repeat breaks the arrival ordering or a changed phase reverses
  it.
