...
- Posterior phase lag is bracketed by a reproducible interior optimum at the
  tested resolution; do not extrapolate it again. At the fraction-`0.35`,
  gain-`1.7` anchor, three exact `tail_lag_gain=0.75` samples preserve the
  visible self-propelled diagonal capture and improve on `0.80` in arrival
  (`38.049` versus `38.362`), mean distance (`1.802L` versus `1.812L`),
  command energy (`52895` versus `53487`), and power (`3965` versus `4007`).
  The inherited `0.70` bracket extension retains capture and the broad route
  but reverses every navigation/effort gain: arrival `39.605`, mean distance
  `1.872L`, energy/power `55461/4197`, crossflow `0.2429`, force/moment
  `43.11/675.66`, and posterior peak `0.570` rad versus the `0.75` anchor's
  `0.2249`, `42.01/653.13`, and `0.521` rad; all three lag settings touch the
  same rate/acceleration caps. Thus retain exact `0.75` when testing a separate
  bounded axis and distrust phase-lag changes as an unloading mechanism. This
  conclusion applies to the certified wake phase/start pose and is falsified
  by failure of another exact `0.75` replicate or a held-out condition that
  reverses the bracket ordering.
