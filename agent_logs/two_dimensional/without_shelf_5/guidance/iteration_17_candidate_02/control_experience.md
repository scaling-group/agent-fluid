...
- A posterior acceleration output bound is a navigation/effort axis, not an
  unloading knob. Against four exact unbounded-policy captures whose episode
  limiter allowed `1800 deg/time²` (`38.049` arrival, `1.802L` mean distance,
  `52895/3965` energy/power, `0.2249` relative crossflow, `42.01/653.13`
  force/moment RMS, and `0.521`-rad posterior excursion), the single sampled
  `1700 deg/time²` bound visibly preserves the safe self-propelled diagonal
  route while improving arrival/mean distance to `34.331/1.714L`, reducing
  energy/power to `45153/3390`, and reducing posterior excursion to `0.509`
  rad. It simultaneously raises crossflow to `0.2312` and force/moment RMS to
  `54.48/766.14`, so do not claim load relief or continue to a tighter or
  interpolated cap from the scalar score. A separate inherited step-16
  score-only capture at `38.214/1.812L` and `40.03/630.70` force/moment RMS
  cannot bracket cap response because its materialized controller and visual
  evidence are unavailable here. Replicate exact `1700` with every other
  controller axis fixed before any continuation; retain it only if the same
  route and capture recur with lower effort and excursion, and reject it if
  navigation regresses beyond `38.049/1.802L` or crossflow and loads materially
  exceed its recorded `0.2312/54.48/766.14` envelope. This applies only to the
  certified wake phase/start pose and is falsified by a materially different
  exact repeat or held-out trajectory.
