...
- Posterior phase-lag gain is locally bracketed and non-monotonic at
  ...
  `0.75` or infer unloading from the `0.80 -> 0.75` navigation improvement;
  if this axis is revisited, isolate a refinement inside `[0.75,0.80]` and
  require capture plus improved navigation without exceeding the `0.75`
  effort/load envelope. This bracket applies only to the certified wake phase
  and start pose and is falsified by a materially different exact repeat or a
  held-out condition that changes the route or ordering.
