...
- Do not infer that a static anterior/posterior steering-center reallocation
  will balance joint excursion. At fixed gain `1.7`, 12-degree bound, and
  propulsive gait, increasing the anterior fraction from `0.40` to `0.45`
  preserved the visible diagonal route and capture but delayed arrival from
  `39.710` to `43.323`, raised mean distance from `1.874L` to `2.025L`, total
  command energy from `54703.2` to `60174.8`, crossflow RMS from `0.2265` to
  `0.2456`, and force/moment RMS from `38.40/618.59` to `41.96/709.54`.
  Contrary to the balancing hypothesis, both joint maxima increased from
  `0.507/0.528` to `0.544/0.562` rad while the same rate and acceleration caps
  were reached. Retain the measured `0.40` distribution and avoid another
  small allocation interpolation until a separately motivated axis is tested.
  This negative result is limited to the certified wake phase and start pose;
  it is falsified if a held-out or repeated `0.45` rollout lowers posterior
  excursion or load without losing the `0.40` arrival and distance envelope.
