...
- Common-factor acceleration projection is a negative actuator-allocation
  result for this clipped two-joint carrier. Gating an L-infinity projection
  to aligned bearing convergence preserved target success, but slowed capture
  from its response-release parent's `39.1104` to `39.4569`, increased mean
  distance from `1.91533L` to `1.91664L`, enlarged joint excursions from
  `0.543/0.464` to `0.575/0.540` rad, and raised RMS force/moment from
  `54.19/754.87` to `94.55/1093.63`; both rate and acceleration caps remained
  reached. It is also dominated in load by the current sampled history-only
  release (`51.63/734.35`) and composite release (`57.05/783.02`). Preserving
  the raw command pair's sign and ratio before downstream clipping therefore
  does not preserve the realized traveling bend. Do not tune the projection
  limit or alignment threshold; revisit coordinated constraint handling only
  if observed joint-response or signed load history can close the loop, and
  falsify this boundary only if that feedback improves loads without delaying
  direct capture.
