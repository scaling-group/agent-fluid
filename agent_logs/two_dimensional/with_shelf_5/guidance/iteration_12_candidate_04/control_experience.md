...
- On the demonstrated bearing-biased carrier, moving posterior steering onto
  ...
  feedback primitive.
- For the direct half-cycle carrier, three identical approach-envelope samples
  reached in `43.9505` with RMS crossflow/force/moment
  `0.2111/49.44/701.26`. Adding joint-rate phase lead preserved the route but
  worsened those values to `0.2136/53.18/736.58`, while an inherited raw
  yaw-moment residual delayed capture to `45.1935` and raised force/moment to
  `71.51/957.29`; do not infer a useful fast-feedback sign from either signal.
  The assigned parent's alignment-gated outward posterior-rate projection is
  the supported contrast: it still captured directly at `44.0220` and reduced
  crossflow/force/moment to `0.2102/44.86/663.89`, despite continued cap
  contact. Treat that projection as distributed load shaping rather than peak
  avoidance. A later terminal localization was byte-distinct but reproduced
  the unguarded carrier's `43.9505` arrival, `2.1391L` mean distance,
  `53082.57` command energy, and `0.2111/49.44/701.26` loads to printed
  precision: restricting the rate projection to the `2.5L` approach erased
  its measurable intervention. Do not tune that terminal gate or its rate
  threshold further. Preserve the direct carrier and test a different
  actuator-feedback primitive that is active after body-frame alignment, with
  target success and reduced load or effort as joint acceptance criteria.
  This boundary applies to this half-cycle carrier and is overturned only if a
  materially different semantic gate produces nonzero action changes and
  recovers load relief without delaying the direct route.
