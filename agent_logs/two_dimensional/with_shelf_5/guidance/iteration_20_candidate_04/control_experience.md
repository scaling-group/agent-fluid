...
- Short-history target filtering is a route primitive, not a demonstrated load
  ...
  to `54.19/754.87` and slightly lowering effort, but raised relative crossflow
  from `0.21935` to `0.21986`. The completed missing factorial shows a stronger,
  non-additive result: releasing only stale anterior route history preserves
  the `39.1159`, `1.9143L` direct route and lowers crossflow/force/moment to
  `0.21787/51.63/734.35`; releasing both history and the posterior boost arrives
  slightly sooner at `39.0499` with lower command energy, but returns loads to
  `57.05/783.02`. Thus small-bearing response completion is validated as a role
  selector, not a generic relief knob: route-memory release is the supported
  load choice, while stacking posterior release trades that load benefit for a
  small route/effort gain. Preserve all components during large-error redirect,
  and do not assume independently plausible releases compose monotonically.
  If extending the route-only mechanism, test a bounded current-response
  predictor and falsify it against its direct-arrival and `51.63/734.35` load
  class. Do not reuse lagged history to trigger a fast guard or tune a blanket
  outward-rate projection: sampled history-gated guards retained the `63/872`
  load class, and inherited total-action projection evidence delayed the route.
  If response prediction fails, require calibrated flow/load histories before
  testing a disturbance residual.
