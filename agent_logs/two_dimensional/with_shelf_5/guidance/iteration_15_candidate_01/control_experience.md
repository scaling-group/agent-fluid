...
- Short-history bearing filtering is a route-speed mechanism, not evidenced
  load rejection.  Blending the current bearing halfway toward its circular
  eight-sample body-frame history improved direct capture from `43.9505` to
  `41.5030` and mean distance from `2.1391L` to `2.0216L`, but raised RMS
  crossflow/force/moment from `0.2111/49.44/701.26` to
  `0.2246/61.80/862.48`, command energy/power proxy from `53082.6/3944.0` to
  `53802.7/4074.6`, and maximum joint excursions from `0.525/0.452` to
  `0.579/0.527` rad while the same rate and acceleration caps remained active.
  Do not tune the blend or history length expecting load relief.  If retaining
  its faster route, first test composition with an independently evidenced
  response guard: on the unfiltered carrier, alignment-gated removal of only
  near-cap outward acceleration preserved the exact `43.9505` capture and
  lowered force/moment to `44.47/657.47`.  That composition is not yet CFD
  evidence.  This lesson applies to the padded circular-history, half-cycle
  carrier and is falsified if a held-out history filter improves both arrival
  and loads, or if the directional guard changes reversals or the direct route.
