...
- On the demonstrated bearing-biased carrier, moving posterior steering onto
  ...
  feedback primitive.
- On the guarded direct carrier, adding a circular mean of recent body-frame
  bearing to both anterior curvature and posterior half-cycle steering improved
  capture from `43.9505` to `41.4205` and mean distance from `2.1372L` to
  `2.0142L`, but increased joint excursions and RMS force/moment from
  `44.47/657.47` to `63.05/872.36`. In the matched unguarded history variants,
  keeping that persistent bearing only for anterior route curvature while
  returning the posterior half-cycle to current bearing further improved
  capture from `41.5030` to `40.6285`, mean distance from `2.0216L` to
  `1.9759L`, and mean command from `1296.36` to `1289.10`, with similarly high
  `63.65/868.82` loads. Therefore treat history/current separation as a
  two-timescale navigation mechanism, not as wake-load rejection: persistent
  geometry may steer the slow route while current error releases fast posterior
  asymmetry. This applies when a compact direct route already exists and is
  falsified in a guarded combination if arrival or mean distance does not beat
  the fully history-driven guarded carrier, or if loads rise without a useful
