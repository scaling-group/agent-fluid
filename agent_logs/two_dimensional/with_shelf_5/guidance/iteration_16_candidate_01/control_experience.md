...
- The latest samples resolve the route/load roles of short bearing history more
  sharply. On the matched unguarded history-filtered carrier, moving only
  posterior half-cycle steering onto current bearing improved arrival from
  `41.5030` to `40.6285`, mean distance from `2.0216L` to `1.9759L`, and command
  energy from `53802.7` to `52374.2`; RMS force/moment instead rose from
  `61.80/862.48` to `63.65/868.82`. The slow/fast sensory split is therefore
  isolated positive route/effort evidence, not wake-load rejection. Separately,
  adding the directional rate projection to the fully history-driven carrier
  retained its fast route (`41.5030` to `41.4205`) and reduced energy to
  `53377.7`, but raised RMS force/moment to `63.05/872.36`; it also is not
  evidenced as load relief on that carrier. Their composition remains untested:
  retain history for mean curvature and guard alignment while using current
  bearing only for posterior asymmetry, then falsify composition if it fails to
  beat `41.4205` or improve load/effort over the unguarded split. If falsified,
  do not tune history, half-cycle, or guard thresholds; require a distinct,
  sign-calibrated observation mechanism.
