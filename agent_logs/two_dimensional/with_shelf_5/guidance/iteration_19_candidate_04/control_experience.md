...
- Semantic coherence and response completion improved that timescale-split
  carrier in two mechanism-level steps.  Withholding current-bearing posterior
  requests when their sign contradicted the persistent body-frame route
  shortened capture from `40.6285` to `39.1104`, reduced mean distance from
  `1.9759L` to `1.9149L`, and reduced RMS force/moment from `63.65/868.82` to
  `57.21/783.64`; command energy fell from `52374` to `50061`.  Releasing only
  the extra half-cycle asymmetry after observed bearing-window convergence then
  retained the `39.1104` capture and `1.9153L` mean distance while further
  reducing force/moment to `54.19/754.87` and command energy to `50045`.
  Preserve both selectors on this circular-history carrier: padded or
  large/diverging error must retain the decisive redirect, while current
  response may withhold contradictory steering and release auxiliary
  asymmetry.  Both actuator caps and joint excursion remain unresolved, so do
  not retune agreement, phase, convergence, or rate thresholds; test a
  response-conditioned gait envelope or require calibrated flow/load histories
  for a disturbance residual.  This applies when the bearing window is padded
  at release and current/persistent requests have established sign semantics,
  and is falsified if held-out wake phases lose the direct capture or if the
  selectors no longer improve both route and load evidence over the raw
  current-bearing split.
- Response-conditioned whole-gait relief is a concrete negative result on this
  carrier, not the next threshold-tuning direction.  Relative to the sampled
  auxiliary-release carrier (`39.1104`, `1.9153L`, command energy `50045`, and
  `0.21986/54.19/754.87` crossflow/force/moment), applying a converged-and-
  aligned anterior-amplitude relief with a `0.90` floor retained capture but
  worsened every one of those measures to `39.2149`, `1.9159L`, `50246`, and
  `0.22014/55.32/764.02`; both joints still reached the exact rate and
  acceleration caps.  Thus small-bearing response is evidenced as a selector
  for withdrawing auxiliary steering, but not for reducing the entire
  propulsive envelope.  Do not tune its alignment gate, amplitude floor, or
  compose another range gate.  Preserve the route selectors and test a
  distinct actuator-level command projection, or wait for calibrated load/flow
  histories before adding a disturbance residual.  This applies to the
  saturated circular-history/half-cycle carrier and is falsified if a held-out
  wake phase shows whole-gait relief improving both direct arrival and
  effort/load/cap evidence over auxiliary release alone.
