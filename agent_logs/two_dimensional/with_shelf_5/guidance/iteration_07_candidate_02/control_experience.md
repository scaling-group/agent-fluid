...
- Once the `0.55`-period, `28 deg` anterior carrier and bounded `8 deg`
  ...
  distinguish where half-cycle asymmetry enters. Amplitude taper, bearing-rate
  lead, and near-target local-frequency duty asymmetry all retained the same
  broad-loop `92.988--93.032` capture, about `4.031L` mean distance,
...
- In the fast posterior-half-cycle topology, the inherited `2.5L` smooth
  amplitude taper is not a demonstrated terminal mechanism. The untapered
  ablation and three tapered samples all show the same hooked keyframe path,
  `target_reached` termination, and recorded `43.9505` arrival; tapering moves
  mean distance only from `2.1412L` to `2.1391L`, total command energy from
  `53082.36` to `53082.58`, and RMS force/moment from `49.45/701.31` to
  `49.44/701.26`, with the same velocity and acceleration cap hits. Do not
  stack further scalar distance schedules or credit this taper for the fast
  redirect. Preserve the successful carrier and next test a normalized
  trajectory-response signal or a changed wake phase. This lesson applies to
  first-crossing capture with the established half-cycle controller and is
  falsified if a controlled taper ablation later changes arrival class,
  trajectory topology, or robustness rather than sub-frame terminal distance.
