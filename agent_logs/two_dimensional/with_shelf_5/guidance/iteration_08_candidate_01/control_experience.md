...
- Once the fast half-cycle route is established, constrain waste at the
  actuator envelope before amplifying steering from target-response error. A
  posterior guard that removed only outward acceleration above `95%` of the
  rate envelope preserved the compact diagonal and capture (`44.242` versus
  `43.9505`) while taking posterior peak rate below the cap and reducing RMS
  lateral force/moment from `49.44/701.26` to `43.46/649.26`; mean distance
  changed only from `2.1391L` to `2.1450L`, and episode command energy fell
  slightly. By contrast, adding up to `20%` more half-cycle boost whenever
  bearing worsened produced a visibly larger correction loop, delayed capture
  to `56.540`, raised mean distance to `2.5747L`, and increased command energy
  to `7.02e4`. Thus bearing-error growth alone is not a license for more tail
  authority in this topology; prefer bounded, outward-only rate compliance.
  This applies after correct-sign target steering and posterior rate contact
  are visible, and is falsified if a changed wake phase loses capture or the
  guard's load advantage does not survive without a material route penalty.
