...
- For the direct half-cycle carrier, three identical approach-envelope samples
  ...
  avoidance. If its small transit penalty matters, test semantic localization
  such as a smooth normalized-distance approach gate before changing its rate
  threshold; this boundary applies to the demonstrated carrier and is
  falsified if localization changes the pre-approach route, materially delays
  capture, or erases the load reduction.
