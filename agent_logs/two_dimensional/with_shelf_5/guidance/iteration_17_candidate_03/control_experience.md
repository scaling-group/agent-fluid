...
- Semantic coherence improved both the route and loads where raw timescale
  separation did not. Driving anterior curvature from circular-history bearing
  and posterior half-cycle steering from current bearing shortened the fully
  history-driven `41.5030` capture to `40.6285`, but raised RMS
  force/moment from `61.80/862.48` to `63.65/868.82`. Smoothly
  withholding that fast posterior request only when its sign contradicted the
  persistent route then improved capture to `39.1104`, mean distance to
  `1.9149L`, and RMS crossflow/force/moment to
  `0.2193/57.21/783.64`; maximum joint excursions fell from
  `0.574/0.510` to `0.543/0.463` rad and command energy from
  `52374.2` to `50060.7`. Preserve the sign-coherence selector on this
  history/current half-cycle carrier and do not retune its transition or the
  ineffective near-cap guard. Both rate and acceleration caps remain reached,
  so the result does not establish actuator-envelope relief. If testing further
  relief, withdraw only auxiliary steering after small-error observed response,
  while preserving the coherent route carrier; reject that direction if it
  delays direct capture or fails to reduce effort, load, excursion, or cap
  evidence. This boundary is falsified if a held-out route requires a
  sign-conflicting fast correction to retain success.
