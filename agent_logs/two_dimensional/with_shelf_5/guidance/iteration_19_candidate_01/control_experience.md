...
- Treat response-conditioned release of route memory and posterior asymmetry as
  separate controller mechanisms.  In the matched sampled pair, releasing the
  auxiliary half-cycle term alone reached in `39.1104` with relative crossflow
  `0.21986` and force/moment `54.19/754.87`; also fading only the stale-history
  contribution in that same selector improved arrival to `39.0499`, mean
  distance to `1.91369L`, relative crossflow to `0.21683`, and command energy
  by `101.93`, but restored loads to `57.05/783.02`, essentially the unreleased
  parent's class.  Because the policies differ only in history fading, route
  memory is evidenced as a transit/crossflow lever, not a load-relief lever.
  When a direct large-error redirect and its response gate already work, test
  history release independently before stacking gait or posterior relief; this
  inference is falsified if the isolated release loses direct capture, leaves
  transit/crossflow unchanged, or exceeds the unreleased parent's load class.
