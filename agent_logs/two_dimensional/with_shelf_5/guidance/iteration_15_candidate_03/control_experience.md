...
- Short-history target filtering is a route primitive, not a demonstrated load
  ...
  and posterior half-cycle steering improved the reproduced parent's capture
  from `43.9505` to `41.5030` and mean distance from `2.1391L` to `2.0216L`,
  with keyframes retaining the direct wake-crossing topology. However, realized
  joint excursions grew from `0.525/0.452` to `0.579/0.527` rad and RMS
  force/moment rose from `49.44/701.26` to `61.80/862.48` at the same rate and
  acceleration caps. By contrast, the sampled alignment-gated outward-rate
  projection kept a `43.9505` capture while lowering loads to `44.47/657.47`,
  consistent with inherited `44.0220`, `44.86/663.89` projection results.
  Preserve history filtering only when faster target approach matters, and
  isolate its slow route role from high-authority posterior steering or a
  separately evidenced envelope guard rather than interpreting smoothing as
  damping. This applies to short circular bearing histories on the current
  saturated half-cycle carrier; it is falsified if role separation loses the
  arrival benefit or fails to lower excursions and loads relative to the fully
  history-driven controller.
