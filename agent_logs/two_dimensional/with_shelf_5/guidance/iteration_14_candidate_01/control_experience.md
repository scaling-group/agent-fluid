...
- On the demonstrated bearing-biased carrier, moving posterior steering onto
  ...
  feedback primitive.
- On this posterior-lag/half-cycle carrier, alignment is a useful selector only
  when it gates the evidenced redundant actuator direction. The sampled
  anterior-rate phase lead preserved direct capture at `43.9780` but raised
  RMS crossflow/force/moment from `0.2111/49.44/701.26` to
  `0.2136/53.18/736.58`. An inherited alignment-conditioned oscillator
  amplitude reduction likewise retained success but delayed arrival to
  `44.8525`, raised mean distance from `2.1391L` to `2.1708L`, and increased
  RMS crossflow/force/moment to `0.2188/69.92/948.62`. In contrast, a smooth
  alignment-gated projection of only near-envelope outward posterior
  acceleration captured at `44.0220` with `2.1418L` mean distance while
  lowering RMS crossflow/force/moment to `0.2102/44.86/663.89`; range-only
  localization was inert and a half-cycle restriction recovered less benefit.
  Preserve the traveling-wave envelope, large-error redirect, and every
  reversal; apply the full directional projection during aligned transit
  rather than tuning phase, range, or amplitude schedules. This supports
  distributed load shaping, not peak-cap avoidance, and is falsified if a
  repeat changes the compact route, materially delays or loses capture, weakens
  reversal, or fails to reproduce lower RMS loads.
