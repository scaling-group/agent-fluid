...
- A successful regime selector does not transfer automatically between
  actuator mechanisms.  Using alignment to lower the Van der Pol carrier's
  amplitude to a `0.90` floor still captured, but regressed the reproduced
  carrier from `43.9505` to `44.8525` arrival, raised mean distance from
  `2.1391L` to `2.1708L`, command energy from `53082.6` to `54872.1`, and RMS
  crossflow/force/moment from `0.2111/49.44/701.26` to
  `0.2188/69.92/948.62`.  In this oscillator, amplitude also appears in the
  nonlinear drive denominator, so a lower setpoint is not equivalent to
  clipping redundant acceleration and can amplify transient drive while the
  state remains large.  Avoid further aligned-amplitude-floor tuning on this
  carrier; prefer direction-aware outward-rate projection, which the inherited
  results show reduces loads while preserving reversal.  This boundary applies
  to amplitude scheduling inside this Van der Pol form and is falsified if a
  structurally different envelope law measurably lowers joint excursion,
  command, and RMS loads without delaying the direct capture.
