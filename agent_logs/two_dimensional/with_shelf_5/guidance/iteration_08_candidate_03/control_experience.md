...
- A near-rate-envelope posterior guard exposes the same tradeoff at the action
  boundary. Suppressing all outward posterior acceleration above `95%` of the
  rate envelope retained target success and the compact visible route, while
  lowering RMS force/moment from `49.44/701.26` to `43.46/649.26` and power
  proxy from `3944.0` to `3848.5`. However, capture slowed from `43.9505` to
  `44.2420`, mean distance worsened from `2.1391L` to `2.1450L`, total command
  energy fell only `0.23%`, and the acceleration cap was still reached while
  maximum posterior rate fell only `0.34%`. The inherited clean taper ablation
  preserved the exact fast arrival, so taper removal is not a sufficient
  explanation for this regression. Do not treat global cap-directed action
  suppression as free efficiency or repeat it as a scalar threshold search;
  if revisited on this half-cycle topology, test phase-selective relief that
  preserves the target-favored posterior stroke. This implication applies
  while that stroke is responsible for the fast redirect and is falsified if a
  controlled global guard later reduces load without worsening arrival,
  distance integral, or cap contact under another wake phase.
