...
- Treat observed bearing convergence as a channel selector, not generic
  permission to reduce controller authority: completion scope has a
  non-monotonic interaction with load. On the route/current sign-coherent
  ...
  `54.19/754.87`. Releasing both that bias and lagged route history produced the
  best arrival/score (`39.0499`, `1.91369L`, `-0.03762`) but returned loads to
  `57.05/783.02`. The missing factorial result—releasing route history while
  retaining the posterior half-cycle—captured at `39.1159` and `1.91432L` yet
  reduced loads further to `51.63/734.35`; its unchanged rate/acceleration caps
  and slightly higher effort mean the result is load redistribution, not
  saturation or energy relief. Extending completion to oscillator amplitude
  instead slowed capture to `39.2149` and worsened distance, effort, and loads.
  For this direct carrier, preserve the rhythmic envelope and do not assume
  that withdrawing more posterior authority is safer: choose route-only release
  for the demonstrated load/arrival balance or route-plus-posterior release for
  the last `0.066` arrival benefit. This applies only to the measured
  convergence-gated half-cycle carrier and is falsified if route-only release
  does not reproduce the `51.6/734` class; until then avoid amplitude-floor,
  alignment-width, or partial-authority tuning and require calibrated histories
  before testing a new wake residual.
