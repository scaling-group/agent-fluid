...
- For this half-cycle carrier, the reusable load-shaping selector is aligned
  transit plus outward joint rate, distributed across the actuated bend rather
  than limited by range or oscillator phase. Inherited repeats of a smooth
  posterior-only projection retained the direct route at `44.0220` and lowered
  RMS crossflow/force/moment from `0.2111/49.44/701.26` to
  `0.2102/44.86/663.89`. A sampled extension of the identical
  direction-aware projection to both joints recovered the unguarded
  `43.9505` arrival, slightly improved mean distance to `2.1372L`, and
  further lowered crossflow/force/moment to `0.2093/44.47/657.47`, command
  energy to `52868.1`, and power proxy to `3889.8`, despite continued rate
  and acceleration cap contact. By contrast, bearing-history filtering arrived
  faster at `41.5030` but enlarged realized joint excursions and raised
  crossflow/force/moment to `0.2246/61.80/862.48`; inherited range
  localization was inert, and phase-lead or half-cycle restriction weakened
  load relief. Preserve large-error redirect and every reversal, and apply
  directional projection to both raw joint commands throughout aligned
  transit. This applies to the demonstrated posterior-lag/half-cycle carrier
  and supports distributed load relief, not peak-cap avoidance; falsify it if
  a held-out wake loses the compact capture topology, weakens reversal, or
  fails to reproduce lower RMS loads and effort.
