...
- Response completion should be allocated by control role, because withdrawing
  more auxiliary steering did not monotonically reduce load. Against the
  no-release coherence carrier's `39.1104` capture, `1.91494L` mean distance,
  and `57.21/783.64` RMS force/moment, releasing only the stale circular-route
  offset preserved the same direct visual topology and `39.1159` capture while
  improving mean distance to `1.91432L` and force/moment to `51.63/734.35` at
  essentially unchanged command energy; crossflow also fell from `0.21935` to
  `0.21787`. Two byte-identical samples that additionally released posterior
  half-cycle steering arrived only `0.0660` sooner but returned to the
  `57.05/783.02` load class. For this response-gated carrier, use measured
  convergence to retire anterior route memory while retaining target-favored
  posterior rhythm when load is the objective; do not infer that broader
  steering withdrawal is safer. This is falsified if reproduction loses the
  direct capture or the lower load class. All variants still reached both rate
  and acceleration caps, so the lesson does not establish cap relief and does
  not justify threshold tuning without time-resolved disturbance evidence.
