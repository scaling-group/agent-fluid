...
- On the successful `0.55`-period, `28 deg` carrier, phase-gated posterior
  ...
  Because the fast sample also contained a terminal amplitude taper, use a
  clean ablation before assigning causality; separate taper evidence was
  neutral. The reusable implication is to test state-phase redistribution of
  an already successful mean turn before more terminal scalar schedules. This
  applies while target bearing has the correct sign and the traveling-bend
  carrier remains intact, and is falsified if removing the taper restores the
  `93`-time route, loses capture, or materially worsens clipping or load.
