...
function target_policy(state, params)
    ...
    blended_halfcycle_request = route_turn_request + agreement_gate *
        (current_turn_request - route_turn_request)

    # Treat the fast channel as response-conditioned release, not a second
    # route command.  It may relax persistent steering toward zero, but it may
    # neither amplify that steering nor reverse it.  This interval projection
    # is exact at release because the padded history makes both requests equal.
    halfcycle_turn_request = clamp(
        blended_halfcycle_request,
        min(route_turn_request, 0.0),
        max(route_turn_request, 0.0),
    )
