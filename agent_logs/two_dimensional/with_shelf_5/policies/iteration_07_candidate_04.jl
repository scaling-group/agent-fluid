function target_policy_params()
    return (
        ...
        turn_release_fraction=0.35,
        turn_response_rate_scale=0.25,
        turn_release_bearing_inner=10.0 * pi / 180,
        turn_release_bearing_outer=20.0 * pi / 180,
...
function target_policy(state, params)
    ...
    # Release part of the asymmetric steering burst only after the response is
    # both close to alignment and still improving. The compact bearing window
    # makes this exactly inactive during the proven large-error redirect.
    bearing_window_rate = Float64(state.bearing_window_rate)
    alignment_response = max(-turn_request * bearing_window_rate, 0.0)
    bearing_gate_fraction = clamp(
        (params.turn_release_bearing_outer - abs(bearing)) /
        max(
            params.turn_release_bearing_outer -
                params.turn_release_bearing_inner,
            eps(Float64),
        ),
        0.0,
        1.0,
    )
    bearing_gate = bearing_gate_fraction^2 * (3 - 2 * bearing_gate_fraction)
    turn_release = params.turn_release_fraction * bearing_gate * tanh(
        alignment_response /
        max(params.turn_response_rate_scale, eps(Float64)),
    )
    halfcycle_bias = params.halfcycle_tail_boost *
        turn_request * preferred_half * (1 - turn_release)
