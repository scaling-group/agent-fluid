...
function target_policy(state, params)
    ...
    # Once the observed body-frame error is already shrinking, release only
    # part of the asymmetric steering burst. A missing history or worsening
    # alignment leaves the evaluated half-cycle allocation unchanged.
    bearing_window_rate = Float64(state.bearing_window_rate)
    alignment_response = max(-turn_request * bearing_window_rate, 0.0)
    turn_release = params.turn_release_fraction * tanh(
        alignment_response /
