...
function target_policy(state, params)
    ...
    # Route memory remains available to anterior curvature, but envelope relief
    # follows fast current alignment. Remove only acceleration that pushes an
    # already near-envelope rate farther outward; every reversal is unchanged.
    alignment_fraction = clamp(
        (params.rate_guard_turn_request_start - abs(halfcycle_turn_request)) /
