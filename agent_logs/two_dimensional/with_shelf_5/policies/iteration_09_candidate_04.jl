...
function target_policy(state, params)
    ...
    # Redistribute part of posterior steering onto the target-opposed motion
    # half-cycle. Using observed posterior rate closes the allocation around
    # the realized tail response: the bias resists motion away from requested
    # curvature and withdraws while the tail moves into the favored bend. A
    # symmetric rate cycle retains the original half-duty and mean authority.
    posterior_motion_phase = -qd2 / max(omega, eps(omega))
    preferred_half = 0.5 * (1 + tanh(
        turn_request * posterior_motion_phase /
