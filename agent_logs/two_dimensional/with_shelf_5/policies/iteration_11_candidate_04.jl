function target_policy_params()
    return (
        ...
        joint_acceleration_envelope=1800.0 * pi / 180,
        coordinated_limit_turn_request_start=0.75,
...
function target_policy(state, params)
    ...
    # Retain the evaluated large-error redirect exactly. Once body-frame
    # bearing demand is small, keep the anterior/posterior command proportions
    # of the traveling bend when the acceleration envelope engages instead of
    # relying on independent component clipping downstream.
    alignment_fraction = clamp(
        (params.coordinated_limit_turn_request_start - abs(turn_request)) /
        max(params.coordinated_limit_turn_request_start, eps(Float64)),
        0.0,
        1.0,
    )
    alignment_gate = alignment_fraction^2 * (3 - 2 * alignment_fraction)
    raw_peak_acceleration = max(abs(raw_a1), abs(raw_a2))
    coordinated_scale = min(
        1.0,
        params.joint_acceleration_envelope /
            max(raw_peak_acceleration, eps(Float64)),
    )
    command_scale = 1 - alignment_gate * (1 - coordinated_scale)
    a1 = command_scale * raw_a1
    a2 = command_scale * raw_a2
