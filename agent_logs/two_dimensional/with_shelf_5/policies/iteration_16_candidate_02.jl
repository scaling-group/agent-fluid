...
function target_policy(state, params)
    ...
    # Let the persistent signal set slow route curvature while retaining the
    # current observation for the faster posterior steering role below.
    ...
    # current bearing prevents persistent route filtering from prolonging the
    # corrective posterior asymmetry after the target error changes.
    ...
    # lag, so envelope shaping does not change its target or phase.
    phase_lag_target = tail_bias - head_wave -
        params.tail_lag_gain * qd1 / max(omega, eps(omega))
    raw_a2 = omega^2 * (phase_lag_target - q2) -
        2 * params.tail_damping * omega * qd2

    # Keep full redirect authority while the history-filtered target error is
    # large. Once aligned, remove only acceleration that drives a near-envelope
    # joint rate farther outward. Reversal acceleration is exactly unchanged,
    # preserving the traveling bend and useful wake-assisted lateral motion.
    alignment_fraction = clamp(
        (params.rate_guard_turn_request_start - abs(route_turn_request)) /
