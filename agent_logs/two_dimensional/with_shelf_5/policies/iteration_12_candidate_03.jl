...
function target_policy(state, params)
    ...
    # lag, so load shaping does not change its target or phase.
    phase_lag_target = tail_bias - head_wave -
        params.tail_lag_gain * qd1 / max(omega, eps(omega))
    raw_a2 = omega^2 * (phase_lag_target - q2) -
        2 * params.tail_damping * omega * qd2

    # The inherited alignment gate preserves the full large-error redirect.
    # Within aligned transit, reuse the demonstrated joint-state phase gate to
    # favor relief on the complementary stroke while preserving the
    # target-favored posterior stroke. Reversal always stays available.
    ...
    nonpreferred_half = 1 - preferred_half
    velocity_sign = sign(qd2)
    outward_a2 = velocity_sign * max(velocity_sign * raw_a2, 0.0)
    a2 = raw_a2 -
        alignment_gate * nonpreferred_half * rate_guard * outward_a2
