...
function target_policy(state, params)
    ...
    # Fast posterior steering is admitted only while it is sign-coherent with
    # the persistent route. This keeps short wake-driven bearing reversals from
    # owning the high-authority half-cycle without imposing a controller mode.
    ...
    coherent_turn_request = route_turn_request + agreement_gate *
        (current_turn_request - route_turn_request)

    # The padded history produces zero release at the start of the initial
    # redirect. Thereafter, remove only extra posterior asymmetry when wrapped
    # body-frame bearing has already progressed toward zero; stagnant or
    # worsening error, anterior curvature, and the carrier remain unchanged.
    bearing_progress = max(
        -sign(coherent_turn_request) * Float64(state.bearing_window_delta),
        0.0,
    )
    response_release = clamp(
        bearing_progress /
            max(params.turn_bearing_scale, eps(Float64)),
        0.0,
        1.0,
    )
    halfcycle_turn_request = coherent_turn_request * (1 - response_release)
