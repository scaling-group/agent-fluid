...
function target_policy(state, params)
    ...
    # Retain the proven range envelope. The additional alignment-conditioned
    # envelope below fades out near the target so the two relief mechanisms do
    # not compound into terminal coasting.
    ...
    # Reduce carrier amplitude only when both the persistent route and the
    # immediate bearing agree that the fish is aligned. Any appreciable slow or
    # fast correction restores the evaluated carrier exactly. Multiplying by
    # the far-to-near approach blend withdraws this extra relief near capture.
    current_turn_request = tanh(
        bearing / max(params.turn_bearing_scale, eps(Float64)),
    )
    current_alignment_fraction = clamp(
        (params.halfcycle_agreement_scale - current_turn_request^2) /
        max(params.halfcycle_agreement_scale, eps(Float64)),
        0.0,
        1.0,
    )
    current_alignment_gate = current_alignment_fraction^2 *
        (3 - 2 * current_alignment_fraction)
    route_alignment_fraction = clamp(
        (params.halfcycle_agreement_scale - route_turn_request^2) /
        max(params.halfcycle_agreement_scale, eps(Float64)),
        0.0,
        1.0,
    )
    route_alignment_gate = route_alignment_fraction^2 *
        (3 - 2 * route_alignment_fraction)
    aligned_envelope_gate = route_alignment_gate * current_alignment_gate
    aligned_relief =
        (1 - clamp(params.aligned_amplitude_floor, 0.0, 1.0)) *
        aligned_envelope_gate * approach_blend
    amplitude_scale = approach_amplitude_scale * (1 - aligned_relief)
    ...
    # half-cycle. Keep immediate posterior steering when it supports the
    # persistent route, but do not let a contradictory fast bearing reversal
    # own the half-cycle. Padded release history preserves redirect authority.
    ...
    # evidence that the redirect is taking effect. Preserve the evaluated
    # response-completion release on only the extra posterior asymmetry.
    ...
    response_release = convergence_fraction * current_alignment_gate
    ...
    # lag, so the new envelope changes gait magnitude rather than beat phase.
