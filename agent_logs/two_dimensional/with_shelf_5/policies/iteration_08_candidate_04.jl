...
function target_policy(state, params)
    ...
    # half-cycle. Angle plus normalized angular velocity identifies local
    # oscillator phase without a clock. The small lead starts the tail bias as
    # the anterior bend enters the favored side and removes it before reversal;
    # shifting a half-cycle gate leaves its cycle-average authority unchanged.
    halfcycle_phase = head_wave +
        params.halfcycle_phase_lead * qd1 / max(omega, eps(omega))
    preferred_half = 0.5 * (1 + tanh(
        turn_request * halfcycle_phase /
