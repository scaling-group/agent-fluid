...
function target_policy(state, params)
    ...
    # diverging error retains the evaluated route-only parent's full authority.
    ...
    # Anticipate only a completed response. The projected current bearing is
    # constrained between zero and the observation, so it can release residual
    # curvature but cannot amplify or reverse target-directed authority.
    bearing_lead = response_release * params.bearing_lead_periods *
        params.control_period * Float64(state.bearing_window_rate)
    anticipated_bearing = clamp(
        bearing + bearing_lead,
        min(0.0, bearing),
        max(0.0, bearing),
    )

    # History initiates and sustains the redirect, then fades after observed
    # convergence. Padded history and every large/diverging-error state exactly
    # reproduce the evaluated route-only carrier.
    route_bearing = anticipated_bearing +
