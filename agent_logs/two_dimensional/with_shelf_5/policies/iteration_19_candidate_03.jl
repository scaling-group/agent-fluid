function target_policy_params()
    return (
        ...
        coherent_acceleration_limit=1800.0 * pi / 180,
    )
end
...
function target_policy(state, params)
    ...
    # Preserve the raw two-joint action ray as turn response completes. The
    # episode otherwise clips each component independently, which can flatten
    # the anterior/posterior relationship when both requests exceed the box
    # envelope. Large, stationary, or diverging bearing error gives an exact
    # pass-through; aligned convergence smoothly blends toward the smallest
    # common scale that places the pair inside the acceleration envelope.
    raw_acceleration_max = max(abs(raw_a1), abs(raw_a2))
    acceleration_limit = max(
        params.coherent_acceleration_limit,
        eps(Float64),
    )
    projected_scale = min(
        1.0,
        acceleration_limit / max(raw_acceleration_max, eps(Float64)),
    )
    coherent_scale = 1 - response_release * (1 - projected_scale)
    a1 = coherent_scale * raw_a1
    a2 = coherent_scale * raw_a2
