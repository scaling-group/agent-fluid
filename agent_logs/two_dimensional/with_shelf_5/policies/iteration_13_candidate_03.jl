function target_policy_params()
    return (
        ...
        cruise_bearing_scale=8.0 * pi / 180,
        cruise_amplitude_floor=0.90,
    )
end
...
function target_policy(state, params)
    ...
    # Use the observed completion of the large-bearing redirect as a continuous
    # gait transition. Full carrier amplitude remains available for turning;
    # once aligned, a modest cruise envelope relieves both joints without
    # changing the steering bias, posterior lag, or reversal equations.
    cruise_alignment = exp(-(
        bearing / max(params.cruise_bearing_scale, eps(Float64))
    )^2)
    cruise_amplitude_scale = 1 -
        (1 - params.cruise_amplitude_floor) * cruise_alignment
    amp = params.oscillator_amplitude *
        approach_amplitude_scale * cruise_amplitude_scale
