function target_policy_params()
    return (
        ...
        posterior_accel_slew_limit=36000.0 * pi / 180,
        slew_turn_request_start=0.75,
    )
end
...
function target_policy(state, params)
    ...
    # lag, so transition shaping does not change its target or phase.
    phase_lag_target = tail_bias - head_wave -
        params.tail_lag_gain * qd1 / max(omega, eps(omega))
    raw_a2 = omega^2 * (phase_lag_target - q2) -
        2 * params.tail_damping * omega * qd2

    # Keep full posterior authority through the large-error redirect. After
    # body-frame alignment, use the observed prior action and actual control
    # interval to bound acceleration-command transitions. This is a stateless
    # slew constraint: it needs neither a clock nor an inferred wake phase.
    alignment_fraction = clamp(
        (params.slew_turn_request_start - abs(turn_request)) /
        max(params.slew_turn_request_start, eps(Float64)),
        0.0,
        1.0,
    )
    alignment_gate = alignment_fraction^2 * (3 - 2 * alignment_fraction)
    history_dt = clamp(Float64(state.history_dt), 0.0, params.control_period)
    max_action_step = params.posterior_accel_slew_limit * history_dt
    previous_a2 = Float64(state.previous_action[2])
    slew_limited_a2 = clamp(
        raw_a2,
        previous_a2 - max_action_step,
        previous_a2 + max_action_step,
    )
    a2 = raw_a2 + alignment_gate * (slew_limited_a2 - raw_a2)
