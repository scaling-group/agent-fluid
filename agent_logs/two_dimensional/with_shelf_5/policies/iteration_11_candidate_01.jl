...
function target_policy(state, params)
    ...
    # lag, so the steering allocation and reversal command remain unchanged.
    phase_lag_target = tail_bias - head_wave -
        params.tail_lag_gain * qd1 / max(omega, eps(omega))
    raw_a2 = omega^2 * (phase_lag_target - q2) -
        2 * params.tail_damping * omega * qd2

    # Preserve full posterior authority during a large-error redirect. Once
    # target-bearing demand is small, smoothly project away only acceleration
    # that pushes an already near-envelope posterior rate farther outward.
    # Inward acceleration remains available for every joint reversal.
