function target_policy_params()
    return (
        ...
        coupled_acceleration_budget=1710.0 * pi / 180,
    )
end
...
function target_policy(state, params)
    ...
    # Preserve the two-joint acceleration direction when the carrier meets its
    # actuation envelope. Independent downstream clipping can erase posterior
    # lag/asymmetry; common scaling retains both while leaving sub-budget
    # champion commands exactly unchanged.
    command_peak = max(abs(a1), abs(a2))
    allocation_scale = min(
        1.0,
        params.coupled_acceleration_budget /
        max(command_peak, eps(Float64)),
    )
