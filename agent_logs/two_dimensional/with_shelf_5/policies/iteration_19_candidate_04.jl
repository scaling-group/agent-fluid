function target_policy_params()
    return (
        ...
        accel_soft_knee=24.0,
        accel_soft_limit=30.0,
    )
end

function smooth_acceleration_projection(value, knee, limit)
    raw_value = Float64(value)
    safe_limit = max(abs(Float64(limit)), eps(Float64))
    safe_knee = clamp(abs(Float64(knee)), 0.0, safe_limit)
    magnitude = abs(raw_value)
    magnitude <= safe_knee && return raw_value

    # Match both value and slope at the knee, then approach the policy-owned
    # limit continuously. Ordinary gait commands remain exactly unchanged.
    projection_span = max(safe_limit - safe_knee, eps(Float64))
    projected_magnitude = safe_limit - projection_span *
        exp(-(magnitude - safe_knee) / projection_span)
    return copysign(projected_magnitude, raw_value)
end
...
function target_policy(state, params)
    ...
    # Treat a decrease in normalized bearing error over the observation window
    # as evidence that the redirect is taking effect. Large, stationary, or
    # diverging error retains full route memory and posterior burst authority.
    ...
    # History supplies persistent curvature during the redirect, but after an
    # observed aligned response it should not keep bending the mean route away
    # from current target geometry. Release only the lagged history component;
    # the current bearing remains authoritative at every response level.
    route_bearing = bearing +
        clamp(params.route_history_blend, 0.0, 1.0) *
        (1 - response_release) * history_offset
    ...
    # The evaluated carrier reaches the target but both raw acceleration
    # channels contact the hard episode cap. Preserve its semantic commands and
    # smoothly project only their actuator-infeasible magnitudes.
    a1_projected = smooth_acceleration_projection(
        a1,
        params.accel_soft_knee,
        params.accel_soft_limit,
    )
    a2_projected = smooth_acceleration_projection(
        a2,
        params.accel_soft_knee,
        params.accel_soft_limit,
    )
