...
function target_policy(state, params)
    ...
    # Preserve the demonstrated angular redirect outside the terminal
    # neighborhood. Near capture, blend toward normalized lateral miss so mean
    # curvature is released as the cross-target error itself vanishes.
    bearing = Float64(state.bearing)
    far_turn_request = tanh(
        bearing / max(params.turn_bearing_scale, eps(Float64)),
    )
    lateral_distance_L = Float64(state.target_body_L[2])
    capture_turn_request = tanh(
        lateral_distance_L / max(params.capture_lateral_scale_L, eps(Float64)),
    )
    turn_request = approach_blend * far_turn_request +
        (1 - approach_blend) * capture_turn_request
