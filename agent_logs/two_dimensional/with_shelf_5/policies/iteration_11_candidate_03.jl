...
function target_policy(state, params)
    ...
    terminal_gate = 1 - approach_blend
    ...
    # lag, so terminal load shaping does not change its target or phase.
    phase_lag_target = tail_bias - head_wave -
        params.tail_lag_gain * qd1 / max(omega, eps(omega))
    raw_a2 = omega^2 * (phase_lag_target - q2) -
        2 * params.tail_damping * omega * qd2

    # Localize the inherited direction-aware rate projection to the terminal
    # neighborhood. Transit and large-error redirect are exactly unchanged;
    # near the target, remove only acceleration that pushes an already
    # near-envelope posterior rate farther outward. Reversal stays available.
    ...
    a2 = raw_a2 - terminal_gate * alignment_gate * rate_guard * outward_a2
