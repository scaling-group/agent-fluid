function target_policy_params()
    return (
        ...
        posterior_release_turn_request=0.50,
...
function target_policy(state, params)
    ...
    # Preserve the full redirect until the fast target error is both small and
    # closer to zero than its persistent history.  This response-conditioned
    # release cannot activate at padded release or during large-error steering,
    # and is strongly suppressed on a contradictory fast reversal; it eases
    # only the extra posterior authority after coherent alignment progress.
    alignment_fraction = clamp(
        (params.posterior_release_turn_request - abs(current_turn_request)) /
        max(params.posterior_release_turn_request, eps(Float64)),
        0.0,
        1.0,
    )
    alignment_gate = alignment_fraction^2 * (3 - 2 * alignment_fraction)
    convergence_fraction = clamp(
        (abs(route_turn_request) - abs(current_turn_request)) /
        max(abs(route_turn_request), eps(Float64)),
        0.0,
        1.0,
    )
    convergence_gate = convergence_fraction^2 * (3 - 2 * convergence_fraction)
    response_coherence_fraction = clamp(
        route_turn_request * current_turn_request /
        max(params.halfcycle_agreement_scale, eps(Float64)),
        0.0,
        1.0,
    )
    response_coherence_gate = response_coherence_fraction^2 *
        (3 - 2 * response_coherence_fraction)
    posterior_release = alignment_gate * response_coherence_gate *
        convergence_gate
    ...
    halfcycle_bias = params.halfcycle_tail_boost *
        halfcycle_turn_request * preferred_half * (1 - posterior_release)
