...
function target_policy(state, params)
    ...
    # as evidence that the redirect is taking effect. Release only the extra
    # half-cycle asymmetry as alignment is achieved; the carrier, mean route
    # curvature, and all large/diverging-error authority remain unchanged.
    ...
    # A target-favored hydrodynamic yaw moment is already supplying part of
    # the requested redirect.  Within the same small-error envelope, release
    # only the extra posterior half-cycle authority instead of reinforcing
    # that helpful response.  Opposing yaw, the large-error redirect, mean
    # curvature, and the propulsive carrier retain full authority.
    yaw_moment_L2 = Float64(state.moment_z_L2)
    assisting_yaw_product = max(
        halfcycle_turn_request * yaw_moment_L2,
        0.0,
    )
    assist_fraction = clamp(
        assisting_yaw_product /
            max(params.yaw_assist_product_scale, eps(Float64)),
        0.0,
        1.0,
    )
    assist_gate = assist_fraction^2 * (3 - 2 * assist_fraction)
    response_release = alignment_gate * max(
