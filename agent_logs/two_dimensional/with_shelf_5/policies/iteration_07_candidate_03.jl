...
function target_policy(state, params)
    ...
    # Preserve the evaluated boost while alignment is steady or improving.
    # Only a growing signed bearing error adds a small, smoothly bounded
    # recovery burst; missing history gives a zero residual automatically.
    bearing_window_rate = Float64(state.bearing_window_rate)
    misalignment_growth = max(turn_request * bearing_window_rate, 0.0)
    recovery_gain = 1 + params.turn_recovery_fraction * tanh(
        misalignment_growth /
        max(params.turn_response_rate_scale, eps(Float64)),
    )
    halfcycle_bias = params.halfcycle_tail_boost *
        turn_request * preferred_half * recovery_gain
