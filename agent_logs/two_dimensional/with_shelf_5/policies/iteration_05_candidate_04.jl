function target_policy_params()
    return (
        ...
        halfcycle_response_rate_scale=0.5,
        halfcycle_response_relief=0.35,
...
function target_policy(state, params)
    ...
    # Preserve full asymmetric authority until a turn response is observed.
    # When bearing changes opposite the requested turn, release only part of
    # the boost; a stationary or worsening bearing retains the proven command.
    bearing_window_rate = Float64(state.bearing_window_rate)
    signed_response = turn_request * tanh(
        bearing_window_rate /
        max(params.halfcycle_response_rate_scale, eps(Float64)),
    )
    converging_response = clamp(-signed_response, 0.0, 1.0)
    response_scale = 1 -
        params.halfcycle_response_relief * converging_response
    halfcycle_bias = params.halfcycle_tail_boost * response_scale *
