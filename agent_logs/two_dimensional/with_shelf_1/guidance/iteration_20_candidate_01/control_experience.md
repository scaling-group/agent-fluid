...
- Once coherent whole-carrier speed release is established, do not stack a
  ...
  regression.
- Crossflow is useful as a redirect-completion cue only when its physical role
  is made explicit. Three same-prewarm children preserve the inherited sharp
  redirect and straight targetward traverse, but differ materially despite
  their visually identical route. Relative crossflow confirmed by co-signed
  lateral force Pareto-improves the assigned local-crossflow-only parent in
  arrival (`33.990 -> 33.726`), mean distance (`1.59701L -> 1.59412L`),
  total/mean command energy (`46157/1357.97 -> 45776/1357.30`), force/moment
  RMS (`94.55/1395.74 -> 93.64/1383.73`), and score
  (`0.27610 -> 0.27859`). Raw relative crossflow is lower-load still
  (`85.04/1244.16`) but slightly slower and less compact (`33.946`,
  `1.60066L`). None matches the older bearing-closure baseline's much lower
  `68.96/1036.40` load. Therefore use co-signed force as validation that
  body-relative crossflow is targetward wake assistance, not as a generic load
  gate or evidence that hydrodynamic load is solved. This result applies only
  to releasing the optional burst with mean steering and base asymmetry intact;
  falsify it if an independent prewarm replication loses capture or its joint
  route/effort/load advantage, and require a changed wake phase before claiming
  robustness.
