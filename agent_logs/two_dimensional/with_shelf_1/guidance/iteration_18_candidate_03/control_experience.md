...
- Do not treat every fast observable as interchangeable evidence for releasing
  the optional redirect burst. Four sampled coherent speed-release policies
  reproduce the same visible redirect-and-traverse success and exact
  `34.7105` arrival, `1.62283L` mean distance, `46985.9` total energy, and
  `68.96/1036.40` force/moment RMS. The inherited direct-heading-rate
  replacement arrives `0.0385` sooner and uses slightly less effort, but
  worsens mean distance to `1.62669L` and raises force/moment RMS to
  `91.30/1389.74`. Stacking previous-command pressure on the coherent speed
  cue is slower (`35.0185`), less route-compact (`1.63314L`), and slightly more
  loaded (`69.66/1037.65`), while total energy rises to `47340.0`; its small
  crossflow reduction is not a joint objective benefit. Preserve bearing-window
  response and one coherent speed cue instead of substituting yaw rate or
  accumulating actuator proxies. After this plateau, test at most one
  semantically direct hydrodynamic assist/rejection mechanism while keeping
  carrier and mean route ownership fixed, and require capture plus a meaningful
  route, load, effort, or limit-contact improvement. This boundary is limited
  to the shared prewarm and is falsified if a changed wake phase establishes a
  repeatable joint benefit for either rejected cue.
