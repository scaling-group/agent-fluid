...
- Children of the same constant-reserve success separate route-error feedback
  from actuator allocation. Course-slip correction shortened arrival from
  `49.142` to `48.032` and reduced RMS force/moment from `39.05/617.13` to
  `37.92/605.38`, but raised mean command energy from `1272.25` to `1299.71`.
  Raw-bearing-scheduled reservation reached at `46.035` with `2.0695L` mean
  distance and `1237.06` mean effort, while raising RMS force/moment to
  `51.40/761.46`. Feeding the slip-corrected error into both mechanisms
  disproved naive compositionality: it still captured, but regressed to
  `46.761`, `2.1087L`, `1259.82`, and `59.04/923.45`, respectively. The sampled
  separated combination—slip only in the residual and raw bearing in the
  reservation—then reached at `45.727` with `2.0543L` mean distance and lower
  force RMS `49.36`, but paid `1264.65` mean effort and higher moment RMS
  `799.31` than raw-bearing scheduling alone. This validates signal-role
  separation for arrival and route compactness, not a universal energy/load
  improvement. Keep slip as route-response feedback and raw bearing as the
  persistent authority signal; test subsequent actuator-allocation mechanisms
  without recoupling them, and retain a change only if capture plus at least
  one arrival/distance/effort/load tradeoff improves. This boundary is limited
  to the shared prewarm and does not establish robustness to another wake
  phase.
