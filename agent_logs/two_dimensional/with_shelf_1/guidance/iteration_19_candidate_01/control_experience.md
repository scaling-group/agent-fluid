...
- Once coherent whole-carrier speed release is established, do not stack a
  ...
  regression.
- Treat targetward relative crossflow as a direct transient-response cue, not
  as demonstrated load rejection. Replacing bearing-window closure only in
  the optional redirect burst preserves the parent's visible redirect-and-
  upstream route and improves arrival (`34.7105 -> 33.9460`), mean distance
  (`1.62283L -> 1.60066L`), and total energy (`46985.9 -> 46092.2`), but raises
  force/moment RMS from `68.96/1036.40` to `85.04/1244.16` and slightly raises
  relative crossflow (`0.24023 -> 0.24249`). A heading-rate replacement is a
  stronger negative control: it barely improves arrival (`34.6720`), worsens
  mean distance (`1.62669L`), and raises load to `91.30/1389.74`. Keep raw
  target geometry in charge of route steering, use crossflow only to release
  surplus burst, and do not wholesale substitute yaw rate when seeking lower
  load. Test a compatible kinematic-and-flow response release before adding
  another scalar pressure gate. This implication is limited to the shared
  prewarm and is falsified if a changed wake phase loses capture or if the
  crossflow cue's arrival/route advantage disappears.
