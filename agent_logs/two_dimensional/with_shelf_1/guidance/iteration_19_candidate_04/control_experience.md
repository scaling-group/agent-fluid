...
- Direct hydrodynamic response cues are not interchangeable. Replacing bearing-
  ...
  release; do not promote direct force credit from this result. A clean next
  test may replace only the response observation with local body-frame flow to
  distinguish wake transport from the fish-velocity component of relative
  flow. This boundary is limited to the shared prewarm and is falsified if the
  crossflow route benefit disappears or force credit becomes load-reducing at
