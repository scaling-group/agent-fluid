...
- Relative crossflow is evidenced as an arrival/route-response cue, not a
  load-relief cue. Against three exact copies of the coherent bearing-closure
  parent (`34.7105` arrival, `1.62283L` mean distance, `46985.9` total command
  energy, and `68.96/1036.40` force/moment RMS), replacing only optional-burst
  bearing closure with targetward body-frame relative crossflow reaches in
  `33.9460`, lowers mean distance to `1.60066L` and total energy to `46092.2`,
  and raises score `0.251012 -> 0.272045`. The cost is higher mean command
  energy (`1353.65 -> 1357.81`), relative-crossflow RMS (`0.24023 -> 0.24249`),
  and force/moment RMS (`85.04/1244.16`). Direct heading-rate replacement is
  worse on route and raises load further (`91.30/1389.74`), while adding a
  targetward force credit to bearing closure regresses arrival, distance,
  effort, and load versus the parent; do not treat arbitrary maneuver or load
  signals as interchangeable response gates. Preserve raw-bearing ownership of
  persistent steering and limit relative crossflow to the surplus burst. This
  same-prewarm result is falsified if it does not replicate, if a changed wake
  phase loses capture, or if a load-sensitive objective cannot justify its
