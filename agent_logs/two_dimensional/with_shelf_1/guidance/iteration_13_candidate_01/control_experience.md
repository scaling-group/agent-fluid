...
- Fast signed residuals have not earned disturbance-rejection status in this
  ...
  negative boundary.
- Localize actuator-pressure relief to surplus transient authority. Against
  two exactly reproduced response-only successes at `34.8205` arrival,
  `1.6270L` mean distance, `47151.5/1354.1` total/mean command energy, and
  `77.09/1142.74` force/moment RMS, restricting assisting-moment credit to the
  anterior targetward half-cycle regresses arrival to `34.8535` and raises
  loads to `82.35/1231.74`; joint phase is not a load-headroom proxy here. The
  assigned parent instead combines sign-consistent moment credit with
  normalized joint-speed pressure only on the extra redirect burst and reaches
  in `34.7105`, with `1.6228L` mean distance, `46985.9/1353.6` energy,
  `0.24023` relative-crossflow RMS, and `68.96/1036.40` loads—the only sampled
  variant improving all of those measures while preserving the visible route.
  Because that parent changed moment and speed release together, do not assign
  the benefit to either cue alone. Preserve carrier, raw-bearing mean
  steering/reserve, and base asymmetry; test further normalized actuation
  headroom only on the surplus burst. Reject it if capture or coherent upstream
  propulsion is lost, or if effort/load/limit relief does not justify any route
  cost. This boundary is limited to the shared prewarm and is falsified if a
  changed wake phase reverses the composite benefit.
