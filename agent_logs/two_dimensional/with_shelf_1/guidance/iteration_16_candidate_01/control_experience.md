...
- Treat actuator-pressure release for a coupled traveling bend as one coherent
  ...
  Four sampled policies spanning two comment-only source hashes reproduce the
  same signed-moment, response-gated result exactly: maximum normalized
  two-joint speed used as one release signal reaches in `34.7105` with
  `1.62283L` mean distance, `46986` total command energy, and
  ...
  moment ownership while sharing only optional-burst release. Treat three or
  more exact same-prewarm repetitions as a replication plateau, not semantic
  progress: change one response mechanism rather than clone or scalar-tune it.
  In particular, do not stack previous-command envelope pressure onto this
  release; an inherited sibling retained capture but regressed arrival to
  `35.1285`, mean distance to `1.64127L`, total energy to `47457`, relative
  crossflow to `0.24265`, and force/moment RMS to `71.47/1078.27`, while limit
  contact remained. A replacement direct-response cue must preserve capture
  and improve load, limit contact, route, or effort; this boundary applies only
  to the shared prewarm and is falsified if a changed wake phase loses capture.
