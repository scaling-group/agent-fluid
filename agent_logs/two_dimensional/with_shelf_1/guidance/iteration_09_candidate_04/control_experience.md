...
- Near-target release of half-cycle steering asymmetry is not an evidenced
  remedy for this allocator's actuator contact. Against the assigned parent,
  the sampled distance schedule preserved the same visible redirect and
  target capture and changed arrival `36.4705 -> 36.4540`, total energy
  `48700.1 -> 48678.8`, relative crossflow `0.23970 -> 0.23913`, and RMS
  force/moment `63.59/953.42 -> 63.25/950.58`; however, mean distance slightly
  worsened `1.68603L -> 1.68617L`, score fell `0.188574 -> 0.188396`, and both
  joints still touched exactly `4.537856` speed and `30.0` acceleration. Treat
  the small load reduction as terminal shaping, not saturation control. Do not
  repeat distance-only asymmetry schedules to solve the limit contact; test a
  joint-state mechanism that acts specifically on outward acceleration near
  the speed boundary while preserving braking and bearing-owned steering.
  Reconsider this negative result only if a changed wake phase reveals a
  capture/robustness advantage that the deterministic same-prewarm rollout
  cannot show.
