...
- Treat actuator-pressure release for a coupled traveling bend as one coherent
  ...
  Within the signed-moment, response-gated burst family, two exact copies using
  maximum normalized two-joint speed as one release signal reach in `34.7105`
  with `1.62283L` mean distance, `46986` total command energy, and
  ...
  moment ownership while sharing only optional-burst release. An inherited
  speed-only gate previously regressed arrival and load, so this lesson does
  not establish speed as a standalone wake proxy; it applies only to the
  compatible response-and-moment mechanism and is falsified by failed
  replication or lost capture under a changed wake phase.
