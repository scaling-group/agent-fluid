...
- Fast signed residuals have not earned disturbance-rejection status in this
  ...
  negative boundary.
- Hydrodynamic yaw has earned a narrower role as response credit, not as an
  additive route residual. Relative to the response-scheduled parent's
  `34.8205` arrival and `77.09/1142.74` RMS force/moment, a phase-agnostic
  gate that relieved only extra burst for same-sign body-frame moment retained
  the redirect/upstream route, arrived at `34.9415`, and lowered load to
  `71.86/1064.16`. Requiring that same moment to coincide with the anterior
  joint's targetward phase instead raised load to `82.35/1231.74`, despite a
  similar `34.8535` arrival. Thus moment sign can identify already-supplied
  yaw, but joint phase should define half-cycle actuation rather than also
  certify hydrodynamic response. Preserve mean bearing authority and posterior
  propulsion when refining this gate; test actuator-role separation or a true
  response history instead of another instantaneous phase product. This is
  bounded to the shared prewarm and is falsified if a changed wake phase loses
  capture or reverses the signed gate's load advantage.
