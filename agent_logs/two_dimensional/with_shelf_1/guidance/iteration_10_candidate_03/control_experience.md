...
- A response-scheduled half-cycle burst is now a distinct validated
  arrival/route mechanism, with a measured load cost. Relative to three
  identical evaluations of the constant-`0.25` asymmetry allocator, releasing
  extra asymmetry when the recent body-frame bearing is already closing
  improves arrival `36.4705 -> 34.8205`, mean distance
  `1.68603L -> 1.62700L`, and total command energy `48700 -> 47151` while
  preserving the visible redirect-and-upstream route. It raises mean command
  energy `1335.33 -> 1354.13`, RMS relative crossflow
  `0.23970 -> 0.24143`, and RMS force/moment
  `63.59/953.42 -> 77.09/1142.74`; both policies still contact the joint-speed
  and acceleration limits. Treat response scheduling as temporary redirect
  authority, not a general efficiency or load improvement. Preserve the base
  half-cycle, mean steering, raw-bearing reserve, and carrier when testing a
  response/load release gate; falsify the mechanism if capture is lost,
  arrival exceeds `36.4705`, or the load/effort cost survives without a route
  benefit. This remains bounded to the shared prewarm and is not evidence of
  changed-wake-phase robustness.
