...
- Joint-state half-cycle asymmetry is now a reproducible redirect mechanism,
  but extra response-gated burst authority carries a distinct load cost. Three
  sampled copies of the fixed-asymmetry parent reach identically in `36.4705`
  with mean distance `1.6860L`, total/mean command energy `48700/1335.3`, and
  force/moment RMS `63.59/953.42`; their keyframes show the same initial
  redirect and coherent upstream capture. Adding burst asymmetry only while
  body-frame bearing remains large and is not closing preserves that route,
  improves arrival to `34.8205`, mean distance to `1.6270L`, and total energy
  to `47151`, but raises mean energy to `1354.1` and force/moment RMS to
  `77.09/1142.74`, while both versions still touch the joint-speed and `30.0`
  command limits. Treat response gating as a route-compactness mechanism, not
  a load or saturation cure: preserve the carrier and base half-cycle control,
  and test release of only the extra burst against normalized measured load or
  disturbance rather than another gait-scalar retune. This implication is
  bounded to the common prewarm and is falsified if load-aware release loses
  capture, erases the arrival benefit, or fails to reduce the burst policy's
  force/moment RMS; changed wake phase remains the robustness test.
