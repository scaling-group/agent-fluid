...
- Targetward relative crossflow is an arrival/route-release cue, but neither a
  force credential nor a local-flow substitution makes it an unloading
  mechanism. Against the coherent-release result (`34.7105` arrival,
  `1.62283L` mean distance, `46985.9` total effort, `68.96/1036.40`
  force/moment RMS), relative-crossflow release reaches in `33.9460`, improves
  mean distance to `1.60066L`, lowers total effort to `46092.2`, and raises
  score `0.251012 -> 0.272045`, while loads worsen to `85.04/1244.16` and the
  released sheet retains the same redirect-and-leftward route. Requiring
  co-signed lateral force to confirm that crossflow reaches still sooner in
  `33.7260` with `1.59412L` mean distance and `45776.3` total effort, but
  force/moment RMS rise another `10.1%/11.2%` to `93.64/1383.73`; targetward
  local flow similarly reaches in `33.9900` at `94.55/1395.74`. A multiplicative
  confirmation narrows release and therefore restores optional burst authority;
  do not describe its faster, higher-load result as load consistency. Preserve
  target geometry as mean-route owner and direct wake cues only at the optional
  burst. If unloading is the objective, test normalized force magnitude as an
  independent burst-release guard rather than an assistance sign check, and
  require capture plus force/moment reduction below the force-confirmed child.
  This is bounded to the shared prewarm and is falsified if a changed wake phase
  loses capture or if independent load release merely tracks self-generated
  beat force and regresses the route without meaningful load relief.
