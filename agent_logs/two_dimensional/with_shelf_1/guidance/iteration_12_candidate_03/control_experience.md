...
- Localize response feedback to the extra half-cycle burst, and give physical
  ...
  `63.59/953.42` to `77.09/1142.74`. On that faster parent, a phase-agnostic
  assisting-sign moment gate retains the same visible redirect/upstream route
  and gives back only `0.121` arrival time while reducing force/moment about
  `6.8%` to `71.86/1064.16`. Restricting the same credit to the anterior
  targetward half-cycle is a concrete negative result: its route is still
  visually indistinguishable and arrival is `34.854`, but force/moment worsen
  beyond even the ungated parent to `82.35/1231.74`. Two magnitude-only gates
  also regress arrival; the stronger inverse-square variant worsens crossflow
  and force/moment to `0.2448` and `78.35/1216.43`. Treat signed body-frame
  moment as bounded response credit for surplus burst, not as a route residual,
  a magnitude-only withdrawal signal, or a beat-side selector. Preserve
  raw-bearing mean steering, reserve, base asymmetry, and the carrier; future
  tests should use smooth phase-agnostic credit (optionally capped) rather than
  the failed one-sided phase gate. This sign boundary is limited to the shared
  prewarm and is falsified if a changed wake phase reverses the assist relation
  or removes the load benefit.
