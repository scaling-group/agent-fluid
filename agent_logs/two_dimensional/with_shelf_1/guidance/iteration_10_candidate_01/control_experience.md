...
- Bearing response can schedule extra half-cycle authority, but its benefit is
  a route/arrival trade rather than load relief. Relative to the replicated
  fixed-asymmetry baseline, adding a burst only while raw bearing is present
  and its recent window is not closing preserves the same visible
  redirect-and-upstream route while improving arrival `36.4705 -> 34.8205`,
  mean distance `1.6860L -> 1.6270L`, total energy `48700 -> 47151`, and mean
  upstream velocity `-0.2978 -> -0.3118`; RMS force/moment nevertheless rise
  `63.59/953.42 -> 77.09/1142.74`, mean effort rises `1335.3 -> 1354.1`, and
  both joint speed and command limits remain touched. A distinct attempt to
  infer relief from normalized joint-speed magnitude arrived later at
  `37.262`, worsened mean distance to `1.7186L`, and raised force/moment to
  `69.02/1065.10` relative to the fixed baseline. Preserve response-triggered
  burst when optimizing compactness, but do not use joint velocity as a load
  proxy; test direct normalized physical-load magnitude only on optional burst
  authority while leaving mean steering and propulsion active. Falsify this
  boundary if a changed wake phase removes the arrival benefit, or if a later
  speed gate produces meaningful load relief without losing the route.
