...
- Targetward relative crossflow is an arrival/route-release cue, not yet an
  unloading mechanism. Against three sampled copies of the assigned parent's
  coherent-release result (`34.7105` arrival, `1.62283L` mean distance,
  `46985.9` total effort, `68.96/1036.40` force/moment RMS), replacing
  bearing-window closure with normalized body-frame relative crossflow reaches
  in `33.9460`, improves mean distance to `1.60066L`, lowers total effort to
  `46092.2`, and raises score `0.251012 -> 0.272045`. The released sheets show
  the same redirect-and-leftward route rather than a new corridor, while loads
  worsen to `85.04/1244.16` and speed/command limits remain touched. Inherited
  heading-rate and previous-command-pressure replacements likewise retain
  capture but regress route or load metrics, so do not retreat to another
  kinematic or scalar pressure proxy. Preserve target geometry as mean-route
  owner and confine direct wake assistance to optional burst release; when
  seeking the crossflow benefit without its load penalty, require a co-signed
  normalized force or moment observation to confirm that the fluid assists the
  requested turn. This is bounded to the shared prewarm and is falsified if a
  changed wake phase loses capture, if direct crossflow no longer improves
  arrival/route, or if a load-consistency gate cannot materially reduce its
  force/moment penalty.
