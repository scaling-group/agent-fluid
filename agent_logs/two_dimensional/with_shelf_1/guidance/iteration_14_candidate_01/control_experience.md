...
- Localize response feedback to the extra half-cycle burst, and distinguish
  hydrodynamic assistance from actuator pressure. Scheduling the burst from
  recent bearing closure improves the fixed-asymmetry route from `36.471` to
  `34.821`, mean distance from `1.6860L` to `1.6270L`, and total energy from
  `48700` to `47151`, but raises force/moment RMS from `63.59/953.42` to
  `77.09/1142.74`. Assisting-sign yaw credit alone gives back `0.121` arrival
  time and lowers load to `71.86/1064.16`; restricting that credit to the
  anterior targetward half-cycle instead raises load to `82.35/1231.74`, so
  joint phase is not an unloading proxy. In contrast, combining assisting-yaw
  credit with joint speed normalized by the carrier's `omega * amplitude`
  scale improves the ungated burst on arrival (`34.8205 -> 34.7105`), mean
  distance (`1.62700L -> 1.62283L`), total energy (`47151 -> 46986`), relative
  crossflow (`0.24143 -> 0.24023`), and force/moment (`77.09/1142.74 ->
  68.96/1036.40`) while preserving the visible redirect/upstream route.
  Treat speed as an actuator-headroom cue only for surplus burst, not as a wake
  load measurement: the evaluated child still touches joint-speed and
  acceleration limits. Localizing this speed gate independently to each joint
  retains capture but regresses the coupled gate on arrival (`34.7105 ->
  34.8590`), mean distance (`1.62283L -> 1.62681L`), total energy (`46986 ->
  47177`), and force/moment (`68.96/1036.40 -> 74.24/1105.81`). A partner's
  high speed is therefore useful body-level maneuver-pressure evidence in this
  two-joint carrier; do not split asymmetry from per-joint speed alone.
  Preserve raw-bearing mean steering, reserve, base asymmetry, and the carrier,
  and distinguish any future local headroom test with a direct actuator cue
  such as sign-aligned command pressure. These implications are limited to the
  shared prewarm and are falsified if a changed wake phase loses capture,
  reverses the assisting-moment sign, or makes independently localized relief
  improve both route and load over the coupled gate.
