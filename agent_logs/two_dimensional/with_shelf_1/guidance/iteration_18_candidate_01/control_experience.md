...
- Do not replace bearing-window closure with direct heading-rate response when
  the objective is to unload that optional burst. Against four exactly
  reproduced bearing-window successes (`34.7105` arrival, `1.62283L` mean
  distance, `46985.9/1353.65` total/mean effort, `68.96/1036.40` force/moment
  RMS), the inherited one-mechanism heading-rate replacement retained the same
  visible redirect-and-upstream route and arrived `0.0385` sooner with slightly
  lower effort, but worsened mean distance to `1.62669L` and raised force/moment
  RMS to `91.30/1389.74`; both speed and acceleration limits remained touched.
  Thus direct yaw response is an arrival/effort trade, not an evidenced
  load-release substitute in this topology. Preserve the window cue for
  unloading tests, and falsify this boundary only if a changed wake phase or a
  separately gated heading-rate child reduces load without losing capture or
  route compactness.
- Do not stack previous-command saturation on the coherent joint-speed release
  as a second proxy for unloading the same optional redirect burst. Against
  four exact baseline replications (`34.7105` arrival, `1.62283L` mean
  distance, `46985.9/1353.65` total/mean effort, `68.96/1036.40` force/moment
  RMS), the inherited maximum-previous-action gate retained capture and nearly
  identical keyframe topology but regressed to `35.0185`, `1.63314L`,
  `47340.0/1351.86`, and `69.66/1037.65`; both policies still touched the
  joint-speed and acceleration limits. Its small crossflow and mean-effort
  reductions therefore did not buy route, total-effort, load, or limit-contact
  improvement. Treat previous action as redundant with speed pressure in this
  burst-release role; after restoring the replicated baseline, test a
  semantically separate wake-disturbance residual rather than another
  saturation proxy. This negative boundary is limited to the shared prewarm
  and does not rule out previous-action feedback in a different actuator role
  or under a changed wake phase.
