...
- Localize response feedback to the transient redirect mechanism rather than
  ...
  than even the ungated burst. Joint targetward phase is therefore not an
  unloading proxy in this shared prewarm. Preserve raw-bearing mean steering,
  reserve, base asymmetry, and the carrier; test actual actuation/limit
  headroom only on the extra burst, and reject it if capture or the coherent
  leftward route is lost without a meaningful load/limit benefit. The
  inherited predictive-bearing child remains the failure boundary: putting
  bearing trend into persistent steering exited after `18.304` with negative
  progress. These implications are falsified if a changed wake phase reverses
  the signed-moment/load relation or makes joint phase reduce both load and
  route cost.
