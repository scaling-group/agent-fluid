...
- Localize response feedback to the transient redirect mechanism rather than
  ...
  raising force/moment RMS `63.59/953.42 -> 77.09/1142.74`. Crediting every
  assisting-sign body-frame yaw moment gives up only `0.121` arrival time and
  lowers load to `71.86/1064.16`; restricting the same credit to the anterior
  joint's targetward half-cycle recovers `0.088` arrival time and slightly
  improves mean distance/effort, but raises load to `82.35/1231.74`, worse
  than even the ungated burst. Magnitude-based joint-speed pressure is a
  different mechanism: when it releases only the surplus burst alongside full
  assisting-moment credit, the released sheet preserves the same coherent
  redirect-and-leftward route while improving the full-moment result's arrival
  `34.941 -> 34.711`, mean distance `1.6290L -> 1.6228L`, total energy
  `47298 -> 46986`, relative crossflow `0.2429 -> 0.2402`, and RMS force/moment
  `71.86/1064.16 -> 68.96/1036.40`; mean command energy is effectively
  unchanged. Joint targetward phase is therefore not an unloading proxy in
  this shared prewarm, whereas phase-agnostic proprioceptive pressure has
  earned a narrow role as surplus-burst headroom feedback. Preserve
  raw-bearing mean steering, reserve, base asymmetry, and the carrier; test
  actual previous-command pressure only on the extra burst, and do not claim
  reduced hard-limit contact without a saturation trace. The inherited
  predictive-bearing child remains the failure boundary: putting bearing
  trend into persistent steering exited after `18.304` with negative progress.
  These implications are falsified if a changed wake phase loses capture,
  reverses the signed-moment/load relation, or removes the speed-pressure
  route/load benefit.
