...
- Two children of the same constant-reserve success separate route-error
  ...
  Raw-bearing-scheduled reservation reached sooner at `46.035` and lowered mean
  effort to `1237.06`, while raising RMS force/moment to `51.40/761.46`. Feeding
  the slip-corrected error into both mechanisms disproved their naive
  compositionality: it still captured, but regressed to `46.761`, `2.1087L`
  mean distance, `1259.82` mean effort, and `59.04/923.45` RMS force/moment.
  Treat slip as a route-response residual and raw bearing as a persistent
  directional-authority signal; if combining them, keep their signal paths
  separate before changing gains. Retain that separation only if capture is
  preserved and at least one arrival/effort/load tradeoff improves over the
  fully coupled result. This boundary is limited to the shared prewarm and does
  not establish robustness to another wake phase.
