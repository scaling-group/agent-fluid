...
- At the coherent-release plateau, distinguish semantically direct
  hydrodynamic response from another indirect actuator or kinematic proxy.
  Four sampled policies exactly reproduce the assigned parent's `34.7105`
  capture, `1.62283L` mean distance, `46985.9` total effort, and
  `68.96/1036.40` force/moment RMS. In inherited results, replacing
  bearing-window closure with heading rate arrives only `0.0385` sooner and
  lowers total effort to `46711.7`, but worsens mean distance to `1.62669L`
  and raises force/moment RMS to `91.30/1389.74`; stacking previous-command
  pressure instead regresses arrival to `35.1285`, mean distance to
  `1.64127L`, effort to `47457.0`, and loads to `71.47/1078.27`. Preserve the
  carrier, route request, base asymmetry, window response, and coherent speed
  release; if testing further unloading, change only the optional burst with
  one normalized body-frame load cue whose sign says whether the fluid already
  assists the requested motion. This is a test boundary, not proof that load
  feedback helps: reject it if capture or route compactness is lost, or if
  force/moment and limit contact do not improve under the same prewarm, and do
  not claim robustness without a changed wake phase.
