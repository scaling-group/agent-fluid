...
- Joint-state-gated half-cycle steering is the first sampled mechanism in this
  lineage to materially accelerate the already successful route: added to the
  course-slip/raw-bearing allocator, it preserves the visible redirect and
  coherent upstream traverse while improving arrival `45.727 -> 36.471`, mean
  distance `2.0543L -> 1.6860L`, and total command energy `57829 -> 48700`.
  Treat this as evidence for phase-aware steering/propulsion coupling, not a
  generic load or efficiency gain, because mean command energy rises
  `1264.7 -> 1335.3` and RMS force/moment rise `49.36/799.31 ->
  63.59/953.42`. Conversely, the assigned parent's latest inherited child
  demonstrates that low effort and load alone are not safe optimization
  targets: at mean command energy `503.4` and RMS force/moment
  `11.62/328.69`, it never establishes upstream propulsion, moves `+2.392L`
  in x, and exits the right boundary after `18.304` with negative progress.
  Preserve leftward displacement and capture as gates before accepting effort
  or load relief; falsify the half-cycle mechanism if another wake phase loses
  capture, or if its arrival/route benefit disappears while the load increase
  remains.
