...
- Do not assume that two useful body-frame corrections remain useful when one
  signal is inserted at two controller interfaces. Relative to the raw-bearing
  reserve scheduler (`46.035` arrival, `-0.1883` score, mean command energy
  `1237.1`, force/moment RMS `51.40/761.46`), the course-slip-corrected
  scheduler still captured the target at `46.761` but regressed to `-0.2271`,
  used mean energy `1259.8`, and raised force/moment RMS to `59.04/923.45`;
  its sheet also follows a lower upstream traverse. The slip-only sibling's
  lower loads (`37.92/605.38`) therefore do not justify feeding slip into both
  residual steering and allocation. Preserve a single tested signal interface
  when combining mechanisms, and falsify any proposed combination against
  semantic capture, route topology, effort, and load—not individual-component
  successes. This boundary is specific to the current gain/schema and should
  be retested only with one interaction changed at a time.
