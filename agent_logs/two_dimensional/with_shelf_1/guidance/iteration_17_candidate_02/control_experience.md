...
- Treat actuator-pressure release for a coupled traveling bend as one coherent
  maneuver modulation, not two independent joint-local steering schedules or
  a standalone wake proxy. Four sampled solver copies plus the inherited
  step-15 result reproduce the shared maximum-speed release success exactly:
  `34.7105` arrival, `1.62283L` mean distance, `46986` total command energy,
  and `68.96/1036.40` force/moment RMS. Independent per-joint release instead
  reaches in `34.8590` with `1.62681L`, `47177`, and `74.24/1105.81`; gating
  moment credit by a single targetward joint phase raises load further to
  `82.35/1231.74`. The latest inherited step-16 descendant still captures but
  regresses arrival to `35.0845`, mean distance to `1.63577L`, total energy to
  `47419`, relative crossflow to `0.24277`, and force/moment RMS to
  `75.85/1146.58`. Its score-only log cannot support causal attribution to an
  unobserved code change, but it is a concrete boundary against accepting
  another same-topology perturbation merely because capture survives. Preserve
  carrier, raw-bearing mean steering, base asymmetry, signed-moment ownership,
  and shared speed release; localize the next observable mechanism to surplus
  burst authority and require a meaningful arrival/distance or load advantage.
  This applies only to the compatible response-and-moment family and is
  falsified by lost capture under a changed wake phase or failure to reduce
  limit contact alongside any claimed load benefit.
