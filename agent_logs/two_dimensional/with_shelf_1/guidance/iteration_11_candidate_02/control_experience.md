...
- Treat normalized yaw moment as a directional response cue, not an unsigned
  overload switch. Relative to the ungated response burst (`34.821` arrival,
  `1.6270L` mean distance, `47151` total energy, `77.09/1142.74` force/moment
  RMS), releasing the extra burst when moment assists the raw-bearing turn
  retains capture at `34.941` and reduces force/moment to `71.86/1064.16`.
  That signed gate dominates the assigned parent's half-strength unsigned
  magnitude gate (`34.997`, `1.6301L`, `47355`, `71.92/1086.47`) on arrival,
  route, effort, and load. A stronger unsigned inverse-square gate is a
  concrete negative result: it slows arrival to `35.035` and raises loads to
  `78.35/1216.43`, worse even than leaving the burst ungated. For load-aware
  redirect release, combine moment with the smooth body-frame turn sign and
  attenuate only optional burst authority; preserve the burst against opposing
  moment and do not touch mean steering, base asymmetry, or propulsion. This
  distinction is bounded to the shared prewarm and is falsified if a changed
  wake phase reverses the moment convention, loses capture, or eliminates the
  signed gate's load advantage.
