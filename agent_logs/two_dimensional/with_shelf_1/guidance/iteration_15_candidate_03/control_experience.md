...
- Treat actuator-pressure release for a coupled traveling bend as one coherent
  ...
  Within the signed-moment, response-gated burst family, three independently
  sampled copies using maximum normalized two-joint speed as one release signal
  reproduce `34.7105` arrival, `1.62283L` mean distance, `46986` total command
  energy, and
  ...
  `82.35/1231.74`. Preserve carrier, mean steering, base asymmetry, and signed
  moment ownership while sharing only optional-burst release. Do not add
  previous-command envelope pressure as a supposedly earlier release cue:
  layered onto common speed release it preserves the visible route and capture
  but regresses arrival to `35.1285`, mean distance to `1.64127L`, total energy
  to `47457`, relative crossflow to `0.24265`, and force/moment RMS to
  `71.47/1078.27`, while both speed and acceleration limits remain touched. An
  inherited speed-only gate also regressed arrival and load, so this lesson
  does not establish speed as a standalone wake proxy; it applies only to the
  compatible response-and-moment mechanism and is falsified by failed
  replication or lost capture under a changed wake phase. Test a new direct
  response signal by replacing, not stacking onto, this release and require
  reduced limit contact or load without route regression.
