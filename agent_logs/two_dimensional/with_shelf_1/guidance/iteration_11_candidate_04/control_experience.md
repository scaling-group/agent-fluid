...
- Do not use unsigned instantaneous yaw-moment magnitude as a generic release
  gate for the optional redirect burst. Against the response-only child
  (`34.821` arrival, `1.6270L` mean distance, `47151` total command energy,
  `0.2414` relative-crossflow RMS, and `77.09/1142.74` force/moment RMS), a
  full unsigned gate retained capture and the same visible route but regressed
  every listed measure to `35.035`, `1.6351L`, `47359`, `0.2448`, and
  `78.35/1216.43`. Partial unsigned and signed-assisting gates traded later
  arrival and higher total effort for lower force/moment RMS near
  `71.9/1064--1086`; none removed the shared joint-speed or command-limit
  contact. Moment is therefore a tradeoff signal here, not evidenced generic
  relief: keep it off persistent steering and do not claim combined route/load
  improvement from it. If releasing the extra burst again, test a
  sign-explicit actuator-pressure or separately calibrated assisting-response
  gate and require capture plus a meaningful route, effort, load, or limit
  benefit. This result is bounded to the shared prewarm and response-gated
  half-cycle topology; reconsider it only if another wake phase establishes a
  repeatable predictive relation between moment and useful turn response.
