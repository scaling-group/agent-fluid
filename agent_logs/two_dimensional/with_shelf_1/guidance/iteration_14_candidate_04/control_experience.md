...
- Treat signed yaw response and joint-speed pressure as a compatible
  surplus-burst release pair, not as interchangeable load sensors. Relative to
  the assigned ungated response-burst parent (`34.8205` arrival, `1.62700L`
  mean distance, `47151` total command energy, `0.24143` relative crossflow,
  and `77.09/1142.74` force/moment RMS), all-cycle assisting-moment credit plus
  bounded joint-speed-headroom release preserves the same visible
  redirect-and-upstream capture while improving every listed metric to
  `34.7105`, `1.62283L`, `46986`, `0.24023`, and `68.96/1036.40`.
  Targetward-half-cycle gating of moment credit instead reaches in `34.8535`
  and raises load to `82.35/1231.74`; do not use beat-side coincidence as an
  unloading proxy. Because the improving child adds both all-cycle moment
  credit and speed release, it validates the localized pair but does not
  identify speed magnitude alone as causal; inherited speed-only relief
  regressed arrival to `37.262` and raised load. Preserve mean steering,
  carrier, and base asymmetry. A later isolated allocator comparison sharpens
  the boundary: replacing the pair's common maximum-two-joint speed release
  with independent per-joint release preserved the visible route and capture
  but regressed arrival `34.7105 -> 34.8590`, mean distance
  `1.62283L -> 1.62681L`, total command energy `46986 -> 47177`, relative
  crossflow `0.24023 -> 0.24086`, and force/moment RMS
  `68.96/1036.40 -> 74.24/1105.81`. Treat speed pressure as a coordinated
  whole-body burst-release cue in this two-joint gait; do not localize it
  merely because joint speeds are measured separately. Test a new local
  allocator only when joint-resolved saturation or trajectory evidence shows
  asynchronous pressure. Falsify this guidance if replication or a changed
  wake phase loses capture or the common release no longer retains the joint
  route-effort-load advantage.
