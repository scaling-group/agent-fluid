...
- Treat normalized yaw moment as a directional response cue independent of
  motor phase, not an unsigned overload switch. Relative to the ungated burst
  (`34.821` arrival, `1.6270L` mean distance, `47151` total energy, and
  `77.09/1142.74` force/moment RMS), half-strength assisting-sign relief gives
  up only `0.033` arrival time at `47158` energy and lowers force/moment to
  `73.63/1098.03`; full signed relief reaches at `34.941` and lowers them
  further to `71.86/1064.16`. The full signed gate also dominates the sampled
  half-strength unsigned-magnitude gate (`34.997`, `1.6301L`, `47355`,
  `71.92/1086.47`) on arrival, route, effort, and load. Two concrete negatives
  bound the mechanism: stronger unsigned inverse-square relief reaches at
  `35.035` with worse-than-ungated `78.35/1216.43` loads, while qualifying full
  signed relief by anterior-joint targetward phase reaches at `34.854` but also
  raises loads above ungated to `82.35/1231.74`. Keep environmental response
  classification separate from motor phase, use the smooth body-frame turn
  sign, attenuate only optional burst authority, and preserve authority against
  opposing moment. Test a different normalized response cue rather than more
  moment-scale or phase gating if further load relief is needed. This boundary
  is limited to the shared prewarm and is falsified if a changed wake phase
  reverses the moment convention, loses capture, or removes the sign-only load
  advantage.
