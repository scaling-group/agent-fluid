function target_policy_params()
    return (
        ...
        yaw_moment_scale_L2=0.25,
        yaw_load_relief_fraction=0.50,
    )
end
...
@inline function response_scheduled_asymmetry(
    ...
    # A body-frame moment already assisting the raw-bearing turn request is a
    # fast response cue. Relieve only optional burst authority in that case;
    # an opposing moment retains the established redirect authority. Neither
    # the base half-cycle steering nor the mean route request is attenuated.
    assisting_yaw_moment = max(
        soft_sign * Float64(yaw_moment_L2),
        0.0,
    )
    yaw_response = tanh(
        assisting_yaw_moment /
        max(yaw_moment_scale_L2, eps(Float64)),
    )
    burst_availability = 1.0 -
        clamp(yaw_load_relief_fraction, 0.0, 1.0) * yaw_response
    unmet_redirect = error_strength * (1.0 - response) * burst_availability
...
function target_policy(state, params)
    ...
    active_asymmetry = response_scheduled_asymmetry(
        ...
        params.yaw_moment_scale_L2,
        params.yaw_load_relief_fraction,
    )
