...
@inline function response_scheduled_asymmetry(
    ...
    # Targetward lateral force is a direct, fast indication that fluid/body
    # interaction already assists the requested maneuver. Merge a capped force
    # credit with yaw-moment credit rather than creating another independent
    # pressure gate: opposing force receives no credit, and neither cue can
    # withdraw the base asymmetry or persistent target steering.
    assisting_force = max(
        soft_sign * Float64(lateral_force_L),
        0.0,
    )
    force_response = clamp(force_credit, 0.0, 1.0) * tanh(
