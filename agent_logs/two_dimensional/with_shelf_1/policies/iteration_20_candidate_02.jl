function target_policy_params()
    return (
        ...
        redirect_opposing_release_fraction=0.45,
...
@inline function response_scheduled_asymmetry(
    ...
    # alignment. Target-aligned relative crossflow is credited as useful
    # wake/body response. Target-opposed crossflow is kept out of route
    # steering and instead acts as a bounded compliance cue: it withdraws only
    # surplus redirect burst rather than commanding an opposite turn.
    soft_sign = raw_bearing / hypot(raw_bearing, 0.25 * scale)
    signed_crossflow = soft_sign * Float64(relative_crossflow_y)
    assisting_crossflow = max(signed_crossflow, 0.0)
    opposing_crossflow = max(-signed_crossflow, 0.0)
    crossflow_response = tanh(
        assisting_crossflow / max(crossflow_scale, eps(Float64)),
    )
    opposing_pressure = tanh(
        opposing_crossflow / max(crossflow_scale, eps(Float64)),
    )
    compliance_release = clamp(opposing_release_fraction, 0.0, 1.0) *
        opposing_pressure
    ...
    unmet_redirect = error_strength * (1.0 - crossflow_response) *
        (1.0 - moment_response) * (1.0 - clamp(burst_release, 0.0, 1.0)) *
        (1.0 - compliance_release)
...
function target_policy(state, params)
    ...
    # direction. Relative crossflow supplies a signed fast-response reflex:
    # targetward flow releases redundant burst, while opposing flow triggers
    # only a capped compliance release. This never changes mean route steering
    # and uses measured state rather than a clock or wake route.
    ...
    active_asymmetry = response_scheduled_asymmetry(
        ...
        params.redirect_opposing_release_fraction,
