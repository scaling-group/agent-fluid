function target_policy_params()
    return (
        ...
        redirect_load_scale_L2=0.30,
        redirect_load_exponent=2.0,
    )
end
...
@inline function response_and_load_scheduled_asymmetry(
    bearing,
    bearing_window_rate,
    moment_z_L2,
    control_period,
    ...
    # Release the redirect burst when the recent bearing window already closes.
    # The response is dimensionless angular change per carrier beat.
    ...
    # Normalized yaw-load magnitude gates only temporary extra asymmetry.  It
    # never reverses steering, encodes vortex phase, or removes the evaluated
    # base half-cycle request.
    normalized_load = abs(Float64(moment_z_L2)) /
        max(load_scale_L2, eps(Float64))
    exponent = max(Float64(load_exponent), 1.0)
    load_availability = inv(1.0 + normalized_load^exponent)
    unmet_redirect = error_strength * (1.0 - response) * load_availability
...
function target_policy(state, params)
    ...
    # A bounded mean-curvature residual follows the body-frame route error. Both
    ...
    # direction. Extra burst authority is released when the bearing already
    # responds or normalized yaw load grows; neither input is a clock or route.
    phase_velocity_scale = max(omega * amp, eps(Float64))
    bearing_window_rate = hasproperty(state, :bearing_window_rate) ?
        state.bearing_window_rate : 0.0
    active_asymmetry = response_and_load_scheduled_asymmetry(
        bearing,
        bearing_window_rate,
        state.moment_z_L2,
        ...
        params.redirect_load_scale_L2,
        params.redirect_load_exponent,
    )
    ...
    # Raw target geometry, rather than the response/load-gated residual, owns
    # finite reservation. This preserves redirect authority during large
    # errors while response feedback only releases the extra half-cycle burst.
