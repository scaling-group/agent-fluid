function target_policy_params()
    return (
        ...
        redirect_lateral_load_scale=1.10,
...
@inline function response_scheduled_asymmetry(
    ...
    # target alignment. Targetward relative crossflow is retained as the
    # evidenced environmental release cue.
    ...
    # Lateral-force magnitude is a load guard rather than a route-response
    # credential. High normalized load independently withdraws only surplus
    # redirect burst, regardless of whether the instantaneous force assists or
    # opposes the requested turn. The mean steering and base asymmetry remain.
    lateral_load_pressure = tanh(
        abs(Float64(lateral_force_L)) /
        max(lateral_load_scale, eps(Float64)),
    )
    burst_release_response = 1.0 -
        (1.0 - crossflow_response) * (1.0 - lateral_load_pressure)
    ...
    unmet_redirect = error_strength * (1.0 - burst_release_response) *
...
function target_policy(state, params)
    ...
    # while targetward crossflow, lateral-load pressure, and yaw moment do not
    # already justify release. This phase comes from measured state, not a
    # clock or wake route.
    ...
    active_asymmetry = response_scheduled_asymmetry(
        ...
        params.redirect_lateral_load_scale,
