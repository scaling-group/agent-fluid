function target_policy_params()
    return (
        ...
        redirect_base_assist_release=0.35,
...
@inline function response_scheduled_asymmetry(
    ...
    # Preserve useful fluid-induced yaw by unloading only a capped share of
    # phase-selective base steering. The continuous mean route request remains
    # intact, and opposing moment receives no credit.
    base_release = clamp(base_assist_release, 0.0, 1.0) *
        error_strength * moment_response
    retained_base = max(base_asymmetry, 0.0) * (1.0 - base_release)
    ...
    # the retained base asymmetry from either joint.
    unmet_redirect = error_strength * (1.0 - response) *
        (1.0 - moment_response) * (1.0 - clamp(burst_release, 0.0, 1.0))

    return clamp(
        retained_base + max(burst_asymmetry, 0.0) * unmet_redirect,
        0.0,
        0.75,
    )
end
...
function target_policy(state, params)
    ...
    active_asymmetry = response_scheduled_asymmetry(
        ...
        params.redirect_base_assist_release,
