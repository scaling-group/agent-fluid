function target_policy_params()
    return (
        ...
        redirect_force_credit_fraction=0.30,
...
@inline function response_scheduled_asymmetry(
    ...
    # Hydrodynamic yaw precedes the slower bearing response. Normalized lateral
    # force can also earn partial credit when it pushes toward the persistent
    # target side. Both cues release only surplus redirect burst; opposing
    # response leaves all established authority intact.
    ...
    force_response = clamp(force_credit_fraction, 0.0, 1.0) * tanh(
        assisting_force / max(force_scale, eps(Float64)),
    )
    useful_wake_response = clamp(
        1.0 - (1.0 - moment_response) * (1.0 - force_response),
        0.0,
        1.0,
    )
