...
@inline function response_scheduled_asymmetry(
    ...
    bearing_response = tanh(
        closing_rate * max(control_period, 0.0) / scale,
    )

    # Bearing closure also contains target-line translation. Credit measured
    # body yaw directly when it has the requested sign, so the optional redirect
    # burst can release as soon as a physical turn appears. Taking the stronger
    # response avoids double-counting the same turn through both observations.
    assisting_heading_rate = max(
        soft_sign * Float64(heading_rate),
        0.0,
    )
    heading_response = tanh(
        assisting_heading_rate * max(control_period, 0.0) / scale,
    )
    response = max(bearing_response, heading_response)
