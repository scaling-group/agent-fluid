function target_policy_params()
    return (
        ...
        redirect_moment_credit_limit=0.60,
    )
end
...
@inline function response_scheduled_asymmetry(
    ...
    # Credit measured yaw only when it assists the persistent target request.
    # The capped, phase-agnostic credit relieves surplus burst smoothly while
    # preserving a minimum redirect contribution through both beat sides.
    assisting_moment = max(
        soft_sign * Float64(moment_z_L2),
        0.0,
    )
    moment_response = clamp(moment_credit_limit, 0.0, 1.0) * tanh(
        assisting_moment / max(moment_scale, eps(Float64)),
    )
    unmet_redirect = error_strength * (1.0 - response) * (1.0 - moment_response)
...
function target_policy(state, params)
    ...
    active_asymmetry = response_scheduled_asymmetry(
        ...
        params.redirect_moment_credit_limit,
