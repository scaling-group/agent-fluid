function target_policy_params()
    return (
        ...
        joint_speed_guard_start=230.0 * pi / 180,
        joint_speed_guard_width=30.0 * pi / 180,
    )
end
...
@inline function outward_velocity_guard(
    command,
    joint_velocity,
    guard_start,
    guard_width,
)
    speed = abs(Float64(joint_velocity))
    start = max(abs(Float64(guard_start)), 0.0)
    width = max(abs(Float64(guard_width)), eps(Float64))
    band_fraction = clamp((speed - start) / width, 0.0, 1.0)
    smooth_guard = band_fraction^2 * (3.0 - 2.0 * band_fraction)

    # Near the speed envelope, release only acceleration that would increase
    # absolute joint speed. Opposite-signed braking remains fully available.
    outward = Float64(command) * Float64(joint_velocity) > 0.0
    return outward ? command * (1.0 - smooth_guard) : command
end
...
function target_policy(state, params)
    ...
    allocated_command1 = bounded_residual_mix(
        a1,
        steering,
        params.acceleration_envelope,
        reserve_fraction,
    )
    allocated_command2 = bounded_residual_mix(
        a2,
        tail_steering,
        params.acceleration_envelope,
        reserve_fraction,
    ...
    # The sampled successful policies all touch the joint-speed limit. Guard
    # only outward acceleration in a soft band below that boundary, after the
    # proven carrier/steering allocator has formed its command.
    command1 = outward_velocity_guard(
        allocated_command1,
        qd1,
        params.joint_speed_guard_start,
        params.joint_speed_guard_width,
    )
    command2 = outward_velocity_guard(
        allocated_command2,
        qd2,
        params.joint_speed_guard_start,
        params.joint_speed_guard_width,
