...
@inline function anterior_response_credited_asymmetry(
    active_asymmetry,
    base_asymmetry,
    bearing,
    moment_z_L2,
    ...
    # Environmental yaw earns credit only when it already assists the raw
    # target request. Relieve surplus anterior redirect, while leaving the
    # base half-cycle action and the posterior propulsion role intact.
    assisting_moment = max(soft_sign * Float64(moment_z_L2), 0.0)
    response_credit = tanh(
        assisting_moment / max(moment_scale, eps(Float64)),
    )
    bounded_base = clamp(base_asymmetry, 0.0, 0.75)
    extra_asymmetry = max(active_asymmetry - bounded_base, 0.0)
    return bounded_base + extra_asymmetry * (1.0 - response_credit)
end
...
function target_policy(state, params)
    ...
    anterior_asymmetry = anterior_response_credited_asymmetry(
        active_asymmetry,
        params.half_cycle_asymmetry,
        bearing,
        state.moment_z_L2,
