function target_policy_params()
    return (
        ...
        redirect_local_crossflow_scale=0.29,
...
@inline function response_scheduled_asymmetry(
    ...
    # alignment. Local crossflow is body-fixed and dimensionless; unlike
    # relative crossflow it does not explicitly mix the measured fluid motion
    # with the fish's own lateral velocity. Only flow aligned with the
    # requested turn is credited as environmental assistance.
    soft_sign = raw_bearing / hypot(raw_bearing, 0.25 * scale)
    assisting_local_crossflow = max(
        soft_sign * Float64(local_crossflow_y),
        0.0,
    )
    local_crossflow_response = tanh(
        assisting_local_crossflow /
        max(local_crossflow_scale, eps(Float64)),
    )
    ...
    unmet_redirect = error_strength * (1.0 - local_crossflow_response) *
...
function target_policy(state, params)
    ...
    # while targetward local crossflow and yaw moment are not already
    # assisting. This phase comes from joint state, not a clock or wake route.
    ...
    local_crossflow_y = hasproperty(state, :local_flow_velocity_body_U) ?
        Float64(state.local_flow_velocity_body_U[2]) : 0.0
    active_asymmetry = response_scheduled_asymmetry(
        ...
        params.redirect_local_crossflow_scale,
