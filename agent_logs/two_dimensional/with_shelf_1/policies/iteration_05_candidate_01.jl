...
@inline function bearing_scheduled_reserve(
    ...
    normalized_error = tanh(
        abs(redirect_error) / max(bearing_scale, eps(Float64)),
    )
    return clamp(
...
function target_policy(state, params)
    ...
    # Discount the part of target error already expressed as targetward
    # lateral motion. The soft normalization is bounded and remains continuous
    # as body speed approaches zero.
    ...
    redirect_error = bearing - params.course_response_gain * course_slip

    # A bounded mean-curvature residual turns the traveling bend toward the
    # target. Both inputs are dimensionless and body-fixed, so the command
    # follows current target geometry instead of a world-frame route.
    steering = params.steering_acceleration *
        tanh(redirect_error / params.steering_bearing_scale)
    tail_steering = params.tail_steering_ratio * steering

    # Large uncorrected direction errors prioritize the turning half-cycle.
    # Targetward course response releases that authority continuously toward
    # the evaluated cruise reservation.
