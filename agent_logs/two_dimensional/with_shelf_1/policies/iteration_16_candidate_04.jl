...
@inline function bounded_coupled_residual_mix(
    carrier1,
    carrier2,
    steering1,
    steering2,
    ...
    bounded_envelope = max(Float64(envelope), eps(Float64))
    fraction = clamp(reserve_fraction, 0.0, 1.0)
    reserve1 = clamp(
        fraction * steering1,
        -bounded_envelope,
        bounded_envelope,
    ...
    residual1 = carrier1 + (1.0 - fraction) * steering1
    residual2 = carrier2 + (1.0 - fraction) * steering2
    headroom1 = max(bounded_envelope - abs(reserve1), 0.0)
    headroom2 = max(bounded_envelope - abs(reserve2), 0.0)

    # Project the coupled carrier residual along one shared direction whenever
    # either joint exhausts its post-reserve envelope. This retains persistent
    # steering authority without independently clipping and reshaping the
    # two-joint traveling bend.
    scale1 = abs(residual1) > headroom1 ?
        headroom1 / max(abs(residual1), eps(Float64)) : 1.0
    scale2 = abs(residual2) > headroom2 ?
        headroom2 / max(abs(residual2), eps(Float64)) : 1.0
    coupled_scale = clamp(min(scale1, scale2), 0.0, 1.0)

    return (
        reserve1 + coupled_scale * residual1,
        reserve2 + coupled_scale * residual2,
    )
end
...
function target_policy(state, params)
    ...
    command1, command2 = bounded_coupled_residual_mix(
        a1,
        a2,
        steering,
        tail_steering,
