function target_policy_params()
    return (
        ...
        wake_crossflow_gain=0.06,
        wake_crossflow_scale=0.25,
...
@inline function bearing_scheduled_reserve(
    ...
    normalized_error = tanh(
        abs(Float64(bearing)) / max(bearing_scale, eps(Float64)),
    )
    return clamp(
        base_fraction + max(reserve_gain, 0.0) * normalized_error^2,
        0.0,
        1.0,
    )
end
...
function target_policy(state, params)
    ...
    # Relative crossflow is an anticipatory wake-disturbance signal: unlike
    # course slip, it is observed before the body has fully acquired lateral
    # motion. Keep it bounded and subordinate to target geometry so useful
    # wake motion and the propulsive carrier are not cancelled wholesale.
    relative_crossflow = Float64(state.relative_flow_velocity_body_U[2])
    crossflow_residual = params.wake_crossflow_gain * tanh(
        relative_crossflow /
        max(params.wake_crossflow_scale, eps(Float64)),
    )
    steering_error = bearing -
        params.course_slip_gain * course_slip -
        crossflow_residual
    ...
    # Keep persistent target geometry in charge of finite redirect authority.
    # Course slip may soften the residual above, but must not also withdraw the
    # steering reservation when the carrier saturates.
