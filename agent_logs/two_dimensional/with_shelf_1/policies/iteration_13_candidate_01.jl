function target_policy_params()
    return (
        ...
        redirect_acceleration_onset_fraction=0.80,
        redirect_acceleration_release_fraction=0.75,
    )
end
...
@inline function acceleration_limited_burst_release(
    previous_action,
    ...
    limit_pressure = clamp(
        (acceleration_ratio - onset) / max(1.0 - onset, eps(Float64)),
        0.0,
        1.0,
    )
    return clamp(release_fraction, 0.0, 1.0) * limit_pressure^2
...
@inline function combined_burst_release(speed_release, acceleration_release)
    speed_relief = clamp(speed_release, 0.0, 1.0)
    acceleration_relief = clamp(acceleration_release, 0.0, 1.0)
    return 1.0 - (1.0 - speed_relief) * (1.0 - acceleration_relief)
end
...
@inline function response_scheduled_asymmetry(
    ...
    # Speed and prior-command pressure are actuation-response signals, not
    # another route error. They withdraw only surplus burst and cannot remove
    # base asymmetry.
...
function target_policy(state, params)
    ...
    acceleration_release = acceleration_limited_burst_release(
        previous_action,
        params.acceleration_envelope,
        params.redirect_acceleration_onset_fraction,
        params.redirect_acceleration_release_fraction,
    )
    burst_release = combined_burst_release(
        speed_release,
        acceleration_release,
    )
