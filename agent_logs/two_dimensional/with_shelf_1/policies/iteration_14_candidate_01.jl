function target_policy_params()
    return (
        ...
        redirect_action_onset_fraction=0.85,
...
@inline function aligned_action_burst_release(
    previous_action1,
    previous_action2,
    steering_direction,
    action_envelope,
    ...
    # Only prior command already spent in the requested turn direction counts
    # as pressure. The maximum is shared across the coupled bend so actuator
    # relief cannot split the anterior and posterior response asymmetries.
    direction = Float64(steering_direction)
    soft_direction = direction / hypot(direction, 0.25)
    aligned_action = max(
        soft_direction * Float64(previous_action1),
        soft_direction * Float64(previous_action2),
        0.0,
    )
    action_ratio = aligned_action / max(action_envelope, eps(Float64))
    onset = clamp(onset_fraction, 0.0, 1.0 - eps(Float64))
    command_pressure = clamp(
        (action_ratio - onset) / max(1.0 - onset, eps(Float64)),
        0.0,
        1.0,
    )
    return clamp(release_fraction, 0.0, 1.0) * command_pressure^2
end
...
@inline function response_scheduled_asymmetry(
    ...
    # Joint response and direct command pressure are actuator cues, not route
    # errors. They can withdraw only surplus burst and never base asymmetry.
...
function target_policy(state, params)
    ...
    speed_release = speed_limited_burst_release(
        qd1,
        qd2,
    ...
    action_release = aligned_action_burst_release(
        state.previous_action[1],
        state.previous_action[2],
        steering_direction,
        params.acceleration_envelope,
        params.redirect_action_onset_fraction,
        params.redirect_action_release_fraction,
    )
    burst_release = 1.0 - (1.0 - speed_release) * (1.0 - action_release)
