...
@inline function response_scheduled_asymmetry(
    ...
    # Credit hydrodynamic yaw only when it assists the raw target request and
    # coincides with the anterior joint's targetward half-cycle. This releases
    # surplus burst without reshaping the recovery half-cycle or withdrawing
    # persistent mean steering.
    assisting_moment = max(
        soft_sign * Float64(moment_z_L2),
        0.0,
    )
    targetward_half_cycle = max(
        tanh(soft_sign * Float64(phase_velocity)),
        0.0,
    )
    moment_response = tanh(
        assisting_moment / max(moment_scale, eps(Float64)),
    ) * targetward_half_cycle
