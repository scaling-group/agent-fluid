...
@inline function response_scheduled_asymmetry(
    ...
    # Hydrodynamic yaw moment is a fast load signal. Release only the optional
    # redirect burst under a large normalized moment of either sign instead of
    # spending surplus authority fighting an opposing wake event. Persistent
    # mean steering and base asymmetry retain the target-directed turn.
    moment_pressure = tanh(
        abs(Float64(moment_z_L2)) / max(moment_scale, eps(Float64)),
    )
    ...
    unmet_redirect = error_strength * (1.0 - response) *
        (1.0 - moment_pressure) * (1.0 - clamp(burst_release, 0.0, 1.0))
