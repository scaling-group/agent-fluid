...
@inline function load_aware_response_asymmetry(
    bearing,
    bearing_window_rate,
    moment_z_L2,
    control_period,
    ...
    # Absolute normalized yaw load has no route sign to calibrate. It only
    # relieves the extra burst during high-load samples; the evidenced base
    # half-cycle asymmetry and propulsive carrier remain available.
    normalized_load = abs(Float64(moment_z_L2)) /
        max(load_scale, eps(Float64))
    load_gate = inv(1.0 + normalized_load^2)

    return clamp(
        base_asymmetry + max(burst_asymmetry, 0.0) * unmet_redirect * load_gate,
        0.0,
        0.75,
    )
end
...
function target_policy(state, params)
    ...
    # measured yaw load grows; this phase comes from joint state, not a clock
    # or wake route.
    phase_velocity_scale = max(omega * amp, eps(Float64))
    active_asymmetry = load_aware_response_asymmetry(
        bearing,
