...
@inline function response_scheduled_asymmetry(
    ...
    # raw-bearing request and the observed traveling bend is in a compatible
    # turn-aligned phase. The two phase contributions form one whole-body
    # response signal; moment relief cannot decouple the joint pair.
    assisting_moment = max(
        soft_sign * Float64(moment_z_L2),
        0.0,
    )
    phase_credit = 0.5 * (
        max(tanh(soft_sign * Float64(anterior_phase_velocity)), 0.0) +
        max(tanh(soft_sign * Float64(posterior_phase_velocity)), 0.0)
    )
    moment_response = phase_credit * tanh(
