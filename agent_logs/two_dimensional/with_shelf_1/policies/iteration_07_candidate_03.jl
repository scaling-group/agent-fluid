...
function target_policy(state, params)
    ...
    # Translation already carrying the fish toward the target and a recent
    # trend closing the body-frame bearing both soften redundant curvature.
    # The trend is normalized as angular change per carrier period; it is
    # response feedback, not an elapsed-time phase or route stage.
    ...
    steering_error = bearing -
        params.course_response_gain * course_slip +
        params.bearing_trend_gain * bearing_trend

    # A bounded mean-curvature residual turns the traveling bend toward the
    # target. Every input is normalized and body-fixed, so no world-frame
    # direction or prescribed vortex phase is encoded.
    ...
    # Persistent raw target geometry, not the response-corrected residual,
    # owns finite redirect authority. Translation and bearing trend may soften
    # redundant steering without also withdrawing its saturation reservation.
