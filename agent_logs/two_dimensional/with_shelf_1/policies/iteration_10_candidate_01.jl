...
@inline function response_scheduled_asymmetry(
    ...
    # Moment magnitude is used only to release optional burst authority. Its
    # sign and wake-phase lag are deliberately not interpreted, and neither
    # the base half-cycle steering nor the mean route request is attenuated.
    normalized_yaw_load = abs(Float64(yaw_moment_L2)) /
        max(yaw_moment_scale_L2, eps(Float64))
    yaw_load = tanh(normalized_yaw_load)
    burst_availability = 1.0 -
        clamp(yaw_load_relief_fraction, 0.0, 1.0) * yaw_load
