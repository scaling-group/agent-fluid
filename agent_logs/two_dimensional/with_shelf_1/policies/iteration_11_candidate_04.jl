function target_policy_params()
    return (
        ...
        response_sign_softness=0.25,
        joint_speed_limit=260.0 * pi / 180,
        limit_release_onset=0.80,
    )
end
...
@inline function outward_limit_headroom(
    phi_dot,
    previous_action,
    joint_speed_limit,
    ...
    speed_limit = max(abs(Float64(joint_speed_limit)), eps(Float64))
    action_limit = max(abs(Float64(acceleration_envelope)), eps(Float64))
    onset = clamp(Float64(release_onset), 0.0, 1.0 - eps(Float64))
    pressure = 0.0

    for joint_index in eachindex(phi_dot, previous_action)
        speed = Float64(phi_dot[joint_index])
        previous_acceleration = Float64(previous_action[joint_index])
        proximity = clamp(
            (abs(speed) / speed_limit - onset) / (1.0 - onset),
            0.0,
            1.0,
        )
        outward_push = clamp(
            speed * previous_acceleration / (speed_limit * action_limit),
            0.0,
            1.0,
        )
        pressure = max(pressure, proximity * outward_push)
    end

    # Smoothly release optional authority only when a near-limit joint was
    # still being accelerated outward. Braking samples retain full headroom.
    smooth_pressure = pressure^2 * (3.0 - 2.0 * pressure)
    return 1.0 - smooth_pressure
end

@inline function limit_aware_response_asymmetry(
    bearing,
    bearing_window_rate,
    phi_dot,
    previous_action,
    ...
    # Release the redirect burst after the observed bearing begins closing.
    # The recent bearing window supplies response state without a clock or a
    # hidden maneuver stage.
    softness = max(sign_softness, eps(Float64)) * scale
    soft_sign = raw_bearing / hypot(raw_bearing, softness)
    closing_rate = max(
    ...
    # The sampled moment gates did not improve both route and load. Instead,
    # use sign-explicit actuator response to avoid spending the optional burst
    # while a joint is pressed farther into its speed limit. The established
    # base half-cycle asymmetry and mean route request are never withdrawn.
    limit_headroom = outward_limit_headroom(
        phi_dot,
        previous_action,
        joint_speed_limit,
        acceleration_envelope,
    ...
    return clamp(
        base_asymmetry + max(burst_asymmetry, 0.0) * unmet_redirect * limit_headroom,
        0.0,
        0.75,
    )
end
...
function target_policy(state, params)
    ...
    # direction. Extra redirect authority is released once bearing responds or
    # observed actuator state shows outward speed-limit pressure; these gates
    # use response and joint state rather than a clock or a wake route.
    phase_velocity_scale = max(omega * amp, eps(Float64))
    active_asymmetry = limit_aware_response_asymmetry(
        bearing,
        ...
        params.response_sign_softness,
        params.joint_speed_limit,
        params.acceleration_envelope,
        params.limit_release_onset,
    )
