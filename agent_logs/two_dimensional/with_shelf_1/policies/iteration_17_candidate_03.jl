function target_policy_params()
    return (
        ...
        redirect_action_onset_ratio=0.85,
...
function target_policy(state, params)
    ...
    action_release = action_limited_burst_release(
        state.previous_action,
        params.acceleration_envelope,
        params.redirect_action_onset_ratio,
        params.redirect_action_release_fraction,
    )
    # Speed and previous-command pressure describe the same coupled actuation
    # envelope. Use the stronger cue coherently across both joints rather than
    # stacking releases or changing their relative wave envelope.
    burst_release = max(burst_release, action_release)
