function target_policy_params()
    return (
        ...
        redirect_action_onset_fraction=0.80,
...
@inline function headroom_limited_burst_release(
    qd1,
    qd2,
    previous_action,
    phase_speed_scale,
    ...
    speed_onset_ratio,
    speed_release_fraction,
    action_onset_fraction,
    action_release_fraction,
)
    speed_ratio = max(abs(Float64(qd1)), abs(Float64(qd2))) /
        max(phase_speed_scale, eps(Float64))
    speed_onset = clamp(speed_onset_ratio, 0.0, 1.0 - eps(Float64))
    speed_pressure = clamp(
        (speed_ratio - speed_onset) /
            max(1.0 - speed_onset, eps(Float64)),
        0.0,
        1.0,
    )^2
    ...
    ) / max(acceleration_envelope, eps(Float64))
    action_onset = clamp(action_onset_fraction, 0.0, 1.0 - eps(Float64))
    action_pressure = clamp(
        (action_ratio - action_onset) /
            max(1.0 - action_onset, eps(Float64)),
        0.0,
        1.0,
    )^2

    # The two proprioceptive observations describe the same missing headroom,
    # so take their maximum instead of double-counting coincident pressure.
    speed_release = clamp(speed_release_fraction, 0.0, 1.0) * speed_pressure
    action_release = clamp(action_release_fraction, 0.0, 1.0) * action_pressure
    return max(speed_release, action_release)
end
...
@inline function response_scheduled_asymmetry(
    ...
    # Headroom pressure is an actuation-response signal, not another route
...
function target_policy(state, params)
    ...
    burst_release = headroom_limited_burst_release(
        qd1,
        qd2,
        previous_action,
