function target_policy_params()
    return (
        ...
        redirect_action_onset_ratio=0.80,
        redirect_action_release_fraction=0.50,
    )
end
...
@inline function speed_limited_burst_release(
    ...
    speed_ratio = max(
        abs(Float64(anterior_velocity)),
        abs(Float64(posterior_velocity)),
    ) /
...
@inline function action_limited_burst_release(
    previous_action,
    ...
    action_ratio = max(
        abs(Float64(previous_action[1])),
        abs(Float64(previous_action[2])),
    ) / max(abs(Float64(acceleration_envelope)), eps(Float64))
    onset = clamp(onset_ratio, 0.0, 1.0 - eps(Float64))
    envelope_pressure = clamp(
        (action_ratio - onset) / max(1.0 - onset, eps(Float64)),
        0.0,
        1.0,
    )
    return clamp(release_fraction, 0.0, 1.0) * envelope_pressure^2
end
...
@inline function response_scheduled_asymmetry(
    ...
    # Hydrodynamic yaw receives credit only when it assists the persistent
    # raw-bearing request. Coordinated actuator pressure is a separate cue: it
    # releases only surplus burst without changing base half-cycle asymmetry,
    # mean steering, or carrier.
...
function target_policy(state, params)
    ...
    # The sampled common speed release outperformed independent per-joint
    # release. Preserve that coordination and add recent acceleration-envelope
    # pressure as a second direct actuator cue. Complementary composition is
    # bounded and can withdraw only the optional redirect burst.
    speed_burst_release = speed_limited_burst_release(
        qd1,
        qd2,
    ...
    action_burst_release = action_limited_burst_release(
        previous_action,
        params.acceleration_envelope,
        params.redirect_action_onset_ratio,
        params.redirect_action_release_fraction,
    )
    coordinated_burst_release = clamp(
        1.0 - (1.0 - speed_burst_release) * (1.0 - action_burst_release),
        0.0,
        1.0,
    )
