...
@inline function response_scheduled_asymmetry(
    ...
    # A smooth bearing sign makes both response observations continuous through
    # target alignment. Kinematic closure and assisting relative crossflow are
    # complementary evidence that the optional redirect burst can be released.
    soft_sign = raw_bearing / hypot(raw_bearing, 0.25 * scale)
    closing_rate = max(
        -soft_sign * Float64(bearing_window_rate),
        0.0,
    )
    kinematic_response = tanh(
    ...
    response = 1.0 -
        (1.0 - kinematic_response) * (1.0 - crossflow_response)
...
function target_policy(state, params)
    ...
    # while neither recent bearing closure nor targetward relative crossflow
    # indicates assistance. Beat phase comes from joint state, not a clock or
    # wake route.
