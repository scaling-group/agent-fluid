...
@inline function speed_limited_burst_release(
    ...
    speed_ratio = abs(Float64(joint_velocity)) /
...
@inline function response_scheduled_asymmetry(
    ...
    # raw-bearing request. Joint-local speed pressure is a separate actuator
    # cue: it releases this joint's surplus burst without changing the other
    # joint, base half-cycle asymmetry, mean steering, or carrier.
...
function target_policy(state, params)
    ...
    anterior_burst_release = speed_limited_burst_release(
        qd1,
        phase_velocity_scale,
        params.redirect_speed_onset_ratio,
        params.redirect_speed_release_fraction,
    )
    posterior_burst_release = speed_limited_burst_release(
        qd2,
        phase_velocity_scale,
        params.redirect_speed_onset_ratio,
        params.redirect_speed_release_fraction,
    ...
    anterior_asymmetry = response_scheduled_asymmetry(
        bearing,
        bearing_window_rate,
        state.moment_z_L2,
        anterior_burst_release,
    ...
    posterior_asymmetry = response_scheduled_asymmetry(
        bearing,
        bearing_window_rate,
        state.moment_z_L2,
        posterior_burst_release,
        params.control_period,
        params.steering_bearing_scale,
        params.redirect_moment_scale,
        params.half_cycle_asymmetry,
