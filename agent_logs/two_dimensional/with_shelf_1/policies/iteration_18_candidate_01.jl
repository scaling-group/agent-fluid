function target_policy_params()
    return (
        ...
        wake_moment_rejection_acceleration=2.0,
        wake_rejection_bearing_scale=0.35,
    )
end
...
@inline function response_scheduled_asymmetry(
    ...
    # Joint-speed pressure is an actuation-response signal, not another route
    # error. It withdraws only surplus burst and cannot remove base asymmetry.
...
@inline function target_aligned_yaw_rejection(
    bearing,
    moment_z_L2,
    bearing_scale,
    moment_scale,
    ...
    # Leave large target-directed turns entirely to the proven route
    # controller. Near alignment, softly oppose body-frame yaw loading without
    # inferring a vortex phase or a world-frame route.
    route_strength = tanh(
        abs(Float64(bearing)) / max(bearing_scale, eps(Float64)),
    )
    alignment = 1.0 - route_strength^2
    normalized_moment = tanh(
        Float64(moment_z_L2) / max(moment_scale, eps(Float64)),
    )
    return -max(max_acceleration, 0.0) * alignment * normalized_moment
end
...
function target_policy(state, params)
    ...
    route_steering = half_cycle_steering(
        mean_steering,
        steering_direction,
        qd1 / phase_velocity_scale,
        active_asymmetry,
    )
    tail_route_steering = params.tail_steering_ratio * half_cycle_steering(
        mean_steering,
        steering_direction,
    ...
    # Persistent target geometry and fast hydrodynamic yaw have distinct
    # ownership. A small opposing-moment residual is admitted only near route
    # alignment, so it cannot erase the validated initial redirect.
    yaw_rejection = target_aligned_yaw_rejection(
        bearing,
        state.moment_z_L2,
        params.wake_rejection_bearing_scale,
        params.redirect_moment_scale,
        params.wake_moment_rejection_acceleration,
    )
    steering = route_steering + yaw_rejection
    tail_steering = tail_route_steering +
        params.tail_steering_ratio * yaw_rejection
