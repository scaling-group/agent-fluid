...
@inline function response_scheduled_asymmetry(
    ...
    # alignment. Direct body-yaw response isolates the redirect maneuver from
    # line-of-sight sweep caused by translation or wake advection. The response
    # is the targetward heading change in one beat relative to the same angular
    # scale used by the steering residual.
...
function target_policy(state, params)
    ...
    # while measured body yaw is not already turning toward the target. This
    # phase comes from joint state, not a clock or wake route.
