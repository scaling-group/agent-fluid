...
@inline function response_scheduled_asymmetry(
    ...
    # Bearing closure mixes translation with reorientation and therefore lags
    # the actual redirect. Credit targetward body yaw as soon as it appears,
    # normalized by the angle turned in one beat. Opposing yaw contributes no
    # release, so base steering and the established redirect sign are intact.
    targetward_turn_rate = max(
        soft_sign * Float64(heading_rate),
        0.0,
    )
    turn_response = tanh(
        targetward_turn_rate * max(control_period, 0.0) / scale,
    )
    ...
    unmet_redirect = error_strength * (1.0 - response) * (1.0 - turn_response) *
