# Figure 5 CFD replay with VTK output at 0, 3 and 12 T, plus target capture.
# Uses the same case modules and environment settings as the 3D EvE engine
# projected_multiwake_policy_smoke3d_hero_v3_moving_window.jl.

ENV["GKSwstype"] = get(ENV, "GKSwstype", "100")

case_dir = get(ENV, "DOGFISH3D_CASE_DIR", @__DIR__)
...
include(joinpath(case_dir, "src", "moving_window3d.jl"))
include(joinpath(case_dir, "src", "virtual_domain3d.jl"))
...
function main()
    ...
    legacy_direct_quiescent_init = env_bool("DOGFISH3D_DIRECT_QUIESCENT_INIT", false)
    initialization_mode = env_string(
        "DOGFISH3D_INITIALIZATION_MODE",
        legacy_direct_quiescent_init ? "uniform_direct" : "snapshot",
    )
    initialization_mode in ("snapshot", "uniform_direct") || error(
        "DOGFISH3D_INITIALIZATION_MODE must be snapshot or uniform_direct",
    )
    direct_uniform_init = initialization_mode == "uniform_direct"
    snapshot_raw = strip(env_string("DOGFISH3D_PREWARM_SNAPSHOT", ""))
    direct_uniform_init && !isempty(snapshot_raw) && error(
        "uniform_direct initialization forbids DOGFISH3D_PREWARM_SNAPSHOT",
    )
    !direct_uniform_init && isempty(snapshot_raw) && error(
        "DOGFISH3D_PREWARM_SNAPSHOT is required for snapshot initialization",
    )
    snapshot_path = isempty(snapshot_raw) ? nothing : abspath(snapshot_raw)
    prewarm_report = if direct_uniform_init
        projected = config["projected_multiwake_3d"]
        direct_L = env_int("DOGFISH3D_ROLLOUT_L", source_spec.L)
        Dict{String, Any}(
            "status" => "not_applicable_uniform_direct_init",
            "runtime_resolution" => direct_L,
            "config_sha256" => sha256_file(config_path),
            "requested_max_dimensionless_dt" => Float64(config["free_swim"]["max_dimensionless_dt"]),
            "runtime_fin_x_start" => Float64(get(projected, "fin_x_start", direct_L < 32 ? 0.70 : 0.80)),
            "fin_min_root_half_cells" => direct_L < 32 ? 0.50 : 1.0,
            "fin_min_midplane_half_cells" => direct_L < 32 ? 0.50 : 0.0,
        )
    else
    ...
    moving_window = env_bool("DOGFISH3D_MOVING_WINDOW", false)
    moving_window_threshold_cells = env_int("DOGFISH3D_MOVING_WINDOW_THRESHOLD_CELLS", 4)
    moving_window_threshold_cells >= 1 || error(
        "DOGFISH3D_MOVING_WINDOW_THRESHOLD_CELLS must be positive",
    )
    virtual_domain_enabled = env_bool("DOGFISH3D_VIRTUAL_DOMAIN_ENABLED", false)
    virtual_domain_x_L = env_float("DOGFISH3D_VIRTUAL_DOMAIN_X_L", 24.0)
    virtual_domain_y_L = env_float("DOGFISH3D_VIRTUAL_DOMAIN_Y_L", 16.0)
    virtual_domain_center_margin_L = env_float(
        "DOGFISH3D_VIRTUAL_DOMAIN_CENTER_MARGIN_L", 0.8,
    )
    virtual_domain_enabled && !moving_window && error(
        "the inertial virtual-domain boundary requires DOGFISH3D_MOVING_WINDOW=true",
    )
    virtual_domain_x_L > 2 * virtual_domain_center_margin_L || error(
        "DOGFISH3D_VIRTUAL_DOMAIN_X_L must exceed twice the center margin",
    )
    virtual_domain_y_L > 2 * virtual_domain_center_margin_L || error(
        "DOGFISH3D_VIRTUAL_DOMAIN_Y_L must exceed twice the center margin",
    )
    ...
    moving_window && !isempty(runtime_spec.cylinder_centers_L) && error(
        "the moving-window reference replay requires an obstacle-free domain",
    )
    world_initial_center_L = (
        env_float("DOGFISH3D_WORLD_INITIAL_CENTER_X_L", runtime_spec.initial_center_L[1]),
        env_float("DOGFISH3D_WORLD_INITIAL_CENTER_Y_L", runtime_spec.initial_center_L[2]),
    )
    state = Base2D.FreeSwimState(
        SVector(T(world_initial_center_L[1]) * Lf, T(world_initial_center_L[2]) * Lf),
        T(initial_heading_deg * DEG),
        SVector(zero(T), zero(T)),
        T(initial_yaw_rate_T / Float64(Lf)),
    )
    frame_anchor_L = (
        env_float("DOGFISH3D_FRAME_ANCHOR_X_L", runtime_spec.initial_center_L[1]),
        env_float("DOGFISH3D_FRAME_ANCHOR_Y_L", runtime_spec.initial_center_L[2]),
    )
    frame_anchor = SVector(T(frame_anchor_L[1]) * Lf, T(frame_anchor_L[2]) * Lf)
    initial_frame_origin = moving_window ?
        state.center - frame_anchor : SVector(zero(T), zero(T))
    all(value -> isinteger(Float64(value)), initial_frame_origin) || error(
        "the initial moving-window origin must be an integer number of cells",
    )
    frame_origin = initial_frame_origin
    local_state = moving_window ? moving_window_local_state(state, frame_origin) : state
    ...
    moving_window_scalar_scratch = moving_window ? similar(sim.flow.p) : nothing
    moving_window_vector_scratch = moving_window ? similar(sim.flow.u) : nothing
    moving_window_shift_events = NamedTuple[]
    ...
    vtk_times = [0.0, 3.0, 12.0]  # visualization-only pre-capture schedule
    vtk_index = 1
    ...
    function write_final_capture_vtk_frame!()
        vtk_stream === nothing && return nothing
        termination == "capture" || return nothing
        T3D.write_free_swim_vtk_frame_3d!(
            vtk_stream,
            sim,
            local_state,
    ...
    if midplane_enabled
        ...
        println(midplane_csv, "frame,elapsed,file,center_x_L,center_y_L,local_center_x_L,local_center_y_L,frame_origin_x_L,frame_origin_y_L,heading_rad,phi1_rad,phi2_rad")
    end
        ...
        while midplane_index <= length(midplane_times) && elapsed_now + 1.0e-8 >= midplane_times[midplane_index]
            ...
            println(
                ...
                join((
                    ...
                    Float64(state.center[1] / Lf),
                    Float64(state.center[2] / Lf),
                    Float64(local_state.center[1] / Lf),
                    Float64(local_state.center[2] / Lf),
                    Float64(frame_origin[1] / Lf),
                    Float64(frame_origin[2] / Lf),
                    Float64(state.theta),
                    joint.phi1,
                    joint.phi2,
                ), ","),
    ...
    while T(sim_time(sim)) + eps(T) < runtime_target_time && steps < max_steps
        ...
        loads = fish_force_moment(
            sim, fish_body, local_state.center, z_plane;
            locked_diagnostics=steps % 50 == 0,
        )
        head = material_head_position(
            state, assembled_params, Lf, raw_t0, body_sdf, mass_profile_2d, reference_model,
            ; centroid_caudal_fin, centroid_fin_material_density,
        )
        local_head = moving_window ? head - frame_origin : head
        raw_flow = centered_flow_velocity_xy(
            sim, SVector(local_head[1], local_head[2], z_plane),
        )
        ...
        next_local_state = moving_window ?
            moving_window_local_state(next_state, frame_origin) : next_state
        body_state = Base2D.free_swim_step_body_state(
            local_state, next_local_state, raw_dt,
        )
        fish_body = T3D.compose_body_3d(
            body_sdf,
            T3D.free_swim_map_3d(
        ...
        steps += 1
        if moving_window
            origin_delta_cells = moving_window_shift_cells(
                local_state.center, frame_anchor, moving_window_threshold_cells,
            )
            if any(value -> !iszero(value), origin_delta_cells)
                shift = shift_uniform_flow_window!(
                    sim,
                    moving_window_scalar_scratch,
                    moving_window_vector_scratch,
                    origin_delta_cells,
                    inflow,
                )
                frame_origin += SVector(T(origin_delta_cells[1]), T(origin_delta_cells[2]))
                local_state = moving_window_local_state(state, frame_origin)
                fish_body = T3D.compose_body_3d(
                    body_sdf,
                    T3D.free_swim_map_3d(
                        design,
                ...
                sim.body = fish_body ∪ cylinders
                WaterLily.measure!(sim, WaterLily.time(sim.flow))
                push!(moving_window_shift_events, (
                    step=steps,
                    elapsed_T=Float64(sim_time(sim)) - start_time,
                    origin_delta_cells=Tuple(origin_delta_cells),
                    array_shift_cells=(shift.shift_x, shift.shift_y),
                    frame_origin_L=Tuple(Float64.(frame_origin ./ Lf)),
                    local_center_L=Tuple(Float64.(local_state.center ./ Lf)),
                ))
            end
        end
        ...
        push!(traces, (
            ...
            local_center=Tuple(Float64.(local_state.center ./ Lf)),
            frame_origin=Tuple(Float64.(frame_origin ./ Lf)),
            distance_L,
        ...
        virtual_exit = virtual_domain_enabled ? virtual_domain_exit(
            state.center,
            new_head,
            Lf,
            virtual_domain_x_L,
            virtual_domain_y_L,
            virtual_domain_center_margin_L,
        ) : nothing
        if virtual_exit !== nothing && virtual_exit.outside
            status = "failed"
            termination = "left_domain"
            failure_message = @sprintf(
                "fish left %.6fL x %.6fL virtual field: center=(%.6fL, %.6fL), head=(%.6fL, %.6fL), center_margin=%.6fL",
                virtual_domain_x_L,
                virtual_domain_y_L,
                virtual_exit.center_L[1],
                virtual_exit.center_L[2],
                virtual_exit.head_L[1],
                virtual_exit.head_L[2],
                virtual_domain_center_margin_L,
            )
            break
        end
        domain_x_L, domain_y_L = runtime_spec.domain_scale_L[1:2]
        new_local_head = moving_window ? new_head - frame_origin : new_head
        head_outside = !(0.0 <= Float64(new_local_head[1] / Lf) <= domain_x_L) ||
            !(0.0 <= Float64(new_local_head[2] / Lf) <= domain_y_L)
        center_outside = !(0.0 <= Float64(local_state.center[1] / Lf) <= domain_x_L) ||
            !(0.0 <= Float64(local_state.center[2] / Lf) <= domain_y_L)
    ...
    report = (
        schema="dogfish.projected_multiwake_policy_smoke3d.moving_window.v1",
        status,
        termination,
        failure_message,
        ...
        initialization_mode,
        direct_uniform_initial_condition=direct_uniform_init,
        direct_quiescent_init=direct_uniform_init && all(iszero, inflow),
        prewarm_snapshot=snapshot_path,
        prewarm_snapshot_sha256=snapshot_path === nothing ? nothing : sha256_file(snapshot_path),
        ...
        control_mode,
        flow_velocity_L_per_T=Tuple(Float64.(inflow)),
        moving_window,
        moving_window_contract=moving_window ? (
            coordinates="x_local=x_world-frame_origin",
            velocity="stored velocity components remain inertial/world values",
            recentering="integer-cell lossless translation of u, u0, and p",
            entering_fluid="newly exposed cells use the inertial far-field velocity",
            entering_velocity=Tuple(Float64.(inflow)),
            threshold_cells=moving_window_threshold_cells,
        ) : nothing,
        virtual_domain=virtual_domain_enabled ? (
            domain_L=(virtual_domain_x_L, virtual_domain_y_L),
            center_margin_L=virtual_domain_center_margin_L,
            center_rule="margin <= center <= domain-margin",
            head_rule="0 <= head <= domain",
            exit_termination="left_domain",
        ) : nothing,
        frame_anchor_local_L=frame_anchor_L,
        initial_frame_origin_L=Tuple(Float64.(initial_frame_origin ./ Lf)),
        final_frame_origin_L=Tuple(Float64.(frame_origin ./ Lf)),
        moving_window_shift_count=length(moving_window_shift_events),
        moving_window_shift_events,
        ...
        fish_initial_center_L=world_initial_center_L,
        source_config_initial_center_L=runtime_spec.initial_center_L,
