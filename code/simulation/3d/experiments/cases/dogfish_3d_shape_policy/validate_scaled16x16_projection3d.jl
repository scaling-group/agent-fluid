# Dry construction gate for the strict 2D -> 3D projected multiwake case.
# This parses and constructs the obstacle bodies but never creates a Simulation.
...
config_path = get(
    ...
    joinpath(
        case_dir,
        "configs",
        "free_swim_multiwake_target_secondrow_capture0p5_scaled16x16_l32_3d.toml",
    ),
)
source_path = joinpath(
    ...
    "configs",
    "free_swim_multiwake_target_secondrow_capture0p5_scaled16x16_l64.toml",
)
...
@assert spec.success_radius_L == source_episode["success_radius_L"]
...
@assert spec.target_L[2] ==
    sum(center[2] for center in spec.cylinder_centers_L) /
    length(spec.cylinder_centers_L)
