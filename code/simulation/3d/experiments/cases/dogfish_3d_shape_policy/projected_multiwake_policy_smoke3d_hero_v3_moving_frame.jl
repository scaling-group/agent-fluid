...
include(joinpath(case_dir, "src", "translating_frame3d.jl"))
...
function main()
    ...
    prewarm_report = JSON.parsefile(prewarm_report_path)
    prewarm_report["status"] == "ok" || error("prewarm report is not successful")
    ...
    moving_frame = env_bool("DOGFISH3D_MOVING_FRAME", false)
    moving_frame && any(abs(value) > eps(T) for value in inflow) && error(
        "the translating-frame validation currently requires quiescent global flow",
    )
    ...
    moving_frame && !isempty(runtime_spec.cylinder_centers_L) && error(
        "the translating-frame reference replay requires an obstacle-free domain",
    )
    ...
    frame_origin = moving_frame ? state.center - frame_anchor : SVector(zero(T), zero(T))
    initial_frame_origin = frame_origin
    frame_velocity = SVector(zero(T), zero(T))
    local_state = moving_frame ?
        translating_frame_local_state(state, frame_origin, frame_velocity) : state
    ...
    frame_bc = moving_frame ? translating_frame_bc(T, backend.mem) : nothing
    u_boundary = moving_frame ? frame_bc : inflow
    ...
    sim = if backend.name == "cuda"
        if moving_frame
            cuda = Base2D.CUDA_MODULE[]
            cuda === nothing && error("CUDA backend was requested but CUDA is unavailable")
            cuda.allowscalar(() -> Base.invokelatest(simulation_builder))
        else
            Base.invokelatest(simulation_builder)
        end
    else
        simulation_builder()
    end
    ...
    while T(sim_time(sim)) + eps(T) < runtime_target_time && steps < max_steps
        ...
        local_head = moving_frame ? head - frame_origin : head
        raw_frame_flow = centered_flow_velocity_xy(
            sim, SVector(local_head[1], local_head[2], z_plane),
        )
        raw_flow = moving_frame ? raw_frame_flow + frame_velocity : raw_frame_flow
        ...
        next_frame_origin = moving_frame ? next_state.center - frame_anchor : frame_origin
        next_frame_velocity = moving_frame ? next_state.velocity : frame_velocity
        next_local_state = moving_frame ?
            translating_frame_local_state(
                next_state, next_frame_origin, next_frame_velocity,
            ) : next_state
        if moving_frame
            set_translating_frame_segment!(
                frame_bc, raw_t0, frame_velocity, next_frame_velocity, raw_dt,
            )
        end
        ...
        sim.flow.Δt[end] = min(T(sim.flow.Δt[end]), raw_dt_cap)
        state = next_state
        frame_origin = next_frame_origin
        frame_velocity = next_frame_velocity
        ...
        push!(traces, (
            ...
            frame_velocity=Tuple(Float64.(frame_velocity)),
            distance_L,
        ...
        new_local_head = moving_frame ? new_head - frame_origin : new_head
    ...
    report = (
        schema="dogfish.projected_multiwake_policy_smoke3d.moving_frame.v1",
        status,
        termination,
        failure_message,
        ...
        prewarm_snapshot_sha256=sha256_file(snapshot_path),
        ...
        control_mode,
        moving_frame,
        translating_frame_contract=moving_frame ? (
            coordinates="x_local=x_world-frame_origin",
            velocity="u_local=u_world-frame_velocity",
            far_field="quiescent world water is -frame_velocity in the CFD frame",
            frame_follow="fish material-reference center held at frame_anchor_local_L",
            acceleration="WaterLily differentiates the piecewise-linear functional uBC",
        ) : nothing,
        ...
        final_frame_velocity_U=Tuple(Float64.(frame_velocity)),
