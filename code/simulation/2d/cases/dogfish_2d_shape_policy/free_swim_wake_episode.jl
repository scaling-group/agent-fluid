...
function build_wake_swim_simulation(
    ...
    flow_speed > 0f0 || error("flow_speed must be positive")
...
function wake_main(;
    ...
    while termination == "horizon" && sim_time(sim) < release_end_time
        ...
        target_reached_now =
            multiwake_target && success_radius > 0 && distance <= success_radius
        ...
        if (plot_frames || render_frames || vtk_stream !== nothing) &&
           (scheduled_frame_due || diagnostic_frame_due || target_reached_now)
            frame_index += 1
            if vtk_stream !== nothing
                DogfishShapePolicyTestbed.write_free_swim_vtk_frame!(
                    vtk_stream,
