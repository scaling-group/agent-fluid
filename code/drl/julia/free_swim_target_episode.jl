testbed_root = get(ENV, "PPO_TESTBED_ROOT", normpath(joinpath(@__DIR__, "..", "..", "simulation", "2d")))
case_dir = joinpath(testbed_root, "cases", "dogfish_2d_shape_policy")
...
function target_control_design(; control_period=1.0f0)
    ...
    schema = (
        phi1=(lower=-1.2f0, upper=1.2f0),
        phi2=(lower=-1.2f0, upper=1.2f0),
        phi_dot1=(lower=-20.0f0, upper=20.0f0),
        phi_dot2=(lower=-20.0f0, upper=20.0f0),
        center1=(lower=0.1f0, upper=0.5f0),
        center2=(lower=0.5f0, upper=0.9f0),
        half_width1=(lower=0.04f0, upper=0.20f0),
        half_width2=(lower=0.04f0, upper=0.20f0),
        reference_time=(lower=-1.0f6, upper=1.0f6),
        inv_L=(lower=0.0f0, upper=1.0f0),
        control_period=(lower=0.1f0, upper=5.0f0),
    )
    return (
        version="dogfish.target_control.v1",
        lineage=(
            body_regime=:fixed_baseline_spine_swept_body_plan,
            motion_regime=:state_feedback_distributed_joint_bend,
            notes="target-reaching controller owns joint angular acceleration; morphology is locked",
        ),
        morphology=(
            schema=NamedTuple(),
            params=NamedTuple(),
        ...
        experiments=[
            (
                id="target_reach_seed_controller",
                assay=(
                    family=:free_swim_target_reach_2d,
                    version="v0",
                    purpose=:selection,
                    capability_axis=:target_reaching,
                    params=NamedTuple(),
                ),
...
function free_swim_reference_center(params, profile, Lf)
    ...
    return DogfishShapePolicyTestbed.deformed_body_centroid(
...
function body_frame_observation(
    ...
    velocity_body_U = SVector(Float64(velocity_body[1]), Float64(velocity_body[2]))
    force_body_L = normalized_body_vector(force_body, Lf)
    moment_z_L2 = Float64(moment_z) / max(Float64(Lf)^2, eps(Float64))
    return (
        head=head,
        head_L=normalized_body_vector(head, Lf),
        target_delta=target_delta,
        target_delta_L=normalized_body_vector(target_delta, Lf),
        target_body=target_body,
        target_body_L=current_target_body_L,
        previous_target_body=previous_target_body,
        previous_target_body_L=normalized_body_vector(previous_target_body, Lf),
        ...
        velocity_body_U=velocity_body_U,
        force_body=force_body,
        force_body_L=force_body_L,
        distance=distance,
        distance_L=current_distance_L,
        previous_distance=previous_distance,
        previous_distance_L=Float64(previous_distance) / Float64(Lf),
        ...
        forward_distance_L=Float64(forward_distance) / Float64(Lf),
        lateral_distance=lateral_distance,
        lateral_distance_L=Float64(lateral_distance) / Float64(Lf),
        bearing=bearing,
...
function render_target_episode_frame(
    ...
    plt = plot(
        ...
        xlabel="x",
        ylabel="y",
        title=@sprintf("target episode | t=%.3f / %.3f | distance=%.3f L", elapsed_time, horizon, distance / Float64(Lf)),
        margin=6Plots.mm,
...
function main()
    ...
    output_root = DogfishShapePolicyTestbed.configured_string(
        ...
        joinpath(pwd(), "logs", "free_swim_target_episode"),
    )
    ...
    legacy_cycles = DogfishShapePolicyTestbed.configured_optional_float32(config, "cycles", "DOGFISH_FREE_SWIM_CYCLES")
    ...
    vtk_force_trigger = DogfishShapePolicyTestbed.configured_optional_float32(
        config,
        "vtk_force_trigger",
        "DOGFISH_TARGET_VTK_FORCE_TRIGGER",
    )
    vtk_speed_trigger = DogfishShapePolicyTestbed.configured_optional_float32(
        config,
        "vtk_speed_trigger",
        "DOGFISH_TARGET_VTK_SPEED_TRIGGER",
    )
    vtk_trigger_min_interval = Float64(DogfishShapePolicyTestbed.configured_float32(
        config,
        "vtk_trigger_min_interval",
        "DOGFISH_TARGET_VTK_TRIGGER_MIN_INTERVAL",
        0.01f0,
    ))
    ...
    sim, state, period, morphology_params, assembled_params, resolved_backend, profile =
        DogfishShapePolicyTestbed.build_free_swim_simulation(
            design,
            experiment;
            L,
            backend,
    ...
    rollout_horizon = requested_horizon === nothing ?
        (legacy_cycles === nothing ? 5.5 : Float64(legacy_cycles) * period) :
        Float64(requested_horizon)
    horizon_source = requested_horizon === nothing ?
        (legacy_cycles === nothing ? "default_10T_reference" : "legacy_cycles_x_control_timescale") :
        "target_horizon"
    ...
    DogfishShapePolicyTestbed.set_free_swim_body!(
        sim,
        design,
        sdf,
        profile,
    ...
    while sim_time(sim) < rollout_horizon
        ...
        force, moment_z = DogfishShapePolicyTestbed.free_swim_body_force_moment(sim, state.center)
        ...
        if !(
            ...
            failure_message = "free-swim target episode state became nonfinite"
            break
        end
        ...
        diagnostic_frame_due = vtk_stream !== nothing && trigger_interval_ok && (force_triggered || speed_triggered || dynamics_unstable)

        if (render_frames || vtk_stream !== nothing) && (scheduled_frame_due || diagnostic_frame_due)
            frame_index += 1
            ...
            scheduled_frame_due && (next_frame_time += frame_dt)
    ...
    video_path = render_frames && frame_index > 0 ?
        encode_mp4(frames_dir, joinpath(output_root, "target_episode.mp4"), fps) :
        nothing
    vtk_collection = vtk_stream === nothing ?
        nothing :
        DogfishShapePolicyTestbed.close_free_swim_vtk_stream!(
            vtk_stream;
            status=termination in ("reached_target", "horizon", "wall_time_limit") ? "complete" : "failed",
        )
    ...
    action_energy = effort.command_energy_mean
    ...
    open(trajectory_path, "w") do io
        println(io, "time,center_x,center_y,head_x,head_y,distance,distance_reward,cumulative_distance_reward,velocity_x,velocity_y,force_x,force_y,moment_z,heading,phi1,phi2,phi_dot1,phi_dot2,phi_ddot1,phi_ddot2")
        for index in eachindex(times)
            ...
            println(io, join((times[index], center[1], center[2], head[1], head[2], distances[index], reward_metrics.distance_rewards[index], reward_metrics.cumulative_distance_rewards[index], velocity[1], velocity[2], force[1], force[2], moments[index], headings[index], joint_row...), ","))
        end
    end

    summary = Dict(
        "schema_version" => "dogfish.free_swim_target_episode.v1",
        ...
        "horizon_sim_time" => Float64(rollout_horizon),
        "horizon_source" => horizon_source,
        "wall_time_limit_s" => wall_time_limit_s === nothing ? nothing : Float64(wall_time_limit_s),
        "legacy_cycles_requested" => legacy_cycles === nothing ? nothing : Float64(legacy_cycles),
        "control_timescale" => period,
        "period" => period,
        ...
        "joint_centers" => [Float64(assembled_params.center1), Float64(assembled_params.center2)],
        "joint_half_widths" => [Float64(assembled_params.half_width1), Float64(assembled_params.half_width2)],
        ...
        "joint_angle_semantics" => "phi_i is the incremental bend of joint i relative to the preceding body segment; body/world tangent angles are derived by cumulative integration plus rigid heading",
        "body_map_time_semantics" => "rendering and diagnostics sample the moving body map at params.reference_time, so phi_dot is not extrapolated back to t=0",
        "final_incremental_joint_angles_deg" => [
            Float64(joint_rows[end][1] / TARGET_DEG),
            Float64(joint_rows[end][2] / TARGET_DEG),
        ],
        "final_body_frame_tail_tangent_deg" => Float64((joint_rows[end][1] + joint_rows[end][2]) / TARGET_DEG),
        ...
        "final_body_map_reference_time" => hasproperty(assembled_params, :reference_time) ? Float64(assembled_params.reference_time) : nothing,
    ...
    println("horizon_source=$horizon_source")
